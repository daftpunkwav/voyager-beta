/** 设置页 Agent 读写附加目录单测(phase-56):
 *  挂载草稿按行展示;保存 trim/去空行后打到 agent.fs.write_roots 的 JSON 数组;
...
describe('读写附加目录(phase-56)', () => {
  it('挂载时读取草稿:数组按行展示', async () => {
    render(<AgentSettingsSection />);
    await waitDraft('E:\\shared\n/srv/dropbox');
  });

  it('编辑保存 trim/去空行后打到 agent.fs.write_roots 的 JSON 数组', async () => {
    render(<AgentSettingsSection />);
    await waitDraft('E:\\shared\n/srv/dropbox');
