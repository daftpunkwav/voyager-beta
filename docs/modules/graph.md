...
- **能力**:
  ...
  初始原语(已实现语义约定,未实现存储):
  - `enqueue_index / cancel_index / reorder_queue / list_index_jobs` — 索引任务队列管理
  - `set_node / set_relationship` — 基础写入原语(upsert 语义;用户手建、引擎解析、
    agent AI 建图同写一份图存储,来源字段区分 actor + pipeline)
  - `query_graph / get_subgraph / graph_stats / list_projects` — 基础查询与项目枚举
  ...
  - 度排行 `degree_top`(graph_stats 已实现)
...
- **状态**:已实现(13 服务测试 + 3 集成测试)。已落地:规范图存储(nodes/edges upsert,
  source/actor 区分)、引擎适配层(C 优先、健康失败回退 Python 并发
  `graph.engine.fallback` 事件)、优先级索引队列(重试 backoff + 待命空转)、
  code 管线(分析 + 引擎id->规范id 边映射 + 索引后自动跨仓关联)、
  AI 管线 guide(词表+校验);能力 13 个见 service.json。
