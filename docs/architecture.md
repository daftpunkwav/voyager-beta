...
│   ├── personas/                     # 人格预设(结构 ID=职责;显示名在数据层)
│   │   ├── orchestrator.py           #   统筹者(唯一常驻;显示名 Lucien)
│   │   ├── recon.py                  #   侦察检索(显示名 Iris)
│   │   ├── explainer.py              #   讲解导师(显示名 Elio)
│   │   ├── organizer.py              #   策展整理(显示名 Miyai)
│   │   └── graph_guide.py            #   图谱向导(显示名 Atlas)
│   ├── subagent/
...
| 结构 ID | 显示名 | 气质来源 | 职责定位 |
|---|---|---|---|
| **orchestrator** | Lucien | lux(光):照亮全局 | 唯一常驻 master:统筹、仲裁、直聊(开关) |
| **recon** | Iris | 信使女神 | 侦察检索:调研、找资料、项目导览 |
| **explainer** | Elio | 太阳 | 讲解导师:串讲、答疑、出题 |
| **organizer** | Miyai | 精致仪式感 | 策展整理:归类、笔记、文档生成 |
| **graph_guide** | Atlas | 背负天球者 | 图谱向导:图谱漫游讲解 |

历史 key(`lucien`/`hub`/`scout`/…)经 `ALIASES` 解析到上表结构 ID。
