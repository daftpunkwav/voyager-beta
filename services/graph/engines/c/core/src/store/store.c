 ...
 *
 * Implements the opaque engine_store_t handle with prepared statement caching,
...
enum {
     ...
     * ENGINE_SZ_1K; percent-encoding can triple it, plus the "file://" prefix,
...
static sqlite3_destructor_type make_transient(void) {
    ...
    u.i = (uintptr_t)ENGINE_NOT_FOUND;
    return u.fn;
}
#define BIND_TRANSIENT (make_transient())

static int bind_text(sqlite3_stmt *s, int col, const char *v) {
    return sqlite3_bind_text(s, col, v, ENGINE_NOT_FOUND, BIND_TRANSIENT);
}
...
static void store_set_error(engine_store_t *s, const char *msg) {
    snprintf(s->errbuf, sizeof(s->errbuf), "%s", msg);
}

static void store_set_error_sqlite(engine_store_t *s, const char *prefix) {
    snprintf(s->errbuf, sizeof(s->errbuf), "%s: %s", prefix, sqlite3_errmsg(s->db));
}

static int exec_sql(engine_store_t *s, const char *sql) {
    if (!s || !s->db) {
...
static sqlite3_stmt *prepare_cached(engine_store_t *s, sqlite3_stmt **slot, const char *sql) {
    if (!s || !s->db) {
        return NULL;
    }
    if (*slot) {
    ...
    int rc = sqlite3_prepare_v2(s->db, sql, ENGINE_NOT_FOUND, slot, NULL);
    if (rc != SQLITE_OK) {
...
static int init_schema(engine_store_t *s) {
    const char *ddl =
         ...
         * built sqlite_autoindex_edges_1 (internal/engine/sqlite_writer.c) —
     ...
     * the column — the widened upsert in engine_store_insert_edge can neither
     ...
     * reindex). Read-only query opens skip init_schema and keep working. */
    {
        ...
        if (sqlite3_prepare_v2(s->db, "SELECT local_name_gen FROM edges LIMIT 0;", ENGINE_NOT_FOUND,
                               &probe, NULL) != SQLITE_OK) {
            engine_log_warn("store.schema", "result", "incompatible", "missing",
     ...
     * because we feed it `engine_camel_split(name)` at insert time but want
...
static int create_user_indexes(engine_store_t *s) {
    const char *sql =
...
int64_t engine_store_resolve_mmap_size(void) {
    ...
    char buf[ST_BUF_64];
    if (engine_safe_getenv("ENGINE_SQLITE_MMAP_SIZE", buf, sizeof(buf), NULL) == NULL) {
...
static int configure_pragmas(engine_store_t *s, bool in_memory, bool read_only) {
    int rc;
    ...
    if (in_memory) {
        ...
        snprintf(mmap_sql, sizeof(mmap_sql), "PRAGMA mmap_size = %lld;",
                 (long long)engine_store_resolve_mmap_size());
         ...
         * engine_store_checkpoint's SIGBUS note), so without a size limit the -wal
...
#define ENGINE_STORE_DENOM_EPS_D 1e-10
#define ENGINE_STORE_DENOM_EPS_F 1e-10F
#define ENGINE_STORE_INT8_MAX 127.0F
#define ENGINE_STORE_UNIT_POS_F 1.0F
#define ENGINE_STORE_UNIT_POS_D 1.0
...
static sqlite3_destructor_type engine_sqlite_transient_destructor(void) {
    static const volatile intptr_t raw = -1;
    sqlite3_destructor_type dtor = NULL;
    memcpy(&dtor, (const void *)&raw, sizeof(dtor));
    return dtor;
}
#define ENGINE_SQLITE_TRANSIENT (engine_sqlite_transient_destructor())
...
static void sqlite_camel_split(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    if (!input || !input[0]) {
        sqlite3_result_text(ctx, input ? input : "", SQLITE_AUTO_LEN, ENGINE_SQLITE_TRANSIENT);
        return;
    }
    ...
    if (len < 0 || len >= (int)sizeof(buf)) {
        ...
        sqlite3_result_text(ctx, input, SQLITE_AUTO_LEN, ENGINE_SQLITE_TRANSIENT);
        return;
    }
    ...
    buf[len] = '\0';
    sqlite3_result_text(ctx, buf, len, ENGINE_SQLITE_TRANSIENT);
}
...
static void regex_free_cb(void *p) {
    engine_regex_t *re = (engine_regex_t *)p;
    engine_regfree(re);
    free(re);
}
 ...
 * value), so engine_regcomp is called exactly once per statement instead of once per row. */
static void sqlite_regexp(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    engine_regex_t *re = (engine_regex_t *)sqlite3_get_auxdata(ctx, 0);
    if (!re) {
        re = malloc(sizeof(engine_regex_t));
        if (!re) {
            sqlite3_result_error_nomem(ctx);
            return;
        }
        if (engine_regcomp(re, pattern, ENGINE_REG_EXTENDED | ENGINE_REG_NOSUB) != 0) {
            free(re);
            sqlite3_result_error(ctx, "invalid regex", ENGINE_NOT_FOUND);
            return;
        }
        sqlite3_set_auxdata(ctx, 0, re, regex_free_cb);
    }

    sqlite3_result_int(ctx, engine_regexec(re, text, 0, NULL, 0) == 0 ? SKIP_ONE : 0);
}
...
static void sqlite_iregexp(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    if (!re) {
        ...
        if (engine_regcomp(re, pattern, ENGINE_REG_EXTENDED | ENGINE_REG_NOSUB | ENGINE_REG_ICASE) != 0) {
            free(re);
            sqlite3_result_error(ctx, "invalid regex", ENGINE_NOT_FOUND);
            return;
        }
...
static void sqlite_cosine_i8(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    sqlite3_result_double(ctx, denom > ENGINE_STORE_DENOM_EPS_D ? (double)dot / denom : 0.0);
}
...
static engine_store_t *store_open_internal(const char *path, bool in_memory, bool create) {
    engine_store_t *s = calloc(ENGINE_ALLOC_ONE, sizeof(engine_store_t));
    if (!s) {
        return NULL;
    }
    ...
    if (path && !in_memory) {
        if (!engine_path_for_file_api(path, open_path, sizeof(open_path))) {
            free(s);
            return NULL;
        }
    ...
    sqlite3_create_function(s->db, "engine_cosine_i8", ST_COL_2, SQLITE_UTF8 | SQLITE_DETERMINISTIC,
                            NULL, sqlite_cosine_i8, NULL, NULL);
    /* camelCase splitter for FTS5 BM25 indexing */
    sqlite3_create_function(s->db, "engine_camel_split", SKIP_ONE, SQLITE_UTF8 | SQLITE_DETERMINISTIC,
                            NULL, sqlite_camel_split, NULL, NULL);

    if (configure_pragmas(s, in_memory, false) != ENGINE_STORE_OK || init_schema(s) != ENGINE_STORE_OK ||
        create_user_indexes(s) != ENGINE_STORE_OK) {
        sqlite3_close(s->db);
...
engine_store_t *engine_store_open_path_query(const char *db_path) {
    ...
    char open_path[4096];
    if (!engine_path_for_file_api(db_path, open_path, sizeof(open_path))) {
        free(s);
        return NULL;
    }
    ...
    if (configure_pragmas(s, false, true) != ENGINE_STORE_OK) {
        sqlite3_close(s->db);
        safe_str_free(&s->db_path);
        free(s);
        return NULL;
...
bool engine_store_check_integrity_deep(engine_store_t *s) {
    if (!engine_store_check_integrity(s)) {
        return false;
    }
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, "PRAGMA quick_check(1);", ENGINE_NOT_FOUND, &stmt, NULL) !=
        SQLITE_OK) {
        return false;
    }
    bool ok = false;
...
bool engine_store_check_integrity(engine_store_t *s) {
    ...
    sqlite3_stmt *stmt = NULL;
    int rc =
        sqlite3_prepare_v2(s->db, "SELECT count(*) FROM projects;", ENGINE_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
        return false;
    }
...
sqlite3 *engine_store_get_db(engine_store_t *s) {
    return s ? s->db : NULL;
}
...
int engine_store_begin_bulk(engine_store_t *s) {
    ...
    return exec_sql(s, "PRAGMA cache_size = -65536;"); /* ENGINE_SZ_64 MB */
}
...
int engine_store_checkpoint(engine_store_t *s) {
    ...
    if (wal_frames >= ST_WAL_STARVE_WARN_FRAMES && checkpointed < wal_frames) {
        ...
        engine_log_warn("store.wal.starving", "wal_frames", frames_buf, "checkpointed", ckpt_buf,
                     "hint",
...
static int prepare_sqlite_for_publish(sqlite3 *db) {
    ...
    sqlite3_stmt *stmt = NULL;
    rc = sqlite3_prepare_v2(db, "PRAGMA journal_mode=DELETE;", ENGINE_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
        return ENGINE_STORE_ERR;
    }
    ...
    sqlite3_finalize(stmt);
    return delete_mode ? ENGINE_STORE_OK : ENGINE_STORE_ERR;
}
...
int engine_store_backup_path(const char *source_path, const char *staging_path) {
    ...
    if (engine_remove_db_sidecars(staging_path) != 0) {
        return ENGINE_STORE_ERR;
    }
    ...
    int result = ENGINE_STORE_ERR;
    if (backup) {
        int step_rc = sqlite3_backup_step(backup, ENGINE_NOT_FOUND);
        int finish_rc = sqlite3_backup_finish(backup);
        if (step_rc == SQLITE_DONE && finish_rc == SQLITE_OK) {
            result = ENGINE_STORE_OK;
        }
    }
...
int engine_store_seal_for_atomic_publish(engine_store_t *s) {
    ...
    if (exec_sql(s, "PRAGMA synchronous = FULL;") != ENGINE_STORE_OK) {
        return ENGINE_STORE_ERR;
    }
    ...
    sqlite3_stmt *stmt = NULL;
    rc = sqlite3_prepare_v2(s->db, "PRAGMA journal_mode = DELETE;", ENGINE_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
...
int engine_store_seal_existing_path_for_replace(const char *db_path) {
    ...
    if (rc != SQLITE_OK) {
        ...
        return primary == SQLITE_CORRUPT || primary == SQLITE_NOTADB || primary == SQLITE_FORMAT
                   ? ENGINE_STORE_NOT_FOUND
                   : ENGINE_STORE_ERR;
    }
    (void)sqlite3_busy_timeout(maintenance.db, 10000);

    sqlite3_stmt *stmt = NULL;
    rc = sqlite3_prepare_v2(maintenance.db, "PRAGMA quick_check(1);", ENGINE_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
        int primary = rc & 0xff;
        sqlite3_close(maintenance.db);
    ...
    if (!engine_store_check_integrity(&maintenance)) {
        sqlite3_close(maintenance.db);
        return ENGINE_STORE_NOT_FOUND;
    }

    int seal_rc = engine_store_seal_for_atomic_publish(&maintenance);
    sqlite3_close(maintenance.db);
    return seal_rc;
}
...
int engine_store_dump_to_file(engine_store_t *s, const char *dest_path) {
    ...
    if (sl) {
        *sl = '\0';
        (void)engine_mkdir(dir);
    }

    /* Write to temp file for atomic swap */
    char tmp_path[ENGINE_SZ_1K];
    ...
    rc = sqlite3_backup_step(bk, ENGINE_NOT_FOUND); /* copy ALL pages in one shot */
    ...
    engine_remove_db_sidecars(dest_path);
    if (engine_rename_replace(tmp_path, dest_path) != 0) {
...
int engine_store_generation(engine_store_t *s, char *buf, size_t bufsz) {
    ...
    if (sqlite3_prepare_v2(s->db,
                           ...
                           ENGINE_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
        return ENGINE_STORE_OK; /* pre-migration DB: stable "legacy" generation */
    }
    if (sqlite3_step(stmt) == SQLITE_ROW) {
...
int engine_store_get_project(engine_store_t *s, const char *name, engine_project_t *out) {
    ...
    if (rc == SQLITE_ROW) {
        ...
        out->root_path = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_2));
...
int engine_store_list_projects(engine_store_t *s, engine_project_t **out, int *count) {
    ...
    int n = 0;
    engine_project_t *arr = malloc(cap * sizeof(engine_project_t));

    int scan_rc1;
    while ((scan_rc1 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(engine_project_t));
        }
        ...
        arr[n].root_path = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_2));
        n++;
    }
    if (scan_rc1 != SQLITE_DONE) { /* SCANCHK:1:stmt */
        store_set_error_sqlite(s, "row scan aborted");
        engine_store_free_projects(arr, n);
        *out = NULL;
        *count = 0;
        return ENGINE_STORE_ERR;
    }
...
int engine_store_delete_project(engine_store_t *s, const char *name) {
    ...
    if (exec_sql(s, "BEGIN;") != ENGINE_STORE_OK) {
        return ENGINE_STORE_ERR;
    }
    ...
    for (size_t i = 0; i < sizeof(cleanup_sql) / sizeof(cleanup_sql[0]); i++) {
        ...
        if (sqlite3_prepare_v2(s->db, cleanup_sql[i], ENGINE_NOT_FOUND, &cleanup, NULL) != SQLITE_OK) {
...
static void scan_node(sqlite3_stmt *stmt, engine_node_t *n) {
    n->id = sqlite3_column_int64(stmt, 0);
    n->project = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
    n->label = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_2));
    n->name = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_3));
    n->qualified_name = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_4));
    n->file_path = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_5));
    n->start_line = sqlite3_column_int(stmt, ENGINE_SZ_6);
    n->end_line = sqlite3_column_int(stmt, ENGINE_SZ_7);
...
int engine_store_find_node_by_qn(engine_store_t *s, const char *project, const char *qn,
                              engine_node_t *out) {
    if (!s || !s->db) {
        return ENGINE_STORE_ERR;
    }
...
int engine_store_find_nodes_by_name_any(engine_store_t *s, const char *name, engine_node_t **out,
    ...
    int n = 0;
    engine_node_t *arr = malloc(cap * sizeof(engine_node_t));
    int scan_rc2;
    while ((scan_rc2 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(engine_node_t));
        }
...
int engine_store_find_node_ids_by_qns(engine_store_t *s, const char *project, const char **qns,
    ...
    for (int i = 0; i < qn_count; i++) {
        ...
        int rc = engine_store_find_node_by_qn(s, project, qns[i], &node);
        if (rc == ENGINE_STORE_OK) {
            out_ids[i] = node.id;
            found++;
            engine_node_free_fields(&node);
...
static int find_nodes_generic(engine_store_t *s, sqlite3_stmt **slot, const char *sql,
                              const char *project, const char *val, engine_node_t **out, int *count) {
    if (!s || !s->db) {
        *out = NULL;
        *count = 0;
        return ENGINE_STORE_ERR;
...
int engine_store_upsert_node_batch(engine_store_t *s, const engine_node_t *nodes, int count,
    ...
    for (int i = 0; i < count; i++) {
        int64_t id = engine_store_upsert_node(s, &nodes[i]);
        if (id == ENGINE_STORE_ERR) {
            exec_sql(s, "ROLLBACK;");
            return ENGINE_STORE_ERR;
        }
        if (out_ids) {
...
int64_t engine_store_insert_edge(engine_store_t *s, const engine_edge_t *e) {
    ...
    sqlite3_bind_int64(stmt, ENGINE_SZ_3, e->target_id);
...
static void scan_edge(sqlite3_stmt *stmt, engine_edge_t *e) {
    e->id = sqlite3_column_int64(stmt, 0);
    e->project = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
    e->source_id = sqlite3_column_int64(stmt, ENGINE_SZ_2);
    e->target_id = sqlite3_column_int64(stmt, ENGINE_SZ_3);
    e->type = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_4));
    e->properties_json = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_5));
}

/* Generic: find multiple edges by a filter. */
static int find_edges_generic(engine_store_t *s, sqlite3_stmt **slot, const char *sql,
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    engine_edge_t *arr = malloc(cap * sizeof(engine_edge_t));

    int scan_rc4;
    while ((scan_rc4 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(engine_edge_t));
        }
...
int engine_store_fetch_call_edges(engine_store_t *s, const char *project, int max_edges,
    ...
    bind_text(stmt, SKIP_ONE, project);
    sqlite3_bind_int(stmt, 2, max_edges > 0 ? max_edges + SKIP_ONE : ENGINE_NOT_FOUND);
...
int engine_store_find_edges_by_source(engine_store_t *s, int64_t source_id, engine_edge_t **out,
                                   int *count) {
...
int engine_store_find_edges_by_target(engine_store_t *s, int64_t target_id, engine_edge_t **out,
                                   int *count) {
...
int engine_store_insert_edge_batch(engine_store_t *s, const engine_edge_t *edges, int count) {
    ...
    exec_sql(s, "BEGIN IMMEDIATE;");
    for (int i = 0; i < count; i++) {
        int64_t id = engine_store_insert_edge(s, &edges[i]);
        if (id == ENGINE_STORE_ERR) {
            exec_sql(s, "ROLLBACK;");
            return ENGINE_STORE_ERR;
        }
...
int engine_store_upsert_file_hash(engine_store_t *s, const char *project, const char *rel_path,
    ...
    sqlite3_bind_int64(stmt, ENGINE_SZ_4, mtime_ns);
    sqlite3_bind_int64(stmt, ENGINE_SZ_5, size);
...
int engine_store_get_file_hashes(engine_store_t *s, const char *project, engine_file_hash_t **out,
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    engine_file_hash_t *arr = malloc(cap * sizeof(engine_file_hash_t));

    int scan_rc5;
    while ((scan_rc5 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(engine_file_hash_t));
        }
        ...
        arr[n].sha256 = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_2));
        arr[n].mtime_ns = sqlite3_column_int64(stmt, ENGINE_SZ_3);
        arr[n].size = sqlite3_column_int64(stmt, ENGINE_SZ_4);
        n++;
    }
    if (scan_rc5 != SQLITE_DONE) { /* SCANCHK:5:stmt */
        store_set_error_sqlite(s, "row scan aborted");
        engine_store_free_file_hashes(arr, n);
        *out = NULL;
        *count = 0;
        return ENGINE_STORE_ERR;
    }
...
int engine_store_get_file_hash(engine_store_t *s, const char *project, const char *rel_path,
    ...
    if (rc == SQLITE_ROW) {
        ...
        out->size = sqlite3_column_int64(stmt, ENGINE_SZ_4);
        sqlite3_finalize(stmt);
        if (!out->project || !out->rel_path || !out->sha256) {
            engine_store_clear_file_hash(out);
            return ENGINE_STORE_ERR;
        }
        return ENGINE_STORE_OK;
    }
...
static void cov_failure_fingerprint(const engine_coverage_row_t *rows, int count,
    ...
    for (int i = 0; i < count; i++) {
        ...
        engine_sha256_update(&sha, rel, strlen(rel) + 1);
        engine_sha256_update(&sha, kind, strlen(kind) + 1);
        engine_sha256_update(&sha, detail, strlen(detail) + 1);
    }
...
static int cov_rebuild_shadow_graph(engine_store_t *s, const char *project) {
    ...
    engine_coverage_row_t *fp_rows = NULL;
    int fp_count = 0;
    if (engine_store_coverage_get(s, project, &fp_rows, &fp_count) != ENGINE_STORE_OK) {
        engine_store_free_coverage(fp_rows, fp_count);
        return ENGINE_STORE_ERR;
    }
    char fp[ENGINE_SHA256_HEX_LEN + 1];
    cov_failure_fingerprint(fp_rows, fp_count, fp);
    engine_store_free_coverage(fp_rows, fp_count);
    char fp_key[ENGINE_SZ_512];
    snprintf(fp_key, sizeof(fp_key), "coverage_shadow_fp:%s", project);
    char prev_fp[ENGINE_SHA256_HEX_LEN + 1] = {0};
    {
        sqlite3_stmt *get = NULL;
        if (sqlite3_prepare_v2(s->db, "SELECT v FROM store_meta WHERE k = ?1;", ENGINE_NOT_FOUND, &get,
                               NULL) == SQLITE_OK) {
    ...
    for (int w = 0; w < 2; w++) {
        ...
        if (sqlite3_prepare_v2(s->db, wipes[w], ENGINE_NOT_FOUND, &del, NULL) != SQLITE_OK) {
    ...
    int count = 0;
    if (engine_store_coverage_get(s, project, &rows, &count) != ENGINE_STORE_OK) {
    ...
    if (engine_store_upsert_project(s, covproj, "") != ENGINE_STORE_OK) {
        engine_store_free_coverage(rows, count);
        return ENGINE_STORE_ERR;
    }

    engine_node_t root = {.project = covproj,
                       .label = "Project",
                       .name = project,
                       .qualified_name = covproj,
                       .properties_json = "{}"};
    int64_t root_id = engine_store_upsert_node(s, &root);
    if (root_id <= 0) {
        engine_store_free_coverage(rows, count);
        return ENGINE_STORE_ERR;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (char *p = pathbuf; *p; p++) {
            ...
            *p = '\0';
            ...
            engine_node_t folder = {.project = covproj,
                                 .label = "Folder",
                                 .name = seg ? seg + 1 : pathbuf,
                                 .qualified_name = pathbuf,
                                 .file_path = pathbuf,
                                 .properties_json = "{}"};
            int64_t fid = engine_store_upsert_node(s, &folder);
            *p = '/';
            if (fid <= 0) {
                engine_store_free_coverage(rows, count);
                return ENGINE_STORE_ERR;
            }
            engine_edge_t e = {.project = covproj,
                            .source_id = parent,
                            .target_id = fid,
                            .type = "CONTAINS_FOLDER",
                            .properties_json = "{}"};
            if (engine_store_insert_edge(s, &e) <= 0) {
                engine_store_free_coverage(rows, count);
                return ENGINE_STORE_ERR;
            }
            parent = fid;
        ...
        engine_node_t file = {.project = covproj,
                           .label = "File",
                           .name = base ? base + 1 : rel,
                           .qualified_name = rel,
                           .file_path = rel,
                           .properties_json = props};
        int64_t file_id = engine_store_upsert_node(s, &file);
        if (file_id <= 0) {
            engine_store_free_coverage(rows, count);
            return ENGINE_STORE_ERR;
        }
    ...
    engine_store_free_coverage(rows, count);
    {
        ...
        if (sqlite3_prepare_v2(s->db, "INSERT OR REPLACE INTO store_meta (k, v) VALUES (?1, ?2);",
                               ENGINE_NOT_FOUND, &set, NULL) == SQLITE_OK) {
...
int engine_store_coverage_replace_ex(engine_store_t *s, const char *project,
    ...
    sqlite3_stmt *del = NULL;
    if (sqlite3_prepare_v2(s->db, "DELETE FROM index_coverage WHERE project = ?1;", ENGINE_NOT_FOUND,
                           &del, NULL) != SQLITE_OK) {
    ...
    if (sqlite3_prepare_v2(
            ...
            "VALUES (?1, ?2, ?3, ?4);",
            ENGINE_NOT_FOUND, &ins, NULL) != SQLITE_OK) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        bind_text(ins, ENGINE_SZ_4, rows[i].detail ? rows[i].detail : "");
    ...
    if (sqlite3_prepare_v2(s->db,
                           ...
                           ENGINE_NOT_FOUND, &prune, NULL) != SQLITE_OK) {
    ...
    if (meta) {
        ...
        if (sqlite3_prepare_v2(
                ...
                ENGINE_NOT_FOUND, &up_meta, NULL) != SQLITE_OK) {
        ...
        bind_text(up_meta, ENGINE_SZ_4, recorded_at);
        bind_text(up_meta, ENGINE_SZ_5, recording_status);
        ...
        if (sqlite3_prepare_v2(s->db, "DELETE FROM index_coverage_meta WHERE project = ?1;",
                               ENGINE_NOT_FOUND, &del_meta, NULL) != SQLITE_OK) {
    ...
    if (cov_rebuild_shadow_graph(s, project) != ENGINE_STORE_OK) {
        (void)exec_sql(s, "ROLLBACK;");
        return ENGINE_STORE_ERR;
    }
    return exec_sql(s, "COMMIT;");
...
int engine_store_coverage_replace(engine_store_t *s, const char *project, const engine_coverage_row_t *rows,
                               int count) {
    return engine_store_coverage_replace_ex(s, project, rows, count, NULL);
}

static int coverage_query_rows(engine_store_t *s, const char *project, const char *selector,
                               const char *sql, engine_coverage_row_t **out, int *count) {
    if (!out || !count) {
        return ENGINE_STORE_ERR;
    }
    *out = NULL;
    *count = 0;
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, sql, ENGINE_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    engine_coverage_row_t *arr = malloc((size_t)cap * sizeof(*arr));
    if (!arr) {
        sqlite3_finalize(stmt);
        return ENGINE_STORE_ERR;
    }
    ...
    while ((scan_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        if (!arr[n].rel_path || !arr[n].kind || !arr[n].detail) {
            sqlite3_finalize(stmt);
            engine_store_free_coverage(arr, n + 1);
            return ENGINE_STORE_ERR;
        }
        n++;
    }
    ...
    if (scan_rc != SQLITE_DONE) {
        ...
        engine_store_free_coverage(arr, n);
        return ENGINE_STORE_ERR;
    }
    *out = arr;
    *count = n;
...
int engine_store_coverage_meta_get(engine_store_t *s, const char *project, engine_coverage_meta_t *out) {
    ...
    if (rc == SQLITE_ROW) {
        ...
        out->recording_status = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_4));
        out->ignored_files_stored = sqlite3_column_int(stmt, ENGINE_SZ_5);
        ...
        if (!out->project || !out->generation || !out->index_mode || !out->recorded_at ||
            ...
            engine_store_coverage_meta_clear(out);
            return ENGINE_STORE_ERR;
        }
        return ENGINE_STORE_OK;
    }
...
int engine_store_coverage_get(engine_store_t *s, const char *project, engine_coverage_row_t **out,
    ...
    *count = 0;
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    engine_coverage_row_t *arr = malloc(cap * sizeof(engine_coverage_row_t));
    int scan_rc6;
    while ((scan_rc6 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(engine_coverage_row_t));
        }
        arr[n].rel_path = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
        arr[n].kind = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
        arr[n].detail = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_2));
        n++;
    }
...
int engine_store_find_nodes_by_file_overlap(engine_store_t *s, const char *project, const char *file_path,
    ...
    *count = 0;
    ...
    sqlite3_stmt *stmt = NULL;
    int rc = sqlite3_prepare_v2(s->db, sql, ENGINE_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    engine_node_t *nodes = malloc(cap * sizeof(engine_node_t));
    int scan_rc7;
    while ((scan_rc7 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            nodes = safe_realloc(nodes, cap * sizeof(engine_node_t));
        }
        memset(&nodes[n], 0, sizeof(engine_node_t));
    ...
    if (scan_rc7 != SQLITE_DONE) { /* SCANCHK:7:stmt */
        store_set_error_sqlite(s, "row scan aborted");
        sqlite3_finalize(stmt);
        engine_store_free_nodes(nodes, n);
        *out = NULL;
        *count = 0;
        return ENGINE_STORE_ERR;
    }
...
int engine_store_find_nodes_by_qn_suffix(engine_store_t *s, const char *project, const char *suffix,
    ...
    *count = 0;
    ...
    char like_pattern[ENGINE_SZ_512];
    ...
    sqlite3_stmt *stmt = NULL;
    int rc =
        sqlite3_prepare_v2(s->db, project ? sql_with_project : sql_any, ENGINE_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
...
void engine_store_node_degree(engine_store_t *s, int64_t node_id, int *in_deg, int *out_deg) {
    ...
    *out_deg = 0;
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, in_sql, ENGINE_NOT_FOUND, &stmt, NULL) == SQLITE_OK) {
    ...
    if (sqlite3_prepare_v2(s->db, out_sql, ENGINE_NOT_FOUND, &stmt, NULL) == SQLITE_OK) {
        sqlite3_bind_int64(stmt, SKIP_ONE, node_id);
        if (sqlite3_step(stmt) == SQLITE_ROW) {
...
int engine_store_list_files(engine_store_t *s, const char *project, char ***out, int *count) {
    ...
    *count = 0;
    ...
    sqlite3_bind_text(stmt, SKIP_ONE, project, ENGINE_NOT_FOUND, SQLITE_STATIC);

    int cap = ENGINE_SZ_64;
    int n = 0;
...
static int query_neighbor_names(sqlite3 *db, const char *sql, int64_t node_id, int limit,
    ...
    *out_count = 0;
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(db, sql, ENGINE_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
...
static int count_degrees_direction(engine_store_t *s, const int64_t *node_ids, int id_count,
...
int engine_store_upsert_file_hash_batch(engine_store_t *s, const engine_file_hash_t *hashes, int count) {
    ...
    int rc = engine_store_begin(s);
    if (rc != ENGINE_STORE_OK) {
        return rc;
    }

    for (int i = 0; i < count; i++) {
        rc = engine_store_upsert_file_hash(s, hashes[i].project, hashes[i].rel_path, hashes[i].sha256,
                                        hashes[i].mtime_ns, hashes[i].size);
        if (rc != ENGINE_STORE_OK) {
            engine_store_rollback(s);
            return rc;
        }
    }

    return engine_store_commit(s);
}

/* ── LSP surface rows (closure-repair incremental) ─────────────── */

static int upsert_lsp_surface_row(engine_store_t *s, const engine_lsp_surface_row_t *row) {
    const char *sql =
...
int engine_store_get_lsp_surfaces(engine_store_t *s, const char *project, engine_lsp_surface_row_t **out,
    ...
    *count = 0;
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    engine_lsp_surface_row_t *rows = calloc((size_t)cap, sizeof(*rows));
    if (!rows) {
        sqlite3_finalize(stmt);
        return ENGINE_STORE_ERR;
    }
    ...
    while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            engine_lsp_surface_row_t *grown = realloc(rows, (size_t)cap * sizeof(*rows));
            if (!grown) {
                oom = true;
                break;
            }
        ...
        engine_lsp_surface_row_t *r = &rows[n];
        memset(r, 0, sizeof(*r));
    ...
    if (oom || step_rc != SQLITE_DONE) {
        engine_store_free_lsp_surfaces(rows, n);
        if (oom) {
            return ENGINE_STORE_ERR;
        }
...
static int dependent_files_chunk(engine_store_t *s, const char *project,
                                 const char *const *target_files, int chunk_count,
                                 EngineHashTable *seen, char ***out, int *out_count, int *out_cap) {
    char sql[ENGINE_SZ_4K];
    ...
    while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        if (!path || !path[0] || engine_ht_get(seen, path)) {
            continue;
        }
        ...
        (*out)[(*out_count)++] = copy;
        engine_ht_set(seen, copy, copy);
    }
    sqlite3_finalize(stmt);
    return step_rc == SQLITE_DONE ? ENGINE_STORE_OK : ENGINE_STORE_ERR;
}
...
int engine_store_get_dependent_files(engine_store_t *s, const char *project,
    ...
    *out_count = 0;
    if (!s || !project || target_count <= 0) {
        return target_count == 0 ? ENGINE_STORE_OK : ENGINE_STORE_ERR;
    }
    ...
    EngineHashTable *seen = engine_ht_create((size_t)target_count * PAIR_LEN);
    if (!seen) {
        return ENGINE_STORE_ERR;
    }
    for (int i = 0; i < target_count; i++) {
        engine_ht_set(seen, target_files[i], (void *)target_files[i]);
    }
    char **files = NULL;
    int count = 0;
    int cap = 0;
    int rc = ENGINE_STORE_OK;
    for (int off = 0; off < target_count && rc == ENGINE_STORE_OK; off += DEPFILE_CHUNK) {
    ...
    engine_ht_free(seen);
    if (rc != ENGINE_STORE_OK) {
        engine_store_free_dependent_files(files, count);
        return rc;
    }
    *out = files;
    *out_count = count;
...
int engine_store_find_edges_by_url_path(engine_store_t *s, const char *project, const char *keyword,
    ...
    *count = 0;
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    engine_edge_t *edges = malloc(cap * sizeof(engine_edge_t));
    int scan_rc12;
    while ((scan_rc12 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            edges = safe_realloc(edges, cap * sizeof(engine_edge_t));
        }
        memset(&edges[n], 0, sizeof(engine_edge_t));
    ...
    if (scan_rc12 != SQLITE_DONE) { /* SCANCHK:12:stmt */
        store_set_error_sqlite(s, "row scan aborted");
        sqlite3_finalize(stmt);
        engine_store_free_edges(edges, n);
        *out = NULL;
        *count = 0;
        return ENGINE_STORE_ERR;
    }
...
int engine_store_restore_from(engine_store_t *dst, engine_store_t *src) {
    ...
    int rc = sqlite3_backup_step(bk, ENGINE_NOT_FOUND); /* copy all pages */
    sqlite3_backup_finish(bk);
...
static void search_apply_degree_filter(char *sql, size_t sql_sz, const engine_search_params_t *p) {
    bool has_degree_filter = (p->min_degree >= 0 || p->max_degree >= 0);
    if (!has_degree_filter) {
        return;
    }
    char inner_sql[ENGINE_SZ_4K];
 ...
 * engine_extract_like_hints bails on alternation, so no false negatives. */
static void where_add_like_hints(const char *column, const char *pattern, char *where, int where_sz,
    ...
    int nhints = engine_extract_like_hints(pattern, hints, ST_LIKE_HINT_MAX);
    char bind_buf[ENGINE_SZ_64];
...
static int search_where_basic(const engine_search_params_t *params, char *where, int where_sz,
                              int *wlen, int *nparams, search_bind_t *binds, int *bind_idx,
                              search_like_pool_t *pool) {
    char bind_buf[ENGINE_SZ_64];
    ...
    if (params->file_pattern) {
        char *lp = engine_glob_to_like(params->file_pattern);
...
static void search_where_advanced(const engine_search_params_t *params, char *where, int where_sz,
                                  int *wlen, int *nparams, search_bind_t *binds, int *bind_idx) {
    if (params->relationship) {
        char rel_clause[ENGINE_SZ_256];
    ...
    if (params->exclude_labels) {
        char excl_clause[ENGINE_SZ_512];
...
static int search_build_where(const engine_search_params_t *params, char *where, int where_sz,
...
int engine_store_search(engine_store_t *s, const engine_search_params_t *params, engine_search_output_t *out) {
    ...
    int rc = sqlite3_prepare_v2(s->db, count_sql, ENGINE_NOT_FOUND, &cnt_stmt, NULL);
    if (rc == SQLITE_OK) {
    ...
    rc = sqlite3_prepare_v2(s->db, sql, ENGINE_NOT_FOUND, &main_stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    engine_search_result_t *results = malloc(cap * sizeof(engine_search_result_t));

    int scan_rc13;
    while ((scan_rc13 = sqlite3_step(main_stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            results = safe_realloc(results, cap * sizeof(engine_search_result_t));
        }
        memset(&results[n], 0, sizeof(engine_search_result_t));
        scan_node(main_stmt, &results[n].node);
        results[n].in_degree = sqlite3_column_int(main_stmt, ST_COL_9);
        results[n].out_degree = sqlite3_column_int(main_stmt, ENGINE_DECIMAL_BASE);
        n++;
    }
...
void engine_store_search_free(engine_search_output_t *out) {
    ...
    for (int i = 0; i < out->count; i++) {
        engine_search_result_t *r = &out->results[i];
...
static int bfs_collect_edges(engine_store_t *s, int64_t start_id, const engine_node_hop_t *visited,
                             int visited_count, const char *types_clause, const char **edge_types,
                             int edge_type_count, engine_edge_info_t **out_edges,
                             int *out_edge_count) {
    *out_edges = NULL;
    *out_edge_count = 0;
    ...
    if (sqlite3_exec(s->db,
        ...
        return ENGINE_STORE_OK; /* best-effort: nodes without edges beat an error */
    }
    sqlite3_stmt *ins = NULL;
    if (sqlite3_prepare_v2(s->db, "INSERT OR IGNORE INTO bfs_ids(id) VALUES (?1)", ENGINE_NOT_FOUND,
                           &ins, NULL) != SQLITE_OK) {
        return ENGINE_STORE_OK;
    }
    ...
    int rc = sqlite3_prepare_v2(s->db, edge_sql, ENGINE_NOT_FOUND, &estmt, NULL);
    if (rc != SQLITE_OK) {
        *out_edges = NULL;
        *out_edge_count = 0;
        return ENGINE_STORE_OK;
    ...
    int en = 0;
    engine_edge_info_t *edges = malloc(ecap * sizeof(engine_edge_info_t));

    int scan_rc14;
    while ((scan_rc14 = sqlite3_step(estmt)) == SQLITE_ROW) {
        if (en >= ecap) {
            ecap *= ST_GROWTH;
            edges = safe_realloc(edges, ecap * sizeof(engine_edge_info_t));
        }
        ...
        edges[en].type = heap_strdup((const char *)sqlite3_column_text(estmt, ENGINE_SZ_2));
        ...
        edges[en].properties_json = heap_strdup((const char *)sqlite3_column_text(estmt, ENGINE_SZ_5));
        en++;
    }
...
int engine_store_bfs(engine_store_t *s, int64_t start_id, const char *direction, const char **edge_types,
    ...
    engine_node_t root = {0};
    int rc = engine_store_find_node_by_id(s, start_id, &root);
    if (rc != ENGINE_STORE_OK) {
        return rc;
    }
    out->root = root;

    char types_clause[ENGINE_SZ_512];
    ...
    sqlite3_stmt *stmt = NULL;
    rc = sqlite3_prepare_v2(s->db, sql, ENGINE_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    engine_node_hop_t *visited = malloc(cap * sizeof(engine_node_hop_t));

    int scan_rc15;
    while ((scan_rc15 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            visited = safe_realloc(visited, cap * sizeof(engine_node_hop_t));
        }
...
int engine_store_bfs_multi(engine_store_t *s, const int64_t *seed_ids, int seed_count,
    ...
    sqlite3_stmt *ins = NULL;
    if (sqlite3_prepare_v2(s->db, "INSERT OR IGNORE INTO bfs_seeds(id) VALUES (?1)", ENGINE_NOT_FOUND,
                           &ins, NULL) != SQLITE_OK) {
    ...
    if (n > max_results) {
        ...
        n = max_results;
        engine_node_free_fields(&visited[n].node);
        if (truncated) {
            *truncated = true;
        }
    }
...
void engine_store_traverse_free(engine_traverse_result_t *out) {
    ...
    for (int i = 0; i < out->visited_count; i++) {
        engine_node_hop_t *h = &out->visited[i];
...
engine_risk_level_t engine_hop_to_risk(int hop) {
    switch (hop) {
    case SKIP_ONE:
        return ENGINE_RISK_CRITICAL;
    case ST_COL_2:
        return ENGINE_RISK_HIGH;
    case ST_COL_3:
        return ENGINE_RISK_MEDIUM;
    default:
        return ENGINE_RISK_LOW;
    }
}

const char *engine_risk_label(engine_risk_level_t level) {
    switch (level) {
    case ENGINE_RISK_CRITICAL:
        return "CRITICAL";
    case ENGINE_RISK_HIGH:
        return "HIGH";
    case ENGINE_RISK_MEDIUM:
        return "MEDIUM";
    case ENGINE_RISK_LOW:
    default:
        return "LOW";
    }
}
...
engine_impact_summary_t engine_build_impact_summary(const engine_node_hop_t *hops, int hop_count,
    ...
    engine_impact_summary_t s = {0};
    for (int i = 0; i < hop_count; i++) {
        switch (engine_hop_to_risk(hops[i].hop)) {
        case ENGINE_RISK_CRITICAL:
            s.critical++;
            break;
        case ENGINE_RISK_HIGH:
...
int engine_deduplicate_hops(const engine_node_hop_t *hops, int hop_count, engine_node_hop_t **out,
    ...
    *out_count = 0;
    ...
    engine_node_hop_t *result = malloc(hop_count * sizeof(engine_node_hop_t));
    int n = 0;
    ...
    *out = safe_realloc(result, n * sizeof(engine_node_hop_t));
    *out_count = n;
    return ENGINE_STORE_OK;
}
...
static void schema_discover_props(sqlite3 *db, const char *sql, const char *project,
    ...
    if (sqlite3_prepare_v2(db, sql, ENGINE_NOT_FOUND, &pstmt, NULL) == SQLITE_OK) {
...
bool engine_store_arch_path_scoped(const char *path) {
    char norm[ENGINE_SZ_512];
    char like[ENGINE_SZ_512 + 4];
...
int engine_store_count_edges_scoped(engine_store_t *s, const char *project, const char *path) {
    ...
    char like[ENGINE_SZ_512];
    if (!arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like))) {
        return engine_store_count_edges(s, project);
    }
    const char *sql =
...
static int get_schema_impl(engine_store_t *s, const char *project, engine_schema_info_t *out,
                           bool with_props) {
    memset(out, 0, sizeof(*out));
    if (!s || !s->db) {
        return ENGINE_NOT_FOUND;
    ...
    /* Node labels */
    {
        ...
        int cap = ST_INIT_CAP_8;
        int n = 0;
        engine_label_count_t *arr = malloc(cap * sizeof(engine_label_count_t));
        if (!arr) {
            sqlite3_finalize(stmt);
            return ENGINE_NOT_FOUND;
        }
        ...
        while ((scan_rc16 = sqlite3_step(stmt)) == SQLITE_ROW) {
            if (n >= cap) {
                ...
                void *tmp = realloc(arr, new_cap * sizeof(engine_label_count_t));
                if (!tmp) {
                    for (int i = 0; i < n; i++) {
    ...
    /* Edge types */
    {
        ...
        int cap = ST_INIT_CAP_8;
        int n = 0;
        engine_type_count_t *arr = malloc(cap * sizeof(engine_type_count_t));
        if (!arr) {
            sqlite3_finalize(stmt);
            engine_store_schema_free(out);
            return ENGINE_NOT_FOUND;
        ...
        while ((scan_rc17 = sqlite3_step(stmt)) == SQLITE_ROW) {
            if (n >= cap) {
                int new_cap = cap * ST_GROWTH;
                void *tmp = realloc(arr, new_cap * sizeof(engine_type_count_t));
                if (!tmp) {
                    for (int i = 0; i < n; i++) {
...
int engine_store_get_schema_counts_scoped(engine_store_t *s, const char *project, const char *path,
    ...
    char sqlbuf[ST_SQL_BUF];
    {
        ...
        snprintf(sqlbuf, sizeof(sqlbuf), "%s%s GROUP BY label ORDER BY COUNT(*) DESC;", base,
                 arch_path_scope_sql());
        sqlite3_stmt *stmt = NULL;
        if (sqlite3_prepare_v2(s->db, sqlbuf, ENGINE_NOT_FOUND, &stmt, NULL) != SQLITE_OK || !stmt) {
            if (stmt) {
                sqlite3_finalize(stmt);
            }
            return ENGINE_NOT_FOUND;
...

    {
        ...
        sqlite3_stmt *stmt = NULL;
        if (sqlite3_prepare_v2(s->db, esql, ENGINE_NOT_FOUND, &stmt, NULL) != SQLITE_OK || !stmt) {
            if (stmt) {
                sqlite3_finalize(stmt);
            }
            engine_store_schema_free(out);
...
const char *engine_qn_to_package(const char *qn) {
    ...
    static ENGINE_TLS char buf[ENGINE_SZ_256];
...
static const char *file_ext(const char *path) {
    ...
    static ENGINE_TLS char buf[ENGINE_SZ_16];
    int len = (int)strlen(dot);
    if (len >= (int)sizeof(buf)) {
        return NULL;
    }
    for (int i = 0; i < len; i++) {
        buf[i] = (char)((dot[i] >= 'A' && dot[i] <= 'Z') ? dot[i] + ENGINE_SZ_32 : dot[i]);
    }
    buf[len] = '\0';
    return buf;
}
...
static int arch_languages(engine_store_t *s, const char *project, const char *path,
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, sqlbuf, ENGINE_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
    ...
    const char *lang_names[ENGINE_SZ_64];
    int lang_counts[ENGINE_SZ_64];
    int nlang = 0;
    ...
    if (nlang > ENGINE_DECIMAL_BASE) {
        nlang = ST_MAX_LANG;
    }

    out->languages = (nlang > 0) ? calloc(nlang, sizeof(engine_language_count_t)) : NULL;
...
static int arch_entry_points(engine_store_t *s, const char *project, const char *path,
                             engine_architecture_info_t *out) {
    char norm[ENGINE_SZ_512];
    char like[ENGINE_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    engine_entry_point_t *arr = calloc(cap, sizeof(engine_entry_point_t));
    int scan_rc21;
    while ((scan_rc21 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(engine_entry_point_t));
        }
        arr[n].name = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
        arr[n].qualified_name = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
        arr[n].file = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_2));
        n++;
    }
...
static char *extract_json_string_prop(const char *json, const char *key, int key_len) {
    ...
    if (!end || end - m >= ENGINE_SZ_256) {
        return NULL;
    }
    char vbuf[ENGINE_SZ_256];
    memcpy(vbuf, m, end - m);
    vbuf[end - m] = '\0';
    return heap_strdup(vbuf);
}

static int arch_routes(engine_store_t *s, const char *project, const char *path,
                       engine_architecture_info_t *out) {
    char norm[ENGINE_SZ_512];
    char like[ENGINE_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    engine_route_info_t *arr = calloc(cap, sizeof(engine_route_info_t));
    int scan_rc22;
    while ((scan_rc22 = sqlite3_step(stmt)) == SQLITE_ROW) {
        const char *name = (const char *)sqlite3_column_text(stmt, 0);
        const char *props = (const char *)sqlite3_column_text(stmt, SKIP_ONE);
        const char *fp = (const char *)sqlite3_column_text(stmt, ENGINE_SZ_2);
        if (engine_is_test_file_path(fp)) {
            continue;
        }
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(engine_route_info_t));
        }
...
static int arch_hotspots(engine_store_t *s, const char *project, const char *path,
                         engine_architecture_info_t *out) {
    char norm[ENGINE_SZ_512];
    char like[ENGINE_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    engine_hotspot_t *arr = calloc(cap, sizeof(engine_hotspot_t));
    int scan_rc23;
    while ((scan_rc23 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(engine_hotspot_t));
        }
        arr[n].name = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
        arr[n].qualified_name = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
        arr[n].fan_in = sqlite3_column_int(stmt, ENGINE_SZ_2);
        n++;
    }
...
static int arch_boundaries(engine_store_t *s, const char *project, const char *path,
                           engine_cross_pkg_boundary_t **out_arr, int *out_count) {
    char norm[ENGINE_SZ_512];
    char like[ENGINE_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    const char *nbase =
        ...
        "(" ENGINE_SQL_CALLABLE_OR_TYPE_LABELS ")";
    if (scoped) {
    ...
    if (sqlite3_prepare_v2(s->db, nsqlbuf, ENGINE_NOT_FOUND, &nstmt, NULL) != SQLITE_OK) {
    ...
    while ((scan_rc24 = sqlite3_step(nstmt)) == SQLITE_ROW) {
        ...
        npkgs[nn] = heap_strdup(engine_qn_to_package(qn));
        nn++;
    }
    ...
    sqlite3_stmt *estmt = NULL;
    if (sqlite3_prepare_v2(s->db, esql, ENGINE_NOT_FOUND, &estmt, NULL) != SQLITE_OK) {
        for (int i = 0; i < nn; i++) {
            free(npkgs[i]);
        }
        free(nids);
    ...
    engine_cross_pkg_boundary_t *result =
        (bn > 0) ? calloc(bn, sizeof(engine_cross_pkg_boundary_t)) : NULL;
    for (int i = 0; i < bn; i++) {
...
static int arch_packages_from_qn(engine_store_t *s, const char *project, const char *path,
                                 engine_package_summary_t **out_arr, int *out_count) {
    char norm[ENGINE_SZ_512];
    char like[ENGINE_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, qsqlbuf, ENGINE_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
    ...
    engine_package_summary_t *arr = (np > 0) ? calloc(np, sizeof(engine_package_summary_t)) : NULL;
    for (int i = 0; i < np; i++) {
        arr[i].name = pnames[i];
...
static int arch_packages(engine_store_t *s, const char *project, const char *path,
                         engine_architecture_info_t *out) {
    char norm[ENGINE_SZ_512];
    char like[ENGINE_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    engine_package_summary_t *arr = calloc(cap, sizeof(engine_package_summary_t));
    int scan_rc27;
    while ((scan_rc27 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(engine_package_summary_t));
        }
        arr[n].name = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
...
static void classify_layer(const char *pkg, int in, int out_deg, bool has_routes,
    ...
    static ENGINE_TLS char reason_buf[ENGINE_SZ_128];
...
static int collect_pkg_names(engine_store_t *s, const char *sql, const char *project, const char *path,
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW && count < max_pkgs) {
        const char *qn = (const char *)sqlite3_column_text(stmt, 0);
        pkgs[count++] = heap_strdup(engine_qn_to_package(qn));
    }
    sqlite3_finalize(stmt);
    return count;
}

static int arch_layers(engine_store_t *s, const char *project, const char *path,
                       engine_architecture_info_t *out) {
    /* Get boundaries for fan analysis */
    engine_cross_pkg_boundary_t *boundaries = NULL;
    int bcount = 0;
    ...
    char *route_pkgs[ENGINE_SZ_32];
    int nrpkgs =
        collect_pkg_names(s, "SELECT qualified_name FROM nodes WHERE project=?1 AND label='Route'",
                          project, path, route_pkgs, ENGINE_SZ_32);

    char *entry_pkgs[ENGINE_SZ_32];
    int nepkgs = collect_pkg_names(s,
                                   ...
                                   project, path, entry_pkgs, ENGINE_SZ_32);

    /* Compute fan-in/out per package */
    char *all_pkgs[ENGINE_SZ_64];
    int fan_in[ENGINE_SZ_64];
    int fan_out[ENGINE_SZ_64];
    int npkgs = 0;
    ...
    out->layers = (npkgs > 0) ? calloc(npkgs, sizeof(engine_package_layer_t)) : NULL;
    ...
    for (int i = SKIP_ONE; i < npkgs; i++) {
        ...
        while (j > 0 && strcmp(out->layers[j].name, out->layers[j - SKIP_ONE].name) < 0) {
            engine_package_layer_t tmp = out->layers[j];
...
static engine_file_tree_entry_t make_tree_entry(const char *path, char **files, int fn,
                                             char **dir_paths, const int *dir_child_counts,
                                             int dn) {
    engine_file_tree_entry_t e = {0};
...
static void push_tree_entry(engine_file_tree_entry_t **entries, int *en, int *ecap,
                            engine_file_tree_entry_t e) {
    if (*en >= *ecap) {
        *ecap *= ST_GROWTH;
        *entries = safe_realloc(*entries, *ecap * sizeof(engine_file_tree_entry_t));
    }
    (*entries)[(*en)++] = e;
}
...
static void arch_collect_entries(char **dir_paths, int *dir_child_counts, char ***dir_children,
                                 int dn, char **files, int fn, engine_file_tree_entry_t **entries_out,
                                 int *en_out) {
    int ecap = ENGINE_SZ_64;
    int en = 0;
    engine_file_tree_entry_t *entries = calloc(ecap, sizeof(engine_file_tree_entry_t));
    ...
    for (int i = SKIP_ONE; i < en; i++) {
        ...
        while (j > 0 && strcmp(entries[j].path, entries[j - SKIP_ONE].path) < 0) {
            engine_file_tree_entry_t tmp = entries[j];
...
static int arch_file_tree(engine_store_t *s, const char *project, const char *path,
                          engine_architecture_info_t *out) {
    char norm[ENGINE_SZ_512];
    char like[ENGINE_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int fcap = ENGINE_SZ_32;
    int fn = 0;
    char **files = malloc(fcap * sizeof(char *));

    int dcap = ENGINE_SZ_64;
    int dn = 0;
...
static void louvain_build_weights(const int64_t *nodes, int n, const engine_louvain_edge_t *edges,
...
static void lg_free(engine_lg_t *g) {
    free(g->off);
    free(g->nbr);
    free(g->w);
    free(g->k);
 ...
 * comm[] in place. */
static void leiden_move(const engine_lg_t *g, int *comm, double gamma, double twom) {
    int n = g->n;
...
static int leiden_relabel(int *comm, int n) {
    ...
    for (int i = 0; i < n; i++) {
        map[i] = ENGINE_NOT_FOUND;
    }
    int next = 0;
    for (int i = 0; i < n; i++) {
        int c = comm[i];
        if (map[c] == ENGINE_NOT_FOUND) {
            map[c] = next++;
        }
        comm[i] = map[c];
    }
...
static int leiden_refine(const engine_lg_t *g, const int *comm, double gamma, double twom,
                         int *refined) {
    int n = g->n;
    double *stot = calloc((size_t)n, sizeof(double));
    double *acc = calloc((size_t)n, sizeof(double));
...
static int leiden_aggregate(const engine_lg_t *g, const int *refined, int r_count, const int *comm,
                            engine_lg_t *out, int *seed) {
    int n = g->n;
    ...
    for (int r = 0; r < r_count; r++) {
        seed[r] = ENGINE_NOT_FOUND;
    }
    for (int i = 0; i < n; i++) {
        int r = refined[i];
        k2[r] += g->k[i];
        gcount[r]++;
        if (seed[r] == ENGINE_NOT_FOUND) {
            seed[r] = comm[i];
        }
    }
    gstart[0] = 0;
...
int engine_leiden(const int64_t *nodes, int node_count, const engine_louvain_edge_t *edges,
    ...
    engine_louvain_result_t *result = malloc((size_t)n * sizeof(*result));
    if (!result) {
        return ENGINE_NOT_FOUND;
    }
    for (int i = 0; i < n; i++) {
    ...
    engine_lg_t g;
    int built = (wn > 0) ? lg_build(n, wsi, wdi, ww, wn, &g) : ENGINE_NOT_FOUND;
    free(wsi);
    free(wdi);
    free(ww);
    if (built != ENGINE_STORE_OK) {
    ...
    for (int level = 0; level < LEIDEN_MAX_LEVELS; level++) {
        ...
        if (!seed || leiden_aggregate(&g, refined, r_count, comm, &g2, seed) != ENGINE_STORE_OK) {
            free(seed);
            free(refined);
            break;
        }
...
int engine_louvain(const int64_t *nodes, int node_count, const engine_louvain_edge_t *edges,
    ...
    return engine_leiden(nodes, node_count, edges, edge_count, 1.0, out, out_count);
}

/* ── Architecture: community clusters via Leiden ───────────────── */

enum {
    ENGINE_CLUSTER_TOP_N = 12,       /* report at most this many clusters */
    ENGINE_CLUSTER_MAX_TOPNODES = 5, /* representative node names per cluster */
    ENGINE_CLUSTER_MAX_PKGS = 5,     /* packages listed per cluster */
    ENGINE_CLUSTER_MIN_MEMBERS = 2,  /* skip singletons */
    ENGINE_CLUSTER_NODE_CAP = 8000   /* bound the work for very large graphs */
};
...
/* Index of node id within the id-sorted ids[], or ENGINE_NOT_FOUND. */
static int cluster_id_index(const int64_t *ids, int n, int64_t id) {
    ...
    return hit ? (int)(hit - ids) : ENGINE_NOT_FOUND;
}
...
static void cluster_build_one(engine_cluster_info_t *ci, int c, int n, const int *comm,
    ...
    /* Top nodes by degree. */
    int top_idx[ENGINE_CLUSTER_MAX_TOPNODES];
    int top_deg[ENGINE_CLUSTER_MAX_TOPNODES];
    int tn = 0;
    for (int i = 0; i < n; i++) {
        if (comm[i] != c) {
            continue;
        ...
        if (pos < ENGINE_CLUSTER_MAX_TOPNODES) {
            int last = (tn < ENGINE_CLUSTER_MAX_TOPNODES) ? tn : ENGINE_CLUSTER_MAX_TOPNODES - 1;
            ...
            top_idx[pos] = i;
            top_deg[pos] = d;
            if (tn < ENGINE_CLUSTER_MAX_TOPNODES) {
                tn++;
            }
        }
    }
    ...
    const char *pkgs[ENGINE_CLUSTER_MAX_PKGS];
    int pkg_counts[ENGINE_CLUSTER_MAX_PKGS];
    int pc = 0;
    for (int i = 0; i < n; i++) {
        if (comm[i] == c) {
            cluster_add_pkg(pkgs, pkg_counts, &pc, ENGINE_CLUSTER_MAX_PKGS,
                            engine_qn_to_top_package(qns[i]));
        }
    }
    if (pc > 0) {
...
static int arch_clusters(engine_store_t *s, const char *project, const char *path,
                         engine_architecture_info_t *out) {
    char norm[ENGINE_SZ_512];
    char like[ENGINE_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    const char *nbase = "SELECT id, name, qualified_name, file_path FROM nodes "
                        "WHERE project=?1 AND label IN (" ENGINE_SQL_CALLABLE_OR_TYPE_LABELS ")";
    if (scoped) {
    ...
    if (sqlite3_prepare_v2(s->db, nsqlbuf, ENGINE_NOT_FOUND, &st, NULL) != SQLITE_OK) {
        return ENGINE_STORE_OK; /* clusters are best-effort */
    }
    bind_text(st, SKIP_ONE, project);
    if (scoped) {
        arch_bind_path_scope(st, ST_COL_2, ST_COL_3, norm, like);
        sqlite3_bind_int(st, ST_COL_4, ENGINE_CLUSTER_NODE_CAP);
    } else {
        sqlite3_bind_int(st, ENGINE_SZ_2, ENGINE_CLUSTER_NODE_CAP);
    }
    int cap = ST_INIT_CAP_8;
    int n = 0;
    ...
    while ((scan_rc29 = sqlite3_step(st)) == SQLITE_ROW) {
        ...
        qns[n] = heap_strdup((const char *)sqlite3_column_text(st, ENGINE_SZ_2));
        n++;
    }
    ...
    sqlite3_finalize(st);
    if (n < ENGINE_CLUSTER_MIN_MEMBERS) {
        for (int i = 0; i < n; i++) {
    ...
    engine_louvain_edge_t *edges = malloc((size_t)n * sizeof(engine_louvain_edge_t));
    int *esrc = malloc((size_t)n * sizeof(int));
    int *edst = malloc((size_t)n * sizeof(int));
    int *degree = calloc((size_t)n, sizeof(int));
    int ne = 0;
    const char *esql = "SELECT source_id, target_id FROM edges WHERE project=?1 AND type='CALLS'";
    if (sqlite3_prepare_v2(s->db, esql, ENGINE_NOT_FOUND, &st, NULL) == SQLITE_OK) {
        int ecap = n;
        bind_text(st, SKIP_ONE, project);
        int scan_rc30;
        while ((scan_rc30 = sqlite3_step(st)) == SQLITE_ROW) {
            ...
            if (ne >= ecap) {
                ecap *= ST_GROWTH;
                edges = safe_realloc(edges, (size_t)ecap * sizeof(engine_louvain_edge_t));
    ...
    engine_louvain_result_t *res = NULL;
    int rn = 0;
    int *comm = NULL;
    int C = 0;
    if (engine_leiden(ids, n, edges, ne, 1.0, &res, &rn) == ENGINE_STORE_OK && res && rn == n) {
    ...
    if (comm && C > 0) {
        ...
        engine_cluster_info_t *clusters =
            malloc((size_t)ENGINE_CLUSTER_TOP_N * sizeof(engine_cluster_info_t));
        int cc = 0;
        for (int r = 0; r < C && cc < ENGINE_CLUSTER_TOP_N; r++) {
            int c = rank[r].comm;
            if (members[c] < ENGINE_CLUSTER_MIN_MEMBERS) {
...
static bool want_aspect(const char **aspects, int aspect_count, const char *name) {
    ...
    for (int i = 0; i < aspect_count; i++) {
        ...
        if (strcmp(aspects[i], "overview") == 0 && engine_store_arch_aspect_in_overview(name)) {
            return true;
        }
...
int engine_store_get_architecture(engine_store_t *s, const char *project, const char *path,
    ...
    if (want_aspect(aspects, aspect_count, "boundaries")) {
        engine_cross_pkg_boundary_t *barr = NULL;
        int bcount = 0;
...
static void adr_save_section(engine_adr_sections_t *result, char *section_name, char *buf,
                             int buf_len) {
...
int engine_adr_validate_content(const char *content, char *errbuf, int errbuf_size) {
    engine_adr_sections_t sections = engine_adr_parse_sections(content);
    char missing[ENGINE_SZ_256] = "";
    int mlen = 0;
    int nmissing = 0;
    ...
    engine_adr_sections_free(&sections);
...
int engine_store_adr_store(engine_store_t *s, const char *project, const char *content) {
    ...
    int rc = sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    return (rc == SQLITE_DONE) ? ENGINE_STORE_OK : ENGINE_STORE_ERR;
}
...
int engine_store_adr_get(engine_store_t *s, const char *project, engine_adr_t *out) {
    ...
    int rc = sqlite3_prepare_v2(
        ...
        ENGINE_NOT_FOUND, &table_stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    out->created_at = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_2));
    out->updated_at = heap_strdup((const char *)sqlite3_column_text(stmt, ENGINE_SZ_3));
    rc = sqlite3_finalize(stmt);
    if (rc != SQLITE_OK) {
        engine_store_adr_free(out);
...
int engine_store_adr_update_sections(engine_store_t *s, const char *project, const char **keys,
    ...
    /* Get existing ADR */
    engine_adr_t existing;
    int rc = engine_store_adr_get(s, project, &existing);
    if (rc != ENGINE_STORE_OK) {
    ...
    engine_adr_sections_t sections = engine_adr_parse_sections(existing.content);
    ...
    /* Render merged */
    char *merged = engine_adr_render(&sections);
    engine_adr_sections_free(&sections);

    /* Check length */
    if ((int)strlen(merged) > ENGINE_ADR_MAX_LENGTH) {
        char msg[ENGINE_SZ_128];
        snprintf(msg, sizeof(msg), "merged ADR exceeds %d chars (%d chars)", ENGINE_ADR_MAX_LENGTH,
                 (int)strlen(merged));
    ...
    /* Store merged */
    rc = engine_store_adr_store(s, project, merged);
    free(merged);
    if (rc != ENGINE_STORE_OK) {
        return rc;
    }

    return engine_store_adr_get(s, project, out);
}
...
void engine_store_free_nodes(engine_node_t *nodes, int count) {
    ...
    for (int i = 0; i < count; i++) {
        engine_node_free_fields(&nodes[i]);
    }
    free(nodes);
}
...
void engine_store_free_projects(engine_project_t *projects, int count) {
    ...
    for (int i = 0; i < count; i++) {
        engine_project_free_fields(&projects[i]);
    }
    free(projects);
}
...
static bool vs_load_enriched_vector(engine_store_t *s, const char *project, const char *token,
                                    float *out) {
    ...
    if (sqlite3_step(tv_stmt) == SQLITE_ROW) {
        ...
        if (vec && vec_len == VS_VEC_DIM) {
            for (int d = 0; d < VS_VEC_DIM; d++) {
                out[d] = (float)vec[d] / ENGINE_STORE_INT8_MAX;
            }
            found = true;
        }
    }
...
static void vs_fill_sparse_random(const char *token, float *out) {
    ...
    for (int i = 0; i < VS_SPARSE_NNZE; i++) {
        ...
        float sign = (h & SKIP_ONE) ? ENGINE_STORE_UNIT_POS_F : -ENGINE_STORE_UNIT_POS_F;
        out[pos] += sign;
    }
}
...
static int8_t vs_clamp_int8(float v) {
    if (v > ENGINE_STORE_INT8_MAX) {
        return (int8_t)ENGINE_STORE_INT8_MAX;
    }
    if (v < -ENGINE_STORE_INT8_MAX) {
        return (int8_t)-ENGINE_STORE_INT8_MAX;
    }
...
static bool vs_normalize_and_quantize(const float *src, int8_t *dst) {
    ...
    mag = sqrtf(mag);
    if (mag < ENGINE_STORE_DENOM_EPS_F) {
        return false;
    }
    float inv = ENGINE_STORE_UNIT_POS_F / mag;
    for (int d = 0; d < VS_VEC_DIM; d++) {
        dst[d] = vs_clamp_int8(src[d] * inv * ENGINE_STORE_INT8_MAX);
    }
    return true;
}
...
static int vs_build_keyword_vectors(engine_store_t *s, const char *project, const char **keywords,
...
static double vs_min_cosine_score(const int8_t *node_vec, int node_vec_len,
    ...
    double min_score = ENGINE_STORE_UNIT_POS_D;
    for (int k = 0; k < actual_kw; k++) {
        ...
        double cos_k = denom > ENGINE_STORE_DENOM_EPS_D ? (double)dot / denom : 0.0;
        if (cos_k < min_score) {
            min_score = cos_k;
        }
    }
...
static engine_vector_result_t *vs_append_result(engine_vector_result_t *results, int *count, int *cap,
                                             sqlite3_stmt *stmt,
                                             const int8_t (*kw_vecs)[VS_VEC_DIM], int actual_kw) {
    if (*count >= *cap) {
        int nc = *cap < ENGINE_SZ_16 ? ENGINE_SZ_16 : *cap * ST_COL_2;
        engine_vector_result_t *grown = realloc(results, (size_t)nc * sizeof(engine_vector_result_t));
        if (!grown) {
            return NULL;
        }
        results = grown;
...
int engine_store_vector_search(engine_store_t *s, const char *project, const char **keywords,
    ...
    *out_count = 0;
    ...
    const char *sql = "SELECT n.id, n.name, n.qualified_name, n.file_path, n.label,"
                      "       engine_cosine_i8(v.vector, ?1) as score, v.vector"
                      " FROM node_vectors v"
                      " INNER JOIN nodes n ON n.id = v.node_id"
                      " WHERE v.project = ?2"
                      " AND n.label IN (" ENGINE_SQL_CALLABLE_OR_TYPE_LABELS ")"
                      " ORDER BY score DESC"
                      " LIMIT ?3";
    ...
    int fetch_limit = (limit > 0 ? limit : ENGINE_SZ_16) * ST_COL_5;
...

    {
        ...
        engine_log_info("vector_search.exec", "kw_count", kw_buf, "fetch_limit", fl_buf, "project",
                     project);
    }

    engine_vector_result_t *results = NULL;
    int count = 0;
    int cap = 0;
    int step_rc = 0;
    while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        engine_vector_result_t *grown =
    ...
    if (step_rc != SQLITE_DONE) {
        ...
        engine_log_warn("vector_search.step_error", "rc", rc_buf, "msg", sqlite3_errmsg(s->db));
    }
    {
        char cnt_buf[VS_STR_BUF];
        snprintf(cnt_buf, sizeof(cnt_buf), "%d", count);
        engine_log_info("vector_search.done", "candidates", cnt_buf);
    }
    sqlite3_finalize(stmt);
    ...
    for (int i = 0; i < count - SKIP_ONE; i++) {
        for (int j = i + SKIP_ONE; j < count; j++) {
            if (results[j].score > results[i].score) {
                engine_vector_result_t tmp = results[i];
                results[i] = results[j];
                results[j] = tmp;
            }
        }
    ...
    int final_limit = limit > 0 ? limit : ENGINE_SZ_16;
