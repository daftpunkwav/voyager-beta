 ...
 * Cypher queries against the engine_store graph database.
 *
 * Supported syntax:
 ...
 */
#ifndef ENGINE_CYPHER_H
#define ENGINE_CYPHER_H
...
typedef struct {
    engine_token_t *tokens;
    int count;
    int capacity;
    char *error; /* NULL if no error */
} engine_lex_result_t;

/* Tokenize a Cypher query string. Caller must call engine_lex_free(). */
...
struct engine_expr {
    engine_expr_type_t type;
    engine_condition_t cond; /* leaf (EXPR_CONDITION only) */
    engine_expr_t *left;     /* AND/OR/XOR left; NOT child */
    engine_expr_t *right;    /* AND/OR/XOR right; NULL for NOT */
};

typedef struct {
    engine_expr_t *root; /* expression tree (NULL = use legacy conditions) */
    /* Legacy flat model — kept during migration, removed after Phase 2 */
    engine_condition_t *conditions;
    int count;
    const char *op; /* "AND" or "OR" */
} engine_where_clause_t;

/* CASE expression: CASE WHEN expr THEN val [ELSE val] END */
typedef struct {
    engine_expr_t *when_expr; /* condition */
    const char *then_val;  /* result if true */
} engine_case_branch_t;

typedef struct {
    engine_case_branch_t *branches;
    int branch_count;
...
typedef struct {
    ...
    engine_case_expr_t *kase; /* CASE expression (NULL if not CASE) */
    engine_func_arg_t *args;  /* args for a multi-argument function (NULL if none) */
    int arg_count;
} engine_return_item_t;
...
#define ENGINE_CYPHER_ORDER_KEYS_MAX 8

typedef struct {
    engine_return_item_t *items;
    int count;
    bool distinct;
    bool star; /* RETURN * */
    ...
    const char *order_keys[ENGINE_CYPHER_ORDER_KEYS_MAX];
    bool order_descs[ENGINE_CYPHER_ORDER_KEYS_MAX]; /* false = ASC (default) */
    int order_key_count;                         /* 0 = no ORDER BY */
    int skip;                                    /* SKIP N, 0 = none */
    int limit;                                   /* 0 = default */
} engine_return_clause_t;

/* Full query AST */
typedef struct engine_query engine_query_t;
struct engine_query {
    engine_pattern_t *patterns; /* array of patterns (first = main MATCH) */
    int pattern_count;
    bool *pattern_optional;              /* pattern_optional[i] = true → OPTIONAL MATCH */
    engine_where_clause_t *where;           /* NULL if no WHERE */
    engine_return_clause_t *with_clause;    /* WITH clause (NULL if none) */
    engine_where_clause_t *post_with_where; /* WHERE after WITH */
    engine_return_clause_t *ret;            /* NULL if no RETURN */
    engine_query_t *union_next;             /* next query in UNION chain (NULL if none) */
...
#define engine_query_pattern(q) ((q)->patterns[0])
...
typedef struct {
    ...
    char *error; /* NULL if no error */
} engine_parse_result_t;

/* Parse tokens into AST. Caller must call engine_parse_free(). */
int engine_parse(const engine_token_t *tokens, int token_count, engine_parse_result_t *out);
 ...
 * ENGINE_NOT_FOUND on overflow. Exposed for arithmetic-boundary unit tests. */
int engine_cypher_cross_join_alloc(int bind_count, int extra_count, bool opt, size_t *out_n);

#endif /* ENGINE_CYPHER_H */
