...
/* Write through the descriptor engine_mkstemp returned rather than reopening its
...
int engine_jsonrpc_parse(const char *line, engine_jsonrpc_request_t *out) {
    memset(out, 0, sizeof(*out));
    out->id = ENGINE_NOT_FOUND;
...
void engine_jsonrpc_request_free(engine_jsonrpc_request_t *r) {
    if (!r) {
        return;
    }
...
char *engine_jsonrpc_format_response(const engine_jsonrpc_response_t *resp) {
...
char *engine_jsonrpc_format_error(int64_t id, int code, const char *message) {
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
...
char *engine_mcp_text_result(const char *text, bool is_error) {
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
...
bool engine_mcp_cancel_request_matches(const char *params_json, int64_t active_id,
...
static const tool_def_t TOOLS[] = {
    {"index_repository", "Index repository",
     ...
     "deliberately NOT indexed (gitignore/.engineignore/skip-lists) — by design, not failures.",
     ...
     "\"Write compressed artifact to .graph-engine/graph.db.zst for team sharing. "
    ...
    {"index_status", "Index status",
     ...
     "BY DESIGN (gitignore/.engineignore/skip-lists): deliberate and deterministic, not failures; "
...
static bool mcp_tool_allowed(engine_mcp_tool_profile_t profile, const char *name) {
...
static const char *mcp_tool_profile_name(engine_mcp_tool_profile_t profile) {
    return profile == ENGINE_MCP_TOOL_PROFILE_SCOUT ? "scout" : "analysis";
}

int engine_mcp_parse_tool_profile_args(int argc, const char *const argv[const],
                                    engine_mcp_tool_profile_t *profile_out) {
    if (argc < 0 || !argv || !profile_out) {
        return -1;
    }
    *profile_out = ENGINE_MCP_TOOL_PROFILE_ALL;
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(arg, "--tool-profile=analysis") == 0) {
            *profile_out = ENGINE_MCP_TOOL_PROFILE_ANALYSIS;
            continue;
        }
        if (strcmp(arg, "--tool-profile=scout") == 0) {
            *profile_out = ENGINE_MCP_TOOL_PROFILE_SCOUT;
            continue;
        }
...
bool engine_mcp_tool_profile_allows_http(engine_mcp_tool_profile_t profile) {
    return profile == ENGINE_MCP_TOOL_PROFILE_ALL;
}

static int mcp_allowed_tool_count(engine_mcp_tool_profile_t profile) {
    int count = 0;
...
static char *engine_mcp_tools_list_range(engine_mcp_tool_profile_t profile, int offset, int limit,
...
char *engine_mcp_tools_list(void) {
    return engine_mcp_tools_list_range(ENGINE_MCP_TOOL_PROFILE_ALL, 0, TOOL_COUNT, false);
}
...
const char *engine_mcp_tool_input_schema(const char *tool_name) {
    if (!tool_name) {
        return NULL;
    }
    for (int i = 0; i < TOOL_COUNT; i++) {
...
int engine_mcp_tool_count(void) {
    return TOOL_COUNT;
}

const char *engine_mcp_tool_name(int index) {
...
char *engine_mcp_tools_help_list(void) {
...
static char *engine_mcp_tools_list_page(engine_mcp_tool_profile_t profile, const char *params_json) {
    bool has_cursor = false;
    int offset = mcp_tools_cursor_offset(params_json, &has_cursor);
    if (!has_cursor) {
        return engine_mcp_tools_list_range(profile, 0, TOOL_COUNT, false);
    }
    return engine_mcp_tools_list_range(profile, offset, MCP_TOOLS_PAGE_SIZE, true);
}
...
static char *engine_mcp_prompts_list(void) {
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
...
static char *engine_mcp_prompt_get(const char *params_json, char **error_json) {
    *error_json = NULL;
...
static char *engine_mcp_initialize_response_for_profile(const char *params_json,
                                                     engine_mcp_tool_profile_t profile) {
    ...
    yyjson_mut_obj_add_str(doc, impl, "name", "graph-engine");
    yyjson_mut_obj_add_str(doc, impl, "version", engine_cli_get_version());
...
char *engine_mcp_initialize_response(const char *params_json) {
    return engine_mcp_initialize_response_for_profile(params_json, ENGINE_MCP_TOOL_PROFILE_ALL);
}
...
char *engine_mcp_get_tool_name(const char *params_json) {
    yyjson_doc *doc = yyjson_read(params_json, strlen(params_json), 0);
    if (!doc) {
        return NULL;
    }
...
char *engine_mcp_get_arguments(const char *params_json) {
    yyjson_doc *doc = yyjson_read(params_json, strlen(params_json), 0);
    if (!doc) {
        return NULL;
    }
...
char *engine_mcp_get_string_arg(const char *args_json, const char *key) {
...
static char *canonicalize_repo_path_if_exists(char *repo_path) {
    ...
    if (engine_canonical_path(repo_path, real, sizeof(real))) {
        engine_normalize_path_sep(real);
...
static char *normalize_project_arg(char *project) {
    ...
    char *normalized = engine_project_name_from_path(project);
    if (normalized) {
        free(project);
        return normalized;
    }
...
bool engine_validate_project_name(const char *project);

/* #1025: agents naturally pass the repo FOLDER name ("graph-engine"),
...
static char *resolve_project_tail(char *project) {
    if (!project || !engine_validate_project_name(project)) {
        return project;
    }
    char dir[ENGINE_SZ_1K];
    cache_dir(dir, sizeof(dir));
    char exact[ENGINE_SZ_2K];
    snprintf(exact, sizeof(exact), "%s/%s.db", dir, project);
    if (engine_file_exists(exact)) {
        return project; /* exact name — untouched fast path */
    }
    size_t plen = strlen(project);
    char match[ENGINE_SZ_1K] = "";
    int matches = 0;
    engine_dir_t *d = engine_opendir(dir);
    if (!d) {
        return project;
    }
    engine_dirent_t *entry;
    while ((entry = engine_readdir(d)) != NULL) {
    ...
    engine_closedir(d);
    if (matches == 1) {
        engine_log_info("mcp.project_tail_resolved", "passed", project, "resolved", match);
        free(project);
...
static char *get_project_arg(const char *args_json) {
    char *p = engine_mcp_get_string_arg(args_json, "project");
    if (!p) {
        p = engine_mcp_get_string_arg(args_json, "project_name");
    }
    if (!p) {
        p = engine_mcp_get_string_arg(args_json, "project_id");
    }
    if (!p) {
        p = engine_mcp_get_string_arg(args_json, "projectName");
    }
    return resolve_project_tail(normalize_project_arg(p));
}

int engine_mcp_get_int_arg(const char *args_json, const char *key, int default_val) {
    yyjson_doc *doc = yyjson_read(args_json, strlen(args_json), 0);
    if (!doc) {
        return default_val;
    }
...
bool engine_mcp_get_bool_arg(const char *args_json, const char *key) {
    yyjson_doc *doc = yyjson_read(args_json, strlen(args_json), 0);
    if (!doc) {
        return false;
    }
...
struct engine_mcp_server {
    engine_store_t *store;     /* currently open project store (or NULL) */
    ...
    char session_root[ENGINE_SZ_1K];     /* detected project root path */
    char session_project[ENGINE_SZ_256]; /* derived project name */
    ...
    struct engine_config *config;        /* external config ref (not owned) */
    engine_mcp_index_executor_fn index_executor;
    void *index_executor_context;
    engine_proc_log_cb index_log_callback;
    void *index_log_context;
    engine_mcp_project_mutation_begin_fn mutation_begin;
    engine_mcp_project_mutation_try_begin_fn mutation_try_begin;
    engine_mcp_project_mutation_end_fn mutation_end;
    void *mutation_context;
    engine_mcp_quarantine_test_hook_fn quarantine_test_hook;
    void *quarantine_test_context;
    engine_mcp_command_test_hook_fn command_test_hook;
    void *command_test_context;
    engine_thread_t autoindex_tid;
    ...
    engine_mutex_t request_scope_mutex;
    unsigned int request_scope_depth;
    atomic_int pipeline_cancel_requested;
    engine_pipeline_t *active_pipeline; /* non-NULL while index_repository runs */
    ...
    engine_mcp_tool_profile_t tool_profile;
};

engine_mcp_server_t *engine_mcp_server_new(const char *store_path) {
    engine_mcp_server_t *srv = calloc(ENGINE_ALLOC_ONE, sizeof(*srv));
    if (!srv) {
        return NULL;
    }
    engine_mutex_init(&srv->request_scope_mutex);
    ...
    if (store_path) {
        srv->store = engine_store_open(store_path);
        srv->current_project = heap_strdup(store_path);
    } else {
        srv->store = engine_store_open_memory();
    }
    srv->owns_store = true;
    srv->tool_profile = ENGINE_MCP_TOOL_PROFILE_ALL;
    srv->background_tasks = true;

    return srv;
}

void engine_mcp_server_set_tool_profile(engine_mcp_server_t *srv, engine_mcp_tool_profile_t profile) {
    if (srv) {
        srv->tool_profile = profile;
    }
}

engine_store_t *engine_mcp_server_store(engine_mcp_server_t *srv) {
    return srv ? srv->store : NULL;
}

void engine_mcp_server_set_project(engine_mcp_server_t *srv, const char *project) {
    if (!srv) {
        return;
    }
...
void engine_mcp_server_set_watcher(engine_mcp_server_t *srv, struct engine_watcher *w) {
    if (srv) {
        srv->watcher = w;
    }
}

void engine_mcp_server_set_config(engine_mcp_server_t *srv, struct engine_config *cfg) {
    if (srv) {
        srv->config = cfg;
    }
}

bool engine_mcp_server_set_session_context(engine_mcp_server_t *srv, const char *session_root,
    ...
    char *project = engine_project_name_from_path(session_root);
...
const char *engine_mcp_server_session_root(const engine_mcp_server_t *srv) {
    return srv ? srv->session_root : NULL;
}

const char *engine_mcp_server_session_project(const engine_mcp_server_t *srv) {
    return srv ? srv->session_project : NULL;
}

const char *engine_mcp_server_allowed_root(const engine_mcp_server_t *srv) {
    return srv ? srv->allowed_root : NULL;
}

void engine_mcp_server_set_background_tasks(engine_mcp_server_t *srv, bool enabled) {
    if (srv) {
        srv->background_tasks = enabled;
    }
}

void engine_mcp_server_set_index_executor(engine_mcp_server_t *srv, engine_mcp_index_executor_fn executor,
                                       void *context) {
    if (srv) {
...
void engine_mcp_server_set_index_log_callback(engine_mcp_server_t *srv, engine_proc_log_cb callback,
                                           void *context) {
    if (srv) {
...
void engine_mcp_server_set_project_mutation_guard(engine_mcp_server_t *srv,
                                               engine_mcp_project_mutation_begin_fn begin,
                                               engine_mcp_project_mutation_end_fn end, void *context) {
    if (!srv) {
        return;
    }
...
void engine_mcp_server_set_project_mutation_try_guard(
    engine_mcp_server_t *srv, engine_mcp_project_mutation_try_begin_fn try_begin) {
    if (srv && srv->mutation_begin) {
        srv->mutation_try_begin = try_begin;
    }
}

static bool mcp_project_mutation_begin(engine_mcp_server_t *srv, const char *project) {
    return !srv->mutation_begin || srv->mutation_begin(srv->mutation_context, project);
}

static bool mcp_project_mutation_try_begin(engine_mcp_server_t *srv, const char *project) {
    return !srv->mutation_begin ||
           (srv->mutation_try_begin && srv->mutation_try_begin(srv->mutation_context, project));
}

static void mcp_project_mutation_end(engine_mcp_server_t *srv, const char *project) {
    if (srv->mutation_end) {
        srv->mutation_end(srv->mutation_context, project);
    }
}

void engine_mcp_server_free(engine_mcp_server_t *srv) {
    if (!srv) {
        return;
    }
    if (srv->autoindex_active) {
        engine_thread_join(&srv->autoindex_tid);
    }
    if (srv->owns_store && srv->store) {
        engine_store_close(srv->store);
    }
    free(srv->current_project);
    free(srv->allowed_root);
    free(srv->active_request_id_str);
    engine_mutex_destroy(&srv->request_scope_mutex);
    free(srv);
}

/* ── Idle store eviction ──────────────────────────────────────── */

void engine_mcp_server_evict_idle(engine_mcp_server_t *srv, int timeout_s) {
    if (!srv || !srv->store) {
        return;
    }
...
bool engine_mcp_server_has_cached_store(engine_mcp_server_t *srv) {
    return (srv && srv->store != NULL) != 0;
}

bool engine_mcp_server_release_pristine_memory_store(engine_mcp_server_t *srv) {
    const char *db_path = srv && srv->store ? engine_store_db_path(srv->store) : NULL;
...
engine_pipeline_t *engine_mcp_server_active_pipeline(engine_mcp_server_t *srv) {
    return srv ? srv->active_pipeline : NULL;
}

bool engine_mcp_server_cancel_active(engine_mcp_server_t *srv) {
    if (!srv) {
        return false;
    }
    engine_mutex_lock(&srv->request_scope_mutex);
    bool active = srv->request_scope_depth != 0;
    if (active) {
        atomic_store_explicit(&srv->pipeline_cancel_requested, 1, memory_order_release);
    }
    engine_mutex_unlock(&srv->request_scope_mutex);
    return active;
}

bool engine_mcp_server_request_scope_begin(engine_mcp_server_t *srv) {
    if (!srv) {
        return false;
    }
    engine_mutex_lock(&srv->request_scope_mutex);
...
void engine_mcp_server_request_scope_end(engine_mcp_server_t *srv) {
    if (!srv) {
        return;
    }
    engine_mutex_lock(&srv->request_scope_mutex);
...
static bool mcp_request_cancelled(const engine_mcp_server_t *srv) {
    return srv && atomic_load_explicit(&srv->pipeline_cancel_requested, memory_order_acquire) != 0;
}

void engine_mcp_server_set_quarantine_test_hook(engine_mcp_server_t *srv,
                                             engine_mcp_quarantine_test_hook_fn hook, void *context) {
    if (!srv) {
        return;
    }
...
void engine_mcp_server_set_command_test_hook(engine_mcp_server_t *srv, engine_mcp_command_test_hook_fn hook,
                                          void *context) {
    if (!srv) {
        return;
    }
 ...
 * handle is transferred to the caller (who must engine_store_close it). On failure
...
static engine_store_t *resolve_store_fallback_scan(const char *project);
...
static bool reserve_unique_corrupt_pending(const char *path, char *pending, size_t pending_size,
    ...
    for (unsigned int attempt = 0; attempt < 128; attempt++) {
        uint64_t token = engine_now_ns() ^ ((uint64_t)(unsigned int)getpid() << 32) ^
        ...
        if (engine_file_exists(backup)) {
            continue;
        }
#ifdef _WIN32
        wchar_t *wide = engine_path_to_wide(pending);
...
static void discard_corrupt_pending(const char *pending) {
    ...
    (void)engine_remove_db_sidecars(pending);
    (void)engine_unlink(pending);
}

#ifndef _WIN32
static bool sync_parent_directory(const char *path) {
    char directory[ENGINE_SZ_2K];
...
#ifdef _WIN32
    wchar_t *wide_pending = engine_path_to_wide(pending);
    wchar_t *wide_backup = engine_path_to_wide(backup);
...
static bool quarantine_step_allowed(engine_mcp_server_t *srv, const char *step) {
...
static bool quarantine_corrupt_store(engine_mcp_server_t *srv, const char *project, const char *path,
                                     char *backup_out, size_t backup_out_size) {
    char backup[ENGINE_SZ_2K];
    char pending[ENGINE_SZ_2K];
    ...
    if (!path || !path[0]) {
        engine_log_error("store.auto_clean_failed", "project", project, "path", "", "reason",
                      "empty store path");
        return false;
    }
    if (!reserve_unique_corrupt_pending(path, pending, sizeof(pending), backup, sizeof(backup))) {
        engine_log_error("store.auto_clean_failed", "project", project, "path", path, "reason",
                      "cannot reserve unique backup");
        return false;
    }

    if (engine_store_backup_path(path, pending) != ENGINE_STORE_OK ||
        engine_store_prepare_path_for_replace(pending) != ENGINE_STORE_OK) {
    ...
    engine_store_t *snapshot = engine_store_open_path_query(pending);
    if (!snapshot) {
        discard_corrupt_pending(pending);
        engine_log_error("store.auto_clean_failed", "project", project, "path", path, "reason",
    ...
    if (engine_unlink(path) != 0 && errno != ENOENT) {
        engine_log_error("store.auto_clean_failed", "project", project, "path", path, "reason",
                      "backup complete; live database removal failed", "backup", backup);
        return false;
    }
    if (engine_remove_db_sidecars(path) != 0) {
        engine_log_error("store.auto_clean_sidecars", "project", project, "path", path, "reason",
...
static engine_store_t *resolve_store_internal(engine_mcp_server_t *srv, const char *project,
     ...
     * validation. */
    char path[ENGINE_SZ_1K];
    project_db_path(project, path, sizeof(path));
    srv->store = path[0] ? engine_store_open_path_query(path) : NULL;
    if (srv->store) {
        /* Check DB integrity — back up (never silently delete) a corrupt DB */
        if (!engine_store_check_integrity(srv->store)) {
            engine_store_close(srv->store);
            srv->store = NULL;
            ...
            srv->store = engine_store_open_path_query(path);
            bool current_valid = srv->store && engine_store_check_integrity(srv->store);
            if (!current_valid) {
                engine_store_close(srv->store);
                srv->store = NULL;
                char backup[ENGINE_SZ_2K] = {0};
                bool quarantined =
                    quarantine_corrupt_store(srv, project, path, backup, sizeof(backup));
                engine_log_error("store.auto_clean", "project", project, "path", path, "action",
        ...
        engine_project_t proj_verify = {0};
        if (engine_store_get_project(srv->store, project, &proj_verify) == ENGINE_STORE_OK) {
            engine_project_free_fields(&proj_verify);
            srv->owns_store = true;
            free(srv->current_project);
    ...
    engine_store_t *scanned = resolve_store_fallback_scan(project);
    if (scanned) {
        srv->store = scanned;
        srv->owns_store = true;
        free(srv->current_project);
...
static engine_store_t *resolve_store(engine_mcp_server_t *srv, const char *project) {
...
static void free_node_contents(engine_node_t *n);
...
static int collect_db_project_names(const char *dir_path, char *out, size_t out_sz) {
    int count = 0;
    int offset = 0;
    engine_dir_t *d = engine_opendir(dir_path);
    if (!d) {
        return 0;
    }
    engine_dirent_t *entry;
    while ((entry = engine_readdir(d)) != NULL) {
        ...
        char full_path[ENGINE_SZ_2K];
        snprintf(full_path, sizeof(full_path), "%s/%s", dir_path, n);
        char iname[ENGINE_SZ_1K];
...
static void add_git_context_json(yyjson_mut_doc *doc, yyjson_mut_val *obj, const char *root_path) {
    engine_git_context_t ctx = {0};
    (void)engine_git_context_resolve(root_path, &ctx);
...
static char *build_project_list_error(const char *reason) {
    char dir_path[ENGINE_SZ_1K];
    cache_dir(dir_path, sizeof(dir_path));

    char projects[ENGINE_SZ_4K] = "";
...
#define REQUIRE_STORE(store, project)                     \
    do {                                                  \
        if (!(store)) {                                   \
            ...
            char *_res = engine_mcp_text_result(_err, true); \
            free(_err);                                   \
            free(project);                                \
            return _res;                                  \
        }                                                 \
...
static bool project_has_adr(engine_store_t *store, const char *project, const char *root_path) {
    if (store && project) {
        engine_adr_t adr;
        memset(&adr, 0, sizeof(adr));
        if (engine_store_adr_get(store, project, &adr) == ENGINE_STORE_OK) {
            engine_store_adr_free(&adr);
            return true;
        }
    }
    ...
    char adr_path[ENGINE_SZ_4K];
    snprintf(adr_path, sizeof(adr_path), "%s/.graph-engine/adr.md", root_path);
    struct stat adr_st;
 ...
 * "tmp-" (engine_project_name_from_path: "/tmp/bench/..." → "tmp-bench-...";
...
static bool db_internal_project_name(const char *full_path, char *name_out, size_t name_sz,
    ...
    engine_store_t *st = engine_store_open_path_query(full_path);
    if (!st) {
        return false; /* nonexistent / unreadable */
    }
    engine_project_t *projs = NULL;
    int n = 0;
    bool ok = false;
    if (engine_store_list_projects(st, &projs, &n) == ENGINE_STORE_OK) {
    ...
    engine_store_free_projects(projs, n);
    if (ok && out_store) {
        *out_store = st; /* transfer ownership to caller */
    } else {
        engine_store_close(st);
    }
    return ok;
}
...
static void build_project_json_entry(yyjson_mut_doc *doc, yyjson_mut_val *arr, const char *dir_path,
    ...
    char project_name[ENGINE_SZ_1K];
    engine_store_t *pstore = NULL;
    ...
    int edges = 0;
    if (!metadata_only) {
        nodes = engine_store_count_nodes(pstore, project_name);
        edges = engine_store_count_edges(pstore, project_name);
    }
    char root_path_buf[ENGINE_SZ_1K] = "";
    engine_project_t proj = {0};
    if (engine_store_get_project(pstore, project_name, &proj) == ENGINE_STORE_OK) {
        if (proj.root_path) {
            snprintf(root_path_buf, sizeof(root_path_buf), "%s", proj.root_path);
        }
        engine_project_free_fields(&proj);
    }
    engine_store_close(pstore);
    ...
    if (!metadata_only && root_path_buf[0]) {
        engine_git_context_t gctx = {0};
        (void)engine_git_context_resolve(root_path_buf, &gctx);
        if (gctx.is_git && gctx.branch) {
            yyjson_mut_obj_add_strcpy(doc, p, "branch", gctx.branch);
        }
        engine_git_context_free(&gctx);
    }
    if (!metadata_only) {
...
static char *handle_list_projects(engine_mcp_server_t *srv, const char *args) {
    (void)srv;
    ...
    while ((entry = engine_readdir(d)) != NULL) {
        ...
        char full_path[ENGINE_SZ_2K];
        snprintf(full_path, sizeof(full_path), "%s/%s", dir_path, name);
        int64_t size_bytes = engine_file_size(full_path);
        if (size_bytes < 0) {
            continue;
        }
    ...
    char *result = engine_mcp_text_result(json, false);
    free(json);
    return result;
}
 ...
 * resolve_store uses engine_store_open_path_query (no SQLITE_OPEN_CREATE), so
...
static char *verify_project_indexed(engine_store_t *store, const char *project) {
    engine_project_t proj_check = {0};
    if (engine_store_get_project(store, project, &proj_check) != ENGINE_STORE_OK) {
        char *err = build_project_list_error("project not indexed — run index_repository first");
        char *res = engine_mcp_text_result(err, true);
        free(err);
        return res;
    }
    engine_project_free_fields(&proj_check);
    return NULL;
}

static bool sg_field_blocked(const char *f); /* internal-only fields, defined with search_graph */

static char *handle_get_graph_schema(engine_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    engine_store_t *store = resolve_store(srv, project);
    ...
    engine_schema_info_t schema = {0};
    engine_store_get_schema(store, project, &schema);
    ...
    /* Check ADR presence */
    engine_project_t proj_info = {0};
    if (engine_store_get_project(store, project, &proj_info) == 0 && proj_info.root_path) {
        ...
        engine_project_free_fields(&proj_info);
    }

    char *json = yy_doc_to_str(doc);
    yyjson_mut_doc_free(doc);
    engine_store_schema_free(&schema);
    free(project);
...
static void enrich_add_bfs(yyjson_mut_doc *doc, yyjson_mut_val *arr, engine_traverse_result_t *tr) {
...
static yyjson_mut_val *enrich_connected(yyjson_mut_doc *doc, engine_store_t *store, int64_t node_id,
    ...
    engine_traverse_result_t tr_in = {0};
    engine_store_bfs(store, node_id, "inbound", et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr_in);
    enrich_add_bfs(doc, conn, &tr_in);
    engine_store_traverse_free(&tr_in);

    engine_traverse_result_t tr_out = {0};
    engine_store_bfs(store, node_id, "outbound", et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr_out);
    enrich_add_bfs(doc, conn, &tr_out);
    engine_store_traverse_free(&tr_out);
...
static void enrich_connected_joined(engine_store_t *store, int64_t node_id, const char *relationship,
    ...
    for (int d = 0; d < 2; d++) {
        engine_traverse_result_t tr = {0};
        engine_store_bfs(store, node_id, dirs[d], et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr);
...
static char *bm25_file_pattern_like(const char *file_pattern) {
    ...
    char *like = engine_glob_to_like(file_pattern);
...
static char *bm25_search(engine_store_t *store, const char *project, const char *query,
                         const char *file_pattern, int limit, int offset, bool toon) {
    sqlite3 *db = engine_store_get_db(store);
    if (!db) {
        return NULL;
    }
    ...
    const char *sql =
        ...
        "               WHEN n.label IN (" ENGINE_SQL_TYPE_LIKE_LABELS ") THEN 5.0 "
    ...
    if (toon) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            engine_tree_row_begin(&rows);
            engine_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_QN), true);
            engine_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_LABEL),
                              false);
            engine_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_FILE), false);
            engine_tree_cell_str(&rows, lines, false);
            engine_tree_cell_real(&rows, sqlite3_column_double(stmt, BM25_COL_RANK), false);
            engine_tree_row_end(&rows);
            emitted++;
        }
        ...
        engine_sb_t sb;
        engine_sb_init(&sb);
        engine_tree_scalar_int(&sb, "total", total);
        engine_tree_scalar_str(&sb, "search_mode", "bm25");
        static const char *const cols[] = {"qn", "label", "file", "lines", "rank"};
        engine_tree_table_header(&sb, "results", emitted, cols, 5);
        char *rows_text = engine_sb_finish(&rows);
        engine_sb_append(&sb, rows_text ? rows_text : "");
        free(rows_text);
        engine_tree_scalar_bool(&sb, "has_more", total > offset + emitted);
        return engine_sb_finish(&sb);
    }
...
static void emit_semantic_results(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                  engine_vector_result_t *vresults, int vcount) {
 ...
 * *out_vcount (caller frees via engine_store_free_vector_results when vcount>0).
 * Returns true if semantic_query was provided as a non-array (type error —
 * caller should surface to the user). */
static bool run_semantic_query_core(const char *args, engine_store_t *store, const char *project,
                                    int limit, engine_vector_result_t **out_vresults, int *out_vcount,
                                    bool *out_present) {
    ...
    *out_vcount = 0;
    ...
    if (sq_val && !yyjson_is_arr(sq_val)) {
        ...
        engine_vector_result_t *vresults = NULL;
        int vcount = 0;
        int sem_limit = limit > 0 ? limit : ENGINE_SZ_16;
        if (engine_store_vector_search(store, project, keywords, ki, sem_limit, &vresults, &vcount) ==
                ENGINE_STORE_OK &&
            vcount > 0) {
...
static bool run_semantic_query(yyjson_mut_doc *doc, yyjson_mut_val *root, const char *args,
                               engine_store_t *store, const char *project, int limit) {
    engine_vector_result_t *vresults = NULL;
    int vcount = 0;
    bool type_error =
    ...
    if (vcount > 0) {
        ...
        engine_store_free_vector_results(vresults, vcount);
    }
    return type_error;
}
...
static void sg_toon_extra_cells(engine_sb_t *sb, const char *props_json, const char *const *fields,
                                int nfields) {
    yyjson_doc *pd =
    ...
    for (int f = 0; f < nfields; f++) {
        ...
        if (v && yyjson_is_str(v)) {
            engine_tree_cell_str(sb, yyjson_get_str(v), false);
        } else if (v && yyjson_is_bool(v)) {
            engine_tree_cell_bool(sb, yyjson_get_bool(v), false);
        } else if (v && yyjson_is_int(v)) {
            engine_tree_cell_int(sb, yyjson_get_int(v), false);
        } else if (v && yyjson_is_real(v)) {
            engine_tree_cell_real(sb, yyjson_get_real(v), false);
        } else {
            engine_tree_cell_str(sb, "", false);
        }
    }
    if (pd) {
        yyjson_doc_free(pd);
...
static void emit_search_results_toon(engine_sb_t *sb, const engine_search_output_t *out, int offset,
                                     const char *const *fields, int nfields, bool detail_ids) {
    engine_tree_scalar_int(sb, "total", out->total);
    if (detail_ids) {
        ...
        engine_tree_table_header(sb, "results", out->count, id_cols, 1);
        for (int i = 0; i < out->count; i++) {
            engine_tree_row_begin(sb);
            engine_tree_cell_str(sb, out->results[i].node.qualified_name, true);
            engine_tree_row_end(sb);
        }
        engine_tree_scalar_bool(sb, "has_more", out->total > offset + out->count);
        return;
    }
    ...
    engine_tree_table_header(sb, "results", out->count, cols, ncols);
    for (int i = 0; i < out->count; i++) {
        const engine_search_result_t *sr = &out->results[i];
        char lines[ENGINE_SZ_32];
        sg_lines_str(lines, sizeof(lines), sr->node.start_line, sr->node.end_line);
        engine_tree_row_begin(sb);
        engine_tree_cell_str(sb, sr->node.qualified_name, true);
        engine_tree_cell_str(sb, sr->node.label, false);
        engine_tree_cell_str(sb, sr->node.file_path, false);
        engine_tree_cell_str(sb, lines, false);
        engine_tree_cell_int(sb, sr->in_degree, false);
        engine_tree_cell_int(sb, sr->out_degree, false);
...
static int sg_cmp_by_qn(const void *pa, const void *pb) {
    const engine_search_result_t *a = (const engine_search_result_t *)pa;
    const engine_search_result_t *b = (const engine_search_result_t *)pb;
    const char *qa = a->node.qualified_name ? a->node.qualified_name : "";
    const char *qb = b->node.qualified_name ? b->node.qualified_name : "";
    return strcmp(qa, qb);
}

static void emit_search_results_tree(engine_sb_t *sb, engine_search_output_t *out, int offset,
                                     const char *const *fields, int nfields, engine_store_t *store,
                                     const char *relationship, bool include_connected) {
    char buf[ENGINE_SZ_512];
    char extra_cols[ENGINE_SZ_256] = "";
    for (int f = 0; f < nfields; f++) {
    ...
    engine_sb_append(sb, buf);
    /* Sort by qn so same-prefix rows are adjacent (module clustering). */
    if (out->count > 1) {
        qsort(out->results, (size_t)out->count, sizeof(engine_search_result_t), sg_cmp_by_qn);
    }
    char cur_group[ENGINE_SZ_1K] = "";
    for (int i = 0; i < out->count; i++) {
        const engine_search_result_t *sr = &out->results[i];
        ...
        if (nfields > 0) {
            ...
            for (int f = 0; f < nfields; f++) {
                ...
                if (v && yyjson_is_str(v)) {
                    ...
                    engine_tree_cell_str(sb, "", false); /* emits "-" */
                }
            }
            if (pd) {
                yyjson_doc_free(pd);
...
static void emit_search_results_tree_json(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                          engine_search_output_t *out, int offset,
                                          const char *const *fields, int nfields,
                                          engine_store_t *store, const char *relationship,
    ...
    char cur_key[ENGINE_SZ_1K] = "";
    for (int i = 0; i < out->count; i++) {
        const engine_search_result_t *sr = &out->results[i];
        const char *qn = sr->node.qualified_name ? sr->node.qualified_name : "";
        const char *file = sr->node.file_path ? sr->node.file_path : "";
        size_t plen = sg_qn_prefix_len(qn);
        char key[ENGINE_SZ_1K];
...
static void emit_semantic_results_toon(engine_sb_t *sb, const engine_vector_result_t *vresults,
                                       int vcount) {
    static const char *const cols[] = {"qn", "label", "file", "score"};
    engine_tree_table_header(sb, "semantic", vcount, cols, 4);
    for (int v = 0; v < vcount; v++) {
        engine_tree_row_begin(sb);
        engine_tree_cell_str(sb, vresults[v].qualified_name, true);
        engine_tree_cell_str(sb, vresults[v].label, false);
        engine_tree_cell_str(sb, vresults[v].file_path, false);
        engine_tree_cell_real(sb, vresults[v].score, false);
        engine_tree_row_end(sb);
    }
}

static char *handle_search_graph(engine_mcp_server_t *srv, const char *args) {
    ...
    engine_mem_phase_mark("handler.args");
    char *project = get_project_arg(args);
    engine_mem_phase_mark("handler.resolve_store");
    engine_store_t *store = resolve_store(srv, project);
    engine_mem_phase_mark("handler.body");
    REQUIRE_STORE(store, project);
    ...
    char *format_arg = engine_mcp_get_string_arg(args, "format");
    ...
    char *query = engine_mcp_get_string_arg(args, "query");
    if (query && query[0]) {
        int q_limit = engine_mcp_get_int_arg(args, "limit", BM25_DEFAULT_LIMIT);
        int q_offset = engine_mcp_get_int_arg(args, "offset", 0);
        char *q_file_pattern = engine_mcp_get_string_arg(args, "file_pattern");
        char *bm25_json =
        ...
        if (bm25_json) {
            free(query);
            free(project);
            char *result = engine_mcp_text_result(bm25_json, false);
            free(bm25_json);
            return result;
        }
    }
    ...
    char *label = engine_mcp_get_string_arg(args, "label");
    char *name_pattern = engine_mcp_get_string_arg(args, "name_pattern");
    char *qn_pattern = engine_mcp_get_string_arg(args, "qn_pattern");
    char *file_pattern = engine_mcp_get_string_arg(args, "file_pattern");
    char *relationship = engine_mcp_get_string_arg(args, "relationship");
    bool exclude_entry_points = engine_mcp_get_bool_arg(args, "exclude_entry_points");
    bool include_connected = engine_mcp_get_bool_arg(args, "include_connected");
    int limit = engine_mcp_get_int_arg(args, "limit", ENGINE_DEFAULT_SEARCH_LIMIT);
    int offset = engine_mcp_get_int_arg(args, "offset", 0);
    int min_degree = engine_mcp_get_int_arg(args, "min_degree", ENGINE_NOT_FOUND);
    int max_degree = engine_mcp_get_int_arg(args, "max_degree", ENGINE_NOT_FOUND);
    ...
    if (relationship && !validate_edge_type(relationship)) {
        ...
        free(file_pattern);
        free(relationship);
        return engine_mcp_text_result("relationship must be uppercase letters and underscores", true);
    }

    engine_search_params_t params = {
        .project = project,
        .label = label,
    ...
    if (!legacy_json) {
        ...
        char *sg_detail = engine_mcp_get_string_arg(args, "detail");
        ...
        if (!sq_type_error) {
            ...
            bool has_filters = label || name_pattern || qn_pattern || file_pattern ||
                               ...
                               min_degree != ENGINE_NOT_FOUND || max_degree != ENGINE_NOT_FOUND;
            bool semantic_only = sq_present && !has_filters;

            engine_sb_t sb;
            engine_sb_init(&sb);
            engine_search_output_t tout = {0};
            if (!semantic_only) {
                engine_store_search(store, &params, &tout);
                ...
                if (tout.total == 0) {
                    if (name_pattern && label) {
                        engine_tree_scalar_str(&sb, "hint",
            ...
            if (vcount > 0) {
                ...
                static const char *const sem_cols[] = {"qn", "label", "file", "score"};
                engine_tree_table_header(&sb, "semantic", 0, sem_cols, 4);
                engine_tree_scalar_str(&sb, "hint",
            ...
            free(file_pattern);
            free(relationship);
            char *text = engine_sb_finish(&sb);
            char *result = engine_mcp_text_result(text ? text : "out of memory", text == NULL);
            free(text);
            return result;
        }
    ...
    engine_search_output_t out = {0};
    engine_store_search(store, &params, &out);
...
static char *handle_query_graph(engine_mcp_server_t *srv, const char *args) {
    char *query = engine_mcp_get_string_arg(args, "query");
    char *project = get_project_arg(args);
    engine_store_t *store = resolve_store(srv, project);
    int max_rows = engine_mcp_get_int_arg(args, "max_rows", 0);
    ...
    char *graph_arg = engine_mcp_get_string_arg(args, "graph");
    bool missed_graph = graph_arg && strcmp(graph_arg, "missed") == 0;
    free(graph_arg);

    if (!query) {
        free(project);
        return engine_mcp_text_result("query is required", true);
    }
    if (missed_graph && !project) {
        free(query);
        return engine_mcp_text_result("project is required when graph=\"missed\"", true);
    }
    if (!store) {
    ...
    char covproj[ENGINE_SZ_512];
    const char *cypher_project = project;
    if (missed_graph) {
        engine_store_coverage_shadow_project(covproj, sizeof(covproj), project);
        cypher_project = covproj;
    }

    engine_cypher_result_t result = {0};
    int rc = engine_cypher_execute(store, query, cypher_project, max_rows, &result);

    if (rc < 0) {
        char *err_msg = result.error ? result.error : "query execution failed";
        char *resp = engine_mcp_text_result(err_msg, true);
        engine_cypher_result_free(&result);
        free(query);
        free(project);
        return resp;
    }
    ...
    char *qg_format = engine_mcp_get_string_arg(args, "format");
    ...
    if (!qg_legacy_json) {
        engine_sb_t sb;
        engine_sb_init(&sb);
        engine_tree_table_header(&sb, "rows", result.row_count, (const char *const *)result.columns,
                              result.col_count);
        for (int r = 0; r < result.row_count; r++) {
            engine_tree_row_begin(&sb);
            for (int c = 0; c < result.col_count; c++) {
                engine_tree_cell_str(&sb, result.rows[r][c], c == 0);
            }
            engine_tree_row_end(&sb);
        }
        engine_tree_scalar_int(&sb, "total", result.row_count);
        if (result.warning) {
            engine_tree_scalar_str(&sb, "warning", result.warning);
        }
    ...
    char *res = engine_mcp_text_result(json ? json : "out of memory", json == NULL);
    free(json);
    return res;
}
...
static void add_coverage_report(yyjson_mut_doc *doc, yyjson_mut_val *root, engine_store_t *store,
                                const char *project) {
    engine_coverage_row_t *rows = NULL;
    int count = 0;
    (void)engine_store_coverage_get(store, project, &rows, &count);
    ...
    engine_store_free_coverage(rows, count);
    ...
    if (ni_dir_n > 0 || ni_file_n > 0) {
        yyjson_mut_obj_add_str(doc, ni, "note",
                               ...
                               "gitignore/.engineignore/skip-lists (see each file's reason). Not an "
...
bool engine_path_within_root(const char *root_path, const char *abs_path); /* defined below */
...
#ifdef __APPLE__
    return ((int64_t)st->st_mtimespec.tv_sec * (int64_t)ENGINE_NSEC_PER_SEC) +
           (int64_t)st->st_mtimespec.tv_nsec;
#elif defined(_WIN32)
    return (int64_t)st->st_mtime * (int64_t)ENGINE_NSEC_PER_SEC;
#else
    return ((int64_t)st->st_mtim.tv_sec * (int64_t)ENGINE_NSEC_PER_SEC) + (int64_t)st->st_mtim.tv_nsec;
#endif
}

static const char *coverage_path_freshness(engine_store_t *store, const char *project,
    ...
    *outside = false;
    ...
    if (!engine_path_within_root(root_path, abs_path)) {
        *outside = true;
        return "outside_project";
    }

    engine_file_hash_t hash = {0};
    int rc = engine_store_get_file_hash(store, project, rel_path, &hash);
    if (rc == ENGINE_STORE_NOT_FOUND) {
        return "not_tracked";
    }
    if (rc != ENGINE_STORE_OK) {
        return "unavailable";
    }
    bool matches = hash.mtime_ns == coverage_stat_mtime_ns(&st) && hash.size == st.st_size;
    engine_store_clear_file_hash(&hash);
...
static void coverage_add_row_json(yyjson_mut_doc *doc, yyjson_mut_val *array,
                                  const engine_coverage_row_t *row, const char *requested_path) {
    yyjson_mut_val *item = yyjson_mut_obj(doc);
...
static const char *coverage_status(const engine_coverage_row_t *rows, int count,
...
static char *handle_check_index_coverage(engine_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    engine_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);
    ...
    engine_project_t proj = {0};
    bool have_project = engine_store_get_project(store, project, &proj) == ENGINE_STORE_OK;
    engine_coverage_meta_t meta = {0};
    bool have_meta = engine_store_coverage_meta_get(store, project, &meta) == ENGINE_STORE_OK;
    ...
    if (paths) {
        yyjson_arr_foreach(paths, idx, max, value) {
            ...
            engine_coverage_row_t *rows = NULL;
            int row_count = 0;
            int cov_rc = engine_store_coverage_get_path(store, project, rel, &rows, &row_count);
            bool lookup_ok = cov_rc == ENGINE_STORE_OK || cov_rc == ENGINE_STORE_NOT_FOUND;
            if (!lookup_ok) {
                row_count = 0;
            ...
            engine_store_free_coverage(rows, row_count);
            yyjson_mut_arr_add_val(path_results, item);
        }
    }
    yyjson_mut_obj_add_val(doc, root, "paths", path_results);

    int scope_limit = engine_mcp_get_int_arg(args, "scope_limit", COVERAGE_SCOPE_DEFAULT_LIMIT);
    int scope_offset = engine_mcp_get_int_arg(args, "scope_offset", 0);
    if (scope_limit < 1) {
        scope_limit = 1;
    ...
    if (scopes) {
        yyjson_arr_foreach(scopes, idx, max, value) {
            ...
            engine_coverage_row_t *rows = NULL;
            int row_count = 0;
            int cov_rc = engine_store_coverage_get_scope(store, project, scope, &rows, &row_count);
            bool lookup_ok = cov_rc == ENGINE_STORE_OK || cov_rc == ENGINE_STORE_NOT_FOUND;
            if (!lookup_ok) {
                row_count = 0;
                yyjson_mut_obj_add_str(doc, item, "coverage_lookup", "error");
    ...
    yyjson_doc_free(adoc);
    if (have_meta) {
        engine_store_coverage_meta_clear(&meta);
    }
    if (have_project) {
...
static char *handle_index_status(engine_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    engine_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);
     ...
     * stays lean. */
    bool verbose = engine_mcp_get_bool_arg(args, "verbose");
    ...
    if (project) {
        int nodes = engine_store_count_nodes(store, project);
        int edges = engine_store_count_edges(store, project);
        yyjson_mut_obj_add_str(doc, root, "project", project);
        yyjson_mut_obj_add_int(doc, root, "nodes", nodes);
        yyjson_mut_obj_add_int(doc, root, "edges", edges);
        yyjson_mut_obj_add_str(doc, root, "status", nodes > 0 ? "ready" : "empty");
        engine_project_t proj_info = {0};
        if (engine_store_get_project(store, project, &proj_info) == ENGINE_STORE_OK) {
...
static char *handle_delete_project(engine_mcp_server_t *srv, const char *args) {
    char *name = get_project_arg(args);
    if (!name) {
        return engine_mcp_text_result("project is required", true);
    }
    if (!mcp_project_mutation_begin(srv, name)) {
        free(name);
        return engine_mcp_text_result("project operation cancelled or blocked by an active index",
                                   true);
    }
    ...
    if (exists) {
        int rc = engine_unlink(path);
        (void)engine_unlink(wal);
        (void)engine_unlink(shm);
        if (rc == 0) {
            status = "deleted";
        } else {
    ...
    engine_pipeline_unlock();

    if (srv->watcher) {
        engine_watcher_unwatch(srv->watcher, name);
    }

    engine_mem_collect(); /* return freed pages to OS after closing database */
    ...
    char *result = engine_mcp_text_result(json, is_error);
    free(json);
    return result;
}
 ...
 * engine_store_arch_aspect_in_overview in store.c), or the aspect name itself. */
...
static bool aspect_wanted(yyjson_doc *aspects_doc, yyjson_val *aspects_arr, const char *name) {
    ...
    while ((val = yyjson_arr_iter_next(&iter)) != NULL) {
        ...
        if (strcmp(s, "overview") == 0 && engine_store_arch_aspect_in_overview(name)) {
            return true;
        }
        if (strcmp(s, name) == 0) {
            return true;
...
static void append_cross_repo_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                      const engine_schema_info_t *schema) {
 ...
 * ENGINE_STORE_OK, or ENGINE_STORE_ERR on failure (all outs zeroed). */
enum { ARCH_SCC_MAX_EDGES = 400000, ARCH_SCC_MAX_CYCLES = 100, ARCH_SCC_MEMBERS_SHOWN = 20 };

static int arch_compute_cycles(engine_store_t *store, const char *project, int64_t ***out_members,
    ...
    *edges_truncated = false;
    ...
    int64_t *tgt = NULL;
    int ecount = 0;
    if (engine_store_fetch_call_edges(store, project, ARCH_SCC_MAX_EDGES, &src, &tgt, &ecount,
                                   edges_truncated) != ENGINE_STORE_OK) {
        return ENGINE_STORE_ERR;
    }
    *scanned_edges = ecount;
    ...
    if (!scc_build(src, tgt, ecount, &g)) {
        free(src);
        free(tgt);
        return ENGINE_STORE_OK; /* no edges = no cycles, not an error */
    }
    free(src);
    free(tgt);
...
static void arch_node_qn(engine_store_t *store, int64_t id, char *out, size_t outsz) {
    engine_node_t n = {0};
    if (engine_store_find_node_by_id(store, id, &n) == ENGINE_STORE_OK && n.qualified_name) {
...
static char *handle_get_architecture(engine_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    engine_store_t *store = resolve_store(srv, project);
    ...
    REQUIRE_STORE(store, project);
    char *scope_path = engine_mcp_get_string_arg(args, "path");
    ...
    /* Build a C string array from aspects for engine_store_get_architecture.
    ...
    engine_store_get_schema_counts_scoped(store, project, scope_path, &schema);

    engine_architecture_info_t arch = {0};
    engine_store_get_architecture(store, project, scope_path, aspects_strs, aspects_strs_count, &arch);

    int node_count = engine_store_count_nodes_scoped(store, project, scope_path);
    int edge_count = engine_store_count_edges_scoped(store, project, scope_path);
    char norm_path[ENGINE_SZ_512];
    bool path_scoped = engine_store_normalize_arch_path(scope_path, norm_path, sizeof(norm_path));

    /* Response encoding: tree tables by default; format:"json" emits the
     * same model as structured JSON ({cols, rows} per section). */
    char *arch_format = engine_mcp_get_string_arg(args, "format");
    ...
    if (!arch_legacy_json) {
        ...
        engine_sb_init(&sb);
        if (project) {
            engine_tree_scalar_str(&sb, "project", project);
        }
        if (default_summary) {
            engine_tree_scalar_str(&sb, "aspects_hint",
        ...
        if (path_scoped) {
            engine_tree_scalar_str(&sb, "path", norm_path);
            engine_tree_scalar_int(&sb, "root_total_nodes", engine_store_count_nodes(store, project));
            engine_tree_scalar_int(&sb, "root_total_edges", engine_store_count_edges(store, project));
            engine_tree_scalar_int(&sb, "scoped_total_nodes", node_count);
            engine_tree_scalar_int(&sb, "scoped_total_edges", edge_count);
        }
        engine_tree_scalar_int(&sb, "total_nodes", node_count);
        engine_tree_scalar_int(&sb, "total_edges", edge_count);

        if (aspect_wanted(aspects_doc, aspects_arr, "structure") && schema.node_label_count > 0) {
            static const char *const lcols[] = {"label", "count"};
            engine_tree_table_header(&sb, "node_labels", schema.node_label_count, lcols, 2);
            for (int i = 0; i < schema.node_label_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, schema.node_labels[i].label, true);
                engine_tree_cell_int(&sb, schema.node_labels[i].count, false);
                engine_tree_row_end(&sb);
            }
        }
        if (aspect_wanted(aspects_doc, aspects_arr, "dependencies") && schema.edge_type_count > 0) {
            static const char *const tcols[] = {"type", "count"};
            engine_tree_table_header(&sb, "edge_types", schema.edge_type_count, tcols, 2);
            for (int i = 0; i < schema.edge_type_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, schema.edge_types[i].type, true);
                engine_tree_cell_int(&sb, schema.edge_types[i].count, false);
                engine_tree_row_end(&sb);
            }
        }
        if (aspect_wanted(aspects_doc, aspects_arr, "routes") && schema.rel_pattern_count > 0) {
            static const char *const pcols[] = {"pattern"};
            engine_tree_table_header(&sb, "relationship_patterns", schema.rel_pattern_count, pcols, 1);
            for (int i = 0; i < schema.rel_pattern_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, schema.rel_patterns[i], true);
                engine_tree_row_end(&sb);
            }
        }
        if (arch.language_count > 0) {
            static const char *const gcols[] = {"language", "files"};
            engine_tree_table_header(&sb, "languages", arch.language_count, gcols, 2);
            for (int i = 0; i < arch.language_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, arch.languages[i].language, true);
                engine_tree_cell_int(&sb, arch.languages[i].file_count, false);
                engine_tree_row_end(&sb);
            }
        }
        if (arch.package_count > 0) {
            static const char *const kcols[] = {"name", "nodes", "fan_in", "fan_out"};
            engine_tree_table_header(&sb, "packages", arch.package_count, kcols, 4);
            for (int i = 0; i < arch.package_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, arch.packages[i].name, true);
                engine_tree_cell_int(&sb, arch.packages[i].node_count, false);
                engine_tree_cell_int(&sb, arch.packages[i].fan_in, false);
                engine_tree_cell_int(&sb, arch.packages[i].fan_out, false);
                engine_tree_row_end(&sb);
            }
        }
        if (arch.entry_point_count > 0) {
            ...
            engine_tree_table_header(&sb, "entry_points", arch.entry_point_count, ecols, 2);
            for (int i = 0; i < arch.entry_point_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, arch.entry_points[i].qualified_name, true);
                engine_tree_cell_str(&sb, arch.entry_points[i].file, false);
                engine_tree_row_end(&sb);
            }
        }
        if (arch.route_count > 0) {
            static const char *const rcols[] = {"method", "path", "handler"};
            engine_tree_table_header(&sb, "routes", arch.route_count, rcols, 3);
            for (int i = 0; i < arch.route_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, arch.routes[i].method, true);
                engine_tree_cell_str(&sb, arch.routes[i].path, false);
                engine_tree_cell_str(&sb, arch.routes[i].handler, false);
                engine_tree_row_end(&sb);
            }
        }
        if (arch.hotspot_count > 0) {
            static const char *const hcols[] = {"qn", "fan_in"};
            engine_tree_table_header(&sb, "hotspots", arch.hotspot_count, hcols, 2);
            for (int i = 0; i < arch.hotspot_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, arch.hotspots[i].qualified_name, true);
                engine_tree_cell_int(&sb, arch.hotspots[i].fan_in, false);
                engine_tree_row_end(&sb);
            }
        }
        if (arch.boundary_count > 0) {
            static const char *const bcols[] = {"from", "to", "calls"};
            engine_tree_table_header(&sb, "boundaries", arch.boundary_count, bcols, 3);
            for (int i = 0; i < arch.boundary_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, arch.boundaries[i].from, true);
                engine_tree_cell_str(&sb, arch.boundaries[i].to, false);
                engine_tree_cell_int(&sb, arch.boundaries[i].call_count, false);
                engine_tree_row_end(&sb);
            }
        }
        if (arch.service_count > 0) {
            static const char *const scols[] = {"from", "to", "type", "count"};
            engine_tree_table_header(&sb, "services", arch.service_count, scols, 4);
            for (int i = 0; i < arch.service_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, arch.services[i].from, true);
                engine_tree_cell_str(&sb, arch.services[i].to, false);
                engine_tree_cell_str(&sb, arch.services[i].type, false);
                engine_tree_cell_int(&sb, arch.services[i].count, false);
                engine_tree_row_end(&sb);
            }
        }
        if (arch.layer_count > 0) {
            static const char *const ycols[] = {"name", "layer", "reason"};
            engine_tree_table_header(&sb, "layers", arch.layer_count, ycols, 3);
            for (int i = 0; i < arch.layer_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, arch.layers[i].name, true);
                engine_tree_cell_str(&sb, arch.layers[i].layer, false);
                engine_tree_cell_str(&sb, arch.layers[i].reason, false);
                engine_tree_row_end(&sb);
            }
        }
        if (arch.cluster_count > 0) {
            ...
            engine_tree_table_header(&sb, "clusters", arch.cluster_count, ccols, 7);
            for (int i = 0; i < arch.cluster_count; i++) {
                const engine_cluster_info_t *c = &arch.clusters[i];
                char joined[ENGINE_SZ_1K];
                engine_tree_row_begin(&sb);
                engine_tree_cell_int(&sb, c->id, true);
                engine_tree_cell_str(&sb, c->label, false);
                engine_tree_cell_int(&sb, c->members, false);
                engine_tree_cell_real(&sb, c->cohesion, false);
                arch_join_list(joined, sizeof(joined), c->top_nodes, c->top_node_count);
                engine_tree_cell_str(&sb, joined, false);
        ...
        if (arch.file_tree_count > 0) {
            ...
            engine_tree_table_header(&sb, "file_tree", arch.file_tree_count, fcols, 3);
            for (int i = 0; i < arch.file_tree_count; i++) {
                engine_tree_row_begin(&sb);
                engine_tree_cell_str(&sb, arch.file_tree[i].path, true);
                engine_tree_cell_str(&sb, arch.file_tree[i].type, false);
                engine_tree_cell_int(&sb, arch.file_tree[i].children, false);
                engine_tree_row_end(&sb);
            }
        }
        /* Cross-repo edge summary (mirrors append_cross_repo_summary). */
        {
            ...
            if (cross_total > 0) {
                engine_tree_scalar_int(&sb, "cross_repo_links_total", cross_total);
            }
        }
        ...
        if (aspect_explicitly_named(aspects_arr, "cycles")) {
            ...
            if (arch_compute_cycles(store, project, &members, &sizes, &ncyc, &total_cyc, &scanned,
                                    &etrunc) == ENGINE_STORE_OK) {
                engine_tree_scalar_int(&sb, "call_edges_scanned", scanned);
                engine_tree_scalar_int(&sb, "cycles_total", total_cyc);
                if (etrunc) {
                    engine_tree_scalar_bool(&sb, "cycles_partial", true);
                    engine_tree_scalar_str(&sb, "cycles_hint",
                                        "call graph exceeded the scan budget; cycle list may be "
                                        "incomplete");
                }
                if (total_cyc > ncyc) {
                    char omit[ENGINE_SZ_128];
                    snprintf(omit, sizeof(omit), "cycles_omitted: %d  (showing the first %d)\n",
                             total_cyc - ncyc, ncyc);
                    engine_sb_append(&sb, omit);
                }
                char hdr[ENGINE_SZ_128];
                snprintf(hdr, sizeof(hdr),
                         "cycles: %d  (rows: size members; circular CALLS dependencies)\n", ncyc);
                engine_sb_append(&sb, hdr);
                for (int c = 0; c < ncyc; c++) {
                    char row[ENGINE_SZ_2K];
        ...
        engine_store_architecture_free(&arch);
        engine_store_schema_free(&schema);
        if (aspects_doc) {
            yyjson_doc_free(aspects_doc);
        }
    ...
    if (path_scoped) {
        ...
        int root_nodes = engine_store_count_nodes(store, project);
        int root_edges = engine_store_count_edges(store, project);
...
static const char *bfs_edge_args_for_hop(engine_traverse_result_t *tr, int64_t hop_node_id,
                                         size_t *out_len) {
...
const char *engine_mcp_edge_strategy_class(const char *strategy) {
...
static bool bfs_edge_evidence_for_hop(engine_traverse_result_t *tr, int64_t hop_node_id,
    ...
    for (int e = 0; e < tr->edge_count; e++) {
        ...
        memcpy(raw, open, len);
        raw[len] = '\0';
        const char *cls = engine_mcp_edge_strategy_class(raw);
        if (!cls) {
            continue;
        }
        *class_out = cls;
...
static void bfs_to_toon_table(engine_sb_t *sb, const char *key, engine_traverse_result_t *tr,
    ...
    engine_tree_table_header(sb, key, visible, cols, ncols);
    for (int i = 0; i < tr->visited_count; i++) {
        ...
        engine_tree_row_begin(sb);
        engine_tree_cell_str(sb, tr->visited[i].node.qualified_name, true);
        engine_tree_cell_int(sb, tr->visited[i].hop, false);
        if (risk_labels) {
            engine_tree_cell_str(sb, engine_risk_label(engine_hop_to_risk(tr->visited[i].hop)), false);
        }
        if (include_tests) {
            engine_tree_cell_bool(sb, test, false);
        }
        if (data_flow) {
            size_t alen = 0;
            const char *ea = bfs_edge_args_for_hop(tr, tr->visited[i].node.id, &alen);
            if (ea && alen > 0 && alen < ENGINE_SZ_1K) {
                char abuf[ENGINE_SZ_1K];
                memcpy(abuf, ea, alen);
                abuf[alen] = '\0';
                engine_tree_cell_str(sb, abuf, false);
            } else {
                engine_tree_cell_str(sb, "", false);
            }
        }
...
static char *snippet_suggestions(const char *input, engine_node_t *nodes, int count);
...
static long node_resolution_score(const engine_node_t *n) {
...
static bool node_is_real_callable_def(const engine_node_t *n) {
    if (!n->label) {
        return false;
    }
    if (strcmp(n->label, "Function") != 0 && strcmp(n->label, "Method") != 0) {
...
static int pick_resolved_node(const engine_node_t *nodes, int count, bool *ambiguous) {
    *ambiguous = false;
    if (count <= 1) {
        return 0;
    }
...
static int node_hop_cmp_hop_id(const void *pa, const void *pb) {
    const engine_node_hop_t *a = (const engine_node_hop_t *)pa;
    const engine_node_hop_t *b = (const engine_node_hop_t *)pb;
    if (a->hop != b->hop) {
 ...
 * transfers into *out, freed by engine_store_traverse_free. */
static void bfs_union_same_name(engine_store_t *store, const engine_node_t *nodes, int node_count,
                                const char *direction, const char **edge_types, int edge_type_count,
                                int depth, int limit, engine_traverse_result_t *out) {
    memset(out, 0, sizeof(*out));
    int vcap = 0, ecap = 0;
    for (int k = 0; k < node_count; k++) {
        engine_traverse_result_t tr = {0};
        engine_store_bfs(store, nodes[k].id, direction, edge_types, edge_type_count, depth, limit,
                      &tr);
        for (int i = 0; i < tr.visited_count; i++) {
            bool dup = false;
            ...
            if (out->visited_count >= vcap) {
                vcap = vcap ? vcap * 2 : 8;
                out->visited = safe_realloc(out->visited, vcap * sizeof(engine_node_hop_t));
            }
        ...
        for (int i = 0; i < tr.edge_count; i++) {
            ...
            if (out->edge_count >= ecap) {
                ecap = ecap ? ecap * 2 : 8;
                out->edges = safe_realloc(out->edges, ecap * sizeof(engine_edge_info_t));
            }
            out->edges[out->edge_count++] = tr.edges[i];
            memset(&tr.edges[i], 0, sizeof(tr.edges[i])); /* ownership moved */
        }
        engine_store_traverse_free(&tr); /* frees only the un-moved (root + dup) fields */
    }
    ...
    if (out->visited_count > 1) {
        qsort(out->visited, (size_t)out->visited_count, sizeof(engine_node_hop_t),
              node_hop_cmp_hop_id);
    }
}
...
static int trace_watermark_index(const engine_traverse_result_t *tr, int hop, int64_t node_id) {
    for (int i = 0; i < tr->visited_count; i++) {
...
static yyjson_mut_val *bfs_to_tree_json(yyjson_mut_doc *doc, engine_traverse_result_t *tr,
                                        bool risk_labels, bool include_tests, bool data_flow) {
    ...
    for (int i = 0; i < tr->visited_count; i++) {
        ...
        if (risk_labels) {
            yyjson_mut_arr_add_str(doc, row, engine_risk_label(engine_hop_to_risk(tr->visited[i].hop)));
        }
        if (data_flow) {
            size_t alen = 0;
            const char *ea = bfs_edge_args_for_hop(tr, tr->visited[i].node.id, &alen);
...
static void bfs_to_tree_table(engine_sb_t *sb, const char *key, engine_traverse_result_t *tr,
    ...
    if (tr->visited_count > 1) {
        qsort(tr->visited, (size_t)tr->visited_count, sizeof(engine_node_hop_t), tree_hop_cmp_qn);
    }
    char cur_group[ENGINE_SZ_1K] = "";
    for (int i = 0; i < tr->visited_count; i++) {
        if (!include_tests && is_test_file(tr->visited[i].node.file_path)) {
        ...
        if (strncmp(cur_group, qn, plen) != 0 || cur_group[plen] != '\0') {
            snprintf(cur_group, sizeof(cur_group), "%.*s", (int)plen, qn);
            engine_sb_append(sb, cur_group);
            engine_sb_append(sb, ":\n");
        }
        char row[ENGINE_SZ_512];
...
/* Clamp a client-supplied traversal depth to the MCP ceiling (engine_mcp_max_depth),
 * WARN-logging when it does so — never a silent truncation (#887). An unclamped
 * `depth` would drive the shared engine_store_bfs to an arbitrary hop count. */
static int clamp_mcp_depth(int depth, const char *tool) {
    int cap = engine_mcp_max_depth();
    if (depth > cap) {
        char req_buf[16];
        char cap_buf[16];
        snprintf(req_buf, sizeof(req_buf), "%d", depth);
        snprintf(cap_buf, sizeof(cap_buf), "%d", cap);
        engine_log_warn("mcp.depth_capped", "tool", tool, "requested", req_buf, "cap", cap_buf);
        return cap;
    }
    return depth;
}

static char *handle_trace_call_path(engine_mcp_server_t *srv, const char *args) {
    char *func_name = engine_mcp_get_string_arg(args, "function_name");
    char *project = get_project_arg(args);
    engine_store_t *store = resolve_store(srv, project);
    char *direction = engine_mcp_get_string_arg(args, "direction");
    char *mode = engine_mcp_get_string_arg(args, "mode");
    char *param_name = engine_mcp_get_string_arg(args, "parameter_name");
    int depth = engine_mcp_get_int_arg(args, "depth", MCP_DEFAULT_DEPTH);
     ...
     * depth clamp, #887). */
    int trace_limit = engine_mcp_get_int_arg(args, "limit", MCP_BFS_LIMIT);
    if (trace_limit < 1) {
        trace_limit = 1;
    }
    ...
    bool risk_labels = engine_mcp_get_bool_arg(args, "risk_labels");
    bool include_tests = engine_mcp_get_bool_arg(args, "include_tests");
    ...
    bool include_evidence = engine_mcp_get_bool_arg(args, "include_evidence");
    ...
    if (!func_name) {
        ...
        free(mode);
        free(param_name);
        return engine_mcp_text_result("function_name is required", true);
    }
    if (!store) {
        char *_err = build_project_list_error("project not found or not indexed");
        char *_res = engine_mcp_text_result(_err, true);
    ...
    char generation[96];
    (void)engine_store_generation(store, generation, sizeof(generation));
    char *cursor_arg = engine_mcp_get_string_arg(args, "cursor");
    ...
    if (cursor_arg && cursor_arg[0]) {
        uint64_t qh =
            trace_params_hash(project, func_name, direction ? direction : "both", mode,
                              engine_mcp_get_int_arg(args, "depth", MCP_DEFAULT_DEPTH), include_tests,
                              engine_mcp_get_int_arg(args, "limit", MCP_BFS_LIMIT));
        ...
        if (cerr) {
            ...
            free(mode);
            free(param_name);
            return engine_mcp_text_result(cerr, true);
        }
        have_cursor = true;
    }
    free(cursor_arg);
    ...
    if (strcmp(direction, "inbound") != 0 && strcmp(direction, "outbound") != 0 &&
        ...
        free(mode);
        free(param_name);
        return engine_mcp_text_result(errbuf, true);
    }
     ...
     * fields cleanly and engine_store_free_nodes will free them. */
    engine_node_t *nodes = NULL;
    int node_count = 0;
    engine_store_find_nodes_by_name(store, project, func_name, &nodes, &node_count);

    if (node_count == 0) {
        engine_node_t qn_node = {0};
        if (engine_store_find_node_by_qn(store, project, func_name, &qn_node) == ENGINE_STORE_OK) {
            nodes = malloc(sizeof(engine_node_t));
            if (nodes) {
                nodes[0] = qn_node;
                node_count = 1;
            } else {
    ...
    if (node_count == 0) {
        ...
        free(mode);
        free(param_name);
        engine_store_free_nodes(nodes, 0);
        return engine_mcp_text_result(hint, true);
    }
    ...
    if (trace_ambiguous) {
        ...
        free(mode);
        free(param_name);
        engine_store_free_nodes(nodes, node_count);
        return result;
    }

    /* Response encoding: tree tables by default; format:"json" emits the
     * same grouped model as structured JSON. */
    char *trace_format = engine_mcp_get_string_arg(args, "format");
    ...
    if (!trace_legacy_json) {
        engine_sb_t sb;
        engine_sb_init(&sb);
        engine_tree_scalar_str(&sb, "function", func_name);
        engine_tree_scalar_str(&sb, "direction", direction);
        if (mode) {
            engine_tree_scalar_str(&sb, "mode", mode);
        }
        ...
        if (do_outbound) {
            engine_tree_scalar_int(&sb, "callees_total", out_total);
            if (flat_trace) {
        ...
        if (do_inbound) {
            engine_tree_scalar_int(&sb, "callers_total", in_total);
            if (flat_trace) {
        ...
        if (more_rows) {
            engine_tree_scalar_bool(&sb, "truncated", true);
            if (next_tok[0]) {
                engine_tree_scalar_str(&sb, "next", next_tok);
                engine_tree_scalar_str(&sb, "hint",
...
static char *read_file_lines(const char *path, int start, int end) {
    FILE *fp = engine_fopen(path, "r");
    if (!fp) {
        return NULL;
    }

    size_t cap = ENGINE_SZ_4K;
    char *buf = malloc(cap);
    size_t len = 0;
    buf[0] = '\0';

    char line[ENGINE_SZ_2K];
    int lineno = 0;
...
static char *project_root_from_store(engine_store_t *store, const char *project) {
    if (!store || !project) {
        return NULL;
    }
    engine_project_t proj = {0};
    if (engine_store_get_project(store, project, &proj) != ENGINE_STORE_OK) {
        return NULL;
    }
...
static char *get_project_root(engine_mcp_server_t *srv, const char *project) {
...
static char *handle_cross_repo_mode(engine_mcp_server_t *srv, const char *repo_path,
                                    const char *name_override, const char *args) {
    if (name_override && name_override[0] && !engine_validate_project_name(name_override)) {
        return engine_mcp_text_result("invalid project name", true);
    }
    char *project = name_override && name_override[0] ? heap_strdup(name_override)
                                                      : engine_project_name_from_path(repo_path);
    if (!project) {
        return engine_mcp_text_result("cannot derive project name", true);
    }
    ...
    if (target_count > MCP_MAX_CROSS_REPO_TARGETS) {
        yyjson_doc_free(jdoc);
        free(project);
        return engine_mcp_text_result("too many cross-repo target projects", true);
    }
    ...
    if (!targets || !lease_keys) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return engine_mcp_text_result("failed to allocate cross-repo project leases", true);
    }
    size_t idx;
    size_t max;
    yyjson_val *val;
    ...
    yyjson_arr_foreach(tp_arr, idx, max, val) {
        ...
        if (!target || !target[0] || strlen(target) >= ENGINE_SZ_256 ||
            (strcmp(target, "*") != 0 && !engine_validate_project_name(target))) {
            invalid_target = true;
            break;
        }
        targets[ti++] = target;
    ...
    if (invalid_target || ti != tp_count) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return engine_mcp_text_result("target_projects must contain valid project names or '*'", true);
    }
    if (all_projects && tp_count != 1) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return engine_mcp_text_result("target_projects wildcard '*' must be the only entry", true);
    }
    if (!all_projects) {
    ...
    if (held_count != lease_count || cancelled) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return engine_mcp_text_result("cross-repo operation cancelled or blocked by active indexing",
                                   true);
    }

    engine_cross_repo_result_t result = engine_cross_repo_match_cancellable(
    ...
    yyjson_mut_doc_free(doc);
    free(project);
    char *out = engine_mcp_text_result(json, result.cancelled);
    free(json);
    return out;
}
...
static void try_artifact_bootstrap(const char *project_name, const char *repo_path) {
    char db_buf[ENGINE_SZ_1K];
    project_db_path(project_name, db_buf, sizeof(db_buf));
    if (engine_file_size(db_buf) < 0 && engine_artifact_exists(repo_path)) {
        engine_log_info("index.artifact_bootstrap", "project", project_name);
        engine_artifact_import(repo_path, db_buf);
    }
}
...
static void add_not_indexed_files_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                          engine_pipeline_t *p) {
    engine_ignored_file_t *ignored = NULL;
    int stored = 0;
    int total = 0;
    engine_pipeline_get_ignored(p, &ignored, &stored, &total);
    ...
    yyjson_mut_obj_add_str(doc, ni, "note",
                           "Excluded by design (gitignore/.engineignore/skip-lists); examples only — "
...
static bool is_parse_partial(const engine_file_error_t *e) {
...
static void add_skipped_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                const engine_file_error_t *errs, int count, const char *logfile) {
    int skips = 0;
    for (int i = 0; i < count; i++) {
...
static void add_parse_partial_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                      const engine_file_error_t *errs, int count) {
    int partials = 0;
    for (int i = 0; i < count; i++) {
        if (is_parse_partial(&errs[i])) {
            partials++;
 ...
 *   $ENGINE_INDEX_LOG (override) else <cache_dir>/logs/<project>-<epoch>.log
 * Returns true and fills out_path on success. */
static bool write_skip_logfile(const char *project, const engine_file_error_t *errs, int count,
                               char *out_path, size_t out_sz) {
    if (!errs || count <= 0) {
        return false;
    }
    char path[ENGINE_SZ_1K];
    const char *override = getenv("ENGINE_INDEX_LOG");
    if (override && override[0]) {
        snprintf(path, sizeof(path), "%s", override);
    } else {
        const char *cdir = engine_resolve_cache_dir();
        if (!cdir) {
            return false;
        }
        char logdir[ENGINE_SZ_1K];
        snprintf(logdir, sizeof(logdir), "%s/logs", cdir);
        engine_mkdir_p(logdir, 0755);
        snprintf(path, sizeof(path), "%s/%s-%lld.log", logdir, project ? project : "index",
                 (long long)time(NULL));
    }
    FILE *f = engine_fopen(path, "wb");
    if (!f) {
        engine_log_warn("index.logfile_open_fail", "path", path);
        return false;
    }
    int partials = 0;
    for (int i = 0; i < count; i++) {
    ...
    (void)fprintf(f, "# graph-engine index coverage report\n");
...
static bool build_index_success_response(engine_mcp_server_t *srv, yyjson_mut_doc *doc,
                                         yyjson_mut_val *root, const char *project_name,
                                         const char *repo_path, bool persistence, engine_pipeline_t *p,
                                         char **excluded_dirs, int excluded_count,
                                         const engine_file_error_t *file_errors, int file_error_count,
                                         const char *logfile) {
    ...
    int exp_nodes = -1;
    int exp_edges = -1;
    engine_pipeline_get_committed_counts(p, &exp_nodes, &exp_edges);

    const double ratio = engine_dump_verify_min_ratio();
    const int min_floor = ENGINE_DUMP_VERIFY_MIN_FLOOR;

    engine_store_t *store = resolve_store(srv, project_name);
    int nodes = 0;
    int edges = 0;
    bool degraded = false;
    ...
    if (!store) {
        ...
        nodes = engine_store_count_nodes(store, project_name);
        edges = engine_store_count_edges(store, project_name);
        if (nodes < 0) {
            degraded = true;
            nodes = 0;
            edges = edges >= 0 ? edges : 0;
        } else if (engine_dump_verify_is_degraded(exp_nodes, nodes, ratio, min_floor)) {
            (void)engine_store_checkpoint(store);
            int nodes2 = engine_store_count_nodes(store, project_name);
            int edges2 = engine_store_count_edges(store, project_name);
            if (nodes2 >= 0) {
                nodes = nodes2;
            }
            if (edges2 >= 0) {
            ...
            degraded = engine_dump_verify_is_degraded(exp_nodes, nodes, ratio, min_floor);
        }
    }
    ...
    if (degraded) {
        if (!store) {
            ...
            engine_log_warn("dump.verify", "reason", "store_missing", "expected_nodes",
            ...
            engine_log_warn("dump.verify", "expected_nodes", exp_buf, "persisted_nodes", got_buf);
        }
    }
    ...
    bool has_artifact = engine_artifact_exists(repo_path);
    ...
    if (persistence && has_artifact) {
        yyjson_mut_obj_add_str(doc, root, "artifact_hint",
                               "Persistent artifact written to .graph-engine/graph.db.zst. "
...
static char *build_worker_failure_response(const char *args, engine_proc_outcome_t outcome) {
    char *repo_path = engine_mcp_get_string_arg(args, "repo_path");
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
    yyjson_mut_obj_add_str(doc, root, "status", "error");
    yyjson_mut_obj_add_str(doc, root, "outcome", engine_proc_outcome_str(outcome));
    const char *hint = NULL;
    if (outcome == ENGINE_PROC_SPAWN_FAILED) {
        hint = "Indexing worker could not be started. Supervision is mandatory in a engine "
               "host, so no in-process fallback was attempted.";
    } else if (outcome == ENGINE_PROC_HANG) {
        hint = "Indexing worker timed out (a file made no progress). The worker was "
               "terminated and the server survived. Re-run to retry.";
    } else if (outcome == ENGINE_PROC_CRASH) {
    ...
    } else if (outcome == ENGINE_PROC_CLEAN) {
    ...
    yyjson_mut_doc_free(doc);
    free(repo_path);
    char *result = engine_mcp_text_result(json, true);
    free(json);
    return result;
}

static char *build_worker_unsafe_terminal_response(const char *args, engine_proc_outcome_t outcome,
    ...
    yyjson_mut_doc_free(doc);
    free(repo_path);
    char *response = engine_mcp_text_result(json, true);
    free(json);
    return response;
}
 ...
 * to invalidate. */
static void invalidate_cached_store(engine_mcp_server_t *srv) {
    if (!srv) {
        return;
    }
    if (srv->owns_store && srv->store) {
 ...
 * per line — see engine_index_mark_start/done) into the crash/hang SUSPECT set:
    ...
    *out_n = 0;
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (line[0] == 'S') {
            ...
            open_paths[open_n++] = engine_strdup(rel);
        } else {
...
engine_mcp_supervised_result_disposition_t engine_mcp_supervised_result_disposition(
    int spawn_result, const engine_index_worker_result_t *worker_result) {
    if (spawn_result != 0 || !worker_result || worker_result->outcome == ENGINE_PROC_SPAWN_FAILED) {
        return ENGINE_MCP_SUPERVISED_RESULT_FALLBACK;
    }
    if (worker_result->cancellation_requested || !worker_result->tree_quiesced ||
        worker_result->supervision_failed) {
        return ENGINE_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL;
    }
    if (worker_result->outcome == ENGINE_PROC_CLEAN) {
        return worker_result->response ? ENGINE_MCP_SUPERVISED_RESULT_SUCCESS
                                       : ENGINE_MCP_SUPERVISED_RESULT_FALLBACK;
    }
    return ENGINE_MCP_SUPERVISED_RESULT_CONTAINED_FAILURE;
}
 ...
 * A physical engine host never falls back to its in-process pipeline: an initial
 * start/protocol failure is returned as an explicit error response. */
static char *index_run_supervised(engine_mcp_server_t *srv, const char *args) {
    invalidate_cached_store(srv);

    /* First attempt: normal parallel run. */
    engine_index_worker_result_t wr;
    int rc = engine_index_spawn_worker_with_log_cancel(
    ...
    engine_mcp_supervised_result_disposition_t disposition =
        engine_mcp_supervised_result_disposition(rc, &wr);

    if (disposition == ENGINE_MCP_SUPERVISED_RESULT_FALLBACK) {
        engine_proc_outcome_t outcome = wr.outcome;
        engine_index_worker_result_free(&wr);
        invalidate_cached_store(srv);
        return build_worker_failure_response(args, outcome);
    }
    if (disposition == ENGINE_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL) {
        char *failure =
    ...
    if (disposition == ENGINE_MCP_SUPERVISED_RESULT_SUCCESS) {
     ...
     * time. */
    engine_proc_outcome_t last_outcome = wr.outcome;
    engine_index_worker_result_free(&wr);

    char marker_path[ENGINE_SZ_1K];
    char quarantine_path[ENGINE_SZ_1K];
    ...
    FILE *qinit = engine_fopen(quarantine_path, "wb");
    if (qinit) {
        (void)fclose(qinit);
    }
    ...
    for (int i = 0; i < cap; i++) {
        engine_index_worker_result_t wr2;
        int rc2 = engine_index_spawn_worker_with_log_cancel(
        ...
        engine_mcp_supervised_result_disposition_t recovery_disposition =
            engine_mcp_supervised_result_disposition(rc2, &wr2);
        if (recovery_disposition == ENGINE_MCP_SUPERVISED_RESULT_FALLBACK) {
            last_outcome = wr2.outcome;
            engine_index_worker_result_free(&wr2);
            break; /* spawn failed mid-recovery — give up */
        }
        if (recovery_disposition == ENGINE_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL) {
            last_outcome = wr2.outcome;
        ...
        if (recovery_disposition == ENGINE_MCP_SUPERVISED_RESULT_SUCCESS) {
            resp = wr2.response; /* transfer ownership to caller */
            wr2.response = NULL;
            engine_index_worker_result_free(&wr2);
            break; /* good files indexed; quarantined files reported as crash/hang */
        }
        if (wr2.outcome == ENGINE_PROC_CRASH || wr2.outcome == ENGINE_PROC_HANG) {
            last_outcome = wr2.outcome;
            engine_index_worker_result_free(&wr2);
            ...
            const char *phase = (last_outcome == ENGINE_PROC_HANG) ? "hang" : "crash";
            int sus_n = 0;
            ...
            if (!suspects || sus_n == 0) {
                ...
                engine_log_warn("index.supervisor.unattributable", "action", "give_up");
                break;
            }
            if (prev_suspects) {
                ...
                if (!supervisor_append_quarantine(quarantine_path, pick, phase)) {
                    engine_log_warn("index.supervisor.quarantine_write_fail", "path", pick);
                    supervisor_free_suspects(suspects, sus_n);
                    break;
                }
                quarantined++;
                ...
                engine_log_warn("index.file_quarantined", "path", pick, "outcome", phase, "attempt",
                             attempt_buf);
            }
    ...
    if (!resp && !unsafe_terminal && quarantined > 0) {
        engine_index_worker_result_t wrp;
        int rcp = engine_index_spawn_worker_with_log_cancel(
            args, /*single_thread=*/false, NULL, quarantine_path,
            srv ? srv->index_log_callback : NULL, srv ? srv->index_log_context : NULL,
            srv ? &srv->pipeline_cancel_requested : NULL, &wrp);
        engine_mcp_supervised_result_disposition_t partial_disposition =
            engine_mcp_supervised_result_disposition(rcp, &wrp);
        if (partial_disposition == ENGINE_MCP_SUPERVISED_RESULT_SUCCESS) {
            resp = wrp.response; /* transfer ownership to caller */
            wrp.response = NULL;
            char qn[MCP_FIELD_SIZE];
            snprintf(qn, sizeof(qn), "%d", quarantined);
            engine_log_error("index.supervisor.partial", "quarantined", qn, "outcome",
                          engine_proc_outcome_str(last_outcome));
        } else if (partial_disposition == ENGINE_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL) {
            last_outcome = wrp.outcome;
            unsafe_terminal = true;
            terminal_cancelled = wrp.cancellation_requested;
        }
        engine_index_worker_result_free(&wrp);
    }
...
static char *index_run_supervised_path(engine_mcp_server_t *srv, const char *root_path) {
    if (!root_path || !root_path[0]) {
        return NULL;
    }
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
...
char *engine_mcp_index_run_supervised_path(const char *root_path) {
...
static bool resolve_session_repo_path(engine_mcp_server_t *srv, char **repo_path) {
    ...
    (void)snprintf(joined, joined_size, "%s%s%s", srv->session_root, needs_separator ? "/" : "",
                   *repo_path);
    engine_normalize_path_sep(joined);
    free(*repo_path);
    *repo_path = joined;
    return true;
}
...
static char *resolved_repo_path_from_project_arg(const char *args) {
    ...
    char db_path[ENGINE_SZ_1K];
    project_db_path(project, db_path, sizeof(db_path));
    engine_store_t *store = db_path[0] ? engine_store_open_path_query(db_path) : NULL;
    char *root_path = NULL;
    if (store) {
        engine_project_t proj = {0};
        if (engine_store_get_project(store, project, &proj) == ENGINE_STORE_OK) {
...
static char *handle_index_repository(engine_mcp_server_t *srv, const char *args) {
    char *repo_path = engine_mcp_get_string_arg(args, "repo_path");
    char *mode_str = engine_mcp_get_string_arg(args, "mode");
    char *name_override = engine_mcp_get_string_arg(args, "name");
    engine_normalize_path_sep(repo_path);
    ...
    if (!repo_path) {
        free(mode_str);
        free(name_override);
        return engine_mcp_text_result("repo_path is required", true);
    }
    ...
    if (!resolve_session_repo_path(srv, &repo_path)) {
        ...
        free(name_override);
        free(repo_path);
        return engine_mcp_text_result("failed to resolve repo_path", true);
    }
     ...
     * back to the process-wide ENGINE_ALLOWED_ROOT. The decision itself lives in one
    ...
    const char *allowed_root =
        srv->allowed_root_policy_set ? srv->allowed_root : getenv("ENGINE_ALLOWED_ROOT");
    ...
    char boundary_err[ENGINE_SZ_1K];
    if (repo_path && repo_path[0] &&
        !engine_workspace_root_allowed(repo_path, engine_workspace_home_dir(), engine_workspace_cache_dir(),
                                    allowed_root, boundary_err, sizeof(boundary_err))) {
        free(mode_str);
        free(name_override);
        free(repo_path);
        return engine_mcp_text_result(boundary_err, true);
    }
    ...
    char *mutation_project =
        engine_project_name_from_path(name_override && name_override[0] ? name_override : repo_path);
    if (!mutation_project) {
        free(repo_path);
        free(mode_str);
        free(name_override);
        return engine_mcp_text_result("could not resolve index project name", true);
    }
    ...
    if (engine_index_supervisor_should_wrap()) {
        char *worker_args = index_args_with_repo_path(args, repo_path);
        if (!worker_args) {
            ...
            free(mode_str);
            free(name_override);
            return engine_mcp_text_result("failed to prepare supervised index request", true);
        }
    ...
    if (!mcp_project_mutation_begin(srv, mutation_project)) {
        ...
        free(mode_str);
        free(name_override);
        return engine_mcp_text_result("index operation blocked by another mutation for this project",
                                   true);
    }
    if (mcp_request_cancelled(srv)) {
        ...
        free(mode_str);
        free(name_override);
        return engine_mcp_text_result("index operation cancelled for this request", true);
    }

    engine_index_mode_t mode = ENGINE_MODE_FULL;
    if (mode_str && strcmp(mode_str, "fast") == 0) {
        mode = ENGINE_MODE_FAST;
    } else if (mode_str && strcmp(mode_str, "moderate") == 0) {
        mode = ENGINE_MODE_MODERATE;
    }
    free(mode_str);

    bool persistence = engine_mcp_get_bool_arg(args, "persistence");

    engine_pipeline_t *p = engine_pipeline_new(repo_path, NULL, mode);
    if (!p) {
        mcp_project_mutation_end(srv, mutation_project);
        free(mutation_project);
        free(name_override);
        free(repo_path);
        return engine_mcp_text_result("failed to create pipeline", true);
    }
    if (name_override && name_override[0] && !engine_pipeline_set_project_name(p, name_override)) {
        engine_pipeline_free(p);
        mcp_project_mutation_end(srv, mutation_project);
        free(mutation_project);
        free(name_override);
        free(repo_path);
    ...
    free(name_override);
    engine_pipeline_set_persistence(p, persistence);

    char *project_name = heap_strdup(engine_pipeline_project_name(p));
    ...
    engine_pipeline_lock();
    engine_pipeline_bind_cancel_flag(p, &srv->pipeline_cancel_requested);
    srv->active_pipeline = p;
    int rc = engine_pipeline_run(p);
     ...
     * JSON doc, so they need only outlive that call, not engine_pipeline_free. */
    char **excluded_dirs = NULL;
    int excluded_count = 0;
    engine_pipeline_get_excluded(p, &excluded_dirs, &excluded_count);
    ...
    engine_file_error_t *file_errors = NULL;
    int file_error_count = 0;
    engine_pipeline_get_file_errors(p, &file_errors, &file_error_count);

    engine_mem_collect(); /* return mimalloc pages to OS after large indexing */
    ...
    if (rc == 0) {
        ...
        char logfile_path[ENGINE_SZ_1K];
        logfile_path[0] = '\0';
    ...
    if (engine_index_worker_active()) {
        engine_log_info("index.worker.fast_exit", "skip", "pipeline_free");
    } else {
        engine_pipeline_free(p);
    }
    free(project_name);
    ...
    char *result = engine_mcp_text_result(json, rc != 0);
    free(json);
    return result;
}
...
static void copy_node(const engine_node_t *src, engine_node_t *dst) {
    dst->id = src->id;
 ...
 * (>= ENGINE_SZ_4K bytes); returns true on success. Isolating the per-OS resolver
 * keeps engine_path_within_root's control flow unconditional: the previous `#ifdef`
...
static bool resolve_canonical_path(const char *path, char *out, size_t out_sz) {
    /* engine_canonical_path: realpath on POSIX; an opened handle plus
    ...
    if (!engine_canonical_path(path, out, out_sz)) {
        return false;
    }
#ifdef _WIN32
    engine_normalize_path_sep(out);
#endif
    return true;
}
...
#ifdef _WIN32
    wchar_t *wide_root = engine_utf8_to_wide(root_path);
    wchar_t *wide_candidate = engine_utf8_to_wide(candidate_path);
...
bool engine_path_within_root(const char *root_path, const char *abs_path) {
    if (!root_path || !abs_path) {
        return false;
    }
    char real_root[ENGINE_SZ_4K];
    char real_file[ENGINE_SZ_4K];
...
static void add_snippet_coverage_note(yyjson_mut_doc *doc, yyjson_mut_val *root_obj,
                                      engine_store_t *store, const engine_node_t *node) {
    if (!node->file_path || !node->file_path[0] || !node->project) {
        return;
    }
    engine_coverage_row_t *rows = NULL;
    int count = 0;
    if (engine_store_coverage_get_path(store, node->project, node->file_path, &rows, &count) !=
        ENGINE_STORE_OK) {
        return;
    }
    for (int i = 0; i < count; i++) {
...
static char *build_snippet_response(engine_mcp_server_t *srv, engine_node_t *node,
                                    const char *match_method, bool include_neighbors,
                                    engine_node_t *alternatives, int alt_count) {
    ...
    engine_store_t *store = srv->store;
    int in_deg = 0;
    int out_deg = 0;
    engine_store_node_degree(store, node->id, &in_deg, &out_deg);
    ...
    if (include_neighbors) {
        engine_store_node_neighbor_names(store, node->id, MCP_DEFAULT_LIMIT, &nb_callers,
...
static char *handle_get_code_snippet(engine_mcp_server_t *srv, const char *args) {
    char *qn = engine_mcp_get_string_arg(args, "qualified_name");
    char *project = get_project_arg(args);
    bool include_neighbors = engine_mcp_get_bool_arg(args, "include_neighbors");

    if (!qn) {
        free(project);
        return engine_mcp_text_result("qualified_name is required", true);
    }
    ...
    engine_node_t node = {0};
    int rc = engine_store_find_node_by_qn(store, effective_project, qn, &node);
    if (rc == ENGINE_STORE_OK) {
    ...
    engine_node_t *suffix_nodes = NULL;
    int suffix_count = 0;
    engine_store_find_nodes_by_qn_suffix(store, effective_project, qn, &suffix_nodes, &suffix_count);

    if (suffix_count == SKIP_ONE) {
        copy_node(&suffix_nodes[0], &node);
        engine_store_free_nodes(suffix_nodes, suffix_count);
...
typedef struct {
    char file[ENGINE_SZ_512];
    int line;
    char content[ENGINE_SZ_1K];
} grep_match_t;

/* Deduped result: one per containing graph node */
typedef struct {
    int64_t node_id; /* 0 = raw match (no containing node) */
    char node_name[ENGINE_SZ_256];
    char qualified_name[ENGINE_SZ_512];
    char label[ENGINE_SZ_64];
    char file[ENGINE_SZ_512];
    int start_line;
    int end_line;
    int in_degree;
...
static yyjson_mut_val *build_dedup_files_array(yyjson_mut_doc *doc, search_result_t *sr,
    ...
    for (int fi = 0; fi < output_count; fi++) {
        ...
        if (!dup && seen_count < ENGINE_SZ_512) {
            seen_files[seen_count++] = sr[fi].file;
            yyjson_mut_arr_add_str(doc, files_arr, sr[fi].file);
        }
    }
    for (int fi = 0; fi < raw_count && seen_count < ENGINE_SZ_512; fi++) {
        bool dup = false;
        for (int j = 0; j < seen_count; j++) {
...
static int aggregate_search_dirs(search_result_t *sr, int sr_count, char dir_names[][ENGINE_SZ_128],
                                 int *dir_counts, int max_dirs) {
    int dir_n = 0;
    for (int di = 0; di < sr_count; di++) {
        char top[ENGINE_SZ_128] = "";
        ...
        if (found >= 0) {
            ...
            snprintf(dir_names[dir_n], ENGINE_SZ_128, "%s", top);
...
static yyjson_mut_val *build_dir_distribution(yyjson_mut_doc *doc, search_result_t *sr,
    ...
    char dir_names[ENGINE_SZ_64][ENGINE_SZ_128];
    int dir_counts[ENGINE_SZ_64];
    int dir_n = aggregate_search_dirs(sr, sr_count, dir_names, dir_counts, ENGINE_SZ_64);
    for (int d = 0; d < dir_n; d++) {
...
static char *assemble_search_output_toon(search_result_t *sr, int sr_count, grep_match_t *raw,
    ...
    engine_tree_table_header(&sb, "results", output_count, cols, 7);
    for (int ri = 0; ri < output_count; ri++) {
        ...
        engine_tree_row_begin(&sb);
        engine_tree_cell_str(&sb, r->qualified_name, true);
        engine_tree_cell_str(&sb, r->label, false);
        engine_tree_cell_str(&sb, r->file, false);
        engine_tree_cell_str(&sb, lines, false);
        engine_tree_cell_str(&sb, matches, false);
        engine_tree_cell_int(&sb, r->in_degree, false);
        engine_tree_cell_int(&sb, r->out_degree, false);
        engine_tree_row_end(&sb);
    }
    ...
    if (raw_output > 0) {
        ...
        engine_tree_table_header(&sb, "raw", raw_output, rcols, 3);
        for (int ri = 0; ri < raw_output; ri++) {
            engine_tree_row_begin(&sb);
            engine_tree_cell_str(&sb, raw[ri].file, true);
            engine_tree_cell_int(&sb, raw[ri].line, false);
            engine_tree_cell_str(&sb, raw[ri].content, false);
            engine_tree_row_end(&sb);
        }
    }
    ...
    if (dir_n > 0) {
        ...
        engine_tree_table_header(&sb, "dirs", dir_n, dcols, 2);
        for (int d = 0; d < dir_n; d++) {
            engine_tree_row_begin(&sb);
            engine_tree_cell_str(&sb, dir_names[d], true);
            engine_tree_cell_int(&sb, dir_counts[d], false);
            engine_tree_row_end(&sb);
        }
    }

    engine_tree_scalar_int(&sb, "total_grep_matches", gm_count);
    engine_tree_scalar_int(&sb, "total_results", sr_count);
    engine_tree_scalar_int(&sb, "raw_match_count", raw_count);
    engine_tree_scalar_int(&sb, "elapsed_ms", (long long)elapsed_ms);
    if (warn_literal_pipe) {
        engine_tree_scalar_str(&sb, "warning",
    ...
    if (elapsed_ms >= SEARCH_SLOW_MS) {
        engine_tree_scalar_str(&sb, "warning_slow",
...
static char *assemble_search_output(search_result_t *sr, int sr_count, grep_match_t *raw,
    ...
    if (elapsed_ms >= SEARCH_SLOW_MS) {
        ...
        char ems[ENGINE_SZ_32];
        snprintf(ems, sizeof(ems), "%d", (int)elapsed_ms);
        engine_log_warn("search.slow", "elapsed_ms", ems); /* visibility in logs */
    }
...
static grep_match_t *collect_grep_matches(FILE *fp, const char *root_path, size_t root_len,
                                          bool has_path_filter, engine_regex_t *path_regex,
                                          int grep_limit, int *out_count) {
    int gm_cap = ENGINE_SZ_64;
    int gm_count = 0;
...
#ifdef _WIN32
        engine_normalize_path_sep(line);
#endif
        const char *path = line;
        const char *file = strip_root_prefix(path, root_path, root_len);

        if (has_path_filter && engine_regexec(path_regex, file, 0, NULL, 0) != ENGINE_REG_OK) {
            continue;
        }

        safe_grow(gm, gm_count, gm_cap, PAIR_LEN);
        snprintf(gm[gm_count].file, sizeof(gm[0].file), "%s", file);
        gm[gm_count].line = (int)strtol(sep1 + SKIP_ONE, NULL, ENGINE_DECIMAL_BASE);
...
static int find_tightest_node(engine_node_t *nodes, int count, int line) {
    int best = ENGINE_NOT_FOUND;
...
static void add_to_search_results(search_result_t **sr, int *sr_count, int *sr_cap, engine_node_t *n,
                                  int line) {
    for (int j = 0; j < *sr_count; j++) {
        if ((*sr)[j].node_id == n->id) {
            if ((*sr)[j].match_count < ENGINE_SZ_64) {
...
static void classify_grep_hit(grep_match_t *hit, engine_node_t *file_nodes, int file_node_count,
    ...
    if (best >= 0) {
        ...
        if (*raw_count >= *raw_cap) {
            *raw_cap = (*raw_cap == 0) ? ENGINE_SZ_32 : *raw_cap * PAIR_LEN;
...
/* Free a file_nodes array returned from engine_store_find_nodes_by_file. */
static void free_file_nodes(engine_node_t *nodes, int count) {
    for (int j = 0; j < count; j++) {
...
static void classify_all_grep_hits(grep_match_t *gm, int gm_count, engine_store_t *store,
    ...
    while (i < gm_count) {
        ...
        engine_node_t *file_nodes = NULL;
        int file_node_count = 0;
        if (store) {
            engine_store_find_nodes_by_file(store, project, cur_file, &file_nodes, &file_node_count);
        }
 ...
 *
 * `fl` is the caller's already-open binary stream on the descriptor engine_mkstemp
...
static bool write_scoped_filelist(engine_mcp_server_t *srv, const char *project, const char *root_path,
                                  FILE *fl, bool has_path_filter, engine_regex_t *path_regex,
                                  int *out_written) {
    *out_written = 0;
    engine_store_t *pre_store = resolve_store(srv, project);
    if (!pre_store) {
        return false;
    }
    char **indexed_files = NULL;
    int indexed_count = 0;
    if (engine_store_list_files(pre_store, project, &indexed_files, &indexed_count) != ENGINE_STORE_OK ||
        indexed_count == 0) {
        return false;
    }
    bool ok = false;
...
#ifdef _WIN32
                engine_normalize_path_sep(indexed_files[fi]);
#endif
                if (engine_regexec(path_regex, indexed_files[fi], 0, NULL, 0) != ENGINE_REG_OK) {
                    continue;
                }
            }
 ...
 * "<tmp>/engine_search_<pid>.pat" and its ".files" companion — opened with a plain
 ...
 * them. Now both live inside a directory created by engine_mkdtemp (0700 on POSIX,
 * an explicit owner-only DACL on Windows) under an unguessable XXXXXX suffix,
 * and each file is created by engine_mkstemp — O_CREAT|O_EXCL at mode 0600, so the
 * create fails rather than following anything already at the name. Every write
 * goes through the descriptor engine_mkstemp returned; neither path is ever
 * reopened by name.
 *
 * Sizing: engine_mkdtemp copies its expanded result back into `dir`, and its own
 * internal buffer is ENGINE_SZ_512, so `dir` must be at least that big to receive
 * it. The two file paths are `dir` plus a short basename. */
typedef struct {
    char dir[ENGINE_SZ_512];
    char pattern_path[ENGINE_SZ_1K];
    char filelist_path[ENGINE_SZ_1K];
...
static FILE *search_scratch_file(const char *dir, const char *basename, char *path_out,
    ...
    int descriptor = engine_mkstemp(path_out);
    if (descriptor < 0) {
        path_out[0] = '\0';
        return NULL;
    }
...
static void search_scratch_close(search_scratch_t *scratch) {
    ...
    if (scratch->pattern_path[0] != '\0') {
        (void)engine_unlink(scratch->pattern_path);
        scratch->pattern_path[0] = '\0';
    }
    if (scratch->filelist_path[0] != '\0') {
        (void)engine_unlink(scratch->filelist_path);
        scratch->filelist_path[0] = '\0';
    }
    if (scratch->dir[0] != '\0') {
        (void)engine_rmdir(scratch->dir);
        scratch->dir[0] = '\0';
    }
}
...
static bool search_scratch_open(search_scratch_t *scratch, const char *pattern) {
    ...
    int written =
        snprintf(scratch->dir, sizeof(scratch->dir), "%s/engine-search-XXXXXX", engine_tmpdir());
    if (written <= 0 || (size_t)written >= sizeof(scratch->dir) || !engine_mkdtemp(scratch->dir)) {
        scratch->dir[0] = '\0';
        return false;
    }
...
static bool compile_path_filter(const char *filter, engine_regex_t *re) {
    if (!filter || !filter[0]) {
        return false;
    }
    return engine_regcomp(re, filter, ENGINE_REG_EXTENDED | ENGINE_REG_NOSUB) == ENGINE_REG_OK;
}

static char *handle_search_code(engine_mcp_server_t *srv, const char *args) {
    char *pattern = engine_mcp_get_string_arg(args, "pattern");
    char *project = get_project_arg(args);
    char *file_pattern = engine_mcp_get_string_arg(args, "file_pattern");
    char *path_filter = engine_mcp_get_string_arg(args, "path_filter");
    char *mode_str = engine_mcp_get_string_arg(args, "mode");
    int limit = engine_mcp_get_int_arg(args, "limit", MCP_DEFAULT_LIMIT);
    int context_lines = engine_mcp_get_int_arg(args, "context", 0);
    bool use_regex = engine_mcp_get_bool_arg(args, "regex");
    uint64_t search_t0 = engine_now_ms();
    ...
    if (!pattern) {
        free(project);
        free(file_pattern);
        return engine_mcp_text_result("pattern is required", true);
    }
    ...
    if (!validate_search_args(root_path, file_pattern)) {
        if (has_path_filter) {
            engine_regfree(&path_regex);
        }
        free(root_path);
        free(pattern);
        free(project);
        free(file_pattern);
        return engine_mcp_text_result("path or file_pattern contains invalid characters", true);
    }
    ...
    if (use_regex) {
        engine_regex_t probe;
        if (engine_regcomp(&probe, pattern, ENGINE_REG_EXTENDED | ENGINE_REG_NOSUB) != ENGINE_REG_OK) {
            if (has_path_filter) {
                engine_regfree(&path_regex);
            }
            free(root_path);
    ...
    if (!search_scratch_open(&scratch, pattern)) {
        ...
        free(project);
        free(file_pattern);
        return engine_mcp_text_result(errmsg, true);
    }
    ...
    if (scoped && scoped_written == 0) {
        ...
        if (!fp) {
            ...
            free(project);
            free(file_pattern);
            return engine_mcp_text_result("search failed", true);
        }
    ...
    if (store && sr_count > 0) {
        ...
        if (engine_store_batch_count_degrees(store, ids, sr_count, "CALLS", in_degs, out_degs) ==
            ENGINE_STORE_OK) {
            for (int j = 0; j < sr_count; j++) {
    ...
    char *sc_format = engine_mcp_get_string_arg(args, "format");
    ...
    if (mode == 0 && !sc_legacy_json) {
        char *toon_text =
            assemble_search_output_toon(sr, sr_count, raw, raw_count, gm_count, limit,
                                        pat_has_pipe && !use_regex, engine_now_ms() - search_t0);
        result = engine_mcp_text_result(toon_text ? toon_text : "out of memory", toon_text == NULL);
        free(toon_text);
    } else {
...
static bool mcp_command_output_path(char out[ENGINE_SZ_2K]) {
    char directory[ENGINE_SZ_1K];
    const char *cache = engine_resolve_cache_dir();
    int written;
    if (cache && cache[0]) {
        written = snprintf(directory, sizeof(directory), "%s/logs", cache);
        if (written <= 0 || written >= (int)sizeof(directory) || !engine_mkdir_p(directory, 0700)) {
            return false;
        }
    } else {
        written = snprintf(directory, sizeof(directory), "%s", engine_tmpdir());
        if (written <= 0 || written >= (int)sizeof(directory)) {
            return false;
        }
    }
    written = snprintf(out, ENGINE_SZ_2K, "%s/.mcp-command-XXXXXX", directory);
    if (written <= 0 || written >= ENGINE_SZ_2K) {
        out[0] = '\0';
        return false;
    }
    int descriptor = engine_mkstemp(out);
    if (descriptor < 0) {
        out[0] = '\0';
        return false;
    }
 ...
 * COMSPEC. engine_subprocess receives this as lpApplicationName and validates the
 * same absolute cmd.exe path before using the dedicated payload serializer. */
static bool mcp_resolve_windows_cmd(char out[ENGINE_SZ_4K]) {
    if (!out) {
        return false;
    }
    out[0] = '\0';
    ...
    char *candidate = engine_wide_to_utf8(system_directory);
    if (!candidate) {
        return false;
    }
    bool resolved = engine_canonical_path(candidate, out, ENGINE_SZ_4K) != 0;
    free(candidate);
    return resolved;
}
#endif

static int mcp_run_shell_command_cancellable(engine_mcp_server_t *srv, const char *command,
                                             char output_path[ENGINE_SZ_2K],
                                             engine_proc_result_t *result_out) {
...
#ifdef _WIN32
    char shell[ENGINE_SZ_4K];
    if (!mcp_resolve_windows_cmd(shell)) {
        (void)engine_unlink(output_path);
        output_path[0] = '\0';
        return -1;
    }
#else
    ...
    engine_proc_opts_t options = {
        ...
        .quiet_timeout_ms = 0,
        .cancel_grace_ms = ENGINE_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS,
        .delete_log_on_exit = false,
    };
    engine_subprocess_t *process = NULL;
    if (engine_subprocess_spawn(&options, &process) != 0) {
        (void)engine_unlink(output_path);
        output_path[0] = '\0';
        return -1;
    }

    engine_proc_poll_t state;
    for (;;) {
        if (mcp_request_cancelled(srv)) {
            (void)engine_subprocess_request_cancel(process);
        }
        state = engine_subprocess_poll(process, result_out);
        if (state != ENGINE_PROC_POLL_RUNNING) {
            break;
        }
        engine_usleep(10000);
    }
    bool contained = state == ENGINE_PROC_POLL_TERMINAL && result_out->tree_quiesced &&
                     !result_out->supervision_failed;
    engine_subprocess_destroy(process);
...
bool engine_detect_node_in_hunks(const engine_node_t *node, const engine_changed_hunk_t *hunks,
...
static void detect_collect_seeds(engine_store_t *store, const char *project, const char *file,
                                 const engine_changed_hunk_t *hunks, int hunk_count, int64_t **seeds,
                                 int *n, int *cap) {
    engine_node_t *nodes = NULL;
    int ncount = 0;
    engine_store_find_nodes_by_file(store, project, file, &nodes, &ncount);
    ...
    if (scope_to_hunks) {
        ...
        for (int i = 0; i < ncount && !any_overlap; i++) {
            any_overlap = detect_is_seedable_label(nodes[i].label) &&
                          engine_detect_node_in_hunks(&nodes[i], hunks, hunk_count, file);
        }
        scope_to_hunks = any_overlap;
    }
    for (int i = 0; i < ncount; i++) {
        if (detect_is_seedable_label(nodes[i].label)) {
            if (scope_to_hunks && !engine_detect_node_in_hunks(&nodes[i], hunks, hunk_count, file)) {
                continue;
            }
            if (*n >= *cap) {
                *cap = *cap ? *cap * 2 : 16;
    ...
    engine_store_free_nodes(nodes, ncount);
}
...
static int detect_module_rollup(const engine_traverse_result_t *impact, char mods[][ENGINE_SZ_128],
                                int *mcnt, int *overflow) {
    int nmods = 0;
    *overflow = 0;
    for (int i = 0; i < impact->visited_count; i++) {
        char m[ENGINE_SZ_128];
        ...
        if (j == nmods) {
            if (nmods < DETECT_MODCAP) {
                snprintf(mods[nmods], ENGINE_SZ_128, "%s", m);
                mcnt[nmods] = 1;
                nmods++;
            } else {
                (*overflow)++;
...
static void detect_emit_impacted_tree(engine_sb_t *sb, engine_traverse_result_t *tr, int limit) {
    engine_tree_scalar_int(sb, "impacted_total", tr->visited_count);
    ...
    engine_sb_append(sb, hdr);
    if (shown > 1) {
        qsort(tr->visited, (size_t)shown, sizeof(engine_node_hop_t), tree_hop_cmp_qn);
    }
    char cur_group[ENGINE_SZ_1K] = "";
    for (int i = 0; i < shown; i++) {
        const char *qn =
    ...
    if (shown < tr->visited_count) {
        ...
        engine_sb_append(sb, more);
    }
}

static char *handle_detect_changes(engine_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    char *base_branch = engine_mcp_get_string_arg(args, "base_branch");
    char *since = engine_mcp_get_string_arg(args, "since");
    char *scope = engine_mcp_get_string_arg(args, "scope");
    int depth = engine_mcp_get_int_arg(args, "depth", MCP_DEFAULT_BFS_DEPTH);
    ...
    if (!engine_validate_shell_arg(base_branch) || base_branch[0] == '-' ||
        !validate_windows_cmd_interpolation_arg(base_branch)) {
        free(project);
        free(base_branch);
        free(scope);
        return engine_mcp_text_result("base_branch contains invalid characters", true);
    }
    ...
    if (!validate_search_path_arg(root_path) ||
        ...
        free(base_branch);
        free(scope);
        return engine_mcp_text_result("project path contains invalid characters", true);
    }
    ...
    char output_path[ENGINE_SZ_2K] = {0};
    engine_proc_result_t git_result = {0};
    ...
    if (git_cancelled) {
        ...
        free(base_branch);
        free(scope);
        return engine_mcp_text_result("detect_changes cancelled for this request", true);
    }
    if (git_run != 0) {
        (void)engine_unlink(output_path);
        char errmsg[ENGINE_SZ_256];
    ...
    FILE *fp = engine_fopen(output_path, "rb");
    if (!fp) {
        (void)engine_unlink(output_path);
        free(root_path);
        free(project);
        free(base_branch);
        free(scope);
        return engine_mcp_text_result("git diff failed: contained output could not be read", true);
    }
    ...
    char *fmt = engine_mcp_get_string_arg(args, "format");
    ...
    int imp_limit = engine_mcp_get_int_arg(args, "limit", MCP_DEFAULT_IMPACT_LIMIT);
    if (imp_limit < 1) {
        imp_limit = 1;
    }
    ...
    engine_changed_hunk_t *hunks = NULL;
    int hunk_count = 0;
    if (want_symbols) {
        char hunk_cmd[ENGINE_SZ_2K];
#ifdef _WIN32
        ...
        snprintf(hunk_cmd, sizeof(hunk_cmd),
                 ...
                 root_path, base_branch, root_path);
#endif
        char hunk_output_path[ENGINE_SZ_2K] = {0};
        engine_proc_result_t hunk_result = {0};
        int hunk_run =
        ...
        FILE *hfp = (!hunk_cancelled && hunk_run == 0) ? engine_fopen(hunk_output_path, "rb") : NULL;
        if (hfp) {
            ...
            if (hsz > 0) {
                ...
                if (hbuf) {
                    ...
                    hbuf[hread] = '\0';
                    enum { HUNK_CAP = 4096 };
                    hunks = safe_realloc(NULL, (size_t)HUNK_CAP * sizeof(engine_changed_hunk_t));
                    hunk_count = engine_parse_hunks(hbuf, hunks, HUNK_CAP);
                    ...
                    if (hunk_count >= HUNK_CAP) {
                        engine_log_info("detect_changes.hunks", "action", "scoping_disabled", "reason",
                                     "hunk_cap_reached");
                        free(hunks);
                        hunks = NULL;
                        hunk_count = 0;
        ...
        if (hunk_output_path[0]) {
            (void)engine_unlink(hunk_output_path);
        }
    }
    ...
    char merge_base[64] = "";
    {
        ...
        snprintf(mbcmd, sizeof(mbcmd), "git -C '%s' merge-base '%s' HEAD 2>/dev/null", root_path,
                 base_branch);
#endif
        char mb_output_path[ENGINE_SZ_2K] = {0};
        engine_proc_result_t mb_result = {0};
        ...
        FILE *mbfp =
            mb_run == 0 && mb_result.exit_code == 0 ? engine_fopen(mb_output_path, "rb") : NULL;
        ...
        if (mb_output_path[0]) {
            (void)engine_unlink(mb_output_path);
        }
    ...
    engine_traverse_result_t impact = {0};
    bool truncated = false;
    if (want_symbols && seed_count > 0) {
        (void)engine_store_bfs_multi(store, seeds, seed_count, direction, NULL, 0, depth,
    ...
    if (!legacy_json) {
        engine_sb_t sb;
        engine_sb_init(&sb);
        engine_tree_scalar_str(&sb, "base", base_branch);
        if (merge_base[0]) {
            engine_tree_scalar_str(&sb, "merge_base", merge_base);
        }
        engine_tree_scalar_str(&sb, "direction", direction);
        if (is_error) {
            char hint_buf[ENGINE_SZ_256];
            snprintf(hint_buf, sizeof(hint_buf),
                     "git diff exited with status %d. Check that branch '%s' exists.", git_status,
                     base_branch);
            engine_tree_scalar_str(&sb, "hint", hint_buf);
        }
        /* changed files (the git result) */
        char cf[ENGINE_SZ_64];
        snprintf(cf, sizeof(cf), "changed_files: %d\n", file_count);
        engine_sb_append(&sb, cf);
        for (int i = 0; i < file_count; i++) {
            engine_sb_append(&sb, "  ");
            engine_sb_append(&sb, files[i]);
            engine_sb_append(&sb, "\n");
        }
        engine_tree_scalar_int(&sb, "seed_symbols", seed_count);
        if (want_symbols) {
            detect_emit_impacted_tree(&sb, &impact, imp_limit);
            /* module rollup: a quotient view of the blast radius */
            if (impact.visited_count > 0) {
                engine_sb_append(&sb, "impacted_modules: (rows: module count)\n");
                char (*mods)[ENGINE_SZ_128] = malloc(DETECT_MODCAP * ENGINE_SZ_128);
                int *mcnt = malloc(DETECT_MODCAP * sizeof(int));
                if (mods && mcnt) {
                    int overflow = 0;
                    int nmods = detect_module_rollup(&impact, mods, mcnt, &overflow);
                    for (int j = 0; j < nmods; j++) {
                        char mrow[ENGINE_SZ_256];
                        snprintf(mrow, sizeof(mrow), "  %s %d\n", mods[j], mcnt[j]);
                        engine_sb_append(&sb, mrow);
                    }
                    if (overflow > 0) {
                        char orow[ENGINE_SZ_128];
                        snprintf(orow, sizeof(orow), "  (other) %d\n", overflow);
                        engine_sb_append(&sb, orow);
                    }
                }
                free(mods);
                free(mcnt);
    ...
    engine_store_traverse_free(&impact);
    for (int i = 0; i < file_count; i++) {
        free(files[i]);
    }
    free(files);
    ...
    char *result = engine_mcp_text_result(out_str, is_error);
    free(out_str);
    return result;
}
...
/* Read the legacy file-based ADR (<root>/.graph-engine/adr.md), used by
...
static char *adr_read_legacy_file(const char *root_path) {
    ...
    char adr_path[ENGINE_SZ_4K];
    snprintf(adr_path, sizeof(adr_path), "%s/.graph-engine/adr.md", root_path);
    FILE *fp = engine_fopen(adr_path, "r");
    if (!fp) {
        return NULL;
    }
...
static engine_store_t *open_adr_store_for_write(engine_mcp_server_t *srv, engine_store_t *resolved,
                                             engine_store_t **owned_rw) {
    if (!srv || !resolved || !owned_rw) {
        return NULL;
    }
    const char *resolved_db_path = engine_store_db_path(resolved);
    if (!resolved_db_path) {
        return resolved;
    }
    ...
    invalidate_cached_store(srv);
    *owned_rw = engine_store_open_path(rw_path);
    free(rw_path);
    return *owned_rw;
}

static char *handle_manage_adr(engine_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    char *mode_str = engine_mcp_get_string_arg(args, "mode");
    char *content = engine_mcp_get_string_arg(args, "content");
    ...
    if (write_request && project) {
        ...
        if (mcp_request_cancelled(srv)) {
            ...
            free(mode_str);
            free(content);
            return engine_mcp_text_result("project operation cancelled for this request", true);
        }
    }
    ...
    engine_store_t *resolved =
        resolve_store_internal(srv, project, mutation_held, !write_request, &recovery_status);
    if (!resolved) {
        char *res = NULL;
        if (recovery_status == STORE_RECOVERY_BUSY) {
            res = engine_mcp_text_result("project is busy; retry after indexing", true);
        } else if (recovery_status == STORE_RECOVERY_TRY_GUARD_UNAVAILABLE) {
            res =
                engine_mcp_text_result("project recovery requires a nonblocking mutation guard", true);
        } else {
            char *err = build_no_store_error(project);
            res = engine_mcp_text_result(err, true);
            free(err);
        }
        if (mutation_held) {
            mcp_project_mutation_end(srv, project);
    ...
    engine_store_t *store = resolved;
    engine_store_t *owned_rw = NULL;
    if (write_request) {
        ...
        if (!store) {
            ...
            free(mode_str);
            free(content);
            return engine_mcp_text_result("failed to open writable ADR store", true);
        }
    }

    /* One-time migration: older versions wrote ADRs to a file at
     * <root>/.graph-engine/adr.md. A read never waits for the project lease:
     * it returns the legacy content immediately and attempts migration only if
     * a nonblocking acquire succeeds. */
    engine_adr_t adr;
    memset(&adr, 0, sizeof(adr));
    bool have_adr = (engine_store_adr_get(store, project, &adr) == ENGINE_STORE_OK);
    char *legacy = NULL;
    if (!have_adr && !write_request) {
        ...
        if (legacy && mcp_project_mutation_try_begin(srv, project)) {
            if (!mcp_request_cancelled(srv)) {
                ...
                if (engine_store_db_path(resolved)) {
                    invalidate_cached_store(srv);
                    resolved = NULL;
                    store = NULL;
                ...
                if (resolved) {
                    store = open_adr_store_for_write(srv, resolved, &owned_rw);
                    if (store) {
                        have_adr = (engine_store_adr_get(store, project, &adr) == ENGINE_STORE_OK);
                        if (!have_adr &&
                            engine_store_adr_store(store, project, legacy) == ENGINE_STORE_OK) {
                            have_adr = (engine_store_adr_get(store, project, &adr) == ENGINE_STORE_OK);
                        }
                    }
                }
    ...
    if (write_request) {
        if (engine_store_adr_store(store, project, content) == ENGINE_STORE_OK) {
...
static char *handle_ingest_traces(engine_mcp_server_t *srv, const char *args) {
    (void)srv;
...
static char *dispatch_tool(engine_mcp_server_t *srv, const char *tool_name, const char *args_json) {
    if (!tool_name) {
        return engine_mcp_text_result("missing tool name", true);
    }
    if (srv && !mcp_tool_allowed(srv->tool_profile, tool_name)) {
        char message[ENGINE_SZ_256];
        ...
        return engine_mcp_text_result(message, true);
    }
 ...
 * process lifetime. */
static void release_request_store(engine_mcp_server_t *srv) {
    if (!srv || !srv->owns_store || !srv->store || !engine_store_db_path(srv->store)) {
        return;
    }
    engine_store_close(srv->store);
    srv->store = NULL;
...
char *engine_mcp_handle_tool(engine_mcp_server_t *srv, const char *tool_name, const char *args_json) {
    /* Phase marks bracket the WHOLE request with no unlabelled gap, so growth
     * cannot hide between them (ENGINE_MEM_PHASES=1; see foundation/mem.h). The
    ...
    engine_mem_phase_mark("request.scope_begin");
    bool request_scope = !srv || engine_mcp_server_request_scope_begin(srv);
    if (!request_scope) {
        release_request_store(srv);
        engine_mem_phase_mark("idle");
        return engine_mcp_text_result("request cancellation scope unavailable", true);
    }
    engine_mem_phase_mark("request.dispatch_tool");
    char *result = dispatch_tool(srv, tool_name, args_json);
    engine_mem_phase_mark("request.scope_end");
    if (srv) {
        engine_mcp_server_request_scope_end(srv);
    }
    engine_mem_phase_mark("request.release_store");
    release_request_store(srv);
    engine_mem_phase_mark("idle");
    ...
    engine_mem_census_log("mcp.request");
    return result;
}
...
static void detect_session(engine_mcp_server_t *srv) {
    ...
    if (getcwd(cwd, sizeof(cwd)) != NULL) {
        ...
        if (strcmp(cwd, "/") != 0 && (home == NULL || strcmp(cwd, home) != 0)) {
            ...
            engine_log_info("session.root.cwd", "path", cwd);
        }
    }

    /* Derive project name from path — must match engine_project_name_from_path
    ...
    if (srv->session_root[0]) {
        char *pname = engine_project_name_from_path(srv->session_root);
        if (pname) {
...
static bool auto_watch_enabled(engine_mcp_server_t *srv) {
    if (!srv->config) {
        return true; /* default on */
    }
    return engine_config_get_bool(srv->config, ENGINE_CONFIG_AUTO_WATCH, true);
}

/* Register the session project with the background watcher for ongoing
 * change detection — unless auto_watch is disabled. */
static void register_watcher_if_enabled(engine_mcp_server_t *srv) {
    if (!srv->watcher || srv->session_project[0] == '\0' || srv->session_root[0] == '\0') {
        return;
    }
    if (!auto_watch_enabled(srv)) {
        engine_log_info("watcher.register.skipped", "reason", "auto_watch_off", "project",
                     srv->session_project);
        return;
    }
    engine_watcher_watch(srv->watcher, srv->session_project, srv->session_root);
}

/* Background auto-index thread function */
static void *autoindex_thread(void *arg) {
    engine_mcp_server_t *srv = (engine_mcp_server_t *)arg;

    engine_log_info("autoindex.start", "project", srv->session_project, "path", srv->session_root);
    ...
    if (engine_index_supervisor_should_wrap()) {
        ...
        if (resp) {
            free(resp);
            engine_log_info("autoindex.done", "project", srv->session_project, "mode", "supervised");
        ...
        engine_log_error("autoindex.supervision_failed", "project", srv->session_project, "action",
                      "fail_closed");
        return NULL;
    }

    engine_pipeline_t *p = engine_pipeline_new(srv->session_root, NULL, ENGINE_MODE_FULL);
    if (!p) {
        engine_log_warn("autoindex.err", "msg", "pipeline_create_failed");
        return NULL;
    }
    ...
    engine_pipeline_free(p);
    engine_mem_collect(); /* return mimalloc pages to OS after indexing (in-process only) */

    if (rc == 0) {
        engine_log_info("autoindex.done", "project", srv->session_project);
        register_watcher_if_enabled(srv);
    } else {
        engine_log_warn("autoindex.err", "msg", "pipeline_run_failed");
    }
    return NULL;
}

bool engine_mcp_auto_index_within_file_limit(const char *root_path, int file_limit,
                                          int *file_count_out) {
    if (file_count_out) {
        *file_count_out = -1;
    }
    ...
    engine_discover_opts_t options = {
        .mode = ENGINE_MODE_FULL,
        .ignore_file = NULL,
        .max_file_size = 0,
    };
    int count = -1;
    engine_discover_status_t status = engine_discover_count_bounded(
        root_path, &options, file_limit, engine_now_ms() + AUTO_INDEX_COUNT_TIMEOUT_MS, &count);
    if (file_count_out) {
        *file_count_out = status == ENGINE_DISCOVER_LIMIT_EXCEEDED
                              ? (file_limit < INT_MAX ? file_limit + 1 : INT_MAX)
                              : count;
    }
    return status == ENGINE_DISCOVER_OK;
}

/* Start auto-indexing if configured and project not yet indexed. */
static void maybe_auto_index(engine_mcp_server_t *srv) {
    ...
    const char *home = engine_get_home_dir();
    if (home) {
        char db_check[ENGINE_SZ_1K];
        snprintf(db_check, sizeof(db_check), "%s/%s.db", engine_resolve_cache_dir(),
                 srv->session_project);
        if (engine_file_size(db_check) >= 0) {
            /* Already indexed → register watcher for change detection */
            engine_log_info("autoindex.skip", "reason", "already_indexed", "project",
                         srv->session_project);
            register_watcher_if_enabled(srv);
            return;
        }
    ...
    int file_limit = ENGINE_MCP_DEFAULT_AUTO_INDEX_LIMIT;
    if (srv->config) {
        auto_index = engine_config_get_bool(srv->config, ENGINE_CONFIG_AUTO_INDEX, false);
        file_limit = engine_config_get_int(srv->config, ENGINE_CONFIG_AUTO_INDEX_LIMIT,
                                        ENGINE_MCP_DEFAULT_AUTO_INDEX_LIMIT);
    }

    if (!auto_index) {
        engine_log_info("autoindex.skip", "reason", "disabled", "hint",
                     "run: graph-engine config set auto_index true");
        return;
    }

    /* Quick tracked-file count check to avoid OOM on massive repos. */
    int file_count = -1;
    if (!engine_mcp_auto_index_within_file_limit(srv->session_root, file_limit, &file_count)) {
        char files[32];
        (void)snprintf(files, sizeof(files), "%d", file_count);
        engine_log_warn("autoindex.skip", "reason",
                     file_count >= 0 ? "too_many_files" : "unsafe_or_unavailable_path", "files",
                     files, "limit", ENGINE_CONFIG_AUTO_INDEX_LIMIT);
        return;
    }

    /* Launch auto-index in background */
    if (engine_thread_create(&srv->autoindex_tid, 0, autoindex_thread, srv) == 0) {
...
bool engine_mcp_jsonrpc_response_prepend_notice(char **response_io, const char *notice) {
...
char *engine_mcp_server_handle(engine_mcp_server_t *srv, const char *line) {
    engine_jsonrpc_request_t req = {0};
    if (engine_jsonrpc_parse(line, &req) < 0) {
        return engine_jsonrpc_format_error(0, JSONRPC_PARSE_ERROR, "Parse error");
    }

    /* Notifications (no id) → handle cancellation, then no response */
    if (!req.has_id) {
        if (req.method && strcmp(req.method, "notifications/cancelled") == 0) {
            if (engine_mcp_cancel_request_matches(req.params_raw, srv->active_request_id,
                                               srv->active_request_id_str) &&
                engine_mcp_server_cancel_active(srv)) {
                engine_log_info("mcp.cancelled", "match", "true");
            }
        }
        engine_jsonrpc_request_free(&req);
        return NULL;
    }

    if (!engine_mcp_server_request_scope_begin(srv)) {
        int64_t request_id = req.id;
        engine_jsonrpc_request_free(&req);
        return engine_jsonrpc_format_error(request_id, JSONRPC_INTERNAL_ERROR,
                                        "Request cancellation scope unavailable");
    }

    struct timespec req_t0;
    engine_clock_gettime(CLOCK_MONOTONIC, &req_t0);
    char *result_json = NULL;
    char *request_error_json = NULL;
    bool request_logged = false;

    if (strcmp(req.method, "initialize") == 0) {
        result_json = engine_mcp_initialize_response_for_profile(req.params_raw, srv->tool_profile);
        detect_session(srv);
        if (srv->background_tasks && srv->tool_profile == ENGINE_MCP_TOOL_PROFILE_ALL) {
        ...
        result_json = engine_mcp_prompts_list();
    } else if (strcmp(req.method, "prompts/get") == 0) {
        result_json = engine_mcp_prompt_get(req.params_raw, &request_error_json);
    } else if (strcmp(req.method, "tools/list") == 0) {
        result_json = engine_mcp_tools_list_page(srv->tool_profile, req.params_raw);
    } else if (strcmp(req.method, "tools/call") == 0) {
        char *tool_name = req.params_raw ? engine_mcp_get_tool_name(req.params_raw) : NULL;
        char *tool_args =
            req.params_raw ? engine_mcp_get_arguments(req.params_raw) : heap_strdup("{}");
        srv->active_request_id = req.id;
        free(srv->active_request_id_str);
        srv->active_request_id_str = req.id_str ? heap_strdup(req.id_str) : NULL;

        struct timespec t0;
        engine_clock_gettime(CLOCK_MONOTONIC, &t0);
        result_json = engine_mcp_handle_tool(srv, tool_name, tool_args);
        srv->active_request_id = ENGINE_NOT_FOUND;
        free(srv->active_request_id_str);
        srv->active_request_id_str = NULL;
        struct timespec t1;
        engine_clock_gettime(CLOCK_MONOTONIC, &t1);
        ...
        engine_jsonrpc_response_t err_resp = {
            .id = req.id,
            .id_str = req.id_str,
            .error_json = err_obj,
        };
        char *err = engine_jsonrpc_format_response(&err_resp);
        struct timespec t1;
        engine_clock_gettime(CLOCK_MONOTONIC, &t1);
        long long dur_us = ((long long)(t1.tv_sec - req_t0.tv_sec) * MCP_S_TO_US) +
                           ((long long)(t1.tv_nsec - req_t0.tv_nsec) / MCP_MS_TO_US);
        engine_log_mcp_request(req.method, NULL, true, dur_us);
        engine_mcp_server_request_scope_end(srv);
        engine_jsonrpc_request_free(&req);
        return err;
    }
    ...
    if (!request_logged) {
        ...
        long long dur_us = ((long long)(t1.tv_sec - req_t0.tv_sec) * MCP_S_TO_US) +
                           ((long long)(t1.tv_nsec - req_t0.tv_nsec) / MCP_MS_TO_US);
        engine_log_mcp_request(req.method, NULL, false, dur_us);
    }

    engine_jsonrpc_response_t resp = {
        .id = req.id,
        .id_str = req.id_str,
        .result_json = result_json,
    };
    char *out = engine_jsonrpc_format_response(&resp);
    free(result_json);
    engine_mcp_server_request_scope_end(srv);
    engine_jsonrpc_request_free(&req);
    return out;
...
static bool parse_content_length(const char *line, size_t *out) {
    ...
    errno = 0;
    char *end = NULL;
    unsigned long long parsed = strtoull(cursor, &end, ENGINE_DECIMAL_BASE);
...
int engine_mcp_read_message(FILE *in, char **message, bool *content_length_framed) {
...
static int poll_for_input_unix(engine_mcp_server_t *srv, int fd, FILE *in) {
    ...
    if (saved_flags < 0) {
        ...
        if (pr == 0) {
            engine_mcp_server_evict_idle(srv, STORE_IDLE_TIMEOUT_S);
            return 0;
        }
        return SKIP_ONE;
    }
    ...
    if (c == EOF) {
        if (feof(in)) {
            return ENGINE_NOT_FOUND; /* true EOF */
        }
        clearerr(in);
...
int engine_mcp_server_run(engine_mcp_server_t *srv, FILE *in, FILE *out) {
    int fd = engine_fileno(in);
    ...
    _setmode(engine_fileno(in), _O_BINARY);
    _setmode(engine_fileno(out), _O_BINARY);
#endif
    ...
    for (;;) {
        ...
        if (engine_mcp_read_message(in, &message, &content_length_framed) <= 0) {
            break;
        }

        char *resp = engine_mcp_server_handle(srv, message);
        free(message);
        if (resp) {
...
/* ── engine_parse_file_uri ──────────────────────────────────────── */

bool engine_parse_file_uri(const char *uri, char *out_path, int out_size) {
