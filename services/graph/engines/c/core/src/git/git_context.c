...
static int git_capture(const char *repo_path, const char *git_args, char **out) {
    ...
    *out = NULL;
...
#endif
    /* Double quotes work for POSIX shells and cmd.exe. engine_validate_shell_arg()
    ...
    int rc = engine_pclose(fp);
    if (rc != 0 || buf[0] == '\0') {
        return ENGINE_NOT_FOUND;
    }

    *out = git_strdup(buf);
    return *out ? 0 : ENGINE_NOT_FOUND;
}
...
static bool json_append_string(char *buf, int buf_size, int *off, const char *name,
    ...
    int actual = engine_json_escape(escaped, needed + 1, value ? value : "");
