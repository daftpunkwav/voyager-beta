...
enum {
    ...
    VERSION_COHORT_RECORD_BUILD_OFFSET =
        VERSION_COHORT_RECORD_VERSION_OFFSET + ENGINE_DAEMON_VERSION_TEXT_SIZE,
    VERSION_COHORT_RECORD_CACHE_OFFSET =
        VERSION_COHORT_RECORD_BUILD_OFFSET + ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE,
    VERSION_COHORT_RECORD_PROTOCOL_OFFSET =
        VERSION_COHORT_RECORD_CACHE_OFFSET + ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE,
...
static const char VERSION_COHORT_ADMISSION_FILE[] = "engine-version-cohort-admission-v1.lock";
static const char VERSION_COHORT_LIFETIME_FILE[] = "engine-version-cohort-lifetime-v1.lock";
static const char VERSION_COHORT_MAINTENANCE_FILE[] = "engine-version-cohort-maintenance-v1.lock";
static const char VERSION_COHORT_DAEMON_FILE[] = "engine-version-cohort-daemon-v1.lock";

struct engine_version_cohort_manager {
...
struct engine_version_cohort_lease {
    engine_version_cohort_manager_t *manager;
    engine_private_file_lock_t *maintenance;
    engine_private_file_lock_t *admission;
    engine_private_file_lock_t *lifetime;
    bool registered;
};

struct engine_version_cohort_daemon_claim {
    engine_version_cohort_manager_t *manager;
    engine_private_file_lock_t *marker;
    bool registered;
};
...
static void version_cohort_retry_sleep(void) {
    uint64_t mix = (engine_now_ms() + version_cohort_current_pid()) * 2654435761ULL;
    engine_usleep(VERSION_COHORT_RETRY_US / 2U + (uint32_t)(mix % VERSION_COHORT_RETRY_US));
}

static engine_private_file_lock_status_t version_cohort_lock_release_until(
    engine_private_file_lock_t **lock_io, uint64_t deadline_ms) {
    engine_private_file_lock_status_t result = ENGINE_PRIVATE_FILE_LOCK_OK;
    if (!lock_io) {
        return ENGINE_PRIVATE_FILE_LOCK_IO;
    }
    while (*lock_io) {
        engine_private_file_lock_status_t status = engine_private_file_lock_release(lock_io);
        ...
        if (engine_now_ms() >= deadline_ms) {
            return result == ENGINE_PRIVATE_FILE_LOCK_OK ? ENGINE_PRIVATE_FILE_LOCK_IO : result;
        }
...
#ifndef _WIN32
static void version_cohort_startup_lock_release_complete(engine_daemon_ipc_startup_lock_t **lock_io) {
...
static bool version_cohort_identity_valid(const engine_daemon_build_identity_t *identity) {
    const char *cache = identity ? identity->cache_fingerprint : NULL;
    if (!cache) {
        return false;
    }
    size_t cache_length = 0;
    while (cache_length < ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE && cache[cache_length]) {
    ...
    if (cache_length != ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE - 1U || cache[cache_length] != '\0') {
        return false;
    }
    engine_daemon_conflict_t comparison;
    return engine_daemon_hello_compare(identity, identity, &comparison) == ENGINE_DAEMON_HELLO_COMPATIBLE;
}
...
static bool version_cohort_record_encode(const engine_daemon_build_identity_t *identity,
...
static bool version_cohort_record_decode(const unsigned char record[VERSION_COHORT_RECORD_SIZE],
                                         size_t length, engine_daemon_build_identity_t *identity_out,
                                         char version_out[ENGINE_DAEMON_VERSION_TEXT_SIZE],
                                         char build_out[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE],
                                         char cache_out[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    ...
    memcpy(version_out, record + VERSION_COHORT_RECORD_VERSION_OFFSET,
           ENGINE_DAEMON_VERSION_TEXT_SIZE);
    memcpy(build_out, record + VERSION_COHORT_RECORD_BUILD_OFFSET,
           ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE);
...
static engine_version_cohort_status_t version_cohort_status_from_lock(
    engine_private_file_lock_status_t status) {
    switch (status) {
    case ENGINE_PRIVATE_FILE_LOCK_OK:
        return ENGINE_VERSION_COHORT_OK;
    case ENGINE_PRIVATE_FILE_LOCK_BUSY:
        return ENGINE_VERSION_COHORT_BUSY;
    case ENGINE_PRIVATE_FILE_LOCK_UNSAFE:
        return ENGINE_VERSION_COHORT_UNSAFE;
    case ENGINE_PRIVATE_FILE_LOCK_IO:
    default:
        return ENGINE_VERSION_COHORT_IO;
    }
}

static engine_private_file_lock_status_t version_cohort_lock_until(
    engine_version_cohort_manager_t *manager, const char *base_name, engine_private_file_lock_mode_t mode,
    uint64_t deadline_ms, engine_private_file_lock_t **lock_out) {
    for (;;) {
        engine_private_file_lock_status_t status =
            engine_private_file_lock_try_acquire(manager->directory, base_name, mode, lock_out);
        if (status != ENGINE_PRIVATE_FILE_LOCK_BUSY ||
            (deadline_ms != UINT64_MAX && engine_now_ms() >= deadline_ms)) {
            return status;
        }
        version_cohort_retry_sleep();
    }
...
static engine_version_cohort_lease_t *version_cohort_lease_new(engine_version_cohort_manager_t *manager) {
    if (!manager) {
        return NULL;
    }
    engine_version_cohort_lease_t *lease = calloc(1, sizeof(*lease));
    if (!lease) {
        return NULL;
    }
    engine_mutex_lock(&manager->mutex);
    bool admitted = !manager->closing && manager->owner_pid == version_cohort_current_pid();
    if (admitted) {
        manager->lease_count++;
    }
    engine_mutex_unlock(&manager->mutex);
    if (!admitted) {
        free(lease);
        return NULL;
    }
...
static bool version_cohort_manager_register(engine_version_cohort_manager_t *manager) {
    if (!manager) {
        return false;
    }
    engine_mutex_lock(&manager->mutex);
...
static bool version_cohort_manager_unregister(engine_version_cohort_manager_t *manager) {
    if (!manager) {
        return false;
    }
    engine_mutex_lock(&manager->mutex);
...
engine_private_file_lock_status_t engine_version_cohort_lease_release(
    ...
    engine_version_cohort_lease_t *lease = *lease_io;
    engine_private_file_lock_status_t result = ENGINE_PRIVATE_FILE_LOCK_OK;
    if (lease->lifetime) {
        engine_private_file_lock_status_t status = engine_private_file_lock_release(&lease->lifetime);
        if (status != ENGINE_PRIVATE_FILE_LOCK_OK) {
            result = status;
        }
    }
    ...
    if (lease->admission) {
        engine_private_file_lock_status_t status = engine_private_file_lock_release(&lease->admission);
        if (status != ENGINE_PRIVATE_FILE_LOCK_OK) {
            result = status;
        }
    }
    ...
    if (lease->maintenance) {
        engine_private_file_lock_status_t status = engine_private_file_lock_release(&lease->maintenance);
        if (status != ENGINE_PRIVATE_FILE_LOCK_OK) {
            result = status;
        }
    }
    ...
    if (lease->registered && lease->manager) {
        engine_mutex_lock(&lease->manager->mutex);
        if (lease->manager->lease_count > 0) {
            lease->manager->lease_count--;
        } else {
            result = ENGINE_PRIVATE_FILE_LOCK_IO;
        }
        engine_mutex_unlock(&lease->manager->mutex);
...
static engine_version_cohort_status_t version_cohort_failed(engine_version_cohort_lease_t *lease,
                                                         engine_version_cohort_status_t status,
                                                         engine_version_cohort_lease_t **lease_out) {
    engine_version_cohort_lease_t *cleanup = lease;
    engine_private_file_lock_status_t cleanup_status = engine_version_cohort_lease_release(&cleanup);
    if (cleanup) {
        *lease_out = cleanup;
        return ENGINE_VERSION_COHORT_IO;
    }
    return cleanup_status == ENGINE_PRIVATE_FILE_LOCK_OK ? status : ENGINE_VERSION_COHORT_IO;
}
...
engine_version_cohort_manager_t *engine_version_cohort_manager_new(
    ...
    if (directory_status != ENGINE_PRIVATE_FILE_LOCK_OK || !directory) {
        if (directory) {
            engine_private_lock_directory_close(directory);
        }
        return NULL;
    }
    engine_version_cohort_manager_t *manager = calloc(1, sizeof(*manager));
    if (!manager) {
        engine_private_lock_directory_close(directory);
        return NULL;
    }
    ...
    engine_mutex_init(&manager->mutex);
    return manager;
}

static engine_version_cohort_status_t version_cohort_claim_new(
    engine_version_cohort_lease_t *lease, const engine_daemon_build_identity_t *identity,
    uint64_t deadline_ms) {
    unsigned char record[VERSION_COHORT_RECORD_SIZE];
    if (!version_cohort_record_encode(identity, record) ||
        engine_private_file_lock_payload_write(lease->lifetime, record, sizeof(record)) !=
            ENGINE_PRIVATE_FILE_LOCK_OK) {
        return ENGINE_VERSION_COHORT_IO;
    }
    engine_private_file_lock_status_t release_status = engine_private_file_lock_release(&lease->lifetime);
    if (release_status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        return ENGINE_VERSION_COHORT_IO;
    }
    return version_cohort_status_from_lock(
        version_cohort_lock_until(lease->manager, VERSION_COHORT_LIFETIME_FILE,
                                  ENGINE_PRIVATE_FILE_LOCK_SH, deadline_ms, &lease->lifetime));
}
...
engine_version_cohort_status_t engine_version_cohort_acquire(engine_version_cohort_manager_t *manager,
    ...
    engine_version_cohort_lease_t *lease = version_cohort_lease_new(manager);
    if (!lease) {
        return ENGINE_VERSION_COHORT_IO;
    }
    ...
    engine_private_file_lock_status_t lock_status =
        engine_private_file_lock_try_acquire(manager->directory, VERSION_COHORT_MAINTENANCE_FILE,
                                          ENGINE_PRIVATE_FILE_LOCK_SH, &lease->maintenance);
    if (lock_status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        return version_cohort_failed(lease, version_cohort_status_from_lock(lock_status),
                                     lease_out);
    }
    lock_status =
        version_cohort_lock_until(manager, VERSION_COHORT_ADMISSION_FILE, ENGINE_PRIVATE_FILE_LOCK_EX,
    ...
    lock_status =
        engine_private_file_lock_try_acquire(manager->directory, VERSION_COHORT_LIFETIME_FILE,
                                          ENGINE_PRIVATE_FILE_LOCK_EX, &lease->lifetime);
    engine_version_cohort_status_t status = version_cohort_status_from_lock(lock_status);
    if (status == ENGINE_VERSION_COHORT_OK) {
        ...
        engine_log_info("version_cohort.claimed_unheld", "build",
                     identity->build_fingerprint ? identity->build_fingerprint : "<null>");
        status = version_cohort_claim_new(lease, identity, deadline_ms);
    } else if (status == ENGINE_VERSION_COHORT_BUSY) {
        lock_status =
            engine_private_file_lock_try_acquire(manager->directory, VERSION_COHORT_LIFETIME_FILE,
                                              ENGINE_PRIVATE_FILE_LOCK_SH, &lease->lifetime);
        status = version_cohort_status_from_lock(lock_status);
        if (status == ENGINE_VERSION_COHORT_OK) {
            unsigned char record[VERSION_COHORT_RECORD_SIZE];
            size_t record_length = 0;
            char active_version[ENGINE_DAEMON_VERSION_TEXT_SIZE];
            char active_build[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE];
            char active_cache[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE];
            engine_daemon_build_identity_t active;
            engine_private_file_lock_status_t payload_status = engine_private_file_lock_payload_read(
                lease->lifetime, record, sizeof(record), &record_length);
            bool decoded = payload_status == ENGINE_PRIVATE_FILE_LOCK_OK &&
            ...
            if (!decoded) {
                ...
                engine_log_warn("version_cohort.record_undecodable", "payload_status",
                             payload_status == ENGINE_PRIVATE_FILE_LOCK_OK ? "read_ok" : "read_failed",
                             "bytes", len_text);
                status = ENGINE_VERSION_COHORT_UNSAFE;
            } else {
                engine_daemon_hello_status_t comparison =
                    engine_daemon_hello_compare(&active, identity, conflict_out);
                if (comparison == ENGINE_DAEMON_HELLO_INVALID) {
                ...
                if (comparison == ENGINE_DAEMON_HELLO_COMPATIBLE &&
                    strcmp(active_cache_fingerprint, requested_cache_fingerprint) != 0) {
                    comparison = ENGINE_DAEMON_HELLO_CACHE_CONFLICT;
                ...
                if (comparison == ENGINE_DAEMON_HELLO_COMPATIBLE) {
                    status = ENGINE_VERSION_COHORT_OK;
                } else if (comparison == ENGINE_DAEMON_HELLO_INVALID) {
                    status = ENGINE_VERSION_COHORT_UNSAFE;
                } else {
                    engine_private_file_lock_status_t release_status =
                        engine_private_file_lock_release(&lease->lifetime);
                    if (release_status != ENGINE_PRIVATE_FILE_LOCK_OK) {
                        status = ENGINE_VERSION_COHORT_IO;
                    } else {
                        lock_status = engine_private_file_lock_try_acquire(
                        ...
                        if (status == ENGINE_VERSION_COHORT_OK) {
                            ...
                            status = ENGINE_VERSION_COHORT_CONFLICT;
                        }
                    }
                }
            }
    ...
    engine_private_file_lock_status_t admission_release =
        engine_private_file_lock_release(&lease->admission);
    if (admission_release != ENGINE_PRIVATE_FILE_LOCK_OK) {
        return version_cohort_failed(lease, ENGINE_VERSION_COHORT_IO, lease_out);
    }
    engine_private_file_lock_status_t maintenance_release =
        engine_private_file_lock_release(&lease->maintenance);
    if (maintenance_release != ENGINE_PRIVATE_FILE_LOCK_OK) {
        return version_cohort_failed(lease, ENGINE_VERSION_COHORT_IO, lease_out);
    }
    *lease_out = lease;
    return ENGINE_VERSION_COHORT_OK;
...
static engine_version_cohort_status_t version_cohort_reserve_for_mutation_internal(
    ...
    engine_version_cohort_quiesce_result_t *quiesce_result_out, engine_version_cohort_lease_t **lease_out,
    bool require_finite_deadline) {
    if (lease_out) {
        *lease_out = NULL;
    }
    if (quiesce_result_out) {
        *quiesce_result_out = ENGINE_VERSION_COHORT_QUIESCE_NOT_NEEDED;
    }
    ...
    engine_private_file_lock_status_t lock_status =
        version_cohort_lock_until(manager, VERSION_COHORT_MAINTENANCE_FILE,
                                  ENGINE_PRIVATE_FILE_LOCK_EX, deadline_ms, &lease->maintenance);
    if (lock_status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        return version_cohort_failed(lease, version_cohort_status_from_lock(lock_status),
                                     lease_out);
    }
    ...
    lock_status =
        engine_private_file_lock_try_acquire(manager->directory, VERSION_COHORT_LIFETIME_FILE,
                                          ENGINE_PRIVATE_FILE_LOCK_EX, &lease->lifetime);
    if (lock_status == ENGINE_PRIVATE_FILE_LOCK_BUSY) {
        if (!quiesce) {
            *quiesce_result_out = ENGINE_VERSION_COHORT_QUIESCE_REFUSED;
            return version_cohort_failed(lease, ENGINE_VERSION_COHORT_BUSY, lease_out);
        }
        ...
        engine_version_cohort_quiesce_result_t quiesce_result = quiesce(quiesce_context);
        *quiesce_result_out = quiesce_result;
        if (quiesce_result == ENGINE_VERSION_COHORT_QUIESCE_REFUSED) {
            return version_cohort_failed(lease, ENGINE_VERSION_COHORT_BUSY, lease_out);
        }
        if (quiesce_result == ENGINE_VERSION_COHORT_QUIESCE_ERROR) {
            return version_cohort_failed(lease, ENGINE_VERSION_COHORT_IO, lease_out);
        }
        if (quiesce_result != ENGINE_VERSION_COHORT_QUIESCE_REQUESTED) {
            return version_cohort_failed(lease, ENGINE_VERSION_COHORT_UNSAFE, lease_out);
        }
        lock_status =
            version_cohort_lock_until(manager, VERSION_COHORT_LIFETIME_FILE,
                                      ENGINE_PRIVATE_FILE_LOCK_EX, deadline_ms, &lease->lifetime);
    }
    if (lock_status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        return version_cohort_failed(lease, version_cohort_status_from_lock(lock_status),
                                     lease_out);
...
engine_version_cohort_status_t engine_version_cohort_reserve_exclusive(
    engine_version_cohort_manager_t *manager, uint64_t deadline_ms,
    engine_version_cohort_lease_t **lease_out) {
    engine_version_cohort_quiesce_result_t ignored_quiesce;
    return version_cohort_reserve_for_mutation_internal(manager, deadline_ms, NULL, NULL,
                                                        &ignored_quiesce, lease_out, false);
}

static engine_version_cohort_maintenance_presence_t version_cohort_maintenance_presence_internal(
    engine_version_cohort_manager_t *manager, bool terminal_observer) {
    if (!manager || !version_cohort_manager_register(manager)) {
        return ENGINE_VERSION_COHORT_MAINTENANCE_UNSAFE;
    }

    engine_version_cohort_maintenance_presence_t presence = ENGINE_VERSION_COHORT_MAINTENANCE_IO;
    engine_private_file_lock_t *observer = NULL;
    engine_private_file_lock_status_t status = engine_private_file_lock_try_acquire(
        manager->directory, VERSION_COHORT_MAINTENANCE_FILE, ENGINE_PRIVATE_FILE_LOCK_SH, &observer);
    if (status == ENGINE_PRIVATE_FILE_LOCK_OK && observer) {
        presence = ENGINE_VERSION_COHORT_MAINTENANCE_ABSENT;
    } else if (status == ENGINE_PRIVATE_FILE_LOCK_BUSY) {
        presence = ENGINE_VERSION_COHORT_MAINTENANCE_REQUESTED;
    } else if (status == ENGINE_PRIVATE_FILE_LOCK_UNSAFE) {
        presence = ENGINE_VERSION_COHORT_MAINTENANCE_UNSAFE;
    }

    engine_private_file_lock_status_t observer_release = version_cohort_lock_release_until(
        &observer, version_cohort_deadline_after(VERSION_COHORT_CLEANUP_TIMEOUT_MS));
    if (observer_release != ENGINE_PRIVATE_FILE_LOCK_OK) {
        presence = ENGINE_VERSION_COHORT_MAINTENANCE_IO;
    }
    if (observer) {
        if (terminal_observer) {
            ...
            return ENGINE_VERSION_COHORT_MAINTENANCE_IO;
        }
...
engine_version_cohort_status_t engine_version_cohort_daemon_claim_acquire(
    ...
    engine_version_cohort_daemon_claim_t *claim = calloc(1, sizeof(*claim));
    ...
    engine_private_file_lock_status_t lock_status = engine_private_file_lock_try_acquire(
        manager->directory, VERSION_COHORT_DAEMON_FILE, ENGINE_PRIVATE_FILE_LOCK_EX, &claim->marker);
    if (lock_status == ENGINE_PRIVATE_FILE_LOCK_OK) {
        *claim_out = claim;
        return ENGINE_VERSION_COHORT_OK;
    }

    engine_private_file_lock_status_t cleanup_status = version_cohort_lock_release_until(
    ...
    free(claim);
    return cleanup_status == ENGINE_PRIVATE_FILE_LOCK_OK && unregistered
...
engine_private_file_lock_status_t engine_version_cohort_daemon_claim_release(
    ...
    engine_version_cohort_daemon_claim_t *claim = *claim_io;
    engine_private_file_lock_status_t result = ENGINE_PRIVATE_FILE_LOCK_OK;
    if (claim->marker) {
        result = engine_private_file_lock_release(&claim->marker);
    }
    if (claim->marker) {
        return ENGINE_PRIVATE_FILE_LOCK_IO;
    }
...
engine_version_cohort_daemon_presence_t engine_version_cohort_daemon_claim_presence(
    engine_version_cohort_manager_t *manager) {
    if (!manager || !version_cohort_manager_register(manager)) {
        return ENGINE_VERSION_COHORT_DAEMON_UNSAFE;
    }

    engine_version_cohort_daemon_presence_t presence = ENGINE_VERSION_COHORT_DAEMON_IO;
    engine_private_file_lock_t *marker = NULL;
    engine_private_file_lock_status_t marker_status = engine_private_file_lock_try_acquire(
        manager->directory, VERSION_COHORT_DAEMON_FILE, ENGINE_PRIVATE_FILE_LOCK_EX, &marker);
    if (marker_status == ENGINE_PRIVATE_FILE_LOCK_BUSY) {
        presence = ENGINE_VERSION_COHORT_DAEMON_COORDINATED;
    } else if (marker_status == ENGINE_PRIVATE_FILE_LOCK_OK && marker) {
        presence = ENGINE_VERSION_COHORT_DAEMON_ABSENT;
    } else if (marker_status == ENGINE_PRIVATE_FILE_LOCK_UNSAFE) {
        presence = ENGINE_VERSION_COHORT_DAEMON_UNSAFE;
    }

    engine_private_file_lock_status_t marker_release = version_cohort_lock_release_until(
        &marker, version_cohort_deadline_after(VERSION_COHORT_CLEANUP_TIMEOUT_MS));
    if (marker_release != ENGINE_PRIVATE_FILE_LOCK_OK) {
        presence = ENGINE_VERSION_COHORT_DAEMON_IO;
    }
    if (marker) {
...
static engine_version_cohort_daemon_presence_t version_cohort_active_daemon_presence(
    ...
    if (marker_status == ENGINE_PRIVATE_FILE_LOCK_BUSY) {
        presence = ENGINE_VERSION_COHORT_DAEMON_COORDINATED;
    } else if (marker_status == ENGINE_PRIVATE_FILE_LOCK_OK) {
        if (!transition) {
            presence = ENGINE_VERSION_COHORT_DAEMON_UNCOORDINATED;
        } else {
            ...
            int lifetime = engine_daemon_ipc_local_transition_lifetime_probe(endpoint, transition);
            presence = lifetime == 0   ? ENGINE_VERSION_COHORT_DAEMON_ABSENT
                       : lifetime == 1 ? ENGINE_VERSION_COHORT_DAEMON_UNCOORDINATED
                                       : ENGINE_VERSION_COHORT_DAEMON_UNSAFE;
        }
    } else if (marker_status == ENGINE_PRIVATE_FILE_LOCK_UNSAFE) {
        presence = ENGINE_VERSION_COHORT_DAEMON_UNSAFE;
    }
...
engine_version_cohort_daemon_presence_t engine_version_cohort_daemon_presence(
    ...
    engine_version_cohort_daemon_presence_t presence = ENGINE_VERSION_COHORT_DAEMON_IO;
    engine_version_cohort_daemon_presence_t claim = engine_version_cohort_daemon_claim_presence(manager);
    if (claim == ENGINE_VERSION_COHORT_DAEMON_COORDINATED) {
        presence = claim;
    } else if (claim != ENGINE_VERSION_COHORT_DAEMON_ABSENT) {
        presence = claim;
    } else {
        int lifetime = engine_daemon_ipc_lifetime_reservation_probe(endpoint);
        if (lifetime == 0) {
#ifdef _WIN32
            int legacy = engine_daemon_ipc_legacy_generation_probe(endpoint);
            if (legacy == 0) {
                presence = ENGINE_VERSION_COHORT_DAEMON_ABSENT;
            } else if (legacy == 1) {
                presence = ENGINE_VERSION_COHORT_DAEMON_UNCOORDINATED;
            ...
            if (startup_status == 0) {
                ...
                int cleanup = engine_daemon_ipc_stale_generation_cleanup(endpoint, startup);
                if (cleanup == 1) {
                    presence = ENGINE_VERSION_COHORT_DAEMON_ABSENT;
                } else if (cleanup == 0) {
                    presence = ENGINE_VERSION_COHORT_DAEMON_UNCOORDINATED;
...
engine_version_cohort_daemon_presence_t engine_version_cohort_daemon_presence_under_transition(
    ...
    int transition_status = engine_daemon_ipc_local_transition_lifetime_probe(endpoint, transition);
    ...
    engine_version_cohort_daemon_presence_t presence =
        transition_status < 0
            ? ENGINE_VERSION_COHORT_DAEMON_UNSAFE
...
engine_private_file_lock_status_t engine_version_cohort_manager_free(
    ...
    engine_version_cohort_manager_t *manager = *manager_io;
    engine_mutex_lock(&manager->mutex);
    ...
    engine_mutex_unlock(&manager->mutex);
    if (!can_free) {
        return leases_active ? ENGINE_PRIVATE_FILE_LOCK_BUSY : ENGINE_PRIVATE_FILE_LOCK_IO;
    }
    engine_private_lock_directory_close(manager->directory);
    manager->directory = NULL;
    engine_mutex_destroy(&manager->mutex);
    free(manager);
    *manager_io = NULL;
    return ENGINE_PRIVATE_FILE_LOCK_OK;
}
...
bool engine_version_cohort_log_conflict(const engine_daemon_conflict_t *conflict) {
    ...
    FILE *seed =
        engine_daemon_ipc_private_log_open(logs, "daemon-conflicts.ndjson", VERSION_COHORT_LOG_CAP);
    if (!seed) {
        return false;
    }
    bool seeded = fclose(seed) == 0;
    return seeded && engine_daemon_conflict_log_append(path, conflict, VERSION_COHORT_LOG_CAP);
}
...
bool engine_version_cohort_log_uncoordinated_daemon(const engine_daemon_build_identity_t *requested) {
    ...
    engine_daemon_conflict_t conflict = {
        .status = ENGINE_DAEMON_HELLO_BUILD_CONFLICT,
    };
    ...
    return engine_version_cohort_log_conflict(&conflict);
}
