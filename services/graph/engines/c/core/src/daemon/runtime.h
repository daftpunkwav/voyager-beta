/*
 * runtime.h — Mandatory per-account engine daemon runtime.
 *
 ...
 */
#ifndef ENGINE_DAEMON_RUNTIME_H
#define ENGINE_DAEMON_RUNTIME_H
...
#define ENGINE_DAEMON_RUNTIME_WIRE_ABI 1U
...
#define ENGINE_DAEMON_RENDEZVOUS_ABI 1U
#define ENGINE_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP 64U
#define ENGINE_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP 65U
#define ENGINE_DAEMON_RENDEZVOUS_MESSAGE_CAP 512U
#define ENGINE_DAEMON_RENDEZVOUS_REQUEST_SIZE 133U
#define ENGINE_DAEMON_RENDEZVOUS_RESPONSE_SIZE 798U
...
#define ENGINE_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE 137U
#define ENGINE_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE 24U
...
#define ENGINE_DAEMON_CONTROL_REQUEST_SIZE 65U
#define ENGINE_DAEMON_CONTROL_CLIENT_CAP 8U
#define ENGINE_DAEMON_STATUS_RESPONSE_SIZE 121U
#define ENGINE_DAEMON_STOP_RESPONSE_SIZE 40U
#define ENGINE_DAEMON_RUNTIME_PROJECT_KEY_MAX 4096U
#define ENGINE_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX (ENGINE_DAEMON_MAX_FRAME_SIZE - 16U)
...
typedef enum {
    ...
    ENGINE_DAEMON_RUNTIME_OP_HEARTBEAT = 2,
    ENGINE_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE = 3,
    ENGINE_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE = 4,
    ENGINE_DAEMON_RUNTIME_OP_DISCONNECT = 5,
    ENGINE_DAEMON_RUNTIME_OP_APPLICATION_REQUEST = 6,
    ENGINE_DAEMON_RUNTIME_OP_APPLICATION_CANCEL = 7,
    ...
    ENGINE_DAEMON_RUNTIME_OP_CLOSE_INTENT = 9,
    ...
    ENGINE_DAEMON_RUNTIME_OP_STATUS = 10,
    ...
    ENGINE_DAEMON_RUNTIME_OP_STOP = 11,
} engine_daemon_runtime_operation_t;
...
} engine_daemon_runtime_activation_action_t;

typedef struct {
    bool accepted;
    uint64_t active_clients;
    uint64_t active_connections;
} engine_daemon_runtime_activation_result_t;

typedef struct engine_daemon_runtime_service engine_daemon_runtime_service_t;
typedef struct engine_daemon_runtime_client engine_daemon_runtime_client_t;

typedef enum {
    ENGINE_DAEMON_RUNTIME_SERVICE_STARTING = 1,
    ENGINE_DAEMON_RUNTIME_SERVICE_RUNNING = 2,
    ENGINE_DAEMON_RUNTIME_SERVICE_STOPPING = 3,
    ENGINE_DAEMON_RUNTIME_SERVICE_EXITED = 4,
} engine_daemon_runtime_service_state_t;
...
typedef enum {
    ENGINE_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR = 0,
    ENGINE_DAEMON_RUNTIME_APPLICATION_OK = 1,
    ENGINE_DAEMON_RUNTIME_APPLICATION_BUSY = 2,
    ENGINE_DAEMON_RUNTIME_APPLICATION_UNAVAILABLE = 3,
    ENGINE_DAEMON_RUNTIME_APPLICATION_REJECTED = 4,
    ENGINE_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR = 5,
    ENGINE_DAEMON_RUNTIME_APPLICATION_CANCELLED = 6,
} engine_daemon_runtime_application_status_t;

typedef uint64_t engine_daemon_runtime_application_token_t;
#define ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID UINT64_C(0)

typedef enum {
    ENGINE_DAEMON_RUNTIME_CANCEL_ERROR = -1,
    ENGINE_DAEMON_RUNTIME_CANCEL_STALE = 0,
    ENGINE_DAEMON_RUNTIME_CANCEL_ACCEPTED = 1,
} engine_daemon_runtime_cancel_result_t;

typedef void engine_daemon_runtime_application_session_t;

typedef engine_daemon_runtime_application_session_t *(*engine_daemon_runtime_application_session_open_fn)(
...
typedef engine_daemon_runtime_application_status_t (*engine_daemon_runtime_application_request_fn)(
    void *context, engine_daemon_runtime_application_session_t *session,
...
typedef void (*engine_daemon_runtime_application_request_cancel_fn)(
    void *context, engine_daemon_runtime_application_session_t *session,
...
typedef void (*engine_daemon_runtime_application_session_cancel_fn)(
    void *context, engine_daemon_runtime_application_session_t *session);
typedef void (*engine_daemon_runtime_application_session_close_fn)(
    void *context, engine_daemon_runtime_application_session_t *session);

typedef struct {
    void *context;
    engine_daemon_runtime_application_session_open_fn session_open;
    engine_daemon_runtime_application_request_fn request;
    engine_daemon_runtime_application_request_cancel_fn request_cancel;
    engine_daemon_runtime_application_session_cancel_fn session_cancel;
    engine_daemon_runtime_application_session_close_fn session_close;
} engine_daemon_runtime_application_callbacks_t;
...
typedef struct {
     ...
     * and therefore must be finite, not ENGINE_DAEMON_IPC_WAIT_FOREVER. */
    uint32_t max_clients;
    uint64_t lease_timeout_ms;
    uint32_t request_timeout_ms;
    /* Bounds cooperative stop and retained participant-guard cleanup. Must be
     * finite, not ENGINE_DAEMON_IPC_WAIT_FOREVER. */
    ...
    bool permanent;
} engine_daemon_runtime_service_config_t;

typedef enum {
    ENGINE_DAEMON_RUNTIME_CONNECT_ERROR = 0,
    ENGINE_DAEMON_RUNTIME_CONNECT_ACCEPTED = 1,
    ENGINE_DAEMON_RUNTIME_CONNECT_CONFLICT = 2,
    ENGINE_DAEMON_RUNTIME_CONNECT_REJECTED = 3,
} engine_daemon_runtime_connect_status_t;

typedef struct {
    engine_daemon_runtime_connect_status_t status;
    engine_daemon_hello_status_t hello_status;
...
} engine_daemon_runtime_connect_result_t;
...
typedef struct {
    ...
    uint32_t client_pids[ENGINE_DAEMON_CONTROL_CLIENT_CAP];
    char build_fingerprint[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE];
    char semantic_version[12];
} engine_daemon_runtime_status_t;
...
typedef struct {
    ...
    uint32_t client_pids[ENGINE_DAEMON_CONTROL_CLIENT_CAP];
} engine_daemon_runtime_stop_result_t;
 ...
 * Client exchange timeouts must be finite, not ENGINE_DAEMON_IPC_WAIT_FOREVER. */
