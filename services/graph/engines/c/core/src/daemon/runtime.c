...
#ifdef ENGINE_ENABLE_TEST_SEAMS
...
void engine_daemon_runtime_force_peer_image_unverified_for_testing(bool force) {
...
enum {
    ...
    RENDEZVOUS_REQUEST_BUILD_OFFSET =
        RENDEZVOUS_REQUEST_VERSION_OFFSET + ENGINE_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
    ...
    RENDEZVOUS_RESPONSE_ACTIVE_BUILD_OFFSET =
        RENDEZVOUS_RESPONSE_ACTIVE_VERSION_OFFSET + ENGINE_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
    RENDEZVOUS_RESPONSE_REQUESTED_VERSION_OFFSET =
        RENDEZVOUS_RESPONSE_ACTIVE_BUILD_OFFSET + ENGINE_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
    RENDEZVOUS_RESPONSE_REQUESTED_BUILD_OFFSET =
        RENDEZVOUS_RESPONSE_REQUESTED_VERSION_OFFSET + ENGINE_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
    RENDEZVOUS_RESPONSE_MESSAGE_OFFSET =
        RENDEZVOUS_RESPONSE_REQUESTED_BUILD_OFFSET + ENGINE_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
...
_Static_assert(RENDEZVOUS_REQUEST_BUILD_OFFSET + ENGINE_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP ==
                   ENGINE_DAEMON_RENDEZVOUS_REQUEST_SIZE,
               "rendezvous request layout changed");
_Static_assert(RENDEZVOUS_RESPONSE_MESSAGE_OFFSET + ENGINE_DAEMON_RENDEZVOUS_MESSAGE_CAP ==
                   ENGINE_DAEMON_RENDEZVOUS_RESPONSE_SIZE,
               "rendezvous response layout changed");
_Static_assert(ACTIVATION_REQUEST_IDENTITY_OFFSET + ENGINE_DAEMON_RENDEZVOUS_REQUEST_SIZE ==
                       ENGINE_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE &&
                   ACTIVATION_RESPONSE_CONNECTIONS_OFFSET + 8 ==
                       ENGINE_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE,
               "activation shutdown layout changed");
_Static_assert(ENGINE_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP == ENGINE_DAEMON_VERSION_TEXT_SIZE,
               "service version capacity requires a new rendezvous strategy");
_Static_assert(ENGINE_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP == ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE,
               "service fingerprint capacity requires a new rendezvous strategy");
_Static_assert(ENGINE_DAEMON_RENDEZVOUS_MESSAGE_CAP == ENGINE_DAEMON_CONFLICT_MESSAGE_SIZE,
               "service message capacity requires a new rendezvous strategy");
_Static_assert(ENGINE_DAEMON_RUNTIME_CONNECT_ERROR == 0 && ENGINE_DAEMON_RUNTIME_CONNECT_ACCEPTED == 1 &&
                   ENGINE_DAEMON_RUNTIME_CONNECT_CONFLICT == 2 &&
                   ENGINE_DAEMON_RUNTIME_CONNECT_REJECTED == 3,
               "rendezvous connect status values changed");
_Static_assert(ENGINE_DAEMON_HELLO_INVALID == 0 && ENGINE_DAEMON_HELLO_COMPATIBLE == 1 &&
                   ENGINE_DAEMON_HELLO_VERSION_CONFLICT == 2 && ENGINE_DAEMON_HELLO_BUILD_CONFLICT == 3 &&
                   ENGINE_DAEMON_HELLO_PROTOCOL_ABI_CONFLICT == 4 &&
                   ENGINE_DAEMON_HELLO_STORE_ABI_CONFLICT == 5 &&
                   ENGINE_DAEMON_HELLO_FEATURE_ABI_CONFLICT == 6,
               "rendezvous hello status values changed");
_Static_assert(ENGINE_DAEMON_RENDEZVOUS_FRAME_VERSION == 1 && ENGINE_DAEMON_RENDEZVOUS_ABI == 1 &&
                   ENGINE_DAEMON_FRAME_REQUEST == 1 && ENGINE_DAEMON_FRAME_RESPONSE == 2 &&
                   ENGINE_DAEMON_RUNTIME_OP_HELLO == 1 &&
                   ENGINE_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN == 8 &&
                   ENGINE_DAEMON_RUNTIME_ACTIVATION_INSTALL == 1 &&
                   ENGINE_DAEMON_RUNTIME_ACTIVATION_UPDATE == 2 &&
                   ENGINE_DAEMON_RUNTIME_ACTIVATION_UNINSTALL == 3,
               "rendezvous framing values changed");

typedef struct engine_daemon_runtime_worker engine_daemon_runtime_worker_t;
...
struct engine_daemon_runtime_service {
    engine_mutex_t mutex;
    engine_daemon_ipc_listener_t *listener;
    engine_daemon_coordinator_t *coordinator;
    engine_daemon_runtime_worker_t *workers;
    ...
    engine_thread_t accept_thread;
    bool accept_thread_started;
    bool accept_thread_joined;
    atomic_bool accept_thread_done;

    engine_daemon_runtime_service_state_t state;
    uint64_t stop_deadline_ms;
    bool emergency_stop;
    bool activation_shutdown_requested;
    bool activation_response_inflight;

    char semantic_version[ENGINE_DAEMON_VERSION_TEXT_SIZE];
    char build_fingerprint[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE];
    ...
    engine_daemon_runtime_application_callbacks_t application;
    /* Owned only by the convenience start() path. start_reserved() callers
     * retain their externally managed participant guard. */
    engine_daemon_ipc_participant_guard_t *owned_participant_guard;
};

struct engine_daemon_runtime_worker {
    engine_daemon_runtime_service_t *service;
    engine_mutex_t send_mutex;
    engine_thread_t thread;
    bool thread_started;
    bool in_use;
    bool joining;
    atomic_bool done;
    engine_daemon_ipc_connection_t *connection;
    ...
    engine_daemon_runtime_application_session_t *application_session;
    bool application_session_opened;
    bool application_cancelled;
    atomic_bool disconnecting;

    engine_thread_t application_thread;
    bool application_thread_started;
    atomic_bool application_thread_done;
    engine_daemon_runtime_application_token_t application_request_token;
    engine_daemon_runtime_application_token_t last_application_request_token;
    uint8_t *application_request;
    uint32_t application_request_length;
};

struct engine_daemon_runtime_client {
    engine_mutex_t exchange_mutex;
    engine_mutex_t send_mutex;
    engine_mutex_t state_mutex;
    engine_daemon_ipc_connection_t *connection;
    engine_daemon_client_id_t client_id;
    ...
    engine_daemon_runtime_application_token_t next_application_token;
    engine_daemon_runtime_application_token_t last_started_application_token;
    engine_daemon_runtime_application_token_t active_application_token;
    engine_daemon_runtime_application_token_t pending_cancel_token;
    bool application_request_sent;
    bool application_cancel_sent;
};

static uint64_t runtime_deadline_after(uint32_t timeout_ms) {
    uint64_t now_ms = engine_now_ms();
...
static void runtime_wait_tick(uint64_t deadline_ms) {
    ...
    (void)engine_nanosleep(&pause, NULL);
}
...
static engine_daemon_hello_status_t runtime_rendezvous_compare(const char *active_version,
                                                            const char *active_build,
                                                            const char *requested_version,
                                                            const char *requested_build,
                                                            engine_daemon_conflict_t *conflict_out) {
    engine_daemon_build_identity_t active = {
    ...
    engine_daemon_build_identity_t requested = {
    ...
    engine_daemon_hello_status_t status = engine_daemon_hello_compare(&active, &requested, conflict_out);
    if (status != ENGINE_DAEMON_HELLO_INVALID && status != ENGINE_DAEMON_HELLO_COMPATIBLE &&
        status != ENGINE_DAEMON_HELLO_VERSION_CONFLICT && status != ENGINE_DAEMON_HELLO_BUILD_CONFLICT) {
        if (conflict_out) {
            memset(conflict_out, 0, sizeof(*conflict_out));
            conflict_out->status = ENGINE_DAEMON_HELLO_INVALID;
        }
        return ENGINE_DAEMON_HELLO_INVALID;
    }
    return status;
}

bool engine_daemon_runtime_hello_request_encode(uint8_t out[ENGINE_DAEMON_RENDEZVOUS_REQUEST_SIZE],
                                             const engine_daemon_build_identity_t *identity) {
    if (!out || !identity) {
        return false;
    }
    engine_daemon_conflict_t validation;
    if (runtime_rendezvous_compare(identity->semantic_version, identity->build_fingerprint,
                                   ...
                                   &validation) != ENGINE_DAEMON_HELLO_COMPATIBLE) {
        return false;
    }
    memset(out, 0, ENGINE_DAEMON_RENDEZVOUS_REQUEST_SIZE);
    runtime_put_u32(out + RENDEZVOUS_REQUEST_ABI_OFFSET, ENGINE_DAEMON_RENDEZVOUS_ABI);
    return runtime_wire_string_encode(out + RENDEZVOUS_REQUEST_VERSION_OFFSET,
                                      ENGINE_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
                                      identity->semantic_version, false) &&
           runtime_wire_string_encode(out + RENDEZVOUS_REQUEST_BUILD_OFFSET,
                                      ENGINE_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
                                      identity->build_fingerprint, false);
}

static bool runtime_rendezvous_request_decode(
    const uint8_t payload[ENGINE_DAEMON_RENDEZVOUS_REQUEST_SIZE],
    char version[ENGINE_DAEMON_VERSION_TEXT_SIZE], char build[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    if (!payload || !version || !build ||
        runtime_get_u32(payload + RENDEZVOUS_REQUEST_ABI_OFFSET) != ENGINE_DAEMON_RENDEZVOUS_ABI ||
        !runtime_wire_string_decode(payload + RENDEZVOUS_REQUEST_VERSION_OFFSET,
                                    ENGINE_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP, version, false) ||
        !runtime_wire_string_decode(payload + RENDEZVOUS_REQUEST_BUILD_OFFSET,
                                    ENGINE_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP, build, false)) {
        return false;
    }
    engine_daemon_conflict_t validation;
...
static bool runtime_activation_action_valid(engine_daemon_runtime_activation_action_t action) {
    return action == ENGINE_DAEMON_RUNTIME_ACTIVATION_INSTALL ||
           action == ENGINE_DAEMON_RUNTIME_ACTIVATION_UPDATE ||
           action == ENGINE_DAEMON_RUNTIME_ACTIVATION_UNINSTALL;
}

static const char *runtime_activation_action_text(engine_daemon_runtime_activation_action_t action) {
    switch (action) {
...
static bool runtime_activation_request_encode(
    uint8_t out[ENGINE_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE],
    engine_daemon_runtime_activation_action_t action, const engine_daemon_build_identity_t *identity) {
    if (!out || !identity || !runtime_activation_action_valid(action)) {
        return false;
    }
    memset(out, 0, ENGINE_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE);
    runtime_put_u32(out + ACTIVATION_REQUEST_ACTION_OFFSET, (uint32_t)action);
    return engine_daemon_runtime_hello_request_encode(out + ACTIVATION_REQUEST_IDENTITY_OFFSET,
                                                   identity);
}

static bool runtime_activation_request_decode(
    const uint8_t payload[ENGINE_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE],
    engine_daemon_runtime_activation_action_t *action_out, char version[ENGINE_DAEMON_VERSION_TEXT_SIZE],
    char build[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    if (!payload || !action_out || !version || !build) {
        return false;
    }
    uint32_t raw_action = runtime_get_u32(payload + ACTIVATION_REQUEST_ACTION_OFFSET);
    if (raw_action < (uint32_t)ENGINE_DAEMON_RUNTIME_ACTIVATION_INSTALL ||
        raw_action > (uint32_t)ENGINE_DAEMON_RUNTIME_ACTIVATION_UNINSTALL ||
    ...
    engine_daemon_runtime_activation_action_t action =
        (engine_daemon_runtime_activation_action_t)raw_action;
    *action_out = action;
    return true;
}

static bool runtime_activation_response_encode(
    uint8_t out[ENGINE_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE],
    const engine_daemon_runtime_activation_result_t *result) {
    if (!out || !result) {
        return false;
    }
    memset(out, 0, ENGINE_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE);
    runtime_put_u32(out + ACTIVATION_RESPONSE_ABI_OFFSET, ENGINE_DAEMON_RENDEZVOUS_ABI);
...
static bool runtime_activation_response_decode(
    const uint8_t payload[ENGINE_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE],
    engine_daemon_runtime_activation_result_t *result_out) {
    if (!payload || !result_out ||
        runtime_get_u32(payload + ACTIVATION_RESPONSE_ABI_OFFSET) != ENGINE_DAEMON_RENDEZVOUS_ABI) {
        return false;
    }
...
static bool runtime_process_image_reference_acquire(
    ...
    wchar_t *open_path = ok ? engine_wide_path_to_extended(process_before.path) : NULL;
    ...
    ok = ok && runtime_windows_file_snapshot(file, &file_before, &size_before) &&
         (!fingerprint || engine_daemon_build_fingerprint_native_file((uintptr_t)file, fingerprint)) &&
    ...
    ok = ok && fd >= 0 && fstat(fd, &file_before) == 0 && S_ISREG(file_before.st_mode) &&
         ...
         (!fingerprint || engine_daemon_build_fingerprint_native_file((uintptr_t)fd, fingerprint)) &&
    ...
    bool ok = image_fd >= 0 && fstat(image_fd, &image_before) == 0 &&
              ...
              (!fingerprint ||
               engine_daemon_build_fingerprint_native_file((uintptr_t)image_fd, fingerprint)) &&
...
bool engine_daemon_runtime_process_build_fingerprint(uint64_t process_id,
...
static bool runtime_hello_response_encode(uint8_t out[ENGINE_DAEMON_RENDEZVOUS_RESPONSE_SIZE],
                                          const engine_daemon_runtime_connect_result_t *result) {
    if (!out || !result) {
        return false;
    }
    memset(out, 0, ENGINE_DAEMON_RENDEZVOUS_RESPONSE_SIZE);
    ...
    if (!runtime_wire_string_encode(out + RENDEZVOUS_RESPONSE_ACTIVE_VERSION_OFFSET,
        ...
        !runtime_wire_string_encode(out + RENDEZVOUS_RESPONSE_MESSAGE_OFFSET,
                                    ENGINE_DAEMON_RENDEZVOUS_MESSAGE_CAP, result->message, true)) {
        return false;
    }
    return true;
}
...
static bool runtime_hello_response_decode(
    const uint8_t payload[ENGINE_DAEMON_RENDEZVOUS_RESPONSE_SIZE],
    engine_daemon_runtime_connect_result_t *result) {
    if (!payload || !result) {
        return false;
    }
    memset(result, 0, sizeof(*result));
    result->status = (engine_daemon_runtime_connect_status_t)runtime_get_u32(
        payload + RENDEZVOUS_RESPONSE_CONNECT_STATUS_OFFSET);
    result->hello_status = (engine_daemon_hello_status_t)runtime_get_u32(
    ...
    result->conflict.status = (engine_daemon_hello_status_t)runtime_get_u32(
    ...
    if (result->status == ENGINE_DAEMON_RUNTIME_CONNECT_ACCEPTED) {
        return result->hello_status == ENGINE_DAEMON_HELLO_COMPATIBLE &&
               result->conflict.status == ENGINE_DAEMON_HELLO_COMPATIBLE &&
               result->client_id != ENGINE_DAEMON_CLIENT_ID_INVALID &&
               result->authenticated_process_id != 0;
    }
    if (result->status == ENGINE_DAEMON_RUNTIME_CONNECT_CONFLICT) {
        char expected[ENGINE_DAEMON_RENDEZVOUS_MESSAGE_CAP];
        return result->hello_status >= ENGINE_DAEMON_HELLO_VERSION_CONFLICT &&
               result->hello_status <= ENGINE_DAEMON_HELLO_FEATURE_ABI_CONFLICT &&
               result->conflict.status == result->hello_status &&
               result->client_id == ENGINE_DAEMON_CLIENT_ID_INVALID &&
               result->authenticated_process_id == 0 &&
               engine_daemon_conflict_format(&result->conflict, expected, sizeof(expected)) &&
               strcmp(result->message, expected) == 0;
    }
    return result->status == ENGINE_DAEMON_RUNTIME_CONNECT_REJECTED &&
           result->hello_status == ENGINE_DAEMON_HELLO_INVALID &&
           result->conflict.status == ENGINE_DAEMON_HELLO_INVALID &&
           result->client_id == ENGINE_DAEMON_CLIENT_ID_INVALID &&
           result->authenticated_process_id == 0;
}

static bool runtime_send_hello_response(engine_daemon_ipc_connection_t *connection,
                                        const engine_daemon_runtime_connect_result_t *result) {
    uint8_t payload[ENGINE_DAEMON_RENDEZVOUS_RESPONSE_SIZE];
    return runtime_hello_response_encode(payload, result) &&
           engine_daemon_ipc_send_frame(connection, ENGINE_DAEMON_FRAME_RESPONSE,
                                     ENGINE_DAEMON_RUNTIME_OP_HELLO, payload,
                                     (uint32_t)sizeof(payload));
}

static bool runtime_worker_send_frame(engine_daemon_runtime_worker_t *worker,
                                      engine_daemon_frame_type_t type,
                                      engine_daemon_runtime_operation_t operation, const void *payload,
                                      uint32_t length) {
    if (!worker) {
        return false;
    }
    engine_mutex_lock(&worker->send_mutex);
    bool sent =
        worker->connection &&
        engine_daemon_ipc_send_frame(worker->connection, type, (uint16_t)operation, payload, length);
    engine_mutex_unlock(&worker->send_mutex);
    return sent;
}

static bool runtime_worker_send_status(engine_daemon_runtime_worker_t *worker,
                                       engine_daemon_runtime_operation_t operation, bool success) {
    uint8_t payload[STATUS_RESPONSE_SIZE];
    runtime_put_u32(payload, success ? 1U : 0U);
    return runtime_worker_send_frame(worker, ENGINE_DAEMON_FRAME_RESPONSE, operation, payload,
                                     (uint32_t)sizeof(payload));
}

static bool runtime_application_status_is_callback_result(
    engine_daemon_runtime_application_status_t status) {
    return status == ENGINE_DAEMON_RUNTIME_APPLICATION_OK ||
           status == ENGINE_DAEMON_RUNTIME_APPLICATION_REJECTED ||
           status == ENGINE_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR ||
           status == ENGINE_DAEMON_RUNTIME_APPLICATION_CANCELLED;
}

static bool runtime_worker_send_application_response(
    engine_daemon_runtime_worker_t *worker, engine_daemon_runtime_application_token_t request_token,
    engine_daemon_runtime_application_status_t status, const uint8_t *response,
    uint32_t response_length, bool suppress_when_disconnecting) {
    if (!worker || request_token == ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID ||
        status <= ENGINE_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR ||
        status > ENGINE_DAEMON_RUNTIME_APPLICATION_CANCELLED ||
        response_length > ENGINE_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX ||
        (response_length > 0 && !response)) {
        return false;
    }
    uint64_t wire_length = APPLICATION_RESPONSE_PREFIX_SIZE + (uint64_t)response_length;
    if (wire_length > ENGINE_DAEMON_MAX_FRAME_SIZE || wire_length > UINT32_MAX) {
        return false;
    }
    ...
    bool sent = !disconnected && worker->connection &&
                engine_daemon_ipc_send_frame(worker->connection, ENGINE_DAEMON_FRAME_RESPONSE,
                                          ENGINE_DAEMON_RUNTIME_OP_APPLICATION_REQUEST, wire,
...
static void runtime_result_rejected(engine_daemon_runtime_connect_result_t *result,
                                    const char *message) {
    memset(result, 0, sizeof(*result));
    result->status = ENGINE_DAEMON_RUNTIME_CONNECT_REJECTED;
    result->hello_status = ENGINE_DAEMON_HELLO_INVALID;
    result->conflict.status = ENGINE_DAEMON_HELLO_INVALID;
    (void)snprintf(result->message, sizeof(result->message), "%s", message);
}

static void runtime_service_begin_stopping_locked(engine_daemon_runtime_service_t *service,
                                                  uint64_t deadline, bool emergency,
                                                  const char *reason) {
    if (service->state == ENGINE_DAEMON_RUNTIME_SERVICE_RUNNING) {
        service->state = ENGINE_DAEMON_RUNTIME_SERVICE_STOPPING;
        ...
        engine_log_info("daemon.runtime_stopping", "reason", reason ? reason : "unspecified");
    } else if (service->state == ENGINE_DAEMON_RUNTIME_SERVICE_STOPPING &&
...
static void runtime_service_begin_stopping(engine_daemon_runtime_service_t *service,
                                           uint32_t timeout_ms, bool emergency,
                                           const char *reason) {
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    engine_mutex_lock(&service->mutex);
    runtime_service_begin_stopping_locked(service, deadline, emergency, reason);
    engine_mutex_unlock(&service->mutex);
}

static void runtime_service_interrupt_connections_except(engine_daemon_runtime_service_t *service,
                                                         engine_daemon_runtime_worker_t *except_worker,
    ...
    for (size_t i = 0; i < service->worker_capacity; i++) {
        engine_daemon_runtime_worker_t *worker = &service->workers[i];
        if (worker != except_worker && worker->in_use && worker->connection &&
            (activation_owner || force || !worker->final_response_inflight)) {
            engine_daemon_ipc_connection_interrupt(worker->connection);
        }
    }
    engine_mutex_unlock(&service->mutex);
}

static void runtime_service_interrupt_connections(engine_daemon_runtime_service_t *service) {
    runtime_service_interrupt_connections_except(service, NULL, false);
}

static void runtime_worker_disconnect(engine_daemon_runtime_worker_t *worker) {
    engine_daemon_runtime_service_t *service = worker->service;
    engine_daemon_client_id_t client_id = ENGINE_DAEMON_CLIENT_ID_INVALID;
    ...
    (void)engine_daemon_client_disconnected(service->coordinator, client_id, engine_now_ms());
    ...
    if (engine_daemon_coordinator_state(service->coordinator) == ENGINE_DAEMON_COORDINATOR_STOPPING) {
        runtime_service_begin_stopping(service, service->shutdown_timeout_ms, false,
                                       "coordinator_stopping");
    }
}

static bool runtime_worker_service_running(engine_daemon_runtime_worker_t *worker) {
    engine_daemon_runtime_service_t *service = worker->service;
    engine_mutex_lock(&service->mutex);
    bool running = service->state == ENGINE_DAEMON_RUNTIME_SERVICE_RUNNING;
    engine_mutex_unlock(&service->mutex);
    return running;
}

static bool runtime_worker_admit(engine_daemon_runtime_worker_t *worker,
                                 engine_daemon_client_id_t *client_id_out) {
    engine_daemon_runtime_service_t *service = worker->service;
    engine_daemon_client_id_t client_id = ENGINE_DAEMON_CLIENT_ID_INVALID;
    engine_mutex_lock(&service->mutex);
    if (service->state == ENGINE_DAEMON_RUNTIME_SERVICE_RUNNING) {
        client_id = engine_daemon_client_connected(service->coordinator, engine_now_ms());
        if (client_id != ENGINE_DAEMON_CLIENT_ID_INVALID) {
    ...
    *client_id_out = client_id;
    return client_id != ENGINE_DAEMON_CLIENT_ID_INVALID;
}

static bool runtime_worker_commit_admission(engine_daemon_runtime_worker_t *worker) {
    engine_daemon_runtime_service_t *service = worker->service;
    engine_mutex_lock(&service->mutex);
    bool committed = service->state == ENGINE_DAEMON_RUNTIME_SERVICE_RUNNING && worker->admitted &&
...
static bool runtime_worker_handle_subscribe(engine_daemon_runtime_worker_t *worker,
    ...
    if (key_length == 0 || key_length > ENGINE_DAEMON_RUNTIME_PROJECT_KEY_MAX ||
        length != SUBSCRIBE_REQUEST_PREFIX_SIZE + key_length) {
        return false;
    }
    char project_key[ENGINE_DAEMON_RUNTIME_PROJECT_KEY_MAX + 1];
    ...
    engine_daemon_subscription_id_t subscription_id = ENGINE_DAEMON_SUBSCRIPTION_ID_INVALID;
    engine_daemon_subscription_result_t result = engine_daemon_job_subscribe(
    ...
    return runtime_worker_send_frame(worker, ENGINE_DAEMON_FRAME_RESPONSE,
                                     ENGINE_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE, response,
                                     (uint32_t)sizeof(response));
}

static bool runtime_worker_handle_unsubscribe(engine_daemon_runtime_worker_t *worker,
                                              const uint8_t *payload, uint32_t length) {
    if (!runtime_worker_service_running(worker) || !payload || length != UNSUBSCRIBE_REQUEST_SIZE) {
        return false;
    }
    engine_daemon_subscription_id_t subscription_id = runtime_get_u64(payload);
    bool removed = engine_daemon_job_unsubscribe(worker->service->coordinator, worker->client_id,
                                              subscription_id);
    return runtime_worker_send_status(worker, ENGINE_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE, removed);
}

static void *runtime_application_worker(void *opaque) {
    engine_daemon_runtime_worker_t *worker = opaque;
    engine_daemon_runtime_service_t *service = worker->service;
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    engine_daemon_runtime_application_status_t status = service->application.request(
    ...
    bool valid_response =
        ...
        (status == ENGINE_DAEMON_RUNTIME_APPLICATION_OK || (response == NULL && response_length == 0));
...
static bool runtime_worker_reap_application(engine_daemon_runtime_worker_t *worker, bool wait) {
    ...
    if (engine_thread_join(&worker->application_thread) != 0) {
        return false;
    }
    ...
    worker->application_request_token = ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    atomic_store_explicit(&worker->application_thread_done, false, memory_order_release);
    return true;
}

static bool runtime_worker_handle_application(engine_daemon_runtime_worker_t *worker,
                                              const uint8_t *payload, uint32_t length) {
    if (!payload || length < APPLICATION_REQUEST_PREFIX_SIZE) {
        return false;
    }
    engine_daemon_runtime_application_token_t request_token = runtime_get_u64(payload);
    uint32_t request_length = runtime_get_u32(payload + 8);
    if (request_length > ENGINE_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX ||
        length != APPLICATION_REQUEST_PREFIX_SIZE + request_length ||
        request_token == ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID ||
    ...
    if (!service->application.request) {
        return runtime_worker_send_application_response(
            worker, request_token, ENGINE_DAEMON_RUNTIME_APPLICATION_UNAVAILABLE, NULL, 0, false);
    }
    (void)runtime_worker_reap_application(worker, false);
    if (worker->application_thread_started) {
        return runtime_worker_send_application_response(
            worker, request_token, ENGINE_DAEMON_RUNTIME_APPLICATION_BUSY, NULL, 0, false);
    }
    ...
    if (request_length > 0) {
        ...
        if (!request_copy) {
            return runtime_worker_send_application_response(
                worker, request_token, ENGINE_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR, NULL, 0,
                false);
        }
    ...
    atomic_store_explicit(&worker->application_thread_done, false, memory_order_release);
    if (engine_thread_create(&worker->application_thread, RUNTIME_WORKER_STACK_SIZE,
                          runtime_application_worker, worker) != 0) {
        free(worker->application_request);
        worker->application_request = NULL;
        worker->application_request_length = 0;
        return runtime_worker_send_application_response(
            worker, request_token, ENGINE_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR, NULL, 0, false);
    }
    worker->application_thread_started = true;
    return true;
}

static bool runtime_worker_handle_application_cancel(engine_daemon_runtime_worker_t *worker,
                                                     const uint8_t *payload, uint32_t length) {
    if (!worker || !payload || length != APPLICATION_CANCEL_REQUEST_SIZE) {
        return false;
    }
    engine_daemon_runtime_application_token_t request_token = runtime_get_u64(payload);
    if (request_token == ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID) {
        return false;
    }
...
static bool runtime_worker_handle_close_intent(engine_daemon_runtime_worker_t *worker,
                                               uint32_t length) {
    if (length != 0) {
        return false;
    }
...
static bool runtime_worker_handle_disconnect(engine_daemon_runtime_worker_t *worker, uint32_t length) {
    if (length != 0) {
        return false;
    }
    engine_daemon_runtime_service_t *service = worker->service;
    ...
    engine_mutex_unlock(&service->mutex);
    runtime_worker_disconnect(worker);
    bool sent = runtime_worker_send_status(worker, ENGINE_DAEMON_RUNTIME_OP_DISCONNECT, true);
    engine_mutex_lock(&service->mutex);
...
static void runtime_worker_finish(engine_daemon_runtime_worker_t *worker) {
    engine_daemon_runtime_service_t *service = worker->service;
    runtime_worker_disconnect(worker);
    ...
    if (worker->connection) {
         ...
         * it before the handle closes; see engine_daemon_ipc_connection_drain. */
        engine_daemon_ipc_connection_drain(worker->connection, RUNTIME_WORKER_DRAIN_TIMEOUT_MS);
    }
    engine_mutex_lock(&service->mutex);
    if (worker->connection) {
        engine_daemon_ipc_connection_close(worker->connection);
...
static bool runtime_activation_peer_matches_claim(engine_daemon_runtime_service_t *service,
    ...
    char peer_fingerprint[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE];
    return engine_daemon_runtime_process_build_fingerprint(process_id, peer_fingerprint) &&
           strcmp(peer_fingerprint, claimed_build) == 0;
}

static bool runtime_control_request_decode(const uint8_t *payload, uint32_t length,
                                           char build_out[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    if (!payload || length != ENGINE_DAEMON_CONTROL_REQUEST_SIZE ||
        payload[ENGINE_DAEMON_CONTROL_REQUEST_SIZE - 1U] != 0U) {
        return false;
    }
    memcpy(build_out, payload, ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE);
    return strlen(build_out) == ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE - 1U;
}

static void runtime_control_collect_clients_locked(engine_daemon_runtime_service_t *service,
                                                   engine_daemon_runtime_worker_t *except,
                                                   uint8_t *count_out,
                                                   uint32_t pids[ENGINE_DAEMON_CONTROL_CLIENT_CAP]) {
    uint8_t count = 0U;
    for (size_t index = 0; index < service->worker_capacity; index++) {
        engine_daemon_runtime_worker_t *candidate = &service->workers[index];
        if (!candidate->in_use || !candidate->admission_committed || candidate == except) {
            continue;
        }
        if (count < ENGINE_DAEMON_CONTROL_CLIENT_CAP) {
            pids[count] = (uint32_t)candidate->peer_process_id;
        }
        count++;
    }
    *count_out =
        count > ENGINE_DAEMON_CONTROL_CLIENT_CAP ? (uint8_t)ENGINE_DAEMON_CONTROL_CLIENT_CAP : count;
}

static void runtime_worker_handle_status(engine_daemon_runtime_worker_t *worker,
                                         const uint8_t *payload, uint32_t length) {
    engine_daemon_runtime_service_t *service = worker->service;
    char requested_build[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool peer_verified =
    ...
    uint8_t response[ENGINE_DAEMON_STATUS_RESPONSE_SIZE];
    memset(response, 0, sizeof(response));
    if (peer_verified) {
        uint8_t count = 0U;
        uint32_t pids[ENGINE_DAEMON_CONTROL_CLIENT_CAP] = {0};
        engine_mutex_lock(&service->mutex);
        response[0] = 1U;
        response[1] =
            (uint8_t)((service->permanent ? 0x01U : 0U) |
                      (service->state != ENGINE_DAEMON_RUNTIME_SERVICE_RUNNING ? 0x02U : 0U));
        ...
        response[8] = count;
        for (size_t index = 0; index < ENGINE_DAEMON_CONTROL_CLIENT_CAP; index++) {
        ...
        (void)snprintf((char *)response + 44U, ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE, "%s",
    ...
    (void)runtime_worker_send_frame(worker, ENGINE_DAEMON_FRAME_RESPONSE, ENGINE_DAEMON_RUNTIME_OP_STATUS,
                                    response, (uint32_t)sizeof(response));
    runtime_worker_finish(worker);
}

static void runtime_worker_handle_stop(engine_daemon_runtime_worker_t *worker, const uint8_t *payload,
                                       uint32_t length) {
    engine_daemon_runtime_service_t *service = worker->service;
    char requested_build[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool peer_verified =
        runtime_control_request_decode(payload, length, requested_build) &&
        runtime_activation_peer_matches_claim(service, worker->peer_process_id, requested_build);
    uint8_t response[ENGINE_DAEMON_STOP_RESPONSE_SIZE];
    memset(response, 0, sizeof(response));
    bool accepted = false;
    if (peer_verified) {
        uint8_t count = 0U;
        ...
        if (service->committed_clients > 0) {
            ...
            response[1] = 0x02U;
        } else if (service->state == ENGINE_DAEMON_RUNTIME_SERVICE_RUNNING) {
    ...
    if (accepted) {
        ...
        engine_log_info("daemon.stop_requested", "requester_pid", requester_pid, "requester_build",
                     requested_build);
        runtime_service_interrupt_connections_except(worker->service, worker, true);
    }
    (void)runtime_worker_send_frame(worker, ENGINE_DAEMON_FRAME_RESPONSE, ENGINE_DAEMON_RUNTIME_OP_STOP,
                                    response, (uint32_t)sizeof(response));
    if (accepted) {
        engine_daemon_runtime_service_t *stop_service = worker->service;
        runtime_worker_finish(worker);
        engine_mutex_lock(&stop_service->mutex);
        worker->final_response_inflight = false;
        stop_service->activation_response_inflight = false;
        engine_mutex_unlock(&stop_service->mutex);
...
static void runtime_worker_handle_activation_shutdown(engine_daemon_runtime_worker_t *worker,
                                                      const uint8_t *payload, uint32_t length) {
    engine_daemon_runtime_service_t *service = worker->service;
    engine_daemon_runtime_activation_result_t result = {0};
    engine_daemon_runtime_activation_action_t action = 0;
    char requested_version[ENGINE_DAEMON_VERSION_TEXT_SIZE] = {0};
    char requested_build[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool request_valid =
        length == ENGINE_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE && payload &&
    ...
    uint8_t response[ENGINE_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE];
    ...
    if (result.accepted) {
        ...
        engine_log_info("daemon.activation_shutdown", "requester_pid", requester_pid,
    ...
    bool response_sent =
        encoded && runtime_worker_send_frame(worker, ENGINE_DAEMON_FRAME_RESPONSE,
                                             ENGINE_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN, response,
                                             (uint32_t)sizeof(response));
    if (result.accepted && !response_sent) {
        engine_log_error("daemon.activation_shutdown_ack_failed", "shutdown", "accepted",
                      "requester_build", requested_build, "action",
...
static void *runtime_connection_worker(void *opaque) {
    engine_daemon_runtime_worker_t *worker = opaque;
    engine_daemon_runtime_service_t *service = worker->service;
    engine_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    char requested_version[ENGINE_DAEMON_VERSION_TEXT_SIZE];
    char requested_build[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE];

    int received = engine_daemon_ipc_receive_frame_bounded(
        worker->connection, service->request_timeout_ms,
        ENGINE_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE, &frame, &payload);
    if (received != 1 || frame.type != ENGINE_DAEMON_FRAME_REQUEST) {
        free(payload);
        runtime_worker_finish(worker);
        return NULL;
    }
    if (frame.flags == ENGINE_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN) {
        runtime_worker_handle_activation_shutdown(worker, payload, frame.length);
        free(payload);
        return NULL;
    }
    if (frame.flags == ENGINE_DAEMON_RUNTIME_OP_STATUS) {
        runtime_worker_handle_status(worker, payload, frame.length);
        free(payload);
        return NULL;
    }
    if (frame.flags == ENGINE_DAEMON_RUNTIME_OP_STOP) {
        runtime_worker_handle_stop(worker, payload, frame.length);
        free(payload);
        return NULL;
    }
    if (frame.flags != ENGINE_DAEMON_RUNTIME_OP_HELLO ||
        frame.length != ENGINE_DAEMON_RENDEZVOUS_REQUEST_SIZE ||
    ...
    engine_daemon_runtime_connect_result_t hello_result;
    memset(&hello_result, 0, sizeof(hello_result));
    engine_daemon_hello_status_t hello_status = runtime_rendezvous_compare(
    ...
    if (hello_status != ENGINE_DAEMON_HELLO_COMPATIBLE) {
        if (hello_status == ENGINE_DAEMON_HELLO_INVALID) {
            runtime_worker_finish(worker);
            return NULL;
        }
        hello_result.status = ENGINE_DAEMON_RUNTIME_CONNECT_CONFLICT;
        hello_result.hello_status = hello_status;
        (void)engine_daemon_conflict_format(&hello_result.conflict, hello_result.message,
                                         sizeof(hello_result.message));
        if (!engine_daemon_conflict_log_append(service->conflict_log_path, &hello_result.conflict,
            ...
            engine_log_error("daemon.conflict_log_append_failed", "path", service->conflict_log_path);
        }
    ...
    if (!peer_image_verified) {
        ...
        peer_image_fingerprinted =
            engine_daemon_runtime_process_build_fingerprint(worker->peer_process_id, peer_fingerprint);
    ...
    if (!peer_image_verified) {
        ...
        engine_log_error("daemon.client_image_rejected", "reason", reason);
        ...
        runtime_result_rejected(&hello_result, "engine daemon rejected this client's binary image");
        (void)snprintf(hello_result.message, sizeof(hello_result.message),
                       "engine daemon rejected this client: %s. The client binary must match the "
                       "running daemon's build; close engine sessions (or run 'daemon stop') and "
    ...
    if (!runtime_worker_admit(worker, &client_id)) {
        runtime_result_rejected(&hello_result, "engine daemon is stopping");
        (void)runtime_send_hello_response(worker->connection, &hello_result);
        runtime_worker_finish(worker);
        return NULL;
    }
    if (service->application.request) {
        ...
        if (!worker->application_session) {
            runtime_result_rejected(&hello_result, "engine daemon session initialization failed");
            engine_mutex_lock(&service->mutex);
            worker->final_response_inflight = true;
            engine_mutex_unlock(&service->mutex);
            runtime_worker_disconnect(worker);
    ...
    hello_result.status = ENGINE_DAEMON_RUNTIME_CONNECT_ACCEPTED;
    hello_result.hello_status = ENGINE_DAEMON_HELLO_COMPATIBLE;
    hello_result.conflict.status = ENGINE_DAEMON_HELLO_COMPATIBLE;
    ...
    while (keep_running && runtime_worker_service_running(worker)) {
        ...
        payload = NULL;
        received = engine_daemon_ipc_receive_frame(worker->connection, ENGINE_DAEMON_IPC_WAIT_FOREVER,
                                                &frame, &payload);
        if (received != 1 || frame.type != ENGINE_DAEMON_FRAME_REQUEST) {
            ...
            engine_log_info("daemon.connection_end", "received", received_text, "frame_type",
                         received == 1 ? "non_request" : "none");
            break;
        }
        switch (frame.flags) {
        case ENGINE_DAEMON_RUNTIME_OP_HEARTBEAT: {
            bool valid = frame.length == 0 &&
                         engine_daemon_client_heartbeat(service->coordinator, client_id, engine_now_ms());
            keep_running =
                valid && runtime_worker_send_status(worker, ENGINE_DAEMON_RUNTIME_OP_HEARTBEAT, true);
            break;
        }
        case ENGINE_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE:
            keep_running = runtime_worker_handle_subscribe(worker, payload, frame.length);
            break;
        case ENGINE_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE:
            keep_running = runtime_worker_handle_unsubscribe(worker, payload, frame.length);
            break;
        case ENGINE_DAEMON_RUNTIME_OP_APPLICATION_REQUEST:
            keep_running = runtime_worker_handle_application(worker, payload, frame.length);
            break;
        case ENGINE_DAEMON_RUNTIME_OP_APPLICATION_CANCEL:
            keep_running = runtime_worker_handle_application_cancel(worker, payload, frame.length);
            break;
        case ENGINE_DAEMON_RUNTIME_OP_CLOSE_INTENT:
            keep_running = runtime_worker_handle_close_intent(worker, frame.length);
            break;
        case ENGINE_DAEMON_RUNTIME_OP_DISCONNECT:
            keep_running = false;
...
static void runtime_worker_reset_after_join(engine_daemon_runtime_worker_t *worker) {
    free(worker->application_request);
    worker->thread_started = false;
    worker->in_use = false;
    worker->joining = false;
    worker->connection = NULL;
    worker->client_id = ENGINE_DAEMON_CLIENT_ID_INVALID;
    ...
    worker->application_thread_started = false;
    worker->application_request_token = ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    worker->last_application_request_token = ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    worker->application_request = NULL;
    worker->application_request_length = 0;
...
static void runtime_reap_completed_workers(engine_daemon_runtime_service_t *service) {
    for (size_t i = 0; i < service->worker_capacity; i++) {
        engine_daemon_runtime_worker_t *worker = &service->workers[i];
        engine_mutex_lock(&service->mutex);
        ...
        int joined = engine_thread_join(&worker->thread);
        engine_mutex_lock(&service->mutex);
        if (joined == 0) {
...
static engine_daemon_runtime_worker_t *runtime_find_free_worker_locked(
    engine_daemon_runtime_service_t *service) {
    for (size_t i = 0; i < service->worker_capacity; i++) {
...
static void runtime_reject_inline(engine_daemon_ipc_connection_t *connection, const char *message) {
    engine_daemon_runtime_connect_result_t result;
    ...
    engine_daemon_ipc_connection_drain(connection, RUNTIME_REJECT_DRAIN_TIMEOUT_MS);
    engine_daemon_ipc_connection_close(connection);
}

static void runtime_accept_connection(engine_daemon_runtime_service_t *service,
                                      engine_daemon_ipc_connection_t *connection) {
    uint64_t peer_pid = engine_daemon_ipc_connection_peer_pid(connection);
    if (peer_pid == 0) {
        engine_daemon_ipc_connection_close(connection);
        return;
    }
    ...
    engine_daemon_runtime_worker_t *worker =
    ...
    if (worker) {
        ...
        atomic_store_explicit(&worker->application_thread_done, false, memory_order_release);
        service->active_connections++;
        int created = engine_thread_create(&worker->thread, RUNTIME_WORKER_STACK_SIZE,
    ...
    if (!running) {
        runtime_reject_inline(connection, "engine daemon is stopping");
    } else {
        runtime_reject_inline(connection, "engine daemon connection capacity reached");
    }
}

static bool runtime_service_can_exit(engine_daemon_runtime_service_t *service, uint64_t now_ms) {
    bool can_exit = false;
    engine_mutex_lock(&service->mutex);
    if (service->state == ENGINE_DAEMON_RUNTIME_SERVICE_STOPPING && service->active_connections == 0) {
        bool drained = engine_daemon_should_exit(service->coordinator, now_ms);
        if (!drained && service->activation_shutdown_requested) {
            ...
            drained = engine_daemon_active_clients(service->coordinator) == 0 &&
                      engine_daemon_active_jobs(service->coordinator) == 0 &&
                      engine_daemon_active_watches(service->coordinator) == 0;
        }
        bool deadline_reached =
            service->stop_deadline_ms != 0 && now_ms >= service->stop_deadline_ms;
        can_exit = drained || deadline_reached || service->emergency_stop;
        if (can_exit) {
            service->state = ENGINE_DAEMON_RUNTIME_SERVICE_EXITED;
        }
    }
    engine_mutex_unlock(&service->mutex);
    return can_exit;
...
static void *runtime_accept_loop(void *opaque) {
    engine_daemon_runtime_service_t *service = opaque;
    for (;;) {
        runtime_reap_completed_workers(service);
        engine_mutex_lock(&service->mutex);
        engine_daemon_runtime_service_state_t state = service->state;
        engine_mutex_unlock(&service->mutex);
        if (state == ENGINE_DAEMON_RUNTIME_SERVICE_EXITED) {
            break;
        }
        if (state == ENGINE_DAEMON_RUNTIME_SERVICE_STOPPING) {
            runtime_service_interrupt_connections(service);
            if (runtime_service_can_exit(service, engine_now_ms())) {
                break;
            }
        ...
        engine_daemon_ipc_connection_t *connection = NULL;
        int accepted =
            engine_daemon_ipc_accept(service->listener, RUNTIME_ACCEPT_POLL_MS, &connection);
...
static bool runtime_application_callbacks_valid(
    const engine_daemon_runtime_application_callbacks_t *application) {
...
static bool runtime_participant_guard_release_complete(
    engine_daemon_ipc_participant_guard_t **guard_io, uint32_t timeout_ms) {
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    while (guard_io && *guard_io) {
        (void)engine_daemon_ipc_participant_guard_release(guard_io);
        if (!*guard_io) {
            return true;
        }
...
static _Noreturn void runtime_cleanup_fail_stop(const char *component) {
    ...
    engine_log_error("daemon.forced_shutdown", "component", component);
    (void)fflush(stdout);
    (void)fflush(stderr);
#ifdef _WIN32
...
static void runtime_startup_lock_release_complete(engine_daemon_ipc_startup_lock_t **lock_io,
                                                  uint32_t timeout_ms) {
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    while (lock_io && *lock_io) {
        (void)engine_daemon_ipc_startup_lock_release(lock_io);
        if (!*lock_io) {
            return;
        }
        if (engine_now_ms() >= deadline) {
...
static void runtime_service_destroy_unstarted(engine_daemon_runtime_service_t *service) {
    if (!service) {
        return;
    }
    engine_daemon_ipc_listener_close(service->listener);
    engine_daemon_coordinator_free(service->coordinator);
    ...
    for (size_t i = 0; i < service->worker_mutexes_initialized; i++) {
        engine_mutex_destroy(&service->workers[i].send_mutex);
    }
    free(service->workers);
    engine_mutex_destroy(&service->mutex);
    free(service);
}

engine_daemon_runtime_service_t *engine_daemon_runtime_service_start_reserved(
    const engine_daemon_runtime_service_config_t *config,
    engine_daemon_ipc_lifetime_reservation_t **reservation_io) {
    uint8_t validation[ENGINE_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    char active_process_fingerprint[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE];
    runtime_process_image_reference_t active_image;
    ...
    if (!reservation_io || !*reservation_io || !config || !config->endpoint ||
        ...
        config->request_timeout_ms == ENGINE_DAEMON_IPC_WAIT_FOREVER ||
        config->shutdown_timeout_ms == 0 ||
        config->shutdown_timeout_ms == ENGINE_DAEMON_IPC_WAIT_FOREVER ||
        !runtime_application_callbacks_valid(&config->application) ||
        !engine_daemon_runtime_hello_request_encode(validation, &config->identity)) {
        engine_log_error("daemon.runtime.start_failed", "stage", "config_validation");
        return NULL;
    }
    ...
    if (!runtime_process_image_reference_acquire(runtime_current_process_id(), &active_image,
        ...
        engine_log_error("daemon.runtime.start_failed", "stage", "active_image_identity");
        (void)runtime_process_image_reference_release(&active_image);
        return NULL;
    }

    engine_daemon_runtime_service_t *service = calloc(1, sizeof(*service));
    if (!service) {
        engine_log_error("daemon.runtime.start_failed", "stage", "service_allocation");
        (void)runtime_process_image_reference_release(&active_image);
        return NULL;
    }
    service->active_image = active_image;
    runtime_process_image_reference_init(&active_image);
    engine_mutex_init(&service->mutex);
    ...
    service->coordinator = engine_daemon_coordinator_new(config->lease_timeout_ms);
    if (service->coordinator && config->permanent) {
        engine_daemon_coordinator_set_permanent(service->coordinator, true);
    }
    ...
    service->state = ENGINE_DAEMON_RUNTIME_SERVICE_STARTING;

    if (!service->workers || !service->coordinator || !service->conflict_log_path ||
        !copied_identity) {
        engine_log_error("daemon.runtime.start_failed", "stage", "service_initialization");
        runtime_service_destroy_unstarted(service);
        return NULL;
    }
    for (size_t i = 0; i < service->worker_capacity; i++) {
        service->workers[i].service = service;
        engine_mutex_init(&service->workers[i].send_mutex);
    ...
    service->listener = engine_daemon_ipc_listen_reserved(config->endpoint, reservation_io);
    if (!service->listener) {
        engine_log_error("daemon.runtime.start_failed", "stage", "listener_handoff");
        runtime_service_destroy_unstarted(service);
        return NULL;
    }
    service->state = ENGINE_DAEMON_RUNTIME_SERVICE_RUNNING;
    if (engine_thread_create(&service->accept_thread, RUNTIME_WORKER_STACK_SIZE, runtime_accept_loop,
                          service) != 0) {
        engine_log_error("daemon.runtime.start_failed", "stage", "accept_thread");
        service->state = ENGINE_DAEMON_RUNTIME_SERVICE_EXITED;
        runtime_service_destroy_unstarted(service);
        return NULL;
    }
...
engine_daemon_runtime_service_t *engine_daemon_runtime_service_start(
    const engine_daemon_runtime_service_config_t *config) {
    if (!config || !config->endpoint) {
        return NULL;
    }
    engine_daemon_ipc_startup_lock_t *startup_lock = NULL;
    if (engine_daemon_ipc_startup_lock_try_acquire(config->endpoint, &startup_lock) != 1) {
        return NULL;
    }
    int generation =
        engine_daemon_ipc_generation_probe_under_startup_lock(config->endpoint, startup_lock);
    if (generation != 0 || !engine_daemon_ipc_startup_lock_prepare_handoff(startup_lock)) {
        runtime_startup_lock_release_complete(&startup_lock, config->shutdown_timeout_ms);
        return NULL;
    }
    engine_daemon_ipc_participant_guard_t *participant_guard = NULL;
    if (engine_daemon_ipc_participant_guard_try_join(config->endpoint, &participant_guard) != 1) {
        runtime_startup_lock_release_complete(&startup_lock, config->shutdown_timeout_ms);
        return NULL;
    }
    engine_daemon_ipc_lifetime_reservation_t *reservation = NULL;
    if (engine_daemon_ipc_lifetime_reservation_try_acquire(config->endpoint, &reservation) != 1) {
    ...
    engine_daemon_runtime_service_t *service =
        engine_daemon_runtime_service_start_reserved(config, &reservation);
    engine_daemon_ipc_lifetime_reservation_release(reservation);
    if (!service) {
        if (!runtime_participant_guard_release_complete(&participant_guard,
                                                        config->shutdown_timeout_ms)) {
            runtime_cleanup_fail_stop("participant_guard_cleanup");
...
engine_daemon_runtime_service_state_t engine_daemon_runtime_service_state(
    engine_daemon_runtime_service_t *service) {
    if (!service) {
        return ENGINE_DAEMON_RUNTIME_SERVICE_EXITED;
    }
    engine_mutex_lock(&service->mutex);
    engine_daemon_runtime_service_state_t state = service->state;
    engine_mutex_unlock(&service->mutex);
...
uint64_t engine_daemon_runtime_service_clients_admitted_total(engine_daemon_runtime_service_t *service) {
    if (!service) {
        return 0;
    }
    engine_mutex_lock(&service->mutex);
...
size_t engine_daemon_runtime_service_active_clients(engine_daemon_runtime_service_t *service) {
    if (!service) {
        return 0;
    }
    engine_mutex_lock(&service->mutex);
...
size_t engine_daemon_runtime_service_active_connections(engine_daemon_runtime_service_t *service) {
    if (!service) {
        return 0;
    }
    engine_mutex_lock(&service->mutex);
...
size_t engine_daemon_runtime_service_job_subscribers(engine_daemon_runtime_service_t *service,
                                                  const char *project_key) {
    return service ? engine_daemon_job_subscribers(service->coordinator, project_key) : 0;
}

uint64_t engine_daemon_runtime_service_client_process_id(engine_daemon_runtime_service_t *service,
                                                      engine_daemon_client_id_t client_id) {
    if (!service || client_id == ENGINE_DAEMON_CLIENT_ID_INVALID) {
        return 0;
    }
...
static bool runtime_wait_for_count(engine_daemon_runtime_service_t *service, size_t expected,
    ...
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    for (;;) {
        size_t actual = connections ? engine_daemon_runtime_service_active_connections(service)
                                    : engine_daemon_runtime_service_active_clients(service);
        if (actual == expected) {
            return true;
        }
        if (engine_now_ms() >= deadline) {
...
bool engine_daemon_runtime_service_wait_for_clients(engine_daemon_runtime_service_t *service,
                                                 size_t expected, uint32_t timeout_ms) {
    return runtime_wait_for_count(service, expected, timeout_ms, false);
}

bool engine_daemon_runtime_service_wait_for_connections(engine_daemon_runtime_service_t *service,
                                                     size_t expected, uint32_t timeout_ms) {
    return runtime_wait_for_count(service, expected, timeout_ms, true);
}

bool engine_daemon_runtime_service_wait_exited(engine_daemon_runtime_service_t *service,
                                            uint32_t timeout_ms) {
    if (!service) {
        return false;
    }
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    for (;;) {
        if (engine_daemon_runtime_service_state(service) == ENGINE_DAEMON_RUNTIME_SERVICE_EXITED &&
...
bool engine_daemon_runtime_service_job_reaped(engine_daemon_runtime_service_t *service,
                                           const char *project_key) {
    return service && engine_daemon_job_reaped(service->coordinator, project_key, engine_now_ms());
}

bool engine_daemon_runtime_service_stop(engine_daemon_runtime_service_t *service, uint32_t timeout_ms) {
    if (!service) {
        return false;
    }
    if (engine_daemon_runtime_service_state(service) == ENGINE_DAEMON_RUNTIME_SERVICE_EXITED) {
        return engine_daemon_runtime_service_wait_exited(service, timeout_ms);
    }
...
bool engine_daemon_runtime_service_free(engine_daemon_runtime_service_t *service) {
    if (!service ||
        engine_daemon_runtime_service_state(service) != ENGINE_DAEMON_RUNTIME_SERVICE_EXITED) {
        return false;
    }
    if (service->accept_thread_started && !service->accept_thread_joined) {
        if (engine_thread_join(&service->accept_thread) != 0) {
            return false;
        }
    ...
    for (size_t i = 0; i < service->worker_capacity; i++) {
        ...
        if (worker->thread_started) {
            if (!atomic_load_explicit(&worker->done, memory_order_acquire) ||
                engine_thread_join(&worker->thread) != 0) {
                return false;
            }
            runtime_worker_reset_after_join(worker);
        }
...
bool engine_daemon_runtime_request_activation_shutdown(
    const engine_daemon_ipc_endpoint_t *endpoint, const engine_daemon_build_identity_t *identity,
    engine_daemon_runtime_activation_action_t action, uint32_t timeout_ms,
    engine_daemon_runtime_activation_result_t *result_out) {
    if (result_out) {
        memset(result_out, 0, sizeof(*result_out));
    }
    uint8_t request[ENGINE_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE];
    if (!endpoint || !identity || !result_out || timeout_ms == ENGINE_DAEMON_IPC_WAIT_FOREVER ||
        !runtime_activation_request_encode(request, action, identity)) {
        return false;
    }
    engine_daemon_ipc_connection_t *connection = engine_daemon_ipc_connect(endpoint, timeout_ms);
    bool sent = connection && engine_daemon_ipc_send_frame(connection, ENGINE_DAEMON_FRAME_REQUEST,
                                                        ENGINE_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN,
                                                        request, (uint32_t)sizeof(request));
    engine_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received = sent ? engine_daemon_ipc_receive_frame_bounded(
                              connection, timeout_ms, ENGINE_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE,
                              &frame, &payload)
                        : 0;
    bool valid = received == 1 && frame.type == ENGINE_DAEMON_FRAME_RESPONSE &&
                 frame.flags == ENGINE_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN &&
                 frame.length == ENGINE_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE &&
...
static bool runtime_control_request_send(const engine_daemon_ipc_endpoint_t *endpoint,
                                         const engine_daemon_build_identity_t *identity,
                                         engine_daemon_runtime_operation_t operation,
                                         uint32_t timeout_ms, uint32_t response_size,
                                         uint8_t **payload_out) {
    *payload_out = NULL;
    if (!endpoint || !identity || !identity->build_fingerprint ||
        timeout_ms == ENGINE_DAEMON_IPC_WAIT_FOREVER) {
        return false;
    }
    uint8_t request[ENGINE_DAEMON_CONTROL_REQUEST_SIZE];
    memset(request, 0, sizeof(request));
    int written = snprintf((char *)request, sizeof(request), "%s", identity->build_fingerprint);
    if (written <= 0 || (size_t)written != ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE - 1U) {
        return false;
    }
    engine_daemon_ipc_connection_t *connection = engine_daemon_ipc_connect(endpoint, timeout_ms);
    bool sent =
        connection && engine_daemon_ipc_send_frame(connection, ENGINE_DAEMON_FRAME_REQUEST, operation,
                                                request, (uint32_t)sizeof(request));
    engine_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received = sent ? engine_daemon_ipc_receive_frame_bounded(connection, timeout_ms,
...
bool engine_daemon_runtime_request_status(const engine_daemon_ipc_endpoint_t *endpoint,
                                       const engine_daemon_build_identity_t *identity,
                                       uint32_t timeout_ms,
                                       engine_daemon_runtime_status_t *status_out) {
    if (!status_out) {
        return false;
    }
    memset(status_out, 0, sizeof(*status_out));
    uint8_t *payload = NULL;
    if (!runtime_control_request_send(endpoint, identity, ENGINE_DAEMON_RUNTIME_OP_STATUS, timeout_ms,
                                      ENGINE_DAEMON_STATUS_RESPONSE_SIZE, &payload)) {
        return false;
    }
    ...
    status_out->client_count = payload[8] > ENGINE_DAEMON_CONTROL_CLIENT_CAP
                                   ? (uint8_t)ENGINE_DAEMON_CONTROL_CLIENT_CAP
                                   : payload[8];
    for (size_t index = 0; index < ENGINE_DAEMON_CONTROL_CLIENT_CAP; index++) {
        status_out->client_pids[index] = runtime_control_read_u32(payload + 12U + index * 4U);
    }
    memcpy(status_out->build_fingerprint, payload + 44U, ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE);
    status_out->build_fingerprint[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE - 1U] = '\0';
...
bool engine_daemon_runtime_request_stop(const engine_daemon_ipc_endpoint_t *endpoint,
                                     const engine_daemon_build_identity_t *identity,
                                     uint32_t timeout_ms,
                                     engine_daemon_runtime_stop_result_t *result_out) {
    if (!result_out) {
        return false;
    }
    memset(result_out, 0, sizeof(*result_out));
    uint8_t *payload = NULL;
    if (!runtime_control_request_send(endpoint, identity, ENGINE_DAEMON_RUNTIME_OP_STOP, timeout_ms,
                                      ENGINE_DAEMON_STOP_RESPONSE_SIZE, &payload)) {
        return false;
    }
    ...
    result_out->client_count = payload[4] > ENGINE_DAEMON_CONTROL_CLIENT_CAP
                                   ? (uint8_t)ENGINE_DAEMON_CONTROL_CLIENT_CAP
                                   : payload[4];
    for (size_t index = 0; index < ENGINE_DAEMON_CONTROL_CLIENT_CAP; index++) {
...
engine_daemon_runtime_client_t *engine_daemon_runtime_client_connect(
    const engine_daemon_ipc_endpoint_t *endpoint, const engine_daemon_build_identity_t *identity,
    uint32_t timeout_ms, engine_daemon_runtime_connect_result_t *result_out) {
    if (result_out) {
        memset(result_out, 0, sizeof(*result_out));
    }
    uint8_t request[ENGINE_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    if (!endpoint || !identity || !result_out || timeout_ms == ENGINE_DAEMON_IPC_WAIT_FOREVER ||
        !engine_daemon_runtime_hello_request_encode(request, identity)) {
        return NULL;
    }
    engine_daemon_ipc_connection_t *connection = engine_daemon_ipc_connect(endpoint, timeout_ms);
    if (!connection || !engine_daemon_ipc_send_frame(connection, ENGINE_DAEMON_FRAME_REQUEST,
                                                  ENGINE_DAEMON_RUNTIME_OP_HELLO, request,
    ...
    uint8_t *payload = NULL;
    int received = engine_daemon_ipc_receive_frame_bounded(
        connection, timeout_ms, ENGINE_DAEMON_RENDEZVOUS_RESPONSE_SIZE, &frame, &payload);
    bool valid = received == 1 && frame.type == ENGINE_DAEMON_FRAME_RESPONSE &&
                 frame.flags == ENGINE_DAEMON_RUNTIME_OP_HELLO &&
                 frame.length == ENGINE_DAEMON_RENDEZVOUS_RESPONSE_SIZE &&
                 runtime_hello_response_decode(payload, result_out);
    free(payload);
    if (!valid || result_out->status != ENGINE_DAEMON_RUNTIME_CONNECT_ACCEPTED) {
        engine_daemon_ipc_connection_close(connection);
        return NULL;
    }

    engine_daemon_runtime_client_t *client = calloc(1, sizeof(*client));
    if (!client) {
        engine_daemon_ipc_connection_close(connection);
        memset(result_out, 0, sizeof(*result_out));
        return NULL;
    }
    engine_mutex_init(&client->exchange_mutex);
    engine_mutex_init(&client->send_mutex);
    engine_mutex_init(&client->state_mutex);
...
engine_daemon_client_id_t engine_daemon_runtime_client_id(const engine_daemon_runtime_client_t *client) {
    return client ? client->client_id : ENGINE_DAEMON_CLIENT_ID_INVALID;
}

uint64_t engine_daemon_runtime_client_process_id(const engine_daemon_runtime_client_t *client) {
    return client ? client->authenticated_process_id : 0;
}

static bool runtime_client_exchange(engine_daemon_runtime_client_t *client,
                                    engine_daemon_runtime_operation_t operation, const void *request,
                                    uint32_t request_length, uint32_t timeout_ms,
                                    uint8_t **response_out, uint32_t expected_response_length) {
    *response_out = NULL;
    if (timeout_ms == ENGINE_DAEMON_IPC_WAIT_FOREVER) {
        return false;
    }
    engine_mutex_lock(&client->exchange_mutex);
    engine_mutex_lock(&client->state_mutex);
    if (!client->usable || client->closing || !client->connection) {
        engine_mutex_unlock(&client->state_mutex);
        engine_mutex_unlock(&client->exchange_mutex);
        return false;
    }
    engine_daemon_ipc_connection_t *connection = client->connection;
    client->exchange_active = true;
    engine_mutex_unlock(&client->state_mutex);

    engine_mutex_lock(&client->send_mutex);
    bool sent = engine_daemon_ipc_send_frame(connection, ENGINE_DAEMON_FRAME_REQUEST, (uint16_t)operation,
                                          request, request_length);
    engine_mutex_unlock(&client->send_mutex);
    engine_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received =
        sent ? engine_daemon_ipc_receive_frame(connection, timeout_ms, &frame, &payload) : -1;
    bool valid = sent && received == 1 && frame.type == ENGINE_DAEMON_FRAME_RESPONSE &&
...
engine_daemon_subscription_result_t engine_daemon_runtime_client_job_subscribe(
    engine_daemon_runtime_client_t *client, const char *project_key,
    engine_daemon_subscription_id_t *subscription_id_out, uint32_t timeout_ms) {
    if (subscription_id_out) {
        *subscription_id_out = ENGINE_DAEMON_SUBSCRIPTION_ID_INVALID;
    }
    size_t key_length = 0;
    if (!client || !subscription_id_out ||
        !runtime_bounded_length(project_key, ENGINE_DAEMON_RUNTIME_PROJECT_KEY_MAX + 1, &key_length) ||
        key_length == 0 || key_length > ENGINE_DAEMON_RUNTIME_PROJECT_KEY_MAX) {
        return ENGINE_DAEMON_SUBSCRIPTION_REJECTED;
    }
    uint8_t request[SUBSCRIBE_REQUEST_PREFIX_SIZE + ENGINE_DAEMON_RUNTIME_PROJECT_KEY_MAX];
    runtime_put_u32(request, (uint32_t)key_length);
    memcpy(request + SUBSCRIBE_REQUEST_PREFIX_SIZE, project_key, key_length);
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client, ENGINE_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE, request,
                                 (uint32_t)(SUBSCRIBE_REQUEST_PREFIX_SIZE + key_length), timeout_ms,
                                 &response, SUBSCRIBE_RESPONSE_SIZE)) {
        return ENGINE_DAEMON_SUBSCRIPTION_REJECTED;
    }
    engine_daemon_subscription_result_t result =
        (engine_daemon_subscription_result_t)runtime_get_u32(response);
    engine_daemon_subscription_id_t subscription_id = runtime_get_u64(response + 4);
    free(response);
    if ((result != ENGINE_DAEMON_SUBSCRIPTION_STARTED && result != ENGINE_DAEMON_SUBSCRIPTION_JOINED) ||
...
bool engine_daemon_runtime_client_job_unsubscribe(engine_daemon_runtime_client_t *client,
                                               engine_daemon_subscription_id_t subscription_id,
                                               uint32_t timeout_ms) {
    if (!client || subscription_id == ENGINE_DAEMON_SUBSCRIPTION_ID_INVALID) {
        return false;
    }
    uint8_t request[UNSUBSCRIBE_REQUEST_SIZE];
    runtime_put_u64(request, subscription_id);
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client, ENGINE_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE, request,
...
bool engine_daemon_runtime_client_heartbeat(engine_daemon_runtime_client_t *client, uint32_t timeout_ms) {
    if (!client) {
        return false;
    }
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client, ENGINE_DAEMON_RUNTIME_OP_HEARTBEAT, NULL, 0, timeout_ms,
...
bool engine_daemon_runtime_client_application_token_reserve(
    engine_daemon_runtime_client_t *client, engine_daemon_runtime_application_token_t *token_out) {
    if (token_out) {
        *token_out = ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    if (!client || !token_out) {
        return false;
    }
    ...
    bool valid = client->usable && !client->closing && client->connection &&
                 client->active_application_token == ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
...
engine_daemon_runtime_cancel_result_t engine_daemon_runtime_client_application_cancel(
    engine_daemon_runtime_client_t *client, engine_daemon_runtime_application_token_t request_token) {
    if (!client || request_token == ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID) {
        return ENGINE_DAEMON_RUNTIME_CANCEL_ERROR;
    }
    ...
    if (client->active_application_token == request_token) {
    ...
    } else if (client->active_application_token == ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
        ...
        engine_mutex_unlock(&client->state_mutex);
        return ENGINE_DAEMON_RUNTIME_CANCEL_STALE;
    }
    engine_mutex_unlock(&client->state_mutex);
    if (!dispatch) {
        return ENGINE_DAEMON_RUNTIME_CANCEL_ACCEPTED;
    }
    ...
    engine_mutex_unlock(&client->state_mutex);
    bool sent = !dispatch || engine_daemon_ipc_send_frame(connection, ENGINE_DAEMON_FRAME_REQUEST,
                                                       ENGINE_DAEMON_RUNTIME_OP_APPLICATION_CANCEL,
    ...
    if (!dispatch) {
        ...
        return already_sent ? ENGINE_DAEMON_RUNTIME_CANCEL_ACCEPTED
                            : (unavailable ? ENGINE_DAEMON_RUNTIME_CANCEL_ERROR
                                           : ENGINE_DAEMON_RUNTIME_CANCEL_STALE);
    }
    if (!sent) {
        engine_mutex_lock(&client->state_mutex);
        client->usable = false;
        engine_mutex_unlock(&client->state_mutex);
        engine_daemon_ipc_connection_interrupt(connection);
        return ENGINE_DAEMON_RUNTIME_CANCEL_ERROR;
    }
    return ENGINE_DAEMON_RUNTIME_CANCEL_ACCEPTED;
}

engine_daemon_runtime_application_status_t engine_daemon_runtime_client_application_request_tagged(
    engine_daemon_runtime_client_t *client, engine_daemon_runtime_application_token_t request_token,
    ...
    if (!client || !response_out || !response_length_out ||
        request_token == ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID ||
        timeout_ms == ENGINE_DAEMON_IPC_WAIT_FOREVER ||
        request_length > ENGINE_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX ||
        (request_length > 0 && !request)) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    }
    ...
    if (client->pending_cancel_token != request_token) {
        client->pending_cancel_token = ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    engine_mutex_unlock(&client->state_mutex);
    ...
    engine_mutex_unlock(&client->state_mutex);
    bool sent =
        request_can_send && engine_daemon_ipc_send_frame(connection, ENGINE_DAEMON_FRAME_REQUEST,
                                                      ENGINE_DAEMON_RUNTIME_OP_APPLICATION_REQUEST,
    ...
    engine_mutex_unlock(&client->state_mutex);
    if (send_cancel) {
        sent = engine_daemon_ipc_send_frame(connection, ENGINE_DAEMON_FRAME_REQUEST,
                                         ENGINE_DAEMON_RUNTIME_OP_APPLICATION_CANCEL, cancel_wire,
    ...
    int received =
        sent ? engine_daemon_ipc_receive_frame(connection, timeout_ms, &frame, &payload) : -1;
    bool protocol_valid = sent && received == 1 && frame.type == ENGINE_DAEMON_FRAME_RESPONSE &&
                          frame.flags == ENGINE_DAEMON_RUNTIME_OP_APPLICATION_REQUEST &&
                          frame.length >= APPLICATION_RESPONSE_PREFIX_SIZE && payload;
    engine_daemon_runtime_application_status_t status = ENGINE_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    uint32_t response_length = 0;
    if (protocol_valid) {
        engine_daemon_runtime_application_token_t response_token = runtime_get_u64(payload);
        status = (engine_daemon_runtime_application_status_t)runtime_get_u32(payload + 8);
        response_length = runtime_get_u32(payload + 12);
        protocol_valid = response_token == request_token &&
                         status >= ENGINE_DAEMON_RUNTIME_APPLICATION_OK &&
                         status <= ENGINE_DAEMON_RUNTIME_APPLICATION_CANCELLED &&
                         response_length <= ENGINE_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX &&
                         frame.length == APPLICATION_RESPONSE_PREFIX_SIZE + response_length &&
                         (status == ENGINE_DAEMON_RUNTIME_APPLICATION_OK || response_length == 0);
    }
...
engine_daemon_runtime_application_status_t engine_daemon_runtime_client_application_request(
    engine_daemon_runtime_client_t *client, const void *request, uint32_t request_length,
    ...
    engine_daemon_runtime_application_token_t request_token =
        ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    if (!engine_daemon_runtime_client_application_token_reserve(client, &request_token)) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    }
    return engine_daemon_runtime_client_application_request_tagged(client, request_token, request,
                                                                request_length, response_out,
                                                                response_length_out, timeout_ms);
}

bool engine_daemon_runtime_client_close_begin(engine_daemon_runtime_client_t *client) {
    if (!client) {
        return false;
    }
    ...
    engine_daemon_ipc_connection_t *connection = client->connection;
    engine_daemon_runtime_application_token_t cancel_token = client->active_application_token;
    bool send_cancel = client->close_interrupted_exchange && connection &&
                       client->application_request_sent && !client->application_cancel_sent &&
                       cancel_token != ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    if (send_cancel) {
        client->application_cancel_sent = true;
    }
    engine_mutex_unlock(&client->state_mutex);
    ...
    if (send_cancel) {
        ...
        (void)engine_daemon_ipc_send_frame(connection, ENGINE_DAEMON_FRAME_REQUEST,
                                        ENGINE_DAEMON_RUNTIME_OP_APPLICATION_CANCEL, wire,
    ...
    if (client->close_interrupted_exchange && connection) {
        ...
        (void)engine_daemon_ipc_send_frame(connection, ENGINE_DAEMON_FRAME_REQUEST,
                                        ENGINE_DAEMON_RUNTIME_OP_CLOSE_INTENT, NULL, 0);
        engine_daemon_ipc_connection_interrupt(connection);
    }
    engine_mutex_unlock(&client->send_mutex);
    return true;
...
bool engine_daemon_runtime_client_close_finish(engine_daemon_runtime_client_t *client,
                                            uint32_t timeout_ms) {
    if (!client) {
        return false;
    }
    ...
    engine_mutex_lock(&client->state_mutex);
    bool can_disconnect = timeout_ms != ENGINE_DAEMON_IPC_WAIT_FOREVER &&
    ...
    if (can_disconnect) {
        engine_mutex_lock(&client->send_mutex);
        disconnect_sent = engine_daemon_ipc_send_frame(connection, ENGINE_DAEMON_FRAME_REQUEST,
                                                    ENGINE_DAEMON_RUNTIME_OP_DISCONNECT, NULL, 0);
        engine_mutex_unlock(&client->send_mutex);
    }
    if (disconnect_sent) {
        engine_daemon_frame_t frame = {0};
        uint8_t *response = NULL;
        int received = engine_daemon_ipc_receive_frame(connection, timeout_ms, &frame, &response);
        acknowledged = received == 1 && frame.type == ENGINE_DAEMON_FRAME_RESPONSE &&
                       frame.flags == ENGINE_DAEMON_RUNTIME_OP_DISCONNECT &&
    ...
    engine_daemon_ipc_connection_close(connection);
    engine_mutex_unlock(&client->exchange_mutex);
    engine_mutex_destroy(&client->state_mutex);
    engine_mutex_destroy(&client->send_mutex);
    engine_mutex_destroy(&client->exchange_mutex);
    free(client);
    return acknowledged;
}

bool engine_daemon_runtime_client_close(engine_daemon_runtime_client_t *client, uint32_t timeout_ms) {
    if (!engine_daemon_runtime_client_close_begin(client)) {
        return false;
    }
    return engine_daemon_runtime_client_close_finish(client, timeout_ms);
}
