...
enum {
    ...
    DAEMON_SERVICE_ESCAPED_VERSION_CAP = (ENGINE_DAEMON_VERSION_TEXT_SIZE - 1) * 6 + 1,
    DAEMON_SERVICE_LOG_RECORD_CAP = 1536,
};

static engine_daemon_conflict_log_test_hook_fn g_conflict_log_test_hook;
...
static void conflict_log_test_hook(engine_daemon_conflict_log_test_stage_t stage) {
...
static bool version_valid(const char *version) {
    size_t length = 0;
    if (!bounded_length(version, ENGINE_DAEMON_VERSION_TEXT_SIZE, &length) || length == 0) {
        return false;
    }
...
static bool fingerprint_valid(const char *fingerprint) {
    size_t length = 0;
    if (!bounded_length(fingerprint, ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE, &length) ||
        length != ENGINE_SHA256_HEX_LEN) {
        return false;
    }
    for (size_t i = 0; i < length; i++) {
...
static bool identity_valid(const engine_daemon_build_identity_t *identity) {
...
static bool conflict_status_valid(engine_daemon_hello_status_t status) {
    return status >= ENGINE_DAEMON_HELLO_VERSION_CONFLICT && status <= ENGINE_DAEMON_HELLO_CACHE_CONFLICT;
}

static bool conflict_valid(const engine_daemon_conflict_t *conflict) {
    return conflict && conflict_status_valid(conflict->status) &&
           ...
           (conflict->status != ENGINE_DAEMON_HELLO_CACHE_CONFLICT ||
            (fingerprint_valid(conflict->active_cache_fingerprint) &&
             fingerprint_valid(conflict->requested_cache_fingerprint)));
}

static const char *conflict_reason(engine_daemon_hello_status_t status) {
    switch (status) {
    case ENGINE_DAEMON_HELLO_VERSION_CONFLICT:
        return "version";
    case ENGINE_DAEMON_HELLO_BUILD_CONFLICT:
        return "build";
    case ENGINE_DAEMON_HELLO_PROTOCOL_ABI_CONFLICT:
        return "protocol_abi";
    case ENGINE_DAEMON_HELLO_STORE_ABI_CONFLICT:
        return "store_abi";
    case ENGINE_DAEMON_HELLO_FEATURE_ABI_CONFLICT:
        return "feature_abi";
    case ENGINE_DAEMON_HELLO_CACHE_CONFLICT:
        return "cache_root";
    default:
        return NULL;
    }
...
static void digest_to_hex(const uint8_t digest[ENGINE_SHA256_DIGEST_LEN],
...
bool engine_daemon_rendezvous_key(char out[ENGINE_DAEMON_KEY_SIZE]) {
    ...
    static const unsigned char domain[] = "graph-engine:coordination-daemon";
    uint64_t hash = 14695981039346656037ULL;
    for (size_t i = 0; i < sizeof(domain) - 1; i++) {
        hash ^= domain[i];
        hash *= 1099511628211ULL;
    }
    int written = snprintf(out, ENGINE_DAEMON_KEY_SIZE, "%016llx", (unsigned long long)hash);
    if (written != (int)ENGINE_DAEMON_KEY_SIZE - 1) {
        out[0] = '\0';
        return false;
    }
    return true;
...
engine_daemon_hello_status_t engine_daemon_hello_compare(const engine_daemon_build_identity_t *active,
    ...
    engine_daemon_hello_status_t status = ENGINE_DAEMON_HELLO_COMPATIBLE;
    if (strcmp(active->semantic_version, requested->semantic_version) != 0) {
        status = ENGINE_DAEMON_HELLO_VERSION_CONFLICT;
        ...
        status = ENGINE_DAEMON_HELLO_PROTOCOL_ABI_CONFLICT;
    } else if (active->store_abi != requested->store_abi) {
        status = ENGINE_DAEMON_HELLO_STORE_ABI_CONFLICT;
    } else if (active->feature_abi != requested->feature_abi) {
        status = ENGINE_DAEMON_HELLO_FEATURE_ABI_CONFLICT;
    }
...
bool engine_daemon_conflict_format(const engine_daemon_conflict_t *conflict, char *out, size_t out_size) {
    ...
    int written;
    if (conflict->status == ENGINE_DAEMON_HELLO_CACHE_CONFLICT) {
        written =
            snprintf(out, out_size,
                     "engine could not start because the active account daemon uses a "
                     "different cache directory (active cache %s; requested cache %s). "
                     "Close all engine sessions and commands, then retry with one "
                     "consistent ENGINE_CACHE_DIR.",
        ...
        written = snprintf(out, out_size,
                           ...
                           "Close all engine sessions and commands, then retry.",
...
static bool json_escape_version(const char *value, char out[DAEMON_SERVICE_ESCAPED_VERSION_CAP]) {
    static const char hex[] = "0123456789abcdef";
    size_t length = 0;
    if (!version_valid(value) || !bounded_length(value, ENGINE_DAEMON_VERSION_TEXT_SIZE, &length)) {
        return false;
    }
    size_t used = 0;
    for (size_t i = 0; i < length; i++) {
...
static bool conflict_log_record(const engine_daemon_conflict_t *conflict,
...
bool engine_daemon_build_fingerprint_native_file(uintptr_t native_file,
    ...
    while (offset < before.st_size) {
        ...
        if (count > 0) {
            engine_sha256_update(&context, buffer, (size_t)count);
            offset += (off_t)count;
        } else if (count == 0) {
            ok = false;
            break;
...
bool engine_daemon_build_fingerprint_file(const char *path,
    ...
    bool ok =
        fd >= 0 && fd_cloexec(fd) && engine_daemon_build_fingerprint_native_file((uintptr_t)fd, out);
    if (fd >= 0 && close(fd) != 0) {
        ok = false;
    }
    if (!ok) {
...
static bool posix_log_append(const char *log_path, const char *record, size_t record_length,
    ...
    if (lock_fd >= 0) {
        conflict_log_test_hook(ENGINE_DAEMON_CONFLICT_LOG_BEFORE_SERIALIZATION_LOCK);
    }
    bool ok = lock_fd >= 0 && posix_lock_exclusive(lock_fd);
    if (ok) {
        conflict_log_test_hook(ENGINE_DAEMON_CONFLICT_LOG_AFTER_SERIALIZATION_LOCK);
    }
    struct stat status;
    bool created = false;
...
bool engine_daemon_build_fingerprint_native_file(uintptr_t native_file,
    ...
    while (ok) {
        ...
        if (!ReadFile(file, buffer, sizeof(buffer), &count, NULL)) {
            ...
            engine_sha256_update(&context, buffer, count);
            total += count;
        }
    }
    LARGE_INTEGER after;
...
bool engine_daemon_build_fingerprint_file(const char *path,
    ...
    bool ok = file != INVALID_HANDLE_VALUE &&
              engine_daemon_build_fingerprint_native_file((uintptr_t)file, out);
...
static bool windows_log_path_open(const char *log_path, windows_log_path_t *path) {
    ...
    *slash = '\0';
    ...
    path->wide = engine_path_to_wide(log_path);
    if (!path->wide) {
        return false;
    }
...
static bool windows_private_file_prepare(const char *directory, const char *base) {
    /* engine_daemon_ipc_private_log_open supplies the same explicit protected
    ...
    for (;;) {
        ...
        FILE *file = engine_daemon_ipc_private_log_open(directory, base, SIZE_MAX);
        if (file) {
