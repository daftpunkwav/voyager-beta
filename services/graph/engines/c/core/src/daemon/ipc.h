/*
 * ipc.h — Authenticated local transport for the engine daemon.
 *
 ...
 */
#ifndef ENGINE_DAEMON_IPC_H
#define ENGINE_DAEMON_IPC_H
...
typedef struct engine_daemon_ipc_endpoint engine_daemon_ipc_endpoint_t;
typedef struct engine_daemon_ipc_listener engine_daemon_ipc_listener_t;
typedef struct engine_daemon_ipc_connection engine_daemon_ipc_connection_t;
typedef struct engine_daemon_ipc_startup_lock engine_daemon_ipc_startup_lock_t;
typedef struct engine_daemon_ipc_local_transition engine_daemon_ipc_local_transition_t;
typedef struct engine_daemon_ipc_participant_guard engine_daemon_ipc_participant_guard_t;
typedef struct engine_daemon_ipc_lifetime_reservation engine_daemon_ipc_lifetime_reservation_t;

/* A receive wait with no wall-clock expiry. The wait remains interruptible by
 * peer EOF and engine_daemon_ipc_connection_interrupt(). This is intentionally a
 ...
 * arriving. */
#define ENGINE_DAEMON_IPC_WAIT_FOREVER UINT32_MAX
 ...
 * receive_frame also accepts ENGINE_DAEMON_IPC_WAIT_FOREVER. */
...
int engine_daemon_ipc_local_transition_try_acquire(const engine_daemon_ipc_endpoint_t *endpoint,
...
int engine_daemon_ipc_local_transition_lifetime_probe(
    const engine_daemon_ipc_endpoint_t *endpoint, const engine_daemon_ipc_local_transition_t *transition);
