...
/* engine_store_batch_count_degrees builds a bound "?,?,..." IN clause into a fixed
...
/* Ceiling for a caller-requested node budget. ENGINE_UI_MAX_RENDER_NODES lowers
...
static int render_node_limit(void) {
    const char *raw = getenv("ENGINE_UI_MAX_RENDER_NODES");
    if (!raw || !raw[0]) {
...
static octree_node_t *octree_new(float ox, float oy, float oz, float half) {
    octree_node_t *n = calloc(ENGINE_ALLOC_ONE, sizeof(*n));
    if (!n)
        return NULL;
    n->ox = ox;
    n->oy = oy;
...
static void free_edge_array(engine_edge_t *edges, int count) {
    if (!edges)
        return;
...
engine_layout_result_t *engine_layout_compute(engine_store_t *store, const char *project,
                                        engine_layout_level_t level, const char *center_node,
    ...
    engine_search_output_t search_out;
    memset(&search_out, 0, sizeof(search_out));
    if (engine_store_search(store, &params, &search_out) != ENGINE_STORE_OK)
        return calloc(ENGINE_ALLOC_ONE, sizeof(engine_layout_result_t));

    int n = search_out.count, total_count = search_out.total;
    if (n == 0) {
        engine_store_search_free(&search_out);
        engine_layout_result_t *r = calloc(ENGINE_ALLOC_ONE, sizeof(*r));
        if (r)
    ...
    int mapped = 0;
    int edge_cap = ENGINE_SZ_256;
    engine_edge_t *all_edges = malloc((size_t)edge_cap * sizeof(engine_edge_t));
    int *es = malloc((size_t)edge_cap * sizeof(int));
    int *ed = malloc((size_t)edge_cap * sizeof(int));
    engine_schema_info_t schema;
    memset(&schema, 0, sizeof(schema));
    if (deg && all_edges && es && ed &&
        engine_store_get_schema(store, project, &schema) == ENGINE_STORE_OK) {
        for (int t = 0; t < schema.edge_type_count; t++) {
            engine_edge_t *te = NULL;
            int tc = 0;
            if (engine_store_find_edges_by_type(store, project, schema.edge_types[t].type, &te, &tc) ==
                ENGINE_STORE_OK) {
                for (int e = 0; e < tc; e++) {
                    ...
                    if (si >= 0 && di >= 0) {
                        if (mapped >= edge_cap) {
                            ...
                            engine_edge_t *te2 = realloc(all_edges, (size_t)nc * sizeof(engine_edge_t));
                        ...
                        memset(&te[e], 0, sizeof(engine_edge_t));
                        es[mapped] = si;
                        ed[mapped] = di;
                        deg[si]++;
                        deg[di]++;
    ...
    engine_layout_result_t *result = calloc(ENGINE_ALLOC_ONE, sizeof(*result));
    if (!result || !bodies) {
        free(bodies);
        free(deg);
        free(es);
        free(ed);
        free(cdepth);
        engine_layout_free(result);
        free_edge_array(all_edges, mapped);
        engine_store_search_free(&search_out);
        return NULL;
    }
    result->nodes = calloc((size_t)n, sizeof(engine_layout_node_t));
    result->node_count = n;
    ...
    if (dead_degrees_valid) {
        ...
        for (int off = 0; off < n; off += DEAD_DEGREE_CHUNK) {
            ...
            if (engine_store_batch_count_degrees(store, node_ids + off, cnt, "CALLS", in_calls + off,
                                              deg_dummy + off) != ENGINE_STORE_OK ||
                engine_store_batch_count_degrees(store, node_ids + off, cnt, "USAGE", in_usage + off,
                                              deg_dummy + off) != ENGINE_STORE_OK ||
                engine_store_batch_count_degrees(store, node_ids + off, cnt, "CALL_REFERENCE",
    ...
    for (int i = 0; i < n; i++) {
        ...
        bool testish = nf.is_test || (sn->file_path && engine_is_test_file_path(sn->file_path));
    ...
    /* 8. Output edges */
    if (mapped > 0) {
        result->edges = calloc((size_t)mapped, sizeof(engine_layout_edge_t));
...
void engine_layout_free(engine_layout_result_t *r) {
    if (!r)
        return;
...
char *engine_layout_to_json(const engine_layout_result_t *r) {
    if (!r)
        return NULL;
    ...
    if (!json) {
        char code[ENGINE_SZ_32];
        snprintf(code, sizeof(code), "%u", write_err.code);
        engine_log_error("layout.json.fail", "code", code, "msg",
