...
#ifdef ENGINE_TOML_EDIT_ENABLE_TEST_API
static ENGINE_TLS engine_toml_precommit_test_hook_t toml_precommit_test_hook = NULL;
static ENGINE_TLS void *toml_precommit_test_context = NULL;
static ENGINE_TLS engine_toml_precommit_test_hook_t toml_prepublish_test_hook = NULL;
static ENGINE_TLS void *toml_prepublish_test_context = NULL;
#endif
...
static int toml_write_atomic(const char *path, const char *old_data, size_t old_len,
    ...
    int fd = engine_mkstemp(temp_path);
    if (fd < 0) {
        free(temp_path);
        return TOML_EDIT_ERR;
    }
...
#endif
    if (!failed && TOML_SYNC(engine_fileno(file)) != 0) {
        failed = 1;
    }
    if (fclose(file) != 0) {
        failed = 1;
...
#ifdef ENGINE_TOML_EDIT_ENABLE_TEST_API
void engine_toml_set_precommit_hook_for_testing(engine_toml_precommit_test_hook_t hook, void *context) {
    toml_precommit_test_hook = hook;
    toml_precommit_test_context = context;
}

void engine_toml_set_prepublish_hook_for_testing(engine_toml_precommit_test_hook_t hook, void *context) {
...
int engine_toml_escape_basic_string(const char *input, char *out, size_t out_size) {
...
int engine_toml_upsert_managed_block(const char *file_path, const char *begin_marker,
...
int engine_toml_remove_managed_block(const char *file_path, const char *begin_marker,
                                  const char *end_marker) {
    if (!toml_valid_path(file_path) || !toml_valid_marker(begin_marker) ||
...
int engine_toml_upsert_named_array_table(const char *file_path, const char *table_name,
                                      const char *identity_key, const char *identity_value,
...
int engine_toml_remove_named_array_table(const char *file_path, const char *table_name,
                                      const char *identity_key, const char *identity_value) {
    if (!toml_named_inputs_valid(file_path, table_name, identity_key, identity_value)) {
        return TOML_EDIT_ERR;
    }
...
static int toml_edit_owned_named_array_table(const char *file_path, const char *table_name,
    ...
    if (!toml_named_inputs_valid(file_path, table_name, identity_key, identity_value) ||
        ...
        return ENGINE_TOML_OWNED_EDIT_ERROR;
    }
    toml_body_spec_t spec;
    ...
    if (scan.matching_count == 1) {
        ...
        if (!is_canonical) {
            free(existing);
            return ENGINE_TOML_OWNED_EDIT_FOREIGN;
        }
        if (!remove) {
            free(existing);
            return ENGINE_TOML_OWNED_EDIT_OK;
        }
    } else if (remove) {
        free(existing);
        return ENGINE_TOML_OWNED_EDIT_OK;
    ...
    toml_buffer_dispose(&output);
    free(existing);
    return result == TOML_EDIT_OK ? ENGINE_TOML_OWNED_EDIT_OK : ENGINE_TOML_OWNED_EDIT_ERROR;
}

int engine_toml_upsert_owned_named_array_table(const char *file_path, const char *table_name,
                                            const char *identity_key, const char *identity_value,
...
int engine_toml_remove_owned_named_array_table(const char *file_path, const char *table_name,
                                            const char *identity_key, const char *identity_value,
                                            const char *canonical_body) {
    return toml_edit_owned_named_array_table(file_path, table_name, identity_key, identity_value,
                                             canonical_body, 1);
...
static int toml_legacy_command_is_owned(const char *data, const toml_assignment_t *assignment,
    ...
    *owned = 0;
    ...
    static const char binary_name[] = "graph-engine";
    static const char windows_binary_name[] = "graph-engine.exe";
...
int engine_toml_remove_legacy_table(const char *file_path, const char *table_name,
