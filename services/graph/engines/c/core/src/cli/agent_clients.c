...
#define AGENT_ENTRY_KEY "graph-engine"
#define AGENT_MAX_CONFIG_BYTES (8U * 1024U * 1024U)
#define AGENT_JSON_MAX_DEPTH 64U

static int agent_install_callback(engine_agent_client_id_t id, const char *config_path,
                                  const char *binary_path);
static int agent_remove_callback(engine_agent_client_id_t id, const char *config_path,
                                 const char *binary_path);

static const engine_agent_client_profile_t agent_profiles[ENGINE_AGENT_CLIENT_COUNT] = {
    {ENGINE_AGENT_CLIENT_QODER, "qoder", "Qoder CLI", ENGINE_AGENT_STABLE,
     ENGINE_AGENT_CAP_MCP | ENGINE_AGENT_CAP_SKILL | ENGINE_AGENT_CAP_AGENT | ENGINE_AGENT_CAP_HOOK, "qodercli",
     agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_KIMI, "kimi", "Kimi Code CLI", ENGINE_AGENT_STABLE,
     ENGINE_AGENT_CAP_MCP | ENGINE_AGENT_CAP_INSTRUCTIONS | ENGINE_AGENT_CAP_SKILL | ENGINE_AGENT_CAP_HOOK,
     "kimi", agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_GITLAB_DUO, "gitlab-duo", "GitLab Duo CLI", ENGINE_AGENT_STABLE,
     ENGINE_AGENT_CAP_MCP | ENGINE_AGENT_CAP_HOOK, "duo", agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_ROVO_DEV, "rovo-dev", "Rovo Dev CLI", ENGINE_AGENT_STABLE,
     ENGINE_AGENT_CAP_MCP | ENGINE_AGENT_CAP_INSTRUCTIONS | ENGINE_AGENT_CAP_SKILL | ENGINE_AGENT_CAP_AGENT,
     "rovodev", agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_AMP, "amp", "Amp", ENGINE_AGENT_STABLE,
     ENGINE_AGENT_CAP_MCP | ENGINE_AGENT_CAP_INSTRUCTIONS | ENGINE_AGENT_CAP_SKILL, "amp",
     agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_DEVIN, "devin", "Devin CLI / Local", ENGINE_AGENT_STABLE,
     ENGINE_AGENT_CAP_MCP | ENGINE_AGENT_CAP_INSTRUCTIONS | ENGINE_AGENT_CAP_SKILL | ENGINE_AGENT_CAP_HOOK,
     "devin", agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_TABNINE, "tabnine", "Tabnine", ENGINE_AGENT_STABLE, ENGINE_AGENT_CAP_MCP, "tabnine",
     agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_CONTINUE, "continue", "Continue / cn", ENGINE_AGENT_CONDITIONAL,
     ENGINE_AGENT_CAP_MCP, "cn", agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_VISUAL_STUDIO, "visual-studio", "Visual Studio", ENGINE_AGENT_CONDITIONAL,
     ENGINE_AGENT_CAP_MCP, "devenv", agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_TRAE, "trae", "TRAE", ENGINE_AGENT_CONDITIONAL, ENGINE_AGENT_CAP_MCP, NULL,
     agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_ROO_CODE, "roo-code", "Roo Code", ENGINE_AGENT_CONDITIONAL, ENGINE_AGENT_CAP_MCP,
     NULL, agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_AMAZON_Q, "amazon-q", "Amazon Q Developer IDE", ENGINE_AGENT_STABLE,
     ENGINE_AGENT_CAP_MCP, NULL, agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_CODEBUDDY, "codebuddy", "CodeBuddy Code CLI", ENGINE_AGENT_STABLE,
     ENGINE_AGENT_CAP_MCP | ENGINE_AGENT_CAP_INSTRUCTIONS | ENGINE_AGENT_CAP_SKILL | ENGINE_AGENT_CAP_AGENT,
     "codebuddy", agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_IBM_BOB_IDE, "ibm-bob-ide", "IBM Bob IDE", ENGINE_AGENT_CONDITIONAL,
     ENGINE_AGENT_CAP_MCP | ENGINE_AGENT_CAP_INSTRUCTIONS | ENGINE_AGENT_CAP_SKILL, NULL,
     agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_IBM_BOB_SHELL, "ibm-bob-shell", "IBM Bob Shell", ENGINE_AGENT_STABLE,
     ENGINE_AGENT_CAP_MCP | ENGINE_AGENT_CAP_INSTRUCTIONS, "bob", agent_install_callback,
     agent_remove_callback},
    {ENGINE_AGENT_CLIENT_POCHI, "pochi", "Pochi", ENGINE_AGENT_STABLE,
     ENGINE_AGENT_CAP_MCP | ENGINE_AGENT_CAP_INSTRUCTIONS | ENGINE_AGENT_CAP_SKILL | ENGINE_AGENT_CAP_AGENT,
     "pochi", agent_install_callback, agent_remove_callback},
    {ENGINE_AGENT_CLIENT_PI, "pi", "Pi", ENGINE_AGENT_STABLE,
     ENGINE_AGENT_CAP_INSTRUCTIONS | ENGINE_AGENT_CAP_SKILL, "pi", NULL, NULL},
    {ENGINE_AGENT_CLIENT_SOURCEGRAPH_CODY, "sourcegraph-cody", "Sourcegraph Cody", ENGINE_AGENT_OPT_IN,
     ENGINE_AGENT_CAP_MCP, NULL, agent_install_callback, agent_remove_callback},
};

size_t engine_agent_client_count(void) {
    return ENGINE_AGENT_CLIENT_COUNT;
}

const engine_agent_client_profile_t *engine_agent_client_at(size_t index) {
    return index < ENGINE_AGENT_CLIENT_COUNT ? &agent_profiles[index] : NULL;
}

const engine_agent_client_profile_t *engine_agent_client_by_id(engine_agent_client_id_t id) {
    return (unsigned)id < ENGINE_AGENT_CLIENT_COUNT ? &agent_profiles[id] : NULL;
}
...
const engine_agent_client_profile_t *engine_agent_client_by_stable_id(const char *stable_id) {
    ...
    for (size_t i = 0U; i < ENGINE_AGENT_CLIENT_COUNT; i++) {
...
static bool agent_path_exists(const engine_agent_client_resolve_options_t *options, const char *path) {
    return options->path_exists ? options->path_exists(path, options->probe_context)
                                : engine_file_exists(path);
}

static bool agent_command_exists(const engine_agent_client_resolve_options_t *options,
                                 const char *command) {
...
static int agent_rovo_override(const char *config_path, const char *home_dir, bool windows,
                               char *path_out, size_t path_out_size) {
    FILE *file = engine_fopen(config_path, "rb");
    if (!file) {
        return 1;
    }
...
int engine_agent_client_resolve_path(engine_agent_client_id_t id,
    ...
    if (!engine_agent_client_by_id(id) || !options || !path_out || path_out_size == 0U ||
    ...
    case ENGINE_AGENT_CLIENT_KIMI:
        return agent_join_path(
            ...
            options->kimi_code_home && options->kimi_code_home[0] ? "mcp.json"
                                                                  : ".kimi-code/mcp.json");
    case ENGINE_AGENT_CLIENT_GITLAB_DUO:
    ...
    case ENGINE_AGENT_CLIENT_ROVO_DEV: {
    ...
    case ENGINE_AGENT_CLIENT_AMP:
        ...
        return agent_join_path(path_out, path_out_size, options->home_dir,
                               ".config/agents/skills/graph-engine/mcp.json");
    case ENGINE_AGENT_CLIENT_DEVIN:
        if (options->is_windows && options->appdata_dir && options->appdata_dir[0]) {
            return agent_join_path(path_out, path_out_size, options->appdata_dir,
                                   "devin/config.json");
        }
    ...
    case ENGINE_AGENT_CLIENT_TABNINE:
        return agent_join_path(path_out, path_out_size, options->home_dir,
                               ".tabnine/mcp_servers.json");
    case ENGINE_AGENT_CLIENT_CONTINUE: {
    ...
    case ENGINE_AGENT_CLIENT_ROO_CODE:
    ...
    case ENGINE_AGENT_CLIENT_AMAZON_Q: {
    ...
    case ENGINE_AGENT_CLIENT_CODEBUDDY: {
...
static int agent_client_marker_path(engine_agent_client_id_t id,
...
bool engine_agent_client_detect(engine_agent_client_id_t id,
                             const engine_agent_client_resolve_options_t *options) {
    const engine_agent_client_profile_t *profile = engine_agent_client_by_id(id);
    if (!profile || !options) {
        return false;
    }
    char path[1024];
    int resolved = engine_agent_client_resolve_path(id, options, path, sizeof(path));
    if (id == ENGINE_AGENT_CLIENT_CONTINUE || id == ENGINE_AGENT_CLIENT_TRAE ||
        id == ENGINE_AGENT_CLIENT_ROO_CODE || id == ENGINE_AGENT_CLIENT_SOURCEGRAPH_CODY) {
        return resolved == 0 && agent_path_exists(options, path);
    }
    if (id == ENGINE_AGENT_CLIENT_VISUAL_STUDIO) {
...
bool engine_agent_client_cleanup_candidate(engine_agent_client_id_t id,
                                        const engine_agent_client_resolve_options_t *options) {
    if (engine_agent_client_detect(id, options)) {
        return true;
    }
    if (id != ENGINE_AGENT_CLIENT_VISUAL_STUDIO || !options || !options->is_windows) {
        return false;
    }
    char path[1024];
    return engine_agent_client_resolve_path(id, options, path, sizeof(path)) == 0 &&
...
static char *agent_json_canonical(engine_agent_client_id_t id, const char *binary_path) {
    char *escaped = agent_json_escape(binary_path);
    if (!escaped) {
        return NULL;
    }
    const char *extra = "";
    if (id == ENGINE_AGENT_CLIENT_GITLAB_DUO || id == ENGINE_AGENT_CLIENT_VISUAL_STUDIO) {
        extra = ", \"type\": \"stdio\"";
    } else if (id == ENGINE_AGENT_CLIENT_ROVO_DEV) {
...
static const char *agent_json_section(engine_agent_client_id_t id) {
    if (id == ENGINE_AGENT_CLIENT_AMP) {
        return NULL;
    }
    if (id == ENGINE_AGENT_CLIENT_VISUAL_STUDIO) {
        return "servers";
    }
    if (id == ENGINE_AGENT_CLIENT_SOURCEGRAPH_CODY) {
        return "cody.mcpServers";
    }
    if (id == ENGINE_AGENT_CLIENT_POCHI) {
        return "mcp";
    }
    return "mcpServers";
}

static int agent_json_edit(engine_agent_client_id_t id, const char *config_path,
                           const char *binary_path, bool remove) {
    char *canonical = agent_json_canonical(id, binary_path);
    if (!canonical) {
        return ENGINE_AGENT_EDIT_ERROR;
    }
    char *document = NULL;
    size_t length = 0U;
    int read_result = engine_json_like_read_document(config_path, &document, &length);
    if (read_result < 0) {
        free(canonical);
        return ENGINE_AGENT_EDIT_ERROR;
    }
    ...
    if (find_result == 0 && !agent_json_owned(document, entry_start, entry_end, canonical)) {
        free(document);
        free(canonical);
        return ENGINE_AGENT_EDIT_FOREIGN;
    }
    if ((remove && find_result == 1) || (!remove && find_result == 0)) {
        free(document);
        free(canonical);
        return ENGINE_AGENT_EDIT_OK;
    }
    const char *section = agent_json_section(id);
    const char *path[1] = {section};
    int edit_result =
        remove
            ? engine_json_like_remove_entry_if_unchanged(config_path, path, section ? 1U : 0U,
                                                      AGENT_ENTRY_KEY, document, length)
            : engine_json_like_upsert_entry_if_unchanged(config_path, path, section ? 1U : 0U,
                                                      AGENT_ENTRY_KEY, canonical,
                                                      read_result == 1 ? NULL : document, length);
    free(document);
    free(canonical);
    return edit_result == 0 ? ENGINE_AGENT_EDIT_OK : ENGINE_AGENT_EDIT_ERROR;
}
...
static int agent_continue_edit(const char *config_path, const char *binary_path, bool remove) {
    char *document = NULL;
    size_t length = 0U;
    if (engine_json_like_read_document(config_path, &document, &length) != 0 || !document) {
        free(document);
        return ENGINE_AGENT_EDIT_NOT_APPLICABLE;
    }
    char *quoted = NULL;
    if (engine_yaml_encode_double_quoted_scalar(binary_path, &quoted) != 0) {
        free(document);
        return ENGINE_AGENT_EDIT_ERROR;
    }
    ...
    int canonical_written = snprintf(canonical, canonical_size,
                                     "  - name: graph-engine%s    command: %s%s"
                                     "    args: []%s",
                                     eol, quoted, eol, eol);
    free(quoted);
    ...
    int result = updated && engine_text_write_owned_document_if_unchanged(config_path, updated,
                                                                       document, length) == 0
...
static bool agent_json_client(engine_agent_client_id_t id) {
    switch (id) {
    case ENGINE_AGENT_CLIENT_QODER:
    case ENGINE_AGENT_CLIENT_KIMI:
    case ENGINE_AGENT_CLIENT_GITLAB_DUO:
    ...
    case ENGINE_AGENT_CLIENT_CONTINUE:
    case ENGINE_AGENT_CLIENT_PI:
    case ENGINE_AGENT_CLIENT_COUNT:
        return false;
    default:
        return false;
    }
...
int engine_agent_client_install_mcp(engine_agent_client_id_t id, const char *config_path,
    ...
    if (!config_path || config_path[0] == '\0' || !binary_path || binary_path[0] == '\0' ||
        !engine_agent_client_by_id(id)) {
        return ENGINE_AGENT_EDIT_ERROR;
    }
    if (id == ENGINE_AGENT_CLIENT_CONTINUE) {
    ...
    return agent_json_client(id) ? agent_json_edit(id, config_path, binary_path, false)
                                 : ENGINE_AGENT_EDIT_NOT_APPLICABLE;
}
...
static int agent_install_callback(engine_agent_client_id_t id, const char *config_path,
                                  const char *binary_path) {
    return engine_agent_client_install_mcp(id, config_path, binary_path);
}

static int agent_remove_callback(engine_agent_client_id_t id, const char *config_path,
                                 const char *binary_path) {
    return engine_agent_client_remove_mcp(id, config_path, binary_path);
}
