 ...
 *
 * Port of Go cmd/graph-engine/ install/update/config logic.
 *
...
typedef struct engine_mcp_server engine_mcp_server_t;
...
#define ENGINE_SKILL_COUNT 1

/* Skill name/content pair. */
typedef struct {
    const char *name;    /* e.g. "graph-engine" */
 ...
 * Format: { "mcpServers": { "graph-engine": { "command": binary_path } } }
 ...
 * Format: { "mcp": { "servers": { "graph-engine":
 ...
 * Format: { "servers": { "graph-engine": { "type": "stdio", "command": binary_path } } }
 * Returns 0 on success. */
 ...
 * Format: { "context_servers": { "graph-engine": { "command": path, "args": [] } } }
 * Returns 0 on success. */
...
void engine_hook_sanitize_metadata_for_testing(const char *input, char *output, size_t output_size);
char *engine_hook_augment_format_context_for_testing(const char *envelope, const char *token,
                                                  bool *is_error);
...
char *engine_hook_augment_lifecycle_json_for_dialect(const char *input, const char *forced_event,
                                                  const char *dialect);
char *engine_hook_augment_tool_json_for_testing(const char *input, const char *dialect,
                                             const char *context, char *path, size_t path_size);
bool engine_hook_augment_invocation_supported_for_testing(const char *dialect,
                                                       const char *forced_event);
bool engine_hook_path_contains_for_testing(const char *root, const char *candidate,
                                        bool case_insensitive);
const char *engine_hook_no_project_index_guidance_for_testing(const char *event);
...
/* Upsert a graph-engine instruction section in a markdown file.
 * Uses <!-- graph-engine:start --> / <!-- graph-engine:end --> markers.
...
/* Remove the graph-engine instruction section from a markdown file.
...
typedef void (*engine_hook_json_prewrite_test_hook_t)(const char *settings_path, void *context);
...
/* ── Codex instructions (legacy, wraps engine_get_agent_instructions) ── */
...
/* Extract a binary named "graph-engine*" from a tar.gz buffer.
 * Returns malloc'd binary content and sets *out_len.
 * Returns NULL on error. Caller must free. */
unsigned char *engine_extract_binary_from_targz(const unsigned char *data, int data_len, int *out_len);

/* Extract the graph-engine binary from a zip archive in memory.
 * Returns malloc'd binary content and sets *out_len.
 * Returns NULL on error. Caller must free. */
...
/* List .db files in the cache directory (~/.cache/graph-engine/).
...
#define ENGINE_CONFIG_AUTO_INDEX "auto_index"
#define ENGINE_CONFIG_AUTO_INDEX_LIMIT "auto_index_limit"
#define ENGINE_CONFIG_AUTO_WATCH "auto_watch"
#define ENGINE_CONFIG_UI_LANG "ui-lang"

/* ── Binary activation safety ─────────────────────────────────── */

typedef void *engine_cli_activation_lock_t;
typedef int (*engine_cli_activation_mutation_fn)(void *context);
 ...
 * quiescence of active modern engine processes before returning an exclusive
...
typedef int (*engine_cli_activation_reserve_fn)(void *context, engine_cli_activation_lock_t *lease_out);
typedef void (*engine_cli_activation_release_fn)(void *context, engine_cli_activation_lock_t lease);
...
typedef struct {
    void *context;
    engine_cli_activation_reserve_fn reserve_for_mutation;
    engine_cli_activation_release_fn mutation_lease_release;
    void (*visible_diagnostic)(void *context, const char *message);
} engine_cli_activation_ops_t;
...
int engine_cmd_hook_augment(int argc, char **argv);
...
char *engine_hook_augment_lifecycle_json(const char *input);
...
char *engine_hook_augment_lifecycle_json_for(const char *input, const char *forced_event,
...
void engine_hook_augment_arm_deadline(void);
char *engine_hook_augment_read_stdin(void);
...
typedef enum {
    ENGINE_HOOK_ADMISSION_DAEMON_ABSENT = 0, /* no daemon running: `daemon start` heals it */
    ENGINE_HOOK_ADMISSION_BUILD_CONFLICT     /* daemon runs another build: needs `daemon stop` */
} engine_hook_admission_t;
...
const char *engine_hook_admission_notice(engine_hook_admission_t reason, const char *hook_dialect);
...
char *engine_hook_augment_process(engine_mcp_server_t *srv, const char *input_json);
...
bool engine_hook_augment_invocation_supported(const char *forced_event, const char *dialect_name);
char *engine_hook_augment_process_for(engine_mcp_server_t *srv, const char *input_json,
 ...
 * no-op (#618). */
bool engine_hook_path_is_abs(const char *path);
