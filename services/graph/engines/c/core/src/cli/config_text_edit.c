...
#ifdef ENGINE_TEXT_EDIT_ENABLE_TEST_API
static ENGINE_TLS engine_text_precommit_test_hook_t text_precommit_test_hook = NULL;
static ENGINE_TLS void *text_precommit_test_context = NULL;
static ENGINE_TLS engine_text_precommit_test_hook_t text_prepublish_test_hook = NULL;
static ENGINE_TLS void *text_prepublish_test_context = NULL;
static ENGINE_TLS engine_text_precommit_test_hook_t text_temp_closed_test_hook = NULL;
static ENGINE_TLS void *text_temp_closed_test_context = NULL;
#endif
...
#ifdef _WIN32
    intptr_t native_handle = _get_osfhandle(engine_fileno(file));
    return native_handle == -1
               ? TEXT_ERROR
               : text_snapshot_from_handle((HANDLE)(uintptr_t)native_handle, snapshot);
#else
    struct stat state;
    return fstat(engine_fileno(file), &state) == 0 ? text_snapshot_from_stat(&state, snapshot)
                                                : TEXT_ERROR;
#endif
}
...
static int text_write_atomic_mode(const char *path, const char *new_data, size_t new_len,
    ...
    for (unsigned attempt = 0U; attempt < TEXT_TEMP_ATTEMPTS; attempt++) {
        ...
        int written = snprintf(temp_path, capacity, "%s.engine-text-%ld-%u.tmp", path,
        ...
        file = engine_fopen(temp_path, "w+bx");
#else
#ifndef O_NOFOLLOW
        free(temp_path);
        return TEXT_ERROR;
    ...
    if (!failed && snapshot->exists &&
        fchown(engine_fileno(file), snapshot->owner, snapshot->group) != 0) {
        failed = 1;
    }
...
#endif
    if (!failed && TEXT_SYNC(engine_fileno(file)) != 0) {
        failed = 1;
    }
...
#ifdef ENGINE_TEXT_EDIT_ENABLE_TEST_API
void engine_text_set_precommit_hook_for_testing(engine_text_precommit_test_hook_t hook, void *context) {
    text_precommit_test_hook = hook;
    text_precommit_test_context = context;
}

void engine_text_set_prepublish_hook_for_testing(engine_text_precommit_test_hook_t hook, void *context) {
    text_prepublish_test_hook = hook;
    text_prepublish_test_context = context;
}

void engine_text_set_temp_closed_hook_for_testing(engine_text_precommit_test_hook_t hook, void *context) {
...
int engine_text_upsert_managed_block(const char *file_path, const char *begin_marker,
                                  const char *end_marker, const char *owned_content) {
    return text_upsert_managed_block(file_path, begin_marker, end_marker, owned_content,
                                     TEXT_MAX_BYTES);
}

int engine_text_upsert_managed_block_limited(const char *file_path, const char *begin_marker,
                                          const char *end_marker, const char *owned_content,
                                          size_t max_document_bytes) {
    return text_upsert_managed_block(file_path, begin_marker, end_marker, owned_content,
                                     max_document_bytes);
...
int engine_text_remove_managed_block(const char *file_path, const char *begin_marker,
...
int engine_text_write_owned_document(const char *file_path, const char *owned_content) {
    size_t content_len = 0U;
    if (!text_valid_path(file_path) ||
        text_bounded_strlen(owned_content, TEXT_MAX_BYTES, &content_len) != TEXT_OK ||
...
int engine_text_write_owned_document_if_unchanged(const char *file_path, const char *owned_content,
...
int engine_text_create_owned_document(const char *file_path, const char *owned_content) {
    size_t content_len = 0U;
    if (!text_valid_path(file_path) ||
        text_bounded_strlen(owned_content, TEXT_MAX_BYTES, &content_len) != TEXT_OK ||
        text_validate_bytes(owned_content, content_len, 1) != TEXT_OK) {
...
int engine_text_ensure_owned_document(const char *file_path, const char *owned_content) {
    size_t content_len = 0U;
    if (!text_valid_path(file_path) ||
        text_bounded_strlen(owned_content, TEXT_MAX_BYTES, &content_len) != TEXT_OK ||
        text_validate_bytes(owned_content, content_len, 1) != TEXT_OK) {
 ...
 * cannot deliver (#1387). */
int engine_text_owned_document_status(const char *file_path, const char *current_content,
...
int engine_text_migrate_owned_document(const char *file_path, const char *current_content,
                                    const char *const *released_contents, size_t released_count) {
    return text_migrate_owned_document(file_path, current_content, released_contents,
                                       released_count, 0, 0U);
}

int engine_text_migrate_owned_document_mode(const char *file_path, const char *current_content,
...
int engine_text_remove_owned_document(const char *file_path, const char *expected_owned_content) {
...
int engine_text_remove_owned_document_any(const char *file_path, const char *current_content,
                                       const char *const *released_contents,
