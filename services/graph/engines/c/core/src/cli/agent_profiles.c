...
static bool tier_valid(engine_graph_tier_t tier) {
    return tier >= ENGINE_GRAPH_TIER_SCOUT && tier < ENGINE_GRAPH_TIER_COUNT;
}

static bool access_valid(engine_graph_access_t access) {
    return access >= ENGINE_GRAPH_ACCESS_DIRECT && access < ENGINE_GRAPH_ACCESS_COUNT;
}

static bool dialect_valid(engine_graph_profile_dialect_t dialect) {
    return dialect >= ENGINE_GRAPH_DIALECT_CLAUDE && dialect < ENGINE_GRAPH_DIALECT_COUNT;
}

bool engine_graph_dialect_direct_capable(engine_graph_profile_dialect_t dialect) {
    switch (dialect) {
    case ENGINE_GRAPH_DIALECT_AUGMENT:
    case ENGINE_GRAPH_DIALECT_CURSOR:
    case ENGINE_GRAPH_DIALECT_ROVO:
    case ENGINE_GRAPH_DIALECT_POCHI:
        return false;
    default:
...
const char *engine_graph_tier_slug(engine_graph_tier_t tier) {
    static const char *const slugs[ENGINE_GRAPH_TIER_COUNT] = {
        "graph-engine-scout",
        "graph-engine",
        "graph-engine-auditor",
    };
...
static const char *profile_description(engine_graph_tier_t tier, engine_graph_access_t access) {
    static const char *const direct[ENGINE_GRAPH_TIER_COUNT] = {
    ...
    static const char *const handoff[ENGINE_GRAPH_TIER_COUNT] = {
    ...
    return access == ENGINE_GRAPH_ACCESS_DIRECT ? direct[tier] : handoff[tier];
}
...
char *engine_render_graph_prompt(engine_graph_tier_t tier, engine_graph_access_t access) {
    ...
    profile_buffer_init(&buffer);
    if (access == ENGINE_GRAPH_ACCESS_DIRECT) {
        switch (tier) {
        case ENGINE_GRAPH_TIER_SCOUT:
        ...
        profile_buffer_append(
            &buffer,
            "Use graph-engine in the exact graph project. Use only read-only graph and "
...
static void tier_tool_set(engine_graph_tier_t tier, const profile_tool_t **tools, size_t *count) {
    if (tier == ENGINE_GRAPH_TIER_SCOUT) {
        *tools = scout_tools;
...
static const char *tier_server_profile(engine_graph_tier_t tier) {
    return tier == ENGINE_GRAPH_TIER_SCOUT ? "scout" : "analysis";
}

static const char *dialect_tool_prefix(engine_graph_profile_dialect_t dialect) {
    switch (dialect) {
    case ENGINE_GRAPH_DIALECT_CLAUDE:
    case ENGINE_GRAPH_DIALECT_QWEN:
    case ENGINE_GRAPH_DIALECT_QODER:
    case ENGINE_GRAPH_DIALECT_CODEBUDDY:
    case ENGINE_GRAPH_DIALECT_FACTORY:
        return "mcp__graph-engine__";
    case ENGINE_GRAPH_DIALECT_CODEX:
        return "";
    case ENGINE_GRAPH_DIALECT_GEMINI:
        return "mcp_graph-engine_";
    case ENGINE_GRAPH_DIALECT_COPILOT:
        return "graph-engine/";
    case ENGINE_GRAPH_DIALECT_OPENCODE:
    case ENGINE_GRAPH_DIALECT_KILO:
    case ENGINE_GRAPH_DIALECT_VIBE:
        return "graph-engine_";
    case ENGINE_GRAPH_DIALECT_KIRO:
        return "@graph-engine/";
    default:
        return NULL;
    }
...
static bool tool_identifier(engine_graph_profile_dialect_t dialect, profile_tool_t tool, char *output,
                            size_t output_size) {
...
static bool append_yaml_mcp_tools(profile_buffer_t *buffer, engine_graph_profile_dialect_t dialect,
                                  engine_graph_tier_t tier) {
...
static bool append_csv_mcp_tools(profile_buffer_t *buffer, engine_graph_profile_dialect_t dialect,
                                 engine_graph_tier_t tier) {
    const profile_tool_t *tools = NULL;
    size_t count = 0U;
    tier_tool_set(tier, &tools, &count);
...
static bool append_toml_mcp_tools(profile_buffer_t *buffer, engine_graph_profile_dialect_t dialect,
                                  engine_graph_tier_t tier, bool leading_items) {
    const profile_tool_t *tools = NULL;
    size_t count = 0U;
    tier_tool_set(tier, &tools, &count);
    for (size_t i = 0U; i < count; i++) {
...
static char *render_kiro_profile(engine_graph_tier_t tier, engine_graph_access_t access,
                                 const char *binary_path, const char *prompt) {
    if (access == ENGINE_GRAPH_ACCESS_DIRECT && (!binary_path || !binary_path[0])) {
        return NULL;
    }
    ...
    if (ok && access == ENGINE_GRAPH_ACCESS_DIRECT) {
        const profile_tool_t *tier_tools = NULL;
        size_t count = 0U;
        tier_tool_set(tier, &tier_tools, &count);
        for (size_t i = 0U; ok && i < count; i++) {
            char identifier[160];
            ok = tool_identifier(ENGINE_GRAPH_DIALECT_KIRO, tier_tools[i], identifier,
                                 sizeof(identifier)) &&
...
static bool append_codex_profile(profile_buffer_t *buffer, engine_graph_tier_t tier,
    ...
    if (!profile_buffer_append(buffer, "name = \"") ||
        !profile_buffer_append(buffer, engine_graph_tier_slug(tier)) ||
        !profile_buffer_append(buffer, "\"\ndescription = \"") ||
    ...
    if (!rc1_transportless) {
        ...
        if (!binary_path || !binary_path[0] ||
            engine_toml_escape_basic_string(binary_path, escaped_binary, sizeof(escaped_binary)) !=
                0) {
            return false;
        }
    ...
    return profile_buffer_append(buffer, "enabled_tools = [") &&
           append_toml_mcp_tools(buffer, ENGINE_GRAPH_DIALECT_CODEX, tier, false) &&
           profile_buffer_append(buffer, "]\n");
}

static bool render_profile_text(profile_buffer_t *buffer, engine_graph_profile_dialect_t dialect,
                                engine_graph_tier_t tier, engine_graph_access_t access,
                                const char *binary_path, const char *prompt) {
    const char *slug = engine_graph_tier_slug(tier);
    const char *display = engine_graph_tier_display_name(tier);
    const char *description = profile_description(tier, access);
    bool direct = access == ENGINE_GRAPH_ACCESS_DIRECT;
    switch (dialect) {
    case ENGINE_GRAPH_DIALECT_CLAUDE:
        if (!append_yaml_identity(buffer, slug, description) ||
            ...
            (direct && !profile_buffer_append(buffer, "mcpServers: [graph-engine]\n")) ||
            !profile_buffer_append(buffer,
                                   "permissionMode: plan\nskills: [graph-engine]\n---\n") ||
    ...
    case ENGINE_GRAPH_DIALECT_KILO:
        ...
        return true;
    case ENGINE_GRAPH_DIALECT_JUNIE:
        if (!profile_buffer_append(buffer, "---\nname: \"") ||
...
char *engine_render_graph_profile(engine_graph_profile_dialect_t dialect, engine_graph_tier_t tier,
    ...
    if (!dialect_valid(dialect) || !tier_valid(tier) || !access_valid(access) ||
        (access == ENGINE_GRAPH_ACCESS_DIRECT && !engine_graph_dialect_direct_capable(dialect))) {
        return NULL;
    }
    char *prompt = engine_render_graph_prompt(tier, access);
    if (!prompt) {
        return NULL;
    }
    if (dialect == ENGINE_GRAPH_DIALECT_KIRO) {
...
char *engine_render_graph_profile_codex_rc1(engine_graph_tier_t tier) {
    ...
    char *prompt = engine_render_graph_prompt(tier, ENGINE_GRAPH_ACCESS_DIRECT);
    if (!prompt) {
        return NULL;
    }
    profile_buffer_t buffer;
    profile_buffer_init(&buffer);
    bool ok = append_codex_profile(&buffer, tier, ENGINE_GRAPH_ACCESS_DIRECT, NULL, prompt, true);
    free(prompt);
    if (!ok) {
        profile_buffer_discard(&buffer);
        return NULL;
