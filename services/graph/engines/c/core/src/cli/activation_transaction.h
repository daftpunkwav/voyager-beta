 ...
 */
#ifndef ENGINE_ACTIVATION_TRANSACTION_H
#define ENGINE_ACTIVATION_TRANSACTION_H

#include <stdbool.h>
#include <stddef.h>

typedef struct engine_activation_transaction engine_activation_transaction_t;

typedef enum {
    ENGINE_ACTIVATION_TRANSACTION_OK = 0,
    ...
    ENGINE_ACTIVATION_TRANSACTION_DEFERRED = 1,
    ENGINE_ACTIVATION_TRANSACTION_INVALID_ARGUMENT = -1,
    ENGINE_ACTIVATION_TRANSACTION_NO_MEMORY = -2,
    ENGINE_ACTIVATION_TRANSACTION_IO = -3,
    ENGINE_ACTIVATION_TRANSACTION_INVALID_STATE = -4,
    /* The post-commit validator rejected the candidate and rollback succeeded. */
    ENGINE_ACTIVATION_TRANSACTION_VALIDATION_FAILED = -5,
    /* The target changed, but restoring the retained backup also failed. */
    ENGINE_ACTIVATION_TRANSACTION_ROLLBACK_FAILED = -6,
} engine_activation_transaction_status_t;

typedef bool (*engine_activation_transaction_validator_fn)(const char *target_path, void *context);
 ...
 * leave this unset. */
typedef void (*engine_activation_transaction_before_absent_publish_for_test_fn)(
...
const char *engine_activation_transaction_target_path(const engine_activation_transaction_t *transaction);
const char *engine_activation_transaction_staged_path(const engine_activation_transaction_t *transaction);
const char *engine_activation_transaction_backup_path(const engine_activation_transaction_t *transaction);
...
#endif /* ENGINE_ACTIVATION_TRANSACTION_H */
