...
const char *engine_confidence_band(double score) {
...
typedef ENGINE_DYN_ARRAY(char *) qn_array_t;
...
struct engine_registry {
    ...
    EngineHashTable *exact;

    /* byName: simpleName → qn_array_t* (heap-owned) */
    EngineHashTable *by_name;
};
...
static const char *best_by_import_distance(const char **candidates, int count,
    ...
    int best_score = ENGINE_NOT_FOUND;
 ...
 * contention. */
static ENGINE_TLS EngineHashTable *_reach_cache = NULL;
...
void engine_registry_reach_cache_begin(int estimated_capacity) {
    if (_reach_cache) {
        /* Defensive: caller forgot to call _end. Clear and reuse. */
        engine_ht_foreach(_reach_cache, reach_cache_free_key, NULL);
        engine_ht_clear(_reach_cache);
        return;
    }
    if (estimated_capacity < 16)
        estimated_capacity = 16;
    _reach_cache = engine_ht_create((uint32_t)estimated_capacity);
}

void engine_registry_reach_cache_end(void) {
    if (!_reach_cache)
        return;
    engine_ht_foreach(_reach_cache, reach_cache_free_key, NULL);
    engine_ht_free(_reach_cache);
    _reach_cache = NULL;
}
...
static ENGINE_TLS EngineHashTable *_import_map_cache = NULL;

void engine_registry_import_map_cache_begin(const char **keys, const char **vals, int count) {
    if (_import_map_cache) {
        engine_ht_free(_import_map_cache);
        _import_map_cache = NULL;
    }
    if (!keys || !vals || count <= 0)
        return;
    _import_map_cache = engine_ht_create((uint32_t)count * 2u + 8u);
    if (!_import_map_cache)
        return;
    for (int i = 0; i < count; i++) {
        if (keys[i] && vals[i]) {
            engine_ht_set(_import_map_cache, keys[i], (void *)(uintptr_t)vals[i]);
        }
    }
}

void engine_registry_import_map_cache_end(void) {
    if (!_import_map_cache)
        return;
...
/* ── Per-file full-result cache for engine_registry_resolve ──────────
 *
 * THE big one: 98.7% of resolve_calls CPU on kubernetes lives inside
 * engine_registry_resolve's strategy chain (suffix_match iterating
 ...
 * to the same QN every time. Cache the full engine_resolution_t result
...
static ENGINE_TLS EngineHashTable *_resolve_cache = NULL;
...
void engine_registry_resolve_cache_begin(int estimated_capacity) {
    if (_resolve_cache) {
        engine_ht_foreach(_resolve_cache, resolve_cache_free_entry, NULL);
        engine_ht_free(_resolve_cache);
        _resolve_cache = NULL;
    }
    if (estimated_capacity < 32)
        estimated_capacity = 32;
    _resolve_cache = engine_ht_create((uint32_t)estimated_capacity);
}

void engine_registry_resolve_cache_end(void) {
    if (!_resolve_cache)
        return;
    engine_ht_foreach(_resolve_cache, resolve_cache_free_entry, NULL);
    engine_ht_free(_resolve_cache);
...
static bool is_import_reachable(const char *candidate_qn, const char **import_vals,
                                int import_count) {
    if (_reach_cache) {
        void *cached = engine_ht_get(_reach_cache, candidate_qn);
    ...
    if (_reach_cache) {
        ...
        if (kdup) {
            engine_ht_set(_reach_cache, kdup, reachable ? REACH_CACHE_TRUE : REACH_CACHE_FALSE);
        }
    }
    return reachable;
}
...
static engine_resolution_t empty_result(void) {
    engine_resolution_t r = {0};
    return r;
}
...
bool engine_perl_is_builtin(const char *name) {
    if (!name || !name[0]) {
        return false;
    }
 ...
 * engine_resolution_t.strategy of a non-empty match;
...
bool engine_perl_suppress_generic_match(bool is_perl, bool is_method, const char *callee_name,
                                     const char *strategy) {
    if (!is_perl) {
        return false;
    }
    if (!(is_method || engine_perl_is_builtin(callee_name))) {
        return false;
    }
...
bool engine_tsjs_suppress_weak_method_match(bool is_tsjs, bool is_method, const char *strategy) {
     ...
     * "fuzzy" is listed as defensive insurance only — engine_registry_fuzzy_resolve
...
engine_registry_t *engine_registry_new(void) {
    engine_registry_t *r = calloc(ENGINE_ALLOC_ONE, sizeof(engine_registry_t));
    if (!r) {
        return NULL;
    }
    r->exact = engine_ht_create(ENGINE_SZ_1K);
    r->by_name = engine_ht_create(ENGINE_SZ_512);
    return r;
}
...
void engine_registry_free(engine_registry_t *r) {
    if (!r) {
        return;
    }
    /* by_name first: its items borrow exact's keys. */
    engine_ht_foreach(r->by_name, free_qn_array, NULL);
    engine_ht_free(r->by_name);
    engine_ht_foreach(r->exact, free_label, NULL);
    engine_ht_free(r->exact);
...
void engine_registry_add(engine_registry_t *r, const char *name, const char *qualified_name,
                      const char *label) {
    (void)name;
    ...
    /* Check for duplicate */
    if (engine_ht_get(r->exact, qualified_name)) {
        return;
    }
    ...
    engine_ht_set(r->exact, strdup(qualified_name), (void *)interned);
    const char *owned_qn = engine_ht_get_key(r->exact, qualified_name);

    /* Index by simple name.
     * No array dedup needed: exact-map check above guarantees uniqueness. */
    const char *simple = simple_name(qualified_name);
    qn_array_t *arr = engine_ht_get(r->by_name, simple);
    if (!arr) {
        arr = calloc(ENGINE_ALLOC_ONE, sizeof(qn_array_t));
        engine_ht_set(r->by_name, strdup(simple), arr);
    }
    engine_da_push(arr, (char *)owned_qn);
}

/* ── Lookup ──────────────────────────────────────────────────────── */

bool engine_registry_exists(const engine_registry_t *r, const char *qn) {
    if (!r || !qn) {
        return false;
    }
    return engine_ht_get(r->exact, qn) != NULL;
}

const char *engine_registry_label_of(const engine_registry_t *r, const char *qn) {
    if (!r || !qn) {
        return NULL;
    }
    return engine_ht_get(r->exact, qn);
}

int engine_registry_find_by_name(const engine_registry_t *r, const char *name, const char ***out,
                              int *count) {
    if (!r || !out || !count) {
        return ENGINE_NOT_FOUND;
    }
    qn_array_t *arr = engine_ht_get(r->by_name, name);
    if (arr && arr->count > 0) {
...
int engine_registry_size(const engine_registry_t *r) {
    return r ? (int)engine_ht_count(r->exact) : 0;
}
...
static engine_resolution_t resolve_import_map(const engine_registry_t *r, const char *prefix,
    ...
    if (_import_map_cache) {
        resolved = (const char *)engine_ht_get(_import_map_cache, prefix);
    } else {
    ...
    if (!suffix || !suffix[0]) {
        const char *direct = engine_ht_get_key(r->exact, resolved);
        if (direct) {
            return (engine_resolution_t){direct, "import_map", CONF_IMPORT_MAP, REG_RESOLVED};
        }
    }
    char candidate[ENGINE_SZ_512];
    if (suffix && suffix[0]) {
        snprintf(candidate, sizeof(candidate), "%s.%s", resolved, suffix);
    } else {
        snprintf(candidate, sizeof(candidate), "%s.%s", resolved, prefix);
    }
    /* Use engine_ht_get_key to get the persistent heap-owned key string */
    const char *stored_key = engine_ht_get_key(r->exact, candidate);
    if (stored_key) {
        return (engine_resolution_t){stored_key, "import_map", CONF_IMPORT_MAP, REG_RESOLVED};
    }
     ...
     * candidates instead of engine_ht_foreach over the WHOLE exact table — that
    ...
    if (suffix && suffix[0]) {
        char resolved_dot[ENGINE_SZ_512];
        char dot_suffix[ENGINE_SZ_256];
        snprintf(resolved_dot, sizeof(resolved_dot), "%s.", resolved);
        snprintf(dot_suffix, sizeof(dot_suffix), ".%s", suffix);
        qn_array_t *arr = engine_ht_get(r->by_name, simple_name(suffix));
        if (arr) {
            ...
            for (int i = 0; i < arr->count; i++) {
                ...
                if (klen >= rd_len + ds_len && strncmp(qn, resolved_dot, rd_len) == 0 &&
                    ...
                    return (engine_resolution_t){qn, "import_map_suffix", CONF_IMPORT_MAP_SUFFIX,
                                              REG_RESOLVED};
                }
            }
        }
...
static engine_resolution_t resolve_same_module(const engine_registry_t *r, const char *callee_name,
                                            const char *suffix, const char *module_qn) {
    char candidate[ENGINE_SZ_512];
    snprintf(candidate, sizeof(candidate), "%s.%s", module_qn, callee_name);
    const char *stored_key = engine_ht_get_key(r->exact, candidate);
    if (stored_key) {
        return (engine_resolution_t){stored_key, "same_module", CONF_SAME_MODULE, REG_RESOLVED};
    }
    if (suffix && suffix[0]) {
        snprintf(candidate, sizeof(candidate), "%s.%s", module_qn, suffix);
        stored_key = engine_ht_get_key(r->exact, candidate);
        if (stored_key) {
            return (engine_resolution_t){stored_key, "same_module", CONF_SAME_MODULE, REG_RESOLVED};
        }
    }
...
static engine_resolution_t resolve_multi_with_imports(const qn_array_t *arr, const char *module_qn,
                                                   const char **import_vals, int import_count) {
    const char *filtered[ENGINE_SZ_256];
    int fcount = 0;
    for (int i = 0; i < arr->count && fcount < ENGINE_SZ_256; i++) {
    ...
    if (fcount == SKIP_ONE) {
        ...
        return (engine_resolution_t){filtered[0], "suffix_match", conf, arr->count};
    }
    if (fcount > SKIP_ONE) {
        ...
        if (best) {
            ...
            return (engine_resolution_t){best, "suffix_match", conf, fcount};
        }
    }
    ...
    if (best) {
        ...
        return (engine_resolution_t){best, "suffix_match", conf, arr->count};
    }
    return empty_result();
}
...
static engine_resolution_t resolve_name_lookup(const engine_registry_t *r, const char *callee_name,
                                            const char *module_qn, const char **import_vals,
                                            int import_count) {
    const char *lookup = simple_name(callee_name);
    qn_array_t *arr = engine_ht_get(r->by_name, lookup);
    if (!arr || arr->count == 0) {
        return empty_result();
    }
    ...
    if (arr->count > 1) {
        ...
        if (q) {
            return (engine_resolution_t){q, "qualified_suffix", CONF_QUALIFIED_SUFFIX, REG_RESOLVED};
        }
    }
    ...
    if (arr->count == SKIP_ONE) {
        ...
        return (engine_resolution_t){arr->items[0], "unique_name", conf, REG_RESOLVED};
    }
...
engine_resolution_t engine_registry_resolve(const engine_registry_t *r, const char *callee_name,
    ...
    if (_resolve_cache) {
        resolve_cache_entry_t *cached =
            (resolve_cache_entry_t *)engine_ht_get(_resolve_cache, callee_name);
        if (cached) {
            return cached->res;
        }
    }
    ...
    if (_resolve_cache) {
        ...
        if (e) {
            ...
            if (kdup) {
                engine_ht_set(_resolve_cache, kdup, e);
            } else {
                free(e);
            }
        }
...
engine_fuzzy_result_t engine_registry_fuzzy_resolve(const engine_registry_t *r, const char *callee_name,
                                              const char *module_qn, const char **import_map_keys,
                                              const char **import_map_vals, int import_map_count) {
    (void)import_map_keys;
    engine_fuzzy_result_t no_match = {{0}, false};
    if (!r || !callee_name) {
        return no_match;
    }
    ...
    if (arr->count == SKIP_ONE) {
        ...
        return (engine_fuzzy_result_t){{arr->items[0], "fuzzy", conf, REG_RESOLVED}, true};
    }
    ...
    if (have_imports) {
        fcount = filter_import_reachable((const char **)arr->items, arr->count, import_map_vals,
                                         import_map_count, filtered, ENGINE_SZ_256);
        fptr = filtered;
    }
...
int engine_registry_find_ending_with(const engine_registry_t *r, const char *suffix, const char ***out) {
    if (!r || !suffix || !out) {
        if (out) {
            *out = NULL;
        }
    ...
    engine_ht_foreach(r->exact, few_scan, &ctx);
...
bool engine_registry_is_import_reachable(const char *candidate_qn, const char **import_vals,
                                      int import_count) {
