 ...
 *   2. Call engine_extract_file() to get defs, calls, imports
 ...
 *
 * Depends on: extraction layer (engine.h), graph_buffer, pipeline internals
 */
...
static const char *itoa_log(int val) {
    static ENGINE_TLS char bufs[PD_RING][ENGINE_SZ_32];
...
static void process_def(engine_pipeline_ctx_t *ctx, const EngineDefinition *def, const char *rel) {
    if (!def->qualified_name || !def->name) {
        return;
    }
    char props[ENGINE_SZ_2K];
    build_def_props(props, sizeof(props), def);
    int64_t node_id = engine_gbuf_upsert_node(
    ...
    if (file_node && node_id > 0) {
        engine_gbuf_insert_edge(ctx->gbuf, file_node->id, node_id, "DEFINES", "{}");
    }
    free(file_qn);
    if (def->parent_class && def->label && strcmp(def->label, "Method") == 0) {
        ...
        if (parent && node_id > 0) {
            engine_gbuf_insert_edge(ctx->gbuf, parent->id, node_id, "DEFINES_METHOD", "{}");
        }
    }
}

/* Create Channel nodes + EMITS / LISTENS_ON edges for one file's channels.
 * Mirrors the parallel path in engine_build_registry_from_cache — keep in sync. */
/* Find the source node for a channel edge: enclosing function or file node. */
static const engine_gbuf_node_t *find_channel_source(engine_pipeline_ctx_t *ctx, const EngineChannel *ch,
                                                  const char *rel) {
...
static void create_channel_edges_for_file(engine_pipeline_ctx_t *ctx, const EngineFileResult *result,
                                          const char *rel) {
    for (int j = 0; j < result->channels.count; j++) {
        const EngineChannel *ch = &result->channels.items[j];
        ...
        const engine_gbuf_node_t *src_node = find_channel_source(ctx, ch, rel);
...
int engine_pipeline_create_env_configures_for_file(engine_pipeline_ctx_t *ctx,
    ...
    int count = 0;
    char *file_qn = NULL;
    const engine_gbuf_node_t *file_node = NULL;
    for (int j = 0; j < result->env_accesses.count; j++) {
        const EngineEnvAccess *ea = &result->env_accesses.items[j];
        if (!ea->env_key || !ea->env_key[0]) {
            continue;
        }
        char env_qn[ENGINE_SZ_512];
        snprintf(env_qn, sizeof(env_qn), "__env__%s", ea->env_key);
        char env_props[ENGINE_SZ_512];
        snprintf(env_props, sizeof(env_props), "{\"env_key\":\"%s\"}", ea->env_key);
        int64_t env_id =
            engine_gbuf_upsert_node(ctx->gbuf, "EnvVar", ea->env_key, env_qn, "", 0, 0, env_props);
        if (env_id <= 0) {
            continue;
        }
        const engine_gbuf_node_t *src = NULL;
        if (ea->enclosing_func_qn && ea->enclosing_func_qn[0]) {
            src = engine_gbuf_find_by_qn(ctx->gbuf, ea->enclosing_func_qn);
        ...
        if (!src) {
            if (!file_qn) {
                file_qn = engine_pipeline_fqn_compute(ctx->project_name, rel, "__file__");
                file_node = engine_gbuf_find_by_qn(ctx->gbuf, file_qn);
            }
            src = file_node;
        }
        if (src && src->id != env_id) {
            engine_gbuf_insert_edge(ctx->gbuf, src->id, env_id, "CONFIGURES",
                                 "{\"strategy\":\"env_access\"}");
            count++;
        }
    }
...
static int create_import_edges_for_file(engine_pipeline_ctx_t *ctx, const EngineFileResult *result,
    ...
    for (int j = 0; j < result->imports.count; j++) {
        const EngineImport *imp = &result->imports.items[j];
        if (!imp->module_path) {
            continue;
        }
...
int engine_pipeline_pass_definitions(engine_pipeline_ctx_t *ctx, const engine_file_info_t *files,
                                  int file_count) {
    engine_log_info("pass.start", "pass", "definitions", "files", itoa_log(file_count));
     ...
     * here so the first engine_extract_file below recreates a fresh parser. */
    ...
    EngineFileResult **local_cache = ctx->result_cache;
    bool owns_local_cache = false;
    if (!local_cache) {
        local_cache = (EngineFileResult **)calloc((size_t)file_count, sizeof(EngineFileResult *));
    ...
    for (int i = 0; i < file_count; i++) {
        if (engine_pipeline_check_cancel(ctx)) {
            ...
            if (owns_local_cache) {
                for (int j = 0; j < file_count; j++) {
                    if (local_cache[j]) {
                        engine_free_result(local_cache[j]);
                    }
                }
                free(local_cache);
            }
         ...
         * but hit the hard guard inside engine_extract_file, so they no-op without
         * re-crashing and without duplicating the skip. No-op unless
         * ENGINE_INDEX_QUARANTINE_FILE is set. */
        if (engine_index_is_quarantined(rel)) {
            const char *phase = engine_index_quarantine_phase(rel);
            if (!phase) {
                phase = "crash";
            }
            const char *reason =
                (strcmp(phase, "hang") == 0) ? "quarantined after hang" : "quarantined after crash";
            engine_pipeline_add_file_error(ctx->pipeline, rel, reason, phase);
            errors++;
            continue;
        }
        ...
        if (!source) {
            ...
            if (rst == ENGINE_READ_OVERSIZED) {
                ...
                engine_pipeline_add_file_error(ctx->pipeline, rel, reason, "oversized");
                engine_log_warn("index.file_oversized", "path", rel, "size_mb",
                ...
                engine_pipeline_add_file_error(ctx->pipeline, rel, "read failed", "read");
            }
            /* ENGINE_READ_EMPTY: benign 0-byte file — nothing to index, not reported. */
            continue;
        }


        EngineFileResult *result =
                engine_extract_file_ex(
                      source, source_len, lang, ctx->project_name, rel, ENGINE_EXTRACT_BUDGET, NULL,
                      NULL /* no extra defines or include paths */, ctx->macro_table, NULL);
        free(source);

        if (!result) {
            errors++;
            engine_pipeline_add_file_error(ctx->pipeline, rel, "extract failed", "extract");
            continue;
        }
        ...
        if (result->has_error) {
            engine_pipeline_add_file_error(ctx->pipeline, rel,
    ...
    if (local_cache) {
        ...
        EngineHashTable *namespace_map =
            engine_pipeline_namespace_map_build(ctx->project_name, local_cache, rels, file_count);
        free(rels);
        for (int i = 0; i < file_count; i++) {
            if (engine_pipeline_check_cancel(ctx)) {
                break;
            }
            EngineFileResult *result = local_cache[i];
            if (!result) {
                continue;
            }
            total_imports +=
            ...
            engine_pipeline_create_env_configures_for_file(ctx, result, files[i].rel_path);
        }
        engine_pipeline_namespace_map_free(namespace_map);
        if (owns_local_cache) {
            for (int i = 0; i < file_count; i++) {
                if (local_cache[i]) {
                    engine_free_result(local_cache[i]);
                }
            }
            free(local_cache);
        }
    ...
    engine_log_info("pass.done", "pass", "definitions", "defs", itoa_log(total_defs), "calls",
