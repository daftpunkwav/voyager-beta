...
#define ENGINE_NS_PER_MS 1000000.0
...
static atomic_int g_incr_test_last_route = ENGINE_INCREMENTAL_ROUTE_NONE;
...
engine_incremental_route_t engine_pipeline_incremental_test_last_route(void) {
    return (engine_incremental_route_t)atomic_load(&g_incr_test_last_route);
}
...
void engine_pipeline_incremental_test_reset_faults(void) {
    ...
    atomic_store(&g_incr_test_last_route, ENGINE_INCREMENTAL_ROUTE_NONE);
    engine_pipeline_persist_test_reset_faults();
}
...
static void incr_test_set_last_route(engine_incremental_route_t route) {
...
static double elapsed_ms(struct timespec start) {
    ...
    return (s * ENGINE_MS_PER_SEC) + (ns / ENGINE_NS_PER_MS);
}
...
#ifdef __APPLE__
    return ((int64_t)st->st_mtimespec.tv_sec * ENGINE_NS_PER_SEC) + (int64_t)st->st_mtimespec.tv_nsec;
#elif defined(_WIN32)
    return (int64_t)st->st_mtime * ENGINE_NS_PER_SEC;
#else
    return ((int64_t)st->st_mtim.tv_sec * ENGINE_NS_PER_SEC) + (int64_t)st->st_mtim.tv_nsec;
#endif
}

static const char *incr_mode_name(int mode) {
    switch (mode) {
    case ENGINE_MODE_FULL:
        return "full";
    case ENGINE_MODE_MODERATE:
        return "moderate";
    case ENGINE_MODE_FAST:
        return "fast";
    default:
        return "unknown";
    }
...
typedef struct {
    engine_file_hash_t *items;
    int count;
    int cap;
    EngineHashTable *seen_paths;
} semantic_manifest_builder_t;

static int semantic_manifest_cmp(const void *a, const void *b) {
    const engine_file_hash_t *ma = a;
    const engine_file_hash_t *mb = b;
    return strcmp(ma->rel_path, mb->rel_path);
}

static bool semantic_manifest_is_virtual_path(const char *rel_path) {
    return rel_path &&
           strncmp(rel_path, ENGINE_SEMANTIC_INPUT_PREFIX, strlen(ENGINE_SEMANTIC_INPUT_PREFIX)) == 0;
}
...
static int semantic_manifest_hash_file(const char *abs_path, char out[ENGINE_SHA256_HEX_LEN + 1],
                                       int64_t *mtime_ns, int64_t *size) {
    for (int attempt = 0; attempt < 2; attempt++) {
        engine_path_info_t before;
        if (engine_path_info_utf8(abs_path, &before) != 0 || !before.is_regular || before.is_symlink) {
            return ENGINE_NOT_FOUND;
        }
        FILE *f = engine_fopen(abs_path, "rb");
        if (!f) {
            return ENGINE_NOT_FOUND;
        }
        engine_sha256_ctx sha;
        engine_sha256_init(&sha);
        unsigned char buf[ENGINE_SZ_64K];
        size_t n;
        while ((n = fread(buf, 1, sizeof(buf), f)) > 0) {
            engine_sha256_update(&sha, buf, n);
        }
        bool read_ok = !ferror(f);
        if (fclose(f) != 0) {
            read_ok = false;
        }
        engine_path_info_t after;
        if (!read_ok || engine_path_info_utf8(abs_path, &after) != 0 || !after.is_regular ||
            after.is_symlink) {
            return ENGINE_NOT_FOUND;
        }
    ...
    engine_log_warn("semantic_manifest.raced", "path", abs_path);
    return ENGINE_NOT_FOUND;
}
...
static int semantic_manifest_add_digest(semantic_manifest_builder_t *builder, const char *project,
    ...
    if (!builder || !project || !rel_path || !rel_path[0] || !sha ||
        strlen(sha) != ENGINE_SHA256_HEX_LEN || engine_ht_has(builder->seen_paths, rel_path)) {
        return ENGINE_NOT_FOUND;
    }
    for (int i = 0; i < ENGINE_SHA256_HEX_LEN; i++) {
        if (!((sha[i] >= '0' && sha[i] <= '9') || (sha[i] >= 'a' && sha[i] <= 'f'))) {
            return ENGINE_NOT_FOUND;
        }
    }
    if (builder->count == builder->cap) {
        int new_cap = builder->cap > 0 ? builder->cap * 2 : ENGINE_SZ_64;
        engine_file_hash_t *grown = realloc(builder->items, (size_t)new_cap * sizeof(*builder->items));
        if (!grown) {
            return ENGINE_NOT_FOUND;
        }
        builder->items = grown;
    ...
    uint32_t before_count = engine_ht_count(builder->seen_paths);
    engine_ht_set(builder->seen_paths, path_copy, path_copy);
    if (engine_ht_count(builder->seen_paths) != before_count + 1U) {
        free(project_copy);
        free(path_copy);
        free(sha_copy);
        return ENGINE_NOT_FOUND;
    }
    builder->items[builder->count++] = (engine_file_hash_t){
...
static int semantic_manifest_add(semantic_manifest_builder_t *builder, const char *project,
    ...
    if (engine_ht_has(builder->seen_paths, rel_path)) {
        return 0;
    }
    char sha[ENGINE_SHA256_HEX_LEN + 1];
    int64_t mtime_ns = 0;
    int64_t size = 0;
    if (semantic_manifest_hash_file(abs_path, sha, &mtime_ns, &size) != 0) {
        engine_log_error("semantic_manifest.err", "path", abs_path);
        return ENGINE_NOT_FOUND;
    }
    return semantic_manifest_add_digest(builder, project, rel_path, sha, mtime_ns, size);
}

static void semantic_manifest_git_digest(const engine_git_context_t *git_ctx,
                                         char out[ENGINE_SHA256_HEX_LEN + 1]) {
    static const char domain[] = "engine-semantic-git-context-v1";
    engine_sha256_ctx sha;
    engine_sha256_init(&sha);
...
#define HASH_GIT_BOOL(field)                                      \
    do {                                                          \
        ...
        engine_sha256_update(&sha, #field, sizeof(#field));          \
        engine_sha256_update(&sha, &value, sizeof(value));           \
    } while (0)
#define HASH_GIT_STRING(field)                                               \
    do {                                                                     \
        const char *value = git_ctx && git_ctx->field ? git_ctx->field : ""; \
        engine_sha256_update(&sha, #field, sizeof(#field));                     \
        engine_sha256_update(&sha, value, strlen(value) + 1);                   \
    } while (0)
...
static int semantic_manifest_walk_controls(semantic_manifest_builder_t *builder,
    ...
    engine_dir_t *dir = engine_opendir(abs_dir);
    if (!dir) {
        return ENGINE_NOT_FOUND;
    }
    int rc = 0;
    engine_dirent_t *entry;
    while (rc == 0 && (entry = engine_readdir(dir)) != NULL) {
        const char *name = entry->name;
        if (strcmp(name, ".") == 0 || strcmp(name, "..") == 0) {
            continue;
        }
        char abs_path[ENGINE_SZ_4K];
        char rel_path[ENGINE_SZ_4K];
        ...
        if (engine_path_info_utf8(abs_path, &path_info) != 0) {
            rc = ENGINE_NOT_FOUND;
            break;
        }
        ...
        if (path_info.is_directory) {
            if (engine_should_skip_dir(name, ENGINE_MODE_FULL) ||
                engine_pipeline_relpath_is_excluded(rel_path, excluded_dirs, excluded_count)) {
                continue;
            }
        ...
        bool root_control =
            (!rel_dir || !rel_dir[0]) &&
            (strcmp(name, ".engineignore") == 0 || strcmp(name, ".graph-engine.json") == 0);
...
typedef struct {
    ...
    int worker_id;
    int worker_count;
    char (*shas)[ENGINE_SHA256_HEX_LEN + 1];
    int64_t *mtimes;
    int64_t *sizes;
    int8_t *rcs;
...
int engine_pipeline_build_semantic_manifest(const char *project, const char *repo_path,
    ...
    *out_count = 0;
    ...
    uint64_t reserve = (uint64_t)(file_count > 0 ? file_count : 0) * 2U + ENGINE_SZ_64;
    if (reserve > UINT32_MAX) {
        reserve = UINT32_MAX;
    }
    builder.seen_paths = engine_ht_create((uint32_t)reserve);
    if (!builder.seen_paths) {
        return ENGINE_NOT_FOUND;
    }
    int rc = 0;
    char git_digest[ENGINE_SHA256_HEX_LEN + 1];
    char absent_config_digest[ENGINE_SHA256_HEX_LEN + 1];
    semantic_manifest_git_digest(git_ctx, git_digest);
    engine_sha256_hex("engine-userconfig-unavailable-v1", strlen("engine-userconfig-unavailable-v1"),
    ...
    rc = semantic_manifest_add_digest(&builder, project, ENGINE_SEMANTIC_INPUT_GIT_CONTEXT, git_digest,
                                      0, 0);
    if (rc == 0) {
        rc = semantic_manifest_add_digest(&builder, project, ENGINE_SEMANTIC_INPUT_GLOBAL_CONFIG,
                                          global_config_digest, 0, 0);
    }
    if (rc == 0) {
        rc = semantic_manifest_add_digest(&builder, project, ENGINE_SEMANTIC_INPUT_PROJECT_CONFIG,
                                          project_config_digest, 0, 0);
    }
    struct timespec t_hash;
    engine_clock_gettime(CLOCK_MONOTONIC, &t_hash);
    int hash_workers = engine_default_worker_count(true);
    if (rc == 0 && file_count >= MANIFEST_PARALLEL_MIN_FILES && hash_workers > SKIP_ONE) {
        char (*shas)[ENGINE_SHA256_HEX_LEN + 1] = malloc((size_t)file_count * sizeof(*shas));
        ...
        engine_thread_t *threads = calloc((size_t)hash_workers, sizeof(engine_thread_t));
        ...
        if (shas && mtimes && sizes && rcs && threads && workers) {
            ...
            for (int w = 0; w < hash_workers; w++) {
                workers[w] = (manifest_hash_worker_t){.files = files,
                                                      ...
                                                      .sizes = sizes,
                                                      .rcs = rcs};
                if (engine_thread_create(&threads[w], 0, manifest_hash_worker, &workers[w]) != 0) {
                    break;
                }
                spawned++;
            }
            ...
            for (int i = 0; rc == 0 && i < file_count; i++) {
                ...
                if (rcs[i] != 0) {
                    engine_log_error("semantic_manifest.err", "path", files[i].path);
                    rc = ENGINE_NOT_FOUND;
                    break;
                }
                if (engine_ht_has(builder.seen_paths, files[i].rel_path)) {
                    continue;
                }
    ...
    engine_log_info("pass.timing", "pass", "semantic_manifest_hash", "files", itoa_buf(file_count),
    ...
    if (rc == 0) {
        engine_path_alias_collection_t *aliases =
            engine_load_path_aliases_excluded(repo_path, excluded_dirs, excluded_count);
        if (aliases) {
            ...
            engine_path_alias_collection_free(aliases);
        }
    }
    if (rc != 0) {
        engine_ht_free(builder.seen_paths);
        engine_pipeline_free_semantic_manifest(builder.items, builder.count);
        return rc;
    }
    engine_ht_free(builder.seen_paths);
...
bool engine_pipeline_semantic_manifests_equal(const engine_file_hash_t *left, int left_count,
    ...
    uint64_t reserve = (uint64_t)(right_count > 0 ? right_count : 0) * 2U + ENGINE_SZ_64;
    if (reserve > UINT32_MAX) {
        reserve = UINT32_MAX;
    }
    EngineHashTable *by_path = engine_ht_create((uint32_t)reserve);
    if (!by_path) {
        return false;
    }
    bool equal = true;
    for (int i = 0; equal && i < right_count; i++) {
        if (!right[i].rel_path || !right[i].sha256 || engine_ht_has(by_path, right[i].rel_path)) {
            equal = false;
            break;
        }
        uint32_t before = engine_ht_count(by_path);
        engine_ht_set(by_path, right[i].rel_path, (void *)right[i].sha256);
        equal = engine_ht_count(by_path) == before + 1U;
    }
    for (int i = 0; equal && i < left_count; i++) {
        ...
        const char *sha = engine_ht_get(by_path, left[i].rel_path);
        equal = sha && strcmp(sha, left[i].sha256) == 0;
    }
    engine_ht_free(by_path);
    return equal;
}
...
int engine_pipeline_build_fresh_semantic_manifest(const char *project, const char *repo_path, int mode,
    ...
    *out = NULL;
    *out_count = 0;
    engine_discover_opts_t opts = {
        .mode = (engine_index_mode_t)mode,
        .ignore_file = NULL,
        .max_file_size = 0,
    };
    engine_file_info_t *fresh_files = NULL;
    int fresh_file_count = 0;
    char **fresh_excluded = NULL;
    int fresh_excluded_count = 0;
    engine_ignored_file_t *fresh_ignored = NULL;
    int fresh_ignored_count = 0;
    int fresh_ignored_total = 0;
    engine_userconfig_t *fresh_userconfig = engine_userconfig_load(repo_path);
    engine_git_context_t fresh_git_ctx = {0};
    const engine_userconfig_t *previous_userconfig = engine_get_user_lang_config();
    int rc = fresh_userconfig ? engine_git_context_resolve(repo_path, &fresh_git_ctx) : ENGINE_NOT_FOUND;
    if (rc == 0) {
        engine_set_user_lang_config(fresh_userconfig);
        rc = engine_discover_ex2(repo_path, &opts, &fresh_files, &fresh_file_count, &fresh_excluded,
                              &fresh_excluded_count, &fresh_ignored, &fresh_ignored_count,
                              &fresh_ignored_total);
    }
    if (rc == 0) {
        rc = engine_pipeline_build_semantic_manifest(project, repo_path, fresh_files, fresh_file_count,
                                                  fresh_excluded, fresh_excluded_count,
                                                  &fresh_git_ctx, fresh_userconfig, out, out_count);
    }
    engine_set_user_lang_config(previous_userconfig);
    engine_git_context_free(&fresh_git_ctx);
    engine_userconfig_free(fresh_userconfig);
    engine_discover_free(fresh_files, fresh_file_count);
    engine_discover_free_excluded(fresh_excluded, fresh_excluded_count);
    engine_discover_free_ignored(fresh_ignored, fresh_ignored_count);
    return rc;
}
...
static bool *classify_files(engine_file_info_t *files, int file_count, engine_file_hash_t *stored,
    ...
    EngineHashTable *ht =
        engine_ht_create(stored_count > 0 ? (size_t)stored_count * PAIR_LEN : ENGINE_SZ_64);
    for (int i = 0; i < stored_count; i++) {
        engine_ht_set(ht, stored[i].rel_path, &stored[i]);
    }

    for (int i = 0; i < file_count; i++) {
        engine_file_hash_t *h = engine_ht_get(ht, files[i].rel_path);
        if (!h) {
            /* New file */
            changed[i] = true;
            n_changed++;
 ...
 * claude-connectors/graph-engine-index-repository-is-destructive-...
 ...
 *   - snprintf truncation (combined path ≥ ENGINE_SZ_4K) → preserve. We can't
...
static int find_deleted_files(const char *repo_path, engine_file_info_t *files, int file_count,
                              engine_file_hash_t *stored, int stored_count, char ***out_deleted,
                              engine_file_hash_t **out_mode_skipped, int *out_mode_skipped_count) {
    *out_deleted = NULL;
    ...
    *out_mode_skipped_count = 0;
    ...
    if (!repo_path) {
        ...
        engine_log_error("incremental.err", "msg", "find_deleted_files_null_repo_path");
        return 0;
    }

    EngineHashTable *current = engine_ht_create((size_t)file_count * PAIR_LEN);
    for (int i = 0; i < file_count; i++) {
        engine_ht_set(current, files[i].rel_path, &files[i]);
    }

    int del_count = 0;
    int del_cap = ENGINE_SZ_64;
    char **deleted = malloc((size_t)del_cap * sizeof(char *));
    if (!deleted) {
        engine_log_error("incremental.err", "msg", "find_deleted_files_oom");
        engine_ht_free(current);
        return 0;
    }

    int ms_count = 0;
    int ms_cap = ENGINE_SZ_64;
    engine_file_hash_t *mode_skipped = malloc((size_t)ms_cap * sizeof(engine_file_hash_t));
    if (!mode_skipped) {
        engine_log_error("incremental.err", "msg", "find_deleted_files_oom_ms");
        free(deleted);
        engine_ht_free(current);
        return 0;
    }
    ...
    for (int i = 0; i < stored_count; i++) {
        ...
        if (engine_ht_get(current, stored[i].rel_path)) {
        ...
        if (n < 0 || n >= (int)sizeof(abs_path)) {
            ...
            engine_log_warn("incremental.path_truncated", "rel_path", stored[i].rel_path);
            preserve = true;
        } else {
            struct stat st;
            if (stat(abs_path, &st) == 0) {
                ...
                engine_log_warn("incremental.stat_uncertain", "rel_path", stored[i].rel_path, "errno",
                             itoa_buf(errno));
                preserve = true;
            }
        }
        ...
        if (preserve) {
            ...
            if (ms_count >= ms_cap) {
                ms_cap *= PAIR_LEN;
                engine_file_hash_t *tmp = realloc(mode_skipped, (size_t)ms_cap * sizeof(*tmp));
                if (!tmp) {
                    engine_log_error("incremental.err", "msg", "find_deleted_files_realloc_oom_ms");
                    break;
                }
                mode_skipped = tmp;
            }
            ...
            if (!rp || (stored[i].sha256 && !sh)) {
                 ...
                 * orphaned-node bug. */
                engine_log_error("incremental.err", "msg", "find_deleted_files_strdup_oom", "rel_path",
                              stored[i].rel_path);
                free(rp);
                free(sh);
                break;
        ...
        if (del_count >= del_cap) {
            ...
            if (!tmp) {
                engine_log_error("incremental.err", "msg", "find_deleted_files_realloc_oom");
                break;
            }
            deleted = tmp;
        }
...
static void free_mode_skipped(engine_file_hash_t *ms, int count) {
    if (!ms) {
        return;
    }
    for (int i = 0; i < count; i++) {
 ...
 *
 * The purge step (engine_gbuf_delete_by_file) removes a changed file's nodes,
 ...
 *   - engine_gbuf_insert_edge dedups, so re-linking an edge the resolver already
...
typedef struct {
    engine_gbuf_t *gbuf;
    EngineHashTable *changed_paths; /* rel_path -> non-NULL sentinel (membership set) */
    engine_saved_edge_t *items;
    int count;
    int cap;
} engine_edge_capture_t;
...
/* engine_gbuf_foreach_edge visitor: snapshot inbound cross-file edges into
 * changed files so they survive the purge and can be re-linked afterward. */
static void incr_capture_inbound_edge(const engine_gbuf_edge_t *edge, void *userdata) {
    engine_edge_capture_t *cap = (engine_edge_capture_t *)userdata;
    if (incr_edge_type_is_recomputed(edge->type)) {
        return;
    }
    const engine_gbuf_node_t *src = engine_gbuf_find_by_id(cap->gbuf, edge->source_id);
    const engine_gbuf_node_t *tgt = engine_gbuf_find_by_id(cap->gbuf, edge->target_id);
    ...
    if (!engine_ht_get(cap->changed_paths, tgt->file_path) ||
        engine_ht_get(cap->changed_paths, src->file_path)) {
        return;
    }
    if (cap->count >= cap->cap) {
        int ncap = (cap->cap > 0) ? cap->cap * PAIR_LEN : ENGINE_SZ_64;
        engine_saved_edge_t *tmp = realloc(cap->items, (size_t)ncap * sizeof(*tmp));
        if (!tmp) {
            engine_log_warn("incremental.edge_snapshot_oom", "captured", itoa_buf(cap->count));
            return; /* best-effort: stop capturing, keep what we have */
        }
        cap->items = tmp;
        cap->cap = ncap;
    }
    engine_saved_edge_t *s = &cap->items[cap->count];
...
static int incr_restore_inbound_edges(engine_gbuf_t *gbuf, engine_edge_capture_t *cap) {
    int restored = 0;
    for (int i = 0; i < cap->count; i++) {
        engine_saved_edge_t *s = &cap->items[i];
        const engine_gbuf_node_t *src = engine_gbuf_find_by_qn(gbuf, s->source_qn);
        const engine_gbuf_node_t *tgt = engine_gbuf_find_by_qn(gbuf, s->target_qn);
        if (src && tgt) {
            engine_gbuf_insert_edge(gbuf, src->id, tgt->id, s->type, s->props);
            restored++;
        }
    }
    return restored;
...
static void incr_free_edge_capture(engine_edge_capture_t *cap) {
    for (int i = 0; i < cap->count; i++) {
...
static bool incr_label_is_registry_symbol(const char *label) {
    ...
    return label && (strcmp(label, "Function") == 0 || strcmp(label, "Method") == 0 ||
                     engine_label_is_type_like(label) || strcmp(label, "Variable") == 0 ||
                     strcmp(label, "Field") == 0);
}

/* Callback for engine_gbuf_foreach_node: seed the registry with the existing
...
static void registry_visitor(const engine_gbuf_node_t *node, void *userdata) {
    engine_registry_t *r = (engine_registry_t *)userdata;
    if (!incr_label_is_registry_symbol(node->label)) {
        return;
    }
    engine_registry_add(r, node->name, node->qualified_name, node->label);
}

static void free_incremental_result_cache(EngineFileResult **cache, int count) {
    if (!cache) {
        return;
    }
    for (int i = 0; i < count; i++) {
        engine_free_result(cache[i]);
    }
    free(cache);
}
...
typedef struct {
    ...
    engine_lsp_surface_row_t *fresh_rows;
    int fresh_count;
    char **def_modules;
    ...
    EngineArena *rehydrate_arenas;
    int rehydrate_arena_count;
} closure_resolve_t;

typedef struct {
    engine_file_info_t *files; /* heap array; entries borrow strings from the
                             * caller's discovery array */
    int count;
    int n_changed;
    int n_dependents;
    engine_lsp_surface_row_t *stored_rows; /* whole previous generation */
    int stored_count;
} closure_plan_t;

static void closure_plan_free(closure_plan_t *plan) {
    free(plan->files);
    engine_store_free_lsp_surfaces(plan->stored_rows, plan->stored_count);
...
static EngineHashTable *surface_name_set(const char *defs_json) {
    ...
    EngineHashTable *set = engine_ht_create(ENGINE_SZ_64);
    if (!set) {
        yyjson_doc_free(doc);
        return NULL;
    }
    ...
    for (int a = 0; a < 2; a++) {
        ...
        yyjson_arr_foreach(arrs[a], idx, max, item) {
            ...
            if (name && name[0] && !engine_ht_get(set, name)) {
                char *copy = strdup(name);
                if (copy) {
                    engine_ht_set(set, copy, copy);
                }
            }
        }
    }
...
static void surface_name_set_free(EngineHashTable *set) {
    if (!set) {
        return;
    }
    engine_ht_foreach(set, surface_name_free_visitor, NULL);
    engine_ht_free(set);
}

typedef struct {
    EngineHashTable *stored_set;
    bool added;
...
static void surface_added_visitor(const char *key, void *value, void *userdata) {
    ...
    if (!check->added && !engine_ht_get(check->stored_set, key)) {
        check->added = true;
    }
}
...
static int surface_added_names(const char *stored_json, const char *fresh_json, bool *out_added) {
    *out_added = true; /* fail closed */
    EngineHashTable *stored_set = surface_name_set(stored_json);
    EngineHashTable *fresh_set = surface_name_set(fresh_json);
    int rc = (stored_set && fresh_set) ? 0 : -1;
    if (rc == 0) {
        surface_added_check_t check = {.stored_set = stored_set, .added = false};
        engine_ht_foreach(fresh_set, surface_added_visitor, &check);
...
static int run_extract_resolve(engine_pipeline_ctx_t *ctx, engine_file_info_t *changed_files, int ci,
...
#define MIN_FILES_FOR_PARALLEL_INCR 50
    int worker_count = engine_default_worker_count(true);
    bool use_parallel =
        closure != NULL || (worker_count > SKIP_ONE && ci > MIN_FILES_FOR_PARALLEL_INCR);

    if (use_parallel) {
        engine_log_info("incremental.mode", "mode", "parallel", "workers", itoa_buf(worker_count),
                     "changed", itoa_buf(ci));

        _Atomic int64_t shared_ids;
        atomic_init(&shared_ids, engine_gbuf_next_id(ctx->gbuf));

        EngineFileResult **cache;
#if defined(ENGINE_INCREMENTAL_TEST_API) && ENGINE_INCREMENTAL_TEST_API
...
#endif
        {
            cache = (EngineFileResult **)calloc((size_t)ci, sizeof(EngineFileResult *));
        }
        if (!cache) {
            engine_log_error("incremental.err", "phase", "result_cache_alloc");
            return ENGINE_NOT_FOUND;
        }

        engine_clock_gettime(CLOCK_MONOTONIC, &t);
        int rc = engine_parallel_extract(ctx, changed_files, ci, cache, &shared_ids, worker_count);
        if (rc == 0) {
            rc = engine_pipeline_check_cancel(ctx);
        }
        engine_log_info("pass.timing", "pass", "incr_extract", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        if (rc != 0) {
            free_incremental_result_cache(cache, ci);
            return rc;
        }
        engine_gbuf_set_next_id(ctx->gbuf, atomic_load(&shared_ids));

        engine_clock_gettime(CLOCK_MONOTONIC, &t);
        rc = engine_build_registry_from_cache(ctx, changed_files, ci, cache);
        if (rc == 0) {
            rc = engine_pipeline_check_cancel(ctx);
        }
        engine_log_info("pass.timing", "pass", "incr_registry", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        if (rc != 0) {
            free_incremental_result_cache(cache, ci);
            return rc;
        ...
        int64_t registry_next_id = engine_gbuf_next_id(ctx->gbuf);
        if (registry_next_id > atomic_load(&shared_ids)) {
            atomic_store(&shared_ids, registry_next_id);
        }

        engine_clock_gettime(CLOCK_MONOTONIC, &t);
        EngineLSPDef *all_defs = NULL;
        int all_def_count = 0;
        char **def_modules = NULL;
        EngineModuleDefIndex *module_def_index = NULL;
        EngineCrossLspRegistries cross_registries = {0};
        EngineCrossLspRegistries *registries_arg = NULL;
        if (closure) {
            ...
            int fresh_count = 0;
            EngineLSPDef *fresh_defs =
                def_modules && def_starts
                    ? engine_pxc_collect_all_defs(cache, changed_files, ci, ctx->project_name,
                                               def_modules, &fresh_count, def_starts)
                    : NULL;
            if ((fresh_defs || fresh_count == 0) && def_starts &&
                engine_lsp_surface_build_rows(ctx->project_name, cache, changed_files, ci, fresh_defs,
            ...
            if (all_def_count > 0) {
                all_defs = (EngineLSPDef *)engine_arena_alloc(&closure->arena,
                                                        (size_t)all_def_count * sizeof(EngineLSPDef));
            }
            if (all_defs) {
                if (closure->base_def_count > 0) {
                    memcpy(all_defs, closure->base_defs,
                           (size_t)closure->base_def_count * sizeof(EngineLSPDef));
                }
                if (fresh_count > 0) {
                    memcpy(all_defs + closure->base_def_count, fresh_defs,
                           (size_t)fresh_count * sizeof(EngineLSPDef));
                }
                module_def_index = engine_pxc_build_module_def_index(all_defs, all_def_count);
                ...
                if (ci > CLOSURE_BUDGET_FLOOR_FILES) {
                    EngineArena *xa = &closure->arena;
                    cross_registries.go = engine_go_build_cross_registry(xa, all_defs, all_def_count);
                    cross_registries.python =
                        engine_py_build_cross_registry(xa, all_defs, all_def_count);
                    cross_registries.c = engine_c_build_cross_registry(xa, all_defs, all_def_count);
                    cross_registries.cs = engine_cs_build_cross_registry(xa, all_defs, all_def_count);
                    cross_registries.ts = engine_ts_build_cross_registry(xa, all_defs, all_def_count);
            ...
            def_modules = NULL;
            engine_log_info("incremental.closure_registries", "base",
                         itoa_buf(closure->base_def_count), "fresh", itoa_buf(fresh_count),
                         "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
        }
        engine_clock_gettime(CLOCK_MONOTONIC, &t);
        rc = engine_parallel_resolve(ctx, changed_files, ci, cache, &shared_ids, worker_count,
                                  all_defs, all_def_count, closure ? closure->def_modules : NULL,
                                  module_def_index, registries_arg);
        if (module_def_index) {
            engine_pxc_free_module_def_index(module_def_index);
        }
        if (rc == 0) {
            rc = engine_pipeline_check_cancel(ctx);
        }
        engine_log_info("pass.timing", "pass", "incr_resolve", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        engine_gbuf_set_next_id(ctx->gbuf, atomic_load(&shared_ids));
        free_incremental_result_cache(cache, ci);
        return rc;
    } else {
        engine_log_info("incremental.mode", "mode", "sequential", "changed", itoa_buf(ci));
        ...
        EngineFileResult **prior_cache = ctx->result_cache;
        EngineFileResult **cache = prior_cache;
        ...
        int rc = engine_pipeline_pass_definitions(ctx, changed_files, ci);
        if (rc == 0) {
            rc = engine_pipeline_check_cancel(ctx);
        }
        if (rc == 0) {
            rc = engine_pipeline_pass_calls(ctx, changed_files, ci);
        }
        if (rc == 0) {
            rc = engine_pipeline_check_cancel(ctx);
        }
        if (rc == 0) {
            rc = engine_pipeline_pass_usages(ctx, changed_files, ci);
        }
        if (rc == 0) {
            rc = engine_pipeline_check_cancel(ctx);
        }
        if (rc == 0) {
            rc = engine_pipeline_pass_semantic(ctx, changed_files, ci);
        }
        if (rc == 0) {
            rc = engine_pipeline_check_cancel(ctx);
        }
...
static int run_postpasses(engine_pipeline_ctx_t *ctx, engine_file_info_t *changed_files, int ci,
                          const char *project) {
    struct timespec t;

    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    int rc = engine_pipeline_pass_tests(ctx, changed_files, ci);
    engine_log_info("pass.timing", "pass", "incr_tests", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    if (rc != 0 || engine_pipeline_check_cancel(ctx)) {
        return rc != 0 ? rc : ENGINE_NOT_FOUND;
    }

    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    rc = engine_pipeline_pass_decorator_tags(ctx->gbuf, project);
    engine_log_info("pass.timing", "pass", "incr_decorator_tags", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));
    if (rc < 0 || engine_pipeline_check_cancel(ctx)) {
        return rc < 0 ? rc : ENGINE_NOT_FOUND;
    }
    ...
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    rc = engine_pipeline_pass_configlink(ctx);
    engine_log_info("pass.timing", "pass", "incr_configlink", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));
    if (rc < 0 || engine_pipeline_check_cancel(ctx)) {
        return rc < 0 ? rc : ENGINE_NOT_FOUND;
    }
    ...
    if (ctx->mode <= ENGINE_MODE_MODERATE) {
        engine_clock_gettime(CLOCK_MONOTONIC, &t);
        rc = engine_pipeline_pass_similarity(ctx);
        engine_log_info("pass.timing", "pass", "incr_similarity", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        if (rc < 0 || engine_pipeline_check_cancel(ctx)) {
            return rc < 0 ? rc : ENGINE_NOT_FOUND;
        }
        ...
        engine_clock_gettime(CLOCK_MONOTONIC, &t);
        rc = engine_pipeline_pass_semantic_edges(ctx);
        engine_log_info("pass.timing", "pass", "incr_semantic_edges", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        if (rc < 0 || engine_pipeline_check_cancel(ctx)) {
            return rc < 0 ? rc : ENGINE_NOT_FOUND;
        }
...
static int dump_and_persist(engine_gbuf_t *gbuf, const char *db_path, const char *project,
                            atomic_int *cancelled, const engine_file_hash_t *manifest,
                            int manifest_count, const char *adr_content, const char *repo_path,
                            const engine_coverage_row_t *cov, int cov_count,
                            const engine_coverage_meta_t *meta_template,
                            const engine_lsp_surface_row_t *surface_rows, int surface_row_count) {
    struct timespec t;
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    engine_pipeline_generation_t generation = {
        .gbuf = gbuf,
        ...
        .coverage_meta = meta_template ? *meta_template : (engine_coverage_meta_t){0},
        .surface_rows = surface_rows,
        .surface_row_count = surface_row_count,
    };
    int rc = engine_pipeline_publish_generation(&generation);
    engine_log_info("incremental.dump", "rc", itoa_buf(rc), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));
    if (rc != 0) {
        return rc;
    }
    ...
    if (repo_path && engine_artifact_exists(repo_path)) {
        engine_artifact_export(db_path, repo_path, project, ENGINE_ARTIFACT_FAST);
    }
    return 0;
}
...
typedef struct {
    const engine_lsp_surface_row_t *const *rows;
    int row_count;
    int worker_id;
    int worker_count;
    EngineArena *arena;      /* this worker's arena (owned by cr) */
    EngineLSPDef **row_defs; /* per-row result pointers */
...
static void *rehydrate_worker(void *arg) {
    ...
    for (int i = w->worker_id; i < w->row_count; i += w->worker_count) {
        EngineLSPDef *defs = NULL;
        int count = engine_lsp_surface_defs_from_json(w->arena, w->rows[i]->defs_json, &defs);
        w->row_defs[i] = defs;
...
static int closure_probe_surfaces(engine_pipeline_t *p, const char *project,
                                  engine_file_info_t *probe_files, int probe_count,
                                  engine_lsp_surface_row_t **out_rows, int *out_count) {
    *out_rows = NULL;
    *out_count = 0;
    engine_gbuf_t *probe_gbuf = engine_gbuf_new(project, engine_pipeline_repo_path(p));
    if (!probe_gbuf) {
        return -1;
    }
    ...
    engine_path_alias_collection_t *aliases =
        engine_load_path_aliases_excluded(engine_pipeline_repo_path(p), excluded_dirs, excluded_count);
    engine_pipeline_ctx_t probe_ctx = {
        .project_name = project,
        .repo_path = engine_pipeline_repo_path(p),
        .gbuf = probe_gbuf,
        .registry = NULL,
        .cancelled = engine_pipeline_cancelled_ptr(p),
        .pipeline = NULL, /* probe must not record file errors twice */
        .mode = engine_pipeline_get_mode(p),
        .path_aliases = aliases,
        .excluded_dirs = excluded_dirs,
        .excluded_count = excluded_count,
    };
    EngineFileResult **cache = (EngineFileResult **)calloc((size_t)probe_count, sizeof(EngineFileResult *));
    int rc = cache ? 0 : -1;
    if (rc == 0) {
        _Atomic int64_t probe_ids;
        atomic_init(&probe_ids, engine_gbuf_next_id(probe_gbuf));
        rc = engine_parallel_extract(&probe_ctx, probe_files, probe_count, cache, &probe_ids,
                                  engine_default_worker_count(true));
    }
    if (rc == 0) {
        ...
        if (def_modules && def_starts) {
            defs = engine_pxc_collect_all_defs(cache, probe_files, probe_count, project, def_modules,
                                            &def_count, def_starts);
            rc = engine_lsp_surface_build_rows(project, cache, probe_files, probe_count, defs,
    ...
    engine_pkgmap_free(engine_pipeline_get_pkgmap());
    engine_pipeline_set_pkgmap(NULL);
    engine_path_alias_collection_free(aliases);
    engine_gbuf_free(probe_gbuf);
    return rc;
}
...
static bool closure_is_alias_config(const engine_path_alias_collection_t *aliases,
                                    const char *rel_path) {
    if (!rel_path) {
        return false;
    }
...
static void closure_seed_governed(EngineHashTable *closure_set, const engine_file_info_t *files,
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (!engine_ht_get(closure_set, files[i].rel_path)) {
            engine_ht_set(closure_set, files[i].rel_path, (void *)files[i].rel_path);
        }
    }
}
...
static int closure_try_plan(engine_pipeline_t *p, engine_store_t *store, const char *project,
                            engine_file_info_t *files, int file_count, const engine_file_hash_t *stored,
                            int stored_count, const engine_file_hash_t *fresh, int fresh_count,
                            closure_plan_t *plan) {
    memset(plan, 0, sizeof(*plan));
    struct timespec t;
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    ...
    EngineHashTable *fresh_by_path = engine_ht_create((size_t)fresh_count * PAIR_LEN + ENGINE_SZ_64);
    EngineHashTable *files_by_path = engine_ht_create((size_t)file_count * PAIR_LEN + ENGINE_SZ_64);
    EngineHashTable *stored_by_path = engine_ht_create((size_t)stored_count * PAIR_LEN + ENGINE_SZ_64);
    EngineHashTable *closure_set = engine_ht_create(ENGINE_SZ_64);
    char **changed_paths = NULL;
    char **dependents = NULL;
    int dependent_count = 0;
    engine_lsp_surface_row_t *probe_rows = NULL;
    int probe_count = 0;
    engine_file_info_t *probe_files = NULL;
    engine_path_alias_collection_t *plan_aliases = NULL;
    int n_changed = 0;
    int n_deleted = 0;
    ...
    for (int i = 0; i < file_count; i++) {
        engine_ht_set(files_by_path, files[i].rel_path, &files[i]);
    }
    for (int i = 0; i < fresh_count; i++) {
        engine_ht_set(fresh_by_path, fresh[i].rel_path, (void *)&fresh[i]);
    }
    for (int i = 0; i < stored_count; i++) {
        engine_ht_set(stored_by_path, stored[i].rel_path, (void *)&stored[i]);
    }
    ...
    plan_aliases = engine_load_path_aliases_excluded(engine_pipeline_repo_path(p), NULL, 0);
    bool governed_seeded = false;
    for (int i = 0; i < fresh_count; i++) {
        const engine_file_hash_t *old_row = engine_ht_get(stored_by_path, fresh[i].rel_path);
        bool synthetic = semantic_manifest_is_virtual_path(fresh[i].rel_path);
        bool discovered = engine_ht_get(files_by_path, fresh[i].rel_path) != NULL;
    ...
    for (int i = 0; i < stored_count; i++) {
        if (!engine_ht_get(fresh_by_path, stored[i].rel_path)) {
            if (!engine_ht_get(files_by_path, stored[i].rel_path) &&
    ...
    if (engine_store_get_lsp_surfaces(store, project, &plan->stored_rows, &plan->stored_count) !=
            ENGINE_STORE_OK ||
        plan->stored_count == 0) {
        decline = "no_surface_rows";
        goto done;
    }
    EngineHashTable *rows_by_path = engine_ht_create((size_t)plan->stored_count * PAIR_LEN);
    if (!rows_by_path) {
        decline = "alloc";
        goto done;
    }
    for (int i = 0; i < plan->stored_count; i++) {
        engine_ht_set(rows_by_path, plan->stored_rows[i].rel_path, &plan->stored_rows[i]);
    }

    probe_files =
        (engine_file_info_t *)calloc((size_t)(n_changed ? n_changed : 1), sizeof(engine_file_info_t));
    if (!probe_files) {
        engine_ht_free(rows_by_path);
        decline = "alloc";
        goto done;
    }
    for (int i = 0; i < n_changed; i++) {
        engine_file_info_t *info = engine_ht_get(files_by_path, changed_paths[i]);
        if (!info) {
            engine_ht_free(rows_by_path);
    ...
    for (int i = 0; i < n_changed; i++) {
        const engine_lsp_surface_row_t *stored_row = engine_ht_get(rows_by_path, changed_paths[i]);
        const engine_lsp_surface_row_t *fresh_row = NULL;
    ...
    if (dep_target_count > 0 &&
        engine_store_get_dependent_files(store, project, dep_targets, dep_target_count, &dependents,
                                      &dependent_count) != ENGINE_STORE_OK) {
        free(dep_targets);
    ...
    for (int i = 0; i < n_changed; i++) {
        if (!engine_ht_get(closure_set, changed_paths[i])) {
            engine_ht_set(closure_set, changed_paths[i], changed_paths[i]);
        }
    }
    for (int i = 0; i < dependent_count; i++) {
        if (!engine_ht_get(files_by_path, dependents[i])) {
            decline = "dependent_not_discovered";
            goto done;
        }
        if (!engine_ht_get(closure_set, dependents[i])) {
            engine_ht_set(closure_set, dependents[i], dependents[i]);
        }
    }
     ...
     * the executor. */
    int closure_count = (int)engine_ht_count(closure_set);
    ...
    plan->files = (engine_file_info_t *)calloc((size_t)(closure_count ? closure_count : 1),
                                            sizeof(engine_file_info_t));
    if (!plan->files) {
        decline = "alloc";
        goto done;
    }
    ...
    engine_log_info("incremental.closure_plan", "changed", itoa_buf(n_changed), "surface_changed",
                 itoa_buf(n_surface_changed), "deleted", itoa_buf(n_deleted), "dependents",
                 itoa_buf(dependent_count));
    engine_log_info("incremental.closure_plan_done", "closure", itoa_buf(plan->count), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));

done:
    engine_path_alias_collection_free(plan_aliases);
    free(probe_files);
    engine_store_free_lsp_surfaces(probe_rows, probe_count);
    engine_store_free_dependent_files(dependents, dependent_count);
    free(changed_paths);
    engine_ht_free(fresh_by_path);
    engine_ht_free(files_by_path);
    engine_ht_free(stored_by_path);
    engine_ht_free(closure_set);
    if (decline) {
        engine_log_info("incremental.closure_decline", "reason", decline, "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        engine_store_free_lsp_surfaces(plan->stored_rows, plan->stored_count);
        memset(plan, 0, sizeof(*plan));
        return 0;
 ...
 * ENGINE_PIPELINE_FORCE_FULL_REINDEX: the live database is never touched, so
...
static int run_closure_delta(engine_pipeline_t *p, const char *db_path, const char *project,
                             const engine_file_hash_t *baseline_manifest, int baseline_count,
                             closure_plan_t *plan, engine_file_info_t *changed_files, int ci,
                             char **deleted, int deleted_count, engine_file_hash_t *mode_skipped,
                             int mode_skipped_count, engine_coverage_row_t *old_cov, int old_cov_count,
                             engine_store_t *route_store, struct timespec t0) {
    struct timespec t;
    engine_store_close(route_store);

    int result = ENGINE_PIPELINE_FORCE_FULL_REINDEX;
    char *stage = NULL;
    engine_store_t *staging = NULL;
    engine_delta_saved_edge_t *snapshot = NULL;
    int snapshot_count = 0;
    engine_gbuf_t *gbuf = NULL;
    engine_registry_t *registry = NULL;
    engine_path_alias_collection_t *path_aliases = NULL;
    EngineHashTable *stale_surface_set = NULL;
    closure_resolve_t cr = {0};
    bool cr_arena_live = false;
    const char **purge_paths = NULL;
    engine_file_hash_t *manifest = NULL;
    int manifest_count = 0;
    engine_coverage_row_t *cov = NULL;
    int cov_n = 0;

    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    if (engine_delta_stage_clone(db_path, &stage) != 0) {
        goto out;
    }
    engine_log_info("delta.clone", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    staging = engine_store_open_path(stage);
    if (!staging) {
        goto out;
    }
    engine_log_info("delta.staging_open", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    ...
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    if (engine_delta_snapshot_inbound(staging, project, purge_paths, purge_count, &snapshot,
                                   &snapshot_count) != 0) {
        goto out;
    }
    if (engine_delta_purge(staging, project, purge_paths, purge_count) != 0) {
        goto out;
    }
    engine_log_info("delta.snapshot_purge", "captured", itoa_buf(snapshot_count), "purged_files",
    ...
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    gbuf = engine_gbuf_new(project, engine_pipeline_repo_path(p));
    if (!gbuf) {
        goto out;
    }
    int64_t max_db_id = engine_delta_preseed(staging, project, gbuf);
    if (max_db_id < 0) {
        goto out;
    }
    registry = engine_registry_new();
    if (!registry) {
        goto out;
    }
    engine_gbuf_foreach_node(gbuf, registry_visitor, registry);
    engine_log_info("delta.preseed_done", "registry", itoa_buf(engine_registry_size(registry)),
                 "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    ...
    stale_surface_set = engine_ht_create((size_t)(purge_count)*PAIR_LEN + ENGINE_SZ_64);
    ...
    for (int i = 0; i < purge_count; i++) {
        ...
        engine_ht_set(stale_surface_set, key, key);
    }
    engine_arena_init(&cr.arena);
    cr_arena_live = true;
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    {
        /* Eligible rows first (skip everything being re-parsed or deleted),
         * then fan the JSON decode out across workers. */
        const engine_lsp_surface_row_t **elig = (const engine_lsp_surface_row_t **)calloc(
        ...
        int elig_count = 0;
        for (int i = 0; i < plan->stored_count; i++) {
            const engine_lsp_surface_row_t *row = &plan->stored_rows[i];
            if (!engine_ht_get(stale_surface_set, row->rel_path)) {
                elig[elig_count++] = row;
            }
        }
        int workers = engine_default_worker_count(true);
        if (workers < 1) {
            workers = 1;
        }
        ...
        EngineLSPDef **row_defs =
            (EngineLSPDef **)calloc((size_t)(elig_count ? elig_count : 1), sizeof(*row_defs));
        int *row_counts = (int *)calloc((size_t)(elig_count ? elig_count : 1), sizeof(int));
        cr.rehydrate_arenas = (EngineArena *)calloc((size_t)workers, sizeof(EngineArena));
        engine_thread_t *threads = (engine_thread_t *)calloc((size_t)workers, sizeof(engine_thread_t));
        ...
        for (int w = 0; w < workers; w++) {
            engine_arena_init(&cr.rehydrate_arenas[w]);
        }
        int spawned = 0;
        for (int w = 0; w < workers; w++) {
            ...
            if (engine_thread_create(&threads[w], 0, rehydrate_worker, &wargs[w]) != 0) {
                break;
            }
            spawned++;
        }
        ...
        for (int i = 0; i < elig_count; i++) {
            if (row_counts[i] < 0) {
                engine_log_error("delta.rehydrate_failed", "path", elig[i]->rel_path);
                decode_ok = false;
                break;
            }
            total += row_counts[i];
        ...
        if (decode_ok && total > 0) {
            cr.base_defs =
                (EngineLSPDef *)engine_arena_alloc(&cr.arena, (size_t)total * sizeof(EngineLSPDef));
            if (!cr.base_defs) {
                decode_ok = false;
            } else {
                for (int i = 0; i < elig_count; i++) {
                    if (row_counts[i] > 0) {
                        memcpy(cr.base_defs + cr.base_def_count, row_defs[i],
                               (size_t)row_counts[i] * sizeof(EngineLSPDef));
    ...
    engine_log_info("delta.rehydrate", "defs", itoa_buf(cr.base_def_count), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));
    ...
    for (int i = 0; i < ci; i++) {
        char *file_qn = engine_pipeline_fqn_compute(project, changed_files[i].rel_path, "__file__");
        if (file_qn) {
            const char *rel = changed_files[i].rel_path;
            const char *slash = strrchr(rel, '/');
            const char *basename = slash ? slash + SKIP_ONE : rel;
            char props[ENGINE_SZ_256];
            const char *ext = strrchr(basename, '.');
            snprintf(props, sizeof(props), "{\"extension\":\"%s\"}", ext ? ext : "");
            engine_gbuf_upsert_node(gbuf, "File", basename, file_qn, rel, 0, 0, props);
            free(file_qn);
        }
    }
    ...
    engine_pipeline_set_pkgmap(NULL);
    if (phase_rc != 0) {
        engine_log_error("delta.err", "phase", "extract_resolve", "rc", itoa_buf(phase_rc));
        goto out;
    }
    engine_log_info("delta.repair", "files", itoa_buf(ci), "elapsed_ms", itoa_buf((int)elapsed_ms(t)));

    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    if (engine_delta_patch(staging, project, gbuf, max_db_id, snapshot, snapshot_count) != 0) {
        goto out;
    }
    engine_log_info("delta.patch_done", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));

    /* Coverage: previous failure rows for files not re-extracted + this
     * run's fresh entries — same merge the legacy tail performs. */
    engine_file_error_t *run_errs = NULL;
    int run_err_count = 0;
    engine_pipeline_get_file_errors(p, &run_errs, &run_err_count);
    char **run_excluded = NULL;
    int run_excluded_count = 0;
    engine_pipeline_get_excluded(p, &run_excluded, &run_excluded_count);
    engine_ignored_file_t *run_ignored = NULL;
    int run_ignored_count = 0;
    int run_ignored_total = 0;
    engine_pipeline_get_ignored(p, &run_ignored, &run_ignored_count, &run_ignored_total);
    int cov_cap = old_cov_count + run_err_count + run_excluded_count + run_ignored_count;
    bool coverage_rows_available = cov_cap == 0;
    if (cov_cap > 0) {
        cov = (engine_coverage_row_t *)malloc((size_t)cov_cap * sizeof(*cov));
    ...
    if (cov) {
        for (int i = 0; i < old_cov_count; i++) {
            ...
            if (!by_design && old_cov[i].rel_path &&
                !engine_ht_get(stale_surface_set, old_cov[i].rel_path)) {
                cov[cov_n++] = old_cov[i];
            }
        }
...
#if defined(ENGINE_INCREMENTAL_TEST_API) && ENGINE_INCREMENTAL_TEST_API
    engine_pipeline_persist_test_run_before_final_manifest();
#endif
    if (engine_pipeline_build_fresh_semantic_manifest(project, engine_pipeline_repo_path(p),
                                                   engine_pipeline_get_mode(p), &manifest,
                                                   &manifest_count) != 0 ||
        !engine_pipeline_semantic_manifests_equal(baseline_manifest, baseline_count, manifest,
                                               manifest_count)) {
        engine_log_warn("delta.abort", "reason", "semantic_inputs_changed");
        result = ENGINE_PIPELINE_ABORT_PRESERVE_DB;
        goto out;
    }
    ...
    {
        sqlite3 *sdb = engine_store_get_db(staging);
        sqlite3_stmt *del = NULL;
        bool surf_ok = sdb != NULL;
        if (surf_ok && sqlite3_prepare_v2(sdb,
                                          "DELETE FROM lsp_surface WHERE project = ?1"
                                          " AND rel_path = ?2",
                                          ENGINE_NOT_FOUND, &del, NULL) == SQLITE_OK) {
            for (int i = 0; surf_ok && i < ci + deleted_count; i++) {
                sqlite3_reset(del);
                sqlite3_bind_text(del, 1, project, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
                sqlite3_bind_text(del, 2, purge_paths[i], ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
        ...
        if (!surf_ok || engine_store_upsert_lsp_surface_batch(staging, cr.fresh_rows,
                                                           cr.fresh_count) != ENGINE_STORE_OK) {
            goto out;
        }
    }
    ...
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    engine_pipeline_set_committed_counts(p, engine_store_count_nodes(staging, project),
                                      engine_store_count_edges(staging, project));
    engine_log_info("delta.committed_counts", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));

    {
        int index_mode = engine_pipeline_get_mode(p);
        engine_pipeline_generation_t generation = {
            .gbuf = NULL,
            .final_db_path = db_path,
            .project = project,
            ...
            .coverage_meta =
                {
                    ...
                    .coverage_version = ENGINE_SEMANTIC_INDEX_VERSION,
        ...
        engine_store_close(staging);
        staging = NULL;
        engine_clock_gettime(CLOCK_MONOTONIC, &t);
        result = engine_pipeline_publish_staged(stage, &generation, false, true);
        engine_log_info("delta.publish_total", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
        stage = NULL; /* publish_staged owns and frees it on every path */
    }
    engine_log_info("incremental.done_delta", "elapsed_ms", itoa_buf((int)elapsed_ms(t0)));
...
out:
    ...
    if (stage) {
        engine_pipeline_discard_stage(stage);
        free(stage);
    }
    if (result == ENGINE_PIPELINE_FORCE_FULL_REINDEX) {
        engine_log_warn("delta.fallback", "reason", "delta_failed_full_rebuild");
#if defined(ENGINE_INCREMENTAL_TEST_API) && ENGINE_INCREMENTAL_TEST_API
        incr_test_set_last_route(ENGINE_INCREMENTAL_ROUTE_FORCED_FULL);
#endif
    }
    engine_delta_free_snapshot(snapshot, snapshot_count);
    surface_name_set_free(stale_surface_set);
    if (cr_arena_live) {
        engine_store_free_lsp_surfaces(cr.fresh_rows, cr.fresh_count);
        for (int i = 0; i < cr.def_module_count; i++) {
            free(cr.def_modules[i]);
        }
        free(cr.def_modules);
        for (int i = 0; i < cr.rehydrate_arena_count; i++) {
            engine_arena_destroy(&cr.rehydrate_arenas[i]);
        }
        free(cr.rehydrate_arenas);
        engine_arena_destroy(&cr.arena);
    }
    engine_pipeline_free_semantic_manifest(manifest, manifest_count);
    free(cov);
    free(purge_paths);
    if (registry) {
        engine_registry_free(registry);
    }
    engine_path_alias_collection_free(path_aliases);
    if (gbuf) {
        engine_gbuf_free(gbuf);
    }
    ...
    engine_store_free_coverage(old_cov, old_cov_count);
    return result;
}
...
int engine_pipeline_run_incremental(engine_pipeline_t *p, const char *db_path, engine_file_info_t *files,
    ...
    const char *project = engine_pipeline_project_name(p);

    bool force_legacy_partial = false;
#if defined(ENGINE_INCREMENTAL_TEST_API) && ENGINE_INCREMENTAL_TEST_API
    force_legacy_partial = incr_test_take_force_legacy_partial();
    incr_test_set_last_route(ENGINE_INCREMENTAL_ROUTE_NONE);
#endif
    ...
    engine_store_t *store =
        force_legacy_partial ? engine_store_open_path(db_path) : engine_store_open_path_query(db_path);
#else
    engine_store_t *store = engine_store_open_path_query(db_path);
#endif
    if (!store) {
        engine_log_error("incremental.err", "msg", "open_db_failed", "path", db_path);
        return ENGINE_NOT_FOUND;
    }

    /* Load stored file hashes */
    engine_file_hash_t *stored = NULL;
    int stored_count = 0;
    if (engine_store_get_file_hashes(store, project, &stored, &stored_count) != ENGINE_STORE_OK) {
        engine_log_error("incremental.err", "msg", "manifest_read_failed", "project", project);
        engine_store_close(store);
        return ENGINE_PIPELINE_ABORT_PRESERVE_DB;
    }
    ...
    if (!force_legacy_partial) {
        ...
        int meta_rc = engine_store_coverage_meta_get(store, project, &meta);
        const char *mode_name = incr_mode_name(engine_pipeline_get_mode(p));
        bool metadata_current = meta_rc == ENGINE_STORE_OK &&
                                meta.coverage_version == ENGINE_SEMANTIC_INDEX_VERSION &&
                                meta.hash_records_complete && meta.index_mode &&
                                strcmp(meta.index_mode, mode_name) == 0;
        bool exact = metadata_current &&
                     engine_pipeline_semantic_manifests_equal(stored, stored_count, baseline_manifest,
                                                           baseline_count);
        engine_store_coverage_meta_clear(&meta);
        if (exact) {
            engine_store_free_file_hashes(stored, stored_count);
            engine_store_close(store);
#if defined(ENGINE_INCREMENTAL_TEST_API) && ENGINE_INCREMENTAL_TEST_API
            incr_test_set_last_route(ENGINE_INCREMENTAL_ROUTE_NOOP);
#endif
            engine_log_info("incremental.noop", "reason", "semantic_manifest_equal");
            return engine_pipeline_refresh_artifact(p, db_path);
        }
        ...
        if (metadata_current &&
            ...
            closure_active = true;
#if defined(ENGINE_INCREMENTAL_TEST_API) && ENGINE_INCREMENTAL_TEST_API
            incr_test_set_last_route(ENGINE_INCREMENTAL_ROUTE_CLOSURE_REPAIR);
#endif
            engine_log_info("incremental.route", "route", "closure_repair", "files",
            ...
            incr_test_set_last_route(ENGINE_INCREMENTAL_ROUTE_FORCED_FULL);
#endif
            engine_log_info("incremental.force_full", "reason", "semantic_manifest_changed");
            return ENGINE_PIPELINE_FORCE_FULL_REINDEX;
        }
    }
    ...
    char **deleted = NULL;
    engine_file_hash_t *mode_skipped = NULL;
    int mode_skipped_count = 0;
    int deleted_count =
        find_deleted_files(engine_pipeline_repo_path(p), files, file_count, stored, stored_count,
                           &deleted, &mode_skipped, &mode_skipped_count);

    engine_log_info("incremental.classify", "changed", itoa_buf(n_changed), "unchanged",
    ...
    if (!closure_active && n_changed == 0 && deleted_count == 0) {
        ...
        incr_test_set_last_route(ENGINE_INCREMENTAL_ROUTE_NOOP);
#endif
        engine_log_info("incremental.noop", "reason", "no_changes");
        free(is_changed);
        free(deleted);
        free_mode_skipped(mode_skipped, mode_skipped_count);
        engine_store_free_file_hashes(stored, stored_count);
...
#if defined(ENGINE_INCREMENTAL_TEST_API) && ENGINE_INCREMENTAL_TEST_API
    if (!closure_active) {
        incr_test_set_last_route(ENGINE_INCREMENTAL_ROUTE_LEGACY_PARTIAL);
    }
#endif
    ...
    engine_coverage_row_t *old_cov = NULL;
    int old_cov_count = 0;
    if (engine_store_coverage_get(store, project, &old_cov, &old_cov_count) != ENGINE_STORE_OK) {
        engine_log_error("incremental.err", "msg", "coverage_read_failed", "project", project);
        engine_store_free_coverage(old_cov, old_cov_count);
        free(is_changed);
        for (int i = 0; i < deleted_count; i++) {
            free(deleted[i]);
    ...
    engine_file_info_t *changed_files =
        (n_changed > 0) ? malloc((size_t)n_changed * sizeof(engine_file_info_t)) : NULL;
    int ci = 0;
    if (closure_active) {
    ...
    engine_log_info("incremental.reparse", "files", itoa_buf(ci));
    ...
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    engine_gbuf_t *existing = engine_gbuf_new(project, engine_pipeline_repo_path(p));
    int load_rc = engine_gbuf_load_from_db(existing, db_path, project);
    engine_log_info("incremental.load_db", "rc", itoa_buf(load_rc), "nodes",
                 itoa_buf(engine_gbuf_node_count(existing)), "edges",
                 itoa_buf(engine_gbuf_edge_count(existing)), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));

    if (load_rc != 0) {
        engine_log_error("incremental.err", "msg", "load_db_failed");
        engine_gbuf_free(existing);
        free(changed_files);
        for (int i = 0; i < deleted_count; i++) {
            free(deleted[i]);
        }
    ...
    char *saved_adr = NULL;
    engine_adr_t existing_adr = {0};
    int adr_rc = engine_store_adr_get(store, project, &existing_adr);
    if (adr_rc == ENGINE_STORE_OK) {
        bool had_adr_content = existing_adr.content != NULL;
        if (had_adr_content) {
            saved_adr = strdup(existing_adr.content);
        }
        engine_store_adr_free(&existing_adr);
    ...
    } else if (adr_rc != ENGINE_STORE_NOT_FOUND) {
        engine_store_adr_free(&existing_adr);
        engine_gbuf_free(existing);
        free(changed_files);
        for (int i = 0; i < deleted_count; i++) {
    ...
    engine_edge_capture_t edge_cap = {0};
    edge_cap.gbuf = existing;
    {
        EngineHashTable *changed_paths = engine_ht_create(ci > 0 ? (size_t)ci * PAIR_LEN : ENGINE_SZ_64);
        for (int i = 0; i < ci; i++) {
            engine_ht_set(changed_paths, changed_files[i].rel_path, &changed_files[i]);
        }
        edge_cap.changed_paths = changed_paths;
        engine_clock_gettime(CLOCK_MONOTONIC, &t);
        engine_gbuf_foreach_edge(existing, incr_capture_inbound_edge, &edge_cap);
        edge_cap.changed_paths = NULL;
        engine_ht_free(changed_paths); /* keys borrowed from changed_files; not freed here */
    }
    engine_log_info("incremental.edge_snapshot", "captured", itoa_buf(edge_cap.count), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));

    /* Step 2: Purge stale nodes */
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    for (int i = 0; i < ci; i++) {
        engine_gbuf_delete_by_file(existing, changed_files[i].rel_path);
    }
    for (int i = 0; i < deleted_count; i++) {
        engine_gbuf_delete_by_file(existing, deleted[i]);
        free(deleted[i]);
    }
    free(deleted);
    engine_log_info("incremental.purge", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));

    /* Step 3-5: Registry + extract + resolve */
    engine_registry_t *registry = engine_registry_new();
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    engine_gbuf_foreach_node(existing, registry_visitor, registry);
    engine_log_info("incremental.registry_seed", "symbols", itoa_buf(engine_registry_size(registry)),
                 "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    ...
    engine_path_alias_collection_t *path_aliases =
        engine_load_path_aliases_excluded(engine_pipeline_repo_path(p), excluded_dirs, excluded_count);
    ...
    for (int i = 0; i < ci; i++) {
        ...
        if (file_qn) {
            ...
            const char *ext = strrchr(basename, '.');
            snprintf(props, sizeof(props), "{\"extension\":\"%s\"}", ext ? ext : "");
            engine_gbuf_upsert_node(existing, "File", basename, file_qn, rel, 0, 0, props);
            free(file_qn);
        }
    }
    ...
    if (phase_rc != 0) {
        engine_log_error("incremental.err", "phase", "extract_resolve", "rc", itoa_buf(phase_rc));
        free(changed_files);
        engine_registry_free(registry);
        engine_path_alias_collection_free(path_aliases);
    ...
    if (cov) {
        EngineHashTable *changed_set = engine_ht_create(ci > 0 ? (size_t)ci * PAIR_LEN : ENGINE_SZ_64);
        for (int i = 0; i < ci; i++) {
            engine_ht_set(changed_set, changed_files[i].rel_path, &changed_files[i]);
        }
        for (int i = 0; i < old_cov_count; i++) {
            bool by_design = old_cov[i].kind && strncmp(old_cov[i].kind, "not_indexed", 11) == 0;
            if (!by_design && old_cov[i].rel_path &&
                !engine_ht_get(changed_set, old_cov[i].rel_path)) {
                cov[cov_n++] = old_cov[i];
            }
        }
        engine_ht_free(changed_set);
        for (int i = 0; i < run_err_count; i++) {
            cov[cov_n].rel_path = run_errs[i].path;
            cov[cov_n].kind = run_errs[i].phase;
            cov[cov_n].detail = run_errs[i].reason;
    ...
    engine_log_info("incremental.edge_relink", "relinked", itoa_buf(relinked), "captured",
    ...
    engine_pipeline_persist_test_run_before_final_manifest();
#endif
    int manifest_rc = engine_pipeline_build_fresh_semantic_manifest(
        project, engine_pipeline_repo_path(p), engine_pipeline_get_mode(p), &manifest, &manifest_count);
    if (manifest_rc != 0 || !engine_pipeline_semantic_manifests_equal(
                                baseline_manifest, baseline_count, manifest, manifest_count)) {
        engine_log_warn("incremental.abort", "reason", "semantic_inputs_changed");
        engine_pipeline_free_semantic_manifest(manifest, manifest_count);
        free(cov);
        engine_store_free_coverage(old_cov, old_cov_count);
        free_mode_skipped(mode_skipped, mode_skipped_count);
    ...
    engine_pipeline_set_committed_counts(p, engine_gbuf_node_count(existing),
                                      engine_gbuf_edge_count(existing));
    int index_mode = engine_pipeline_get_mode(p);
    ...
    int persist_rc = dump_and_persist(
        existing, db_path, project, engine_pipeline_cancelled_ptr(p), manifest, manifest_count,
        saved_adr, engine_pipeline_repo_path(p), cov, cov_n, &coverage_meta, NULL, 0);
    engine_pipeline_free_semantic_manifest(manifest, manifest_count);
    free(saved_adr);
    free(cov);
    engine_store_free_coverage(old_cov, old_cov_count);
    ...
    engine_log_info("incremental.done", "elapsed_ms", itoa_buf((int)elapsed_ms(t0)));
    return persist_rc;
}
