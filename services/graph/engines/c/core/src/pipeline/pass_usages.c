...
static bool pu_module_is_dir(EngineLanguage lang) {
...
static int build_import_map(engine_pipeline_ctx_t *ctx, const char *rel_path,
                            ...
                            const char ***out_vals, int *out_count, EngineLanguage lang) {
    return engine_pxc_build_import_map(ctx->gbuf, ctx->project_name, rel_path, lang, result, out_keys,
                                    out_vals, out_count);
}

static void free_import_map(const char **keys, const char **vals, int count) {
    engine_pxc_free_import_map(keys, vals, count);
}

/* Find the graph buffer node for an enclosing function QN, falling back to file node. */
static const engine_gbuf_node_t *find_enclosing_node(engine_pipeline_ctx_t *ctx, const char *func_qn,
                                                  const char *rel_path) {
    const engine_gbuf_node_t *node = NULL;
    if (func_qn && func_qn[0]) {
        node = engine_gbuf_find_by_qn(ctx->gbuf, func_qn);
        ...
        if (engine_pipeline_node_is_dir_container(node)) {
            node = NULL;
        }
    }
    if (!node) {
...
static int resolve_usage_edges(engine_pipeline_ctx_t *ctx, const EngineFileResult *result,
    ...
    for (int u = 0; u < result->usages.count; u++) {
        ...
        const engine_gbuf_node_t *src = find_enclosing_node(ctx, usage->enclosing_func_qn, rel);
        if (!src) {
            continue;
        }
        ...
        if (engine_pipeline_usage_semantic_reference_candidate(usage)) {
            ...
            if (semantic_reference) {
                tgt = engine_pipeline_lsp_target_node(ctx->gbuf, ctx->project_name,
        ...
        if (!tgt) {
            ...
            engine_resolution_t res = engine_registry_resolve(ctx->registry, usage->ref_name, module_qn,
            ...
            tgt = engine_gbuf_find_by_qn(ctx->gbuf, res.qualified_name);
        ...
        engine_gbuf_insert_edge(ctx->gbuf, src->id, tgt->id, edge_type, uprops);
        resolved++;
    }
...
static int resolve_throw_edges(engine_pipeline_ctx_t *ctx, const EngineFileResult *result,
                               const char *rel, const char *module_qn, const char **imp_keys,
    ...
    for (int t = 0; t < result->throws.count; t++) {
        ...
        const engine_gbuf_node_t *src = find_enclosing_node(ctx, thr->enclosing_func_qn, rel);
        if (!src) {
            continue;
        }

        const char *edge_type = is_checked_exception(thr->exception_name) ? "THROWS" : "RAISES";
        engine_resolution_t res = engine_registry_resolve(ctx->registry, thr->exception_name, module_qn,
                                                    imp_keys, imp_vals, imp_count);
        ...
        engine_gbuf_insert_edge(ctx->gbuf, src->id, tgt->id, edge_type, "{}");
        resolved++;
    }
    return resolved;
}
...
static int resolve_rw_edges(engine_pipeline_ctx_t *ctx, const EngineFileResult *result, const char *rel,
    ...
    for (int r = 0; r < result->rw.count; r++) {
        ...
        const engine_gbuf_node_t *src = find_enclosing_node(ctx, rw->enclosing_func_qn, rel);
        if (!src) {
            continue;
        }

        engine_resolution_t res = engine_registry_resolve(ctx->registry, rw->var_name, module_qn,
                                                    imp_keys, imp_vals, imp_count);
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            continue;
        }

        const engine_gbuf_node_t *tgt = engine_gbuf_find_by_qn(ctx->gbuf, res.qualified_name);
        if (!tgt || src->id == tgt->id) {
            continue;
        }
...
int engine_pipeline_pass_usages(engine_pipeline_ctx_t *ctx, const engine_file_info_t *files,
                             int file_count) {
    engine_log_info("pass.start", "pass", "usages", "files", itoa_log(file_count));
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (!result) {
            ...
            result = engine_extract_file(source, source_len, files[i].language, ctx->project_name, rel,
    ...
    engine_log_info("pass.done", "pass", "usages", "usage", itoa_log(usage_resolved), "throws",
