 ...
 * since that information is in EngineDefinition fields, not stored in node properties JSON.
 *
 ...
 * def-node QNs. */
static bool ps_module_is_dir(EngineLanguage lang) {
...
static const char *itoa_log(int val) {
    ...
    static ENGINE_TLS char bufs[RING_BUF_COUNT][ENGINE_SZ_32];
    ...
    *out_count = 0;
    ...
    for (int i = 0; i < edge_count; i++) {
        ...
        if (start) {
            ...
            if (end && end > start) {
                keys[count] = engine_strndup(start, end - start);
                vals[count] = target->qualified_name;
                count++;
            }
        }
...
static int check_go_class_implements(engine_pipeline_ctx_t *ctx, const engine_gbuf_node_t *cls,
                                     const engine_gbuf_node_t *iface, const go_imethod_t *imethods,
                                     int im_count) {
    ...
    const engine_gbuf_edge_t **cls_dm = NULL;
    int cls_dm_count = 0;
    engine_gbuf_find_edges_by_source_type(ctx->gbuf, cls->id, "DEFINES_METHOD", &cls_dm,
                                       &cls_dm_count);

    char prefix[ENGINE_SZ_512];
    snprintf(prefix, sizeof(prefix), "%s.", cls->qualified_name);

    /* For each interface method, find the matching struct method node. */
    const engine_gbuf_node_t *matched[ENGINE_SZ_128];
    for (int m = 0; m < im_count; m++) {
        const engine_gbuf_node_t *found = NULL;
        /* (a) DEFINES_METHOD edge by name */
        for (int d = 0; d < cls_dm_count; d++) {
            const engine_gbuf_node_t *cm = engine_gbuf_find_by_id(ctx->gbuf, cls_dm[d]->target_id);
        ...
        if (!found) {
            char method_qn[ENGINE_SZ_512];
            snprintf(method_qn, sizeof(method_qn), "%s%s", prefix, imethods[m].name);
            found = engine_gbuf_find_by_qn(ctx->gbuf, method_qn);
        }
        if (!found) {
    ...
    engine_gbuf_insert_edge(ctx->gbuf, cls->id, iface->id, "IMPLEMENTS", "{}");
    int edges = SKIP_ONE;
    for (int m = 0; m < im_count; m++) {
        engine_gbuf_insert_edge(ctx->gbuf, matched[m]->id, imethods[m].id, "OVERRIDE", "{}");
        edges++;
    }
    return edges;
}
...
int engine_pipeline_implements_go(engine_pipeline_ctx_t *ctx) {
    ...
    const engine_gbuf_node_t **ifaces = NULL;
    int iface_count = 0;
    if (engine_gbuf_find_by_label(ctx->gbuf, "Interface", &ifaces, &iface_count) != 0) {
        return 0;
    }
    ...
    int class_count = 0;
    engine_gbuf_find_by_label(ctx->gbuf, "Class", &classes, &class_count);
    const engine_gbuf_node_t **structs = NULL;
    int struct_count = 0;
    engine_gbuf_find_by_label(ctx->gbuf, "Struct", &structs, &struct_count);
    if (class_count == 0 && struct_count == 0) {
        return 0;
    }

    for (int i = 0; i < iface_count; i++) {
        const engine_gbuf_node_t *iface = ifaces[i];
        if (!iface->file_path || !fp_ends_with(iface->file_path, ".go")) {
            continue;
        }

        /* Get interface methods via DEFINES_METHOD edges */
        const engine_gbuf_edge_t **dm_edges = NULL;
        int dm_count = 0;
        if (engine_gbuf_find_edges_by_source_type(ctx->gbuf, iface->id, "DEFINES_METHOD", &dm_edges,
                                               &dm_count) != 0 ||
            dm_count == 0) {
            continue;
        }
        ...
        go_imethod_t imethods[ENGINE_SZ_128];
        int im_count = 0;
        for (int j = 0; j < dm_count && im_count < ENGINE_SZ_128; j++) {
            const engine_gbuf_node_t *m = engine_gbuf_find_by_id(ctx->gbuf, dm_edges[j]->target_id);
            if (m && m->name) {
...
static void resolve_decorator(engine_pipeline_ctx_t *ctx, const engine_gbuf_node_t *node,
                              const char *decorator, const char *module_qn, const char **imp_keys,
                              const char **imp_vals, int imp_count, int *count) {
    char func_name[ENGINE_SZ_256];
    extract_decorator_func(decorator, func_name, sizeof(func_name));
    if (func_name[0] == '\0') {
        return;
    }
    engine_resolution_t res =
        engine_registry_resolve(ctx->registry, func_name, module_qn, imp_keys, imp_vals, imp_count);
    if ((!res.qualified_name || res.qualified_name[0] == '\0') && !strchr(func_name, '.')) {
        ...
        if (wn > 0 && (size_t)wn < sizeof(with_suffix)) {
            res = engine_registry_resolve(ctx->registry, with_suffix, module_qn, imp_keys, imp_vals,
                                       imp_count);
        }
    }
    const engine_gbuf_node_t *dec = NULL;
    if (res.qualified_name && res.qualified_name[0] != '\0') {
        dec = engine_gbuf_find_by_qn(ctx->gbuf, res.qualified_name);
    }
    ...
    if (!dec) {
        ...
        int64_t syn_id =
            engine_gbuf_upsert_node(ctx->gbuf, "Decorator", func_name, syn_qn, "", 0, 0, "{}");
        if (syn_id != 0) {
            dec = engine_gbuf_find_by_qn(ctx->gbuf, syn_qn);
        }
    }
    if (dec && node->id != dec->id) {
        ...
        engine_json_escape(esc_dec, sizeof(esc_dec), decorator);
        char props[ENGINE_SZ_512];
        snprintf(props, sizeof(props), "{\"decorator\":\"%s\"}", esc_dec);
        engine_gbuf_insert_edge(ctx->gbuf, node->id, dec->id, "DECORATES", props);
        ...
        engine_gbuf_insert_edge(ctx->gbuf, node->id, dec->id, "USAGE", "{}");
        (*count)++;
    }
}

static void sem_process_def_edges(engine_pipeline_ctx_t *ctx, const EngineDefinition *def,
                                  const char *module_qn, const char **imp_keys,
    ...
    const engine_gbuf_node_t *node = engine_gbuf_find_by_qn(ctx->gbuf, def->qualified_name);
    if (!node) {
        return;
    }
    if (def->base_classes) {
        for (int b = 0; def->base_classes[b]; b++) {
            ...
            const engine_gbuf_node_t *base_node = engine_gbuf_find_by_qn(ctx->gbuf, base_qn);
            if (base_node && node->id != base_node->id) {
                 ...
                 * implements at scale). */
                engine_gbuf_insert_edge(ctx->gbuf, node->id, base_node->id,
                                     engine_semantic_base_edge_type(base_node), "{}");
                (*inherits_count)++;
            }
        }
    }
...
static EngineFileResult *sem_get_or_extract(engine_pipeline_ctx_t *ctx, int file_idx,
    ...
    *owned = false;
    ...
    EngineFileResult *r = engine_extract_file(source, source_len, fi->language, ctx->project_name,
                                        fi->rel_path, ENGINE_EXTRACT_BUDGET, NULL, NULL);
    free(source);
    if (r) {
        *owned = true;
    }
...
static int resolve_impl_traits(engine_pipeline_ctx_t *ctx, const EngineFileResult *result,
    ...
    for (int t = 0; t < result->impl_traits.count; t++) {
        ...
        const engine_gbuf_node_t *tn = engine_gbuf_find_by_qn(ctx->gbuf, trait_qn);
        const engine_gbuf_node_t *sn = engine_gbuf_find_by_qn(ctx->gbuf, struct_qn);
        if (tn && sn && tn->id != sn->id) {
            engine_gbuf_insert_edge(ctx->gbuf, sn->id, tn->id, "IMPLEMENTS", "{}");
            count++;
        }
    }
    return count;
...
int engine_pipeline_pass_semantic(engine_pipeline_ctx_t *ctx, const engine_file_info_t *files,
                               int file_count) {
    engine_log_info("pass.start", "pass", "semantic", "files", itoa_log(file_count));
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        EngineFileResult *result = sem_get_or_extract(ctx, i, &files[i], &result_owned);
        if (!result) {
            errors++;
            continue;
        }
    ...
    int overrides = engine_pipeline_override_explicit(ctx);

    engine_log_info("pass.done", "pass", "semantic", "inherits", itoa_log(inherits_count), "decorates",
...
static int override_match_methods(engine_pipeline_ctx_t *ctx, const engine_gbuf_node_t *cls,
                                  const engine_gbuf_node_t *base) {
    const engine_gbuf_edge_t **cls_dm = NULL;
    int cls_dm_count = 0;
    engine_gbuf_find_edges_by_source_type(ctx->gbuf, cls->id, "DEFINES_METHOD", &cls_dm,
                                       &cls_dm_count);
    ...
    const engine_gbuf_edge_t **base_dm = NULL;
    int base_dm_count = 0;
    engine_gbuf_find_edges_by_source_type(ctx->gbuf, base->id, "DEFINES_METHOD", &base_dm,
                                       &base_dm_count);
    int created = 0;
    for (int c = 0; c < cls_dm_count; c++) {
        const engine_gbuf_node_t *cm = engine_gbuf_find_by_id(ctx->gbuf, cls_dm[c]->target_id);
        if (!cm || !cm->name) {
            continue;
        }
        for (int b = 0; b < base_dm_count; b++) {
            const engine_gbuf_node_t *bm = engine_gbuf_find_by_id(ctx->gbuf, base_dm[b]->target_id);
            if (bm && bm->name && cm->id != bm->id && strcmp(cm->name, bm->name) == 0) {
                engine_gbuf_insert_edge(ctx->gbuf, cm->id, bm->id, "OVERRIDE", "{}");
                created++;
                break;
            }
        }
...
int engine_pipeline_override_explicit(engine_pipeline_ctx_t *ctx) {
    ...
    for (size_t t = 0; t < sizeof(base_edge_types) / sizeof(base_edge_types[0]); t++) {
        const engine_gbuf_edge_t **edges = NULL;
        int edge_count = 0;
        engine_gbuf_find_edges_by_type(ctx->gbuf, base_edge_types[t], &edges, &edge_count);
        for (int e = 0; e < edge_count; e++) {
            const engine_gbuf_node_t *cls = engine_gbuf_find_by_id(ctx->gbuf, edges[e]->source_id);
            const engine_gbuf_node_t *base = engine_gbuf_find_by_id(ctx->gbuf, edges[e]->target_id);
            if (!cls || !base) {
                continue;
            }
