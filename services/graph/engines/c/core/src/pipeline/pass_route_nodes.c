...
#include "foundation/str_util.h" // engine_json_escape
...
static void route_edge_visitor(const engine_gbuf_edge_t *edge, void *userdata) {
    ...
    if (strcmp(edge->type, "HTTP_CALLS") == 0) {
        ...
        snprintf(route_qn, sizeof(route_qn), "__route__%s__%s", method ? method : "ANY",
                 engine_route_canon_path(url, cpath, sizeof(cpath)));
    } else {
    ...
    engine_gbuf_upsert_node(ctx->gb, "Route", url, route_qn, "", 0, 0, route_props);
    ctx->created++;
...
static int match_one_infra_route(engine_gbuf_t *gb, const engine_gbuf_node_t *infra,
                                 const char *infra_path, const char *svc_name,
                                 const engine_gbuf_node_t **all_routes, int route_count) {
    for (int j = 0; j < route_count; j++) {
        const engine_gbuf_node_t *handler_route = all_routes[j];
        ...
        if (path_match || root_svc_match) {
            const engine_gbuf_edge_t **fn_handles = NULL;
            int fn_hcount = 0;
            engine_gbuf_find_edges_by_target_type(gb, handler_route->id, "HANDLES", &fn_handles,
                                               &fn_hcount);
            for (int fh = 0; fh < fn_hcount; fh++) {
                engine_gbuf_insert_edge(gb, fn_handles[fh]->source_id, infra->id, "HANDLES",
                                     "{\"source\":\"infra_match\"}");
            }
            return SKIP_ONE;
        }
...
/* Phase 2: Match infra Route URLs to handler Route nodes by URL path + service name. */
static void match_infra_routes(engine_gbuf_t *gb) {
    const engine_gbuf_node_t **all_routes = NULL;
    int route_count = 0;
    if (engine_gbuf_find_by_label(gb, "Route", &all_routes, &route_count) != 0 || route_count == 0) {
        return;
    }
    ...
    for (int i = 0; i < route_count; i++) {
        const engine_gbuf_node_t *infra = all_routes[i];
    ...
    if (matched > 0) {
        ...
        engine_log_info("pass.route_match", "infra_matched", buf);
    }
}
...
static int ensure_one_decorator_route(engine_gbuf_t *gb, const engine_gbuf_node_t *func) {
    if (!func->properties_json) {
        return 0;
    }

    char path[ENGINE_SZ_256];
    ...
    snprintf(route_qn, sizeof(route_qn), "__route__%s__%s", method,
             engine_route_canon_path(path, cpath, sizeof(cpath)));
    const engine_gbuf_node_t *existing = engine_gbuf_find_by_qn(gb, route_qn);

    char rprops[ENGINE_SZ_256];
    snprintf(rprops, sizeof(rprops), "{\"method\":\"%s\",\"source\":\"decorator\"}", method);
    int64_t route_id = engine_gbuf_upsert_node(gb, "Route", path, route_qn,
                                            func->file_path ? func->file_path : "", 0, 0, rprops);

    if (existing) {
        const engine_gbuf_edge_t **existing_handles = NULL;
        int eh_count = 0;
        engine_gbuf_find_edges_by_target_type(gb, route_id, "HANDLES", &existing_handles, &eh_count);
    ...
    engine_gbuf_insert_edge(gb, func->id, route_id, "HANDLES", hprops);
    return SKIP_ONE;
}

/* Phase 2a: Ensure all functions with route_path properties have Route+HANDLES edges. */
static void ensure_decorator_routes(engine_gbuf_t *gb) {
    ...
    for (int li = 0; li < RN_STRIP_PASSES; li++) {
        ...
        int count = 0;
        if (engine_gbuf_find_by_label(gb, labels[li], &nodes, &count) != 0) {
            continue;
        }
    ...
    if (created > 0) {
        ...
        engine_log_info("pass.ensure_decorator_routes", "created", buf);
    }
}
...
static int bridge_funcs_to_prefix(engine_gbuf_t *gb, const engine_gbuf_node_t *prefix_route,
                                  const char *registrar_path, int dir_len,
                                  const char *prefix_segs) {
    const engine_gbuf_node_t **funcs = NULL;
    int func_count = 0;
    engine_gbuf_find_by_label(gb, "Function", &funcs, &func_count);

    int connected = 0;
    for (int fi = 0; fi < func_count; fi++) {
        const engine_gbuf_node_t *func = funcs[fi];
        if (!func->file_path) {
            continue;
        }
        ...
        engine_gbuf_insert_edge(gb, func->id, prefix_route->id, "HANDLES",
...
/* Phase 2b: Connect prefix Routes to decorator handler Functions. */
static void connect_prefix_to_decorators(engine_gbuf_t *gb) {
    const engine_gbuf_node_t **routes = NULL;
    int route_count = 0;
    if (engine_gbuf_find_by_label(gb, "Route", &routes, &route_count) != 0) {
        return;
    }
    ...
    for (int ri = 0; ri < route_count; ri++) {
        const engine_gbuf_node_t *prefix_route = routes[ri];
        ...
        const engine_gbuf_edge_t **calls_in = NULL;
        int calls_count = 0;
        engine_gbuf_find_edges_by_target_type(gb, prefix_route->id, "CALLS", &calls_in, &calls_count);
        if (calls_count == 0) {
            continue;
        }

        const engine_gbuf_node_t *registrar = engine_gbuf_find_by_id(gb, calls_in[0]->source_id);
    ...
    if (connected > 0) {
        ...
        engine_log_info("pass.prefix_bridge", "connected", buf);
    }
}
...
static int has_direct_call(const engine_gbuf_t *gb, int64_t source, int64_t target) {
    const engine_gbuf_edge_t **edges = NULL;
    int count = 0;
    engine_gbuf_find_edges_by_source_type(gb, source, "CALLS", &edges, &count);
    for (int i = 0; i < count; i++) {
...
static void extract_param_names(const engine_gbuf_node_t *node, char *buf, int bufsize) {
    buf[0] = '\0';
...
static bool http_call_edge_has_valid_route(const engine_gbuf_edge_t *edge) {
    char url_buf[ENGINE_SZ_512];
    const char *url = json_extract(edge->properties_json, "url_path", url_buf, sizeof(url_buf));
    char callee_buf[ENGINE_SZ_256];
    const char *callee =
        json_extract(edge->properties_json, "callee", callee_buf, sizeof(callee_buf));
    return engine_service_pattern_is_http_route_literal(url, callee);
}
...
static int try_create_data_flow(engine_gbuf_t *gb, int64_t caller_id, int64_t handler_id,
                                const engine_gbuf_node_t *route, const char *edge_type,
    ...
    const engine_gbuf_node_t *handler_node = engine_gbuf_find_by_id(gb, handler_id);
    char handler_params[ENGINE_SZ_512];
    ...
    char esc_rname[ENGINE_SZ_256];
    char esc_rqn[ENGINE_SZ_512];
    engine_json_escape(esc_rname, sizeof(esc_rname), route->name ? route->name : "");
    engine_json_escape(esc_rqn, sizeof(esc_rqn), route->qualified_name ? route->qualified_name : "");
    ...
    engine_gbuf_insert_edge(gb, caller_id, handler_id, "DATA_FLOWS", props);
    return SKIP_ONE;
}

/* Collect extra handler IDs reachable via INFRA_MAPS chain from a route. */
static int collect_infra_handlers(engine_gbuf_t *gb, int64_t route_id, int64_t *out, int max_out) {
    const engine_gbuf_edge_t **infra_edges = NULL;
    int infra_count = 0;
    engine_gbuf_find_edges_by_source_type(gb, route_id, "INFRA_MAPS", &infra_edges, &infra_count);

    int n = 0;
    for (int ie = 0; ie < infra_count; ie++) {
        const engine_gbuf_edge_t **ep_handles = NULL;
        int ep_hcount = 0;
        engine_gbuf_find_edges_by_target_type(gb, infra_edges[ie]->target_id, "HANDLES", &ep_handles,
                                           &ep_hcount);
...
static int collect_caller_edges(engine_gbuf_t *gb, int64_t route_id, caller_edge_ref_t *out,
                                int max_out) {
    int n = 0;
    const engine_gbuf_edge_t **http_edges = NULL;
    int http_count = 0;
    engine_gbuf_find_edges_by_target_type(gb, route_id, "HTTP_CALLS", &http_edges, &http_count);
    ...
    const engine_gbuf_edge_t **async_edges = NULL;
    int async_count = 0;
    engine_gbuf_find_edges_by_target_type(gb, route_id, "ASYNC_CALLS", &async_edges, &async_count);
...
static void create_route_data_flows(engine_gbuf_t *gb, const engine_gbuf_node_t *route,
                                    const caller_edge_ref_t *callers, int n_callers, int *flows,
                                    int *skipped) {
    const engine_gbuf_edge_t **handles_edges = NULL;
    int handles_count = 0;
    engine_gbuf_find_edges_by_target_type(gb, route->id, "HANDLES", &handles_edges, &handles_count);

    int64_t extra_handlers[ENGINE_SZ_32];
    int n_extra = collect_infra_handlers(gb, route->id, extra_handlers, ENGINE_SZ_32);
...
static const engine_gbuf_node_t *find_enclosing_service(const engine_gbuf_node_t *fn,
                                                     const engine_gbuf_node_t **services,
                                                     int svc_count) {
...
static void create_grpc_routes(engine_gbuf_t *gb) {
    const engine_gbuf_node_t **classes = NULL;
    int class_count = 0;
    if (engine_gbuf_find_by_label(gb, "Class", &classes, &class_count) != 0 || class_count == 0) {
        return;
    }

    const engine_gbuf_node_t *services[ENGINE_SZ_64];
    int svc_count = 0;
    for (int i = 0; i < class_count && svc_count < ENGINE_SZ_64; i++) {
    ...
    const engine_gbuf_node_t **funcs = NULL;
    int func_count = 0;
    if (engine_gbuf_find_by_label(gb, "Function", &funcs, &func_count) != 0 || func_count == 0) {
        return;
    }

    int grpc_routes = 0;
    for (int f = 0; f < func_count; f++) {
        const engine_gbuf_node_t *fn = funcs[f];
        if (!fn->file_path || !strstr(fn->file_path, ".proto") || !fn->name) {
            continue;
        }
        const engine_gbuf_node_t *svc = find_enclosing_service(fn, services, svc_count);
        if (!svc) {
            continue;
        }
        char route_qn[ENGINE_ROUTE_QN_SIZE];
        ...
        char props[ENGINE_SZ_128];
        snprintf(props, sizeof(props), "{\"source\":\"proto\",\"service\":\"%s\"}", svc->name);

        int64_t route_id = engine_gbuf_upsert_node(gb, "Route", fn->name, route_qn, fn->file_path,
                                                fn->start_line, fn->end_line, props);
        engine_gbuf_insert_edge(gb, fn->id, route_id, "HANDLES", "{\"via\":\"proto_rpc\"}");
        grpc_routes++;
    }
    if (grpc_routes > 0) {
        char buf[ENGINE_SZ_16];
        snprintf(buf, sizeof(buf), "%d", grpc_routes);
        engine_log_info("pass.route_nodes.grpc", "routes", buf);
    }
}

/* Phase 3: Create DATA_FLOWS edges by linking callers through Route to handlers. */
static void create_data_flows(engine_gbuf_t *gb) {
    const engine_gbuf_node_t **routes = NULL;
    int route_count = 0;
    if (engine_gbuf_find_by_label(gb, "Route", &routes, &route_count) != 0 || route_count == 0) {
        return;
    }
    ...
    for (int ri = 0; ri < route_count; ri++) {
        caller_edge_ref_t caller_edges[ENGINE_SZ_64];
        int n_callers = collect_caller_edges(gb, routes[ri]->id, caller_edges, ENGINE_SZ_64);
        create_route_data_flows(gb, routes[ri], caller_edges, n_callers, &flows, &skipped);
    }

    if (flows > 0 || skipped > 0) {
        char buf1[ENGINE_SZ_16];
        char buf2[ENGINE_SZ_16];
        snprintf(buf1, sizeof(buf1), "%d", flows);
        snprintf(buf2, sizeof(buf2), "%d", skipped);
        engine_log_info("pass.data_flows", "created", buf1, "skipped_has_call", buf2);
    }
}
...
static void sveltekit_file_visitor(const engine_gbuf_node_t *node, void *userdata) {
    ...
    const engine_gbuf_edge_t **edges = NULL;
    int edge_count = 0;
    if (engine_gbuf_find_edges_by_source_type(ctx->gb, node->id, "DEFINES", &edges, &edge_count) !=
            0 ||
        edge_count == 0) {
        return;
    }
    ...
    for (int i = 0; i < edge_count; i++) {
        const engine_gbuf_node_t *child = engine_gbuf_find_by_id(ctx->gb, edges[i]->target_id);
        ...
        int64_t route_id =
            engine_gbuf_upsert_node(ctx->gb, "Route", route_path, route_qn, "", 0, 0, route_props);
        if (route_id == 0) {
            continue;
        }
        ctx->routes_created++;

        char hprops[ENGINE_SZ_256];
        ...
        engine_gbuf_insert_edge(ctx->gb, child->id, route_id, "HANDLES", hprops);
        ctx->handles_created++;
    }
}
...
static void create_sveltekit_routes(engine_gbuf_t *gb) {
    if (!gb) {
        return;
    }
    sveltekit_ctx_t ctx = {.gb = gb, .routes_created = 0, .handles_created = 0, .files_seen = 0};
    engine_gbuf_foreach_node(gb, sveltekit_file_visitor, &ctx);
    if (ctx.files_seen > 0) {
        char b1[ENGINE_SZ_16];
        char b2[ENGINE_SZ_16];
        char b3[ENGINE_SZ_16];
        ...
        engine_log_info("pass.sveltekit_routes", "files", b1, "routes", b2, "handles", b3);
    }
}
...
void engine_pipeline_create_route_nodes(engine_gbuf_t *gb) {
    ...
    engine_gbuf_foreach_edge(gb, route_edge_visitor, &ctx);

    if (ctx.created > 0) {
        char buf[ENGINE_SZ_16];
        snprintf(buf, sizeof(buf), "%d", ctx.created);
        engine_log_info("pass.route_nodes", "created", buf);
    }
