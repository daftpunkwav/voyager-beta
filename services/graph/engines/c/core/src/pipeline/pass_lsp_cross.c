 ...
 * (EngineDefinition / EngineImport / IMPORTS-edge gbuf state) into the input
 * shape each language LSP's engine_run_X_lsp_cross expects, then merges
 * the resulting EngineResolvedCall entries back into per-file results.
 *
 * The pass is a no-op for any file whose EngineFileResult is missing or
 ...
 * extraction-side engine_lang_module_is_dir() in internal/engine/helpers.c so the
 * cross-file LSP caller_qn agrees with the def-node QN (the lsp_resolve join
 * keys on exact equality). */
static bool pxc_module_is_dir(EngineLanguage lang) {
...
static char *pxc_read_file(const char *path, int *out_len) {
    ...
    if (size <= 0 || size > (long)PXC_MAX_FILE_BYTES_FACTOR * (long)ENGINE_SZ_1K * (long)ENGINE_SZ_1K) {
        (void)fclose(f);
        return NULL;
    }
    /* +pad: tree-sitter lexer lookahead reads past EOF; keep it in-bounds */
    enum { ENGINE_TS_LOOKAHEAD_PAD = 16 };
    char *buf = (char *)malloc((size_t)size + ENGINE_TS_LOOKAHEAD_PAD);
    if (!buf) {
        (void)fclose(f);
        return NULL;
    }
    ...
    if (nread > (size_t)size)
        nread = (size_t)size;
    memset(buf + nread, 0, ENGINE_TS_LOOKAHEAD_PAD);
    *out_len = (int)nread;
    return buf;
}

/* Map a EngineDefinition.label to a EngineLSPDef.label. Per-language LSP registrars
...
static const char *pxc_map_label(const char *label) {
    if (!label)
        return NULL;
    if (engine_label_is_type_like(label) || strcmp(label, "Protocol") == 0 ||
...
static const char *pxc_join_pipe(EngineArena *arena, const char *const *items) {
    if (!items || !items[0])
        return NULL;
    int count = 0;
    size_t total = 0;
    ...
    char *buf = (char *)engine_arena_alloc(arena, bufsz);
    if (!buf)
        return NULL;
    char *p = buf;
...
static bool pxc_is_jvm_lang(EngineLanguage lang);
...
static const char *pxc_jvm_type_qn(EngineArena *arena, const char *namespace_name,
    ...
    return engine_arena_sprintf(arena, "%s.%s", namespace_name, short_name);
}

static const char *pxc_jvm_def_qn(EngineArena *arena, const EngineDefinition *src,
    ...
    if (strcmp(label, "Method") == 0 || strcmp(label, "Function") == 0 ||
        ...
        if (src->parent_class && src->parent_class[0]) {
            return engine_arena_sprintf(arena, "%s.%s.%s", namespace_name,
                                     pxc_last_component(src->parent_class), src->name);
        }
        return engine_arena_sprintf(arena, "%s.%s", namespace_name, src->name);
    }
    return engine_arena_sprintf(arena, "%s.%s", namespace_name, src->name);
}

static const char *pxc_infer_jvm_namespace(EngineArena *arena, const char *rel_path,
                                           EngineLanguage lang) {
    if (!arena || !rel_path || !pxc_is_jvm_lang(lang)) {
        return NULL;
    }
    const char *root = NULL;
    const char *lang_root = lang == ENGINE_LANG_KOTLIN ? "kotlin/" : "java/";
    if (strncmp(rel_path, "src/main/", 9) == 0 &&
        ...
        const char *needle = lang == ENGINE_LANG_KOTLIN ? "/kotlin/" : "/java/";
...
/* Convert one EngineDefinition into a EngineLSPDef. Returns 0 on success, -1
...
static int pxc_build_lsp_def(EngineArena *arena, const EngineDefinition *src, const char *module_qn,
                             const char *namespace_name, EngineLanguage lang, EngineLSPDef *dst) {
...
static int pxc_build_rust_impl_relation(EngineArena *arena, const EngineImplTrait *impl,
                                        const char *project_name, const char *rel_path,
                                        const char *module_qn, EngineLSPDef *dst) {
    ...
    dst->lang = ENGINE_LANG_RUST;
    dst->is_rust_impl_relation = true;
    return 0;
}

/* Collect a project-wide EngineLSPDef[] from all cached results. Returns a
...
EngineLSPDef *engine_pxc_collect_all_defs(EngineFileResult **cache, const engine_file_info_t *files,
    ...
    for (int i = 0; i < file_count; i++) {
        if (cache[i]) {
            ...
            if (files[i].language == ENGINE_LANG_RUST) {
    ...
    EngineLSPDef *defs = (EngineLSPDef *)calloc((size_t)total, sizeof(EngineLSPDef));
    if (!defs) {
        *out_count = 0;
        if (out_def_starts) {
            memset(out_def_starts, 0, (size_t)(file_count + 1) * sizeof(int));
    ...
    for (int fi = 0; fi < file_count; fi++) {
        ...
        if (!cache[fi])
            continue;
        if (!def_modules[fi]) {
            def_modules[fi] = engine_pipeline_fqn_module_dir(project_name, files[fi].rel_path,
        ...
        if (files[fi].language == ENGINE_LANG_RUST) {
...
static const char *pxc_unique_import_path(const EngineFileResult *result, const char *local_name,
                                          bool *out_ambiguous) {
    if (out_ambiguous)
        *out_ambiguous = false;
...
static char *pxc_kotlin_import_from_metadata(const EngineFileResult *result, const char *local_name) {
...
static char *pxc_import_value_qn(EngineLanguage lang, const EngineFileResult *result,
                                 const char *local_name, const engine_gbuf_node_t *target) {
...
static char *pxc_python_import_from_metadata(const engine_gbuf_t *gbuf, const char *project_name,
    ...
    const engine_gbuf_node_t *exact =
        engine_pipeline_lsp_target_node(gbuf, project_name, module_path, false);
    ...
    if (last_dot && last_dot > module_path) {
        ...
        module[module_len] = '\0';
        const engine_gbuf_node_t *module_node =
            engine_pipeline_lsp_target_node(gbuf, project_name, module, false);
        free(module);
...
int engine_pxc_build_import_map(const engine_gbuf_t *gbuf, const char *project_name, const char *rel_path,
    ...
    *out_count = 0;
    ...
    const engine_gbuf_edge_t **edges = NULL;
    int edge_count = 0;
    char *file_qn = engine_pipeline_fqn_compute(project_name, rel_path, "__file__");
    const engine_gbuf_node_t *file_node = file_qn ? engine_gbuf_find_by_qn(gbuf, file_qn) : NULL;
    free(file_qn);
    if (file_node && engine_gbuf_find_edges_by_source_type(gbuf, file_node->id, "IMPORTS", &edges,
                                                        &edge_count) != 0) {
        edges = NULL;
        edge_count = 0;
    }
    ...
    int metadata_count =
        (lang == ENGINE_LANG_PYTHON || lang == ENGINE_LANG_KOTLIN) && result ? result->imports.count : 0;
    ...
    for (int i = 0; i < edge_count; i++) {
        const engine_gbuf_edge_t *e = edges[i];
        const engine_gbuf_node_t *target = engine_gbuf_find_by_id(gbuf, e->target_id);
...
void engine_pxc_ts_modes(EngineLanguage lang, const char *rel_path, bool *out_js, bool *out_jsx,
                      bool *out_dts) {
    *out_js = (lang == ENGINE_LANG_JAVASCRIPT);
    *out_jsx = (lang == ENGINE_LANG_TSX);
    *out_dts = false;
    if (!rel_path)
        return;
    size_t rl = strlen(rel_path);
    if (lang == ENGINE_LANG_JAVASCRIPT && rl >= 4 && strcmp(rel_path + rl - 4, ".jsx") == 0) {
        *out_jsx = true;
    }
    if (lang == ENGINE_LANG_TYPESCRIPT && rl >= 5 && strcmp(rel_path + rl - 5, ".d.ts") == 0) {
        *out_dts = true;
    }
}
...
bool engine_pxc_has_cross_lsp(EngineLanguage lang) {
    ...
    case ENGINE_LANG_CSHARP: /* tier-2 prebuilt registry path (pass_parallel.c) */
    case ENGINE_LANG_JAVA:   /* fallback engine_pxc_run_one path */
    case ENGINE_LANG_KOTLIN: /* fallback engine_pxc_run_one path */
    case ENGINE_LANG_RUST:   /* fallback engine_pxc_run_one path (manifest-aware) */
        return true;
    default:
        return false;
    }
...
static void pxc_append_results(EngineArena *dst_arena, EngineResolvedCallArray *dst_calls,
                               const EngineResolvedCallArray *src_out) {
    if (!dst_calls || !src_out)
        return;

    EngineArena keys;
    engine_arena_init(&keys);
    EngineHashTable *seen = engine_ht_create((uint32_t)(dst_calls->count + src_out->count + 1));

    for (int i = 0; i < dst_calls->count; i++) {
        const EngineResolvedCall *rc = &dst_calls->items[i];
        if (rc->caller_qn && rc->callee_qn) {
            char *k = engine_arena_sprintf(&keys, "%u\x1f%s\x1f%s\x1f%u:%u\x1f%u", (unsigned)rc->kind,
                                        rc->caller_qn, rc->callee_qn, rc->site_start_byte,
                                        rc->site_end_byte, (unsigned)rc->source_origin);
            if (k) {
                void *prior = engine_ht_get(seen, k);
                if (!prior ||
                    rc->confidence > dst_calls->items[(int)(uintptr_t)prior - 1].confidence) {
                    const char *stored = engine_ht_get_key(seen, k);
                    engine_ht_set(seen, stored ? stored : k, (void *)(uintptr_t)(i + 1));
                }
            }
        }
    }
    ...
    for (int j = 0; j < src_out->count; j++) {
        const EngineResolvedCall *src = &src_out->items[j];
        if (!src->caller_qn || !src->callee_qn)
            continue;
        char *k = engine_arena_sprintf(&keys, "%u\x1f%s\x1f%s\x1f%u:%u\x1f%u", (unsigned)src->kind,
                                    src->caller_qn, src->callee_qn, src->site_start_byte,
                                    src->site_end_byte, (unsigned)src->source_origin);
        void *prior = k ? engine_ht_get(seen, k) : NULL;
        if (prior) {
            EngineResolvedCall *dst = &dst_calls->items[(int)(uintptr_t)prior - 1];
            if (src->confidence > dst->confidence) {
                dst->caller_qn = engine_arena_strdup(dst_arena, src->caller_qn);
                dst->callee_qn = engine_arena_strdup(dst_arena, src->callee_qn);
                dst->strategy = src->strategy ? engine_arena_strdup(dst_arena, src->strategy) : NULL;
                dst->confidence = src->confidence;
                dst->reason = src->reason ? engine_arena_strdup(dst_arena, src->reason) : NULL;
                dst->kind = src->kind;
        ...
        dst.site_end_byte = src->site_end_byte;
        dst.source_origin = src->source_origin;
        engine_resolvedcall_push(dst_calls, dst_arena, dst);
        if (k) {
            engine_ht_set(seen, k, (void *)(uintptr_t)dst_calls->count);
        }
    }

    engine_ht_free(seen);
    engine_arena_destroy(&keys);
}

/* Merge exact synthetic call carriers produced by a cross-LSP resolver. The
 * source may live in the per-file result arena or in engine_pxc_run_one's scratch
...
static void pxc_append_synthetic_calls(EngineArena *dst_arena, EngineCallArray *dst_calls,
                                       const EngineCallArray *src_calls) {
    if (!dst_arena || !dst_calls || !src_calls || src_calls->count <= 0)
        return;

    EngineArena keys;
    engine_arena_init(&keys);
    EngineHashTable *seen = engine_ht_create((uint32_t)(dst_calls->count + src_calls->count + 1));

    for (int i = 0; i < dst_calls->count; i++) {
        const EngineCall *call = &dst_calls->items[i];
        if (!call->callee_name || !call->enclosing_func_qn ||
            call->site_end_byte <= call->site_start_byte) {
            continue;
        }
        char *key = engine_arena_sprintf(
        ...
        if (key && !engine_ht_get(seen, key))
            engine_ht_set(seen, key, (void *)(uintptr_t)(i + 1));
    }

    for (int i = 0; i < src_calls->count; i++) {
        const EngineCall *src = &src_calls->items[i];
        ...
        EngineCall dst = *src;
        dst.callee_name = engine_arena_strdup(dst_arena, src->callee_name);
        dst.enclosing_func_qn = engine_arena_strdup(dst_arena, src->enclosing_func_qn);
        dst.first_string_arg =
            src->first_string_arg ? engine_arena_strdup(dst_arena, src->first_string_arg) : NULL;
        dst.second_arg_name =
            src->second_arg_name ? engine_arena_strdup(dst_arena, src->second_arg_name) : NULL;
        for (int ai = 0; ai < ENGINE_MAX_CALL_ARGS; ai++) {
            dst.args[ai].expr =
                src->args[ai].expr ? engine_arena_strdup(dst_arena, src->args[ai].expr) : NULL;
            dst.args[ai].value =
                src->args[ai].value ? engine_arena_strdup(dst_arena, src->args[ai].value) : NULL;
            dst.args[ai].keyword =
                src->args[ai].keyword ? engine_arena_strdup(dst_arena, src->args[ai].keyword) : NULL;
        }
        engine_calls_push(dst_calls, dst_arena, dst);
        if (key)
            engine_ht_set(seen, key, (void *)(uintptr_t)dst_calls->count);
    }
 ...
 *
 * engine_pxc_run_one's signature is shared with the parallel pass
 ...
 * sequential driver (engine_pipeline_pass_lsp_cross, below) sets up once per
...
static _Thread_local const EngineCargoManifest *g_pxc_rust_manifest = NULL;

void engine_pxc_set_rust_manifest(const EngineCargoManifest *m) {
...
/* Convert a EngineLSPDef array (the pipeline's lingua franca, go_lsp.h:73)
 * into a EngineRustLSPDef array (rust_lsp.h) inside `arena`. The two structs
 * have similar fields but different layouts; EngineLSPDef adds
 * language/namespace metadata, so a memcpy is unsafe — copy field-by-field. */
static EngineRustLSPDef *pxc_lspdefs_to_rust(EngineArena *arena, const EngineLSPDef *defs, int def_count) {
    if (!defs || def_count <= 0)
        return NULL;
    EngineRustLSPDef *out =
        (EngineRustLSPDef *)engine_arena_alloc(arena, (size_t)def_count * sizeof(EngineRustLSPDef));
    if (!out)
        return NULL;
...
void engine_pxc_run_one(EngineLanguage lang, EngineFileResult *r, const char *source, int source_len,
                     const char *module_qn, EngineLSPDef *defs, int def_count, const char **imp_names,
                     const char **imp_qns, int imp_count) {
    TSTree *tree = r->cached_tree; /* may be NULL — LSP re-parses then */

    EngineArena scratch;
    engine_arena_init(&scratch);
    ...
    switch (lang) {
    case ENGINE_LANG_GO:
        engine_run_go_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
    ...
    case ENGINE_LANG_CPP: {
        ...
        engine_run_c_lsp_cross(&scratch, source, source_len, module_qn, cpp_mode, defs, def_count,
                            NULL, NULL, 0, tree, &out);
        break;
    }
    case ENGINE_LANG_PYTHON:
        engine_run_py_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
                             imp_qns, imp_count, tree, &out, &synthetic_calls);
        break;
    case ENGINE_LANG_PHP:
        engine_run_php_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
                              imp_qns, imp_count, tree, &out);
        break;
    case ENGINE_LANG_JAVA:
        engine_run_java_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
                               imp_qns, imp_count, tree, &out);
        break;
    case ENGINE_LANG_KOTLIN:
        engine_run_kotlin_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count,
                                 imp_names, imp_qns, imp_count, tree, &out);
        break;
    case ENGINE_LANG_RUST: {
        /* The Rust resolver wants EngineRustLSPDef (rust_lsp.h), not the
         * pipeline's EngineLSPDef — the structs share their first 9 fields
        ...
        EngineRustLSPDef *rdefs = pxc_lspdefs_to_rust(&scratch, defs, def_count);
        engine_run_rust_lsp_cross_with_manifest(&scratch, source, source_len, module_qn, rdefs,
    ...
    engine_arena_destroy(&scratch);
}

/* Variant of engine_pxc_run_one for TS/JS/JSX/TSX with explicit dialect
 * flags. Same scratch-arena lifecycle as engine_pxc_run_one. */
void engine_pxc_run_one_ts(EngineFileResult *r, const char *source, int source_len, const char *module_qn,
                        EngineLSPDef *defs, int def_count, const char **imp_names,
    ...
    engine_run_ts_lsp_cross(&scratch, source, source_len, module_qn, js_mode, jsx_mode, dts_mode, defs,
...
void engine_pxc_dispatch_file(EngineLanguage lang, EngineFileResult *result, const char *source,
    ...
    EngineTypeRegistry *prebuilt =
        cross_registries ? engine_pxc_registry_for_lang(cross_registries, lang) : NULL;
    if (prebuilt) {
        switch (lang) {
        case ENGINE_LANG_GO:
            /* Tier 3 (metadata-driven): pure lookup over the Tier-1
             * lsp_unresolved entries — no parse, no AST walk. */
            engine_go_fast_resolve_qualified_calls(result, prebuilt, imp_keys, imp_vals, imp_count);
            used_prebuilt = true;
            break;
        case ENGINE_LANG_PYTHON: {
            EngineArena scratch;
            engine_arena_init(&scratch);
            EngineResolvedCallArray out = {0};
            EngineCallArray synthetic_calls = {0};
            engine_run_py_lsp_cross_with_registry(&scratch, source, source_len, def_module, prebuilt,
        ...
        case ENGINE_LANG_CSHARP:
            engine_run_cs_lsp_cross_with_registry(&result->arena, source, source_len, def_module,
        ...
        case ENGINE_LANG_TSX: {
            ...
            bool jsx;
            bool dts;
            engine_pxc_ts_modes(lang, rel, &js, &jsx, &dts);
            EngineLSPDef *ts_defs = all_defs;
            int ts_def_count = all_def_count;
            EngineLSPDef *ts_filtered = NULL;
            if (module_def_index) {
                int fc = 0;
                bool filter_succeeded = false;
                ts_filtered = engine_pxc_filter_defs_for_file(
            ...
            engine_run_ts_lsp_cross_with_registry(&result->arena, source, source_len, def_module, js,
    ...
    EngineLSPDef *filtered = NULL;
    EngineLSPDef *file_defs = all_defs;
    int file_def_count = all_def_count;
    if (module_def_index && lang != ENGINE_LANG_RUST) {
        int filtered_count = 0;
        bool filter_succeeded = false;
        filtered = engine_pxc_filter_defs_for_file(module_def_index, all_defs, lang,
    ...
    if (lang == ENGINE_LANG_RUST) {
        EngineTypeRegistry *shared = rust_shared_get ? rust_shared_get(rust_shared_ctx) : NULL;
        if (shared) {
            EngineArena scratch;
            engine_arena_init(&scratch);
            EngineResolvedCallArray out = {0};
            EngineCallArray synthetic_calls = {0};
            engine_run_rust_lsp_cross_with_registry(
                &scratch, source, source_len, def_module, shared, imp_keys, imp_vals, imp_count,
                result->cached_tree, engine_pxc_get_rust_manifest(), &out, &synthetic_calls);
            pxc_append_results(&result->arena, &result->resolved_calls, &out);
            pxc_append_synthetic_calls(&result->arena, &result->calls, &synthetic_calls);
            engine_arena_destroy(&scratch);
        } else {
            engine_pxc_run_one(lang, result, source, source_len, def_module, file_defs, file_def_count,
                            imp_keys, imp_vals, imp_count);
        }
    } else if (lang == ENGINE_LANG_JAVASCRIPT || lang == ENGINE_LANG_TYPESCRIPT || lang == ENGINE_LANG_TSX) {
        bool js;
        bool jsx;
        bool dts;
        engine_pxc_ts_modes(lang, rel, &js, &jsx, &dts);
        engine_pxc_run_one_ts(result, source, source_len, def_module, file_defs, file_def_count,
...
static bool pxc_build_rust_manifest(const engine_pipeline_ctx_t *ctx, EngineArena *marena,
                                    EngineCargoManifest *out_m) {
    ...
    engine_cargo_parse(marena, toml, toml_len, out_m);
...
int engine_pipeline_pass_lsp_cross(engine_pipeline_ctx_t *ctx, const engine_file_info_t *files,
    ...
    engine_log_info("pass.start", "pass", "lsp_cross", "files", itoa_buf(file_count));
    ...
    for (int i = 0; i < file_count; i++) {
        if (cache[i] && files[i].language == ENGINE_LANG_RUST) {
            have_rust = true;
            break;
        }
    }
    EngineArena cargo_arena;
    EngineCargoManifest cargo_manifest;
    bool have_manifest = false;
    if (have_rust) {
        engine_arena_init(&cargo_arena);
        have_manifest = pxc_build_rust_manifest(ctx, &cargo_arena, &cargo_manifest);
        engine_pxc_set_rust_manifest(have_manifest ? &cargo_manifest : NULL);
    }

    /* Per-file module QN cache so we don't recompute it once per def + once
     * per call. engine_pipeline_fqn_module mallocs; freed at end. */
    char **def_modules = (char **)calloc((size_t)file_count, sizeof(char *));
    if (!def_modules) {
        engine_log_error("pass.err", "pass", "lsp_cross", "phase", "alloc");
        return 0;
    }

    int def_count = 0;
    int *def_starts = (int *)calloc((size_t)file_count + 1, sizeof(int));
    EngineLSPDef *all_defs = engine_pxc_collect_all_defs(cache, files, file_count, ctx->project_name,
    ...
    if (all_defs) {
        EngineArena *xa = &ctx->seq_cross_arena;
        if (!ctx->seq_cross_arena_live) {
            engine_arena_init(xa);
            ctx->seq_cross_arena_live = true;
        }
        cross_registries.go = engine_go_build_cross_registry(xa, all_defs, def_count);
        cross_registries.python = engine_py_build_cross_registry(xa, all_defs, def_count);
        cross_registries.c = engine_c_build_cross_registry(xa, all_defs, def_count);
        cross_registries.cs = engine_cs_build_cross_registry(xa, all_defs, def_count);
        cross_registries.ts = engine_ts_build_cross_registry(xa, all_defs, def_count);
    }
    ...
    for (int i = 0; i < file_count; i++) {
        if (!cache[i])
            continue;
        EngineLanguage lang = files[i].language;
        if (!engine_pxc_has_cross_lsp(lang)) {
            skipped_no_lsp++;
            continue;
        }
        ...
        if (!def_modules[i]) {
            def_modules[i] = engine_pipeline_fqn_module_dir(ctx->project_name, files[i].rel_path,
        ...
        int imp_count = 0;
        engine_pxc_build_import_map(ctx->gbuf, ctx->project_name, files[i].rel_path, lang, cache[i],
                                 &imp_keys, &imp_vals, &imp_count);
         ...
         * failure mode). */
        engine_index_mark_start(files[i].rel_path);
        engine_pxc_dispatch_file(lang, cache[i], source, source_len, files[i].rel_path, def_modules[i],
                              &cross_registries, module_def_index, all_defs, def_count, imp_keys,
                              imp_vals, imp_count, NULL, NULL);
        engine_index_mark_done(files[i].rel_path);
        per_lang_calls++;
        processed++;
    ...
    if (have_rust) {
        engine_pxc_set_rust_manifest(NULL);
        engine_arena_destroy(&cargo_arena);
    }
    (void)have_manifest;

    engine_log_info("pass.done", "pass", "lsp_cross", "files_processed", itoa_buf(processed),
...
struct EngineModuleDefIndex {
    EngineHashTable *ht;           /* module_qn → pxc_module_entry_t* */
    EngineHashTable *namespace_ht; /* declared package/namespace → pxc_module_entry_t* */
    int def_count;              /* total entries in the all_defs[] array */
};

/* engine_ht_foreach callback: free each pxc_module_entry_t. */
...
static pxc_module_entry_t *pxc_module_entry_get_or_create(EngineHashTable *ht, const char *key) {
    if (!ht || !key || !key[0]) {
        return NULL;
    }
    pxc_module_entry_t *e = (pxc_module_entry_t *)engine_ht_get(ht, key);
    if (e) {
        return e;
    }
...
static bool pxc_is_jvm_lang(EngineLanguage lang);
static bool pxc_def_lang_matches(EngineLanguage caller_lang, EngineLanguage def_lang);

static int pxc_mark_entry_defs(bool *selected, const pxc_module_entry_t *e,
                               const EngineLSPDef *all_defs, EngineLanguage caller_lang) {
    if (!selected || !e) {
        return 0;
    }
    int added = 0;
    for (int j = 0; j < e->count; j++) {
        int idx = e->indices[j];
        const EngineLSPDef *def = &all_defs[idx];
...
static bool pxc_is_jvm_lang(EngineLanguage lang) {
    return lang == ENGINE_LANG_JAVA || lang == ENGINE_LANG_KOTLIN;
}
...
static void pxc_mark_module_defs(const EngineModuleDefIndex *idx, bool *selected,
                                 const EngineLSPDef *all_defs, EngineLanguage caller_lang,
                                 const char *module_qn, int *total) {
    if (!idx || !idx->ht || !module_qn || !module_qn[0]) {
        return;
    }
    pxc_module_entry_t *e = (pxc_module_entry_t *)engine_ht_get(idx->ht, module_qn);
...
static void pxc_mark_import_defs(const EngineModuleDefIndex *idx, bool *selected,
                                 const EngineLSPDef *all_defs, EngineLanguage caller_lang,
    ...
    bool matched = false;
    for (;;) {
        pxc_module_entry_t *entry = (pxc_module_entry_t *)engine_ht_get(idx->ht, candidate);
        if (entry) {
    ...
    if (!matched && pxc_is_jvm_lang(caller_lang) && idx->namespace_ht) {
        ...
        for (;;) {
            pxc_module_entry_t *entry = (pxc_module_entry_t *)engine_ht_get(
...
EngineModuleDefIndex *engine_pxc_build_module_def_index(EngineLSPDef *all_defs, int def_count) {
    ...
    EngineHashTable *ht = engine_ht_create(64);
    EngineHashTable *namespace_ht = engine_ht_create(64);
    if (!ht || !namespace_ht) {
        engine_ht_free(ht);
        engine_ht_free(namespace_ht);
        return NULL;
    }
    ...
    EngineModuleDefIndex *idx = (EngineModuleDefIndex *)calloc(1, sizeof(*idx));
    if (!idx) {
        engine_ht_foreach(ht, pxc_module_entry_free_cb, NULL);
        engine_ht_free(ht);
        engine_ht_foreach(namespace_ht, pxc_module_entry_free_cb, NULL);
        engine_ht_free(namespace_ht);
        return NULL;
    }
    idx->ht = ht;
...
void engine_pxc_free_module_def_index(EngineModuleDefIndex *idx) {
    ...
    if (idx->ht) {
        engine_ht_foreach(idx->ht, pxc_module_entry_free_cb, NULL);
        engine_ht_free(idx->ht);
    }
    if (idx->namespace_ht) {
        engine_ht_foreach(idx->namespace_ht, pxc_module_entry_free_cb, NULL);
        engine_ht_free(idx->namespace_ht);
    }
    free(idx);
}
...
EngineLSPDef *engine_pxc_filter_defs_for_file(const EngineModuleDefIndex *idx, EngineLSPDef *all_defs,
    ...
    if (pxc_is_jvm_lang(caller_lang) && idx->namespace_ht) {
        ...
        pxc_module_entry_t *e = (pxc_module_entry_t *)engine_ht_get(idx->namespace_ht, namespace_key);
    ...
    EngineLSPDef *out = (EngineLSPDef *)malloc((size_t)total * sizeof(EngineLSPDef));
    if (!out) {
        free(selected);
        return NULL;
    }
