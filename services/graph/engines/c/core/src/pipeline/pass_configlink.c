...
static bool is_dep_section(const char *s) {
    ...
    for (int i = 0; secs[i]; i++) {
        if (engine_strcasestr(s, secs[i]) != NULL) {
            return true;
        }
    }
    return false;
...
typedef struct {
    int64_t node_id;
    char normalized[ENGINE_SZ_256];
    char name[ENGINE_SZ_256];
} config_entry_t;

/* Collect config Variable nodes with ≥2 tokens, each ≥3 chars. */
static int collect_config_entries(const engine_gbuf_node_t *const *vars, int var_count,
                                  config_entry_t *out, int max_out) {
    int n = 0;
    for (int i = 0; i < var_count && n < max_out; i++) {
        if (!engine_has_config_extension(vars[i]->file_path)) {
            continue;
        }

        char norm[ENGINE_SZ_256];
        int tokens = engine_normalize_config_key(vars[i]->name, norm, sizeof(norm));
        if (tokens < PAIR_LEN) {
            continue;
        }
...
static int collect_code_entries(engine_gbuf_t *gb, code_entry_t *out, int max_out) {
    int n = 0;
    ...
    for (int li = 0; labels[li] && n < max_out; li++) {
        ...
        for (int i = 0; i < count && n < max_out; i++) {
            if (engine_has_config_extension(nodes[i]->file_path)) {
                continue;
            }

            char norm[ENGINE_SZ_256];
            int tokens = engine_normalize_config_key(nodes[i]->name, norm, sizeof(norm));
...
static int strategy_key_symbols(engine_gbuf_t *gb) {
    /* Get all Variable nodes */
    const engine_gbuf_node_t **vars = NULL;
    int var_count = 0;
    if (engine_gbuf_find_by_label(gb, "Variable", &vars, &var_count) != 0) {
        return 0;
    }

    config_entry_t config_entries[ENGINE_SZ_4K];
    int config_count = collect_config_entries(vars, var_count, config_entries, ENGINE_SZ_4K);

    if (config_count == 0) {
        return 0;
    }

    code_entry_t code_entries[ENGINE_SZ_8K];
    int code_count = collect_code_entries(gb, code_entries, ENGINE_SZ_8K);
    ...
    for (int ci = 0; ci < config_count; ci++) {
        for (int co = 0; co < code_count; co++) {
            ...
            if (confidence > 0.0) {
                ...
                engine_gbuf_insert_edge(gb, code_entries[co].node_id, config_entries[ci].node_id,
                                     "CONFIGURES", props);
                edge_count++;
            }
        }
...
static int collect_manifest_deps(const engine_gbuf_node_t *const *vars, int var_count,
...
static double match_dep_to_import(const engine_gbuf_node_t *target, const char *dep_lower) {
    char target_lower[ENGINE_SZ_256];
...
static int strategy_dep_imports(engine_gbuf_t *gb) {
    const engine_gbuf_node_t **vars = NULL;
    int var_count = 0;
    if (engine_gbuf_find_by_label(gb, "Variable", &vars, &var_count) != 0) {
        return 0;
    ...
    dep_entry_t deps[ENGINE_SZ_2K];
    int dep_count = collect_manifest_deps(vars, var_count, deps, ENGINE_SZ_2K);
    ...
    const engine_gbuf_edge_t **imports = NULL;
    int import_count = 0;
    if (engine_gbuf_find_edges_by_type(gb, "IMPORTS", &imports, &import_count) != 0) {
        return 0;
    }
    ...
    for (int di = 0; di < dep_count; di++) {
        char dep_lower[ENGINE_SZ_256];
        lowercase_into(dep_lower, sizeof(dep_lower), deps[di].name);

        for (int ii = 0; ii < import_count; ii++) {
            const engine_gbuf_node_t *target = engine_gbuf_find_by_id(gb, imports[ii]->target_id);
            if (!target) {
                continue;
            }

            const engine_gbuf_node_t *source = engine_gbuf_find_by_id(gb, imports[ii]->source_id);
            if (!source) {
                continue;
            }
            ...
            if (confidence > 0.0) {
                ...
                engine_gbuf_insert_edge(gb, source->id, deps[di].node_id, "CONFIGURES", props);
                edge_count++;
            }
        }
    }
...
int engine_pipeline_pass_configlink(engine_pipeline_ctx_t *ctx) {
    engine_gbuf_t *gb = ctx->gbuf;
    /* Early exit: check if any config files exist in the project. */
    bool has_config = false;

    const engine_gbuf_node_t **vars_check = NULL;
    int var_check_count = 0;
    if (!has_config && engine_gbuf_find_by_label(gb, "Variable", &vars_check, &var_check_count) == 0) {
        for (int i = 0; i < var_check_count; i++) {
            if (engine_has_config_extension(vars_check[i]->file_path)) {
                has_config = true;
                break;
            }
        }
    ...
    if (!has_config) {
        const engine_gbuf_node_t **mods_check = NULL;
        int mod_check_count = 0;
        if (engine_gbuf_find_by_label(gb, "Module", &mods_check, &mod_check_count) == 0) {
            for (int i = 0; i < mod_check_count; i++) {
                if (engine_has_config_extension(mods_check[i]->file_path)) {
                    has_config = true;
                    break;
                }
            }
    ...
    if (!has_config) {
        engine_log_info("configlinker.skip", "reason", "no_config_files");
        return 0;
    }

    char buf1[ENGINE_SZ_16];
    char buf2[ENGINE_SZ_16];
    char buf3[ENGINE_SZ_16];
    char buf4[ENGINE_SZ_16];

    int key_edges = strategy_key_symbols(gb);
    snprintf(buf1, sizeof(buf1), "%d", key_edges);
    engine_log_info("configlinker.strategy", "name", "key_symbol", "edges", buf1);

    int dep_edges = strategy_dep_imports(gb);
    snprintf(buf2, sizeof(buf2), "%d", dep_edges);
    engine_log_info("configlinker.strategy", "name", "dep_import", "edges", buf2);
    ...
    engine_log_info("configlinker.strategy", "name", "file_ref", "edges", buf3);

    snprintf(buf4, sizeof(buf4), "%d", key_edges + dep_edges + ref_edges);
    engine_log_info("configlinker.done", "total", buf4);
