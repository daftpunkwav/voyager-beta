...
static char *pkgmap_read_file(const char *path, int *out_len) {
    ...
    if (size <= 0 || size > (long)ENGINE_SZ_1K * ENGINE_SZ_1K) { /* 1MB cap for manifests */
        (void)fclose(f);
        return NULL;
    }
...
static void pkg_entries_push(engine_pkg_entries_t *e, char *pkg_name, char *entry_rel) {
    if (e->count >= e->cap) {
        int new_cap = e->cap == 0 ? PKGMAP_INIT_CAP : e->cap * SKIP_ONE * PAIR_LEN;
        engine_pkg_entry_t *tmp = realloc(e->items, new_cap * sizeof(engine_pkg_entry_t));
        if (!tmp) {
            free(pkg_name);
            free(entry_rel);
            return;
...
static char *path_dirname(const char *rel_path) {
    ...
    return engine_strndup(rel_path, (size_t)(last - rel_path));
}
...
static char *strip_extension(const char *path) {
    ...
    for (size_t i = len; i > 0; i--) {
        if (path[i - SKIP_ONE] == '.') {
            return engine_strndup(path, i - SKIP_ONE);
        }
        if (path[i - SKIP_ONE] == '/') {
            break;
        }
...
static char *extract_quoted(const char *p, const char *end) {
    ...
    return engine_strndup(start, (size_t)(p - start));
}
...
static void parse_go_mod(const char *source, int source_len, const char *rel_path,
    ...
    char *module_path = engine_strndup(start, (size_t)(val - start));
    char *dir = path_dirname(rel_path);
...
static void extract_psr4(yyjson_val *root, const char *dir, engine_pkg_entries_t *entries) {
...
static void parse_pubspec_yaml(const char *source, int source_len, const char *rel_path,
                               engine_pkg_entries_t *entries) {
    engine_yaml_node_t *root = engine_yaml_parse(source, source_len);
    if (!root) {
        return;
    }
    const char *name = engine_yaml_get_str(root, "name");
    if (name && name[0] != '\0') {
        char *dir = path_dirname(rel_path);
...
static char *xml_tag_content(const char *p, const char *end) {
    ...
    return engine_strndup(s, (size_t)(p - s));
}
...
static void parse_mix_exs(const char *source, int source_len, const char *rel_path,
    ...
    char *app_name = engine_strndup(start, (size_t)(val - start));
    char *dir = path_dirname(rel_path);
    char entry[PKGMAP_PATH_BUF];
 ...
 * manifest walk (engine_pkgmap_scan_repo) reaches it, exactly like a JS
...
static char *swift_quoted_literal(const char *p, const char *end) {
    ...
    char *value = engine_strndup(start, (size_t)(p - start));
...
static void swift_register_target(const char *rel_path, const char *target_name,
                                  const char *literal_path, engine_pkg_entries_t *entries) {
...
EngineHashTable *engine_pkgmap_build(engine_pkg_entries_t *worker_entries, int worker_count,
    ...
    EngineHashTable *map = engine_ht_create(PKGMAP_HT_INIT);
    int merged = 0;

    for (int w = 0; w < worker_count; w++) {
        engine_pkg_entries_t *we = &worker_entries[w];
        for (int i = 0; i < we->count; i++) {
            /* Convert entry_rel to QN: project.dir.parts */
            char *qn = engine_pipeline_fqn_module(project_name, we->items[i].entry_rel);
            if (!qn) {
                continue;
            }

            /* Check for duplicate — first wins */
            if (engine_ht_has(map, we->items[i].pkg_name)) {
                free(qn);
                continue;
            }
    ...
    engine_log_info("pkgmap.build", "entries", pkgmap_itoa(merged));
    return map;
}

/* Returns true if basename is a package manifest we know how to parse.
 * Used by the filesystem walker; engine_pkgmap_try_parse is the source of
 ...
 * matched by the shared engine_should_skip_dir helper so we don't walk
 * node_modules, .git, build, etc. Returns the number of manifests
 * parsed, accumulated across the whole walk.
 *
 * Cross-platform: uses the portable engine_opendir/engine_readdir/engine_closedir
...
static int pkgmap_walk_dir(const char *abs_dir, const char *rel_dir, engine_pkg_entries_t *entries,
                           int depth, char **excluded_dirs, int excluded_count) {
    if (depth >= PKGMAP_WALK_MAX_DEPTH) {
        engine_log_info("pkgmap.walk", "depth_cap", rel_dir && rel_dir[0] ? rel_dir : ".");
        return 0;
    }
    ...
    while ((entry = engine_readdir(dir)) != NULL) {
        ...
        if (engine_pkgmap_try_parse(name, rel_path, source, source_len, entries)) {
            parsed++;
        }
        free(source);
    }
...
int engine_pkgmap_scan_repo(const char *repo_path, engine_pkg_entries_t *entries, char **excluded_dirs,
    ...
    engine_log_info("pkgmap.scan_repo", "manifests", pkgmap_itoa(parsed));
    return parsed;
}
...
EngineHashTable *engine_pkgmap_build_from_files(const engine_file_info_t *files, int file_count,
                                          const char *project_name) {
    engine_pkg_entries_t entries;
    engine_pkg_entries_init(&entries);
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        engine_pkgmap_try_parse(basename, files[i].rel_path, source, source_len, &entries);
        free(source);
    }

    EngineHashTable *map = engine_pkgmap_build(&entries, SKIP_ONE, project_name);
    engine_pkg_entries_free(&entries);
    return map;
}

/* Variant of engine_pkgmap_build_from_files that ALSO walks the repo
...
EngineHashTable *engine_pkgmap_build_from_repo(const char *repo_path, const engine_file_info_t *files,
    ...
    int from_walk = engine_pkgmap_scan_repo(repo_path, &entries, excluded_dirs, excluded_count);
    engine_log_info("pkgmap.scan", "manifests_from_files", pkgmap_itoa(from_files),
...
void engine_pkgmap_free(EngineHashTable *pkgmap) {
    ...
    engine_ht_foreach(pkgmap, pkgmap_free_entry, NULL);
    engine_ht_free(pkgmap);
}
...
static char *resolve_slash_prefix(EngineHashTable *map, const char *module_path) {
    ...
    for (char *slash = buf + strlen(buf) - SKIP_ONE; slash > buf; slash--) {
        ...
        *slash = '\0';
        const char *base_qn = (const char *)engine_ht_get(map, buf);
        if (!base_qn) {
            continue;
        }
 ...
 * Returns heap QN or NULL. */
static char *resolve_dot_prefix(EngineHashTable *map, const char *module_path,
                                const char *project_name) {
    char *buf = strdup(module_path);
    if (!buf) {
        return NULL;
    ...
    for (char *dot = buf + strlen(buf) - SKIP_ONE; dot > buf; dot--) {
        ...
        *dot = '\0';
        ...
        free(buf);
        return engine_pipeline_fqn_module(project_name, result);
    }
    free(buf);
    return NULL;
}
 ...
 * Returns heap QN or NULL. */
static char *resolve_backslash_prefix(EngineHashTable *map, const char *module_path,
                                      const char *project_name) {
    char *buf = strdup(module_path);
    if (!buf) {
        return NULL;
    ...
    for (char *bs = buf + strlen(buf) - SKIP_ONE; bs > buf; bs--) {
        ...
        *bs = '\0';
        ...
        const char *base_dir = (const char *)engine_ht_get(map, prefix);
        if (!base_dir) {
            continue;
        }
        ...
        free(buf);
        return engine_pipeline_fqn_module(project_name, path_result);
    }
    free(buf);
    return NULL;
}
...
char *engine_pipeline_resolve_module(const engine_pipeline_ctx_t *ctx, const char *source_rel,
    ...
    if (!ctx || !module_path) {
        return engine_pipeline_fqn_module(ctx ? ctx->project_name : NULL, module_path);
    }

    /* 1. Try relative import resolution (existing logic) */
    char *resolved = engine_pipeline_resolve_relative_import(source_rel, module_path);
    if (resolved) {
        char *qn = engine_pipeline_fqn_module(ctx->project_name, resolved);
        free(resolved);
        return qn;
    }
    ...
    if (ctx->path_aliases && source_rel) {
        const engine_path_alias_map_t *amap =
            engine_path_alias_find_for_file(ctx->path_aliases, source_rel);
        if (amap) {
            char *aliased = engine_path_alias_resolve(amap, module_path);
            if (aliased) {
                char *qn = engine_pipeline_fqn_module(ctx->project_name, aliased);
                free(aliased);
                return qn;
            }
        }
    ...
    EngineHashTable *pkgmap = engine_pipeline_get_pkgmap();
    if (!pkgmap) {
        return engine_pipeline_fqn_module(ctx->project_name, module_path);
    }

    /* 3. Exact lookup */
    const char *mapped_qn = (const char *)engine_ht_get(pkgmap, module_path);
    if (mapped_qn) {
...
static const engine_gbuf_node_t *resolve_exact_file_node(const engine_pipeline_ctx_t *ctx,
    ...
    const engine_gbuf_node_t *best = NULL;
    for (int ni = 0; ni < 2; ni++) {
        ...
        int hit_count = 0;
        if (engine_gbuf_find_by_name(ctx->gbuf, name, &hits, &hit_count) != 0 || !hits) {
            continue;
        }
...
static const engine_gbuf_node_t *resolve_header_include(const engine_pipeline_ctx_t *ctx,
    ...
    const engine_gbuf_node_t *exact = resolve_exact_file_node(ctx, base, source_file_qn);
    if (exact) {
        return exact;
    }
...
static const engine_gbuf_node_t *resolve_sibling_file(const engine_pipeline_ctx_t *ctx,
                                                   const char *source_rel,
                                                   const char *source_file_qn,
                                                   const char *module_path) {
    if (!module_path || !module_path[0]) {
    ...
    /* 2. SCSS partial: dir/[subdir/]_<basename>.scss (underscore-prefixed). */
    {
        ...
        char *dpart = slash ? engine_strndup(base, (size_t)(slash - base)) : strdup("");
        if (dpart && bn[0] != '_') {
    ...
    for (int i = 0; i < ncand; i++) {
        char *qn = engine_pipeline_fqn_module(ctx->project_name, cands[i]);
        if (!qn) {
            continue;
        }
        const engine_gbuf_node_t *n = engine_gbuf_find_by_qn(ctx->gbuf, qn);
        free(qn);
...
const engine_gbuf_node_t *engine_pipeline_resolve_import_node(const engine_pipeline_ctx_t *ctx,
    ...
    const engine_gbuf_node_t *header_target =
    ...
    char *target_qn = engine_pipeline_resolve_module(ctx, source_rel, imp->module_path);
    const engine_gbuf_node_t *target = target_qn ? engine_gbuf_find_by_qn(ctx->gbuf, target_qn) : NULL;
    free(target_qn);
    if (target) {
        return target;
    }
    ...
    if (namespace_map) {
        ...
        for (;;) {
            ...
            const char *list = (const char *)engine_ht_get(namespace_map, norm);
            for (const char *seg = list; seg && *seg;) {
                ...
                if (len > 0 && len < sizeof(qbuf)) {
                    memcpy(qbuf, seg, len);
                    qbuf[len] = '\0';
                    const engine_gbuf_node_t *n = engine_gbuf_find_by_qn(ctx->gbuf, qbuf);
    ...
    }
    {
        ...
        for (int ci = 0; ci < ncands; ci++) {
            const engine_gbuf_node_t **hits = NULL;
            int n = 0;
            if (engine_gbuf_find_by_name(ctx->gbuf, cands[ci], &hits, &n) == 0 && hits) {
     ...
     * remaining path (and successive prefixes) to a Module/File node. */
    {
        ...
        if (body[0]) {
            ...
            for (;;) {
                char *rqn = engine_pipeline_resolve_module(ctx, source_rel, work);
                const engine_gbuf_node_t *n = rqn ? engine_gbuf_find_by_qn(ctx->gbuf, rqn) : NULL;
                free(rqn);
                if (n && import_targetable_label(n->label) &&
                    (!source_file_qn || !n->qualified_name ||
                     strcmp(n->qualified_name, source_file_qn) != 0)) {
...
EngineHashTable *engine_pipeline_namespace_map_build(const char *project_name,
    ...
    EngineHashTable *map = NULL;
    for (int i = 0; i < count; i++) {
        const EngineFileResult *r = results[i];
        if (!r || !r->namespace_name || !r->namespace_name[0] || !rels[i]) {
            continue;
        }
        if (!map) {
            map = engine_ht_create(ENGINE_SZ_64);
            if (!map) {
                return NULL;
            }
        }
        char *file_qn = engine_pipeline_fqn_compute(project_name, rels[i], "__file__");
        if (!file_qn) {
            continue;
        }
        /* Normalize the namespace key to dot-separated form so it matches the
         * dot-normalized lookups in engine_pipeline_resolve_import_node (PHP uses
         * '\\', some grammars '::' or '/'). */
        ...
        if (!engine_ht_has(map, key)) {
            engine_ht_set(map, key, file_qn); /* map owns key + file_qn */
        } else {
            ...
            const char *stored_key = engine_ht_get_key(map, key);
            const char *cur = (const char *)engine_ht_get(map, key);
            char *combined = NULL;
            if (stored_key && cur) {
                ...
                if (combined) {
                    ...
                    void *prev = engine_ht_set(map, stored_key, combined);
...
static void ns_map_free_entry(const char *key, void *value, void *ud) {
    (void)ud;
    free((void *)key); /* strdup'd in engine_pipeline_namespace_map_build */
    free(value);
}
...
void engine_pipeline_namespace_map_free(EngineHashTable *map) {
    ...
    engine_ht_foreach(map, ns_map_free_entry, NULL);
    engine_ht_free(map);
}
