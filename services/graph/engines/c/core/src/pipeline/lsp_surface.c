 ...
 *     values in the EngineLSPDef contract — receiver_type NULL means "not a
 ...
 * carry into the EngineLSPDef set. Their (name, qn, label) triple must still
...
static char *surface_file_to_json(const EngineFileResult *result, const EngineLSPDef *defs,
                                  int def_count) {
    ...
    if (result) {
        for (int i = 0; i < result->defs.count; i++) {
            const EngineDefinition *d = &result->defs.items[i];
...
int engine_lsp_surface_build_rows(const char *project, EngineFileResult **cache,
                               const engine_file_info_t *files, int file_count,
                               const EngineLSPDef *all_defs, const int *def_starts,
    ...
    *out_count = 0;
    ...
    engine_lsp_surface_row_t *rows = calloc((size_t)file_count, sizeof(*rows));
    if (!rows) {
        return -1;
    }
    int n = 0;
    for (int i = 0; i < file_count; i++) {
        ...
        engine_sha256_hex(json, strlen(json), sha);
...
static const char *arena_str_or_null(EngineArena *arena, yyjson_val *v) {
    if (!v || yyjson_is_null(v)) {
        return NULL;
    }
    const char *s = yyjson_get_str(v);
    return s ? engine_arena_strdup(arena, s) : NULL;
}

static const char **arena_str_array(EngineArena *arena, yyjson_val *arr, bool null_terminated,
                                    int *out_count) {
    *out_count = 0;
    ...
    const char **items =
        engine_arena_alloc(arena, (size_t)(count + (null_terminated ? 1 : 0)) * sizeof(char *));
    if (!items) {
        return NULL;
    }
    for (int i = 0; i < count; i++) {
        const char *s = yyjson_get_str(yyjson_arr_get(arr, (size_t)i));
        items[i] = s ? engine_arena_strdup(arena, s) : "?";
    }
    if (null_terminated) {
        items[count] = NULL;
    }
...
int engine_lsp_surface_defs_from_json(EngineArena *arena, const char *defs_json, EngineLSPDef **out_defs) {
    *out_defs = NULL;
    if (!defs_json) {
        return -1;
    }
    ...
    EngineLSPDef *defs = engine_arena_alloc(arena, (size_t)count * sizeof(EngineLSPDef));
    if (!defs) {
        yyjson_doc_free(doc);
        return -1;
    }
    memset(defs, 0, (size_t)count * sizeof(EngineLSPDef));
    for (int i = 0; i < count; i++) {
        ...
        d->lang = (EngineLanguage)yyjson_get_int(yyjson_obj_get(o, "lg"));
