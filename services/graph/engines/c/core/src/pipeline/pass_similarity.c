...
static bool parse_fp_from_props(const char *props_json, engine_minhash_t *out) {
    if (!props_json) {
        return false;
    }
    ...
    if (hex_len != ENGINE_MINHASH_HEX_LEN) {
        return false;
    }
    char hex_buf[ENGINE_MINHASH_HEX_BUF];
    memcpy(hex_buf, hex_start, (size_t)hex_len);
    hex_buf[hex_len] = '\0';
    return engine_minhash_from_hex(hex_buf, out);
}
...
static int collect_fp_entries(engine_gbuf_t *gbuf, fp_entry_t **out_entries) {
    ...
    for (int li = 0; labels[li]; li++) {
        ...
        for (int i = 0; i < node_count; i++) {
            const engine_gbuf_node_t *n = nodes[i];
            engine_minhash_t fp;
...
static void sim_query_worker(int worker_id, void *ctx_ptr) {
    ...
    const engine_lsh_entry_t *cands[SIM_CAND_CAP];
    ...
    while (true) {
        ...
        if (ec >= ENGINE_MINHASH_MAX_EDGES_PER_NODE) {
            continue;
        }

        const fp_entry_t *src = &sc->entries[i];
        int cand_count = engine_lsh_query_into(sc->lsh, &src->fp, cands, SIM_CAND_CAP);

        int emitted = 0;
        for (int c = 0; c < cand_count; c++) {
            const engine_lsh_entry_t *cand = cands[c];
            ...
            if (cur + emitted >= ENGINE_MINHASH_MAX_EDGES_PER_NODE) {
                break;
            }

            double jaccard = engine_minhash_jaccard(&src->fp, cand->fingerprint);
            if (jaccard < ENGINE_MINHASH_JACCARD_THRESHOLD) {
                continue;
            }
...
static int merge_sim_edges(engine_gbuf_t *gbuf, sim_edge_buf_t *worker_bufs, int worker_count) {
    int total = 0;
    for (int w = 0; w < worker_count; w++) {
        for (int e = 0; e < worker_bufs[w].count; e++) {
            ...
            engine_gbuf_insert_edge(gbuf, de->source_id, de->target_id, "SIMILAR_TO", props);
            total++;
        }
...
int engine_pipeline_pass_similarity(engine_pipeline_ctx_t *ctx) {
    engine_log_info("pass.start", "pass", "similarity");

    engine_gbuf_t *gbuf = ctx->gbuf;

    /* Phase 1: Collect fingerprints from Function/Method nodes */
    ENGINE_PROF_START(t_collect);
    fp_entry_t *entries = NULL;
    int entry_count = collect_fp_entries(gbuf, &entries);
    ENGINE_PROF_END_N("similarity", "1_collect_fp", t_collect, entry_count);

    engine_log_info("pass.similarity.collected", "nodes_with_fp", itoa_log(entry_count));

    if (entry_count < MIN_FP_ENTRIES) {
        free(entries);
        engine_log_info("pass.done", "pass", "similarity", "edges", "0");
        return 0;
    }

    /* Phase 2: Build LSH index (sequential — engine_lsh_insert mutates shared state) */
    ENGINE_PROF_START(t_lsh_build);
    engine_lsh_index_t *lsh = engine_lsh_new();
    engine_lsh_entry_t *lsh_entries = malloc((size_t)entry_count * sizeof(engine_lsh_entry_t));
    if (!lsh_entries) {
        free(entries);
        engine_lsh_free(lsh);
    ...
    for (int i = 0; i < entry_count; i++) {
        lsh_entries[i] = (engine_lsh_entry_t){
        ...
        engine_lsh_insert(lsh, &lsh_entries[i]);
    }
    ENGINE_PROF_END_N("similarity", "2_lsh_build_seq", t_lsh_build, entry_count);

    /* Phase 3: Query LSH + emit edges (PARALLEL via engine_lsh_query_into).
    ...
    ENGINE_PROF_START(t_query_emit);
...

    {
        ...
        engine_parallel_for(worker_count, sim_query_worker, &sc, opts);
    }
    ENGINE_PROF_END_N("similarity", "3_query_parallel", t_query_emit, entry_count);

    ENGINE_PROF_START(t_merge);
    int total_edges = merge_sim_edges(gbuf, worker_bufs, worker_count);
    ENGINE_PROF_END_N("similarity", "4_edge_merge_seq", t_merge, total_edges);

    engine_log_info("pass.done", "pass", "similarity", "edges", itoa_log(total_edges));
