 ...
 *   Matches tree-sitter SubtreeHeapData (ENGINE_SZ_64 bytes). Backed by
 ...
 *
 * Usage:
 *   engine_slab_install();         // once, before any parsing
 *   ... parse files ...
 *   engine_slab_destroy_thread();  // on thread exit — frees owned memory
 */
#ifndef ENGINE_SLAB_ALLOC_H
#define ENGINE_SLAB_ALLOC_H
 ...
 * Only safe after engine_destroy_thread_parser() has been called. */
void engine_slab_reset_thread(void);
...
void engine_slab_destroy_thread(void);
...
void engine_slab_reclaim(void);

/* Test/diagnostic API: direct access to the slab allocator.
 * Use these to unit test slab (≤64B) and heap (>64B) paths. */
void *engine_slab_test_malloc(size_t size);
void engine_slab_test_free(void *ptr);
void *engine_slab_test_realloc(void *ptr, size_t size);
void *engine_slab_test_calloc(size_t count, size_t size);

#endif /* ENGINE_SLAB_ALLOC_H */
