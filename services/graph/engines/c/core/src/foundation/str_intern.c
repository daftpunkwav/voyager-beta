...
EngineInternPool *engine_intern_create(void) {
    EngineInternPool *p = (EngineInternPool *)calloc(ENGINE_ALLOC_ONE, sizeof(*p));
    if (!p) {
        return NULL;
    }
    engine_arena_init(&p->arena);
    p->capacity = ENGINE_SZ_256;
    p->mask = p->capacity - SKIP_ONE;
    p->buckets = (InternEntry *)calloc(p->capacity, sizeof(InternEntry));
    if (!p->buckets) {
        engine_arena_destroy(&p->arena);
        free(p);
        return NULL;
    }
    return p;
...
void engine_intern_free(EngineInternPool *pool) {
    ...
    engine_arena_destroy(&pool->arena);
    free(pool->buckets);
    free(pool);
}

static void intern_resize(EngineInternPool *p) {
...
const char *engine_intern_n(EngineInternPool *pool, const char *s, size_t len) {
    ...
    /* Resize at 70% load */
    if (pool->count * ENGINE_DECIMAL_BASE >= pool->capacity * INTERN_LOAD_NUM) {
        intern_resize(pool);
    ...
    char *copy = engine_arena_strndup(&pool->arena, s, len);
    if (!copy) {
        return NULL;
    }
...
const char *engine_intern(EngineInternPool *pool, const char *s) {
    ...
    return engine_intern_n(pool, s, strlen(s));
}
