...
long engine_max_file_bytes(void) {
    ...
    const char *raw = getenv("ENGINE_MAX_FILE_BYTES");
    if (raw && raw[0]) {
        errno = 0;
        char *end = NULL;
 ...
 * Read fresh each call (see engine_max_file_bytes rationale — cheap, test-friendly,
...
int engine_cypher_max_depth(void) {
    ...
    return env_positive_int("ENGINE_CYPHER_MAX_DEPTH", 10);
}
...
int engine_mcp_max_depth(void) {
    ...
    return env_positive_int("ENGINE_MCP_MAX_DEPTH", 15);
}
