...
#define ENGINE_NSEC_PER_SEC 1000000000ULL

static uint64_t engine_platform_scale_fraction(uint64_t remainder, uint64_t multiplier,
                                            uint64_t divisor) {
    uint64_t quotient = 0;
    uint64_t reduced = 0;
    uint64_t mask = 1;
...
uint64_t engine_platform_scale_counter_ns(uint64_t counter, uint64_t frequency) {
    ...
    if (whole_seconds > UINT64_MAX / ENGINE_NSEC_PER_SEC) {
        return UINT64_MAX;
    }
    uint64_t whole_ns = whole_seconds * ENGINE_NSEC_PER_SEC;
    uint64_t fraction_ns = engine_platform_scale_fraction(remainder, ENGINE_NSEC_PER_SEC, frequency);
...
static void engine_canonicalize_drive(char *path) {
...
void *engine_mmap_read(const char *path, size_t *out_size) {
    if (!path || !out_size) {
        return NULL;
    }
    *out_size = 0;
...
void engine_munmap(void *addr, size_t size) {
    (void)size;
    if (addr) {
...
uint64_t engine_now_ns(void) {
    LARGE_INTEGER freq, count;
    QueryPerformanceFrequency(&freq);
    QueryPerformanceCounter(&count);
    return engine_platform_scale_counter_ns((uint64_t)count.QuadPart, (uint64_t)freq.QuadPart);
}

#define ENGINE_USEC_PER_SEC 1000000ULL

uint64_t engine_now_ms(void) {
    return engine_now_ns() / ENGINE_USEC_PER_SEC;
}

int engine_nprocs(void) {
    SYSTEM_INFO si;
    GetSystemInfo(&si);
    return (int)si.dwNumberOfProcessors > 0 ? (int)si.dwNumberOfProcessors : 1;
}

bool engine_file_exists(const char *path) {
    wchar_t *wpath = engine_path_to_wide(path);
    if (!wpath) {
        return false;
    }
...
bool engine_is_dir(const char *path) {
    wchar_t *wpath = engine_path_to_wide(path);
    if (!wpath) {
        return false;
    }
...
int64_t engine_file_size(const char *path) {
    wchar_t *wpath = engine_path_to_wide(path);
    if (!wpath) {
...
char *engine_normalize_path_sep(char *path) {
    if (path) {
        for (char *p = path; *p; p++) {
            if (*p == '\\') {
                *p = '/';
        ...
        engine_canonicalize_drive(path);
    }
    return path;
}
...
static void engine_timebase_initialize(void) {
...
uint64_t engine_now_ns(void) {
    (void)pthread_once(&timebase_once, engine_timebase_initialize);
...
#ifdef _WIN32
#include <stdlib.h>
#define ENGINE_ENVIRON _environ
#elif defined(__APPLE__)
#include <crt_externs.h>
#define ENGINE_ENVIRON (*_NSGetEnviron())
#else
extern char **environ;
#define ENGINE_ENVIRON environ
#endif
...
const char *engine_safe_getenv(const char *name, char *buf, size_t buf_sz, const char *fallback) {
     ...
     * ENGINE_CACHE_DIR) arrives here either mojibake'd or with unrepresentable
     ...
     * UTF-8-path convention the rest of the codebase (engine_fopen, _wmkdir,
     * SQLite VFS) already assumes. */
    {
        wchar_t wname[ENGINE_SZ_256];
        int wn = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, name, -1, wname, ENGINE_SZ_256);
        if (wn > 0) {
            ...
            char *utf8 = engine_wide_to_utf8(wval);
            free(wval);
            if (!utf8) {
                return NULL;
            }
...
const char *engine_get_home_dir(void) {
    static ENGINE_TLS char buf[ENGINE_SZ_1K];
    char tmp[ENGINE_SZ_256] = "";

    engine_safe_getenv("HOME", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
    ...
    engine_safe_getenv("USERPROFILE", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        engine_normalize_path_sep(buf);
        return buf;
...
const char *engine_app_config_dir(void) {
    static ENGINE_TLS char buf[ENGINE_SZ_1K];
    char tmp[ENGINE_SZ_256] = "";
#ifdef _WIN32
    engine_safe_getenv("APPDATA", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        engine_normalize_path_sep(buf);
        return buf;
    ...
    engine_safe_getenv("XDG_CONFIG_HOME", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        return buf;
    }
...
const char *engine_app_local_dir(void) {
#ifdef _WIN32
    static ENGINE_TLS char buf[ENGINE_SZ_1K];
    char tmp[ENGINE_SZ_256] = "";
    engine_safe_getenv("LOCALAPPDATA", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        engine_normalize_path_sep(buf);
        return buf;
...
const char *engine_resolve_cache_dir(void) {
    static ENGINE_TLS char buf[ENGINE_SZ_4K];
    static const char missing[] = "\x1f"
                                  "ENGINE_CACHE_DIR_MISSING"
                                  "\x1f";
    const char *configured = engine_safe_getenv("ENGINE_CACHE_DIR", buf, sizeof(buf), missing);
    if (!configured) {
    ...
    int written = snprintf(buf, sizeof(buf), "%s/.cache/graph-engine", home);
