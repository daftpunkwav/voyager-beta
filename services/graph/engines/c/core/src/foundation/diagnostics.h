 ...
 *
 * When ENGINE_DIAGNOSTICS=1, writes a snapshot and retained trajectory below a
...
} engine_query_stats_t;

/* Singleton query stats — MCP server increments these. */
extern engine_query_stats_t g_query_stats;

/* Record a completed tool call. */
void engine_diag_record_query(long long duration_us, bool is_error);

/* Start the diagnostics writer thread (if ENGINE_DIAGNOSTICS env is set).
