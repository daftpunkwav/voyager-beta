...
engine_query_stats_t g_query_stats = {0};
static atomic_bool g_diag_stop = false;
static atomic_bool g_diag_done = false;
static engine_thread_t g_diag_thread;
static bool g_diag_started = false;
static bool g_diag_abandoned = false;
static time_t g_start_time = 0;
static engine_private_lock_directory_t *g_diag_directory = NULL;
#ifndef _WIN32
static int g_diag_directory_fd = -1;
#endif
static char g_diag_directory_path[ENGINE_PATH_MAX] = "";
static char g_diag_path[ENGINE_PATH_MAX] = "";
static char g_diag_ndjson_path[ENGINE_PATH_MAX] = "";
static size_t g_diag_ndjson_size = 0;

#ifdef ENGINE_DIAGNOSTICS_ENABLE_TEST_API
...
void engine_diag_record_query(long long duration_us, bool is_error) {
 ...
 * committed bytes cannot tell apart. Off unless ENGINE_MEM_STATS=1. */
...
#ifdef _WIN32
    /* _wopen mirrors engine_mkstemp's Windows contract — the ANSI CRT interprets
     * the UTF-8 bytes of a non-ASCII %TEMP% in the local codepage and fails. */
    wchar_t *wide = engine_path_to_wide(path);
    if (!wide) {
        return NULL;
    }
...
static void diag_write_allocator_stats(void) {
    char flag[ENGINE_SZ_16];
    if (engine_safe_getenv("ENGINE_MEM_STATS", flag, sizeof(flag), NULL) == NULL || flag[0] != '1') {
        return;
    }
    char path[ENGINE_PATH_MAX];
    ...
    int written =
        snprintf(path, sizeof(path), "%s/engine-allocator-stats-%d.txt", engine_tmpdir(), (int)getpid());
...
#endif
    engine_private_lock_directory_close(g_diag_directory);
    g_diag_directory = NULL;
    if (remove_directory && g_diag_directory_path[0] != '\0') {
        (void)engine_rmdir(g_diag_directory_path);
    }
}

/* Diagnostics placement honors the documented contract ($TMPDIR, /tmp
 * fallback) without touching engine_tmpdir(), whose many other callers keep
...
static const char *diag_tmp_base(char *buffer, size_t size) {
    ...
    (void)buffer;
    (void)size;
    return engine_tmpdir();
#else
    engine_safe_getenv("TMPDIR", buffer, size, NULL);
    if (buffer[0] != '\0') {
        return buffer;
    }
    return "/tmp";
...
static bool diag_directory_prepare(void) {
    char tmp_base[ENGINE_PATH_MAX];
    char directory_template[ENGINE_PATH_MAX];
    int written =
        snprintf(directory_template, sizeof(directory_template), "%s/engine-diagnostics-%d-XXXXXX",
                 diag_tmp_base(tmp_base, sizeof(tmp_base)), (int)getpid());
    if (written <= 0 || (size_t)written >= sizeof(directory_template) ||
        !engine_mkdtemp(directory_template)) {
        return false;
    }
    written =
    ...
    if (written <= 0 || (size_t)written >= sizeof(g_diag_directory_path)) {
        (void)engine_rmdir(directory_template);
        return false;
    }

#ifdef _WIN32
    wchar_t *wide_directory = engine_path_to_wide(g_diag_directory_path);
    HANDLE handle =
        wide_directory
    ...
    free(wide_directory);
    engine_private_file_lock_status_t status = ENGINE_PRIVATE_FILE_LOCK_IO;
    if (handle != INVALID_HANDLE_VALUE) {
        status = engine_private_lock_directory_adopt_windows(handle, g_diag_directory_path,
                                                          &g_diag_directory);
    ...
    engine_private_file_lock_status_t status = ENGINE_PRIVATE_FILE_LOCK_IO;
    if (same_directory) {
        status = engine_private_lock_directory_adopt_posix(validation_fd, g_diag_directory_path,
                                                        &g_diag_directory);
        if (status == ENGINE_PRIVATE_FILE_LOCK_OK) {
            validation_fd = -1;
    ...
    if (status != ENGINE_PRIVATE_FILE_LOCK_OK || !diag_set_output_paths()) {
...
static bool diag_write_file(const char *base_name, const char *data, size_t length, bool append) {
    ...
    wchar_t *wide_path = engine_path_to_wide(path);
...
static bool diag_posix_file_valid(int descriptor) {
    ...
    return descriptor >= 0 && fstat(descriptor, &state) == 0 && S_ISREG(state.st_mode) &&
           ...
           engine_macos_extended_acl_fd_is_empty(descriptor);
}
...
static void diag_wait_for_interval(void) {
    uint64_t deadline = engine_now_ms() + DIAG_INTERVAL_MS;
...
static void write_diagnostics(void) {
    ...
    (void)engine_mem_map_collect_os(&map);
    size_t residual =
        map.os_committed_bytes > map.live_bytes ? map.os_committed_bytes - map.live_bytes : 0;
    char buckets[ENGINE_SZ_512];
    int bucket_length = 0;
    for (int i = 0;
         i < ENGINE_MEM_MAP_BUCKETS && bucket_length >= 0 && (size_t)bucket_length < sizeof(buckets);
         i++) {
        int written =
            snprintf(buckets + bucket_length, sizeof(buckets) - (size_t)bucket_length,
                     "%s{\"limit\": %zu, \"bytes\": %zu, \"blocks\": %zu}", i == 0 ? "" : ", ",
                     engine_mem_map_bucket_limit(i), map.bucket_bytes[i], map.bucket_blocks[i]);
        if (written < 0) {
            break;
        }
...
bool engine_diag_start(void) {
    char env_buf[ENGINE_SZ_32] = "";
    engine_safe_getenv("ENGINE_DIAGNOSTICS", env_buf, sizeof(env_buf), NULL);
    ...
    if (engine_thread_create(&g_diag_thread, 0, diag_thread_fn, NULL) != 0) {
        diag_directory_close(true);
        return false;
    }

    g_diag_started = true;
    char interval[ENGINE_SZ_32];
    (void)snprintf(interval, sizeof(interval), "%d", DIAG_INTERVAL_MS / 1000);
    /* Discovery must survive ENGINE_LOG_LEVEL suppression and paths containing
     * spaces: a control record is the only announced copy of these paths. */
    engine_log_control("diagnostics.start", "snapshot", g_diag_path, "trajectory", g_diag_ndjson_path,
                    "interval_s", interval);
    return true;
}

void engine_diag_stop(void) {
    if (!g_diag_started) {
        return;
    }
    atomic_store_explicit(&g_diag_stop, true, memory_order_release);

    uint64_t deadline = engine_now_ms() + DIAG_STOP_TIMEOUT_MS;
    while (!atomic_load_explicit(&g_diag_done, memory_order_acquire) && engine_now_ms() < deadline) {
        diag_sleep_ms(DIAG_WAIT_SLICE_MS);
    }
    ...
    if (completed) {
        ...
        (void)engine_thread_detach(&g_diag_thread);
...
bool engine_diag_test_copy_paths(char *directory, size_t directory_size, char *snapshot,
