...
#include "compat.h" /* engine_nanosleep */
#include "compat_fs.h"
#include "log.h"
#include "platform.h" /* engine_now_ms */
...
#ifdef _WIN32
#include <windows.h>
#include "win_utf8.h" /* engine_utf8_to_wide — spawn the worker with a wide command line so a
...
#define ENGINE_WIN_CRASH_CODE_MIN 0xC0000000u
#define ENGINE_WIN_CONTROL_C_EXIT 0xC000013Au

#ifndef _WIN32
static bool engine_is_fault_signal(int sig) {
    switch (sig) {
    case SIGSEGV:
    case SIGBUS:
    case SIGILL:
...
engine_proc_outcome_t engine_proc_classify(bool exited_normally, int exit_code, int term_signal,
                                     bool timed_out) {
    if (timed_out) {
        return ENGINE_PROC_HANG;
    }
    if (!exited_normally) {
        /* POSIX signal death. */
#ifndef _WIN32
        if (engine_is_fault_signal(term_signal)) {
            return ENGINE_PROC_CRASH;
        }
#else
        (void)term_signal;
#endif
        return ENGINE_PROC_KILLED;
    }
    ...
    if ((unsigned)exit_code == ENGINE_WIN_CONTROL_C_EXIT) {
        return ENGINE_PROC_KILLED;
    }
    if ((unsigned)exit_code >= ENGINE_WIN_CRASH_CODE_MIN) {
        return ENGINE_PROC_CRASH;
    }
    return (exit_code == 0) ? ENGINE_PROC_CLEAN : ENGINE_PROC_EXIT_NONZERO;
}

const char *engine_proc_outcome_str(engine_proc_outcome_t o) {
    switch (o) {
    case ENGINE_PROC_CLEAN:
        return "clean";
    case ENGINE_PROC_EXIT_NONZERO:
        return "exit_nonzero";
    case ENGINE_PROC_CRASH:
        return "crash";
    case ENGINE_PROC_HANG:
        return "hang";
    case ENGINE_PROC_KILLED:
        return "killed";
    case ENGINE_PROC_SPAWN_FAILED:
    default:
        return "spawn_failed";
    }
}
...
typedef enum {
    ENGINE_TAIL_MORE = 0,
    ENGINE_TAIL_CAUGHT_UP,
    ENGINE_TAIL_ERROR,
} engine_tail_status_t;

typedef struct {
    engine_tail_status_t status;
    bool progressed;
} engine_tail_result_t;
...
static engine_tail_result_t engine_tail_log(const char *log_file, long *tail_pos, engine_proc_log_cb cb,
                                      void *ud, bool final) {
    engine_tail_result_t result = {.status = ENGINE_TAIL_ERROR, .progressed = false};
    if (!log_file || !tail_pos) {
        return result;
    }
#ifdef _WIN32
    FILE *lf = engine_fopen(log_file, "r");
#else
    ...
    if (fseek(lf, *tail_pos, SEEK_SET) == 0) {
        ...
        enum {
            ENGINE_TAIL_MAX_LINES_PER_POLL = 64,
            ENGINE_TAIL_MAX_BYTES_PER_POLL = 64 * 1024,
        };
        while (delivered_lines < ENGINE_TAIL_MAX_LINES_PER_POLL &&
               delivered_bytes < ENGINE_TAIL_MAX_BYTES_PER_POLL) {
            long before = ftell(lf);
            if (!fgets(line, sizeof(line), lf)) {
                result.status = ferror(lf) ? ENGINE_TAIL_ERROR : ENGINE_TAIL_CAUGHT_UP;
                break;
            }
            ...
            if (complete) {
                ...
                *tail_pos = before;
                result.status = ENGINE_TAIL_MORE;
                break;
            } else {
                ...
                *tail_pos = ftell(lf);
                ...
                result.status = ENGINE_TAIL_CAUGHT_UP;
                break;
            }
        }
        if (result.status == ENGINE_TAIL_ERROR && !ferror(lf)) {
    ...
    if (fclose(lf) != 0) {
        result.status = ENGINE_TAIL_ERROR;
    }
    return result;
}
...
static size_t engine_cmdline_put(char *buf, size_t cap, size_t pos, char c, bool *ovf) {
    if (pos + 1 >= cap) {
        *ovf = true;
        return pos;
    }
...
static size_t engine_cmdline_append_arg(char *buf, size_t cap, size_t pos, const char *arg, bool first,
                                     bool *ovf) {
    if (!first) {
        pos = engine_cmdline_put(buf, cap, pos, ' ', ovf);
    }
    pos = engine_cmdline_put(buf, cap, pos, '"', ovf);
    for (const char *p = arg; *p;) {
        ...
        if (*p == '"') {
            ...
            pos = engine_cmdline_put(buf, cap, pos, *p, ovf);
            p++;
        }
    }
    pos = engine_cmdline_put(buf, cap, pos, '"', ovf);
 ...
 * only runs on Windows. Shared by engine_subprocess_spawn_win AND the UI http_server
...
bool engine_build_win_cmdline(char *buf, size_t cap, const char *const *argv) {
    if (!buf || cap == 0 || !argv) {
        return false;
    }
    size_t pos = 0;
    ...
    for (int i = 0; argv[i]; i++) {
        pos = engine_cmdline_append_arg(buf, cap, pos, argv[i], i == 0, &ovf);
        if (ovf) {
...
static bool engine_ascii_case_equal(const char *left, const char *right) {
    if (!left || !right) {
        return false;
    }
    while (*left && *right) {
...
static bool engine_win_cmd_path_is_absolute(const char *path) {
    if (!path) {
        return false;
    }
    ...
    return engine_ascii_case_equal(basename, "cmd.exe");
}

bool engine_build_win_cmd_payload(char *buf, size_t cap, const char *cmd_executable,
                               const char *payload) {
    if (!buf || cap == 0) {
        return false;
    }
    buf[0] = '\0';
    if (!engine_win_cmd_path_is_absolute(cmd_executable) || !payload || !payload[0]) {
        return false;
    }
...
enum { ENGINE_SUBPROCESS_ARGV_LIMIT = 4096 };

typedef enum {
    ENGINE_SUBPROCESS_ACTIVE = 0,
    ENGINE_SUBPROCESS_CANCEL_REQUESTED,
    ENGINE_SUBPROCESS_DRAINING,
    ENGINE_SUBPROCESS_TERMINAL,
} engine_subprocess_lifecycle_t;

struct engine_subprocess {
    char *bin;
    char **argv;
    size_t argc;
    char *windows_cmd_payload;
    char *log_file;
    engine_proc_log_cb on_log_line;
    void *log_ud;
...
static void engine_subprocess_result_init(engine_proc_result_t *result) {
    result->outcome = ENGINE_PROC_SPAWN_FAILED;
    result->exit_code = -1;
...
static void engine_subprocess_free_config(engine_subprocess_t *process) {
    if (!process) {
        return;
    }
    if (process->argv) {
...
static engine_subprocess_t *engine_subprocess_copy_opts(const engine_proc_opts_t *opts) {
    if (!opts || !opts->bin || !opts->bin[0]) {
        return NULL;
    }
#ifdef _WIN32
    if (opts->windows_cmd_payload &&
        (!opts->windows_cmd_payload[0] || opts->argv || !engine_win_cmd_path_is_absolute(opts->bin))) {
        return NULL;
    }
#else
    if (opts->windows_cmd_payload) {
    ...
    if (opts->argv) {
        argc = 0;
        while (argc < ENGINE_SUBPROCESS_ARGV_LIMIT && opts->argv[argc]) {
            argc++;
        }
        if (argc == 0 || argc == ENGINE_SUBPROCESS_ARGV_LIMIT) {
            return NULL;
        }
    }

    engine_subprocess_t *process = (engine_subprocess_t *)calloc(1, sizeof(*process));
    if (!process) {
        return NULL;
    }
    process->bin = engine_strdup(opts->bin);
    process->windows_cmd_payload =
        opts->windows_cmd_payload ? engine_strdup(opts->windows_cmd_payload) : NULL;
    ...
    if (!process->bin || !process->argv ||
        ...
        engine_subprocess_free_config(process);
        return NULL;
    }
    for (size_t i = 0; i < argc; i++) {
        const char *arg = opts->argv ? opts->argv[i] : opts->bin;
        process->argv[i] = engine_strdup(arg);
        if (!process->argv[i]) {
            engine_subprocess_free_config(process);
            return NULL;
        }
    ...
    if (opts->log_file) {
        process->log_file = engine_strdup(opts->log_file);
        if (!process->log_file) {
            engine_subprocess_free_config(process);
            return NULL;
        }
    ...
    process->cancel_grace_ms =
        opts->cancel_grace_ms > 0 ? opts->cancel_grace_ms : ENGINE_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS;
    if (process->cancel_grace_ms > ENGINE_SUBPROCESS_MAX_CANCEL_GRACE_MS) {
        process->cancel_grace_ms = ENGINE_SUBPROCESS_MAX_CANCEL_GRACE_MS;
    }
    process->delete_log_on_exit = opts->delete_log_on_exit;
    atomic_init(&process->lifecycle, ENGINE_SUBPROCESS_ACTIVE);
    engine_subprocess_result_init(&process->result);
    return process;
}

static engine_tail_result_t engine_subprocess_poll_log(engine_subprocess_t *process, bool final) {
    engine_tail_result_t result = engine_tail_log(process->log_file, &process->tail_pos,
                                            process->on_log_line, process->log_ud, final);
    if (result.progressed) {
        process->last_activity_ms = engine_now_ms();
    }
    return result;
}

static bool engine_subprocess_cancellation_requested(const engine_subprocess_t *process) {
    return atomic_load_explicit(&process->lifecycle, memory_order_acquire) ==
           ENGINE_SUBPROCESS_CANCEL_REQUESTED;
}

static void engine_subprocess_delete_log(engine_subprocess_t *process) {
    if (!process->log_file || !process->delete_log_on_exit) {
        return;
    }
#ifdef _WIN32
    wchar_t *path = engine_path_to_wide(process->log_file);
    if (path) {
...
static bool engine_subprocess_begin_terminal_transition(engine_subprocess_t *process) {
    int lifecycle = atomic_load_explicit(&process->lifecycle, memory_order_acquire);
    for (;;) {
        if (lifecycle == ENGINE_SUBPROCESS_DRAINING) {
            return true;
        }
        if (lifecycle == ENGINE_SUBPROCESS_TERMINAL) {
            return false;
        }
        int desired = ENGINE_SUBPROCESS_DRAINING;
        if (atomic_compare_exchange_weak_explicit(&process->lifecycle, &lifecycle, desired,
                                                  memory_order_acq_rel, memory_order_acquire)) {
            process->result.cancellation_requested = lifecycle == ENGINE_SUBPROCESS_CANCEL_REQUESTED;
            return true;
        }
    }
}

static engine_proc_poll_t engine_subprocess_publish_terminal(engine_subprocess_t *process,
                                                       engine_proc_result_t *out, bool delete_log) {
    if (delete_log) {
        engine_subprocess_delete_log(process);
    }
    atomic_store_explicit(&process->lifecycle, ENGINE_SUBPROCESS_TERMINAL, memory_order_release);
    if (out) {
        *out = process->result;
    }
    return ENGINE_PROC_POLL_TERMINAL;
}

static engine_proc_poll_t engine_subprocess_finish(engine_subprocess_t *process, engine_proc_result_t *out) {
    if (!engine_subprocess_begin_terminal_transition(process)) {
        return ENGINE_PROC_POLL_ERROR;
    }
    process->result.tree_quiesced = true;
    process->result.supervision_failed = false;
    if (process->log_file && process->on_log_line) {
        return ENGINE_PROC_POLL_RUNNING;
    }
    return engine_subprocess_publish_terminal(process, out, true);
}

static engine_proc_poll_t engine_subprocess_finish_failed(engine_subprocess_t *process,
                                                    engine_proc_result_t *out) {
    if (!engine_subprocess_begin_terminal_transition(process)) {
        return ENGINE_PROC_POLL_ERROR;
    }
    ...
    if (process->timed_out) {
        process->result.outcome = ENGINE_PROC_HANG;
    } else if (process->result.outcome == ENGINE_PROC_SPAWN_FAILED) {
        process->result.outcome = ENGINE_PROC_KILLED;
    }
    process->containment_failed = true;
    return engine_subprocess_publish_terminal(process, out, false);
}

#ifdef _WIN32

static void engine_win_close_spawn_handles(HANDLE nul, HANDLE log, LPPROC_THREAD_ATTRIBUTE_LIST attrs,
                                        bool attrs_init) {
    if (attrs) {
        if (attrs_init) {
...
static int engine_subprocess_spawn_win(engine_subprocess_t *process) {
    char cmdline[8192];
    bool built =
        process->windows_cmd_payload
            ? engine_build_win_cmd_payload(cmdline, sizeof(cmdline), process->bin,
                                        process->windows_cmd_payload)
            : engine_build_win_cmdline(cmdline, sizeof(cmdline), (const char *const *)process->argv);
    if (!built) {
        return -1;
    }
    wchar_t *wbin = engine_utf8_to_wide(process->bin);
    wchar_t *wcmdline = engine_utf8_to_wide(cmdline);
    if (!wbin || !wcmdline) {
        free(wbin);
        free(wcmdline);
        return -1;
    ...
    if (process->log_file) {
        wchar_t *wlog = engine_path_to_wide(process->log_file);
        if (wlog) {
    ...
    if (!attrs_ready) {
        engine_win_close_spawn_handles(nul, log, attrs, attrs_init);
        CloseHandle(job);
        free(wbin);
        free(wcmdline);
        return -1;
...
static bool engine_win_job_active(engine_subprocess_t *process, bool *known) {
...
static void engine_win_begin_termination(engine_subprocess_t *process, uint64_t now) {
...
static void engine_win_force_tree(engine_subprocess_t *process, uint64_t now) {
...
static void engine_win_capture_root(engine_subprocess_t *process, DWORD code) {
    ...
    if (process->timed_out) {
        process->result.outcome = ENGINE_PROC_HANG;
               ...
               (engine_subprocess_cancellation_requested(process) && code == ENGINE_WIN_CONTROL_C_EXIT)) {
        process->result.outcome = ENGINE_PROC_KILLED;
    } else {
        process->result.outcome = engine_proc_classify(true, (int)code, 0, false);
    }
}

static engine_proc_poll_t engine_subprocess_poll_win(engine_subprocess_t *process, engine_proc_result_t *out) {
    ...
    if (!process->root_reaped) {
        ...
        if (waited == WAIT_OBJECT_0) {
            ...
            engine_win_capture_root(process, code);
        } else if (waited == WAIT_FAILED) {
            DWORD code = STILL_ACTIVE;
            if (GetExitCodeProcess(process->process, &code) && code != STILL_ACTIVE) {
                engine_win_capture_root(process, code);
            } else {
                engine_win_begin_termination(process, now);
                engine_win_force_tree(process, now);
            }
        }
    }

    bool job_known = false;
    bool job_active = engine_win_job_active(process, &job_known);
    if (!process->termination_started) {
        if (engine_subprocess_cancellation_requested(process)) {
            engine_win_begin_termination(process, now);
    ...
    if (process->force_started_ms != 0 &&
        now - process->force_started_ms >= ENGINE_SUBPROCESS_FORCE_SETTLE_MS &&
        (!job_known || job_active || !process->root_reaped)) {
        return engine_subprocess_finish_failed(process, out);
    }
    if (process->root_reaped && !job_active) {
        return engine_subprocess_finish(process, out);
    }
    return ENGINE_PROC_POLL_RUNNING;
}
...
static void engine_posix_reset_child_signals(void) {
 ...
 * fallback (see engine_posix_spawn_apple), which preserves the documented
 * "bogus binary => child exits 127" contract across platforms. */
static void engine_posix_child_exec(engine_subprocess_t *process, int input, int output, long max_fd) {
    if (setpgid(0, 0) < 0) {
        _exit(127);
    }
    engine_posix_reset_child_signals();
...
static int engine_posix_fd_at_least_three(int fd) {
...
static int engine_posix_spawn_apple(engine_subprocess_t *process, int input, int output, pid_t *pid_out) {
...
static int engine_subprocess_spawn_posix(engine_subprocess_t *process) {
    int input_flags = O_RDONLY;
#ifdef O_CLOEXEC
    input_flags |= O_CLOEXEC;
#endif
    int input = engine_posix_fd_at_least_three(open("/dev/null", input_flags));
...
#endif
    int output = engine_posix_fd_at_least_three(open(target, output_flags, 0600));
    if (input < 0 || output < 0) {
        if (input >= 0) {
            (void)close(input);
        }
    ...
    pid_t pid = -1;
#ifdef __APPLE__
    int spawn_rc = engine_posix_spawn_apple(process, input, output, &pid);
    if (spawn_rc < 0) {
        (void)close(input);
        (void)close(output);
        return -1;
    ...
    if (spawn_rc > 0) { /* exec-class failure: reproduce the fork+exec 127 */
        ...
        if (pid == 0) {
            engine_posix_child_exec(process, input, output, max_fd);
        }
    }
#else
    pid = fork();
...
static bool engine_posix_group_active(engine_subprocess_t *process) {
...
static void engine_posix_begin_termination(engine_subprocess_t *process, uint64_t now) {
    if (process->termination_started) {
        return;
    }
    process->termination_started = true;
...
static void engine_posix_force_tree(engine_subprocess_t *process, uint64_t now) {
    if (process->force_sent) {
        return;
    }
    if (process->force_started_ms == 0) {
...
static void engine_posix_capture_root(engine_subprocess_t *process, int status) {
    process->root_reaped = true;
    if (WIFEXITED(status)) {
        process->result.exit_code = WEXITSTATUS(status);
        process->result.term_signal = 0;
        process->result.outcome =
            engine_proc_classify(true, process->result.exit_code, 0, process->timed_out);
    } else if (WIFSIGNALED(status)) {
        process->result.exit_code = -1;
        process->result.term_signal = WTERMSIG(status);
        process->result.outcome =
            engine_proc_classify(false, -1, process->result.term_signal, process->timed_out);
    } else {
        process->result.exit_code = -1;
        process->result.term_signal = 0;
        process->result.outcome = process->timed_out ? ENGINE_PROC_HANG : ENGINE_PROC_KILLED;
    }
}

static engine_proc_poll_t engine_subprocess_poll_posix(engine_subprocess_t *process,
                                                 engine_proc_result_t *out) {
    uint64_t now = engine_now_ms();
    ...
    if (!process->root_reaped) {
        ...
        if (waited == process->pid) {
            engine_posix_capture_root(process, status);
            ...
            process->result.exit_code = -1;
            process->result.term_signal = 0;
            engine_posix_begin_termination(process, now);
        }
    }

    bool group_active = engine_posix_group_active(process);
    if (!process->termination_started) {
        if (engine_subprocess_cancellation_requested(process)) {
            engine_posix_begin_termination(process, now);
        } else if (!process->root_reaped && process->quiet_timeout_ms > 0 &&
    ...
    if (process->termination_started && group_active && !process->force_sent &&
        now - process->termination_started_ms >= (uint64_t)process->cancel_grace_ms) {
        engine_posix_force_tree(process, now);
    }
    group_active = engine_posix_group_active(process);
...
int engine_subprocess_spawn(const engine_proc_opts_t *opts, engine_subprocess_t **out) {
    if (!out) {
        return -1;
    }
    *out = NULL;
    engine_subprocess_t *process = engine_subprocess_copy_opts(opts);
    if (!process) {
        return -1;
    }
#ifdef _WIN32
    int spawn_rc = engine_subprocess_spawn_win(process);
#else
    int spawn_rc = engine_subprocess_spawn_posix(process);
#endif
    if (spawn_rc != 0) {
        engine_subprocess_free_config(process);
        return -1;
...
engine_proc_poll_t engine_subprocess_poll(engine_subprocess_t *process, engine_proc_result_t *out) {
    if (!process) {
        return ENGINE_PROC_POLL_ERROR;
    }
    int lifecycle = atomic_load_explicit(&process->lifecycle, memory_order_acquire);
    ...
    if (lifecycle == ENGINE_SUBPROCESS_DRAINING) {
        engine_tail_result_t tail = engine_subprocess_poll_log(process, true);
        if (tail.status == ENGINE_TAIL_MORE) {
            return ENGINE_PROC_POLL_RUNNING;
        }
        if (tail.status == ENGINE_TAIL_ERROR) {
            engine_log_error("subprocess.log_drain_failed", "reason", "io_error");
            return engine_subprocess_publish_terminal(process, out, false);
        }
        return engine_subprocess_publish_terminal(process, out, true);
    }
    ...
    (void)engine_subprocess_poll_log(process, false);
#ifdef _WIN32
    return engine_subprocess_poll_win(process, out);
#else
    return engine_subprocess_poll_posix(process, out);
#endif
}

bool engine_subprocess_request_cancel(engine_subprocess_t *process) {
    if (!process) {
        return false;
    }
    int lifecycle = atomic_load_explicit(&process->lifecycle, memory_order_acquire);
    for (;;) {
        if (lifecycle == ENGINE_SUBPROCESS_CANCEL_REQUESTED) {
            return true;
        }
        if (lifecycle != ENGINE_SUBPROCESS_ACTIVE) {
            return false;
        }
        int desired = ENGINE_SUBPROCESS_CANCEL_REQUESTED;
        if (atomic_compare_exchange_weak_explicit(&process->lifecycle, &lifecycle, desired,
                                                  memory_order_acq_rel, memory_order_acquire)) {
            return true;
        }
...
void engine_subprocess_destroy(engine_subprocess_t *process) {
...
int engine_subprocess_run(const engine_proc_opts_t *opts, engine_proc_result_t *out) {
    engine_proc_result_t local;
    if (!out) {
        out = &local;
    }
    engine_subprocess_result_init(out);

    engine_subprocess_t *process = NULL;
    if (engine_subprocess_spawn(opts, &process) != 0) {
        return -1;
    }
    for (;;) {
        engine_proc_poll_t state = engine_subprocess_poll(process, out);
        if (state == ENGINE_PROC_POLL_TERMINAL) {
        ...
        (void)engine_nanosleep(&delay, NULL);
    }
}
