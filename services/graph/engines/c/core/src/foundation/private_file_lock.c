...
struct engine_private_lock_directory {
    int fd;
    char *path;
    dev_t device;
    ino_t inode;
...
struct engine_private_file_lock {
    int fd;
    pid_t owner_pid;
    engine_private_file_lock_mode_t mode;
    struct engine_private_file_lock *next_tracked;
    bool unlocked;
...
struct engine_private_fork_condition {
...
static engine_private_file_lock_t *private_tracked_locks;
...
static void private_atfork_child(void) {
    engine_private_file_lock_t *lock = private_tracked_locks;
    while (lock) {
        if (lock->fd >= 0) {
...
bool engine_private_file_lock_fork_guard_enter(void) {
    return private_atfork_ensure() && pthread_mutex_lock(&private_fork_mutex) == 0;
}

void engine_private_file_lock_fork_guard_leave(void) {
    (void)pthread_mutex_unlock(&private_fork_mutex);
}

engine_private_fork_condition_t *engine_private_fork_condition_new(void) {
    engine_private_fork_condition_t *condition = calloc(1, sizeof(*condition));
...
void engine_private_fork_condition_free(engine_private_fork_condition_t *condition) {
    if (!condition) {
        return;
    }
...
void engine_private_fork_condition_broadcast_while_guarded(engine_private_fork_condition_t *condition) {
    if (condition) {
        (void)pthread_cond_broadcast(&condition->value);
    }
}

engine_private_fork_wait_status_t engine_private_fork_condition_wait_until_while_guarded(
    engine_private_fork_condition_t *condition, uint64_t deadline_ms) {
    if (!condition) {
        return ENGINE_PRIVATE_FORK_WAIT_ERROR;
    }
    int status;
    ...
    if (status == 0) {
        return ENGINE_PRIVATE_FORK_WAIT_SIGNALED;
    }
    return status == ETIMEDOUT ? ENGINE_PRIVATE_FORK_WAIT_TIMEOUT : ENGINE_PRIVATE_FORK_WAIT_ERROR;
}
...
static bool private_directory_revalidate(const engine_private_lock_directory_t *directory) {
    ...
    return directory && directory->owner_pid == getpid() && directory->fd >= 0 &&
           ...
           engine_macos_extended_acl_fd_is_empty(directory->fd);
}
...
static bool private_file_revalidate(const engine_private_lock_directory_t *directory,
    ...
    return private_directory_revalidate(directory) && fstat(fd, &by_handle) == 0 &&
           ...
           by_path.st_ino == by_handle.st_ino && engine_macos_extended_acl_fd_is_empty(fd) &&
           (!expected ||
            (expected->st_dev == by_handle.st_dev && expected->st_ino == by_handle.st_ino));
}

static engine_private_file_lock_status_t private_open_failure_status(
    const engine_private_lock_directory_t *directory, const char *base_name) {
    struct stat status;
    if (fstatat(directory->fd, base_name, &status, AT_SYMLINK_NOFOLLOW) == 0) {
        return ENGINE_PRIVATE_FILE_LOCK_UNSAFE;
    }
    return errno == ENOENT ? ENGINE_PRIVATE_FILE_LOCK_IO : ENGINE_PRIVATE_FILE_LOCK_UNSAFE;
}
...
static int private_release_unlock(engine_private_file_lock_t *lock) {
...
static int private_release_close(engine_private_file_lock_t *lock, bool *attempted_out) {
    *attempted_out = false;
...
engine_private_file_lock_status_t engine_private_lock_directory_adopt_posix(
    int directory_fd, const char *stable_path, engine_private_lock_directory_t **directory_out) {
    if (directory_out) {
        *directory_out = NULL;
    }
    ...
    if (fstat(directory_fd, &status) != 0 || lstat(stable_path, &by_path) != 0 ||
        ...
        status.st_ino != by_path.st_ino || !engine_macos_extended_acl_fd_is_empty(directory_fd)) {
        return ENGINE_PRIVATE_FILE_LOCK_UNSAFE;
    }
    engine_private_lock_directory_t *directory = calloc(1, sizeof(*directory));
    if (directory) {
...
engine_private_file_lock_status_t engine_private_file_lock_try_acquire(
    ...
    if (!directory || !lock_out || !private_base_name_valid(base_name) ||
        (mode != ENGINE_PRIVATE_FILE_LOCK_SH && mode != ENGINE_PRIVATE_FILE_LOCK_EX)) {
        return ENGINE_PRIVATE_FILE_LOCK_UNSAFE;
    }
    ...
    if (fd < 0) {
        engine_private_file_lock_status_t status = private_open_failure_status(directory, base_name);
    ...
    engine_private_file_lock_t *lock = calloc(1, sizeof(*lock));
    if (!lock) {
        (void)close(fd);
        engine_private_file_lock_fork_guard_leave();
        return ENGINE_PRIVATE_FILE_LOCK_IO;
    ...
    int operation = mode == ENGINE_PRIVATE_FILE_LOCK_SH ? LOCK_SH : LOCK_EX;
    if (private_flock_set(fd, operation | LOCK_NB) != 0) {
        int lock_error = errno;
        (void)close(fd);
        free(lock);
        engine_private_file_lock_fork_guard_leave();
        return lock_error == EWOULDBLOCK || lock_error == EAGAIN ? ENGINE_PRIVATE_FILE_LOCK_BUSY
                                                                 : ENGINE_PRIVATE_FILE_LOCK_IO;
    }
    ...
    if (forced_cleanup || !private_file_revalidate(directory, base_name, fd, &initial)) {
        engine_private_file_lock_status_t failure_status =
            forced_cleanup ? ENGINE_PRIVATE_FILE_LOCK_IO : ENGINE_PRIVATE_FILE_LOCK_UNSAFE;
        lock->test_fail_unlock_once = fail_cleanup_unlock;
        lock->test_fail_close_once = fail_cleanup_close;
        *lock_out = lock;
        engine_private_file_lock_fork_guard_leave();
        engine_private_file_lock_status_t cleanup_status = engine_private_file_lock_release(lock_out);
        return cleanup_status == ENGINE_PRIVATE_FILE_LOCK_OK ? failure_status
                                                          : ENGINE_PRIVATE_FILE_LOCK_IO;
    }
    *lock_out = lock;
    engine_private_file_lock_fork_guard_leave();
...
static bool private_lock_is_tracked(const engine_private_file_lock_t *lock) {
    for (const engine_private_file_lock_t *cursor = private_tracked_locks; cursor;
...
static bool private_payload_fd_valid(const engine_private_file_lock_t *lock, struct stat *status_out) {
    struct stat status;
    bool valid = lock && lock->fd >= 0 && !lock->unlocked && lock->owner_pid == getpid() &&
                 ...
                 engine_macos_extended_acl_fd_is_empty(lock->fd);
    if (valid && status_out) {
        *status_out = status;
    }
    return valid;
...
engine_private_file_lock_status_t engine_private_file_lock_payload_read(engine_private_file_lock_t *lock,
...
engine_private_file_lock_status_t engine_private_file_lock_payload_write(engine_private_file_lock_t *lock,
                                                                   const void *buffer,
                                                                   size_t length) {
    if (!lock || !buffer || length == 0 || length > PRIVATE_FILE_LOCK_PAYLOAD_CAP ||
        lock->mode != ENGINE_PRIVATE_FILE_LOCK_EX) {
        return ENGINE_PRIVATE_FILE_LOCK_UNSAFE;
    }
    if (!engine_private_file_lock_fork_guard_enter()) {
        return ENGINE_PRIVATE_FILE_LOCK_IO;
    ...
    return valid ? ENGINE_PRIVATE_FILE_LOCK_OK : ENGINE_PRIVATE_FILE_LOCK_IO;
}
...
engine_private_file_lock_status_t engine_private_file_lock_release(engine_private_file_lock_t **lock_io) {
    ...
    engine_private_file_lock_t *lock = *lock_io;
    ...
    engine_private_file_lock_t **cursor = &private_tracked_locks;
    ...
    *lock_io = NULL;
    free(lock);
    return close_status == 0 ? ENGINE_PRIVATE_FILE_LOCK_OK : ENGINE_PRIVATE_FILE_LOCK_IO;
}
...
bool engine_private_file_lock_is_cloexec_for_test(const engine_private_file_lock_t *lock) {
    if (!lock || lock->fd < 0) {
        return false;
    }
...
bool engine_private_file_lock_unlock_complete(const engine_private_file_lock_t *lock) {
    return lock && lock->unlocked;
}

bool engine_private_lock_directory_fail_post_acquire_cleanup_for_test(
    engine_private_lock_directory_t *directory, bool fail_unlock, bool fail_close) {
...
bool engine_private_file_lock_fail_next_release_step_for_test(
    engine_private_file_lock_t *lock, engine_private_file_lock_release_step_t step) {
    if (!lock) {
        return false;
    }
    if (step == ENGINE_PRIVATE_FILE_LOCK_RELEASE_UNLOCK) {
        lock->test_fail_unlock_once = true;
        return true;
    }
    if (step == ENGINE_PRIVATE_FILE_LOCK_RELEASE_CLOSE) {
...
unsigned int engine_private_file_lock_release_step_attempts_for_test(
    const engine_private_file_lock_t *lock, engine_private_file_lock_release_step_t step) {
    if (!lock) {
        return 0;
    }
    if (step == ENGINE_PRIVATE_FILE_LOCK_RELEASE_UNLOCK) {
...
bool engine_private_file_lock_fail_close_after_consuming_for_test(engine_private_file_lock_t *lock) {
    if (!lock) {
        return false;
    }
...
int engine_private_file_lock_native_fd_for_test(const engine_private_file_lock_t *lock) {
...
static engine_private_file_lock_status_t private_win_path_from_utf8(const char *path,
                                                                 wchar_t **wide_out) {
    *wide_out = NULL;
...
static bool private_win_directory_handle_valid(engine_private_lock_directory_t *directory,
...
static bool private_win_directory_path_matches(engine_private_lock_directory_t *directory) {
    HANDLE probe =
...
static bool private_win_directory_revalidate(engine_private_lock_directory_t *directory) {
    BY_HANDLE_FILE_INFORMATION information;
...
static wchar_t *private_win_file_path(const engine_private_lock_directory_t *directory,
...
static bool private_win_file_handle_valid(engine_private_lock_directory_t *directory, HANDLE handle,
                                          BY_HANDLE_FILE_INFORMATION *information_out) {
    BY_HANDLE_FILE_INFORMATION information;
...
static bool private_win_file_revalidate(engine_private_lock_directory_t *directory,
...
static engine_private_file_lock_status_t private_win_open_failure_status(const wchar_t *path) {
    DWORD attributes = GetFileAttributesW(path);
    return attributes == INVALID_FILE_ATTRIBUTES ? ENGINE_PRIVATE_FILE_LOCK_IO
                                                 : ENGINE_PRIVATE_FILE_LOCK_UNSAFE;
}

engine_private_file_lock_status_t engine_private_lock_directory_adopt_windows(
    void *directory_handle, const char *stable_path, engine_private_lock_directory_t **directory_out) {
    if (directory_out) {
        *directory_out = NULL;
    }
    ...
    engine_private_file_lock_status_t path_status =
        private_win_path_from_utf8(stable_path, &wide_path);
    if (path_status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        return path_status;
    }
    private_win_security_t security;
    ...
    engine_private_lock_directory_t candidate;
...
engine_private_file_lock_status_t engine_private_file_lock_try_acquire(
    ...
    if (handle == INVALID_HANDLE_VALUE) {
        engine_private_file_lock_status_t status = private_win_open_failure_status(path);
        engine_private_file_lock_fork_guard_leave();
        free(path);
        return status;
    }
    ...
    if (mode == ENGINE_PRIVATE_FILE_LOCK_EX) {
    ...
    if (!native_locked) {
        ...
        *lock_out = lock;
        engine_private_file_lock_fork_guard_leave();
        free(path);
        engine_private_file_lock_status_t failure_status = lock_error == ERROR_LOCK_VIOLATION
                                                            ? ENGINE_PRIVATE_FILE_LOCK_BUSY
                                                            : ENGINE_PRIVATE_FILE_LOCK_IO;
        engine_private_file_lock_status_t cleanup_status = engine_private_file_lock_release(lock_out);
        return cleanup_status == ENGINE_PRIVATE_FILE_LOCK_OK ? failure_status
                                                          : ENGINE_PRIVATE_FILE_LOCK_IO;
...
static bool private_win_payload_handle_valid(const engine_private_file_lock_t *lock) {
    BY_HANDLE_FILE_INFORMATION information;
...
static bool private_win_release_unlock(engine_private_file_lock_t *lock) {
    lock->test_unlock_attempts++;
    if (lock->test_fail_unlock_once) {
        lock->test_fail_unlock_once = false;
...
static bool private_win_release_close(engine_private_file_lock_t *lock) {
    lock->test_close_attempts++;
    if (lock->test_fail_close_once) {
        lock->test_fail_close_once = false;
        SetLastError(ERROR_GEN_FAILURE);
...
bool engine_private_lock_directory_fail_lock_attempt_cleanup_for_test(
...
engine_private_fork_wait_status_t engine_private_fork_condition_wait_until_while_guarded(
    ...
    return GetLastError() == ERROR_TIMEOUT ? ENGINE_PRIVATE_FORK_WAIT_TIMEOUT
                                           : ENGINE_PRIVATE_FORK_WAIT_ERROR;
}
