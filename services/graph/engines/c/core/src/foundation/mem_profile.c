...
static engine_mutex_t g_profile_mutex;
static profile_site_t g_sites[PROFILE_SITES];
static profile_pointer_t g_pointers[PROFILE_POINTERS];
static engine_mem_profile_totals_t g_totals;
static atomic_int g_profile_enabled = -1;
static size_t g_profile_min = PROFILE_DEFAULT_MIN;

size_t engine_mem_profile_threshold(void) {
    (void)engine_mem_profile_enabled();
    return g_profile_min;
}

bool engine_mem_profile_enabled(void) {
    int state = atomic_load_explicit(&g_profile_enabled, memory_order_acquire);
    if (state >= 0) {
        return state == 1;
    }
    char buf[ENGINE_SZ_32];
    bool on = engine_safe_getenv("ENGINE_MEM_PROFILE", buf, sizeof(buf), NULL) != NULL && buf[0] == '1';
    if (on) {
        char min_buf[ENGINE_SZ_32];
        if (engine_safe_getenv("ENGINE_MEM_PROFILE_MIN", min_buf, sizeof(min_buf), NULL) != NULL) {
            long parsed = strtol(min_buf, NULL, 10);
            if (parsed > 0) {
                g_profile_min = (size_t)parsed;
            }
        }
        engine_mutex_init(&g_profile_mutex);
    }
...
void engine_mem_profile_alloc(void *block, size_t size) {
    engine_mem_profile_alloc_at(block, size, NULL);
}

void engine_mem_profile_alloc_at(void *block, size_t size, void *caller) {
    if (!block || g_profile_reentrant) {
        return;
    }
    g_profile_reentrant = true;
    if (!engine_mem_profile_enabled() || size < g_profile_min) {
    ...
    if (count <= 0) {
        engine_mutex_lock(&g_profile_mutex);
        g_totals.capture_failed++;
        engine_mutex_unlock(&g_profile_mutex);
        g_profile_reentrant = false;
        return;
    }
    engine_mutex_lock(&g_profile_mutex);
...
void engine_mem_profile_free(void *block) {
    if (!block || g_profile_reentrant) {
        return;
    }
    g_profile_reentrant = true;
    if (!engine_mem_profile_enabled()) {
        g_profile_reentrant = false;
        return;
    }
    engine_mutex_lock(&g_profile_mutex);
...
void engine_mem_profile_totals(engine_mem_profile_totals_t *out) {
    if (!out) {
        return;
    }
...
bool engine_mem_profile_dump(const char *path, const char *label) {
    if (!path || !engine_mem_profile_enabled()) {
        return false;
    }
    FILE *out = engine_fopen(path, "ab");
    if (!out) {
        return false;
    }
    engine_mutex_lock(&g_profile_mutex);
