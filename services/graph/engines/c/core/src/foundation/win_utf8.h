#ifndef ENGINE_WIN_UTF8_H
#define ENGINE_WIN_UTF8_H
...
static inline wchar_t *engine_utf8_to_wide(const char *utf8) {
    if (!utf8) {
        return NULL;
    }
...
static inline int engine_win_path_separator(wchar_t value) {
    return value == L'\\' || value == L'/';
}

static inline int engine_win_path_has_namespace_prefix(const wchar_t *path, size_t length) {
    return length >= 4U && engine_win_path_separator(path[0]) && engine_win_path_separator(path[1]) &&
           (path[2] == L'?' || path[2] == L'.') && engine_win_path_separator(path[3]);
}

static inline void engine_win_path_normalize_wide_separators(wchar_t *path) {
    if (!path) {
        return;
    }
 ...
 * registry paths). */
static inline wchar_t *engine_wide_path_to_extended(const wchar_t *wide_path) {
    if (!wide_path) {
        return NULL;
    }
    ...
    if (engine_win_path_has_namespace_prefix(wide, length)) {
        engine_win_path_normalize_wide_separators(wide);
        return wide;
    }
    if (length < 240U) {
        return wide;
    ...
    engine_win_path_normalize_wide_separators(full);
...
static inline wchar_t *engine_path_to_wide(const char *utf8_path) {
    wchar_t *wide = engine_utf8_to_wide(utf8_path);
    wchar_t *extended = engine_wide_path_to_extended(wide);
    free(wide);
    return extended;
}
...
static inline void engine_module_path_unprefix(wchar_t *wide) {
    if (!wide) {
        return;
    }
...
static inline char *engine_wide_to_utf8(const wchar_t *wide) {
    if (!wide) {
        return NULL;
    }
...
static inline char *engine_module_path_utf8(void) {
    DWORD capacity = 512U;
    while (capacity <= 32768U) {
        ...
        if (length > 0U && length < capacity) {
            engine_module_path_unprefix(wide);
            char *utf8 = engine_wide_to_utf8(wide);
            free(wide);
            return utf8;
        }
