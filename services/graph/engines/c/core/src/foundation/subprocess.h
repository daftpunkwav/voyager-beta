 ...
 */
#ifndef ENGINE_SUBPROCESS_H
#define ENGINE_SUBPROCESS_H

#include <stdbool.h>
#include <stddef.h> /* size_t (engine_build_win_cmdline) */

/* How a supervised child ended. */
typedef enum {
    ENGINE_PROC_CLEAN = 0,    /* exited with code 0 */
    ENGINE_PROC_EXIT_NONZERO, /* exited with a nonzero code (a graceful failure) */
    ENGINE_PROC_CRASH,        /* died from a fault: POSIX SIGSEGV/BUS/ILL/FPE/ABRT/SYS,
                            * or a Windows NTSTATUS exception exit code (>= 0xC0000000) */
    ENGINE_PROC_HANG,         /* made no progress within the quiet-timeout; we killed it */
    ENGINE_PROC_KILLED,       /* terminated by a non-fault signal we did not initiate */
    ENGINE_PROC_SPAWN_FAILED  /* fork/exec/CreateProcess failed — no child ever ran */
} engine_proc_outcome_t;

typedef struct {
    engine_proc_outcome_t outcome;
...
typedef void (*engine_proc_log_cb)(const char *line, void *ud);
...
typedef struct {
    ...
    engine_proc_log_cb on_log_line;     /* optional per-line callback */
    ...
    int cancel_grace_ms;             /* graceful tree-termination window; <= 0 uses the finite
                                      * ENGINE_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS */
    bool delete_log_on_exit;         /* unlink log_file after reaping */
} engine_proc_opts_t;

#define ENGINE_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS 1000
#define ENGINE_SUBPROCESS_MAX_CANCEL_GRACE_MS 1000
#define ENGINE_SUBPROCESS_FORCE_SETTLE_MS 1000
...
typedef struct engine_subprocess engine_subprocess_t;

typedef enum {
    ENGINE_PROC_POLL_ERROR = -1, /* invalid handle/arguments */
    ENGINE_PROC_POLL_RUNNING = 0,
    ENGINE_PROC_POLL_TERMINAL = 1
} engine_proc_poll_t;
 ...
 * but the terminal outcome remains ENGINE_PROC_HANG. */
 ...
 * (out->outcome == ENGINE_PROC_SPAWN_FAILED). */
 ...
 * is unit-tested on Linux/macOS CI, and so both spawn sites (engine_subprocess_run and
 ...
 * command-language payload. Unlike engine_build_win_cmdline(), payload is not a C
