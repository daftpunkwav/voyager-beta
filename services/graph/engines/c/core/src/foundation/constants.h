...
/* ── Allocation counts ───────────────────────────────────────── */
enum { ENGINE_ALLOC_ONE = 1 }; /* calloc(ENGINE_ALLOC_ONE, sizeof(T)) */

/* ── Byte / character constants ──────────────────────────────── */
enum {
    ENGINE_BYTE_RANGE = 256, /* full byte range 0x00–0xFF */
    ENGINE_QUOTE_PAIR = 2,   /* two quote characters (open + close) */
    ENGINE_QUOTE_OFFSET = 1, /* skip opening quote */
};
...
/* ── Sentinel values ─────────────────────────────────────────── */
enum {
    ENGINE_NOT_FOUND = -1, /* search miss, invalid index */
    ENGINE_INIT_DONE = 1,  /* initialization flag */
};
...
enum { ENGINE_DEFAULT_SEARCH_LIMIT = 50 };

/* ── Time conversion factors ─────────────────────────────────── */
#define ENGINE_NSEC_PER_SEC 1000000000ULL
#define ENGINE_USEC_PER_SEC 1000000ULL
#define ENGINE_MSEC_PER_SEC 1000ULL
#define ENGINE_NSEC_PER_USEC 1000ULL
#define ENGINE_NSEC_PER_MSEC 1000000ULL

/* ── Common string/buffer sizes ──────────────────────────────── */
enum {
    ENGINE_SMALL_BUF = 3,   /* small scratch buffers */
    ENGINE_NAME_BUF = 4,    /* name buffer slots */
    ENGINE_PATH_MAX = 1024, /* path buffer size */
    ENGINE_LINE_BUF = 512,  /* line read buffer */
};
 ...
 * SQL mirror of engine_label_is_type_like() (internal/engine/helpers.c). That
 ...
 * them against engine_label_is_type_like(), so adding a type-like label there
 * without updating these fails CI rather than quietly shrinking query results. */
#define ENGINE_SQL_TYPE_LIKE_LABELS "'Class','Struct','Interface','Enum','Type','Trait'"
#define ENGINE_SQL_CALLABLE_LABELS "'Function','Method'"
#define ENGINE_SQL_CALLABLE_OR_TYPE_LABELS ENGINE_SQL_CALLABLE_LABELS "," ENGINE_SQL_TYPE_LIKE_LABELS
