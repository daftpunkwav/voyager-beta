 ...
 *
 * Linux PROD builds now set MI_MALLOC_OVERRIDE (see Makefile.engine), so the strong
...
void *__wrap_malloc(size_t size) {
    ...
    engine_mem_profile_alloc(block, size);
    return block;
}

void *__wrap_calloc(size_t count, size_t size) {
    void *block = mi_calloc(count, size);
    engine_mem_profile_alloc(block, count * size);
    return block;
}

void *__wrap_realloc(void *block, size_t size) {
    if (block) {
        engine_mem_profile_free(block);
    }
    void *grown = mi_realloc(block, size);
    engine_mem_profile_alloc(grown, size);
    return grown;
}
...
char *__wrap_strdup(const char *text) {
    ...
    if (copy) {
        engine_mem_profile_alloc(copy, mi_usable_size(copy));
    }
    return copy;
}
