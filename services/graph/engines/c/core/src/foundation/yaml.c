...
static engine_yaml_node_t *node_new(yaml_type_t type) {
    engine_yaml_node_t *n = calloc(ENGINE_ALLOC_ONE, sizeof(*n));
    if (n) {
        n->type = type;
    }
    return n;
...
static bool node_add_child(engine_yaml_node_t *parent, engine_yaml_node_t *child) {
    if (!parent || !child) {
        return false;
    }
    if (parent->child_count >= parent->child_cap) {
        ...
        engine_yaml_node_t **new_arr = realloc(parent->children, (size_t)new_cap * sizeof(*new_arr));
        if (!new_arr) {
            return false;
        }
...
void engine_yaml_free(engine_yaml_node_t *root) {
    ...
    engine_yaml_node_t *stack[YAML_FREE_STACK];
    int top = 0;
    stack[top++] = root;
    while (top > 0) {
        engine_yaml_node_t *node = stack[--top];
...
typedef struct {
    engine_yaml_node_t *node;
    int indent;
} stack_entry_t;

/* Parse a list item "- value" and add to the correct parent. */
static void parse_list_item(const char *content, int content_len, engine_yaml_node_t *parent) {
    ...
    engine_yaml_node_t *list_parent = parent;
    if (parent->type == YAML_MAP && parent->child_count > 0) {
        engine_yaml_node_t *last = parent->children[parent->child_count - SKIP_ONE];
        if (last->type == YAML_LIST) {
            list_parent = last;
        }
    }

    engine_yaml_node_t *item = node_new(YAML_SCALAR);
    if (item) {
        item->value = trim_dup(item_start, item_start + item_len);
        if (!node_add_child(list_parent, item)) {
            engine_yaml_free(item);
        }
    }
}
...
static void parse_key_line(const char *content, int content_len, int indent,
                           engine_yaml_node_t *parent, stack_entry_t *stack, int *stack_depth,
    ...
    if (after_len > 0) {
        ...
        engine_yaml_node_t *child = node_new(is_list ? YAML_LIST : YAML_MAP);
        if (child) {
            child->key = key;
            if (!node_add_child(parent, child)) {
                engine_yaml_free(child);
            } else if (*stack_depth < ENGINE_SZ_32) {
...
engine_yaml_node_t *engine_yaml_parse(const char *text, int len) {
    ...
    engine_yaml_node_t *root = node_new(YAML_MAP);
    if (!root) {
        return NULL;
    }

    /* Stack for tracking parent context */
    stack_entry_t stack[ENGINE_SZ_32];
    int stack_depth = 0;
    ...
    while (p < end) {
        ...
        engine_yaml_node_t *parent = stack[stack_depth - SKIP_ONE].node;
...
static const engine_yaml_node_t *find_child(const engine_yaml_node_t *node, const char *key) {
    if (!node || !key) {
        return NULL;
    }
...
static const engine_yaml_node_t *navigate(const engine_yaml_node_t *root, const char *path) {
    if (!root || !path) {
        return NULL;
    }

    const engine_yaml_node_t *cur = root;
...
const char *engine_yaml_get_str(const engine_yaml_node_t *root, const char *path) {
    const engine_yaml_node_t *node = navigate(root, path);
...
double engine_yaml_get_float(const engine_yaml_node_t *root, const char *path, double default_val) {
    const char *str = engine_yaml_get_str(root, path);
    if (!str) {
        return default_val;
    }
    char *endptr;
