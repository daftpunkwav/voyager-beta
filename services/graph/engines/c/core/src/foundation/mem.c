 ...
 *
 * Set by Makefile.engine alongside MI_MALLOC_OVERRIDE itself — the two are
...
#if defined(ENGINE_MEM_GLOBAL_OVERRIDE)
#define ENGINE_MEM_EXPECT_GLOBAL_OVERRIDE 1
#else
#define ENGINE_MEM_EXPECT_GLOBAL_OVERRIDE 0
#endif
...
#define MB_DIVISOR ((size_t)(ENGINE_SZ_1K * ENGINE_SZ_1K))
...
static size_t os_rss(void) {
    ...
    return rss_pages * (ps > 0 ? (size_t)ps : ENGINE_SZ_4K);
#endif
}
...
static void check_pressure(size_t rss) {
    ...
    if (over && !was) {
        ...
        char rss_mb[ENGINE_SZ_32];
        char budget_mb[ENGINE_SZ_32];
        char pct_str[ENGINE_SZ_16];
        ...
        snprintf(pct_str, sizeof(pct_str), "%zu",
                 g_budget > 0 ? (rss * ENGINE_PERCENT) / g_budget : 0);
        engine_log_warn("mem.pressure.warn", "rss_mb", rss_mb, "budget_mb", budget_mb, "pct", pct_str);
    } else if (!over && was) {
        ...
        snprintf(pct_str, sizeof(pct_str), "%zu",
                 g_budget > 0 ? (rss * ENGINE_PERCENT) / g_budget : 0);
        engine_log_info("mem.pressure.ok", "rss_mb", rss_mb, "budget_mb", budget_mb, "pct", pct_str);
    }
}
...
static void mem_setup_fatal(const char *what, const char *detail) {
    engine_log_error("mem.allocator.setup_failed", "check", what, "detail", detail);
...
double engine_mem_ram_fraction_for_total(size_t total_ram_bytes) {
...
engine_mem_budget_t engine_mem_resolve_budget(size_t total_ram, double ram_fraction,
                                        const char *budget_mb) {
    if (ram_fraction <= 0.0 || ram_fraction > MAX_RAM_FRACTION) {
        ram_fraction = DEFAULT_RAM_FRACTION;
    }
    engine_mem_budget_t result = {
    ...
    errno = 0;
    char *end = NULL;
    long long want_mb = strtoll(budget_mb, &end, ENGINE_DECIMAL_BASE);
    if (errno != 0 || end == budget_mb || *end != '\0' || want_mb <= 0) {
        result.invalid = true; /* keep the fraction-derived budget */
        return result;
    }

    result.source = "ENGINE_MEM_BUDGET_MB";
...
engine_mem_budget_t engine_mem_resolve_budget_capped(size_t total_ram, double ram_fraction,
                                               const char *budget_mb, size_t hard_cap_bytes) {
    engine_mem_budget_t result = engine_mem_resolve_budget(total_ram, ram_fraction, budget_mb);
...
void engine_mem_init_with_cap(double ram_fraction, size_t hard_cap_bytes) {
    int expected = 0;
    ...
    if (_mi_preloading()) {
        ...
        engine_log_warn("mem.allocator.preloading_completed", "still_preloading", "false", "detail",
                     "allocator was still preloading, so arena creation and purging were "
                     "disabled; process init completed explicitly");
    } else {
        engine_log_info("mem.allocator.preloading", "state", "already_complete");
    }
#endif
    ...
    engine_mem_ownership_audit_t audit;
    engine_mem_audit_ownership(&audit);
    char owned_str[ENGINE_SZ_32];
    snprintf(owned_str, sizeof(owned_str), "%d/%d", audit.owned_count, audit.probed_count);
    if (!audit.all_owned && !ENGINE_MEM_EXPECT_GLOBAL_OVERRIDE) {
        ...
        engine_log_info("mem.allocator.bound_populations_only", "owned_classes", owned_str,
        ...
        char classes[ENGINE_SZ_256];
        int length = 0;
        for (int i = 0;
             i < ENGINE_MEM_OWNERSHIP_CLASSES && length >= 0 && (size_t)length < sizeof(classes);
             i++) {
        ...
        engine_log_warn("mem.allocator.not_owned", "owned_classes", owned_str, "unowned_bytes",
        ...
        engine_log_info("mem.allocator.owned", "classes", "all");
    }

    /* ENGINE_MEM_BUDGET_MB env override (memory analogue of ENGINE_WORKERS).
     ...
     * testable engine_mem_resolve_budget(); this path only reads the env and
     * emits the log/warn lines. */
    engine_system_info_t info = engine_system_info();

    char env_buf[ENGINE_SZ_32];
    const char *env = engine_safe_getenv("ENGINE_MEM_BUDGET_MB", env_buf, sizeof(env_buf), NULL);
    engine_mem_budget_t resolved =
        engine_mem_resolve_budget_capped(info.total_ram, ram_fraction, env, hard_cap_bytes);
    ...
    if (resolved.invalid) {
        engine_log_warn("mem.budget.env.invalid", "value", env, "fallback", "ram_fraction");
    } else if (resolved.clamped) {
        char cap_mb[ENGINE_SZ_32];
        snprintf(cap_mb, sizeof(cap_mb), "%zu", info.total_ram / MB_DIVISOR);
        engine_log_warn("mem.budget.clamped", "requested_mb", env, "cap_mb", cap_mb);
    }
    if (resolved.hard_capped) {
        char cap_bytes[ENGINE_SZ_32];
        snprintf(cap_bytes, sizeof(cap_bytes), "%zu", hard_cap_bytes);
        engine_log_info("mem.budget.worker_cap", "cap_bytes", cap_bytes);
    }

    char budget_mb[ENGINE_SZ_32];
    char ram_mb[ENGINE_SZ_32];
    snprintf(budget_mb, sizeof(budget_mb), "%zu", g_budget / MB_DIVISOR);
    snprintf(ram_mb, sizeof(ram_mb), "%zu", info.total_ram / MB_DIVISOR);
    engine_log_info("mem.init", "budget_mb", budget_mb, "total_ram_mb", ram_mb, "source",
                 resolved.source);
}

void engine_mem_init(double ram_fraction) {
    engine_mem_init_with_cap(ram_fraction, 0);
}

size_t engine_mem_rss(void) {
     ...
     * (engine_mem_init) to reduce upfront memory. So on Linux "current_rss" is a
     ...
     * silently blinding engine_mem_over_budget()'s backpressure to real memory
...
size_t engine_mem_peak_rss(void) {
    size_t peak_rss = 0;
    mi_process_info(NULL, NULL, NULL, NULL, &peak_rss, NULL, NULL, NULL);
    /* Peak RSS is by definition >= current RSS. On Linux engine_mem_rss() reads the
...
size_t engine_mem_budget(void) {
    return g_budget;
}

void engine_mem_set_budget_for_tests(size_t bytes) {
    g_budget = bytes;
}

bool engine_mem_over_budget(void) {
    size_t rss = engine_mem_rss();
    check_pressure(rss);
    return rss > g_budget;
}

size_t engine_mem_worker_budget(int num_workers) {
    if (num_workers <= 0) {
...
void engine_mem_collect(void) {
    mi_collect(true);
}

/* ── Memory map (see mem.h for how to read the triple) ─────────────── */

static const size_t MEM_MAP_BUCKET_LIMITS[ENGINE_MEM_MAP_BUCKETS] = {
    64, 256, 1024, 4096, 16384, 65536, 1048576, 0, /* 0 = open-ended tail */
};

size_t engine_mem_map_bucket_limit(int bucket) {
    if (bucket < 0 || bucket >= ENGINE_MEM_MAP_BUCKETS) {
        return 0;
    }
...
static int mem_map_bucket_of(size_t block_size) {
    for (int i = 0; i < ENGINE_MEM_MAP_BUCKETS - 1; i++) {
        if (block_size <= MEM_MAP_BUCKET_LIMITS[i]) {
            return i;
        }
    }
    return ENGINE_MEM_MAP_BUCKETS - 1;
}
...
static bool mem_map_collect_impl(engine_mem_map_t *out, bool walk_allocator);

bool engine_mem_map_collect_os(engine_mem_map_t *out) {
    return mem_map_collect_impl(out, false);
}

bool engine_mem_map_collect(engine_mem_map_t *out) {
...
static bool mem_map_collect_impl(engine_mem_map_t *out, bool walk_allocator) {
    ...
    out->os_rss_bytes = current_rss ? current_rss : engine_mem_rss();
    ...
    void *probe = malloc(ENGINE_SZ_64);
    if (probe) {
...
void engine_mem_audit_ownership(engine_mem_ownership_audit_t *out) {
    ...
    static const size_t PROBES[ENGINE_MEM_OWNERSHIP_CLASSES] = {
        64, 4096, 65536, 262144, 1048576, 8388608,
    };
    for (int i = 0; i < ENGINE_MEM_OWNERSHIP_CLASSES; i++) {
...
static engine_mutex_t g_mem_phase_mutex;
...
static bool mem_phase_enabled(void) {
    ...
    bool on = engine_safe_getenv("ENGINE_MEM_PHASES", buf, sizeof(buf), NULL) != NULL && buf[0] == '1';
    if (on) {
        engine_mutex_init(&g_mem_phase_mutex);
    }
    atomic_store_explicit(&g_mem_phase_enabled, on ? 1 : 0, memory_order_release);
    return on;
}

void engine_mem_phase_mark(const char *label) {
    if (!mem_phase_enabled()) {
        return;
    }
    size_t now = mem_phase_committed();
    engine_mutex_lock(&g_mem_phase_mutex);
    if (g_mem_phase_open) {
        int slot = -1;
    ...
    engine_mutex_unlock(&g_mem_phase_mutex);
}

void engine_mem_phase_reset(void) {
    if (!mem_phase_enabled()) {
        return;
    }
    engine_mutex_lock(&g_mem_phase_mutex);
...
int engine_mem_phase_report_json(char *out, size_t size) {
    if (!out || size == 0) {
        return 0;
    }
    out[0] = '\0';
...
bool engine_mem_census_enabled(void) {
    int state = atomic_load_explicit(&g_mem_census_enabled, memory_order_acquire);
    if (state >= 0) {
        return state == 1;
    }
    char buf[ENGINE_SZ_16];
    bool on = engine_safe_getenv("ENGINE_MEM_CENSUS", buf, sizeof(buf), NULL) != NULL && buf[0] == '1';
    atomic_store_explicit(&g_mem_census_enabled, on ? 1 : 0, memory_order_release);
    return on;
}

bool engine_mem_win_census(engine_mem_win_census_t *out) {
    if (!out) {
        return false;
    }
    memset(out, 0, sizeof(*out));
    ...
    while (VirtualQuery(address, &mbi, sizeof(mbi)) == sizeof(mbi)) {
        ...
        if (mbi.State == MEM_COMMIT) {
            ...
            case MEM_PRIVATE:
                ...
                if (mbi.RegionSize < (size_t)ENGINE_SZ_64K) {
                    out->private_small_bytes += mbi.RegionSize;
                    out->private_small_count++;
                } else if (mbi.RegionSize < (size_t)ENGINE_SZ_1K * ENGINE_SZ_1K) {
                    ...
                    if (out->large_tracked < ENGINE_MEM_TRACKED_REGIONS) {
                ...
                if (mbi.RegionSize >= (size_t)ENGINE_SZ_1K * ENGINE_SZ_1K) {
...
void engine_mem_census_log(const char *tag) {
    if (!engine_mem_census_enabled()) {
        return;
    }
    engine_mem_win_census_t census;
    ...
    (void)engine_mem_win_census(&census);
    ...
    char priv_kb[ENGINE_SZ_32];
    char crt_kb[ENGINE_SZ_32];
    char busy_kb[ENGINE_SZ_32];
    char mapped_kb[ENGINE_SZ_32];
    char rss_kb[ENGINE_SZ_32];
    char biggest_kb[ENGINE_SZ_32];
    char regions[ENGINE_SZ_32];
    char base[ENGINE_SZ_32];
    char big_regions[ENGINE_SZ_32];
    char psmall[ENGINE_SZ_32];
    char pmed[ENGINE_SZ_32];
    char plarge[ENGINE_SZ_32];
    char pcounts[ENGINE_SZ_64];
    char large_map[ENGINE_SZ_256];
    char heaps[ENGINE_SZ_32];
    (void)snprintf(priv_kb, sizeof(priv_kb), "%zu", census.committed_private / ENGINE_SZ_1K);
    (void)snprintf(crt_kb, sizeof(crt_kb), "%zu", census.crt_heap_committed / ENGINE_SZ_1K);
    (void)snprintf(busy_kb, sizeof(busy_kb), "%zu", census.crt_heap_busy / ENGINE_SZ_1K);
    (void)snprintf(mapped_kb, sizeof(mapped_kb), "%zu", census.committed_mapped / ENGINE_SZ_1K);
    (void)snprintf(rss_kb, sizeof(rss_kb), "%zu", engine_mem_rss() / ENGINE_SZ_1K);
    (void)snprintf(biggest_kb, sizeof(biggest_kb), "%zu",
                   census.largest_private_region / ENGINE_SZ_1K);
    ...
    (void)snprintf(psmall, sizeof(psmall), "%zu", census.private_small_bytes / ENGINE_SZ_1K);
    (void)snprintf(pmed, sizeof(pmed), "%zu", census.private_medium_bytes / ENGINE_SZ_1K);
    (void)snprintf(plarge, sizeof(plarge), "%zu", census.private_large_bytes / ENGINE_SZ_1K);
    ...
    engine_mem_map_t map;
    (void)engine_mem_map_collect(&map);
    char map_live_kb[ENGINE_SZ_32];
    char map_area_kb[ENGINE_SZ_32];
    (void)snprintf(map_live_kb, sizeof(map_live_kb), "%zu", map.live_bytes / ENGINE_SZ_1K);
    (void)snprintf(map_area_kb, sizeof(map_area_kb), "%zu", map.area_committed_bytes / ENGINE_SZ_1K);
    engine_log_info("mem.census", "at", tag ? tag : "?", "mi_area_kb", map_area_kb, "mi_live_kb",
    ...
    if (tag && strcmp(tag, "connection.exit") == 0) {
        ...
        char stats_path[ENGINE_SZ_1K];
        if (engine_safe_getenv("ENGINE_MEM_STATS_OUT", stats_path, sizeof(stats_path), NULL) != NULL) {
            FILE *stats = engine_fopen(stats_path, "wb");
            if (stats) {
                mi_stats_print_out(mem_stats_writer, stats);
                (void)fclose(stats);
            }
        }
        char phases[ENGINE_SZ_1K];
        if (engine_mem_phase_report_json(phases, sizeof(phases)) > 0) {
            engine_log_info("mem.phases", "table", phases);
        }
    }
}
