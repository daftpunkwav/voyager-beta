...
/* Default signal weights for engine_sem_combined_score.  Must sum to ~1.0;
 * proximity is a multiplier applied on top. */
#define ENGINE_SEM_W_TFIDF 0.20F
#define ENGINE_SEM_W_RI 0.25F
#define ENGINE_SEM_W_MINHASH 0.10F
#define ENGINE_SEM_W_API 0.15F
#define ENGINE_SEM_W_TYPE 0.10F
#define ENGINE_SEM_W_DECORATOR 0.05F
#define ENGINE_SEM_W_STRUCT_PROFILE 0.10F
#define ENGINE_SEM_W_DATAFLOW 0.05F

/* Threshold bounds for ENGINE_SEMANTIC_THRESHOLD env override. */
#define ENGINE_SEM_THRESHOLD_MIN 0.0F
#define ENGINE_SEM_THRESHOLD_MAX 1.0F

/* Epsilon for denominator guards in cosine math. */
#define ENGINE_SEM_DENOM_EPS 1e-10F

/* Int8 quantization bounds for vector storage (maps [-1.0, 1.0] to [-127, 127]). */
#define ENGINE_SEM_INT8_MAX 127.0F
#define ENGINE_SEM_INT8_MIN (-127.0F)

/* Unit vector bounds (normalized similarity / cosine value limits). */
#define ENGINE_SEM_UNIT_POS 1.0F
#define ENGINE_SEM_UNIT_NEG (-1.0F)

/* Proximity boost: same-file gets 1.0 + ENGINE_SEM_PROX_MAX_BOOST, distant gets 1.0. */
#define ENGINE_SEM_PROX_MAX_BOOST 0.10F

/* Reflective Random Indexing blend factors for pass2 (context mixing). */
#define ENGINE_SEM_RRI_ALPHA 0.3F
#define ENGINE_SEM_RRI_BETA 0.7F

/* IDF smoothing constant (log base): log2(1 + docs / doc_freq). */
#define ENGINE_SEM_IDF_BASE 0.5F

/* Strings of exactly two parts / dimensions used for co-occurrence window math. */
#define ENGINE_SEM_COOCCUR_STRIDE 2

/* Worker tile/chunk sizes for parallel cooccurrence passes. */
enum {
    ENGINE_SEM_WORKER_STACK_CAP = 256,
    ENGINE_SEM_TILE_SIZE = 40,
    ENGINE_SEM_SEEN_INIT_CAP = 256,
    ENGINE_SEM_RESOLVE_CHUNK = 64,
    ENGINE_SEM_COOCCUR_CHUNK = 32,
    ENGINE_SEM_RRI_TILE = 128,
    ENGINE_SEM_INT8_I_LO = -128,
    ENGINE_SEM_INT8_I_HI = 127,
    ENGINE_SEM_ATOMIC_INC = 1,
    ENGINE_SEM_RI_NONZERO_COUNT = 8,
};
...
engine_sem_config_t engine_sem_get_config(void) {
    engine_sem_config_t cfg = {
        .w_tfidf = ENGINE_SEM_W_TFIDF,
        .w_ri = ENGINE_SEM_W_RI,
        .w_minhash = ENGINE_SEM_W_MINHASH,
        .w_api = ENGINE_SEM_W_API,
        .w_type = ENGINE_SEM_W_TYPE,
        .w_decorator = ENGINE_SEM_W_DECORATOR,
        .w_struct_profile = ENGINE_SEM_W_STRUCT_PROFILE,
        .w_dataflow = ENGINE_SEM_W_DATAFLOW,
        .threshold = (float)ENGINE_SEM_EDGE_THRESHOLD,
        .max_edges = ENGINE_SEM_MAX_EDGES,
    };
    const char *thresh = getenv("ENGINE_SEMANTIC_THRESHOLD");
    if (thresh) {
        /* strtod reports errors via endptr; reject non-numeric input silently. */
        char *end = NULL;
        double parsed = strtod(thresh, &end);
        if (end != thresh && parsed > ENGINE_SEM_THRESHOLD_MIN && parsed <= ENGINE_SEM_THRESHOLD_MAX) {
...
bool engine_sem_is_enabled(void) {
    const char *val = getenv("ENGINE_SEMANTIC_ENABLED");
    return val && val[0] == '1';
}
...
int engine_sem_tokenize(const char *name, char **out, int max_out) {
...
float engine_sem_cosine(const engine_sem_vec_t *a, const engine_sem_vec_t *b) {
    if (!a || !b) {
        return 0.0F;
    }
    float dot = 0.0F;
    float mag_a = 0.0F;
    float mag_b = 0.0F;
    for (int i = 0; i < ENGINE_SEM_DIM; i++) {
        dot += a->v[i] * b->v[i];
        mag_a += a->v[i] * a->v[i];
        mag_b += b->v[i] * b->v[i];
    }
    float denom = sqrtf(mag_a) * sqrtf(mag_b);
    if (denom < ENGINE_SEM_DENOM_EPS) {
        return 0.0F;
    }
    return dot / denom;
}
...
static EngineHashTable *g_pretrained_map = NULL;
static _Atomic int g_pretrained_ready = 0;
static engine_mutex_t g_pretrained_mtx;
...
static void ensure_pretrained_map(void) {
    ...
    if (atomic_compare_exchange_strong_explicit(&g_pretrained_mtx_init, &expected,
        ...
        engine_mutex_init(&g_pretrained_mtx);
    ...
    engine_mutex_lock(&g_pretrained_mtx);
    if (!atomic_load_explicit(&g_pretrained_ready, memory_order_acquire)) {
        g_pretrained_map = engine_ht_create(PRETRAINED_TOKEN_COUNT);
        char idx_buf[ENGINE_SZ_16];
        for (int i = 0; i < PRETRAINED_TOKEN_COUNT; i++) {
            const char *tok = PRETRAINED_TOKENS[i];
            if (tok && tok[0]) {
                snprintf(idx_buf, sizeof(idx_buf), "%d", i);
                engine_ht_set(g_pretrained_map, strdup(tok), strdup(idx_buf));
            }
        }
        atomic_store_explicit(&g_pretrained_ready, MAP_READY, memory_order_release);
    }
    engine_mutex_unlock(&g_pretrained_mtx);
}

void engine_sem_ensure_ready(void) {
    ensure_pretrained_map();
}

void engine_sem_random_index(const char *token, engine_sem_vec_t *out) {
    ...
    ensure_pretrained_map();
    const char *idx_str = engine_ht_get(g_pretrained_map, token);
    if (idx_str) {
        char *end = NULL;
        ...
        if (end != idx_str && idx >= 0 && idx < PRETRAINED_TOKEN_COUNT) {
            ...
            for (int d = 0; d < ENGINE_SEM_DIM && d < PRETRAINED_DIM; d++) {
                out->v[d] = (float)pvec[d] / ENGINE_SEM_INT8_MAX;
            }
            return;
        }
    }
    ...
    for (int i = 0; i < ENGINE_SEM_SPARSE_NNZE; i++) {
        uint64_t h = XXH3_64bits_withSeed(&i, sizeof(i), seed + RI_SEED_BASE);
        int pos = (int)(h % ENGINE_SEM_DIM);
        float sign = (h & SKIP_ONE) ? ENGINE_SEM_UNIT_POS : -ENGINE_SEM_UNIT_POS;
        out->v[pos] += sign;
    }
}

void engine_sem_normalize(engine_sem_vec_t *v) {
    if (!v) {
        return;
    }
    float mag = 0.0F;
    ...
    mag = sqrtf(mag);
    if (mag < ENGINE_SEM_DENOM_EPS) {
        return;
    }
    float inv = ENGINE_SEM_UNIT_POS / mag;
    for (int i = 0; i < ENGINE_SEM_DIM; i++) {
        v->v[i] *= inv;
    }
}

void engine_sem_vec_add_scaled(engine_sem_vec_t *dst, const engine_sem_vec_t *src, float scale) {
    if (!dst || !src) {
        return;
    }
    for (int i = 0; i < ENGINE_SEM_DIM; i++) {
...
typedef struct {
    char *token;
    int doc_freq;
    engine_sem_vec_t enriched_vec; /* context-enriched via co-occurrence */
} corpus_entry_t;

struct engine_sem_corpus {
    EngineHashTable *token_map; /* token → index into entries[] */
...
static int corpus_get_or_add(engine_sem_corpus_t *c, const char *token) {
    char idx_buf[ENGINE_SZ_16];
    const char *existing = engine_ht_get(c->token_map, token);
    if (existing) {
        char *end = NULL;
        long parsed = strtol(existing, &end, BASE_DECIMAL);
        return (end != existing) ? (int)parsed : ENGINE_NOT_FOUND;
    }
    ...
    memset(&c->entries[idx].enriched_vec, 0, sizeof(engine_sem_vec_t));
    snprintf(idx_buf, sizeof(idx_buf), "%d", idx);
    engine_ht_set(c->token_map, strdup(token), strdup(idx_buf));
    return idx;
}

engine_sem_corpus_t *engine_sem_corpus_new(void) {
    engine_sem_corpus_t *c = calloc(SKIP_ONE, sizeof(engine_sem_corpus_t));
    if (c) {
        c->token_map = engine_ht_create(CORPUS_INIT_CAP);
    }
    return c;
}

void engine_sem_corpus_add_doc(engine_sem_corpus_t *corpus, const char **tokens, int count) {
...
static void batch_resolve_one_doc(batch_resolve_ctx_t *bc, int doc_index, int *seen) {
    ...
    for (int i = 0; i < count; i++) {
        const char *idx_str = engine_ht_get(bc->corpus->token_map, tokens[i]);
        int tid = ENGINE_NOT_FOUND;
        if (idx_str) {
            char *end = NULL;
        ...
        if (is_new) {
            seen[seen_count++] = tid;
            atomic_fetch_add_explicit(&bc->doc_freq_atomic[tid], ENGINE_SEM_ATOMIC_INC,
...
static void batch_resolve_worker(int worker_id, void *ctx_ptr) {
    ...
    int local_seen_cap = ENGINE_SEM_SEEN_INIT_CAP;
    ...
    while (true) {
        int start =
            atomic_fetch_add_explicit(&bc->next_idx, ENGINE_SEM_RESOLVE_CHUNK, memory_order_relaxed);
        if (start >= bc->doc_count) {
            break;
        }
        int end = start + ENGINE_SEM_RESOLVE_CHUNK;
        if (end > bc->doc_count) {
            end = bc->doc_count;
        }
...
void engine_sem_corpus_add_docs_batch(engine_sem_corpus_t *corpus, char **all_tokens,
    ...
    if (!doc_freq_atomic) {
        ...
        for (int d = 0; d < doc_count; d++) {
            int count = token_counts[d];
            char **tokens = &all_tokens[(ptrdiff_t)d * max_tokens_per_doc];
            engine_sem_corpus_add_doc(corpus, (const char **)tokens, count);
        }
        return;
    }

    int worker_count = engine_default_worker_count(false);
    ...
    engine_parallel_for(worker_count, batch_resolve_worker, &bc, opts);
    corpus->doc_token_ids -= base_doc;
    corpus->doc_token_counts -= base_doc;
 ...
 *      redundant engine_sem_random_index calls on kernel-scale corpora).
...
typedef struct {
    ...
    uint16_t _pad;
    uint16_t indices[ENGINE_SEM_SPARSE_NNZE]; /* 8 * 2 = 16 bytes */
    float values[ENGINE_SEM_SPARSE_NNZE];     /* 8 * 4 = 32 bytes */
    const int8_t *dense_int8;              /* points into PRETRAINED_VECTOR_BLOB */
} engine_sem_src_entry_t;

/* Inline helper: initialize a target vector from a sparse/dense source. */
static inline void sem_target_init_from_src(engine_sem_vec_t *dst, const engine_sem_src_entry_t *src) {
    ...
    if (src->is_sparse) {
        ...
        const float inv127 = ENGINE_SEM_UNIT_POS / ENGINE_SEM_INT8_MAX;
        for (int d = 0; d < ENGINE_SEM_DIM; d++) {
...
static inline void sem_vec_add_src_scaled(engine_sem_vec_t *dst, const engine_sem_src_entry_t *src,
                                          float scale) {
    if (src->is_sparse) {
        for (int k = 0; k < src->nnz; k++) {
        ...
        const int8_t *s = src->dense_int8;
        const float mul = scale * (ENGINE_SEM_UNIT_POS / ENGINE_SEM_INT8_MAX);
        for (int d = 0; d < ENGINE_SEM_DIM; d++) {
...
typedef struct {
    corpus_entry_t *entries;
    const engine_sem_src_entry_t *src_entries; /* sparse or int8-dense per token */
    int **doc_token_ids;
 ...
 * Reads neighbors within ±ENGINE_SEM_WINDOW positions across all documents that
 * reference this token id via the reverse index. */
static void cooccur_sparse_one_target(cooccur_sparse_ctx_t *cc, int tid, engine_sem_vec_t *target) {
     ...
     * after) while bounding work to ~ENGINE_SEM_MAX_OCCUR samples. See semantic.h. */
    int occ_step = SKIP_ONE;
    if (occ_end - occ_start > ENGINE_SEM_MAX_OCCUR) {
        occ_step = (occ_end - occ_start) / ENGINE_SEM_MAX_OCCUR;
    }
    for (int p = occ_start; p < occ_end; p += occ_step) {
        ...
        for (int w = -ENGINE_SEM_WINDOW; w <= ENGINE_SEM_WINDOW; w++) {
            if (w == 0) {
                continue;
            }
            int j = i + w;
            ...
            float weight = ENGINE_SEM_UNIT_POS / (float)abs(w);
...
static void cooccur_worker_sparse(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        int ci =
            atomic_fetch_add_explicit(&cc->next_chunk, ENGINE_SEM_ATOMIC_INC, memory_order_relaxed);
        if (ci >= cc->num_chunks) {
            break;
        }
...
typedef struct {
    corpus_entry_t *entries;
    const int8_t *pass1_q; /* [entry_count × ENGINE_SEM_DIM] int8 quantized pass1 */
    int **doc_token_ids;
    const int *doc_token_counts;
    const reverse_index_t *rev;
    int doc_count;
...
static inline void sem_vec_add_int8_scaled(engine_sem_vec_t *dst, const int8_t *src, float scale) {
    const float mul = scale * (ENGINE_SEM_UNIT_POS / ENGINE_SEM_INT8_MAX);
    for (int d = 0; d < ENGINE_SEM_DIM; d++) {
...
static void cooccur_int8_one_target(cooccur_int8_ctx_t *cc, int tid, engine_sem_vec_t *target) {
    int occ_start = cc->rev->offsets[tid];
    int occ_end = cc->rev->offsets[tid + SKIP_ONE];
    ...
    for (int p = occ_start; p < occ_end; p += occ_step) {
        ...
        for (int w = -ENGINE_SEM_WINDOW; w <= ENGINE_SEM_WINDOW; w++) {
            ...
            float weight = ENGINE_SEM_UNIT_POS / (float)abs(w);
            sem_vec_add_int8_scaled(target, &cc->pass1_q[(size_t)nid * ENGINE_SEM_DIM], weight);
        }
    }
}
    ...
    while (true) {
        ...
        for (int tile_start = chunk_start; tile_start < chunk_end; tile_start += cc->tile_size) {
            ...
            for (int tid = tile_start; tid < tile_end; tid++) {
                ...
                memset(&cc->entries[tid].enriched_vec, 0, sizeof(engine_sem_vec_t));
...
static void normalize_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        int start = atomic_fetch_add_explicit(&nc->next_idx, ENGINE_SEM_WORKER_STACK_CAP,
                                              memory_order_relaxed);
        if (start >= nc->entry_count) {
            break;
        }
        int end = start + ENGINE_SEM_WORKER_STACK_CAP;
        if (end > nc->entry_count) {
            end = nc->entry_count;
        }
        for (int i = start; i < end; i++) {
            engine_sem_normalize(&nc->entries[i].enriched_vec);
        }
    }
}
...
static void blend_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        int start = atomic_fetch_add_explicit(&bc->next_idx, ENGINE_SEM_WORKER_STACK_CAP,
                                              memory_order_relaxed);
        ...
        for (int i = start; i < end; i++) {
            engine_sem_normalize(&bc->entries[i].enriched_vec);
            for (int d = 0; d < ENGINE_SEM_DIM; d++) {
                bc->entries[i].enriched_vec.v[d] =
                    (ENGINE_SEM_RRI_BETA * bc->pass1[i].v[d]) +
                    (ENGINE_SEM_RRI_ALPHA * bc->entries[i].enriched_vec.v[d]);
            }
        }
    }
}
...
typedef struct {
    corpus_entry_t *entries;
    engine_sem_src_entry_t *src_entries;
    int entry_count;
    _Atomic int next_idx;
} src_build_ctx_t;
...
static void build_src_entry(const char *token, engine_sem_src_entry_t *out) {
    memset(out, 0, sizeof(*out));
    if (!token) {
    ...
    out->is_sparse = SKIP_ONE;
    uint16_t tmp_idx[ENGINE_SEM_SPARSE_NNZE];
    float tmp_val[ENGINE_SEM_SPARSE_NNZE];
    int count = 0;
    uint64_t seed = XXH3_64bits(token, strlen(token));
    for (int i = 0; i < ENGINE_SEM_SPARSE_NNZE; i++) {
        uint64_t h = XXH3_64bits_withSeed(&i, sizeof(i), seed + RI_SEED_BASE);
...
static void src_build_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        int start = atomic_fetch_add_explicit(&sc->next_idx, ENGINE_SEM_WORKER_STACK_CAP,
                                              memory_order_relaxed);
...
static void pass1_quantize_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        int start =
            atomic_fetch_add_explicit(&qc->next_idx, ENGINE_SEM_RRI_TILE, memory_order_relaxed);
        if (start >= qc->entry_count) {
            break;
        }
        int end = start + ENGINE_SEM_RRI_TILE;
        if (end > qc->entry_count) {
            end = qc->entry_count;
        }
        for (int i = start; i < end; i++) {
            const engine_sem_vec_t *src = &qc->entries[i].enriched_vec;
            int8_t *dst = &qc->pass1_q[(size_t)i * ENGINE_SEM_DIM];
            for (int d = 0; d < ENGINE_SEM_DIM; d++) {
                float v = src->v[d] * ENGINE_SEM_INT8_MAX;
                if (v > ENGINE_SEM_INT8_MAX) {
                    v = ENGINE_SEM_INT8_MAX;
                }
                if (v < -ENGINE_SEM_INT8_MAX) {
                    v = -ENGINE_SEM_INT8_MAX;
                }
                dst[d] = (int8_t)(v >= 0 ? v + ENGINE_SEM_IDF_BASE : v - ENGINE_SEM_IDF_BASE);
            }
        }
    }
}
...
static reverse_index_t *build_reverse_index(engine_sem_corpus_t *corpus) {
...
typedef struct {
    ...
    int chunk_size;
    int tile_size;
    engine_parallel_for_opts_t opts;
} finalize_params_t;
...
static void finalize_build_sources(finalize_params_t *p) {
    ...
    engine_parallel_for(p->worker_count, src_build_worker, &sc, p->opts);
}
...
static void finalize_pass2(finalize_params_t *p) {
    ...
    if (pass1_q) {
        ...
        engine_parallel_for(p->worker_count, pass1_quantize_worker, &qc, p->opts);
    }

    engine_sem_vec_t *pass1 = malloc((size_t)p->corpus->entry_count * sizeof(engine_sem_vec_t));
    if (pass1) {
    ...
    if (pass1_q) {
        ...
        atomic_init(&cc.next_chunk, 0);
        engine_parallel_for(p->worker_count, cooccur_worker_int8, &cc, p->opts);
    }
    ...
    if (pass1) {
        ...
        atomic_init(&bc.next_idx, 0);
        engine_parallel_for(p->worker_count, blend_worker, &bc, p->opts);
        free(pass1);
    }
    free(pass1_q);
...
void engine_sem_corpus_finalize(engine_sem_corpus_t *corpus) {
    ...
    int num_chunks = worker_count * ENGINE_SEM_COOCCUR_CHUNK;
    ...
    engine_sem_src_entry_t *src_entries =
        calloc((size_t)corpus->entry_count, sizeof(engine_sem_src_entry_t));
    if (!src_entries) {
    ...
    finalize_params_t params = {
        ...
        .tile_size = ENGINE_SEM_TILE_SIZE,
        .opts = opts,
    };
...
/* Parse a decimal index string via strtol, returning ENGINE_NOT_FOUND on parse
...
static int parse_token_index(const char *idx_str) {
    ...
    char *end = NULL;
    long parsed = strtol(idx_str, &end, BASE_DECIMAL);
    return (end != idx_str) ? (int)parsed : ENGINE_NOT_FOUND;
}

float engine_sem_corpus_idf(const engine_sem_corpus_t *corpus, const char *token) {
    if (!corpus || !token || corpus->doc_count == 0) {
        return 0.0F;
    }
    int idx = parse_token_index(engine_ht_get(corpus->token_map, token));
...
const engine_sem_vec_t *engine_sem_corpus_ri_vec(const engine_sem_corpus_t *corpus, const char *token) {
    if (!corpus || !token) {
        return NULL;
    }
    int idx = parse_token_index(engine_ht_get(corpus->token_map, token));
...
int engine_sem_corpus_doc_count(const engine_sem_corpus_t *corpus) {
    return corpus ? corpus->doc_count : 0;
}

int engine_sem_corpus_token_count(const engine_sem_corpus_t *corpus) {
    return corpus ? corpus->entry_count : 0;
}

const char *engine_sem_corpus_token_at(const engine_sem_corpus_t *corpus, int index,
                                    const engine_sem_vec_t **out_vec, float *out_idf) {
...
void engine_sem_corpus_free(engine_sem_corpus_t *corpus) {
    if (!corpus) {
        return;
    }
    for (int i = 0; i < corpus->entry_count; i++) {
    ...
    if (corpus->token_map) {
        engine_ht_foreach(corpus->token_map, free_ht_kv, NULL);
        engine_ht_free(corpus->token_map);
    }
    free(corpus);
}
...
float engine_sem_proximity(const char *path_a, const char *path_b) {
    if (!path_a || !path_b) {
        return ENGINE_SEM_UNIT_POS;
    }
    ...
    return ENGINE_SEM_UNIT_POS + (ratio * ENGINE_SEM_PROX_MAX_BOOST);
}
...
static float small_cosine(const float *a, const float *b, int dims) {
    ...
    return denom < ENGINE_SEM_DENOM_EPS ? 0.0F : dot / denom;
}
...
static float sparse_tfidf_cosine(const engine_sem_func_t *a, const engine_sem_func_t *b) {
    ...
    float denom = sqrtf(ma) * sqrtf(mb);
    return denom > ENGINE_SEM_DENOM_EPS ? (dot / denom) : 0.0F;
}

float engine_sem_combined_score(const engine_sem_func_t *a, const engine_sem_func_t *b,
                             const engine_sem_config_t *cfg) {
    if (!a || !b || !cfg) {
        return 0.0F;
    }
    ...
    if (a->has_minhash && b->has_minhash) {
        double early_j = engine_minhash_jaccard((const engine_minhash_t *)a->minhash,
                                             (const engine_minhash_t *)b->minhash);
        if (early_j >= ENGINE_MINHASH_JACCARD_THRESHOLD) {
            return 0.0F;
        }
    }
    ...
    score += cfg->w_ri * engine_rsq_ip(&a->ri_code, &b->ri_code);

    /* Signal 3: MinHash Jaccard */
    if (a->has_minhash && b->has_minhash) {
        double j = engine_minhash_jaccard((const engine_minhash_t *)a->minhash,
                                       (const engine_minhash_t *)b->minhash);
        score += cfg->w_minhash * (float)j;
    }

    /* Signal 4: API Signatures */
    score += cfg->w_api * engine_rsq_ip(&a->api_code, &b->api_code);

    /* Signal 5: Type Signatures */
    score += cfg->w_type * engine_rsq_ip(&a->type_code, &b->type_code);

    /* Signal 7: Decorator Pattern */
    score += cfg->w_decorator * engine_rsq_ip(&a->deco_code, &b->deco_code);

    /* Signal 8+9+11: Structural profile + data flow + Halstead */
    float sp_score = small_cosine(a->struct_profile, b->struct_profile, ENGINE_SEM_AST_PROFILE_DIMS);
    ...
    score *= engine_sem_proximity(a->file_path, b->file_path);
    if (score > ENGINE_SEM_UNIT_POS) {
        score = ENGINE_SEM_UNIT_POS;
    }
    if (score < 0.0F) {
        score = 0.0F;
    }
...
void engine_sem_diffuse(engine_sem_vec_t *combined, const engine_sem_vec_t *neighbors, int neighbor_count,
                     float alpha) {
    if (!combined || !neighbors || neighbor_count <= 0) {
        return;
    }
    /* Blend: combined = (1-α) × combined + α × mean(neighbors) */
    engine_sem_vec_t mean;
    ...
    float inv_n = ENGINE_SEM_UNIT_POS / (float)neighbor_count;
    float one_minus_alpha = ENGINE_SEM_UNIT_POS - alpha;
    for (int i = 0; i < ENGINE_SEM_DIM; i++) {
        combined->v[i] = (one_minus_alpha * combined->v[i]) + (alpha * mean.v[i] * inv_n);
    }
    engine_sem_normalize(combined);
}
