...
static float g_rsq_diag[ENGINE_RSQ_DIM];
enum {
...
static void rsq_fwht(float *v) {
    for (int len = 1; len < ENGINE_RSQ_DIM; len <<= 1) {
        for (int i = 0; i < ENGINE_RSQ_DIM; i += len << 1) {
...
void engine_rsq_encode(const float *v, engine_rsq_code_t *out) {
    ...
    float rot[ENGINE_RSQ_DIM];
    for (int d = 0; d < ENGINE_RSQ_IN_DIM; d++) {
        rot[d] = v[d] * g_rsq_diag[d];
    }
    for (int d = ENGINE_RSQ_IN_DIM; d < ENGINE_RSQ_DIM; d++) {
        rot[d] = 0.0F;
    }
    rsq_fwht(rot);
    ...
    /* Per-vector scalar quantization over [lo, hi] at ENGINE_RSQ_BITS. */
    float range = hi - lo;
    float step = range > 0.0F ? range / (float)ENGINE_RSQ_LEVELS : 1.0F;
    out->offset = lo;
    out->scale = step;
...
float engine_rsq_ip(const engine_rsq_code_t *a, const engine_rsq_code_t *b) {
    ...
    int64_t dot = 0;
    for (int i = 0; i < ENGINE_RSQ_CODE_BYTES; i++) {
    ...
    double d = (double)ENGINE_RSQ_DIM;
