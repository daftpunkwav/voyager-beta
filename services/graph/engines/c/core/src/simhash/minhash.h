 ...
 *
 * Workflow:
 *   1. engine_minhash_compute()  — during AST extraction (per function)
 *   2. engine_minhash_jaccard()  — pairwise similarity (K=64 agreement)
 *   3. engine_lsh_*              — locality-sensitive hashing for O(n)
 *                               candidate generation
 */
#ifndef ENGINE_MINHASH_H
#define ENGINE_MINHASH_H
 ...
 * Returns false if the body has fewer than ENGINE_MINHASH_MIN_NODES
...
typedef struct engine_lsh_index engine_lsh_index_t;

/* Entry stored in the LSH index. */
typedef struct {
    int64_t node_id;
    const engine_minhash_t *fingerprint;
