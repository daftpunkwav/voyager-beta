...
typedef struct {
    ...
    engine_subprocess_t *active_git;
    /* Room for Git's 64-hex SHA-256 object ID plus newline/NUL while reading. */
    char last_head[ENGINE_SZ_128]; /* git HEAD hash (committed baseline) */
    ...
    uint64_t first_missing_ms;  /* engine_now_ms() of the streak's first miss (0 = no streak) */
    ...
    char pending_head[ENGINE_SZ_128]; /* HEAD observed at check time */
} project_state_t;
...
struct engine_watcher {
    ...
    engine_index_fn index_fn;
    void *user_data;
    EngineHashTable *projects; /* name → project_state_t* */
    engine_mutex_t projects_lock;
    ...
    engine_mutex_t coordination_lock;
    engine_watcher_project_mutation_begin_fn mutation_begin;
    engine_watcher_project_mutation_end_fn mutation_end;
    engine_watcher_project_pruned_fn project_pruned;
...
#define PRUNE_GRACE_DEFAULT_S 600 /* 10 min; override: ENGINE_WATCHER_PRUNE_GRACE_S */
...
int engine_watcher_poll_interval_ms(int file_count) {
    int ms = POLL_BASE_MS + ((file_count / POLL_FILE_STEP) * ENGINE_MSEC_PER_SEC);
    if (ms > POLL_MAX_MS) {
        ms = POLL_MAX_MS;
    }
    return ms;
...
static void watcher_git_output_cleanup(watcher_git_output_t *output) {
    if (output && output->path[0]) {
        (void)engine_unlink(output->path);
        output->path[0] = '\0';
        output->size = 0;
    }
}
...
static bool watcher_git_output_create(watcher_git_output_t *output) {
    ...
    int written =
        snprintf(output->path, sizeof(output->path), "%s/engine-watcher-git-XXXXXX", engine_tmpdir());
    if (written <= 0 || written >= (int)sizeof(output->path)) {
        output->path[0] = '\0';
        return false;
    }
    int descriptor = engine_mkstemp(output->path);
    if (descriptor < 0) {
        output->path[0] = '\0';
        return false;
    }
...
static bool watcher_windows_git_candidate(const wchar_t *entry, size_t entry_length,
    ...
    if (entry_length == 0U || entry_length >= ENGINE_SZ_4K) {
        return false;
    }
    wchar_t directory[ENGINE_SZ_4K];
    ...
    wchar_t candidate[ENGINE_SZ_4K];
    int written =
        swprintf(candidate, ENGINE_SZ_4K, separator ? L"%lsgit.exe" : L"%ls\\git.exe", directory);
    if (written <= 0 || written >= ENGINE_SZ_4K) {
        return false;
    }
    ...
    char *utf8 = regular ? engine_wide_to_utf8(normalized) : NULL;
    size_t utf8_length = utf8 ? strlen(utf8) : 0U;
    bool valid = utf8 && utf8_length > 0U && utf8_length < ENGINE_SZ_4K;
    if (valid) {
...
static bool watcher_resolve_git_executable(char output[ENGINE_SZ_4K]) {
    output[0] = '\0';
...
static watcher_git_status_t watcher_git_run(engine_watcher_t *w, project_state_t *state,
...
#ifdef _WIN32
    char git_executable[ENGINE_SZ_4K];
    ...
    engine_mutex_lock(&w->projects_lock);
    if (state->active_git) {
        publication_conflict = true;
        cancelled = engine_subprocess_request_cancel(process);
    } else {
    ...
    engine_mutex_unlock(&w->projects_lock);

    uint64_t started_at = engine_now_ms();
    engine_proc_result_t result = {0};
    for (;;) {
        ...
        if (!cancelled && output_limit > 0) {
            int64_t observed_size = engine_file_size(output->path);
            if (observed_size > 0 && (uint64_t)observed_size > output_limit) {
                output_exceeded = true;
                cancelled = engine_subprocess_request_cancel(process);
            }
        }
        engine_proc_poll_t polled = engine_subprocess_poll(process, &result);
        if (polled == ENGINE_PROC_POLL_TERMINAL) {
            break;
        }
        if (polled == ENGINE_PROC_POLL_ERROR) {
            /* Valid owned handles have no ERROR transition. Fail closed, but
             * continue polling so the contained tree is never abandoned. */
            cancelled = engine_subprocess_request_cancel(process) || cancelled;
        }
        engine_usleep(WATCHER_GIT_POLL_US);
    }
    ...
    } else if (result.outcome != ENGINE_PROC_CLEAN) {
        status = WATCHER_GIT_COMMAND_FAILED;
    }

    if (status == WATCHER_GIT_OK && output_limit > 0) {
        int64_t final_size = engine_file_size(output->path);
        if (final_size < 0) {
            status = WATCHER_GIT_SUPERVISION_FAILED;
    ...
    if (status == WATCHER_GIT_DEADLINE || status == WATCHER_GIT_OUTPUT_LIMIT ||
        status == WATCHER_GIT_SUPERVISION_FAILED) {
        engine_log_warn("watcher.git.failed", "project", state->project_name, "reason",
                     status == WATCHER_GIT_DEADLINE
                         ? "deadline"
...
static watcher_git_status_t git_repo_status(engine_watcher_t *w, project_state_t *state) {
    const char *argv[] = {"git", "-C", state->root_path, "rev-parse", "--git-dir", NULL};
    return watcher_git_run(w, state, argv, 0, NULL);
}

static watcher_git_status_t git_head(engine_watcher_t *w, project_state_t *state, char *out,
                                     size_t out_size) {
    if (!out || out_size < 2) {
        return WATCHER_GIT_SUPERVISION_FAILED;
    }
    ...
    FILE *file = engine_fopen(output.path, "rb");
...
static watcher_git_status_t git_dirty_signature(engine_watcher_t *w, project_state_t *state,
    ...
    *signature_out = 0;
    ...
    FILE *fp = engine_fopen(output.path, "rb");
    if (!fp) {
        watcher_git_output_cleanup(&output);
        return WATCHER_GIT_SUPERVISION_FAILED;
    }
    ...
    if (submodule_status == WATCHER_GIT_OK) {
        fp = engine_fopen(output.path, "rb");
        if (!fp) {
            watcher_git_output_cleanup(&output);
            return WATCHER_GIT_SUPERVISION_FAILED;
        }
...
static watcher_git_status_t git_file_count(engine_watcher_t *w, project_state_t *state,
                                           int *count_out) {
    if (!count_out) {
        return WATCHER_GIT_SUPERVISION_FAILED;
    }
...
static project_state_t *state_new(const char *name, const char *root_path) {
    project_state_t *s = calloc(ENGINE_ALLOC_ONE, sizeof(*s));
    if (!s) {
        return NULL;
    }
...
static bool defer_state_free(engine_watcher_t *w, project_state_t *s) {
    if (w->pending_free_count >= w->pending_free_cap) {
        ...
        if (!tmp) {
            engine_log_warn("watcher.unwatch.oom", "project", s->project_name);
            return false;
        }
        w->pending_free = tmp;
...
bool engine_watcher_root_missing_errno(int err) {
...
static root_status_t root_status(const char *root_path, int *out_errno) {
    ...
    *out_errno = errno;
    return engine_watcher_root_missing_errno(errno) ? ROOT_MISSING : ROOT_UNCERTAIN;
}

/* Sustained-absence window (seconds) before a missing root may be pruned.
 * Generous default: 10 minutes. Override with ENGINE_WATCHER_PRUNE_GRACE_S
 ...
 * same convention as engine_max_file_bytes in limits.c. */
static long prune_grace_s(void) {
    const char *raw = getenv("ENGINE_WATCHER_PRUNE_GRACE_S");
    if (raw && raw[0]) {
        errno = 0;
        char *end = NULL;
...
static const char *itoa_buf(int v) {
    static ENGINE_TLS char buf[ENGINE_SZ_32];
...
static bool watcher_unlink_cached_file(const char *project_name, const char *path,
    ...
    char errno_text[ENGINE_SZ_32];
    snprintf(errno_text, sizeof(errno_text), "%d", unlink_errno);
    engine_log_warn("watcher.root_prune_delete_failed", "project", project_name, "artifact", artifact,
                 "path", path, "errno", errno_text);
    return false;
}

static bool delete_cached_project_db(const char *project_name) {
    if (!engine_validate_project_name(project_name)) {
        return false;
    }
...
engine_watcher_t *engine_watcher_new(engine_store_t *store, engine_index_fn index_fn, void *user_data) {
    engine_watcher_t *w = calloc(ENGINE_ALLOC_ONE, sizeof(*w));
    if (!w) {
        return NULL;
    }
    w->store = store;
    ...
    w->projects = engine_ht_create(ENGINE_SZ_32);
    if (!w->projects) {
        free(w);
        return NULL;
    }
    engine_mutex_init(&w->projects_lock);
    engine_mutex_init(&w->coordination_lock);
    atomic_init(&w->stopped, 0);
    return w;
}

void engine_watcher_free(engine_watcher_t *w) {
    if (!w) {
        return;
    }
    /* Safety net: ensure stopped is set before draining pending_free.
     * In production the caller should engine_watcher_stop() + join first. */
    atomic_store(&w->stopped, 1);
    engine_mutex_lock(&w->coordination_lock);
    engine_mutex_lock(&w->projects_lock);
    engine_ht_foreach(w->projects, free_state_entry, NULL);
    engine_ht_free(w->projects);
    for (int i = 0; i < w->pending_free_count; i++) {
        state_free(w->pending_free[i]);
    }
    free(w->pending_free);
    engine_mutex_unlock(&w->projects_lock);
    engine_mutex_unlock(&w->coordination_lock);
    engine_mutex_destroy(&w->projects_lock);
    engine_mutex_destroy(&w->coordination_lock);
    free(w);
}

void engine_watcher_set_project_mutation_guard(engine_watcher_t *w,
                                            engine_watcher_project_mutation_begin_fn begin,
                                            engine_watcher_project_mutation_end_fn end,
                                            engine_watcher_project_pruned_fn pruned, void *context) {
    if (!w) {
        return;
    }
    engine_mutex_lock(&w->coordination_lock);
...
bool engine_watcher_watch(engine_watcher_t *w, const char *project_name, const char *root_path) {
    ...
    if (!s) {
        engine_log_warn("watcher.watch.oom", "project", project_name, "path", root_path);
        return false;
    }
    ...
    project_state_t *old = engine_ht_get(w->projects, project_name);
    ...
    if (old) {
        ...
        engine_ht_delete(w->projects, project_name);
    }
    engine_ht_set(w->projects, s->project_name, s);
    bool registered = engine_ht_get(w->projects, project_name) == s;
    if (!registered && old) {
        ...
        engine_ht_set(w->projects, old->project_name, old);
        if (engine_ht_get(w->projects, project_name) == old) {
            w->pending_free[--w->pending_free_count] = NULL;
        } else {
            atomic_store_explicit(&old->registered, false, memory_order_release);
            if (old->active_git) {
                (void)engine_subprocess_request_cancel(old->active_git);
            }
        }
    }
    if (registered && old) {
    ...
    if (!registered) {
        state_free(s);
        engine_log_warn("watcher.watch.failed", "project", project_name, "reason", "registration");
        return false;
    }
    engine_log_info("watcher.watch", "project", project_name, "path", root_path);
    return true;
}

void engine_watcher_unwatch(engine_watcher_t *w, const char *project_name) {
    if (!w || !project_name) {
        return;
    }
    bool removed = false;
    engine_mutex_lock(&w->projects_lock);
    project_state_t *s = engine_ht_get(w->projects, project_name);
    if (s && defer_state_free(w, s)) {
        ...
        if (s->active_git) {
            (void)engine_subprocess_request_cancel(s->active_git);
        }
        engine_ht_delete(w->projects, project_name);
        removed = true;
    }
    engine_mutex_unlock(&w->projects_lock);
    if (removed) {
        engine_log_info("watcher.unwatch", "project", project_name);
    }
}

void engine_watcher_touch(engine_watcher_t *w, const char *project_name) {
    if (!w || !project_name) {
        return;
    }
    engine_mutex_lock(&w->projects_lock);
...
int engine_watcher_watch_count(engine_watcher_t *w) {
    if (!w) {
        return 0;
    }
    engine_mutex_lock(&w->projects_lock);
    int count = (int)engine_ht_count(w->projects);
    engine_mutex_unlock(&w->projects_lock);
    return count;
}
...
static bool init_baseline(engine_watcher_t *w, project_state_t *s) {
    struct stat st;
    if (stat(s->root_path, &st) != 0) {
        engine_log_warn("watcher.root_gone", "project", s->project_name, "path", s->root_path);
    ...
    if (s->is_git) {
        ...
        s->interval_ms = engine_watcher_poll_interval_ms(s->file_count);
        engine_log_info("watcher.baseline", "project", s->project_name, "strategy", "git", "files",
                     s->file_count > 0 ? "yes" : "0");
    } else {
        engine_log_info("watcher.baseline", "project", s->project_name, "strategy", "none");
    }
...
static bool check_changes(engine_watcher_t *w, project_state_t *s, bool *changed_out) {
    if (!changed_out) {
        return false;
    }
    *changed_out = false;
...
static void prune_missing_project(engine_watcher_t *w, project_state_t *s) {
    if (!w || !s || !s->project_name) {
        return;
    }

    char *project_name = engine_strdup(s->project_name);
    char *root_path = engine_strdup(s->root_path);
    ...
    engine_mutex_lock(&w->projects_lock);
    project_state_t *current = engine_ht_get(w->projects, project_name);
    /* Deferred free (same discipline as engine_watcher_unwatch): this state
    ...
    bool still_eligible =
        ...
        now_ms - current->first_missing_ms >= (uint64_t)prune_grace_s() * ENGINE_MSEC_PER_SEC &&
    ...
    if (removed) {
        engine_log_info("watcher.root_pruned", "project", project_name);
    }
    free(project_name);
    free(root_path);
}
...
static void poll_project(const char *key, void *val, void *ud) {
    ...
    if (rs == ROOT_UNCERTAIN) {
        ...
        engine_log_warn("watcher.root_stat_error", "project", s->project_name, "path", s->root_path,
    ...
    if (rs == ROOT_MISSING) {
        ...
        engine_log_warn("watcher.root_missing", "project", s->project_name, "path", s->root_path,
                     "polls", itoa_buf(s->missing_root_count));
        if (s->missing_root_count >= MISSING_ROOT_DELETE_AFTER &&
            now_ms - s->first_missing_ms >= (uint64_t)prune_grace_s() * ENGINE_MSEC_PER_SEC) {
            prune_missing_project(ctx->w, s);
        }
        return;
    }
    if (s->missing_root_count > 0) {
        engine_log_info("watcher.root_restored", "project", s->project_name, "path", s->root_path);
        s->missing_root_count = 0;
        s->first_missing_ms = 0;
    }
    ...
    engine_log_info("watcher.changed", "project", s->project_name, "strategy", "git");
    if (ctx->w->index_fn) {
        ...
        if (rc == 0) {
            ...
            engine_log_info("watcher.index.retry", "project", s->project_name);
        } else {
            engine_log_warn("watcher.index.err", "project", s->project_name);
        }
    }
...
int engine_watcher_poll_once(engine_watcher_t *w) {
    if (!w) {
        return 0;
    }
    ...
    int n = engine_ht_count(w->projects);
    if (n == 0) {
        engine_mutex_unlock(&w->projects_lock);
        return 0;
    }
    ...
    engine_ht_foreach(w->projects, snapshot_project, &sc);
    engine_mutex_unlock(&w->projects_lock);
...
static void cancel_active_git_entry(const char *key, void *value, void *user_data) {
    ...
    if (state && state->active_git) {
        (void)engine_subprocess_request_cancel(state->active_git);
    }
}

void engine_watcher_stop(engine_watcher_t *w) {
    if (w) {
        atomic_store_explicit(&w->stopped, 1, memory_order_release);
        engine_mutex_lock(&w->projects_lock);
        engine_ht_foreach(w->projects, cancel_active_git_entry, NULL);
        for (int i = 0; i < w->pending_free_count; i++) {
...
int engine_watcher_run(engine_watcher_t *w, int base_interval_ms) {
    if (!w) {
    ...
    engine_log_info("watcher.start", "interval_ms", base_interval_ms > 999 ? "multi-sec" : "fast");

    while (!atomic_load(&w->stopped)) {
        engine_watcher_poll_once(w);
        ...
        while (slept < base_interval_ms && !atomic_load(&w->stopped)) {
            ...
            engine_usleep((unsigned)chunk * ENGINE_MSEC_PER_SEC);
            slept += chunk;
        }
    }

    engine_log_info("watcher.stop");
    return 0;
}
