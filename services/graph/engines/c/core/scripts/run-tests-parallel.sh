...
#   jobs defaults to ENGINE_TEST_PAR_JOBS, then the CPU count.

set -uo pipefail

RUNNER="${1:?usage: run-tests-parallel.sh <path-to-test-runner> [jobs]}"
JOBS="${2:-${ENGINE_TEST_PAR_JOBS:-}}"
...
# ENGINE_TEST_SHARD="i/N" runs this invocation's deterministic slice of the
...
SHARD_INDEX=1
SHARD_TOTAL=1
if [ -n "${ENGINE_TEST_SHARD:-}" ]; then
    if ! printf '%s' "$ENGINE_TEST_SHARD" | grep -qE '^[0-9]+/[0-9]+$'; then
        echo "FAIL: ENGINE_TEST_SHARD must be i/N, got '$ENGINE_TEST_SHARD'" >&2
        exit 1
    fi
    SHARD_INDEX="${ENGINE_TEST_SHARD%%/*}"
    SHARD_TOTAL="${ENGINE_TEST_SHARD##*/}"
    if [ "$SHARD_TOTAL" -lt 1 ] || [ "$SHARD_INDEX" -lt 1 ] ||
        [ "$SHARD_INDEX" -gt "$SHARD_TOTAL" ]; then
        echo "FAIL: ENGINE_TEST_SHARD out of range: $ENGINE_TEST_SHARD" >&2
        exit 1
    fi
fi
...
# against a mis-plumbed ENGINE_TEST_SHARD (two jobs running the same slice
...
{
    echo "leg=${ENGINE_TEST_LEG:-local}"
...
run_wave() {
    ...
    if ! python3 "$SCHEDULER" \
        ...
        --jobs "$jobs" \
        --timeout "${ENGINE_SUITE_TIMEOUT:-900}" \
        --slow-timeout "${ENGINE_SUITE_TIMEOUT_SLOW:-3600}" \
        --kill-grace 15 \
        "$RUNNER"; then
...
# so a small fixed overlap (ENGINE_TAIL_JOBS, default 2) is safe and converts
...
run_wave "$FLEX_FILE" "${ENGINE_TAIL_JOBS:-2}"
