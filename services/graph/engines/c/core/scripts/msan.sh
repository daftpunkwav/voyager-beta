...
#
# Runs inside the engine-msan image (test-infrastructure/Dockerfile.msan), which
...
# Makefile): zstd's MSan block needs stdint.h that its amalgamation lost,
...
make -f Makefile clean-c BUILD_DIR=build/msan >/dev/null 2>&1 || true

make -j"$(nproc)" -f Makefile build/msan/test-runner \
...
# engine_thread_create) — see the exclusion note below for what this does NOT fix.
ulimit -s 262144 2>/dev/null || true
export ENGINE_THREAD_STACK_MB="${ENGINE_THREAD_STACK_MB:-256}"
...
#  2. ENGINE_THREAD_STACK_MB at 256 MiB and at 1024 MiB (the cap). The knob is
#     verified compiled in (ENGINE_SANITIZED_BUILD is defined for this lane) and
#     the floor is applied in engine_thread_create for both the default and
...
#  5. ENGINE_WORKERS=1, to push the recursion onto the main thread where the
