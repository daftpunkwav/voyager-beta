...
Makefile lint targets `make lint`/`make lint-ci` run, plus the no-skips
...
if $CI_ONLY; then
    ...
    make -j2 -f Makefile lint-ci "${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}"
    echo "=== CI linters passed ==="
else
    echo "=== Full lint: clang-tidy + cppcheck + clang-format ==="
    make -j3 -f Makefile lint "${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}"
fi
