...
for arg in "$@"; do
    case "$arg" in
        arm64|x86_64) [[ "$prev_arg" == "--arch" ]] && export ENGINE_ARCH="$arg" ;;
...
OUT="$(mktemp "${TMPDIR:-/tmp}/engine-repro-out.XXXXXX")"
if [[ -z "$OUT" ]]; then
...
set +e
make -j"$NPROC" -f Makefile test-repro $MAKE_ARGS 2>&1 | tee "$OUT"
set -e
