<#
.SYNOPSIS
Sweep engine test residue from a Windows test host and assert a runner-like free
...
Sweeps only engine-prefixed directories under the profile and TEMP. The checkout
(C:\engine) is not under either, and is never touched.
...
# non-representable characters, deep .engine-retired/generations nesting - whose
...
foreach ($root in $roots) {
    ...
    Get-ChildItem -LiteralPath $root -Directory -Filter 'engine-*' -ErrorAction SilentlyContinue |
        ForEach-Object {
