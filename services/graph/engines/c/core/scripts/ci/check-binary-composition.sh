...
SEAM_NEEDLES=(
    'ENGINE_TEST_WORKER_DESCENDANT_PID_FILE'
    'ENGINE_TEST_CRASH_ON'
    'ENGINE_TEST_HANG_ON'
)
...
# engine-integrations.json (the binary embeds only its SHA-256); these needles
...
check_file() {
    ...
    # "graph-engine", so the parent directory is what tells them apart.
    ...
    standard) is_ui=0 ;;
    auto)
        # The UI archive is graph-engine-ui-<os>-<arch>, but the binary
        # inside it is just "graph-engine" — so the whole path decides,
    ...
    # Sweep for seams nobody thought to pin: a new ENGINE_TEST_* env var added to
    ...
    seam_allowed='ENGINE_TEST_WINDOWS_USER_PATH_RUN_ID'
    seam_unexpected=''
    for found in $(LC_ALL=C grep -a -o -E 'ENGINE_TEST_[A-Za-z0-9_]+' "$file" 2>/dev/null |
        sort -u || true); do
    ...
    if [ -n "$seam_unexpected" ]; then
        report FAIL A2-no-test-seams "$token" \
            "unexpected ENGINE_TEST_* seam(s):$seam_unexpected (allowlist: $seam_allowed)"
    else
        report PASS A2-no-test-seams "$token" 'no ENGINE_TEST_* seams beyond the smoke allowlist'
    fi
    ...
    # ENGINE_CHECK_UI_ABSENT=1 is available for any future true no-UI composition.
    if [ "$is_ui" -eq 1 ]; then
        printf 'n/a  %-22s %s: UI artifact, HTTP server ships here by design\n' \
            A5-no-ui-http "$token"
    elif [ "${ENGINE_CHECK_UI_ABSENT:-0}" = "1" ]; then
        ...
        printf 'INFO %-22s %s: %d/%d UI/HTTP needles present (temporary standard variant; set ENGINE_CHECK_UI_ABSENT=1 to enforce a true no-UI composition)\n' \
