...
#   (ENGINE_SMOKE_ARTIFACT_DIR), whose completeness checks make a broken or
...
artifact through the canonical wrapper, exactly like the release venue:
  unix:    scripts/smoke-local.sh with ENGINE_SMOKE_ARTIFACT_DIR
  windows: test-infrastructure/vm/vm-smoke.sh with ENGINE_SMOKE_ARTIFACT_DIR
...
if [ "$GOOS" = "darwin" ]; then
    codesign --sign - --force "$BUILD_DIR/graph-engine"
fi

WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/engine-smoke-artifact.XXXXXX")"
cleanup() {
    local status=$?
    trap - EXIT
    if [ "$status" -eq 0 ]; then
