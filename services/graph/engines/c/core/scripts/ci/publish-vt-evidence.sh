...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/engine-vt-public.XXXXXX")"
...
publish_copy "$VT_ASSOCIATIONS" engine-release-scan-associations-v3 virustotal-associations.tsv
publish_copy "$VT_EXPECTED_SCAN_SET" engine-release-scan-set-v2 virustotal-scan-set.tsv
publish_copy "$VT_RESULTS_PATH" engine-virustotal-results-v1 virustotal-results.tsv
