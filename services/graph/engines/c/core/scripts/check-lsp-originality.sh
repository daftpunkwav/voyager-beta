...
#
# WHY THIS EXISTS
#   The LSP layer (internal/engine/lsp/) is an original C implementation whose
...
TAG="[lsp-orig]"
LSP_DIR="$ROOT/internal/engine/lsp"
...
if [ "$total_hits" -gt 0 ] || [ "$structural_hits" -gt 0 ]; then
  if [ "$total_hits" -gt 0 ]; then
    ...
    echo "$TAG with internal/engine/lsp/. A hit is NOT proof of copying (common phrases collide)"
