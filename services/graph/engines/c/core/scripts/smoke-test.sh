...
shopt -s nullglob
SMOKE_UI_PACK_CANDIDATES=("$(dirname "$BINARY")"/engine-ui-*.pack)
shopt -u nullglob
for candidate in "${SMOKE_UI_PACK_CANDIDATES[@]+"${SMOKE_UI_PACK_CANDIDATES[@]}"}"; do
  ...
  if ! [[ "$candidate_name" =~ ^engine-ui-[0-9a-f]{64}\.pack$ ]]; then
...
smoke_mktemp_file() {
  if [ -n "${SMOKE_TEMP_ROOT:-}" ]; then
    mktemp "$SMOKE_TEMP_ROOT/engine-smoke.XXXXXX"
  else
    mktemp
  fi
}
...
smoke_mktemp_dir() {
  if [ -n "${SMOKE_TEMP_ROOT:-}" ]; then
    mktemp -d "$SMOKE_TEMP_ROOT/engine-smoke.XXXXXX"
  else
    mktemp -d
  fi
}
...
#                     engine_mem_init is decoration and freed pages stay committed
...
SC=$(cli search_code --project "$PROJECT" --pattern engine_ --mode compact --limit 5)
...
SCW=$(cli search_code --project "$PROJECT" --pattern "engine_init|engine_nope" --regex false --limit 5)
...
SCA=$(cli search_code --project "$PROJECT" --pattern engine_ --file-pattern "*R&D*.c" --limit 5)
case "$SCA" in
...
if command -v pgrep &>/dev/null && [ "$(uname)" != "MINGW64_NT" ] 2>/dev/null; then
  ...
  sleep 1
  RESIDUAL=$(pgrep -f "graph-engine.*cli" 2>/dev/null | wc -l | tr -d ' \n' || echo "0")
  RESIDUAL="${RESIDUAL:-0}"
  if [ "$RESIDUAL" -gt 0 ]; then
    echo "WARNING: $RESIDUAL residual graph-engine process(es) found"
  else
...
mcp_dump_diagnostics() {
  ...
  local cache="${ENGINE_CACHE_DIR:-$HOME/.cache/graph-engine}"
  for log in engine-daemon.log daemon-conflicts.ndjson; do
...
DRYRUN_CACHE="$DRYRUN_HOME/.cache/graph-engine"
...
if [[ "$BINARY" == *.exe ]] &&
   [ -n "${ENGINE_TEST_WINDOWS_USER_PATH_RUN_ID:-}" ]; then
  if INVALID_PATH_OUT=$(
    ENGINE_TEST_WINDOWS_USER_PATH_RUN_ID=invalid \
...
copy_smoke_binary "$INSTALL_DIR/graph-engine"
chmod 755 "$INSTALL_DIR/graph-engine"

# Verify installed binary works
INSTALLED_VER=$("$INSTALL_DIR/graph-engine" --version 2>&1)
...
copy_smoke_binary "$REPLACE_DIR/smoke-graph-engine"

# 3. Simulate engine_replace_binary: unlink old, copy new
rm -f "$INSTALL_DIR/graph-engine"
cp "$REPLACE_DIR/smoke-graph-engine" "$INSTALL_DIR/graph-engine"
chmod 755 "$INSTALL_DIR/graph-engine"

# 4. Verify replaced binary works
REPLACED_VER=$("$INSTALL_DIR/graph-engine" --version 2>&1)
...
# 5. Test replacement of read-only binary (edge case — engine_replace_binary
#    handles this via unlink-before-write, which works even on read-only files)
chmod 444 "$INSTALL_DIR/graph-engine"
rm -f "$INSTALL_DIR/graph-engine"
cp "$REPLACE_DIR/smoke-graph-engine" "$INSTALL_DIR/graph-engine"
chmod 755 "$INSTALL_DIR/graph-engine"
READONLY_VER=$("$INSTALL_DIR/graph-engine" --version 2>&1)
...
DEVIN_SKILL="$DEVIN_DIR/skills/graph-engine/SKILL.md"
CODEBUDDY_MCP="$FAKE_HOME/.codebuddy/.mcp.json"
CODEBUDDY_INSTRUCTIONS="$FAKE_HOME/.codebuddy/CODEBUDDY.md"
CODEBUDDY_SKILL="$FAKE_HOME/.codebuddy/skills/graph-engine/SKILL.md"
CODEBUDDY_AGENT="$FAKE_HOME/.codebuddy/agents/graph-engine.md"
...
BOB_RULE="$FAKE_HOME/.bob/rules/graph-engine.md"
BOB_SKILL="$FAKE_HOME/.bob/skills/graph-engine/SKILL.md"
BOB_AGENT="$FAKE_HOME/.bob/agents/graph-engine.md"
POCHI_MCP="$FAKE_HOME/.pochi/config.jsonc"
POCHI_INSTRUCTIONS="$FAKE_HOME/.pochi/README.pochi.md"
POCHI_SKILL="$FAKE_HOME/.pochi/skills/graph-engine/SKILL.md"
POCHI_AGENT="$FAKE_HOME/.pochi/agents/graph-engine.md"
ROVO_MCP="$FAKE_HOME/.rovodev/mcp.json"
ROVO_INSTRUCTIONS="$FAKE_HOME/.rovodev/AGENTS.md"
ROVO_SKILL="$FAKE_HOME/.rovodev/skills/graph-engine/SKILL.md"
ROVO_AGENT="$FAKE_HOME/.rovodev/subagents/graph-engine.md"
...
if [[ "$BINARY" == *.exe ]]; then
  SELF_PATH="$FAKE_HOME/.local/bin/graph-engine.exe"
else
  cp "$BINARY" "$FAKE_HOME/.local/bin/graph-engine"
  SELF_PATH="$FAKE_HOME/.local/bin/graph-engine"
fi
create_agent_stub() {
  local name="$1"
  if [[ "$BINARY" == *.exe ]]; then
...
# Run install — override platform config dirs so engine_app_config_dir() and
# engine_app_local_dir() resolve to FAKE_HOME paths on all platforms.
PHASE8_INSTALL_RC=0
...
HOME="$FAKE_HOME" \
  ...
  ENGINE_ROO_CONFIG_PATH="$ROO_CFG" \
...
if [[ "$BINARY" == *.exe ]]; then
  ...
  PHASE8_CANONICAL="$FAKE_HOME/.local/bin/graph-engine.exe"
  ...
  if [ -e "$FAKE_HOME/.local/bin/graph-engine.payload.exe" ]; then
...
assert_tier_profile_set() {
  ...
  for spec in \
    "graph-engine-scout|Tier 1" \
    "graph-engine|Tier 2" \
    "graph-engine-auditor|Tier 3"; do
    slug=${spec%%|*}
    tier=${spec##*|}
    ...
    if [ "$access" = "direct" ]; then
      if ! grep -Fq 'source read/grep fallback' "$file" 2>/dev/null; then
        ...
        exit 1
         ...
         grep -qE '(mcpServers|mcp__graph-engine__|mcp_graph-engine_|@graph-engine/|graph-engine/)' "$file" 2>/dev/null; then
...
assert_tier_profile_set_removed() {
  ...
  local suffix="$3"
  local slug file
  for slug in graph-engine-scout graph-engine graph-engine-auditor; do
    file="$directory/$slug$suffix"
    if [ -e "$file" ]; then
...
CMD=$(json_get "$FAKE_HOME/.claude.json" "d.get('mcpServers',{}).get('graph-engine',{}).get('command','')")
...
sys.exit(0 if 'graph-engine' in d.get('mcpServers', {}) else 1)
" 2>/dev/null; then
...
CLAUDE_AGENT="$FAKE_HOME/.claude/agents/graph-engine.md"
if ! grep -q '^mcpServers: \[graph-engine\]$' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__graph-engine__search_graph' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__graph-engine__check_index_coverage' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q '^permissionMode: plan$' "$CLAUDE_AGENT" 2>/dev/null ||
   grep -qE 'mcp__graph-engine__(index_repository|delete_project|manage_adr|ingest_traces)' "$CLAUDE_AGENT" 2>/dev/null; then
...
case "$(uname -s)" in
  MINGW*|MSYS*|CYGWIN*)
    GATE_SCRIPT="$FAKE_HOME/.claude/hooks/engine-code-discovery-gate.cmd"
    if [ ! -f "$GATE_SCRIPT" ]; then
      echo "FAIL 8e: .cmd shim missing on Windows"
      exit 1
    fi
    if [ -f "$FAKE_HOME/.claude/hooks/engine-code-discovery-gate" ]; then
      echo "FAIL 8e: legacy extensionless shim still installed on Windows"
      exit 1
    fi
    ;;
  *)
    GATE_SCRIPT="$FAKE_HOME/.claude/hooks/engine-code-discovery-gate"
...
# 8f-8h: Codex TOML
if ! grep -q '\[mcp_servers.graph-engine\]' "$FAKE_HOME/.codex/config.toml"; then
...
if [ ! -f "$FAKE_HOME/.codex/AGENTS.md" ] || ! grep -q 'graph-engine' "$FAKE_HOME/.codex/AGENTS.md"; then
...
CMD=$(json_get "$FAKE_HOME/.gemini/settings.json" "d['mcpServers']['graph-engine']['command']")
...
GEMINI_AGENT="$FAKE_HOME/.gemini/agents/graph-engine.md"
if ! grep -q '^name: graph-engine$' "$GEMINI_AGENT" 2>/dev/null ||
   ...
   ! grep -q 'mcp_graph-engine_search_graph' "$GEMINI_AGENT" 2>/dev/null ||
   ! grep -q 'mcp_graph-engine_check_index_coverage' "$GEMINI_AGENT" 2>/dev/null ||
   grep -qE 'mcp_graph-engine_(index_repository|delete_project|manage_adr|ingest_traces)' "$GEMINI_AGENT" 2>/dev/null; then
...
if [ -f "$ZED_CFG" ]; then
  CMD=$(json_get "$ZED_CFG" "d['context_servers']['graph-engine']['command']")
  if ! path_match "$CMD" "$SELF_PATH"; then
...
CMD=$(json_get "$FAKE_HOME/.config/opencode/opencode.json" "d['mcp']['graph-engine']['command'][0]")
if [ -n "$CMD" ]; then
  if ! path_match "$CMD" "$SELF_PATH"; then
...
CMD=$(json_get "$FAKE_HOME/.gemini/config/mcp_config.json" "d['mcpServers']['graph-engine']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
if [ -f "$FAKE_HOME/CONVENTIONS.md" ]; then
  if ! grep -q 'graph-engine' "$FAKE_HOME/CONVENTIONS.md"; then
...
CMD=$(json_get "$KILO_CFG" "d['mcp']['graph-engine']['command'][0]")
if ! path_match "$CMD" "$SELF_PATH"; then
...
KILO_RULE="$FAKE_HOME/.config/kilo/rules/graph-engine.md"
if [ ! -f "$KILO_RULE" ]; then
...
KILO_AGENT="$FAKE_HOME/.config/kilo/agents/graph-engine.md"
if ! grep -q '^mode: subagent$' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"*": deny' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"graph-engine_search_graph": allow' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"graph-engine_get_code_snippet": allow' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"graph-engine_check_index_coverage": allow' "$KILO_AGENT" 2>/dev/null ||
   grep -Fq '"graph-engine_*": allow' "$KILO_AGENT" 2>/dev/null ||
   grep -qE 'graph-engine_(index_repository|delete_project|manage_adr|ingest_traces)' "$KILO_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$VSCODE_CFG" "d['servers']['graph-engine']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8v: VS Code command='$CMD'"
  exit 1
fi
echo "OK 8v: VS Code MCP"
CMD=$(json_get "$VSCODE_PROFILE_CFG" "d['servers']['graph-engine']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
# 8w: OpenClaw MCP
CMD=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "d['mcp']['servers']['graph-engine']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8w: OpenClaw command='$CMD'"
  exit 1
fi
ENABLED=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "d['mcp']['servers']['graph-engine'].get('enabled')")
...
for OPENCLAW_CONTEXT in \
  ...
  if ! grep -q '^## Codebase Knowledge Graph (graph-engine)$' "$OPENCLAW_CONTEXT" 2>/dev/null ||
     ! grep -q 'subagent' "$OPENCLAW_CONTEXT" 2>/dev/null; then
    echo "FAIL 8w-i: OpenClaw durable context missing in $OPENCLAW_CONTEXT"
    exit 1
  fi
done
OPENCLAW_COMPACTION=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "str('Codebase Knowledge Graph (graph-engine)' in d['agents']['defaults']['compaction']['postCompactionSections'])")
...
CMD=$(json_get "$FAKE_HOME/.kiro/settings/mcp.json" "d['mcpServers']['graph-engine']['command']")
KIRO_AGENT="$FAKE_HOME/.kiro/agents/graph-engine.json"
KIRO_AGENT_CMD=$(json_get "$KIRO_AGENT" "d['mcpServers']['graph-engine']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! path_match "$KIRO_AGENT_CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$FAKE_HOME/.kiro/steering/graph-engine.md" 2>/dev/null ||
   ! cat "$KIRO_AGENT" 2>/dev/null | python3 -c "
import json, sys
d = json.load(sys.stdin)
tools = d.get('tools', [])
server = d.get('mcpServers', {}).get('graph-engine', {})
ok = (d.get('name') == 'graph-engine' and tools[:3] == ['read', 'grep', 'glob'] and
      '@graph-engine/search_graph' in tools and
      '@graph-engine/check_index_coverage' in tools and
      all('@graph-engine/' + name not in tools for name in
          ('index_repository', 'delete_project', 'manage_adr', 'ingest_traces')) and
      '@graph-engine' not in tools and d.get('includeMcpJson') is False and
      set(d.get('mcpServers', {})) == {'graph-engine'} and
...
if ! grep -q '^name: graph-engine$' "$FAKE_HOME/.hermes/skills/graph-engine/SKILL.md" 2>/dev/null ||
   ! grep -q 'delegate_task' "$FAKE_HOME/.hermes/skills/graph-engine/SKILL.md" 2>/dev/null ||
   ! grep -q '`context`' "$FAKE_HOME/.hermes/skills/graph-engine/SKILL.md" 2>/dev/null; then
...
# 8y: OpenHands MCP
CMD=$(json_get "$FAKE_HOME/.openhands/mcp.json" "d['mcpServers']['graph-engine']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8y: OpenHands command='$CMD'"
  exit 1
fi
echo "OK 8y: OpenHands MCP"
if ! grep -q '^name: graph-engine$' "$FAKE_HOME/.agents/skills/graph-engine/SKILL.md" 2>/dev/null ||
   ! grep -q 'trace_path' "$FAKE_HOME/.agents/skills/graph-engine/SKILL.md" 2>/dev/null; then
...
CLINE_RULE="$FAKE_HOME/.cline/rules/graph-engine.md"
for CLINE_CFG in "$FAKE_HOME/.cline/mcp.json" \
                 "$FAKE_HOME/.cline/data/settings/cline_mcp_settings.json"; do
  CMD=$(json_get "$CLINE_CFG" "d['mcpServers']['graph-engine']['command']")
  if ! path_match "$CMD" "$SELF_PATH"; then
...
CMD=$(json_get "$FAKE_HOME/.qwen/settings.json" "d['mcpServers']['graph-engine']['command']")
...
COPILOT_SKILL="$FAKE_HOME/.copilot/skills/graph-engine/SKILL.md"
COPILOT_AGENT="$FAKE_HOME/.copilot/agents/graph-engine.agent.md"
if ! grep -q 'search_graph' "$COPILOT_SKILL" 2>/dev/null ||
   ! grep -q '^tools:' "$COPILOT_AGENT" 2>/dev/null ||
   ! grep -q 'graph-engine/trace_path' "$COPILOT_AGENT" 2>/dev/null ||
   grep -qE '^  - (edit|shell|bash|graph-engine/(index_repository|delete_project|manage_adr|ingest_traces))$' "$COPILOT_AGENT" 2>/dev/null; then
...
COPILOT_HOOKS="$FAKE_HOME/.copilot/hooks/graph-engine.json"
...
CMD=$(json_get "$FAKE_HOME/.factory/mcp.json" "d['mcpServers']['graph-engine']['command']")
FACTORY_TYPE=$(json_get "$FAKE_HOME/.factory/mcp.json" "d['mcpServers']['graph-engine']['type']")
...
FACTORY_AGENT="$FAKE_HOME/.factory/droids/graph-engine.md"
if ! grep -q '^tools: \["Read", "LS", "Grep", "Glob",' "$FACTORY_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__graph-engine__search_graph' "$FACTORY_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__graph-engine__check_index_coverage' "$FACTORY_AGENT" 2>/dev/null ||
   grep -q '^mcpServers:' "$FACTORY_AGENT" 2>/dev/null ||
   grep -qE 'mcp__graph-engine__(index_repository|delete_project|manage_adr|ingest_traces)' "$FACTORY_AGENT" 2>/dev/null; then
...
CMD=$(json_get "$FAKE_HOME/.config/crush/crush.json" "d['mcp']['graph-engine']['command']")
CRUSH_TYPE=$(json_get "$FAKE_HOME/.config/crush/crush.json" "d['mcp']['graph-engine']['type']")
CRUSH_CONTEXT=$(json_get "$FAKE_HOME/.config/crush/crush.json" "str(any(str(p).endswith('graph-engine.md') for p in d['options']['context_paths']))")
if ! path_match "$CMD" "$SELF_PATH" || [ "$CRUSH_TYPE" != "stdio" ] ||
   [ "$CRUSH_CONTEXT" != "True" ] ||
   ! grep -q 'does not inherit MCP access' "$FAKE_HOME/.config/crush/graph-engine.md" 2>/dev/null; then
...
GOOSE_CMD=$(sed -n '/^  graph-engine:/,/^  [^ ]/{s/^ *cmd: *//p;}' \
  "$GOOSE_CFG" 2>/dev/null | head -1)
if ! grep -q '^extensions:' "$GOOSE_CFG" 2>/dev/null ||
   ! grep -q '^  graph-engine:' "$GOOSE_CFG" 2>/dev/null ||
...
if ! grep -q '^\[\[mcp_servers\]\]' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null ||
   ! grep -q '^name = "graph-engine"' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null ||
...
VIBE_AGENT="$FAKE_HOME/.vibe/agents/graph-engine.toml"
VIBE_PROMPT="$FAKE_HOME/.vibe/prompts/graph-engine.md"
if ! grep -q '^agent_type = "subagent"$' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq 'enabled_tools = ["read_file", "grep_search", "graph-engine_search_graph"' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq '"graph-engine_get_code_snippet"' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq '"graph-engine_check_index_coverage"' "$VIBE_AGENT" 2>/dev/null ||
   grep -Fq '"graph-engine_*"' "$VIBE_AGENT" 2>/dev/null ||
   grep -qE 'graph-engine_(index_repository|delete_project|manage_adr|ingest_traces)' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -q '^system_prompt_id = "graph-engine"$' "$VIBE_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$FAKE_HOME/.codeium/windsurf/mcp_config.json" "d['mcpServers']['graph-engine']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
...
AUGMENT_RULE="$FAKE_HOME/.augment/rules/graph-engine.md"
AUGMENT_AGENT="$FAKE_HOME/.augment/agents/graph-engine.md"
if [[ "$BINARY" == *.exe ]]; then
  AUGMENT_SCRIPT="$FAKE_HOME/.augment/hooks/graph-engine-session.ps1"
else
  AUGMENT_SCRIPT="$FAKE_HOME/.augment/hooks/graph-engine-session.sh"
fi
CMD=$(json_get "$AUGMENT_SETTINGS" "d['mcpServers']['graph-engine']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$AUGMENT_RULE" 2>/dev/null ||
   ! grep -q '^name: graph-engine$' "$AUGMENT_AGENT" 2>/dev/null ||
...
owned = [e for e in entries if any('graph-engine-session' in str(h.get('command', '')) for h in e.get('hooks', []))]
...
SKILL_FILE="$FAKE_HOME/.claude/skills/graph-engine/SKILL.md"
if [ ! -s "$SKILL_FILE" ]; then
  echo "FAIL 8ai: skill graph-engine missing or empty"
  exit 1
fi
...
QODER_SKILL="$FAKE_HOME/.qoder/skills/graph-engine/SKILL.md"
QODER_AGENT="$FAKE_HOME/.qoder/agents/graph-engine.md"
CMD=$(json_get "$QODER_SETTINGS" "d['mcpServers']['graph-engine']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$QODER_SKILL" 2>/dev/null ||
   ! grep -q '^tools: Read,Grep,Glob,mcp__graph-engine__search_graph' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__graph-engine__check_index_coverage' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q '^mcpServers:$' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q '^  - graph-engine$' "$QODER_AGENT" 2>/dev/null ||
   grep -qE 'mcp__graph-engine__(index_repository|delete_project|manage_adr|ingest_traces)' "$QODER_AGENT" 2>/dev/null ||
...
KIMI_SKILL="$CUSTOM_KIMI_HOME/skills/graph-engine/SKILL.md"
KIMI_CONFIG="$CUSTOM_KIMI_HOME/config.toml"
CMD=$(json_get "$KIMI_MCP" "d['mcpServers']['graph-engine']['command']")
...
PI_SKILL="$FAKE_HOME/.pi/agent/skills/graph-engine/SKILL.md"
...
WARP_SKILL="$FAKE_HOME/.agents/skills/graph-engine/SKILL.md"
...
JUNIE_SKILL="$FAKE_HOME/.junie/skills/graph-engine/SKILL.md"
JUNIE_AGENT="$FAKE_HOME/.junie/agents/graph-engine.md"
CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['graph-engine']['command']")
JUNIE_SCOUT_CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['graph-engine-scout']['command']")
JUNIE_ANALYSIS_CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['graph-engine-analysis']['command']")
JUNIE_SCOUT_ARGS=$(json_get "$JUNIE_MCP" "d['mcpServers']['graph-engine-scout']['args']")
JUNIE_ANALYSIS_ARGS=$(json_get "$JUNIE_MCP" "d['mcpServers']['graph-engine-analysis']['args']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ...
   ! grep -q 'mcpServers: \["graph-engine-analysis"\]' "$JUNIE_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$ROO_CFG" "d['mcpServers']['graph-engine']['command']")
...
CMD=$(json_get "$GITLAB_MCP" "d['mcpServers']['graph-engine']['command']")
...
CMD=$(json_get "$DEVIN_CONFIG" "d['mcpServers']['graph-engine']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
...
" 2>/dev/null ||
   ...
   ! grep -q 'engine-code-discovery-gate' "$FAKE_HOME/.claude/settings.json" 2>/dev/null; then
...
CMD=$(json_get "$CODEBUDDY_MCP" "d['mcpServers']['graph-engine']['command']")
...
if ! path_match "$CMD" "$SELF_PATH" || [ "$CODEBUDDY_KEEP" != "codebuddy" ] ||
   ...
   ! grep -q '^tools: Read,Grep,Glob,mcp__graph-engine__search_graph,' "$CODEBUDDY_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__graph-engine__check_index_coverage' "$CODEBUDDY_AGENT" 2>/dev/null ||
   grep -qE 'mcp__graph-engine__(index_repository|delete_project|manage_adr|ingest_traces)' "$CODEBUDDY_AGENT" 2>/dev/null ||
   grep -q '^tools:$' "$CODEBUDDY_AGENT" 2>/dev/null ||
   grep -q 'mcp__graph-engine__search_graph' "$CODEBUDDY_AGENT" 2>/dev/null ||
   ! grep -q '^skills: graph-engine$' "$CODEBUDDY_AGENT" 2>/dev/null ||
...
BOB_IDE_CMD=$(json_get "$BOB_IDE_MCP" "d['mcpServers']['graph-engine']['command']")
BOB_SHELL_CMD=$(json_get "$BOB_SHELL_MCP" "d['mcpServers']['graph-engine']['command']")
...
POCHI_CMD=$(sed '/^[[:space:]]*\/\//d' "$POCHI_MCP" 2>/dev/null | python3 -c "import json,sys; print(json.load(sys.stdin)['mcp']['graph-engine']['command'])" 2>/dev/null || echo "")
...
ROVO_CMD=$(json_get "$ROVO_MCP" "d['mcpServers']['graph-engine']['command']")
...
AMAZON_Q_CMD=$(json_get "$AMAZON_Q_MCP" "d['mcpServers']['graph-engine']['command']")
...
for VIBE_SLUG in graph-engine-scout graph-engine graph-engine-auditor; do
...
import json, sys
d = json.load(sys.stdin)
if 'graph-engine' in d.get('mcpServers', {}):
    sys.exit(1)
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'graph-engine' in d.get('mcpServers', {}) else 0)
" 2>/dev/null; then
...
d = json.load(sys.stdin)
hooks = d.get('hooks', {})
found = any('engine-code-discovery-gate' in str(h) or
            'engine-session-reminder' in str(h) or
            'engine-subagent-reminder' in str(h)
            for entries in hooks.values() for h in entries)
sys.exit(1 if found else 0)
" 2>/dev/null; then
  for HOOK_SCRIPT in engine-code-discovery-gate engine-session-reminder engine-subagent-reminder; do
...
if grep -q '\[mcp_servers.graph-engine\]' "$FAKE_HOME/.codex/config.toml" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
has_mcp = 'graph-engine' in d.get('mcpServers', {})
has_existing = d.get('existingKey', False)
hooks = d.get('hooks', {}).get('BeforeTool', [])
has_hook = any('graph-engine' in str(h) for h in hooks)
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'graph-engine' in d.get('servers', {}) else 0)
" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
has_mcp = 'graph-engine' in d.get('mcp', {}).get('servers', {})
sections = d.get('agents', {}).get('defaults', {}).get('compaction', {}).get('postCompactionSections', [])
sys.exit(1 if has_mcp or 'Codebase Knowledge Graph (graph-engine)' in sections else 0)
" 2>/dev/null; then
  if grep -q 'graph-engine:start' "$FAKE_HOME/.openclaw/workspace/AGENTS.md" 2>/dev/null ||
     grep -q 'graph-engine:start' "$FAKE_HOME/.openclaw/workspace/TOOLS.md" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'graph-engine' in d.get('$ROOT', {}) else 0)
" 2>/dev/null; then
...
names = {'graph-engine', 'graph-engine-scout', 'graph-engine-analysis'}
...
import json, sys
d = json.load(sys.stdin)
print('present' if 'graph-engine' in d.get('mcp', {}) else 'absent')
...
if grep -q 'hook-augment' "$QODER_SETTINGS" 2>/dev/null ||
   ...
   grep -q 'hook-augment' "$FAKE_HOME/.copilot/hooks/graph-engine.json" 2>/dev/null ||
   grep -q 'hook-augment' "$FAKE_HOME/.factory/hooks.json" 2>/dev/null ||
   grep -q 'graph-engine-session' "$AUGMENT_SETTINGS" 2>/dev/null ||
...
if CRUSH_CONTEXT=$(json_get "$FAKE_HOME/.config/crush/crush.json" "str(any(str(p).endswith('graph-engine.md') for p in d.get('options', {}).get('context_paths', [])))") &&
...
if grep -q '^  graph-engine:' "$FAKE_HOME/.hermes/config.yaml" 2>/dev/null ||
   grep -q '^  pre_llm_call:' "$FAKE_HOME/.hermes/config.yaml" 2>/dev/null ||
   grep -q '^  graph-engine:' "$GOOSE_CFG" 2>/dev/null ||
   grep -q '^name = "graph-engine"' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null; then
...
for CONTEXT_FILE in \
  "$ZED_INSTR" \
  "$FAKE_HOME/.kiro/steering/graph-engine.md" \
  "$FAKE_HOME/.factory/AGENTS.md" \
  "$AUGMENT_RULE" \
  "$FAKE_HOME/.config/crush/graph-engine.md" \
  ...
  "$PI_INSTRUCTIONS"; do
  if grep -q 'graph-engine:start' "$CONTEXT_FILE" 2>/dev/null; then
...
if [ -d "$FAKE_HOME/.claude/skills/graph-engine" ] ||
   [ -d "$FAKE_HOME/.hermes/skills/graph-engine" ] ||
   [ -d "$FAKE_HOME/.agents/skills/graph-engine" ] ||
   [ -d "$FAKE_HOME/.qoder/skills/graph-engine" ] ||
   [ -d "$CUSTOM_KIMI_HOME/skills/graph-engine" ] ||
   [ -d "$FAKE_HOME/.pi/agent/skills/graph-engine" ] ||
   [ -d "$FAKE_HOME/.junie/skills/graph-engine" ] ||
   [ -d "$FAKE_HOME/.rovodev/skills/graph-engine" ] ||
   [ -d "$FAKE_HOME/.copilot/skills/graph-engine" ] ||
   [ -d "$FAKE_HOME/.vibe/skills/graph-engine" ] ||
   [ -e "$DEVIN_SKILL" ] ||
   [ -e "$CODEBUDDY_SKILL" ] ||
   [ -e "$BOB_SKILL" ] ||
   [ -e "$POCHI_SKILL" ] ||
...
# Note: engine_find_cli searches hardcoded paths (/usr/local/bin, /opt/homebrew/bin)
...
import json, sys
d = json.load(sys.stdin)
print(list(d.get('mcpServers',{}).keys()).count('graph-engine'))
" 2>/dev/null || echo "0")
if [ "$COUNT" != "1" ]; then
...
CLEAN_UNINSTALLER="$CLEAN_HOME/engine-uninstall-driver"
if [[ "$BINARY" == *.exe ]]; then
...
copy_smoke_binary "$CORRUPT_HOME/.local/bin/graph-engine"
...
DBL_UNINSTALLER="$DBL_HOME/.local/bin/graph-engine"
DBL_RETRY_UNINSTALLER="$DBL_HOME/engine-uninstall-retry"
if [[ "$BINARY" == *.exe ]]; then
...
SECURITY_BIN="$SECURITY_DIR/graph-engine"
...
if [ -n "${SMOKE_DOWNLOAD_URL:-}" ]; then
  ...
  # ENGINE_DOWNLOAD_URL test override, so point it directly at the same fixture.
  ...
  # through install.sh or `install`, so nothing populates ~/.engine/assets. The
  ...
  BINARY_ASSET="$(dirname "$BINARY")/engine-integrations.json"
  if [[ "$BINARY" == *.exe ]]; then
    copy_smoke_binary "$UPDATE_HOME/.local/bin/graph-engine.exe"
    mkdir -p "$UPDATE_HOME/retired-install"
    copy_smoke_binary "$UPDATE_HOME/retired-install/graph-engine.exe"
    [ -f "$BINARY_ASSET" ] && cp "$BINARY_ASSET" "$UPDATE_HOME/.local/bin/engine-integrations.json"
  else
    copy_smoke_binary "$UPDATE_HOME/.local/bin/graph-engine"
    chmod 755 "$UPDATE_HOME/.local/bin/graph-engine"
    mkdir -p "$UPDATE_HOME/retired-install"
    copy_smoke_binary "$UPDATE_HOME/retired-install/graph-engine"
    chmod 755 "$UPDATE_HOME/retired-install/graph-engine"
    [ -f "$BINARY_ASSET" ] && cp "$BINARY_ASSET" "$UPDATE_HOME/.local/bin/engine-integrations.json"
    if [ "$(uname -s)" = "Darwin" ]; then
      codesign --sign - --force "$UPDATE_HOME/.local/bin/graph-engine" 2>/dev/null || true
      codesign --sign - --force "$UPDATE_HOME/retired-install/graph-engine" \
        2>/dev/null || true
    fi
  fi
  ...
  # uninstall phases.
  if [[ "$BINARY" == *.exe ]]; then
    UPDATE_DRIVER="$UPDATE_HOME/.local/bin/graph-engine.exe"
  else
    UPDATE_DRIVER="$UPDATE_HOME/.local/bin/graph-engine"
  fi

  # Pre-install agent config with positive prior-install identity. POSIX runs
  # update from that exact retired engine image, so refresh requires only string
  ...
  STALE_CMD="$STALE_CMD" python3 -c \
    'import json, os; print(json.dumps({"mcpServers":{"graph-engine":{"command":os.environ["STALE_CMD"]}}}))' \
  ...
  HOME="$UPDATE_HOME" ENGINE_DOWNLOAD_URL="$UPDATE_DOWNLOAD_URL" \
  ...
  if [[ "$BINARY" == *.exe ]]; then
    UPD_BIN="$UPDATE_HOME/.local/bin/graph-engine.exe"
  else
    UPD_BIN="$UPDATE_HOME/.local/bin/graph-engine"
  fi
  if [ ! -f "$UPD_BIN" ]; then
  ...
  if [ -f "$UPDATE_HOME/.local/bin/graph-engine" ] || [ -f "$UPDATE_HOME/.local/bin/graph-engine.exe" ]; then
...
import json, sys
d = json.load(sys.stdin)
if 'graph-engine' in d.get('mcpServers', {}): sys.exit(1)
sys.exit(0)
" 2>/dev/null; then
  ...
  copy_smoke_binary "$UPDATE_DIR/install/graph-engine"
  chmod 755 "$UPDATE_DIR/install/graph-engine"
  copy_smoke_binary "$UPDATE_DIR/smoke-downloaded"
  rm -f "$UPDATE_DIR/install/graph-engine"
  cp "$UPDATE_DIR/smoke-downloaded" "$UPDATE_DIR/install/graph-engine"
  chmod 755 "$UPDATE_DIR/install/graph-engine"
  if [ "$(uname -s)" = "Darwin" ]; then
    codesign --sign - --force "$UPDATE_DIR/install/graph-engine" 2>/dev/null || true
  fi
  if ! "$UPDATE_DIR/install/graph-engine" --version > /dev/null 2>&1; then
...
DL_ARCHIVE="graph-engine-${DL_OS}-${DL_ARCH}.${DL_EXT}"
DL_ARCHIVE_UI="graph-engine-ui-${DL_OS}-${DL_ARCH}.${DL_EXT}"
...
if [ "$DL_OS" != "windows" ] && [ -f "$REPO_ROOT/install.sh" ]; then
  ...
  HOME="$INSTALL_TEST_HOME" ENGINE_DOWNLOAD_URL="$SMOKE_DOWNLOAD_URL" \
    "$REPO_ROOT/install.sh" "$DL_VARIANT_ARG" --dir="$INSTALL_TEST_DIR" 2>&1 || true

  # 13b: binary placed
  if [ ! -f "$INSTALL_TEST_DIR/graph-engine" ]; then
  ...
  shopt -s nullglob
  INSTALLED_UI_PACKS=("$INSTALL_TEST_DIR"/engine-ui-*.pack)
  shopt -u nullglob
  if [ "$DL_VARIANT_ARG" = "--ui" ]; then
    if [ "${#INSTALLED_UI_PACKS[@]}" -ne 1 ] ||
       ! [[ "$(basename "${INSTALLED_UI_PACKS[0]}")" =~ ^engine-ui-[0-9a-f]{64}\.pack$ ]]; then
  ...
  if [ "$DL_OS" = "darwin" ]; then
    codesign --sign - --force "$INSTALL_TEST_DIR/graph-engine" 2>/dev/null || true
  fi
  if ! "$INSTALL_TEST_DIR/graph-engine" --version > /dev/null 2>&1; then
  ...
  if [ "$DL_OS" = "darwin" ]; then
    if codesign -v "$INSTALL_TEST_DIR/graph-engine" 2>/dev/null; then
  ...
  if [ -f "$INSTALL_TEST_HOME/.claude.json" ] && grep -q 'graph-engine' "$INSTALL_TEST_HOME/.claude.json" 2>/dev/null; then
  ...
  if ! HOME="$WIN_HOME" TEMP="$WIN_HOME" TMP="$WIN_HOME" \
    ENGINE_DOWNLOAD_URL="$WIN_URL" ENGINE_ARCH="$DL_ARCH" MSYS2_ARG_CONV_EXCL='*' \
  ...
  # 13g: binary placed
  PS1_BIN="$PS1_TEST_DIR/graph-engine.exe"
  if [ ! -f "$PS1_BIN" ] && [ -f "$PS1_TEST_DIR/graph-engine" ]; then
    PS1_BIN="$PS1_TEST_DIR/graph-engine"
  fi
  if [ -f "$PS1_BIN" ]; then
  ...
  shopt -s nullglob
  PS1_UI_PACKS=("$PS1_TEST_DIR"/engine-ui-*.pack)
  shopt -u nullglob
  if [ "$DL_VARIANT_ARG" = "--ui" ]; then
    if [ "${#PS1_UI_PACKS[@]}" -ne 1 ] ||
       ! [[ "$(basename "${PS1_UI_PACKS[0]}")" =~ ^engine-ui-[0-9a-f]{64}\.pack$ ]]; then
