...
def write_tokens_h(path: str, tokens: list):
    ...
    with open(path, "w") as f:
        ...
        f.write("#ifndef ENGINE_NOMIC_TOKENS_H\n")
        f.write("#define ENGINE_NOMIC_TOKENS_H\n\n")
        f.write(f"static const char *PRETRAINED_TOKENS[{len(tokens)}] = {{\n")
        for t in tokens:
            escaped = t.replace("\\", "\\\\").replace('"', '\\"')
            f.write(f'"{escaped}",\n')
        f.write("};\n\n")
        f.write("#endif /* ENGINE_NOMIC_TOKENS_H */\n")
 ...
 * License: Apache 2.0
 */
#ifndef ENGINE_NOMIC_VECTORS_H
#define ENGINE_NOMIC_VECTORS_H
