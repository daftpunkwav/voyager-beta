#include "engine.h"
#include "arena.h" // EngineArena, engine_arena_strndup
#include "helpers.h"
...
static const char *unquote(EngineArena *a, const char *s) {
    if (!s || !s[0]) {
        return NULL;
    }
    // Trim whitespace
...
static const char *extract_env_key_from_call(EngineExtractCtx *ctx, TSNode node,
    ...
    char *callee = engine_node_text(ctx->arena, func_node, ctx->source);
    if (!callee) {
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < ts_node_child_count(args); i++) {
        ...
        char *arg_text = engine_node_text(ctx->arena, child, ctx->source);
...
static const char *extract_env_key_from_member(EngineExtractCtx *ctx, TSNode node,
                                               const EngineLangSpec *spec) {
    ...
    for (const char **pat = spec->env_access_member_patterns; *pat; pat++) {
        ...
        if (strncmp(text, *pat, plen) == 0 && text[plen] == '.') {
            const char *key = text + plen + ENGINE_QUOTE_OFFSET;
        ...
        if (strncmp(text, *pat, plen) == 0 && text[plen] == '[') {
            const char *inner = text + plen + ENGINE_QUOTE_OFFSET;
            size_t ilen = strlen(inner);
            if (ilen > 0 && inner[ilen - ENGINE_QUOTE_OFFSET] == ']') {
                char *bracket_content =
                    engine_arena_strndup(ctx->arena, inner, ilen - ENGINE_QUOTE_OFFSET);
...
static void walk_env_accesses(EngineExtractCtx *ctx, TSNode root, const EngineLangSpec *spec) {
    TSNodeStack stack;
    ...
    while (stack.count > 0) {
        ...
        if (engine_kind_in_set(node, spec->call_node_types)) {
        ...
        if (env_key && env_key[0] && is_env_var_name(env_key)) {
            EngineEnvAccess ea;
            ea.env_key = env_key;
            ea.enclosing_func_qn = engine_enclosing_func_qn_cached(ctx, node);
            engine_envaccess_push(&ctx->result->env_accesses, ctx->arena, ea);
...
void handle_env_accesses(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec,
    ...
    if (has_funcs && spec->call_node_types && engine_kind_in_set(node, spec->call_node_types)) {
        env_key = extract_env_key_from_call(ctx, node, spec);
