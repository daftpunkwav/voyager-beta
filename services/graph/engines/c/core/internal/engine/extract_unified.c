...
#include "arena.h" // engine_arena_sprintf
#include "engine.h"   // EngineExtractCtx
#include "helpers.h"
#include "lang_specs.h"      // EngineLangSpec, engine_lang_spec, ENGINE_LANG_*
...
static bool ensure_scope_capacity(WalkState *state) {
    ...
    EngineWalkScope *grown =
        (EngineWalkScope *)engine_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        return false;
    }
...
static const EngineLexicalScope *lexical_scope_by_id(const WalkState *state, uint32_t id) {
    return state && id > 0 && id <= (uint32_t)state->lexical_scope_count
               ? &state->lexical_scopes[id - 1U]
               : NULL;
}

static EngineLexicalScope *mutable_lexical_scope_by_id(WalkState *state, uint32_t id) {
    return state && id > 0 && id <= (uint32_t)state->lexical_scope_count
               ? &state->lexical_scopes[id - 1U]
               : NULL;
}
...
static bool ensure_lexical_scope_capacity(WalkState *state) {
    ...
    EngineLexicalScope *grown =
        (EngineLexicalScope *)engine_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        state->lexical_binding_tracking_failed = true;
        return false;
    }
...
static uint32_t add_lexical_scope(WalkState *state, TSNode node, EngineLexicalScopeKind kind) {
    ...
    if (kind == ENGINE_LEXICAL_SCOPE_FUNCTION && state->language == ENGINE_LANG_PYTHON) {
        lookup_parent_id = python_function_lookup_parent(state, parent_id);
    }
    EngineLexicalScope *scope = &state->lexical_scopes[state->lexical_scope_count];
...
static bool push_scope(WalkState *state, uint8_t kind, uint32_t depth, const char *qn) {
    ...
    EngineWalkScope *f = &state->scopes[state->scope_top];
    f->kind = kind;
    f->depth = depth;
    f->qn = qn;
    f->lexical_scope_id = 0;
    f->invocation_kind = ENGINE_INVOCATION_NONE;
     ...
     * EngineWalkScope comment for the quadratic recompute this replaces. */
...
static bool push_lexical_scope(WalkState *state, uint8_t walk_kind, uint32_t depth, const char *qn,
                               TSNode node, EngineLexicalScopeKind lexical_kind) {
...
static bool push_existing_lexical_scope(WalkState *state, uint8_t walk_kind, uint32_t depth,
    ...
    EngineLexicalScope *scope = mutable_lexical_scope_by_id(state, lexical_scope_id);
...
static uint32_t active_same_function_scope_id(const WalkState *state, const char *function_qn,
    ...
    for (int i = state->scope_top - 1; i >= 0; i--) {
        ...
        const EngineLexicalScope *scope =
...
static bool push_function_scope(WalkState *state, uint32_t depth, const char *function_qn,
    ...
    return existing_id ? push_existing_lexical_scope(state, SCOPE_FUNC, depth, function_qn,
                       ...
                       : push_lexical_scope(state, SCOPE_FUNC, depth, function_qn, node,
                                            ENGINE_LEXICAL_SCOPE_FUNCTION);
}

static void push_call_scope(WalkState *state, uint32_t depth,
                            const EngineInvocationDescriptor *invocation) {
    if (!invocation ||
        (invocation->kind != ENGINE_INVOCATION_CALLABLE_REFERENCE && !invocation->raw_call_emitted) ||
...
static void pop_expired_scopes(WalkState *state, uint32_t cur_depth) {
    while (state->scope_top > 0 && state->scopes[state->scope_top - SKIP_ONE].depth >= cur_depth) {
        const EngineWalkScope *f = &state->scopes[--state->scope_top];
...
static const char *compute_lisp_func_qn(EngineExtractCtx *ctx, TSNode node) {
    if (ts_node_named_child_count(node) < 2) {
        return NULL;
    }
    char *head = engine_node_text(ctx->arena, ts_node_named_child(node, 0), ctx->source);
    ...
    return engine_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, name);
}
...
static const char *compute_elixir_func_qn(EngineExtractCtx *ctx, TSNode node) {
    if (ts_node_child_count(node) == 0) {
        return NULL;
    }
    char *macro = engine_node_text(ctx->arena, ts_node_child(node, 0), ctx->source);
    ...
    if (strcmp(fk, "call") == 0 && ts_node_child_count(first_arg) > 0) {
        name = engine_node_text(ctx->arena, ts_node_child(first_arg, 0), ctx->source);
    } else if (strcmp(fk, "identifier") == 0) {
        name = engine_node_text(ctx->arena, first_arg, ctx->source);
    }
    if (!name || !name[0]) {
        return NULL;
    }
...
static const char *compute_gotemplate_func_qn(EngineExtractCtx *ctx, TSNode node) {
    ...
    char *raw = engine_node_text(ctx->arena, s, ctx->source);
    if (!raw) {
        return NULL;
    }
    size_t len = strlen(raw);
    if (len >= 2 && (raw[0] == '"' || raw[0] == '`')) {
        raw = engine_arena_strndup(ctx->arena, raw + 1, len - 2); // strip surrounding quotes
    }
    if (!raw || !raw[0]) {
        return NULL;
    }
    return engine_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, raw);
}



// Compute function QN for scope tracking (mirrors engine_enclosing_func_qn logic).
static const char *compute_func_qn(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec,
                                   WalkState *state) {
    (void)spec;
     ...
     * child. Resolve the call-scope QN HERE (not via the shared engine_resolve_func_name)
        ...
        if (!ts_node_is_null(id)) {
            ...
            if (mname && mname[0]) {
                if (state->enclosing_class_qn) {
                    return engine_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, mname);
                }
                return engine_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, mname,
                                                   ctx->language);
            }
        }
    }
    ...
    if (1 && (strcmp(ts_node_type(node), "function_signature") == 0 ||
        ...
        if (strcmp(ts_node_type(node), "method_signature") == 0) {
            TSNode fs = engine_find_child_by_kind(node, "function_signature");
            if (!ts_node_is_null(fs)) {
                sig = fs;
            }
        }
        TSNode id = engine_find_child_by_kind(sig, "identifier");
        if (!ts_node_is_null(id)) {
            char *nm = engine_node_text(ctx->arena, id, ctx->source);
            if (nm && nm[0]) {
                if (state->enclosing_class_qn) {
                    return engine_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, nm);
                }
                return engine_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, nm,
                                                   ctx->language);
            }
        }
    }
    ...
    char *name = engine_func_name_node_text(ctx->arena, name_node, ctx->source, ctx->language);
    if (!name || !name[0]) {
        return NULL;
    }
    ...
    if ((1 || 1) &&
        ...
        if (scope_name && scope_name[0]) {
            const char *class_qn =
                engine_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, scope_name);
            return engine_arena_sprintf(ctx->arena, "%s.%s", class_qn, name);
        }
    }
    ...
    if (state->enclosing_class_qn) {
        return engine_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, qn_name);
    }
    ...
    return engine_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, qn_name,
                                       ctx->language);
}

// Compute class QN for scope tracking.
static const char *compute_class_qn(EngineExtractCtx *ctx, TSNode node, const WalkState *state) {
    ...
    if (state && state->enclosing_class_qn) {
        return engine_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, name);
    }

    /* Java/Go: directory-based module (see compute_func_qn). */
    return engine_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, name,
                                       ctx->language);
}
...
static void handle_string_constants(EngineExtractCtx *ctx, TSNode node, const WalkState *state) {
    ...
    if (strcmp(value_kind, "template_string") == 0) {
        flat_value = engine_template_string_text(ctx->arena, value_node, ctx->source);
        if (!flat_value) {
            return;
        }
    ...
    char *name = engine_node_text(ctx->arena, name_node, ctx->source);
    char *value =
        flat_value ? (char *)flat_value : engine_node_text(ctx->arena, value_node, ctx->source);
    ...
    if (!flat_value && vlen >= ENGINE_QUOTE_PAIR && (value[0] == '"' || value[0] == '\'')) {
        value = engine_arena_strndup(ctx->arena, value + SKIP_ONE, (size_t)(vlen - PAIR_LEN));
        if (!value) {
            return;
        }
    }
    ...
    /* Add to constant map */
    EngineStringConstantMap *map = &ctx->string_constants;
    if (map->count < ENGINE_MAX_STRING_CONSTANTS) {
...
static void handle_string_refs(EngineExtractCtx *ctx, TSNode node, const WalkState *state) {
    const char *kind = ts_node_type(node);
    ...
    if (strcmp(kind, "template_string") == 0) {
        const char *flat = engine_template_string_text(ctx->arena, node, ctx->source);
        if (!flat) {
            return;
        }
        int kind_val = engine_classify_string(flat, (int)strlen(flat));
        if (kind_val < 0) {
            return;
        }
        EngineStringRef ref = {
            ...
            .kind = (EngineStringRefKind)kind_val,
        };
        engine_stringref_push(&ctx->result->string_refs, ctx->arena, ref);
        return;
    }
    ...
    /* Classify */
    int kind_val = engine_classify_string(content, len);
    if (kind_val < 0) {
        return;
    }

    /* Build null-terminated content string in arena */
    char *val = engine_arena_strndup(ctx->arena, content, (size_t)len);
    if (!val) {
        return;
    }
...
static void emit_yaml_leaf_value(EngineExtractCtx *ctx, TSNode val, const char *path) {
    char *val_text = engine_node_text(ctx->arena, val, ctx->source);
    ...
    if (vlen >= ENGINE_QUOTE_PAIR && (val_text[0] == '"' || val_text[0] == '\'')) {
    ...
    int kind_val = engine_classify_string(content, vlen);
    if (kind_val < 0) {
        return;
    }

    char *stored = engine_arena_strndup(ctx->arena, content, (size_t)vlen);
    if (!stored) {
        return;
    }
...
#define YAML_WALK_STACK_CAP ENGINE_SZ_256
...
static void walk_yaml_mapping(EngineExtractCtx *ctx, TSNode root, const char *root_prefix) {
    ...
    while (top > 0) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            char *key_text = engine_node_text(ctx->arena, key, ctx->source);
            if (!key_text || !key_text[0]) {
                continue;
            }
            const char *path =
                prefix ? engine_arena_sprintf(ctx->arena, "%s.%s", prefix, key_text) : key_text;
...
static char *strip_yaml_quotes(EngineArena *a, char *v) {
    if (!v || !v[0]) {
        return v;
    }
    int vlen = (int)strlen(v);
    if (vlen >= ENGINE_QUOTE_PAIR && (v[0] == '"' || v[0] == '\'')) {
        return engine_arena_strndup(a, v + SKIP_ONE, (size_t)(vlen - PAIR_LEN));
    }
    return v;
}

// Scan a nested YAML block_mapping for target keys (push_endpoint, uri, etc.).
static void scan_nested_mapping_targets(EngineExtractCtx *ctx, TSNode val, const char **targets,
                                        int *n_targets) {
    uint32_t vnc = ts_node_named_child_count(val);
    for (uint32_t vi = 0; vi < vnc; vi++) {
        ...
        for (uint32_t mi = 0; mi < mnc; mi++) {
            ...
            char *mktext = engine_node_text(ctx->arena, mk, ctx->source);
            if (mktext && is_target_key(mktext) && *n_targets < MAX_INFRA_BINDINGS) {
                char *mvtext =
                    strip_yaml_quotes(ctx->arena, engine_node_text(ctx->arena, mv, ctx->source));
...
static void emit_infra_bindings(EngineExtractCtx *ctx, const char **sources, const char **source_keys,
    ...
    for (int si = 0; si < n_sources; si++) {
        for (int ti = 0; ti < n_targets; ti++) {
            ...
            EngineInfraBinding ib = {
                .source_name = sources[si],
                .target_url = targets[ti],
                .broker = infer_broker(ctx->rel_path, source_keys[si]),
            };
            engine_infrabinding_push(&ctx->result->infra_bindings, ctx->arena, ib);
        }
    }
}

static void scan_mapping_for_bindings(EngineExtractCtx *ctx, TSNode mapping) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        char *k = engine_node_text(ctx->arena, key, ctx->source);
        if (!k) {
            continue;
        }
        ...
        if (strcmp(vtype, "block_node") != 0 && strcmp(vtype, "block_mapping") != 0) {
            char *v = strip_yaml_quotes(ctx->arena, engine_node_text(ctx->arena, val, ctx->source));
...
#define INFRA_SCAN_STACK_CAP ENGINE_SZ_512
static void scan_yaml_for_infra_bindings(EngineExtractCtx *ctx, TSNode root) {
...
static char *extract_hcl_string_val(EngineArena *a, TSNode val_node, const char *source) {
    ...
    for (int depth = 0; depth < HCL_MAX_UNWRAP; depth++) {
        ...
        if (strcmp(vk, "quoted_template") == 0 || strcmp(vk, "template_literal") == 0 ||
            ...
            char *val = engine_node_text(a, val_node, source);
...
static TSNode hcl_block_body(TSNode block) {
    TSNode body = engine_find_child_by_kind(block, "body");
    return ts_node_is_null(body) ? block : body;
}

// Scan a nested HCL block for target keys (push_endpoint, uri, etc.).
static void scan_hcl_nested_block_targets(EngineExtractCtx *ctx, TSNode block, const char **targets,
                                          int *n_targets) {
    ...
    for (uint32_t bi = 0; bi < bnc; bi++) {
        ...
        char *bk = engine_node_text(ctx->arena, bkey, ctx->source);
...
static const char *hcl_scheduler_source(EngineExtractCtx *ctx, TSNode block, const char **broker_out) {
    ...
    for (uint32_t i = 0; i < cc; i++) {
        ...
        char *label = engine_node_text(ctx->arena, lit, ctx->source);
        if (!label || !label[0]) {
            continue;
        }
        if (!first_label) {
...
static void scan_hcl_block_for_bindings(EngineExtractCtx *ctx, TSNode block) {
    const char *sources[MAX_INFRA_BINDINGS] = {NULL};
    const char *source_keys[MAX_INFRA_BINDINGS] = {NULL};
    int n_sources = 0;
    const char *targets[MAX_INFRA_BINDINGS] = {NULL};
...
static bool is_actual_import_boundary(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec) {
    bool direct_import = spec->import_node_types && engine_kind_in_set(node, spec->import_node_types);
    bool from_import = spec->import_from_types && engine_kind_in_set(node, spec->import_from_types);
    ...
    char *name = ts_node_is_null(head) ? NULL : engine_node_text(ctx->arena, head, ctx->source);
...
static bool push_pre_node_scope(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec,
...
static bool lexical_boundary_kind(TSNode node, EngineLexicalScopeKind *out_kind) {
    const char *kind = ts_node_type(node);
    ...
    if (strcmp(kind, "mod_item") == 0) {
        *out_kind = ENGINE_LEXICAL_SCOPE_MODULE;
        return true;
    }
    if (unified_kind_in_set(kind, comprehension_kinds)) {
        *out_kind = ENGINE_LEXICAL_SCOPE_COMPREHENSION;
        return true;
    }
    if (unified_kind_in_set(kind, anonymous_function_kinds)) {
        *out_kind = ENGINE_LEXICAL_SCOPE_FUNCTION;
        return true;
    }
    if (unified_kind_in_set(kind, block_kinds)) {
        *out_kind = ENGINE_LEXICAL_SCOPE_BLOCK;
        return true;
    }
    return false;
}
...
static void push_boundary_scopes(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec,
                                 WalkState *state, uint32_t depth,
                                 const EngineInvocationDescriptor *invocation) {
    if (spec->function_node_types && engine_kind_in_set(node, spec->function_node_types)) {
    ...
    } else if (engine_is_namespace_scope_kind(ctx->language, ts_node_type(node))) {
        const char *namespace_qn = compute_class_qn(ctx, node, state);
        if (namespace_qn) {
            push_lexical_scope(state, SCOPE_NAMESPACE, depth, namespace_qn, node,
                               ENGINE_LEXICAL_SCOPE_MODULE);
        }
    } else if (spec->class_node_types && engine_kind_in_set(node, spec->class_node_types)) {
        ...
        if (!ts_node_is_null(type_node)) {
            ...
            if (type_name && type_name[0]) {
                const char *tqn =
                    engine_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, type_name);
    ...
    if (ts_node_is_named(node) && engine_is_loop_node_type(ts_node_type(node))) {
        push_scope(state, SCOPE_LOOP, depth, NULL);
    } else if (spec->branching_node_types && engine_kind_in_set(node, spec->branching_node_types)) {
...
void engine_extract_unified(EngineExtractCtx *ctx) {
    ...
    state.root_lexical_scope_id = add_lexical_scope(&state, ctx->root, ENGINE_LEXICAL_SCOPE_MODULE);
    ...
    state.invocation_kind = ENGINE_INVOCATION_NONE;
    ...
    for (;;) {
        ...
        if (!trivia) {
            ...
            EngineInvocationDescriptor invocation = handle_calls(ctx, node, spec, &state);
    ...
    engine_finalize_lexical_usages(ctx, &state);
