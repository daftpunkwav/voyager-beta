...
// engine_is_keyword: it only covers builtins with a target node, so un-filtering
// them cannot produce a node-less / Module-sourced edge. The Python set MUST
// stay in sync with kPyBuiltinNodes in internal/engine/lsp/py_builtins.c.
bool engine_is_resolvable_builtin(const char *name, EngineLanguage lang);

// Classify a string literal as URL, config, or neither.
// Returns ENGINE_STRREF_URL (0), ENGINE_STRREF_CONFIG (1), or -1 for neither.
...
#define ENGINE_DECLARATOR_DEPTH_LIMIT 8
...
// Free the calling thread's engine_kind_in_set bitset cache (call at thread/process
...
// engine_fqn_module returns (no behavior change).
...
// language this is exactly engine_fqn_compute (no behavior change).
