#include "engine.h"
#include "arena.h" // EngineArena, engine_arena_sprintf
#include "helpers.h"
...
#include "service_patterns.h" // engine_service_pattern_route_method (#952)
...
enum { MAX_STRING_ARG_LEN = ENGINE_SZ_512 };
...
static const char *lookup_string_constant(const EngineExtractCtx *ctx, const char *name) {
    if (!name || !name[0]) {
        return NULL;
    }
    const EngineStringConstantMap *map = &ctx->string_constants;
...
static const char *strip_quotes(EngineArena *a, const char *text) {
    if (!text || !text[0]) {
        return NULL;
    }
    int len = (int)strlen(text);
    if (len >= ENGINE_QUOTE_PAIR && (text[0] == '"' || text[0] == '\'')) {
        return engine_arena_strndup(a, text + ENGINE_QUOTE_OFFSET, (size_t)(len - ENGINE_QUOTE_PAIR));
    }
    return text;
}
...
static char *extract_callee_name(EngineArena *a, TSNode node, const char *source, EngineLanguage lang);
static char *gotemplate_callee(EngineArena *a, TSNode node, const char *source);
static const char *strip_and_validate_string_arg(EngineArena *a, char *text);
 ...
 * Without this, engine_node_text returns full text including args/parens.
 * Iteratively walks the chain: selector → operand(call) → function(selector) → ... */
static char *resolve_chained_selector(EngineArena *a, TSNode sel, const char *source) {
    ...
    if (ts_node_is_null(operand) || ts_node_is_null(field) ||
        ...
        return engine_node_text(a, sel, source);
    }
    ...
    char *method = engine_node_text(a, field, source);
    ...
    for (int depth = 0; depth < MAX_CHAIN_DEPTH; depth++) {
        ...
        char *base = engine_node_text(a, fn, source);
        if (base && method) {
            return engine_arena_sprintf(a, "%s.%s", base, method);
        }
        return method;
    }
...
static char *extract_constructor_callee(EngineArena *a, TSNode node, const char *source,
                                        const char *nk) {
    ...
    for (const char **f = type_fields; *f; f++) {
        ...
        if (!ts_node_is_null(tn)) {
            ...
            char *t = strip_generic_args(engine_node_text(a, tn, source));
            if (t && t[0]) {
                return t;
            }
        }
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "identifier") == 0 ||
            ...
            char *t = strip_generic_args(engine_node_text(a, child, source));
            if (t && t[0]) {
                return t;
            }
        }
...
static char *extract_callee_from_fields(EngineArena *a, TSNode node, const char *source) {
    // Try "function" field
    ...
    if (!ts_node_is_null(func_node)) {
        ...
        if (strcmp(fk, "identifier") == 0 || strcmp(fk, "simple_identifier") == 0 ||
            ...
            return engine_node_text(a, func_node, source);
        }
        ...
        if (strcmp(fk, "extract_operator") == 0) {
            ...
            if (!ts_node_is_null(rhs)) {
                char *rt = engine_node_text(a, rhs, source);
                if (!ts_node_is_null(lhs)) {
                    char *lt = engine_node_text(a, lhs, source);
                    if (lt && lt[0] && rt && rt[0]) {
                        return engine_arena_sprintf(a, "%s.%s", lt, rt);
                    }
                }
                return rt;
            }
    ...
    if (!ts_node_is_null(name_node)) {
        ...
        if (!ts_node_is_null(obj) && name) {
            char *obj_text = engine_node_text(a, obj, source);
            if (obj_text && obj_text[0]) {
                return engine_arena_sprintf(a, "%s.%s", obj_text, name);
            }
        }
        return name;
    }
    ...
    if (!ts_node_is_null(method_node)) {
        char *method = engine_node_text(a, method_node, source);
        TSNode recv = ts_node_child_by_field_name(node, TS_FIELD("receiver"));
        if (!ts_node_is_null(recv) && method) {
            char *recv_text = engine_node_text(a, recv, source);
            if (recv_text && recv_text[0]) {
                return engine_arena_sprintf(a, "%s.%s", recv_text, method);
            }
        }
        return method;
    }
...
static char *extract_fp_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    ...
    while (fp_is_apply_kind(nk) && ts_node_child_count(node) > 0) {
        ...
        if (strcmp(ck, "identifier") == 0 || strcmp(ck, "variable") == 0 ||
            ...
            return engine_node_text(a, callee, source);
        }
    ...
    if (strcmp(nk, "infix") == 0 || strcmp(nk, "infix_expression") == 0) {
        ...
        if (!ts_node_is_null(op)) {
            return engine_node_text(a, op, source);
        }
        enum { INFIX_MIN_CHILDREN = 3, INFIX_OP_IDX = 1 };
        if (ts_node_child_count(node) >= INFIX_MIN_CHILDREN) {
            return engine_node_text(a, ts_node_child(node, INFIX_OP_IDX), source);
        }
    }
    return NULL;
}
...
static char *extract_wolfram_callee(EngineArena *a, TSNode node, const char *source) {
    ...
    if (ts_node_named_child_count(node) > 0) {
        ...
        if (strcmp(hk, "user_symbol") == 0 || strcmp(hk, "builtin_symbol") == 0) {
            return engine_node_text(a, head, source);
        }
    }
    return NULL;
}
...
static char *extract_swift_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_scripting_callee(EngineArena *a, TSNode node, const char *source,
    ...
    if (1 && strcmp(nk, "binary_operator") == 0) {
        ...
        char *operator_name = engine_node_text(a, op, source);
    ...
    if (1 && strcmp(nk, "call") == 0 && ts_node_child_count(node) > 0) {
        ...
        if (strcmp(fk, "identifier") == 0 || strcmp(fk, "dot") == 0) {
            return engine_node_text(a, first, source);
        }
        return NULL;
    }
    if (1 && ts_node_child_count(node) > 0) {
        ...
        char *pn = engine_node_text(a, name_node, source);
...
static char *extract_objc_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "message_expression") != 0) {
        return NULL;
    }
    TSNode selector = ts_node_child_by_field_name(node, TS_FIELD("selector"));
    return ts_node_is_null(selector) ? NULL : engine_node_text(a, selector, source);
}

// Erlang: extract callee from call node's first child.
static char *extract_erlang_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static bool lisp_definition_head(EngineLanguage lang, TSNode head, const char *source) {
...
static bool lisp_list_is_definition_role(EngineLanguage lang, TSNode node, const char *source) {
...
static char *extract_lisp_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_fsharp_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_css_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "call_expression") != 0) {
        return NULL;
    }
    TSNode fn = engine_find_child_by_kind(node, "function_name");
    return ts_node_is_null(fn) ? NULL : engine_node_text(a, fn, source);
}
...
static char *extract_linkerscript_callee(EngineArena *a, TSNode node, const char *source,
                                         const char *nk) {
    if (strcmp(nk, "call_expression") != 0) {
        return NULL;
    }
    ...
    return engine_node_text(a, function, source);
}

// PowerShell: a `command` node's callee is its `command_name` child.
static char *extract_powershell_callee(EngineArena *a, TSNode node, const char *source,
                                       const char *nk) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(ts_node_type(c), "command_name") == 0) {
            return engine_node_text(a, c, source);
        }
    }
    return NULL;
}
...
static char *extract_ada_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_solidity_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_groovy_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_wgsl_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_dart_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "selector") == 0) {
        ...
        if (!ts_node_is_null(prev) && strcmp(ts_node_type(prev), "identifier") == 0) {
            return engine_node_text(a, prev, source);
        }
        return NULL;
    }
...
static char *extract_scss_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "include_statement") == 0) {
        TSNode id = engine_find_child_by_kind(node, "identifier");
        return ts_node_is_null(id) ? NULL : engine_node_text(a, id, source);
    }
    ...
    if (strcmp(nk, "call_expression") == 0) {
        TSNode fn = engine_find_child_by_kind(node, "function_name");
        if (!ts_node_is_null(fn)) {
            return engine_node_text(a, fn, source);
        }
    }
    return NULL;
}
...
static char *extract_sql_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    ...
    return ts_node_is_null(nm) ? NULL : engine_node_text(a, nm, source);
}
...
static char *extract_cobol_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "call_statement") != 0) {
        return NULL;
    }
    TSNode x = ts_node_child_by_field_name(node, TS_FIELD("x"));
    if (ts_node_is_null(x)) {
        x = engine_find_child_by_kind(node, "string");
    }
    if (ts_node_is_null(x)) {
        return NULL;
    }
    char *text = engine_node_text(a, x, source);
...
static char *extract_elm_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    ...
    TSNode ve = strcmp(ts_node_type(target), "value_expr") == 0
                    ? target
                    : engine_find_child_by_kind(target, "value_expr");
    if (ts_node_is_null(ve)) {
        return NULL;
    }
    TSNode qid = ts_node_child_by_field_name(ve, TS_FIELD("name"));
    if (ts_node_is_null(qid)) {
        qid = engine_find_child_by_kind(ve, "value_qid");
    }
    if (ts_node_is_null(qid)) {
        return NULL;
    }
    TSNode id = engine_find_child_by_kind(qid, "lower_case_identifier");
    if (ts_node_is_null(id)) {
        // module-qualified call: emit the whole qualified id text
        return engine_node_text(a, qid, source);
    }
    return engine_node_text(a, id, source);
}
...
static char *extract_jsonnet_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "functioncall") != 0) {
        return NULL;
    }
    TSNode id = engine_find_child_by_kind(node, "id");
    return ts_node_is_null(id) ? NULL : engine_node_text(a, id, source);
}
...
// nickel_call_types.)
static char *extract_nickel_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    ...
    for (int depth = 0; depth < NICKEL_APPLY_DEPTH && !ts_node_is_null(cur); depth++) {
        ...
        if (strcmp(ck, "ident") == 0) {
            return engine_node_text(a, cur, source);
        }
...
static char *extract_typst_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "call") != 0) {
        return NULL;
    }
    TSNode item = ts_node_child_by_field_name(node, TS_FIELD("item"));
    return ts_node_is_null(item) ? NULL : engine_node_text(a, item, source);
}

// Meson: a builtin invocation (`executable(...)`, `dependency(...)`) is a
// `normal_command` whose `command` field is the called identifier.
static char *extract_meson_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "normal_command") != 0) {
        return NULL;
    }
    TSNode cmd = ts_node_child_by_field_name(node, TS_FIELD("command"));
    return ts_node_is_null(cmd) ? NULL : engine_node_text(a, cmd, source);
}
...
static char *first_leaf_identifier(EngineArena *a, TSNode node, const char *source) {
    TSNode cur = node;
    for (int depth = 0; depth < 8 && !ts_node_is_null(cur); depth++) {
        ...
        if (strcmp(k, "simple_identifier") == 0 || strcmp(k, "identifier") == 0 ||
            ...
            char *t = engine_node_text(a, cur, source);
            return (t && t[0]) ? t : NULL;
        }
...
static char *extract_hdl_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_vhdl_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    ...
    if (strcmp(pk, "library_function") == 0 || strcmp(pk, "identifier") == 0 ||
        ...
        char *t = engine_node_text(a, prev, source);
        return (t && t[0]) ? t : NULL;
    }
    return NULL;
}
...
static char *extract_nasm_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    ...
    char *m = engine_node_text(a, mnem, source);
...
static char *extract_llvm_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    ...
    if (!t) {
        t = engine_node_text(a, callee, source);
    }
    if (t && (t[0] == '@' || t[0] == '%')) {
        return t + 1;
    }
...
static char *extract_func_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_nix_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_agda_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "expr") != 0) {
        return NULL;
    }
    TSNode qid = agda_head_qid(agda_application_head_atom(node));
    return ts_node_is_null(qid) ? NULL : engine_node_text(a, qid, source);
}
...
static char *extract_make_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "shell_function") == 0) {
        return engine_arena_strndup(a, "shell", 5);
    }
...
static char *extract_just_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_puppet_callee(EngineArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "include_statement") == 0) {
        return engine_arena_strndup(a, "include", 7);
    }
    if (strcmp(nk, "function_call") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            TSNode head = ts_node_named_child(node, 0);
...
static char *extract_callee_lang_specific(EngineArena *a, TSNode node, const char *source,
                                          EngineLanguage lang) {
    ...
    if (1 && strcmp(nk, "call") == 0) {
        ...
        if (!ts_node_is_null(fnf) && strcmp(ts_node_type(fnf), "subscript") == 0) {
            ...
            if (!ts_node_is_null(val) && !ts_node_is_null(idx) &&
                ...
                return engine_node_text(a, val, source);
            }
        }
    }
...
static const char *php_chain_prefix_arg(EngineArena *a, TSNode call_node, const char *source) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ts_node_type(inner), "string") == 0 ||
            ...
            char *txt = engine_node_text(a, inner, source);
            if (txt && txt[0]) {
...
static const char *php_group_prefix_for_call(EngineArena *a, TSNode node, const char *source) {
    ...
    for (int depth = 0; depth < PHP_GROUP_WALK_MAX && !ts_node_is_null(cur); depth++) {
        if (strcmp(ts_node_type(cur), "anonymous_function") == 0 ||
            ...
            if (!ts_node_is_null(p) && (strcmp(ts_node_type(p), "member_call_expression") == 0 ||
                ...
                char *gtxt = ts_node_is_null(gname) ? NULL : engine_node_text(a, gname, source);
                if (gtxt && strcmp(gtxt, "group") == 0) {
                    ...
                    for (int hops = 0; hops < PHP_GROUP_WALK_MAX && !ts_node_is_null(recv);
                        ...
                        if (strcmp(rk, "member_call_expression") == 0 ||
                            ...
                            char *rtxt =
                                ts_node_is_null(rname) ? NULL : engine_node_text(a, rname, source);
    ...
    buf[pos] = '\0';
    return pos ? engine_arena_strndup(a, buf, pos) : NULL;
}
...
static char *extract_callee_name(EngineArena *a, TSNode node, const char *source, EngineLanguage lang) {
    ...
    if (1) {
        ...
        if (!ts_node_is_null(m) && !ts_node_is_null(recv) &&
            ...
            char *mt = engine_node_text(a, m, source);
            if (mt && strcmp(mt, "new") == 0) {
                char *rt = engine_node_text(a, recv, source);
                if (rt && rt[0]) {
                    return rt;
                }
            }
    ...
    if (1 && strcmp(ts_node_type(node), "scoped_call_expression") == 0) {
        ...
        if (!ts_node_is_null(scope) && !ts_node_is_null(mname)) {
            char *sc = engine_node_text(a, scope, source);
            char *mn = engine_node_text(a, mname, source);
            if (sc && mn && strcmp(sc, "Route") == 0) {
                char *qual = engine_arena_sprintf(a, "%s::%s", sc, mn);
                if (qual && engine_service_pattern_route_method(qual) != NULL) {
                    return qual;
                }
            }
        }
...
static const char *strip_and_validate_string_arg(EngineArena *a, char *text) {
    ...
    int len = (int)strlen(text);
    if (len >= ENGINE_QUOTE_PAIR && (text[0] == '"' || text[0] == '\'')) {
        text = engine_arena_strndup(a, text + ENGINE_QUOTE_OFFSET, (size_t)(len - ENGINE_QUOTE_PAIR));
        len -= ENGINE_QUOTE_PAIR;
    }
...
static char *gotemplate_string_child(EngineArena *a, TSNode parent, const char *source) {
    TSNode s = engine_find_child_by_kind(parent, "interpreted_string_literal");
    if (ts_node_is_null(s)) {
        return NULL;
    }
    char *text = engine_node_text(a, s, source);
...
static char *gotemplate_callee(EngineArena *a, TSNode node, const char *source) {
    ...
    if (strcmp(k, "function_call") == 0) {
        TSNode fn = engine_find_child_by_kind(node, "identifier");
        if (ts_node_is_null(fn)) {
            return NULL;
        }
        char *fname = engine_node_text(a, fn, source);
        ...
        if (ts_node_is_null(args)) {
            args = engine_find_child_by_kind(node, "argument_list");
        }
        if (ts_node_is_null(args)) {
            return NULL;
        }
...
static const char *extract_nth_string_arg(EngineExtractCtx *ctx, TSNode args, uint32_t n) {
    uint32_t nc = ts_node_named_child_count(args);
    uint32_t found = 0;
    for (uint32_t ai = 0; ai < nc && ai < MAX_POSITIONAL_SCAN + n; ai++) {
        ...
        if (is_string_like(ak)) {
            if (found == n) {
                char *text = engine_node_text(ctx->arena, arg, ctx->source);
...
static void process_keyword_arg(EngineExtractCtx *ctx, TSNode arg_node, EngineCallArg *ca) {
    ...
    if (!ts_node_is_null(key_n)) {
        ca->keyword = engine_node_text(ctx->arena, key_n, ctx->source);
    }
    if (!ts_node_is_null(val_n)) {
        ca->expr = engine_node_text(ctx->arena, val_n, ctx->source);
...
static void extract_call_args(EngineExtractCtx *ctx, TSNode args, EngineCall *call) {
    uint32_t argc = ts_node_named_child_count(args);
    int positional_idx = 0;
    for (uint32_t ai = 0; ai < argc && call->arg_count < ENGINE_MAX_CALL_ARGS; ai++) {
        TSNode arg_node = ts_node_named_child(args, ai);
        const char *ak = ts_node_type(arg_node);
        EngineCallArg *ca = &call->args[call->arg_count];
        ...
        if (strcmp(ak, "keyword_argument") == 0 || strcmp(ak, "pair") == 0) {
            ...
            ca->expr = engine_node_text(ctx->arena, arg_node, ctx->source);
            ca->index = positional_idx++;
...
static const char *extract_string_value(EngineExtractCtx *ctx, TSNode val_node) {
    const char *vk = ts_node_type(val_node);
    if (strcmp(vk, "template_string") == 0) {
        return engine_template_string_text(ctx->arena, val_node, ctx->source);
    }
    if (is_string_like(vk)) {
        char *text = engine_node_text(ctx->arena, val_node, ctx->source);
        if (text && text[0]) {
            return strip_quotes(ctx->arena, text);
        }
    } else if (strcmp(vk, "identifier") == 0) {
        char *const_name = engine_node_text(ctx->arena, val_node, ctx->source);
        if (const_name) {
...
static const char *extract_composite_queue_field(EngineExtractCtx *ctx, TSNode node) {
    ...
    if (ts_node_is_null(body)) {
        body = engine_find_child_by_kind(node, "literal_value");
    }
    if (ts_node_is_null(body)) {
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        char *key = engine_node_text(ctx->arena, key_n, ctx->source);
        ...
        char *raw = engine_node_text(ctx->arena, val_n, ctx->source);
        if (raw && raw[0]) {
            return raw;
        }
    }
...
static const char *extract_keyword_url(EngineExtractCtx *ctx, TSNode arg) {
    ...
    char *key = engine_node_text(ctx->arena, key_node, ctx->source);
...
static const char *extract_positional_url(EngineExtractCtx *ctx, TSNode arg, const char *ak) {
    ...
    if (strcmp(ak, "template_string") == 0) {
        const char *flat = engine_template_string_text(ctx->arena, arg, ctx->source);
        if (flat) {
    ...
    if (strcmp(ak, "identifier") == 0) {
        char *const_name = engine_node_text(ctx->arena, arg, ctx->source);
        if (const_name) {
            return lookup_string_constant(ctx, const_name);
        }
    }
...
static const char *extract_url_or_topic_arg(EngineExtractCtx *ctx, TSNode args) {
    uint32_t nc = ts_node_named_child_count(args);
...
static const char *normalize_string_handler(EngineArena *a, const char *raw) {
    ...
    if (at && at[1]) {
        return engine_arena_strdup(a, at + 1);
    }
    return unq;
}

static const char *extract_handler_arg(EngineExtractCtx *ctx, TSNode args) {
    uint32_t nc = ts_node_named_child_count(args);
    for (uint32_t ai = HANDLER_START_IDX; ai < nc && ai < MAX_HANDLER_SCAN; ai++) {
        ...
        if (strcmp(ak2, "identifier") == 0 || strcmp(ak2, "member_expression") == 0 ||
            ...
            return engine_node_text(ctx->arena, arg2, ctx->source);
        }
        if (is_string_like(ak2)) {
            const char *h =
                normalize_string_handler(ctx->arena, engine_node_text(ctx->arena, arg2, ctx->source));
            if (h && h[0]) {
                return h;
            }
        }
...
static void extract_jsx_component_ref(EngineExtractCtx *ctx, TSNode node, const char *kind,
    ...
    if (name && name[0] >= 'A' && name[0] <= 'Z') {
        ...
        engine_calls_push(&ctx->result->calls, ctx->arena, call);
    }
}
...
static bool kotlin_operator_call_matches(const EngineCall *existing, const char *callee_name,
...
static void push_kotlin_operator_call(EngineExtractCtx *ctx, TSNode node, const char *callee_name,
                                      const char *enclosing_func_qn) {
    ...
    if (ctx->result->calls.count > 0) {
        const EngineCall *existing = &ctx->result->calls.items[ctx->result->calls.count - SKIP_ONE];
...
static void extract_kotlin_operator_call(EngineExtractCtx *ctx, TSNode node, const char *kind,
                                         const char *enclosing_func_qn) {
...
static void kt_push_implicit_call(EngineExtractCtx *ctx, TSNode node, const char *callee,
                                  const char *enclosing_func_qn) {
    EngineCall call = {0};
...
static void push_cpp_semantic_call_candidate(EngineExtractCtx *ctx, TSNode node,
...
static const char *cpp_operator_token(EngineExtractCtx *ctx, TSNode node) {
    for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
        TSNode child = ts_node_child(node, i);
        if (!ts_node_is_named(child)) {
            const char *token = engine_node_text(ctx->arena, child, ctx->source);
            if (token && token[0]) {
                return token;
            }
        }
...
static void extract_cpp_operator_call(EngineExtractCtx *ctx, TSNode node, const char *kind,
                                      const char *enclosing_func_qn) {
    ...
    if (strcmp(kind, "unary_expression") == 0 || strcmp(kind, "pointer_expression") == 0 ||
        ...
        if (!ts_node_is_null(operand) &&
            ...
            push_cpp_semantic_call_candidate(
                ctx, node, engine_arena_sprintf(ctx->arena, "operator%s", token), enclosing_func_qn);
        }
        return;
    }
...
static void extract_cpp_implicit_calls(EngineExtractCtx *ctx, TSNode node, const char *kind,
                                       const char *enclosing_func_qn) {
    ...
    if (strcmp(kind, "delete_expression") == 0) {
        ...
        if (!ts_node_is_null(type) && !ts_node_is_null(decl) &&
            ...
            if (!ts_node_is_null(value) && strcmp(ts_node_type(value), "identifier") == 0) {
                char *tn = engine_node_text(ctx->arena, type, ctx->source);
                if (tn) {
...
static void extract_kotlin_desugared_calls(EngineExtractCtx *ctx, TSNode node, const char *kind,
                                           const char *enclosing_func_qn) {
    if (strcmp(kind, "property_declaration") == 0) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            for (uint32_t j = 0; j < vc; j++) {
                ...
                comp++;
                kt_push_implicit_call(ctx, node, engine_arena_sprintf(ctx->arena, "component%u", comp),
                                      enclosing_func_qn);
            }
            break;
        }
...
static bool is_dynamic_callee_expr(const EngineExtractCtx *ctx, TSNode expr) {
...
static TSNode terminal_callee_leaf(const EngineExtractCtx *ctx, TSNode expr) {
...
static TSNode language_specific_callee_expr(EngineLanguage language, TSNode node) {
    const char *kind = ts_node_type(node);
...
static TSNode language_specific_callee_leaf(EngineLanguage language, TSNode expr,
                                            const EngineExtractCtx *ctx) {
    if (ts_node_is_null(expr)) {
        return (TSNode){0};
    }
...
static EnginePrimaryCalleeSelection select_primary_callee(EngineExtractCtx *ctx, TSNode node,
                                                       WalkState *state) {
    EnginePrimaryCalleeSelection selection = {0};
...
static bool primary_callee_name_is_allowed(const EngineExtractCtx *ctx,
                                           const EnginePrimaryCalleeSelection *callee) {
    if (!callee->name || !callee->name[0]) {
        return false;
    }
    if (!engine_is_keyword(callee->name, ctx->language) ||
        engine_is_resolvable_builtin(callee->name, ctx->language)) {
        return true;
    }
...
static EngineInvocationDescriptor describe_callable_reference(EngineExtractCtx *ctx, TSNode node) {
    EngineInvocationDescriptor descriptor = {0};
    descriptor.kind = ENGINE_INVOCATION_CALLABLE_REFERENCE;
    ...
    if (!ts_node_is_null(name_node)) {
        descriptor.callee_name = engine_node_text(ctx->arena, name_node, ctx->source);
    }
    return descriptor;
}

static EngineInvocationDescriptor describe_emitted_primary_call(
    TSNode node, const EnginePrimaryCalleeSelection *callee) {
    EngineInvocationDescriptor descriptor = {0};
    descriptor.kind = ENGINE_INVOCATION_PRIMARY;
    descriptor.site = node;
...
EngineInvocationDescriptor handle_calls(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec,
                                     WalkState *state) {
    EngineInvocationDescriptor invocation = {0};
    ...
    if (!callable_reference && spec->call_node_types && spec->call_node_types[0] &&
        engine_kind_in_set(node, spec->call_node_types)) {
        EnginePrimaryCalleeSelection callee = select_primary_callee(ctx, node, state);
        ...
        if (primary_callee_name_is_allowed(ctx, &callee)) {
                ...
                if (1 && call.first_string_arg &&
                    ...
                    if (gp && gp[0]) {
                        ...
                        call.first_string_arg =
                            rel[0] ? engine_arena_sprintf(ctx->arena, "%s/%s", gp, rel)
                                   : engine_arena_strndup(ctx->arena, gp, strlen(gp));
                    }
                }
            ...
            const char **dispatch_suffixes = engine_string_dispatch_suffixes(ctx->language);
            if (dispatch_suffixes && !ts_node_is_null(args)) {
                ...
                for (const char **nm = dispatch_suffixes; *nm; nm++) {
                    ...
                    if (len >= nlen && strcmp(cn + len - nlen, *nm) == 0) {
                        ...
                        if (cls && mth) {
                            EngineCall xcall = {0};
                            xcall.callee_name = engine_arena_sprintf(ctx->arena, "%s.%s", cls, mth);
                            ...
                            engine_calls_push(&ctx->result->calls, ctx->arena, xcall);
                        }
                        break;
                    }
                }
