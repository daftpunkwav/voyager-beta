#include "engine.h"
#include "arena.h" // EngineArena, engine_arena_alloc/strdup/sprintf
#include "helpers.h"
#include "lang_specs.h"
#include "foundation/constants.h"
#include "foundation/platform.h" // safe_realloc (frees old on failure)
#include "foundation/log.h"      // engine_log_warn
...
#define MAX_PARAMS ENGINE_SZ_32
...
enum {
    ...
    DECLARATOR_DEPTH_LIMIT = ENGINE_DECLARATOR_DEPTH_LIMIT, // shared define in helpers.h

    EXPORT_ANCESTOR_DEPTH = 4,
    FUNC_PARENT_CLIMB_LIMIT = 4,    /* Nix header lambdas to descend before the file's body: `{ pkgs, ... }:` is one,
...
static char *extract_body_ident_tokens(EngineExtractCtx *ctx, TSNode body) {
    ...
    while (bt_top > 0 && bt_count < BT_MAX_IDENTS) {
        ...
        if (nc == 0) {
            ...
            if (strcmp(k, "identifier") == 0 || strcmp(k, "field_identifier") == 0 ||
                ...
                if (len > 0 && len < ENGINE_SZ_64 && s < (uint32_t)ctx->source_len) {
    ...
    if (bt_pos > 0) {
        bt_buf[bt_pos] = '\0';
        return engine_arena_strdup(ctx->arena, bt_buf);
    }
    return NULL;
}
...
static void compute_fingerprint(EngineExtractCtx *ctx, EngineDefinition *def, TSNode func_node) {
    ...
    engine_minhash_t result;
    if (!engine_minhash_compute(body, ctx->source, (int)ctx->language, &result)) {
        return; /* Too short or empty — no fingerprint */
    }
    /* Arena-allocate the fingerprint array */
    uint32_t *fp = engine_arena_alloc(ctx->arena, ENGINE_MINHASH_K * sizeof(uint32_t));
    if (!fp) {
        return;
    }
    memcpy(fp, result.values, ENGINE_MINHASH_K * sizeof(uint32_t));
    def->fingerprint = fp;
    def->fingerprint_k = ENGINE_MINHASH_K;

    /* AST structural profile (signals 8, 9, 11) — rides the same body node */
    engine_ast_profile_t profile;
    int pc = 0;
    if (def->param_names) {
    ...
    if (engine_ast_profile_compute(body, ctx->source, def->param_names, pc, &profile)) {
        profile.body_lines = (uint16_t)def->lines;
        char sp_buf[ENGINE_AST_PROFILE_BUF];
        engine_ast_profile_to_str(&profile, sp_buf, sizeof(sp_buf));
        def->structural_profile = engine_arena_strdup(ctx->arena, sp_buf);
    }
}
...
static void extract_func_def(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec);
static void extract_class_def(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec);
static void walk_defs(EngineExtractCtx *ctx, TSNode root, const EngineLangSpec *spec, int depth_unused);
static void extract_variables(EngineExtractCtx *ctx, TSNode root, const EngineLangSpec *spec);
static void extract_var_names(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec);
static void extract_class_variables(EngineExtractCtx *ctx, TSNode class_node, const char *class_qn,
                                    const EngineLangSpec *spec);
static void extract_rust_impl(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec);
static void extract_class_methods(EngineExtractCtx *ctx, TSNode class_node, const char *class_qn,
                                  const EngineLangSpec *spec);
static void extract_class_fields(EngineExtractCtx *ctx, TSNode class_node, const char *class_qn,
                                 const EngineLangSpec *spec);
static TSNode find_class_body(TSNode class_node, EngineLanguage lang);
static void extract_enum_members(EngineExtractCtx *ctx, TSNode node, const char *class_qn);
static void extract_elixir_call(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec);
...
static TSNode func_name_node(TSNode node) {
    ...
    if (ts_node_is_null(name)) {
        ...
        name = engine_find_child_by_kind(node, "rpc_name");
    }
    return name;
}
...
static TSNode resolve_ocaml_func_name(TSNode node) {
    TSNode binding = engine_find_child_by_kind(node, "let_binding");
...
static TSNode resolve_sql_func_name(TSNode node) {
    TSNode obj_ref = engine_find_child_by_kind(node, "object_reference");
    if (!ts_node_is_null(obj_ref)) {
        TSNode id = engine_find_child_by_kind(obj_ref, "identifier");
        if (!ts_node_is_null(id)) {
            return id;
        }
    }
    return engine_find_child_by_kind(node, "identifier");
}

// Zig: resolve test_declaration name from string→string_content.
static TSNode resolve_zig_test_name(TSNode node) {
    TSNode str_node = engine_find_child_by_kind(node, "string");
    if (!ts_node_is_null(str_node)) {
        TSNode content = engine_find_child_by_kind(str_node, "string_content");
...
static TSNode resolve_vimscript_func_name(TSNode node) {
    TSNode decl = engine_find_child_by_kind(node, "function_declaration");
...
static TSNode resolve_commonlisp_func_name(TSNode node) {
    ...
    TSNode header = engine_find_child_by_kind(node, "defun_header");
    if (!ts_node_is_null(header)) {
        return engine_find_child_by_kind(header, "sym_lit");
    }
    TSNode null_node = {0};
    return null_node;
}
...
static TSNode resolve_elm_func_name(TSNode node) {
    ...
    if (ts_node_is_null(fdl)) {
        fdl = engine_find_child_by_kind(node, "function_declaration_left");
    }
...
char *engine_cpp_out_of_line_parent_class(EngineArena *a, TSNode node, const char *source) {
    ...
    char *text = engine_node_text(a, scope, source);
...
// like port lists).
enum { ENGINE_DESCENDANT_MAX_DEPTH = 6 };
...
// unified/calls extractor shares this one resolver — see engine_resolve_func_name.
TSNode engine_resolve_func_name(TSNode node, EngineLanguage lang);
    ...
    for (int tmpl_depth = 0; tmpl_depth < MAX_TEMPLATE_DEPTH; tmpl_depth++) {
        ...
        if (lang == ENGINE_LANG_ADA &&
            ...
            if (ts_node_is_null(spec)) {
                spec = engine_find_child_by_kind(node, "function_specification");
            }
            if (!ts_node_is_null(spec)) {
        ...
        if (lang == ENGINE_LANG_NIX && strcmp(kind, "function_expression") == 0) {
            ...
            if (!ts_node_is_null(parent) && strcmp(ts_node_type(parent), "binding") == 0) {
                ...
                if (!ts_node_is_null(attrpath)) {
                    TSNode attr = engine_nix_attrpath_last_attr(attrpath);
        ...
        if (lang == ENGINE_LANG_FORTRAN &&
            ...
            if (ts_node_is_null(stmt)) {
                stmt = engine_find_child_by_kind(node, "function_statement");
            }
            if (!ts_node_is_null(stmt)) {
        ...
        if (lang == ENGINE_LANG_FSHARP && strcmp(kind, "function_or_value_defn") == 0) {
            ...
            if (!ts_node_is_null(lhs)) {
                TSNode nm = engine_find_child_by_kind(lhs, "identifier");
                if (ts_node_is_null(nm)) {
                    nm = engine_find_child_by_kind(lhs, "long_identifier");
                }
                if (!ts_node_is_null(nm)) {
                    return nm;
                }
        ...
        if (lang == ENGINE_LANG_ELM && strcmp(kind, "value_declaration") == 0) {
            ...
            if (!ts_node_is_null(lhs)) {
                TSNode nm = engine_find_child_by_kind(lhs, "lower_case_identifier");
                if (!ts_node_is_null(nm)) {
                    return nm;
                }
            }
        ...
        if (lang == ENGINE_LANG_VHDL && (strcmp(kind, "subprogram_declaration") == 0 ||
            ...
            if (ts_node_is_null(spec)) {
                spec = engine_find_child_by_kind(node, "procedure_specification");
            }
            if (!ts_node_is_null(spec)) {
        ...
        if (lang == ENGINE_LANG_CMAKE &&
            ...
            TSNode cmd = engine_find_child_by_kind(node, cmd_kind);
            if (!ts_node_is_null(cmd)) {
                TSNode alist = engine_find_child_by_kind(cmd, "argument_list");
                if (!ts_node_is_null(alist)) {
                    TSNode arg = engine_find_child_by_kind(alist, "argument");
                    if (!ts_node_is_null(arg)) {
                        TSNode uq = engine_find_child_by_kind(arg, "unquoted_argument");
        ...
        if (lang == ENGINE_LANG_PUPPET &&
            ...
            if (ts_node_is_null(id)) {
                id = engine_find_child_by_kind(node, "class_identifier");
            }
            if (!ts_node_is_null(id)) {
                return id;
            }
...
static bool is_js_exported(TSNode node) {
    return engine_has_ancestor_kind(node, "export_statement", EXPORT_ANCESTOR_DEPTH);
}
...
static char *extract_comment_text(EngineArena *a, TSNode node, const char *source) {
    char *text = engine_node_text(a, node, source);
...
static const char *extract_go_type_docstring(EngineArena *a, TSNode node, const char *source) {
    const char *kind = ts_node_type(node);
...
static const char *extract_python_docstring(EngineArena *a, TSNode node, const char *source) {
...
static TSNode find_jvm_modifiers(TSNode node, EngineLanguage lang);
...
static const char *route_path_from_string_node(EngineArena *a, TSNode node, const char *source) {
    if (!is_route_string_kind(ts_node_type(node))) {
        return NULL;
    }
    char *path = engine_node_text(a, node, source);
    if (!path) {
        return NULL;
    }
    int plen = (int)strlen(path);
    if (plen >= PAIR_CHARS && (path[0] == '"' || path[0] == '\'')) {
        path = engine_arena_strndup(a, path + SKIP_CHAR, (size_t)(plen - PAIR_CHARS));
    }
    return (path && path[0] == '/') ? path : NULL;
}

static const char *find_route_path_literal(EngineArena *a, TSNode node, const char *source,
                                           int max_depth) {
...
static const char *extract_route_path_from_args(EngineArena *a, TSNode args, const char *source) {
    ...
    for (uint32_t ai = 0; ai < nc && ai < DECORATOR_SCAN_LIMIT; ai++) {
        ...
        const char *path = find_route_path_literal(a, arg, source, ENGINE_DESCENDANT_MAX_DEPTH);
        if (path) {
            return path;
        }
    }
...
static TSNode find_drf_kwarg_in_args(EngineArena *a, TSNode args, const char *kwarg_name,
                                     const char *source) {
    uint32_t nc = ts_node_named_child_count(args);
    for (uint32_t ai = 0; ai < nc; ai++) {
        ...
        if (ts_node_is_null(name_node))
            continue;
        char *name = engine_node_text(a, name_node, source);
...
static bool try_drf_action_decorator(EngineArena *a, TSNode dchild, const char *source,
    ...
    char *fn_text = engine_node_text(a, fn, source);
    ...
    if (!ts_node_is_null(methods_val) && strcmp(ts_node_type(methods_val), "list") == 0) {
        ...
        for (uint32_t mi = 0; mi < mc && !method; mi++) {
            ...
            if (strcmp(ts_node_type(item), "string") != 0)
                continue;
            char *text = engine_node_text(a, item, source);
            if (!text)
                continue;
            int tlen = (int)strlen(text);
            if (tlen < PAIR_CHARS || (text[0] != '"' && text[0] != '\''))
                continue;
            char inner[ENGINE_SZ_16];
            ...
            method = engine_arena_strdup(a, inner);
        }
    }
    if (!method) {
        method = "GET";
    ...
    if (!ts_node_is_null(url_path_val) && strcmp(ts_node_type(url_path_val), "string") == 0) {
        char *text = engine_node_text(a, url_path_val, source);
        if (text) {
            int tlen = (int)strlen(text);
            if (tlen >= PAIR_CHARS && (text[0] == '"' || text[0] == '\'')) {
                segment = engine_arena_strndup(a, text + SKIP_CHAR, (size_t)(tlen - PAIR_CHARS));
            }
        }
    }
    if (!segment) {
        ...
        if (!ts_node_is_null(name_node)) {
            segment = engine_node_text(a, name_node, source);
        }
    }
    if (!segment) {
        return false;
    ...
    if (detail) {
        *out_path = engine_arena_sprintf(a, "/{pk}/%s", segment);
    } else {
        *out_path = engine_arena_sprintf(a, "/%s", segment);
    }
    *out_method = method;
    return true;
}
...
static bool try_route_from_decorator_call(EngineArena *a, TSNode dchild, const char *source,
...
static TSNode annotation_name_node(TSNode annotation) {
    ...
    TSNode ut = engine_find_child_by_kind(annotation, "user_type");
    if (ts_node_is_null(ut)) {
        TSNode ci = engine_find_child_by_kind(annotation, "constructor_invocation");
        if (!ts_node_is_null(ci)) {
            ut = engine_find_child_by_kind(ci, "user_type");
        }
    }
    if (!ts_node_is_null(ut)) {
        TSNode ti = engine_find_child_by_kind(ut, "type_identifier");
        if (ts_node_is_null(ti)) {
            ti = engine_find_child_by_kind(ut, "simple_identifier");
        }
        return ti;
    }
    TSNode null_node = {0};
...
static TSNode annotation_args_node(TSNode annotation) {
    ...
    TSNode ci = engine_find_child_by_kind(annotation, "constructor_invocation");
    if (!ts_node_is_null(ci)) {
        return engine_find_child_by_kind(ci, "value_arguments");
    }
    return args;
}
...
static bool try_route_from_annotation(EngineArena *a, TSNode annotation, const char *source,
                                      const char **out_path, const char **out_method) {
...
static void scan_route_annotations(EngineArena *a, TSNode owner, const char *source,
                                   const EngineLangSpec *spec, const char **out_map_path,
    ...
    *out_jax_path = NULL;
    ...
    for (int w = 0; w < wn; w++) {
        ...
        for (uint32_t ci = 0; ci < cc; ci++) {
            ...
            if (!engine_kind_in_set(child, spec->decorator_node_types)) {
                continue;
            }
...
static bool extract_route_from_annotations(EngineArena *a, TSNode func_node, const char *source,
                                           const EngineLangSpec *spec, const char **out_path,
                                           const char **out_method) {
...
static void extract_route_from_decorators(EngineArena *a, TSNode func_node, const char *source,
                                          const EngineLangSpec *spec, const char **out_path,
                                          const char **out_method) {
    *out_path = NULL;
    *out_method = NULL;
    ...
    while (!ts_node_is_null(prev)) {
        if (!engine_kind_in_set(prev, spec->decorator_node_types)) {
            break;
        }
...
static const char *join_route_paths(EngineArena *a, const char *prefix, const char *path) {
    if (!path || !path[0]) {
        return prefix;
    }
    ...
    if (prefix_slash && path_slash) {
        return engine_arena_sprintf(a, "%s%s", prefix, path + SKIP_CHAR);
    }
    if (!prefix_slash && !path_slash) {
        return engine_arena_sprintf(a, "%s/%s", prefix, path);
    }
    return engine_arena_sprintf(a, "%s%s", prefix, path);
}

static const char *spring_class_route_prefix(EngineArena *a, TSNode class_node, const char *source,
                                             const EngineLangSpec *spec) {
    const char *map_path = NULL;
    const char *method = NULL;
    const char *jax_path = NULL;
...
static int count_modifier_annotations(TSNode modifiers, const EngineLangSpec *spec) {
    int count = 0;
    uint32_t mc = ts_node_child_count(modifiers);
    for (uint32_t mi = 0; mi < mc; mi++) {
        TSNode mchild = ts_node_child(modifiers, mi);
        if (engine_kind_in_set(mchild, spec->decorator_node_types)) {
            count++;
        }
    }
    return count;
...
static TSNode find_jvm_modifiers(TSNode node, EngineLanguage lang) {
    ...
    switch (lang) {
    case ENGINE_LANG_JAVA:
    case ENGINE_LANG_KOTLIN:
    case ENGINE_LANG_SWIFT:
        wrapper = "modifiers";
        break;
    ...
    if (ts_node_is_null(w)) {
        w = engine_find_child_by_kind(node, wrapper);
    }
    return w;
}
...
static int count_child_decorators(TSNode node, const EngineLangSpec *spec) {
    int count = 0;
...
static int collect_child_decorators(EngineArena *a, TSNode node, const char *source,
                                    const EngineLangSpec *spec, const char **result, int idx,
                                    int max) {
    uint32_t cc = ts_node_child_count(node);
    for (uint32_t ci = 0; ci < cc && idx < max; ci++) {
        TSNode child = ts_node_child(node, ci);
        if (engine_kind_in_set(child, spec->decorator_node_types)) {
            result[idx++] = engine_node_text(a, child, source);
        }
    }
    return idx;
}
...
static int collect_modifier_decorators(EngineArena *a, TSNode modifiers, const char *source,
                                       const EngineLangSpec *spec, const char **result, int idx,
                                       int max) {
    uint32_t mc = ts_node_child_count(modifiers);
    for (uint32_t mi = 0; mi < mc && idx < max; mi++) {
        TSNode mchild = ts_node_child(modifiers, mi);
        if (engine_kind_in_set(mchild, spec->decorator_node_types)) {
            result[idx++] = engine_node_text(a, mchild, source);
        }
    }
    return idx;
}
...
static const char **extract_decorators(EngineArena *a, TSNode node, const char *source,
                                       EngineLanguage lang, const EngineLangSpec *spec) {
    if (!spec->decorator_node_types || !spec->decorator_node_types[0]) {
        return NULL;
    }
    ...
    const char **result =
        (const char **)engine_arena_alloc(a, sizeof(const char *) * (total + NULL_TERM));
    if (!result) {
        return NULL;
    }
    ...
    while (!ts_node_is_null(prev) && idx < count) {
        if (engine_kind_in_set(prev, spec->decorator_node_types)) {
            result[idx++] = engine_node_text(a, prev, source);
        } else if (ts_node_is_named(prev) && !is_comment_node(ts_node_type(prev))) {
            break;
        }
        prev = ts_node_prev_sibling(prev);
 ...
 * engine's test detection is otherwise file-path-based (engine_is_test_file:
...
static const char *rust_cfg_qualified_name(EngineArena *a, const char *base_qn,
    ...
    for (int i = 0; decorators[i]; i++) {
        ...
        buf[bi] = '\0';
        return engine_arena_sprintf(a, "%s#%s", base_qn, buf);
    }
    return base_qn;
}

// Extract base class name text from a single base_class child node.
static char *extract_cpp_base_text(EngineArena *a, TSNode bc, const char *source) {
    ...
    if (strcmp(bk, "type_identifier") == 0 || strcmp(bk, "qualified_identifier") == 0 ||
        ...
        char *t = engine_node_text(a, bc, source);
        if (t) {
            char *angle = strchr(t, '<');
            if (angle) {
                *angle = '\0';
    ...
    if (strcmp(bk, "template_type") == 0) {
        ...
        if (!ts_node_is_null(tname)) {
            return engine_node_text(a, tname, source);
        }
    }
    return NULL;
}
...
static const char **extract_cpp_base_classes(EngineArena *a, TSNode clause, const char *source) {
    ...
    if (base_count > 0) {
        const char **result =
            (const char **)engine_arena_alloc(a, (base_count + NULL_TERM) * sizeof(const char *));
        if (result) {
...
static const char **make_single_base(EngineArena *a, const char *text) {
    if (!text || !text[0]) {
        return NULL;
    }
    const char **result = (const char **)engine_arena_alloc(a, sizeof(const char *) * RT_PAIR_SIZE);
    if (result) {
        result[0] = text;
...
static const char **find_base_from_children(EngineArena *a, TSNode node, const char *source,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        for (const char **t = base_types; *t; t++) {
            if (strcmp(ck, *t) == 0) {
                const char **r = make_single_base(a, engine_node_text(a, child, source));
                if (r) {
                    return r;
                }
            }
...
static const char *extract_csharp_base_child_text(EngineArena *a, TSNode bc, const char *source) {
    const char *bk = ts_node_type(bc);
    char *text = NULL;
    if (strcmp(bk, "identifier") == 0 || strcmp(bk, "generic_name") == 0 ||
        strcmp(bk, "qualified_name") == 0) {
        text = engine_node_text(a, bc, source);
    } else {
        TSNode inner = ts_node_named_child(bc, 0);
        if (!ts_node_is_null(inner)) {
            text = engine_node_text(a, inner, source);
        }
    }
    if (text && text[0]) {
...
static const char **collect_csharp_bases(EngineArena *a, TSNode base_list, const char *source) {
    const char *bases[MAX_BASES];
    int base_count = 0;
...
static const char **extract_csharp_base_list(EngineArena *a, TSNode node, const char *source,
                                             uint32_t count) {
    for (uint32_t i = 0; i < count; i++) {
        TSNode child = ts_node_child(node, i);
...
static void push_base_text(EngineArena *a, TSNode n, const char *source, const char **out, int out_cap,
                           int *count) {
    if (*count >= out_cap) {
        return;
    }
    char *t = engine_node_text(a, n, source);
    if (!t) {
        return;
    }
    char *angle = strchr(t, '<');
...
static int collect_ts_bases(EngineArena *a, TSNode clause, const char *source, const char **out,
...
static const char **extract_ts_bases(EngineArena *a, TSNode node, const char *source) {
    const char *bases[MAX_BASES];
    int count = 0;
    ...
    const char **result =
        (const char **)engine_arena_alloc(a, (size_t)(count + NULL_TERM) * sizeof(const char *));
    if (!result) {
        return NULL;
    }
...
static const char **extract_php_bases(EngineArena *a, TSNode node, const char *source) {
    const char *bases[MAX_BASES];
    int count = 0;
    uint32_t nc = ts_node_child_count(node);
    for (uint32_t i = 0; i < nc; i++) {
...
static const char **extract_kotlin_bases(EngineArena *a, TSNode node, const char *source) {
    const char *bases[MAX_BASES];
    int count = 0;
    uint32_t nc = ts_node_child_count(node);
...
static int collect_bases_from_field(EngineArena *a, TSNode field_node, const char *source,
    ...
    if (strcmp(fk, "type_identifier") == 0 || strcmp(fk, "generic_type") == 0 ||
        ...
        char *t = engine_node_text(a, field_node, source);
        if (t) {
            char *angle = strchr(t, '<');
            if (angle) {
                *angle = '\0';
    ...
    for (uint32_t i = 0; i < nc && count < out_cap; i++) {
        ...
        if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "generic_type") == 0 ||
            ...
            char *t = engine_node_text(a, child, source);
            if (t) {
                char *angle = strchr(t, '<');
                if (angle) {
                    *angle = '\0';
            ...
            if (!ts_node_is_null(val)) {
                char *t = engine_node_text(a, val, source);
                if (t && t[0]) {
                    out[count++] = t;
                }
            }
            ...
            for (uint32_t ti = 0; ti < tlnc && count < out_cap; ti++) {
                ...
                if (strcmp(tlk, "type_identifier") == 0 || strcmp(tlk, "generic_type") == 0 ||
                    ...
                    char *t = engine_node_text(a, tl_child, source);
                    if (t) {
                        char *angle = strchr(t, '<');
                        if (angle) {
                            *angle = '\0';
...
static const char **extract_julia_base_classes(EngineArena *a, TSNode node, const char *source) {
    ...
    char *bname = engine_node_text(a, base, source);
    if (!bname || !bname[0]) {
        return NULL;
    }
    const char **result = (const char **)engine_arena_alloc(a, 2 * sizeof(const char *));
    if (!result) {
        return NULL;
    }
    result[0] = bname;
...
static const char **extract_base_classes(EngineArena *a, TSNode node, const char *source,
                                         EngineLanguage lang) {
    ...
    if (lang == ENGINE_LANG_FSHARP) {
        ...
        if (!ts_node_is_null(inh)) {
            TSNode st = engine_find_child_by_kind(inh, "simple_type");
            char *bn = ts_node_is_null(st) ? NULL : engine_node_text(a, st, source);
            if (bn && bn[0]) {
                const char **result = (const char **)engine_arena_alloc(a, 2 * sizeof(const char *));
                if (result) {
                    result[0] = bn;
    ...
    if (lang == ENGINE_LANG_DLANG) {
        ...
        for (uint32_t i = 0; i < nc && pc < MAX_BASES_MINUS_1; i++) {
            ...
            char *bn = engine_node_text(a, c, source);
            if (bn && bn[0]) {
                pbases[pc++] = bn;
            }
        }
        if (pc > 0) {
            const char **result =
                (const char **)engine_arena_alloc(a, (pc + NULL_TERM) * sizeof(const char *));
            if (result) {
                for (int i = 0; i < pc; i++) {
                    result[i] = pbases[i];
                }
    ...
    if (lang == ENGINE_LANG_PASCAL && strcmp(ts_node_type(node), "declClass") == 0) {
        ...
        for (uint32_t i = 0; i < nc && pc < MAX_BASES_MINUS_1; i++) {
            ...
            char *bn = engine_node_text(a, pn, source);
            if (bn && bn[0]) {
                pbases[pc++] = bn;
            }
        }
...
static char *clean_type_name(EngineArena *a, const char *raw) {
    if (!raw || !raw[0]) {
        return NULL;
    }
    const char *s = raw;
    ...
    char *result = engine_arena_alloc(a, len + NULL_TERM);
    memcpy(result, s, len);
    result[len] = '\0';
    return result;
}
...
static char *resolve_param_name(EngineArena *a, TSNode param, const char *source) {
    ...
    if (strcmp(pk, "argument") == 0 || strcmp(pk, "tag_parameter") == 0) {
        ...
        return ts_node_is_null(name) ? NULL : engine_node_text(a, name, source);
    }

    if (strcmp(pk, "parameter_declaration") == 0) {
        TSNode nm = ts_node_child_by_field_name(param, TS_FIELD("name"));
        if (!ts_node_is_null(nm)) {
            return engine_node_text(a, nm, source);
        }
        return NULL;
    }
    if (strcmp(pk, "identifier") == 0) {
        return engine_node_text(a, param, source);
    }
...
static const char **extract_param_names(EngineArena *a, TSNode params, const char *source,
                                        EngineLanguage lang) {
    (void)lang;
    if (ts_node_is_null(params)) {
        return NULL;
    ...
    const char **result =
        (const char **)engine_arena_alloc(a, (count + NULL_TERM) * sizeof(const char *));
    for (int i = 0; i < count; i++) {
        result[i] = names[i];
    }
    result[count] = NULL;
...
static void add_cleaned_type(EngineArena *a, const char **types, int *count, char *type_text) {
...
static void extract_go_multi_return(EngineArena *a, TSNode rt_node, const char *source,
    ...
    for (uint32_t i = 0; i < nc && *count < MAX_RETURN_TYPES_MINUS_1; i++) {
        ...
        if (strcmp(ts_node_type(child), "parameter_declaration") == 0) {
            ...
            if (!ts_node_is_null(tn)) {
                add_cleaned_type(a, types, count, engine_node_text(a, tn, source));
            }
        } else {
            add_cleaned_type(a, types, count, engine_node_text(a, child, source));
        }
    }
}

// Build a NULL-terminated arena-allocated string array from a types buffer.
static const char **build_type_array(EngineArena *a, const char **types, int count) {
    if (count == 0) {
        return NULL;
    }
    const char **result =
...
static const char **extract_return_types(EngineArena *a, TSNode rt_node, const char *source,
                                         EngineLanguage lang) {
    (void)lang;
    ...
    if (strcmp(ts_node_type(rt_node), "parameter_list") == 0) {
        ...
        add_cleaned_type(a, types, &count, engine_node_text(a, rt_node, source));
    }
...
static char *extract_ts_param_type(EngineArena *a, TSNode param, const char *source) {
    TSNode ta = engine_find_child_by_kind(param, "type_annotation");
    ...
    for (uint32_t ti = 0; ti < tanc; ti++) {
        ...
        if (strcmp(tk, "type_identifier") == 0 || strcmp(tk, "generic_type") == 0 ||
            ...
            return engine_node_text(a, tch, source);
        }
    }
    return NULL;
}
...
static char *resolve_jvm_param_type(EngineArena *a, TSNode param, const char *pk, const char *source,
                                    EngineLanguage lang) {
...
static char *resolve_param_type_text(EngineArena *a, TSNode param, const char *source,
                                     EngineLanguage lang) {
    const char *pk = ts_node_type(param);
...
static void add_dedup_type(EngineArena *a, const char **types, int *count, char *type_text) {
    if (!type_text || !type_text[0]) {
        return;
    }
    char *cleaned = clean_type_name(a, type_text);
...
static const char **extract_param_types(EngineArena *a, TSNode params, const char *source,
                                        EngineLanguage lang) {
    if (ts_node_is_null(params)) {
        return NULL;
    }
...
static char *signature_receiver_name(EngineArena *a, TSNode param, const char *source,
    ...
    for (int depth = 0; depth < 4 && !ts_node_is_null(candidate); depth++) {
        if (ts_node_named_child_count(candidate) == 0) {
            return engine_node_text(a, candidate, source);
        }
        if (!unwrap_wrappers) {
            return NULL;
        }
...
static bool is_signature_implicit_receiver(EngineArena *a, TSNode param, const char *source,
                                           EngineLanguage lang, bool method, bool first_parameter) {
    const char *pk = ts_node_type(param);
...
static bool is_signature_non_parameter(TSNode param, const char *source, EngineLanguage lang) {
    const char *pk = ts_node_type(param);
...
static char *resolve_signature_param_type_text(EngineArena *a, TSNode param, const char *source,
                                               EngineLanguage lang) {
    ...
    if (!ts_node_is_null(type_node)) {
        ...
        char *type_text = engine_node_text(a, type_node, source);
        if (type_text && type_text[0]) {
            return type_text;
        }
    }

    TSNode annotation = engine_find_child_by_kind(param, "type_annotation");
    if (!ts_node_is_null(annotation)) {
        uint32_t nc = ts_node_named_child_count(annotation);
        if (nc > 0) {
            char *type_text = engine_node_text(a, ts_node_named_child(annotation, 0), source);
            if (type_text && type_text[0]) {
                return type_text;
            }
        }
    ...
    TSNode typed_pattern = engine_find_child_by_kind(param, "typed_pattern");
...
static const char **extract_signature_param_types(EngineArena *a, TSNode params, const char *source,
                                                  EngineLanguage lang, bool method, int *out_count) {
    *out_count = 0;
    if (ts_node_is_null(params)) {
        return NULL;
    }
    ...
    const char **result =
        (const char **)engine_arena_alloc(a, (size_t)(*out_count + NULL_TERM) * sizeof(const char *));
    if (!result) {
        *out_count = 0;
        return NULL;
    }
...
static TSNode unwrap_template_inner(TSNode node, EngineLanguage lang) {
...
static void resolve_cpp_trailing_return(EngineArena *a, TSNode func_node, const char *source,
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ts_node_type(ch), "trailing_return_type") == 0) {
            ...
            def->return_type = engine_node_text(a, type_desc, source);
            if (def->return_type && def->return_type[0]) {
                const char **rt =
                    (const char **)engine_arena_alloc(a, RT_PAIR_SIZE * sizeof(const char *));
                if (rt) {
...
static void set_def_complexity(EngineDefinition *def, TSNode body, const EngineLangSpec *spec) {
    engine_complexity_t cx;
    engine_compute_complexity(body, spec->branching_node_types, &cx);
...
static char *go_receiver_type_name(EngineArena *a, TSNode recv, const char *source) {
...
static char *resolve_cpp_test_macro_name(EngineArena *a, const char *macro, TSNode node,
                                         const char *source) {
    ...
    for (uint32_t i = 0; i < nc && count < 2; i++) {
        ...
        char *text = engine_node_text(a, ts_node_is_null(type_node) ? child : type_node, source);
        if (text && text[0]) {
            args[count++] = text;
        }
    }
    ...
    if (count == 2) {
        return engine_arena_sprintf(a, "%s_%s_%s", macro, args[0], args[1]);
    }
    if (count == 1) {
        return engine_arena_sprintf(a, "%s_%s", macro, args[0]);
    }
    return NULL;
}
...
static void extract_func_def(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec) {
    ...
    TSNode name_node = engine_resolve_func_name(node, ctx->language);
    if (ts_node_is_null(name_node)) {
        return;
    }

    char *name = engine_func_name_node_text(a, name_node, ctx->source, ctx->language);
    ...
    def.qualified_name =
        engine_fqn_compute_source_lang(a, ctx->project, ctx->rel_path, qn_name, ctx->language);
    ...
    if (!ts_node_is_null(params)) {
        def.signature = engine_node_text(a, params, ctx->source);
    ...
    for (const char **f = rt_fields; *f; f++) {
        ...
        if (!ts_node_is_null(rt)) {
            def.return_type = engine_node_text(a, rt, ctx->source);
    ...
    if (!ts_node_is_null(recv)) {
        def.receiver = engine_node_text(a, recv, ctx->source);
        def.label = "Method";
         ...
         * is computed the same way (engine_fqn_compute on the type name). */
        ...
        if (recv_type && recv_type[0]) {
            ...
            def.parent_class = engine_fqn_compute_source_lang(a, ctx->project, ctx->rel_path,
    ...
    if ((ctx->language == ENGINE_LANG_CPP || ctx->language == ENGINE_LANG_CUDA) &&
        ...
        if (scope_name && scope_name[0]) {
            const char *class_qn = engine_fqn_compute(a, ctx->project, ctx->rel_path, scope_name);
            def.qualified_name = engine_arena_sprintf(a, "%s.%s", class_qn, name);
            def.label = "Method";
    ...
    engine_defs_push(&ctx->result->defs, a, def);
}
...
// well-formed QN segment. Markdown headings (e.g. "Graph Engine") legitimately
...
static const char *qn_safe_segment(EngineArena *a, const char *name) {
    if (!name) {
        return name;
    }
    bool has_ws = false;
    ...
    char *out = engine_arena_strdup(a, name);
    if (!out) {
        return name;
    }
    char *w = out;
...
static void push_simple_class_def(EngineExtractCtx *ctx, TSNode node, char *name, const char *label) {
    EngineArena *a = ctx->arena;
    EngineDefinition def;
    memset(&def, 0, sizeof(def));
    def.name = name;
    def.qualified_name = engine_fqn_compute(a, ctx->project, ctx->rel_path, qn_safe_segment(a, name));
    def.label = label;
    def.file_path = ctx->rel_path;
    def.start_line = ts_node_start_point(node).row + TS_LINE_OFFSET;
    def.end_line = ts_node_end_point(node).row + TS_LINE_OFFSET;
...
static char *find_toml_key_name(EngineArena *a, TSNode node, const char *source) {
    uint32_t nc = ts_node_child_count(node);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode child = ts_node_child(node, i);
        const char *ck = ts_node_type(child);
        if (strcmp(ck, "bare_key") == 0 || strcmp(ck, "dotted_key") == 0 ||
            ...
            return engine_node_text(a, child, source);
        }
    }
    return NULL;
}
...
static char *find_xml_element_name(EngineArena *a, TSNode node, const char *source) {
    uint32_t nc = ts_node_child_count(node);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode child = ts_node_child(node, i);
        const char *ck = ts_node_type(child);
        if (strcmp(ck, "start_tag") == 0 || strcmp(ck, "self_closing_tag") == 0 ||
            ...
            for (uint32_t j = 0; j < tnc; j++) {
                ...
                if (strcmp(tk, "tag_name") == 0 || strcmp(tk, "Name") == 0) {
                    return engine_node_text(a, tag, source);
                }
            }
        }
    }
    // Fallback: try "Name" field directly for some XML grammars
    TSNode name_child = engine_find_child_by_kind(node, "Name");
    if (!ts_node_is_null(name_child)) {
        return engine_node_text(a, name_child, source);
    }
    return NULL;
}

// Extract text from an atx_heading node (# Title).
static char *extract_atx_heading_text(EngineArena *a, TSNode node, const char *source) {
    uint32_t nc = ts_node_child_count(node);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode child = ts_node_child(node, i);
        const char *ck = ts_node_type(child);
    ...
    char *full = engine_node_text(a, node, source);
    if (full) {
        char *p = full;
        while (*p == '#') {
            p++;
        ...
        if (*p) {
            return engine_arena_strdup(a, p);
        }
    }
    return NULL;
}
...
static char *extract_markdown_heading_name(EngineArena *a, TSNode node, const char *kind,
                                           const char *source) {
    char *name = NULL;
    if (strcmp(kind, "atx_heading") == 0) {
        ...
        if (ts_node_child_count(node) > 0) {
            name = engine_node_text(a, ts_node_child(node, 0), source);
        }
    }
...
static char *find_ini_section_name(EngineArena *a, TSNode node, const char *source) {
    uint32_t nc = ts_node_child_count(node);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode child = ts_node_child(node, i);
        ...
        TSNode text = engine_find_child_by_kind(child, "text");
        if (!ts_node_is_null(text)) {
            return engine_node_text(a, text, source);
        }
        return engine_node_text(a, child, source);
    }
    return NULL;
...
static char *find_hcl_block_name(EngineArena *a, TSNode node, const char *source) {
    TSNode id = engine_find_child_by_kind(node, "identifier");
    if (ts_node_is_null(id)) {
        return NULL;
    }
    char *name = engine_node_text(a, id, source);
    if (!name || !name[0]) {
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < cc; i++) {
        ...
        TSNode lit = engine_find_child_by_kind(ch, "template_literal");
        if (ts_node_is_null(lit)) {
            continue;
        }
        char *label = engine_node_text(a, lit, source);
        if (!label || !label[0]) {
            continue;
        }
        char *joined = engine_arena_sprintf(a, "%s.%s", name, label);
        if (joined) {
            name = joined;
        }
    }
...
static bool extract_config_class_def(EngineExtractCtx *ctx, TSNode node, const char *kind) {
    EngineArena *a = ctx->arena;
    char *name = NULL;
...
static void extract_class_def(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec) {
    ...
    if (ts_node_is_null(name_node) && ctx->language == ENGINE_LANG_PROTOBUF) {
        ...
        if (ts_node_is_null(name_node)) {
            name_node = engine_find_child_by_kind(node, "enum_name");
        }
    }
    ...
    if (ts_node_is_null(name_node) && ctx->language == ENGINE_LANG_FSHARP) {
        ...
        if (!ts_node_is_null(tn)) {
            ...
            if (ts_node_is_null(id)) {
                id = engine_find_child_by_kind(tn, "identifier");
            }
            if (!ts_node_is_null(id)) {
                name_node = id;
            }
    ...
    if (ts_node_is_null(name_node) && ctx->language == ENGINE_LANG_JULIA) {
        ...
        if (!ts_node_is_null(th)) {
            ...
            if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "binary_expression") == 0 &&
                ...
                name_node = engine_find_child_by_kind(th, "identifier");
            }
        }
    }
...
case ENGINE_LANG_FSHARP: { // type_definition > *_type_defn > type_name > identifier
        ...
        case ENGINE_LANG_ZIG: { // `const Foo = struct {...}`: struct/enum/union_declaration
            ...
            if (!ts_node_is_null(parent) &&
                ...
                name_node = engine_find_child_by_kind(parent, "identifier");
            }
            break;
        }
        default:
    ...
    char *name = engine_node_text(a, name_node, ctx->source);
    if (!name || !name[0]) {
        return;
    }
    ...
    const char *class_qn;
    if (ctx->enclosing_class_qn) {
        class_qn = engine_arena_sprintf(a, "%s.%s", ctx->enclosing_class_qn, name);
    } else {
        class_qn = engine_fqn_compute_source_lang(a, ctx->project, ctx->rel_path, name, ctx->language);
    }
    ...
    // its methods/fields/embedding resolve through engine_label_is_type_like(), and
    // engine_pipeline_implements_go() collects Struct nodes too).
    ...
    if (ctx->language == ENGINE_LANG_CSHARP) {
        ...
        if (!ts_node_is_null(primary_params)) {
            ...
            for (uint32_t k = 0; k < pcount; k++) {
                ...
                EngineDefinition pdef;
                memset(&pdef, 0, sizeof(pdef));
                pdef.name = pname;
                pdef.qualified_name = engine_arena_sprintf(a, "%s.%s", class_qn, pname);
                pdef.label = "Field";
                ...
                engine_defs_push(&ctx->result->defs, a, pdef);
            }
        }
    }
}
...
static TSNode find_class_member_body(TSNode class_node, EngineLanguage lang) {
    ...
    TSNode declarations = engine_find_child_by_kind(body, "enum_body_declarations");
...
static TSNode resolve_dart_method_name(TSNode child, const char *ck) {
    if (strcmp(ck, "method_signature") == 0) {
        TSNode func_sig = engine_find_child_by_kind(child, "function_signature");
        if (!ts_node_is_null(func_sig)) {
            TSNode name_node = func_name_node(func_sig);
            if (!ts_node_is_null(name_node)) {
                return name_node;
            }
            return engine_find_child_by_kind(func_sig, "identifier");
        }
    }
...
static TSNode resolve_method_name(TSNode child, EngineLanguage lang) {
...
static void push_method_def(EngineExtractCtx *ctx, TSNode child, TSNode class_node,
                            const char *class_qn, const EngineLangSpec *spec, TSNode name_node) {
    EngineArena *a = ctx->arena;
    ...
    const char *method_qn = engine_arena_sprintf(a, "%s.%s", class_qn, name);
...
static void extract_objc_impl_methods(EngineExtractCtx *ctx, TSNode impl_node, const char *class_qn,
                                      const EngineLangSpec *spec) {
    ...
    for (uint32_t j = 0; j < nc; j++) {
        ...
        if (engine_kind_in_set(inner, spec->function_node_types)) {
...
static void extract_class_methods(EngineExtractCtx *ctx, TSNode class_node, const char *class_qn,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (strcmp(ts_node_type(child), "decorated_definition") == 0) {
            ...
            if (ts_node_is_null(def) || !engine_kind_in_set(def, spec->function_node_types)) {
                continue;
            }
            method_node = def;
        }
        ...
        if (strcmp(ts_node_type(child), "public_field_definition") == 0) {
            ...
            if (ts_node_is_null(value) || !engine_kind_in_set(value, spec->function_node_types)) {
                continue;
            }
        ...
        if (!engine_kind_in_set(method_node, spec->function_node_types)) {
            continue;
        }
        ...
        if (ctx->language == ENGINE_LANG_KOTLIN &&
            ...
            if (ts_node_is_null(constructor_name)) {
                constructor_name = engine_find_child_by_kind(class_node, "type_identifier");
            }
            if (!ts_node_is_null(constructor_name)) {
...
static void extract_rust_impl(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec) {
    ...
    char *type_name = engine_node_text(a, type_node, ctx->source);
    ...
    const char *type_qn = engine_fqn_compute(a, ctx->project, ctx->rel_path, type_name);
    ...
    if (!ts_node_is_null(trait_node)) {
        char *trait_name = engine_node_text(a, trait_node, ctx->source);
        ...
        if (trait_name && trait_name[0]) {
            EngineImplTrait it = {0};
            it.trait_name = trait_name;
            it.struct_name = type_name;
            it.struct_qn = type_qn;
            engine_impltrait_push(&ctx->result->impl_traits, a, it);
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (!engine_kind_in_set(child, spec->function_node_types)) {
            continue;
        }
        ...
        const char *method_qn = engine_arena_sprintf(a, "%s.%s", type_qn, name);
...
static void extract_elixir_func_def(EngineExtractCtx *ctx, TSNode node, const char *macro) {
    EngineArena *a = ctx->arena;
    ...
    if (strcmp(fk, "call") == 0 && ts_node_child_count(first_arg) > 0) {
        name = engine_node_text(a, ts_node_child(first_arg, 0), ctx->source);
    } else if (strcmp(fk, "identifier") == 0) {
        name = engine_node_text(a, first_arg, ctx->source);
    }
    if (!name || !name[0]) {
        return;
    }
    ...
    memset(&def, 0, sizeof(def));
    def.name = name;
    def.qualified_name = engine_fqn_compute(a, ctx->project, ctx->rel_path, name);
    def.label = "Function";
    def.file_path = ctx->rel_path;
    def.start_line = ts_node_start_point(node).row + TS_LINE_OFFSET;
    def.end_line = ts_node_end_point(node).row + TS_LINE_OFFSET;
...
static TSNode emit_elixir_module_class(EngineExtractCtx *ctx, TSNode cur) {
    EngineArena *a = ctx->arena;
    TSNode null_node = {0};
    ...
    def.is_exported = true;
    engine_defs_push(&ctx->result->defs, a, def);
    return engine_find_child_by_kind(cur, "do_block");
}
...
static void extract_elixir_call(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec) {
    (void)spec;
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, ENGINE_SZ_64);
    ...
    while (stack.count > 0) {
        ...
        char *macro = engine_node_text(a, callee, ctx->source);
        if (!macro) {
            continue;
        }
...
static void push_var_def_qn(EngineExtractCtx *ctx, const char *name, const char *qn_name,
                            TSNode node) {
    ...
    def.qualified_name = engine_fqn_compute_source_lang(a, ctx->project, ctx->rel_path,
...
static void push_var_def(EngineExtractCtx *ctx, const char *name, TSNode node) {
...
static const char *extract_c_declarator_name(EngineArena *a, TSNode decl, const char *source) {
    ...
    if (strcmp(dk, "identifier") == 0) {
        return engine_node_text(a, declarator, source);
    }
    return NULL;
}

// Helper: extract name from Java/C# field_declaration (declarator > name)
static const char *extract_java_field_name(EngineArena *a, TSNode field, const char *source) {
    ...
    if (!ts_node_is_null(name)) {
        return engine_node_text(a, name, source);
    }
    return NULL;
}
...
static void extract_csharp_vars(EngineExtractCtx *ctx, TSNode node, EngineArena *a) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        for (uint32_t j = 0; j < nc; j++) {
            ...
            if (strcmp(ts_node_type(decl), "variable_declarator") == 0) {
                ...
                if (ts_node_is_null(id)) {
                    id = engine_find_child_by_kind(decl, "identifier");
                }
                if (!ts_node_is_null(id)) {
                    push_var_def(ctx, engine_node_text(a, id, ctx->source), decl);
                }
            }
        }
    }
...
static void extract_enum_members(EngineExtractCtx *ctx, TSNode node, const char *class_qn) {
    ...
    for (uint32_t mi = 0; mi < mc; mi++) {
        ...
        if (ts_node_is_null(mname)) {
            mname = engine_find_child_by_kind(member, "identifier");
        }
        if (ts_node_is_null(mname)) {
            continue;
        }
        char *member_name = engine_node_text(a, mname, ctx->source);
        if (!member_name || !member_name[0]) {
            continue;
        }
        EngineDefinition mdef;
        memset(&mdef, 0, sizeof(mdef));
        mdef.name = member_name;
        mdef.qualified_name = engine_arena_sprintf(a, "%s.%s", class_qn, member_name);
        mdef.label = "Variable";
        ...
        engine_defs_push(&ctx->result->defs, a, mdef);
    }
}
...
static void extract_destructured_vars(EngineExtractCtx *ctx, TSNode pattern, TSNode decl,
                                      EngineArena *a) {
    ...
    for (uint32_t pi = 0; pi < pc; pi++) {
        ...
        char *id_text = engine_node_text(a, ident, ctx->source);
...
static bool is_require_import_call(TSNode value, const char *source, EngineArena *a) {
    ...
    char *fn_name = engine_node_text(a, fn, source);
...
static void extract_js_vars(EngineExtractCtx *ctx, TSNode node, EngineArena *a) {
    uint32_t n = ts_node_named_child_count(node);
    for (uint32_t i = 0; i < n; i++) {
        TSNode child = ts_node_named_child(node, i);
        if (strcmp(ts_node_type(child), "variable_declarator") != 0) {
        ...
        if (!ts_node_is_null(vname)) {
            ...
            if (strcmp(nk, "object_pattern") == 0 || strcmp(nk, "array_pattern") == 0) {
                ...
                push_var_def(ctx, engine_node_text(a, vname, ctx->source), child);
            }
        }
    }
}

static void extract_vars_mainstream(EngineExtractCtx *ctx, TSNode node, EngineArena *a,
                                    const char *kind) {
    (void)kind;
    switch (ctx->language) {
    case ENGINE_LANG_PYTHON: {
        ...
        if (strcmp(lt, "identifier") == 0) {
            push_var_def(ctx, engine_node_text(a, left, ctx->source), node);
            ...
            for (uint32_t li = 0; li < ln; li++) {
                ...
                if (strcmp(ts_node_type(part), "identifier") == 0) {
                    push_var_def(ctx, engine_node_text(a, part, ctx->source), node);
                }
            }
        }
        break;
    ...
    case ENGINE_LANG_RUST: {
        TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
        if (!ts_node_is_null(name_node)) {
            push_var_def(ctx, engine_node_text(a, name_node, ctx->source), node);
        }
        break;
    }
    default:
...
static void extract_lua_vars(EngineExtractCtx *ctx, TSNode node, EngineArena *a) {
    uint32_t n = ts_node_named_child_count(node);
    for (uint32_t i = 0; i < n; i++) {
        TSNode child = ts_node_named_child(node, i);
        ...
        TSNode expr_list = engine_find_child_by_kind(child, "expression_list");
...
static void extract_perl_vars(EngineExtractCtx *ctx, TSNode node, EngineArena *a) {
    uint32_t n = ts_node_named_child_count(node);
    for (uint32_t i = 0; i < n; i++) {
        TSNode child = ts_node_named_child(node, i);
        const char *ck = ts_node_type(child);
        if (is_perl_var_type(ck)) {
            push_var_def(ctx, strip_perl_sigil(engine_node_text(a, child, ctx->source)), node);
            return;
        }
        ...
        push_var_def(ctx, strip_perl_sigil(engine_node_text(a, left, ctx->source)), node);
        return;
    }
}

// R variable extraction: skip function-definitions, then extract left/lhs.
static void extract_r_vars(EngineExtractCtx *ctx, TSNode node, EngineArena *a) {
...
static void extract_php_vars(EngineExtractCtx *ctx, TSNode node, EngineArena *a, const char *kind) {
    ...
    for (uint32_t j = 0; j < nc; j++) {
        ...
        if (strcmp(ts_node_type(inner), "assignment_expression") == 0) {
            ...
            if (!ts_node_is_null(left)) {
                char *name = engine_node_text(a, left, ctx->source);
                if (name && name[0] == '$') {
                    name++;
                }
...
static void extract_vars_dynamic(EngineExtractCtx *ctx, TSNode node, EngineArena *a, const char *kind) {
    switch (ctx->language) {
    case ENGINE_LANG_PHP:
        extract_php_vars(ctx, node, a, kind);
        break;
    case ENGINE_LANG_LUA:
        extract_lua_vars(ctx, node, a);
        break;
    case ENGINE_LANG_RUBY: {
        TSNode left = ts_node_child_by_field_name(node, TS_FIELD("left"));
        if (!ts_node_is_null(left)) {
            const char *lk = ts_node_type(left);
            if (strcmp(lk, "identifier") == 0 || strcmp(lk, "constant") == 0) {
...
static TSNode resolve_kotlin_var_name(TSNode node) {
    ...
    name_node = engine_find_child_by_kind(node, "simple_identifier");
    if (!ts_node_is_null(name_node)) {
        return name_node;
    }
    name_node = engine_find_child_by_kind(node, "identifier");
    ...
    TSNode var_decl = engine_find_child_by_kind(node, "variable_declaration");
    if (!ts_node_is_null(var_decl)) {
        name_node = engine_find_child_by_kind(var_decl, "simple_identifier");
        if (!ts_node_is_null(name_node)) {
            return name_node;
        }
        return engine_find_child_by_kind(var_decl, "identifier");
    }
    TSNode null_node = {0};
    return null_node;
}

static void extract_vars_jvm(EngineExtractCtx *ctx, TSNode node, EngineArena *a) {
    switch (ctx->language) {
    case ENGINE_LANG_SCALA: {
        TSNode pattern = ts_node_child_by_field_name(node, TS_FIELD("pattern"));
        if (!ts_node_is_null(pattern)) {
            push_var_def(ctx, engine_node_text(a, pattern, ctx->source), node);
        } else {
            TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
            if (!ts_node_is_null(name_node)) {
                push_var_def(ctx, engine_node_text(a, name_node, ctx->source), node);
...
static void extract_ini_vars(EngineExtractCtx *ctx, TSNode node, EngineArena *a) {
    uint32_t nc = ts_node_child_count(node);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode child = ts_node_child(node, i);
        const char *ck = ts_node_type(child);
        if (strcmp(ck, "setting_name") == 0 || strcmp(ck, "name") == 0) {
            push_var_def(ctx, trim_whitespace(engine_node_text(a, child, ctx->source)), node);
            return;
        }
    }
    if (nc > 0) {
        ...
        if (!found_name) {
            push_var_def(
                ctx, trim_whitespace(engine_node_text(a, ts_node_child(node, 0), ctx->source)), node);
        }
    }
}

// Find first named child matching one of the given types and push as var def.
static void push_first_matching_child(EngineExtractCtx *ctx, TSNode node, EngineArena *a,
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        for (const char **t = match_types; *t; t++) {
            if (strcmp(ck, *t) == 0) {
                push_var_def(ctx, engine_node_text(a, child, ctx->source), node);
                return;
            }
        }
    }
...
static void extract_json_var(EngineExtractCtx *ctx, TSNode node, EngineArena *a) {
    TSNode key_node = ts_node_child_by_field_name(node, TS_FIELD("key"));
    if (ts_node_is_null(key_node)) {
        return;
    }
    char *raw = engine_node_text(a, key_node, ctx->source);
    if (raw) {
...
static void extract_scss_var(EngineExtractCtx *ctx, TSNode node, EngineArena *a) {
    ...
    if (ts_node_is_null(prop)) {
        prop = engine_find_child_by_kind(node, "property_name");
    }
    if (ts_node_is_null(prop)) {
        prop = engine_find_child_by_kind(node, "variable_name");
    }
    if (!ts_node_is_null(prop)) {
        push_var_def(ctx, engine_node_text(a, prop, ctx->source), node);
    }
}

static void extract_vars_config(EngineExtractCtx *ctx, TSNode node, EngineArena *a, const char *kind) {
    switch (ctx->language) {
    case ENGINE_LANG_YAML: {
    ...
    case ENGINE_LANG_JSON:
        extract_json_var(ctx, node, a);
        break;
    case ENGINE_LANG_INI:
...
static void extract_vars_nix(EngineExtractCtx *ctx, TSNode node, EngineArena *a) {
    ...
    if (engine_nix_binding_is_attrset_scope(node)) {
        return; /* a scope, not a value */
    }
    TSNode attrpath = ts_node_child_by_field_name(node, TS_FIELD("attrpath"));
    TSNode leaf = engine_nix_attrpath_last_attr(attrpath);
    if (ts_node_is_null(leaf) || engine_nix_attr_is_interpolated(leaf)) {
        return;
    }
    char *name = engine_node_text(a, leaf, ctx->source);
    if (!name || !name[0]) {
        return;
    }
    engine_nix_strip_attr_quotes(name);
    const char *scope = engine_nix_attrpath_scope(a, attrpath, ctx->source);
    const char *qn_name = scope ? engine_arena_sprintf(a, "%s.%s", scope, name) : name;
    push_var_def_qn(ctx, name, qn_name, node);
}

/* Mint every direct binding of one Nix binding container. */
static void extract_nix_binding_set(EngineExtractCtx *ctx, TSNode set, const EngineLangSpec *spec) {
...
static void extract_nix_module_vars(EngineExtractCtx *ctx, TSNode root, const EngineLangSpec *spec) {
    ...
    if (strcmp(ts_node_type(cur), "let_expression") == 0) {
        extract_nix_binding_set(ctx, engine_find_child_by_kind(cur, "binding_set"), spec);
        cur = ts_node_child_by_field_name(cur, TS_FIELD("body"));
        if (ts_node_is_null(cur)) {
            return;
        }
    ...
    case ENGINE_LANG_DOCKERFILE:
        if (strcmp(kind, "env_instruction") == 0) {
            ...
            for (uint32_t i = 0; i < ec; i++) {
                ...
                if (!ts_node_is_null(nm)) {
                    push_var_def(ctx, engine_node_text(a, nm, ctx->source), pair);
                }
            }
        } else if (strcmp(kind, "arg_instruction") == 0) {
            TSNode nm = ts_node_child_by_field_name(node, TS_FIELD("name"));
            if (ts_node_is_null(nm)) {
                nm = engine_find_child_by_kind(node, "unquoted_string");
            }
            if (!ts_node_is_null(nm)) {
                push_var_def(ctx, engine_node_text(a, nm, ctx->source), node);
            }
        }
        return;
...
static void walk_variables_iter(EngineExtractCtx *ctx, TSNode root, const EngineLangSpec *spec) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, ENGINE_SZ_256);
...
static void extract_yaml_toplevel_keys(EngineExtractCtx *ctx, TSNode root) {
    EngineArena *a = ctx->arena;
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!ts_node_is_null(key)) {
            push_var_def(ctx, engine_node_text(a, key, ctx->source), pair);
        }
    }
}
...
static void extract_variables(EngineExtractCtx *ctx, TSNode root, const EngineLangSpec *spec) {
    ...
    if (!engine_is_module_level_p(root, ctx->language)) {
        return;
    }
        ...
        do {
            ...
            if (strcmp(ck, "expression_statement") == 0 || strcmp(ck, "export_statement") == 0 ||
                ...
                for (uint32_t j = 0; j < nc; j++) {
                    ...
                    if (engine_kind_in_set(inner, spec->variable_node_types)) {
...
static bool extract_schema_field(EngineExtractCtx *ctx, TSNode child, const char *class_qn) {
    EngineArena *a = ctx->arena;
    TSNode name_node = {0};
    TSNode type_node = {0};
    ...
    def.parent_class = class_qn;
    if (!ts_node_is_null(type_node)) {
        def.return_type = engine_node_text(a, type_node, ctx->source);
    }
    def.start_line = ts_node_start_point(child).row + TS_LINE_OFFSET;
    def.end_line = ts_node_end_point(child).row + TS_LINE_OFFSET;
    def.is_exported = engine_is_exported(name, ctx->language);
...
static void extract_class_fields(EngineExtractCtx *ctx, TSNode class_node, const char *class_qn,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (!engine_kind_in_set(child, spec->field_node_types)) {
            continue;
        }
        ...
        char *type_text = engine_node_text(a, type_node, ctx->source);
        if (!type_text || !type_text[0]) {
            continue;
        }
        ...
        const char *field_qn = engine_arena_sprintf(a, "%s.%s", class_qn, name);
...
// Generous safety ceiling (frames), env-overridable via ENGINE_WALK_DEFS_MAX.
    ...
    if (s->top >= s->cap) {
        ...
        if (ncap > wd_stack_max()) {
            if (!s->warned) {
                ...
                engine_log_warn("extract.walk_defs_capped", "limit", lim, "path",
                             s->path ? s->path : "");
                s->warned = true;
            }
...
static void push_nested_class_nodes(TSNode body, const EngineLangSpec *spec, wd_stack_t *s,
                                    const char *enclosing_qn, EngineArena *arena) {
    ...
    while (nc_stack.count > 0) {
        ...
        for (int i = (int)nc - SKIP_CHAR; i >= 0; i--) {
            ...
            if (engine_kind_in_set(child, spec->class_node_types)) {
...
static bool is_namespace_scope_kind(EngineLanguage lang, const char *kind, TSNode node) {
    ...
    if (engine_is_namespace_scope_kind(lang, kind)) {
        return true;
    }
...
static const char *compute_class_qn(EngineExtractCtx *ctx, TSNode node, const char *saved_enclosing) {
    ...
    if (!ts_node_is_null(name_node)) {
        char *cname = engine_node_text(ctx->arena, name_node, ctx->source);
        if (cname && cname[0]) {
            if (saved_enclosing) {
                return engine_arena_sprintf(ctx->arena, "%s.%s", saved_enclosing, cname);
            }
            /* Top-level: language-aware module so Java/Go don't double the
             * filename stem (matches extract_class_def above). */
            return engine_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, cname,
                                               ctx->language);
        }
    }
...
static void extract_typescript_namespace_def(EngineExtractCtx *ctx, TSNode node,
                                             const char *saved_enclosing) {
    TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
    if (ts_node_is_null(name_node)) {
        return;
    }
    char *name = engine_node_text(ctx->arena, name_node, ctx->source);
    const char *namespace_qn = compute_class_qn(ctx, node, saved_enclosing);
    if (!name || !name[0] || !namespace_qn || !namespace_qn[0]) {
        return;
    }
    EngineDefinition def = {0};
    def.name = name;
    ...
    def.lines = (int)(def.end_line - def.start_line + TS_LINE_OFFSET);
    def.is_exported = true;
    engine_defs_push(&ctx->result->defs, ctx->arena, def);
}

// Push nested class children from a class body container onto the walk stack.
static void push_class_body_children(TSNode node, const EngineLangSpec *spec, wd_stack_t *s,
                                     const char *new_enclosing, EngineArena *arena) {
...
static void extract_cfml_function_tag(EngineExtractCtx *ctx, TSNode node) {
    EngineArena *a = ctx->arena;
    char *name = NULL;
    uint32_t cc = ts_node_named_child_count(node);
    for (uint32_t i = 0; i < cc && !name; i++) {
        ...
        TSNode an = engine_find_child_by_kind(ch, "cf_attribute_name");
        if (ts_node_is_null(an)) {
            continue;
        }
        char *aname = engine_node_text(a, an, ctx->source);
        if (!ascii_ci_equals(aname, "name")) {
            continue;
        }
        TSNode val = engine_find_child_by_kind(ch, "quoted_cf_attribute_value");
        if (ts_node_is_null(val)) {
            val = engine_find_child_by_kind(ch, "cf_attribute_value");
        }
        if (ts_node_is_null(val)) {
            continue;
        }
        TSNode inner = engine_find_child_by_kind(val, "attribute_value");
        name = engine_node_text(a, ts_node_is_null(inner) ? val : inner, ctx->source);
    }
    if (!name || !name[0]) {
        return;
    }
...
static void extract_gotemplate_define(EngineExtractCtx *ctx, TSNode node) {
    EngineArena *a = ctx->arena;
    TSNode s = engine_find_child_by_kind(node, "interpreted_string_literal");
    if (ts_node_is_null(s)) {
        return;
    }
    char *raw = engine_node_text(a, s, ctx->source);
    if (!raw) {
        return;
    }
    size_t len = strlen(raw);
    if (len >= 2 && (raw[0] == '"' || raw[0] == '`')) {
        raw = engine_arena_strndup(a, raw + 1, len - 2); // strip surrounding quotes
    }
    if (!raw || !raw[0]) {
        return;
    }
    ...
    memset(&def, 0, sizeof(def));
    def.name = raw;
    def.qualified_name = engine_fqn_compute(a, ctx->project, ctx->rel_path, raw);
    def.label = "Function";
    def.file_path = ctx->rel_path;
    def.start_line = ts_node_start_point(node).row + TS_LINE_OFFSET;
    def.end_line = ts_node_end_point(node).row + TS_LINE_OFFSET;
...
static void extract_janet_def(EngineExtractCtx *ctx, TSNode node) {
    EngineArena *a = ctx->arena;
    ...
    char *head_text = engine_node_text(a, head, ctx->source);
...
static void extract_c_macro_def(EngineExtractCtx *ctx, TSNode node) {
    EngineArena *a = ctx->arena;
    TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
    if (ts_node_is_null(name_node)) {
        return;
...
static void extract_lisp_def(EngineExtractCtx *ctx, TSNode node) {
    EngineArena *a = ctx->arena;
    if (ts_node_named_child_count(node) < 2) {
        return;
    }
    char *head = engine_node_text(a, ts_node_named_child(node, 0), ctx->source);
 ...
 */
static void recover_kotlin_error_classes(EngineExtractCtx *ctx, TSNode err_node) {
    EngineArena *a = ctx->arena;
    ...
    for (uint32_t i = 0; i < cc; i++) {
        ...
        if (ctx->enclosing_class_qn) {
            ...
            class_qn = engine_fqn_compute(a, ctx->project, ctx->rel_path, name);
        }
        ...
        def.is_exported = engine_is_exported(name, ctx->language);
        if (bcount > 0) {
            const char **result = (const char **)engine_arena_alloc(a, (size_t)(bcount + NULL_TERM) *
                                                                        sizeof(const char *));
            if (result) {
...
static void walk_defs(EngineExtractCtx *ctx, TSNode root, const EngineLangSpec *spec, int depth_unused) {
    ...
    while (s.top > 0) {
        ...
        if (is_c_preprocessor_lang(ctx->language) &&
            ...
            if (engine_macro_extraction_enabled()) {
        ...
        if (engine_kind_in_set(node, spec->function_node_types)) {
            if (!is_template_class_node(node, ctx->language)) {
                extract_func_def(ctx, node, spec);
                // Most languages stop here. JS/TS descend into the
        ...
        if (engine_kind_in_set(node, spec->class_node_types)) {
...
void engine_extract_definitions(EngineExtractCtx *ctx) {
    const EngineLangSpec *spec = engine_lang_spec(ctx->language);
    if (!spec) {
        return;
    }
    ...
    engine_defs_push(&ctx->result->defs, a, mod);
