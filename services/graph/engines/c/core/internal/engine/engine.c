#include "engine.h"
#include "arena.h" // EngineArena, engine_arena_init/alloc/strdup/destroy
#include "helpers.h"
...
#include "foundation/compat_fs.h"  // engine_fopen — crash-supervisor per-file marker write
#include "foundation/hash_table.h" // EngineHashTable — crash-supervisor quarantine set
...
#if defined(ENGINE_BIND_TS_ALLOCATOR) && ENGINE_BIND_TS_ALLOCATOR
...
/* Use compat.h's engine_clock_gettime which accepts CLOCK_MONOTONIC (value
 * varies by platform: 1 on Linux/Windows, 6 on macOS). We pass the
 * platform value via the compat.h fallback. */
#if defined(CLOCK_MONOTONIC)
#define ENGINE_CLOCK_MONO CLOCK_MONOTONIC
#elif defined(__APPLE__)
#define ENGINE_CLOCK_MONO 6
#else
#define ENGINE_CLOCK_MONO 1
#endif

static uint64_t now_ns(void) {
    struct timespec ts;
    engine_clock_gettime(ENGINE_CLOCK_MONO, &ts);
    return ((uint64_t)ts.tv_sec * NSEC_PER_SEC) + (uint64_t)ts.tv_nsec;
}

// engine_get_profile returns accumulated parse/extract times and file count.
...
// engine_reset_profile zeros the profiling counters.
...
#define GROW_ARRAY(arr, arena)                                                                   \
    do {                                                                                         \
        if ((arr)->count >= (arr)->cap) {                                                        \
            int new_cap = (arr)->cap == 0 ? ENGINE_SZ_32 : (arr)->cap * PAIR_LEN;                   \
            void *new_items = engine_arena_alloc((arena), (size_t)new_cap * sizeof(*(arr)->items)); \
            if (!new_items)                                                                      \
                return;                                                                          \
...
static const char *engine_string_read(void *payload, uint32_t byte, TSPoint point,
                                   uint32_t *bytes_read) {
    (void)point;
    EngineStringInput *self = (EngineStringInput *)payload;
    if (byte >= self->length) {
        *bytes_read = 0;
        return "";
    }
...
static bool engine_timeout_cb(TSParseState *state) {
...
static ENGINE_TLS TSParser *tl_parser = NULL;
static ENGINE_TLS EngineLanguage tl_parser_lang = ENGINE_LANG_COUNT; // invalid sentinel

// Get or create a thread-local parser configured for the given language.
static TSParser *get_thread_parser(const TSLanguage *ts_lang, EngineLanguage lang) {
    if (!tl_parser) {
        tl_parser = ts_parser_new();
        if (!tl_parser) {
            return NULL;
        }
        tl_parser_lang = ENGINE_LANG_COUNT;
    }
 ...
 *
 * Guarded to the production build (ENGINE_BIND_TS_ALLOCATOR=1, which CFLAGS_PROD
 ...
 * (#581). */
static void *engine_sqlite_malloc(int n) {
    void *block = mi_malloc((size_t)n);
    return block;
}
static void engine_sqlite_free(void *p) {
    mi_free(p);
}
static void *engine_sqlite_realloc(void *p, int n) {
    if (p) {}
    void *grown = mi_realloc(p, (size_t)n);
    return grown;
}
static int engine_sqlite_size(void *p) {
    return (int)mi_usable_size(p);
}
static int engine_sqlite_roundup(int n) {
...
static void *engine_ts_malloc(size_t n) {
    void *block = mi_malloc(n);
    return block;
}
static void *engine_ts_calloc(size_t count, size_t size) {
    void *block = mi_calloc(count, size);
    return block;
}
static void *engine_ts_realloc(void *p, size_t n) {
    if (p) {}
    void *grown = mi_realloc(p, n);
    return grown;
}
static void engine_ts_free(void *p) {
    mi_free(p);
}

static int engine_sqlite_meminit(void *appdata) {
    (void)appdata;
    return SQLITE_OK;
}
static void engine_sqlite_memshutdown(void *appdata) {
    (void)appdata;
}
#endif /* ENGINE_BIND_TS_ALLOCATOR */
...
void engine_alloc_init(void) {
    ...
    /* tree-sitter runtime (was previously bound in engine_init; consolidated here). */
    ts_set_allocator(engine_ts_malloc, engine_ts_calloc, engine_ts_realloc, engine_ts_free);
     ...
     * silently and the binding is ignored. engine_alloc_init() runs as the very
     * first statement of main(), before engine_mcp_server_new → engine_store_open*. */
    static sqlite3_mem_methods engine_sqlite_mem = {
        engine_sqlite_malloc,      /* xMalloc */
        engine_sqlite_free,        /* xFree */
        engine_sqlite_realloc,     /* xRealloc */
        engine_sqlite_size,        /* xSize */
        engine_sqlite_roundup,     /* xRoundup */
        engine_sqlite_meminit,     /* xInit */
        engine_sqlite_memshutdown, /* xShutdown */
        NULL,                   /* pAppData */
    };
    int sqlite_rc = sqlite3_config(SQLITE_CONFIG_MALLOC, &engine_sqlite_mem);
...
int engine_init(void) {
    ...
    enum { ENGINE_INIT_DONE = 1 };
    engine_initialized = ENGINE_INIT_DONE;
    /* Defense-in-depth allocator binds (idempotent). main() calls engine_alloc_init
     * first; this covers non-main entry points (pipeline passes call engine_init).
...
void engine_reset_thread_parser(void) {
    ...
    // Must be called BEFORE engine_slab_reset_thread() to avoid corrupting
...
// (Go: `s` in `func (s *Store) save()`, from EngineDefinition.receiver). Matching
...
static void engine_index_mark(const char *rel_path, char event) {
    const char *mf = getenv("ENGINE_INDEX_MARKER_FILE");
    if (!mf || !mf[0] || !rel_path || !rel_path[0]) {
        return;
    }
    FILE *f = engine_fopen(mf, "ab");
    if (f) {
        (void)fprintf(f, "%c %s\n", event, rel_path);
        (void)fclose(f);
    }
 ...
 * (engine_extract_file): whether a pass re-extracts from disk on a cache miss
 ...
 * engine_index_is_quarantined() so the crasher surfaces in the response skipped[].
...
static EngineHashTable *g_quarantine_set = NULL;
enum { ENGINE_QSET_UNINIT = 0, ENGINE_QSET_INITING = 1, ENGINE_QSET_INITED = 2 };
static atomic_int g_quarantine_state = ENGINE_QSET_UNINIT;

static void engine_quarantine_load(void) {
    const char *qf = getenv("ENGINE_INDEX_QUARANTINE_FILE");
    if (!qf || !qf[0]) {
        return; /* normal path: empty set */
    }
    FILE *f = engine_fopen(qf, "rb");
    if (!f) {
        return;
    }
    EngineHashTable *set = engine_ht_create(16);
    if (!set) {
        (void)fclose(f);
        return;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
         ...
         * The value stores the phase so engine_index_quarantine_phase() can report
         * "crash" vs "hang"; membership (engine_index_is_quarantined) is value != NULL. */
        char *pval = engine_strdup(phase);
        if (!pval) {
            continue;
        }
        if (engine_ht_has(set, line)) {
             ...
             * value it displaces. */
            free(engine_ht_set(set, line, (void *)pval));
        } else {
            char *key = engine_strdup(line);
            if (key) {
                engine_ht_set(set, key, (void *)pval);
            } else {
...
bool engine_index_is_quarantined(const char *rel_path) {
    ...
    if (state != ENGINE_QSET_INITED) {
        /* First caller wins the CAS and loads; racers spin until INITED.
         * Same once-init pattern as engine_ui_log_init (http_server.c). */
        state = ENGINE_QSET_UNINIT;
        if (atomic_compare_exchange_strong(&g_quarantine_state, &state, ENGINE_QSET_INITING)) {
            engine_quarantine_load();
            atomic_store(&g_quarantine_state, ENGINE_QSET_INITED);
        } else {
            while (atomic_load(&g_quarantine_state) != ENGINE_QSET_INITED) {
                engine_usleep(1000); /* 1ms */
            }
        }
    }
    return g_quarantine_set && engine_ht_has(g_quarantine_set, rel_path);
}

const char *engine_index_quarantine_phase(const char *rel_path) {
    /* engine_index_is_quarantined drives the lazy once-load and returns true only
    ...
    if (!engine_index_is_quarantined(rel_path)) {
        return NULL;
    }
    return (const char *)engine_ht_get(g_quarantine_set, rel_path);
}
...
static void engine_test_fault_inject(const char *rel_path) {
    if (!rel_path || !rel_path[0]) {
        return;
    }
    const char *crash_on = getenv("ENGINE_TEST_CRASH_ON");
    if (crash_on && crash_on[0] && strstr(rel_path, crash_on)) {
        abort(); /* SIGABRT → WIFSIGNALED → classified as a crash */
    }
    const char *hang_on = getenv("ENGINE_TEST_HANG_ON");
 ...
 *
 * engine_source_nesting_exceeds scans the raw bytes for the maximum bracket-nesting
...
#define ENGINE_PERL_MAX_PARSE_NESTING 128

static bool engine_source_nesting_exceeds(const char *source, int source_len, int cap) {
    int depth = 0;
 ...
 * Bounded by ENGINE_MAX_ERROR_REGIONS so pathological input can't blow up the
 * output. The ranges mark where constructs were dropped; they are a detection
 * aid, never a completeness proof. */
#define ENGINE_MAX_ERROR_REGIONS 64
typedef struct {
    uint32_t starts[ENGINE_MAX_ERROR_REGIONS];
    uint32_t ends[ENGINE_MAX_ERROR_REGIONS];
    int count;
} engine_error_regions_t;

static void engine_error_regions_push(engine_error_regions_t *acc, TSNode n) {
    if (acc->count >= ENGINE_MAX_ERROR_REGIONS) {
        return;
    }
...
static void engine_collect_error_regions(TSNode n, engine_error_regions_t *acc) {
    if (acc->count >= ENGINE_MAX_ERROR_REGIONS) {
        return;
    }
    uint32_t k = ts_node_child_count(n);
    for (uint32_t i = 0; i < k && acc->count < ENGINE_MAX_ERROR_REGIONS; i++) {
        TSNode c = ts_node_child(n, i);
        if (ts_node_is_missing(c) || strcmp(ts_node_type(c), "ERROR") == 0) {
            engine_error_regions_push(acc, c); /* top-most region; do not descend */
        } else if (ts_node_has_error(c)) {
            engine_collect_error_regions(c, acc);
        }
    }
}
...
static bool engine_region_is_recovered(uint32_t rs, uint32_t re, const EngineDefArray *defs) {
    ...
    for (int i = 0; i < defs->count && n < MAX_COVER_DEFS; i++) {
        const EngineDefinition *d = &defs->items[i];
...
static bool engine_line_contains(const char *src, int src_len, uint32_t line, const char *name) {
...
static bool engine_identifier_char(char c) {
...
static bool engine_span_contains_callable_def(const char *src, int src_len, uint32_t start_line,
    ...
    for (int pos = span_start; pos + (int)name_len <= span_end; pos++) {
        if (strncmp(src + pos, name, name_len) != 0 ||
            (pos > 0 && engine_identifier_char(src[pos - 1])) ||
            (pos + (int)name_len < src_len && engine_identifier_char(src[pos + name_len]))) {
            continue;
        }
        int before = pos;
...
static bool engine_remap_preprocessed_def(EngineDefinition *def, const EnginePreprocessedSource *pp) {
...
static void engine_subtract_recovered_regions(engine_error_regions_t *regs, const EngineDefArray *defs) {
    int kept = 0;
    for (int i = 0; i < regs->count; i++) {
        if (!engine_region_is_recovered(regs->starts[i], regs->ends[i], defs)) {
...
static bool engine_span_is_macro_invocation(const char *src, int src_len, uint32_t start_line,
                                         uint32_t end_line, const EngineDefArray *defs) {
    ...
    for (int di = 0; di < defs->count; di++) {
        const EngineDefinition *d = &defs->items[di];
        ...
        for (int pos = span_start; pos + nlen <= span_end; pos++) {
            if (strncmp(src + pos, d->name, (size_t)nlen) != 0 ||
                (pos > 0 && engine_identifier_char(src[pos - 1])) ||
                (pos + nlen < src_len && engine_identifier_char(src[pos + nlen]))) {
                continue;
            }
            int open = pos + nlen;
            while (open < span_end && isspace((unsigned char)src[open])) {
...
static bool engine_region_inside_callable(uint32_t rs, uint32_t re, const EngineDefArray *defs) {
...
static void engine_subtract_macro_invocation_regions(engine_error_regions_t *regs,
                                                  const EngineDefArray *defs, const char *src,
                                                  int src_len) {
    int kept = 0;
    for (int i = 0; i < regs->count; i++) {
        bool benign =
            engine_span_is_macro_invocation(src, src_len, regs->starts[i], regs->ends[i], defs) &&
            engine_region_inside_callable(regs->starts[i], regs->ends[i], defs);
        if (!benign) {
            regs->starts[kept] = regs->starts[i];
            regs->ends[kept] = regs->ends[i];
            kept++;
...
static const char *engine_error_ranges_str(EngineArena *a, const engine_error_regions_t *regs) {
    if (regs->count <= 0) {
        return NULL;
    }
    enum { RANGE_MAX = 24 }; /* "4294967295-4294967295," */
    char *buf = (char *)engine_arena_alloc(a, (size_t)regs->count * RANGE_MAX);
    if (!buf) {
        return NULL;
    }
    size_t off = 0;
...
EngineFileResult *engine_extract_file(const char *source, int source_len, EngineLanguage language,
    ...
    EngineFileResult *r =
        engine_extract_file_ex(source, source_len, language, project, rel_path, timeout_micros,
                            extra_defines, include_paths, NULL, NULL);
    return r;
}

EngineFileResult *engine_extract_file_ex(const char *source, int source_len, EngineLanguage language,
                                   const char *project, const char *rel_path,
                                   int64_t timeout_micros, const char **extra_defines,
                                   const char **include_paths, const EngineMacroTable *macro_table,
                                   const EngineReturnTypeTable *return_type_table) {
    // Allocate result on heap (arena inside for all string data)
    enum { SINGLE = 1 };
    EngineFileResult *result = (EngineFileResult *)calloc(SINGLE, sizeof(EngineFileResult));
    if (!result) {
        return NULL;
    }

    engine_arena_init(&result->arena);
    EngineArena *a = &result->arena;
    ...
    engine_index_mark_start(rel_path);
#ifdef ENGINE_ENABLE_TEST_SEAMS
    engine_test_fault_inject(rel_path);
#endif

    // Get language spec
    const EngineLangSpec *spec = engine_lang_spec(language);
    if (!spec) {
        result->has_error = true;
        result->error_msg = engine_arena_strdup(a, "unsupported language");
        engine_index_mark_done(rel_path);
        return result;
    }

    // Get tree-sitter language
    const TSLanguage *ts_lang = engine_ts_language(language);
    if (!ts_lang) {
        result->has_error = true;
        result->error_msg = engine_arena_strdup(a, "no tree-sitter grammar");
        engine_index_mark_done(rel_path);
        return result;
    }
    ...
    if (!parser) {
        result->has_error = true;
        result->error_msg = engine_arena_strdup(a, "parser alloc failed");
        engine_index_mark_done(rel_path);
        return result;
    }
    ...
    EngineStringInput str_input = {source, (uint32_t)source_len};
    TSInput ts_input = {
        &str_input,
        engine_string_read,
        TSInputEncodingUTF8,
    ...
    if (timeout_micros > 0) {
        ...
        opts.progress_callback = engine_timeout_cb;
    }
    ...
    if (!tree) {
        result->has_error = true;
        result->error_msg =
            engine_arena_strdup(a, timeout_micros > 0 ? "parse timeout" : "parse failed");
        engine_index_mark_done(rel_path);
        return result;
    }
    ...
    result->module_qn = engine_fqn_module_source_lang(a, project, rel_path, language);
    result->is_test_file = engine_is_test_file(rel_path, language);

    // Build extraction context
    EngineExtractCtx ctx = {
        .arena = a,
        .result = result,
        .source = source,
    ...
    engine_extract_definitions(&ctx);
    engine_extract_imports(&ctx);
    engine_extract_unified(&ctx);
    ...
    if (1 || 1 || 1) {
        ...
        if (preprocessed && preprocessed->source) {
            ...
            if (pp_parser) {
                ...
                EngineStringInput pp_input = {expanded, (uint32_t)expanded_len};
                TSInput pp_ts_input = {
                    &pp_input,
                    engine_string_read,
                    TSInputEncodingUTF8,
                ...
                if (pp_tree) {
                    ...
                    for (int i = calls_before; i < result->calls.count; i++) {
                        result->calls.items[i].source_origin = ENGINE_SOURCE_ORIGIN_PREPROCESSED;
                    }
                    for (int i = usages_before; i < result->usages.count; i++) {
                        result->usages.items[i].source_origin = ENGINE_SOURCE_ORIGIN_PREPROCESSED;
                    }
                    ...
                    if (ts_node_has_error(root)) {
                        engine_error_regions_t raw_regs = {{0}, {0}, 0};
                        engine_collect_error_regions(root, &raw_regs);
                        if (raw_regs.count > 0) {
                            int defs_before = result->defs.count;
                            engine_extract_definitions(&pp_ctx);
                            int w = defs_before;
                            for (int i = defs_before; i < result->defs.count; i++) {
                                EngineDefinition *d = &result->defs.items[i];
                                bool adopt = false;
                                if (engine_remap_preprocessed_def(d, preprocessed)) {
                                ...
                                if (adopt && (!d->name ||
                                              !engine_line_contains(source, source_len, d->start_line,
                                                                 d->name) ||
                                              !engine_span_contains_callable_def(
            ...
            engine_preprocessed_source_free(preprocessed);
    ...
    // EngineFileResult share the same file). Range matching is used instead of
    ...
    for (int di = 0; di < def_count; di++) {
        EngineDefinition *d = &result->defs.items[di];
        int pc = 0;
        if (d->param_names) {
    ...
    for (int ci = 0; ci < orig_calls_count; ci++) {
        const EngineCall *c = &result->calls.items[ci];
        ...
        int best_span = -1;
        for (int di = 0; di < def_count; di++) {
            const EngineDefinition *d = &result->defs.items[di];
            if (!d->name || !d->label ||
        ...
        EngineDefinition *d = &result->defs.items[best];
    ...
    if (ts_node_has_error(root)) {
        engine_error_regions_t regs = {{0}, {0}, 0};
        if (strcmp(ts_node_type(root), "ERROR") == 0) {
            engine_error_regions_push(&regs, root); /* whole file unparseable */
        } else {
            engine_collect_error_regions(root, &regs);
        }
        engine_subtract_recovered_regions(&regs, &result->defs);
        /* #1071: don't flag a benign function-like-macro call (defined in-file)
         * that tree-sitter can't parse without the preprocessor. */
        engine_subtract_macro_invocation_regions(&regs, &result->defs, source, source_len);
        if (regs.count > 0) {
            result->parse_incomplete = true;
            result->error_region_count = regs.count;
            result->error_ranges = engine_error_ranges_str(a, &regs);
        }
    }
    ...
    // Retain tree for cross-file LSP reuse (caller frees via engine_free_tree)
