#ifndef ENGINE_PREPROCESSOR_H
#define ENGINE_PREPROCESSOR_H
...
// The returned string must be freed with engine_preprocess_free().
char *engine_preprocess(const char *source, int source_len, const char *filename,
...
// Free with engine_preprocessed_source_free().
EnginePreprocessedSource *engine_preprocess_with_map(const char *source, int source_len,
                                               const char *filename, const char **extra_defines,
                                               const char **include_paths, int cpp_mode);

// Free preprocessed source returned by engine_preprocess.
void engine_preprocess_free(char *expanded);

// Free preprocessed source and line maps returned by engine_preprocess_with_map.
void engine_preprocessed_source_free(EnginePreprocessedSource *pp);

#ifdef __cplusplus
}
#endif

#endif // ENGINE_PREPROCESSOR_H
