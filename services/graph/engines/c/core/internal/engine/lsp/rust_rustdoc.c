...
static const char* join_path(EngineArena* arena, yyjson_val* path_arr,
    const char* crate_qn) {
    ...
    return engine_arena_strndup(arena, buf, (size_t)off);
}

/* Convert a rustdoc Type JSON value into a EngineType. Best-effort —
 ...
 */
static const EngineType* parse_rustdoc_type(yyjson_val* t, EngineArena* arena,
    const char* crate_qn) {
    if (!t || !yyjson_is_obj(t)) return engine_type_unknown();

    yyjson_val* prim = yyjson_obj_get(t, "primitive");
    if (prim && yyjson_is_str(prim)) {
        return engine_type_builtin(arena, yyjson_get_str(prim));
    }
    ...
    if (resolved && yyjson_is_obj(resolved)) {
        ...
        if (name && yyjson_is_str(name)) {
            qn = engine_arena_strdup(arena, yyjson_get_str(name));
        }
        if (!qn) return engine_type_unknown();
        ...
        if (args && yyjson_is_obj(args)) {
            ...
            if (angle && yyjson_is_obj(angle)) {
                ...
                if (arr && yyjson_is_arr(arr) && yyjson_arr_size(arr) > 0) {
                    ...
                    if (ti > 0) {
                        return engine_type_template(arena, qn, targs, ti);
                    }
                }
            }
        }
    ...
    if (borrowed && yyjson_is_obj(borrowed)) {
        ...
        return engine_type_reference(arena,
    ...
    if (tuple && yyjson_is_arr(tuple)) {
        ...
        if (n == 0) return engine_type_builtin(arena, "()");
        const EngineType* elems[8];
        int ei = 0;
        yyjson_val* item;
        yyjson_arr_iter it;
        ...
        return engine_type_tuple(arena, elems, ei);
    }
...
static bool register_function(EngineTypeRegistry* reg, EngineArena* arena,
     ...
     * (implicit unit). */
    const EngineType* ret = engine_type_builtin(arena, "()");
    ...
    const EngineType** ra = (const EngineType**)engine_arena_alloc(arena,
...
int engine_rust_rustdoc_ingest(EngineTypeRegistry* reg, EngineArena* arena,
    ...
    while ((key = yyjson_obj_iter_next(&it))) {
        ...
        if (kind && (strcmp(kind, "struct") == 0 ||
            ...
            if (register_function(reg, arena, engine_arena_strdup(arena, short_name),
                                  qn, NULL, fn, crate_qn)) {
                count++;
            }
        }
