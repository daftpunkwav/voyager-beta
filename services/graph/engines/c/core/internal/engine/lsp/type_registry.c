...
static void build_qn_index(EngineTypeRegistry *reg, EngineArena *idx_arena, bool for_funcs) {
    ...
    int *buckets = (int *)engine_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    EngineRegistryHashEntry *entries = (EngineRegistryHashEntry *)engine_arena_alloc(
        idx_arena, (size_t)count * sizeof(EngineRegistryHashEntry));
    if (!buckets || !entries)
        return;
...
static void build_method_index(EngineTypeRegistry *reg, EngineArena *idx_arena) {
    if (reg->func_count == 0)
        return;
    ...
    int *buckets = (int *)engine_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    EngineRegistryHashEntry *entries = (EngineRegistryHashEntry *)engine_arena_alloc(
        reg->arena, (size_t)reg->func_count * sizeof(EngineRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
    ...
    for (int i = 0; i < reg->func_count; i++) {
        const EngineRegisteredFunc *f = &reg->funcs[i];
...
static void build_type_embed_index(EngineTypeRegistry *reg, EngineArena *idx_arena) {
    int ecount = 0;
    ...
    int *buckets = (int *)engine_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    EngineRegistryHashEntry *entries = (EngineRegistryHashEntry *)engine_arena_alloc(
        idx_arena, (size_t)ecount * sizeof(EngineRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
...
static void build_ffunc_short_index(EngineTypeRegistry *reg, EngineArena *idx_arena) {
    int fcount = 0;
    for (int i = 0; i < reg->func_count; i++) {
        const EngineRegisteredFunc *f = &reg->funcs[i];
        if (!f->receiver_type && f->short_name)
    ...
    int *buckets = (int *)engine_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    EngineRegistryHashEntry *entries = (EngineRegistryHashEntry *)engine_arena_alloc(
        idx_arena, (size_t)fcount * sizeof(EngineRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
...
int engine_type_embed_iter_next(EngineTypeEmbedIter *it) {
    const EngineTypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const EngineRegistryHashEntry *e = &reg->type_embed_entries[it->chain_idx];
...
int engine_free_func_iter_next(EngineFreeFuncIter *it) {
    const EngineTypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const EngineRegistryHashEntry *e = &reg->ffunc_short_entries[it->chain_idx];
        int p = e->payload_index;
        uint64_t h = e->hash;
        it->chain_idx = e->next_index;
        if (h != it->hash)
...
int engine_method_iter_next(EngineMethodIter *it) {
    ...
    const EngineTypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const EngineRegistryHashEntry *e = &reg->method_entries[it->chain_idx];
        int p = e->payload_index;
        uint64_t h = e->hash;
        it->chain_idx = e->next_index;
        if (h != it->hash) {
        ...
        const EngineRegisteredFunc *f = &reg->funcs[p];
...
void engine_registry_finalize(EngineTypeRegistry *reg) {
    if (!reg)
        return;
    engine_registry_finalize_into(reg, reg->arena);
}

void engine_registry_init(EngineTypeRegistry *reg, EngineArena *arena) {
    memset(reg, 0, sizeof(EngineTypeRegistry));
    reg->arena = arena;
}
...
void engine_registry_add_func(EngineTypeRegistry *reg, EngineRegisteredFunc func) {
    ...
    if (reg->func_count >= reg->func_cap) {
        ...
        EngineRegisteredFunc *new_items = (EngineRegisteredFunc *)engine_arena_alloc(
            reg->arena, (size_t)new_cap * sizeof(EngineRegisteredFunc));
        if (!new_items)
            return;
        if (reg->funcs && reg->func_count > 0) {
            memcpy(new_items, reg->funcs, (size_t)reg->func_count * sizeof(EngineRegisteredFunc));
        }
        reg->funcs = new_items;
...
void engine_registry_add_type(EngineTypeRegistry *reg, EngineRegisteredType type) {
    ...
    if (reg->type_count >= reg->type_cap) {
        ...
        EngineRegisteredType *new_items = (EngineRegisteredType *)engine_arena_alloc(
            reg->arena, (size_t)new_cap * sizeof(EngineRegisteredType));
        if (!new_items)
            return;
        if (reg->types && reg->type_count > 0) {
            memcpy(new_items, reg->types, (size_t)reg->type_count * sizeof(EngineRegisteredType));
        }
        reg->types = new_items;
...
static const EngineRegisteredFunc *lookup_method_self(const EngineTypeRegistry *reg,
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        for (int idx = reg->method_buckets[slot]; idx >= 0;
            ...
            if (reg->method_entries[idx].hash != h)
                continue;
            const EngineRegisteredFunc *f = &reg->funcs[reg->method_entries[idx].payload_index];
...
const EngineRegisteredFunc *engine_registry_lookup_method(const EngineTypeRegistry *reg,
    ...
    if (!reg || !receiver_qn || !method_name)
        return NULL;
    const EngineRegisteredFunc *r = lookup_method_self(reg, receiver_qn, method_name);
    if (!r && reg->fallback) {
        return engine_registry_lookup_method(reg->fallback, receiver_qn, method_name);
    }
    return r;
}

static const EngineRegisteredType *lookup_type_self(const EngineTypeRegistry *reg,
...
const EngineRegisteredType *engine_registry_lookup_type(const EngineTypeRegistry *reg,
    ...
    if (!reg || !qualified_name)
        return NULL;
    const EngineRegisteredType *r = lookup_type_self(reg, qualified_name);
    if (!r && reg->fallback) {
        return engine_registry_lookup_type(reg->fallback, qualified_name);
    }
    return r;
}

static const EngineRegisteredFunc *lookup_func_self(const EngineTypeRegistry *reg,
                                                 const char *qualified_name) {
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        uint64_t h = fnv1a(qualified_name);
...
const EngineRegisteredFunc *engine_registry_lookup_func(const EngineTypeRegistry *reg,
    ...
    if (!reg || !qualified_name)
        return NULL;
    const EngineRegisteredFunc *r = lookup_func_self(reg, qualified_name);
    if (!r && reg->fallback) {
        return engine_registry_lookup_func(reg->fallback, qualified_name);
    }
    return r;
}
...
const EngineRegisteredType *engine_registry_resolve_alias(const EngineTypeRegistry *reg,
    ...
    if (!reg || !type_qn)
        return NULL;
    const EngineRegisteredType *rt = engine_registry_lookup_type(reg, type_qn);
    for (int i = 0; i < 16 && rt && rt->alias_of; i++) {
        const EngineRegisteredType *next = engine_registry_lookup_type(reg, rt->alias_of);
        if (!next)
            return rt;
        rt = next;
    }
...
const EngineRegisteredFunc *engine_registry_lookup_method_aliased(const EngineTypeRegistry *reg,
    ...
    // Direct lookup first
    const EngineRegisteredFunc *f = engine_registry_lookup_method(reg, receiver_qn, method_name);
    if (f)
        return f;

    // Follow alias chain
    const EngineRegisteredType *rt = engine_registry_lookup_type(reg, receiver_qn);
    for (int i = 0; i < 16 && rt && rt->alias_of; i++) {
        f = engine_registry_lookup_method(reg, rt->alias_of, method_name);
        if (f)
            return f;
        rt = engine_registry_lookup_type(reg, rt->alias_of);
    }
    return NULL;
}
...
const EngineRegisteredFunc *engine_registry_lookup_symbol(const EngineTypeRegistry *reg,
    ...
    return engine_registry_lookup_func(reg, buf);
}

// Count parameters in a FUNC signature.
static int count_func_params(const EngineRegisteredFunc *f) {
...
const EngineRegisteredFunc *engine_registry_lookup_method_by_args(const EngineTypeRegistry *reg,
    ...
    const EngineRegisteredFunc *first_match = NULL;
    const EngineRegisteredFunc *range_match = NULL;
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        for (int idx = reg->method_buckets[slot]; idx >= 0;
            ...
            const EngineRegisteredFunc *f = &reg->funcs[pi];
            if (!f->receiver_type || !f->short_name || strcmp(f->receiver_type, receiver_qn) != 0 ||
                strcmp(f->short_name, method_name) != 0)
                continue;
        ...
        if (reg->fallback) {
            return engine_registry_lookup_method_by_args(reg->fallback, receiver_qn, method_name,
                                                      arg_count);
        }
        return NULL;
    }
    ...
    const EngineRegisteredFunc *res = range_match ? range_match : first_match;
    if (!res && reg->fallback) {
        return engine_registry_lookup_method_by_args(reg->fallback, receiver_qn, method_name,
                                                  arg_count);
    }
...
static const char *type_to_qn_simple(const EngineType *t) {
    if (!t)
        return NULL;
...
static int score_overload_match(const EngineRegisteredFunc *f, const EngineType **arg_types,
                                int arg_count) {
    int pc = count_func_params(f);
    int min_pc = (f->min_params >= 0) ? f->min_params : pc;
    if (arg_count < min_pc || arg_count > pc)
    ...
    for (int i = 0; i < arg_count; i++) {
        const EngineType *expected = f->signature->data.func.param_types[i];
        const EngineType *actual = arg_types[i];
        if (!expected || !actual || engine_type_is_unknown(actual))
            continue; // neutral
...
const EngineRegisteredFunc *engine_registry_lookup_method_by_types(const EngineTypeRegistry *reg,
    ...
    if (!arg_types)
        return engine_registry_lookup_method_by_args(reg, receiver_qn, method_name, arg_count);
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return engine_registry_lookup_method_by_types(reg->fallback, receiver_qn, method_name,
                                                       arg_types, arg_count);
        }
        return NULL;
    }
    ...
    const EngineRegisteredFunc *res = best ? best : first_match;
    if (!res && reg->fallback) {
        return engine_registry_lookup_method_by_types(reg->fallback, receiver_qn, method_name,
                                                   arg_types, arg_count);
    }
...
const EngineRegisteredFunc *engine_registry_lookup_symbol_by_types(const EngineTypeRegistry *reg,
    ...
    if (!reg || !package_qn || !name)
        return NULL;
    if (!arg_types)
        return engine_registry_lookup_symbol_by_args(reg, package_qn, name, arg_count);
    ...
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return engine_registry_lookup_symbol_by_types(reg->fallback, package_qn, name, arg_types,
                                                       arg_count);
        }
        return NULL;
    }
...
const EngineRegisteredFunc *engine_registry_lookup_symbol_by_args(const EngineTypeRegistry *reg,
    ...
    // engine_registry_lookup_symbol_by_types). O(overloads) not O(func_count).
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        int exact_pi = -1, range_pi = -1, first_pi = -1;
        uint64_t h = fnv1a(buf);
        int slot = (int)(h & (uint64_t)(reg->func_qn_bucket_count - 1));
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return engine_registry_lookup_symbol_by_args(reg->fallback, package_qn, name, arg_count);
        }
        return NULL;
    }
...
const EngineRegisteredFunc *engine_registry_lookup_callable(const EngineTypeRegistry *reg, EngineArena *arena,
    ...
    // Synthesise a EngineRegisteredFunc whose QN is "<type_qn>.__call".
    ...
    char *qn = (char *)engine_arena_alloc(arena, total);
    if (!qn)
        return NULL;
    memcpy(qn, type_qn, qn_len);
    memcpy(qn + qn_len, suffix, sizeof(suffix));

    EngineRegisteredFunc *f = (EngineRegisteredFunc *)engine_arena_alloc(arena, sizeof(EngineRegisteredFunc));
    if (!f)
        return NULL;
    memset(f, 0, sizeof(*f));
    f->qualified_name = qn;
...
const EngineType *engine_registry_lookup_index_signature(const EngineTypeRegistry *reg, const char *type_qn,
    ...
    if (rt->index_key_type->kind == ENGINE_TYPE_BUILTIN && key_type->kind == ENGINE_TYPE_BUILTIN) {
