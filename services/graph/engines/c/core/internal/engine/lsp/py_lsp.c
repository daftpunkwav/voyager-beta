...
static void py_register_func_decorators(EngineRegisteredFunc *func, const char **decorators) {
    if (!func)
        return;
...
static bool py_func_is_exact_callable_value(const EngineRegisteredFunc *func) {
    return func && func->qualified_name &&
           !(func->flags &
             (ENGINE_FUNC_FLAG_PROPERTY | ENGINE_FUNC_FLAG_OVERLOAD | ENGINE_FUNC_FLAG_AMBIGUOUS_BINDING)) &&
           (!func->decorator_qns || !func->decorator_qns[0]);
}

static int py_func_qn_pointer_cmp(const void *left, const void *right) {
    const EngineRegisteredFunc *a = *(EngineRegisteredFunc *const *)left;
    const EngineRegisteredFunc *b = *(EngineRegisteredFunc *const *)right;
 ...
 * behavior. */
static void py_mark_ambiguous_callable_bindings(EngineTypeRegistry *registry) {
    if (!registry || registry->func_count <= 1)
        return;
    size_t count = (size_t)registry->func_count;
    EngineRegisteredFunc **sorted = count <= SIZE_MAX / sizeof(*sorted)
                                     ? (EngineRegisteredFunc **)malloc(count * sizeof(*sorted))
                                     : NULL;
    if (!sorted) {
        /* Allocation failure must reduce precision, never fabricate it. */
        for (int i = 0; i < registry->func_count; i++)
            registry->funcs[i].flags |= ENGINE_FUNC_FLAG_AMBIGUOUS_BINDING;
        return;
    }
    for (int i = 0; i < registry->func_count; i++)
    ...
    for (size_t first = 0; first < count;) {
        ...
        if (qn && end - first > 1) {
            for (size_t i = first; i < end; i++)
                sorted[i]->flags |= ENGINE_FUNC_FLAG_AMBIGUOUS_BINDING;
        }
        first = end;
    }
    free(sorted);
 ...
 * crash. The cap is ENGINE_LSP_MAX_WALK_DEPTH, env-overridable via the same name.
...
static void py_resolve_calls_in(PyLSPContext *ctx, TSNode node) {
    if (ctx->walk_depth >= engine_lsp_max_walk_depth())
        return;
    ctx->walk_depth++;
    py_resolve_calls_in_inner(ctx, node);
    ctx->walk_depth--;
}
static const EngineType *py_eval_expr_type(PyLSPContext *ctx, TSNode node);
static const EngineType *py_eval_expr_type_uncached(PyLSPContext *ctx, TSNode node);
...
static const EngineRegisteredFunc *py_lookup_attribute(PyLSPContext *ctx, const char *type_qn,
...
static const EngineType *py_resolve_annotation(PyLSPContext *ctx, const char *ann);
static const EngineType *py_iterable_element_type(PyLSPContext *ctx, const EngineType *iter_type);
static const EngineType *py_lookup_field(PyLSPContext *ctx, const char *type_qn,
                                      const char *field_name);
static void py_register_instance_field(PyLSPContext *ctx, const char *class_qn,
                                       const char *field_name, const EngineType *field_type);
static void py_bind_for_target(PyLSPContext *ctx, TSNode left, const EngineType *elem_type);
 ...
 * wrappers — never call engine_scope_bind(ctx->current_scope, ...) or assign
...
static void py_scope_bind(PyLSPContext *ctx, const char *name, const EngineType *type) {
    ctx->type_cache_gen++;
...
static void py_scope_bind_callable(PyLSPContext *ctx, const char *name, const EngineType *type,
                                   const char *callable_qn) {
    ctx->type_cache_gen++;
    engine_scope_bind_callable(ctx->current_scope, name, type, callable_qn);
    const char *bound = name ? engine_scope_lookup_callable(ctx->current_scope, name) : NULL;
    if (name && (!engine_scope_contains(ctx->current_scope, name) ||
...
static EngineScope *py_scope_push_checked(PyLSPContext *ctx) {
    if (!ctx)
        return NULL;
    EngineScope *parent = ctx->current_scope;
    EngineScope *child = engine_scope_push(ctx->arena, parent);
...
static void py_scope_clear_callable(PyLSPContext *ctx, const char *name) {
    if (engine_scope_update_callable(ctx->current_scope, name, NULL)) {
        ctx->type_cache_gen++;
    }
}

static void py_scope_restore(PyLSPContext *ctx, EngineScope *saved) {
    ctx->type_cache_gen++;
...
void py_lsp_add_import(PyLSPContext *ctx, const char *local_name, const char *module_qn) {
    ...
    const char **names =
        (const char **)engine_arena_alloc(ctx->arena, (size_t)(new_count + 1) * sizeof(const char *));
    const char **qns =
        (const char **)engine_arena_alloc(ctx->arena, (size_t)(new_count + 1) * sizeof(const char *));
    unsigned char *kinds =
        (unsigned char *)engine_arena_alloc(ctx->arena, (size_t)new_count * sizeof(unsigned char));
    if (!names || !qns || !kinds)
        return;
...
static char *py_import_node_text_dup(PyLSPContext *ctx, TSNode node) {
    ...
    if (end < start || (int)end > ctx->source_len)
        return NULL;
    return engine_arena_strndup(ctx->arena, ctx->source + start, (size_t)(end - start));
}
...
static const char *py_canonical_from_import_qn(PyLSPContext *ctx, TSNode statement,
    ...
    const char *full_suffix =
        module_suffix && module_suffix[0]
            ? engine_arena_sprintf(ctx->arena, "%s.%s", module_suffix, member)
            : member;
    if (!full_suffix)
        return NULL;
    ...
    return engine_arena_sprintf(ctx->arena, "%s.%s", incoming_qn, member);
}

/* EngineImport intentionally stores only the local spelling and module path.
...
static void py_bind_dotted_prefixes(PyLSPContext *ctx, const char *qn) {
    ...
    for (;;) {
        ...
        char *prefix = (char *)engine_arena_alloc(ctx->arena, prefix_len + 1);
        if (!prefix)
            return;
        ...
        if (engine_type_is_unknown(engine_scope_lookup(ctx->current_scope, bind_short))) {
            py_scope_bind(ctx, bind_short, engine_type_module(ctx->arena, prefix));
        }
        p = dot + 1;
    }
}
...
static void py_bind_import_index(PyLSPContext *ctx, int index, bool synthetic_fallback) {
    ...
    if (direct_kind == PY_DIRECT_IMPORT_UNALIASED) {
        ...
        t = engine_type_named(ctx->arena, qn);
            ...
            // EngineImport shape cannot distinguish the two, but NAMED(qn)
    ...
    const EngineRegisteredFunc *imported =
        from_style && ctx->registry ? engine_registry_lookup_func(ctx->registry, qn) : NULL;
...
const EngineType *py_lsp_lookup_in_scope(const PyLSPContext *ctx, const char *name) {
    ...
    return engine_scope_lookup(ctx->current_scope, name);
}
...
static char *py_node_text(PyLSPContext *ctx, TSNode node) {
    ...
    char *out = (char *)engine_arena_alloc(ctx->arena, len + 1);
    if (!out)
        return NULL;
...
static void py_register_lambda(PyLSPContext *ctx, const char *name, TSNode lambda_node) {
    ...
    if (ctx->lambda_count >= ctx->lambda_cap) {
        ...
        EngineLambdaEntry *grown =
            (EngineLambdaEntry *)engine_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(EngineLambdaEntry));
        if (!grown)
            return;
        if (ctx->lambdas && ctx->lambda_count > 0) {
            memcpy(grown, ctx->lambdas, (size_t)ctx->lambda_count * sizeof(EngineLambdaEntry));
        }
        ctx->lambdas = grown;
    ...
    ctx->lambdas[ctx->lambda_count].name = engine_arena_strdup(ctx->arena, name);
...
static void py_register_dict_literal(PyLSPContext *ctx, const char *var, TSNode dict_node) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const EngineRegisteredFunc *f =
            engine_registry_lookup_symbol(ctx->registry, ctx->module_qn, val_text);
        if (!f) {
            f = engine_registry_lookup_symbol(ctx->registry, "builtins", val_text);
        }
        if (!f)
            continue;
        ...
        if (ctx->dict_literal_count >= ctx->dict_literal_cap) {
            ...
            EngineDictLiteralEntry *grown = (EngineDictLiteralEntry *)engine_arena_alloc(
                ctx->arena, (size_t)new_cap * sizeof(EngineDictLiteralEntry));
            if (!grown)
                return;
            if (ctx->dict_literals && ctx->dict_literal_count > 0) {
                memcpy(grown, ctx->dict_literals,
                       (size_t)ctx->dict_literal_count * sizeof(EngineDictLiteralEntry));
            }
            ctx->dict_literals = grown;
            ctx->dict_literal_cap = new_cap;
        }
        ctx->dict_literals[ctx->dict_literal_count].var_name = engine_arena_strdup(ctx->arena, var);
        ctx->dict_literals[ctx->dict_literal_count].literal_key =
            engine_arena_strdup(ctx->arena, key_text);
        ctx->dict_literals[ctx->dict_literal_count].target_qn =
            engine_arena_strdup(ctx->arena, f->qualified_name);
...
static const char *py_lookup_dict_dispatch(PyLSPContext *ctx, const char *var, const char *key) {
    ...
    for (int i = 0; i < ctx->dict_literal_count; i++) {
        const EngineDictLiteralEntry *e = &ctx->dict_literals[i];
...
static void py_emit_resolved_call_reason(PyLSPContext *ctx, const char *callee_qn,
    ...
    for (int i = start; i < n; i++) {
        EngineResolvedCall *rc = &ctx->resolved_calls->items[i];
        if (rc->kind == ENGINE_RESOLVED_INVOCATION && rc->site_start_byte == site_start &&
            ...
            if (confidence > rc->confidence) {
                ...
                rc->reason = reason ? engine_arena_strdup(ctx->arena, reason) : NULL;
            }
            return;
        }
    }
    ...
    rc.callee_qn = engine_arena_strdup(ctx->arena, callee_qn);
    rc.strategy = strategy;
    rc.confidence = confidence;
    rc.reason = reason ? engine_arena_strdup(ctx->arena, reason) : NULL;
...
static void py_emit_resolved_reference(PyLSPContext *ctx, const char *callee_qn,
    ...
    for (int i = ctx->resolved_calls->count - 1; i >= 0; i--) {
        EngineResolvedCall *existing = &ctx->resolved_calls->items[i];
        if (existing->kind == ENGINE_RESOLVED_CALL_REFERENCE && existing->site_start_byte == start &&
    ...
    rc.reason = source_name && strcmp(source_name, leaf) != 0
                    ? engine_arena_strdup(ctx->arena, source_name)
                    : NULL;
...
static void py_emit_unresolved_reference(PyLSPContext *ctx, const char *candidate_qn, TSNode site) {
    ...
    EngineResolvedCall rc = {0};
    rc.caller_qn = ctx->enclosing_func_qn;
    rc.callee_qn = engine_arena_strdup(ctx->arena, candidate_qn);
...
static const char *py_exact_callable_target_ex(PyLSPContext *ctx, TSNode node,
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (engine_scope_contains(ctx->current_scope, name)) {
            const char *bound = engine_scope_lookup_callable(ctx->current_scope, name);
            if (bound && lexical_alias_out) {
                ...
                const EngineRegisteredFunc *module_symbol =
    ...
    if (strcmp(kind, "attribute") == 0) {
        ...
        const EngineType *receiver = py_eval_expr_type(ctx, object);
        if (!member || !receiver)
            return NULL;
        receiver = engine_type_resolve_alias(receiver);
        if (receiver->kind == ENGINE_TYPE_MODULE) {
            const EngineRegisteredFunc *f =
                engine_registry_lookup_symbol(ctx->registry, receiver->data.module.module_qn, member);
            return py_func_is_exact_callable_value(f) ? f->qualified_name : NULL;
        }
        if (receiver->kind == ENGINE_TYPE_NAMED) {
...
static void py_resolve_value_references_at(PyLSPContext *ctx, TSNode call) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        const char *candidate = strcmp(kind, "identifier") == 0
                                    ...
                                    : NULL;
        if (candidate && engine_scope_contains(ctx->current_scope, source_name)) {
            const EngineType *binding =
                engine_type_resolve_alias(engine_scope_lookup(ctx->current_scope, source_name));
            if (binding && binding->kind == ENGINE_TYPE_NAMED &&
...
static const EngineRegisteredFunc *py_lookup_attribute_depth(PyLSPContext *ctx, const char *type_qn,
...
static const EngineType *py_overlay_lookup_field(const PyLSPContext *ctx, const char *class_qn,
                                              const char *field_name) {
...
static void py_overlay_register_field(PyLSPContext *ctx, const char *class_qn,
    ...
    if (ctx->field_overlay_count >= ctx->field_overlay_cap) {
        ...
        void *na = engine_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*ctx->field_overlay));
        if (!na) {
            py_disable_callable_value_proof(ctx);
            return;
        }
    ...
    const char *stored_class = engine_arena_strdup(ctx->arena, class_qn);
    const char *stored_field = engine_arena_strdup(ctx->arena, field_name);
 ...
 * or NULL if no match. */
static const EngineType *py_lookup_field_depth(PyLSPContext *ctx, const char *type_qn,
    ...
    {
        const EngineType *ov = py_overlay_lookup_field(ctx, type_qn, field_name);
        if (ov)
            return ov;
    }
    if (rt->alias_of) {
        const EngineType *a = py_lookup_field_depth(ctx, rt->alias_of, field_name, depth + 1);
        if (a)
            return a;
    }
    if (rt->embedded_types) {
...
static void py_register_instance_field(PyLSPContext *ctx, const char *class_qn,
    ...
    // Find the type entry. engine_registry_lookup_type returns a const pointer;
    ...
    if (rt->field_names && rt->field_types) {
        for (int i = 0; rt->field_names[i]; i++) {
            if (strcmp(rt->field_names[i], field_name) == 0) {
                ((const EngineType **)rt->field_types)[i] = field_type;
                return;
            }
        }
    }
    ...
    const char **new_names =
        (const char **)engine_arena_alloc(ctx->arena, (size_t)(new_count + 1) * sizeof(const char *));
    const EngineType **new_types = (const EngineType **)engine_arena_alloc(
        ctx->arena, (size_t)(new_count + 1) * sizeof(const EngineType *));
    ...
    new_names[existing] = engine_arena_strdup(ctx->arena, field_name);
...
static const EngineType *py_literal_type(PyLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "list_comprehension") == 0)
        return engine_type_builtin(ctx->arena, "list");
    if (strcmp(k, "dictionary_comprehension") == 0)
        return engine_type_builtin(ctx->arena, "dict");
    if (strcmp(k, "set_comprehension") == 0)
        return engine_type_builtin(ctx->arena, "set");
    if (strcmp(k, "generator_expression") == 0)
        return engine_type_builtin(ctx->arena, "generator");
    return NULL;
}
...
static const EngineType *py_substitute_self(PyLSPContext *ctx, const EngineType *t,
    ...
    if (t->kind == ENGINE_TYPE_NAMED && t->data.named.qualified_name) {
        ...
        if (strcmp(qn, "Self") == 0 || strcmp(qn, "typing.Self") == 0 ||
            ...
            return engine_type_named(ctx->arena, receiver_qn);
        }
    }
    if (t->kind == ENGINE_TYPE_UNION) {
        int n = t->data.union_type.count;
        const EngineType **members = (const EngineType **)engine_arena_alloc(
        ...
        return engine_type_union(ctx->arena, members, n);
    }
    return t;
}
...
static const EngineType *py_func_return_type_recv(PyLSPContext *ctx, const char *func_qn,
                                               const char *receiver_qn) {
    if (!ctx || !func_qn)
        return engine_type_unknown();
    const EngineRegisteredFunc *f = engine_registry_lookup_func(ctx->registry, func_qn);
    if (!f || !f->signature)
        return engine_type_unknown();
    if (f->signature->kind != ENGINE_TYPE_FUNC)
        return engine_type_unknown();
    const EngineType **rets = f->signature->data.func.return_types;
    if (!rets || !rets[0])
        return engine_type_unknown();
    ...
    if (!rets[1]) {
        ...
        while (rets[count])
            count++;
        base = engine_type_tuple(ctx->arena, rets, count);
    }
    if (receiver_qn) {
...
static const EngineType *py_func_return_type(PyLSPContext *ctx, const char *func_qn) {
...
static void py_bind_for_target(PyLSPContext *ctx, TSNode left, const EngineType *elem_type) {
    ...
    if (strcmp(lk, "pattern_list") == 0 || strcmp(lk, "tuple_pattern") == 0 ||
        ...
        const EngineType *const *elems = NULL;
        int count = 0;
        if (elem_type) {
            if (elem_type->kind == ENGINE_TYPE_TUPLE) {
                elems = elem_type->data.tuple.elems;
                count = elem_type->data.tuple.count;
            } else if (elem_type->kind == ENGINE_TYPE_TEMPLATE &&
        ...
        for (uint32_t i = 0; i < lc; i++) {
            ...
            if (strcmp(tk, "identifier") == 0) {
                ...
                if (!nm)
                    continue;
                const EngineType *bind_type;
                if (elems && (int)i < count && elems[i])
                    bind_type = elems[i];
                else
                    bind_type = engine_type_unknown();
...
static const EngineType *py_iterable_element_type(PyLSPContext *ctx, const EngineType *iter_type) {
    if (!iter_type)
        return engine_type_unknown();
    if (iter_type->kind == ENGINE_TYPE_TEMPLATE) {
        const char *name = iter_type->data.template_type.template_name;
        const EngineType **args = iter_type->data.template_type.template_args;
        ...
        if (strcmp(name, "ItemsView") == 0 || strcmp(name, "dict_items") == 0) {
            ...
            if (n >= 2) {
                const EngineType *elems[3] = {args[0], args[1], NULL};
                return engine_type_tuple(ctx->arena, elems, 2);
            }
            return engine_type_unknown();
        }
        ...
        if (strcmp(name, "tuple") == 0) {
            if (n == 1)
                return args[0];
            return engine_type_union(ctx->arena, args, n);
        }
...
static const EngineType *py_eval_expr_type_uncached(PyLSPContext *ctx, TSNode node) {
    ...
    const EngineType *lit = py_literal_type(ctx, node);
    if (lit)
        return lit;
    ...
    if (strcmp(k, "tuple") == 0) {
        ...
        if (cn == 0)
            return engine_type_builtin(ctx->arena, "tuple");
        const EngineType **elems = (const EngineType **)engine_arena_alloc(
            ctx->arena, (size_t)(cn + 1) * sizeof(const EngineType *));
        if (!elems)
            return engine_type_builtin(ctx->arena, "tuple");
        ...
        elems[cn] = NULL;
        return engine_type_tuple(ctx->arena, elems, (int)cn);
    }
    if (strcmp(k, "list") == 0) {
        uint32_t cn = ts_node_named_child_count(node);
        if (cn == 0)
            return engine_type_builtin(ctx->arena, "list");
        const EngineType *first = py_eval_expr_type(ctx, ts_node_named_child(node, 0));
        if (!first || engine_type_is_unknown(first))
            return engine_type_builtin(ctx->arena, "list");
        return engine_type_template(ctx->arena, "list", &first, 1);
    }
    if (strcmp(k, "set") == 0) {
        uint32_t cn = ts_node_named_child_count(node);
        if (cn == 0)
        ...
        if (!first || engine_type_is_unknown(first))
            return engine_type_builtin(ctx->arena, "set");
        return engine_type_template(ctx->arena, "set", &first, 1);
    }
    if (strcmp(k, "dictionary") == 0) {
        ...
        for (uint32_t i = 0; i < cn; i++) {
            ...
            if (ts_node_is_null(key) || ts_node_is_null(value))
                continue;
            const EngineType *kt = py_eval_expr_type(ctx, key);
            const EngineType *vt = py_eval_expr_type(ctx, value);
            if (!kt || !vt)
                break;
            const EngineType *args[3] = {kt, vt, NULL};
            return engine_type_template(ctx->arena, "dict", args, 2);
        }
        return engine_type_builtin(ctx->arena, "dict");
    }
    ...
    if (strcmp(k, "identifier") == 0) {
        ...
        f = engine_registry_lookup_symbol(ctx->registry, "builtins", name);
        if (f)
            return py_func_return_type(ctx, f->qualified_name);
        const EngineRegisteredType *rt = engine_registry_lookup_type(
            ctx->registry, engine_arena_sprintf(ctx->arena, "builtins.%s", name));
        if (rt)
    ...
    if (strcmp(k, "attribute") == 0) {
        ...
        if (ts_node_is_null(obj) || ts_node_is_null(attr))
            return engine_type_unknown();
        const EngineType *obj_type = py_eval_expr_type(ctx, obj);
        if (obj_type)
            obj_type = engine_type_resolve_alias(obj_type);
        char *attr_name = py_node_text(ctx, attr);
        if (!attr_name || !obj_type)
            return engine_type_unknown();

        if (obj_type->kind == ENGINE_TYPE_MODULE) {
            // module.attr — return type of the registered symbol if known.
            const char *mod = obj_type->data.module.module_qn;
            const EngineRegisteredFunc *f = engine_registry_lookup_symbol(ctx->registry, mod, attr_name);
            if (f)
                return py_func_return_type(ctx, f->qualified_name);
            // Could also be a class — return NAMED("mod.attr")
            const char *qn = engine_arena_sprintf(ctx->arena, "%s.%s", mod, attr_name);
            ...
            const char *prefix = engine_arena_sprintf(ctx->arena, "%s.", qn);
            ...
            if (is_submodule)
                return engine_type_module(ctx->arena, qn);
            return engine_type_unknown();
        }
        if (obj_type->kind == ENGINE_TYPE_NAMED) {
            const EngineRegisteredFunc *f =
            ...
            const EngineType *field =
                py_lookup_field(ctx, obj_type->data.named.qualified_name, attr_name);
            if (field)
                return field;
        }
        if (obj_type->kind == ENGINE_TYPE_BUILTIN && obj_type->data.builtin.name) {
            // Builtins map to typeshed stdlib via "builtins.<typename>".
            const char *recv_qn =
                engine_arena_sprintf(ctx->arena, "builtins.%s", obj_type->data.builtin.name);
            const EngineRegisteredFunc *f = py_lookup_attribute(ctx, recv_qn, attr_name);
            if (f)
                return py_func_return_type_recv(ctx, f->qualified_name, recv_qn);
        }
        if (obj_type->kind == ENGINE_TYPE_TEMPLATE && obj_type->data.template_type.template_name) {
            ...
            const char *recv_qn = engine_arena_sprintf(ctx->arena, "builtins.%s", tname);
            const EngineRegisteredFunc *f = py_lookup_attribute(ctx, recv_qn, attr_name);
            if (!f) {
        ...
        if (obj_type->kind == ENGINE_TYPE_UNION) {
            int matches = 0;
            const EngineRegisteredFunc *hit = NULL;
            const char *hit_recv = NULL;
            for (int i = 0; i < obj_type->data.union_type.count; i++) {
                const EngineType *m = obj_type->data.union_type.members[i];
            ...
            int field_matches = 0;
            const EngineType *field_hit = NULL;
            for (int i = 0; i < obj_type->data.union_type.count; i++) {
                const EngineType *m = obj_type->data.union_type.members[i];
                if (!m || m->kind != ENGINE_TYPE_NAMED)
                    continue;
                const EngineType *fld = py_lookup_field(ctx, m->data.named.qualified_name, attr_name);
                if (fld) {
                    field_matches++;
                    field_hit = fld;
                }
    ...
    if (strcmp(k, "call") == 0) {
        ...
        if (strcmp(fk, "attribute") == 0) {
            ...
            if (!ts_node_is_null(obj) && !ts_node_is_null(attr)) {
                ...
                if (mname && obj_type && obj_type->kind == ENGINE_TYPE_TEMPLATE) {
                    const char *tname = obj_type->data.template_type.template_name;
                    const EngineType **args = obj_type->data.template_type.template_args;
                    ...
                    if (is_dict_like && args && n >= 2) {
                        if (strcmp(mname, "items") == 0) {
                            const EngineType *pair[3] = {args[0], args[1], NULL};
                            return engine_type_template(ctx->arena, "ItemsView", pair, 2);
                        }
                        if (strcmp(mname, "keys") == 0) {
                            const EngineType *k1[2] = {args[0], NULL};
                            return engine_type_template(ctx->arena, "KeysView", k1, 1);
                        }
                        if (strcmp(mname, "values") == 0) {
                            const EngineType *v1[2] = {args[1], NULL};
                            return engine_type_template(ctx->arena, "ValuesView", v1, 1);
                        }
                        if (strcmp(mname, "get") == 0) {
                            // dict.get(k) -> Optional[V]
                            return engine_type_optional(ctx->arena, args[1]);
                        }
        ...
        if (strcmp(fk, "identifier") == 0) {
            ...
            if (strcmp(fname, "next") == 0) {
                ...
                if (!ts_node_is_null(args) && ts_node_named_child_count(args) >= 1) {
                    const EngineType *iter_type = py_eval_expr_type(ctx, ts_node_named_child(args, 0));
                    const EngineType *elem = py_iterable_element_type(ctx, iter_type);
            ...
            const EngineType *in_scope = engine_scope_lookup(ctx->current_scope, fname);
            const char *callable_qn = engine_scope_lookup_callable(ctx->current_scope, fname);
            if (callable_qn) {
                return py_func_return_type(ctx, callable_qn);
            }
            if (!engine_type_is_unknown(in_scope)) {
                if (in_scope->kind == ENGINE_TYPE_NAMED) {
                    const EngineRegisteredType *rt = engine_registry_lookup_type(
                ...
                if (in_scope->kind == ENGINE_TYPE_CALLABLE) {
                ...
                if (in_scope->kind == ENGINE_TYPE_TEMPLATE &&
            ...
            if (engine_scope_contains(ctx->current_scope, fname))
                return engine_type_unknown();
            // Module-local function call.
            const EngineRegisteredFunc *f =
                engine_registry_lookup_symbol(ctx->registry, ctx->module_qn, fname);
            if (f)
                return py_func_return_type(ctx, f->qualified_name);
            // Builtins fallback: range / len / list / str / int / etc.
            f = engine_registry_lookup_symbol(ctx->registry, "builtins", fname);
            if (f)
                return py_func_return_type(ctx, f->qualified_name);
            // Builtin class constructor: list() / dict() / set() / str() / ...
            const EngineRegisteredType *rt = engine_registry_lookup_type(
                ctx->registry, engine_arena_sprintf(ctx->arena, "builtins.%s", fname));
            if (rt)
                return engine_type_builtin(ctx->arena, fname);
        }
        ...
        if (strcmp(fk, "call") == 0 || strcmp(fk, "parenthesized_expression") == 0 ||
            strcmp(fk, "lambda") == 0) {
            const EngineType *fn_type = py_eval_expr_type(ctx, fn);
            if (fn_type && fn_type->kind == ENGINE_TYPE_CALLABLE) {
                return fn_type->data.callable.return_type ? fn_type->data.callable.return_type
                                                          : engine_type_unknown();
            }
            if (fn_type && fn_type->kind == ENGINE_TYPE_TEMPLATE &&
        ...
        if (strcmp(fk, "attribute") == 0) {
            ...
            if (obj_type->kind == ENGINE_TYPE_MODULE) {
                const char *mod = obj_type->data.module.module_qn;
                const EngineRegisteredFunc *f =
                    engine_registry_lookup_symbol(ctx->registry, mod, attr_name);
                if (f)
                    return py_func_return_type(ctx, f->qualified_name);
            }
            if (obj_type->kind == ENGINE_TYPE_NAMED) {
    ...
    if (strcmp(k, "binary_operator") == 0) {
        ...
        const EngineType *lt = py_eval_expr_type(ctx, left);
        ...
        if (lt && lt->kind == ENGINE_TYPE_NAMED && !ts_node_is_null(op)) {
        ...
        return lt ? lt : engine_type_unknown();
    }
    ...
    if (strcmp(k, "conditional_expression") == 0) {
        ...
        if (nc >= 2) {
            const EngineType *a = py_eval_expr_type(ctx, ts_node_named_child(node, 0));
            const EngineType *b = py_eval_expr_type(ctx, ts_node_named_child(node, 2));
            const EngineType *members[2] = {a, b};
            return engine_type_union(ctx->arena, members, 2);
        }
    }
    ...
    if (strcmp(k, "subscript") == 0) {
        ...
        if (ts_node_is_null(value))
            return engine_type_unknown();
        const EngineType *container = py_eval_expr_type(ctx, value);
        ...
        if (container && container->kind == ENGINE_TYPE_NAMED) {
        ...
        if (!container || container->kind != ENGINE_TYPE_TEMPLATE) {
            return engine_type_unknown();
        }
        const char *tname = container->data.template_type.template_name;
        const EngineType **args = container->data.template_type.template_args;
 ...
 * store NULL (the wrapper normalizes to engine_type_unknown() first), so NULL
 * unambiguously means "miss". */
static const EngineType *py_type_cache_lookup(const PyLSPContext *ctx, const void *id) {
    ...
    for (int probed = 0; probed < ctx->type_cache_cap; probed++) {
        const EnginePyTypeCacheEntry *e = &ctx->type_cache[idx];
        if (!e->node_id)
...
static bool py_type_cache_grow(PyLSPContext *ctx) {
    ...
    EnginePyTypeCacheEntry *new_entries = (EnginePyTypeCacheEntry *)engine_arena_alloc(
        ctx->arena, (size_t)new_cap * sizeof(EnginePyTypeCacheEntry));
    if (!new_entries)
        return false;
    memset(new_entries, 0, (size_t)new_cap * sizeof(EnginePyTypeCacheEntry));

    EnginePyTypeCacheEntry *old_entries = ctx->type_cache;
...
static void py_type_cache_insert(PyLSPContext *ctx, const void *id, const EngineType *result) {
    if (!id || !result)
        return;
    ...
    uint32_t idx = py_type_cache_hash(id) & mask;
    for (int probed = 0; probed < ctx->type_cache_cap; probed++) {
        EnginePyTypeCacheEntry *e = &ctx->type_cache[idx];
        if (!e->node_id) {
            e->node_id = id;
...
static const EngineType *py_eval_expr_type(PyLSPContext *ctx, TSNode node) {
    ...
    const EngineType *cached = py_type_cache_lookup(ctx, node.id);
    if (cached)
        return cached;
    ...
    ctx->eval_depth++;
    const EngineType *result = py_eval_expr_type_uncached(ctx, node);
    ctx->eval_depth--;
    if (!result)
        result = engine_type_unknown();
...
static void py_process_statement(PyLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "assignment") == 0) {
        ...
        const EngineType *rhs_type =
            ts_node_is_null(right) ? engine_type_unknown() : py_eval_expr_type(ctx, right);
        ...
        if (strcmp(lk, "pattern_list") == 0 || strcmp(lk, "tuple_pattern") == 0 ||
            ...
            const EngineType *const *rhs_elems = NULL;
            int rhs_count = 0;
            if (rhs_type) {
                if (rhs_type->kind == ENGINE_TYPE_TUPLE) {
                    rhs_elems = rhs_type->data.tuple.elems;
                    rhs_count = rhs_type->data.tuple.count;
                } else if (rhs_type->kind == ENGINE_TYPE_TEMPLATE &&
            ...
            // is heterogeneous).
            const EngineType *elem_type = py_iterable_element_type(ctx, rhs_type);
            ...
            for (uint32_t i = 0; i < lc; i++) {
                ...
                const EngineType *bind_type;
                if (is_rest) {
                    bind_type = elem_type ? engine_type_template(ctx->arena, "list", &elem_type, 1)
                                          : engine_type_unknown();
                } else if (rhs_elems && (int)i < rhs_count && rhs_elems[i]) {
                    bind_type = rhs_elems[i];
                } else {
                    bind_type = elem_type ? elem_type : engine_type_unknown();
                }
                py_scope_bind(ctx, nm, bind_type);
            }
            return;
    ...
    if (strcmp(k, "for_statement") == 0) {
        ...
        if (!ts_node_is_null(right)) {
            const EngineType *iter_type = py_eval_expr_type(ctx, right);
            ...
            if (engine_type_is_unknown(elem_type) && iter_type && iter_type->kind == ENGINE_TYPE_NAMED) {
                const EngineRegisteredFunc *anext =
                    py_lookup_attribute(ctx, iter_type->data.named.qualified_name, "__anext__");
                if (anext) {
                    const EngineType *ret = py_func_return_type_recv(
    ...
    if (strcmp(k, "with_statement") == 0) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            for (uint32_t j = 0; j < cn; j++) {
                ...
                if (!name)
                    continue;
                const EngineType *cm_type =
                    ts_node_is_null(value) ? engine_type_unknown() : py_eval_expr_type(ctx, value);
                const EngineType *bind_type = cm_type;
                if (cm_type && cm_type->kind == ENGINE_TYPE_NAMED) {
                    ...
                    const EngineRegisteredFunc *enter =
...
static void py_emit_call_for(PyLSPContext *ctx, TSNode call_node) {
    ...
    if (strcmp(fk, "identifier") == 0) {
        ...
        if (!ts_node_is_null(lambda_node)) {
            ...
            if (!ts_node_is_null(params) && !ts_node_is_null(args)) {
                ...
                for (uint32_t pi = 0; pi < pn && pi < an; pi++) {
                    ...
                    if (pname) {
                        const EngineType *arg_type = py_eval_expr_type(ctx, a);
            ...
            ctx->enclosing_func_qn = engine_arena_sprintf(ctx->arena, "%s.<lambda>",
        ...
        if (engine_scope_contains(ctx->current_scope, fname)) {
            const char *alias_target = engine_scope_lookup_callable(ctx->current_scope, fname);
            if (alias_target) {
            ...
            const EngineType *in_scope = engine_scope_lookup(ctx->current_scope, fname);
            if (!engine_type_is_unknown(in_scope) && in_scope->kind == ENGINE_TYPE_NAMED) {
                ...
                if (imported_alias) {
                    py_emit_resolved_call_reason(ctx, qn, "lsp_import_alias", 0.85f, fname,
                                                 call_node);
                } else if (qn && engine_registry_lookup_type(ctx->registry, qn)) {
    ...
    if (strcmp(fk, "attribute") == 0) {
        ...
        if (strcmp(ts_node_type(obj), "call") == 0) {
            ...
            if (!ts_node_is_null(super_fn) && strcmp(ts_node_type(super_fn), "identifier") == 0) {
                ...
                if (super_name && strcmp(super_name, "super") == 0 && ctx->enclosing_class_qn) {
                    const EngineRegisteredType *enclosing =
                    ...
                    if (enclosing && enclosing->embedded_types) {
                        for (int i = 0; enclosing->embedded_types[i]; i++) {
                            ...
                            if (strcmp(attr_name, "__init__") == 0) {
                                const EngineRegisteredFunc *fi = py_lookup_attribute(
                                    ctx, enclosing->embedded_types[i], attr_name);
                                const char *init_qn =
                                    fi ? fi->qualified_name
                                       : engine_arena_sprintf(ctx->arena, "%s.__init__",
            ...
            if (mod && ctx->module_qn) {
                ...
                if (last_dot && last_dot > ctx->module_qn) {
                    ...
                    if (!(strncmp(mod, ctx->module_qn, root_len) == 0 && mod[root_len] == '.')) {
                        char *qual_mod = (char *)engine_arena_alloc(ctx->arena, root_len + 1 +
                                                                                strlen(mod) + 1);
                        if (qual_mod) {
                            memcpy(qual_mod, ctx->module_qn, root_len);
                            qual_mod[root_len] = '.';
                            strcpy(qual_mod + root_len + 1, mod);
                            const EngineRegisteredFunc *qf =
                                engine_registry_lookup_symbol(ctx->registry, qual_mod, attr_name);
                            if (qf) {
        ...
        if (obj_type->kind == ENGINE_TYPE_UNION) {
            ...
            for (int i = 0; i < obj_type->data.union_type.count; i++) {
                ...
                if (!m)
                    continue;
                if (m->kind == ENGINE_TYPE_BUILTIN && m->data.builtin.name &&
                    strcmp(m->data.builtin.name, "None") == 0)
                    continue;
                if (m->kind != ENGINE_TYPE_NAMED)
                    continue;
                const EngineRegisteredFunc *f =
                    py_lookup_attribute(ctx, m->data.named.qualified_name, attr_name);
                if (f) {
...
static const EngineType *py_strip_none(PyLSPContext *ctx, const EngineType *t) {
    if (!t || t->kind != ENGINE_TYPE_UNION)
        return t;
    int n = t->data.union_type.count;
    int retained = 0;
    const EngineType **kept =
        (const EngineType **)engine_arena_alloc(ctx->arena, (size_t)(n + 1) * sizeof(const EngineType *));
    if (!kept)
        return t;
    for (int i = 0; i < n; i++) {
        const EngineType *m = t->data.union_type.members[i];
        if (m && m->kind == ENGINE_TYPE_BUILTIN && m->data.builtin.name &&
            strcmp(m->data.builtin.name, "None") == 0)
            continue;
        kept[retained++] = m;
    }
    ...
    if (retained == 1)
        return kept[0];
    return engine_type_union(ctx->arena, kept, retained);
}
...
static void py_bind_walrus_in(PyLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "named_expression") == 0 || strcmp(k, "assignment_expression") == 0) {
        ...
        if (!ts_node_is_null(left) && !ts_node_is_null(right) &&
            ...
            char *name = py_node_text(ctx, left);
            if (name) {
                const EngineType *t = py_eval_expr_type(ctx, right);
...
static void py_walk_if_statement(PyLSPContext *ctx, TSNode if_node) {
    ...
    char *neg_name = NULL;
    const EngineType *neg_type = NULL;
    int neg_none_kind = 0;
    ...
    if (!ts_node_is_null(body)) {
        ...
        if (none_kind == 1 && none_var) {
            const EngineType *current = engine_scope_lookup(ctx->current_scope, none_var);
            if (current && !engine_type_is_unknown(current)) {
                const EngineType *narrowed = py_strip_none(ctx, current);
    ...
    if (!ts_node_is_null(body) && py_block_terminates(body)) {
        ...
        if (neg_none_kind == -1 && neg_none_var) {
            ...
            const EngineType *current = engine_scope_lookup(ctx->current_scope, neg_none_var);
            if (current && !engine_type_is_unknown(current)) {
                const EngineType *narrowed = py_strip_none(ctx, current);
...
static void py_emit_dunder_call(PyLSPContext *ctx, const EngineType *recv, const char *dunder,
                                TSNode site) {
    if (!recv || recv->kind != ENGINE_TYPE_NAMED || !dunder)
        return;
    const EngineRegisteredFunc *f = py_lookup_attribute(ctx, recv->data.named.qualified_name, dunder);
    if (f && f->qualified_name) {
         ...
         * `call` node, so the extractor produced no EngineCall for it and the
         * resolved_call above would never be matched into a CALLS edge. Inject
         * a synthetic EngineCall keyed on (enclosing_func_qn, short dunder name)
        ...
        if (ctx->syn_calls && ctx->arena && ctx->enclosing_func_qn) {
            ...
            for (int i = 0; i < ctx->syn_calls->count; i++) {
                const EngineCall *existing = &ctx->syn_calls->items[i];
            ...
            call.callee_name = engine_arena_strdup(ctx->arena, dunder);
        ...
        const char *candidate =
            engine_arena_sprintf(ctx->arena, "%s.%s", recv->data.named.qualified_name, dunder);
            ...
            for (uint32_t i = 0; i < bcc; i++) {
                ...
                if (subject_name && !ts_node_is_null(actual_pattern) &&
                    ...
                    if (ts_node_named_child_count(actual_pattern) > 0) {
                        ...
                        if (cls_text) {
                            const EngineType *narrowed = py_resolve_annotation(ctx, cls_text);
                    ...
                    for (uint32_t j = 1; j < ts_node_named_child_count(actual_pattern); j++) {
                        ...
                        if (strcmp(ts_node_type(sub), "capture_pattern") == 0) {
                            if (ts_node_named_child_count(sub) > 0) {
                                ...
                                if (nm)
                                    py_scope_bind(ctx, nm, engine_type_unknown());
                            }
                        }
                    }
                }
                ...
                if (subject_name && !ts_node_is_null(actual_pattern) &&
                    ...
                    const EngineType *subj_t = engine_scope_lookup(ctx->current_scope, subject_name);
                    const EngineType *elem_t = py_iterable_element_type(ctx, subj_t);
                    ...
                    for (uint32_t j = 0; j < pn; j++) {
                        ...
                        if (strcmp(ek, "splat_pattern") == 0 ||
                            ...
                            if (!ts_node_is_null(id)) {
                                char *nm = py_node_text(ctx, id);
                                if (nm && elem_t) {
                                    const EngineType *lst =
                                        engine_type_template(ctx->arena, "list", &elem_t, 1);
...
/* Parse a type-annotation TEXT into a EngineType without requiring a
...
static const EngineType *py_parse_type_text(EngineArena *arena, const char *ann);
static const EngineType *py_parse_type_text_qn(EngineArena *arena, const char *ann,
                                            const char *module_qn);

/* Trim ASCII whitespace from both ends of an arena-allocated copy. */
static char *py_trim_ws(EngineArena *arena, const char *start, size_t len) {
...
static const char **py_split_subscript_args(EngineArena *arena, const char *s, int *out_n) {
    if (!s) {
        *out_n = 0;
        return NULL;
    }
    int cap = 8;
    const char **out =
        (const char **)engine_arena_alloc(arena, (size_t)(cap + 1) * sizeof(const char *));
    if (!out) {
        *out_n = 0;
        return NULL;
    }
    ...
    for (size_t i = 0; i <= len; i++) {
        ...
        if (c == '[' || c == '(' || c == '{')
            ...
            if (part && part[0]) {
                if (n >= cap) {
                    int new_cap = cap * 2;
                    const char **grown = (const char **)engine_arena_alloc(
...
static const EngineType *py_parse_type_text_qn(EngineArena *arena, const char *ann,
    ...
    if (len >= 2 && (ann[0] == '"' || ann[0] == '\'') && ann[len - 1] == ann[0]) {
        char *unquoted = (char *)engine_arena_alloc(arena, len - 1);
        if (unquoted) {
    ...
    if (lb && len > 0 && ann[len - 1] == ']') {
        ...
        char *base = (char *)engine_arena_alloc(arena, base_len + 1);
        if (base) {
            ...
            char *args_text = (char *)engine_arena_alloc(arena, inner_len + 1);
            if (args_text) {
                ...
                const EngineType **arg_types = NULL;
                if (arg_n > 0 && arg_strs) {
                    arg_types = (const EngineType **)engine_arena_alloc(
                        arena, (size_t)(arg_n + 1) * sizeof(const EngineType *));
                    if (arg_types) {
                ...
                if (strcmp(btrim, "Optional") == 0 || strcmp(btrim, "typing.Optional") == 0) {
                    if (arg_types && arg_n >= 1 && arg_types[0]) {
                        return engine_type_optional(arena, arg_types[0]);
                    }
                }
                if (strcmp(btrim, "Union") == 0 || strcmp(btrim, "typing.Union") == 0) {
                    if (arg_types && arg_n > 0) {
                        return engine_type_union(arena, arg_types, arg_n);
                    }
                }
                ...
                if (arg_types && arg_n > 0) {
                    return engine_type_template(arena, btrim, arg_types, arg_n);
                }
...

    {
        ...
        if (out) {
            ...
            if (seen_pipe && onum >= 2) {
                const EngineType **members = (const EngineType **)engine_arena_alloc(
                    arena, (size_t)(onum + 1) * sizeof(const EngineType *));
                if (members) {
                    for (int j = 0; j < onum; j++) {
                        members[j] = py_parse_type_text_qn(arena, out[j], module_qn);
                    }
                    return engine_type_union(arena, members, onum);
                }
            }
        }
    }
    ...
    for (int i = 0; builtins[i]; i++) {
        if (strcmp(ann, builtins[i]) == 0) {
            return engine_type_builtin(arena, ann);
        }
    }
    ...
    if (module_qn && !strchr(ann, '.')) {
        ...
        if (!is_typing) {
            const char *qn = engine_arena_sprintf(arena, "%s.%s", module_qn, ann);
            return engine_type_named(arena, qn);
        }
    }
    return engine_type_named(arena, ann);
}
...
static const EngineType *py_signature_param_type_adapter(EngineArena *arena, const char *text,
                                                      void *parser_ctx) {
...
/* Resolve a type-annotation text into a EngineType. Tries: scope lookup
...
static const EngineType *py_resolve_annotation(PyLSPContext *ctx, const char *ann) {
    ...
    size_t len = strlen(ann);
    if (len >= 2 && (ann[0] == '"' || ann[0] == '\'') && ann[len - 1] == ann[0]) {
        char *unquoted = (char *)engine_arena_alloc(ctx->arena, len - 1);
        if (unquoted) {
            memcpy(unquoted, ann + 1, len - 2);
            unquoted[len - 2] = '\0';
    ...
    if (lb && len > 0 && ann[len - 1] == ']') {
        size_t base_len = (size_t)(lb - ann);
        char *base = (char *)engine_arena_alloc(ctx->arena, base_len + 1);
        if (base) {
            memcpy(base, ann, base_len);
            base[base_len] = '\0';
            ...
            size_t inner_start = (size_t)(lb - ann) + 1;
            size_t inner_len = len - inner_start - 1;
            char *args_text = (char *)engine_arena_alloc(ctx->arena, inner_len + 1);
            if (args_text) {
                memcpy(args_text, ann + inner_start, inner_len);
                args_text[inner_len] = '\0';
                int arg_n = 0;
                const char **arg_strs = py_split_subscript_args(ctx->arena, args_text, &arg_n);
                // Recursively resolve each arg to a EngineType.
                const EngineType **arg_types = NULL;
                if (arg_n > 0 && arg_strs) {
                    arg_types = (const EngineType **)engine_arena_alloc(
                        ctx->arena, (size_t)(arg_n + 1) * sizeof(const EngineType *));
                    if (arg_types) {
                        for (int i = 0; i < arg_n; i++) {
                ...
                if (strcmp(btrim, "Optional") == 0 || strcmp(btrim, "typing.Optional") == 0) {
                    if (arg_types && arg_n >= 1 && arg_types[0]) {
                        return engine_type_optional(ctx->arena, arg_types[0]);
                    }
                }
                // Union[A, B, ...] -> UNION
                if (strcmp(btrim, "Union") == 0 || strcmp(btrim, "typing.Union") == 0) {
                    if (arg_types && arg_n > 0) {
                        return engine_type_union(ctx->arena, arg_types, arg_n);
                    }
                }
                ...
                if (arg_types && arg_n > 0) {
                    return engine_type_template(ctx->arena, btrim, arg_types, arg_n);
                }
    ...
    // X | Y top-level union (PEP 604).
    {
        ...
        for (size_t i = 0; i < len; i++) {
            ...
            if (c == '[' || c == '(' || c == '{')
                ...
                int cap = 4;
                const char **out = (const char **)engine_arena_alloc(
                ...
                if (onum >= 2) {
                    const EngineType **members = (const EngineType **)engine_arena_alloc(
                        ctx->arena, (size_t)(onum + 1) * sizeof(const EngineType *));
                    if (!members)
                        break;
                    for (int j = 0; j < onum; j++) {
                        members[j] = py_resolve_annotation(ctx, out[j]);
                    }
                    return engine_type_union(ctx->arena, members, onum);
                }
                break;
            }
        }
    ...
    const EngineType *t = engine_scope_lookup(ctx->current_scope, ann);
    if (!engine_type_is_unknown(t))
        return t;
    if (ctx->module_qn) {
        const char *qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, ann);
        const EngineRegisteredType *rt = engine_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return engine_type_named(ctx->arena, qn);
    }
    ...
    for (int i = 0; builtins[i]; i++) {
        if (strcmp(ann, builtins[i]) == 0) {
            return engine_type_builtin(ctx->arena, ann);
        }
    }
    return engine_type_named(ctx->arena, ann);
}
...
static void py_bind_parameters(PyLSPContext *ctx, TSNode params) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (is_splat && t && !engine_type_is_unknown(t)) {
            t = engine_type_template(ctx->arena, "tuple", &t, 1);
        }
        if (is_kwargs && t && !engine_type_is_unknown(t)) {
            const EngineType *str_t = engine_type_builtin(ctx->arena, "str");
            const EngineType *args[3] = {str_t, t, NULL};
            t = engine_type_template(ctx->arena, "dict", args, 2);
        }
        py_scope_bind(ctx, name, t);
    }
}
...
static void py_process_function(PyLSPContext *ctx, TSNode func_node, const char *container_qn) {
    ...
    ctx->enclosing_func_qn = engine_arena_sprintf(ctx->arena, "%s.%s", base_qn, fname);
...
static void py_process_class(PyLSPContext *ctx, TSNode class_node) {
    ...
    ctx->enclosing_class_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
            ...
            if (strcmp(ck, "expression_statement") == 0 && ts_node_named_child_count(c) > 0) {
                ...
                if (strcmp(ik, "assignment") == 0) {
                    ...
                    if (!ts_node_is_null(left) && !ts_node_is_null(ann) &&
                        ...
                        if (fname && atext) {
                            const EngineType *ft = py_resolve_annotation(ctx, atext);
...
static void py_bind_external_module_classes(PyLSPContext *ctx, TSNode root) {
    ...
    for (int i = 0; i < ctx->registry->type_count; i++) {
        const EngineRegisteredType *type = &ctx->registry->types[i];
        ...
        py_scope_bind(ctx, name, engine_type_named(ctx->arena, qn));
    }
}
...
static void py_bind_module_class(PyLSPContext *ctx, TSNode class_node) {
    ...
    if (!qn) {
        py_disable_callable_value_proof(ctx);
        py_scope_bind(ctx, name, engine_type_unknown());
        return;
    }
    const EngineRegisteredType *type = engine_registry_lookup_type(ctx->registry, qn);
    py_scope_bind(ctx, name,
                  type ? engine_type_named(ctx->arena, type->qualified_name) : engine_type_unknown());
}
...
static void py_bind_module_function(PyLSPContext *ctx, TSNode func_node) {
    ...
    const EngineRegisteredFunc *func = engine_registry_lookup_func(ctx->registry, qn);
    if (py_func_is_exact_callable_value(func)) {
        py_scope_bind_callable(ctx, name, engine_type_unknown(), func->qualified_name);
    } else {
        py_scope_bind(ctx, name, engine_type_unknown());
    }
}
...
static void py_invalidate_registry_module_functions(PyLSPContext *ctx,
    ...
    size_t prefix_len = strlen(ctx->module_qn);
    for (int i = 0; i < registry->func_count; i++) {
        const EngineRegisteredFunc *func = &registry->funcs[i];
        ...
        if (!suffix[0] || strchr(suffix, '.'))
            continue;
        py_scope_bind(ctx, func->short_name ? func->short_name : suffix, engine_type_unknown());
    }
...
static void py_invalidate_module_bindings_for_wildcard(PyLSPContext *ctx) {
    if (!ctx || !ctx->current_scope)
        return;
    for (EngineScopeChunk *chunk = ctx->current_scope->chunks; chunk; chunk = chunk->next) {
...
static void py_invalidate_import_bindings(PyLSPContext *ctx, TSNode stmt) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (local) {
            py_scope_bind(ctx, local, engine_type_unknown());
...
void py_lsp_process_file(PyLSPContext *ctx, TSNode root) {
    ...
    if (ctx->import_count > 0) {
        consumed_imports =
            (unsigned char *)engine_arena_alloc(ctx->arena, (size_t)ctx->import_count);
        if (consumed_imports)
    ...
    const char *prev_func = ctx->enclosing_func_qn;
    ctx->enclosing_func_qn = engine_arena_sprintf(ctx->arena, "%s.__module__", ctx->module_qn);
...
static bool py_register_def(EngineArena *arena, EngineTypeRegistry *reg, EngineDefinition *d,
                            const char *module_qn) {
                ...
                for (size_t k = lo; k <= hi; k++) {
                    ...
                    if (c == '[' || c == '(')
                        ...
                        if (eq >= name_end && name_end > s) {
                            ...
                            char *bname = (char *)engine_arena_alloc(arena, blen + 1);
                            if (bname) {
                                ...
                                if (n < cap) {
                                    embedded[n++] =
                                        strchr(bname, '.')
                                            ? bname
                                            : engine_arena_sprintf(arena, "%s.%s", module_qn, bname);
                                }
                            }
                        }
                        start = k + 1;
    ...
    if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
        ...
        // is the engine_node_text of the whole annotation node, which is the
        ...
        if (prefer_full) {
            ...
            while (d->return_types[count])
                count++;
            ret_types = (const EngineType **)engine_arena_alloc(arena, (size_t)(count + 1) *
        ...
        if (strcmp(d->label, "Method") == 0) {
            ...
            if (last_dot && last_dot != qn) {
                ...
                rf.receiver_type = engine_arena_strndup(arena, qn, prefix_len);
            }
        }
...
/* ── engine_run_py_lsp: single-file entry point ──────────────────── */
...
void engine_run_py_lsp(EngineArena *arena, EngineFileResult *result, const char *source, int source_len,
     ...
     * graph node. We run inside engine_extract_file, before the pipeline mints
    ...
    engine_python_stdlib_register(&reg, arena);
...
static const char **py_split_pipe(EngineArena *arena, const char *text) {
    if (!text || !text[0])
        return NULL;
    int count = 1;
    ...
    for (const char *p = text;; p++) {
        if (*p == '|' || *p == '\0') {
            ...
            char *s = (char *)engine_arena_alloc(arena, n + 1);
            if (!s)
                return NULL;
            memcpy(s, start, n);
            s[n] = '\0';
...
/* Build a registry from EngineLSPDef[] supplied by the caller — covers both
 * the source file's own defs and cross-file referenced defs. */
static void py_register_lsp_defs(EngineArena *arena, EngineArena *idx_arena, EngineTypeRegistry *reg,
     ...
     * registry arena. */
    if (idx_arena) {
        engine_registry_finalize_into(reg, idx_arena);
    }
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            if (ret_strs) {
                ...
                if (n > 0) {
                    ...
                    for (int j = 0; j < n; j++) {
                        ret_types[j] = engine_type_named(arena, ret_strs[j]);
                    }
                    ret_types[n] = NULL;
                }
            }
            ...
            if (strcmp(d->label, "Method") == 0 && d->receiver_type && d->receiver_type[0]) {
                ...
                if (!engine_registry_lookup_type(reg, rf.receiver_type)) {
...
void engine_run_py_lsp_cross(EngineArena *arena, const char *source, int source_len,
    ...
    EngineArena idx_arena;
    engine_arena_init(&idx_arena);
    ...
    py_lsp_process_file(&ctx, root);
    engine_arena_destroy(&idx_arena);
...
void engine_run_py_lsp_cross_with_registry(EngineArena *arena, const char *source, int source_len,
                                        const char *module_qn, EngineTypeRegistry *reg,
...
void engine_batch_py_lsp_cross(EngineArena *arena, EngineBatchPyLSPFile *files, int file_count,
    ...
    for (int f = 0; f < file_count; f++) {
        EngineBatchPyLSPFile *file = &files[f];
        ...
        engine_run_py_lsp_cross(&file_arena, file->source, file->source_len, file->module_qn,
        ...
        if (file_out.count > 0) {
            ...
            out[f].items = (EngineResolvedCall *)engine_arena_alloc(arena, (size_t)file_out.count *
                                                                         sizeof(EngineResolvedCall));
            if (out[f].items) {
                for (int j = 0; j < file_out.count; j++) {
                    ...
                    dst->caller_qn =
                        src->caller_qn ? engine_arena_strdup(arena, src->caller_qn) : NULL;
                    dst->callee_qn =
                        src->callee_qn ? engine_arena_strdup(arena, src->callee_qn) : NULL;
...
#ifndef ENGINE_PYTHON_STDLIB_GENERATED
