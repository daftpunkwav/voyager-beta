...
static void reg_method(EngineTypeRegistry* reg, EngineArena* arena,
    const char* recv_qn, const char* method_name, const EngineType* ret_type) {
    ...
    char* qn = (char*)engine_arena_alloc(arena, rlen + 1 + mlen + 1);
    if (!qn) return;
    ...
    rf.signature = engine_type_func(arena, NULL, NULL,
        ret_type ? (const EngineType*[]){ret_type, NULL} : NULL);
    engine_registry_add_func(reg, rf);
}

// Helper: register a type
static void reg_type(EngineTypeRegistry* reg, const char* qn, const char* short_name) {
...
static void reg_type_with_fields(EngineTypeRegistry* reg, EngineArena* arena,
    const char* qn, const char* short_name,
    const char** fnames, const EngineType** ftypes) {
    EngineRegisteredType rt;
    memset(&rt, 0, sizeof(rt));
    rt.qualified_name = qn;
    rt.short_name = short_name;
...
static void reg_func(EngineTypeRegistry* reg, EngineArena* arena,
    const char* qn, const char* short_name, const EngineType* ret_type) {
    EngineRegisteredFunc rf;
    memset(&rf, 0, sizeof(rf));
    rf.min_params = -1;
    rf.qualified_name = qn;
...
void engine_cpp_stdlib_register(EngineTypeRegistry* reg, EngineArena* arena) {
    const EngineType* t_void = engine_type_builtin(arena, "void");
    const EngineType* t_void_ptr = engine_type_pointer(arena, t_void);
    const EngineType* t_bool = engine_type_builtin(arena, "bool");
    const EngineType* t_int = engine_type_builtin(arena, "int");
    const EngineType* t_size_t = engine_type_builtin(arena, "size_t");
    const EngineType* t_long = engine_type_builtin(arena, "long");
    const EngineType* t_char = engine_type_builtin(arena, "char");
    const EngineType* t_char_ptr = engine_type_pointer(arena, engine_type_builtin(arena, "const char"));
    ...
    const EngineType* t_string = engine_type_named(arena, string_qn);
    const EngineType* t_string_ref = engine_type_reference(arena, t_string);
    ...
    reg_method(reg, arena, string_qn, "front", engine_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "back", engine_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "at", engine_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "operator[]", engine_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "begin", engine_type_named(arena, "std.string.iterator"));
    reg_method(reg, arena, string_qn, "end", engine_type_named(arena, "std.string.iterator"));
    ...
    reg_method(reg, arena, sv_qn, "substr", engine_type_named(arena, sv_qn));
    ...
    const EngineType* t_T = engine_type_type_param(arena, "T");
    const EngineType* t_T_ref = engine_type_reference(arena, t_T);
    const EngineType* t_T_ptr = engine_type_pointer(arena, t_T);
    ...
    reg_method(reg, arena, vec_qn, "begin", engine_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "end", engine_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "erase", engine_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "insert", engine_type_named(arena, "std.vector.iterator"));
    ...
    const EngineType* t_V = engine_type_type_param(arena, "V");
    const EngineType* t_V_ref = engine_type_reference(arena, t_V);

    reg_method(reg, arena, map_qn, "operator[]", t_V_ref);
    reg_method(reg, arena, map_qn, "at", t_V_ref);
    reg_method(reg, arena, map_qn, "find", engine_type_named(arena, "std.map.iterator"));
    reg_method(reg, arena, map_qn, "insert", engine_type_named(arena, "std.pair"));
    reg_method(reg, arena, map_qn, "emplace", engine_type_named(arena, "std.pair"));
    ...
    reg_method(reg, arena, map_qn, "begin", engine_type_named(arena, "std.map.iterator"));
    reg_method(reg, arena, map_qn, "end", engine_type_named(arena, "std.map.iterator"));
    ...
    reg_method(reg, arena, umap_qn, "find", engine_type_named(arena, "std.unordered_map.iterator"));
    reg_method(reg, arena, umap_qn, "insert", engine_type_named(arena, "std.pair"));
    reg_method(reg, arena, umap_qn, "emplace", engine_type_named(arena, "std.pair"));
    ...
    reg_method(reg, arena, umap_qn, "begin", engine_type_named(arena, "std.unordered_map.iterator"));
    reg_method(reg, arena, umap_qn, "end", engine_type_named(arena, "std.unordered_map.iterator"));
    ...
    reg_method(reg, arena, set_qn, "insert", engine_type_named(arena, "std.pair"));
    reg_method(reg, arena, set_qn, "find", engine_type_named(arena, "std.set.iterator"));
    ...
    reg_method(reg, arena, set_qn, "begin", engine_type_named(arena, "std.set.iterator"));
    reg_method(reg, arena, set_qn, "end", engine_type_named(arena, "std.set.iterator"));

    const char* uset_qn = "std.unordered_set";
    reg_type(reg, uset_qn, "unordered_set");
    reg_method(reg, arena, uset_qn, "insert", engine_type_named(arena, "std.pair"));
    reg_method(reg, arena, uset_qn, "find", engine_type_named(arena, "std.unordered_set.iterator"));
    ...
    reg_method(reg, arena, wptr_qn, "lock", engine_type_named(arena, sptr_qn));
    ...
    reg_func(reg, arena, "std.make_unique", "make_unique",
        engine_type_named(arena, uptr_qn));
    reg_func(reg, arena, "std.make_shared", "make_shared",
        engine_type_named(arena, sptr_qn));
    ...
    const char* pair_qn = "std.pair";
    {
        ...
        const EngineType** pair_field_types = (const EngineType**)engine_arena_alloc(arena,
            3 * sizeof(const EngineType*));
        pair_field_types[0] = engine_type_type_param(arena, "T1");
        pair_field_types[1] = engine_type_type_param(arena, "T2");
    ...
    reg_func(reg, arena, "std.get", "get", engine_type_unknown());
    ...
    reg_method(reg, arena, "std.function", "operator()", engine_type_unknown());
    ...
    const EngineType* t_ostream_ref = engine_type_reference(arena, engine_type_named(arena, os_qn));
    ...
    const EngineType* t_istream_ref = engine_type_reference(arena, engine_type_named(arena, is_qn));
    ...
    reg_func(reg, arena, "std.cout", "cout", engine_type_named(arena, os_qn));
    reg_func(reg, arena, "std.cin", "cin", engine_type_named(arena, is_qn));
    reg_func(reg, arena, "std.cerr", "cerr", engine_type_named(arena, os_qn));
    reg_func(reg, arena, "std.clog", "clog", engine_type_named(arena, os_qn));
    ...
    const EngineType* t_path = engine_type_named(arena, fspath_qn);
    ...
    reg_method(reg, arena, fspath_qn, "operator/=", engine_type_reference(arena, t_path));
    ...
    reg_method(reg, arena, thread_qn, "get_id", engine_type_named(arena, "std.thread.id"));
    ...
    reg_func(reg, arena, "std.chrono.system_clock.now", "now",
        engine_type_named(arena, "std.chrono.time_point"));
    ...
    reg_func(reg, arena, "std.find", "find", engine_type_unknown());
    reg_func(reg, arena, "std.find_if", "find_if", engine_type_unknown());
    reg_func(reg, arena, "std.for_each", "for_each", engine_type_unknown());
    reg_func(reg, arena, "std.transform", "transform", engine_type_unknown());
    reg_func(reg, arena, "std.copy", "copy", engine_type_unknown());
    reg_func(reg, arena, "std.move", "move", engine_type_unknown());
    reg_func(reg, arena, "std.swap", "swap", t_void);
    reg_func(reg, arena, "std.min", "min", engine_type_unknown());
    reg_func(reg, arena, "std.max", "max", engine_type_unknown());
    reg_func(reg, arena, "std.accumulate", "accumulate", engine_type_unknown());
    reg_func(reg, arena, "std.count", "count", t_size_t);
    reg_func(reg, arena, "std.count_if", "count_if", t_size_t);
    reg_func(reg, arena, "std.remove", "remove", engine_type_unknown());
    reg_func(reg, arena, "std.remove_if", "remove_if", engine_type_unknown());
    reg_func(reg, arena, "std.reverse", "reverse", t_void);
    reg_func(reg, arena, "std.unique", "unique", engine_type_unknown());
    reg_func(reg, arena, "std.lower_bound", "lower_bound", engine_type_unknown());
    reg_func(reg, arena, "std.upper_bound", "upper_bound", engine_type_unknown());
    ...
    reg_func(reg, arena, "std.stof", "stof", engine_type_builtin(arena, "float"));
    reg_func(reg, arena, "std.stod", "stod", engine_type_builtin(arena, "double"));
    ...
    reg_method(reg, arena, "std.any", "type", engine_type_unknown()); // type_info
    reg_method(reg, arena, "std.any", "reset", t_void);
    reg_func(reg, arena, "std.any_cast", "any_cast", t_T);
    reg_func(reg, arena, "std.make_any", "make_any", engine_type_named(arena, "std.any"));
    ...
    reg_method(reg, arena, "std.smatch", "prefix", engine_type_unknown());
    reg_method(reg, arena, "std.smatch", "suffix", engine_type_unknown());
    ...
    reg_func(reg, arena, "std.make_pair", "make_pair",
        engine_type_named(arena, "std.pair"));
    ...
    reg_func(reg, arena, "std.partial_sum", "partial_sum", engine_type_unknown());
    reg_func(reg, arena, "std.adjacent_difference", "adjacent_difference", engine_type_unknown());
    ...
    reg_func(reg, arena, "std.distance", "distance", engine_type_builtin(arena, "ptrdiff_t"));
    reg_func(reg, arena, "std.next", "next", engine_type_unknown()); // iterator
    reg_func(reg, arena, "std.prev", "prev", engine_type_unknown());
    reg_func(reg, arena, "std.begin", "begin", engine_type_unknown());
    reg_func(reg, arena, "std.end", "end", engine_type_unknown());
    reg_func(reg, arena, "std.rbegin", "rbegin", engine_type_unknown());
    reg_func(reg, arena, "std.rend", "rend", engine_type_unknown());
    ...
    reg_func(reg, arena, "std.min_element", "min_element", engine_type_unknown());
    reg_func(reg, arena, "std.max_element", "max_element", engine_type_unknown());
    ...
    reg_func(reg, arena, "std.partition", "partition", engine_type_unknown());
    reg_func(reg, arena, "std.stable_partition", "stable_partition", engine_type_unknown());
    ...
    reg_func(reg, arena, "std.merge", "merge", engine_type_unknown());
    reg_func(reg, arena, "std.includes", "includes", t_bool);
    reg_func(reg, arena, "std.set_union", "set_union", engine_type_unknown());
    reg_func(reg, arena, "std.set_intersection", "set_intersection", engine_type_unknown());
    reg_func(reg, arena, "std.set_difference", "set_difference", engine_type_unknown());
    reg_func(reg, arena, "std.generate", "generate", t_void);
    reg_func(reg, arena, "std.generate_n", "generate_n", engine_type_unknown());
    ...
    reg_method(reg, arena, "std.bitset", "set", engine_type_named(arena, "std.bitset"));
    reg_method(reg, arena, "std.bitset", "reset", engine_type_named(arena, "std.bitset"));
    reg_method(reg, arena, "std.bitset", "flip", engine_type_named(arena, "std.bitset"));
    ...
    reg_method(reg, arena, "std.bitset", "to_ulong", engine_type_builtin(arena, "unsigned long"));
    ...
    reg_func(reg, arena, "std.filesystem.file_size", "file_size", engine_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.remove_all", "remove_all", engine_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.current_path", "current_path",
        engine_type_named(arena, fspath_qn));
    ...
    reg_method(reg, arena, direntry_qn, "path", engine_type_named(arena, fspath_qn));
    ...
    reg_method(reg, arena, direntry_qn, "file_size", engine_type_builtin(arena, "uintmax_t"));

    // =========================================================================
    // std.chrono — additional
    // =========================================================================
    reg_type(reg, "std.chrono.time_point", "time_point");
    reg_method(reg, arena, "std.chrono.time_point", "time_since_epoch", engine_type_unknown());
    reg_method(reg, arena, "std.chrono.system_clock", "now",
        engine_type_named(arena, "std.chrono.time_point"));
    reg_method(reg, arena, "std.chrono.steady_clock", "now",
        engine_type_named(arena, "std.chrono.time_point"));
    reg_func(reg, arena, "std.chrono.duration_cast", "duration_cast", engine_type_unknown());
    ...
    reg_method(reg, arena, bfspath_qn, "parent_path", engine_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "filename", engine_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "extension", engine_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "stem", engine_type_named(arena, bfspath_qn));
    ...
    reg_method(reg, arena, absv_qn, "data", engine_type_pointer(arena, engine_type_builtin(arena, "char")));
    ...
    reg_method(reg, arena, absv_qn, "substr", engine_type_named(arena, absv_qn));

    const char* abstat_qn = "absl.Status";
    reg_type(reg, abstat_qn, "Status");
    reg_method(reg, arena, abstat_qn, "ok", t_bool);
    reg_method(reg, arena, abstat_qn, "code", engine_type_unknown());
    reg_method(reg, arena, abstat_qn, "message", engine_type_named(arena, absv_qn));
    reg_method(reg, arena, abstat_qn, "ToString", t_string);
    reg_func(reg, arena, "absl.OkStatus", "OkStatus", engine_type_named(arena, abstat_qn));

    const char* absor_qn = "absl.StatusOr";
    reg_type(reg, absor_qn, "StatusOr");
    reg_method(reg, arena, absor_qn, "ok", t_bool);
    reg_method(reg, arena, absor_qn, "status", engine_type_named(arena, abstat_qn));
    ...
    reg_method(reg, arena, abfhm_qn, "find", engine_type_unknown());
    ...
    reg_method(reg, arena, abfhm_qn, "insert", engine_type_unknown());
    ...
    reg_method(reg, arena, abfhs_qn, "insert", engine_type_unknown());
    ...
    reg_method(reg, arena, abspan_qn, "subspan", engine_type_named(arena, abspan_qn));
    ...
    reg_func(reg, arena, "absl.StrSplit", "StrSplit",
        engine_type_named(arena, "std.vector"));
    ...
    reg_method(reg, arena, grpc_status_qn, "error_code", engine_type_unknown());
    reg_method(reg, arena, grpc_status_qn, "error_message", t_string);
    reg_func(reg, arena, "grpc.Status.OK", "OK", engine_type_named(arena, grpc_status_qn));

    const char* grpc_sb_qn = "grpc.ServerBuilder";
    reg_type(reg, grpc_sb_qn, "ServerBuilder");
    reg_method(reg, arena, grpc_sb_qn, "AddListeningPort", engine_type_named(arena, grpc_sb_qn));
    reg_method(reg, arena, grpc_sb_qn, "RegisterService", engine_type_named(arena, grpc_sb_qn));
    reg_method(reg, arena, grpc_sb_qn, "BuildAndStart", engine_type_unknown());

    const char* grpc_chan_qn = "grpc.Channel";
    reg_type(reg, grpc_chan_qn, "Channel");
    reg_func(reg, arena, "grpc.CreateChannel", "CreateChannel",
        engine_type_named(arena, grpc_chan_qn));
    ...
    reg_func(reg, arena, "spdlog.get", "get",
        engine_type_named(arena, spdlog_qn));
    ...
    reg_method(reg, arena, qobj_qn, "parent", engine_type_pointer(arena, engine_type_named(arena, qobj_qn)));
    reg_method(reg, arena, qobj_qn, "children", engine_type_unknown());
    reg_method(reg, arena, qobj_qn, "objectName", engine_type_named(arena, "QString"));
    ...
    reg_method(reg, arena, qstr_qn, "toUtf8", engine_type_unknown());
    reg_method(reg, arena, qstr_qn, "toLatin1", engine_type_unknown());
    ...
    reg_method(reg, arena, qstr_qn, "trimmed", engine_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "simplified", engine_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toLower", engine_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toUpper", engine_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "arg", engine_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "split", engine_type_unknown());
    reg_method(reg, arena, qstr_qn, "replace", engine_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "mid", engine_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "left", engine_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "right", engine_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toInt", t_int);
    reg_method(reg, arena, qstr_qn, "toDouble", engine_type_builtin(arena, "double"));
    reg_func(reg, arena, "QString.number", "number", engine_type_named(arena, qstr_qn));
    reg_func(reg, arena, "QString.fromStdString", "fromStdString", engine_type_named(arena, qstr_qn));
    reg_func(reg, arena, "QString.fromUtf8", "fromUtf8", engine_type_named(arena, qstr_qn));
