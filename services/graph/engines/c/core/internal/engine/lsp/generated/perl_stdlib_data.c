 ...
 *
 * Return types are left UNKNOWN (engine_type_unknown) for v1: real signature
...
#define REG_FUNC(module_dot_, name_, ret_type_)                                                 \
    do {                                                                                        \
        memset(&rf, 0, sizeof(rf));                                                             \
        rf.min_params = -1;                                                                     \
        rf.qualified_name = engine_arena_sprintf(arena, "%s.%s", (module_dot_), (name_));          \
        rf.short_name = (name_);                                                                \
        {                                                                                       \
            const EngineType **rets = (const EngineType **)engine_arena_alloc(arena, 2 * sizeof(*rets)); \
            rets[0] = (ret_type_);                                                              \
...
void engine_perl_stdlib_register(EngineTypeRegistry *reg, EngineArena *arena) {
    ...
    REG_BUILTIN("sprintf", engine_type_builtin(arena, "string"));
    REG_BUILTIN("open", MIXED);
    REG_BUILTIN("close", MIXED);
    REG_BUILTIN("push", engine_type_builtin(arena, "int"));
    REG_BUILTIN("pop", MIXED);
    REG_BUILTIN("shift", MIXED);
    REG_BUILTIN("unshift", engine_type_builtin(arena, "int"));
    REG_BUILTIN("map", MIXED);
    REG_BUILTIN("grep", MIXED);
    REG_BUILTIN("sort", MIXED);
    REG_BUILTIN("join", engine_type_builtin(arena, "string"));
    REG_BUILTIN("split", MIXED);
    REG_BUILTIN("length", engine_type_builtin(arena, "int"));
    REG_BUILTIN("substr", engine_type_builtin(arena, "string"));
    REG_BUILTIN("chomp", MIXED);
    REG_BUILTIN("chop", MIXED);
    REG_BUILTIN("die", MIXED);
    REG_BUILTIN("warn", MIXED);
    REG_BUILTIN("ref", engine_type_builtin(arena, "string"));
    REG_BUILTIN("bless", MIXED);
    REG_BUILTIN("defined", engine_type_builtin(arena, "bool"));
    REG_BUILTIN("exists", engine_type_builtin(arena, "bool"));
    ...
    REG_FUNC("Scalar.Util", "reftype", engine_type_builtin(arena, "string"));
    ...
    REG_FUNC("POSIX", "strftime", engine_type_builtin(arena, "string"));
    REG_FUNC("POSIX", "INT_MAX", engine_type_builtin(arena, "int"));

    /* ── Storable ───────────────────────────────────────────────────
     * Source: RESEARCH.md L367. Module QN "Storable". */
    REG_FUNC("Storable", "dclone", MIXED);
    REG_FUNC("Storable", "freeze", engine_type_builtin(arena, "string"));
    ...
    REG_FUNC("Data.Dumper", "Dumper", engine_type_builtin(arena, "string"));
}
