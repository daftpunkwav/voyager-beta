#ifndef ENGINE_LSP_TS_LSP_H
#define ENGINE_LSP_TS_LSP_H

#include "type_rep.h"
#include "scope.h"
#include "type_registry.h"
#include "../engine.h"
#include "go_lsp.h" // for EngineLSPDef, EngineResolvedCallArray (shared cross-language)

// TSLSPContext holds state for TypeScript / JavaScript / JSX / TSX expression type
// evaluation within a single file. One context per engine_run_ts_lsp invocation.
//
...
typedef struct {
    ...
    const EngineType **type_param_constraints;
    ...
    bool debug;  // ENGINE_LSP_DEBUG env
...
// Evaluate the type of a TS/JS expression node. Returns engine_type_unknown() on miss.
const EngineType *ts_eval_expr_type(TSLSPContext *ctx, TSNode node);

// Parse a TS/JS type-position AST node into a EngineType.
...
// Cross-file LSP. Caller passes in cross-file definitions (EngineLSPDef list) and the
...
typedef struct {
    ...
    int import_count;
} EngineBatchTSLSPFile;

// Process multiple TS/JS files' cross-file LSP in one CGo call.
// out must point to file_count pre-zeroed EngineResolvedCallArray structs.
