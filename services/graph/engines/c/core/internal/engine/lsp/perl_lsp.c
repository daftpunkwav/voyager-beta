 ...
 *          method/function call expressions into EngineResolvedCall edges.
 *
 ...
 * at internal/engine/vendored/grammars/perl/parser.c (ts_symbol_names and
 ...
 *
 * QN scheme (verified against helpers.c engine_enclosing_func_qn): Perl has no
 ...
 * per-package EngineRegisteredType (keyed by the package name) carries
...
#define ENGINE_LSP_PERL_MAX_WALK_DEPTH 512
...
static const EngineType *perl_eval_function_call_type(PerlLSPContext *ctx, TSNode node);
static const EngineType *perl_eval_method_call_type(PerlLSPContext *ctx, TSNode node);
static const EngineType *perl_eval_new_type(PerlLSPContext *ctx, TSNode node);
...
static char *perl_pkg_to_dot(EngineArena *a, const char *pkg) {
    if (!pkg)
        return NULL;
    size_t n = strlen(pkg);
...
static char *perl_unquote(EngineArena *a, const char *s) {
    if (!s || !s[0])
        return NULL;
    size_t n = strlen(s);
    if ((s[0] == '\'' || s[0] == '"') && n >= 2 && s[n - 1] == s[0]) {
        return engine_arena_strndup(a, s + 1, n - 2);
    }
    return NULL;
}
...
void perl_lsp_init(PerlLSPContext *ctx, EngineArena *arena, const char *source, int source_len,
...
void perl_lsp_add_use(PerlLSPContext *ctx, const char *local_name, const char *target_qn) {
    ...
    if (ctx->use_count >= ctx->use_cap) {
        ...
        const char **ln =
            (const char **)engine_arena_alloc(ctx->arena, (size_t)newcap * sizeof(char *));
        const char **tq =
            (const char **)engine_arena_alloc(ctx->arena, (size_t)newcap * sizeof(char *));
        if (!ln || !tq)
            return;
...
static void perl_add_isa(PerlLSPContext *ctx, const char *pkg, const char *parent) {
    ...
    ctx->isa_pkg_qns[ctx->isa_count] = engine_arena_strdup(ctx->arena, pkg);
    ctx->isa_parent_qns[ctx->isa_count] = engine_arena_strdup(ctx->arena, parent);
    ctx->isa_count++;
}
 ...
 * EngineRegisteredFunc or NULL. Bounded by ENGINE_LSP_MAX_LOOKUP_DEPTH * 2 visited.
 *
...
const EngineRegisteredFunc *perl_lookup_method(PerlLSPContext *ctx, const char *package_qn,
                                            const char *method_name) {
    if (!ctx || !package_qn || !method_name)
        return NULL;

    enum { CAP = ENGINE_LSP_MAX_LOOKUP_DEPTH * 2 };
    ...
    while (frontier_count > 0 && visited_count < CAP) {
        ...
        const EngineRegisteredType *t = engine_registry_lookup_type(ctx->registry, pkg);
        if (!t) {
            ...
            const EngineRegisteredFunc *direct =
                engine_registry_lookup_method(ctx->registry, pkg, method_name);
            if (direct)
                return direct;
            continue;
        }
            ...
            for (int i = 0; t->method_names[i]; i++) {
                if (strcmp(t->method_names[i], method_name) == 0) {
                    const EngineRegisteredFunc *f =
                        engine_registry_lookup_func(ctx->registry, t->method_qns[i]);
                    if (f)
                        return f;
                }
            }
...
static const EngineType *perl_eval_bless(PerlLSPContext *ctx, TSNode call_node) {
    ...
    if (!have_class) {
        ...
        if (pkg && pkg[0])
            return engine_type_named(ctx->arena, pkg);
    ...
    if (perl_is_string_node(ack)) {
        ...
        if (inner && inner[0])
            return engine_type_named(ctx->arena, perl_resolve_package_name(ctx, inner));
        ...
        if (bw && strcmp(bw, "__PACKAGE__") == 0) {
            ...
            return engine_type_named(ctx->arena, perl_resolve_package_name(ctx, bw));
        }
    } else {
...
const EngineType *perl_eval_expr_type(PerlLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "scalar") == 0 || strcmp(k, "scalar_variable") == 0) {
        ...
        if (txt) {
            ...
            const EngineType *t = engine_scope_lookup(ctx->current_scope, bare);
            if (t)
                result = t;
        }
        ...
        result = perl_eval_method_call_type(ctx, node);
               ...
               strcmp(k, "ambiguous_function_call_expression") == 0) {
        const EngineType *blessed = perl_eval_bless(ctx, node);
        if (blessed && !engine_type_is_unknown(blessed))
            result = blessed;
        else
...
static const EngineType *perl_eval_new_type(PerlLSPContext *ctx, TSNode node) {
    ...
    if (perl_is_bareword_node(ik)) {
        ...
        if (cls && cls[0])
            return engine_type_named(ctx->arena, perl_resolve_package_name(ctx, cls));
...
static const EngineType *perl_eval_function_call_type(PerlLSPContext *ctx, TSNode node) {
    ...
    if (colons) {
        ...
        char *pkg = engine_arena_strndup(ctx->arena, name, plen);
        const char *shortn = colons + 2;
        f = perl_lookup_method(ctx, pkg, shortn);
        if (!f)
            f = engine_registry_lookup_symbol(ctx->registry, pkg, shortn);
    } else {
        /* Bare func() — Exporter import map, then file-local/global func. */
        const char *imp = perl_find_import(ctx, name);
        if (imp)
            f = engine_registry_lookup_func(ctx->registry, imp);
        if (!f)
            f = engine_registry_lookup_symbol(ctx->registry, ctx->module_qn, name);
    }
...
static const EngineType *perl_eval_method_call_type(PerlLSPContext *ctx, TSNode node) {
    ...
    const EngineType *ctor = perl_eval_new_type(ctx, node);
    if (ctor)
        return ctor;
    ...
    if (!ts_node_is_null(inv)) {
        ...
        if (perl_is_bareword_node(ik)) {
            ...
            const EngineType *recv = perl_eval_expr_type(ctx, inv);
    ...
    const EngineRegisteredFunc *f = perl_lookup_method(ctx, class_qn, mname);
    if (f && f->signature && f->signature->kind == ENGINE_TYPE_FUNC &&
        f->signature->data.func.return_types && f->signature->data.func.return_types[0]) {
        return f->signature->data.func.return_types[0];
    }
...
static char *perl_exact_coderef_name(PerlLSPContext *ctx, TSNode refgen) {
    ...
    if (*p != '\0' || end == start)
        return NULL;
    return engine_arena_strndup(ctx->arena, start, (size_t)(end - start));
}

static const EngineRegisteredFunc *perl_resolve_coderef_target(PerlLSPContext *ctx, char *name) {
    if (!ctx || !name || !name[0])
        return NULL;
    char *colons = NULL;
    for (char *p = strstr(name, "::"); p; p = strstr(p + 2, "::"))
        colons = p;
    if (colons) {
        char *pkg = engine_arena_strndup(ctx->arena, name, (size_t)(colons - name));
        const char *short_name = colons + 2;
        const EngineRegisteredFunc *f = perl_lookup_method(ctx, pkg, short_name);
        return f ? f : engine_registry_lookup_symbol(ctx->registry, pkg, short_name);
    }
    const char *import_qn = perl_find_import(ctx, name);
    if (import_qn) {
        const EngineRegisteredFunc *f = engine_registry_lookup_func(ctx->registry, import_qn);
        if (f)
            return f;
    }
    return engine_registry_lookup_symbol(ctx->registry, ctx->module_qn, name);
}

static void perl_resolve_one_coderef(PerlLSPContext *ctx, TSNode refgen) {
    char *name = perl_exact_coderef_name(ctx, refgen);
    const EngineRegisteredFunc *target = name ? perl_resolve_coderef_target(ctx, name) : NULL;
...
static void perl_emit_non_direct_coderef_usage(PerlLSPContext *ctx, TSNode refgen) {
    ...
    for (int i = 0; i < ctx->usages->count; i++) {
        const EngineUsage *existing = &ctx->usages->items[i];
        if (existing->kind == ENGINE_USAGE_VALUE && existing->ref_name &&
    ...
    engine_usages_push(ctx->usages, ctx->arena, usage);
}
...
static void perl_resolve_method_call(PerlLSPContext *ctx, TSNode call) {
    ...
    if (strncmp(mname, "SUPER::", 7) == 0) {
        ...
        if (!parent_qn || !parent_qn[0])
            return;
        const EngineRegisteredFunc *sf = perl_lookup_method(ctx, parent_qn, super_method);
        if (sf)
...
static void perl_process_assignment(PerlLSPContext *ctx, TSNode assign) {
    ...
    const EngineType *rt = perl_eval_expr_type(ctx, right);
    if (rt && rt->kind == ENGINE_TYPE_NAMED)
        engine_scope_bind(ctx->current_scope, bare, rt);
}
 ...
 * for the Java/C++ walkers). Past ENGINE_LSP_PERL_MAX_WALK_DEPTH the subtree is
 * skipped — graceful degradation, never a wrong edge. */
static void perl_resolve_calls_in_node(PerlLSPContext *ctx, TSNode node) {
    if (ctx->walk_depth >= ENGINE_LSP_PERL_MAX_WALK_DEPTH)
        return;
    ctx->walk_depth++;
...
static void perl_infer_self_type(PerlLSPContext *ctx, TSNode body) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const char *bare = perl_strip_sigil(vtxt);
        if (bare && bare[0])
            engine_scope_bind(ctx->current_scope, bare, engine_type_named(ctx->arena, pkg));
        free(kids);
...
static void process_subroutine(PerlLSPContext *ctx, TSNode node) {
    ...
    if (sname && sname[0]) {
        if (ctx->module_qn)
            ctx->enclosing_func_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, sname);
        else
            ctx->enclosing_func_qn = engine_arena_strdup(ctx->arena, sname);
    }
...
static void process_package_decl(PerlLSPContext *ctx, TSNode node) {
    ...
    if (!pkg || !pkg[0])
        return;
    ctx->current_package_qn = engine_arena_strdup(ctx->arena, pkg);
...
static void perl_collect_qw_imports(PerlLSPContext *ctx, TSNode container,
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        char *target = engine_arena_sprintf(ctx->arena, "%s.%s", module_dot, fn);
...
void perl_lsp_process_file(PerlLSPContext *ctx, TSNode root) {
     ...
     * (engine_run_perl_lsp) has already run a pre-pass to build registry types. */
    ctx->current_package_qn = "";
...
/* Register a per-package EngineRegisteredType for every package that participates
 * in @ISA (as child or parent), then attach @ISA parents (embedded_types). */
static void perl_register_packages(PerlLSPContext *ctx, EngineTypeRegistry *reg) {
    for (int i = 0; i < ctx->isa_count; i++) {
        const char *names[2] = {ctx->isa_pkg_qns[i], ctx->isa_parent_qns[i]};
        for (int s = 0; s < 2; s++) {
            const char *pkg = names[s];
            if (!pkg || !pkg[0] || engine_registry_lookup_type(reg, pkg))
                continue;
            EngineRegisteredType rt;
            memset(&rt, 0, sizeof(rt));
            rt.qualified_name = engine_arena_strdup(ctx->arena, pkg);
    ...
    for (int t = 0; t < reg->type_count; t++) {
        EngineRegisteredType *rt = &reg->types[t];
        ...
        if (pc == 0)
            continue;
        const char **parents =
            (const char **)engine_arena_alloc(ctx->arena, (size_t)(pc + 1) * sizeof(char *));
        if (!parents)
            continue;
        int w = 0;
        for (int i = 0; i < ctx->isa_count; i++) {
...
static void perl_type_set_methods(PerlLSPContext *ctx, EngineTypeRegistry *reg, const char *pkg,
    ...
    if (!rt) {
        EngineRegisteredType nt;
        memset(&nt, 0, sizeof(nt));
        nt.qualified_name = engine_arena_strdup(ctx->arena, pkg);
        nt.short_name = nt.qualified_name;
        engine_registry_add_type(reg, nt);
        if (reg->type_count == 0)
    ...
    const char **mn =
        (const char **)engine_arena_alloc(ctx->arena, (size_t)(total + 1) * sizeof(char *));
    const char **mq =
        (const char **)engine_arena_alloc(ctx->arena, (size_t)(total + 1) * sizeof(char *));
    if (!mn || !mq)
        return;
...
static void perl_attach_methods(PerlLSPContext *ctx, EngineTypeRegistry *reg, TSNode root) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(k, "package_statement") == 0) {
            ...
            if (!ts_node_is_null(name)) {
                ...
                if (p && p[0])
                    cur_pkg = engine_arena_strdup(ctx->arena, p);
            }
            ...
            for (uint32_t bi = 0; bi < bn; bi++) {
                ...
                const char *bqn = ctx->module_qn
                                      ? engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bsn)
                                      : engine_arena_strdup(ctx->arena, bsn);
        ...
        const char *sub_qn = ctx->module_qn
                                 ? engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, sname)
                                 : engine_arena_strdup(ctx->arena, sname);
...
/* ── entry: engine_run_perl_lsp ────────────────────────────────────── */

void engine_run_perl_lsp(EngineArena *arena, EngineFileResult *result, const char *source, int source_len,
                      TSNode root) {
    ...
    engine_perl_stdlib_register(&reg, arena);
    ...
    if (ctx.debug) {
        ...
        for (int i = 0; i < result->resolved_calls.count; i++) {
            EngineResolvedCall *r = &result->resolved_calls.items[i];
