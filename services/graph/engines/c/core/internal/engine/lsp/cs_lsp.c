 ...
 * a EngineScope stack plus a CSLSPContext that tracks type/namespace/using
 * state that does not naturally live in the value scope.
 *
 * Resolution order for a bare identifier `X` inside member `M` in type
 * `T` in namespace `N`:
 *
 *    1. local variables / parameters in the current block          (EngineScope)
...
static const EngineType *cs_eval_invocation_type(CSLSPContext *ctx, TSNode call);
static const EngineType *cs_eval_member_access_type(CSLSPContext *ctx, TSNode node);
static const EngineType *cs_eval_object_creation_type(CSLSPContext *ctx, TSNode node);
static const EngineType *cs_eval_identifier_type(CSLSPContext *ctx, TSNode node);
static const EngineType *cs_substitute_type_params(EngineArena *arena, const EngineType *t,
                                                 const char **param_names,
                                                 const EngineType **param_args);
...
static void cs_register_type_decls(CSLSPContext *ctx, EngineTypeRegistry *reg, TSNode root);
static char *cs_node_text_cached(CSLSPContext *ctx, TSNode node);
static const EngineType *cs_unwrap_task(CSLSPContext *ctx, const EngineType *t);
static const EngineType *cs_unwrap_nullable(const EngineType *t);
...
static char *cs_normalize_name(EngineArena *a, const char *name) {
    if (!name) return NULL;
...
static char *cs_strip_generic_args(EngineArena *a, const char *name) {
    if (!name) return NULL;
    const char *lt = strchr(name, '<');
    if (!lt) return engine_arena_strdup(a, name);
    size_t len = (size_t)(lt - name);
    return engine_arena_strndup(a, name, len);
}
...
void cs_lsp_init(CSLSPContext *ctx, EngineArena *arena, const char *source, int source_len,
    ...
    cs_lsp_add_using(ctx, ENGINE_CS_USING_NAMESPACE, "", "System", false);
...
void cs_lsp_add_using(CSLSPContext *ctx, EngineCSUsingKind kind, const char *local_name,
                      const char *target_qn, bool is_global) {
    if (!target_qn) return;
    /* Dedupe by (kind, local_name, target). */
    for (int i = 0; i < ctx->using_count; i++) {
        EngineCSUsing *u = &ctx->usings[i];
    ...
    if (ctx->using_count >= ctx->using_cap) {
        ...
        EngineCSUsing *nu = (EngineCSUsing *)engine_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*nu));
        if (!nu) return;
        for (int i = 0; i < ctx->using_count; i++) nu[i] = ctx->usings[i];
        ctx->usings = nu;
        ctx->using_cap = new_cap;
    }
    EngineCSUsing *slot = &ctx->usings[ctx->using_count++];
    slot->kind = kind;
    slot->local_name = local_name ? engine_arena_strdup(ctx->arena, local_name) : "";
    slot->target_qn = engine_arena_strdup(ctx->arena, target_qn);
...
static void cs_namespace_push(CSLSPContext *ctx, const char *ns_name) {
    ...
    if (ctx->namespace_count >= ctx->namespace_cap) {
        ...
        const char **arr = (const char **)engine_arena_alloc(ctx->arena,
    ...
    ctx->namespace_stack[ctx->namespace_count++] = engine_arena_strdup(ctx->arena, ns_name);
}
...
static const char *cs_namespace_qn(CSLSPContext *ctx) {
    ...
    char *out = (char *)engine_arena_alloc(ctx->arena, total + 1);
    if (!out) return "";
    size_t w = 0;
    for (int i = 0; i < ctx->namespace_count; i++) {
        if (i > 0) out[w++] = '.';
...
static const EngineRegisteredType *cs_lookup_type_qn(CSLSPContext *ctx, const char *qn) {
    if (!ctx->registry || !qn) return NULL;
    return engine_registry_lookup_type(ctx->registry, qn);
}
...
const char *cs_resolve_type_name(CSLSPContext *ctx, const char *raw) {
    ...
    for (int i = 0; i < ctx->type_param_count; i++) {
        if (strcmp(ctx->type_param_names[i], bare) == 0) {
            const EngineType *arg = ctx->type_param_args[i];
            if (arg && arg->kind == ENGINE_TYPE_NAMED) return arg->data.named.qualified_name;
    ...
    if (ctx->enclosing_class_qn) {
        const char *try_qn =
            engine_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, bare);
    ...
    if (ctx->namespace_count > 0) {
        ...
        for (;;) {
            if (!ns || !*ns) break;
            const char *try_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ns, bare);
            if (cs_lookup_type_qn(ctx, try_qn)) return try_qn;
            const char *dot = strrchr(ns, '.');
            if (!dot) break;
            char *trim = engine_arena_strndup(ctx->arena, ns, (size_t)(dot - ns));
            ns = trim;
        }
    }
    ...
    if (ctx->module_qn && ctx->module_qn[0]) {
        const char *try_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare);
        if (cs_lookup_type_qn(ctx, try_qn)) return try_qn;
    }

    /* 7. using namespace X — try X.bare. */
    for (int i = 0; i < ctx->using_count; i++) {
        const EngineCSUsing *u = &ctx->usings[i];
        if (u->kind != ENGINE_CS_USING_NAMESPACE) continue;
        const char *try_qn = engine_arena_sprintf(ctx->arena, "%s.%s", u->target_qn, bare);
        if (cs_lookup_type_qn(ctx, try_qn)) return try_qn;
    }
    ...
    for (int i = 0; i < ctx->using_count; i++) {
        const EngineCSUsing *u = &ctx->usings[i];
        if (u->kind != ENGINE_CS_USING_ALIAS) continue;
        ...
        if (strncmp(bare, u->local_name, alias_len) == 0) {
            if (bare[alias_len] == '\0') {
                ...
                if (target_bare && cs_lookup_type_qn(ctx, target_bare))
                    return target_bare;
                return engine_arena_strdup(ctx->arena, u->target_qn);
            }
            if (bare[alias_len] == '.') {
                /* `Alias.Sub` -> `Target.Sub` */
                char *target_bare = cs_strip_generic_args(ctx->arena, u->target_qn);
                const char *base = target_bare ? target_bare : u->target_qn;
                const char *try_qn = engine_arena_sprintf(ctx->arena, "%s%s", base,
                                                        bare + alias_len);
                if (cs_lookup_type_qn(ctx, try_qn)) return try_qn;
            }
        }
...
const EngineRegisteredFunc *cs_lookup_method(CSLSPContext *ctx, const char *type_qn,
                                           const char *method_name) {
    if (!type_qn || !method_name) return NULL;

    const EngineRegisteredFunc *f =
        engine_registry_lookup_method(ctx->registry, type_qn, method_name);
    if (f) return f;

    /* Walk inheritance chain (base + interfaces + transitive bases). */
    const EngineRegisteredType *t = cs_lookup_type_qn(ctx, type_qn);
    if (!t) return NULL;
    ...
    while (frontier_count > 0 && visited_count < CS_LSP_PARENT_WALK_MAX) {
        ...
        const EngineRegisteredType *next = cs_lookup_type_qn(ctx, parent);
        if (!next) continue;
    ...
    if (strcmp(type_qn, "System.Object") != 0) {
        f = engine_registry_lookup_method(ctx->registry, "System.Object", method_name);
        if (f) return f;
    }
    return NULL;
}
 ...
 * dependent. */
static const EngineRegisteredFunc *cs_lookup_using_static_method(CSLSPContext *ctx,
                                                              const char *method_name,
                                                              bool *ambiguous) {
    if (ambiguous)
        *ambiguous = false;
    ...
    const EngineRegisteredFunc *only = NULL;
    for (int i = 0; i < ctx->using_count; i++) {
        const EngineCSUsing *u = &ctx->usings[i];
        if (u->kind != ENGINE_CS_USING_STATIC)
            continue;
...
static const EngineRegisteredFunc *cs_lookup_extension(CSLSPContext *ctx,
    ...
    for (int i = 0; i < ctx->registry->func_count; i++) {
        ...
        if (!cand->signature || cand->signature->kind != ENGINE_TYPE_FUNC) continue;
        const EngineType *sig = cand->signature;
        if (!sig->data.func.param_types) continue;
        const EngineType *first = sig->data.func.param_types[0];
        if (!first) continue;
        ...
        bool match = false;
        if (first->kind == ENGINE_TYPE_NAMED && first->data.named.qualified_name) {
            ...
            const EngineRegisteredType *rt = cs_lookup_type_qn(ctx, receiver_qn);
        ...
        } else if (first->kind == ENGINE_TYPE_TEMPLATE) {
...
const EngineType *cs_parse_type_node(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "nullable_type") == 0) {
        ...
        if (ts_node_is_null(inner)) return engine_type_unknown();
        return cs_parse_type_node(ctx, inner);
    }
    if (strcmp(kind, "array_type") == 0) {
        TSNode inner = ts_node_child_by_field_name(node, "type", 4);
        if (ts_node_is_null(inner)) inner = cs_first_named_child(node);
        const EngineType *elem = cs_parse_type_node(ctx, inner);
        return engine_type_template(ctx->arena, "System.Array",
                                 (const EngineType *[]){elem, NULL}, 1);
    }
    ...
    if (strcmp(kind, "tuple_type") == 0) {
        ...
        if (!elems) return engine_type_unknown();
        int count = 0;
        for (uint32_t i = 0; i < nc; i++) {
    ...
    if (strcmp(kind, "generic_name") == 0) {
        ...
        if (ts_node_is_null(name_node)) return engine_type_unknown();
        char *raw = cs_node_text(ctx, name_node);
        if (!raw) return engine_type_unknown();
        const char *resolved = cs_resolve_type_name(ctx, raw);
        if (!resolved) return engine_type_unknown();
        ...
        const EngineType **args = (const EngineType **)engine_arena_alloc(
            ctx->arena, (size_t)(nc + 1) * sizeof(*args));
        if (!args) return engine_type_named(ctx->arena, resolved);
        int count = 0;
        for (uint32_t i = 0; i < nc; i++) {
        ...
        args[count] = NULL;
        return engine_type_template(ctx->arena, resolved, args, count);
    }
    if (strcmp(kind, "qualified_name") == 0 || strcmp(kind, "identifier") == 0 ||
        ...
        if (pre) return engine_type_named(ctx->arena, pre);
        return engine_type_named(ctx->arena, resolved);
    }
    if (strcmp(kind, "ref_type") == 0) {
        TSNode inner = ts_node_child_by_field_name(node, "type", 4);
        if (ts_node_is_null(inner)) inner = cs_first_named_child(node);
        const EngineType *t = cs_parse_type_node(ctx, inner);
        return engine_type_reference(ctx->arena, t);
    }
...
const EngineType *cs_eval_expr_type(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (ctx->enclosing_base_qn) {
            result = engine_type_named(ctx->arena, ctx->enclosing_base_qn);
        }
        ...
        result = engine_type_named(ctx->arena, "System.String");
    } else if (strcmp(kind, "character_literal") == 0) {
        result = engine_type_named(ctx->arena, "System.Char");
    } else if (strcmp(kind, "integer_literal") == 0) {
        result = engine_type_named(ctx->arena, "System.Int32");
    } else if (strcmp(kind, "real_literal") == 0) {
        result = engine_type_named(ctx->arena, "System.Double");
    } else if (strcmp(kind, "boolean_literal") == 0) {
        result = engine_type_named(ctx->arena, "System.Boolean");
        ...
        if (!ts_node_is_null(inner)) {
            const EngineType *t = cs_eval_expr_type(ctx, inner);
        ...
        if (ts_node_is_null(b)) {
            ...
            for (uint32_t i = 0; i < nc; i++) {
                ...
                if (!ts_node_is_null(c) && ts_node_is_named(c)) {
                    const EngineType *t = cs_eval_expr_type(ctx, c);
        ...
        const EngineType **elems =
            (const EngineType **)engine_arena_alloc(ctx->arena, (size_t)(nc + 1) * sizeof(*elems));
        if (!elems) {
            ctx->eval_depth--;
            return engine_type_unknown();
        }
        ...
        elems[count] = NULL;
        result = engine_type_tuple(ctx->arena, elems, count);
        ...
        result = engine_type_named(ctx->arena, "System.Type");
        ...
        if (!ts_node_is_null(obj)) {
            const EngineType *recv = cs_eval_expr_type(ctx, obj);
        ...
        if (!ts_node_is_null(tnode)) {
            const EngineType *elem = cs_parse_type_node(ctx, tnode);
            result = engine_type_template(ctx->arena, "System.Array",
                                       (const EngineType *[]){elem, NULL}, 1);
        }
    } else if (strcmp(kind, "implicit_array_creation_expression") == 0) {
        result = engine_type_template(ctx->arena, "System.Array",
                                   (const EngineType *[]){engine_type_unknown(), NULL}, 1);
    }
...
static const EngineType *cs_eval_identifier_type(CSLSPContext *ctx, TSNode node) {
    ...
    /* Local / parameter scope. engine_scope_lookup returns engine_type_unknown()
    ...
    if (bound && bound->kind != ENGINE_TYPE_UNKNOWN) return bound;

    /* Implicit `this` member. Try to find a field on the enclosing class. */
    if (ctx->enclosing_class_qn) {
        const EngineRegisteredType *rt = cs_lookup_type_qn(ctx, ctx->enclosing_class_qn);
    ...
    if (resolved) {
        const EngineRegisteredType *rt = cs_lookup_type_qn(ctx, resolved);
        if (rt) return engine_type_named(ctx->arena, rt->qualified_name);
    }
...
static const EngineType *cs_eval_invocation_type(CSLSPContext *ctx, TSNode call) {
    ...
    if (ts_node_is_null(fn)) return engine_type_unknown();
    ...
    if (strcmp(fk, "member_access_expression") == 0 ||
        ...
        if (ts_node_is_null(name)) return engine_type_unknown();
        char *mname = cs_node_text(ctx, name);
        if (!mname) return engine_type_unknown();
        char *bare_name = cs_strip_generic_args(ctx->arena, mname);

        const EngineType *recv_type = engine_type_unknown();
        ...
        if (!type_qn) return engine_type_unknown();

        const EngineRegisteredFunc *f = cs_lookup_method(ctx, type_qn, bare_name);
        if (!f) f = cs_lookup_extension(ctx, type_qn, bare_name);
        if (!f || !f->signature) return engine_type_unknown();
        if (f->signature->kind == ENGINE_TYPE_FUNC && f->signature->data.func.return_types &&
            f->signature->data.func.return_types[0]) {
            const EngineType *ret = f->signature->data.func.return_types[0];
            ...
            if (recv_type && recv_type->kind == ENGINE_TYPE_TEMPLATE) {
                const EngineRegisteredType *rt = cs_lookup_type_qn(ctx, type_qn);
    ...
    if (strcmp(fk, "identifier") == 0 || strcmp(fk, "generic_name") == 0) {
        ...
        if (!fname) return engine_type_unknown();
        ...
        if (strcmp(bare, "nameof") == 0) {
            return engine_type_named(ctx->arena, "System.String");
        }
        if (strcmp(bare, "typeof") == 0) {
            return engine_type_named(ctx->arena, "System.Type");
        }

        /* Try enclosing class member. */
        if (ctx->enclosing_class_qn) {
            const EngineRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_class_qn, bare);
        ...
        const char *ns = cs_namespace_qn(ctx);
        if (ns && *ns) {
            const char *qn = engine_arena_sprintf(ctx->arena, "%s.%s", ns, bare);
        ...
        const EngineRegisteredFunc *f = cs_lookup_using_static_method(ctx, bare, &ambiguous);
        if (ambiguous)
            return engine_type_unknown();
        if (f && f->signature && f->signature->data.func.return_types &&
            f->signature->data.func.return_types[0]) {
...
static const EngineType *cs_eval_member_access_type(CSLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(obj) || ts_node_is_null(name)) return engine_type_unknown();
    ...
    if (obj_text) {
        ...
        if (type_qn && cs_lookup_type_qn(ctx, type_qn)) {
            ...
            const EngineRegisteredFunc *f = cs_lookup_method(ctx, type_qn, fname);
            if (f && f->signature && f->signature->kind == ENGINE_TYPE_FUNC &&
                f->signature->data.func.return_types &&
                f->signature->data.func.return_types[0]) {
                return f->signature->data.func.return_types[0];
    ...
    if (!recv) return engine_type_unknown();
        ...
        for (int i = 0; rt->field_names[i]; i++) {
            if (strcmp(rt->field_names[i], fname) == 0) {
                if (rt->field_types && rt->field_types[i]) {
                    const EngineType *ft = rt->field_types[i];
                    if (recv->kind == ENGINE_TYPE_TEMPLATE && rt->type_param_names &&
    ...
    if (f && f->signature && f->signature->kind == ENGINE_TYPE_FUNC &&
        ...
        const EngineType *ret = f->signature->data.func.return_types[0];
        if (recv->kind == ENGINE_TYPE_TEMPLATE && rt && rt->type_param_names &&
            recv->data.template_type.template_args) {
...
static const EngineType *cs_eval_object_creation_type(CSLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(tnode)) return engine_type_unknown();
...
static const EngineType *cs_substitute_type_params(EngineArena *arena, const EngineType *t,
    ...
    if (t->kind == ENGINE_TYPE_TEMPLATE) {
        const EngineType *const *old_args = t->data.template_type.template_args;
    ...
    if (t->kind == ENGINE_TYPE_TYPE_PARAM) {
...
static const char *cs_resolve_callable_name(CSLSPContext *ctx, const char *name) {
    ...
    if (ctx->enclosing_class_qn) {
        const EngineRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_class_qn, name);
        if (f && f->qualified_name)
            return f->qualified_name;
    }
    if (ctx->enclosing_base_qn) {
        const EngineRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_base_qn, name);
        if (f && f->qualified_name)
            return f->qualified_name;
    }

    bool using_static_ambiguous = false;
    const EngineRegisteredFunc *using_static =
    ...
    const char *ns = cs_namespace_qn(ctx);
    if (ns && ns[0]) {
        const char *qn = engine_arena_sprintf(ctx->arena, "%s.%s", ns, name);
        const EngineRegisteredFunc *f = engine_registry_lookup_func(ctx->registry, qn);
        if (f && f->qualified_name)
            return f->qualified_name;
    }
    ...
    for (int i = 0; ctx->registry && i < ctx->registry->func_count; i++) {
        const EngineRegisteredFunc *candidate = &ctx->registry->funcs[i];
...
static const char *cs_resolve_callable_value(CSLSPContext *ctx, TSNode node,
    ...
    if (receiver_text) {
        ...
        if (type_qn && cs_lookup_type_qn(ctx, type_qn)) {
            const EngineRegisteredFunc *f = cs_lookup_method(ctx, type_qn, member_name);
            if (f && callable_leaf)
    ...
    const EngineType *receiver_type = cs_unwrap_nullable(cs_eval_expr_type(ctx, receiver));
    const char *type_qn = NULL;
    if (receiver_type && receiver_type->kind == ENGINE_TYPE_NAMED) {
        type_qn = receiver_type->data.named.qualified_name;
    } else if (receiver_type && receiver_type->kind == ENGINE_TYPE_TEMPLATE) {
...
static void cs_process_local_decl(CSLSPContext *ctx, TSNode node) {
    ...
    const EngineType *declared_type = ts_node_is_null(tnode) ? engine_type_unknown()
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (!rhs_t || rhs_t->kind == ENGINE_TYPE_UNKNOWN) {
            ...
            for (uint32_t j = 0; j < cc; j++) {
                ...
                if (strcmp(ck, "equals_value_clause") == 0) {
                    ...
                    if (!ts_node_is_null(rhs)) {
                        rhs_node = rhs;
                        const EngineType *t = cs_eval_expr_type(ctx, rhs);
                        if (t && t->kind != ENGINE_TYPE_UNKNOWN) rhs_t = t;
                    }
                    break;
                }
                if (seen_named == 0) {
                ...
                if (seen_named >= 1) {
                    rhs_node = cn;
                    const EngineType *t = cs_eval_expr_type(ctx, cn);
                    if (t && t->kind != ENGINE_TYPE_UNKNOWN) {
                        rhs_t = t;
                        break;
                    }
        ...
        const EngineType *bind = is_var ? (rhs_t ? rhs_t : declared_type) : declared_type;
        if (!bind) bind = engine_type_unknown();
        const char *callable_qn =
            ts_node_is_null(rhs_node) ? NULL : cs_resolve_callable_value(ctx, rhs_node, NULL);
        if (callable_qn) {
            engine_scope_bind_callable(ctx->current_scope, vname, bind, callable_qn);
        } else {
            engine_scope_bind(ctx->current_scope, vname, bind);
        }
    }
}
...
static void cs_process_foreach(CSLSPContext *ctx, TSNode node) {
    ...
    const EngineType *element_t = engine_type_unknown();
    if (!ts_node_is_null(iter)) {
        const EngineType *iter_t = cs_eval_expr_type(ctx, iter);
        if (iter_t && iter_t->kind == ENGINE_TYPE_TEMPLATE) {
            const EngineType *const *args = iter_t->data.template_type.template_args;
            if (args && args[0]) element_t = args[0];
        }
    }
    if (!ts_node_is_null(tnode)) {
        const EngineType *declared = cs_parse_type_node(ctx, tnode);
        char *tt = cs_node_text(ctx, tnode);
        bool is_var = (tt && (strcmp(tt, "var") == 0));
        if (!is_var && declared && declared->kind != ENGINE_TYPE_UNKNOWN) element_t = declared;
    }
    engine_scope_bind(ctx->current_scope, vname, element_t);
}
...
static void cs_process_assignment(CSLSPContext *ctx, TSNode node) {
    ...
    if (callable_qn) {
        engine_scope_update_callable(ctx->current_scope, vname, callable_qn);
    } else {
        ...
        engine_scope_update_callable(ctx->current_scope, vname, NULL);
    }
}
...
static void cs_emit_resolved_kind_reason(CSLSPContext *ctx, const char *callee_qn,
                                         ...
                                         EngineResolvedKind kind, const char *reason) {
...
static void cs_emit_resolved(CSLSPContext *ctx, const char *callee_qn, const char *strategy,
                             float confidence) {
    cs_emit_resolved_kind(ctx, callee_qn, strategy, confidence, ENGINE_RESOLVED_INVOCATION);
}
...
static void cs_emit_resolved_reason(CSLSPContext *ctx, const char *callee_qn, const char *strategy,
    ...
    cs_emit_resolved_kind_reason(ctx, callee_qn, strategy, confidence, ENGINE_RESOLVED_INVOCATION,
                                 reason);
}
...
static void cs_emit_resolved_kind_reason_at(CSLSPContext *ctx, const char *callee_qn,
                                            const char *strategy, float confidence,
                                            EngineResolvedKind kind, const char *reason, TSNode site) {
...
static void cs_retarget_callable_carrier(CSLSPContext *ctx, TSNode site, const char *source_name,
    ...
    for (int i = 0; i < ctx->call_carriers->count; i++) {
        EngineCall *call = &ctx->call_carriers->items[i];
...
static void cs_resolve_callable_arguments(CSLSPContext *ctx, TSNode call) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        cs_emit_resolved_kind_reason_at(ctx, target, "lsp_callable_value_reference", 0.95f,
                                        ENGINE_RESOLVED_CALL_REFERENCE, source_name, callable_leaf);
    }
}
...
static void cs_resolve_invocation(CSLSPContext *ctx, TSNode call) {
    ...
    if (strcmp(fk, "member_access_expression") == 0 ||
        ...
        if (recv_text) {
            ...
            if (type_qn && cs_lookup_type_qn(ctx, type_qn)) {
                const EngineRegisteredFunc *f = cs_lookup_method(ctx, type_qn, bare);
                if (f) {
                ...
                cs_emit_resolved(ctx,
                                  engine_arena_sprintf(ctx->arena, "%s.%s", type_qn, bare),
    ...
    if (strcmp(fk, "identifier") == 0 || strcmp(fk, "generic_name") == 0) {
        ...
        if (engine_scope_contains(ctx->current_scope, bare)) {
            const char *target = engine_scope_lookup_callable(ctx->current_scope, bare);
            if (target) {
        ...
        if (ctx->enclosing_base_qn) {
            const EngineRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_base_qn, bare);
            if (f) {
...
static void cs_resolve_object_creation(CSLSPContext *ctx, TSNode call) {
    ...
    const EngineType *t = cs_parse_type_node(ctx, tnode);
    const char *tqn = NULL;
    if (t && t->kind == ENGINE_TYPE_NAMED) tqn = t->data.named.qualified_name;
    else if (t && t->kind == ENGINE_TYPE_TEMPLATE) tqn = t->data.template_type.template_name;
    if (!tqn) return;
    ...
    const EngineRegisteredFunc *f = cs_lookup_method(ctx, tqn, short_name);
    if (f) {
...
static void cs_resolve_calls_in_node(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "block") == 0) {
        EngineScope *prev = ctx->current_scope;
        ctx->current_scope = engine_scope_push(ctx->arena, prev);
...
static void cs_collect_type_params(CSLSPContext *ctx, TSNode node, const char ***out_names,
    ...
    *out_count = 0;
    ...
    int cap = 4;
    const char **arr = (const char **)engine_arena_alloc(ctx->arena, (size_t)cap * sizeof(*arr));
    if (!arr) return;
    int n = 0;
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (n + 1 >= cap) {
            int new_cap = cap * 2;
            const char **ne =
                (const char **)engine_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*ne));
            if (!ne) break;
...
static void cs_process_function_like(CSLSPContext *ctx, TSNode node) {
    ...
    const EngineType **saved_tp_args = ctx->type_param_args;
    ...
    if (short_name) {
        if (ctx->enclosing_class_qn) {
            ctx->enclosing_func_qn =
                engine_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, short_name);
        } else if (ctx->module_qn) {
            ctx->enclosing_func_qn =
                engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, short_name);
        } else {
            ctx->enclosing_func_qn = engine_arena_strdup(ctx->arena, short_name);
        }
    }
    ...
    if (is_method) {
        ...
        if (tp_count > 0) {
            const EngineType **args =
                (const EngineType **)engine_arena_alloc(ctx->arena,
                                                  (size_t)(tp_count + 1) * sizeof(*args));
            if (args) {
                for (int i = 0; i < tp_count; i++) {
                    args[i] = engine_type_type_param(ctx->arena, tp_names[i]);
                }
                args[tp_count] = NULL;
...
static void cs_process_type_decl(CSLSPContext *ctx, TSNode node) {
    ...
    if (ctx->module_qn) {
        ctx->enclosing_class_qn =
            engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    } else {
    ...
    /* Base list -> enclosing_base_qn + enclosing_iface_qns. */
    {
        ...
        if (!ts_node_is_null(bl)) {
            ...
            int icap = 4, icount = 0;
            const char **ifs =
                (const char **)engine_arena_alloc(ctx->arena, (size_t)icap * sizeof(*ifs));
            for (uint32_t i = 0; i < nc; i++) {
                ...
                const EngineRegisteredType *rt = cs_lookup_type_qn(ctx, qn);
                if (rt && rt->is_interface) {
                    if (icount + 1 >= icap) {
                        int nc2 = icap * 2;
                        const char **ne = (const char **)engine_arena_alloc(
    ...
    if (!is_enum) {
        ...
        if (!ts_node_is_null(pctor)) {
            ...
            ctx->enclosing_func_qn =
                engine_arena_sprintf(ctx->arena, "%s..ctor", ctx->enclosing_class_qn);
...
static void cs_collect_imports(CSLSPContext *ctx, TSNode root) {
    ...
    while (top > 0) {
        ...
        if (strcmp(k, "using_directive") == 0) {
            ...
            if (target) {
                if (is_alias) {
                    cs_lsp_add_using(ctx, ENGINE_CS_USING_ALIAS, alias_name ? alias_name : "",
                                      target, is_global);
                } else if (is_static) {
                    cs_lsp_add_using(ctx, ENGINE_CS_USING_STATIC, "", target, is_global);
                } else {
                    cs_lsp_add_using(ctx, ENGINE_CS_USING_NAMESPACE, "", target, is_global);
                }
            }
        }
...
static void cs_parse_signature(EngineArena *arena, const char *signature,
                                CSLSPContext *ctx, const char ***out_names,
                                const EngineType ***out_types) {
    *out_names = NULL;
    *out_types = NULL;
    if (!signature) return;
    ...
    int cap = 8;
    int count = 0;
    const char **names = (const char **)engine_arena_alloc(arena, (size_t)cap * sizeof(*names));
    const EngineType **types = (const EngineType **)engine_arena_alloc(arena, (size_t)cap * sizeof(*types));
    ...
    while (*p && *p != ')') {
        ...
        if (!last_space) {
            ...
            char *type_text = engine_arena_strndup(arena, type_start, (size_t)(name_end - type_start));
            const EngineType *t = engine_type_unknown();
            if (ctx) t = cs_resolve_type_name(ctx, type_text)
                          ? engine_type_named(ctx->arena, cs_resolve_type_name(ctx, type_text))
                          : engine_type_unknown();
            (void)t;
            if (count + 1 >= cap) break;
            names[count] = engine_arena_sprintf(arena, "_arg%d", count);
            types[count] = t;
            count++;
        } else {
            char *type_text = engine_arena_strndup(arena, type_start, (size_t)(last_space - type_start));
            char *pname = engine_arena_strndup(arena, last_space + 1, (size_t)(name_end - last_space - 1));
            const EngineType *t = engine_type_unknown();
            if (ctx) {
                const char *resolved = cs_resolve_type_name(ctx, type_text);
                if (resolved) t = engine_type_named(ctx->arena, resolved);
            }
            if (count + 1 >= cap) break;
            names[count] = pname;
            types[count] = t;
...
static void cs_register_type_decls(CSLSPContext *ctx, EngineTypeRegistry *reg, TSNode root) {
    /* We rely on EngineFileResult.defs entries already being filled by the
...
static cs_fields_t *cs_fields_get(EngineArena *arena, cs_fields_table_t *tab,
                                    const char *class_qn) {
    ...
    if (tab->count + 1 >= tab->cap) {
        ...
        cs_fields_t *ne =
            (cs_fields_t *)engine_arena_alloc(arena, (size_t)new_cap * sizeof(*ne));
        if (!ne) return NULL;
    ...
    slot->cap = 8;
    slot->field_names =
        (const char **)engine_arena_alloc(arena, (size_t)slot->cap * sizeof(*slot->field_names));
    slot->field_types =
        (const EngineType **)engine_arena_alloc(arena, (size_t)slot->cap * sizeof(*slot->field_types));
    return slot;
}

static void cs_fields_add_debug(EngineArena *arena, cs_fields_t *f, const char *name,
                                 const EngineType *type, bool debug);

static void cs_fields_add(EngineArena *arena, cs_fields_t *f, const char *name,
...
static void cs_fields_add_debug(EngineArena *arena, cs_fields_t *f, const char *name,
    ...
    if (f->count + 2 >= f->cap) {
        int new_cap = f->cap * 2;
        const char **nn = (const char **)engine_arena_alloc(arena, (size_t)new_cap * sizeof(*nn));
        const EngineType **nt = (const EngineType **)engine_arena_alloc(arena, (size_t)new_cap * sizeof(*nt));
        if (!nn || !nt) return;
        for (int i = 0; i < f->count; i++) {
    ...
    f->field_types[f->count] = type ? type : engine_type_unknown();
    f->count++;
    f->field_names[f->count] = NULL;
    f->field_types[f->count] = NULL;
}

static void cs_collect_class_fields(CSLSPContext *ctx, EngineTypeRegistry *reg, TSNode root,
    ...
    while (top > 0) {
        ...
        if (strcmp(k, "field_declaration") == 0 || strcmp(k, "property_declaration") == 0 ||
            ...
            if (cls_short) {
                const char *cls_qn =
                    ctx->module_qn
                        ? engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cls_short)
                        : engine_arena_strdup(ctx->arena, cls_short);
                ...
                if (strcmp(k, "field_declaration") == 0 ||
                    ...
                    if (!ts_node_is_null(vd)) {
                        TSNode tnode = ts_node_child_by_field_name(vd, "type", 4);
                        if (ts_node_is_null(tnode)) tnode = cs_first_named_child(vd);
                        const EngineType *t = ts_node_is_null(tnode)
                                                ? engine_type_unknown()
                                                : cs_parse_type_node(ctx, tnode);
            ...
            if (!ts_node_is_null(params) && !ts_node_is_null(nm)) {
                ...
                if (cls_short) {
                    ...
                    for (uint32_t i = 0; i < pc; i++) {
                        ...
                        const EngineType *t = cs_parse_type_node(ctx, tn);
 ...
 *
 * The unified extractor leaves EngineDefinition.return_type NULL for C#
...
static void cs_method_rt_add(EngineArena *arena, cs_method_rt_table_t *tab,
                              const char *qn, const EngineType *rt) {
    if (!qn || !rt) return;
    for (int i = 0; i < tab->count; i++) {
    ...
    if (tab->count + 1 >= tab->cap) {
        ...
        cs_method_rt_entry_t *ne = (cs_method_rt_entry_t *)engine_arena_alloc(
    ...
    while (top > 0) {
        ...
        if (is_method || is_property || is_indexer) {
            ...
            if (is_indexer) {
                ...
                if (!ts_node_is_null(tnode)) {
                    const EngineType *rt = cs_parse_type_node(ctx, tnode);
                    cs_method_rt_add(ctx->arena, tab,
                                      engine_arena_sprintf(ctx->arena, "%s.%s", cls_qn,
                                                         short_name),
                                      rt);
                }
...
void engine_cs_refine_ast_return_types(CSLSPContext *ctx, EngineTypeRegistry *reg, TSNode root) {
    ...
    for (int i = 0; i < mtab.count; i++) {
        ...
        const EngineType *rt = mtab.items[i].rt;
        bool patched = false;
            ...
            if (strcmp(reg->funcs[j].qualified_name, qn) == 0) {
                ...
                if (rets) {
                    ...
                    reg->funcs[j].signature =
                        engine_type_func_replace_returns(ctx->arena, reg->funcs[j].signature, rets);
                }
                patched = true;
                break;
            }
...
/* ── parse return type from EngineDefinition.return_type ──────────── */

static const EngineType *cs_parse_return_type_text(CSLSPContext *ctx, const char *text) {
    if (!text || !*text) return engine_type_unknown();
    const char *p = text;
    while (*p == ' ' || *p == ':') p++;
    if (!*p) return engine_type_unknown();
    /* Strip trailing whitespace + '?' nullability. */
    size_t n = strlen(p);
    while (n > 0 && (p[n - 1] == ' ' || p[n - 1] == '?')) n--;
    if (n == 0) return engine_type_unknown();
    char *trimmed = engine_arena_strndup(ctx->arena, p, n);
...
static const EngineType *cs_signature_param_type_adapter(EngineArena *arena, const char *text,
                                                      void *parser_ctx) {
    (void)arena;
    CSLSPContext *ctx = (CSLSPContext *)parser_ctx;
    return ctx ? cs_parse_return_type_text(ctx, text) : engine_type_unknown();
}

/* ── single-file entry: engine_run_cs_lsp ──────────────────────────── */

void engine_run_cs_lsp(EngineArena *arena, EngineFileResult *result, const char *source, int source_len,
                    TSNode root) {
    ...
    /* Phase A: stdlib. */
    engine_csharp_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->defs.count; i++) {
                    ...
                    if (emb) {
                        for (int j = 0; j < bc; j++) {
                            ...
                            if (base[0] != '.' && !strchr(base, '.') && module_qn) {
                                q = engine_arena_sprintf(arena, "%s.%s", module_qn, base);
                            }
                            emb[j] = q;
                        }
                        emb[bc] = NULL;
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            const EngineType *rt =
                d->return_type ? engine_type_unknown() /* will refine below with ctx */
                               : engine_type_unknown();
            const EngineType **rets =
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        for (int j = 0; j < reg.func_count; j++) {
            if (strcmp(reg.funcs[j].qualified_name, d->qualified_name) != 0)
                continue;
            const EngineType **return_types = NULL;
            if (reg.funcs[j].signature && reg.funcs[j].signature->kind == ENGINE_TYPE_FUNC) {
                return_types = reg.funcs[j].signature->data.func.return_types;
            }
            reg.funcs[j].signature = engine_type_func(arena, NULL, param_types, return_types);
            break;
        }
    }
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        const EngineType *rt = cs_parse_return_type_text(&ctx, d->return_type);
        ...
        for (int j = 0; j < reg.func_count; j++) {
            if (strcmp(reg.funcs[j].qualified_name, d->qualified_name) == 0) {
                ...
                if (rets) {
                    ...
                    rets[1] = NULL;
                    reg.funcs[j].signature =
                        engine_type_func_replace_returns(arena, reg.funcs[j].signature, rets);
                }
                break;
            }
        }
     ...
     * leaves EngineDefinition.return_type NULL for C# methods, so we walk the
     * AST ourselves to recover declared return types. */
    engine_cs_refine_ast_return_types(&ctx, &reg, root);
...
static void cs_register_lsp_defs(EngineArena *arena, EngineTypeRegistry *reg,
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Interface") == 0 ||
            ...
            if (d->embedded_types && *d->embedded_types) {
                ...
                const char **arr =
                    (const char **)engine_arena_alloc(arena, (size_t)(n + 1) * sizeof(*arr));
                if (arr) {
                    int idx = 0;
                    ...
                    for (const char *p = d->embedded_types; ; p++) {
                        if (*p == '|' || *p == '\0') {
                            ...
                            if (len > 0) {
                                arr[idx++] = engine_arena_strndup(arena, start, len);
                            }
                            if (*p == '\0') break;
                            start = p + 1;
                        }
...
EngineTypeRegistry *engine_cs_build_cross_registry(EngineArena *arena, EngineLSPDef *defs, int def_count) {
    ...
    engine_csharp_stdlib_register(reg, arena);
    for (int i = 0; i < def_count; i++) {
        if (defs[i].lang != ENGINE_LANG_CSHARP) continue;
...
void engine_run_cs_lsp_cross_with_registry(EngineArena *arena, const char *source, int source_len,
    ...
    for (int i = 0; i < using_count; i++) {
        if (using_targets[i]) {
            cs_lsp_add_using(&ctx, ENGINE_CS_USING_NAMESPACE, "", using_targets[i], false);
        }
    }
    cs_lsp_process_file(&ctx, root);
...
void engine_run_cs_lsp_cross(EngineArena *arena, const char *source, int source_len,
...
void engine_batch_cs_lsp_cross(EngineArena *arena, EngineBatchCSLSPFile *files, int file_count,
                             EngineResolvedCallArray *out) {
    if (!arena || !files) return;
    for (int i = 0; i < file_count; i++) {
        EngineBatchCSLSPFile *f = &files[i];
        engine_run_cs_lsp_cross(arena, f->source, f->source_len, f->module_qn, f->defs, f->def_count,
