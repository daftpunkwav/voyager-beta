 ...
 * scope, infers expression types, and emits EngineResolvedCall entries for
 ...
 * call-edge and method-dispatch attribution problems engine cares about.
 *
 ...
 */
#ifndef ENGINE_LSP_KOTLIN_LSP_H
#define ENGINE_LSP_KOTLIN_LSP_H
...
typedef enum {
    ENGINE_KT_USE_UNKNOWN = 0,
    ENGINE_KT_USE_TYPE,     /* class / interface / object / typealias */
    ENGINE_KT_USE_FUNCTION, /* top-level fun, extension fun */
    ENGINE_KT_USE_PROPERTY, /* top-level val/var */
    ENGINE_KT_USE_WILDCARD, /* import a.b.* — local_name is a.b prefix */
} EngineKotlinUseKind;

typedef struct {
    const EngineScope *scope; /* exact lexical declaration frame, NULL for members */
    const char *owner_qn;  /* class/object QN for a member, NULL for lexical values */
    const char *name;
    const char *getter_qn;
    const char *setter_qn;
} EngineKotlinDelegateBinding;
...
typedef struct KotlinLSPContext {
    ...
    EngineKotlinUseKind *import_kinds;
    int import_count;
    int import_cap;
    ...
    EngineKotlinDelegateBinding *delegate_bindings;
    int delegate_count;
    int delegate_cap;
     ...
     * QN, built from the project defs by engine_run_kotlin_lsp_cross. Consulted
     ...
     * engine_run_kotlin_lsp_cross; opaque pointer to avoid a foundation include
     * in this header. */
    const void *cross_type_short;
    /* Cross-file only (NULL per-file): receiver type QN -> KtCrossFieldList,
     * from engine_kotlin_register_lsp_defs. Property-reference emission reads
 ...
 * pass the package prefix as target_qn and ENGINE_KT_USE_WILDCARD as kind. */
...
/* Evaluate a Kotlin expression's type. Returns engine_type_unknown() on
 * failure — never NULL. */
 ...
 * Called from engine_extract_file() after definitions and imports have been
 * extracted. */
 ...
 * Mirrors engine_run_java_lsp_cross. `defs` carries the graph QNs of every
 ...
 * list map built during registration (opaque EngineHashTable; the caller frees
 * with engine_ht_free). Pass NULL to discard it. The forward declaration lives at
...
void engine_kotlin_register_lsp_defs(EngineArena *arena, EngineTypeRegistry *reg, const EngineLSPDef *defs,
                                  int def_count, struct EngineHashTable **out_field_map);
...
void engine_kotlin_stdlib_register(EngineTypeRegistry *reg, EngineArena *arena);

/* Register Kotlin default-import targets — the prefixes auto-imported
 * by every Kotlin file. Used by the LSP context init. */
const char *const *engine_kotlin_default_import_packages(int *count_out);

#endif /* ENGINE_LSP_KOTLIN_LSP_H */
