 ...
 */
#ifndef ENGINE_SERVICE_PATTERNS_H
#define ENGINE_SERVICE_PATTERNS_H

#include <stdbool.h>

/* Edge type returned by pattern match. */
typedef enum {
    ENGINE_SVC_NONE = 0,      /* Not a service pattern — use normal CALLS */
    ENGINE_SVC_HTTP = 1,      /* Synchronous HTTP client call */
    ENGINE_SVC_ASYNC = 2,     /* Async dispatch (message broker, task queue) */
    ENGINE_SVC_CONFIG = 3,    /* Config/env accessor */
    ENGINE_SVC_ROUTE_REG = 4, /* Route registration (router.GET, app.get, Route::post) */
    ENGINE_SVC_GRPC = 5,      /* gRPC client call (protobuf stub invocation) */
    ENGINE_SVC_GRAPHQL = 6,   /* GraphQL client query/mutation */
    ENGINE_SVC_TRPC = 7,      /* tRPC client procedure call */
} engine_svc_kind_t;
 ...
 * Returns the pattern kind, or ENGINE_SVC_NONE if no match.
...
/* Per-worker TLS cache for engine_service_pattern_match results. The
...
#endif /* ENGINE_SERVICE_PATTERNS_H */
