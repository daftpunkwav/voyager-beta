...
// Forward declaration
static void walk_usages(EngineExtractCtx *ctx, TSNode root, const EngineLangSpec *spec);
...
static bool is_inside_call(TSNode node, const EngineLangSpec *spec) {
    TSNode cur = ts_node_parent(node);
    while (!ts_node_is_null(cur)) {
        if (engine_kind_in_set(cur, spec->call_node_types)) {
            return true;
        }
...
static bool is_inside_import(TSNode node, const EngineLangSpec *spec) {
    ...
    TSNode cur = ts_node_parent(node);
    while (!ts_node_is_null(cur)) {
        if ((has_imports && engine_kind_in_set(cur, spec->import_node_types)) ||
            (has_from_imports && engine_kind_in_set(cur, spec->import_from_types))) {
            return true;
        }
        cur = ts_node_parent(cur);
    }
...
static bool is_reference_node(TSNode node, EngineLanguage lang) {
...
typedef enum {
    ENGINE_OCCURRENCE_STANDARD = 0,
    ENGINE_OCCURRENCE_LISP_DEF,
    ENGINE_OCCURRENCE_COMMONLISP_DEFUN,
    ENGINE_OCCURRENCE_FENNEL_FN,
    ENGINE_OCCURRENCE_ELIXIR_DEF,
    ENGINE_OCCURRENCE_JULIA_FUNCTION,
    ENGINE_OCCURRENCE_WOLFRAM_SET,
    ENGINE_OCCURRENCE_TYPST_LET,
    ENGINE_OCCURRENCE_AGDA_FUNCTION,
    ENGINE_OCCURRENCE_TLAPLUS_OPERATOR,
    ENGINE_OCCURRENCE_COBOL_MOVE,
    ENGINE_OCCURRENCE_HCL_ATTRIBUTE,
    ENGINE_OCCURRENCE_ELM_VALUE,
    ENGINE_OCCURRENCE_RESCRIPT_LET,
    ENGINE_OCCURRENCE_PURESCRIPT_LHS,
    ENGINE_OCCURRENCE_NICKEL_LET,
    ENGINE_OCCURRENCE_ERLANG_CLAUSE,
    ENGINE_OCCURRENCE_NIX_FUNCTION,
    ENGINE_OCCURRENCE_MATLAB_ARGUMENTS,
    ENGINE_OCCURRENCE_LEAN_BINDER,
    ENGINE_OCCURRENCE_PASCAL_PROC,
    ENGINE_OCCURRENCE_TEAL_FUNCTION,
    ENGINE_OCCURRENCE_VHDL_INTERFACE,
    ENGINE_OCCURRENCE_PINE_FUNCTION,
    ENGINE_OCCURRENCE_LLVM_FUNCTION,
} EngineOccurrencePolicy;

typedef struct {
    const char *const *whole_binding_nodes;
    const char *const *write_nodes;
    EngineOccurrencePolicy policy;
    bool first_named_child_is_write;
} EngineOccurrenceSpec;
 ...
 * the positional EngineLangSpec initializer used by every language. */
...
static bool text_equals(EngineExtractCtx *ctx, TSNode node, const char *expected) {
    char *text = engine_node_text(ctx->arena, node, ctx->source);
...
static bool is_lisp_def_binding(EngineExtractCtx *ctx, TSNode node) {
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
        ...
        char *head = engine_node_text(ctx->arena, head_node, ctx->source);
...
static bool is_fennel_fn_binding(EngineExtractCtx *ctx, TSNode node) {
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
...
static bool is_elixir_def_binding(EngineExtractCtx *ctx, TSNode node) {
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
...
static bool fish_argument_marker(EngineExtractCtx *ctx, TSNode node) {
    char *text = engine_node_text(ctx->arena, node, ctx->source);
    return text && (strcmp(text, "-a") == 0 || strcmp(text, "--argument") == 0 ||
                    strcmp(text, "--argument-names") == 0);
}

static bool is_fish_function_parameter(EngineExtractCtx *ctx, TSNode node, WalkState *state) {
    ...
    for (TSNode previous = ts_node_prev_named_sibling(node); !ts_node_is_null(previous);
        ...
        char *text = engine_node_text(ctx->arena, previous, ctx->source);
        if (!text || text[0] != '-') {
            continue;
        }
...
static bool cfml_argument_tag_name_binding(EngineExtractCtx *ctx, TSNode node) {
    ...
    TSNode attribute_name = engine_find_child_by_kind(attribute, "cf_attribute_name");
    char *name_text = engine_node_text(ctx->arena, attribute_name, ctx->source);
    ...
    char *tag_text = engine_node_text(ctx->arena, tag, ctx->source);
...
static bool is_policy_binding(EngineExtractCtx *ctx, TSNode node,
                              const EngineOccurrenceSpec *occurrence) {
    switch (occurrence->policy) {
    case ENGINE_OCCURRENCE_LISP_DEF:
        return is_lisp_def_binding(ctx, node);
    case ENGINE_OCCURRENCE_COMMONLISP_DEFUN:
        return is_commonlisp_defun_binding(node);
    case ENGINE_OCCURRENCE_FENNEL_FN:
        return is_fennel_fn_binding(ctx, node);
    case ENGINE_OCCURRENCE_ELIXIR_DEF:
        return is_elixir_def_binding(ctx, node);
    case ENGINE_OCCURRENCE_JULIA_FUNCTION:
        return is_first_named_part_of(node, "function_definition");
    case ENGINE_OCCURRENCE_WOLFRAM_SET:
        return is_wolfram_lhs(node);
    case ENGINE_OCCURRENCE_TYPST_LET:
        return is_first_named_part_of(node, "let");
    case ENGINE_OCCURRENCE_AGDA_FUNCTION:
        for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
             parent = ts_node_parent(parent)) {
        ...
        return false;
    case ENGINE_OCCURRENCE_TLAPLUS_OPERATOR:
        return is_tlaplus_binding(node);
    case ENGINE_OCCURRENCE_COBOL_MOVE:
        return is_cobol_move_destination(node);
    case ENGINE_OCCURRENCE_HCL_ATTRIBUTE:
        return is_hcl_attribute_binding(node);
    case ENGINE_OCCURRENCE_ELM_VALUE:
        return is_first_named_part_of(node, "value_declaration");
    case ENGINE_OCCURRENCE_RESCRIPT_LET:
        return is_first_named_part_of(node, "let_binding");
    case ENGINE_OCCURRENCE_PURESCRIPT_LHS:
        return is_first_named_part_of(node, "function");
    case ENGINE_OCCURRENCE_NICKEL_LET:
        return is_first_named_part_of(node, "let_binding") ||
               is_first_named_part_of(node, "pattern_fun");
    case ENGINE_OCCURRENCE_ERLANG_CLAUSE:
        return is_erlang_clause_binding(node);
    case ENGINE_OCCURRENCE_NIX_FUNCTION:
        return is_nix_function_binding(node);
    case ENGINE_OCCURRENCE_MATLAB_ARGUMENTS:
        return is_matlab_argument_binding(node);
    case ENGINE_OCCURRENCE_LEAN_BINDER:
        return is_lean_binder_name(node);
    case ENGINE_OCCURRENCE_PASCAL_PROC:
        return is_pascal_proc_binding(node);
    case ENGINE_OCCURRENCE_TEAL_FUNCTION:
        return is_teal_function_binding(node);
    case ENGINE_OCCURRENCE_VHDL_INTERFACE:
        return is_vhdl_interface_binding(node);
    case ENGINE_OCCURRENCE_PINE_FUNCTION:
        return is_pine_function_binding(node);
    case ENGINE_OCCURRENCE_LLVM_FUNCTION:
        for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
             parent = ts_node_parent(parent)) {
        ...
        return false;
    case ENGINE_OCCURRENCE_STANDARD:
    default:
        return false;
    }
}
...
static bool is_binding_occurrence(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec,
                                  WalkState *state) {
    const EngineOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    ...
    while (!ts_node_is_null(current)) {
        ...
        bool declared_container =
            ...
            (spec->function_node_types && engine_kind_in_set(parent, spec->function_node_types)) ||
            (spec->class_node_types && engine_kind_in_set(parent, spec->class_node_types)) ||
            (spec->field_node_types && engine_kind_in_set(parent, spec->field_node_types)) ||
            variable_container;
...
static bool is_write_occurrence(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec,
                                WalkState *state) {
    const EngineOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    TSNode current = node;
    TSTreeCursor *cursor = reset_occurrence_cursor(state, node);
...
static bool language_may_stamp_exact_callable_value_candidate(EngineLanguage language) {
    switch (language) {
    case ENGINE_LANG_JAVASCRIPT:
    case ENGINE_LANG_TYPESCRIPT:
    case ENGINE_LANG_TSX:
...
static TSNode call_reference_candidate_site(EngineExtractCtx *ctx, TSNode node, const char *name,
                                            WalkState *state) {
...
static char *reference_name(EngineExtractCtx *ctx, TSNode node) {
    ...
    return parenthesized ? engine_arena_strndup(ctx->arena, name + 2, length - 3) : name;
}
...
static const char *lexical_binding_key(EngineExtractCtx *ctx, WalkState *state, const char *name) {
    if (!ctx || !name) {
        return name;
    }
    ...
    char *folded = (char *)engine_arena_alloc(ctx->arena, n + 1U);
    if (!folded) {
        if (state) {
...
static void stamp_usage_site(EngineExtractCtx *ctx, EngineUsage *usage, TSNode node, const char *name,
                             WalkState *state) {
...
static const EngineLexicalScope *usage_lexical_scope(const WalkState *state, uint32_t id) {
...
static uint32_t lexical_ancestor_of_kind(const WalkState *state, uint32_t start_id,
    ...
    while (id != 0 && remaining-- > 0) {
        const EngineLexicalScope *scope = usage_lexical_scope(state, id);
        if (!scope) {
            return 0;
        }
        bool function_scope = scope->kind == ENGINE_LEXICAL_SCOPE_FUNCTION ||
                              scope->kind == ENGINE_LEXICAL_SCOPE_COMPREHENSION;
        bool block_scope = scope->kind == ENGINE_LEXICAL_SCOPE_BLOCK;
...
static uint32_t python_nearest_namespace(const WalkState *state, uint32_t start_id) {
    ...
    while (id != 0 && remaining-- > 0) {
        ...
        if (scope->kind == ENGINE_LEXICAL_SCOPE_FUNCTION ||
            scope->kind == ENGINE_LEXICAL_SCOPE_COMPREHENSION ||
            scope->kind == ENGINE_LEXICAL_SCOPE_CLASS || scope->kind == ENGINE_LEXICAL_SCOPE_MODULE) {
            return id;
        }
        id = scope->parent_id;
    }
...
static bool ensure_lexical_binding_capacity(WalkState *state) {
    ...
    EngineLexicalBinding *grown =
        (EngineLexicalBinding *)engine_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        state->lexical_binding_tracking_failed = true;
        return false;
    }
...
static TSNode declared_function_name_owner(TSNode node, const EngineLangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        if (spec->function_node_types && engine_kind_in_set(parent, spec->function_node_types)) {
...
static TSNode declared_class_name_owner(TSNode node, const EngineLangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        if (spec->class_node_types && engine_kind_in_set(parent, spec->class_node_types)) {
            return field_contains_node(parent, "name", node) ? parent : (TSNode){0};
        }
    }
    return (TSNode){0};
...
static bool ensure_python_directive_capacity(WalkState *state) {
    ...
    EnginePythonDirective *grown =
        (EnginePythonDirective *)engine_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        state->lexical_binding_tracking_failed = true;
        return false;
    }
...
static void record_python_directive(WalkState *state, uint32_t function_scope_id, const char *name,
    ...
    EnginePythonDirective *directive = &state->python_directives[state->python_directive_count++];
    directive->function_scope_id = function_scope_id;
    directive->name = name;
    directive->kind = (uint8_t)kind;
}

static EnginePythonDirectiveKind python_directive_for(const WalkState *state,
                                                   uint32_t function_scope_id, const char *name) {
    for (int i = state ? state->python_directive_count - 1 : -1; i >= 0; i--) {
        const EnginePythonDirective *directive = &state->python_directives[i];
        if (directive->function_scope_id == function_scope_id && directive->name &&
            strcmp(directive->name, name) == 0) {
            return (EnginePythonDirectiveKind)directive->kind;
        }
    }
    return 0;
}
...
static uint32_t python_enclosing_function_namespace(const WalkState *state,
    ...
    const EngineLexicalScope *inner = usage_lexical_scope(state, inner_function_id);
...
static bool binding_is_parameter(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec) {
    const EngineOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        const char *kind = ts_node_type(parent);
...
static bool import_kind_matches(TSNode node, const EngineLangSpec *spec) {
    return (spec->import_node_types && engine_kind_in_set(node, spec->import_node_types)) ||
           (spec->import_from_types && engine_kind_in_set(node, spec->import_from_types));
}

static TSNode nearest_import_ancestor(TSNode node, const EngineLangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
 ...
 * name. */
static bool is_import_binding_occurrence(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec) {
...
static uint32_t lexical_import_scope(const WalkState *state, uint32_t start_id) {
    ...
    while (id != 0 && remaining-- > 0) {
        ...
        if (scope->kind == ENGINE_LEXICAL_SCOPE_MODULE || scope->kind == ENGINE_LEXICAL_SCOPE_FUNCTION ||
            scope->kind == ENGINE_LEXICAL_SCOPE_COMPREHENSION ||
            scope->kind == ENGINE_LEXICAL_SCOPE_BLOCK) {
            return id;
        }
        id = scope->parent_id;
    }
...
static void record_lexical_binding(EngineExtractCtx *ctx, WalkState *state, TSNode node,
                                   const EngineLangSpec *spec, const char *raw_name,
                                   bool import_binding) {
    ...
    if (1 && (lexical_ancestor_kind(node, "global_statement") ||
        ...
        EnginePythonDirectiveKind directive = lexical_ancestor_kind(node, "global_statement")
                                               ? ENGINE_PYTHON_DIRECTIVE_GLOBAL
                                               : ENGINE_PYTHON_DIRECTIVE_NONLOCAL;
    ...
    if (!ts_node_is_null(function_declaration)) {
        ...
        uint32_t function_id =
            namespace_scope && (namespace_scope->kind == ENGINE_LEXICAL_SCOPE_FUNCTION ||
                                namespace_scope->kind == ENGINE_LEXICAL_SCOPE_COMPREHENSION)
                ? namespace_id
                : lexical_ancestor_of_kind(state, current_id, true, false);
        EnginePythonDirectiveKind directive = python_directive_for(state, function_id, name);
        if (directive == ENGINE_PYTHON_DIRECTIVE_GLOBAL) {
            scope_id = state->root_lexical_scope_id;
        } else if (directive == ENGINE_PYTHON_DIRECTIVE_NONLOCAL) {
    ...
    const EngineLexicalScope *scope = usage_lexical_scope(state, scope_id);
    if (!scope || !ensure_lexical_binding_capacity(state)) {
        return;
    }
    EngineLexicalBinding *binding = &state->lexical_bindings[state->lexical_binding_count++];
...
static int compare_lexical_bindings(const void *left_ptr, const void *right_ptr) {
    const EngineLexicalBinding *left = (const EngineLexicalBinding *)left_ptr;
    const EngineLexicalBinding *right = (const EngineLexicalBinding *)right_ptr;
...
static int compare_binding_key(const EngineLexicalBinding *binding, uint32_t scope_id,
                               const char *name) {
...
static bool lexical_binding_active_at(const WalkState *state, uint32_t scope_id, const char *name,
    ...
    while (index < state->lexical_binding_count) {
        const EngineLexicalBinding *binding = &state->lexical_bindings[index++];
...
static const EngineLexicalScope *lexical_binding_scope_for_usage(const WalkState *state,
...
void engine_finalize_lexical_usages(EngineExtractCtx *ctx, WalkState *state) {
    if (!ctx || !state) {
        return;
    }
    ...
    for (int i = state->usage_start_index; i < ctx->result->usages.count; i++) {
        EngineUsage *usage = &ctx->result->usages.items[i];
        ...
        const EngineLexicalScope *binding_scope = lexical_binding_scope_for_usage(state, usage, name);
        if (!binding_scope) {
            continue;
        }
        usage->semantic_reference_blocked = true;
        usage->semantic_reference_local_shadow = binding_scope->kind != ENGINE_LEXICAL_SCOPE_MODULE;
    }
    ...
    if (state->lexical_binding_tracking_failed) {
        for (int i = state->usage_start_index; i < ctx->result->usages.count; i++) {
            EngineUsage *usage = &ctx->result->usages.items[i];
            usage->semantic_reference_blocked = true;
            const EngineLexicalScope *scope = usage_lexical_scope(state, usage->lexical_scope_id);
            usage->semantic_reference_local_shadow =
                scope && scope->kind != ENGINE_LEXICAL_SCOPE_MODULE;
        }
    }
}
...
static char *perl_direct_coderef_name(EngineExtractCtx *ctx, TSNode node) {
    ...
    return end > start ? engine_arena_strndup(ctx->arena, start, (size_t)(end - start)) : NULL;
}
...
static bool emit_direct_perl_coderef_usage(EngineExtractCtx *ctx, TSNode node,
    ...
    engine_usages_push(&ctx->result->usages, ctx->arena, usage);
    return true;
}

// Try to emit a usage for a reference node. Returns early if the node should be skipped.
static void try_emit_usage(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec) {
    if (emit_direct_perl_coderef_usage(ctx, node, engine_enclosing_func_qn_cached(ctx, node), 0)) {
        return;
    }
    ...
    if (name && name[0] && !engine_is_keyword(name, ctx->language)) {
        EngineUsage usage = {0};
        usage.ref_name = name;
        usage.enclosing_func_qn = engine_enclosing_func_qn_cached(ctx, node);
...
void handle_usages(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec, WalkState *state) {
    ...
    if (exact_callee) {
        if (reference_node && state->invocation_kind == ENGINE_INVOCATION_CALLABLE_REFERENCE) {
            char *name = reference_name(ctx, node);
            if (name && name[0] && !engine_is_keyword(name, ctx->language)) {
                EngineUsage usage = {0};
                usage.ref_name = name;
    ...
    if (state->inside_import) {
        if (is_import_binding_occurrence(ctx, node, spec)) {
            ...
            if (binding_name && binding_name[0] && !engine_is_keyword(binding_name, ctx->language)) {
