...
if [[ "$BINARY" == *.exe ]] && command -v cygpath >/dev/null 2>&1 &&
    ...
    WIN_ROOT=$(mktemp -d "$(cygpath "$USERPROFILE")/engine-memlab.XXXXXX")
    ...
    cp "$BINARY" "$WIN_ROOT/graph-engine.exe"
    BINARY="$WIN_ROOT/graph-engine.exe"
    WORK="$WIN_ROOT"
...
if command -v cygpath >/dev/null 2>&1 && ! command -v winepath >/dev/null 2>&1; then
    mkdir -p "$WORK/cache"
    export ENGINE_CACHE_DIR="$(cygpath -w "$WORK/cache")"
else
    export ENGINE_CACHE_DIR="$WORK/cache"
fi
export ENGINE_MEM_PROFILE=1
export ENGINE_MEM_PROFILE_OUT="$PROFILE_OUT"
export ENGINE_MEM_CENSUS=1
export ENGINE_LOG_LEVEL=info
export ENGINE_LOG_FORMAT=text
...
RC=$?
# With ENGINE_CACHE_DIR set the process logs to its own file rather than stderr,
...
if [ "$CENSUS" -eq 0 ]; then
    echo "WARN: no census samples — check ENGINE_MEM_CENSUS wiring" >&2
fi
if [ "$SITES" -eq 0 ]; then
