...
#   <binary>  product binary to smoke (e.g. build/c/graph-engine)
...
Environment:
  ENGINE_SMOKE_ARTIFACT_DIR   Release mode: an EXTRACTED release artifact
...
smoke-linux-portable (with ENGINE_SMOKE_ARTIFACT_DIR) · compose smoke services.
...
ARTIFACT_DIR="${ENGINE_SMOKE_ARTIFACT_DIR:-}"
if [ -n "$ARTIFACT_DIR" ]; then
    ARTIFACT_DIR="$(cd "$ARTIFACT_DIR" && pwd)"
    for required in engine-integrations.json LICENSE install.sh THIRD_PARTY_NOTICES.md; do
...
shopt -s nullglob
UI_PACK_CANDIDATES=("$UI_PACK_SOURCE_DIR"/engine-ui-*.pack)
shopt -u nullglob
...
WORK_DIR=$(mktemp -d "${TMPDIR:-/tmp}/engine-smoke-server-XXXXXX")
...
cp "$BINARY" "$FIXTURE_DIR/graph-engine"
if [ -n "$ARTIFACT_DIR" ]; then
    cp "$ARTIFACT_DIR/engine-integrations.json" "$ARTIFACT_DIR/LICENSE" \
        "$ARTIFACT_DIR/install.sh" "$ARTIFACT_DIR/THIRD_PARTY_NOTICES.md" "$FIXTURE_DIR/"
else
    cp "$ROOT/assets/engine-integrations.json" "$ROOT/LICENSE" "$ROOT/install.sh" "$FIXTURE_DIR/"
...
# The archive must carry engine-integrations.json — install verifies it against the
...
EXPECTED_ARTIFACT="graph-engine${SUFFIX}-${OS}-${ARCH}.tar.gz"
ARCHIVE_MEMBERS=(
    graph-engine engine-integrations.json LICENSE install.sh
...
if [ "$OS" = "linux" ]; then
    cp "$FIXTURE_DIR/$EXPECTED_ARTIFACT" \
        "$FIXTURE_DIR/graph-engine${SUFFIX}-${OS}-${ARCH}-portable.tar.gz"
fi
...
env \
    ...
    -u GLAB_CONFIG_DIR \
    -u KIMI_CODE_HOME \
    -u ENGINE_CONTINUE_CONFIG_PATH \
    -u ENGINE_TRAE_CONFIG_PATH \
    -u ENGINE_ROO_CONFIG_PATH \
    -u ENGINE_CODY_CONFIG_PATH \
    -u ENGINE_TEST_WINDOWS_USER_PATH_RUN_ID \
    HOME="$SMOKE_HOME" \
    ...
    TMP="$SMOKE_TEMP_DIR" \
    SHELL="/bin/sh" \
    ENGINE_CACHE_DIR="$WORK_DIR/cache" \
