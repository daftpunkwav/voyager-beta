<#
.SYNOPSIS
    Run the native-Windows product-surface test suite for graph-engine.
...
.DESCRIPTION
    ...
    Two categories of test:
      ...
      GUARDS      - regression guards for Windows bugs already fixed on main.
                    ...
                    regressed and fails this runner.
                      ...
                      * test_non_ascii_cache_dump.py guards #996 (writer engine_fopen)
                      ...
                      * test_daemon_open_readiness.py guards that
                        `daemon start --open` waits for the verified engine UI
    ...
    under the mandatory coordination daemon ENGINE_INDEX_SUPERVISOR=0 is a
...
.PARAMETER Target
    Makefile target used when building: 'engine-with-ui' (default; needed for the
    drive-picker guard's UI-enabled HTTP service) or 'engine' (no UI - the drive guard then
...
.EXAMPLE
    pwsh -File scripts/test-windows.ps1 -GuardsOnly -Binary build\c\graph-engine.exe
#>
[CmdletBinding()]
param(
    [string]$Binary,
    [ValidateSet("engine-with-ui", "engine")]
    [string]$Target = "engine-with-ui",
    [switch]$GuardsOnly,
    [string]$Make = "make"
)
...
function Resolve-Binary {
    ...
    $built = Join-Path $repoRoot "build\c\graph-engine.exe"
    Write-Host "Building $Target via Makefile ..." -ForegroundColor Cyan
    $makeArgs = @(
        "-j", "-f", "Makefile", $Target, "SANITIZE=",
        "TMP=$tmp", "TEMP=$tmp", "TMPDIR=$tmp"
    )
    if ($Target -eq "engine-with-ui") { $makeArgs += "TEST_SEAMS=1" }
...
try {
    ...
    $guardRoot = Join-Path $userProfile ("engine-windows-guards-root-" + [guid]::NewGuid().ToString("N"))
    ...
    $guardBundle = Join-Path $guardRoot ("engine-windows-guards-" + [guid]::NewGuid().ToString("N"))
    New-Item -ItemType Directory -Path $guardBundle | Out-Null
    $guardBin = Join-Path $guardBundle "graph-engine.exe"
    Copy-Item -LiteralPath $bin -Destination $guardBin
    $sourceRuntimeDir = Split-Path -Parent $bin
    $sourceIntegration = Join-Path $sourceRuntimeDir "engine-integrations.json"
    if (-not (Test-Path -LiteralPath $sourceIntegration -PathType Leaf)) {
        throw "Windows guard source runtime is missing engine-integrations.json beside $bin"
    }
    Copy-Item -LiteralPath $sourceIntegration -Destination $guardBundle
    Get-ChildItem -LiteralPath $sourceRuntimeDir -File -Filter "engine-ui-*.pack" |
...
# build and its external asset pack (target engine-with-ui); against a non-UI
...
if (Get-ChildItem -LiteralPath $guardBundle -File -Filter "engine-ui-*.pack") {
...
if ($guardSkips.Count -gt 0) {
    ...
    Write-Host "needs a UI build (-Target engine-with-ui, the default)." -ForegroundColor Yellow
}
