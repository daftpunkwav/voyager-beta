#!/usr/bin/env bash
# soak-test.sh — Endurance test for graph-engine.
#
...
Environment:
  ENGINE_SOAK_MODE   default | query-leak (#581 detector: never reindex/mutate
...
#                 mutate files, so the mimalloc page-return path (engine_mem_collect,
...
ENGINE_SOAK_MODE="${ENGINE_SOAK_MODE:-default}"

RESULTS_DIR="${RESULTS_DIR:-soak-results}"
mkdir -p "$RESULTS_DIR"

# Isolate daemon coordination from interactive engine sessions and give this run
...
if [[ "$BINARY" == *.exe ]] && command -v cygpath >/dev/null 2>&1 &&
    ...
    SOAK_WIN_ROOT=$(mktemp -d "$(cygpath "$USERPROFILE")/engine-soak.XXXXXX")
    if ! soak_stamp_windows_dir "$SOAK_WIN_ROOT"; then
        rm -rf -- "$SOAK_WIN_ROOT"
        exit 1
    fi
    cp "$BINARY" "$SOAK_WIN_ROOT/graph-engine.exe"
    BINARY="$SOAK_WIN_ROOT/graph-engine.exe"
    ...
    SOAK_CACHE_DIR_HOST=$(mktemp -d "${TMPDIR:-/tmp}/engine-soak-cache.XXXXXX")
fi
...
DAEMON_LOG="$SOAK_CACHE_DIR_HOST/logs/engine-daemon.log"
DIAG_FILE=""
...
echo "=== soak-test: binary=$BINARY duration=${DURATION_MIN}m mode=${ENGINE_SOAK_MODE} ==="
...
soak_cleanup() {
    ...
    if [ -f "$DAEMON_LOG" ]; then
        cp "$DAEMON_LOG" "$RESULTS_DIR/engine-daemon.log" 2>/dev/null || true
    fi
...
start_mcp_server() {
    ...
    if $SOAK_NATIVE_WINDOWS; then
        unset ENGINE_SOAK_SERVER ENGINE_SOAK_SERVER_PID || true
        if [ "$stderr_mode" = "append" ]; then
            eval 'coproc ENGINE_SOAK_SERVER {
                export ENGINE_CACHE_DIR="$SOAK_CACHE_DIR_VALUE"
                export ENGINE_DIAGNOSTICS=1 ENGINE_LOG_LEVEL=info ENGINE_LOG_FORMAT=text
        ...
        SERVER_PID=$ENGINE_SOAK_SERVER_PID
        local server_read_fd="${ENGINE_SOAK_SERVER[0]}"
        local server_write_fd="${ENGINE_SOAK_SERVER[1]}"
    ...
    if [ "$stderr_mode" = "append" ]; then
        ENGINE_CACHE_DIR="$SOAK_CACHE_DIR_VALUE" ENGINE_DIAGNOSTICS=1 ENGINE_LOG_LEVEL=info \
            ENGINE_LOG_FORMAT=text "$BINARY" < "$SERVER_IN" > "$SERVER_OUT" \
...
# This mirrors the path canonicalization actually performed by engine instead of
...
refresh_diagnostics_paths() {
    ...
    # The discovery record is JSON (a control record survives ENGINE_LOG_LEVEL
...
while [ "$(date +%s)" -lt "$END_TIME" ]; do
    ...
    if [ "$ENGINE_SOAK_MODE" = "query-leak" ]; then
        # ── #581 query-only leak mode ────────────────────────────────
        # Pure read-query hammering: no mutation, no reindex — so
        # engine_mem_collect (mimalloc page return) is NEVER triggered and
...
while [ "$churn_index" -lt "$CHURN_CYCLES" ]; do
    if ! ENGINE_CACHE_DIR="$SOAK_CACHE_DIR_VALUE" "$BINARY" cli list_projects '{}' \
...
# index_repository), which triggers engine_mem_collect and would mask the #581
# query-only leak the whole run is trying to surface.

if [ "$SKIP_CRASH" != "--skip-crash-test" ] && [ "$ENGINE_SOAK_MODE" != "query-leak" ]; then
