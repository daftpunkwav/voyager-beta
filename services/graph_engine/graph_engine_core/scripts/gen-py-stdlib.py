...
"""
gen-py-stdlib.py — Generate engine_python_stdlib_register from typeshed stubs.
...
source file (internal/engine/lsp/generated/python_stdlib_data.c) that
populates a EngineTypeRegistry with classes + their method names and
...
internal/engine/lsp/generated/python_stdlib_data.c when run without args.
"""
...
#define ENGINE_PYTHON_STDLIB_GENERATED 1
...
def emit(stubs: list[ModuleStubs], output_path: Path) -> None:
    ...
    for ms in stubs:
        ...
        for cls in ms.classes:
            ...
            out_lines.append("    engine_registry_add_type(reg, rt);\n")
            ...
            for m in cls.methods:
                ...
                out_lines.append("    engine_registry_add_func(reg, rf);\n")
...
def main() -> int:
    ...
    parser.add_argument(
        "output",
        nargs="?",
        default="internal/engine/lsp/generated/python_stdlib_data.c",
    )
