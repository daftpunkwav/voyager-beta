...
GRAMMAR_DIR = os.path.join(PROJECT_DIR, "internal", "engine")
...
def generate_enum(langs):
    """Print enum entries for engine.h."""
    print("\n=== Enum Entries (paste into engine.h before ENGINE_LANG_KUSTOMIZE) ===")
    for lang in langs:
        print(f"    ENGINE_LANG_{lang['enum']},")
...
def generate_specs(langs):
    ...
    for lang in langs:
        ...
        print(f"    // ENGINE_LANG_{lang['enum']}")
        print(
            f"    [ENGINE_LANG_{lang['enum']}] = {{ENGINE_LANG_{lang['enum']}, "
...
def generate_language_c(langs):
    ...
    for ext, enum, display in sorted(ext_entries, key=lambda x: x[0].lower()):
        print(f'    /* {display} */')
        print(f'    {{"{ext}", ENGINE_LANG_{enum}}},')
        print()

    print("\n=== FILENAME_TABLE Entries ===")
    for lang in langs:
        for fn in lang["filenames"]:
            print(f'    {{"{fn}", ENGINE_LANG_{lang["enum"]}}},')

    print("\n=== LANG_NAMES Entries ===")
    for lang in langs:
        print(
            f'    [ENGINE_LANG_{lang["enum"]}] = "{lang["display"]}",'
        )
