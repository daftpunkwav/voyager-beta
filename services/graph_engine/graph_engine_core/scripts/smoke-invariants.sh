...
#
# A comprehensive, fast, portable smoke battery for the graph-engine
...
#
# Usage:
#   scripts/smoke-invariants.sh <binary>        # e.g. build/c/graph-engine(.exe)
#
# Portability notes:
...
SCRATCH="$(mktemp -d 2>/dev/null || mktemp -d -t enginesmoke)"
cleanup() {
...
inv_help() {
    ...
    if printf '%s' "$RB_OUT" | grep -qiE 'usage|graph-engine'; then
        pass "help"
    else
...
GOEOF
    ...
    # Project name derivation mirrors engine_project_name_from_path: every char not
...
inv_second_index_inprocess() {
    local base
    base=$(mktemp -d "${TMPDIR:-/tmp}/engine_inv773.XXXXXX")
    ...
    run_bounded 120 env ENGINE_INDEX_SUPERVISOR=0 sh -c 'exec "$1" < "$2"' _ "$BINARY" "$infile"
    local rc="$RB_RC"
    local out="$RB_OUT"
    rm -rf "$base"
    if [ "$rc" -gt 128 ]; then
...
inv_every_tool() {
    ...
    CALLS=(
        ...
        "delete_project|{\"project\":\"__engine_smoke_nonexistent__\"}"
    )
    ...
    local ureq="{\"jsonrpc\":\"2.0\",\"id\":$((MCP_ID++)),\"method\":\"tools/call\",\"params\":{\"name\":\"__engine_no_such_tool__\",\"arguments\":{}}}"
...
inv_malformed_input() {
    ...
    BADLINES=(
        ...
        '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"index_repository","arguments":{"repo_path":"/engine/does/not/exist/xyz"}}}'
...
inv_nonexistent_repo_cli() {
    cli_call 30 --json index_repository '{"repo_path":"/engine/definitely/not/here/zzz"}'
    if [ "$CLI_RC" -eq 124 ]; then
...
# (ENGINE_TEST_CRASH_ON) so the guard is honest: with the supervisor OFF the crash
...
inv_crasher_skipped_cli() {
    ...
    export ENGINE_TEST_CRASH_ON=crash_me
    export ENGINE_INDEX_SUPERVISOR=0
    cli_call 60 index_repository "{\"repo_path\":\"$cn\"}"
    local base_rc="$CLI_RC"
    unset ENGINE_INDEX_SUPERVISOR
...
# progress (external-scanner infinite loop, modelled by ENGINE_TEST_HANG_ON's busy-
...
# ENGINE_INDEX_WORKER_TIMEOUT_S the run must COMPLETE (rc<128, not 124), report
...
inv_hanger_skipped_cli() {
    ...
    export ENGINE_TEST_HANG_ON=hang_me
    export ENGINE_INDEX_SUPERVISOR=0
    ...
    export ENGINE_INDEX_WORKER_TIMEOUT_S=5
    cli_call 90 index_repository "{\"repo_path\":\"$hn\"}"
    local sup_rc="$CLI_RC"
    local sup_out="$CLI_OUT"
    unset ENGINE_INDEX_WORKER_TIMEOUT_S
    unset ENGINE_TEST_HANG_ON
