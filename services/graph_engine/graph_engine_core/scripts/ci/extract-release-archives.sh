...
#   associations.tsv  every extracted member and EngineUIPK asset -> scan object
...
# uncompressed HTML/JavaScript/CSS/etc payloads inside every EngineUIPK v1 pack are
...
UI_ARCHIVES = frozenset(
    [
        f"graph-engine-ui-{target}.tar.gz"
        for target in UNIX_TARGETS
    ]
    + [
        f"graph-engine-ui-{target}.zip"
        for target in WINDOWS_TARGETS
    ]
)
STANDARD_ARCHIVES = frozenset(
    [f"graph-engine-{target}.tar.gz" for target in UNIX_TARGETS]
    + [f"graph-engine-{target}.zip" for target in WINDOWS_TARGETS]
)
CANONICAL_ARCHIVES = UI_ARCHIVES | STANDARD_ARCHIVES
PACK_NAME = re.compile(r"engine-ui-([0-9a-f]{64})\.pack\Z")
...
def validate_namespace(archive_name: str, names: Iterable[str]) -> Tuple[str, Dict[str, str]]:
    ...
    variant = "ui" if archive_name.startswith("graph-engine-ui-") else "standard"
    binary = "graph-engine.exe" if windows else "graph-engine"
    installer = "install.ps1" if windows else "install.sh"
    fixed = {
        binary: "binary",
        "engine-integrations.json": "runtime",
        "LICENSE": "runtime",
        installer: "runtime",
...
def parse_pack_assets(path: pathlib.Path) -> List[PackAsset]:
    ...
    if size < PACK_HEADER_BYTES or size > MAX_PACK_BYTES:
        raise ContractError(f"EngineUIPK size is outside bounds: {size}")
    with path.open("rb") as handle:
        header = handle.read(PACK_HEADER_BYTES)
        if len(header) != PACK_HEADER_BYTES:
            raise ContractError("EngineUIPK header is truncated")
        try:
            (
                magic,
                version,
            ...
            raise ContractError(f"EngineUIPK header is malformed: {error}") from error
        if (
            magic != b"EngineUIPK\0"
            or version != 1
            ...
            raise ContractError("EngineUIPK header/offset contract failed")
        handle.seek(index_offset)
        index = handle.read(index_size)
        paths = handle.read(paths_size)
        if len(index) != index_size or len(paths) != paths_size:
            raise ContractError("EngineUIPK index or path table is truncated")
    ...
    for item in range(count):
        ...
        if (
            ...
            raise ContractError("EngineUIPK entry bounds/contiguity contract failed")
        ...
        if not valid_asset_path(raw_path) or (previous_path is not None and previous_path >= raw_path):
            raise ContractError("EngineUIPK asset path is invalid, duplicate or unsorted")
        try:
            asset_path = raw_path.decode("ascii")
        except UnicodeDecodeError as error:
            raise ContractError("EngineUIPK asset path is not ASCII") from error
        ...
        if mapping is None or mime_id != mapping[0]:
            raise ContractError(f"EngineUIPK MIME/path contract failed: {asset_path}")
        is_index = asset_path == "/index.html"
        if cache_id != (1 if is_index else 2) or (is_index and has_index):
            raise ContractError(f"EngineUIPK cache/index contract failed: {asset_path}")
    ...
    if not has_index or expected_path_offset != paths_size or expected_data_offset != payload_size:
        raise ContractError("EngineUIPK tables do not cover exactly one complete payload")
    return assets
...
def main(argv: Sequence[str]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix=".engine-release-scan-", dir=str(output_dir.parent)) as temporary:
        ...
        write_tsv(
            ...
            marker="engine-release-scan-associations-v3",
        ...
        write_tsv(
            ...
            marker="engine-release-scan-set-v2",
