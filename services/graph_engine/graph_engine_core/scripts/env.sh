...
# ENGINE_ARCH env var or --arch flag override (parsed by calling script)
ARCH="${ENGINE_ARCH:-$HW_ARCH}"
...
# Makefile folds $(ARCHFLAGS) into CC/CXX so it reaches every compile and
...
verify_compiler() {
    ...
    if [[ "$OS" == "darwin" ]]; then
        local probe_out
        probe_out="$(mktemp -t engine-archprobe.XXXXXX)"
...
# ── Verified compiler cache (ccache, opt-out ENGINE_NO_CCACHE=1) ──
...
if [[ "${ENGINE_NO_CCACHE:-0}" != "1" ]] && command -v ccache >/dev/null 2>&1; then
    for _engine_ccache_masq in \
        /usr/lib/ccache \
        /opt/homebrew/opt/ccache/libexec \
        /usr/local/opt/ccache/libexec \
        /clang64/lib/ccache/bin \
        /clangarm64/lib/ccache/bin; do
        if [[ -d "$_engine_ccache_masq" ]]; then
            case ":$PATH:" in
            *":$_engine_ccache_masq:"*) ;;
            *) PATH="$_engine_ccache_masq:$PATH" ;;
            esac
        fi
    done
    unset _engine_ccache_masq
    export PATH
