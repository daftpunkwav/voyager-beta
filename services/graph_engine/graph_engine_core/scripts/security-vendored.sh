...
#
# Scope: every regular file under vendored/ and internal/engine/vendored/.
#
...
INTERNAL_VENDORED_ROOT="$ROOT/internal/engine/vendored"
...
MANIFEST_PATHS="$(mktemp "${TMPDIR:-/tmp}/engine-vendored-paths.XXXXXX")" || {
    echo "BLOCKED: cannot create checksum verification workspace"
    exit 1
}
VENDORED_FILES="$(mktemp "${TMPDIR:-/tmp}/engine-vendored-files.XXXXXX")" || {
    rm -f "$MANIFEST_PATHS"
    echo "BLOCKED: cannot create vendored inventory workspace"
    exit 1
}
VENDORED_SYMLINKS="$(mktemp "${TMPDIR:-/tmp}/engine-vendored-links.XXXXXX")" || {
...
valid_vendored_path() {
    local path="$1"
    [[ "$path" == vendored/* || "$path" == internal/engine/vendored/* ]] || return 1
    [[ "$path" != *'\\'* ]] || return 1
    [[ "$path" != *'//'* ]] || return 1
    [[ "$path" != *'/./'* && "$path" != */. ]] || return 1
    [[ "$path" != *'/../'* && "$path" != */.. ]] || return 1
...
# Also scan tree-sitter grammars (internal/engine/vendored/) — 650MB, 20M lines.
# Use fixed-string grep for each pattern to avoid slow regex on the large tree.
if [[ -d "$ROOT/internal/engine/vendored" ]]; then
    GRAMMAR_FAIL=false
    for pattern in 'system(' 'popen(' 'execl(' 'execv(' 'fork(' 'connect(' 'socket(' 'sendto(' 'dlopen(' 'LoadLibrary('; do
        HITS="$(grep -rl -F "$pattern" "$ROOT/internal/engine/vendored/" --include='*.c' --include='*.h' 2>/dev/null | head -3 || true)"
        if [[ -n "$HITS" ]]; then
