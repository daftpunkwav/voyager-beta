...
local artifact-flow smoke lane.
  ...
  --variant  standard (default) | ui — selects the archive NAME prefix; the
             ...
             UI archives add exactly one root-level engine-ui-<sha256>.pack.
...
Archive contents (defined here, canonical) — ONE executable per runtime set:
  unix:    graph-engine engine-integrations.json LICENSE install.sh
           THIRD_PARTY_NOTICES.md [engine-ui-<sha256>.pack] (.tar.gz)
  windows: graph-engine.exe engine-integrations.json LICENSE install.ps1
           THIRD_PARTY_NOTICES.md [engine-ui-<sha256>.pack] (.zip)
...
engine-integrations.json is the integration-template data file: the binary
...
NAME="graph-engine${SUFFIX}-${GOOS}-${GOARCH}"
...
if [ "$VARIANT" = "ui" ]; then
    shopt -s nullglob
    UI_PACK_CANDIDATES=("$BUILD_DIR"/engine-ui-*.pack)
    shopt -u nullglob
    if [ "${#UI_PACK_CANDIDATES[@]}" -ne 1 ]; then
        echo "package-release: ui build must contain exactly one engine-ui-<sha256>.pack" >&2
        exit 2
    fi
    UI_PACK_SOURCE="${UI_PACK_CANDIDATES[0]}"
    UI_PACK_NAME="$(basename "$UI_PACK_SOURCE")"
    if ! [[ "$UI_PACK_NAME" =~ ^engine-ui-[0-9a-f]{64}\.pack$ ]] || [ ! -s "$UI_PACK_SOURCE" ]; then
...
PACK_DIR="$(mktemp -d "$BUILD_DIR/.engine-package.XXXXXX")"
...
cp assets/engine-integrations.json "$PACK_DIR/engine-integrations.json"
if [ "$VARIANT" = "ui" ]; then
...
if [ "$GOOS" = "windows" ]; then
    (
        ...
        ARCHIVE_MEMBERS=(
            graph-engine.exe engine-integrations.json LICENSE install.ps1
