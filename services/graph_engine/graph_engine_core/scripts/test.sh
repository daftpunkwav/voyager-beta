...
Environment:
  ENGINE_TEST_SEQUENTIAL=1   Single-process runner instead of the parallel harness.
  ENGINE_RUN_HANG_TEST=1     Opt-in C++ index-hang regression (#410, needs prod).
  ENGINE_NO_CCACHE=1         Disable the content-verified compiler cache.
  ENGINE_TEST_SHARD/_LEG     Set by CI's sharded legs; leave unset locally.
...
if [ "$TSAN" -eq 1 ]; then
    ...
    make -j"$NPROC" -f Makefile "$BUILD_DIR/test-runner-tsan" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
    make -f Makefile test-tsan ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
    exit "$?"
fi
...
if [ -n "$SUITES" ]; then
    ...
    make -j"$NPROC" -f Makefile "$BUILD_DIR/test-runner" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
...
# ENGINE_TEST_SEQUENTIAL=1 restores the single-process runner.
make -j"$NPROC" -f Makefile "$BUILD_DIR/test-runner" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
if [ "${ENGINE_TEST_SEQUENTIAL:-0}" = "1" ]; then
    make -f Makefile test ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
else
    make -f Makefile test-par ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
fi
...
# Opt-in via ENGINE_RUN_HANG_TEST=1 (it needs the prod binary, which the ASan unit
# run above does not build). Skipped by default so the fast unit run stays fast.
if [ "${ENGINE_RUN_HANG_TEST:-0}" = "1" ]; then
...
make -j"$NPROC" -f Makefile engine TEST_SEAMS=1 ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
WATCHDOG_BINARY="$ROOT/$BUILD_DIR/graph-engine"
ENGINE_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_parent_watchdog.sh"
...
ENGINE_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_worker_watchdog.sh"
...
ENGINE_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_worker_error_response.sh"
...
# ENGINE_TEST_HOOK_CLIENT_BUILD seam and asserts the stdout systemMessage. It is
...
#     ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
# python3 "$ROOT/tests/test_daemon_open_readiness.py" "$ROOT/$BUILD_DIR/graph-engine"
