...
FUNC_LIST="system popen engine_popen execl fork"
...
while IFS= read -r file; do
    ...
    for func in $FUNC_LIST; do
        ...
        # 'popen(' must NOT match 'engine_popen(' — use [^a-z] negative class
        case "$func" in
            engine_popen) pattern="engine_popen(" ;;
...
while IFS= read -r file; do
    ...
    DANGER_LINES=$(grep -n 'system(\|popen(\|engine_popen(\|connect(\|execl(' "$file" 2>/dev/null \
