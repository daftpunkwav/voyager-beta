...
# graph-engine setup script (macOS + Linux)
...
SOURCE_DIR="$HOME/.local/share/graph-engine"
...
install_integration_asset() {
    local source_dir="$1"
    local source_path="${source_dir}/engine-integrations.json"

    if [ ! -f "$source_path" ]; then
        die "Runtime asset missing from build or release: engine-integrations.json"
    fi

    cp "$source_path" "${INSTALL_DIR}/engine-integrations.json"
    chmod 644 "${INSTALL_DIR}/engine-integrations.json"
}
...
download_binary() {
    ...
    local asset="graph-engine-${platform}.tar.gz"
...
build_from_source() {
    ...
    cp "${SOURCE_DIR}/build/c/graph-engine" "${INSTALL_DIR}/${BINARY_NAME}"
    chmod +x "${INSTALL_DIR}/${BINARY_NAME}"
...
configure_claude() {
    ...
    printf "%s" "${BOLD}Configure Claude Code to use graph-engine? [y/N] ${RESET}"
    read -r answer
    if [[ ! "$answer" =~ ^[Yy]$ ]]; then
        echo ""
...
JSONEOF
    ...
    if command -v jq &>/dev/null; then
        ...
        if [ -f "$settings_file" ]; then
            local tmp
            tmp=$(mktemp)
            jq --argjson entry "$mcp_entry" '.mcpServers["graph-engine"] = $entry' "$settings_file" > "$tmp"
            mv "$tmp" "$settings_file"
        else
            echo "{}" | jq --argjson entry "$mcp_entry" '.mcpServers["graph-engine"] = $entry' > "$settings_file"
        fi
...
if os.path.exists(path):
    with open(path) as f:
        data = json.load(f)
data.setdefault('mcpServers', {})['graph-engine'] = json.loads('$mcp_entry')
...
echo ""
echo "${BOLD}graph-engine installer${RESET}"
echo ""
...
info "  rm ${INSTALL_DIR}/engine-integrations.json"
info "  rm -rf ${SOURCE_DIR}  # if built from source"
info "  rm -rf ~/.cache/graph-engine/  # graph database"
