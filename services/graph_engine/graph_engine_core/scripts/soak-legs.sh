...
Legs (default: quick,query-leak — the release-gating sequence):
  ...
  query-leak   ENGINE_SOAK_MODE=query-leak, --skip-crash-test, results in
...
Examples:
  scripts/soak-legs.sh build/c/graph-engine 10          # both legs, 10m each
  scripts/soak-legs.sh --legs quick build/c/graph-engine 15   # ASan-style
EOF
}
...
# keys its native-Windows handling — cygpath'd ENGINE_CACHE_DIR, coproc stdio —
...
run_leg() {
    ...
    log="$(mktemp "${TMPDIR:-/tmp}/engine-soak-leg-XXXXXX")"
    ...
    quick)
        ...
        ;;
    query-leak)
        ENGINE_SOAK_MODE=query-leak RESULTS_DIR=soak-results-query-leak \
