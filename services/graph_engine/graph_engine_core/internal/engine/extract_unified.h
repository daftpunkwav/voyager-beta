#ifndef ENGINE_EXTRACT_UNIFIED_H
#define ENGINE_EXTRACT_UNIFIED_H
...
typedef enum {
    ENGINE_INVOCATION_NONE = 0,
    ENGINE_INVOCATION_PRIMARY,
    ENGINE_INVOCATION_CALLABLE_REFERENCE,
} EngineInvocationKind;

typedef struct {
    EngineInvocationKind kind;
    TSNode site;
    TSNode callee_expr;
    TSNode callee_leaf;
    const char *callee_name;
    bool raw_call_emitted;
} EngineInvocationDescriptor;
...
typedef struct {
    ...
    uint8_t kind;
    EngineInvocationKind invocation_kind;
    TSNode callee_expr;
    TSNode callee_leaf;
    ...
    EngineInvocationKind prev_invocation_kind;
...
typedef enum {
    ENGINE_LEXICAL_SCOPE_MODULE = 0,
    ENGINE_LEXICAL_SCOPE_CLASS,
    ENGINE_LEXICAL_SCOPE_FUNCTION,
    ENGINE_LEXICAL_SCOPE_BLOCK,
    ENGINE_LEXICAL_SCOPE_COMPREHENSION,
} EngineLexicalScopeKind;
...
typedef struct {
    ...
    uint32_t active_end;
    const char *name;
} EngineLexicalBinding;

typedef enum {
    ENGINE_PYTHON_DIRECTIVE_GLOBAL = 1,
    ENGINE_PYTHON_DIRECTIVE_NONLOCAL,
} EnginePythonDirectiveKind;

typedef struct {
    uint32_t function_scope_id;
    const char *name;
    uint8_t kind;
} EnginePythonDirective;
...
typedef struct {
    ...
    EngineInvocationKind invocation_kind;  // exact active invocation/reference role
    ...
    EngineArena *arena;
    EngineWalkScope *scopes;
    EngineWalkScope inline_scopes[MAX_SCOPES];
    int scope_capacity;
    int scope_top;

    EngineLexicalScope *lexical_scopes;
    EngineLexicalScope inline_lexical_scopes[INLINE_LEXICAL_SCOPES];
    ...
    EngineLexicalBinding *lexical_bindings;
    EngineLexicalBinding inline_lexical_bindings[INLINE_LEXICAL_BINDINGS];
    int lexical_binding_capacity;
    int lexical_binding_count;
    int usage_start_index;
    bool lexical_binding_tracking_failed;
    EnginePythonDirective *python_directives;
    EnginePythonDirective inline_python_directives[INLINE_PYTHON_DIRECTIVES];
...
void handle_env_accesses(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec,
                         WalkState *state);
void handle_type_assigns(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec,
                         WalkState *state);
...
#endif // ENGINE_EXTRACT_UNIFIED_H
