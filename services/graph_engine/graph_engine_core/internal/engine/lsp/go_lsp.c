...
static const EngineType* go_lookup_field(GoLSPContext* ctx, const char* type_qn, const char* field_name, int depth);
static void extract_type_params_from_ast(EngineArena* arena, EngineTypeRegistry* reg,
    TSNode root, const char* source, const char* module_qn);

// --- Initialization ---

void go_lsp_init(GoLSPContext* ctx, EngineArena* arena, const char* source, int source_len,
    const EngineTypeRegistry* registry, const char* package_qn, EngineResolvedCallArray* out) {
    ...
    ctx->current_scope = engine_scope_push(arena, NULL); // root scope
...
void go_lsp_add_import(GoLSPContext* ctx, const char* local_name, const char* pkg_qn) {
    ...
    if (ctx->import_count % 32 == 0) {
        ...
        const char** new_names = (const char**)engine_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(const char*));
        const char** new_qns = (const char**)engine_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(const char*));
    ...
    ctx->import_local_names[ctx->import_count] = engine_arena_strdup(ctx->arena, local_name);
    ctx->import_package_qns[ctx->import_count] = engine_arena_strdup(ctx->arena, pkg_qn);
    ctx->import_count++;
}
...
static const EngineType* resolve_builtin_type(GoLSPContext* ctx, const char* name) {
...
// --- go_parse_type_node: AST type node -> EngineType ---

const EngineType* go_parse_type_node(GoLSPContext* ctx, TSNode node) {
    if (ts_node_is_null(node)) return engine_type_unknown();
    ...
    if (strcmp(kind, "type_identifier") == 0) {
        ...
        if (!name) return engine_type_unknown();
        const EngineType* builtin = resolve_builtin_type(ctx, name);
        if (builtin) return builtin;
        // Resolve as local type: package_qn.TypeName
        return engine_type_named(ctx->arena,
            engine_arena_sprintf(ctx->arena, "%s.%s", ctx->package_qn, name));
    }
    ...
    if (strcmp(kind, "qualified_type") == 0) {
        ...
        if (!ts_node_is_null(pkg_node) && !ts_node_is_null(name_node)) {
            ...
            if (pkg_qn) {
                return engine_type_named(ctx->arena,
                    engine_arena_sprintf(ctx->arena, "%s.%s", pkg_qn, name));
            }
        }
    ...
    if (strcmp(kind, "slice_type") == 0) {
        ...
        return engine_type_slice(ctx->arena, go_parse_type_node(ctx, elem));
    }
    ...
    if (strcmp(kind, "map_type") == 0) {
        ...
        return engine_type_map(ctx->arena,
    ...
    if (strcmp(kind, "channel_type") == 0) {
        ...
        return engine_type_channel(ctx->arena, go_parse_type_node(ctx, value), dir);
    }

    // function_type: func(...)...
    if (strcmp(kind, "function_type") == 0) {
        return engine_type_func(ctx->arena, NULL, NULL, NULL); // simplified
    }
    ...
    if (strcmp(kind, "parameter_list") == 0) {
        ...
        if (count > 1) return engine_type_tuple(ctx->arena, elems, count);
    }
...
static void go_unify_type(const EngineType* param_type, const EngineType* arg_type,
    const char** type_param_names, const EngineType** inferred, int param_count) {
    if (!param_type || !arg_type || engine_type_is_unknown(arg_type)) return;
    ...
    if (param_type->kind == ENGINE_TYPE_MAP && arg_type->kind == ENGINE_TYPE_MAP) {
    ...
    if (param_type->kind == ENGINE_TYPE_CHANNEL && arg_type->kind == ENGINE_TYPE_CHANNEL) {
        go_unify_type(param_type->data.channel.elem, arg_type->data.channel.elem,
            type_param_names, inferred, param_count);
    }
    if (param_type->kind == ENGINE_TYPE_FUNC && arg_type->kind == ENGINE_TYPE_FUNC) {
        // Match param types
...
const EngineType* go_eval_expr_type(GoLSPContext* ctx, TSNode node) {
    if (ts_node_is_null(node)) return engine_type_unknown();
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        const EngineRegisteredFunc* f = engine_registry_lookup_symbol(ctx->registry, ctx->package_qn, name);
        if (f && f->signature) return f->signature;

        // Check if it's a builtin type (for type conversions like string(x), int(x))
        const EngineType* bt = resolve_builtin_type(ctx, name);
        if (bt) return engine_type_named(ctx->arena, name);

        // Check if it's a registered type (for type conversions like MyType(x))
        const char* type_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->package_qn, name);
        const EngineRegisteredType* rt = engine_registry_lookup_type(ctx->registry, type_qn);
        if (rt) return engine_type_named(ctx->arena, type_qn);
    ...
    if (strcmp(kind, "selector_expression") == 0) {
        ...
        if (ts_node_is_null(operand) || ts_node_is_null(field)) return engine_type_unknown();

        char* field_name = lsp_node_text(ctx, field);
        if (!field_name) return engine_type_unknown();
        ...
        if (strcmp(ts_node_type(operand), "identifier") == 0) {
            ...
            if (pkg_name) {
                ...
                if (pkg_qn) {
                    ...
                    const EngineRegisteredFunc* f = engine_registry_lookup_symbol(ctx->registry, pkg_qn, field_name);
                    if (f && f->signature) return f->signature;
                    // Check if it's a type
                    char* type_qn = engine_arena_sprintf(ctx->arena, "%s.%s", pkg_qn, field_name);
                    const EngineRegisteredType* rt = engine_registry_lookup_type(ctx->registry, type_qn);
                    if (rt) return engine_type_named(ctx->arena, type_qn);
                    return engine_type_unknown();
                }
        ...
        const EngineType* recv_type = go_eval_expr_type(ctx, operand);
        if (engine_type_is_unknown(recv_type)) return engine_type_unknown();

        // Auto-deref pointers for method calls
        const EngineType* base_type = recv_type;
        if (base_type->kind == ENGINE_TYPE_POINTER) {
            base_type = engine_type_deref(base_type);
        }

        if (base_type->kind == ENGINE_TYPE_NAMED) {
            const char* type_qn = base_type->data.named.qualified_name;

            // Look up method/field (methods recurse through embeddings)
            const EngineRegisteredFunc* method = go_lookup_field_or_method(ctx, type_qn, field_name);
            if (method && method->signature) return method->signature;

            // Check struct fields (with embedded field promotion)
            const EngineType* field_type = go_lookup_field(ctx, type_qn, field_name, 0);
            if (field_type && !engine_type_is_unknown(field_type)) return field_type;
        }
    ...
    if (strcmp(kind, "call_expression") == 0) {
        ...
        if (ts_node_is_null(func_node)) return engine_type_unknown();
        ...
        const EngineType* func_type = go_eval_expr_type(ctx, func_node);

        // If it's a FUNC type, return its return type
        if (func_type && func_type->kind == ENGINE_TYPE_FUNC &&
            ...
            if (!ts_node_is_null(targs_node)) {
                ...
                const EngineRegisteredFunc* rfunc = NULL;
                char* func_name = lsp_node_text(ctx, func_node);
                if (func_name) {
                    const char* func_qn = engine_arena_sprintf(ctx->arena, "%s.%s",
                        ctx->package_qn, func_name);
                    rfunc = engine_registry_lookup_func(ctx->registry, func_qn);
                    ...
                    if (!rfunc && strcmp(ts_node_type(func_node), "selector_expression") == 0) {
                        ...
                        if (!ts_node_is_null(operand) && !ts_node_is_null(field)) {
                            ...
                            if (pkg && fn) {
                                const char* pkg_qn = resolve_import(ctx, pkg);
                                if (pkg_qn) {
                                    rfunc = engine_registry_lookup_symbol(ctx->registry, pkg_qn, fn);
                                }
                            }
                        }
                    }
                ...
                if (rfunc && rfunc->type_param_names) {
                    ...
                    if (targ_count > 0 && targ_count == param_count) {
                        const EngineType** targ_arr = (const EngineType**)engine_arena_alloc(
                        ...
                        if (ret_count == 1) {
                            return engine_type_substitute(ctx->arena,
                        ...
                        const EngineType** new_rets = (const EngineType**)engine_arena_alloc(
                            ctx->arena, (ret_count + 1) * sizeof(const EngineType*));
                        for (int ri = 0; ri < ret_count; ri++) {
                            new_rets[ri] = engine_type_substitute(ctx->arena,
                                func_type->data.func.return_types[ri],
                                rfunc->type_param_names, targ_arr);
                        }
                        new_rets[ret_count] = NULL;
                        return engine_type_tuple(ctx->arena, new_rets, ret_count);
                    }
                }
            }
            ...
            if (ts_node_is_null(targs_node)) {
                ...
                if (rfunc && rfunc->type_param_names && func_type->data.func.param_types) {
                    ...
                    for (int ri = 0; ri < ret_count && !has_type_param; ri++) {
                        ...
                        const EngineType* rt = func_type->data.func.return_types[ri];
                        if (rt->kind == ENGINE_TYPE_TYPE_PARAM) has_type_param = true;
                        else if (rt->kind == ENGINE_TYPE_SLICE && rt->data.slice.elem &&
                                 rt->data.slice.elem->kind == ENGINE_TYPE_TYPE_PARAM) has_type_param = true;
                        else if (rt->kind == ENGINE_TYPE_POINTER && rt->data.pointer.elem &&
                                 rt->data.pointer.elem->kind == ENGINE_TYPE_TYPE_PARAM) has_type_param = true;
                        else if (rt->kind == ENGINE_TYPE_MAP) {
                            if (rt->data.map.key && rt->data.map.key->kind == ENGINE_TYPE_TYPE_PARAM) has_type_param = true;
                            if (rt->data.map.value && rt->data.map.value->kind == ENGINE_TYPE_TYPE_PARAM) has_type_param = true;
                        }
                    }

                    if (has_type_param && tpc > 0 && tpc <= 16) {
                        // Evaluate argument types and unify
                        const EngineType* inferred[16] = {0};
                        ...
                        if (!ts_node_is_null(args_node)) {
                            ...
                            for (uint32_t ai = 0; ai < argc && func_type->data.func.param_types[pi]; ai++) {
                                ...
                                const EngineType* arg_type = go_eval_expr_type(ctx, arg);
                        ...
                        if (all_inferred) {
                            const EngineType** targ_arr = (const EngineType**)engine_arena_alloc(
                                ctx->arena, (tpc + 1) * sizeof(const EngineType*));
            ...
            if (count > 1) {
                return engine_type_tuple(ctx->arena, func_type->data.func.return_types, count);
            }
        }
    ...
    if (strcmp(kind, "unary_expression") == 0) {
        TSNode operand = ts_node_child_by_field_name(node, "operand", 7);
        if (ts_node_is_null(operand)) return engine_type_unknown();
        ...
        for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
            ...
            if (!ts_node_is_named(child)) {
                ...
                if (!op) continue;
                if (strcmp(op, "&") == 0) {
                    return engine_type_pointer(ctx->arena, go_eval_expr_type(ctx, operand));
                }
                if (strcmp(op, "*") == 0) {
                    return engine_type_deref(go_eval_expr_type(ctx, operand));
                }
                if (strcmp(op, "<-") == 0) {
                    const EngineType* ch_type = go_eval_expr_type(ctx, operand);
                    if (ch_type && ch_type->kind == ENGINE_TYPE_CHANNEL) {
    ...
    if (strcmp(kind, "index_expression") == 0) {
        TSNode operand = ts_node_child_by_field_name(node, "operand", 7);
        if (ts_node_is_null(operand)) return engine_type_unknown();
        const EngineType* op_type = go_eval_expr_type(ctx, operand);
        if (!op_type) return engine_type_unknown();
        if (op_type->kind == ENGINE_TYPE_MAP) return op_type->data.map.value;
        if (op_type->kind == ENGINE_TYPE_SLICE) return op_type->data.slice.elem;
        return engine_type_unknown();
    }
    ...
    if (strcmp(kind, "func_literal") == 0) {
        ...
        if (!ts_node_is_null(body)) {
            ...
            if (!ts_node_is_null(params)) {
                ...
                for (uint32_t i = 0; i < nc; i++) {
                    ...
                    const EngineType* pt = go_parse_type_node(ctx, type_node);
                    ...
                    for (uint32_t j = 0; j < pnc; j++) {
                        ...
                        if (!ts_node_is_null(ch) && ts_node_is_named(ch) &&
                            ...
                            if (pname && strcmp(pname, "_") != 0)
                                engine_scope_bind(ctx->current_scope, pname, pt);
                        }
                    }
                }
            }
        ...
        return engine_type_func(ctx->arena, NULL,
            pt_count > 0 ? (const EngineType**)pt_arr : NULL,
            rt_count > 0 ? (const EngineType**)rt_arr : NULL);
    }
...
const EngineType* go_eval_builtin_call(GoLSPContext* ctx, const char* name, TSNode args) {
    // make(Type, ...) -> Type
    ...
    if (strcmp(name, "new") == 0 && !ts_node_is_null(args)) {
        ...
        if (nc > 0) {
            TSNode first_arg = ts_node_named_child(args, 0);
            return engine_type_pointer(ctx->arena, go_parse_type_node(ctx, first_arg));
        }
    }
...
static const EngineType* go_lookup_field(GoLSPContext* ctx,
    ...
    if (rt->embedded_types) {
        for (int i = 0; rt->embedded_types[i]; i++) {
            const EngineType* f = go_lookup_field(ctx, rt->embedded_types[i], field_name, depth + 1);
            if (f) return f;
        }
    }
...
static const EngineRegisteredFunc* go_lookup_field_or_method_depth(GoLSPContext* ctx,
    const char* type_qn, const char* member_name, int depth) {
    if (!type_qn || !member_name) return NULL;
    if (depth > ENGINE_LSP_MAX_LOOKUP_DEPTH) return NULL;
...
const EngineRegisteredFunc* go_lookup_field_or_method(GoLSPContext* ctx,
...
static const char *go_exact_callable_target(GoLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (engine_scope_contains(ctx->current_scope, name)) {
            return engine_scope_lookup_callable(ctx->current_scope, name);
        }
        const EngineRegisteredFunc *f =
            engine_registry_lookup_symbol(ctx->registry, ctx->package_qn, name);
    ...
    if (strcmp(kind, "selector_expression") == 0) {
        ...
        if (strcmp(ts_node_type(operand), "identifier") == 0) {
            ...
            if (package_qn) {
                const EngineRegisteredFunc *f =
                    engine_registry_lookup_symbol(ctx->registry, package_qn, member);
                return f ? f->qualified_name : NULL;
            }
        }

        const EngineType *recv = go_eval_expr_type(ctx, operand);
        if (recv && recv->kind == ENGINE_TYPE_POINTER)
            recv = engine_type_deref(recv);
        if (recv && recv->kind == ENGINE_TYPE_NAMED) {
            ...
            const EngineRegisteredFunc *method =
                engine_registry_lookup_method(ctx->registry, recv->data.named.qualified_name, member);
...
static void go_scope_bind_value(GoLSPContext *ctx, const char *name, const EngineType *type,
...
void go_process_statement(GoLSPContext* ctx, TSNode node) {
    ...
    if (strcmp(kind, "short_var_declaration") == 0) {
        ...
        const EngineType* rhs_type = NULL;
        TSNode single_rhs = {0};
        ...
        if (strcmp(ts_node_type(left), "expression_list") == 0) {
            ...
            for (uint32_t i = 0; i < lhs_count; i++) {
                ...
                const EngineType* var_type = engine_type_unknown();
                if (rhs_type) {
                    if (rhs_type->kind == ENGINE_TYPE_TUPLE && (int)i < rhs_type->data.tuple.count) {
    ...
    if (strcmp(kind, "assignment_statement") == 0) {
        ...
        for (uint32_t i = 0; i < lhs_count; i++) {
            ...
            const EngineType *type =
                ts_node_is_null(rhs) ? engine_type_unknown() : go_eval_expr_type(ctx, rhs);
    ...
    if (strcmp(kind, "const_spec") == 0) {
        ...
        const EngineType* const_type = engine_type_unknown();
        TSNode callable_value = value_node;
        if (!ts_node_is_null(value_node) &&
            strcmp(ts_node_type(value_node), "expression_list") == 0) {
            callable_value = ts_node_named_child_count(value_node) == 1
    ...
    if (strcmp(kind, "range_clause") == 0) {
        ...
        const EngineType* container_type = go_eval_expr_type(ctx, right);

        // Determine key and value types based on container type
        const EngineType* key_type = engine_type_unknown();
        const EngineType* val_type = engine_type_unknown();

        if (container_type) {
            switch (container_type->kind) {
            case ENGINE_TYPE_SLICE:
                key_type = engine_type_builtin(ctx->arena, "int");
            ...
            case ENGINE_TYPE_CHANNEL:
                ...
                break;
            default:
                if (container_type->kind == ENGINE_TYPE_BUILTIN &&
                    strcmp(container_type->data.builtin.name, "string") == 0) {
                    key_type = engine_type_builtin(ctx->arena, "int");
                    val_type = engine_type_builtin(ctx->arena, "rune");
                }
                break;
            }
        }
        ...
        if (!ts_node_is_null(left) && strcmp(ts_node_type(left), "expression_list") == 0) {
            ...
            for (uint32_t i = 0; i < lhs_count; i++) {
                ...
                if (!var_name || strcmp(var_name, "_") == 0) continue;
                engine_scope_bind(ctx->current_scope, var_name, i == 0 ? key_type : val_type);
            }
        }
        return;
    }
...
static void go_emit_resolved_kind(GoLSPContext *ctx, const char *callee_qn, const char *strategy,
                                  float confidence, const char *source_name, EngineResolvedKind kind,
                                  TSNode site) {
...
static void emit_resolved_call(GoLSPContext *ctx, const char *callee_qn, const char *strategy,
                               float confidence, TSNode site) {
    go_emit_resolved_kind(ctx, callee_qn, strategy, confidence, NULL, ENGINE_RESOLVED_INVOCATION,
                          site);
}
...
static void emit_resolved_alias_call(GoLSPContext *ctx, const char *callee_qn,
    ...
    go_emit_resolved_kind(ctx, callee_qn, "lsp_callable_alias", 0.97f, source_name,
                          ENGINE_RESOLVED_INVOCATION, site);
}
...
static void emit_resolved_reference(GoLSPContext *ctx, const char *callee_qn,
    ...
    go_emit_resolved_kind(ctx, callee_qn, "lsp_callable_value_reference", 0.97f, join_name,
                          ENGINE_RESOLVED_CALL_REFERENCE, site);
}
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (target) {
            emit_resolved_reference(ctx, target, source_name, arg);
                   ...
                   !engine_scope_contains(ctx->current_scope, source_name)) {
    ...
    if (strcmp(kind, "assignment_statement") == 0) {
        ...
        if (!ts_node_is_null(left)) {
            ...
            for (uint32_t i = 0; i < count; i++) {
                ...
                if (name && engine_scope_lookup_callable(ctx->current_scope, name)) {
        ...
        if (!ts_node_is_null(func_node)) {
            ...
            if (strcmp(fk, "selector_expression") == 0) {
                ...
                if (!ts_node_is_null(operand) && !ts_node_is_null(field)) {
                        ...
                        if (pkg_name) {
                            ...
                            if (pkg_qn && field_name) {
                                ...
                                emit_unresolved_call(
                                    ctx,
                                    engine_arena_sprintf(ctx->arena, "%s.%s", pkg_name, field_name),
                    ...
                    if (field_name) {
                        const EngineType* recv_type = go_eval_expr_type(ctx, operand);
                        const EngineType* base = recv_type;
                        if (base && base->kind == ENGINE_TYPE_POINTER) base = engine_type_deref(base);

                        if (base && base->kind == ENGINE_TYPE_NAMED) {
                            const EngineRegisteredType *receiver_type = engine_registry_lookup_type(
                            ...
                            if (!receiver_type || !receiver_type->is_interface) {
                                const EngineRegisteredFunc *method = go_lookup_field_or_method(
                        ...
                        if (base && field_name) {
                            bool is_iface = (base->kind == ENGINE_TYPE_INTERFACE);
                            const char* iface_qn = NULL;
                            if (!is_iface && base->kind == ENGINE_TYPE_NAMED) {
                                const EngineRegisteredType* rt = engine_registry_lookup_type(ctx->registry,
                            ...
                            if (is_iface) {
                                ...
                                const EngineRegisteredType* iface_rt = iface_qn ?
                                    engine_registry_lookup_type(ctx->registry, iface_qn) : NULL;
                                if (iface_rt && iface_rt->method_names && iface_rt->method_names[0]) {
                                    ...
                                    for (int ti = 0; ti < ctx->registry->type_count && impl_count < 2; ti++) {
                                        const EngineRegisteredType* cand = &ctx->registry->types[ti];
                                        ...
                                        for (int mi = 0; mi < iface_mcount; mi++) {
                                            if (!engine_registry_lookup_method(ctx->registry,
                                    ...
                                    if (impl_count == 1 && sole_impl_qn) {
                                        ...
                                        const EngineRegisteredFunc* concrete_method =
                                            engine_registry_lookup_method(ctx->registry, sole_impl_qn, field_name);
                                        if (concrete_method) {
                        ...
                        } else if (engine_type_is_unknown(recv_type)) {
                ...
                if (name && !is_go_builtin_func(name)) {
                    if (engine_scope_contains(ctx->current_scope, name)) {
                        const char *alias_target =
                            engine_scope_lookup_callable(ctx->current_scope, name);
                        if (alias_target) {
...
recurse:;
    ...
    if (strcmp(kind, "type_switch_statement") == 0) {
        ...
        for (uint32_t i = 0; i < nc2; i++) {
            ...
            for (uint32_t j = 0; j < cc_count; j++) {
                ...
                if (strcmp(cc_kind, "type_identifier") == 0 ||
                    ...
                    if (switch_var) {
                        engine_scope_bind(ctx->current_scope, switch_var,
        ...
        if (push_scope) ctx->current_scope = engine_scope_pop(ctx->current_scope);
        return;
    }
                ...
                for (uint32_t j = 0; j < cc_count; j++) {
                    ...
                    if (strcmp(cc_kind, "receive_statement") == 0) {
                        ...
                        if (!ts_node_is_null(right)) {
                            const EngineType* right_type = go_eval_expr_type(ctx, right);
                            // right_type might be a channel (if right is the channel)
                            // or already the elem type (if right is a <-ch unary expr)
                            const EngineType* recv_type = right_type;
                            if (right_type && right_type->kind == ENGINE_TYPE_CHANNEL) {
                            ...
                            if (!ts_node_is_null(left)) {
                                if (strcmp(ts_node_type(left), "expression_list") == 0) {
                                    ...
                                    for (uint32_t k = 0; k < lhs_count; k++) {
                                        ...
                                        if (!var_name || strcmp(var_name, "_") == 0) continue;
                                        if (k == 0) {
                                            engine_scope_bind(ctx->current_scope, var_name, recv_type);
                                        } else {
                                            // second var is the ok bool
                                            engine_scope_bind(ctx->current_scope, var_name,
                                                engine_type_builtin(ctx->arena, "bool"));
                                        }
                                    }
                                } else if (strcmp(ts_node_type(left), "identifier") == 0) {
                                    char* var_name = lsp_node_text(ctx, left);
...
static void process_function(GoLSPContext* ctx, TSNode func_node) {
    ...
    // call's enclosing_func_qn, so engine_pipeline_find_lsp_resolution never
    ...
    if (recv_type_name) {
        ctx->enclosing_func_qn =
            engine_arena_sprintf(ctx->arena, "%s.%s.%s", ctx->package_qn, recv_type_name, func_name);
    } else {
        ctx->enclosing_func_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->package_qn, func_name);
    }
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            // Get type
            TSNode type_node = ts_node_child_by_field_name(param, "type", 4);
            const EngineType* param_type = go_parse_type_node(ctx, type_node);

            // Variadic: ...T is []T in the function body
            if (is_variadic && param_type) {
                param_type = engine_type_slice(ctx->arena, param_type);
            }
            ...
            for (uint32_t j = 0; j < pnc; j++) {
                ...
                if (strcmp(ts_node_type(child), "identifier") == 0) {
                    ...
                    if (pname && strcmp(pname, "_") != 0) {
                        engine_scope_bind(ctx->current_scope, pname, param_type);
                    }
                }
            }
        }
    ...
    if (!ts_node_is_null(result_node)) {
        ...
        if (strcmp(rk, "parameter_list") == 0) {
            ...
            for (uint32_t i = 0; i < rnc; i++) {
                ...
                const EngineType* ret_type = go_parse_type_node(ctx, rtype);
                ...
                for (uint32_t j = 0; j < rpnc; j++) {
                    ...
                    if (strcmp(ts_node_type(child), "identifier") == 0) {
                        ...
                        if (rname && strcmp(rname, "_") != 0) {
                            engine_scope_bind(ctx->current_scope, rname, ret_type);
                        }
                    }
                }
            }
    ...
    if (!ts_node_is_null(recv)) {
        ...
        for (uint32_t i = 0; i < rnc; i++) {
            ...
            TSNode rtype = ts_node_child_by_field_name(rp, "type", 4);
            const EngineType* recv_type = go_parse_type_node(ctx, rtype);
            ...
            for (uint32_t j = 0; j < rpnc; j++) {
                ...
                if (!ts_node_is_null(rc) && ts_node_is_named(rc) &&
                    ...
                    if (rname && strcmp(rname, "_") != 0) {
                        engine_scope_bind(ctx->current_scope, rname, recv_type);
                    }
                    break;
                }
            }
...
// --- Helper: parse Go return type text into EngineType ---

const EngineType* engine_parse_return_type_text(EngineArena* a, const char* text, const char* module_qn) {
    if (!text || !text[0]) return engine_type_unknown();

    // Pointer: *Foo
    if (text[0] == '*') {
        return engine_type_pointer(a, engine_parse_return_type_text(a, text + 1, module_qn));
    }

    // Slice: []Foo
    if (text[0] == '[' && text[1] == ']') {
        return engine_type_slice(a, engine_parse_return_type_text(a, text + 2, module_qn));
    }
    ...
    for (const char** b = builtins; *b; b++) {
        if (strcmp(text, *b) == 0) return engine_type_builtin(a, text);
    }
...
} EngineGoLSPSignatureParserContext;

static const EngineType *engine_go_lsp_parse_signature_param_text(EngineArena *arena, const char *text,
                                                            void *parser_ctx) {
    const EngineGoLSPSignatureParserContext *ctx = (const EngineGoLSPSignatureParserContext *)parser_ctx;
    const char *module_qn = ctx && ctx->module_qn ? ctx->module_qn : "";
    return engine_parse_return_type_text(arena, text, module_qn);
}

// --- Entry point: build registry from file defs + run LSP ---

void engine_run_go_lsp(EngineArena* arena, EngineFileResult* result,
    ...
    engine_go_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (engine_label_is_type_like(d->label)) {
            ...
            if (d->base_classes) {
                ...
                if (bc_count > 0) {
                    const char** embedded = (const char**)engine_arena_alloc(arena, (bc_count + 1) * sizeof(const char*));
        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            if (d->return_types) {
                ...
                if (count > 0) {
                    ret_types = (const EngineType**)engine_arena_alloc(arena, (count + 1) * sizeof(const EngineType*));
                    for (int j = 0; j < count; j++) {
                        ret_types[j] = engine_parse_return_type_text(arena, d->return_types[j], module_qn);
                    }
                ...
                ret_types[0] = engine_parse_return_type_text(arena, d->return_type, module_qn);
                ret_types[1] = NULL;
            }
            ...
            const EngineType** param_types_arr = NULL;
            const char **param_names_arr = NULL;
            bool has_ordered_params =
                d->signature_param_types != NULL || d->signature_param_count > 0;
            EngineGoLSPSignatureParserContext param_parser_ctx = {module_qn};
            if (has_ordered_params) {
                param_types_arr = engine_type_materialize_signature_params(
                    arena, d->signature_param_types, d->signature_param_count,
                    engine_go_lsp_parse_signature_param_text, &param_parser_ctx);
            } else if (d->param_types) {
                param_names_arr = d->param_names;
                int count = 0;
                while (d->param_types[count]) count++;
                if (count > 0) {
                    param_types_arr = (const EngineType**)engine_arena_alloc(arena, (count + 1) * sizeof(const EngineType*));
                    for (int j = 0; j < count; j++) {
                        param_types_arr[j] = engine_parse_return_type_text(arena, d->param_types[j], module_qn);
                    }
            ...
            rf.signature = engine_type_func(arena, param_names_arr, param_types_arr, ret_types);
            ...
            if (strcmp(d->label, "Method") == 0) {
                if (d->parent_class && d->parent_class[0]) {
                    ...
                    if (end > r) {
                        char *type_name = engine_arena_strndup(arena, r, (size_t)(end - r));
                        rf.receiver_type = engine_arena_sprintf(arena, "%s.%s", module_qn, type_name);
                    }
                }

                if (rf.receiver_type && !engine_registry_lookup_type(&reg, rf.receiver_type)) {
    ...
    // Phase 1b: Scan AST for struct definitions to populate embedded_types
    {
        // Collect top-level children once (O(n)); ts_node_child(root,i) is O(i) → O(n²).
        uint32_t rkn = 0;
        TSNode* rkids = engine_lsp_collect_children(arena, root, &rkn);
            ...
            for (uint32_t j = 0; j < td_nc; j++) {
                ...
                if (strcmp(spec_kind, "type_alias") == 0) {
                    ...
                    if (!ts_node_is_null(alias_name) && !ts_node_is_null(alias_type)) {
                        char* aname = engine_node_text(arena, alias_name, source);
                        char* atarget = engine_node_text(arena, alias_type, source);
                        if (aname && aname[0] && atarget && atarget[0]) {
                            const char* alias_type_qn = engine_arena_sprintf(arena, "%s.%s", module_qn, aname);
                            const char* alias_target_qn = engine_arena_sprintf(arena, "%s.%s", module_qn, atarget);
                            bool found_a = false;
                            ...
                            if (!found_a) {
                                ...
                                engine_registry_add_type(&reg, alias_rt);
                            }
                        }
                    }
                    continue;
                ...
                char* type_name = engine_node_text(arena,name_node, source);
                ...
                if (strcmp(ts_node_type(type_node), "interface_type") == 0) {
                    ...
                    for (uint32_t k = 0; k < inl_nc && iface_method_count < 63; k++) {
                        ...
                        if (strcmp(ck, "method_spec") == 0 || strcmp(ck, "method_elem") == 0) {
                            ...
                            if (!ts_node_is_null(mname)) {
                                char* mn = engine_node_text(arena, mname, source);
                                if (mn && mn[0]) {
                    ...
                    if (iface_method_count > 0) {
                        for (int ti = 0; ti < reg.type_count; ti++) {
                            ...
                            const char** names = (const char**)engine_arena_alloc(arena,
                ...
                for (uint32_t k = 0; k < fl_nc; k++) {
                    ...
                    if (ts_node_is_null(fname) && !ts_node_is_null(ftype)) {
                        ...
                        if (embed_count < 15) {
                            char* embed_text = engine_node_text(arena, ftype, source);
                            if (embed_text && embed_text[0]) {
                                const char* et = embed_text;
                                while (*et == '*') et++;
                                embeds[embed_count++] = engine_arena_sprintf(arena, "%s.%s", module_qn, et);
                            }
                        }
                    } else if (!ts_node_is_null(fname) && !ts_node_is_null(ftype) && fld_count < 63) {
                        // Named field: name + type
                        char* fn = engine_node_text(arena, fname, source);
                        if (fn && fn[0]) {
                ...
                for (int ti = 0; ti < reg.type_count; ti++) {
                    ...
                    if (embed_count > 0) {
                        const char** arr = (const char**)engine_arena_alloc(arena,
                    ...
                    if (fld_count > 0) {
                        ...
                        const EngineType** types = (const EngineType**)engine_arena_alloc(arena,
                            (fld_count + 1) * sizeof(const EngineType*));
...
// Helper: split "|"-separated string into array of EngineType*.
static const EngineType** split_pipe_types(EngineArena* a, const char* text, const char* module_qn) {
    ...
    const EngineType** arr = (const EngineType**)engine_arena_alloc(a, (count + 1) * sizeof(const EngineType*));
    if (!arr) return NULL;

    // Split and parse each type
    char* buf = engine_arena_strdup(a, text);
    int idx = 0;
    char* start = buf;
    for (char* p = buf; ; p++) {
        if (*p == '|' || *p == '\0') {
            ...
            *p = '\0';
            if (start[0]) {
                arr[idx++] = engine_parse_return_type_text(a, start, module_qn);
            }
            if (save == '\0') break;
            start = p + 1;
        }
...
static const char** split_pipe_strings(EngineArena* a, const char* text) {
    if (!text || !text[0]) return NULL;
    ...
    for (char* p = buf; ; p++) {
        if (*p == '|' || *p == '\0') {
            ...
            *p = '\0';
            if (start[0]) {
                arr[idx++] = engine_arena_strdup(a, start);
            }
            if (save == '\0') break;
            start = p + 1;
        }
...
static void parse_field_defs_into_type(EngineArena* arena, EngineTypeRegistry* reg,
    ...
    const char** names = (const char**)engine_arena_alloc(arena, (count + 1) * sizeof(const char*));
    const EngineType** types = (const EngineType**)engine_arena_alloc(arena, (count + 1) * sizeof(const EngineType*));
    if (!names || !types) return;

    char* buf = engine_arena_strdup(arena, field_defs);
    int idx = 0;
    char* start = buf;
    for (char* p = buf; ; p++) {
        if (*p == '|' || *p == '\0') {
            ...
            *p = '\0';
            ...
            if (colon && idx < count) {
                *colon = '\0';
                names[idx] = engine_arena_strdup(arena, start);
                types[idx] = engine_parse_return_type_text(arena, colon + 1, def_module_qn);
                idx++;
            }
            if (save == '\0') break;
            start = p + 1;
...
static const EngineType* parse_type_node_with_params(EngineArena* arena, TSNode node,
    ...
    if (strcmp(kind, "type_identifier") == 0) {
        ...
        if (strcmp(name, "int") == 0 || strcmp(name, "string") == 0 ||
            ...
            return engine_type_builtin(arena, name);
        }
        return engine_type_named(arena, engine_arena_sprintf(arena, "%s.%s", module_qn, name));
    }
    ...
    if (strcmp(kind, "qualified_type") == 0) {
        ...
        TSNode name_node = ts_node_child_by_field_name(node, "name", 4);
        if (!ts_node_is_null(pkg_node) && !ts_node_is_null(name_node)) {
            char* pkg = engine_node_text(arena, pkg_node, source);
            char* name = engine_node_text(arena, name_node, source);
            if (pkg && name) {
                return engine_type_named(arena, engine_arena_sprintf(arena, "%s.%s", pkg, name));
            }
        }
        return engine_type_unknown();
    }
    ...
    if (strcmp(kind, "pointer_type") == 0) {
        uint32_t nc = ts_node_named_child_count(node);
        if (nc > 0)
            return engine_type_pointer(arena, parse_type_node_with_params(arena,
    ...
    if (strcmp(kind, "slice_type") == 0) {
        ...
        if (ts_node_is_null(elem) && ts_node_named_child_count(node) > 0)
            elem = ts_node_named_child(node, ts_node_named_child_count(node) - 1);
        return engine_type_slice(arena, parse_type_node_with_params(arena,
    ...
    if (strcmp(kind, "channel_type") == 0) {
        ...
        if (ts_node_is_null(value) && ts_node_named_child_count(node) > 0)
            value = ts_node_named_child(node, ts_node_named_child_count(node) - 1);
        char* text = engine_node_text(arena, node, source);
        int dir = 0;
        if (text) {
            if (strncmp(text, "chan<-", 6) == 0 || strncmp(text, "chan <-", 7) == 0) dir = 1;
            else if (strncmp(text, "<-chan", 6) == 0 || strncmp(text, "<- chan", 7) == 0) dir = 2;
        }
        return engine_type_channel(arena, parse_type_node_with_params(arena,
    ...
    if (strcmp(kind, "function_type") == 0) {
        ...
        const EngineType* param_types_arr[16];
        int pc = 0;
        ...
        const EngineType* ret_types_arr[16];
        int rc = 0;
        if (!ts_node_is_null(result_node)) {
        ...
        const EngineType** pt = pc > 0 ? (const EngineType**)param_types_arr : NULL;
        const EngineType** rt = rc > 0 ? (const EngineType**)ret_types_arr : NULL;
        return engine_type_func(arena, NULL, pt, rt);
    }

    // interface_type
    if (strcmp(kind, "interface_type") == 0) {
        EngineType* t = (EngineType*)engine_arena_alloc(arena, sizeof(EngineType));
        memset(t, 0, sizeof(EngineType));
        t->kind = ENGINE_TYPE_INTERFACE;
        return t;
    }
...
static int engine_go_lsp_signature_param_multiplicity(TSNode param) {
...
static const EngineType *engine_go_lsp_keep_known_refinement(const EngineType *candidate,
                                                       const EngineType *existing) {
    if (engine_type_is_unknown(candidate) && !engine_type_is_unknown(existing)) {
        return existing;
    }
    return candidate ? candidate : engine_type_unknown();
}
...
static void extract_type_params_from_ast(EngineArena* arena, EngineTypeRegistry* reg,
    ...
    for (uint32_t i = 0; i < rkn; i++) {
        ...
        char* func_name = engine_node_text(arena, name_node, source);
        ...
        for (uint32_t j = 0; j < tp_nc && param_count < 15; j++) {
            ...
            if (!ts_node_is_null(pname)) {
                char* pn = engine_node_text(arena, pname, source);
                if (pn && pn[0]) {
                    params[param_count++] = engine_arena_strdup(arena, pn);
                }
            }
        }
        ...
        const char** tp_names = (const char**)engine_arena_alloc(arena, (param_count + 1) * sizeof(const char*));
        for (int j = 0; j <= param_count; j++) tp_names[j] = params[j];

        // Find the matching registered function and set type_param_names
        const char* func_qn = engine_arena_sprintf(arena, "%s.%s", module_qn, func_name);
        for (int fi = 0; fi < reg->func_count; fi++) {
            if (reg->funcs[fi].qualified_name &&
                ...
                if (reg->funcs[fi].signature && reg->funcs[fi].signature->kind == ENGINE_TYPE_FUNC &&
                    reg->funcs[fi].signature->data.func.return_types) {
                    const EngineType** old_rets = reg->funcs[fi].signature->data.func.return_types;
                    int ret_count = 0;
                    ...
                    for (int ri = 0; ri < ret_count && !needs_reparse; ri++) {
                        const EngineType* check = old_rets[ri];
                        // Unwrap slice/pointer to get inner named type
                        if (check->kind == ENGINE_TYPE_SLICE && check->data.slice.elem)
                            check = check->data.slice.elem;
                        else if (check->kind == ENGINE_TYPE_POINTER && check->data.pointer.elem)
                            check = check->data.pointer.elem;
                        if (check->kind == ENGINE_TYPE_NAMED) {
                    ...
                    if (needs_reparse) {
                        ...
                        if (!ts_node_is_null(result_node)) {
                            const EngineType** new_rets = (const EngineType**)engine_arena_alloc(arena,
                                (ret_count + 1) * sizeof(const EngineType*));
                            ...
                            if (strcmp(ts_node_type(result_node), "parameter_list") == 0) {
                                ...
                                for (uint32_t ri = 0; ri < rnc && idx < ret_count; ri++) {
                                    ...
                                    const EngineType *parsed = parse_type_node_with_params(
                                        arena, rtype, source, module_qn, tp_names);
                                    new_rets[idx] =
                                        engine_go_lsp_keep_known_refinement(parsed, old_rets[idx]);
                                    idx++;
                                }
                                while (idx < ret_count) {
                                ...
                                new_rets[0] = engine_go_lsp_keep_known_refinement(parsed, old_rets[0]);
                            ...
                            const EngineType *new_sig = engine_type_func_replace_returns(
                                arena, reg->funcs[fi].signature, new_rets);
                            if (new_sig && new_sig->kind == ENGINE_TYPE_FUNC) {
                ...
                if (reg->funcs[fi].signature && reg->funcs[fi].signature->kind == ENGINE_TYPE_FUNC &&
                    ...
                    const EngineType** old_params = reg->funcs[fi].signature->data.func.param_types;
                    int pc = 0;
                    while (old_params[pc]) pc++;
                    // Re-parse from AST parameter list
                    TSNode params_node = ts_node_child_by_field_name(top, "parameters", 10);
                    if (!ts_node_is_null(params_node)) {
                        const EngineType** new_params = (const EngineType**)engine_arena_alloc(arena,
                            (pc + 1) * sizeof(const EngineType*));
                        int idx = 0;
                        bool overflow = false;
                        uint32_t pnc = ts_node_child_count(params_node);
                        for (uint32_t pi = 0; pi < pnc && !overflow; pi++) {
                            ...
                            const EngineType *parsed =
                                ts_node_is_null(ptype)
                                    ? engine_type_unknown()
                                    : parse_type_node_with_params(arena, ptype, source, module_qn,
                                                                  tp_names);
                            int multiplicity = engine_go_lsp_signature_param_multiplicity(pchild);
                            for (int occurrence = 0; occurrence < multiplicity; occurrence++) {
                                ...
                                new_params[idx] =
                                    engine_go_lsp_keep_known_refinement(parsed, old_params[idx]);
                                idx++;
                            }
                        }
                        new_params[idx] = NULL;
                        if (!overflow && idx == pc) {
                            ...
                            const EngineType *new_sig =
                                engine_type_func(arena, old_sig->data.func.param_names, new_params,
    ...
    EngineResolvedCallArray* out)
{
    ...
    // caller's arena which outlives this call, so engine_arena_strdup is
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            const EngineType** ret_types = split_pipe_types(arena, d->return_types, def_mod);
            EngineGoLSPSignatureParserContext param_parser_ctx = {def_mod};
    ...
    // engine_registry_lookup_method / lookup_type. Without this, every
 ...
 *
 * engine_go_build_cross_registry runs the registry build ONCE per project (in
 ...
 *
 * engine_run_go_lsp_cross_with_registry is the per-file entrypoint used by
...
EngineTypeRegistry* engine_go_build_cross_registry(
    EngineArena* arena, EngineLSPDef* defs, int def_count) {
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (d->lang != ENGINE_LANG_GO) continue;
        /* In the pre-built path every def carries its own def_module_qn
         * (set by engine_pxc_collect_all_defs). There is no caller module to
...
void engine_run_go_lsp_cross_with_registry(
    EngineArena* arena,
    const char* source, int source_len,
    const char* module_qn,
    EngineTypeRegistry* reg,
 ...
 * EngineResolvedCall per call site, with strategy="lsp_unresolved" for
...
int engine_go_fast_resolve_qualified_calls(
    ...
    for (int i = 0; i < initial_count; i++) {
        const EngineResolvedCall* uc = &result->resolved_calls.items[i];
        ...
        const EngineRegisteredFunc* f = engine_registry_lookup_func(reg, uc->callee_qn);
                    ...
                    for (int j = 0; j < import_count; j++) {
                        if (import_names[j] && import_qns[j] &&
                            ...
                            if (n > 0 && n < (int)sizeof(fq)) {
                                f = engine_registry_lookup_func(reg, fq);
                            }
                            break;
                        }
                    }
        ...
        if (!f && !strchr(uc->callee_qn, '.') && result->module_qn) {
            f = engine_registry_lookup_symbol(reg, result->module_qn, uc->callee_qn);
        }

        if (!f) continue;

        /* Emit a resolved entry. engine_pipeline_find_lsp_resolution
        ...
        engine_resolvedcall_push(&result->resolved_calls, &result->arena, rc);
        newly_resolved++;
    }
...
void engine_batch_go_lsp_cross(
    EngineArena* arena,
    EngineBatchGoLSPFile* files, int file_count,
    EngineResolvedCallArray* out)
{
    if (!files || file_count <= 0 || !out) return;

    for (int f = 0; f < file_count; f++) {
        EngineBatchGoLSPFile* file = &files[f];
        ...
        if (file_out.count > 0) {
            ...
            out[f].items = (EngineResolvedCall*)engine_arena_alloc(arena,
                file_out.count * sizeof(EngineResolvedCall));
