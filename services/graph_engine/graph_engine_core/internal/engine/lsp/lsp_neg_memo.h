 ...
 * (reg->read_only, set at finalize; engine_registry_add_* hard-return then) and
 ...
 *
 * Per-language wiring is a few lines: add a EngineNegMemo to the language ctx,
 * key each cascade entry with engine_negmemo_key (site tag + query strings),
 ...
 */
#ifndef ENGINE_LSP_NEG_MEMO_H
#define ENGINE_LSP_NEG_MEMO_H
...
enum {
    ENGINE_NEGMEMO_INIT_CAP = 1024,
    ENGINE_NEGMEMO_GROW = 2,
    ENGINE_NEGMEMO_LOAD_NUM = 7, /* grow at 70% load */
    ENGINE_NEGMEMO_LOAD_DEN = 10,
};
...
static inline uint64_t engine_negmemo_key(uint8_t site, const char *a, const char *b) {
...
static inline bool engine_negmemo_contains(const EngineNegMemo *m, uint64_t key) {
...
static inline void engine_negmemo_insert_raw(uint64_t *slots, int cap, uint64_t key) {
...
static inline void engine_negmemo_insert(EngineNegMemo *m, EngineArena *arena, uint64_t key) {
    if (!arena) {
        return; /* no arena — memo silently disabled */
    }
    if (!m->slots) {
        m->slots = engine_arena_alloc(arena, sizeof(uint64_t) * ENGINE_NEGMEMO_INIT_CAP);
        if (!m->slots) {
            return;
        }
        memset(m->slots, 0, sizeof(uint64_t) * ENGINE_NEGMEMO_INIT_CAP);
        m->cap = ENGINE_NEGMEMO_INIT_CAP;
        m->count = 0;
    }
    if ((int64_t)(m->count + 1) * ENGINE_NEGMEMO_LOAD_DEN >=
        (int64_t)m->cap * ENGINE_NEGMEMO_LOAD_NUM) {
        int new_cap = m->cap * ENGINE_NEGMEMO_GROW;
        uint64_t *ns = engine_arena_alloc(arena, sizeof(uint64_t) * (size_t)new_cap);
        if (!ns) {
        ...
        for (int i = 0; i < m->cap; i++) {
            if (m->slots[i]) {
                engine_negmemo_insert_raw(ns, new_cap, m->slots[i]);
            }
        }
        m->slots = ns;
        m->cap = new_cap;
    }
    if (!engine_negmemo_contains(m, key)) {
        engine_negmemo_insert_raw(m->slots, m->cap, key);
        m->count++;
    }
}

/* ── EngineIdxMemo: exact-match string-key → int index map ─────────────────────
...
static inline int32_t engine_idxmemo_get(const EngineIdxMemo *m, const char *key) {
    if (!m->slots || !key || m->count == 0) {
        return -1;
    }
    uint64_t h = engine_negmemo_key(0, key, NULL);
    uint64_t mask = (uint64_t)(m->cap - 1);
...
static inline void engine_idxmemo_put_raw(struct engine_idxmemo_slot *slots, int cap, uint64_t h,
...
static inline void engine_idxmemo_put_if_absent(EngineIdxMemo *m, EngineArena *arena, const char *key,
                                             int32_t val) {
    if (!arena || !key) {
        return;
    }
    if (!m->slots) {
        m->slots = engine_arena_alloc(arena,
                                   sizeof(struct engine_idxmemo_slot) * ENGINE_NEGMEMO_INIT_CAP);
        if (!m->slots) {
            return;
        }
        memset(m->slots, 0, sizeof(struct engine_idxmemo_slot) * ENGINE_NEGMEMO_INIT_CAP);
        m->cap = ENGINE_NEGMEMO_INIT_CAP;
        m->count = 0;
    }
    if ((int64_t)(m->count + 1) * ENGINE_NEGMEMO_LOAD_DEN >=
        (int64_t)m->cap * ENGINE_NEGMEMO_LOAD_NUM) {
        int new_cap = m->cap * ENGINE_NEGMEMO_GROW;
        struct engine_idxmemo_slot *ns =
            engine_arena_alloc(arena, sizeof(struct engine_idxmemo_slot) * (size_t)new_cap);
        if (!ns) {
            return;
        }
        memset(ns, 0, sizeof(struct engine_idxmemo_slot) * (size_t)new_cap);
        for (int i = 0; i < m->cap; i++) {
            if (m->slots[i].h) {
                engine_idxmemo_put_raw(ns, new_cap, m->slots[i].h, m->slots[i].key, m->slots[i].val);
            }
        }
        m->slots = ns;
        m->cap = new_cap;
    ...
    uint64_t h = engine_negmemo_key(0, key, NULL);
    if (engine_idxmemo_get(m, key) < 0) {
        engine_idxmemo_put_raw(m->slots, m->cap, h, key, val);
        m->count++;
    }
}

#endif /* ENGINE_LSP_NEG_MEMO_H */
