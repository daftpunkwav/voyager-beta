 ...
 *   - We populate a EngineTypeRegistry from this file's EngineDefinitions plus a
 ...
 *     compatibility (engine_registry_lookup_method_by_types). Ambiguity ⇒
 ...
 *
 * Confidence policy (consumed by lsp_resolve.h with ENGINE_LSP_CONFIDENCE_FLOOR
 * = 0.6):
...
static const EngineType *eval_method_invocation(JavaLSPContext *ctx, TSNode node);
static const EngineType *eval_object_creation(JavaLSPContext *ctx, TSNode node);
static const EngineType *eval_field_access(JavaLSPContext *ctx, TSNode node);
static const EngineType *eval_array_access(JavaLSPContext *ctx, TSNode node);
static const EngineType *eval_cast(JavaLSPContext *ctx, TSNode node);
static const EngineType *eval_ternary(JavaLSPContext *ctx, TSNode node);
static const EngineType *eval_binary(JavaLSPContext *ctx, TSNode node);
static const EngineType *eval_unary(JavaLSPContext *ctx, TSNode node);
static const EngineType *eval_lambda(JavaLSPContext *ctx, TSNode node);
static const EngineType *eval_method_reference(JavaLSPContext *ctx, TSNode node);
static const EngineType *resolve_identifier_type(JavaLSPContext *ctx, const char *name);
static const EngineType *resolve_member_type(JavaLSPContext *ctx, const EngineType *recv_type,
                                          const char *member_name);
static void resolve_method_call(JavaLSPContext *ctx, TSNode call);
static void register_local_func_or_type_from_file(JavaLSPContext *ctx, EngineTypeRegistry *reg,
                                                  EngineFileResult *result);
static const char *strip_generics(EngineArena *a, const char *type_text);
static const char *unwrap_array_text(EngineArena *a, const char *type_text, int *out_array_dim);
...
static const EngineType *box_primitive(EngineArena *a, const char *prim);
static int count_call_args(TSNode call_node);
static const EngineType *propagate_template(EngineArena *a, const char *recv_qn, const char *method_name,
                                         const EngineType *const *recv_targs, int recv_targ_count,
                                         const EngineType *return_t);
static bool method_implies_lambda_args(const char *recv_qn, const char *method_name,
                                       const EngineType *const *targs, int targ_count, int *out_arity,
                                       const EngineType **out_param0, const EngineType **out_param1);
static void resolve_method_reference(JavaLSPContext *ctx, TSNode mref,
                                     const EngineRegisteredFunc *outer_resolved, int arg_index,
                                     const EngineType *recv_type);
...
static const char *strip_generics(EngineArena *a, const char *type_text) {
    ...
    if (!lt)
        return type_text;
    return engine_arena_strndup(a, type_text, (size_t)(lt - type_text));
}

/* Strip array suffix (`int[]` → `int`, `String[][]` → `String`, dim=2). */
static const char *unwrap_array_text(EngineArena *a, const char *type_text, int *out_dim) {
    if (out_dim)
        *out_dim = 0;
    if (!type_text)
        return NULL;
    ...
    if (dim == 0)
        return type_text;
    return engine_arena_strndup(a, type_text, n);
}
...
static const EngineType *box_primitive(EngineArena *a, const char *prim) {
    ...
    for (size_t i = 0; i < sizeof(map) / sizeof(map[0]); i++) {
        if (strcmp(prim, map[i][0]) == 0)
            return engine_type_named(a, map[i][1]);
    }
    return engine_type_unknown();
}
...
void java_lsp_init(JavaLSPContext *ctx, EngineArena *arena, const char *source, int source_len,
                   const EngineTypeRegistry *registry, const char *package_name, const char *module_qn,
...
void java_lsp_add_import(JavaLSPContext *ctx, const char *local_name, const char *target_qn,
    ...
    if (ctx->import_count >= ctx->import_cap) {
        ...
        const char **new_names =
            (const char **)engine_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_names));
        const char **new_qns =
            (const char **)engine_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_qns));
        int *new_kinds = (int *)engine_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_kinds));
    ...
    ctx->import_target_qns[ctx->import_count] = engine_arena_strdup(ctx->arena, target_qn);
...
static void push_enclosing_class(JavaLSPContext *ctx, const char *class_qn) {
    if (ctx->enclosing_class_depth >= ctx->enclosing_class_cap) {
        ...
        const char **arr =
            (const char **)engine_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*arr));
        if (!arr)
            return;
...
static const EngineType *parse_type_arguments(JavaLSPContext *ctx, TSNode targs_node);

const EngineType *java_parse_type_node(JavaLSPContext *ctx, TSNode node) {
    if (ts_node_is_null(node))
        return engine_type_unknown();
    ...
    if (strcmp(kind, "void_type") == 0)
        return engine_type_builtin(ctx->arena, "void");
    if (strcmp(kind, "boolean_type") == 0)
        return engine_type_builtin(ctx->arena, "boolean");
    if (strcmp(kind, "integral_type") == 0 || strcmp(kind, "floating_point_type") == 0) {
        char *txt = java_node_text(ctx, node);
        return engine_type_builtin(ctx->arena, txt ? txt : "int");
    }
    ...
    if (strcmp(kind, "scoped_type_identifier") == 0) {
        ...
        if (engine_registry_lookup_type(ctx->registry, full)) {
            return engine_type_named(ctx->arena, full);
        }
        /* Otherwise resolve the head segment and append the rest. */
        char *dot = strchr(full, '.');
        if (dot) {
            char *head = engine_arena_strndup(ctx->arena, full, (size_t)(dot - full));
            const char *head_qn = java_resolve_type_name(ctx, head);
            if (head_qn) {
                return engine_type_named(ctx->arena,
                                      engine_arena_sprintf(ctx->arena, "%s%s", head_qn, dot));
            }
        }
        return engine_type_named(ctx->arena, full);
    }
    ...
    if (strcmp(kind, "generic_type") == 0) {
        ...
        const EngineType *base = java_parse_type_node(ctx, raw);
        ...
        if (arg_count == 0) {
            arg_buf[arg_count++] = engine_type_unknown();
        }
        const EngineType **args =
            (const EngineType **)engine_arena_alloc(ctx->arena, (size_t)(arg_count + 1) * sizeof(*args));
        for (int i = 0; i < arg_count; i++)
            args[i] = arg_buf[i];
        args[arg_count] = NULL;
        return engine_type_template(ctx->arena, base_qn, args, arg_count);
    }
    ...
    if (strcmp(kind, "array_type") == 0) {
        ...
        return engine_type_slice(ctx->arena, java_parse_type_node(ctx, elem));
    }
...
const char *java_resolve_type_name(JavaLSPContext *ctx, const char *name) {
    ...
    for (int i = ctx->enclosing_class_depth - 1; i >= 0; i--) {
        ...
        if (n > 0 && (size_t)n < sizeof(buf)) {
            if (engine_registry_lookup_type(ctx->registry, buf)) {
                return engine_arena_strdup(ctx->arena, buf);
            }
        }
    }
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != ENGINE_JAVA_IMPORT_TYPE)
            continue;
    ...
    /* 4. On-demand imports. */
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != ENGINE_JAVA_IMPORT_ON_DEMAND)
            continue;
        char buf[JAVA_LSP_BUF];
    ...
    if (engine_registry_lookup_type(ctx->registry, name)) {
    ...
    if (ctx->registry && ctx->registry->types) {
        ...
        for (int i = 0; i < ctx->registry->type_count && matches < 2; i++) {
            const EngineRegisteredType *t = &ctx->registry->types[i];
...
const EngineType *java_eval_expr_type(JavaLSPContext *ctx, TSNode node) {
    if (ts_node_is_null(node))
        return engine_type_unknown();
    ...
    if (strcmp(kind, "decimal_integer_literal") == 0 || strcmp(kind, "hex_integer_literal") == 0 ||
        ...
        if (txt) {
            ...
            if (len > 0 && (txt[len - 1] == 'l' || txt[len - 1] == 'L')) {
                result = engine_type_builtin(ctx->arena, "long");
                goto out;
            }
        }
        result = engine_type_builtin(ctx->arena, "int");
        goto out;
    }
    if (strcmp(kind, "decimal_floating_point_literal") == 0 ||
        ...
        if (txt) {
            ...
            if (len > 0 && (txt[len - 1] == 'f' || txt[len - 1] == 'F')) {
                result = engine_type_builtin(ctx->arena, "float");
                goto out;
            }
        }
        result = engine_type_builtin(ctx->arena, "double");
        goto out;
    }
    ...
    if (strcmp(kind, "character_literal") == 0) {
        result = engine_type_builtin(ctx->arena, "char");
        goto out;
    }
    if (strcmp(kind, "string_literal") == 0 || strcmp(kind, "string_fragment") == 0) {
        result = engine_type_named(ctx->arena, "java.lang.String");
        goto out;
    }
    ...
    if (strcmp(kind, "super") == 0) {
        if (ctx->enclosing_super_qn) {
            result = engine_type_named(ctx->arena, ctx->enclosing_super_qn);
        } else {
            result = engine_type_named(ctx->arena, "java.lang.Object");
        }
        goto out;
    }
    ...
    if (strcmp(kind, "class_literal") == 0) {
        result = engine_type_named(ctx->arena, "java.lang.Class");
        goto out;
    }
    ...
    if (strcmp(kind, "array_creation_expression") == 0) {
        ...
        const EngineType *elem = java_parse_type_node(ctx, type_node);
        result = engine_type_slice(ctx->arena, elem);
        goto out;
    }
...
static const EngineType *resolve_identifier_type(JavaLSPContext *ctx, const char *name) {
    ...
    if (ctx->enclosing_class_qn) {
        const EngineType *ft = java_lookup_field_type(ctx, ctx->enclosing_class_qn, name);
        if (ft && !engine_type_is_unknown(ft))
            return ft;
    }
    /* Outer classes. */
    for (int i = ctx->enclosing_class_depth - 2; i >= 0; i--) {
        ...
        if (!outer)
            continue;
        const EngineType *ft = java_lookup_field_type(ctx, outer, name);
        if (ft && !engine_type_is_unknown(ft))
            return ft;
    }
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != ENGINE_JAVA_IMPORT_STATIC)
            continue;
        if (strcmp(ctx->import_local_names[i], name) == 0) {
            ...
            if (last_dot) {
                char *cls = engine_arena_strndup(ctx->arena, target, (size_t)(last_dot - target));
                const EngineType *ft = java_lookup_field_type(ctx, cls, name);
                if (ft && !engine_type_is_unknown(ft))
                    return ft;
            }
            return engine_type_unknown();
...
static const EngineType *eval_field_access(JavaLSPContext *ctx, TSNode node) {
    ...
    if (!ts_node_is_null(obj) && strcmp(ts_node_type(obj), "identifier") == 0) {
        ...
        if (oname && strcmp(oname, "System") == 0) {
            if (strcmp(fname, "out") == 0 || strcmp(fname, "err") == 0) {
                return engine_type_named(ctx->arena, "java.io.PrintStream");
            }
            if (strcmp(fname, "in") == 0) {
                return engine_type_named(ctx->arena, "java.io.InputStream");
            }
        }
    }

    /* Special-case: `length` on array types is `int`. */
    const EngineType *recv = ts_node_is_null(obj) ? engine_type_unknown() : java_eval_expr_type(ctx, obj);
    if (recv && recv->kind == ENGINE_TYPE_SLICE && strcmp(fname, "length") == 0) {
    ...
    if (!ts_node_is_null(obj) && strcmp(ts_node_type(obj), "this") == 0) {
        const EngineType *scope_t = engine_scope_lookup(ctx->current_scope, fname);
        if (scope_t && !engine_type_is_unknown(scope_t))
            return scope_t;
    }

    const EngineType *res = resolve_member_type(ctx, recv, fname);
    if (res && !engine_type_is_unknown(res))
        return res;
...
const EngineType *java_lookup_field_type(JavaLSPContext *ctx, const char *class_qn,
                                      const char *field_name) {
    if (!class_qn || !field_name)
        return engine_type_unknown();
    const char *cur = class_qn;
    for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur; hops++) {
        const EngineRegisteredType *rt = engine_registry_lookup_type(ctx->registry, cur);
        if (!rt)
            break;
...
static const EngineType *resolve_member_type(JavaLSPContext *ctx, const EngineType *recv,
                                          const char *member_name) {
    if (!recv || !member_name)
        return engine_type_unknown();
    ...
    if (recv->kind == ENGINE_TYPE_NAMED)
        recv_qn = recv->data.named.qualified_name;
    else if (recv->kind == ENGINE_TYPE_TEMPLATE)
...
const EngineRegisteredFunc *java_lookup_method(JavaLSPContext *ctx, const char *class_qn,
                                            const char *method_name, int arg_count) {
    if (!class_qn || !method_name)
        return NULL;
    const char *cur = class_qn;
    const EngineRegisteredFunc *fallback = NULL;
    for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur; hops++) {
        /* Try arg-count-aware lookup first. */
        const EngineRegisteredFunc *m =
            engine_registry_lookup_method_by_args(ctx->registry, cur, method_name, arg_count);
        if (m)
            return m;
        /* Otherwise capture any name match as fallback. */
        if (!fallback) {
            fallback = engine_registry_lookup_method(ctx->registry, cur, method_name);
        }
        const EngineRegisteredType *rt = engine_registry_lookup_type(ctx->registry, cur);
        if (!rt)
            break;
...
static const EngineType *java_return_type_of(const EngineRegisteredFunc *f) {
    if (!f || !f->signature)
        return engine_type_unknown();
    if (f->signature->kind != ENGINE_TYPE_FUNC)
        return engine_type_unknown();
    const EngineType *const *rets = f->signature->data.func.return_types;
    if (!rets || !rets[0])
        return engine_type_unknown();
    return rets[0];
}
...
static const EngineType *substitute_generic_return(JavaLSPContext *ctx, const char *recv_qn,
                                                const char *method_name,
                                                const EngineType *const *arg_arr, int targ_count,
                                                const EngineType *fallback) {
    (void)ctx;
    ...
    if (fallback->kind != ENGINE_TYPE_NAMED)
        return fallback;
...
static const EngineType *eval_method_invocation(JavaLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(obj)) {
        ...
        for (int i = 0; i < ctx->import_count; i++) {
            ...
            if (!last_dot)
                continue;
            char *cls = engine_arena_strndup(ctx->arena, target, (size_t)(last_dot - target));
            const EngineRegisteredFunc *f = java_lookup_method(ctx, cls, mname, arity);
            if (f)
                return java_return_type_of(f);
        }
        return engine_type_unknown();
    ...
    if (strcmp(ts_node_type(obj), "super") == 0) {
        ...
        const EngineRegisteredFunc *f = java_lookup_method(ctx, super_qn, mname, arity);
        if (f)
            return java_return_type_of(f);
        return engine_type_unknown();
    }
    ...
    if (strcmp(ts_node_type(obj), "identifier") == 0) {
        ...
        if (oname) {
            ...
            if (cls_qn) {
                ...
                if (engine_type_is_unknown(engine_scope_lookup(ctx->current_scope, oname))) {
                    const EngineRegisteredFunc *f = java_lookup_method(ctx, cls_qn, mname, arity);
                    if (f)
                        return java_return_type_of(f);
                }
            }
    ...
    /* Instance call. */
    const EngineType *recv = java_eval_expr_type(ctx, obj);
    const EngineType *base = recv;
    if (base && base->kind == ENGINE_TYPE_TEMPLATE) {
        const char *base_qn = base->data.template_type.template_name;
        if (base_qn) {
            const EngineRegisteredFunc *f = java_lookup_method(ctx, base_qn, mname, arity);
            if (f) {
                const EngineType *ret = java_return_type_of(f);
                /* Generic substitution + carrier propagation. */
                const EngineType *subst = substitute_generic_return(
...
static const EngineType *eval_array_access(JavaLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(arr))
        return engine_type_unknown();
    const EngineType *t = java_eval_expr_type(ctx, arr);
    if (t && t->kind == ENGINE_TYPE_SLICE)
...
static const EngineType *eval_ternary(JavaLSPContext *ctx, TSNode node) {
    ...
    const EngineType *t = java_eval_expr_type(ctx, then_n);
    if (t && !engine_type_is_unknown(t))
        return t;
...
static bool is_string_concat(JavaLSPContext *ctx, TSNode node) {
    ...
    TSNode rhs = ts_node_child_by_field_name(node, "right", 5);
    const EngineType *l = java_eval_expr_type(ctx, lhs);
    const EngineType *r = java_eval_expr_type(ctx, rhs);
    if (l && l->kind == ENGINE_TYPE_NAMED &&
        strcmp(l->data.named.qualified_name, "java.lang.String") == 0)
        return true;
    if (r && r->kind == ENGINE_TYPE_NAMED &&
...
static const EngineType *eval_binary(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(op, "+") == 0 && is_string_concat(ctx, node)) {
        return engine_type_named(ctx->arena, "java.lang.String");
    }
...
void java_process_statement(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "local_variable_declaration") == 0) {
        ...
        if (rn) {
            const EngineType *rt = engine_type_unknown();
            bool is_var = false;
            ...
            if ((is_var || engine_type_is_unknown(rt)) && !ts_node_is_null(rvalue)) {
                rt = java_eval_expr_type(ctx, rvalue);
            }
            engine_scope_bind(ctx->current_scope, rn, rt ? rt : engine_type_unknown());
        }
        ...
        if (!ts_node_is_null(formal)) {
            ...
            if (pn) {
                ...
                engine_scope_bind(ctx->current_scope, pn, pt);
            }
        }
    }
...
static void process_local_var_decl(JavaLSPContext *ctx, TSNode node) {
    TSNode type_node = ts_node_child_by_field_name(node, "type", 4);
    const EngineType *static_type =
        ts_node_is_null(type_node) ? engine_type_unknown() : java_parse_type_node(ctx, type_node);
    bool is_var =
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!vname)
            continue;
        const EngineType *bind_type = static_type;
        if ((is_var || engine_type_is_unknown(bind_type)) && !ts_node_is_null(value_node)) {
            bind_type = java_eval_expr_type(ctx, value_node);
        }
        engine_scope_bind(ctx->current_scope, vname, bind_type ? bind_type : engine_type_unknown());
    }
}
...
static void process_enhanced_for(JavaLSPContext *ctx, TSNode node) {
    ...
    const EngineType *t = (is_var || ts_node_is_null(type_node))
                           ? engine_type_unknown()
                           : java_parse_type_node(ctx, type_node);

    /* When type is `var` or unparseable, infer from the iterable. */
    if (engine_type_is_unknown(t) && !ts_node_is_null(value_node)) {
        const EngineType *iter_t = java_eval_expr_type(ctx, value_node);
        if (iter_t && iter_t->kind == ENGINE_TYPE_SLICE) {
            t = iter_t->data.slice.elem;
        } else if (iter_t && iter_t->kind == ENGINE_TYPE_TEMPLATE &&
    ...
    engine_scope_bind(ctx->current_scope, vname, t ? t : engine_type_unknown());
}
...
static void process_field_decl(JavaLSPContext *ctx, TSNode node) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!fname)
            continue;
        engine_scope_bind(ctx->current_scope, fname, static_type);
    }
}
...
static void process_method_decl(JavaLSPContext *ctx, TSNode node, const char *class_qn,
    ...
    if (!mname)
        return;
    char *method_qn = engine_arena_sprintf(ctx->arena, "%s.%s", class_qn, mname);
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            ...
            if (!pn)
                continue;
            const EngineType *pt =
                ts_node_is_null(ptype) ? engine_type_unknown() : java_parse_type_node(ctx, ptype);
            if (strcmp(pk, "spread_parameter") == 0 && pt) {
                pt = engine_type_slice(ctx->arena, pt);
            }
            engine_scope_bind(ctx->current_scope, pn, pt);
        }
    }
...
static void process_constructor_decl(JavaLSPContext *ctx, TSNode node, const char *class_qn,
    ...
    char *ctor_qn;
    if (cname) {
        ctor_qn = engine_arena_sprintf(ctx->arena, "%s.%s", class_qn, cname);
    } else {
        ctor_qn = engine_arena_sprintf(ctx->arena, "%s.<init>", class_qn);
    }
...
static const char *class_super_qn(JavaLSPContext *ctx, TSNode class_node) {
    ...
    if (ts_node_is_null(tnode))
        return NULL;
    const EngineType *t = java_parse_type_node(ctx, tnode);
...
static void java_process_class_decl(JavaLSPContext *ctx, TSNode node) {
    ...
    char *class_qn;
    if (ctx->enclosing_class_qn) {
        class_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, cname);
    } else if (ctx->module_qn && ctx->module_qn[0]) {
        class_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    } else {
        class_qn = engine_arena_strdup(ctx->arena, cname);
    }
...
void java_lsp_process_file(JavaLSPContext *ctx, TSNode root) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(k, "package_declaration") == 0) {
            ...
            if (path) {
                if (is_on_demand) {
                    int kind = is_static ? ENGINE_JAVA_IMPORT_STATIC_OD : ENGINE_JAVA_IMPORT_ON_DEMAND;
                    ...
                    java_lsp_add_import(ctx, local, path, ENGINE_JAVA_IMPORT_STATIC);
                } else {
                    const char *last_dot = strrchr(path, '.');
                    const char *local = last_dot ? last_dot + 1 : path;
                    java_lsp_add_import(ctx, local, path, ENGINE_JAVA_IMPORT_TYPE);
                }
            }
        }
    }
...
static void java_emit_unresolved_kind(JavaLSPContext *ctx, const char *expr_text,
                                      const char *reason, EngineResolvedKind kind) {
...
static void java_emit_unresolved(JavaLSPContext *ctx, const char *expr_text, const char *reason) {
    java_emit_unresolved_kind(ctx, expr_text, reason, ENGINE_RESOLVED_INVOCATION);
}

static void java_emit_unresolved_reference(JavaLSPContext *ctx, const char *expr_text,
                                           const char *reason) {
    java_emit_unresolved_kind(ctx, expr_text, reason, ENGINE_RESOLVED_CALL_REFERENCE);
}
...
static const char *java_find_sole_impl(JavaLSPContext *ctx, const char *iface_qn, const char *mname,
    ...
    for (int ti = 0; ti < ctx->registry->type_count && distinct < 2; ti++) {
         ...
         * name array. */
        bool has = engine_registry_lookup_method(ctx->registry, cand->qualified_name, mname) != NULL;
        ...
        for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur && !subtype; hops++) {
            const EngineRegisteredType *ct = engine_registry_lookup_type(ctx->registry, cur);
...
static bool java_emit_interface_resolution(JavaLSPContext *ctx, const char *iface_qn,
    ...
    if (impl_count == 1 && sole_impl) {
        const EngineRegisteredFunc *cf = engine_registry_lookup_method(ctx->registry, sole_impl, mname);
        const char *target =
            cf ? cf->qualified_name : engine_arena_sprintf(ctx->arena, "%s.%s", sole_impl, mname);
        java_emit_resolved(ctx, target, "lsp_interface_resolve", 0.85f);
        return true;
    }
    if (impl_count >= 2) {
        java_emit_resolved(ctx, engine_arena_sprintf(ctx->arena, "%s.%s", iface_qn, mname),
...
static void resolve_method_call(JavaLSPContext *ctx, TSNode call) {
    ...
    if (ts_node_is_null(obj)) {
        ...
        for (int i = ctx->enclosing_class_depth - 2; i >= 0; i--) {
            ...
            if (!outer)
                continue;
            const EngineRegisteredFunc *f = java_lookup_method(ctx, outer, mname, arity);
            if (f) {
    ...
    if (strcmp(ts_node_type(obj), "identifier") == 0) {
        char *oname = java_node_text(ctx, obj);
        if (oname && engine_type_is_unknown(engine_scope_lookup(ctx->current_scope, oname))) {
            const char *cls_qn = java_resolve_type_name(ctx, oname);
            if (cls_qn) {
                const EngineRegisteredFunc *f = java_lookup_method(ctx, cls_qn, mname, arity);
                if (f) {
    ...
    if (recv_qn) {
        const EngineRegisteredFunc *f = java_lookup_method(ctx, recv_qn, mname, arity);
        if (f) {
            ...
            const EngineRegisteredType *rt0 = engine_registry_lookup_type(ctx->registry, recv_qn);
        ...
        if (rt && rt->is_interface) {
            ...
            java_emit_resolved(ctx, engine_arena_sprintf(ctx->arena, "%s.%s", recv_qn, mname),
                               "lsp_interface_dispatch", 0.80f);
            return;
        }
        java_emit_unresolved(ctx, engine_arena_sprintf(ctx->arena, "%s.%s", recv_qn, mname),
                             "no_method_match");
        return;
    }
 ...
 * `fi_qn` parameterized with `targs`. Returns engine_type_unknown() when the
 * parametric mapping isn't modeled (e.g. specialized primitive variants). */
static const EngineType *sam_param_type(EngineArena *a, const char *fi_qn, int param_idx,
                                     const EngineType *const *targs, int targ_count) {
    if (!fi_qn)
        return engine_type_unknown();
    ...
    for (int i = 0; single_t[i]; i++) {
        if (strcmp(single_t[i], fi_qn) == 0 && param_idx == 0 && targ_count >= 1) {
            return targs[0] ? targs[0] : engine_type_unknown();
        }
    }
    ...
    if (strcmp(fi_qn, "java.util.function.BiFunction") == 0 ||
        ...
        if (param_idx <= 1 && targ_count > param_idx) {
            return targs[param_idx] ? targs[param_idx] : engine_type_unknown();
        }
    }
 ...
 *
 * Used when the registry's EngineRegisteredFunc lacks param-type metadata
...
static const EngineType *propagate_template(EngineArena *a, const char *recv_qn, const char *method_name,
                                         const EngineType *const *recv_targs, int recv_targ_count,
                                         const EngineType *return_t) {
    if (!return_t || return_t->kind != ENGINE_TYPE_NAMED)
        return return_t;
    ...
    if (is_t0) {
        const EngineType **args = (const EngineType **)engine_arena_alloc(a, 2 * sizeof(*args));
        if (!args)
            return return_t;
        args[0] = recv_targs[0];
        args[1] = NULL;
        return engine_type_template(a, ret_qn, args, 1);
    }
    ...
    if (is_map_like(recv_qn) && recv_targ_count >= 2) {
        ...
        if (strcmp(method_name, "keySet") == 0) {
            args[0] = recv_targs[0];
            args[1] = NULL;
            return engine_type_template(a, "java.util.Set", args, 1);
        }
        if (strcmp(method_name, "values") == 0) {
            args[0] = recv_targs[1];
            args[1] = NULL;
            return engine_type_template(a, "java.util.Collection", args, 1);
        }
    }
    return return_t;
}

/* Given a method-invocation node and its resolved EngineRegisteredFunc, walk
...
static uint32_t bind_lambda_args(JavaLSPContext *ctx, TSNode call_node,
                                 const EngineRegisteredFunc *resolved, const EngineType *recv_type) {
    uint32_t handled_mask = 0;
    TSNode args_node = ts_node_child_by_field_name(call_node, "arguments", 9);
    if (ts_node_is_null(args_node))
        return handled_mask;
    const EngineType *const *param_types = NULL;
    if (resolved && resolved->signature && resolved->signature->kind == ENGINE_TYPE_FUNC) {
    ...
    const EngineType *const *recv_targs = NULL;
    int recv_targ_count = 0;
    const char *recv_qn = NULL;
    if (recv_type && recv_type->kind == ENGINE_TYPE_TEMPLATE) {
    ...
    } else if (recv_type && recv_type->kind == ENGINE_TYPE_NAMED) {
    ...
    for (uint32_t i = 0; i < n && i < 32; i++) {
        ...
        if (!expected && recv_qn && mname && recv_targ_count > 0) {
            ...
            int h_arity = 0;
            const EngineType *h_p0 = NULL;
            const EngineType *h_p1 = NULL;
                ...
                if (!ts_node_is_null(body_node)) {
                    ...
                    if (!ts_node_is_null(params_node)) {
                        ...
                        if (strcmp(pn_kind, "identifier") == 0) {
                            ...
                            if (pname)
                                engine_scope_bind(ctx->current_scope, pname, h_p0);
                            ...
                            for (uint32_t pi = 0; pi < pc; pi++) {
                                ...
                                const char *pk = ts_node_type(p);
                                char *pname = NULL;
                                const EngineType *pt = (idx == 0) ? h_p0 : h_p1;
        ...
        const EngineType *const *fi_targs = NULL;
        int fi_targ_count = 0;
        if (expected->kind == ENGINE_TYPE_NAMED) {
            fi_qn = expected->data.named.qualified_name;
        } else if (expected->kind == ENGINE_TYPE_TEMPLATE) {
         ...
         * cases below. */
        const EngineType *resolved_fi_targs[8] = {0};
        ...
        if (!ts_node_is_null(params_node)) {
            ...
            if (strcmp(pn_kind, "identifier") == 0) {
                ...
                for (uint32_t pi = 0; pi < pc; pi++) {
                    ...
                    if (pname) {
                        engine_scope_bind(ctx->current_scope, pname, pt ? pt : engine_type_unknown());
                        idx++;
                    }
                }
            }
 ...
 *
 * Emits a EngineResolvedCall edge for the referenced method when we can pin
...
static void resolve_method_reference(JavaLSPContext *ctx, TSNode mref,
    ...
    if (outer_resolved && outer_resolved->signature &&
        outer_resolved->signature->kind == ENGINE_TYPE_FUNC &&
        ...
        const EngineType *const *pts = outer_resolved->signature->data.func.param_types;
        int ptc = 0;
        while (pts[ptc]) {
            ptc++;
        }
        const EngineType *expected = (arg_index >= 0 && arg_index < ptc) ? pts[arg_index] : NULL;
        if (expected) {
            const char *fi_qn = NULL;
            if (expected->kind == ENGINE_TYPE_NAMED)
                fi_qn = expected->data.named.qualified_name;
    ...
    if (lhs_is_type) {
        ...
        if (lhs_t && lhs_t->kind == ENGINE_TYPE_NAMED)
            type_qn = lhs_t->data.named.qualified_name;
        else if (lhs_t && lhs_t->kind == ENGINE_TYPE_TEMPLATE)
    ...
    if (strcmp(mname, "new") == 0) {
        ...
        const EngineRegisteredFunc *cf =
            engine_registry_lookup_method(ctx->registry, type_qn, short_name);
    ...
    if (m) {
        ...
        java_emit_unresolved_reference_at(ctx,
                                          engine_arena_sprintf(ctx->arena, "%s.%s", type_qn, mname),
...
static const EngineRegisteredFunc *lookup_method_for_call(JavaLSPContext *ctx, TSNode call,
                                                       const EngineType **out_recv_type) {
    if (out_recv_type)
        *out_recv_type = NULL;
    TSNode obj = ts_node_child_by_field_name(call, "object", 6);
    TSNode name_node = ts_node_child_by_field_name(call, "name", 4);
    ...
    if (recv && recv->kind == ENGINE_TYPE_NAMED)
        recv_qn = recv->data.named.qualified_name;
    else if (recv && recv->kind == ENGINE_TYPE_TEMPLATE)
        recv_qn = recv->data.template_type.template_name;
    if (recv_qn)
...
static void java_resolve_calls_in_node_inner(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "method_invocation") == 0) {
        ...
        const EngineType *recv_type = NULL;
        const EngineRegisteredFunc *outer = lookup_method_for_call(ctx, node, &recv_type);
        if (outer) {
        ...
        TSNode type_node = ts_node_child_by_field_name(node, "type", 4);
        if (!ts_node_is_null(type_node)) {
            const EngineType *t = java_parse_type_node(ctx, type_node);
            const char *qn = NULL;
            if (t && t->kind == ENGINE_TYPE_NAMED)
                qn = t->data.named.qualified_name;
            else if (t && t->kind == ENGINE_TYPE_TEMPLATE)
            ...
            if (qn) {
                ...
                const EngineRegisteredFunc *cf =
                    engine_registry_lookup_method_by_args(ctx->registry, qn, short_name, arity);
                if (!cf)
                    cf = engine_registry_lookup_method(ctx->registry, qn, short_name);
                if (cf) {
...
static const char **split_generic_args(EngineArena *a, const char *inside, int *out_count) {
    *out_count = 0;
    if (!inside || !inside[0])
        return NULL;
    const char *args[16];
    ...
    for (const char *p = inside; *p; p++) {
        if (*p == '<')
            ...
            if (count < 16 && seg_end > seg_start) {
                args[count++] = engine_arena_strndup(a, seg_start, (size_t)(seg_end - seg_start));
            }
            seg_start = p + 1;
        }
    }
    ...
    if (count == 0)
        return NULL;
    const char **result = (const char **)engine_arena_alloc(a, (size_t)(count + 1) * sizeof(*result));
...
/* Parse a type-text into a EngineType, with full inner-class qualification.
...
static const EngineType *parse_param_text_full(EngineArena *a, const char *text, const char *parent_class,
    ...
    if (no_arr) {
        ...
        if (lt) {
            ...
            if (gt && gt > lt + 1) {
                gen_args_inner = engine_arena_strndup(a, lt + 1, (size_t)(gt - lt - 1));
            }
        }
    }

    const EngineType *base = NULL;
    if (no_gen && is_java_primitive(no_gen)) {
        base = engine_type_builtin(a, no_gen);
    } else if (no_gen) {
        if (strchr(no_gen, '.')) {
            base = engine_type_named(a, no_gen);
        } else {
            /* Try java.lang well-knowns. */
            for (int i = 0; JAVA_LANG_TYPES[i]; i++) {
                if (strcmp(JAVA_LANG_TYPES[i], no_gen) == 0) {
                    base = engine_type_named(a, engine_arena_sprintf(a, "java.lang.%s", no_gen));
                    break;
                }
            }
            ...
            if (!base && parent_class && reg) {
                ...
                while (cur) {
                    const char *cand = engine_arena_sprintf(a, "%s.%s", cur, no_gen);
                    if (engine_registry_lookup_type(reg, cand)) {
                        base = engine_type_named(a, cand);
                        break;
                    }
                    const char *last_dot = strrchr(cur, '.');
                    if (!last_dot)
                        break;
                    cur = engine_arena_strndup(a, cur, (size_t)(last_dot - cur));
                }
            }
            ...
            if (!base && reg) {
                for (int i = 0; JAVA_FALLBACK_PACKAGES[i]; i++) {
                    const char *cand =
                        engine_arena_sprintf(a, "%s.%s", JAVA_FALLBACK_PACKAGES[i], no_gen);
                    if (engine_registry_lookup_type(reg, cand)) {
                        base = engine_type_named(a, cand);
                        break;
                    }
            ...
            if (!base) {
                if (module_qn && module_qn[0]) {
                    base = engine_type_named(a, engine_arena_sprintf(a, "%s.%s", module_qn, no_gen));
                } else {
                    base = engine_type_named(a, no_gen);
                }
            }
    ...
    if (!base)
        base = engine_type_unknown();
    /* If we extracted generic args and the base is NAMED, wrap as TEMPLATE
     * with each arg recursively parsed. */
    if (gen_args_inner && base && base->kind == ENGINE_TYPE_NAMED) {
        int gc = 0;
        const char **arg_strs = split_generic_args(a, gen_args_inner, &gc);
        if (arg_strs && gc > 0) {
            const EngineType **gargs =
                (const EngineType **)engine_arena_alloc(a, (size_t)(gc + 1) * sizeof(*gargs));
            for (int i = 0; i < gc; i++) {
                gargs[i] = parse_param_text_full(a, arg_strs[i], parent_class, module_qn, reg);
            }
            gargs[gc] = NULL;
            base = engine_type_template(a, base->data.named.qualified_name, gargs, gc);
        }
    }
    while (dim-- > 0)
        base = engine_type_slice(a, base);
    return base;
}

/* Backwards-compatible wrapper for callers that don't know the parent_class
 * context. Used by engine_run_java_lsp_cross. */
static const EngineType *parse_param_text(EngineArena *a, const char *text, const char *module_qn) {
...
static const EngineType *java_signature_parse_type(EngineArena *arena, const char *text,
                                                void *parser_ctx) {
...
static void register_local_func_or_type_from_file(JavaLSPContext *ctx, EngineTypeRegistry *reg,
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (engine_registry_lookup_type(reg, d->qualified_name))
            continue;
        EngineRegisteredType stub;
        memset(&stub, 0, sizeof(stub));
        stub.qualified_name = d->qualified_name;
        stub.short_name = d->name;
        stub.is_interface = (strcmp(d->label, "Interface") == 0);
        engine_registry_add_type(reg, stub);
    }
     ...
     * registry entry in place via the type lookup (EngineRegisteredType *). */
    for (int i = 0; i < result->defs.count; i++) {
        EngineDefinition *d = &result->defs.items[i];
        if (!d->qualified_name || !d->name || !d->label)
            continue;
        ...
        const EngineRegisteredType *existing = engine_registry_lookup_type(reg, d->qualified_name);
        if (!existing)
            continue;
        EngineRegisteredType rt = *existing; /* Will be re-added; lookup_type returns the
                                             registry's stored copy. We instead build a
                                             local copy and let engine_registry_add_type's
        ...
        EngineRegisteredType *slot = NULL;
        ...
        if (d->base_classes) {
            ...
            if (bc_count > 0) {
                const char **emb =
                    (const char **)engine_arena_alloc(a, (size_t)(bc_count + 1) * sizeof(*emb));
                    ...
                    if (!strchr(no_gen, '.')) {
                        ...
                        for (int k = 0; JAVA_LANG_TYPES[k]; k++) {
                            if (strcmp(JAVA_LANG_TYPES[k], no_gen) == 0) {
                                resolved = engine_arena_sprintf(a, "java.lang.%s", no_gen);
                                break;
                            }
                        }
                        ...
                        if (!parent_qn && d->qualified_name) {
                            ...
                            if (last_dot_qn) {
                                parent_qn =
                                    engine_arena_strndup(a, d->qualified_name,
                        ...
                        if (!resolved && module_qn && module_qn[0]) {
                            resolved = engine_arena_sprintf(a, "%s.%s", module_qn, no_gen);
                        }
                        emb[j] = resolved ? resolved : engine_arena_strdup(a, no_gen);
                    } else {
                        emb[j] = engine_arena_strdup(a, no_gen);
                    }
                }
                emb[bc_count] = NULL;
        ...
        if (!slot->embedded_types && !slot->is_interface) {
            const char **emb = (const char **)engine_arena_alloc(a, 2 * sizeof(*emb));
            if (emb) {
     ...
     * since EngineDefinition doesn't carry a per-field type for Java. See
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (d->signature_param_types || d->signature_param_count > 0) {
            ...
            ptypes = engine_type_materialize_signature_params(a, d->signature_param_types,
                                                           d->signature_param_count,
                                                           java_signature_parse_type, &parse_ctx);
        } else if (!d->signature_param_types && d->signature_param_count == 0) {
            /* Compatibility for callers that still construct EngineDefinition by
            ...
            if (d->param_types) {
                ...
                if (pc > 0) {
                    ptypes =
                        (const EngineType **)engine_arena_alloc(a, (size_t)(pc + 1) * sizeof(*ptypes));
                    for (int j = 0; j < pc; j++) {
                        ptypes[j] =
        ...
        const EngineType **rtypes = NULL;
        if (d->return_type && d->return_type[0]) {
            rtypes = (const EngineType **)engine_arena_alloc(a, 2 * sizeof(*rtypes));
        ...
        rf.signature = engine_type_func(a, param_names, ptypes, rtypes);
        if (d->parent_class) {
...
static void patch_method_signatures_from_ast(JavaLSPContext *ctx, EngineTypeRegistry *reg,
                                             TSNode class_node, const char *enclosing_class_qn);

static void patch_one_method(JavaLSPContext *ctx, EngineTypeRegistry *reg, TSNode method_node,
    ...
    const EngineType **ptypes = NULL;
    if (pn > 0) {
        ptypes = (const EngineType **)engine_arena_alloc(ctx->arena, (size_t)(pn + 1) * sizeof(*ptypes));
        if (!ptypes)
            return;
    }
    int idx = 0;
    ...
    EngineRegisteredFunc *slot = NULL;
    EngineRegisteredFunc *first_qn_match = NULL;
    for (int fi = 0; fi < reg->func_count; fi++) {
        EngineRegisteredFunc *candidate = &reg->funcs[fi];
        if (!candidate->qualified_name || strcmp(candidate->qualified_name, method_qn) != 0)
            continue;
        if (!first_qn_match)
            first_qn_match = candidate;
        if (!candidate->signature || candidate->signature->kind != ENGINE_TYPE_FUNC)
            continue;
        int candidate_arity = 0;
        const EngineType *const *candidate_params = candidate->signature->data.func.param_types;
        if (candidate_params) {
    ...
    if (!is_constructor) {
        ...
        if (!ts_node_is_null(rtype)) {
            rtypes = (const EngineType **)engine_arena_alloc(ctx->arena, 2 * sizeof(*rtypes));
    ...
    if (is_constructor) {
        rtypes = (const EngineType **)engine_arena_alloc(ctx->arena, 2 * sizeof(*rtypes));
        rtypes[0] = engine_type_named(ctx->arena, class_qn);
        rtypes[1] = NULL;
    }

    /* Rebuild the FUNC type with both param-types and return-types. */
    slot->signature = engine_type_func(ctx->arena, NULL, ptypes, rtypes);
}
...
static void patch_method_signatures_from_ast(JavaLSPContext *ctx, EngineTypeRegistry *reg,
    ...
    if (enclosing_class_qn) {
        class_qn = engine_arena_sprintf(ctx->arena, "%s.%s", enclosing_class_qn, cname);
    } else if (ctx->module_qn && ctx->module_qn[0]) {
        class_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    } else {
        class_qn = engine_arena_strdup(ctx->arena, cname);
...
static void append_field_to_class(EngineTypeRegistry *reg, EngineArena *a, const char *class_qn,
                                  const char *field_name, const EngineType *ftype) {
    EngineRegisteredType *slot = NULL;
    for (int ti = 0; ti < reg->type_count; ti++) {
    ...
    const char **new_names =
        (const char **)engine_arena_alloc(a, (size_t)(existing + 2) * sizeof(*new_names));
    const EngineType **new_types =
        (const EngineType **)engine_arena_alloc(a, (size_t)(existing + 2) * sizeof(*new_types));
    ...
    new_names[existing] = engine_arena_strdup(a, field_name);
    new_types[existing] = ftype ? ftype : engine_type_unknown();
...
static void populate_class_fields_from_ast(JavaLSPContext *ctx, EngineTypeRegistry *reg,
                                           TSNode class_node, const char *enclosing_class_qn) {
    if (ts_node_is_null(class_node))
        return;
    const char *kind = ts_node_type(class_node);
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(ck, "field_declaration") == 0) {
            ...
            const EngineType *ftype =
                ts_node_is_null(tn) ? engine_type_unknown() : java_parse_type_node(ctx, tn);
...
/* ── Top-level entry: engine_run_java_lsp ────────────────────────────── */

extern const TSLanguage *tree_sitter_java(void);

void engine_run_java_lsp(EngineArena *arena, EngineFileResult *result, const char *source, int source_len,
                      TSNode root) {
    ...
    engine_java_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->imports.count; i++) {
        ...
        if (!imp->local_name || !imp->module_path)
            continue;
        java_lsp_add_import(&ctx, imp->local_name, imp->module_path, ENGINE_JAVA_IMPORT_TYPE);
    }
...
void engine_java_register_lsp_defs(EngineArena *arena, EngineTypeRegistry *reg, const EngineLSPDef *defs,
                                int def_count) {
    for (int i = 0; i < def_count; i++) {
        ...
        if (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Interface") == 0 ||
            ...
            const EngineType **ptypes = engine_type_materialize_signature_params(
            ...
            if (d->return_types && d->return_types[0]) {
                ...
                const char *first;
                if (bar) {
                    first = engine_arena_strndup(arena, no_pipe, (size_t)(bar - no_pipe));
                } else {
                    first = no_pipe;
                }
            ...
            const EngineType **rtypes = NULL;
            if (ret) {
                rtypes = (const EngineType **)engine_arena_alloc(arena, 2 * sizeof(*rtypes));
                rtypes[0] = ret;
                rtypes[1] = NULL;
            }
            rf.signature = engine_type_func(arena, NULL, ptypes, rtypes);
            engine_registry_add_func(reg, rf);
        }
    }
}

void engine_run_java_lsp_cross(EngineArena *arena, const char *source, int source_len,
    ...
    engine_java_register_lsp_defs(arena, &reg, defs, def_count);
    ...
    for (int i = 0; i < import_count; i++) {
        if (!import_names[i] || !import_qns[i])
            continue;
        java_lsp_add_import(&ctx, import_names[i], import_qns[i], ENGINE_JAVA_IMPORT_TYPE);
    }
...
void engine_batch_java_lsp_cross(EngineArena *arena, EngineBatchJavaLSPFile *files, int file_count,
                              EngineResolvedCallArray *out) {
    if (!files || file_count <= 0)
        return;
    for (int i = 0; i < file_count; i++) {
        engine_run_java_lsp_cross(arena, files[i].source, files[i].source_len, files[i].module_qn,
