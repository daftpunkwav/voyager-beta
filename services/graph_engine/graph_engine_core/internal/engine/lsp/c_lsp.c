...
/* Safe kind accessor — returns ENGINE_TYPE_UNKNOWN for NULL types.
 * Prevents SEGV in c_eval_expr_type_inner on unusual C++ AST shapes. */
static inline EngineTypeKind safe_kind(const EngineType *t) {
    return t ? t->kind : ENGINE_TYPE_UNKNOWN;
}
...
static const EngineType *c_lookup_field_type(CLSPContext *ctx, const char *type_qn,
...
static const char *type_to_qn(const EngineType *t);
const EngineType *c_simplify_type(CLSPContext *ctx, const EngineType *t, bool unwrap_pointer);
const EngineType *c_eval_expr_type(CLSPContext *ctx, TSNode node);
...
void c_lsp_init(CLSPContext *ctx, EngineArena *arena, const char *source, int source_len,
                const EngineTypeRegistry *registry, const char *module_qn, bool cpp_mode,
    ...
    ctx->source_origin = ENGINE_SOURCE_ORIGIN_RAW;
    ctx->current_scope = engine_scope_push(arena, NULL);
...
void c_lsp_add_include(CLSPContext *ctx, const char *header_path, const char *ns_qn) {
    if (ctx->include_count % 32 == 0) {
        ...
        const char **new_paths =
            (const char **)engine_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(const char *));
        const char **new_qns =
            (const char **)engine_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(const char *));
    ...
    ctx->include_paths[ctx->include_count] = engine_arena_strdup(ctx->arena, header_path);
    ctx->include_ns_qns[ctx->include_count] = engine_arena_strdup(ctx->arena, ns_qn);
    ctx->include_count++;
}
...
static void c_add_using_namespace(CLSPContext *ctx, const char *ns_qn) {
    if (ctx->using_ns_count >= ctx->using_ns_cap) {
        ...
        const char **new_arr =
            (const char **)engine_arena_alloc(ctx->arena, new_cap * sizeof(const char *));
        if (!new_arr)
            return;
    ...
    ctx->using_namespaces[ctx->using_ns_count++] = engine_arena_strdup(ctx->arena, ns_qn);
}
...
static void c_add_using_decl(CLSPContext *ctx, const char *short_name, const char *full_qn) {
    ...
    ctx->using_decl_names[ctx->using_decl_count] = engine_arena_strdup(ctx->arena, short_name);
    ctx->using_decl_qns[ctx->using_decl_count] = engine_arena_strdup(ctx->arena, full_qn);
...
static void c_add_ns_alias(CLSPContext *ctx, const char *alias, const char *full_qn) {
    ...
    ctx->ns_alias_names[ctx->ns_alias_count] = engine_arena_strdup(ctx->arena, alias);
    ctx->ns_alias_qns[ctx->ns_alias_count] = engine_arena_strdup(ctx->arena, full_qn);
    ctx->ns_alias_count++;
}
...
static void c_add_fp_target(CLSPContext *ctx, const char *var_name, const char *target_qn) {
    if (ctx->fp_count >= ctx->fp_cap) {
        ...
        const char **new_qns =
            (const char **)engine_arena_alloc(ctx->arena, new_cap * sizeof(const char *));
        const EngineScope **new_scopes =
            (const EngineScope **)engine_arena_alloc(ctx->arena, new_cap * sizeof(const EngineScope *));
        ...
        if (ctx->fp_var_names && ctx->fp_count > 0) {
            ...
            memcpy(new_scopes, ctx->fp_binding_scopes, ctx->fp_count * sizeof(const EngineScope *));
        }
    ...
    ctx->fp_var_names[ctx->fp_count] = engine_arena_strdup(ctx->arena, var_name);
    ctx->fp_target_qns[ctx->fp_count] = target_qn ? engine_arena_strdup(ctx->arena, target_qn) : NULL;
    ctx->fp_binding_scopes[ctx->fp_count] = ctx->current_scope;
    ctx->fp_count++;
}

static bool c_scope_declares_name(const EngineScope *scope, const char *var_name) {
    if (!scope || !var_name) {
        return false;
    }
    for (const EngineScopeChunk *chunk = scope->chunks; chunk; chunk = chunk->next) {
...
static int c_find_fp_binding_index(CLSPContext *ctx, const char *var_name) {
    ...
    for (int i = ctx->fp_count - 1; i >= 0; i--) {
        ...
        const EngineScope *binding_scope = ctx->fp_binding_scopes[i];
        for (const EngineScope *scope = ctx->current_scope; scope; scope = scope->parent) {
...
static void c_update_fp_target(CLSPContext *ctx, const char *var_name, const char *target_qn) {
    ...
    int index = c_find_fp_binding_index(ctx, var_name);
    if (index >= 0) {
        ctx->fp_target_qns[index] = target_qn ? engine_arena_strdup(ctx->arena, target_qn) : NULL;
    }
}
    ...
    *out_has_cast = false;
    ...
    for (uint32_t i = 0; i < anc; i++) {
        ...
        if (len < 2 || text[0] != '"' || text[len - 1] != '"')
            continue;
        char *name = engine_arena_strndup(ctx->arena, text + 1, len - 2);
        if (!name || !name[0])
            continue;
...
static void c_add_pending_template_call(CLSPContext *ctx, const char *func_qn,
    ...
    if (ctx->pending_tc_count >= ctx->pending_tc_cap) {
        ...
        void *new_arr = engine_arena_alloc(ctx->arena, sz);
        if (!new_arr)
            return;
    ...
    const char *func_qn_copy = engine_arena_strdup(ctx->arena, func_qn);
    const char *type_param_copy = engine_arena_strdup(ctx->arena, type_param);
    const char *method_name_copy = engine_arena_strdup(ctx->arena, method_name);
...
static void c_resolve_pending_template_calls(CLSPContext *ctx, const EngineRegisteredFunc *callee,
                                             const EngineType **call_arg_types, int call_arg_count) {
    ...
    const EngineType *param_map[8] = {0};
    int tpn_count = 0;
    ...
    if (callee->signature && callee->signature->kind == ENGINE_TYPE_FUNC &&
        ...
        for (int i = 0; i < limit; i++) {
            const EngineType *formal = callee->signature->data.func.param_types[i];
            if (!formal || !call_arg_types[i])
                continue;
            // Unwrap references/pointers
            while (formal &&
                   (formal->kind == ENGINE_TYPE_REFERENCE || formal->kind == ENGINE_TYPE_RVALUE_REF ||
                    formal->kind == ENGINE_TYPE_POINTER)) {
                formal = (formal->kind == ENGINE_TYPE_POINTER) ? formal->data.pointer.elem
                                                            : formal->data.reference.elem;
            }
            if (formal && formal->kind == ENGINE_TYPE_TYPE_PARAM) {
                for (int j = 0; j < tpn_count; j++) {
                    if (strcmp(tpn[j], formal->data.type_param.name) == 0) {
                        const EngineType *arg = call_arg_types[i];
    ...
    for (int i = 0; i < ctx->pending_tc_count; i++) {
        ...
        for (int j = 0; j < tpn_count; j++) {
            ...
            const EngineRegisteredFunc *m = c_lookup_member(ctx, concrete_qn, method);
            if (m) {
...
static const EngineType **c_extract_call_arg_types(CLSPContext *ctx, TSNode call_node,
                                                int *out_count) {
    ...
    const EngineType **types =
        (const EngineType **)engine_arena_alloc(ctx->arena, (nc + 1) * sizeof(const EngineType *));
    if (!types) {
        *out_count = 0;
        return NULL;
    }
...
static void c_parse_template_params(CLSPContext *ctx, TSNode template_decl) {
    ...
    const char **names =
        (const char **)engine_arena_alloc(ctx->arena, (nc + 1) * sizeof(const char *));
    const EngineType **defaults =
        (const EngineType **)engine_arena_alloc(ctx->arena, (nc + 1) * sizeof(const EngineType *));
    if (!names || !defaults)
        return;
    ...
    for (uint32_t i = 0; i < nc && idx < 16; i++) {
        ...
        if (strcmp(pk, "type_parameter_declaration") == 0 ||
            ...
            if (!pname)
                pname = engine_arena_sprintf(ctx->arena, "T%d", idx);
...
static const EngineType *c_resolve_template_param(CLSPContext *ctx, const char *param_name) {
...
static const char *c_build_qn(CLSPContext *ctx, const char *text) {
    ...
    size_t len = strlen(text);
    char *buf = (char *)engine_arena_alloc(ctx->arena, len + 1);
    if (!buf)
        return text;
    size_t j = 0;
...
static const char *c_resolve_name(CLSPContext *ctx, const char *name) {
    ...
    if (ctx->enclosing_class_qn) {
        const EngineRegisteredFunc *f =
            engine_registry_lookup_method(ctx->registry, ctx->enclosing_class_qn, name);
        if (f)
    ...
    for (int i = 0; i < ctx->using_ns_count; i++) {
        const EngineRegisteredFunc *f =
            engine_registry_lookup_symbol(ctx->registry, ctx->using_namespaces[i], name);
        if (f)
            return f->qualified_name;
    }
    ...
    if (ctx->current_namespace) {
        ...
        while (ns && ns[0]) {
            const EngineRegisteredFunc *f = engine_registry_lookup_symbol(ctx->registry, ns, name);
            if (f)
                return f->qualified_name;
            // Walk up: "a.b.c" -> "a.b" -> "a"
            ...
            if (!dot)
                break;
            char *parent = (char *)engine_arena_alloc(ctx->arena, (size_t)(dot - ns) + 1);
            if (!parent)
                break;
    ...
    if (ctx->module_qn) {
        const EngineRegisteredFunc *f =
            engine_registry_lookup_symbol(ctx->registry, ctx->module_qn, name);
        if (f)
            return f->qualified_name;
    }
...
static const char *extract_namespace_from_qn(EngineArena *arena, const char *qn) {
    if (!qn)
        return NULL;
    ...
    char *ns = (char *)engine_arena_alloc(arena, len + 1);
    if (!ns)
        return NULL;
    memcpy(ns, qn, len);
    ns[len] = '\0';
...
static const char *c_adl_resolve(CLSPContext *ctx, const char *name, TSNode call_node) {
    ...
    for (uint32_t i = 0; i < nc && ns_count < 8; i++) {
        ...
        const EngineType *arg_type = c_eval_expr_type(ctx, arg);
    ...
    for (int i = 0; i < ns_count; i++) {
        const EngineRegisteredFunc *f = engine_registry_lookup_symbol(ctx->registry, namespaces[i], name);
        if (!f && ctx->module_qn) {
            const char *prefixed =
                engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, namespaces[i]);
            f = engine_registry_lookup_symbol(ctx->registry, prefixed, name);
        }
        if (f)
            return f->qualified_name;
    }
...
static const char *c_resolve_name_to_func_qn(CLSPContext *ctx, const char *name) {
    ...
    // Global scope
    const EngineRegisteredFunc *gf = engine_registry_lookup_func(ctx->registry, name);
    if (gf)
...
static const char *c_exact_function_target_from_expr(CLSPContext *ctx, TSNode expr) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (engine_scope_contains(ctx->current_scope, name)) {
            return NULL;
        }
    ...
    if (!name || engine_scope_contains(ctx->current_scope, name)) {
        return NULL;
    }
    return c_resolve_name_to_func_qn(ctx, name);
}
...
static const EngineType *c_resolve_name_to_type(CLSPContext *ctx, const char *name) {
    if (!name)
        return engine_type_unknown();

    // Builtin types
    if (is_c_builtin_type(name))
        return engine_type_builtin(ctx->arena, name);

    // Scope lookup: typedef/using aliases are bound here.
    // If the name resolves to an alias, follow it to the underlying type.
    {
        const EngineType *scoped = engine_scope_lookup(ctx->current_scope, name);
        if (scoped && scoped->kind == ENGINE_TYPE_ALIAS) {
            const EngineType *resolved = engine_type_resolve_alias(scoped);
            if (resolved && !engine_type_is_unknown(resolved))
                return resolved;
        }
    }
    ...
    if (ctx->in_template && ctx->template_param_names) {
        for (int i = 0; i < ctx->template_param_count; i++) {
            if (ctx->template_param_names[i] && strcmp(ctx->template_param_names[i], name) == 0) {
                ...
                return engine_type_type_param(ctx->arena, name);
            }
        }
    }
    ...
    for (int i = 0; i < ctx->using_decl_count; i++) {
        if (strcmp(ctx->using_decl_names[i], name) == 0) {
            const EngineRegisteredType *rt =
                engine_registry_lookup_type(ctx->registry, ctx->using_decl_qns[i]);
            if (rt)
                return engine_type_named(ctx->arena, rt->qualified_name);
        }
    }

    // Enclosing class (nested types: Factory::Product)
    if (ctx->enclosing_class_qn) {
        const char *qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, name);
        const EngineRegisteredType *rt = engine_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return engine_type_named(ctx->arena, qn);
    }

    // Current namespace
    if (ctx->current_namespace) {
        const char *qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, name);
        const EngineRegisteredType *rt = engine_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return engine_type_named(ctx->arena, qn);
    }
    ...
    // Module scope
    if (ctx->module_qn) {
        const char *qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, name);
        const EngineRegisteredType *rt = engine_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return engine_type_named(ctx->arena, qn);
    }
    ...
    // Using namespaces
    for (int i = 0; i < ctx->using_ns_count; i++) {
        const char *qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->using_namespaces[i], name);
        const EngineRegisteredType *rt = engine_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return engine_type_named(ctx->arena, qn);
    }
    ...
    if (ctx->cpp_mode) {
        const char *std_qn = engine_arena_sprintf(ctx->arena, "std.%s", name);
        const EngineRegisteredType *rt = engine_registry_lookup_type(ctx->registry, std_qn);
        if (rt)
            return engine_type_named(ctx->arena, std_qn);
    }

    // Unresolved — return named with module prefix
    if (ctx->module_qn) {
        return engine_type_named(ctx->arena,
                              engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, name));
    }
    return engine_type_named(ctx->arena, name);
}

// ============================================================================
// c_parse_type_node: AST type node -> EngineType
// ============================================================================

const EngineType *c_parse_type_node(CLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "primitive_type") == 0) {
        ...
        return name ? engine_type_builtin(ctx->arena, name) : engine_type_unknown();
    }
    ...
    if (strcmp(kind, "qualified_identifier") == 0 || strcmp(kind, "scoped_type_identifier") == 0) {
        ...
        if (!ts_node_is_null(name_child) &&
            ...
            if (scope_text) {
                const char *scoped =
                    engine_arena_sprintf(ctx->arena, "%s::%s", scope_text, tmpl_name_text);
            ...
            const EngineRegisteredType *trt = engine_registry_lookup_type(ctx->registry, template_qn);
            if (!trt && ctx->module_qn) {
                const char *mod_qn =
                    engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, template_qn);
                trt = engine_registry_lookup_type(ctx->registry, mod_qn);
                if (trt)
                    template_qn = mod_qn;
            }

            // Parse template arguments
            const EngineType *targs[16] = {
                NULL}; /* zero-fill: engine_type_substitute requires NULL-terminated args
            ...
            if (targ_count > 0)
                return engine_type_template(ctx->arena, template_qn, targs, targ_count);
            return engine_type_named(ctx->arena, template_qn);
        }
            ...
            if (scope_text) {
                ...
                if (alias_qn) {
                    if (!ts_node_is_null(name_child)) {
                        ...
                        if (name)
                            text = (char *)engine_arena_sprintf(ctx->arena, "%s::%s", alias_qn, name);
                    }
                }
            }
        }
    ...
    if (strcmp(kind, "template_type") == 0) {
        ...
        if (!trt && ctx->current_namespace) {
            ...
            // "std.shared_ptr")
            const char *ns_qn =
                engine_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, template_qn);
            trt = engine_registry_lookup_type(ctx->registry, ns_qn);
            if (trt)
                template_qn = ns_qn;
            // Also try module + namespace
            if (!trt && ctx->module_qn) {
                const char *full_qn = engine_arena_sprintf(ctx->arena, "%s.%s.%s", ctx->module_qn,
                                                        ctx->current_namespace, template_qn);
                trt = engine_registry_lookup_type(ctx->registry, full_qn);
                if (trt)
                    template_qn = full_qn;
            }
        }
        if (!trt) {
            // Try with std:: prefix
            const char *std_qn = engine_arena_sprintf(ctx->arena, "std.%s", template_qn);
            trt = engine_registry_lookup_type(ctx->registry, std_qn);
            if (trt)
                template_qn = std_qn;
        }
        ...
        // Parse template arguments
        const EngineType *targs[16] = {
            NULL}; /* zero-fill: engine_type_substitute requires NULL-terminated args (uninitialized
    ...
    if (strcmp(kind, "pointer_declarator") == 0 || strcmp(kind, "pointer_type") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            return engine_type_pointer(ctx->arena,
                                    c_parse_type_node(ctx, ts_node_named_child(node, 0)));
        }
        return engine_type_pointer(ctx->arena, engine_type_unknown());
    }
    ...
    if (strcmp(kind, "reference_declarator") == 0 ||
        ...
        if (ts_node_named_child_count(node) > 0) {
            return engine_type_reference(ctx->arena,
                                      c_parse_type_node(ctx, ts_node_named_child(node, 0)));
        }
        return engine_type_reference(ctx->arena, engine_type_unknown());
    }

    // auto keyword
    if (strcmp(kind, "auto") == 0 || strcmp(kind, "placeholder_type_specifier") == 0) {
        return engine_type_unknown(); // will be resolved from initializer
    }
    ...
    if (strcmp(kind, "type_qualifier") == 0) {
        return engine_type_unknown(); // qualifier itself has no type
    }

    // array_declarator: int[10]
    if (strcmp(kind, "array_declarator") == 0 || strcmp(kind, "array_type") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            return engine_type_slice(ctx->arena, c_parse_type_node(ctx, ts_node_named_child(node, 0)));
        }
        return engine_type_unknown();
    }
    ...
    if (strcmp(kind, "function_declarator") == 0 ||
        ...
        return engine_type_func(ctx->arena, NULL, NULL, NULL);
    }
    ...
    if (strcmp(kind, "struct_specifier") == 0 || strcmp(kind, "class_specifier") == 0) {
        ...
        // Anonymous struct
        EngineType *t = (EngineType *)engine_arena_alloc(ctx->arena, sizeof(EngineType));
        memset(t, 0, sizeof(EngineType));
        t->kind = ENGINE_TYPE_STRUCT;
        return t;
    }
    ...
    if (strcmp(kind, "enum_specifier") == 0) {
        ...
        return engine_type_builtin(ctx->arena, "int");
    }
    ...
    if (strcmp(kind, "dependent_type") == 0 || strcmp(kind, "dependent_name") == 0) {
        char *text = c_node_text(ctx, node);
        if (text)
            return engine_type_named(ctx->arena, c_build_qn(ctx, text));
        return engine_type_unknown();
    }
...
const EngineType *c_simplify_type(CLSPContext *ctx, const EngineType *t, bool unwrap_pointer) {
    for (int i = 0; i < 64 && t; i++) {
        ...
        if (t->kind == ENGINE_TYPE_TYPE_PARAM && ctx->in_template) {
            const EngineType *resolved = c_resolve_template_param(ctx, t->data.type_param.name);
            if (resolved) {
                t = resolved;
                continue;
            }
        ...
        if (t->kind == ENGINE_TYPE_TEMPLATE && is_smart_ptr(t->data.template_type.template_name)) {
        ...
        if (t->kind == ENGINE_TYPE_NAMED) {
            const EngineRegisteredFunc *op = engine_registry_lookup_method(
                ctx->registry, t->data.named.qualified_name, "operator->");
            if (op && op->signature && op->signature->kind == ENGINE_TYPE_FUNC &&
        ...
        if (t->kind == ENGINE_TYPE_TEMPLATE) {
            const char *tqn = t->data.template_type.template_name;
            const EngineRegisteredFunc *op = c_lookup_member(ctx, tqn, "operator->");
            if (op && op->signature && op->signature->kind == ENGINE_TYPE_FUNC &&
                op->signature->data.func.return_types && op->signature->data.func.return_types[0]) {
                const EngineType *ret = op->signature->data.func.return_types[0];
                // Substitute template params using receiver's template args
                if (ret->kind == ENGINE_TYPE_TYPE_PARAM || ret->kind == ENGINE_TYPE_POINTER ||
                    ret->kind == ENGINE_TYPE_REFERENCE) {
                    const EngineRegisteredType *rt = engine_registry_lookup_type(ctx->registry, tqn);
                    if (!rt && ctx->module_qn) {
                        rt = engine_registry_lookup_type(
                            ctx->registry,
                            engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, tqn));
                    }
                    if (rt && rt->type_param_names) {
                        ret = engine_type_substitute(ctx->arena, ret, rt->type_param_names,
                                                  t->data.template_type.template_args);
                    } else {
                        const char *fb[] = {"T", "K", "V", NULL};
                        const EngineType *fa[3] = {NULL};
                        ...
                        ret = engine_type_substitute(ctx->arena, ret, fb, fa);
                    }
                }
                t = ret;
                if (t->kind == ENGINE_TYPE_POINTER)
...
static const EngineType *c_eval_expr_type_inner(CLSPContext *ctx, TSNode node);
...
const EngineType *c_eval_expr_type(CLSPContext *ctx, TSNode node) {
    ...
    ctx->eval_depth++;
    const EngineType *result = c_eval_expr_type_inner(ctx, node);
    ctx->eval_depth--;
    return result ? result : engine_type_unknown();
}
...
static const EngineType *c_eval_expr_type_inner(CLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (fqn) {
            const EngineRegisteredFunc *f = engine_registry_lookup_func(ctx->registry, fqn);
            if (f && f->signature)
                return f->signature;
        }

        // Check if it's a type name (for constructor calls / casts)
        const EngineType *type = c_resolve_name_to_type(ctx, name);
        if (type && type->kind == ENGINE_TYPE_NAMED) {
    ...
    if (strcmp(kind, "this") == 0) {
        if (ctx->enclosing_class_qn) {
            return engine_type_pointer(ctx->arena,
                                    engine_type_named(ctx->arena, ctx->enclosing_class_qn));
        }
        return engine_type_unknown();
    }
    ...
    if (strcmp(kind, "field_expression") == 0) {
        ...
        const EngineType *obj_type = c_eval_expr_type(ctx, arg_node);
        if (engine_type_is_unknown(obj_type))
            return engine_type_unknown();
        ...
        // Look up method
        const EngineRegisteredFunc *method = c_lookup_member(ctx, type_qn, field_name);
        if (method && method->signature)
            return method->signature;

        // Look up field
        const EngineType *field_type = c_lookup_field_type(ctx, type_qn, field_name, 0);
        if (field_type && !engine_type_is_unknown(field_type)) {
            ...
            bool is_tparam = (field_type->kind == ENGINE_TYPE_TYPE_PARAM);
            ...
            if (!is_tparam && field_type->kind == ENGINE_TYPE_NAMED &&
                base->kind == ENGINE_TYPE_TEMPLATE) {
                const EngineRegisteredType *trt = engine_registry_lookup_type(ctx->registry, type_qn);
                if (!trt && ctx->module_qn) {
                    trt = engine_registry_lookup_type(
                        ctx->registry,
                        engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn));
                }
                if (trt && trt->type_param_names) {
                    ...
                    for (int tpi = 0; trt->type_param_names[tpi]; tpi++) {
                        if (strcmp(trt->type_param_names[tpi], short_fn) == 0) {
                            ...
                            field_type = engine_type_type_param(ctx->arena, short_fn);
                            break;
                        }
                    }
                }
            }
            if (is_tparam && base->kind == ENGINE_TYPE_TEMPLATE &&
                base->data.template_type.template_args) {
                const EngineRegisteredType *rt = engine_registry_lookup_type(ctx->registry, type_qn);
                if (!rt && ctx->module_qn) {
                    rt = engine_registry_lookup_type(
                        ctx->registry,
                        engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn));
                ...
                if (rt && rt->type_param_names) {
                    field_type = engine_type_substitute(ctx->arena, field_type, rt->type_param_names,
                                                     base->data.template_type.template_args);
                } else {
                    // Fallback: positional T, K, V
                    const char *params[] = {"T", "K", "V", "T1", "T2", NULL};
                    const EngineType *args[5] = {NULL};
                    ...
                    field_type = engine_type_substitute(ctx->arena, field_type, params, args);
                }
            }
            return field_type;
        }
    ...
    if (strcmp(kind, "qualified_identifier") == 0 || strcmp(kind, "scoped_identifier") == 0) {
        ...
        if (!ts_node_is_null(qi_name_child) &&
            ...
            if (bare_name) {
                text = scope_text
                           ? (char *)engine_arena_sprintf(ctx->arena, "%s::%s", scope_text, bare_name)
                           : bare_name;
            }
        }
        ...
        const EngineRegisteredFunc *f = NULL;
        if (ctx->module_qn) {
            f = engine_registry_lookup_func(
                ctx->registry, engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn));
        }
        if (!f)
            f = engine_registry_lookup_func(ctx->registry, qn);
        ...
        if (!f) {
            ...
            if (ns_dot && ctx->module_qn) {
                ...
                f = engine_registry_lookup_func(
                    ctx->registry, engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare));
                if (!f)
                    f = engine_registry_lookup_func(ctx->registry, bare);
            }
        }
        if (f && f->signature) {
            // If template args present, substitute into return type
            if (qi_has_tmpl_args && !ts_node_is_null(qi_tmpl_args) &&
                f->signature->kind == ENGINE_TYPE_FUNC && f->signature->data.func.return_types &&
                f->signature->data.func.return_types[0]) {
                const EngineType *base_ret = f->signature->data.func.return_types[0];
                const EngineType *targs[16] = {
                    NULL}; /* zero-fill: engine_type_substitute requires NULL-terminated args
                              (uninitialized tail bound T to stack garbage -> corrupt type graph,
                              bitcoin serialize.h) */
                ...
                if (targ_count > 0) {
                    ...
                    if (tpn) {
                        base_ret = engine_type_substitute(ctx->arena, base_ret, tpn, targs);
                    } else {
                        const char *fallback[] = {"T", "U", "V", "W", NULL};
                        base_ret = engine_type_substitute(ctx->arena, base_ret, fallback, targs);
                    }
                    ...
                    if (base_ret->kind == ENGINE_TYPE_NAMED) {
                        const EngineType **final_targs = (const EngineType **)engine_arena_alloc(
                            ctx->arena, (targ_count + 1) * sizeof(const EngineType *));
                        for (int i = 0; i < targ_count; i++)
                            final_targs[i] = targs[i];
                        final_targs[targ_count] = NULL;
                        base_ret =
                            engine_type_template(ctx->arena, base_ret->data.named.qualified_name,
                                              final_targs, targ_count);
                    }
                }
                const EngineType **rets =
                    (const EngineType **)engine_arena_alloc(ctx->arena, 2 * sizeof(const EngineType *));
                rets[0] = base_ret;
                rets[1] = NULL;
                return engine_type_func_replace_returns(ctx->arena, f->signature, rets);
            }
            return f->signature;
        }
        ...
        if (rt)
            return engine_type_named(ctx->arena, qn);
        if (!rt && ctx->module_qn) {
            const char *mod_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn);
            rt = engine_registry_lookup_type(ctx->registry, mod_qn);
            if (rt)
                return engine_type_named(ctx->arena, mod_qn);
        }
        ...
        if (dot) {
            ...
            char *enum_qn = (char *)engine_arena_alloc(ctx->arena, prefix_len + 1);
            if (enum_qn) {
                memcpy(enum_qn, qn, prefix_len);
                enum_qn[prefix_len] = '\0';
                const EngineRegisteredType *enum_rt = engine_registry_lookup_type(ctx->registry, enum_qn);
                if (enum_rt) {
                    return engine_type_named(ctx->arena, enum_qn);
                }
                // Try with module prefix
                if (ctx->module_qn) {
                    const char *fqn_enum =
                        engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, enum_qn);
                    enum_rt = engine_registry_lookup_type(ctx->registry, fqn_enum);
                    if (enum_rt)
                        return engine_type_named(ctx->arena, fqn_enum);
                }
            }
            ...
            if (ctx->in_template && dot) {
                ...
                char *scope_name = (char *)engine_arena_alloc(ctx->arena, prefix_len2 + 1);
                if (scope_name) {
                    memcpy(scope_name, qn, prefix_len2);
                    scope_name[prefix_len2] = '\0';
                    const EngineType *default_type = c_resolve_template_param(ctx, scope_name);
                    if (default_type) {
                        ...
                        if (default_qn) {
                            ...
                            const EngineRegisteredFunc *mf =
                                engine_registry_lookup_method(ctx->registry, default_qn, member);
                            if (mf && mf->signature)
                                return mf->signature;
                            // Try as static function
                            const char *dep_qn =
                                engine_arena_sprintf(ctx->arena, "%s.%s", default_qn, member);
                            const EngineRegisteredFunc *sf =
                                engine_registry_lookup_func(ctx->registry, dep_qn);
                            if (sf && sf->signature)
                                return sf->signature;
                        }
                    }
    ...
    if (strcmp(kind, "template_function") == 0) {
        ...
        if (strcmp(nk, "qualified_identifier") == 0 || strcmp(nk, "scoped_identifier") == 0) {
            ...
            if (!f && ctx->module_qn) {
                const char *mod_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn);
                f = engine_registry_lookup_func(ctx->registry, mod_qn);
            }
        } else {
            const char *fqn = c_resolve_name(ctx, name);
            if (fqn)
                f = engine_registry_lookup_func(ctx->registry, fqn);
        }
        if (!f || !f->signature)
            return engine_type_unknown();

        // Get the base return type from the function signature
        const EngineType *base_ret = NULL;
        if (f->signature->kind == ENGINE_TYPE_FUNC && f->signature->data.func.return_types &&
            f->signature->data.func.return_types[0]) {
        ...
        if (targ_count > 0) {
            ...
            if (tpn) {
                ...
                base_ret = engine_type_substitute(ctx->arena, base_ret, fallback_names, targs);
            }
        }

        // Wrap return type in FUNC signature
        const EngineType **rets = engine_arena_alloc(ctx->arena, 2 * sizeof(EngineType *));
        rets[0] = base_ret;
        rets[1] = NULL;
        return engine_type_func_replace_returns(ctx->arena, f->signature, rets);
    }
    ...
    if (strcmp(kind, "call_expression") == 0) {
        ...
        const EngineType *func_type = c_eval_expr_type(ctx, func_node);
        if (!func_type)
            return engine_type_unknown();
        // FUNC type -> return its return type
        if (func_type->kind == ENGINE_TYPE_FUNC && func_type->data.func.return_types &&
            func_type->data.func.return_types[0]) {
            const EngineType *ret = func_type->data.func.return_types[0];
            ...
            bool needs_subst = (ret->kind == ENGINE_TYPE_TYPE_PARAM || ret->kind == ENGINE_TYPE_NAMED ||
                                ret->kind == ENGINE_TYPE_POINTER || ret->kind == ENGINE_TYPE_REFERENCE ||
                                ret->kind == ENGINE_TYPE_RVALUE_REF ||
                                ret->kind == ENGINE_TYPE_TEMPLATE || ret->kind == ENGINE_TYPE_SLICE);
            ...
            if (needs_subst) {
                ...
                if (strcmp(ts_node_type(func_node), "field_expression") == 0) {
                    ...
                    if (!ts_node_is_null(arg_node)) {
                        ...
                        if (obj_type && obj_type->kind == ENGINE_TYPE_TEMPLATE &&
                            ...
                            if (!tpn && recv_qn) {
                                const EngineRegisteredType *rrt =
                                    engine_registry_lookup_type(ctx->registry, recv_qn);
                                if (!rrt && ctx->module_qn) {
                                    rrt = engine_registry_lookup_type(
                                        ctx->registry, engine_arena_sprintf(ctx->arena, "%s.%s",
                            ...
                            if (tpn) {
                                ...
                                ret =
                                    engine_type_substitute(ctx->arena, ret, tpn,
                                                        obj_type->data.template_type.template_args);
                                substituted = true;
                            } else {
                                // Fallback: positional params T, K, V, T1, T2
                                const char *params[] = {"T", "K", "V", "T1", "T2", NULL};
                                const EngineType *pargs[5] = {NULL};
                                if (nargs > 0)
                                ...
                                ret = engine_type_substitute(ctx->arena, ret, params, pargs);
                                substituted = true;
                            }
                        }
                    }
                ...
                if (!substituted) {
                    ...
                    const EngineRegisteredFunc *rf = NULL;
                    ...
                    if (strcmp(fn_type, "identifier") == 0) {
                        ...
                        if (fname) {
                            ...
                            if (fqn)
                                rf = engine_registry_lookup_func(ctx->registry, fqn);
                        }
                        ...
                        if (fname) {
                            ...
                            rf = engine_registry_lookup_func(ctx->registry, qn);
                        }
                        ...
                        if (!ts_node_is_null(name_node)) {
                            ...
                            if (fname) {
                                ...
                                if (strcmp(nk, "qualified_identifier") == 0 ||
                                    ...
                                    rf = engine_registry_lookup_func(ctx->registry, qn);
                                    if (!rf && ctx->module_qn) {
                                        rf = engine_registry_lookup_func(
                                            ctx->registry, engine_arena_sprintf(ctx->arena, "%s.%s",
                                                                             ctx->module_qn, qn));
                                    }
                                } else {
                    ...
                    if (rf && rf->type_param_names && rf->signature &&
                        rf->signature->kind == ENGINE_TYPE_FUNC &&
                        ...
                        if (tp_count > 0 && tp_count <= 8) {
                            const EngineType *deduced[8] = {NULL};
                            TSNode call_args = ts_node_child_by_field_name(node, "arguments", 9);
                            int pi = 0;
                            if (!ts_node_is_null(call_args)) {
                                ...
                                for (uint32_t ai = 0;
                                    ...
                                    if (ts_node_is_null(carg))
                                        continue;
                                    const EngineType *arg_t = c_eval_expr_type(ctx, carg);
                                    if (engine_type_is_unknown(arg_t)) {
                                        pi++;
                                        continue;
                                    }
                                    // Unwrap ref/ptr from param type
                                    const EngineType *param_t =
                                        rf->signature->data.func.param_types[pi];
                                    while (param_t && (param_t->kind == ENGINE_TYPE_REFERENCE ||
                                                       param_t->kind == ENGINE_TYPE_RVALUE_REF ||
                                                       param_t->kind == ENGINE_TYPE_POINTER)) {
                                        if (param_t->kind == ENGINE_TYPE_POINTER)
                                            param_t = param_t->data.pointer.elem;
                                        else
                                            param_t = param_t->data.reference.elem;
                                    }
                                    // If param type is TYPE_PARAM, bind it
                                    if (param_t && param_t->kind == ENGINE_TYPE_TYPE_PARAM) {
                                        for (int ti = 0; ti < tp_count; ti++) {
                                            if (strcmp(rf->type_param_names[ti],
                                                ...
                                                const EngineType *unwrapped = arg_t;
                                                while (unwrapped &&
                                                       (unwrapped->kind == ENGINE_TYPE_REFERENCE ||
                                                        unwrapped->kind == ENGINE_TYPE_RVALUE_REF ||
                                                        unwrapped->kind == ENGINE_TYPE_POINTER)) {
                                                    if (unwrapped->kind == ENGINE_TYPE_POINTER)
                            ...
                            if (any_deduced) {
                                const EngineType *substituted_ret = engine_type_substitute(
            ...
            if (!ret)
                return engine_type_unknown();
            if (ret->kind == ENGINE_TYPE_REFERENCE || ret->kind == ENGINE_TYPE_RVALUE_REF)
                ret = ret->data.reference.elem;
            return ret;
        }

        // Constructor call: Type(args) — if func_node resolves to a named type
        if (func_type->kind == ENGINE_TYPE_NAMED) {
            return func_type;
        }
    ...
    if (strcmp(kind, "new_expression") == 0) {
        ...
        if (!ts_node_is_null(type_node)) {
            return engine_type_pointer(ctx->arena, c_parse_type_node(ctx, type_node));
        }
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "template_type") == 0 ||
                ...
                return engine_type_pointer(ctx->arena, c_parse_type_node(ctx, child));
            }
        }
        return engine_type_pointer(ctx->arena, engine_type_unknown());
    }
    ...
    if (strcmp(kind, "delete_expression") == 0) {
        return engine_type_unknown(); // void
    }
    ...
    if (strcmp(kind, "subscript_expression") == 0) {
        ...
        if (ts_node_is_null(arg_node))
            return engine_type_unknown();
        const EngineType *arr_type = c_eval_expr_type(ctx, arg_node);
        if (!arr_type)
            return engine_type_unknown();

        // Array/slice: return element
        if (arr_type->kind == ENGINE_TYPE_SLICE)
            return arr_type->data.slice.elem;
        if (arr_type->kind == ENGINE_TYPE_POINTER)
        ...
        if (qn) {
            const EngineRegisteredFunc *op =
                engine_registry_lookup_method(ctx->registry, qn, "operator[]");
            if (op && op->signature && op->signature->kind == ENGINE_TYPE_FUNC &&
                op->signature->data.func.return_types && op->signature->data.func.return_types[0]) {
                const EngineType *ret = op->signature->data.func.return_types[0];
                if (ret->kind == ENGINE_TYPE_REFERENCE)
                    ret = ret->data.reference.elem;

                // Substitute template params
                if (ret->kind == ENGINE_TYPE_TYPE_PARAM && arr_type->kind == ENGINE_TYPE_TEMPLATE) {
                    const char *params[] = {"T", "K", "V", NULL};
                    const EngineType *args[3] = {NULL};
                    ...
                    ret = engine_type_substitute(ctx->arena, ret, params, args);
                }
                return ret;
            }
        }
        ...
        for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
            ...
            if (!ts_node_is_named(child)) {
                ...
                if (!op)
                    continue;
                if (strcmp(op, "*") == 0) {
                    const EngineType *inner = c_eval_expr_type(ctx, operand);
                    inner = c_simplify_type(ctx, inner, false);
                    // TEMPLATE with operator*(): look up and substitute return type
                    if (inner && inner->kind == ENGINE_TYPE_TEMPLATE) {
                        const char *tqn = inner->data.template_type.template_name;
                        const EngineRegisteredFunc *opf = c_lookup_member(ctx, tqn, "operator*");
                        if (opf && opf->signature && opf->signature->kind == ENGINE_TYPE_FUNC &&
                            opf->signature->data.func.return_types &&
                            opf->signature->data.func.return_types[0]) {
                            const EngineType *ret = opf->signature->data.func.return_types[0];
                            const EngineRegisteredType *rt =
                                engine_registry_lookup_type(ctx->registry, tqn);
                            if (!rt && ctx->module_qn) {
                                rt = engine_registry_lookup_type(
                                    ctx->registry,
                                    engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, tqn));
                    ...
                    return engine_type_deref(inner);
                }
                if (strcmp(op, "&") == 0) {
                    return engine_type_pointer(ctx->arena, c_eval_expr_type(ctx, operand));
                }
    ...
    if (strcmp(kind, "pointer_expression") == 0) {
        ...
        for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
            ...
            if (!ts_node_is_named(child)) {
                char *op = c_node_text(ctx, child);
                if (op && strcmp(op, "*") == 0) {
                    const EngineType *inner = c_eval_expr_type(ctx, arg);
                    inner = c_simplify_type(ctx, inner, false);
                    if (inner && inner->kind == ENGINE_TYPE_TEMPLATE) {
                        const char *tqn = inner->data.template_type.template_name;
                        const EngineRegisteredFunc *opf = c_lookup_member(ctx, tqn, "operator*");
                ...
                if (op && strcmp(op, "&") == 0)
                    return engine_type_pointer(ctx->arena, c_eval_expr_type(ctx, arg));
                break;
            }
        }
        return engine_type_unknown();
        ...
        for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
            ...
            if (!ts_node_is_named(child)) {
                ...
                if (op && (strcmp(op, "==") == 0 || strcmp(op, "!=") == 0 || strcmp(op, "<") == 0 ||
                    ...
                    return engine_type_builtin(ctx->arena, "bool");
                }
                break;
            }
        }
    ...
    if (strcmp(kind, "sizeof_expression") == 0 || strcmp(kind, "alignof_expression") == 0) {
        return engine_type_builtin(ctx->arena, "size_t");
    }

    // --- lambda_expression ---
    if (strcmp(kind, "lambda_expression") == 0) {
        const EngineType *ret_type = NULL;
        ...
        if (!ret_type || engine_type_is_unknown(ret_type)) {
        ...
        if (ret_type && !engine_type_is_unknown(ret_type)) {
            const EngineType **ret_types =
                (const EngineType **)engine_arena_alloc(ctx->arena, 2 * sizeof(const EngineType *));
            if (ret_types) {
                ret_types[0] = ret_type;
                ret_types[1] = NULL;
                return engine_type_func(ctx->arena, NULL, NULL, ret_types);
            }
        }
        return engine_type_func(ctx->arena, NULL, NULL, NULL);
    }
    ...
    if (strcmp(kind, "co_await_expression") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            ...
            const EngineType *op_type = c_eval_expr_type(ctx, operand);
            ...
            if (op_type && op_type->kind == ENGINE_TYPE_TEMPLATE &&
            ...
            if (op_qn) {
                const EngineRegisteredFunc *ar = c_lookup_member(ctx, op_qn, "await_resume");
                if (ar && ar->signature && ar->signature->kind == ENGINE_TYPE_FUNC &&
    ...
    if (strcmp(kind, "fold_expression") == 0) {
        ...
        for (uint32_t fi = 0; fi < fnc; fi++) {
            ...
            if (!ts_node_is_null(child)) {
                const EngineType *ct = c_eval_expr_type(ctx, child);
                if (!engine_type_is_unknown(ct))
                    return ct;
            }
        }
        return engine_type_unknown();
        ...
        for (uint32_t gi = 1; gi < gnc; gi++) {
            ...
            if (!ts_node_is_null(assoc)) {
                ...
                if (anc > 0) {
                    ...
                    const EngineType *vt = c_eval_expr_type(ctx, val);
                    if (!engine_type_is_unknown(vt))
                        return vt;
                }
            }
        }
        ...
        if (text) {
            ...
            for (const char *p = text; *p; p++) {
                if (*p == '.' || *p == 'f' || *p == 'F' || *p == 'e' || *p == 'E')
                    return engine_type_builtin(ctx->arena, "double");
            }
        }
        return engine_type_builtin(ctx->arena, "int");
    }
    if (strcmp(kind, "string_literal") == 0 || strcmp(kind, "concatenated_string") == 0 ||
        ...
        return engine_type_pointer(ctx->arena, engine_type_builtin(ctx->arena, "char"));
    }
    if (strcmp(kind, "char_literal") == 0) {
        return engine_type_builtin(ctx->arena, "char");
    }
    ...
    if (strcmp(kind, "initializer_list") == 0) {
        return engine_type_unknown(); // context-dependent
    }
...
// engine_arena_sprintf). The buffer is only read by the immediate registry lookup — a
...
static const EngineRegisteredFunc *c_lookup_member_depth(CLSPContext *ctx, const char *type_qn,
                                                      const char *member_name, int depth) {
    if (!type_qn || !member_name)
        return NULL;
    if (depth > ENGINE_LSP_MAX_LOOKUP_DEPTH)
        return NULL;
    ...
    const EngineRegisteredFunc *f = engine_registry_lookup_method(ctx->registry, type_qn, member_name);
    if (f)
        return f;
    ...
    if (!neg_memo_hit && ctx->module_qn) {
        ...
        if (!prefixed)
            prefixed = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn);
        f = engine_registry_lookup_method(ctx->registry, prefixed, member_name);
        if (f)
            return f;
    }
    ...
    {
        const EngineType *scoped = engine_scope_lookup(ctx->current_scope, type_qn);
        if (scoped && scoped->kind == ENGINE_TYPE_ALIAS) {
            const EngineType *underlying = engine_type_resolve_alias(scoped);
            if (underlying && !engine_type_is_unknown(underlying)) {
    ...
    if (!rt && ctx->module_qn) {
        ...
        if (!prefixed)
            prefixed = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn);
        rt = engine_registry_lookup_type(ctx->registry, prefixed);
    }
    if (rt) {
        // Alias chain
        if (rt->alias_of) {
...
const EngineRegisteredFunc *c_lookup_member(CLSPContext *ctx, const char *type_qn,
...
static bool c_base_declares_member(CLSPContext *ctx, const char *type_qn, const char *member_name) {
    ...
    if (!rt && ctx->module_qn) {
        rt = engine_registry_lookup_type(
            ctx->registry, engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn));
    }
...
static const EngineType *c_parse_declaration_type(CLSPContext *ctx, TSNode decl_node) {
...
void c_process_statement(CLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "declaration") == 0) {
        const EngineType *base_type = c_parse_declaration_type(ctx, node);
        bool has_auto = false;
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(ck, "init_declarator") == 0) {
                ...
                const EngineType *var_type = base_type;
                ...
                if (!ts_node_is_null(decl)) {
                    ...
                    if (strcmp(dk, "identifier") == 0) {
                        ...
                        if (!has_auto) {
                            for (int d = 0; d < ptr_depth; d++)
                                var_type = engine_type_pointer(ctx->arena, var_type);
                        }
                        ...
                        if (ts_node_named_child_count(decl) > 0) {
                            ...
                            if (strcmp(ik, "identifier") == 0) {
                                ...
                                const EngineType *rhs_type = engine_type_unknown();
                                ...
                                for (uint32_t bi = 0; bi < bnc; bi++) {
                                    ...
                                    if (strcmp(ts_node_type(bn), "identifier") == 0) {
                                        ...
                                        if (!bname)
                                            continue;
                                        const EngineType *elem_type = engine_type_unknown();
                                        if (rhs_type && rhs_type->kind == ENGINE_TYPE_TEMPLATE &&
                                        ...
                                        if (engine_type_is_unknown(elem_type) && rhs_type &&
                                            rhs_type->kind == ENGINE_TYPE_SLICE)
                                            elem_type = rhs_type->data.slice.elem;
                                        if (engine_type_is_unknown(elem_type) && rhs_type) {
                                            const char *rhs_qn = type_to_qn(rhs_type);
                                            const EngineRegisteredType *rt =
                                                rhs_qn ? engine_registry_lookup_type(ctx->registry,
                                                                                  rhs_qn)
                                                       : NULL;
                                            if (!rt && rhs_qn && ctx->module_qn)
                                                rt = engine_registry_lookup_type(
                                                    ctx->registry,
                                                    engine_arena_sprintf(ctx->arena, "%s.%s",
                                                                      ctx->module_qn, rhs_qn));
                                            if (rt && rt->field_types &&
                                                rt->field_types[binding_idx])
                                                elem_type = rt->field_types[binding_idx];
                                        }
                                        engine_scope_bind(ctx->current_scope, bname, elem_type);
                                        binding_idx++;
                                    }
                                }
                                continue;
                        ...
                        if (!has_auto)
                            var_type = engine_type_reference(ctx->arena, var_type);
                        ...
                        var_type = engine_type_slice(ctx->arena, var_type);
                        ...
                        for (uint32_t bi = 0; bi < bnc; bi++) {
                            ...
                            if (strcmp(ts_node_type(bn), "identifier") == 0) {
                                ...
                                if (engine_type_is_unknown(elem_type) && rhs_type) {
                                    const char *rhs_qn = type_to_qn(rhs_type);
                                    const EngineRegisteredType *rt =
                                        rhs_qn ? engine_registry_lookup_type(ctx->registry, rhs_qn)
                                               : NULL;
                                    if (!rt && rhs_qn && ctx->module_qn)
                                        rt = engine_registry_lookup_type(
                                            ctx->registry,
                                            engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn,
                                                              rhs_qn));
                    ...
                    if (var_name && var_name[0] && strcmp(var_name, "_") != 0) {
                        engine_scope_bind(ctx->current_scope, var_name, var_type);
                        ...
                        bool function_pointer_like =
                            var_type &&
                            (var_type->kind == ENGINE_TYPE_FUNC || var_type->kind == ENGINE_TYPE_POINTER);
                        ...
                        if (!fp_target && !ts_node_is_null(value)) {
                            ...
                            if (dll_func) {
                                bool is_fp_type = var_type && (var_type->kind == ENGINE_TYPE_FUNC ||
                                                               var_type->kind == ENGINE_TYPE_POINTER);
                                if (is_fp_type || has_cast) {
                                    fp_target =
                                        engine_arena_sprintf(ctx->arena, "external.%s", dll_func);
                                }
                            }
                        }
                ...
                if (var_name && var_name[0]) {
                    engine_scope_bind(ctx->current_scope, var_name, base_type);
                }
                ...
                if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "identifier") == 0) {
                    ...
                    if (var_name) {
                        const EngineType *vt = base_type;
                        for (int d = 0; d < ptr_depth; d++)
                            vt = engine_type_pointer(ctx->arena, vt);
                        engine_scope_bind(ctx->current_scope, var_name, vt);
                    }
                }
                ...
                while (!ts_node_is_null(inner)) {
                    ...
                    if (strcmp(inner_kind, "identifier") == 0) {
                        if (saw_pointer) {
                            char *var_name = c_node_text(ctx, inner);
                            if (var_name && var_name[0]) {
                                const EngineType *var_type = engine_type_pointer(ctx->arena, base_type);
                                engine_scope_bind(ctx->current_scope, var_name, var_type);
                ...
                if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "identifier") == 0) {
                    char *var_name = c_node_text(ctx, inner);
                    if (var_name) {
                        engine_scope_bind(ctx->current_scope, var_name,
                                       engine_type_slice(ctx->arena, base_type));
                    }
                }
            }
        }
            ...
            for (uint32_t i = 0; i < nc; i++) {
                ...
                if (strcmp(ck, "identifier") == 0 || strcmp(ck, "qualified_identifier") == 0 ||
                    ...
                    if (enum_text) {
                        ...
                        const EngineRegisteredType *et =
                            engine_registry_lookup_type(ctx->registry, enum_qn);
                        if (!et && ctx->module_qn) {
                            enum_qn =
                                engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, enum_qn);
                            et = engine_registry_lookup_type(ctx->registry, enum_qn);
                        }
                        if (!et && ctx->current_namespace) {
                            enum_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace,
                                                        enum_text);
                            et = engine_registry_lookup_type(ctx->registry, enum_qn);
                        }
    ...
    if (strcmp(kind, "alias_declaration") == 0) {
        ...
        if (!ts_node_is_null(name_node) && !ts_node_is_null(type_node)) {
            ...
            const EngineType *target = c_parse_type_node(ctx, type_node);
            if (alias_name && target) {
                // Build alias QN
                const char *alias_qn =
                    ctx->current_namespace
                        ? engine_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, alias_name)
                        : engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, alias_name);
                engine_scope_bind(ctx->current_scope, alias_name,
                               engine_type_alias(ctx->arena, alias_qn, target));
            }
        }
        return;
    }
    ...
    if (strcmp(kind, "type_definition") == 0) {
        ...
        const EngineType *target = engine_type_unknown();
        uint32_t nc = ts_node_named_child_count(node);
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (!found_type && strcmp(ck, "type_identifier") != 0 &&
                ...
                if (alias_name) {
                    const char *alias_qn =
                        ctx->current_namespace
                            ? engine_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace,
                                                alias_name)
                            : engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, alias_name);
                    engine_scope_bind(ctx->current_scope, alias_name,
                                   engine_type_alias(ctx->arena, alias_qn, target));
    ...
    if (strcmp(kind, "for_range_loop") == 0) {
        ...
        const EngineType *elem_type = engine_type_unknown();
        if (!ts_node_is_null(right)) {
            const EngineType *container_type = c_eval_expr_type(ctx, right);
            // Deduce element type: container's begin()->operator* or element type
            if (container_type) {
                if (container_type->kind == ENGINE_TYPE_SLICE) {
                    elem_type = container_type->data.slice.elem;
                } else if (container_type->kind == ENGINE_TYPE_TEMPLATE &&
                    ...
                    if (tname && container_type->data.template_type.arg_count >= 2 &&
                        ...
                        pair_args[2] = NULL;
                        elem_type = engine_type_template(ctx->arena, "std.pair", pair_args, 2);
                    } else {
                        elem_type = container_type->data.template_type.template_args[0];
                    }
                }
                // Iterator protocol fallback: begin() -> iter -> operator*() -> elem
                if (engine_type_is_unknown(elem_type) && container_type->kind == ENGINE_TYPE_NAMED) {
                    const char *cqn = container_type->data.named.qualified_name;
                    const EngineRegisteredFunc *begin_fn = c_lookup_member(ctx, cqn, "begin");
                    if (begin_fn && begin_fn->signature &&
                        begin_fn->signature->kind == ENGINE_TYPE_FUNC &&
                        begin_fn->signature->data.func.return_types &&
                        begin_fn->signature->data.func.return_types[0]) {
                        const EngineType *iter_type = begin_fn->signature->data.func.return_types[0];
                        const char *iter_qn = type_to_qn(iter_type);
                        if (iter_qn) {
                            const EngineRegisteredFunc *deref =
                                c_lookup_member(ctx, iter_qn, "operator*");
                            if (deref && deref->signature &&
                                deref->signature->kind == ENGINE_TYPE_FUNC &&
        ...
        if (!ts_node_is_null(decl)) {
            ...
            if (strcmp(dk, "identifier") == 0) {
                ...
                if (var_name)
                    engine_scope_bind(ctx->current_scope, var_name, elem_type);
            } else if (strcmp(dk, "structured_binding_declarator") == 0) {
                ...
                for (uint32_t bi = 0; bi < bnc; bi++) {
                    ...
                    if (strcmp(ts_node_type(bn), "identifier") == 0) {
                        ...
                        if (!bname)
                            continue;
                        const EngineType *bt = engine_type_unknown();
                        // TEMPLATE decomposition (e.g., pair<K,V>)
                        if (elem_type && elem_type->kind == ENGINE_TYPE_TEMPLATE &&
                        ...
                        // SLICE decomposition
                        if (engine_type_is_unknown(bt) && elem_type &&
                            elem_type->kind == ENGINE_TYPE_SLICE) {
                        ...
                        if (engine_type_is_unknown(bt) && elem_type) {
                            ...
                            const EngineRegisteredType *rt =
                                eq ? engine_registry_lookup_type(ctx->registry, eq) : NULL;
                            if (!rt && eq && ctx->module_qn)
                                rt = engine_registry_lookup_type(
                                    ctx->registry,
                                    engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, eq));
                            if (rt && rt->field_types && rt->field_types[binding_idx])
                                bt = rt->field_types[binding_idx];
                        }
                        engine_scope_bind(ctx->current_scope, bname, bt);
                        binding_idx++;
                    }
                }
            }
    ...
    if (strcmp(kind, "parameter_declaration") == 0) {
        ...
        const EngineType *param_type = c_parse_type_node(ctx, type_node);
        ...
        if (!ts_node_is_null(decl)) {
            ...
            if (strcmp(dk, "identifier") == 0) {
                ...
                if (name)
                    engine_scope_bind(ctx->current_scope, name, param_type);
            } else if (strcmp(dk, "pointer_declarator") == 0) {
                int ptr_depth = 0;
                TSNode inner = decl;
                while (!ts_node_is_null(inner) &&
                ...
                if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "identifier") == 0) {
                    ...
                    if (name) {
                        const EngineType *vt = param_type;
                        for (int d = 0; d < ptr_depth; d++)
                            vt = engine_type_pointer(ctx->arena, vt);
                        engine_scope_bind(ctx->current_scope, name, vt);
                    }
                }
            } else if (strcmp(dk, "reference_declarator") == 0) {
                if (ts_node_named_child_count(decl) > 0) {
                    ...
                    if (strcmp(ts_node_type(inner), "identifier") == 0) {
                        char *name = c_node_text(ctx, inner);
                        if (name)
                            engine_scope_bind(ctx->current_scope, name,
                                           engine_type_reference(ctx->arena, param_type));
                    }
                }
            }
        }
...
static int c_operator_param_count(const EngineRegisteredFunc *candidate) {
    if (!candidate || !candidate->signature || candidate->signature->kind != ENGINE_TYPE_FUNC) {
        return -1;
    }
    int count = 0;
    const EngineType *const *params = candidate->signature->data.func.param_types;
...
static bool c_operator_signature_matches(CLSPContext *ctx, const EngineRegisteredFunc *candidate,
                                         const EngineType *const *actuals, int actual_count) {
    if (!candidate || candidate->receiver_type || !candidate->signature ||
        candidate->signature->kind != ENGINE_TYPE_FUNC || actual_count < 0 ||
        c_operator_param_count(candidate) != actual_count) {
        return false;
    }
    const EngineType *const *params = candidate->signature->data.func.param_types;
    for (int i = 0; i < actual_count; i++) {
        const EngineType *expected = c_simplify_type(ctx, params[i], false);
        const EngineType *actual = c_simplify_type(ctx, actuals[i], false);
...
static const EngineRegisteredFunc *c_lookup_free_operator(CLSPContext *ctx, const char *operator_name,
                                                       const EngineType *left_type,
                                                       const EngineType *right_type) {
    ...
    const EngineRegisteredFunc *match = NULL;
    EngineFreeFuncIter it;
    engine_registry_free_funcs_by_short_name(ctx->registry, operator_name, &it);
    for (int index = engine_free_func_iter_next(&it); index >= 0;
         index = engine_free_func_iter_next(&it)) {
        if (index >= ctx->registry->func_count)
            continue;
        const EngineRegisteredFunc *candidate = &ctx->registry->funcs[index];
        ...
        const EngineType *actuals[2] = {left_type, right_type};
        bool signature_matches = c_operator_signature_matches(ctx, candidate, actuals, 2);
        if (ctx->debug) {
            const EngineType *const *params =
                candidate->signature && candidate->signature->kind == ENGINE_TYPE_FUNC
...
static const EngineRegisteredFunc *c_lookup_free_unary_operator(CLSPContext *ctx,
                                                             const char *operator_name,
                                                             const EngineType *operand_type) {
    ...
    const EngineRegisteredFunc *match = NULL;
    const EngineType *actuals[1] = {operand_type};
    EngineFreeFuncIter it;
    engine_registry_free_funcs_by_short_name(ctx->registry, operator_name, &it);
    for (int index = engine_free_func_iter_next(&it); index >= 0;
         index = engine_free_func_iter_next(&it)) {
...
static const EngineRegisteredFunc *c_lookup_member_operator_by_arity(CLSPContext *ctx,
                                                                  const char *type_qn,
                                                                  const char *operator_name,
    ...
    const EngineRegisteredFunc *candidate = engine_registry_lookup_method_by_args(
        ctx->registry, type_qn, operator_name, explicit_arg_count);
    if (candidate && c_operator_param_count(candidate) == explicit_arg_count) {
        return candidate;
    }
    if (ctx->module_qn) {
        const char *prefixed = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn);
        candidate = engine_registry_lookup_method_by_args(ctx->registry, prefixed, operator_name,
                                                       explicit_arg_count);
        if (candidate && c_operator_param_count(candidate) == explicit_arg_count) {
            return candidate;
        }
...
static void c_emit_resolved_reference_at(CLSPContext *ctx, const char *callee_qn,
    ...
    resolved.kind = ENGINE_RESOLVED_CALL_REFERENCE;
    ...
    engine_resolvedcall_push(ctx->resolved_calls, ctx->arena, resolved);
}
...
static void c_resolve_callable_argument_references(CLSPContext *ctx, TSNode call) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (!target && engine_scope_contains(ctx->current_scope, name)) {
            continue;
        }
        if (!target) {
 ...
 * engine_type_substitute under hundreds of recursive c_resolve_calls_in_node
...
static void c_resolve_calls_in_node_inner(CLSPContext *ctx, TSNode node) {
        ...
        if (!ts_node_is_null(func_node)) {
            ...
            if (strcmp(fk, "field_expression") == 0) {
                ...
                if (!ts_node_is_null(arg_node) && !ts_node_is_null(field_node)) {
                    ...
                    if (field_name) {
                        ...
                        const EngineType *base = is_arrow ? c_simplify_type(ctx, obj_type, true)
                        ...
                        if (type_qn) {
                            ...
                            const EngineRegisteredFunc *method = engine_registry_lookup_method_by_types(
                            ...
                            if (method) {
                                ...
                                if (method->receiver_type &&
                                    ...
                                    const EngineRegisteredFunc *override_m =
                                        engine_registry_lookup_method(ctx->registry, type_qn,
                                                                   field_name);
                                    if (override_m) {
                                        method = override_m;
                                ...
                                if (is_arrow && obj_type->kind == ENGINE_TYPE_TEMPLATE &&
                        ...
                        if (base && base->kind == ENGINE_TYPE_TYPE_PARAM && ctx->enclosing_func_qn &&
                            ctx->in_template) {
                            int ac = 0;
                        ...
                        if (engine_type_is_unknown(obj_type)) {
                            ...
                            c_emit_unresolved_call(
                                ctx, engine_arena_sprintf(ctx->arena, "%s.%s", type_qn, field_name),
                                "method_not_found");
                        }
                    }
                }
                ...
                if (text) {
                    ...
                    if (angle) {
                        ...
                        if (!dot_after) {
                            ...
                            char *stripped = (char *)engine_arena_alloc(ctx->arena, strip_len + 1);
                            ...
                            if (close && close < dot_after) {
                                ...
                                char *stripped = (char *)engine_arena_alloc(
                    ...
                    int arg_count = 0;
                    const EngineType **arg_types = c_extract_call_arg_types(ctx, node, &arg_count);
                    // Module-prefixed first (shadows stdlib stubs), then bare QN
                    const EngineRegisteredFunc *f = NULL;
                    if (ctx->module_qn) {
                        f = engine_registry_lookup_func(
                            ctx->registry,
                            engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn));
                    }
                    if (!f)
                        f = engine_registry_lookup_func(ctx->registry, qn);
                    if (f) {
                    ...
                    if (dot) {
                        ...
                        char *class_qn = (char *)engine_arena_alloc(ctx->arena, prefix_len + 1);
                        memcpy(class_qn, qn, prefix_len);
                        class_qn[prefix_len] = '\0';
                        const EngineRegisteredFunc *m = engine_registry_lookup_method_by_types(
                            ctx->registry, class_qn, dot + 1, arg_types, arg_count);
                        // Try with module prefix
                        if (!m && ctx->module_qn) {
                            const char *mod_class =
                                engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, class_qn);
                            m = engine_registry_lookup_method_by_types(ctx->registry, mod_class,
                    ...
                    if (dot && ctx->module_qn) {
                        ...
                        const EngineRegisteredFunc *nf =
                            engine_registry_lookup_func(ctx->registry, bare_name);
                        if (!nf) {
                            nf = engine_registry_lookup_func(
                                ctx->registry,
                                engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare_name));
                        }
                        if (nf) {
                    ...
                    if (name) {
                        ...
                        if (strcmp(nk, "qualified_identifier") == 0 ||
                            ...
                            if (!f) {
                                ...
                                if (dot) {
                                    size_t plen = dot - qn;
                                    char *cls = (char *)engine_arena_alloc(ctx->arena, plen + 1);
                                    memcpy(cls, qn, plen);
                                    cls[plen] = '\0';
                                    const EngineRegisteredFunc *m =
                                        engine_registry_lookup_method(ctx->registry, cls, dot + 1);
                                    if (!m && ctx->module_qn) {
                                        const char *mc = engine_arena_sprintf(ctx->arena, "%s.%s",
                                                                           ctx->module_qn, cls);
                                        m = engine_registry_lookup_method(ctx->registry, mc, dot + 1);
                                    }
                                    if (m)
                                        f = m;
                                }
                        ...
                        if (f) {
                            ...
                            if (ctx->pending_tc_count > 0 && f->type_param_names) {
                                int ac = 0;
                                const EngineType **at = c_extract_call_arg_types(ctx, node, &ac);
                                if (at)
            ...
            if (strcmp(fk, "identifier") == 0) {
                ...
                if (name && !is_c_builtin_func(name)) {
                    ...
                    const EngineType *var_type = engine_scope_lookup(ctx->current_scope, name);
                    if (!engine_type_is_unknown(var_type)) {
                        if (var_type->kind == ENGINE_TYPE_FUNC) {
                            // Function type variable without tracked target — can't resolve
                            goto recurse;
                        }
                        // Functor call: variable with operator()
                        const EngineType *base = c_simplify_type(ctx, var_type, false);
                        const char *type_qn = type_to_qn(base);
                        if (type_qn) {
                            const EngineRegisteredFunc *op =
                    ...
                    if (type && type->kind == ENGINE_TYPE_NAMED) {
                        ...
                        const EngineRegisteredType *rt =
                            engine_registry_lookup_type(ctx->registry, type_qn_str);
                        if (rt && !rt->is_interface) {
                            // Constructor call
                            const char *ctor_qn =
                                engine_arena_sprintf(ctx->arena, "%s.%s", type_qn_str, name);
                    ...
                    if (fqn) {
                        ...
                        if (ctx->enclosing_class_qn) {
                            const EngineRegisteredFunc *m = engine_registry_lookup_method(
                        ...
                        if (ctx->pending_tc_count > 0) {
                            const EngineRegisteredFunc *called =
                                engine_registry_lookup_func(ctx->registry, fqn);
    ...
    if (strcmp(kind, "declaration") == 0 && ctx->cpp_mode) {
        ...
        uint32_t nc = ts_node_named_child_count(node);
        const EngineType *decl_type = engine_type_unknown();
        bool has_type = false;
        for (uint32_t i = 0; i < nc; i++) {
            TSNode child = ts_node_named_child(node, i);
            const char *ck = ts_node_type(child);
                ...
                if (!ts_node_is_null(value)) {
                    ...
                    if (strcmp(vk, "argument_list") == 0 || strcmp(vk, "initializer_list") == 0) {
                        ...
                        if (type_qn) {
                            ...
                            const char *ctor_qn =
                                engine_arena_sprintf(ctx->arena, "%s.%s", type_qn, short_name);
    ...
    if (strcmp(kind, "new_expression") == 0) {
        ...
        if (!ts_node_is_null(type_node)) {
            const EngineType *t = c_parse_type_node(ctx, type_node);
            const char *type_qn = type_to_qn(t);
            if (type_qn) {
                const char *short_name = strrchr(type_qn, '.');
                short_name = short_name ? short_name + 1 : type_qn;
                const char *ctor_qn = engine_arena_sprintf(ctx->arena, "%s.%s", type_qn, short_name);
                c_emit_resolved_call_at(ctx, ctor_qn, "lsp_constructor", 0.95f, node);
            }
        }
    }
    ...
    if (strcmp(kind, "delete_expression") == 0 && ctx->cpp_mode) {
        ...
        if (!ts_node_is_null(operand)) {
            const EngineType *ptr_type = c_eval_expr_type(ctx, operand);
            const EngineType *base = c_simplify_type(ctx, ptr_type, true);
            const char *type_qn = type_to_qn(base);
            if (type_qn) {
                const char *short_name = strrchr(type_qn, '.');
                short_name = short_name ? short_name + 1 : type_qn;
                const char *dtor_name = engine_arena_sprintf(ctx->arena, "~%s", short_name);
                const EngineRegisteredFunc *dtor =
    ...
    if (ctx->cpp_mode && strcmp(kind, "binary_expression") == 0) {
        ...
        if (!ts_node_is_null(left) && !ts_node_is_null(right)) {
            const EngineType *lhs_type = c_eval_expr_type(ctx, left);
            const EngineType *rhs_type = c_eval_expr_type(ctx, right);
            const EngineType *base = c_simplify_type(ctx, lhs_type, false);
            ...
            for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
                ...
                if (!ts_node_is_named(child)) {
                    char *op = c_node_text(ctx, child);
                    if (op) {
                        const char *op_name = engine_arena_sprintf(ctx->arena, "operator%s", op);
                        const EngineRegisteredFunc *m = NULL;
                        if (base && base->kind != ENGINE_TYPE_BUILTIN && !engine_type_is_unknown(base)) {
                            const char *type_qn = type_to_qn(base);
                            if (type_qn) {
    ...
    if (ctx->cpp_mode && strcmp(kind, "field_expression") == 0) {
        ...
        if (is_arrow && !ts_node_is_null(receiver) && !ts_node_is_null(member)) {
            const EngineType *receiver_type = c_eval_expr_type(ctx, receiver);
            const EngineType *base = c_simplify_type(ctx, receiver_type, false);
            if (base && base->kind != ENGINE_TYPE_BUILTIN && base->kind != ENGINE_TYPE_POINTER &&
                !engine_type_is_unknown(base)) {
                const char *type_qn = type_to_qn(base);
    ...
    if (ctx->cpp_mode && strcmp(kind, "subscript_expression") == 0) {
        ...
        if (!ts_node_is_null(arg_node)) {
            const EngineType *arr_type = c_eval_expr_type(ctx, arg_node);
            const EngineType *base = c_simplify_type(ctx, arr_type, false);
            if (base && base->kind != ENGINE_TYPE_BUILTIN && !engine_type_is_unknown(base) &&
                base->kind != ENGINE_TYPE_POINTER && base->kind != ENGINE_TYPE_SLICE) {
                const char *type_qn = type_to_qn(base);
                if (type_qn) {
                    const EngineRegisteredFunc *m =
    ...
    if (ctx->cpp_mode &&
        ...
        if (!ts_node_is_null(operand)) {
            const EngineType *op_type = c_eval_expr_type(ctx, operand);
            const EngineType *base = c_simplify_type(ctx, op_type, false);
            if (base && base->kind != ENGINE_TYPE_BUILTIN && !engine_type_is_unknown(base) &&
                base->kind != ENGINE_TYPE_POINTER) {
                const char *type_qn = type_to_qn(base);
                if (type_qn) {
                    ...
                    if (op_name) {
                        ...
                        const EngineRegisteredFunc *m = c_lookup_member_operator_by_arity(
    ...
    if (ctx->cpp_mode && strcmp(kind, "declaration") == 0) {
        const EngineType *decl_type = c_parse_declaration_type(ctx, node);
        if (decl_type && decl_type->kind == ENGINE_TYPE_NAMED) {
                ...
                if (strcmp(ts_node_type(dchild), "init_declarator") == 0) {
                    ...
                    if (!ts_node_is_null(val)) {
                        ...
                        if (strcmp(vk, "argument_list") != 0 &&
                            ...
                            const EngineType *val_type = c_eval_expr_type(ctx, val);
                            const EngineType *val_base = c_simplify_type(ctx, val_type, false);
    ...
    if (ctx->cpp_mode &&
        ...
        if (!ts_node_is_null(cond)) {
            ...
            const EngineType *cond_type = c_eval_expr_type(ctx, cond);
            const EngineType *base = c_simplify_type(ctx, cond_type, false);
            if (base && base->kind != ENGINE_TYPE_BUILTIN && !engine_type_is_unknown(base) &&
                base->kind != ENGINE_TYPE_POINTER) {
                const char *type_qn = type_to_qn(base);
                if (type_qn) {
                    const EngineRegisteredFunc *m = c_lookup_member(ctx, type_qn, "operator bool");
                    if (m) {
...
recurse:;
    ...
    if (push_scope) {
        ctx->current_scope = engine_scope_push(ctx->arena, ctx->current_scope);
    }
                ...
                for (uint32_t ci = 0; ci < cnc; ci++) {
                    ...
                    if (strcmp(ck, "lambda_capture_initializer") == 0) {
                        ...
                        if (!ts_node_is_null(name_node) && !ts_node_is_null(val_node)) {
                            ...
                            const EngineType *cap_type = c_eval_expr_type(ctx, val_node);
                            if (cap_name && !engine_type_is_unknown(cap_type)) {
                                engine_scope_bind(ctx->current_scope, cap_name, cap_type);
                            }
                        }
                    }
                }
    ...
    if (push_scope) {
        ctx->current_scope = engine_scope_pop(ctx->current_scope);
...
static void c_process_function(CLSPContext *ctx, TSNode func_node) {
        ...
        if (strcmp(dk, "qualified_identifier") == 0 || strcmp(dk, "scoped_identifier") == 0) {
            ...
            if (!ts_node_is_null(scope_node)) {
                ...
                if (scope_text) {
                    ...
                    const EngineRegisteredType *rt = engine_registry_lookup_type(ctx->registry, scope_qn);
                    if (rt) {
                        ctx->enclosing_class_qn = scope_qn;
                    } else if (ctx->module_qn) {
                        const char *fqn =
                            engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, scope_qn);
                        rt = engine_registry_lookup_type(ctx->registry, fqn);
                        if (rt)
    ...
    // engine_pipeline_find_lsp_resolution never joined them — every in-method call
    ...
    if (ctx->enclosing_class_qn && saved_class_qn == ctx->enclosing_class_qn &&
        !strchr(func_qn, '.')) {
        func_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, func_qn);
        ...
        func_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, dot + 1);
        ...
        if (scope) {
            func_qn = engine_arena_sprintf(ctx->arena, "%s.%s", scope, func_qn);
        }
    }
    ...
    if (ctx->in_template && ctx->template_param_count > 0 && !ctx->registry_shared) {
        ...
        for (int ri = 0; ri < ((EngineTypeRegistry *)ctx->registry)->func_count; ri++) {
            EngineRegisteredFunc *rf = &((EngineTypeRegistry *)ctx->registry)->funcs[ri];
            if (strcmp(rf->qualified_name, func_qn) == 0 && !rf->type_param_names) {
                const char **tpn = (const char **)engine_arena_alloc(
    ...
    if (ctx->enclosing_class_qn) {
        engine_scope_bind(
            ...
            engine_type_pointer(ctx->arena, engine_type_named(ctx->arena, ctx->enclosing_class_qn)));
    }
...
static void c_process_body_child(CLSPContext *ctx, TSNode child) {
    ...
    if (strcmp(ck, "function_definition") == 0) {
                        ...
                        if (fname && fname[0]) {
                            ...
                            for (uint32_t ri = 0; ri < dnc; ri++) {
                                ...
                                if (strcmp(rk, "function_declarator") != 0 &&
                                    ...
                                    if (!engine_type_is_unknown(ret_type))
                                        break;
                                }
                            }
                            if (ret_type && !engine_type_is_unknown(ret_type)) {
                                if (is_ref_ret)
                                    ret_type = engine_type_reference(ctx->arena, ret_type);
                                if (is_ptr_ret)
                                    ret_type = engine_type_pointer(ctx->arena, ret_type);
                                const char *func_qn = c_build_qn(ctx, fname);
                                if (ctx->module_qn && !strchr(func_qn, '.'))
                                    func_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn,
                                                                func_qn);
                                // Use namespace if available
                                if (ctx->current_namespace &&
                                    !strstr(func_qn, ctx->current_namespace))
                                    func_qn = engine_arena_sprintf(ctx->arena, "%s.%s",
                                ...
                                if (!ctx->registry_shared &&
                                    !engine_registry_lookup_func(ctx->registry, func_qn)) {
                                    const EngineType **rets = (const EngineType **)engine_arena_alloc(
                                        ctx->arena, 2 * sizeof(const EngineType *));
                                    rets[0] = ret_type;
                                    rets[1] = NULL;
                                    EngineRegisteredFunc rf = {0};
                                    rf.qualified_name = func_qn;
                                    rf.short_name = fname;
                                    rf.signature = engine_type_func(ctx->arena, NULL, NULL, rets);
                                    ...
                                    engine_registry_add_func((EngineTypeRegistry *)ctx->registry, rf);
                                }
                            }
                        }
                    }
        ...
        const EngineType **saved_tpd = ctx->template_param_defaults;
...
static void c_process_namespace(CLSPContext *ctx, TSNode ns_node) {
    ...
    if (!ts_node_is_null(name_node)) {
        ...
        if (ns_name) {
            if (saved_ns) {
                ctx->current_namespace = engine_arena_sprintf(ctx->arena, "%s.%s", saved_ns, ns_name);
            } else if (ctx->module_qn) {
                ctx->current_namespace =
                    engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, ns_name);
            } else {
...
static void c_process_class(CLSPContext *ctx, TSNode class_node) {
    ...
    if (!ts_node_is_null(name_node)) {
        ...
        if (class_name) {
            ...
            if (ctx->current_namespace) {
                class_qn =
                    engine_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, class_name);
            } else if (ctx->module_qn) {
                class_qn = engine_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, class_name);
            } else {
                class_qn = class_name;
            }
            ...
            if (ctx->in_template && ctx->template_param_names && ctx->template_param_count > 0) {
                ...
                for (int ri = 0; ri < ((EngineTypeRegistry *)ctx->registry)->type_count; ri++) {
                    if (strcmp(((EngineTypeRegistry *)ctx->registry)->types[ri].qualified_name,
                               class_qn) == 0) {
                        rt = &((EngineTypeRegistry *)ctx->registry)->types[ri];
                        break;
                    }
                }
                if (rt && !rt->type_param_names) {
    ...
    if (!ts_node_is_null(body)) {
        ...
        if (ctx->enclosing_class_qn) {
            uint32_t bkn = 0;
            TSNode *bkids = engine_lsp_collect_children(ctx->arena, body, &bkn);
            for (uint32_t i = 0; i < bkn; i++) {
                ...
                const EngineType *ret_type = c_parse_declaration_type(ctx, child);
                ...
                for (uint32_t di = 0; di < dnc; di++) {
                    ...
                    // function_declarator
                    TSNode cur = decl;
                    const EngineType *actual_ret = ret_type;
                    for (int depth = 0; depth < 5 && !ts_node_is_null(cur); depth++) {
                        ...
                        if (strcmp(dk, "function_declarator") == 0) {
                            ...
                            if (!method_name || !method_name[0])
                                break;
                            const char *method_qn = engine_arena_sprintf(
                            ...
                            const EngineRegisteredFunc *existing = engine_registry_lookup_method(
                            ...
                            if (!existing && ctx->module_qn) {
                                ...
                                if (strncmp(class_qn, ctx->module_qn, mod_len) == 0 &&
                                    class_qn[mod_len] == '.') {
                                    existing = engine_registry_lookup_method(
                            ...
                            if (existing) {
                                ...
                                if (existing->signature &&
                                    existing->signature->kind == ENGINE_TYPE_FUNC &&
                                    ...
                                    int nk2 = actual_ret ? actual_ret->kind : ENGINE_TYPE_UNKNOWN;
                                    // Upgrade if existing is NAMED and new is more specific
                                    if (ek == ENGINE_TYPE_NAMED &&
                                        (nk2 == ENGINE_TYPE_TEMPLATE || nk2 == ENGINE_TYPE_TYPE_PARAM ||
                                         nk2 == ENGINE_TYPE_POINTER || nk2 == ENGINE_TYPE_REFERENCE))
                                        should_upgrade = true;
                                }
                                if (should_upgrade) {
                                    // Update existing entry's signature return type
                                    const EngineType **new_rets = (const EngineType **)engine_arena_alloc(
                                        ctx->arena, 2 * sizeof(const EngineType *));
                                    new_rets[0] = actual_ret;
                                    new_rets[1] = NULL;
                                    EngineRegisteredFunc *mut = (EngineRegisteredFunc *)existing;
                                    mut->signature = engine_type_func_replace_returns(
                            ...
                            actual_ret = engine_type_reference(ctx->arena, actual_ret);
                            cur = ts_node_named_child_count(cur) > 0 ? ts_node_named_child(cur, 0)
                                                                     : (TSNode){0};
                        } else if (strcmp(dk, "pointer_declarator") == 0) {
                            actual_ret = engine_type_pointer(ctx->arena, actual_ret);
                            cur = ts_node_named_child_count(cur) > 0 ? ts_node_named_child(cur, 0)
                                                                     : (TSNode){0};
                        } else {
                            break;
...
void c_lsp_process_file(CLSPContext *ctx, TSNode root) {
    ...
    uint32_t kn = 0;
    TSNode *kids = engine_lsp_collect_children(ctx->arena, root, &kn);
...
// ============================================================================
// Parse C/C++ return type text into EngineType (for cross-file defs)
// ============================================================================

static const EngineType *c_parse_return_type_text(EngineArena *a, const char *text,
    ...
    size_t len = strlen(text);
    if (len > 0 && text[len - 1] == '*') {
        char *inner = engine_arena_strndup(a, text, len - 1);
        // Trim trailing space
        size_t ilen = strlen(inner);
        while (ilen > 0 && inner[ilen - 1] == ' ')
            inner[--ilen] = '\0';
        return engine_type_pointer(a, c_parse_return_type_text(a, inner, module_qn));
    }
    ...
    if (len > 0 && text[len - 1] == '&') {
        ...
        if (ilen > 0 && inner[ilen - 1] == '&') {
            ...
            while (ilen > 1 && inner[ilen - 2] == ' ')
                inner[--ilen - 1] = '\0';
            return engine_type_rvalue_ref(a, c_parse_return_type_text(a, inner, module_qn));
        }
        return engine_type_reference(a, c_parse_return_type_text(a, inner, module_qn));
    }

    // decltype(expr) — extract the type from the expression
    if (strncmp(text, "decltype(", 9) == 0 && len > 10 && text[len - 1] == ')') {
        char *inner = engine_arena_strndup(a, text + 9, len - 10);
        size_t ilen = strlen(inner);
    ...
    if (is_c_builtin_type(text))
        return engine_type_builtin(a, text);
    ...
    if (len == 1 &&
        ...
        return engine_type_type_param(a, text);
    }
    ...
    // Template type: Name<Arg1, Arg2, ...>
    {
        ...
        if (angle && len > 0 && text[len - 1] == '>') {
            ...
            char *tmpl_name = engine_arena_strndup(a, text, name_len);
            ...
            char *args_text = engine_arena_strndup(a, args_start, args_len);

            // Split by comma at nesting depth 0
            const EngineType *arg_types[16];
            int arg_count = 0;
            int depth = 0;
            ...
            for (const char *p = args_text;; p++) {
                ...
                if ((*p == ',' && depth == 0) || *p == '\0') {
                    if (arg_count < 16) {
                        ...
                        char *arg = engine_arena_strndup(a, arg_begin, alen);
                        // Trim whitespace
                        while (*arg == ' ')
                            arg++;
            ...
            if (!strchr(tmpl_name, '.') && module_qn && module_qn[0]) {
                qname = engine_arena_sprintf(a, "%s.%s", module_qn, tmpl_name);
            }
            return engine_type_template(a, qname, arg_types, arg_count);
        }
    }
    ...
    if (strstr(text, "::")) {
        // Replace :: with .
        char *qn = engine_arena_strdup(a, text);
        for (char *p = qn; *p; p++) {
            if (p[0] == ':' && p[1] == ':') {
                p[0] = '.';
                memmove(p + 1, p + 2, strlen(p + 2) + 1);
        ...
        return engine_type_named(a, qn);
    }

    // Named type: qualify with module
    if (module_qn && module_qn[0]) {
        return engine_type_named(a, engine_arena_sprintf(a, "%s.%s", module_qn, text));
    }
    return engine_type_named(a, text);
}
 ...
 * before `count` and never leave an interior hole in the EngineType array. */
static const EngineType **c_parse_signature_param_types(EngineArena *arena, const char **type_texts,
...
static void c_rewrite_proved_destructor_candidates(EngineFileResult *result) {
    if (!result) {
        return;
    }
    for (int ci = 0; ci < result->calls.count; ci++) {
        EngineCall *call = &result->calls.items[ci];
        ...
        for (int ri = 0; ri < result->resolved_calls.count; ri++) {
            const EngineResolvedCall *resolved = &result->resolved_calls.items[ri];
...
void engine_run_c_lsp(EngineArena *arena, EngineFileResult *result, const char *source, int source_len,
                   TSNode root, bool cpp_mode, EngineSourceOrigin source_origin) {

    EngineTypeRegistry reg;
    engine_registry_init(&reg, arena);

    // Register stdlib
    engine_c_stdlib_register(&reg, arena);
    if (cpp_mode)
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (d->label && (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Type") == 0)) {
            ...
            if (d->base_classes) {
                ...
                while (d->base_classes[bc_count])
                    bc_count++;
                if (bc_count > 0) {
                    const char **embedded = (const char **)engine_arena_alloc(
                    ...
                    for (int j = 0; j < bc_count; j++) {
                        ...
                        if (strchr(bc, '.') || strstr(bc, "::")) {
                            ...
                            embedded[j] = engine_arena_sprintf(arena, "%s.%s", module_qn, bc);
                        }
                    }
        ...
        if (d->label && strcmp(d->label, "Field") == 0 && d->parent_class && d->return_type) {
            ...
            EngineRegisteredType *parent_rt = NULL;
            ...
            if (parent_rt) {
                ...
                const char **new_fnames =
                    (const char **)engine_arena_alloc(arena, (existing + 2) * sizeof(const char *));
                const EngineType **new_ftypes = (const EngineType **)engine_arena_alloc(
                    arena, (existing + 2) * sizeof(const EngineType *));
        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            const EngineType **ret_types = NULL;
            if (d->return_type && d->return_type[0]) {
                ret_types = (const EngineType **)engine_arena_alloc(arena, 2 * sizeof(const EngineType *));
                ...
                while (d->return_types[count])
                    count++;
                if (count > 0) {
                    ret_types = (const EngineType **)engine_arena_alloc(
                        arena, (count + 1) * sizeof(const EngineType *));
            ...
            const EngineType **param_types_arr = c_parse_signature_param_types(
                arena, d->signature_param_types, d->signature_param_count, module_qn);

            rf.signature = engine_type_func(arena, NULL, param_types_arr, ret_types);

            // Method receiver
            if (strcmp(d->label, "Method") == 0 && d->parent_class) {
                rf.receiver_type = d->parent_class;
                // Auto-create type if needed
                if (!engine_registry_lookup_type(&reg, d->parent_class)) {
                    EngineRegisteredType auto_type;
                    ...
                    engine_registry_add_type(&reg, auto_type);
                }
            }
...
/* Register one batch of EngineLSPDef[] into a registry. Shared by the
...
static void c_register_lsp_defs(EngineArena *arena, EngineTypeRegistry *reg, const char *module_qn,
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (d->label && (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Type") == 0 ||
            ...
            if (d->embedded_types) {
                ...
                while (*src && embed_count < 31) {
                    ...
                    if (sep) {
                        embeds[embed_count++] = engine_arena_strndup(arena, src, sep - src);
                        src = sep + 1;
                    } else {
                        embeds[embed_count++] = engine_arena_strdup(arena, src);
                        break;
                    }
                }
                if (embed_count > 0) {
                    const char **arr = (const char **)engine_arena_alloc(
            ...
            if (d->field_defs) {
                ...
                if (fcount > 0) {
                    const char **fnarr =
                        (const char **)engine_arena_alloc(arena, (fcount + 1) * sizeof(const char *));
                    const EngineType **ftarr = (const EngineType **)engine_arena_alloc(
                        arena, (fcount + 1) * sizeof(const EngineType *));
        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            if (d->return_types) {
                ...
                if (rcount > 0) {
                    const EngineType **rarr = (const EngineType **)engine_arena_alloc(
                        arena, (rcount + 1) * sizeof(const EngineType *));
                    for (int j = 0; j < rcount; j++)
                        rarr[j] = rets[j];
                    rarr[rcount] = NULL;
                    rf.signature = engine_type_func(arena, NULL, NULL, rarr);
                }
            }
            if (!rf.signature)
                rf.signature = engine_type_func(arena, NULL, NULL, NULL);

            const EngineType **param_types_arr = c_parse_signature_param_types(
                arena, d->signature_param_types, d->signature_param_count, def_module);
            const EngineType **return_types_arr =
                rf.signature ? rf.signature->data.func.return_types : NULL;
            rf.signature = engine_type_func(arena, NULL, param_types_arr, return_types_arr);
...
EngineTypeRegistry *engine_c_build_cross_registry(EngineArena *arena, EngineLSPDef *defs, int def_count) {
    if (!arena)
        return NULL;
...
void engine_run_c_lsp_cross_with_registry(EngineArena *arena, const char *source, int source_len,
                                       const char *module_qn, bool cpp_mode, EngineTypeRegistry *reg,
...
void engine_run_c_lsp_cross(EngineArena *arena, const char *source, int source_len, const char *module_qn,
                         bool cpp_mode, EngineLSPDef *defs, int def_count, const char **include_paths,
...
void engine_batch_c_lsp_cross(EngineArena *arena, EngineBatchCLSPFile *files, int file_count,
                           EngineResolvedCallArray *out) {
    if (!files || file_count <= 0 || !out)
        return;

    for (int f = 0; f < file_count; f++) {
        EngineBatchCLSPFile *file = &files[f];
        memset(&out[f], 0, sizeof(EngineResolvedCallArray));

        if (!file->source || file->source_len <= 0 || file->def_count <= 0)
            continue;

        // Per-file arena: registry + temp data freed after each file
        EngineArena file_arena;
        engine_arena_init(&file_arena);

        EngineResolvedCallArray file_out;
        memset(&file_out, 0, sizeof(file_out));

        // Delegate to existing per-file function
        engine_run_c_lsp_cross(&file_arena, file->source, file->source_len, file->module_qn,
        ...
        if (file_out.count > 0) {
            ...
            out[f].items =
                (EngineResolvedCall *)engine_arena_alloc(arena, file_out.count * sizeof(EngineResolvedCall));
            for (int j = 0; j < file_out.count; j++) {
                EngineResolvedCall *src = &file_out.items[j];
                EngineResolvedCall *dst = &out[f].items[j];
                memset(dst, 0, sizeof(*dst));
                dst->caller_qn = src->caller_qn ? engine_arena_strdup(arena, src->caller_qn) : NULL;
                dst->callee_qn = src->callee_qn ? engine_arena_strdup(arena, src->callee_qn) : NULL;
                dst->strategy = src->strategy ? engine_arena_strdup(arena, src->strategy) : NULL;
                dst->confidence = src->confidence;
                dst->reason = src->reason ? engine_arena_strdup(arena, src->reason) : NULL;
                dst->kind = src->kind;
        ...
        engine_arena_destroy(&file_arena);
    }
}
