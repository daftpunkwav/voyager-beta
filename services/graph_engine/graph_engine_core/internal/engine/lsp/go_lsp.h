#ifndef ENGINE_LSP_GO_LSP_H
#define ENGINE_LSP_GO_LSP_H
...
typedef struct {
    ...
    // on deeply-nested/cyclic files; see engine_lsp_max_walk_depth). Zero via memset.
    int walk_depth;
...
// Parse a Go type AST node into a EngineType.
...
// Called from engine_extract_file() after definitions and imports are extracted.
...
void engine_go_stdlib_register(EngineTypeRegistry* reg, EngineArena* arena);

// --- Cross-file LSP resolution ---

// Simplified definition for cross-file type/function registration.
// String fields are borrowed (caller owns memory until engine_run_go_lsp_cross returns).
typedef struct {
    ...
    bool is_interface;
    EngineLanguage lang;           // language of the file that defined this — used by Tier 2 per-language registry build to filter all_defs
    ...
    /* Python-only raw decorator syntax, borrowed from EngineDefinition.  The
 ...
 * Build a project-wide, finalized EngineTypeRegistry from all Go defs ONCE.
 ...
 * value is 0, the caller can skip the engine_run_go_lsp_cross_with_
