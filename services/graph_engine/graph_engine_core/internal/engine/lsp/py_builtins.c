 ...
 * during the per-file Python LSP run (which executes inside engine_extract_file,
 ...
 */
static void py_builtins_inject_defs(EngineFileResult *result, EngineArena *arena) {
    if (!result || !arena) {
        return;
    }
