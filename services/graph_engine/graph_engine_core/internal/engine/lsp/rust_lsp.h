 ...
 *
 * Architecture mirrors go_lsp / py_lsp: build a `EngineTypeRegistry` from the
 ...
 *
 * Public entry points:
 *   - `engine_run_rust_lsp` — single-file resolution invoked from
 *     `engine_extract_file()`.
 *   - `engine_run_rust_lsp_cross` — cross-file resolution given a list of
 *     `EngineLSPDef`s gathered by the pipeline.
 *   - `engine_batch_rust_lsp_cross` — batch wrapper that processes several
 *     files in one CGo call (per-file arenas + result copy).
 */
#ifndef ENGINE_LSP_RUST_LSP_H
#define ENGINE_LSP_RUST_LSP_H
...
#include "../engine.h"
#include "go_lsp.h" // for EngineLSPDef (pipeline def), used by the Tier-2 builder
 ...
 * scoring >= ENGINE_LSP_CONFIDENCE_FLOOR (0.6). Numbers here mirror the Go
 * LSP so the call-edge mix from a polyglot project stays comparable. */
#define ENGINE_RUST_CONF_DIRECT 0.95f      /* path::to::func or alias hit  */
#define ENGINE_RUST_CONF_METHOD 0.95f      /* inherent method dispatch     */
#define ENGINE_RUST_CONF_TRAIT_SOLE 0.92f  /* trait method, single impl    */
#define ENGINE_RUST_CONF_TRAIT_AMB 0.85f   /* trait method, many impls     */
#define ENGINE_RUST_CONF_UFCS 0.93f        /* T::method() / Self::new()    */
#define ENGINE_RUST_CONF_PROMOTED 0.90f    /* through Deref / blanket impl */
#define ENGINE_RUST_CONF_MACRO_KNOWN 0.85f /* known std macro mapped to fn */
#define ENGINE_RUST_CONF_OPERATOR 0.88f    /* a+b → T::add (operator trait) */

/* Rust-flavoured LSP context: one per file, lifetime tied to a single
 * `engine_extract_file()` invocation (or the cross-file caller's arena). */
typedef struct {
    EngineArena *arena;
    const char *source;
    int source_len;
    ...
    EngineIdxMemo declared_modules;
     ...
     * to every emitted EngineResolvedCall as `caller_qn`. */
    ...
    const EngineType *pending_closure_param_type;
    ...
    const struct EngineCargoManifest *cargo_manifest;
     ...
     * EngineCall entries so those recovered calls become real edges. Cross-file
     ...
     * EngineCall into `syn_calls` so the recovered call becomes an edge. Set
    ...
    /* ENGINE_LSP_DEBUG=1 in env enables verbose stderr trace. */
    bool debug;
 ...
 * `engine_type_unknown()` for anything we cannot type. */
 ...
 * reference_type, primitive_type, …) into a EngineType. */
...
/* Populate the same per-file registry used by engine_run_rust_lsp, including
 ...
 * `EngineLSPDef` in `go_lsp.h` for compatibility with the pipeline plumbing
...
/* Same as `engine_run_rust_lsp_cross`, plus an optional parsed Cargo manifest
