...
#define REG_FUNC(qn, short, ret_type) do { \
    ...
    rf.short_name = (short); \
    rf.signature = engine_type_func(arena, NULL, NULL, (const EngineType*[]){(ret_type), NULL}); \
...
void engine_c_stdlib_register(EngineTypeRegistry* reg, EngineArena* arena) {
    ...
    const EngineType* t_double = engine_type_builtin(arena, "double");
    const EngineType* t_void = engine_type_builtin(arena, "void");
    const EngineType* t_char_ptr = engine_type_pointer(arena, engine_type_builtin(arena, "char"));
    const EngineType* t_void_ptr = engine_type_pointer(arena, t_void);

    // FILE type
    REG_TYPE("FILE", "FILE");
    const EngineType* t_file_ptr = engine_type_pointer(arena, engine_type_named(arena, "FILE"));
    ...
    REG_FUNC("ftell", "ftell", engine_type_builtin(arena, "long"));
    ...
    REG_FUNC("atol", "atol", engine_type_builtin(arena, "long"));
    REG_FUNC("strtol", "strtol", engine_type_builtin(arena, "long"));
    ...
    REG_FUNC("time", "time", engine_type_builtin(arena, "time_t"));
    REG_FUNC("clock", "clock", engine_type_builtin(arena, "clock_t"));
