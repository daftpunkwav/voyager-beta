 ...
 * engine_pipeline_lsp_target_node() resolves the callee_qn to a graph node
 ...
 * during the per-file Kotlin LSP run (engine_run_kotlin_lsp, which executes inside
 * engine_extract_file, BEFORE the parallel pipeline mints def nodes from
 ...
 *
 * Node minting upserts by QN (engine_gbuf_upsert_node), so injecting the same
 ...
 */
static void kt_builtins_inject_defs(EngineFileResult *result, EngineArena *arena) {
    if (!result || !arena) {
        return;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        def.start_line = 1;
        def.end_line = 1;
        engine_defs_push(&result->defs, arena, def);
    }
}
