...
static bool is_throw_node(TSNode node, const EngineLangSpec *spec) {
    if (engine_kind_in_set(node, spec->throw_node_types)) {
        return true;
    }
    if (spec->language == ENGINE_LANG_KOTLIN && strcmp(ts_node_type(node), "jump_expression") == 0) {
...
static char *resolve_exception_name(EngineArena *a, TSNode throw_node, const char *source) {
...
static void extract_throws_clause(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec,
                                  const char *func_qn) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "identifier") == 0 ||
            ...
            char *exc = engine_node_text(ctx->arena, child, ctx->source);
            if (exc && exc[0]) {
                EngineThrow thr = {.exception_name = exc, .enclosing_func_qn = func_qn};
                engine_throws_push(&ctx->result->throws, ctx->arena, thr);
            }
        }
    }
}
...
static void process_throw_node(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec) {
    if (is_throw_node(node, spec)) {
        ...
        if (exc_name && exc_name[0]) {
            ...
            thr.enclosing_func_qn = engine_enclosing_func_qn_cached(ctx, node);
            engine_throws_push(&ctx->result->throws, ctx->arena, thr);
        }
    }

    extract_throws_clause(ctx, node, spec, engine_enclosing_func_qn_cached(ctx, node));
}

// Iterative throw walker
static void walk_throws(EngineExtractCtx *ctx, TSNode root, const EngineLangSpec *spec) {
    TSNodeStack stack;
...
static char *resolve_lhs_write_name(EngineExtractCtx *ctx, TSNode left) {
    ...
    if (strcmp(lk, "identifier") == 0 || strcmp(lk, "simple_identifier") == 0) {
        return engine_node_text(ctx->arena, left, ctx->source);
    }
    ...
    if (strcmp(lk, "index_expression") == 0 || strcmp(lk, "subscript_expression") == 0) {
        ...
        if (!ts_node_is_null(base)) {
            ...
            if (strcmp(bk, "identifier") == 0 || strcmp(bk, "simple_identifier") == 0) {
                return engine_node_text(ctx->arena, base, ctx->source);
            }
        }
        return NULL;
    }
    ...
    if (strcmp(lk, "field_expression") == 0 || strcmp(lk, "member_access_expression") == 0 ||
        ...
        if (!ts_node_is_null(fld)) {
            return engine_node_text(ctx->arena, fld, ctx->source);
        }
        return NULL;
    }
    return NULL;
...
static void try_emit_assignment_write(EngineExtractCtx *ctx, TSNode node, const char *func_qn) {
    ...
    if (name && name[0] && !engine_is_keyword(name, ctx->language)) {
        ...
        engine_rw_push(&ctx->result->rw, ctx->arena, rw);
    }
}

static void walk_readwrites(EngineExtractCtx *ctx, TSNode root, const EngineLangSpec *spec) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, ENGINE_SZ_512);
    ts_nstack_push(&stack, ctx->arena, root);
    while (stack.count > 0) {
        TSNode node = ts_nstack_pop(&stack);
        if (engine_kind_in_set(node, spec->assignment_node_types)) {
            try_emit_assignment_write(ctx, node, engine_enclosing_func_qn_cached(ctx, node));
        }
        uint32_t count = ts_node_child_count(node);
        for (int i = (int)count - LAST_IDX; i >= 0; i--) {
            ts_nstack_push(&stack, ctx->arena, ts_node_child(node, (uint32_t)i));
...
void handle_throws(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec, WalkState *state) {
...
void handle_readwrites(EngineExtractCtx *ctx, TSNode node, const EngineLangSpec *spec, WalkState *state) {
    if (!spec->assignment_node_types || !spec->assignment_node_types[0]) {
        return;
    }
