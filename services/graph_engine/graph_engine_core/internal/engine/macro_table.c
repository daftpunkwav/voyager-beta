...
void engine_macro_table_init_system(EngineMacroTable *t) {
    ...
    for (int i = 0; SYSTEM_MACROS[i].name; i++) {
        if (t->count >= ENGINE_MACRO_TABLE_CAP)
            break;
        EngineMacroEntry *e = &t->entries[t->count++];
        e->name = SYSTEM_MACROS[i].name;
        e->param_count = SYSTEM_MACROS[i].param_count;
        e->expansion = NULL;
        e->resolved_callee = SYSTEM_MACROS[i].callee;
        for (int p = 0; p < ENGINE_MACRO_MAX_PARAMS; p++)
...
void engine_macro_table_add(EngineMacroTable *t, EngineArena *arena, const char *name, int param_count,
    ...
    if (t->count >= ENGINE_MACRO_TABLE_CAP || !name)
        return;
    ...
    EngineMacroEntry *e = &t->entries[t->count++];
    e->name = engine_arena_strdup(arena, name);
    e->param_count = param_count;
    e->expansion = expansion ? engine_arena_strdup(arena, expansion) : NULL;
    e->resolved_callee = resolved_callee ? engine_arena_strdup(arena, resolved_callee) : NULL;
    for (int p = 0; p < ENGINE_MACRO_MAX_PARAMS; p++) {
        e->param_names[p] =
            (param_names && p < param_count) ? engine_arena_strdup(arena, param_names[p]) : NULL;
    }
}
...
void engine_parse_inc_file(EngineMacroTable *t, EngineArena *arena, const char *content) {
    ...
    while (*line) {
        ...
        if (strncmp(p, "#define", 7) == 0 && (p[7] == ' ' || p[7] == '\t')) {
            ...
            int param_count = -1;
            char param_names_buf[ENGINE_MACRO_MAX_PARAMS][64];
            const char *param_name_ptrs[ENGINE_MACRO_MAX_PARAMS] = {NULL};
            ...
            if (*p == '(') {
                ...
                while (*p && *p != ')' && p < end) {
                    ...
                    while (plen > 0 && (pn_start[plen - 1] == ' ' || pn_start[plen - 1] == '\t'))
                        plen--;
                    if (plen > 0 && param_count < ENGINE_MACRO_MAX_PARAMS) {
            ...
            char *expansion = NULL;
            if (explen > 0) {
                expansion = engine_arena_strndup(arena, p, explen);
            }

            engine_macro_table_add(t, arena, name, param_count,
...
char *engine_macro_expand(EngineArena *arena, const EngineMacroEntry *entry, const char **args,
    ...
    while (*p && out < (int)sizeof(buf) - 1) {
        if (*p == '%') {
            bool matched = false;
            for (int i = 0; i < entry->param_count && i < ENGINE_MACRO_MAX_PARAMS; i++) {
    ...
    buf[out] = '\0';
    return engine_arena_strdup(arena, buf);
}
...
char *engine_macro_extract_callee(EngineArena *arena, const char *expansion) {
    ...
    if (p) {
        ...
        if (clen <= 0 || mlen <= 0)
            return NULL;
        return engine_arena_sprintf(arena, "%.*s.%.*s", clen, p, mlen, method_start);
    }
    ...
    if (p && p[2] != '$') {
        ...
        if (*tag_end == '^') {
            ...
            if (tlen > 0 && rlen > 0) {
                return engine_arena_sprintf(arena, "%.*s^%.*s", tlen, p, rlen, rtn);
            }
        }
    }
