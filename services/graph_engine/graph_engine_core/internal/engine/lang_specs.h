#ifndef ENGINE_LANG_SPECS_H
#define ENGINE_LANG_SPECS_H

#include "engine.h"

// EngineEmbeddedLangSpec describes a sub-language embedded inside a host AST.
...
typedef struct {
    ...
    EngineLanguage embedded_language; // grammar used to re-parse the content slice
} EngineEmbeddedLangSpec;

// EngineLangSpec mirrors Go's lang.LanguageSpec with NULL-terminated string arrays.
typedef struct {
    ...
    const EngineEmbeddedLangSpec *embedded_imports;
} EngineLangSpec;
...
// Kept out of EngineLangSpec to avoid -Wmissing-field-initializers across 155
