#ifndef ENGINE_SQLITE_WRITER_H
#define ENGINE_SQLITE_WRITER_H
...
// indexes, sqlite_master) is written at finalize. engine_write_db() above is a
// one-shot wrapper over this API (open -> append all nodes -> finalize) and
// produces byte-identical output.
//
// Usage: w = engine_writer_open(path);
//        engine_writer_append_nodes(w, batch, n) x N  (ascending, contiguous ids);
//        engine_writer_finalize(w, ...);   // consumes + frees w, closes the file.
typedef struct engine_db_writer engine_db_writer_t;
