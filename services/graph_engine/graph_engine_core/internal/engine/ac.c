...
#define ENGINE_AC_MAX_BITMASK 64

#define ENGINE_AC_BYTE_RANGE 256
#define ENGINE_AC_NO_STATE (-1)
#define ENGINE_AC_ROOT_STATES 1
#define ENGINE_AC_ALLOC_ONE 1
#define ENGINE_AC_PATTERN_BIT(p) (1ULL << (p))
#define ENGINE_AC_CLEAR_LOW_BIT(b) ((b) & ((b) - 1ULL))

// Decompression buffer alignment mask (round up to 64KB chunks).
#define DECOMP_BUF_ALIGN_MASK 0xFFFF

struct EngineAutomaton {
    int num_states;
    int num_patterns;
    int alpha_size;                       // 256 for raw byte, or smaller for mapped alphabet
    uint8_t alpha_map[ENGINE_AC_BYTE_RANGE]; // byte → mapped index (identity if alpha_size==256)
...
static int ac_build_trie(EngineAutomaton *ac, const char **patterns, const int *lengths, int count) {
    int alpha_size = ac->alpha_size;
    int num_states = ENGINE_AC_ROOT_STATES; // state 0 = root
    ...
    for (int p = 0; p < count; p++) {
        ...
        for (int j = 0; j < lengths[p]; j++) {
            ...
            if (ac->go_table[idx] == ENGINE_AC_NO_STATE) {
                ac->go_table[idx] = num_states++;
            }
            state = ac->go_table[idx];
        }
        if (p < ENGINE_AC_MAX_BITMASK) {
            ac->output[state] |= ENGINE_AC_PATTERN_BIT(p);
        }
    ...
    for (int c = 0; c < alpha_size; c++) {
        if (ac->go_table[c] == ENGINE_AC_NO_STATE) {
            ac->go_table[c] = 0;
        }
    }
    return num_states;
...
static void ac_build_failure(EngineAutomaton *ac, int num_states) {
    int alpha_size = ac->alpha_size;
    ...
    while (!queue_empty(&q)) {
        ...
        for (int c = 0; c < alpha_size; c++) {
            ...
            if (s != ENGINE_AC_NO_STATE) {
                fail[s] = ac->go_table[(fail[r] * alpha_size) + c];
                ac->output[s] |= ac->output[fail[s]];
                if (ac->output_next[s] == ENGINE_AC_NO_STATE &&
                    ac->output_list[fail[s]] != ENGINE_AC_NO_STATE) {
...
static void ac_shrink_tables(EngineAutomaton *ac, int num_states, int max_states) {
...
// engine_ac_build constructs an Aho-Corasick automaton from a set of patterns.
//
// Parameters:
...
//
// Returns a heap-allocated automaton. Caller must call engine_ac_free().
EngineAutomaton *engine_ac_build(const char **patterns, const int *lengths, int count,
    ...
    if (alpha_size <= 0) {
        alpha_size = ENGINE_AC_BYTE_RANGE;
    }

    int max_states = ENGINE_AC_ROOT_STATES;
    for (int i = 0; i < count; i++) {
        max_states += lengths[i];
    }

    EngineAutomaton *ac = (EngineAutomaton *)calloc(ENGINE_AC_ALLOC_ONE, sizeof(EngineAutomaton));
    ac->alpha_size = alpha_size;
    ac->num_patterns = count;

    if (alpha_map) {
        memcpy(ac->alpha_map, alpha_map, ENGINE_AC_BYTE_RANGE);
    } else {
        for (int i = 0; i < ENGINE_AC_BYTE_RANGE; i++) {
            ac->alpha_map[i] = (uint8_t)i;
        }
    }

    ac->go_table = (int *)malloc((size_t)max_states * alpha_size * sizeof(int));
    memset(ac->go_table, ENGINE_AC_NO_STATE, (size_t)max_states * alpha_size * sizeof(int));
    ...
    for (int i = 0; i < max_states; i++) {
        ac->output_list[i] = ENGINE_AC_NO_STATE;
        ac->output_next[i] = ENGINE_AC_NO_STATE;
    }
...
// engine_ac_free releases all memory for an automaton.
...
// engine_ac_scan_bitmask scans text through the automaton and returns a bitmask
...
// Each goroutine gets its own OS thread (via CGo), so ENGINE_TLS is safe.
static ENGINE_TLS char *tls_decomp_buf = NULL;
static ENGINE_TLS int tls_decomp_cap = 0;
...
// engine_ac_scan_lz4_bitmask decompresses LZ4 data into a thread-local buffer
...
uint64_t engine_ac_scan_lz4_bitmask(const EngineAutomaton *ac, const char *compressed, int compressed_len,
    ...
    int decompressed = engine_lz4_decompress(compressed, compressed_len, buf, original_len);
    if (decompressed < 0) {
        return 0;
    }

    return engine_ac_scan_bitmask(ac, buf, decompressed);
}

// ─── Batch LZ4 + AC scan ───────────────────────────────────────────────────

// EngineLz4Entry and EngineLz4Match defined in ac.h.

// engine_ac_scan_lz4_batch decompresses and scans multiple files in one call.
...
int engine_ac_scan_lz4_batch(const EngineAutomaton *ac, const EngineLz4Entry *entries, int num_entries,
    ...
    for (int i = 0; i < num_entries && total < max_matches; i++) {
        ...
        int decompressed = engine_lz4_decompress(entries[i].data, entries[i].compressed_len, buf,
...
// EngineMatchResult defined in ac.h.

// engine_ac_scan_batch scans multiple NUL-separated names through the automaton.
        ...
        for (int i = 0; i < text_len; i++) {
            ...
            while (s > 0 && total < max_matches) {
                ...
                while (bits && total < max_matches) {
                    ...
                    total++;
                    seen |= ENGINE_AC_PATTERN_BIT(pid);
                    bits = ENGINE_AC_CLEAR_LOW_BIT(bits);
                }

                // Follow output_next for patterns beyond bitmask range.
                int next_state = ac->output_next[s];
                if (next_state == ENGINE_AC_NO_STATE || next_state == s) {
                    break;
                }
                s = next_state;
            }
...
// engine_ac_table_bytes returns the approximate memory used by the goto table.
