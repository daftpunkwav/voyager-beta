...
void engine_parse_range(const char *s, int *out_start, int *out_count) {
    ...
    *out_count = SKIP_ONE;
    ...
    if (!comma) {
        *out_start = (int)strtol(s, NULL, ENGINE_DECIMAL_BASE);
        return;
    }
    ...
    memcpy(buf, s, len);
    buf[len] = '\0';
    *out_start = (int)strtol(buf, NULL, ENGINE_DECIMAL_BASE);

    /* Parse count */
    *out_count = (int)strtol(comma + SKIP_ONE, NULL, ENGINE_DECIMAL_BASE);
}

#define HUNK_LINE_BUF 1536

/* Parse a single name-status line into a engine_changed_file_t.
 * Returns true if a valid entry was produced. */
static bool parse_one_name_status(const char *line, size_t line_len, engine_changed_file_t *out_f) {
    ...
    *tab1 = '\0';
    ...
    return engine_is_trackable_file(out_f->path);
}
...
static bool parse_hunk_line(const char *line, size_t line_len, const char *current_file,
    ...
    int start;
    int cnt;
    engine_parse_range(range_str, &start, &cnt);

    if (start <= 0 || !engine_is_trackable_file(current_file)) {
        return false;
    }
...
int engine_parse_hunks(const char *output, engine_changed_hunk_t *out, int max_out) {
    ...
    int count = 0;
    char current_file[ENGINE_SZ_512] = {0};
    const char *line = output;
