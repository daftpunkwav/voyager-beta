...
static const char *itoa_log(int val) {
    static char bufs[PT_RING][ENGINE_SZ_32];
    static int idx = 0;
    int i = idx;
...
static bool node_is_test(const engine_gbuf_node_t *n) {
...
static int create_tests_edges(engine_pipeline_ctx_t *ctx) {
    int count = 0;
    const engine_gbuf_edge_t **call_edges = NULL;
    int call_count = 0;
    int rc = engine_gbuf_find_edges_by_type(ctx->gbuf, "CALLS", &call_edges, &call_count);
    if (rc != 0 || call_count == 0) {
        return 0;
    }

    for (int i = 0; i < call_count; i++) {
        const engine_gbuf_edge_t *e = call_edges[i];
        const engine_gbuf_node_t *src = engine_gbuf_find_by_id(ctx->gbuf, e->source_id);
        const engine_gbuf_node_t *tgt = engine_gbuf_find_by_id(ctx->gbuf, e->target_id);
        if (!src || !tgt) {
            continue;
        }

        bool src_is_test =
            node_is_test(src) || (src->file_path && engine_is_test_path(src->file_path));
        if (!src_is_test) {
            continue;
        }

        bool tgt_is_test =
            node_is_test(tgt) || (tgt->file_path && engine_is_test_path(tgt->file_path));
        if (tgt_is_test) {
            continue;
        }

        if (!engine_is_test_func_name(src->name)) {
            continue;
        }

        engine_gbuf_insert_edge(ctx->gbuf, src->id, tgt->id, "TESTS", "{}");
        count++;
    }
    return count;
}
...
static int create_tests_file_edges(engine_pipeline_ctx_t *ctx) {
    int count = 0;
    const engine_gbuf_node_t **file_nodes = NULL;
    int file_node_count = 0;
    int rc = engine_gbuf_find_by_label(ctx->gbuf, "File", &file_nodes, &file_node_count);
    if (rc != 0) {
        return 0;
    }

    for (int i = 0; i < file_node_count; i++) {
        const engine_gbuf_node_t *fnode = file_nodes[i];
        if (!fnode->file_path || !engine_is_test_path(fnode->file_path)) {
            continue;
        }
        ...
        char *prod_qn = engine_pipeline_fqn_compute(ctx->project_name, prod_path, "__file__");
        const engine_gbuf_node_t *prod_node = engine_gbuf_find_by_qn(ctx->gbuf, prod_qn);
        free(prod_qn);
        free(prod_path);

        if (prod_node && fnode->id != prod_node->id) {
            engine_gbuf_insert_edge(ctx->gbuf, fnode->id, prod_node->id, "TESTS_FILE", "{}");
            count++;
        }
    }
    return count;
...
int engine_pipeline_pass_tests(engine_pipeline_ctx_t *ctx, const engine_file_info_t *files, int file_count) {
    engine_log_info("pass.start", "pass", "tests");
    (void)files;
    (void)file_count;
    ...
    engine_log_info("pass.done", "pass", "tests", "tests", itoa_log(tests_count), "tests_file",
