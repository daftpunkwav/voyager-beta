#ifndef ENGINE_PATH_ALIAS_H
#define ENGINE_PATH_ALIAS_H
 ...
 *
 * Lookup is two-step:
 *   1. engine_path_alias_find_for_file picks the nearest ancestor scope for
 *      a given source file (longest matching dir_prefix wins).
 *   2. engine_path_alias_resolve runs the chosen scope's alias map against
...
typedef struct {
    ...
    engine_path_alias_map_t *map;
} engine_path_alias_scope_t;
 ...
 * linear scan. */
typedef struct {
    engine_path_alias_scope_t *scopes;
    int count;
} engine_path_alias_collection_t;
 ...
 * engine_path_alias_collection_free. */
...
/* Free a collection produced by engine_load_path_aliases. NULL-safe. */
