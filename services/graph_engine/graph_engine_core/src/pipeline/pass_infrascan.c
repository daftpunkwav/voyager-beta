...
bool engine_is_k8s_manifest(const char *name, const char *content) {
    if (!name || !content || engine_is_kustomize_file(name)) {
        return false;
    }
...
bool engine_is_secret_binding(const char *key, const char *value) {
    ...
    if (value && engine_is_secret_value(value)) {
        return true;
    }
    return false;
}
...
static void df_parse_from(const char *line, engine_dockerfile_result_t *r) {
...
static void df_parse_expose(const char *line, engine_dockerfile_result_t *r) {
...
static void df_parse_env(const char *line, engine_dockerfile_result_t *r) {
    ...
    /* Filter secrets */
    if (engine_is_secret_binding(key, value)) {
        return;
    }
...
static void df_parse_arg(const char *line, engine_dockerfile_result_t *r) {
    ...
    if (r->build_arg_count >= ENGINE_SZ_32) {
        return;
    }
...
static void df_parse_json_cmd(const char *line, int prefix_len, char *out, size_t out_sz) {
    ...
    char raw[ENGINE_SZ_512];
    snprintf(raw, sizeof(raw), "%s", p);
    rtrim(raw);
    engine_clean_json_brackets(raw, out, out_sz);
}

static void df_parse_directives(const char *line, engine_dockerfile_result_t *r) {
...
static void strip_surrounding_quotes(char *value) {
    ...
    if (vlen >= ENGINE_QUOTE_PAIR && ((value[0] == '"' && value[vlen - SKIP_ONE] == '"') ||
                                   (value[0] == '\'' && value[vlen - SKIP_ONE] == '\''))) {
        memmove(value, value + ENGINE_QUOTE_OFFSET, vlen - ENGINE_QUOTE_PAIR);
        value[vlen - IS_QUOTE_TRIM] = '\0';
    }
}

/* Parse a single dotenv line: KEY=VALUE. Returns 1 if env var added. */
static int parse_dotenv_line(const char *trimmed, engine_dotenv_result_t *out) {
    char key[ENGINE_SZ_128];
    int klen = 0;
    ...
    if (engine_is_secret_binding(key, value) || out->env_count >= ENGINE_SZ_64) {
        return 0;
    }
...
static void shell_parse_export(const char *line, engine_shell_result_t *r) {
    /* export VAR=VALUE */
...
static void shell_parse_plain_var(const char *line, engine_shell_result_t *r) {
...
static void shell_parse_source(const char *line, engine_shell_result_t *r) {
    /* source <file> or . <file> */
    const char *p = NULL;
    ...
    if (plen >= ENGINE_QUOTE_PAIR && ((path[0] == '"' && path[plen - SKIP_ONE] == '"') ||
                                   (path[0] == '\'' && path[plen - SKIP_ONE] == '\''))) {
        memmove(path, path + ENGINE_QUOTE_OFFSET, plen - ENGINE_QUOTE_PAIR);
...
static void shell_parse_docker(const char *line, engine_shell_result_t *r) {
...
static bool shell_try_shebang(const char *trimmed, engine_shell_result_t *out, bool *shebang_checked) {
    if (*shebang_checked) {
        return false;
    }
...
static void shell_dispatch_line(const char *trimmed, engine_shell_result_t *out) {
...
static bool tf_detect_block_header(const char *trimmed, engine_terraform_result_t *out,
                                   tf_block_kind_t *cur_block, int *cur_var_idx, int *cur_mod_idx) {
    char ident1[ENGINE_SZ_128];

    /* resource "type" "name" */
    {
        int rc = tf_extract_ident(trimmed, "resource", ident1, sizeof(ident1));
        if (rc > 0 && out->resource_count < ENGINE_SZ_32) {
            char ident2[ENGINE_SZ_128];
    ...
    if (consumed > 0 && out->variable_count < ENGINE_SZ_32) {
...
static void tf_parse_variable_attrs(const char *trimmed, engine_tf_variable_t *v) {
    char def_val[ENGINE_SZ_256] = {0};
    tf_extract_quoted(trimmed, "default", def_val, sizeof(def_val));
    if (def_val[0] && !engine_is_secret_binding(v->name, def_val)) {
        snprintf(v->default_val, sizeof(v->default_val), "%s", def_val);
    }
    char type_val[ENGINE_SZ_64] = {0};
    tf_extract_quoted(trimmed, "type", type_val, sizeof(type_val));
    if (type_val[0]) {
        snprintf(v->type, sizeof(v->type), "%s", type_val);
    }
    char desc_val[ENGINE_SZ_256] = {0};
...
static void tf_parse_block_attrs(const char *trimmed, engine_terraform_result_t *out,
                                 tf_block_kind_t cur_block, int cur_var_idx, int cur_mod_idx) {
    switch (cur_block) {
    ...
    case TF_BLOCK_TERRAFORM: {
        ...
        char backend_name[ENGINE_SZ_128] = {0};
...
static bool tf_result_has_content(const engine_terraform_result_t *out) {
...
char *engine_infra_qn(const char *project_name, const char *rel_path, const char *infra_type,
    ...
    char *base = engine_pipeline_fqn_compute(project_name, rel_path, "");
    if (!base) {
        return NULL;
    }

    char result[ENGINE_SZ_1K];
        ...
        if (content < eol && *content != '#') {
            if (indent == 0) {
                ...
                if (!in_deps && nv && out->chart_name[0] == '\0') {
                    helm_copy_scalar(nv, eol, out->chart_name, ENGINE_HELM_NAME_MAX);
                }
            } else if (in_deps) {
                ...
                if (nv && out->dep_count < ENGINE_HELM_MAX_DEPS) {
                    char buf[ENGINE_HELM_NAME_MAX];
                    helm_copy_scalar(nv, eol, buf, ENGINE_HELM_NAME_MAX);
                    if (buf[0]) {
                        memcpy(out->deps[out->dep_count], buf, ENGINE_HELM_NAME_MAX);
                        out->dep_count++;
                    }
                }
            }
