...
#include "foundation/str_util.h" /* engine_validate_shell_arg — git shell-out hardening */
...
static _Thread_local char g_export_error[ENGINE_SZ_512];

static const char *itoa_buf(int v) {
    static _Thread_local char bufs[ART_RING][ENGINE_SZ_32];
...
static int artifact_export_fail(const char *stage, const char *path, const char *err, int err_no) {
    ...
    if (path && err_no != 0) {
        engine_log_error("artifact.export", "stage", safe_stage, "err", safe_err, "errno",
                      itoa_buf(err_no), "path", path);
    } else if (path) {
        engine_log_error("artifact.export", "stage", safe_stage, "err", safe_err, "path", path);
    } else if (err_no != 0) {
        engine_log_error("artifact.export", "stage", safe_stage, "err", safe_err, "errno",
                      itoa_buf(err_no));
    } else {
        engine_log_error("artifact.export", "stage", safe_stage, "err", safe_err);
    }
...
/* Build path: <repo>/.graph-engine/<name> into caller-owned buf. */
static bool artifact_path(char *buf, size_t bufsz, const char *repo_path, const char *name) {
    int n = snprintf(buf, bufsz, "%s/%s/%s", repo_path, ENGINE_ARTIFACT_DIR, name);
 ...
 * git shell-out): engine_validate_shell_arg rejects quote / backslash / substitution
...
bool engine_artifact_repo_path_is_shell_safe(const char *repo_path) {
    return engine_validate_shell_path_arg(repo_path);
}

/* Get current git HEAD hash. buf must be >= ENGINE_SZ_64. Returns false on error. */
static bool git_head_hash(const char *repo_path, char *buf, size_t bufsz) {
    char cmd[ENGINE_SZ_1K];
    if (!engine_artifact_repo_path_is_shell_safe(repo_path)) {
        buf[0] = '\0';
        return false;
    }
    int n =
...
static int read_metadata_version(const char *repo_path) {
    char meta_path[ENGINE_SZ_4K];
    artifact_path(meta_path, sizeof(meta_path), repo_path, ENGINE_ARTIFACT_META);
    ...
    int version = ver ? yyjson_get_int(ver) : ENGINE_NOT_FOUND;
    yyjson_doc_free(doc);
    return version;
}
...
static int write_metadata(const char *repo_path, const char *project_name, int nodes, int edges,
    ...
    yyjson_mut_obj_add_int(doc, root, "schema_version", ENGINE_ARTIFACT_SCHEMA_VERSION);
    ...
    char meta_path[ENGINE_SZ_4K];
    if (!artifact_path(meta_path, sizeof(meta_path), repo_path, ENGINE_ARTIFACT_META)) {
        free(json);
...
static void ensure_gitattributes(const char *repo_path) {
    ...
    if (fd < 0) {
        if (errno != EEXIST) {
            engine_log_warn("artifact.gitattributes.open path=%s err=%s", ga_path, strerror(errno));
        }
        ...
        if (fp) {
            ...
            (void)fputs("# Auto-generated by graph-engine\n"
                        "# Prevent merge conflicts on compressed artifact\n" ENGINE_ARTIFACT_FILENAME
                        " binary merge=ours\n",
                        fp);
            (void)fclose(fp);
        } else {
    ...
    FILE *p = engine_popen(cmd, "r");
    if (p) {
        (void)engine_pclose(p);
    }
}
 ...
 * engine_mkstemp would — sqlite has to be the one that creates it. Containing it in
 * a directory only this user can enter buys the same protection: engine_mkdtemp
 ...
 * "<tmp>/engine_artifact_tmp.db" was vulnerable on both counts — another local user
 * could pre-plant a symlink there to redirect the copy, and two concurrent
 * exports collided on the one name. */
typedef struct {
    char dir[ENGINE_SZ_512]; /* engine_mkdtemp copies its result back into this buffer */
    char db[ENGINE_SZ_4K];
} artifact_snapshot_tmp_t;

static bool artifact_snapshot_tmp_open(artifact_snapshot_tmp_t *tmp) {
    tmp->dir[0] = '\0';
    tmp->db[0] = '\0';
    int written = snprintf(tmp->dir, sizeof(tmp->dir), "%s/engine-artifact-XXXXXX", engine_tmpdir());
    if (written <= 0 || (size_t)written >= sizeof(tmp->dir) || !engine_mkdtemp(tmp->dir)) {
        tmp->dir[0] = '\0';
        return false;
    }
    written = snprintf(tmp->db, sizeof(tmp->db), "%s/snapshot.db", tmp->dir);
    if (written <= 0 || (size_t)written >= sizeof(tmp->db)) {
        (void)engine_rmdir(tmp->dir);
        tmp->dir[0] = '\0';
        return false;
    }
    return true;
...
static char *prepare_snapshot_db(const char *db_path, size_t *out_size, bool strip_indexes) {
    ...
    if (!artifact_snapshot_tmp_open(&tmp)) {
        artifact_export_fail("prepare_snapshot_dir", engine_tmpdir(), "private_tmpdir_failed", errno);
        return NULL;
    }
...
int engine_artifact_export(const char *db_path, const char *repo_path, const char *project_name,
    ...
    /* Ensure .graph-engine/ directory exists */
    char art_dir[ENGINE_SZ_4K];
    int dir_len = snprintf(art_dir, sizeof(art_dir), "%s/%s", repo_path, ENGINE_ARTIFACT_DIR);
    if (dir_len < 0 || (size_t)dir_len >= sizeof(art_dir)) {
        return artifact_export_fail("prepare_artifact_dir", repo_path, "path_too_long", 0);
    }
    errno = 0;
    if (!engine_mkdir_p(art_dir, ART_DIR_PERMS)) {
        return artifact_export_fail("prepare_artifact_dir", art_dir, "mkdir_or_not_directory",
                                    errno);
    }
    if (!engine_is_dir(art_dir)) {
    ...
    if (quality == ENGINE_ARTIFACT_BEST) {
    ...
    if (!db_data || db_size == 0) {
        free(db_data);
        if (engine_artifact_export_last_error()) {
            return ENGINE_NOT_FOUND;
        }
    ...
    /* Compress with zstd */
    size_t bound = engine_zstd_compress_bound(db_size);
    ...
    int64_t clen = engine_zstd_compress(db_data, db_size, compressed, bound, compression_level);
    free(db_data);
    ...
    char zst_path[ENGINE_SZ_4K];
    if (!artifact_path(zst_path, sizeof(zst_path), repo_path, ENGINE_ARTIFACT_FILENAME)) {
        free(compressed);
    ...
    int nodes = 0;
    int edges = 0;
    engine_store_t *count_store = engine_store_open_path_query(db_path);
    if (count_store) {
        nodes = engine_store_count_nodes(count_store, project_name);
        edges = engine_store_count_edges(count_store, project_name);
        engine_store_close(count_store);
    }

    /* Write metadata */
    if (write_metadata(repo_path, project_name, nodes, edges, db_size, (size_t)clen,
                       compression_level) != 0) {
        engine_unlink(zst_path);
        return ENGINE_NOT_FOUND;
    }
    ...
    engine_log_info("artifact.export", "quality", quality == ENGINE_ARTIFACT_BEST ? "best" : "fast",
...
int engine_artifact_import(const char *repo_path, const char *cache_db_path) {
    ...
    if (version < 0 || version > ENGINE_ARTIFACT_SCHEMA_VERSION) {
        engine_log_info("artifact.import", "skip", "schema_version_mismatch", "artifact_ver",
                     itoa_buf(version), "current_ver", itoa_buf(ENGINE_ARTIFACT_SCHEMA_VERSION));
        return ENGINE_NOT_FOUND;
    }
    ...
    if (original_size == 0) {
        engine_log_error("artifact.import", "err", "missing_original_size");
        return ENGINE_NOT_FOUND;
    }

    /* Read compressed artifact */
    char zst_path[ENGINE_SZ_4K];
    artifact_path(zst_path, sizeof(zst_path), repo_path, ENGINE_ARTIFACT_FILENAME);

    size_t clen = 0;
    char *compressed = read_file_alloc(zst_path, &clen);
    if (!compressed) {
        engine_log_error("artifact.import", "err", "read_artifact");
        return ENGINE_NOT_FOUND;
    }
    ...
    size_t frame_size = engine_zstd_frame_content_size(compressed, clen);
    if (frame_size == 0 || frame_size > ART_MAX_DECOMPRESSED_BYTES || frame_size != original_size) {
        free(compressed);
        engine_log_error("artifact.import", "err", "bad_decompressed_size");
        return ENGINE_NOT_FOUND;
    }
    ...
    int64_t dlen = engine_zstd_decompress(compressed, clen, decompressed, frame_size);
    free(compressed);

    if (dlen <= 0 || (size_t)dlen != frame_size) {
        free(decompressed);
        engine_log_error("artifact.import", "err", "zstd_decompress");
        return ENGINE_NOT_FOUND;
    }

    /* Write to temp file, then rename for atomicity */
    char tmp_path[ENGINE_SZ_4K];
    snprintf(tmp_path, sizeof(tmp_path), "%s.import_tmp", cache_db_path);

    /* Ensure cache directory exists */
    char cache_dir[ENGINE_SZ_1K];
    snprintf(cache_dir, sizeof(cache_dir), "%s", cache_db_path);
    char *last_slash = strrchr(cache_dir, '/');
    if (last_slash) {
        *last_slash = '\0';
        engine_mkdir_p(cache_dir, ART_DIR_PERMS);
    }
    ...
    if (wrc != 0) {
        if (ioerr.err_no != 0) {
            engine_log_error("artifact.import", "err", "write_temp_db", "detail", ioerr.err, "errno",
                          itoa_buf(ioerr.err_no), "path", tmp_path);
        } else {
            engine_log_error("artifact.import", "err", "write_temp_db", "detail", ioerr.err, "path",
                          tmp_path);
        }
        return ENGINE_NOT_FOUND;
    }

    /* Open with engine_store_open_path to auto-create missing indexes + FTS5 */
    engine_store_t *store = engine_store_open_path(tmp_path);
    if (!store) {
        engine_log_error("artifact.import", "err", "open_imported_db");
    ...
    if (!engine_store_check_integrity_deep(store)) {
        engine_log_error("artifact.import", "err", "integrity_check_failed");
    ...
    engine_remove_db_sidecars(cache_db_path);
    if (rename(tmp_path, cache_db_path) != 0) {
        engine_log_error("artifact.import", "err", "rename_to_cache");
        engine_unlink(tmp_path);
        return ENGINE_NOT_FOUND;
    }

    /* Clean up any stale WAL/SHM from the temp open */
    char wal[ENGINE_SZ_4K];
    char shm[ENGINE_SZ_4K];
    snprintf(wal, sizeof(wal), "%s-wal", tmp_path);
    snprintf(shm, sizeof(shm), "%s-shm", tmp_path);
    engine_unlink(wal);
    engine_unlink(shm);

    engine_log_info("artifact.import", "db", cache_db_path, "size_mb",
...
bool engine_artifact_exists(const char *repo_path) {
    ...
    int version = read_metadata_version(repo_path);
    return version >= 0 && version <= ENGINE_ARTIFACT_SCHEMA_VERSION;
}
