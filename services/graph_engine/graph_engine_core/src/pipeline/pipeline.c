...
enum { ENGINE_DIR_PERMS = 0755, PL_RING = 4, PL_RING_MASK = 3, PL_SEQ_PASSES = 6 };
...
#ifdef _WIN32
#include <process.h>
#define engine_pipeline_getpid _getpid
#else
#include <unistd.h>
#define engine_pipeline_getpid getpid
#endif
...
static engine_pipeline_test_hook_fn g_persist_test_before_final_manifest = NULL;
...
void engine_pipeline_persist_test_run_before_final_manifest(void) {
    engine_pipeline_test_hook_fn hook = g_persist_test_before_final_manifest;
...
bool engine_pipeline_try_lock(void) {
    return atomic_exchange(&g_pipeline_busy, 1) == 0;
}

#define LOCK_SPIN_NS 100000000 /* 100ms between lock retries */

void engine_pipeline_lock(void) {
    while (atomic_exchange(&g_pipeline_busy, 1) != 0) {
        struct timespec ts = {0, LOCK_SPIN_NS};
        engine_nanosleep(&ts, NULL);
    }
}

void engine_pipeline_unlock(void) {
    atomic_store(&g_pipeline_busy, 0);
}

/* ── Internal state ──────────────────────────────────────────────── */

struct engine_pipeline {
    char *repo_path;
    char *db_path;
    char *project_name;
    engine_git_context_t git_ctx;
    char *branch_qn;
    engine_index_mode_t mode;
    atomic_int cancelled_storage;
    atomic_int *cancelled;
    bool persistence; /* write .graph-engine/graph.db.zst after indexing */

    /* Indexing state (set during run) */
    engine_gbuf_t *gbuf;
    engine_registry_t *registry;

    /* Directory subtrees skipped during discovery (rel paths). Captured from
     * engine_discover_ex so the MCP layer can report excluded subtrees (#411).
     * Owned by the pipeline; freed in engine_pipeline_free. */
    char **excluded_dirs;
    int excluded_count;
    ...
    engine_ignored_file_t *ignored_files;
    int ignored_count;
    int ignored_total;
     ...
     * pipeline; freed in engine_pipeline_free. */
    engine_file_error_t *file_errors;
    int file_errors_count;
    int file_errors_cap;

    /* User-defined extension overrides (loaded once per run) */
    engine_userconfig_t *userconfig;
     ...
     * released with engine_store_free_lsp_surfaces in engine_pipeline_free. NULL
    ...
    engine_lsp_surface_row_t *surface_rows;
    ...
    void (*before_publish_hook)(engine_pipeline_t *, const char *, void *);
...
static EngineHashTable *g_pkgmap = NULL;
...
static double elapsed_ms(struct timespec start) {
    ...
    return ((double)(now.tv_sec - start.tv_sec) * ENGINE_MS_PER_SEC) +
           ((double)(now.tv_nsec - start.tv_nsec) / ENGINE_US_PER_SEC_F);
}

/* Format int to string for logging. Thread-safe via TLS rotating buffers. */
static const char *itoa_buf(int val) {
    static ENGINE_TLS char bufs[PL_RING][ENGINE_SZ_32];
...
static void log_phase_mem(const char *phase) {
    ...
    engine_log_info("mem.phase", "phase", phase, "rss_mb",
                 itoa_buf((int)(engine_mem_rss() / PL_BYTES_PER_MB)), "peak_mb",
                 itoa_buf((int)(engine_mem_peak_rss() / PL_BYTES_PER_MB)));
}

/* ── Lifecycle ──────────────────────────────────────────────────── */

engine_pipeline_t *engine_pipeline_new(const char *repo_path, const char *db_path,
                                 engine_index_mode_t mode) {
    if (!repo_path) {
        return NULL;
    }

    engine_pipeline_t *p = calloc(ENGINE_ALLOC_ONE, sizeof(engine_pipeline_t));
    if (!p) {
        return NULL;
    }
    ...
    p->project_name = engine_project_name_from_path(repo_path);
    (void)engine_git_context_resolve(repo_path, &p->git_ctx);
    p->branch_qn = engine_git_context_branch_qn(p->project_name, &p->git_ctx);
    p->mode = mode;
    p->persistence = false;
    p->committed_nodes = -1;
    p->committed_edges = -1;
...
static int pipeline_refresh_git_context(engine_pipeline_t *p) {
    engine_git_context_t fresh = {0};
    if (!p || engine_git_context_resolve(p->repo_path, &fresh) != 0) {
        engine_git_context_free(&fresh);
        return ENGINE_NOT_FOUND;
    }
    char *fresh_branch_qn = engine_git_context_branch_qn(p->project_name, &fresh);
    if (!fresh_branch_qn) {
        engine_git_context_free(&fresh);
        return ENGINE_NOT_FOUND;
    }
    engine_git_context_free(&p->git_ctx);
    free(p->branch_qn);
    p->git_ctx = fresh;
...
void engine_pipeline_set_persistence(engine_pipeline_t *p, bool enabled) {
    if (p) {
        p->persistence = enabled;
    }
}

bool engine_pipeline_set_project_name(engine_pipeline_t *p, const char *name) {
    if (!p || !name || !name[0]) {
        return false;
    }

    char *normalized = engine_project_name_from_path(name);
    if (!normalized) {
        return false;
    }
    if (!engine_validate_project_name(normalized)) {
        free(normalized);
        return false;
    }
...
void engine_pipeline_set_lsp_surfaces(engine_pipeline_t *p, engine_lsp_surface_row_t *rows, int count) {
    if (!p) {
        engine_store_free_lsp_surfaces(rows, count);
        return;
    }
    engine_store_free_lsp_surfaces(p->surface_rows, p->surface_row_count);
    p->surface_rows = rows;
    p->surface_row_count = count;
}

void engine_pipeline_free(engine_pipeline_t *p) {
    if (!p) {
        return;
    }
    free(p->repo_path);
    free(p->db_path);
    free(p->project_name);
    engine_discover_free_excluded(p->excluded_dirs, p->excluded_count);
    p->excluded_dirs = NULL;
    p->excluded_count = 0;
    engine_discover_free_ignored(p->ignored_files, p->ignored_count);
    ...
    if (p->userconfig) {
        engine_set_user_lang_config(NULL);
        engine_userconfig_free(p->userconfig);
        p->userconfig = NULL;
    }
    free(p);
}

void engine_pipeline_cancel(engine_pipeline_t *p) {
    if (p && p->cancelled) {
        atomic_store(p->cancelled, 1);
    }
}

void engine_pipeline_bind_cancel_flag(engine_pipeline_t *p, atomic_int *cancelled) {
    if (p && cancelled) {
...
const char *engine_pipeline_project_name(const engine_pipeline_t *p) {
...
int engine_pipeline_get_mode(const engine_pipeline_t *p) {
    return p ? (int)p->mode : 0;
}

void engine_pipeline_get_excluded(const engine_pipeline_t *p, char ***out, int *count) {
    if (out) {
...
void engine_pipeline_add_file_error(engine_pipeline_t *p, const char *path, const char *reason,
                                 const char *phase) {
    if (!p) {
        return;
    }
    if (p->file_errors_count >= p->file_errors_cap) {
        ...
        engine_file_error_t *grown =
            (engine_file_error_t *)realloc(p->file_errors, (size_t)ncap * sizeof(*grown));
        if (!grown) {
    ...
    engine_file_error_t *e = &p->file_errors[p->file_errors_count];
...
void engine_pipeline_get_file_errors(const engine_pipeline_t *p, engine_file_error_t **out, int *count) {
    if (out) {
...
void engine_pipeline_get_ignored(const engine_pipeline_t *p, engine_ignored_file_t **out, int *count,
                              int *total) {
    if (out) {
...
void engine_pipeline_get_committed_counts(const engine_pipeline_t *p, int *nodes, int *edges) {
    if (nodes) {
 ...
 * threaded (ENGINE_INDEX_SINGLE_THREAD=1) so a per-file marker can pin the EXACT
...
static int effective_worker_count(bool initial) {
    const char *st = getenv("ENGINE_INDEX_SINGLE_THREAD");
    if (st && st[0] == '1') {
        return 1;
    }
    return engine_default_worker_count(initial);
}

/* Resolve the DB path for this pipeline. Caller must free(). */
static char *resolve_db_path(const engine_pipeline_t *p) {
    if (!p) {
        return NULL;
    }
    if (p->db_path) {
    ...
    const char *cache_dir = engine_resolve_cache_dir();
    cache_dir = cache_dir ? cache_dir : engine_tmpdir();
...
static int check_cancel(const engine_pipeline_t *p) {
    return atomic_load(p->cancelled) ? ENGINE_NOT_FOUND : 0;
}
...
static void create_folder_chain(engine_pipeline_t *p, const char *dir, EngineHashTable *seen_dirs) {
    char *walk = strdup(dir);
    while (walk[0] != '\0' && !engine_ht_get(seen_dirs, walk)) {
        engine_ht_set(seen_dirs, strdup(walk), intptr_to_ptr(SKIP_ONE));
        char *folder_qn = engine_pipeline_fqn_folder(p->project_name, walk);
        const char *dir_base = strrchr(walk, '/');
        dir_base = dir_base ? dir_base + SKIP_ONE : walk;
        engine_gbuf_upsert_node(p->gbuf, "Folder", dir_base, folder_qn, walk, 0, 0, "{}");
        ...
        if (pdir[0] == '\0') {
            ...
            pqn_heap = engine_pipeline_fqn_folder(p->project_name, pdir);
            pqn = pqn_heap;
        }
        const engine_gbuf_node_t *fn = engine_gbuf_find_by_qn(p->gbuf, folder_qn);
        const engine_gbuf_node_t *pn = engine_gbuf_find_by_qn(p->gbuf, pqn);
        if (fn && pn) {
            engine_gbuf_insert_edge(p->gbuf, pn->id, fn->id, "CONTAINS_FOLDER", "{}");
        }
        free(folder_qn);
        free(pqn_heap);
...
static int pass_structure(engine_pipeline_t *p, const engine_file_info_t *files, int file_count) {
    engine_log_info("pass.start", "pass", "structure", "files", itoa_buf(file_count));

    /* Project node */
    engine_gbuf_upsert_node(p->gbuf, "Project", p->project_name, p->project_name, NULL, 0, 0, "{}");
    ...
    char branch_props[ENGINE_SZ_2K];
    const char *branch_props_json = "{}";
    if (engine_git_context_props_json(&p->git_ctx, branch_props, sizeof(branch_props)) > 0) {
        branch_props_json = branch_props;
    }
    if (p->branch_qn) {
        int64_t branch_id = engine_gbuf_upsert_node(p->gbuf, "Branch", branch_name, branch_qn, NULL, 0,
                                                 0, branch_props_json);
        const engine_gbuf_node_t *project_node = engine_gbuf_find_by_qn(p->gbuf, p->project_name);
        if (project_node && branch_id > 0) {
            engine_gbuf_insert_edge(p->gbuf, project_node->id, branch_id, "HAS_BRANCH",
                                 branch_props_json);
        }
    }

    /* Collect unique directories and create Folder/Package nodes */
    EngineHashTable *seen_dirs = engine_ht_create(ENGINE_SZ_256);
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        /* Create File node */
        char *file_qn = engine_pipeline_fqn_compute(p->project_name, rel, "__file__");
        /* Extract basename */
        ...
        engine_gbuf_upsert_node(p->gbuf, "File", basename, qualified_name, file_path, 0, 0, props);
        ...
        if (dir[0] == '\0') {
            ...
            parent_qn_heap = engine_pipeline_fqn_folder(p->project_name, dir);
        ...
        const engine_gbuf_node_t *fnode = engine_gbuf_find_by_qn(p->gbuf, file_qn);
        const engine_gbuf_node_t *pnode = engine_gbuf_find_by_qn(p->gbuf, parent_qn);
        if (fnode && pnode) {
            engine_gbuf_insert_edge(p->gbuf, pnode->id, fnode->id, "CONTAINS_FILE", "{}");
        }
    ...
    /* Free seen_dirs keys */
    engine_ht_foreach(seen_dirs, free_seen_dir_key, NULL);
    engine_ht_free(seen_dirs);

    engine_log_info("pass.done", "pass", "structure", "nodes", itoa_buf(engine_gbuf_node_count(p->gbuf)),
                 "edges", itoa_buf(engine_gbuf_edge_count(p->gbuf)));
    return 0;
}

/* ── Pass 2: Definitions ─────────────────────────────────────────── */

/* Implemented in pass_definitions.c via engine_pipeline_pass_definitions() */

/* ── Githistory compute thread (for fused post-pass parallelism) ─── */

typedef struct {
    const char *repo_path;
    engine_githistory_result_t *result;
} gh_compute_arg_t;

static void *gh_compute_thread_fn(void *arg) {
    gh_compute_arg_t *a = arg;
    engine_pipeline_githistory_compute(a->repo_path, a->result);
    return NULL;
}
...
static int process_one_infra_binding(engine_gbuf_t *gbuf, const EngineInfraBinding *ib,
                                     const char *rel_path) {
    char url_route_qn[ENGINE_ROUTE_QN_SIZE];
    snprintf(url_route_qn, sizeof(url_route_qn), "__route__infra__%s", ib->target_url);
    int64_t url_route_id = engine_gbuf_upsert_node(gbuf, "Route", ib->target_url, url_route_qn,
                                                rel_path, 0, 0, "{\"source\":\"infra\"}");
    char topic_route_qn[ENGINE_ROUTE_QN_SIZE];
    snprintf(topic_route_qn, sizeof(topic_route_qn), "__route__%s__%s",
             ib->broker ? ib->broker : "async", ib->source_name);
    const engine_gbuf_node_t *topic_route = engine_gbuf_find_by_qn(gbuf, topic_route_qn);
    ...
    if (topic_route) {
         ...
         * manifest). */
        topic_route_id = engine_gbuf_upsert_node(gbuf, "Route", ib->source_name, topic_route_qn,
                                              rel_path, 0, 0, ib->broker ? ib->broker : "async");
        if (topic_route_id <= 0) {
            return 0;
        }
    }
    char props[ENGINE_SZ_512];
    snprintf(props, sizeof(props), "{\"broker\":\"%s\",\"topic\":\"%s\",\"endpoint\":\"%s\"}",
             ib->broker ? ib->broker : "async", ib->source_name, ib->target_url);
    engine_gbuf_insert_edge(gbuf, topic_route_id, url_route_id, "INFRA_MAPS", props);
    return SKIP_ONE;
}

static void engine_pipeline_process_infra_bindings(engine_gbuf_t *gbuf, const engine_file_info_t *files,
                                                EngineFileResult **result_cache, int file_count) {
    int bindings = 0;
    for (int i = 0; i < file_count; i++) {
        if (!result_cache[i]) {
            continue;
        ...
        for (int bi = 0; bi < result_cache[i]->infra_bindings.count; bi++) {
            const EngineInfraBinding *ib = &result_cache[i]->infra_bindings.items[bi];
    ...
    if (bindings > 0) {
        char buf[ENGINE_SZ_16];
        snprintf(buf, sizeof(buf), "%d", bindings);
        engine_log_info("pass.infra_bindings", "linked", buf);
    }
}
...
static void try_upsert_infra_route(engine_gbuf_t *gbuf, const EngineStringRef *sr, const char *fp) {
    if (sr->kind != ENGINE_STRREF_URL || !sr->value || !strstr(sr->value, "://")) {
        return;
    }
    ...
    char route_props[ENGINE_SZ_512];
    if (sr->key_path) {
    ...
    engine_gbuf_upsert_node(gbuf, "Route", sr->value, route_qn, fp, 0, 0, route_props);
}
...
static bool route_sr_denied(const EngineStringRef *sr) {
...
static void engine_pipeline_extract_infra_routes(engine_gbuf_t *gbuf, const engine_file_info_t *files,
                                              EngineFileResult **result_cache, int file_count) {
    ...
    EngineHashTable *denied = engine_ht_create(16);
    for (int pass = 0; pass < 2; pass++) {
        for (int i = 0; i < file_count; i++) {
            ...
            for (int si = 0; si < result_cache[i]->string_refs.count; si++) {
                const EngineStringRef *sr = &result_cache[i]->string_refs.items[si];
                if (sr->kind != ENGINE_STRREF_URL || !sr->value || !strstr(sr->value, "://")) {
                    continue;
                }
                if (pass == 0) {
                    if (denied && route_sr_denied(sr)) {
                        engine_ht_set(denied, sr->value, (void *)1);
                    }
                } else if (!denied || !engine_ht_has(denied, sr->value)) {
                    try_upsert_infra_route(gbuf, sr, files[i].rel_path);
                }
            }
        }
    }
    engine_ht_free(denied);
}

/* Run decorator_tags, configlink, and route matching passes. */
typedef void (*predump_pass_fn)(engine_pipeline_ctx_t *);
static void predump_deco(engine_pipeline_ctx_t *ctx) {
    engine_pipeline_pass_decorator_tags(ctx->gbuf, ctx->project_name);
}
static void predump_route(engine_pipeline_ctx_t *ctx) {
    engine_pipeline_create_route_nodes(ctx->gbuf);
}
static void predump_sim(engine_pipeline_ctx_t *ctx) {
    engine_pipeline_pass_similarity(ctx);
}
static void predump_sem(engine_pipeline_ctx_t *ctx) {
    engine_pipeline_pass_semantic_edges(ctx);
}
static void predump_cfg(engine_pipeline_ctx_t *ctx) {
    engine_pipeline_pass_configlink(ctx);
}
static void predump_complexity(engine_pipeline_ctx_t *ctx) {
    engine_pipeline_pass_complexity(ctx);
}
static void run_predump_passes(engine_pipeline_t *p, engine_pipeline_ctx_t *ctx) {
    static const struct {
        predump_pass_fn fn;
        const char *name;
    ...
    for (int i = 0; i < PREDUMP_PASS_COUNT && !check_cancel(p); i++) {
        ...
        if (passes[i].moderate_only && p->mode == ENGINE_MODE_FAST) {
            continue;
        }
        engine_clock_gettime(CLOCK_MONOTONIC, &t);
        passes[i].fn(ctx);
        engine_log_info("pass.timing", "pass", passes[i].name, "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
    }
}

/* Adapter that lets engine_pipeline_pass_lsp_cross slot into the seq_passes
 * dispatch table. The cross-file LSP needs the per-file EngineFileResult cache
...
static int seq_pass_lsp_cross_dispatch(engine_pipeline_ctx_t *ctx, const engine_file_info_t *files,
                                       int file_count) {
    if (!ctx || !ctx->result_cache)
        return 0;
    /* Cross-file LSP runs in every mode. */
    return engine_pipeline_pass_lsp_cross(ctx, files, file_count, ctx->result_cache);
}
...
static int run_sequential_pipeline(engine_pipeline_t *p, engine_pipeline_ctx_t *ctx,
                                   const engine_file_info_t *files, int file_count,
                                   struct timespec *t) {
    engine_log_info("pipeline.mode", "mode", "sequential", "files", itoa_buf(file_count));
    ...
    engine_pipeline_set_pkgmap(engine_pkgmap_build_from_repo(ctx->repo_path, files, file_count,
                                                       ctx->project_name, ctx->excluded_dirs,
                                                       ctx->excluded_count));

    EngineFileResult **seq_cache = (EngineFileResult **)calloc(file_count, sizeof(EngineFileResult *));
    if (seq_cache) {
        ctx->result_cache = seq_cache;
    }

    typedef int (*seq_pass_fn)(engine_pipeline_ctx_t *, const engine_file_info_t *, int);
    static const struct {
        seq_pass_fn fn;
        const char *name;
        bool ignore_err;
    } seq_passes[] = {
        {engine_pipeline_pass_definitions, "definitions", false},
        {engine_pipeline_pass_k8s, "k8s", true},
        {seq_pass_lsp_cross_dispatch, "lsp_cross", true},
        {engine_pipeline_pass_calls, "calls", false},
        {engine_pipeline_pass_usages, "usages", false},
        {engine_pipeline_pass_semantic, "semantic", false},
    };
    int rc = 0;
    for (int si = 0; si < PL_SEQ_PASSES && rc == 0; si++) {
        ...
        engine_log_info("pass.timing", "pass", seq_passes[si].name, "elapsed_ms",
                     itoa_buf((int)elapsed_ms(*t)));
        if (check_cancel(p)) {
            rc = ENGINE_NOT_FOUND;
        }
    ...
    if (seq_cache && rc == 0) {
        engine_pipeline_extract_infra_routes(p->gbuf, files, seq_cache, file_count);
        engine_pipeline_process_infra_bindings(p->gbuf, files, seq_cache, file_count);
    }
    if (seq_cache) {
        for (int i = 0; i < file_count; i++) {
            if (seq_cache[i]) {
                engine_free_result(seq_cache[i]);
            }
        }
        free(seq_cache);
    ...
    if (ctx->seq_cross_arena_live) {
        engine_arena_destroy(&ctx->seq_cross_arena);
     ...
     * (engine_slab_install); destroying the stale parser then frees
    ...
    if (ctx->macro_table) {
        engine_macro_table_free((EngineMacroTable *)ctx->macro_table);
...
static int run_parallel_pipeline(engine_pipeline_t *p, engine_pipeline_ctx_t *ctx,
                                 const engine_file_info_t *files, int file_count, int worker_count,
                                 struct timespec *t) {
    engine_log_info("pipeline.mode", "mode", "parallel", "workers", itoa_buf(worker_count), "files",
                 itoa_buf(file_count));
    _Atomic int64_t shared_ids;
    atomic_init(&shared_ids, engine_gbuf_next_id(p->gbuf));
    EngineFileResult **cache = (EngineFileResult **)calloc(file_count, sizeof(EngineFileResult *));
    if (!cache) {
        engine_log_error("pipeline.err", "phase", "cache_alloc");
        return ENGINE_NOT_FOUND;
    }
    engine_clock_gettime(CLOCK_MONOTONIC, t);
    int rc = engine_parallel_extract(ctx, files, file_count, cache, &shared_ids, worker_count);
    engine_log_info("pass.timing", "pass", "parallel_extract", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(*t)));
    ...
    engine_gbuf_set_next_id(p->gbuf, atomic_load(&shared_ids));
    ...
    engine_mem_collect();
    engine_log_info("mem.collect", "phase", "post_extract", "rss_mb",
                 itoa_buf((int)(engine_mem_rss() / (1024 * 1024))));
    engine_clock_gettime(CLOCK_MONOTONIC, t);
    rc = engine_build_registry_from_cache(ctx, files, file_count, cache);
    engine_log_info("pass.timing", "pass", "registry_build", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(*t)));
    ...
    int64_t registry_next_id = engine_gbuf_next_id(p->gbuf);
    if (registry_next_id > atomic_load(&shared_ids)) {
        atomic_store(&shared_ids, registry_next_id);
    }
    /* Cross-file LSP precondition: build a project-wide EngineLSPDef[]
     * once. The fused resolve_worker invokes engine_pxc_run_one(_ts) per
     ...
     * engine_pipeline_pass_lsp_cross pass which re-read every source from
     ...
     * most expensive phase. ENGINE_DISABLE_LSP_CROSS=1 opts out (it can SIGSEGV
    ...
    char engine_lsp_cross_env[ENGINE_SZ_16];
    const bool run_cross_lsp = engine_safe_getenv("ENGINE_DISABLE_LSP_CROSS", engine_lsp_cross_env,
                                               sizeof(engine_lsp_cross_env), NULL) == NULL;
    if (!run_cross_lsp) {
        engine_log_info("lsp_cross.skipped", "reason", "ENGINE_DISABLE_LSP_CROSS env set");
    }
    ...
    if (run_cross_lsp) {
        ...
        all_defs = def_modules
                       ? engine_pxc_collect_all_defs(cache, files, file_count, ctx->project_name,
    ...
    if (ctx->pipeline && all_defs && def_starts) {
        engine_lsp_surface_row_t *surface_rows = NULL;
        int surface_count = 0;
        if (engine_lsp_surface_build_rows(ctx->project_name, cache, files, file_count, all_defs,
                                       def_starts, &surface_rows, &surface_count) == 0) {
            engine_pipeline_set_lsp_surfaces(ctx->pipeline, surface_rows, surface_count);
        } else {
            engine_log_warn("lsp_surface.serialize_failed", "files", itoa_buf(file_count));
        }
    }
    free(def_starts);
     ...
     * smaller per file. */
    EngineModuleDefIndex *module_def_index =
        all_defs ? engine_pxc_build_module_def_index(all_defs, def_count) : NULL;
    ...
    EngineArena cross_lsp_arena;
    engine_arena_init(&cross_lsp_arena);
    EngineCrossLspRegistries cross_registries = {0};
    if (all_defs) {
        cross_registries.go = engine_go_build_cross_registry(&cross_lsp_arena, all_defs, def_count);
        cross_registries.python =
            engine_py_build_cross_registry(&cross_lsp_arena, all_defs, def_count);
        cross_registries.c = engine_c_build_cross_registry(&cross_lsp_arena, all_defs, def_count);
        cross_registries.cs = engine_cs_build_cross_registry(&cross_lsp_arena, all_defs, def_count);
        cross_registries.ts = engine_ts_build_cross_registry(&cross_lsp_arena, all_defs, def_count);
        /* Rust: NOT built here. The shared all_defs registry is built LAZILY on the
         * first NULL-filter rust file (the amplifier files) inside engine_parallel_resolve
         * — repos whose rust files all filter to subsets never pay the build/RSS. */
    }
    engine_log_info("pass.timing", "pass", "lsp_cross_prepare", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(*t)));
    log_phase_mem("lsp_cross_prepare");
    engine_clock_gettime(CLOCK_MONOTONIC, t);
    rc = engine_parallel_resolve(ctx, files, file_count, cache, &shared_ids, worker_count, all_defs,
                              def_count, def_modules, module_def_index, &cross_registries);
    engine_log_info("pass.timing", "pass", "parallel_resolve", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(*t)));
    log_phase_mem("parallel_resolve");
    engine_pxc_free_module_def_index(module_def_index);
    engine_arena_destroy(&cross_lsp_arena); /* releases all per-lang registries */
    free(all_defs);
    if (def_modules) {
        for (int i = 0; i < file_count; i++) {
            free(def_modules[i]);
    ...
    engine_gbuf_set_next_id(p->gbuf, atomic_load(&shared_ids));
    engine_pipeline_extract_infra_routes(p->gbuf, files, cache, file_count);
    engine_pipeline_process_infra_bindings(p->gbuf, files, cache, file_count);
    for (int i = 0; i < file_count; i++) {
        if (cache[i]) {
            engine_free_result(cache[i]);
        }
    ...
    engine_clock_gettime(CLOCK_MONOTONIC, t);
    engine_pipeline_pass_k8s(ctx, files, file_count);
    engine_log_info("pass.timing", "pass", "k8s", "elapsed_ms", itoa_buf((int)elapsed_ms(*t)));
    return check_cancel(p) ? ENGINE_NOT_FOUND : 0;
}

static int capture_existing_adr(engine_pipeline_t *p, const char *db_path) {
#if defined(ENGINE_INCREMENTAL_TEST_API) && ENGINE_INCREMENTAL_TEST_API
    if (atomic_exchange(&g_persist_test_fail_adr_capture, false)) {
        return ENGINE_PIPELINE_ABORT_PRESERVE_DB;
    }
#endif
    engine_store_t *adr_store = engine_store_open_path_query(db_path);
    if (!adr_store) {
        return ENGINE_PIPELINE_ABORT_PRESERVE_DB;
    }
    engine_adr_t existing = {0};
    int adr_rc = engine_store_adr_get(adr_store, p->project_name, &existing);
    if (adr_rc == ENGINE_STORE_NOT_FOUND) {
        engine_store_close(adr_store);
        free(p->saved_adr);
        p->saved_adr = NULL;
        return 0;
    }
    if (adr_rc != ENGINE_STORE_OK || !existing.content) {
        engine_store_adr_free(&existing);
        engine_store_close(adr_store);
        return ENGINE_PIPELINE_ABORT_PRESERVE_DB;
    }
...
static int try_incremental_or_delete_db(engine_pipeline_t *p, engine_file_info_t *files, int file_count,
                                        const engine_file_hash_t *baseline_manifest,
                                        int baseline_count) {
    ...
    engine_store_t *check_store = engine_store_open_path_query(db_path);
    bool valid = check_store && engine_store_check_integrity(check_store);
    if (check_store) {
        engine_store_close(check_store);
    }
    if (!valid) {
        engine_log_warn("pipeline.route", "path", "full", "reason", "invalid_existing_db");
        free(db_path);
        return ENGINE_PIPELINE_FORCE_FULL_REINDEX;
    }
    engine_log_info("pipeline.route", "path", "incremental_manifest");
    int rc = engine_pipeline_run_incremental(p, db_path, files, file_count, baseline_manifest,
                                          baseline_count);
    ...
    if (rc == ENGINE_PIPELINE_FORCE_FULL_REINDEX) {
        int adr_rc = capture_existing_adr(p, db_path);
        if (adr_rc != 0) {
            rc = adr_rc;
        }
        (void)engine_unlink(db_path);
        (void)engine_remove_db_sidecars(db_path);
    }
    free(db_path);
    return rc;
}

static const char *pipeline_mode_name(engine_index_mode_t mode) {
    switch (mode) {
...
static int pipeline_mode_coverage_rank(engine_index_mode_t mode) {
    switch (mode) {
    case ENGINE_MODE_FULL:
        return 3;
    case ENGINE_MODE_MODERATE:
...
static void promote_mode_to_existing_coverage(engine_pipeline_t *p) {
    if (!p || !p->project_name) {
        return;
    }
    char *db_path = resolve_db_path(p);
    ...
    if (engine_store_coverage_meta_get(store, p->project_name, &meta) == ENGINE_STORE_OK &&
        meta.index_mode) {
        engine_index_mode_t stored_mode = p->mode;
        if (strcmp(meta.index_mode, "full") == 0) {
            stored_mode = ENGINE_MODE_FULL;
        } else if (strcmp(meta.index_mode, "moderate") == 0) {
            stored_mode = ENGINE_MODE_MODERATE;
        } else if (strcmp(meta.index_mode, "fast") == 0) {
            stored_mode = ENGINE_MODE_FAST;
        }
        if (pipeline_mode_coverage_rank(stored_mode) > pipeline_mode_coverage_rank(p->mode)) {
            engine_log_info("pipeline.mode", "requested", pipeline_mode_name(p->mode), "effective",
...
int engine_pipeline_refresh_artifact(engine_pipeline_t *p, const char *db_path) {
    ...
    bool existing = engine_artifact_exists(p->repo_path);
    if (!p->persistence && !existing) {
        return 0;
    }
    int quality = p->persistence ? ENGINE_ARTIFACT_BEST : ENGINE_ARTIFACT_FAST;
    int rc = engine_artifact_export(db_path, p->repo_path, p->project_name, quality);
    if (rc != 0) {
        const char *err = engine_artifact_export_last_error();
        if (p->persistence) {
            engine_log_error("pipeline.err", "phase", "artifact_export", "err", err ? err : "unknown");
            return rc;
        }
        engine_log_warn("artifact.refresh_failed", "err", err ? err : "unknown");
    }
    return 0;
}
...
static void discard_generation_stage(const char *stage_path) {
    ...
    engine_unlink(stage_path);
    engine_remove_db_sidecars(stage_path);
}

static int generation_rebuild_fts(engine_store_t *store) {
    if (engine_store_exec(store, "INSERT INTO nodes_fts(nodes_fts) VALUES('delete-all');") !=
        ENGINE_STORE_OK) {
        return ENGINE_STORE_ERR;
    }
    if (engine_store_exec(store,
                       "INSERT INTO nodes_fts(rowid, name, qualified_name, label, file_path) "
                       "SELECT id, engine_camel_split(name), qualified_name, label, file_path "
                       "FROM nodes;") == ENGINE_STORE_OK) {
        return ENGINE_STORE_OK;
    }
    return engine_store_exec(store,
                          "INSERT INTO nodes_fts(rowid, name, qualified_name, label, file_path) "
...
typedef struct {
    bool quarantined;
    char backup_path[ENGINE_SZ_4K];
} engine_replacement_prepare_t;

static int replacement_sidecar_path(char *out, size_t out_size, const char *base,
                                    const char *suffix) {
    int n = snprintf(out, out_size, "%s%s", base, suffix);
    return n > 0 && (size_t)n < out_size ? 0 : ENGINE_PIPELINE_PERSIST_FAILED;
}

static bool replacement_path_exists(const char *path) {
    engine_path_info_t info;
    return engine_path_info_utf8(path, &info) == 0;
}

static int rollback_quarantined_generation(const char *db_path,
                                           engine_replacement_prepare_t *prepared) {
    if (!prepared || !prepared->quarantined) {
        return 0;
    }
    static const char *const suffixes[] = {"-wal", "-shm"};
    if (engine_rename_noreplace(prepared->backup_path, db_path) != 0) {
        return ENGINE_PIPELINE_PERSIST_FAILED;
    }
    for (size_t i = 0; i < sizeof(suffixes) / sizeof(suffixes[0]); i++) {
        char source[ENGINE_SZ_4K];
        char destination[ENGINE_SZ_4K];
        ...
        if (replacement_path_exists(source) && engine_rename_noreplace(source, destination) != 0) {
            return ENGINE_PIPELINE_PERSIST_FAILED;
        }
    }
...
static int quarantine_existing_generation(const char *db_path,
    ...
    for (int attempt = 0; attempt < 10000; attempt++) {
        ...
        for (size_t i = 0; available && i < sizeof(suffixes) / sizeof(suffixes[0]); i++) {
            char candidate_sidecar[ENGINE_SZ_4K];
        ...
        if (engine_rename_noreplace(db_path, candidate) != 0) {
...
static int prepare_existing_generation_for_replace(const char *db_path,
    ...
    engine_path_info_t info;
    if (engine_path_info_utf8(db_path, &info) == 0) {
        if (!info.is_regular || info.is_symlink) {
            return ENGINE_PIPELINE_PERSIST_FAILED;
        }
        int seal_rc = engine_store_seal_existing_path_for_replace(db_path);
        if (seal_rc == ENGINE_STORE_NOT_FOUND) {
            if (!quarantine_invalid) {
                (void)engine_unlink(db_path);
                return engine_remove_db_sidecars(db_path) == 0 ? 0 : ENGINE_PIPELINE_PERSIST_FAILED;
            }
            return quarantine_existing_generation(db_path, prepared);
        }
        if (seal_rc != ENGINE_STORE_OK) {
            return ENGINE_PIPELINE_PERSIST_FAILED;
        }
    }
    return engine_remove_db_sidecars(db_path) == 0 ? 0 : ENGINE_PIPELINE_PERSIST_FAILED;
...
int engine_pipeline_publish_generation(const engine_pipeline_generation_t *generation) {
    ...
    int dump_rc = engine_gbuf_dump_to_sqlite(generation->gbuf, stage_path);
    if (dump_rc != 0) {
...
#if defined(ENGINE_INCREMENTAL_TEST_API) && ENGINE_INCREMENTAL_TEST_API
    if (engine_pipeline_persist_test_take_failure_after_stage_dump()) {
        discard_generation_stage(stage_path);
        free(stage_path);
        return ENGINE_PIPELINE_PERSIST_FAILED;
    }
    ...
    return engine_pipeline_publish_staged(stage_path, generation, true, false);
}
...
int engine_pipeline_publish_staged(char *stage_path, const engine_pipeline_generation_t *generation,
    ...
    engine_clock_gettime(CLOCK_MONOTONIC, &t_pub);
    engine_store_t *store = engine_store_open_path(stage_path);
    if (!store) {
        discard_generation_stage(stage_path);
        free(stage_path);
        return ENGINE_PIPELINE_PERSIST_FAILED;
    }
    bool ok = engine_store_exec(store, "PRAGMA synchronous=FULL;") == ENGINE_STORE_OK;
    ok = ok && engine_store_delete_file_hashes(store, generation->project) == ENGINE_STORE_OK &&
         engine_store_upsert_file_hash_batch(store, generation->manifest,
                                          generation->manifest_count) == ENGINE_STORE_OK;
    ...
    if (ok && !generation->surfaces_in_place) {
        ok = engine_store_delete_lsp_surfaces(store, generation->project) == ENGINE_STORE_OK &&
             engine_store_upsert_lsp_surface_batch(store, generation->surface_rows,
                                                generation->surface_row_count) == ENGINE_STORE_OK;
    }
    if (ok && generation->adr_content) {
        ok = engine_store_adr_store(store, generation->project, generation->adr_content) ==
             ENGINE_STORE_OK;
    }
    engine_log_info("publish.timing", "block", "writes", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t_pub)));
    engine_clock_gettime(CLOCK_MONOTONIC, &t_pub);

    engine_project_t project_info = {0};
    bool have_project_info =
        engine_store_get_project(store, generation->project, &project_info) == ENGINE_STORE_OK;
    engine_log_info("publish.timing", "block", "get_project", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t_pub)));
    engine_clock_gettime(CLOCK_MONOTONIC, &t_pub);
    engine_coverage_meta_t meta = generation->coverage_meta;
    ...
    if (!have_project_info ||
        engine_store_coverage_replace_ex(store, generation->project, generation->coverage,
                                      generation->coverage_count, &meta) != ENGINE_STORE_OK) {
        ok = false;
    }
    if (have_project_info) {
        engine_project_free_fields(&project_info);
    }
    engine_log_info("publish.timing", "block", "coverage_replace", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t_pub)));
    engine_clock_gettime(CLOCK_MONOTONIC, &t_pub);
    if (fts_wholesale && generation_rebuild_fts(store) != ENGINE_STORE_OK) {
        ok = false;
    }
    engine_log_info("publish.timing", "block", "fts", "elapsed_ms", itoa_buf((int)elapsed_ms(t_pub)));
    engine_clock_gettime(CLOCK_MONOTONIC, &t_pub);
    if (ok && !engine_store_check_integrity(store)) {
        ok = false;
    }
    engine_log_info("publish.timing", "block", "integrity", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t_pub)));
    engine_clock_gettime(CLOCK_MONOTONIC, &t_pub);
    if (ok && engine_store_seal_for_atomic_publish(store) != ENGINE_STORE_OK) {
        ok = false;
    }
    engine_log_info("publish.timing", "block", "seal", "elapsed_ms", itoa_buf((int)elapsed_ms(t_pub)));
    engine_store_close(store);
    if (!ok) {
        discard_generation_stage(stage_path);
        free(stage_path);
    ...
    int fin_rc = engine_pipeline_finalize_staged_generation(
...
int engine_pipeline_finalize_staged_generation(char *stage_path, const char *final_db_path,
    ...
    engine_clock_gettime(CLOCK_MONOTONIC, &t_fin);
    if (engine_remove_db_sidecars(stage_path) != 0) {
        discard_generation_stage(stage_path);
        return ENGINE_PIPELINE_PERSIST_FAILED;
    }
    ...
    engine_log_info("finalize.timing", "block", "stage_sidecars", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t_fin)));
    engine_clock_gettime(CLOCK_MONOTONIC, &t_fin);
    engine_replacement_prepare_t prepared = {0};
    ...
    if (destination_known_healthy) {
        if (engine_remove_db_sidecars(final_db_path) != 0) {
            engine_pipeline_discard_stage(stage_path);
            return ENGINE_PIPELINE_PERSIST_FAILED;
        }
...
#if defined(ENGINE_INCREMENTAL_TEST_API) && ENGINE_INCREMENTAL_TEST_API
    if (engine_pipeline_persist_test_take_cancel_after_destination_prepare() && cancelled) {
    ...
    if (cancelled && atomic_load(cancelled)) {
        ...
        discard_generation_stage(stage_path);
        return rollback_rc == 0 ? ENGINE_PIPELINE_ABORT_PRESERVE_DB : ENGINE_PIPELINE_PERSIST_FAILED;
    }
    engine_log_info("finalize.timing", "block", "prepare_live", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t_fin)));
    engine_clock_gettime(CLOCK_MONOTONIC, &t_fin);
    if (engine_rename_replace(stage_path, final_db_path) != 0) {
        (void)rollback_quarantined_generation(final_db_path, &prepared);
        discard_generation_stage(stage_path);
        return ENGINE_PIPELINE_PERSIST_FAILED;
    }
    engine_log_info("finalize.timing", "block", "rename", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t_fin)));
    return 0;
}

/* Dump graph to SQLite and persist file hashes for incremental indexing. */
static int dump_and_persist_hashes(engine_pipeline_t *p, const engine_file_hash_t *baseline_manifest,
    ...
    if (last_slash) {
        *last_slash = '\0';
        engine_mkdir_p(db_dir, ENGINE_DIR_PERMS);
    }
...
#endif
    if (engine_pipeline_build_fresh_semantic_manifest(p->project_name, p->repo_path, p->mode,
                                                   &manifest, &manifest_count) != 0) {
        engine_log_error("pipeline.err", "phase", "semantic_manifest");
    ...
    if (!engine_pipeline_semantic_manifests_equal(baseline_manifest, baseline_count, manifest,
                                               manifest_count)) {
        engine_log_warn("pipeline.abort", "reason", "semantic_inputs_changed");
    ...
    if (cov_total > 0) {
        ...
        if (cov) {
            ...
            for (int i = 0; i < p->file_errors_count; i++) {
                cov[cov_count++] = (engine_coverage_row_t){.rel_path = p->file_errors[i].path,
                                                        .kind = p->file_errors[i].phase,
                                                        .detail = p->file_errors[i].reason};
            }
            for (int i = 0; i < p->excluded_count; i++) {
                cov[cov_count++] = (engine_coverage_row_t){.rel_path = p->excluded_dirs[i],
                                                        .kind = "not_indexed_dir",
                                                        .detail = "excluded subtree"};
            }
            for (int i = 0; i < p->ignored_count; i++) {
                cov[cov_count++] = (engine_coverage_row_t){.rel_path = p->ignored_files[i].rel_path,
    ...
    free(db_dir);
    /* Capture committed counts BEFORE the dump. engine_gbuf_dump_to_sqlite calls
     * release_gbuf_indexes(), which frees node_by_qn (graph_buffer.c), after
     * which engine_gbuf_node_count() returns 0. Reading these post-dump left
     * committed_nodes at 0, so the #334 plausibility gate never fired. */
    p->committed_nodes = engine_gbuf_node_count(p->gbuf);
    p->committed_edges = engine_gbuf_edge_count(p->gbuf);
    ...
    engine_log_info("pass.timing", "pass", "dump_and_persist", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(*t)), "files", itoa_buf(manifest_count));
    if (p->ignored_total > p->ignored_count) {
        engine_log_warn("index.ignored_capped", "stored", itoa_buf(p->ignored_count), "total",
    ...
    int artifact_rc = engine_pipeline_refresh_artifact(p, db_path);
    free(db_path);
    return artifact_rc;
}

/* Run githistory pass. */
static int run_githistory(engine_pipeline_t *p, engine_pipeline_ctx_t *ctx) {
    struct timespec t_gh;
    engine_clock_gettime(CLOCK_MONOTONIC, &t_gh);

    engine_githistory_result_t gh_result = {0};
    engine_thread_t gh_thread;
    bool gh_threaded = false;
    gh_compute_arg_t gh_arg = {.repo_path = ctx->repo_path, .result = &gh_result};

    if (p->mode != ENGINE_MODE_FAST) {
        if (effective_worker_count(true) > SKIP_ONE) {
            if (engine_thread_create(&gh_thread, 0, gh_compute_thread_fn, &gh_arg) == 0) {
                gh_threaded = true;
            }
        }
        if (!gh_threaded) {
            engine_pipeline_githistory_compute(ctx->repo_path, &gh_result);
            engine_log_info("pass.timing", "pass", "githistory_compute", "elapsed_ms",
                         itoa_buf((int)elapsed_ms(t_gh)));
        }
    } else {
        engine_log_info("pass.skip", "pass", "githistory", "reason", "fast_mode");
    }

    if (gh_threaded) {
        engine_thread_join(&gh_thread);
        engine_log_info("pass.timing", "pass", "githistory_compute", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t_gh)));
    }
    ...
    if (gh_result.count > 0 || gh_result.file_temporal_count > 0) {
        gh_edges = engine_pipeline_githistory_apply(ctx, &gh_result);
    }
    engine_log_info("pass.done", "pass", "githistory", "commits", itoa_buf(gh_result.commit_count),
...
static int run_tests_and_history(engine_pipeline_t *p, engine_pipeline_ctx_t *ctx,
                                 const engine_file_info_t *files, int file_count) {
    struct timespec t;
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    ENGINE_PROF_START(t_tests);
    int rc = engine_pipeline_pass_tests(ctx, files, file_count);
    ENGINE_PROF_END_N("pipeline", "pass_tests", t_tests, file_count);
    engine_log_info("pass.timing", "pass", "tests", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    if (rc == 0 && !check_cancel(p)) {
        ENGINE_PROF_START(t_gh);
        rc = run_githistory(p, ctx);
        ENGINE_PROF_END("pipeline", "pass_githistory", t_gh);
    }
    if (check_cancel(p)) {
        return ENGINE_NOT_FOUND;
    }
...
static int run_post_extraction(engine_pipeline_t *p, engine_pipeline_ctx_t *ctx,
                               const engine_file_info_t *files, int file_count,
    ...
    ENGINE_PROF_START(t_predump);
    run_predump_passes(p, ctx);
    ENGINE_PROF_END("pipeline", "3_predump_passes_total", t_predump);

#if defined(ENGINE_INCREMENTAL_TEST_API) && ENGINE_INCREMENTAL_TEST_API
    if (engine_pipeline_persist_test_take_cancel_after_predump()) {
        atomic_store(p->cancelled, 1);
    }
#endif
    ...
    struct timespec t;
    ENGINE_PROF_START(t_dump);
    rc = dump_and_persist_hashes(p, baseline_manifest, baseline_count, &t);
    ENGINE_PROF_END("pipeline", "4_dump_and_persist", t_dump);
    return rc;
}
...
static int run_extraction_phase(engine_pipeline_t *p, engine_pipeline_ctx_t *ctx,
                                const engine_file_info_t *files, int file_count) {
    struct timespec t;
    engine_clock_gettime(CLOCK_MONOTONIC, &t);
    ENGINE_PROF_START(t_struct);
    pass_structure(p, files, file_count);
    ENGINE_PROF_END_N("pipeline", "pass_structure", t_struct, file_count);
    engine_log_info("pass.timing", "pass", "structure", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    if (check_cancel(p)) {
        return ENGINE_NOT_FOUND;
    }

    int worker_count = effective_worker_count(true);
    ENGINE_PROF_START(t_extract_total);
    ...
    ENGINE_PROF_END_N("pipeline", "2_extraction_total", t_extract_total, file_count);
    if (check_cancel(p)) {
        return ENGINE_NOT_FOUND;
    }
    return rc;
...
static int engine_pipeline_run_staged(engine_pipeline_t *p, bool *was_incremental) {
    if (!p) {
        return ENGINE_NOT_FOUND;
    }
    *was_incremental = false;

    ENGINE_PROF_START(t_pipeline_total);
    struct timespec t0;
    engine_clock_gettime(CLOCK_MONOTONIC, &t0);
    engine_path_alias_collection_t *path_aliases = NULL;
    engine_file_hash_t *baseline_manifest = NULL;
    int baseline_count = 0;

    promote_mode_to_existing_coverage(p);

    /* engine_pipeline_new() may precede the actual run by an arbitrary interval.
    ...
    engine_set_macro_extraction(p->mode == ENGINE_MODE_FULL);

    /* Load user-defined extension overrides (fail-open: NULL on error) */
    ENGINE_PROF_START(t_userconfig);
    p->userconfig = engine_userconfig_load(p->repo_path);
    engine_set_user_lang_config(p->userconfig);
    ENGINE_PROF_END("pipeline", "0_userconfig_load", t_userconfig);

    /* Phase 1: Discover files */
    ENGINE_PROF_START(t_discover);
    engine_discover_opts_t opts = {
        .mode = p->mode,
        .ignore_file = NULL,
        .max_file_size = 0,
    };
    engine_file_info_t *files = NULL;
    int file_count = 0;
    ...
    p->ignored_count = 0;
    p->ignored_total = 0;
    int rc = engine_discover_ex2(p->repo_path, &opts, &files, &file_count, &p->excluded_dirs,
                              &p->excluded_count, &p->ignored_files, &p->ignored_count,
                              &p->ignored_total);
    if (rc != 0) {
        engine_log_error("pipeline.err", "phase", "discover", "rc", itoa_buf(rc));
    }
    ENGINE_PROF_END_N("pipeline", "1_discover", t_discover, file_count);
    engine_log_info("pipeline.discover", "files", itoa_buf(file_count), "elapsed_ms",
    ...
    rc = engine_pipeline_build_semantic_manifest(p->project_name, p->repo_path, files, file_count,
                                              p->excluded_dirs, p->excluded_count, &p->git_ctx,
                                              p->userconfig, &baseline_manifest, &baseline_count);
    if (rc != 0) {
        rc = ENGINE_PIPELINE_ABORT_PRESERVE_DB;
        goto cleanup;
    }
    ...
    if (rc == ENGINE_PIPELINE_ABORT_PRESERVE_DB || rc == ENGINE_PIPELINE_PERSIST_FAILED) {
        goto cleanup;
    }
    if (rc >= 0) {
    ...
    engine_log_info("pipeline.route", "path", "full");

    /* Phase 2: Create graph buffer and registry */
    p->gbuf = engine_gbuf_new(p->project_name, p->repo_path);
    p->registry = engine_registry_new();
    ...
    path_aliases =
        engine_load_path_aliases_excluded(p->repo_path, p->excluded_dirs, p->excluded_count);
    ...
    engine_log_info("pipeline.done", "nodes", itoa_buf(p->committed_nodes), "edges",
                 itoa_buf(p->committed_edges), "elapsed_ms", itoa_buf((int)elapsed_ms(t0)));
    ENGINE_PROF_END("pipeline", "TOTAL", t_pipeline_total);

cleanup:
    engine_pkgmap_free(engine_pipeline_get_pkgmap());
    engine_pipeline_set_pkgmap(NULL);
    engine_discover_free(files, file_count);
    engine_pipeline_free_semantic_manifest(baseline_manifest, baseline_count);
    engine_gbuf_free(p->gbuf);
    p->gbuf = NULL;
    engine_registry_free(p->registry);
    p->registry = NULL;
...
static void cleanup_staging_db(const char *path) {
    ...
    (void)engine_unlink(path);
    (void)engine_remove_db_sidecars(path);
}
...
static bool ensure_db_parent(const char *path) {
    ...
    *slash = '\0';
    bool ok = dir[0] == '\0' || engine_mkdir_p(dir, ENGINE_DIR_PERMS);
    free(dir);
    return ok;
}
...
static char *create_staging_path(const char *final_path) {
...
#ifdef _WIN32
    /* The Windows engine_mkstemp compatibility contract may expand a /tmp/
    ...
    if (path_size > ENGINE_SZ_4K) {
        return NULL;
    }
    path_size = ENGINE_SZ_4K;
#endif
 ...
 * ENGINE_PIPELINE_* code the caller propagates.
 *
...
static int prepare_publish_destination(const char *final_path, bool final_existed,
                                       bool backup_succeeded, engine_replacement_prepare_t *prepared) {
    memset(prepared, 0, sizeof(*prepared));
    ...
    if (!final_exists_now) {
        ...
        return engine_remove_db_sidecars(final_path) == 0 ? 0 : ENGINE_PIPELINE_PERSIST_FAILED;
    }
    if (!backup_succeeded) {
        ...
        if (!db_sidecars_absent(final_path)) {
            engine_log_error("pipeline.err", "phase", "publish", "reason",
    ...
    return engine_store_prepare_path_for_replace(final_path) == ENGINE_STORE_OK &&
                   engine_remove_db_sidecars(final_path) == 0
               ? 0
               : ENGINE_PIPELINE_PERSIST_FAILED;
}

static int seal_staging_db(const char *staging_path) {
    engine_store_t *store = engine_store_open_path(staging_path);
    if (!store) {
        return ENGINE_NOT_FOUND;
    }
    int rc =
        engine_store_check_integrity(store) && engine_store_prepare_for_publish(store) == ENGINE_STORE_OK
            ? 0
            : ENGINE_NOT_FOUND;
    engine_store_close(store);
    if (rc == 0 && engine_remove_db_sidecars(staging_path) != 0) {
        rc = ENGINE_NOT_FOUND;
    }
    return rc;
}

static int export_after_publish(engine_pipeline_t *p, const char *final_path, bool was_incremental) {
    if (p->persistence) {
        ENGINE_PROF_START(t_art);
        int rc = engine_artifact_export(final_path, p->repo_path, p->project_name, ENGINE_ARTIFACT_BEST);
        ENGINE_PROF_END("persist", "6_artifact_export", t_art);
        if (rc != 0) {
            const char *err = engine_artifact_export_last_error();
            engine_log_error("pipeline.err", "phase", "artifact_export", "err", err ? err : "unknown");
        }
    ...
    if (was_incremental && p->repo_path && engine_artifact_exists(p->repo_path)) {
        (void)engine_artifact_export(final_path, p->repo_path, p->project_name, ENGINE_ARTIFACT_FAST);
    }
    return 0;
}

int engine_pipeline_run(engine_pipeline_t *p) {
    if (!p) {
        return ENGINE_NOT_FOUND;
    }
    ...
    if (final_existed) {
        backup_succeeded = engine_store_backup_path(final_path, staging_path) == ENGINE_STORE_OK;
        if (!backup_succeeded) {
            engine_log_warn("pipeline.stage", "action", "backup_failed_full_rebuild", "path",
                         final_path);
    ...
    int rc = engine_pipeline_run_staged(p, &was_incremental);
    free(p->db_path);
    p->db_path = configured_db_path;
    ...
    if (prepare_rc != 0) {
        engine_log_error("pipeline.err", "phase", "publish", "path", final_path);
        cleanup_staging_db(staging_path);
        free(staging_path);
        free(final_path);
        return prepare_rc;
    ...
    if ((p->rename_hook ? p->rename_hook(staging_path, final_path, p->rename_hook_ctx)
                        : engine_rename_replace(staging_path, final_path)) != 0) {
        engine_log_error("pipeline.err", "phase", "publish", "path", final_path);
