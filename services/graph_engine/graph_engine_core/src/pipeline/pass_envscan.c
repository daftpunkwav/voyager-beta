...
#include "foundation/platform.h" /* engine_normalize_path_sep */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/stat.h>
...
#ifdef _WIN32
#include "foundation/win_utf8.h" /* engine_path_to_wide */
#include <windows.h>
...
static engine_regex_t dockerfile_re;  /* ENV|ARG KEY=VALUE or KEY VALUE */
static engine_regex_t yaml_kv_re;     /* key: "https://..." */
static engine_regex_t yaml_setenv_re; /* --set-env-vars KEY=VALUE */
static engine_regex_t terraform_re;   /* default|value = "https://..." */
static engine_regex_t shell_re;       /* [export] KEY=https://... */
static engine_regex_t envfile_re;     /* KEY=https://... */
static engine_regex_t toml_re;        /* key = "https://..." */
static engine_regex_t properties_re;  /* key=https://... */
...
static void compile_patterns(void) {
    ...
    engine_regcomp(&dockerfile_re, "^(ENV|ARG)[[:space:]]+(" W "+)[= ](.*)", ENGINE_REG_EXTENDED);
    engine_regcomp(&yaml_kv_re, "(" W "+):[[:space:]]*[\"']?(https?://" NW "+)", ENGINE_REG_EXTENDED);
    engine_regcomp(&yaml_setenv_re, "--set-env-vars[[:space:]]+(" W "+)=([^ \t]+)", ENGINE_REG_EXTENDED);
    engine_regcomp(&terraform_re, "(default|value)[[:space:]]*=[[:space:]]*\"(https?://[^\"]+)\"",
                ENGINE_REG_EXTENDED);
    engine_regcomp(&shell_re, "(export[[:space:]]+)?(" W "+)=[\"']?(https?://" NW "+)",
                ENGINE_REG_EXTENDED);
    engine_regcomp(&envfile_re, "^(" W "+)=(https?://[^ \t]+)", ENGINE_REG_EXTENDED);
    engine_regcomp(&toml_re, "(" W "+)[[:space:]]*=[[:space:]]*\"(https?://[^\"]+)\"",
                ENGINE_REG_EXTENDED);
    engine_regcomp(&properties_re, "(" W "+)[[:space:]]*=[[:space:]]*(https?://[^ \t]+)",
                ENGINE_REG_EXTENDED);
...
static int extract_kv_groups(const char *trimmed, const engine_regmatch_t *m, int key_grp, int val_grp,
...
static int scan_dockerfile_line(const char *line, char *key, size_t ksz, char *val, size_t vsz) {
    engine_regmatch_t m[ENV_REGEX_MAX];
    if (engine_regexec(&dockerfile_re, line, ENV_GRP_4, m, 0) != 0) {
        return 0;
    }
...
static int scan_yaml_line(const char *line, char *key, size_t ksz, char *val, size_t vsz) {
    engine_regmatch_t m[ENV_REGEX_MAX];
    if (engine_regexec(&yaml_kv_re, line, ENV_GRP_3, m, 0) == 0 &&
        extract_kv_groups(line, m, ENV_GRP_1, ENV_GRP_2, key, ksz, val, vsz)) {
        return SKIP_ONE;
    }
    if (engine_regexec(&yaml_setenv_re, line, ENV_GRP_3, m, 0) == 0 &&
        extract_kv_groups(line, m, ENV_GRP_1, ENV_GRP_2, key, ksz, val, vsz)) {
        return SKIP_ONE;
    }
    return 0;
...
static int scan_terraform_line(const char *line, char *key, size_t ksz, char *val, size_t vsz) {
    engine_regmatch_t m[ENV_REGEX_MAX];
    if (engine_regexec(&terraform_re, line, ENV_GRP_3, m, 0) != 0) {
        return 0;
    }
...
static int scan_regex_line(engine_regex_t *re, const char *line, int kg, int vg, char *key, size_t ksz,
                           char *val, size_t vsz) {
    engine_regmatch_t m[ENV_REGEX_MAX];
    if (engine_regexec(re, line, ENV_GRP_5, m, 0) == 0 &&
...
static bool envscan_join_path(char *out, size_t out_sz, const char *dir, const char *name) {
    ...
    if (written <= 0 || (size_t)written >= out_sz) {
        out[0] = '\0';
        engine_log_warn("envscan.path_too_long", "dir", dir, "name", name);
        return false;
    }
    return true;
}
 ...
 * Returns 0 on success, ENGINE_NOT_FOUND to skip the entry.
...
static int envscan_safe_stat(const char *abs_path, struct stat *st) {
    ...
    wchar_t *wide = engine_path_to_wide(abs_path);
    if (!wide) {
 ...
 * scan because engine_canonical_path costs a syscall per component. */
static bool envscan_within_root(const char *candidate, const char *canonical_root) {
    char resolved[ENGINE_SZ_4K];
    if (!engine_canonical_path(candidate, resolved, sizeof(resolved))) {
        return false;
    }
#ifdef _WIN32
    engine_normalize_path_sep(resolved);
#endif
 ...
 * allows it. Mode "r" is preserved from the previous engine_fopen call so the
...
static FILE *envscan_open_file(const char *full_path) {
     ...
     * envscan_safe_stat. engine_fopen is required for UTF-8 → _wfopen mapping. */
    return engine_fopen(full_path, "r");
#else
     ...
     * engine_fopen rule exists for Windows' wide-API mapping. */
...
static int scan_env_file(const char *full_path, const char *rel, file_type_t ft,
                         engine_env_binding_t *out, int max_out) {
    ...
    struct stat fst;
    if (fstat(fileno(f), &fst) != 0 || fst.st_size > (long)ENGINE_SZ_1K * ENGINE_SZ_1K) {
        (void)fclose(f);
        return 0;
    }
    ...
    while (fgets(line, sizeof(line), f) && count < max_out) {
        ...
        if (engine_is_secret_binding(key, value) || engine_is_secret_value(value)) {
            continue;
        }
...
static int process_env_entry(engine_dirent_t *ent, const char *dir_path, const char *root_path,
                             size_t root_len, const char *canonical_root, engine_env_binding_t *out,
                             int max_out, char path_stack[][ENGINE_SZ_512], int *stack_top,
                             char **excluded_dirs, int excluded_count) {
    char full_path[ENGINE_SZ_512];
    ...
    if (S_ISDIR(st.st_mode)) {
        if (is_ignored_dir(ent->name) ||
            engine_pipeline_relpath_is_excluded(rel, excluded_dirs, excluded_count) ||
            *stack_top >= ENGINE_SZ_256) {
            return 0;
        }
        if (!envscan_within_root(full_path, canonical_root)) {
            engine_log_warn("envscan.dir_outside_root", "path", full_path, "root", canonical_root);
            return 0;
        }
        ...
        if (written <= 0 || (size_t)written >= sizeof(path_stack[0])) {
            engine_log_warn("envscan.path_too_long", "dir", dir_path, "name", ent->name);
            return 0;
        }
        (*stack_top)++;
        return 0;
...
int engine_scan_project_env_urls_excluded(const char *root_path, engine_env_binding_t *out, int max_out,
    ...
    int count = 0;
    char path_stack[ENGINE_SZ_256][ENGINE_SZ_512];
    ...
    if (written <= 0 || (size_t)written >= sizeof(path_stack[0])) {
        engine_log_warn("envscan.root_too_long", "root", root_path);
        return 0;
    }
    ...
    char canonical_root[ENGINE_SZ_4K];
...
#ifdef _WIN32
    engine_normalize_path_sep(canonical_root);
#endif

    while (stack_top > 0 && count < max_out) {
        stack_top--;
        char dir_path[ENGINE_SZ_512];
        ...
        engine_dirent_t *ent;
        while ((ent = engine_readdir(d)) && count < max_out) {
...
int engine_scan_project_env_urls(const char *root_path, engine_env_binding_t *out, int max_out) {
    return engine_scan_project_env_urls_excluded(root_path, out, max_out, NULL, 0);
}
