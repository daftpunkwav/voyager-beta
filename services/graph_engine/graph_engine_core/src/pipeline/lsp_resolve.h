 ...
 *
 * Both pipeline paths (sequential engine_pipeline_pass_calls and parallel
 * engine_parallel_extract → resolve_file_calls) need to look up an
 ...
 */
#ifndef ENGINE_PIPELINE_LSP_RESOLVE_H
#define ENGINE_PIPELINE_LSP_RESOLVE_H
 ...
 * (Go, C/C++, Python, PHP). */
#define ENGINE_LSP_CONFIDENCE_FLOOR 0.6f
...
void engine_pipeline_lsp_reference_lookup_test_note_row(void);
#else
static inline void engine_pipeline_lsp_reference_lookup_test_note_row(void) {}
#endif
 ...
 * for them. */
static inline const char *engine_lsp_bare_segment(const char *name) {
    if (!name) {
        return name;
    }
    const char *seg = name;
...
static inline const char *engine_pipeline_qn_class_method_tail(const char *qn) {
    if (!qn) {
        return NULL;
    }
...
static inline const char *engine_pipeline_call_callee_leaf(const char *callee_name) {
    return engine_lsp_bare_segment(callee_name);
}
...
static inline bool engine_pipeline_lsp_allow_tail_match(EngineLanguage lang) {
...
static inline bool engine_pipeline_call_reference_usage_fallback_allowed(EngineLanguage lang,
                                                                      const char *strategy) {
    if (lang != ENGINE_LANG_JAVA && lang != ENGINE_LANG_KOTLIN) {
        return true;
    }
...
static inline bool engine_pipeline_reference_candidate_fallback_allowed(
    EngineLanguage lang, const EngineUsage *usage, const char *strategy, const char *const *import_names,
    int import_count) {
    if (!usage) {
        return false;
    }
    bool reference_candidate =
        usage->kind == ENGINE_USAGE_CALL_REFERENCE ||
        (lang == ENGINE_LANG_KOTLIN && usage->kind == ENGINE_USAGE_VALUE && usage->may_be_call_reference);
    if (!reference_candidate ||
        engine_pipeline_call_reference_usage_fallback_allowed(lang, strategy)) {
        return true;
    }
    ...
    if (lang == ENGINE_LANG_KOTLIN && usage->ref_name && import_names) {
...
static inline bool engine_pipeline_usage_semantic_reference_candidate(const EngineUsage *usage) {
    if (!usage) {
        return false;
    }
    return usage->kind == ENGINE_USAGE_CALL_REFERENCE ||
           (usage->kind == ENGINE_USAGE_VALUE && usage->may_be_call_reference);
}
...
static inline bool engine_pipeline_usage_allows_semantic_reference(const EngineUsage *usage,
                                                                const EngineResolvedCall *resolved) {
    if (!usage || !resolved) {
        return false;
    }
    if (usage->kind != ENGINE_USAGE_VALUE || !usage->semantic_reference_blocked ||
...
static inline bool engine_pipeline_node_is_callable_target(const engine_gbuf_node_t *node) {
    if (!node || !node->label) {
        return false;
    }
...
static inline int engine_pipeline_qn_class_method_tail_eq(const char *qn, const char *tail) {
    const char *qt = engine_pipeline_qn_class_method_tail(qn);
    return qt && tail && strcmp(qt, tail) == 0;
}

static inline const engine_gbuf_node_t *engine_pipeline_lsp_target_node_policy(
    const engine_gbuf_t *gbuf, const char *project_name, const char *callee_qn, bool allow_tail_match,
 ...
 * class and method. */
static inline bool engine_pipeline_invocation_targets_equal(const char *left_qn, const char *right_qn,
                                                         const engine_gbuf_t *gbuf,
    ...
    const engine_gbuf_node_t *left_exact =
        engine_pipeline_lsp_target_node_policy(gbuf, project_name, left_qn, false, false);
    const engine_gbuf_node_t *right_exact =
        engine_pipeline_lsp_target_node_policy(gbuf, project_name, right_qn, false, false);
    ...
    const char *left_tail = engine_pipeline_qn_class_method_tail(left_qn);
    const char *right_tail = engine_pipeline_qn_class_method_tail(right_qn);
    ...
    const engine_gbuf_node_t *left =
        left_exact ? left_exact
                   : engine_pipeline_lsp_target_node_policy(gbuf, project_name, left_qn, true, true);
    const engine_gbuf_node_t *right =
        right_exact ? right_exact
                    : engine_pipeline_lsp_target_node_policy(gbuf, project_name, right_qn, true, true);
...
static inline bool engine_pipeline_source_site_present(uint32_t start, uint32_t end) {
    return end > start;
}

static inline bool engine_pipeline_source_site_legacy(uint32_t start, uint32_t end) {
    return start == 0 && end == 0;
}

static inline bool engine_pipeline_source_site_eq(uint32_t lhs_start, uint32_t lhs_end,
                                               uint32_t rhs_start, uint32_t rhs_end) {
    return engine_pipeline_source_site_present(lhs_start, lhs_end) &&
           engine_pipeline_source_site_present(rhs_start, rhs_end) && lhs_start == rhs_start &&
           lhs_end == rhs_end;
}

static inline bool engine_pipeline_source_occurrence_eq(uint32_t lhs_start, uint32_t lhs_end,
                                                     EngineSourceOrigin lhs_origin, uint32_t rhs_start,
                                                     uint32_t rhs_end, EngineSourceOrigin rhs_origin) {
    return lhs_origin == rhs_origin &&
           engine_pipeline_source_site_eq(lhs_start, lhs_end, rhs_start, rhs_end);
}
...
static inline int engine_pipeline_invocation_site_rank(const EngineResolvedCall *resolved,
                                                    const EngineCall *call) {
    if (!resolved || !call) {
        return 0;
    }
    bool call_has_site =
        engine_pipeline_source_site_present(call->site_start_byte, call->site_end_byte);
    bool call_is_legacy =
        engine_pipeline_source_site_legacy(call->site_start_byte, call->site_end_byte);
    if (!call_has_site && !call_is_legacy) {
        return 0;
    }
    if (engine_pipeline_source_site_present(resolved->site_start_byte, resolved->site_end_byte)) {
        return engine_pipeline_source_occurrence_eq(call->site_start_byte, call->site_end_byte,
                                                 call->source_origin, resolved->site_start_byte,
                                                 resolved->site_end_byte, resolved->source_origin)
                   ? 2
                   : 0;
    }
    if (!engine_pipeline_source_site_legacy(resolved->site_start_byte, resolved->site_end_byte)) {
        return 0;
    }
    return call->requires_lsp_resolution || call->source_origin != resolved->source_origin ? 0 : 1;
}

static inline bool engine_pipeline_invocation_reason_join_strategy(const char *strategy) {
    return strategy &&
...
static inline bool engine_pipeline_invocation_leaf_matches(const EngineResolvedCall *resolved,
                                                        const EngineCall *call, int site_rank) {
    if (!resolved || !resolved->callee_qn || !call || !call->callee_name) {
        return false;
    }
    const char *resolved_leaf = engine_lsp_bare_segment(resolved->callee_qn);
    const char *call_leaf = engine_lsp_bare_segment(call->callee_name);
    ...
    return engine_pipeline_invocation_reason_join_strategy(resolved->strategy) && resolved->reason &&
           call_leaf && strcmp(engine_lsp_bare_segment(resolved->reason), call_leaf) == 0;
}
 ...
 *      (JVM callers only, see engine_pipeline_lsp_allow_tail_match), a
 ...
 * bare last segment by engine_lsp_bare_segment before matching.
 *
 * The pointer returned aliases into `arr` and stays valid as long as the
 * underlying EngineFileResult is alive. */
static inline const EngineResolvedCall *engine_pipeline_find_lsp_resolution_in_graph(
    const EngineResolvedCallArray *arr, const EngineCall *call, bool allow_tail_match,
    ...
    const EngineResolvedCall *best_exact_caller = NULL;
    int best_exact_caller_rank = 0;
    bool exact_caller_ambiguous = false;
    for (int i = 0; i < arr->count; i++) {
        const EngineResolvedCall *rc = &arr->items[i];
        if (rc->kind != ENGINE_RESOLVED_INVOCATION) {
            continue;
        }
        if (!rc->caller_qn || !rc->callee_qn) {
            continue;
        }
        if (rc->confidence < ENGINE_LSP_CONFIDENCE_FLOOR) {
            continue;
        }
        if (strcmp(rc->caller_qn, call->enclosing_func_qn) != 0) {
            continue;
        }
        int site_rank = engine_pipeline_invocation_site_rank(rc, call);
        if (site_rank == 0 || !engine_pipeline_invocation_leaf_matches(rc, call, site_rank)) {
            continue;
        }
        ...
        if (!engine_pipeline_invocation_targets_equal(best_exact_caller->callee_qn, rc->callee_qn,
    ...
    const char *call_tail = engine_pipeline_qn_class_method_tail(call->enclosing_func_qn);

    const EngineResolvedCall *best_tail_exact = NULL;
    const EngineResolvedCall *best_tail_legacy = NULL;
    ...
    for (int i = 0; i < arr->count; i++) {
        ...
        bool caller_matches =
            call_tail && engine_pipeline_qn_class_method_tail_eq(rc->caller_qn, call_tail);
        if (!caller_matches && site_rank == 2) {
            const char *resolved_caller = engine_lsp_bare_segment(rc->caller_qn);
            const char *source_caller = engine_lsp_bare_segment(call->enclosing_func_qn);
            caller_matches =
                resolved_caller && source_caller && strcmp(resolved_caller, source_caller) == 0;
        }
        if (!caller_matches || !engine_pipeline_invocation_leaf_matches(rc, call, site_rank)) {
            continue;
        }

        const EngineResolvedCall **best = site_rank == 2 ? &best_tail_exact : &best_tail_legacy;
        bool *ambiguous = site_rank == 2 ? &tail_exact_ambiguous : &tail_legacy_ambiguous;
        if (!*best) {
            *best = rc;
            continue;
        }
        if (!engine_pipeline_invocation_targets_equal((*best)->callee_qn, rc->callee_qn, gbuf,
...
static inline const EngineResolvedCall *engine_pipeline_find_lsp_resolution(
    const EngineResolvedCallArray *arr, const EngineCall *call, bool allow_tail_match) {
    return engine_pipeline_find_lsp_resolution_in_graph(arr, call, allow_tail_match, NULL, NULL);
}

static inline bool engine_pipeline_reference_reason_matches(const EngineResolvedCall *resolved,
    ...
    return reason_join_strategy &&
           strcmp(engine_lsp_bare_segment(resolved->reason), reference_leaf) == 0;
}

static inline const engine_gbuf_node_t *engine_pipeline_lsp_target_node(const engine_gbuf_t *gbuf,
                                                                  const char *project_name,
                                                                  const char *callee_qn,
                                                                  bool allow_tail_match) {
    return engine_pipeline_lsp_target_node_policy(gbuf, project_name, callee_qn, allow_tail_match,
                                               true);
}
...
static inline const engine_gbuf_node_t *engine_pipeline_lsp_target_node_strict(const engine_gbuf_t *gbuf,
                                                                         const char *project_name,
                                                                         const char *callee_qn,
                                                                         bool allow_tail_match) {
    (void)allow_tail_match;
    return engine_pipeline_lsp_target_node_policy(gbuf, project_name, callee_qn, false, false);
}
...
static inline bool engine_pipeline_kotlin_external_target(EngineLanguage lang, const char *callee_qn) {
    if (lang != ENGINE_LANG_KOTLIN || !callee_qn) {
        return false;
    }
...
static inline bool engine_pipeline_reference_targets_equal(const char *left_qn, const char *right_qn,
                                                        const engine_gbuf_t *gbuf,
                                                        const char *project_name,
                                                        bool allow_tail_match) {
    if (!left_qn || !right_qn) {
    ...
    (void)allow_tail_match;
    const engine_gbuf_node_t *left = engine_pipeline_lsp_target_node(gbuf, project_name, left_qn, false);
    const engine_gbuf_node_t *right =
        engine_pipeline_lsp_target_node(gbuf, project_name, right_qn, false);
    return left && right && left->id == right->id;
}

typedef struct {
    const EngineResolvedCall *row;
    int original_index;
} engine_pipeline_lsp_reference_index_entry_t;

typedef struct {
    engine_pipeline_lsp_reference_index_entry_t *entries;
    int count;
} engine_pipeline_lsp_reference_index_t;
...
static inline int engine_pipeline_lsp_reference_index_entry_cmp(const void *left_ptr,
                                                             const void *right_ptr) {
    const engine_pipeline_lsp_reference_index_entry_t *left =
        (const engine_pipeline_lsp_reference_index_entry_t *)left_ptr;
    const engine_pipeline_lsp_reference_index_entry_t *right =
        (const engine_pipeline_lsp_reference_index_entry_t *)right_ptr;
...
static inline bool engine_pipeline_lsp_reference_index_build(
    const EngineResolvedCallArray *arr, engine_pipeline_lsp_reference_index_t *index) {
    if (!index) {
        return false;
    }
    index->entries = NULL;
    ...
    for (int i = 0; i < arr->count; i++) {
        if (arr->items[i].kind == ENGINE_RESOLVED_CALL_REFERENCE) {
            reference_count++;
        }
    }
    ...
    if ((size_t)reference_count > SIZE_MAX / sizeof(engine_pipeline_lsp_reference_index_entry_t)) {
        return false;
    }

    engine_pipeline_lsp_reference_index_entry_t *entries =
        (engine_pipeline_lsp_reference_index_entry_t *)malloc((size_t)reference_count *
                                                           sizeof(*entries));
    if (!entries) {
        return false;
    }
    ...
    qsort(entries, (size_t)reference_count, sizeof(*entries),
          engine_pipeline_lsp_reference_index_entry_cmp);
    index->entries = entries;
    index->count = reference_count;
    return true;
}

static inline void engine_pipeline_lsp_reference_index_free(
    engine_pipeline_lsp_reference_index_t *index) {
    if (!index) {
        return;
    }
    free(index->entries);
...
static inline int engine_pipeline_lsp_reference_site_cmp_usage(const EngineResolvedCall *row,
...
static inline const EngineResolvedCall *engine_pipeline_lsp_reference_view_row(
    const EngineResolvedCallArray *arr, const engine_pipeline_lsp_reference_index_entry_t *entries,
...
static inline const EngineResolvedCall *engine_pipeline_find_lsp_reference_view_in_graph(
    const EngineResolvedCallArray *arr, const engine_pipeline_lsp_reference_index_entry_t *entries,
    int first, int row_count, const EngineUsage *usage, bool allow_tail_match, const engine_gbuf_t *gbuf,
    const char *project_name) {
    ...
    const char *ref_leaf = engine_lsp_bare_segment(usage->ref_name);
    if (!ref_leaf || !ref_leaf[0]) {
        return NULL;
    }

    const EngineResolvedCall *best_exact = NULL;
    bool exact_ambiguous = false;
    for (int i = 0; i < row_count; i++) {
        engine_pipeline_lsp_reference_lookup_test_note_row();
        const EngineResolvedCall *rc = engine_pipeline_lsp_reference_view_row(arr, entries, first, i);
        if (rc->kind != ENGINE_RESOLVED_CALL_REFERENCE || !rc->caller_qn || !rc->callee_qn ||
            rc->confidence < ENGINE_LSP_CONFIDENCE_FLOOR) {
            continue;
        }
        if (strcmp(rc->caller_qn, usage->enclosing_func_qn) != 0 ||
            !engine_pipeline_source_occurrence_eq(usage->site_start_byte, usage->site_end_byte,
                                               usage->source_origin, rc->site_start_byte,
                                               rc->site_end_byte, rc->source_origin)) {
            continue;
        }
        bool name_matches = strcmp(engine_lsp_bare_segment(rc->callee_qn), ref_leaf) == 0;
        bool semantic_reason_matches = engine_pipeline_reference_reason_matches(rc, ref_leaf);
        ...
        if (!engine_pipeline_reference_targets_equal(best_exact->callee_qn, rc->callee_qn, gbuf,
                                                  project_name, allow_tail_match)) {
    ...
    const char *caller_tail = engine_pipeline_qn_class_method_tail(usage->enclosing_func_qn);
    if (!caller_tail) {
        return NULL;
    }

    const EngineResolvedCall *best_tail = NULL;
    ...
    for (int i = 0; i < row_count; i++) {
        ...
        if (!engine_pipeline_qn_class_method_tail_eq(rc->caller_qn, caller_tail) ||
            !engine_pipeline_source_occurrence_eq(usage->site_start_byte, usage->site_end_byte,
                                               usage->source_origin, rc->site_start_byte,
                                               rc->site_end_byte, rc->source_origin)) {
            continue;
        ...
        if (!engine_pipeline_reference_targets_equal(best_tail->callee_qn, rc->callee_qn, gbuf,
                                                  project_name, allow_tail_match)) {
            tail_ambiguous = true;
            continue;
        }
...
static inline const EngineResolvedCall *engine_pipeline_find_lsp_reference_in_graph(
    const EngineResolvedCallArray *arr, const EngineUsage *usage, bool allow_tail_match,
    const engine_gbuf_t *gbuf, const char *project_name) {
    int count = arr ? arr->count : 0;
    return engine_pipeline_find_lsp_reference_view_in_graph(arr, NULL, 0, count, usage,
...
static inline const EngineResolvedCall *engine_pipeline_find_lsp_reference_indexed_in_graph(
    const EngineResolvedCallArray *arr, const engine_pipeline_lsp_reference_index_t *index,
    const EngineUsage *usage, bool allow_tail_match, const engine_gbuf_t *gbuf,
    const char *project_name) {
    if (!index) {
        return engine_pipeline_find_lsp_reference_in_graph(arr, usage, allow_tail_match, gbuf,
                                                        project_name);
    }
    ...
    while (low < high) {
        ...
        if (engine_pipeline_lsp_reference_site_cmp_usage(index->entries[middle].row, usage) < 0) {
            low = middle + 1;
        } else {
            high = middle;
        }
    ...
    return engine_pipeline_find_lsp_reference_view_in_graph(
...
static inline const EngineResolvedCall *engine_pipeline_find_lsp_reference(
    const EngineResolvedCallArray *arr, const EngineUsage *usage, bool allow_tail_match) {
    return engine_pipeline_find_lsp_reference_in_graph(arr, usage, allow_tail_match, NULL, NULL);
}
 ...
 *      engine_pipeline_lsp_allow_tail_match), use the exact node-name index
...
static inline const engine_gbuf_node_t *engine_pipeline_lsp_target_node_policy(
    ...
    const engine_gbuf_node_t *direct = engine_gbuf_find_by_qn(gbuf, callee_qn);
    if (direct) {
        return direct;
    }
        ...
        if (!(strncmp(callee_qn, project_name, proj_len) == 0 && callee_qn[proj_len] == '.')) {
            ...
            if (proj_len <= SIZE_MAX - callee_len - 2U) {
                ...
                if (prefixed_qn) {
                    ...
                    const engine_gbuf_node_t *prefixed = engine_gbuf_find_by_qn(gbuf, prefixed_qn);
                    free(prefixed_qn);
                    if (prefixed) {
                        return prefixed;
                    }
    ...
    const char *callee_tail = engine_pipeline_qn_class_method_tail(callee_qn);
    if (!callee_tail) {
        return NULL;
    }
    const engine_gbuf_node_t **hits = NULL;
    int hit_count = 0;
    if (engine_gbuf_find_by_name(gbuf, short_name, &hits, &hit_count) != 0 || hit_count == 0) {
        return NULL;
    }

    const engine_gbuf_node_t *match = NULL;
    const engine_gbuf_node_t *unique_callable = NULL;
    bool tail_ambiguous = false;
    bool callable_ambiguous = false;
    for (int i = 0; i < hit_count; i++) {
        const engine_gbuf_node_t *cand = hits[i];
        ...
        if (!engine_pipeline_qn_class_method_tail_eq(cand->qualified_name, callee_tail)) {
            continue;
        }
...
#endif /* ENGINE_PIPELINE_LSP_RESOLVE_H */
