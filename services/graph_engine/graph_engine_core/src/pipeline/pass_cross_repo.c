...
#include "pipeline/pipeline_internal.h" // engine_route_canon_path
...
static ENGINE_TLS engine_cross_repo_after_insert_test_hook_t cr_after_insert_test_hook = NULL;
static ENGINE_TLS void *cr_after_insert_test_context = NULL;
...
static ENGINE_TLS char cr_ibuf[ENGINE_SZ_32];
...
static bool cr_store_has_exact_project(engine_store_t *store, const char *project) {
    engine_project_t *projects = NULL;
    int count = 0;
    bool matches = store && engine_store_check_integrity(store) &&
                   engine_store_list_projects(store, &projects, &count) == ENGINE_STORE_OK &&
                   count == 1 && projects[0].name && strcmp(projects[0].name, project) == 0;
    engine_store_free_projects(projects, count);
    return matches;
}
...
static bool cr_project_exists(const char *project) {
    ...
    engine_store_t *store = engine_store_open_path_query(path);
    bool exists = cr_store_has_exact_project(store, project);
    engine_store_close(store);
    return exists;
}

static engine_store_t *cr_open_existing_project(const char *project) {
    char path[CR_PATH_BUF];
    if (!cr_db_path(project, path, sizeof(path))) {
        return NULL;
    }
    engine_store_t *store = engine_store_open_path_existing(path);
...
static cr_run_status_t delete_cross_edges(engine_store_t *store, const char *project,
    ...
    for (size_t i = 0; i < sizeof(edge_types) / sizeof(edge_types[0]); i++) {
        ...
        if (engine_store_delete_edges_by_type(store, project, edge_types[i]) != ENGINE_STORE_OK) {
            return CR_RUN_FAILED;
        }
 ...
 * table is UNIQUE(source_id, target_id, type) and engine_store_insert_edge
 * upserts on conflict, so a pair reached from both match directions or on a
 * repeat run never duplicates a row. (#523) */
static bool insert_cross_edge(engine_store_t *store, const char *project, int64_t from_id,
    ...
    bool inserted = engine_store_insert_edge(store, &edge) > 0;
    if (inserted) {
        ctx->mutated = true;
 ...
 * engine_route_canon_path only canonicalizes placeholder syntax, never strips
...
static bool lookup_node_info(struct sqlite3 *db, int64_t node_id, char *name_out, size_t name_sz,
    ...
    if (sqlite3_prepare_v2(db, "SELECT name, file_path FROM nodes WHERE id = ?1", ENGINE_NOT_FOUND,
                           &st, NULL) != SQLITE_OK) {
        return false;
    }
...
static int64_t find_route_handler(engine_store_t *target_store, const char *route_qn,
                                  char *handler_name, size_t name_sz, char *handler_file,
                                  size_t file_sz, bool *failed) {
    *failed = false;
    handler_name[0] = '\0';
    handler_file[0] = '\0';
    struct sqlite3 *db = engine_store_get_db(target_store);
    if (!db) {
        *failed = true;
        return 0;
    }
    ...
    if (sqlite3_prepare_v2(
            ...
            ENGINE_NOT_FOUND, &s, NULL) != SQLITE_OK) {
        *failed = true;
        return 0;
    }
    if (sqlite3_bind_text(s, SKIP_ONE, route_qn, ENGINE_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK) {
        sqlite3_finalize(s);
        *failed = true;
        return 0;
    }
...
static int64_t find_route_handler_fuzzy(engine_store_t *target_store, const char *concrete_path,
...
static bool emit_cross_route_bidirectional(
    engine_store_t *src_store, const char *src_project, struct sqlite3 *src_db, int64_t caller_id,
    int64_t local_route_id, engine_store_t *tgt_store, const char *tgt_project, int64_t handler_id,
    ...
    struct sqlite3 *tgt_db = engine_store_get_db(tgt_store);
    if (!tgt_db) {
        return false;
    }
    sqlite3_stmt *rq = NULL;
    if (sqlite3_prepare_v2(tgt_db, "SELECT id FROM nodes WHERE qualified_name = ?1 LIMIT 1",
                           ENGINE_NOT_FOUND, &rq, NULL) != SQLITE_OK) {
        return false;
    }
    if (sqlite3_bind_text(rq, SKIP_ONE, route_qn, ENGINE_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK) {
        sqlite3_finalize(rq);
        return false;
    }
    ...
    char caller_name[ENGINE_SZ_256] = {0};
    char caller_file[ENGINE_SZ_512] = {0};
...
static cr_match_result_t match_http_routes(engine_store_t *src_store, const char *src_project,
                                           engine_store_t *tgt_store, const char *tgt_project,
                                           cr_run_context_t *ctx) {
    struct sqlite3 *src_db = engine_store_get_db(src_store);
    if (!src_db) {
    ...
    if (sqlite3_bind_text(s, SKIP_ONE, src_project, ENGINE_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK) {
        sqlite3_finalize(s);
        return cr_match_finish(ctx, 0, true);
    }
    ...
    while (scanned < CR_MAX_EDGES && !cr_cancel_requested(ctx) &&
        ...
        char url_path[ENGINE_SZ_256] = {0};
        char method[ENGINE_SZ_32] = {0};
        ...
        const char *curl = engine_route_canon_path(cr_url_path(url_path), cpath, sizeof(cpath));
        snprintf(route_qn, sizeof(route_qn), "__route__%s__%s", method[0] ? method : "ANY", curl);

        char handler_name[ENGINE_SZ_256] = {0};
        char handler_file[ENGINE_SZ_512] = {0};
...
static cr_match_result_t match_async_routes(engine_store_t *src_store, const char *src_project,
                                            engine_store_t *tgt_store, const char *tgt_project,
                                            cr_run_context_t *ctx) {
    struct sqlite3 *src_db = engine_store_get_db(src_store);
    if (!src_db) {
 ...
 * for a complete bidirectional pair, 0 for no match, and ENGINE_STORE_ERR when a
 * query or write fails. */
static int try_match_channel_listener(engine_store_t *src_store, const char *src_project,
                                      engine_store_t *tgt_store, const char *tgt_project,
    ...
    if (sqlite3_prepare_v2(tgt_db,
                           ...
                           ENGINE_NOT_FOUND, &tq, NULL) != SQLITE_OK) {
        return ENGINE_STORE_ERR;
    }
    if (sqlite3_bind_text(tq, SKIP_ONE, tgt_project, ENGINE_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK ||
        sqlite3_bind_text(tq, PAIR_LEN, channel_name, ENGINE_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK) {
        sqlite3_finalize(tq);
        return ENGINE_STORE_ERR;
    }
    ...
    char listener_name[ENGINE_SZ_256] = {0};
    char listener_file[ENGINE_SZ_512] = {0};
    ...
    char caller_name[ENGINE_SZ_256] = {0};
    char caller_file[ENGINE_SZ_512] = {0};
    if (!lookup_node_info(engine_store_get_db(src_store), emitter_id, caller_name, sizeof(caller_name),
    ...
    if (!insert_cross_edge(src_store, src_project, emitter_id, channel_id, "CROSS_CHANNEL", fwd,
                           ctx)) {
        return cr_cancel_requested(ctx) ? ENGINE_STORE_NOT_FOUND : ENGINE_STORE_ERR;
    }
    if (cr_cancel_requested(ctx)) {
        return ENGINE_STORE_NOT_FOUND;
    }
    ...
    return insert_cross_edge(tgt_store, tgt_project, listener_id, tgt_channel_id, "CROSS_CHANNEL",
                             rev, ctx)
               ? 1
               : (cr_cancel_requested(ctx) ? ENGINE_STORE_NOT_FOUND : ENGINE_STORE_ERR);
}

static cr_match_result_t match_channels(engine_store_t *src_store, const char *src_project,
                                        engine_store_t *tgt_store, const char *tgt_project,
                                        cr_run_context_t *ctx) {
    struct sqlite3 *src_db = engine_store_get_db(src_store);
    if (!src_db) {
    ...
    while (scanned < CR_MAX_EDGES && !cr_cancel_requested(ctx) &&
        ...
        char *channel_name_copy = engine_strdup(channel_name);
        if (!channel_name_copy) {
            failed = true;
            break;
        }
        char transport[ENGINE_SZ_64] = {0};
        ...
        if (matched == ENGINE_STORE_ERR) {
            failed = true;
            break;
        }
        if (matched == ENGINE_STORE_NOT_FOUND && cr_cancel_requested(ctx)) {
            break;
        }
        if (matched > 0) {
            count++;
...
static bool lookup_node_qn(struct sqlite3 *db, int64_t node_id, char *out, size_t out_sz,
    ...
    *failed = false;
    ...
    sqlite3_stmt *st = NULL;
    if (sqlite3_prepare_v2(db, "SELECT qualified_name FROM nodes WHERE id = ?1", ENGINE_NOT_FOUND, &st,
                           NULL) != SQLITE_OK) {
        *failed = true;
        return false;
    }
...
static cr_match_result_t match_typed_routes(engine_store_t *src_store, const char *src_project,
                                            engine_store_t *tgt_store, const char *tgt_project,
    ...
    sqlite3_stmt *s = NULL;
    if (sqlite3_prepare_v2(src_db, sql, ENGINE_NOT_FOUND, &s, NULL) != SQLITE_OK) {
        return cr_match_finish(ctx, 0, true);
    }
    if (sqlite3_bind_text(s, SKIP_ONE, src_project, ENGINE_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK) {
        sqlite3_finalize(s);
    ...
    while (scanned < CR_MAX_EDGES && !cr_cancel_requested(ctx) &&
        ...
        char svc_val[ENGINE_SZ_256] = {0};
        char meth_val[ENGINE_SZ_256] = {0};
...
static int collect_all_projects(char ***out, cr_run_context_t *ctx) {
    *out = NULL;
    ...
    while ((ent = engine_readdir(d)) != NULL) {
        ...
        char project[ENGINE_DIRENT_NAME_MAX];
        ...
        if (!engine_validate_project_name(project) || !cr_project_exists(project)) {
            continue;
        }
        if (count >= cap) {
            cap *= PAIR_LEN;
        ...
        projects[count] = engine_strdup(project);
        if (!projects[count]) {
            failed = true;
            break;
        }
...
static int collect_named_projects(const char **targets, int target_count, char ***out,
    ...
    *out = NULL;
    ...
    for (int i = 0; i < target_count; i++) {
        ...
        if (!targets[i] || strcmp(targets[i], "*") == 0 || !engine_validate_project_name(targets[i])) {
            free_project_list(projects, count);
            return -1;
        }
        projects[count] = engine_strdup(targets[i]);
        if (!projects[count]) {
            free_project_list(projects, count);
            return -1;
        }
...
engine_cross_repo_result_t engine_cross_repo_match_cancellable(const char *project,
                                                         const char **target_projects,
                                                         int target_count,
                                                         const atomic_int *cancelled) {
    engine_cross_repo_result_t result = {0};
    cr_run_context_t run = {.cancelled = cancelled};
    struct timespec t0;
    clock_gettime(CLOCK_MONOTONIC, &t0);

    if (!project || !engine_validate_project_name(project) || !target_projects || target_count <= 0 ||
    ...
    engine_store_t *src_store = cr_open_existing_project(project);
    if (!src_store) {
        result.failed = true;
        free_project_list(resolved, resolved_count);
        return result;
    ...
    for (int i = 0; i < resolved_count; i++) {
        ...
        engine_store_t *tgt_store = cr_open_existing_project(tgt);
        if (!tgt_store) {
            result.failed = true;
            break;
        }
    ...
    if (result.cancelled) {
        engine_log_info("cross_repo.cancelled", "project", project, "partial_results",
        ...
        engine_log_info("cross_repo.done", "project", project, "total", cr_itoa(total));
    }

    return result;
}

engine_cross_repo_result_t engine_cross_repo_match(const char *project, const char **target_projects,
                                             int target_count) {
    return engine_cross_repo_match_cancellable(project, target_projects, target_count, NULL);
}
