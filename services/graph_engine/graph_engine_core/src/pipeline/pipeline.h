...
#include "discover/discover.h"    /* engine_ignored_file_t (#963) */
#include "foundation/constants.h" /* ENGINE_SZ_512 */

/* Forward declarations */
typedef struct engine_store engine_store_t;
typedef struct engine_gbuf engine_gbuf_t;

/* ── Opaque handle ──────────────────────────────────────────────── */

typedef struct engine_pipeline engine_pipeline_t;
...
typedef enum {
    ...
    ENGINE_MODE_FULL = 0,     /* Full: everything including SIMILAR_TO + SEMANTICALLY_RELATED */
    ENGINE_MODE_MODERATE = 1, /* Moderate: fast discovery + SIMILAR_TO + SEMANTICALLY_RELATED */
    ENGINE_MODE_FAST = 2,     /* Fast: skip non-essential files, no similarity/semantic edges */
} engine_index_mode_t;
#endif
...
engine_pipeline_t *engine_pipeline_new(const char *repo_path, const char *db_path, engine_index_mode_t mode);

/* Enable persistent artifact export (.graph-engine/graph.db.zst).
 ...
 * existing database in place. Those codes (ENGINE_PIPELINE_ABORT_PRESERVE_DB and
 * ENGINE_PIPELINE_PERSIST_FAILED) are defined in pipeline_internal.h alongside the
 ...
 * owned by the pipeline. Valid until engine_pipeline_free(). */
...
/* Get the index mode (ENGINE_MODE_FULL, ENGINE_MODE_MODERATE, ENGINE_MODE_FAST). */
 ...
 * valid until engine_pipeline_free()); *count receives its length. Both are set
 ...
 * the pipeline (copied on record, freed in engine_pipeline_free). A skip is the
 ...
 * engine_pipeline_free()). out and count are set to NULL and 0 when p is NULL or
...
char *engine_pipeline_fqn_compute(const char *project, const char *rel_path, const char *name);

/* Module QN: project.dir.parts (no name). Caller must free(). */
char *engine_pipeline_fqn_module(const char *project, const char *rel_path);
 ...
 * it is exactly engine_pipeline_fqn_module(). Caller must free(). */
char *engine_pipeline_fqn_module_dir(const char *project, const char *rel_path, bool module_is_dir);

/* Folder QN: project.dir.parts. Caller must free(). */
char *engine_pipeline_fqn_folder(const char *project, const char *rel_dir);
 ...
 * (e.g. "src/api/helpers") suitable for passing to engine_pipeline_fqn_module,
...
char *engine_pipeline_resolve_relative_import(const char *source_rel, const char *module_path);

/* Derive project name from an absolute path.
 * Replaces / and : with -, collapses --, trims leading -.
 * Caller must free() the returned string. */
char *engine_project_name_from_path(const char *abs_path);

/* ── Function Registry ──────────────────────────────────────────── */

typedef struct engine_registry engine_registry_t;
...
/* Per-file full-result cache for engine_registry_resolve. The same
...
typedef struct {
    engine_resolution_t result;
    bool ok;
} engine_fuzzy_result_t;
...
int engine_parse_hunks(const char *output, engine_changed_hunk_t *out, int max_out);
