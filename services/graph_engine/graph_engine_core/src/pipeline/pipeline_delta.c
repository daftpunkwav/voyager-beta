 ...
 * nodes, via the same engine_camel_split SQL function the wholesale rebuild
 * uses.
 */
...
int engine_delta_stage_clone(const char *final_db_path, char **out_stage_path) {
    *out_stage_path = NULL;
    char *stage = engine_pipeline_create_staging_path(final_db_path);
    if (!stage) {
        return ENGINE_NOT_FOUND;
    }
    if (engine_clone_or_copy_file(final_db_path, stage) != 0) {
...
int engine_delta_snapshot_inbound(engine_store_t *store, const char *project, const char *const *paths,
    ...
    *out_count = 0;
    ...
    engine_delta_saved_edge_t *items = NULL;
    int count = 0;
    int cap = 0;
    for (int off = 0; off < path_count; off += DELTA_IN_CHUNK) {
        ...
        if (n < 0 || (size_t)n >= sizeof(sql)) {
            engine_delta_free_snapshot(items, count);
            return ENGINE_NOT_FOUND;
        }
        ...
        sqlite3_bind_text(stmt, 1, project, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
        for (int i = 0; i < chunk; i++) {
            sqlite3_bind_text(stmt, 2 + i, paths[off + i], ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
            sqlite3_bind_text(stmt, 2 + chunk + i, paths[off + i], ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
        }
        int step_rc;
        while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            if (count >= cap) {
                ...
                engine_delta_saved_edge_t *grown = realloc(items, (size_t)ncap * sizeof(*items));
                if (!grown) {
            ...
            engine_delta_saved_edge_t *e = &items[count];
...
int64_t engine_delta_preseed(engine_store_t *store, const char *project, engine_gbuf_t *gbuf) {
    ...
    int64_t max_id = 0;
    {
        ...
        if (sqlite3_prepare_v2(db, "SELECT COALESCE(MAX(id), 0) FROM nodes", ENGINE_NOT_FOUND, &stmt,
                               NULL) != SQLITE_OK) {
            return -1;
        }
     ...
     * symbols (engine_label_is_type_like + Function/Method/Variable/Field, the
    ...
    while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        engine_gbuf_set_next_id(gbuf, id);
        int64_t got = engine_gbuf_upsert_node(gbuf, label, name, qn, fp ? fp : "", 0, 0, "{}");
        if (got != id) {
            /* A QN collision inside the preseed set would silently split
             * identity between RAM and disk; the run cannot be trusted. */
            sqlite3_finalize(stmt);
            engine_log_error("delta.preseed_id_mismatch", "qn", qn ? qn : "");
            return -1;
        }
        seeded++;
    }
    ...
    engine_gbuf_set_next_id(gbuf, max_id + 1);
    char seeded_buf[32];
    snprintf(seeded_buf, sizeof(seeded_buf), "%lld", (long long)seeded);
    engine_log_info("delta.preseed", "proxies", seeded_buf);
    return max_id;
}
...
typedef struct {
     ...
     * search -- EngineHashTable stores key POINTERS without copying,
...
static void delta_patch_node(const engine_gbuf_node_t *node, void *userdata) {
    ...
    if (node->qualified_name) {
        ...
        sqlite3_bind_text(ctx->qn_lookup, 1, ctx->project, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
        sqlite3_bind_text(ctx->qn_lookup, 2, node->qualified_name, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
    ...
    sqlite3_bind_text(ctx->node_stmt, 2, ctx->project, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
    sqlite3_bind_text(ctx->node_stmt, 3, node->label, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
    sqlite3_bind_text(ctx->node_stmt, 4, node->name, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
    sqlite3_bind_text(ctx->node_stmt, 5, node->qualified_name, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
    sqlite3_bind_text(ctx->node_stmt, 6, node->file_path ? node->file_path : "", ENGINE_NOT_FOUND,
                      SQLITE_TRANSIENT);
...
static void delta_patch_edge(const engine_gbuf_edge_t *edge, void *userdata) {
    delta_patch_ctx_t *ctx = (delta_patch_ctx_t *)userdata;
    if (ctx->failed) {
        return;
    }
    ...
    sqlite3_bind_text(ctx->edge_stmt, 1, ctx->project, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
    sqlite3_bind_int64(ctx->edge_stmt, 2, src);
    sqlite3_bind_int64(ctx->edge_stmt, 3, tgt);
    sqlite3_bind_text(ctx->edge_stmt, 4, edge->type, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
    sqlite3_bind_text(ctx->edge_stmt, 5, edge->properties_json ? edge->properties_json : "{}",
                      ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
    if (sqlite3_step(ctx->edge_stmt) != SQLITE_DONE) {
        ctx->failed = true;
        engine_log_error("delta.patch_edge_failed", "type", edge->type ? edge->type : "");
        return;
    }
    ctx->edges++;
}
...
int engine_delta_patch(engine_store_t *store, const char *project, engine_gbuf_t *gbuf, int64_t max_db_id,
    ...
    if (engine_store_begin(store) != ENGINE_STORE_OK) {
        return ENGINE_NOT_FOUND;
    }
    ...
    if (sqlite3_prepare_v2(db,
                           ...
                           ENGINE_NOT_FOUND, &ctx.qn_lookup, NULL) != SQLITE_OK) {
        engine_store_rollback(store);
        return ENGINE_NOT_FOUND;
    }
    if (sqlite3_prepare_v2(db,
    ...
    engine_gbuf_foreach_node(gbuf, delta_patch_node, &ctx);
    if (!ctx.failed) {
        engine_gbuf_foreach_edge(gbuf, delta_patch_edge, &ctx);
    }
    sqlite3_finalize(ctx.node_stmt);
    sqlite3_finalize(ctx.edge_stmt);
    sqlite3_finalize(ctx.qn_lookup);
    ...
    if (!ctx.failed && snapshot_count > 0) {
        ...
        if (sqlite3_prepare_v2(db,
                               ...
                               ENGINE_NOT_FOUND, &relink, NULL) != SQLITE_OK) {
            ctx.failed = true;
        } else {
            for (int i = 0; i < snapshot_count && !ctx.failed; i++) {
                sqlite3_reset(relink);
                sqlite3_bind_text(relink, 1, project, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
                sqlite3_bind_text(relink, 2, snapshot[i].type, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
                sqlite3_bind_text(relink, 3, snapshot[i].props, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
                sqlite3_bind_text(relink, 4, snapshot[i].source_qn, ENGINE_NOT_FOUND,
                                  SQLITE_TRANSIENT);
                sqlite3_bind_text(relink, 5, snapshot[i].target_qn, ENGINE_NOT_FOUND,
                                  SQLITE_TRANSIENT);
    ...
    if (!ctx.failed) {
        ...
        if (sqlite3_prepare_v2(db,
                               ...
                               " file_path)"
                               " SELECT id, engine_camel_split(name), qualified_name, label,"
                               " file_path FROM nodes WHERE project = ?1 AND id > ?2",
                               ENGINE_NOT_FOUND, &fts, NULL) == SQLITE_OK) {
            sqlite3_bind_text(fts, 1, project, ENGINE_NOT_FOUND, SQLITE_TRANSIENT);
            ...
            engine_log_warn("delta.fts_insert_unavailable", "project", project);
        }
    }
    ...
    free(ctx.remap);
    if (engine_store_commit(store) != ENGINE_STORE_OK) {
        return ENGINE_NOT_FOUND;
    }
    char nodes_buf[32];
    char edges_buf[32];
    ...
    engine_log_info("delta.patch", "nodes", nodes_buf, "edges", edges_buf, "remapped", remap_buf);
    return 0;
}
