...
#define ENGINE_WORKER_STACK_SIZE ((size_t)8 * ENGINE_SZ_1K * ENGINE_SZ_1K)

/* ── Serial fallback ─────────────────────────────────────────────── */

static void run_serial(int count, engine_parallel_fn fn, void *ctx) {
...
static void run_pthreads(int count, engine_parallel_fn fn, void *ctx, int nworkers) {
    ...
    engine_thread_t *threads = (engine_thread_t *)malloc((size_t)nworkers * sizeof(engine_thread_t));
    if (!threads) {
    ...
    for (int i = 0; i < nworkers; i++) {
        if (engine_thread_create(&threads[i], ENGINE_WORKER_STACK_SIZE, pthread_worker, &wa) != 0) {
...
void engine_parallel_for(int count, engine_parallel_fn fn, void *ctx, engine_parallel_for_opts_t opts) {
    if (count <= 0 || !fn) {
        return;
    }
    ...
    if (nworkers <= 0) {
        nworkers = engine_default_worker_count(true);
    }
    if (nworkers < WP_MIN) {
        nworkers = SKIP_ONE;
    }
