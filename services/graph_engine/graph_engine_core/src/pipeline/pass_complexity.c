...
enum { ENGINE_TLD_MAX_DEPTH = 256 }; /* recursion-depth cap (cycle/stack guard) */
...
static int json_get_int(const char *json, const char *key, int dflt) {
    ...
    return (int)strtol(p, NULL, ENGINE_DECIMAL_BASE);
}
...
static void append_complexity_props(engine_gbuf_node_t *node, int tld, bool recursive) {
    ...
    bool empty = (olen == 2); /* "{}" */
    char *neu = malloc(olen + ENGINE_SZ_64);
    if (!neu) {
        return;
    }
    memcpy(neu, old, olen - 1); /* copy without trailing '}' */
    int w =
        snprintf(neu + (olen - 1), ENGINE_SZ_64, "%s\"transitive_loop_depth\":%d,\"recursive\":%s}",
...
static int tld_dfs(const engine_gbuf_t *gb, int64_t id, const int *loop_depth, int *tld, char *state,
    ...
    int ne = 0;
    engine_gbuf_find_edges_by_source_type(gb, id, "CALLS", &edges, &ne);
    for (int i = 0; i < ne; i++) {
...
static void seed_loop_depths(const engine_gbuf_t *gb, const char *label, int *loop_depth,
                             bool *recursive, engine_gbuf_node_t **nptr, int64_t maxid) {
    const engine_gbuf_node_t **nodes = NULL;
    int count = 0;
    if (engine_gbuf_find_by_label(gb, label, &nodes, &count) != 0) {
        return;
    }
    for (int i = 0; i < count; i++) {
        ...
        if (n->id >= 1 && n->id <= maxid) {
            ...
            nptr[n->id] = (engine_gbuf_node_t *)n;
        }
    }
}
...
void engine_pipeline_pass_complexity(engine_pipeline_ctx_t *ctx) {
    ...
    int64_t maxid = engine_gbuf_next_id(gb) - 1;
    if (maxid < 1) {
        return;
    }
    ...
    engine_gbuf_node_t **nptr = calloc(sz, sizeof(engine_gbuf_node_t *));
    ...
    engine_log_info("pass.complexity", "functions", itoa_cx(updated));
