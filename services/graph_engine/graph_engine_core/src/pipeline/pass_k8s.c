 ...
 * supported)
 *
 * Depends on: pass_infrascan.c (engine_is_kustomize_file, engine_is_k8s_manifest, engine_infra_qn),
 *             extraction layer (engine.h), graph_buffer, pipeline internals.
 */
...
static void handle_kustomize(engine_pipeline_ctx_t *ctx, const char *path, const char *rel_path,
                             EngineFileResult *result) {
    /* Emit Module node for this kustomize overlay file */
    char *mod_qn = engine_infra_qn(ctx->project_name, rel_path, "kustomize", NULL);
    if (!mod_qn) {
        return;
    }

    int64_t mod_id = engine_gbuf_upsert_node(ctx->gbuf, "Module", k8s_basename(rel_path), mod_qn,
    ...
    if (!res) {
        ...
        if (source) {
            res = engine_extract_file(source, src_len, ENGINE_LANG_KUSTOMIZE, ctx->project_name, rel_path,
                                   ENGINE_EXTRACT_BUDGET, NULL, NULL);
            free(source);
            allocated = true;
        }
    }
    ...
    if (res) {
        for (int j = 0; j < res->imports.count; j++) {
            EngineImport *imp = &res->imports.items[j];
            if (!imp->module_path) {
                continue;
            }
            ...
            char *target_qn =
                engine_pipeline_fqn_compute(ctx->project_name, imp->module_path, "__file__");
            if (!target_qn) {
                continue;
            }
            ...
            if (target) {
                engine_gbuf_insert_edge(ctx->gbuf, mod_id, target->id, "IMPORTS",
                                     "{\"via\":\"kustomize\"}");
                import_count++;
            }
        }
        ...
        if (allocated) {
            engine_free_result(res);
        }
    }

    engine_log_info("pass.k8s.kustomize", "file", rel_path, "imports", itoa_k8s(import_count));
}
...
static void k8s_link_selectors(engine_pipeline_ctx_t *ctx, const k8s_record_array_t *recs) {
    int edges = 0;
        ...
        for (int j = 0; j < recs->count; j++) {
            ...
            if (k8s_selector_matches(svc, wl)) {
                ...
                snprintf(props, sizeof(props),
                         ...
                         svc->name, wl->name);
                engine_gbuf_insert_edge(ctx->gbuf, svc->node_id, wl->node_id, "INFRA_MAPS", props);
                edges++;
            }
        }
    }
    if (edges > 0) {
        engine_log_info("pass.k8s.selectors", "linked", itoa_k8s(edges));
    }
}
...
static void handle_k8s_manifest(engine_pipeline_ctx_t *ctx, const char *path, const char *rel_path,
                                const char *source, int src_len, k8s_record_t *rec) {
    (void)path; /* retained for symmetry; source is always provided now */
    int resource_count = 0;

    EngineFileResult *res = engine_extract_file(source, src_len, ENGINE_LANG_K8S, ctx->project_name,
                                          rel_path, ENGINE_EXTRACT_BUDGET, NULL, NULL);
    if (!res) {
        return;
    }
    ...
    const engine_gbuf_node_t *file_node = file_qn ? engine_gbuf_find_by_qn(ctx->gbuf, file_qn) : NULL;
    free(file_qn);

    for (int d = 0; d < res->defs.count; d++) {
        EngineDefinition *def = &res->defs.items[d];
        ...
        int64_t node_id =
            engine_gbuf_upsert_node(ctx->gbuf, "Resource", def->name, def->qualified_name, rel_path,
    ...
    engine_log_info("pass.k8s.manifest", "file", rel_path, "resources", itoa_k8s(resource_count));
}
...
static void handle_helm_chart(engine_pipeline_ctx_t *ctx, const char *rel_path, const char *source) {
    engine_helm_chart_t hc;
    if (engine_parse_helm_chart(source, &hc) != 0) {
        return;
    }

    const char *cname = hc.chart_name[0] ? hc.chart_name : k8s_basename(rel_path);
    char *chart_qn = engine_infra_qn(ctx->project_name, rel_path, "helm-chart", NULL);
    if (!chart_qn) {
        return;
    }
    int64_t chart_id = engine_gbuf_upsert_node(ctx->gbuf, "Chart", cname, chart_qn, rel_path, SKIP_ONE,
                                            0, "{\"source\":\"helm\"}");
    free(chart_qn);
    ...
    if (file_node && chart_id > 0) {
        engine_gbuf_insert_edge(ctx->gbuf, file_node->id, chart_id, "DEFINES", "{}");
    }
    free(file_qn);
    ...
    for (int i = 0; i < hc.dep_count && chart_id > 0; i++) {
        ...
        char dep_qn[ENGINE_SZ_512];
        snprintf(dep_qn, sizeof(dep_qn), "%s.__helm_dep__.%s", ctx->project_name, hc.deps[i]);
        int64_t dep_id =
            engine_gbuf_upsert_node(ctx->gbuf, "Chart", hc.deps[i], dep_qn, rel_path, SKIP_ONE, 0,
                                 "{\"source\":\"helm\",\"external\":true}");
        if (dep_id > 0) {
            engine_gbuf_insert_edge(ctx->gbuf, chart_id, dep_id, "DEPENDS_ON", "{}");
            dep_edges++;
        }
    }
    engine_log_info("pass.k8s.helm", "file", rel_path, "deps", itoa_k8s(dep_edges));
}
...
static int emit_dep_edge(engine_pipeline_ctx_t *ctx, const engine_gbuf_node_t *src, const char *rel_path,
    ...
    char dep_props[ENGINE_SZ_256];
    snprintf(dep_props, sizeof(dep_props), "{\"source\":\"%s\",\"external\":true}", ecosystem);
    int64_t dep_id =
        engine_gbuf_upsert_node(ctx->gbuf, "Package", name, dep_qn, rel_path, SKIP_ONE, 0, dep_props);
    if (dep_id > 0 && dep_id != src->id) {
        engine_gbuf_insert_edge(ctx->gbuf, src->id, dep_id, "DEPENDS_ON", "{}");
        return 1;
    }
    return 0;
}
...
static int parse_gomod_deps(engine_pipeline_ctx_t *ctx, const engine_gbuf_node_t *src,
...
static int parse_requirements_deps(engine_pipeline_ctx_t *ctx, const engine_gbuf_node_t *src,
                                   const char *rel_path, const char *source) {
    int edges = 0;
    const char *p = source;
    while (p && *p) {
...
static void handle_dep_manifest(engine_pipeline_ctx_t *ctx, const char *rel_path, const char *source,
                                const char *ecosystem) {
    if (!source) {
        return;
    }
    char *file_qn = engine_pipeline_fqn_compute(ctx->project_name, rel_path, "__file__");
    const engine_gbuf_node_t *src = file_qn ? engine_gbuf_find_by_qn(ctx->gbuf, file_qn) : NULL;
    free(file_qn);
    if (!src) {
        return;
    }
    ...
    engine_log_info("pass.k8s.depmanifest", "file", rel_path, "deps", itoa_k8s(dep_edges));
}

/* ── Pass entry point ────────────────────────────────────────────── */

int engine_pipeline_pass_k8s(engine_pipeline_ctx_t *ctx, const engine_file_info_t *files, int file_count) {
    engine_log_info("pass.start", "pass", "k8s", "files", itoa_k8s(file_count));
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        EngineFileResult *cached =
            (ctx->result_cache && ctx->result_cache[i]) ? ctx->result_cache[i] : NULL;

        if (is_gomod_file(base) || lang == ENGINE_LANG_GOMOD || is_requirements_file(base)) {
            int dep_len = 0;
        ...
        } else if (engine_is_kustomize_file(base)) {
            handle_kustomize(ctx, path, rel, cached);
            kustomize_count++;
        } else if (lang == ENGINE_LANG_YAML || lang == ENGINE_LANG_K8S) {
            ...
            if (source) {
                if (is_helm_chart_file(base)) {
                    ...
                    helm_count++;
                } else if (engine_is_k8s_manifest(base, source)) {
                    /* Always re-extract with ENGINE_LANG_K8S regardless of any cached
    ...
    engine_log_info("pass.done", "pass", "k8s", "kustomize", itoa_k8s(kustomize_count), "manifests",
