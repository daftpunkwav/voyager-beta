 ...
 *
 * Port of Go cmd/graph-engine/ install/update logic.
...
#include "mcp/mcp.h" // engine_mcp_tool_input_schema — CLI flag parser + per-tool --help
...
static int engine_shell_quote_word(const char *value, char *out, size_t out_size);
static int engine_powershell_quote_word(const char *value, char *out, size_t out_size);
...
static void (*engine_sqlite_transient_fn(void))(void *) {
    uintptr_t bits = (uintptr_t)CLI_ERR;
    void (*fp)(void *) = NULL;
    memcpy(&fp, &bits, sizeof(fp));
    return fp;
}
#define engine_sqlite_transient (engine_sqlite_transient_fn())
...
#define DECOMPRESS_MAX_BYTES ((size_t)500 * CLI_BUF_1K * ENGINE_SZ_1K)

bool engine_cli_mcp_result_is_error(const char *result) {
    if (!result) {
        return false;
    }
...
int engine_cli_exit_status_after_maintenance(int exit_status, bool maintenance_cancelled) {
...
static const char CLI_ACTIVATION_REFUSED_MESSAGE[] =
    "error: active engine sessions and operations could not be stopped safely; "
...
static const char CLI_ACTIVATION_MUTATION_FAILED_MESSAGE[] =
    "error: activation failed while engine sessions were stopped; filesystem "
...
typedef struct {
    engine_daemon_ipc_endpoint_t *endpoint;
    engine_version_cohort_manager_t *cohort_manager;
    engine_version_cohort_lease_t *cohort_lease;
    engine_daemon_ipc_startup_lock_t *startup_lock;
    engine_daemon_build_identity_t identity;
    char source_build[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE];
    char canonical_cache[CLI_BUF_4K];
    char cache_fingerprint[ENGINE_SHA256_HEX_LEN + 1];
    char *original_cache_environment;
    engine_daemon_runtime_activation_action_t action;
    engine_daemon_runtime_activation_result_t daemon_result;
...
static engine_cli_activation_ops_t g_cli_activation_test_ops;
static bool g_cli_activation_test_ops_set = false;
static const char *g_cli_activation_runtime_parent_for_test = NULL;

static void cli_activation_diagnostic(const engine_cli_activation_ops_t *ops, const char *message) {
     ...
     * filesystem safety check), say THAT. The generic text blames "active engine
    ...
    char attributed[ENGINE_SZ_1K];
    const char *note = engine_activation_transaction_refusal_note();
    if (diagnostic == CLI_ACTIVATION_REFUSED_MESSAGE && note && note[0]) {
        (void)snprintf(attributed, sizeof(attributed),
                       ...
                       "or use an owner-private directory for --dir/ENGINE_CACHE_DIR, then retry.",
                       note);
...
int engine_cli_activation_guard_with_ops(const engine_cli_activation_ops_t *ops,
                                      engine_cli_activation_mutation_fn mutation,
    ...
    engine_cli_activation_lock_t mutation_lease = NULL;
...
void engine_cli_set_activation_ops_for_test(const engine_cli_activation_ops_t *ops) {
    if (!ops) {
...
bool engine_cli_activation_test_ops_installed(void) {
    return g_cli_activation_test_ops_set;
}

void engine_cli_set_activation_runtime_parent_for_test(const char *runtime_parent) {
    g_cli_activation_runtime_parent_for_test = runtime_parent;
}

static const char *cli_activation_action_text(engine_daemon_runtime_activation_action_t action) {
    switch (action) {
    case ENGINE_DAEMON_RUNTIME_ACTIVATION_INSTALL:
        return "install";
    case ENGINE_DAEMON_RUNTIME_ACTIVATION_UPDATE:
        return "update";
    case ENGINE_DAEMON_RUNTIME_ACTIVATION_UNINSTALL:
        return "uninstall";
    default:
        return "activation";
    }
...
static bool cli_activation_log_event(cli_activation_production_context_t *context,
    ...
    bool encoded =
        yyjson_mut_obj_add_str(document, root, "event", "engine.activation") &&
        ...
        yyjson_mut_obj_add_str(document, root, "source_version", ENGINE_VERSION) &&
    ...
    if (written) {
        int descriptor = engine_fileno(context->activation_log);
#ifdef _WIN32
...
static int cli_activation_startup_lock_acquire(cli_activation_production_context_t *context) {
    ...
    do {
        engine_daemon_ipc_startup_lock_t *lock = NULL;
        int status = engine_daemon_ipc_startup_lock_try_acquire(context->endpoint, &lock);
        if (status == 1 && lock) {
        ...
        engine_usleep(CLI_ACTIVATION_RETRY_US);
    } while (engine_now_ms() < context->control_deadline_ms);
    return 0;
}
...
static _Noreturn void cli_activation_cleanup_fail_stop(cli_activation_production_context_t *context,
    ...
    engine_log_error("coordination.cleanup_timeout", "component", component, "action", "process_exit");
    (void)fflush(stdout);
    (void)fflush(stderr);
    _Exit(EXIT_FAILURE);
}
...
static void cli_activation_startup_lock_release_complete(
    ...
    while (context && context->startup_lock) {
        (void)engine_daemon_ipc_startup_lock_release(&context->startup_lock);
        if (!context->startup_lock) {
            return;
        }
        if (engine_now_ms() >= deadline) {
...
static void cli_activation_merge_daemon_result(
    cli_activation_production_context_t *context,
    const engine_daemon_runtime_activation_result_t *result) {
...
static engine_version_cohort_quiesce_result_t cli_activation_request_quiescence(void *opaque) {
    cli_activation_production_context_t *context = opaque;
    if (!context || !context->endpoint) {
        return ENGINE_VERSION_COHORT_QUIESCE_ERROR;
    }
    ...
    engine_daemon_runtime_activation_result_t result = {0};
    if (engine_daemon_runtime_request_activation_shutdown(
    ...
    context->shutdown_requested = true;
    return ENGINE_VERSION_COHORT_QUIESCE_REQUESTED;
}
...
static void cli_activation_release_cleanup_lease(cli_activation_production_context_t *context,
    ...
    while (lease_io && *lease_io && engine_now_ms() < cleanup_deadline) {
        if (engine_version_cohort_lease_release(lease_io) == ENGINE_PRIVATE_FILE_LOCK_OK) {
            return;
        }
        engine_usleep(CLI_ACTIVATION_RETRY_US);
    }
...
static int cli_activation_production_reserve(void *opaque, engine_cli_activation_lock_t *lease_out) {
    cli_activation_production_context_t *context = opaque;
    if (lease_out) {
        *lease_out = NULL;
    }
    ...
    engine_version_cohort_quiesce_result_t quiesce = ENGINE_VERSION_COHORT_QUIESCE_NOT_NEEDED;
    engine_version_cohort_lease_t *lease = NULL;
    ...
    engine_daemon_runtime_activation_result_t eager_result = {0};
    if (engine_daemon_runtime_request_activation_shutdown(
            context->endpoint, &context->identity, context->action,
    ...
    engine_version_cohort_status_t status = engine_version_cohort_reserve_for_mutation(
        context->cohort_manager, context->deadline_ms, cli_activation_request_quiescence, context,
        &quiesce, &lease);
    if (status != ENGINE_VERSION_COHORT_OK || !lease) {
        cli_activation_release_cleanup_lease(context, &lease);
        context->cohort_lease = lease;
        return status == ENGINE_VERSION_COHORT_BUSY ? 0 : CLI_ERR;
    }
    ...
    int generation = engine_daemon_ipc_generation_probe_under_startup_lock(context->endpoint,
...
static void cli_activation_production_release(void *opaque, engine_cli_activation_lock_t lease) {
    cli_activation_production_context_t *context = opaque;
    if (!context) {
        return;
    }
    ...
    engine_version_cohort_lease_t *cohort_lease = (engine_version_cohort_lease_t *)lease;
...
static bool cli_activation_production_context_init(cli_activation_production_context_t *context,
    ...
    context->control_deadline_ms = cli_activation_deadline_after(CLI_ACTIVATION_CONTROL_TIMEOUT_MS);
    const char *original_cache_environment = getenv("ENGINE_CACHE_DIR");
    ...
    const char *requested_cache = engine_resolve_cache_dir();
    bool cache_ready = requested_cache && requested_cache[0] &&
                       engine_canonical_path(requested_cache, context->canonical_cache,
                                          sizeof(context->canonical_cache));
    if (!cache_ready && requested_cache && requested_cache[0] &&
        engine_mkdir_p(requested_cache, 0700)) {
        cache_ready = engine_canonical_path(requested_cache, context->canonical_cache,
                                         sizeof(context->canonical_cache));
    }
    if (!cache_ready || !engine_is_dir(context->canonical_cache) ||
        !engine_daemon_ipc_private_directory_secure(context->canonical_cache)) {
        return false;
    }
    engine_normalize_path_sep(context->canonical_cache);
    if (engine_setenv("ENGINE_CACHE_DIR", context->canonical_cache, 1) != 0) {
        return false;
    }
    context->cache_environment_overridden = true;
    engine_sha256_hex(context->canonical_cache, strlen(context->canonical_cache),
                   context->cache_fingerprint);
    context->endpoint = engine_daemon_bootstrap_endpoint_new(g_cli_activation_runtime_parent_for_test);
    context->cohort_manager =
        context->endpoint ? engine_version_cohort_manager_new(context->endpoint) : NULL;
    const char *captured_build = engine_index_supervisor_build_fingerprint();
    bool build_ready = captured_build && captured_build[0]
                           ? snprintf(context->source_build, sizeof(context->source_build), "%s",
                                      captured_build) > 0
                           : engine_daemon_runtime_process_build_fingerprint(
    ...
    context->activation_log = engine_daemon_ipc_private_log_open(log_dir, "activation-events.ndjson",
...
static void cli_activation_production_context_close(cli_activation_production_context_t *context) {
    ...
    while (context->cohort_manager && engine_now_ms() < manager_deadline) {
        if (engine_version_cohort_manager_free(&context->cohort_manager) == ENGINE_PRIVATE_FILE_LOCK_OK) {
            break;
        }
        engine_usleep(CLI_ACTIVATION_RETRY_US);
    }
    ...
    engine_daemon_ipc_endpoint_free(context->endpoint);
    context->endpoint = NULL;
    if (context->cache_environment_overridden) {
        int restore_result =
            context->original_cache_environment_present
                ? engine_setenv("ENGINE_CACHE_DIR", context->original_cache_environment, 1)
                : engine_unsetenv("ENGINE_CACHE_DIR");
        if (restore_result != 0) {
            context->cleanup_ok = false;
        }
...
static int cli_activation_guard(engine_daemon_runtime_activation_action_t action,
                                const char *target_version, const char *target_build,
                                engine_cli_activation_mutation_fn mutation, void *mutation_context) {
    if (g_cli_activation_test_ops_set) {
        return engine_cli_activation_guard_with_ops(&g_cli_activation_test_ops, mutation,
                                                 mutation_context);
    }
    ...
    printf("Stopping active engine sessions and operations for %s...\n",
    ...
    engine_cli_activation_ops_t ops = {
        .context = &context,
    ...
    int rc = engine_cli_activation_guard_with_ops(&ops, mutation, mutation_context);
    if (rc == CLI_OK) {
...
#define TAR_BINARY_NAME "graph-engine"
#define TAR_BINARY_NAME_LEN 19
#define TAR_BLOCK_SIZE ENGINE_SZ_512 /* tar record alignment */
...
void engine_cli_set_version(const char *ver) {
    if (ver) {
        cli_version = ver;
    }
}

const char *engine_cli_get_version(void) {
    return cli_version;
}
...
int engine_compare_versions(const char *a, const char *b) {
    int pa[SEMVER_PARTS];
    int pb[SEMVER_PARTS];
    parse_semver(a, pa);
    parse_semver(b, pb);
...
const char *engine_detect_shell_rc(const char *home_dir) {
    ...
    const char *shell = engine_safe_getenv("SHELL", shell_buf, sizeof(shell_buf), "");
    if (!shell) {
        shell = "";
    }
...
static bool find_in_path(const char *name, char *out, size_t out_sz) {
    ...
    if (!engine_safe_getenv("PATH", path_copy, sizeof(path_copy), NULL)) {
        return false;
    }
    char *saveptr;
...
const char *engine_find_cli(const char *name, const char *home_dir) {
    static char buf[CLI_BUF_512];
    if (!name || !name[0]) {
        return "";
    }
...
static bool engine_agent_cli_exists(const char *name, const char *home_dir) {
    const char *actual_home = engine_get_home_dir();
    if (actual_home && home_dir && strcmp(actual_home, home_dir) == 0) {
        return engine_find_cli(name, home_dir)[0] != '\0';
    }
...
int engine_copy_file(const char *src, const char *dst) {
 ...
 * copying the running binary onto itself during install (engine_copy_file would
 * truncate it, since it opens the destination "wb" before reading the source). */
static bool engine_same_file(const char *a, const char *b) {
    struct stat sa;
    struct stat sb;
    ...
    engine_normalize_path_sep(na);
    engine_normalize_path_sep(nb);
...
typedef struct {
    char fingerprint[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE];
...
static bool cli_binary_fingerprint_validator(const char *target_path, void *opaque) {
    ...
    char actual[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE];
    return validator && target_path && engine_daemon_build_fingerprint_file(target_path, actual) &&
           strcmp(actual, validator->fingerprint) == 0;
}

static int cli_activation_transaction_abort(engine_activation_transaction_t **transaction_io) {
    if (!transaction_io || !*transaction_io) {
        return CLI_OK;
    }
    return engine_activation_transaction_close(transaction_io) == ENGINE_ACTIVATION_TRANSACTION_OK
               ? CLI_OK
               : CLI_ERR;
}
...
void engine_cli_set_activation_cleanup_failure_for_test(bool enabled);
int engine_cli_activation_abort_cleanup_probe_for_test(void);
#endif

static void cli_activation_transaction_abort_or_fail_stop(
    engine_activation_transaction_t **transaction_io, const char *component) {
...
int engine_cli_activation_abort_cleanup_probe_for_test(void) {
    engine_activation_transaction_t *transaction = NULL;
...
static int cli_activation_transaction_commit_validated(engine_activation_transaction_t *transaction,
                                                       const cli_binary_validator_t *validator,
                                                       int mode) {
    if (!transaction) {
        return CLI_ERR;
    }
    engine_activation_transaction_status_t status = engine_activation_transaction_commit(
        transaction, validator ? cli_binary_fingerprint_validator : NULL, (void *)validator);
    if (status != ENGINE_ACTIVATION_TRANSACTION_OK) {
        return CLI_ERR;
    }
#ifndef _WIN32
    const char *target_path = engine_activation_transaction_target_path(transaction);
    if (!target_path || chmod(target_path, (mode_t)mode) != 0) {
        (void)engine_activation_transaction_rollback(transaction);
        return CLI_ERR;
    }
#else
    (void)mode;
...
static int cli_activation_transaction_finalize_close(
    engine_activation_transaction_t **transaction_io) {
    if (!transaction_io || !*transaction_io) {
        return CLI_ERR;
    }
    engine_activation_transaction_status_t status =
        engine_activation_transaction_finalize(*transaction_io);
    if (status != ENGINE_ACTIVATION_TRANSACTION_OK && status != ENGINE_ACTIVATION_TRANSACTION_DEFERRED) {
        (void)cli_activation_transaction_abort(transaction_io);
        return CLI_ERR;
    }
    if (status == ENGINE_ACTIVATION_TRANSACTION_DEFERRED) {
        const char *deferred = engine_activation_transaction_deferred_path(*transaction_io);
        engine_log_warn("cli.activation_backup_cleanup_deferred", "path",
...
static bool cli_activation_transaction_expected_build(engine_activation_transaction_t *transaction,
                                                      cli_binary_validator_t *validator) {
    const char *staged = engine_activation_transaction_staged_path(transaction);
    return staged && validator && engine_daemon_build_fingerprint_file(staged, validator->fingerprint);
}
...
int engine_copy_binary_to_target(const char *src, const char *dst) {
    if (!src || !dst) {
        return CLI_ERR;
    }
    if (engine_same_file(src, dst)) {
        return CLI_OK;
    }
    engine_activation_transaction_t *transaction = NULL;
    engine_activation_transaction_status_t status =
        engine_activation_transaction_stage_file(dst, src, &transaction);
    cli_binary_validator_t validator;
    if (status != ENGINE_ACTIVATION_TRANSACTION_OK || !transaction ||
...
int engine_replace_binary(const char *path, const unsigned char *data, int len, int mode) {
    if (!path || !data || len <= 0) {
        return CLI_ERR;
    }
    engine_activation_transaction_t *transaction = NULL;
    engine_activation_transaction_status_t status =
        engine_activation_transaction_stage_bytes(path, data, (size_t)len, &transaction);
    cli_binary_validator_t validator;
    if (status != ENGINE_ACTIVATION_TRANSACTION_OK || !transaction ||
        !cli_activation_transaction_expected_build(transaction, &validator)) {
        (void)cli_activation_transaction_abort(&transaction);
...
static const char codex_instructions_content[] =
    ...
    "\n"
    "This project uses graph-engine to maintain a knowledge graph of the codebase.\n"
...
static const engine_skill_t skills[ENGINE_SKILL_COUNT] = {
    {"graph-engine", skill_content},
};

const engine_skill_t *engine_get_skills(void) {
    return skills;
}

const char *engine_get_codex_instructions(void) {
...
static int mkdirp(const char *path, int mode) {
    return (int)engine_mkdir_p(path, mode) ? 0 : CLI_ERR;
}
 ...
 * directory symlink; on Windows engine_rmdir removes only an empty directory or
 * the directory link itself and never traverses its target. */
static bool engine_remove_empty_directory(const char *path, bool dry_run) {
    struct stat state;
#ifndef _WIN32
    ...
    engine_dir_t *directory = engine_opendir(path);
    if (!directory) {
        return false;
    }
    bool empty = true;
    engine_dirent_t *entry;
    while ((entry = engine_readdir(directory)) != NULL) {
        if (strcmp(entry->name, ".") != 0 && strcmp(entry->name, "..") != 0) {
            empty = false;
            break;
        }
    }
    engine_closedir(directory);
    return empty && (dry_run || engine_rmdir(path) == 0);
}

/* ── Skill management ─────────────────────────────────────────── */

int engine_install_skills(const char *skills_dir, bool force, bool dry_run) {
    if (!skills_dir) {
        return 0;
    }
    int count = 0;
    ...
    for (int i = 0; i < OLD_SKILL_COUNT; i++) {
        ...
        (void)engine_remove_empty_directory(old_path, dry_run);
    }

    for (int i = 0; i < ENGINE_SKILL_COUNT; i++) {
        ...
        if (engine_text_write_owned_document(file_path, skills[i].content) == 0) {
            count++;
        }
    }
    return count;
...
int engine_remove_skills(const char *skills_dir, bool dry_run) {
    if (!skills_dir) {
        return 0;
    }
    int count = 0;
    ...
    for (int i = 0; i < ENGINE_SKILL_COUNT; i++) {
        ...
        if (engine_text_remove_owned_document(file_path, skills[i].content) == 0) {
            (void)engine_rmdir(skill_path); /* only succeeds when no user files remain */
            count++;
        }
    }
    return count;
...
bool engine_remove_old_monolithic_skill(const char *skills_dir, bool dry_run) {
    if (!skills_dir) {
        return false;
    }

    char old_path[CLI_BUF_1K];
    snprintf(old_path, sizeof(old_path), "%s/graph-engine", skills_dir);
    return engine_remove_empty_directory(old_path, dry_run);
}
...
typedef enum {
    ENGINE_JSON_MCP_STANDARD,
    ENGINE_JSON_MCP_OPENCLAW,
    ENGINE_JSON_MCP_VSCODE,
    ENGINE_JSON_MCP_LOCAL_ARRAY,
    ENGINE_JSON_MCP_CLINE,
    ENGINE_JSON_MCP_COPILOT,
    ENGINE_JSON_MCP_FACTORY,
    ENGINE_JSON_MCP_CRUSH,
} engine_json_mcp_schema_t;

static bool engine_json_mcp_command_is_array(engine_json_mcp_schema_t schema) {
    return schema == ENGINE_JSON_MCP_LOCAL_ARRAY;
}

static const char *engine_json_mcp_required_type(engine_json_mcp_schema_t schema) {
    switch (schema) {
    case ENGINE_JSON_MCP_VSCODE:
    case ENGINE_JSON_MCP_FACTORY:
    case ENGINE_JSON_MCP_CRUSH:
        return "stdio";
    case ENGINE_JSON_MCP_LOCAL_ARRAY:
    case ENGINE_JSON_MCP_COPILOT:
        return "local";
    case ENGINE_JSON_MCP_STANDARD:
    case ENGINE_JSON_MCP_OPENCLAW:
    case ENGINE_JSON_MCP_CLINE:
        return NULL;
    }
    return NULL;
}

static const char ENGINE_DEFAULT_MCP_SERVER_NAME[] = "graph-engine";
static const char ENGINE_ANALYSIS_MCP_SERVER_NAME[] = "graph-engine-analysis";
static const char ENGINE_SCOUT_MCP_SERVER_NAME[] = "graph-engine-scout";
static const char ENGINE_ANALYSIS_PROFILE_ARGUMENT[] = "--tool-profile=analysis";
static const char ENGINE_SCOUT_PROFILE_ARGUMENT[] = "--tool-profile=scout";

static char *engine_build_json_mcp_entry(const char *binary_path, engine_json_mcp_schema_t schema,
                                      const char *argument) {
    ...
    bool command_is_array = engine_json_mcp_command_is_array(schema);
    ...
    const char *type = engine_json_mcp_required_type(schema);
    if (ok && type) {
...
static size_t engine_json_mcp_ownership_fields(engine_json_mcp_schema_t schema, const char *argument,
                                            engine_json_like_object_field_t fields[3]) {
    fields[0] = (engine_json_like_object_field_t){
        .key = "command",
        .shape = engine_json_mcp_command_is_array(schema) ? ENGINE_JSON_LIKE_VALUE_SINGLE_STRING_ARRAY
                                                       : ENGINE_JSON_LIKE_VALUE_STRING,
        .expected_string = NULL,
        .flags = ENGINE_JSON_LIKE_FIELD_REQUIRED | ENGINE_JSON_LIKE_FIELD_CAPTURE_STRING,
    };
    fields[1] = (engine_json_like_object_field_t){
        .key = "args",
        .shape =
            argument ? ENGINE_JSON_LIKE_VALUE_SINGLE_STRING_ARRAY : ENGINE_JSON_LIKE_VALUE_EMPTY_ARRAY,
        .expected_string = argument,
        .flags = argument ? ENGINE_JSON_LIKE_FIELD_REQUIRED : 0U,
    };
    const char *type = engine_json_mcp_required_type(schema);
    if (!type) {
        return 2U;
    }
    fields[2] = (engine_json_like_object_field_t){
        .key = "type",
        .shape = ENGINE_JSON_LIKE_VALUE_STRING,
        .expected_string = type,
        .flags = ENGINE_JSON_LIKE_FIELD_REQUIRED,
    };
    return 3U;
}
...
static ENGINE_TLS const char *g_previous_managed_mcp_command = NULL;

static bool engine_json_mcp_owned_command(const char *command, const char *expected_binary,
    ...
    return strcmp(command, "graph-engine") == 0 ||
           strcmp(command, "graph-engine.exe") == 0;
}
...
enum { ENGINE_JSON_MCP_OWNERSHIP_STALE = 100 };

#ifdef _WIN32
typedef enum {
    ENGINE_JSON_MCP_COMMAND_UNKNOWN = 0,
    ENGINE_JSON_MCP_COMMAND_PRESENT,
    ENGINE_JSON_MCP_COMMAND_MISSING,
} engine_json_mcp_command_availability_t;
#endif

#if defined(_WIN32) || defined(ENGINE_CLI_ENABLE_TEST_API)
static bool engine_json_mcp_command_path_probe_safe_for_platform(const char *command, bool windows) {
...
#ifdef _WIN32
static bool engine_json_mcp_command_path_probe_safe(const char *command) {
    return engine_json_mcp_command_path_probe_safe_for_platform(command, true);
}
#endif

#ifdef ENGINE_CLI_ENABLE_TEST_API
static ENGINE_TLS int *g_mcp_command_path_probe_counter = NULL;

bool engine_mcp_command_path_probe_safe_for_testing(const char *command, bool windows) {
    return engine_json_mcp_command_path_probe_safe_for_platform(command, windows);
}

void engine_set_mcp_command_path_probe_counter_for_testing(int *counter) {
    g_mcp_command_path_probe_counter = counter;
}
#endif

#ifdef _WIN32
static bool engine_json_mcp_windows_path_character_forbidden(wchar_t character) {
    return character < L' ' || character == L':' || character == L'*' || character == L'?' ||
           character == L'"' || character == L'<' || character == L'>' || character == L'|';
}

static bool engine_json_mcp_windows_path_syntax_valid(const wchar_t *path) {
    ...
    for (size_t index = component_start; index <= length; index++) {
        if (index < length && path[index] != L'\\') {
            if (engine_json_mcp_windows_path_character_forbidden(path[index])) {
                return false;
            }
            continue;
        }
...
static wchar_t *engine_json_mcp_windows_path_from_utf8(const char *path) {
    ...
    if (!engine_json_mcp_windows_path_syntax_valid(wide)) {
        free(wide);
        return NULL;
    }
    return wide;
...
static bool engine_json_mcp_windows_handle_is_local(HANDLE handle, wchar_t expected_drive,
...
static engine_json_mcp_command_availability_t engine_json_mcp_probe_windows_command_path(
    const char *path) {
    wchar_t *wide = engine_json_mcp_windows_path_from_utf8(path);
    if (!wide) {
        return ENGINE_JSON_MCP_COMMAND_UNKNOWN;
    }
    ...
    if (length == 3U) {
        free(wide);
        return ENGINE_JSON_MCP_COMMAND_PRESENT;
    }
    ...
    engine_json_mcp_command_availability_t availability = ENGINE_JSON_MCP_COMMAND_UNKNOWN;
    ...
    if (!engine_json_mcp_windows_handle_is_local(root_handle, wide[0], &root_information) ||
    ...
    for (size_t index = 3U; index <= length; index++) {
        ...
        BY_HANDLE_FILE_INFORMATION information;
        bool local = engine_json_mcp_windows_handle_is_local(component, wide[0], &information);
        if (local) {
        ...
        if (error == ERROR_FILE_NOT_FOUND || error == ERROR_PATH_NOT_FOUND) {
            availability = ENGINE_JSON_MCP_COMMAND_MISSING;
            break;
        }
        if (error != ERROR_SUCCESS || !local ||
            (!final_component && (information.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0)) {
            availability = ENGINE_JSON_MCP_COMMAND_UNKNOWN;
            break;
        }
        if (final_component) {
            availability = ENGINE_JSON_MCP_COMMAND_PRESENT;
            break;
        }
    }
...
#ifdef _WIN32
static engine_json_mcp_command_availability_t engine_json_mcp_probe_command_path(const char *path) {
#ifdef ENGINE_CLI_ENABLE_TEST_API
    if (g_mcp_command_path_probe_counter) {
        (*g_mcp_command_path_probe_counter)++;
    }
#endif
    return engine_json_mcp_probe_windows_command_path(path);
}
...
static engine_json_mcp_command_availability_t engine_json_mcp_command_availability(const char *command) {
    if (!engine_json_mcp_command_path_probe_safe(command) || strchr(command, '$') ||
        strchr(command, '%')) {
        return ENGINE_JSON_MCP_COMMAND_UNKNOWN;
    }
    engine_json_mcp_command_availability_t availability = engine_json_mcp_probe_command_path(command);
    if (availability != ENGINE_JSON_MCP_COMMAND_MISSING) {
        return availability;
    }
    ...
    if (!basename[0] || strchr(basename, '.')) {
        return ENGINE_JSON_MCP_COMMAND_MISSING;
    }
...
static int engine_json_mcp_snapshot_ownership(const char *document, size_t document_length,
                                           const char *const *object_path, size_t path_len,
                                           engine_json_mcp_schema_t schema, const char *entry_name,
                                           const char *argument, const char *expected_binary,
                                           const char *previous_managed_binary) {
    engine_json_like_object_field_t fields[3];
    size_t field_count = engine_json_mcp_ownership_fields(schema, argument, fields);
    char *command = NULL;
    int result = engine_json_like_match_object_entry(document, document_length, object_path, path_len,
                                                  entry_name, fields, field_count, &command);
    if (result == ENGINE_JSON_LIKE_OBJECT_MATCH &&
        !engine_json_mcp_owned_command(command, expected_binary, previous_managed_binary)) {
#ifdef _WIN32
        result = engine_json_mcp_command_availability(command) == ENGINE_JSON_MCP_COMMAND_MISSING
                     ? ENGINE_JSON_MCP_OWNERSHIP_STALE
                     : ENGINE_JSON_LIKE_OBJECT_MISMATCH;
#else
        result = ENGINE_JSON_LIKE_OBJECT_MISMATCH;
#endif
    }
    free(command);
    return result;
...
static int engine_upsert_json_named_mcp(const char *binary_path, const char *config_path,
                                     const char *const *object_path, size_t path_len,
                                     engine_json_mcp_schema_t schema, const char *entry_name,
                                     const char *argument) {
    ...
    int read_result = engine_json_like_read_document(config_path, &document, &document_length);
    if (read_result < 0) {
        return CLI_ERR;
    }
    if (read_result == 0) {
        int ownership = engine_json_mcp_snapshot_ownership(
        ...
        if (ownership != ENGINE_JSON_LIKE_OBJECT_MATCH && ownership != ENGINE_JSON_LIKE_OBJECT_MISSING &&
            ownership != ENGINE_JSON_MCP_OWNERSHIP_STALE) {
            free(document);
            return CLI_ERR;
        }
    }

    char *entry = engine_build_json_mcp_entry(binary_path, schema, argument);
    if (!entry) {
        free(document);
        return CLI_ERR;
    }
    int edit_result = engine_json_like_upsert_entry_if_unchanged(
...
static int engine_upsert_json_mcp(const char *binary_path, const char *config_path,
                               const char *const *object_path, size_t path_len,
                               engine_json_mcp_schema_t schema) {
    return engine_upsert_json_named_mcp(binary_path, config_path, object_path, path_len, schema,
                                     ENGINE_DEFAULT_MCP_SERVER_NAME, NULL);
}

static int engine_remove_json_named_mcp(const char *config_path, const char *const *object_path,
                                     size_t path_len, engine_json_mcp_schema_t schema,
    ...
    int ownership =
        engine_json_mcp_snapshot_ownership(document, document_length, object_path, path_len, schema,
                                        entry_name, argument, expected_binary, NULL);
    if (ownership == ENGINE_JSON_LIKE_OBJECT_MISSING || ownership == ENGINE_JSON_LIKE_OBJECT_MISMATCH ||
        ownership == ENGINE_JSON_MCP_OWNERSHIP_STALE) {
        free(document);
        return CLI_OK;
    }
    if (ownership != ENGINE_JSON_LIKE_OBJECT_MATCH) {
        free(document);
        return CLI_ERR;
    }
    int edit_result = engine_json_like_remove_entry_if_unchanged(
        config_path, object_path, path_len, entry_name, document, document_length);
    free(document);
    return edit_result == 0 ? CLI_OK : CLI_ERR;
}

static int engine_remove_json_mcp(const char *config_path, const char *const *object_path,
                               size_t path_len, engine_json_mcp_schema_t schema,
                               const char *expected_binary) {
    return engine_remove_json_named_mcp(config_path, object_path, path_len, schema,
                                     ENGINE_DEFAULT_MCP_SERVER_NAME, NULL, expected_binary);
}

/* ── Editor MCP: Cursor/Gemini/OpenHands/Qwen (mcpServers) ───── */

int engine_install_editor_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return engine_upsert_json_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_STANDARD);
}

#ifdef ENGINE_CLI_ENABLE_TEST_API
int engine_install_editor_mcp_with_previous_for_testing(const char *binary_path,
    ...
    int result = engine_install_editor_mcp(binary_path, config_path);
    g_previous_managed_mcp_command = saved;
    return result;
}
#endif

int engine_remove_editor_mcp(const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_STANDARD, NULL);
}

int engine_remove_editor_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_STANDARD, binary_path);
}

/* ── OpenClaw MCP (nested mcp.servers with command + args) ────── */

int engine_install_openclaw_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp", "servers"};
    return engine_upsert_json_mcp(binary_path, config_path, path, 2U, ENGINE_JSON_MCP_OPENCLAW);
}

int engine_remove_openclaw_mcp(const char *config_path) {
    static const char *const path[] = {"mcp", "servers"};
    return engine_remove_json_mcp(config_path, path, 2U, ENGINE_JSON_MCP_OPENCLAW, NULL);
}

int engine_remove_openclaw_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp", "servers"};
    return engine_remove_json_mcp(config_path, path, 2U, ENGINE_JSON_MCP_OPENCLAW, binary_path);
}

static const char engine_openclaw_compaction_section[] =
    "Codebase Knowledge Graph (graph-engine)";

static int engine_upsert_openclaw_compaction(const char *config_path) {
    static const char *const path[] = {"agents", "defaults", "compaction"};
    return engine_json_like_add_unique_string_at_path(config_path, path, 3U, "postCompactionSections",
                                                   engine_openclaw_compaction_section) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int engine_remove_openclaw_compaction(const char *config_path) {
    static const char *const path[] = {"agents", "defaults", "compaction"};
    return engine_json_like_remove_string_at_path(config_path, path, 3U, "postCompactionSections",
                                               engine_openclaw_compaction_section) == 0
               ? CLI_OK
               : CLI_ERR;
}
...
int engine_install_vscode_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"servers"};
    return engine_upsert_json_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_VSCODE);
}

int engine_remove_vscode_mcp(const char *config_path) {
    static const char *const path[] = {"servers"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_VSCODE, NULL);
}

int engine_remove_vscode_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"servers"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_VSCODE, binary_path);
}

/* ── Zed MCP (context_servers with command + args) ────────────── */

int engine_install_zed_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"context_servers"};
    return engine_upsert_json_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_STANDARD);
}

int engine_remove_zed_mcp(const char *config_path) {
    static const char *const path[] = {"context_servers"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_STANDARD, NULL);
}

int engine_remove_zed_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"context_servers"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_STANDARD, binary_path);
}
...
static void engine_claude_config_dir(const char *home_dir, char *out, size_t out_sz) {
    if (out_sz == 0) {
        return;
    }
    out[0] = '\0';
    char env_buf[CLI_BUF_1K];
    const char *env = engine_safe_getenv("CLAUDE_CONFIG_DIR", env_buf, sizeof(env_buf), NULL);
    if (env && env[0]) {
...
static void engine_claude_user_root(const char *home_dir, char *out, size_t out_sz) {
    if (out_sz == 0) {
        return;
    }
    out[0] = '\0';
...
static void engine_codex_config_dir(const char *home_dir, char *out, size_t out_sz) {
    if (out_sz == 0) {
        return;
    }
    out[0] = '\0';
    char env_buf[CLI_BUF_1K];
    const char *env = engine_safe_getenv("CODEX_HOME", env_buf, sizeof(env_buf), NULL);
    if (env && env[0]) {
        snprintf(out, out_sz, "%s", env);
    } else if (home_dir && home_dir[0]) {
...
static void engine_zed_config_dir(const char *home_dir, char *out, size_t out_sz) {
    if (out_sz == 0) {
        return;
    }
    out[0] = '\0';
...
#else
    char env_buf[CLI_BUF_1K];
    const char *xdg = engine_safe_getenv("XDG_CONFIG_HOME", env_buf, sizeof(env_buf), NULL);
    if (xdg && xdg[0]) {
...
static void engine_zed_instructions_path(const char *home_dir, char *out, size_t out_sz) {
#ifdef _WIN32
...
static bool engine_expand_user_path(const char *home_dir, const char *value, char *out,
                                 size_t out_sz) {
...
static void engine_env_home_dir(const char *env_name, const char *home_dir, const char *fallback,
                             char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = engine_safe_getenv(env_name, env_buf, sizeof(env_buf), NULL);
    out[0] = '\0';
    if (custom && custom[0] && engine_expand_user_path(home_dir, custom, out, out_sz)) {
        return;
    }
...
static void engine_kiro_home_dir(const char *home_dir, char *out, size_t out_sz) {
    engine_env_home_dir("KIRO_HOME", home_dir, ".kiro", out, out_sz);
}

static void engine_hermes_home_dir(const char *home_dir, char *out, size_t out_sz) {
    engine_env_home_dir("HERMES_HOME", home_dir, ".hermes", out, out_sz);
}

static void engine_qwen_home_dir(const char *home_dir, char *out, size_t out_sz) {
    engine_env_home_dir("QWEN_HOME", home_dir, ".qwen", out, out_sz);
}

static void engine_cline_root_dir(const char *home_dir, char *out, size_t out_sz) {
...
static void engine_cline_data_dir(const char *home_dir, char *out, size_t out_sz) {
    engine_env_home_dir("CLINE_DATA_DIR", home_dir, ".cline/data", out, out_sz);
}

static bool engine_openclaw_internal_home(const char *home_dir, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = engine_safe_getenv("OPENCLAW_HOME", env_buf, sizeof(env_buf), NULL);
    if (!custom || !custom[0]) {
        int written = snprintf(out, out_sz, "%s", home_dir);
        return written > 0 && (size_t)written < out_sz;
    }
    return engine_expand_user_path(home_dir, custom, out, out_sz);
}

static bool engine_openclaw_state_dir(const char *home_dir, char *out, size_t out_sz) {
    char internal_home[CLI_BUF_1K];
    if (!engine_openclaw_internal_home(home_dir, internal_home, sizeof(internal_home))) {
        return false;
    }
    char env_buf[CLI_BUF_1K];
    const char *custom = engine_safe_getenv("OPENCLAW_STATE_DIR", env_buf, sizeof(env_buf), NULL);
    if (custom && custom[0]) {
        return engine_expand_user_path(internal_home, custom, out, out_sz);
    }
    char profile_buf[CLI_BUF_256];
    const char *profile =
        engine_safe_getenv("OPENCLAW_PROFILE", profile_buf, sizeof(profile_buf), NULL);
...
static bool engine_openclaw_config_path(const char *home_dir, char *out, size_t out_sz) {
    char internal_home[CLI_BUF_1K];
    if (!engine_openclaw_internal_home(home_dir, internal_home, sizeof(internal_home))) {
        return false;
    }
    char env_buf[CLI_BUF_1K];
    const char *custom = engine_safe_getenv("OPENCLAW_CONFIG_PATH", env_buf, sizeof(env_buf), NULL);
    if (custom && custom[0]) {
        return engine_expand_user_path(internal_home, custom, out, out_sz);
    }
    char state_dir[CLI_BUF_1K];
    if (!engine_openclaw_state_dir(home_dir, state_dir, sizeof(state_dir))) {
        return false;
    }
...
static bool engine_openclaw_workspace_path(const char *home_dir, const char *config_path, char *out,
                                        size_t out_sz) {
    char internal_home[CLI_BUF_1K];
    if (!engine_openclaw_internal_home(home_dir, internal_home, sizeof(internal_home))) {
        return false;
    ...
    int lookup =
        engine_json_like_get_string_at_path(config_path, defaults_path, 2U, "workspace", &configured);
    if (lookup == 0) {
        bool ok = engine_expand_user_path(internal_home, configured, out, out_sz);
        free(configured);
        return ok;
    }
    free(configured);
    ...
    int include_lookup =
        engine_json_like_get_string_at_path(config_path, root_path, 0U, "$include", &include_value);
    free(include_value);
    if (include_lookup != 1) {
        return false;
    }
    ...
    const char *workspace =
        engine_safe_getenv("OPENCLAW_WORKSPACE_DIR", env_buf, sizeof(env_buf), NULL);
    if (workspace && workspace[0]) {
        return engine_expand_user_path(internal_home, workspace, out, out_sz);
    }
...
static void engine_opencode_config_path(const char *home_dir, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = engine_safe_getenv("OPENCODE_CONFIG", env_buf, sizeof(env_buf), NULL);
    if (custom && custom[0]) {
...
static void engine_copilot_config_dir(const char *home_dir, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = engine_safe_getenv("COPILOT_HOME", env_buf, sizeof(env_buf), NULL);
...
static void engine_crush_config_path(const char *home_dir, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = engine_safe_getenv("CRUSH_GLOBAL_CONFIG", env_buf, sizeof(env_buf), NULL);
    if (custom && custom[0]) {
        snprintf(out, out_sz, "%s", custom);
        return;
    }
...
static void engine_goose_config_dir(const char *home_dir, char *out, size_t out_sz) {
#ifdef _WIN32
...
static void engine_vibe_config_dir(const char *home_dir, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *custom = engine_safe_getenv("VIBE_HOME", env_buf, sizeof(env_buf), NULL);
    snprintf(out, out_sz, "%s", custom && custom[0] ? custom : "");
    if ((!custom || !custom[0]) && home_dir && home_dir[0]) {
...
static bool engine_hook_script_name_safe(const char *script_name) {
...
static int engine_build_claude_hook_command(const char *script_name, const char *config_dir,
                                         bool windows, char *out, size_t out_sz) {
    if (!engine_hook_script_name_safe(script_name) || !out || out_sz == 0U) {
        return CLI_ERR;
    }
    out[0] = '\0';
    if (windows) {
    ...
    if (config_dir && config_dir[0]) {
        ...
        return written > 0 && (size_t)written < sizeof(path)
                   ? engine_shell_quote_word(path, out, out_sz)
                   : CLI_ERR;
    }
...
static int engine_resolve_hook_command(const char *script_name, char *out, size_t out_sz) {
    char env_buf[CLI_BUF_1K];
    const char *env = engine_safe_getenv("CLAUDE_CONFIG_DIR", env_buf, sizeof(env_buf), NULL);
#ifdef _WIN32
    return engine_build_claude_hook_command(script_name, env, true, out, out_sz);
#else
    return engine_build_claude_hook_command(script_name, env, false, out, out_sz);
#endif
}

#ifdef ENGINE_CLI_ENABLE_TEST_API
int engine_resolve_claude_hook_command_for_testing(const char *script_name, bool windows,
                                                char *command, size_t command_size) {
    char env_buf[CLI_BUF_1K];
    const char *env = engine_safe_getenv("CLAUDE_CONFIG_DIR", env_buf, sizeof(env_buf), NULL);
    return engine_build_claude_hook_command(script_name, env, windows, command, command_size);
}
#endif
...
static int engine_resolve_previous_hook_command(const char *script_name, char *out, size_t out_sz) {
    if (!engine_hook_script_name_safe(script_name) || !out || out_sz == 0U) {
        return CLI_ERR;
    }
    char env_buf[CLI_BUF_1K];
...
static int engine_resolve_released_hook_command(const char *script_name, char *out, size_t out_sz) {
...
engine_detected_agents_t engine_detect_agents(const char *home_dir) {
    engine_detected_agents_t agents;
    ...
    engine_claude_config_dir(home_dir, path, sizeof(path));
    agents.claude_code = path[0] != '\0' && dir_exists(path);

    engine_codex_config_dir(home_dir, path, sizeof(path));
    agents.codex = path[0] != '\0' && dir_exists(path);

    snprintf(path, sizeof(path), "%s/.gemini/antigravity-cli", home_dir);
    agents.antigravity = dir_exists(path) || engine_agent_cli_exists("antigravity", home_dir);

    snprintf(path, sizeof(path), "%s/.gemini/settings.json", home_dir);
    agents.gemini = engine_file_exists(path) || engine_agent_cli_exists("gemini", home_dir);

    engine_zed_config_dir(home_dir, path, sizeof(path));
    agents.zed = dir_exists(path);

    engine_opencode_config_path(home_dir, path, sizeof(path));
    agents.opencode = engine_file_exists(path) || engine_agent_cli_exists("opencode", home_dir);
    if (!agents.opencode) {
    ...
    if (!agents.opencode) {
        ...
        const char *config_dir =
            engine_safe_getenv("OPENCODE_CONFIG_DIR", env_buf, sizeof(env_buf), NULL);
        agents.opencode = config_dir && config_dir[0] && dir_exists(config_dir);
    }

    agents.aider = engine_agent_cli_exists("aider", home_dir);
    ...
    agents.kilocode = agents.kilocode || dir_exists(path) || engine_agent_cli_exists("kilo", home_dir);
    ...
    agents.augment = dir_exists(path) || engine_agent_cli_exists("auggie", home_dir);

    char openclaw_config[CLI_BUF_1K];
    char openclaw_state[CLI_BUF_1K];
    bool has_openclaw_config =
        engine_openclaw_config_path(home_dir, openclaw_config, sizeof(openclaw_config));
    bool has_openclaw_state =
        engine_openclaw_state_dir(home_dir, openclaw_state, sizeof(openclaw_state));
    agents.openclaw = (has_openclaw_config && engine_file_exists(openclaw_config)) ||
                      (has_openclaw_state && dir_exists(openclaw_state)) ||
                      engine_agent_cli_exists("openclaw", home_dir);

    engine_kiro_home_dir(home_dir, path, sizeof(path));
    agents.kiro = path[0] && (dir_exists(path) || engine_agent_cli_exists("kiro-cli", home_dir) ||
                              engine_agent_cli_exists("kiro", home_dir));

    /* Junie (JetBrains): ~/.junie/ */
    snprintf(path, sizeof(path), "%s/.junie", home_dir);
    agents.junie = dir_exists(path);

    engine_hermes_home_dir(home_dir, path, sizeof(path));
    agents.hermes = path[0] && (dir_exists(path) || engine_agent_cli_exists("hermes", home_dir));

    snprintf(path, sizeof(path), "%s/.openhands", home_dir);
    agents.openhands = dir_exists(path) || engine_agent_cli_exists("openhands", home_dir);

    char cline_root[CLI_BUF_1K];
    char cline_data[CLI_BUF_1K];
    engine_cline_root_dir(home_dir, cline_root, sizeof(cline_root));
    engine_cline_data_dir(home_dir, cline_data, sizeof(cline_data));
    agents.cline = (cline_root[0] && dir_exists(cline_root)) ||
                   (cline_data[0] && dir_exists(cline_data)) ||
                   engine_agent_cli_exists("cline", home_dir);
...
#endif
    agents.warp = agents.warp || engine_agent_cli_exists("oz", home_dir) ||
                  engine_agent_cli_exists("oz-preview", home_dir) ||
                  engine_agent_cli_exists("warp-cli", home_dir);

    engine_qwen_home_dir(home_dir, path, sizeof(path));
    agents.qwen = path[0] && (dir_exists(path) || engine_agent_cli_exists("qwen", home_dir));

    engine_copilot_config_dir(home_dir, path, sizeof(path));
    ...
    agents.copilot_cli = engine_file_exists(copilot_mcp) || engine_file_exists(copilot_instructions) ||
                         engine_agent_cli_exists("copilot", home_dir);

    snprintf(path, sizeof(path), "%s/.factory", home_dir);
    agents.factory_droid = dir_exists(path) || engine_agent_cli_exists("droid", home_dir);

    engine_crush_config_path(home_dir, path, sizeof(path));
    agents.crush = engine_file_exists(path) || engine_agent_cli_exists("crush", home_dir);
    if (!agents.crush) {
        snprintf(path, sizeof(path), "%s/.config/crush", home_dir);
        agents.crush = dir_exists(path);
    }

    engine_goose_config_dir(home_dir, path, sizeof(path));
    agents.goose = dir_exists(path) || engine_agent_cli_exists("goose", home_dir);

    engine_vibe_config_dir(home_dir, path, sizeof(path));
    agents.mistral_vibe = dir_exists(path) || engine_agent_cli_exists("vibe", home_dir);
...
static const char legacy_augment_verify_agent_content[] =
    ...
    "graph.\n"
    "---\n"
    "Use graph-engine for structural discovery. Start with search_graph, continue with "
...
static const char legacy_gemini_verify_agent_content[] =
    ...
    "  - read_file\n"
    "  - grep_search\n"
    "  - mcp_graph-engine_search_graph\n"
    "  - mcp_graph-engine_trace_path\n"
    "  - mcp_graph-engine_get_code_snippet\n"
    "  - mcp_graph-engine_query_graph\n"
    "  - mcp_graph-engine_get_architecture\n"
    "  - mcp_graph-engine_search_code\n"
    "  - mcp_graph-engine_get_graph_schema\n"
    "  - mcp_graph-engine_list_projects\n"
    "  - mcp_graph-engine_index_status\n"
    "  - mcp_graph-engine_detect_changes\n"
    "---\n"
    "Use graph-engine for structural discovery. Start with search_graph, continue with "
    "trace_path, and retrieve exact definitions with get_code_snippet. Use query_graph or "
...
#define LEGACY_ENGINE_GRAPH_PROFILE_GUIDANCE                                                       \
    "Use graph-engine for read-only structural discovery. Start with search_graph, "     \
...
#define LEGACY_ENGINE_GRAPH_HANDOFF_GUIDANCE                                                       \
...
static const char legacy_claude_verify_agent_content[] =
    ...
    "  - Grep\n"
    "  - Glob\n"
    "  - mcp__graph-engine__search_graph\n"
    "  - mcp__graph-engine__trace_path\n"
    "  - mcp__graph-engine__get_code_snippet\n"
    "  - mcp__graph-engine__query_graph\n"
    "  - mcp__graph-engine__get_architecture\n"
    "  - mcp__graph-engine__search_code\n"
    "  - mcp__graph-engine__get_graph_schema\n"
    "  - mcp__graph-engine__list_projects\n"
    "  - mcp__graph-engine__index_status\n"
    "  - mcp__graph-engine__detect_changes\n"
    "mcpServers: [graph-engine]\n"
    "permissionMode: plan\n"
    "skills: [graph-engine]\n"
    "---\n" LEGACY_ENGINE_GRAPH_PROFILE_GUIDANCE;
...
static const char legacy_cursor_verify_agent_content[] =
    ...
    "model: inherit\n"
    "readonly: true\n"
    "---\n" LEGACY_ENGINE_GRAPH_HANDOFF_GUIDANCE;
...
static const char legacy_copilot_verify_agent_content[] =
    ...
    "  - read\n"
    "  - search\n"
    "  - graph-engine/search_graph\n"
    "  - graph-engine/trace_path\n"
    "  - graph-engine/get_code_snippet\n"
    "  - graph-engine/get_graph_schema\n"
    "  - graph-engine/get_architecture\n"
    "  - graph-engine/search_code\n"
    "  - graph-engine/query_graph\n"
    "  - graph-engine/list_projects\n"
    "  - graph-engine/index_status\n"
    "  - graph-engine/detect_changes\n"
    "---\n" LEGACY_ENGINE_GRAPH_PROFILE_GUIDANCE;
...
static const char legacy_kilo_verify_agent_content[] =
    ...
    "permission:\n"
    "  \"*\": deny\n"
    "  \"graph-engine_search_graph\": ask\n"
    "  \"graph-engine_trace_path\": ask\n"
    "  \"graph-engine_get_code_snippet\": ask\n"
    "  \"graph-engine_query_graph\": ask\n"
    "  \"graph-engine_get_architecture\": ask\n"
    "  \"graph-engine_search_code\": ask\n"
    "  \"graph-engine_get_graph_schema\": ask\n"
    "  \"graph-engine_list_projects\": ask\n"
    "  \"graph-engine_index_status\": ask\n"
    "  \"graph-engine_detect_changes\": ask\n"
    "---\n"
...
static const char legacy_vibe_verify_agent_content[] =
    ...
    "chains.\"\n"
    "safety = \"safe\"\n"
    "system_prompt_id = \"graph-engine\"\n"
    "enabled_tools = [\"graph-engine_search_graph\", "
    "\"graph-engine_trace_path\", \"graph-engine_get_code_snippet\", "
    "\"graph-engine_query_graph\", \"graph-engine_get_architecture\", "
    "\"graph-engine_search_code\", \"graph-engine_get_graph_schema\", "
    "\"graph-engine_list_projects\", \"graph-engine_index_status\", "
    "\"graph-engine_detect_changes\"]\n";

static const char legacy_vibe_verify_prompt_content[] =
    "Use the graph-engine graph: search_graph first for structural discovery, trace_path for "
...
static char *engine_build_legacy_kiro_verify_agent_content(const char *binary_path) {
    ...
    yyjson_mut_doc_set_root(doc, root);
    bool ok =
        yyjson_mut_obj_add_str(doc, root, "name", "graph-engine") &&
        yyjson_mut_obj_add_str(
            doc, root, "description",
            "Read-only code structure and call-chain investigation with the knowledge graph.") &&
        yyjson_mut_obj_add_str(
            doc, root, "prompt",
            "Use graph-engine for structural discovery. Start with search_graph, use "
        ...
        yyjson_mut_arr_add_str(doc, tools, "@graph-engine/search_graph") &&
        yyjson_mut_arr_add_str(doc, tools, "@graph-engine/trace_path") &&
        yyjson_mut_arr_add_str(doc, tools, "@graph-engine/get_code_snippet") &&
        yyjson_mut_arr_add_str(doc, tools, "@graph-engine/query_graph") &&
        yyjson_mut_arr_add_str(doc, tools, "@graph-engine/get_architecture") &&
        yyjson_mut_arr_add_str(doc, tools, "@graph-engine/search_code") &&
        yyjson_mut_arr_add_str(doc, tools, "@graph-engine/get_graph_schema") &&
        yyjson_mut_arr_add_str(doc, tools, "@graph-engine/list_projects") &&
        yyjson_mut_arr_add_str(doc, tools, "@graph-engine/index_status") &&
        yyjson_mut_arr_add_str(doc, tools, "@graph-engine/detect_changes") &&
        ...
        yyjson_mut_obj_add_val(doc, servers, "graph-engine", server) &&
...
static const char legacy_codebuddy_verify_agent_content[] =
    ...
    "tools: mcp__graph-engine__search_graph,mcp__graph-engine__trace_path,"
    "mcp__graph-engine__get_code_snippet,mcp__graph-engine__query_graph,"
    "mcp__graph-engine__get_architecture,mcp__graph-engine__search_code,"
    "mcp__graph-engine__get_graph_schema\n"
    "model: inherit\n"
    "permissionMode: plan\n"
    "skills: graph-engine\n"
...
static const char legacy_pochi_verify_agent_content[] =
    "---\n"
    "name: graph-engine\n"
    "description: Analyze code structure, dependencies, and call chains from graph-engine "
...
#undef LEGACY_ENGINE_GRAPH_PROFILE_GUIDANCE
#undef LEGACY_ENGINE_GRAPH_HANDOFF_GUIDANCE
 ...
 * exact same discovery priority, expressed as runnable `graph-engine
...
static const char aider_instructions_content[] =
    ...
    "   graph-engine cli search_graph "
    "'{\"project\":\"<name>\",\"name_pattern\":\".*Foo.*\"}'\n"
    "2. Who calls X / what does X call:\n"
    "   graph-engine cli trace_path "
    "'{\"project\":\"<name>\",\"function_name\":\"Foo\",\"direction\":\"both\"}'\n"
    "3. Read a specific function/class:\n"
    "   graph-engine cli get_code_snippet "
    "'{\"project\":\"<name>\",\"qualified_name\":\"<qn>\"}'\n"
    "4. Complex patterns (Cypher):\n"
    "   graph-engine cli query_graph '{\"project\":\"<name>\",\"query\":\"MATCH ...\"}'\n"
    "5. Project overview:\n"
    "   graph-engine cli get_architecture '{\"project\":\"<name>\"}'\n"
    "\n"
    "First use in a repo: graph-engine cli index_repository '{\"repo_path\":\"<abs "
    "path>\"}'\n"
    "List indexed projects (for <name>): graph-engine cli list_projects '{}'\n"
    "\n"
    "## When to fall back to grep/glob\n"
    "- Searching for string literals, error messages, config values\n"
    "- Searching non-code files (Dockerfiles, shell scripts, configs)\n"
...
const char *engine_get_aider_instructions(void) {
    return aider_instructions_content;
}

const char *engine_get_agent_instructions(void) {
    return agent_instructions_content;
}

/* ── Instructions file upsert ─────────────────────────────────── */

#define CMM_MARKER_START "<!-- graph-engine:start -->"
#define CMM_MARKER_END "<!-- graph-engine:end -->"
...
int engine_upsert_instructions(const char *path, const char *content) {
    if (!path || !content) {
        return CLI_ERR;
    }
    ...
    return engine_text_upsert_managed_block(path, CMM_MARKER_START, CMM_MARKER_END, content) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int engine_upsert_windsurf_rules(const char *path, const char *content) {
    if (!path || !content || ensure_parent_dir(path) != CLI_OK) {
        return CLI_ERR;
    }
    return engine_text_upsert_managed_block_limited(path, CMM_MARKER_START, CMM_MARKER_END, content,
                                                 WINDSURF_GLOBAL_RULES_MAX_BYTES) == 0
               ? CLI_OK
               : CLI_ERR;
}

int engine_remove_instructions(const char *path) {
    if (!path) {
        return CLI_ERR;
    }
    return engine_text_remove_managed_block(path, CMM_MARKER_START, CMM_MARKER_END) == 0 ? CLI_OK
                                                                                      : CLI_ERR;
}

/* ── Codex MCP config (TOML) ─────────────────────────────────── */

#define CODEX_CMM_TABLE "mcp_servers.graph-engine"
#define CODEX_CMM_SECTION "[" CODEX_CMM_TABLE "]"
#define CODEX_MCP_BEGIN "# >>> graph-engine MCP >>>"
#define CODEX_MCP_END "# <<< graph-engine MCP <<<"
 ...
 * remains fail-closed. */
static int engine_remove_codex_legacy_mcp(const char *config_path) {
    return engine_toml_remove_legacy_table(config_path, CODEX_CMM_TABLE, CODEX_MCP_BEGIN,
                                        CODEX_MCP_END);
}

int engine_upsert_codex_mcp(const char *binary_path, const char *config_path) {
    if (!binary_path || !config_path) {
        return CLI_ERR;
    }
    char escaped[CLI_BUF_8K];
    if (engine_toml_escape_basic_string(binary_path, escaped, sizeof(escaped)) != 0) {
        return CLI_ERR;
    }
    ...
    if (written < 0 || (size_t)written >= sizeof(block) ||
        engine_remove_codex_legacy_mcp(config_path) != 0) {
        return CLI_ERR;
    }
    return engine_toml_upsert_managed_block(config_path, CODEX_MCP_BEGIN, CODEX_MCP_END, block) == 0
               ? CLI_OK
               : CLI_ERR;
}

int engine_remove_codex_mcp(const char *config_path) {
    if (!config_path ||
        engine_toml_remove_managed_block(config_path, CODEX_MCP_BEGIN, CODEX_MCP_END) != 0) {
        return CLI_ERR;
    }
    return engine_remove_codex_legacy_mcp(config_path) >= 0 ? CLI_OK : CLI_ERR;
}

static int engine_remove_codex_mcp_owned(const char *binary_path, const char *config_path) {
    (void)binary_path;
    return engine_remove_codex_mcp(config_path);
}
...
#define CODEX_HOOK_BEGIN "# >>> graph-engine SessionStart >>>"
#define CODEX_HOOK_END "# <<< graph-engine SessionStart <<<"

static int engine_build_augment_command(const char *binary_path, char *out, size_t out_size) {
    char quoted[CLI_BUF_8K];
    if (engine_shell_quote_word(binary_path, quoted, sizeof(quoted)) != CLI_OK) {
        return CLI_ERR;
    }
...
static int engine_build_augment_dialect_command(const char *binary_path, const char *dialect,
    ...
    if (engine_build_augment_command(binary_path, base, sizeof(base)) != CLI_OK) {
        return CLI_ERR;
    }
...
static int engine_build_augment_command_windows(const char *binary_path, char *out, size_t out_size) {
    char quoted[CLI_BUF_8K];
    if (engine_powershell_quote_word(binary_path, quoted, sizeof(quoted)) != CLI_OK) {
        return CLI_ERR;
    }
    int written = snprintf(out, out_size, "& %s hook-augment", quoted);
    return written > 0 && (size_t)written < out_size ? CLI_OK : CLI_ERR;
...
static int engine_build_dialect_hook_command(const char *binary_path, const char *dialect,
    ...
    if (!windows) {
        shell[0] = '\0';
        return engine_build_augment_dialect_command(binary_path, dialect, command, command_size);
    }
    int shell_written = snprintf(shell, shell_size, "%s", "powershell");
    char base[CLI_BUF_8K];
    if (shell_written < 0 || (size_t)shell_written >= shell_size ||
        engine_build_augment_command_windows(binary_path, base, sizeof(base)) != CLI_OK) {
        return CLI_ERR;
    }
...
static int engine_build_qwen_hook_command(const char *binary_path, bool windows, char *command,
                                       size_t command_size, char *shell, size_t shell_size) {
    return engine_build_dialect_hook_command(binary_path, "qwen", windows, command, command_size,
                                          shell, shell_size);
}

static int engine_build_qoder_hook_command(const char *binary_path, bool windows, char *command,
                                        size_t command_size, char *shell, size_t shell_size) {
    return engine_build_dialect_hook_command(binary_path, "qoder", windows, command, command_size,
                                          shell, shell_size);
}

static bool engine_optional_hook_supported(const char *agent_name, bool windows) {
...
static bool engine_current_platform_is_windows(void) {
#ifdef _WIN32
    return true;
#else
    return false;
...
#ifdef ENGINE_CLI_ENABLE_TEST_API
int engine_build_qwen_hook_command_for_testing(const char *binary_path, bool windows, char *command,
                                            size_t command_size, char *shell, size_t shell_size) {
    return engine_build_qwen_hook_command(binary_path, windows, command, command_size, shell,
                                       shell_size);
}

int engine_build_qoder_hook_command_for_testing(const char *binary_path, bool windows, char *command,
                                             size_t command_size, char *shell, size_t shell_size) {
    return engine_build_qoder_hook_command(binary_path, windows, command, command_size, shell,
                                        shell_size);
}

/* Expose the current install behavior so platform-policy regressions can be
 * exercised on a non-Windows test host. */
bool engine_optional_hook_supported_for_testing(const char *agent_name, bool windows) {
    return engine_optional_hook_supported(agent_name, windows);
}
#endif

static int engine_upsert_codex_hooks_command(const char *config_path, const char *command,
    ...
    if (engine_toml_escape_basic_string(command, escaped, sizeof(escaped)) != CLI_OK ||
        engine_toml_escape_basic_string(command_windows, escaped_windows, sizeof(escaped_windows)) !=
            CLI_OK) {
        return CLI_ERR;
    }
    char block[CLI_BUF_8K];
    ...
    return engine_toml_upsert_managed_block(config_path, CODEX_HOOK_BEGIN, CODEX_HOOK_END, block) == 0
               ? CLI_OK
               : CLI_ERR;
}

/* Public path used by config-level regression tests and manual callers. */
int engine_upsert_codex_hooks(const char *config_path) {
    return engine_upsert_codex_hooks_command(config_path, "graph-engine hook-augment",
                                          "graph-engine hook-augment");
}

int engine_remove_codex_hooks(const char *config_path) {
    return config_path &&
                   engine_toml_remove_managed_block(config_path, CODEX_HOOK_BEGIN, CODEX_HOOK_END) == 0
               ? CLI_OK
               : CLI_ERR;
}
...
int engine_upsert_opencode_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp"};
    return engine_upsert_json_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_LOCAL_ARRAY);
}

int engine_remove_opencode_mcp(const char *config_path) {
    static const char *const path[] = {"mcp"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_LOCAL_ARRAY, NULL);
}

int engine_remove_opencode_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_LOCAL_ARRAY, binary_path);
}

static int engine_upsert_kilo_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp"};
    return engine_upsert_json_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_LOCAL_ARRAY);
}

static int engine_remove_kilo_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_LOCAL_ARRAY, binary_path);
}

static int engine_upsert_cline_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return engine_upsert_json_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_CLINE);
}

static int engine_remove_cline_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_CLINE, binary_path);
}

static int engine_upsert_copilot_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return engine_upsert_json_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_COPILOT);
}

static int engine_remove_copilot_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_COPILOT, binary_path);
}

enum { COPILOT_HOOK_TIMEOUT_SEC = 5 };

static int engine_build_copilot_hook_command(const char *binary_path, const char *event,
    ...
    char quoted[CLI_BUF_8K];
    int quote_rc = powershell ? engine_powershell_quote_word(binary_path, quoted, sizeof(quoted))
                              : engine_shell_quote_word(binary_path, quoted, sizeof(quoted));
    if (quote_rc != CLI_OK) {
        return CLI_ERR;
    }
...
static bool engine_copilot_add_hook_event(yyjson_mut_doc *doc, yyjson_mut_val *hooks,
...
static char *engine_build_copilot_hook_manifest(const char *binary_path) {
    ...
    if (engine_build_copilot_hook_command(binary_path, "SessionStart", false, session_bash,
                                       sizeof(session_bash)) != CLI_OK ||
        engine_build_copilot_hook_command(binary_path, "SessionStart", true, session_powershell,
                                       sizeof(session_powershell)) != CLI_OK ||
        engine_build_copilot_hook_command(binary_path, "SubagentStart", false, subagent_bash,
                                       sizeof(subagent_bash)) != CLI_OK ||
        engine_build_copilot_hook_command(binary_path, "SubagentStart", true, subagent_powershell,
    ...
    bool ok =
        ...
        engine_copilot_add_hook_event(doc, hooks, "sessionStart", session_bash, session_powershell) &&
        engine_copilot_add_hook_event(doc, hooks, "subagentStart", subagent_bash,
                                   subagent_powershell) &&
...
static int engine_upsert_copilot_hooks(const char *binary_path, const char *manifest_path) {
    char *manifest = engine_build_copilot_hook_manifest(binary_path);
    if (!manifest || !manifest_path) {
        free(manifest);
        return CLI_ERR;
    }
    int rc = engine_text_ensure_owned_document(manifest_path, manifest);
    free(manifest);
    return rc == 0 ? CLI_OK : CLI_ERR;
}

static int engine_remove_copilot_hooks(const char *manifest_path, const char *binary_path) {
    char *manifest = engine_build_copilot_hook_manifest(binary_path);
    if (!manifest || !manifest_path) {
        free(manifest);
        return CLI_ERR;
    }
    int rc = engine_text_remove_owned_document(manifest_path, manifest);
    free(manifest);
    return rc >= 0 ? CLI_OK : CLI_ERR;
}

static int engine_upsert_factory_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return engine_upsert_json_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_FACTORY);
}

static int engine_remove_factory_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_FACTORY, binary_path);
}

static int engine_upsert_crush_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp"};
    return engine_upsert_json_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_CRUSH);
}

static int engine_upsert_crush_context_path(const char *config_path, const char *context_path) {
    static const char *const path[] = {"options"};
    return engine_json_like_add_unique_string_at_path(config_path, path, 1U, "context_paths",
                                                   context_path) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int engine_remove_crush_context_path(const char *config_path, const char *context_path) {
    static const char *const path[] = {"options"};
    return engine_json_like_remove_string_at_path(config_path, path, 1U, "context_paths",
                                               context_path) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int engine_remove_crush_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcp"};
    return engine_remove_json_mcp(config_path, path, 1U, ENGINE_JSON_MCP_CRUSH, binary_path);
}

static int engine_build_yaml_stdio_mcp_block(const char *binary_path, bool goose_schema, char *block,
                                          size_t block_size) {
    if (!binary_path || !block || block_size == 0U) {
        return CLI_ERR;
    }
    char *encoded = NULL;
    if (engine_yaml_encode_double_quoted_scalar(binary_path, &encoded) != 0 || !encoded) {
        free(encoded);
        return CLI_ERR;
    }
...
static int engine_upsert_yaml_stdio_mcp(const char *binary_path, const char *config_path,
                                     const char *section_key, bool goose_schema) {
    char block[CLI_BUF_8K];
    if (!config_path || !section_key ||
        engine_build_yaml_stdio_mcp_block(binary_path, goose_schema, block, sizeof(block)) != CLI_OK) {
        return CLI_ERR;
    }
    return engine_yaml_upsert_owned_mapping_entry(config_path, section_key, "graph-engine",
                                               block) == ENGINE_YAML_IDENTITY_EDIT_OK
               ? CLI_OK
               : CLI_ERR;
}

static int engine_remove_yaml_stdio_mcp(const char *binary_path, const char *config_path,
                                     const char *section_key, bool goose_schema) {
    char block[CLI_BUF_8K];
    if (!config_path || !section_key ||
        engine_build_yaml_stdio_mcp_block(binary_path, goose_schema, block, sizeof(block)) != CLI_OK) {
    ...
    return engine_yaml_remove_owned_mapping_entry(config_path, section_key, "graph-engine",
                                               block);
}

static int engine_upsert_hermes_mcp(const char *binary_path, const char *config_path) {
    return engine_upsert_yaml_stdio_mcp(binary_path, config_path, "mcp_servers", false);
}

static int engine_remove_hermes_mcp_owned(const char *binary_path, const char *config_path) {
    return engine_remove_yaml_stdio_mcp(binary_path, config_path, "mcp_servers", false);
}

static int engine_upsert_goose_mcp(const char *binary_path, const char *config_path) {
    return engine_upsert_yaml_stdio_mcp(binary_path, config_path, "extensions", true);
}

static int engine_remove_goose_mcp_owned(const char *binary_path, const char *config_path) {
    return engine_remove_yaml_stdio_mcp(binary_path, config_path, "extensions", true);
}

/* ── Antigravity MCP config (JSON, same mcpServers format) ────── */

int engine_upsert_antigravity_mcp(const char *binary_path, const char *config_path) {
    /* Antigravity uses same mcpServers format as Cursor/Gemini */
    return engine_install_editor_mcp(binary_path, config_path);
}

int engine_remove_antigravity_mcp(const char *config_path) {
    return engine_remove_editor_mcp(config_path);
}

int engine_remove_antigravity_mcp_owned(const char *binary_path, const char *config_path) {
    return engine_remove_editor_mcp_owned(binary_path, config_path);
}

/* ── Junie MCP config (JSON, same mcpServers format) ──────────── */

static int engine_junie_mcp_preflight(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    static const struct {
        const char *name;
        const char *argument;
        ...
        {ENGINE_DEFAULT_MCP_SERVER_NAME, NULL},
        {ENGINE_SCOUT_MCP_SERVER_NAME, ENGINE_SCOUT_PROFILE_ARGUMENT},
        {ENGINE_ANALYSIS_MCP_SERVER_NAME, ENGINE_ANALYSIS_PROFILE_ARGUMENT},
    };
    char *document = NULL;
    size_t document_length = 0U;
    int read_result = engine_json_like_read_document(config_path, &document, &document_length);
    ...
    for (size_t i = 0U; i < sizeof(entries) / sizeof(entries[0]); i++) {
        int ownership = engine_json_mcp_snapshot_ownership(
            document, document_length, path, 1U, ENGINE_JSON_MCP_STANDARD, entries[i].name,
...
int engine_upsert_junie_mcp(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    if (engine_junie_mcp_preflight(binary_path, config_path) != CLI_OK ||
        engine_upsert_json_named_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_STANDARD,
                                  ENGINE_DEFAULT_MCP_SERVER_NAME, NULL) != CLI_OK ||
        engine_upsert_json_named_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_STANDARD,
                                  ENGINE_SCOUT_MCP_SERVER_NAME,
                                  ENGINE_SCOUT_PROFILE_ARGUMENT) != CLI_OK ||
        engine_upsert_json_named_mcp(binary_path, config_path, path, 1U, ENGINE_JSON_MCP_STANDARD,
                                  ENGINE_ANALYSIS_MCP_SERVER_NAME,
                                  ENGINE_ANALYSIS_PROFILE_ARGUMENT) != CLI_OK) {
        return CLI_ERR;
    }
    return CLI_OK;
}
...
#ifdef ENGINE_CLI_ENABLE_TEST_API
int engine_upsert_junie_mcp_with_previous_for_testing(const char *binary_path,
                                                   const char *previous_binary_path,
                                                   const char *config_path) {
    const char *saved = g_previous_managed_mcp_command;
    g_previous_managed_mcp_command = previous_binary_path;
    int result = engine_upsert_junie_mcp(binary_path, config_path);
    g_previous_managed_mcp_command = saved;
    return result;
}
#endif

int engine_remove_junie_mcp(const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    int result = CLI_OK;
    if (engine_remove_json_named_mcp(config_path, path, 1U, ENGINE_JSON_MCP_STANDARD,
                                  ENGINE_DEFAULT_MCP_SERVER_NAME, NULL, NULL) != CLI_OK) {
        result = CLI_ERR;
    }
    if (engine_remove_json_named_mcp(config_path, path, 1U, ENGINE_JSON_MCP_STANDARD,
                                  ENGINE_SCOUT_MCP_SERVER_NAME, ENGINE_SCOUT_PROFILE_ARGUMENT,
...
int engine_remove_junie_mcp_owned(const char *binary_path, const char *config_path) {
    static const char *const path[] = {"mcpServers"};
    int result = CLI_OK;
    if (engine_remove_json_named_mcp(config_path, path, 1U, ENGINE_JSON_MCP_STANDARD,
                                  ENGINE_DEFAULT_MCP_SERVER_NAME, NULL, binary_path) != CLI_OK) {
        result = CLI_ERR;
    }
    if (engine_remove_json_named_mcp(config_path, path, 1U, ENGINE_JSON_MCP_STANDARD,
                                  ENGINE_SCOUT_MCP_SERVER_NAME, ENGINE_SCOUT_PROFILE_ARGUMENT,
...
static int engine_build_vibe_mcp_body(const char *binary_path, char *body, size_t body_size) {
...
static int engine_upsert_vibe_mcp(const char *binary_path, const char *config_path) {
    char body[CLI_BUF_8K];
    if (!config_path || engine_build_vibe_mcp_body(binary_path, body, sizeof(body)) != CLI_OK) {
        return CLI_ERR;
    }
    return engine_toml_upsert_owned_named_array_table(config_path, "mcp_servers", "name",
                                                   "graph-engine",
                                                   body) == ENGINE_TOML_OWNED_EDIT_OK
               ? CLI_OK
               : CLI_ERR;
}

static int engine_remove_vibe_mcp_owned(const char *binary_path, const char *config_path) {
    char body[CLI_BUF_8K];
    if (!config_path || engine_build_vibe_mcp_body(binary_path, body, sizeof(body)) != CLI_OK) {
        return CLI_ERR;
    }
    return engine_toml_remove_owned_named_array_table(config_path, "mcp_servers", "name",
                                                   "graph-engine", body);
}
 ...
 * engine_resolve_hook_command so $CLAUDE_CONFIG_DIR is honored. */
#ifdef _WIN32
...
#define CMM_HOOK_GATE_SCRIPT "engine-code-discovery-gate.cmd"
#else
#define CMM_HOOK_GATE_SCRIPT "engine-code-discovery-gate"
#endif
#define CMM_HOOK_GATE_SCRIPT_LEGACY "engine-code-discovery-gate"
...
#ifdef ENGINE_JSON_LIKE_ENABLE_TEST_API
static ENGINE_TLS engine_hook_json_prewrite_test_hook_t engine_hook_json_prewrite_test_hook = NULL;
static ENGINE_TLS void *engine_hook_json_prewrite_test_context = NULL;

void engine_set_hook_json_prewrite_hook_for_testing(engine_hook_json_prewrite_test_hook_t hook,
                                                 void *context) {
    engine_hook_json_prewrite_test_hook = hook;
    engine_hook_json_prewrite_test_context = context;
}
#endif
...
static int write_hook_event_array(const char *settings_path, const char *hook_event,
...
#ifdef ENGINE_JSON_LIKE_ENABLE_TEST_API
    if (engine_hook_json_prewrite_test_hook) {
        engine_hook_json_prewrite_test_hook(settings_path, engine_hook_json_prewrite_test_context);
    }
#endif
    if (yyjson_mut_arr_size(event_arr) == 0U) {
        return engine_json_like_remove_entry_if_unchanged(settings_path, hooks_path, 1U, hook_event,
    ...
    int rc = engine_json_like_upsert_entry_if_unchanged(settings_path, hooks_path, 1U, hook_event,
...
static int upsert_hooks_json(hooks_upsert_args_t args) {
    ...
    int read_result =
        engine_json_like_read_document(settings_path, &expected_content, &expected_length);
    if (read_result < 0) {
        return CLI_ERR;
    }
...
static int engine_upsert_qoder_context_hook(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    char shell[CLI_BUF_32];
    if (engine_build_qoder_hook_command(binary_path, engine_current_platform_is_windows(), command,
...
static int engine_remove_qoder_context_hook(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    char shell[CLI_BUF_32];
    if (engine_build_qoder_hook_command(binary_path, engine_current_platform_is_windows(), command,
                                     sizeof(command), shell, sizeof(shell)) != CLI_OK) {
...
#ifdef ENGINE_CLI_ENABLE_TEST_API
int engine_upsert_qoder_context_hooks_for_testing(const char *settings_path, const char *binary_path) {
    return engine_upsert_qoder_context_hook(settings_path, binary_path);
}

int engine_remove_qoder_context_hooks_for_testing(const char *settings_path, const char *binary_path) {
    return engine_remove_qoder_context_hook(settings_path, binary_path);
}
#endif

#define KIMI_HOOK_BEGIN "# >>> graph-engine Kimi UserPromptSubmit >>>"
#define KIMI_HOOK_END "# <<< graph-engine Kimi UserPromptSubmit <<<"

static int engine_upsert_kimi_context_hook(const char *config_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    char escaped[CLI_BUF_8K];
    char block[CLI_BUF_8K];
    if (engine_build_augment_dialect_command(binary_path, "kimi", command, sizeof(command)) !=
            CLI_OK ||
        engine_toml_escape_basic_string(command, escaped, sizeof(escaped)) != 0) {
        return CLI_ERR;
    }
    int written = snprintf(block, sizeof(block),
                           "[[hooks]]\n"
    ...
    return engine_toml_upsert_managed_block(config_path, KIMI_HOOK_BEGIN, KIMI_HOOK_END, block) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int engine_remove_kimi_context_hook(const char *config_path) {
    return engine_toml_remove_managed_block(config_path, KIMI_HOOK_BEGIN, KIMI_HOOK_END) == 0
               ? CLI_OK
               : CLI_ERR;
}

static int engine_upsert_gitlab_session_hook(const char *hooks_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    if (engine_build_augment_command(binary_path, command, sizeof(command)) != CLI_OK) {
        return CLI_ERR;
    }
...
static int engine_remove_gitlab_session_hook(const char *hooks_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    if (engine_build_augment_command(binary_path, command, sizeof(command)) != CLI_OK) {
        return CLI_ERR;
    }
...
static int engine_edit_devin_context_hooks(const char *config_path, const char *binary_path,
                                        bool remove, bool include_session_start) {
    static const char *const events[] = {"SessionStart", "UserPromptSubmit", "PostCompaction"};
    char command[CLI_BUF_8K];
    if (engine_build_augment_dialect_command(binary_path, "devin", command, sizeof(command)) !=
        CLI_OK) {
        return CLI_ERR;
    }
    int result = CLI_OK;
...
static int engine_upsert_devin_context_hooks(const char *config_path, const char *binary_path,
                                          bool include_session_start) {
    return engine_edit_devin_context_hooks(config_path, binary_path, false, include_session_start);
}

static int engine_remove_devin_context_hooks(const char *config_path, const char *binary_path) {
    return engine_edit_devin_context_hooks(config_path, binary_path, true, true);
}

static int engine_remove_devin_session_hook(const char *config_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    if (engine_build_augment_dialect_command(binary_path, "devin", command, sizeof(command)) !=
        CLI_OK) {
        return CLI_ERR;
...
#define CMM_HERMES_HOOK_ID "graph-engine"

static int engine_build_hermes_context_hook_item(const char *binary_path, char *item,
                                              size_t item_size) {
    char command[CLI_BUF_8K];
    if (engine_build_augment_dialect_command(binary_path, "hermes", command, sizeof(command)) !=
        CLI_OK) {
        return CLI_ERR;
    }
    char *encoded_command = NULL;
    if (engine_yaml_encode_double_quoted_scalar(command, &encoded_command) != CLI_OK) {
        return CLI_ERR;
    }
...
static int engine_upsert_hermes_context_hook(const char *config_path, const char *binary_path) {
    ...
    if (engine_build_hermes_context_hook_item(binary_path, item, sizeof(item)) != CLI_OK) {
        return ENGINE_YAML_IDENTITY_EDIT_ERROR;
    }
    return engine_yaml_upsert_mapping_sequence_item(config_path, sequence_path,
                                                 sizeof(sequence_path) / sizeof(sequence_path[0]),
                                                 "id", identity, item);
}

static bool engine_yaml_line_is_empty_key(const char *line, size_t line_len, size_t indent,
                                       const char *key) {
...
static bool engine_hermes_pre_llm_sequence_is_empty(const char *document) {
    bool in_hooks = false;
    ...
    while (cursor && *cursor) {
        ...
        if (!blank) {
            ...
            if (indent == 0U) {
                in_hooks = engine_yaml_line_is_empty_key(line, line_len, 0U, "hooks");
            } else if (in_hooks && indent == CLI_PAIR_LEN &&
                       engine_yaml_line_is_empty_key(line, line_len, CLI_PAIR_LEN, "pre_llm_call")) {
                in_pre_llm = true;
            }
        }
...
static int engine_remove_hermes_context_hook(const char *config_path, const char *binary_path) {
    static const char *const sequence_path[] = {"hooks", "pre_llm_call"};
    static const char identity[] = "\"" CMM_HERMES_HOOK_ID "\"";
    char item[CLI_BUF_8K];
    if (engine_build_hermes_context_hook_item(binary_path, item, sizeof(item)) != CLI_OK) {
    ...
    int result = engine_yaml_remove_mapping_sequence_item(
        config_path, sequence_path, sizeof(sequence_path) / sizeof(sequence_path[0]), "id",
        identity, item);
    if (result != ENGINE_YAML_IDENTITY_EDIT_OK || !before) {
        free(before);
        return result;
    }
    size_t after_len = 0U;
    ...
    bool remove_empty_sequence = exact_removed && engine_hermes_pre_llm_sequence_is_empty(after);
    free(before);
    free(after);
    if (remove_empty_sequence &&
        engine_yaml_remove_mapping_entry(config_path, "hooks", "pre_llm_call") != CLI_OK) {
        return ENGINE_YAML_IDENTITY_EDIT_ERROR;
    }
    return ENGINE_YAML_IDENTITY_EDIT_OK;
}

int engine_upsert_claude_hooks(const char *settings_path) {
    char command[CLI_BUF_8K];
    ...
    if (engine_resolve_hook_command(CMM_HOOK_GATE_SCRIPT, command, sizeof(command)) != CLI_OK ||
        engine_resolve_previous_hook_command(CMM_HOOK_GATE_SCRIPT, previous_command,
                                          sizeof(previous_command)) != CLI_OK ||
        engine_resolve_released_hook_command(CMM_HOOK_GATE_SCRIPT, released_command,
                                          sizeof(released_command)) != CLI_OK ||
        engine_resolve_previous_hook_command(CMM_HOOK_GATE_SCRIPT_LEGACY, previous_legacy_command,
                                          sizeof(previous_legacy_command)) != CLI_OK ||
        engine_resolve_released_hook_command(CMM_HOOK_GATE_SCRIPT_LEGACY, released_legacy_command,
...
int engine_remove_claude_hooks(const char *settings_path) {
    char command[CLI_BUF_8K];
    char previous_command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
    char previous_legacy_command[CLI_BUF_8K];
...
static bool engine_write_owned_hook_script_with_legacy(const char *path, const char *script,
                                                    const char *const *legacy_scripts,
                                                    size_t legacy_count) {
    return engine_text_migrate_owned_document_mode(path, script, legacy_scripts, legacy_count,
                                                CLI_OCTAL_PERM) == CLI_OK;
}

static bool engine_write_owned_hook_script(const char *path, const char *script) {
    return engine_write_owned_hook_script_with_legacy(path, script, NULL, 0U);
}
 ...
 * hash-verified engine-integrations.json via integration_assets.h
...
#ifdef _WIN32
static int engine_escape_batch_value(const char *value, char *escaped, size_t escaped_size) {
...
static int engine_encode_template_binary(const char *bin_mode, const char *binary_path, char *out,
                                      size_t out_size) {
    if (!bin_mode || !binary_path || !out || out_size == 0U) {
        return CLI_ERR;
    }
    if (strcmp(bin_mode, "sh") == 0) {
        return engine_shell_quote_word(binary_path, out, out_size);
    }
    if (strcmp(bin_mode, "ps") == 0) {
        return engine_powershell_quote_word(binary_path, out, out_size);
    }
#ifdef _WIN32
    if (strcmp(bin_mode, "cmd") == 0) {
        return engine_escape_batch_value(binary_path, out, out_size);
    }
#endif
...
static int engine_template_append(char *out, size_t out_size, size_t *used, const char *chunk,
                               size_t chunk_length) {
...
static int engine_render_integration_body(const engine_integration_body_t *body, const char *binary_path,
    ...
    encoded_binary[0] = '\0';
    if (body->bin_mode && engine_encode_template_binary(body->bin_mode, binary_path, encoded_binary,
    ...
    while (*cursor) {
        ...
        if (!brace) {
            return engine_template_append(out, out_size, &used, cursor, strlen(cursor));
        }
        if (engine_template_append(out, out_size, &used, cursor, (size_t)(brace - cursor)) != CLI_OK) {
            return CLI_ERR;
        }
        if (body->bin_mode && strncmp(brace, bin_token, SLEN(bin_token)) == 0) {
            if (engine_template_append(out, out_size, &used, encoded_binary, strlen(encoded_binary)) !=
                CLI_OK) {
                return CLI_ERR;
            }
            cursor = brace + SLEN(bin_token);
        } else if (event && strncmp(brace, event_token, SLEN(event_token)) == 0) {
            if (engine_template_append(out, out_size, &used, event, strlen(event)) != CLI_OK) {
                return CLI_ERR;
            }
...
static int engine_render_integration_script(const char *template_id, const char *binary_path,
                                         const char *event, char *script, size_t script_size) {
    const engine_integration_template_t *tpl = engine_integration_template(template_id);
    if (!tpl) {
        return CLI_ERR;
    }
    return engine_render_integration_body(&tpl->current, binary_path, event, script, script_size);
}
...
static size_t engine_render_released_scripts(const engine_integration_template_t *tpl,
    ...
    for (size_t i = 0U; tpl && i < tpl->released_count && count < max_count; i++) {
        if (engine_render_integration_body(&tpl->released[i], binary_path, NULL, storage[count],
                                        CLI_BUF_8K) == CLI_OK) {
...
#ifdef _WIN32
#define AUGMENT_SESSION_SCRIPT "graph-engine-session.ps1"
#define AUGMENT_COVERAGE_SCRIPT "graph-engine-coverage.ps1"
#else
#define AUGMENT_SESSION_SCRIPT "graph-engine-session.sh"
#define AUGMENT_COVERAGE_SCRIPT "graph-engine-coverage.sh"
#endif

static int engine_build_augment_session_script(const char *binary_path, char *script,
                                            size_t script_size) {
    if (!binary_path || !script || script_size == 0U) {
        return CLI_ERR;
    }
    return engine_render_integration_script("augment_session", binary_path, NULL, script, script_size);
}

static bool engine_install_augment_session_script(const char *binary_path, const char *script_path) {
    char script[CLI_BUF_8K];
    return ensure_parent_dir(script_path) == CLI_OK &&
           engine_build_augment_session_script(binary_path, script, sizeof(script)) == CLI_OK &&
           engine_write_owned_hook_script(script_path, script);
}

static int engine_build_augment_coverage_script(const char *binary_path, char *script,
                                             size_t script_size) {
    if (!binary_path || !script || script_size == 0U) {
        return CLI_ERR;
    }
    return engine_render_integration_script("augment_coverage", binary_path, NULL, script,
                                         script_size);
}

static bool engine_install_augment_coverage_script(const char *binary_path, const char *script_path) {
    char script[CLI_BUF_8K];
    return ensure_parent_dir(script_path) == CLI_OK &&
           engine_build_augment_coverage_script(binary_path, script, sizeof(script)) == CLI_OK &&
           engine_write_owned_hook_script(script_path, script);
}
...
static int engine_cline_hook_path(const char *cline_root, const char *event, char *path,
                               size_t path_size) {
#ifdef _WIN32
...
static int engine_build_cline_context_script(const char *binary_path, const char *event, char *script,
                                          size_t script_size) {
    if (!binary_path || !event || !script || script_size == 0U) {
        return CLI_ERR;
    }
    return engine_render_integration_script("cline_context", binary_path, event, script, script_size);
}

/* Claude Code hook templates in engine-integrations.json. Their "released"
...
static int engine_remove_owned_hook_script(const char *path, const char *expected_current,
                                        const char *const *released_scripts,
                                        size_t released_script_count) {
    return engine_text_remove_owned_document_any(path, expected_current, released_scripts,
 ...
 * The legacy filename `engine-code-discovery-gate` is retained so existing
...
#ifdef _WIN32
static int engine_remove_owned_legacy_hook_script(const char *hooks_dir, const char *legacy_name,
    ...
    int result = engine_text_remove_owned_document_any(legacy_path, current_script, released_scripts,
                                                    released_script_count);
    return result < CLI_OK ? CLI_ERR : CLI_OK;
}
#endif

bool engine_install_hook_gate_script(const char *home, const char *binary_path) {
    if (!home || !binary_path) {
        return false;
    }
    char config_dir[CLI_BUF_1K];
    engine_claude_config_dir(home, config_dir, sizeof(config_dir));
    if (!config_dir[0]) {
        return false;
    }
    ...
    if (!engine_mkdir_p(hooks_dir, CLI_OCTAL_PERM)) {
        return false;
    }
    ...
    const engine_integration_template_t *tpl = engine_integration_template(CMM_HOOK_GATE_TEMPLATE);
    char script[CLI_BUF_8K];
    if (!tpl || engine_render_integration_body(&tpl->current, binary_path, NULL, script,
                                            sizeof(script)) != CLI_OK) {
        return false;
    }
    char released_storage[CMM_MAX_RELEASED_SCRIPTS][CLI_BUF_8K];
    const char *legacy[CMM_MAX_RELEASED_SCRIPTS];
    size_t legacy_count = engine_render_released_scripts(tpl, binary_path, released_storage, legacy,
                                                      CMM_MAX_RELEASED_SCRIPTS);
#ifdef _WIN32
    if (engine_remove_owned_legacy_hook_script(hooks_dir, CMM_HOOK_GATE_SCRIPT_LEGACY, script, legacy,
                                            legacy_count) != CLI_OK) {
        return false;
    }
#endif
    return engine_write_owned_hook_script_with_legacy(script_path, script, legacy, legacy_count);
}

/* SessionStart hook: remind agent to use MCP tools on every context reset. */
#ifdef _WIN32
#define CMM_SESSION_REMINDER_SCRIPT "engine-session-reminder.cmd"
#else
#define CMM_SESSION_REMINDER_SCRIPT "engine-session-reminder"
#endif
#define CMM_SESSION_REMINDER_SCRIPT_LEGACY "engine-session-reminder"

static bool engine_install_session_reminder_script(const char *home, const char *binary_path) {
    if (!home || !binary_path) {
        return false;
    }
    char config_dir[CLI_BUF_1K];
    ...
    const engine_integration_template_t *tpl = engine_integration_template(CMM_SESSION_REMINDER_TEMPLATE);
    char script[CLI_BUF_8K];
    if (!tpl || engine_render_integration_body(&tpl->current, binary_path, NULL, script,
                                            sizeof(script)) != CLI_OK) {
        return false;
    ...
    size_t legacy_count = engine_render_released_scripts(tpl, binary_path, released_storage, legacy,
                                                      CMM_MAX_RELEASED_SCRIPTS);
#ifdef _WIN32
    if (engine_remove_owned_legacy_hook_script(hooks_dir, CMM_SESSION_REMINDER_SCRIPT_LEGACY, script,
...
static int engine_upsert_session_hooks(const char *settings_path) {
    ...
    char previous_legacy_command[CLI_BUF_8K];
    char released_legacy_command[CLI_BUF_8K];
    if (engine_resolve_hook_command(CMM_SESSION_REMINDER_SCRIPT, command, sizeof(command)) != CLI_OK ||
        engine_resolve_previous_hook_command(CMM_SESSION_REMINDER_SCRIPT, previous_command,
                                          sizeof(previous_command)) != CLI_OK ||
        engine_resolve_released_hook_command(CMM_SESSION_REMINDER_SCRIPT, released_command,
                                          sizeof(released_command)) != CLI_OK ||
        engine_resolve_previous_hook_command(CMM_SESSION_REMINDER_SCRIPT_LEGACY,
                                          previous_legacy_command,
                                          sizeof(previous_legacy_command)) != CLI_OK ||
        engine_resolve_released_hook_command(CMM_SESSION_REMINDER_SCRIPT_LEGACY,
...
static int engine_remove_session_hooks(const char *settings_path) {
    static const char *matchers[] = {"startup", "resume", "clear", "compact"};
    char command[CLI_BUF_8K];
    char previous_command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
...
static bool engine_has_complete_claude_session_hooks(const char *home) {
    ...
    engine_claude_config_dir(home, config_dir, sizeof(config_dir));
    if (!config_dir[0] || engine_resolve_hook_command(CMM_SESSION_REMINDER_SCRIPT, expected_command,
    ...
    if (engine_json_like_read_document(settings_path, &content, &content_length) != 0) {
        free(content);
        return false;
    }
...
#ifdef _WIN32
#define CMM_SUBAGENT_REMINDER_SCRIPT "engine-subagent-reminder.cmd"
#else
#define CMM_SUBAGENT_REMINDER_SCRIPT "engine-subagent-reminder"
#endif
#define CMM_SUBAGENT_REMINDER_SCRIPT_LEGACY "engine-subagent-reminder"

static bool engine_install_subagent_reminder_script(const char *home, const char *binary_path) {
    if (!home || !binary_path) {
        return false;
    }
    char config_dir[CLI_BUF_1K];
    ...
    const engine_integration_template_t *tpl =
        engine_integration_template(CMM_SUBAGENT_REMINDER_TEMPLATE);
    char script[CLI_BUF_8K];
    if (!tpl || engine_render_integration_body(&tpl->current, binary_path, NULL, script,
                                            sizeof(script)) != CLI_OK) {
        return false;
    ...
    size_t legacy_count = engine_render_released_scripts(tpl, binary_path, released_storage, legacy,
                                                      CMM_MAX_RELEASED_SCRIPTS);
#ifdef _WIN32
    if (engine_remove_owned_legacy_hook_script(hooks_dir, CMM_SUBAGENT_REMINDER_SCRIPT_LEGACY, script,
                                            legacy, legacy_count) != CLI_OK) {
        return false;
    }
#endif
...
static bool engine_hook_script_write_would_succeed(const char *home, const char *binary_path,
    ...
    size_t candidate_count = engine_render_released_scripts(tpl, binary_path, released_storage,
                                                         candidates, CMM_MAX_RELEASED_SCRIPTS);
    return engine_text_owned_document_status(script_path, script, candidates, candidate_count) == 0;
}

int engine_upsert_claude_subagent_hooks(const char *settings_path) {
    char command[CLI_BUF_8K];
    char previous_command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
    char previous_legacy_command[CLI_BUF_8K];
    char released_legacy_command[CLI_BUF_8K];
    if (engine_resolve_hook_command(CMM_SUBAGENT_REMINDER_SCRIPT, command, sizeof(command)) !=
            CLI_OK ||
        engine_resolve_previous_hook_command(CMM_SUBAGENT_REMINDER_SCRIPT, previous_command,
                                          sizeof(previous_command)) != CLI_OK ||
        engine_resolve_released_hook_command(CMM_SUBAGENT_REMINDER_SCRIPT, released_command,
                                          sizeof(released_command)) != CLI_OK ||
        engine_resolve_previous_hook_command(CMM_SUBAGENT_REMINDER_SCRIPT_LEGACY,
                                          previous_legacy_command,
                                          sizeof(previous_legacy_command)) != CLI_OK ||
        engine_resolve_released_hook_command(CMM_SUBAGENT_REMINDER_SCRIPT_LEGACY,
                                          released_legacy_command,
                                          sizeof(released_legacy_command)) != CLI_OK) {
        return CLI_ERR;
    }
...
int engine_remove_claude_subagent_hooks(const char *settings_path) {
    char command[CLI_BUF_8K];
    char previous_command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
    char previous_legacy_command[CLI_BUF_8K];
...
#define GEMINI_HOOK_COMMAND                                                            \
    ...
    "graph-engine search_graph, trace_path, and get_code_snippet over grep or " \
    "file search.'}}))\""
static const char *const cmm_gemini_released_hook_commands[] = {
    "echo 'Reminder: prefer graph-engine search_graph/trace_path/get_code_snippet over "
    "grep/file search for code discovery.' >&2",
    "echo 'Reminder: prefer graph-engine search_graph/trace_call_path/get_code_snippet "
    "over grep/file search for code discovery.' >&2",
    NULL,
};

int engine_upsert_gemini_hooks(const char *settings_path) {
    return upsert_hooks_json((hooks_upsert_args_t){
        .settings_path = settings_path,
...
int engine_remove_gemini_hooks(const char *settings_path) {
    return remove_hooks_json((hooks_remove_args_t){
        .settings_path = settings_path,
        .hook_event = "BeforeTool",
        .matcher_str = GEMINI_HOOK_MATCHER,
...
#ifndef _WIN32
static int engine_upsert_gemini_coverage_hook(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    if (engine_build_augment_dialect_command(binary_path, "gemini", command, sizeof(command)) !=
        CLI_OK) {
        return CLI_ERR;
    }
    return upsert_hooks_json((hooks_upsert_args_t){
...
static int engine_remove_gemini_coverage_hook(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    if (engine_build_augment_dialect_command(binary_path, "gemini", command, sizeof(command)) !=
        CLI_OK) {
        return CLI_ERR;
...
int engine_remove_gemini_session_hooks(const char *settings_path) {
    static const char *const matchers[] = {"startup", "resume", "clear"};
    int rc = CLI_OK;
    for (size_t i = 0U; i < sizeof(matchers) / sizeof(matchers[0]); i++) {
        const char *const *old_matchers = i == 0U ? cmm_gemini_session_old_matchers : NULL;
...
static int engine_upsert_paired_lifecycle_hooks_json(const char *settings_path, const char *command,
...
static int engine_remove_paired_lifecycle_hooks_json(const char *settings_path,
                                                  const char *canonical_command);

static int engine_upsert_qwen_lifecycle_hooks(const char *settings_path, const char *binary_path,
                                           bool windows) {
    char command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
    char shell[CLI_BUF_32];
    if (engine_build_qwen_hook_command(binary_path, windows, command, sizeof(command), shell,
                                    sizeof(shell)) != CLI_OK ||
        (windows ? engine_build_augment_command_windows(binary_path, released_command,
                                                     sizeof(released_command))
                 : engine_build_augment_command(binary_path, released_command,
                                             sizeof(released_command))) != CLI_OK) {
        return CLI_ERR;
    }
    int legacy_result = engine_remove_paired_lifecycle_hooks_json(settings_path, released_command);
    int lifecycle_result = engine_upsert_paired_lifecycle_hooks_json(settings_path, command, NULL,
...
#ifdef ENGINE_CLI_ENABLE_TEST_API
int engine_upsert_qwen_lifecycle_hooks_for_testing(const char *settings_path, const char *binary_path,
                                                bool windows) {
    return engine_upsert_qwen_lifecycle_hooks(settings_path, binary_path, windows);
}
#endif
...
static int engine_remove_qwen_lifecycle_hooks(const char *settings_path, const char *binary_path,
                                           bool windows) {
    char command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
    char shell[CLI_BUF_32];
    ...
    int lifecycle_result = engine_remove_paired_lifecycle_hooks_json(settings_path, command);
    int legacy_result = engine_remove_paired_lifecycle_hooks_json(settings_path, released_command);
    int read_result = remove_hooks_json((hooks_remove_args_t){
        .settings_path = settings_path,
        .hook_event = "PostToolUse",
...
static int engine_upsert_factory_hooks(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
    if (engine_build_augment_dialect_command(binary_path, "factory", command, sizeof(command)) !=
            CLI_OK ||
        engine_build_augment_command(binary_path, released_command, sizeof(released_command)) !=
            CLI_OK) {
        return CLI_ERR;
    }
...
static int engine_remove_factory_hooks(const char *settings_path, const char *binary_path) {
    char command[CLI_BUF_8K];
    char released_command[CLI_BUF_8K];
    if (engine_build_augment_dialect_command(binary_path, "factory", command, sizeof(command)) !=
            CLI_OK ||
...
static int engine_upsert_augment_session_hook(const char *settings_path, const char *script_path) {
    return upsert_hooks_json((hooks_upsert_args_t){
        .settings_path = settings_path,
        .hook_event = "SessionStart",
        .matcher_str = NULL,
...
static int engine_remove_augment_session_hook(const char *settings_path, const char *script_path) {
    return remove_hooks_json((hooks_remove_args_t){
        .settings_path = settings_path,
        .hook_event = "SessionStart",
        .matcher_str = NULL,
...
static int engine_upsert_augment_coverage_hook(const char *settings_path, const char *script_path) {
    return upsert_hooks_json((hooks_upsert_args_t){
        .settings_path = settings_path,
        .hook_event = "PostToolUse",
        .matcher_str = "view",
...
static int engine_remove_augment_coverage_hook(const char *settings_path, const char *script_path) {
    return remove_hooks_json((hooks_remove_args_t){
        .settings_path = settings_path,
        .hook_event = "PostToolUse",
        .matcher_str = "view",
...
int engine_ensure_path(const char *bin_dir, const char *rc_file, bool dry_run) {
    if (!bin_dir || !rc_file) {
        return CLI_ERR;
    }
    ...
    bool is_fish = rc_len >= ENGINE_SZ_5 && strcmp(rc_file + rc_len - ENGINE_SZ_5, ".fish") == 0;
...
static int cli_windows_open_user_path_key(HKEY *environment) {
    ...
    *environment = NULL;
    ...
    DWORD needed = GetEnvironmentVariableW(L"ENGINE_TEST_WINDOWS_USER_PATH_RUN_ID", NULL, 0U);
    ...
    if (needed != CLI_BUF_32 + 1U ||
        GetEnvironmentVariableW(L"ENGINE_TEST_WINDOWS_USER_PATH_RUN_ID", run_id,
    ...
    LONG sentinel_status = RegQueryValueExW(*environment, L"EngineSmokeRunId", NULL, &sentinel_type,
...
unsigned char *engine_extract_binary_from_targz(const unsigned char *data, int data_len,
                                             int *out_len) {
    ...
    /* Parse tar: find entry starting with "graph-engine" */
    size_t pos = 0;
...
unsigned char *engine_extract_binary_from_zip(const unsigned char *data, int data_len, int *out_len) {
    if (!data || data_len <= 0 || !out_len) {
        return NULL;
    }
    *out_len = 0;
    ...
    while (pos + ZIP_HDR_SZ <= data_len) {
        ...
        if (strcmp(basename, "graph-engine") == 0 ||
            strcmp(basename, "graph-engine.exe") == 0) {
...
#endif /* ENGINE_CLI_ENABLE_TEST_API — tar.gz / zip extraction */

/* ── Index management ─────────────────────────────────────────── */

static const char *get_cache_dir(const char *home_dir) {
    if (!home_dir) {
        home_dir = engine_get_home_dir();
    }
    if (!home_dir) {
        return NULL;
    }
    return engine_resolve_cache_dir();
}

int engine_list_indexes(const char *home_dir) {
    const char *cache_dir = get_cache_dir(home_dir);
    if (!cache_dir) {
        return 0;
    }

    engine_dir_t *d = engine_opendir(cache_dir);
    if (!d) {
        return 0;
    }
    ...
    int count = 0;
    engine_dirent_t *ent;
    while ((ent = engine_readdir(d)) != NULL) {
...
int engine_remove_indexes(const char *home_dir) {
    const char *cache_dir = get_cache_dir(home_dir);
    if (!cache_dir) {
        return 0;
    }
    ...
    while ((ent = engine_readdir(d)) != NULL) {
        ...
        if (len > DB_EXT_LEN && strcmp(ent->name + len - DB_EXT_LEN, ".db") == 0) {
            ...
            engine_unlink(tmp_path);
            if (engine_unlink(path) == 0) {
                count++;
            }
        }
    }
...
engine_config_t *engine_config_open(const char *cache_dir) {
    if (!cache_dir) {
        return NULL;
    }
    ...
    engine_config_t *cfg = calloc(ENGINE_ALLOC_ONE, sizeof(*cfg));
    if (!cfg) {
        sqlite3_close(db);
        return NULL;
    }
...
void engine_config_close(engine_config_t *cfg) {
    if (!cfg) {
        return;
    }
    if (cfg->db) {
...
const char *engine_config_get(engine_config_t *cfg, const char *key, const char *default_val) {
    static ENGINE_TLS char result_buf[CLI_BUF_4K];
    if (!cfg || !key) {
        return default_val;
    }
    ...
    sqlite3_bind_text(stmt, SQL_PARAM_1, key, SQL_NUL_TERM, engine_sqlite_transient);
...
bool engine_config_get_bool(engine_config_t *cfg, const char *key, bool default_val) {
    const char *val = engine_config_get(cfg, key, NULL);
    if (!val) {
        return default_val;
    }
...
int engine_config_get_int(engine_config_t *cfg, const char *key, int default_val) {
    const char *val = engine_config_get(cfg, key, NULL);
    if (!val) {
        return default_val;
    }
...
int engine_config_set(engine_config_t *cfg, const char *key, const char *value) {
    if (!cfg || !key || !value) {
        return CLI_ERR;
    }
    ...
    sqlite3_bind_text(stmt, SQL_PARAM_1, key, SQL_NUL_TERM, engine_sqlite_transient);
    sqlite3_bind_text(stmt, SQL_PARAM_2, value, SQL_NUL_TERM, engine_sqlite_transient);
...
int engine_config_delete(engine_config_t *cfg, const char *key) {
    if (!cfg || !key) {
        return CLI_ERR;
    }
...
int engine_cmd_config(int argc, char **argv) {
    ...
    if (argc == 0 || !argv || strcmp(argv[0], "--help") == 0 || strcmp(argv[0], "-h") == 0) {
        printf("Usage: graph-engine config <command> [args]\n\n");
        printf("Commands:\n");
    ...
    snprintf(cache_dir, sizeof(cache_dir), "%s", engine_resolve_cache_dir());

    engine_config_t *cfg = engine_config_open(cache_dir);
    if (!cfg) {
    ...
    if (strcmp(argv[0], "list") == 0 || strcmp(argv[0], "ls") == 0) {
        ...
        printf("  %-25s = %-10s\n", ENGINE_CONFIG_AUTO_INDEX,
               engine_config_get(cfg, ENGINE_CONFIG_AUTO_INDEX, "false"));
        printf("  %-25s = %-10s\n", ENGINE_CONFIG_AUTO_INDEX_LIMIT,
               engine_config_get(cfg, ENGINE_CONFIG_AUTO_INDEX_LIMIT, "50000"));
        printf("  %-25s = %-10s\n", ENGINE_CONFIG_AUTO_WATCH,
               engine_config_get(cfg, ENGINE_CONFIG_AUTO_WATCH, "true"));
        printf("  %-25s = %-10s\n", ENGINE_CONFIG_UI_LANG,
               engine_config_get(cfg, ENGINE_CONFIG_UI_LANG, "auto"));
    } else if (strcmp(argv[0], "get") == 0) {
        if (argc < MIN_ARGC_GET) {
            (void)fprintf(stderr, "Usage: config get <key>\n");
            rc = CLI_TRUE;
        } else {
            printf("%s\n", engine_config_get(cfg, argv[CLI_SKIP_ONE], ""));
        }
        ...
        if (argc < MIN_ARGC_CMD) {
            ...
            if (engine_config_set(cfg, argv[CLI_SKIP_ONE], argv[CLI_PAIR_LEN]) == 0) {
        ...
        if (argc < MIN_ARGC_GET) {
            ...
            engine_config_delete(cfg, argv[CLI_SKIP_ONE]);
...
void engine_set_auto_answer_for_test(int value);
void engine_set_auto_answer_for_test(int value) {
    g_auto_answer = value;
}
...
#define SHA256_HEX_LEN ENGINE_SZ_64
...
int engine_cli_sha256_file(const char *path, char *out, size_t out_size) {
    if (out_size < SHA256_BUF_SIZE) {
        return CLI_ERR;
    }
    FILE *fp = engine_fopen(path, "rb");
    if (!fp) {
        return CLI_ERR;
    }
    engine_sha256_ctx ctx;
    engine_sha256_init(&ctx);
    unsigned char buf[CLI_BUF_1K];
    size_t n;
    while ((n = fread(buf, 1, sizeof(buf), fp)) > 0) {
        engine_sha256_update(&ctx, buf, n);
    }
    ...
    engine_sha256_final(&ctx, digest);
...
int engine_cli_checksum_manifest_digest(const char *manifest_path, const char *archive_name, char *out,
                                     size_t out_size) {
    if (out && out_size > 0) {
        out[0] = '\0';
    }
    ...
    FILE *fp = engine_fopen(manifest_path, "rb");
    if (!fp) {
        return CLI_ERR;
    }
...
static bool cli_download_is_explicit_file_override(const char *url) {
    ...
    const char *override =
        engine_safe_getenv("ENGINE_DOWNLOAD_URL", override_buffer, sizeof(override_buffer), NULL);
...
static int engine_download_to_file(const char *url, const char *dest) {
    const char *protocol = cli_download_protocol(url);
    if (!protocol || !dest) {
        (void)fprintf(stderr, "error: update downloads require HTTPS (file:// is "
                              "reserved for an explicit ENGINE_DOWNLOAD_URL test "
                              "override)\n");
        return CLI_TRUE;
    }
    const char *argv[] = {"curl",    "-fSL",   "--progress-bar",
                          ...
                          protocol,  "-o",     dest,
                          url,       NULL};
    return engine_exec_no_shell(argv);
}

static int engine_download_to_file_quiet(const char *url, const char *dest) {
    const char *protocol = cli_download_protocol(url);
    if (!protocol || !dest) {
...
#endif /* ENGINE_CLI_ENABLE_TEST_API */

/* ── macOS ad-hoc signing ─────────────────────────────────────── */

#ifdef __APPLE__
static int engine_macos_adhoc_sign(const char *binary_path) {
...
static int verify_download_checksum(const char *archive_path, const char *archive_name) {
    ...
    int checksum_path_length =
        snprintf(checksum_file, sizeof(checksum_file), "%s/engine-checksums-XXXXXX", engine_tmpdir());
    if (checksum_path_length <= 0 || (size_t)checksum_path_length >= sizeof(checksum_file)) {
        return CLI_ERR;
    }
    int checksum_descriptor = engine_mkstemp(checksum_file);
    ...
    if (checksum_close_status != 0) {
        engine_unlink(checksum_file);
        return CLI_ERR;
    }

    char dl_base_buf[CLI_BUF_512];
    const char *dl_base =
        engine_safe_getenv("ENGINE_DOWNLOAD_URL", dl_base_buf, sizeof(dl_base_buf), NULL);
    ...
    if (dl_base && dl_base[0]) {
        ...
        checksum_url_length =
            snprintf(checksum_url, sizeof(checksum_url), "%s",
                     "https://github.com/DeusData/graph-engine/releases/latest/"
    ...
    int rc = engine_download_to_file_quiet(checksum_url, checksum_file);
    if (rc != 0) {
    ...
    int manifest_status =
        engine_cli_checksum_manifest_digest(checksum_file, archive_name, expected, sizeof(expected));
    engine_unlink(checksum_file);
    ...
    if (engine_cli_sha256_file(archive_path, actual, sizeof(actual)) != 0) {
...
static void print_detected_agents(const engine_detected_agents_t *a, const char *home) {
    struct {
        bool flag;
        const char *name;
    } agents[] = {
...
typedef struct {
    engine_plan_entry_t *items;
    int count;
    int cap;
} engine_install_plan_t;

static engine_install_plan_t *g_install_plan = NULL;
...
static void plan_record(const char *agent, const char *kind, const char *path) {
    ...
    engine_install_plan_t *pl = g_install_plan;
    if (pl->count >= pl->cap) {
        int ncap = pl->cap ? pl->cap * 2 : CLI_BUF_16;
        engine_plan_entry_t *ni = realloc(pl->items, (size_t)ncap * sizeof(*ni));
        if (!ni) {
            return;
        }
        pl->items = ni;
    ...
    engine_plan_entry_t *e = &pl->items[pl->count++];
...
static bool prepare_config_parent(const char *path) {
    ...
    *slash = '\0';
    return engine_mkdir_p(parent, CLI_OCTAL_PERM);
}
...
typedef struct {
    ...
    engine_graph_profile_dialect_t dialect;
    bool force_handoff;
} engine_tiered_profile_set_t;

static void install_tiered_agent_profiles(engine_tiered_profile_set_t profiles, bool dry_run);
static void uninstall_tiered_agent_profiles(engine_tiered_profile_set_t profiles, bool dry_run);
...
static void install_claude_code_config(const char *home, const char *binary_path, bool force,
    ...
    engine_claude_user_root(home, user_root, sizeof(user_root));

    char skills_dir[CLI_BUF_1K];
    char agent_path[CLI_BUF_1K];
    snprintf(skills_dir, sizeof(skills_dir), "%s/skills", config_dir);
    snprintf(agent_path, sizeof(agent_path), "%s/agents/graph-engine.md", config_dir);

    /* Plan mode: record the planned writes and return without mutating (#388). */
    if (g_install_plan) {
        char p[CLI_BUF_1K];
        snprintf(p, sizeof(p), "%s/graph-engine/SKILL.md", skills_dir);
        ...
        install_tiered_agent_profiles(
            (engine_tiered_profile_set_t){
                ...
                .dialect = ENGINE_GRAPH_DIALECT_CLAUDE,
            },
            dry_run);
    ...
    int skill_count = engine_install_skills(skills_dir, force, dry_run);
    ...
    if (engine_remove_old_monolithic_skill(skills_dir, dry_run)) {
    ...
    if (!dry_run && engine_file_exists(legacy_mcp_path) &&
        engine_remove_editor_mcp_owned(binary_path, legacy_mcp_path) != CLI_OK) {
    ...
    if (!dry_run) {
        if (!prepare_config_parent(mcp_path2) ||
            engine_install_editor_mcp(binary_path, mcp_path2) != CLI_OK) {
    ...
    if (dry_run) {
        ...
        gate_ok = engine_hook_script_write_would_succeed(home, binary_path, CMM_HOOK_GATE_SCRIPT);
        session_ok =
            engine_hook_script_write_would_succeed(home, binary_path, CMM_SESSION_REMINDER_SCRIPT);
        subagent_ok =
            engine_hook_script_write_would_succeed(home, binary_path, CMM_SUBAGENT_REMINDER_SCRIPT);
    }
    if (!dry_run) {
        char hook_path[CLI_BUF_1K];
        gate_ok = engine_install_hook_gate_script(home, binary_path);
        ...
        } else if (engine_upsert_claude_hooks(settings_path) != CLI_OK) {
            gate_ok = false;
            record_agent_config_error(false, "Claude Code", "hook_register", settings_path);
        }

        session_ok = engine_install_session_reminder_script(home, binary_path);
        snprintf(hook_path, sizeof(hook_path), "%s/hooks/%s", config_dir,
                 CMM_SESSION_REMINDER_SCRIPT);
        if (!session_ok) {
            record_agent_config_error(false, "Claude Code", "hook_script_install", hook_path);
        } else if (engine_upsert_session_hooks(settings_path) != CLI_OK) {
            session_ok = false;
            record_agent_config_error(false, "Claude Code", "hook_register", settings_path);
        }

        subagent_ok = engine_install_subagent_reminder_script(home, binary_path);
        snprintf(hook_path, sizeof(hook_path), "%s/hooks/%s", config_dir,
                 CMM_SUBAGENT_REMINDER_SCRIPT);
        if (!subagent_ok) {
            record_agent_config_error(false, "Claude Code", "hook_script_install", hook_path);
        } else if (engine_upsert_claude_subagent_hooks(settings_path) != CLI_OK) {
            subagent_ok = false;
            record_agent_config_error(false, "Claude Code", "hook_register", settings_path);
        }
    }
    ...
    if (instr_path) {
        if (!dry_run) {
            if (!prepare_config_parent(instr_path) ||
                engine_upsert_instructions(instr_path, agent_instructions_content) != CLI_OK) {
...
static void install_windsurf_config(const char *binary_path, const char *config_path,
    ...
    if (!dry_run) {
        if (!prepare_config_parent(config_path) ||
            engine_install_editor_mcp(binary_path, config_path) != CLI_OK) {
            record_agent_config_error(false, "Windsurf", "mcp_install", config_path);
        }
        if (!prepare_config_parent(rules_path) ||
            engine_upsert_windsurf_rules(rules_path, agent_instructions_content) != CLI_OK) {
...
static void install_agent_skill(const char *label, const char *skills_dir, bool force,
    ...
    char skill_path[CLI_BUF_1K];
    int written =
        snprintf(skill_path, sizeof(skill_path), "%s/graph-engine/SKILL.md", skills_dir);
    ...
    int installed = engine_install_skills(skills_dir, force, dry_run);
...
static int engine_tiered_profile_path(const char *verify_path, engine_graph_tier_t tier, char *output,
                                   size_t output_size) {
    static const char verify_basename[] = "graph-engine";
    ...
    const char *slug = engine_graph_tier_slug(tier);
    if (!slug) {
        return CLI_ERR;
    }
...
static engine_graph_access_t engine_tiered_profile_access(engine_graph_profile_dialect_t dialect) {
    return engine_graph_dialect_direct_capable(dialect) ? ENGINE_GRAPH_ACCESS_DIRECT
                                                     : ENGINE_GRAPH_ACCESS_HANDOFF;
}

static engine_graph_access_t engine_tiered_profile_set_access(engine_tiered_profile_set_t profiles) {
    return !profiles.force_handoff && engine_graph_dialect_direct_capable(profiles.dialect)
               ? ENGINE_GRAPH_ACCESS_DIRECT
               : ENGINE_GRAPH_ACCESS_HANDOFF;
}

static void install_tiered_agent_profiles(engine_tiered_profile_set_t profiles, bool dry_run) {
    engine_graph_access_t access = engine_tiered_profile_set_access(profiles);
    for (int value = 0; value < (int)ENGINE_GRAPH_TIER_COUNT; value++) {
        engine_graph_tier_t tier = (engine_graph_tier_t)value;
        char path[CLI_BUF_1K];
        if (engine_tiered_profile_path(profiles.verify_path, tier, path, sizeof(path)) != CLI_OK) {
        ...
        char *current =
            engine_render_graph_profile(profiles.dialect, tier, access, profiles.binary_path);
        if (!current) {
            record_agent_config_error(false, profiles.label, "agent_render", path);
            continue;
        }
        engine_graph_access_t alternate_access =
            access == ENGINE_GRAPH_ACCESS_DIRECT ? ENGINE_GRAPH_ACCESS_HANDOFF : ENGINE_GRAPH_ACCESS_DIRECT;
        char *alternate = engine_render_graph_profile(profiles.dialect, tier, alternate_access,
                                                   profiles.binary_path);
        char *codex_rc1 =
            profiles.dialect == ENGINE_GRAPH_DIALECT_CODEX && access == ENGINE_GRAPH_ACCESS_DIRECT
                ? engine_render_graph_profile_codex_rc1(tier)
                : NULL;
        ...
        if (tier == ENGINE_GRAPH_TIER_VERIFY && profiles.legacy_verify_content) {
            released[released_count++] = profiles.legacy_verify_content;
        }
        int result = prepare_config_parent(path)
                         ? engine_text_migrate_owned_document(path, current, released, released_count)
                         : CLI_ERR;
        free(codex_rc1);
        free(alternate);
        free(current);
...
static void uninstall_tiered_agent_profiles(engine_tiered_profile_set_t profiles, bool dry_run) {
    ...
    for (int value = 0; value < (int)ENGINE_GRAPH_TIER_COUNT; value++) {
        ...
        int result = engine_text_remove_owned_document_any(path, current, released, released_count);
        free(codex_rc1);
        free(alternate);
        free(current);
        if (result < CLI_OK) {
...
static void install_tiered_profile_prompts(const char *label, const char *verify_path,
                                           engine_graph_profile_dialect_t dialect,
                                           const char *legacy_verify_content, bool dry_run) {
    engine_graph_access_t access = engine_tiered_profile_access(dialect);
    for (int value = 0; value < (int)ENGINE_GRAPH_TIER_COUNT; value++) {
        engine_graph_tier_t tier = (engine_graph_tier_t)value;
        char path[CLI_BUF_1K];
        if (engine_tiered_profile_path(verify_path, tier, path, sizeof(path)) != CLI_OK) {
        ...
        char *current = engine_render_graph_prompt(tier, access);
        if (!current) {
            record_agent_config_error(false, label, "prompt_render", path);
            continue;
        }
        const char *released[] = {legacy_verify_content};
        size_t released_count = tier == ENGINE_GRAPH_TIER_VERIFY && legacy_verify_content ? 1U : 0U;
        int result = prepare_config_parent(path)
                         ? engine_text_migrate_owned_document(path, current, released, released_count)
                         : CLI_ERR;
        free(current);
...
static void install_copilot_durable_context(const char *home, const char *binary_path, bool force,
    ...
    char skills_dir[CLI_BUF_1K];
    char agent_path[CLI_BUF_1K];
    engine_copilot_config_dir(home, config_dir, sizeof(config_dir));
    snprintf(hooks_dir, sizeof(hooks_dir), "%s/hooks", config_dir);
    snprintf(hook_path, sizeof(hook_path), "%s/hooks/graph-engine.json", config_dir);
    snprintf(skills_dir, sizeof(skills_dir), "%s/skills", config_dir);
    snprintf(agent_path, sizeof(agent_path), "%s/agents/graph-engine.agent.md", config_dir);
    ...
    install_tiered_agent_profiles(
        (engine_tiered_profile_set_t){
            ...
            .dialect = ENGINE_GRAPH_DIALECT_COPILOT,
        },
        dry_run);
    if (g_install_plan) {
    ...
    bool hook_ok = true;
    if (!dry_run && (!engine_mkdir_p(hooks_dir, CLI_OCTAL_PERM) ||
                     engine_upsert_copilot_hooks(binary_path, hook_path) != CLI_OK)) {
        hook_ok = false;
...
typedef struct {
    engine_agent_client_resolve_options_t options;
...
} engine_agent_registry_context_t;

static const char *engine_agent_registry_env_path(const char *env_name, const char *home,
                                               char *resolved, size_t resolved_size) {
    char value[CLI_BUF_1K];
    resolved[0] = '\0';
    const char *configured = engine_safe_getenv(env_name, value, sizeof(value), NULL);
    return configured && configured[0] &&
                   engine_expand_user_path(home, configured, resolved, resolved_size)
               ? resolved
               : NULL;
}

static bool engine_agent_registry_path_exists(const char *path, const void *context) {
    (void)context;
    struct stat state;
#ifndef _WIN32
...
static bool engine_agent_registry_command_exists(const char *command, const void *context) {
    const engine_agent_registry_context_t *registry = context;
    return registry && engine_agent_cli_exists(command, registry->options.home_dir);
}

static void engine_init_agent_registry_context(const char *home,
                                            engine_agent_registry_context_t *registry) {
    memset(registry, 0, sizeof(*registry));
    registry->options.home_dir = home;
    registry->options.xdg_config_home = engine_agent_registry_env_path(
        "XDG_CONFIG_HOME", home, registry->xdg_config_home, sizeof(registry->xdg_config_home));
    registry->options.appdata_dir = engine_agent_registry_env_path(
        "APPDATA", home, registry->appdata_dir, sizeof(registry->appdata_dir));
    registry->options.glab_config_dir = engine_agent_registry_env_path(
        "GLAB_CONFIG_DIR", home, registry->glab_config_dir, sizeof(registry->glab_config_dir));
    registry->options.kimi_code_home = engine_agent_registry_env_path(
        "KIMI_CODE_HOME", home, registry->kimi_code_home, sizeof(registry->kimi_code_home));
    registry->options.continue_config_path = engine_agent_registry_env_path(
        "ENGINE_CONTINUE_CONFIG_PATH", home, registry->continue_config_path,
        sizeof(registry->continue_config_path));
    registry->options.trae_config_path =
        engine_agent_registry_env_path("ENGINE_TRAE_CONFIG_PATH", home, registry->trae_config_path,
                                    sizeof(registry->trae_config_path));
    registry->options.roo_config_path = engine_agent_registry_env_path(
        "ENGINE_ROO_CONFIG_PATH", home, registry->roo_config_path, sizeof(registry->roo_config_path));
    registry->options.cody_config_path =
        engine_agent_registry_env_path("ENGINE_CODY_CONFIG_PATH", home, registry->cody_config_path,
...
#endif
    registry->options.path_exists = engine_agent_registry_path_exists;
    registry->options.command_exists = engine_agent_registry_command_exists;
    registry->options.probe_context = registry;
}

static void print_detected_registry_agents(const char *home, bool *any) {
    engine_agent_registry_context_t registry;
    engine_init_agent_registry_context(home, &registry);
    for (size_t index = 0U; index < engine_agent_client_count(); index++) {
        const engine_agent_client_profile_t *profile = engine_agent_client_at(index);
        if (profile && engine_agent_client_detect(profile->id, &registry.options)) {
...
static void engine_agent_installed_binary_path(const char *home, char *binary_path,
                                            size_t binary_path_size) {
#ifdef _WIN32
    snprintf(binary_path, binary_path_size, "%s/.local/bin/graph-engine.exe", home);
#else
    snprintf(binary_path, binary_path_size, "%s/.local/bin/graph-engine", home);
#endif
}
...
static void install_managed_agent_instructions(const char *label, const char *instructions_path,
    ...
    if (!dry_run &&
        (!prepare_config_parent(instructions_path) ||
         engine_upsert_instructions(instructions_path, agent_instructions_content) != CLI_OK)) {
        installed = false;
...
static void install_qoder_durable_context(const char *home, const char *binary_path,
    ...
    snprintf(agent_path, sizeof(agent_path), "%s/.qoder/agents/graph-engine.md", home);
    ...
    install_tiered_agent_profiles(
        (engine_tiered_profile_set_t){
            ...
            .dialect = ENGINE_GRAPH_DIALECT_QODER,
        },
        dry_run);
    bool hook_supported = engine_optional_hook_supported("qoder", engine_current_platform_is_windows());
    if (g_install_plan) {
        if (hook_supported) {
    ...
    if (!dry_run && (!prepare_config_parent(settings_path) ||
                     engine_upsert_qoder_context_hook(settings_path, binary_path) != CLI_OK)) {
        installed = false;
...
static bool engine_gitlab_hooks_path(const engine_agent_registry_context_t *registry, char *path,
                                  size_t path_size) {
...
static bool engine_devin_user_dir(const engine_agent_registry_context_t *registry, char *path,
                               size_t path_size) {
    const char *base = registry->options.home_dir;
...
static void install_gitlab_durable_context(const engine_agent_registry_context_t *registry,
                                           const char *binary_path, bool dry_run) {
    char hooks_path[CLI_BUF_1K];
    if (!engine_gitlab_hooks_path(registry, hooks_path, sizeof(hooks_path))) {
        record_agent_config_error(false, "GitLab Duo CLI", "hook_resolve", "hooks.json");
        return;
    }
    bool hook_supported = engine_optional_hook_supported("gitlab", registry->options.is_windows);
    if (g_install_plan) {
        if (hook_supported) {
    ...
    if (!dry_run && (!prepare_config_parent(hooks_path) ||
                     engine_upsert_gitlab_session_hook(hooks_path, binary_path) != CLI_OK)) {
        installed = false;
...
static void install_devin_durable_context(const engine_agent_registry_context_t *registry,
    ...
    char skills_dir[CLI_BUF_1K];
    if (!engine_devin_user_dir(registry, devin_dir, sizeof(devin_dir))) {
    ...
    bool hook_supported = engine_optional_hook_supported("devin", registry->options.is_windows);
    if (g_install_plan) {
        if (hook_supported) {
    ...
    if (!dry_run && ((inherit_claude_session &&
                      engine_remove_devin_session_hook(config_path, binary_path) != CLI_OK) ||
                     engine_upsert_devin_context_hooks(config_path, binary_path,
...
static void install_generated_client_extension(const char *label, const char *path,
    ...
    if (!dry_run && (!prepare_config_parent(path) ||
                     engine_text_upsert_managed_block(path, ENGINE_ADAPTER_MARKER_START,
                                                   ENGINE_ADAPTER_MARKER_END, content) != 0)) {
        installed = false;
...
static void uninstall_generated_client_extension(const char *label, const char *path,
                                                 bool dry_run) {
    if (!dry_run && engine_file_exists(path) &&
        engine_text_remove_managed_block(path, ENGINE_ADAPTER_MARKER_START, ENGINE_ADAPTER_MARKER_END) !=
            0) {
...
static void install_pi_durable_context(const char *home, const char *binary_path, bool force,
    ...
    snprintf(extension_path, sizeof(extension_path), "%s/.pi/agent/extensions/enginemem.ts", home);
    ...
    install_generated_client_extension("Pi", extension_path, binary_path, engine_client_adapter_pi,
                                       dry_run);
}

static void install_kimi_durable_context(const engine_agent_registry_context_t *registry,
    ...
    if (!dry_run && (!prepare_config_parent(config_path) ||
                     engine_upsert_kimi_context_hook(config_path, binary_path) != CLI_OK)) {
        installed = false;
...
static void install_rovo_durable_context(const char *home, bool force, bool dry_run) {
    ...
    snprintf(agent_path, sizeof(agent_path), "%s/.rovodev/subagents/graph-engine.md", home);
    ...
    install_tiered_agent_profiles(
        (engine_tiered_profile_set_t){
            ...
            .dialect = ENGINE_GRAPH_DIALECT_ROVO,
        },
        dry_run);
}
...
static void install_codebuddy_durable_context(const char *home, bool force, bool dry_run) {
    ...
    snprintf(agent_path, sizeof(agent_path), "%s/.codebuddy/agents/graph-engine.md", home);
    ...
    install_tiered_agent_profiles(
        (engine_tiered_profile_set_t){
            ...
            .dialect = ENGINE_GRAPH_DIALECT_CODEBUDDY,
        },
        dry_run);
}
...
static void install_bob_durable_context(const char *home, bool ide, bool force, bool dry_run) {
    ...
    snprintf(rules_path, sizeof(rules_path), "%s/.bob/rules/graph-engine.md", home);
...
static void install_pochi_durable_context(const char *home, bool force, bool dry_run) {
    ...
    snprintf(agent_path, sizeof(agent_path), "%s/.pochi/agents/graph-engine.md", home);
    ...
    install_tiered_agent_profiles(
        (engine_tiered_profile_set_t){
            ...
            .dialect = ENGINE_GRAPH_DIALECT_POCHI,
        },
        dry_run);
}
...
static void install_agent_client_registry(const char *home, const char *binary_path,
    ...
    for (size_t index = 0U; index < engine_agent_client_count(); index++) {
        ...
        if ((profile->capabilities & ENGINE_AGENT_CAP_MCP) != 0U) {
            int resolved = engine_agent_client_resolve_path(profile->id, &registry.options,
...
static void install_gemini_config(const char *home, const char *binary_path, bool dry_run) {
    ...
    snprintf(ap, sizeof(ap), "%s/.gemini/agents/graph-engine.md", home);
    install_generic_agent_config("Gemini CLI", binary_path, cp, ip, dry_run,
                                 engine_install_editor_mcp);
    install_tiered_agent_profiles(
        (engine_tiered_profile_set_t){
            .label = "Gemini CLI",
            .verify_path = ap,
            ...
            .dialect = ENGINE_GRAPH_DIALECT_GEMINI,
        },
        dry_run);
    if (g_install_plan) {
    ...
    if (!dry_run) {
        if (engine_upsert_gemini_hooks(cp) != CLI_OK) {
            record_agent_config_error(false, "Gemini CLI", "before_tool_hook_install", cp);
        }
#ifndef _WIN32
        if (engine_upsert_gemini_coverage_hook(cp, binary_path) != CLI_OK) {
            record_agent_config_error(false, "Gemini CLI", "after_tool_hook_install", cp);
        }
#endif
        if (engine_upsert_gemini_session_hooks(cp) != CLI_OK) {
...
static void install_cli_agent_configs(const engine_detected_agents_t *agents, const char *home,
                                      const char *binary_path, bool force, bool dry_run) {
    if (agents->codex) {
        char config_dir[CLI_BUF_1K];
        char cp[CLI_BUF_1K];
        ...
        install_tiered_agent_profiles(
            (engine_tiered_profile_set_t){
                ...
                .dialect = ENGINE_GRAPH_DIALECT_CODEX,
            },
            dry_run);
        ...
        bool use_hooks_json = engine_file_exists(hooks_json);
        ...
        if (g_install_plan) {
            ...
            if (!dry_run) {
                ...
                if (engine_build_augment_command(binary_path, command, sizeof(command)) == CLI_OK &&
                    engine_build_augment_command_windows(binary_path, command_windows,
                                                      sizeof(command_windows)) == CLI_OK) {
                    if (use_hooks_json) {
                        if (engine_upsert_paired_lifecycle_hooks_json(
                                hooks_json, command, command_windows, NULL, CMM_HOOK_TIMEOUT_SEC) ==
                            CLI_OK) {
                            if (engine_remove_codex_hooks(cp) != CLI_OK) {
                                hook_ok = false;
                        ...
                        if (engine_upsert_codex_hooks_command(cp, command, command_windows) !=
                            CLI_OK) {
                            hook_ok = false;
    ...
    if (agents->opencode) {
        ...
        char skills_dir[CLI_BUF_1K];
        char ap[CLI_BUF_1K];
        engine_opencode_config_path(home, cp, sizeof(cp));
        ...
        snprintf(ap, sizeof(ap), "%s/.config/opencode/agents/graph-engine.md", home);
        install_generic_agent_config("OpenCode", binary_path, cp, ip, dry_run,
                                     engine_upsert_opencode_mcp);
        ...
        install_tiered_agent_profiles(
            (engine_tiered_profile_set_t){
                ...
                .dialect = ENGINE_GRAPH_DIALECT_OPENCODE,
            },
            dry_run);
        ...
        snprintf(plugin_path, sizeof(plugin_path), "%s/.config/opencode/plugins/engine-augment.ts",
                 home);
        install_generated_client_extension("OpenCode", plugin_path, binary_path,
                                           engine_client_adapter_opencode, dry_run);
    }
    if (agents->antigravity) {
        ...
        if (!dry_run && !g_install_plan) {
            ...
            engine_mkdir_p(cfg_dir, CLI_OCTAL_PERM);
        }
        install_generic_agent_config("Antigravity", binary_path, cp, ip, dry_run,
                                     engine_upsert_antigravity_mcp);
        ...
        if (!dry_run && !g_install_plan) {
            ...
            if (engine_file_exists(legacy_settings) &&
                engine_remove_gemini_session_hooks(legacy_settings) != CLI_OK) {
    ...
    if (agents->aider) {
        ...
        if (g_install_plan) {
            ...
            if (!dry_run) {
                ...
                if (engine_upsert_instructions(ip, aider_instructions_content) != CLI_OK) {
                    record_agent_config_error(false, "Aider", "instructions_install", ip);
                }
                if (engine_yaml_upsert_string_list_item(cp, "read", ip) != CLI_OK) {
...
static void install_vscode_profile_configs(const char *code_user, const char *binary_path,
    ...
    engine_dir_t *d = engine_opendir(profiles_dir);
    if (!d) {
        return;
    }
    engine_dirent_t *ent;
...
static void uninstall_vscode_profile_configs(const char *code_user, const char *binary_path,
    ...
    char profiles_dir[CLI_BUF_1K];
    snprintf(profiles_dir, sizeof(profiles_dir), "%s/profiles", code_user);
    engine_dir_t *directory = engine_opendir(profiles_dir);
    if (!directory) {
        return;
    }
    engine_dirent_t *entry;
    while ((entry = engine_readdir(directory)) != NULL) {
        ...
        if (!dry_run && engine_remove_vscode_mcp_owned(binary_path, config_path) != CLI_OK) {
...
static void install_editor_agent_configs(const engine_detected_agents_t *agents, const char *home,
                                         const char *binary_path, bool force, bool dry_run) {
    if (agents->zed) {
        char config_dir[CLI_BUF_1K];
        char cp[CLI_BUF_1K];
        char ip[CLI_BUF_1K];
        char skills_dir[CLI_BUF_1K];
        engine_zed_config_dir(home, config_dir, sizeof(config_dir));
        engine_zed_instructions_path(home, ip, sizeof(ip));
        snprintf(cp, sizeof(cp), "%s/settings.json", config_dir);
        snprintf(skills_dir, sizeof(skills_dir), "%s/.agents/skills", home);
        install_generic_agent_config("Zed", binary_path, cp, ip, dry_run, engine_install_zed_mcp);
    ...
    if (agents->kilocode) {
        ...
        snprintf(ip, sizeof(ip), "%s/.config/kilo/rules/graph-engine.md", home);
        snprintf(ap, sizeof(ap), "%s/.config/kilo/agents/graph-engine.md", home);
        install_generic_agent_config("KiloCode", binary_path, cp, ip, dry_run, engine_upsert_kilo_mcp);
        install_tiered_agent_profiles(
            (engine_tiered_profile_set_t){
                .label = "KiloCode",
                .verify_path = ap,
                ...
                .dialect = ENGINE_GRAPH_DIALECT_KILO,
            },
            dry_run);
        if (!dry_run && !g_install_plan) {
            if (engine_json_like_add_unique_string(cp, "instructions", ip) != CLI_OK) {
            ...
            snprintf(legacy_cp, sizeof(legacy_cp),
                     ...
                     home);
#endif
            snprintf(legacy_ip, sizeof(legacy_ip), "%s/.kilocode/rules/graph-engine.md",
                     home);
            if (engine_file_exists(legacy_cp)) {
                if (engine_remove_editor_mcp_owned(binary_path, legacy_cp) != CLI_OK) {
                    record_agent_config_error(false, "KiloCode", "legacy_mcp_cleanup", legacy_cp);
                }
            }
            if (engine_file_exists(legacy_ip)) {
                if (engine_remove_instructions(legacy_ip) != CLI_OK) {
...
#else
        snprintf(code_user, sizeof(code_user), "%s/Code/User", engine_app_config_dir());
#endif
        char cp[CLI_BUF_1K];
    ...
    if (agents->cursor) {
        ...
        snprintf(ap, sizeof(ap), "%s/.cursor/agents/graph-engine.md", home);
        ...
        install_tiered_agent_profiles(
            (engine_tiered_profile_set_t){
                ...
                .dialect = ENGINE_GRAPH_DIALECT_CURSOR,
            },
            dry_run);
        ...
        if (!engine_openclaw_config_path(home, cp, sizeof(cp))) {
            ...
            if (workspace_ok) {
                ...
                if (g_install_plan) {
                    ...
                    if (!dry_run) {
                        if (engine_upsert_instructions(agents_path, agent_instructions_content) !=
                            CLI_OK) {
                            record_agent_config_error(false, "OpenClaw", "instructions_install",
                                                      agents_path);
                        }
                        if (engine_upsert_instructions(tools_path, agent_instructions_content) !=
                            CLI_OK) {
                            record_agent_config_error(false, "OpenClaw", "tools_context_install",
                                                      tools_path);
                        }
                        if (engine_upsert_openclaw_compaction(cp) != CLI_OK) {
    ...
    if (agents->kiro) {
        ...
        char skills_dir[CLI_BUF_1K];
        char ap[CLI_BUF_1K];
        engine_kiro_home_dir(home, kiro_home, sizeof(kiro_home));
        snprintf(cp, sizeof(cp), "%s/settings/mcp.json", kiro_home);
        snprintf(ip, sizeof(ip), "%s/steering/graph-engine.md", kiro_home);
        snprintf(skills_dir, sizeof(skills_dir), "%s/skills", kiro_home);
        snprintf(ap, sizeof(ap), "%s/agents/graph-engine.json", kiro_home);
        install_generic_agent_config("Kiro", binary_path, cp, ip, dry_run, engine_install_editor_mcp);
        install_agent_skill("Kiro", skills_dir, force, dry_run);
        char *legacy_agent_content = engine_build_legacy_kiro_verify_agent_content(binary_path);
        ...
        install_tiered_agent_profiles(
            (engine_tiered_profile_set_t){
                ...
                .dialect = ENGINE_GRAPH_DIALECT_KIRO,
            },
            dry_run);
    ...
    if (agents->junie) {
        ...
        snprintf(agent_path, sizeof(agent_path), "%s/.junie/agents/graph-engine.md", home);
        if (!dry_run && !g_install_plan) {
            engine_mkdir_p(sd, CLI_OCTAL_PERM);
        }
        bool direct_profiles_ready = install_generic_agent_config("Junie", binary_path, cp, NULL,
                                                                  dry_run, engine_upsert_junie_mcp);
        ...
        install_tiered_agent_profiles(
            (engine_tiered_profile_set_t){
                ...
                .dialect = ENGINE_GRAPH_DIALECT_JUNIE,
...
static void install_additional_agent_configs(const engine_detected_agents_t *agents, const char *home,
                                             const char *binary_path, bool force, bool dry_run) {
    if (agents->hermes) {
        char hermes_home[CLI_BUF_1K];
        char cp[CLI_BUF_1K];
        char skills_dir[CLI_BUF_1K];
        engine_hermes_home_dir(home, hermes_home, sizeof(hermes_home));
        ...
        install_generic_agent_config("Hermes", binary_path, cp, NULL, dry_run,
                                     engine_upsert_hermes_mcp);
        install_agent_skill("Hermes", skills_dir, force, dry_run);
        if (g_install_plan) {
            plan_record("Hermes", "hook", cp);
        } else {
            int hook_result = ENGINE_YAML_IDENTITY_EDIT_OK;
            if (!dry_run) {
                hook_result = prepare_config_parent(cp)
                                  ? engine_upsert_hermes_context_hook(cp, binary_path)
                                  : ENGINE_YAML_IDENTITY_EDIT_ERROR;
            }
            if (hook_result == ENGINE_YAML_IDENTITY_EDIT_FOREIGN) {
                record_agent_config_error(false, "Hermes", "pre_llm_hook_foreign", cp);
            } else if (hook_result != ENGINE_YAML_IDENTITY_EDIT_OK) {
    ...
    if (agents->augment) {
        ...
        snprintf(ip, sizeof(ip), "%s/.augment/rules/graph-engine.md", home);
        snprintf(ap, sizeof(ap), "%s/.augment/agents/graph-engine.md", home);
        ...
        install_tiered_agent_profiles(
            (engine_tiered_profile_set_t){
                ...
                .dialect = ENGINE_GRAPH_DIALECT_AUGMENT,
            },
            dry_run);
        if (g_install_plan) {
            ...
            bool hook_ok = true;
            if (!dry_run) {
                if (!engine_install_augment_session_script(binary_path, session_hp)) {
                    hook_ok = false;
                    record_agent_config_error(false, "Augment/Auggie", "session_script_install",
                                              session_hp);
                } else if (engine_upsert_augment_session_hook(cp, session_hp) != CLI_OK) {
                    hook_ok = false;
                    record_agent_config_error(false, "Augment/Auggie", "session_hook_install", cp);
                }
                if (!engine_install_augment_coverage_script(binary_path, coverage_hp)) {
                    hook_ok = false;
                    record_agent_config_error(false, "Augment/Auggie", "coverage_script_install",
                                              coverage_hp);
                } else if (engine_upsert_augment_coverage_hook(cp, coverage_hp) != CLI_OK) {
                    hook_ok = false;
    ...
    if (agents->cline) {
        ...
        char ip[CLI_BUF_1K];
        char skills_dir[CLI_BUF_1K];
        engine_cline_root_dir(home, cline_root, sizeof(cline_root));
        engine_cline_data_dir(home, cline_data, sizeof(cline_data));
        ...
        snprintf(ip, sizeof(ip), "%s/rules/graph-engine.md", cline_root);
    ...
    if (agents->qwen) {
        ...
        char skills_dir[CLI_BUF_1K];
        char ap[CLI_BUF_1K];
        engine_qwen_home_dir(home, qwen_home, sizeof(qwen_home));
        ...
        snprintf(ap, sizeof(ap), "%s/agents/graph-engine.md", qwen_home);
        ...
        install_tiered_agent_profiles(
            (engine_tiered_profile_set_t){
                ...
                .dialect = ENGINE_GRAPH_DIALECT_QWEN,
            },
            dry_run);
        if (g_install_plan) {
            ...
            bool windows = false;
#endif
            if (engine_upsert_qwen_lifecycle_hooks(cp, binary_path, windows) != CLI_OK) {
    ...
    if (agents->factory_droid) {
        ...
        snprintf(ap, sizeof(ap), "%s/.factory/droids/graph-engine.md", home);
        ...
        install_tiered_agent_profiles(
            (engine_tiered_profile_set_t){
                ...
                .dialect = ENGINE_GRAPH_DIALECT_FACTORY,
            },
            dry_run);
        bool hook_supported =
            engine_optional_hook_supported("factory", engine_current_platform_is_windows());
        if (g_install_plan) {
            if (hook_supported) {
            ...
            bool hook_ok = true;
            if (!dry_run) {
                if (engine_upsert_factory_hooks(hp, binary_path) != CLI_OK) {
                    hook_ok = false;
    ...
    if (agents->crush) {
        char cp[CLI_BUF_1K];
        char ip[CLI_BUF_1K];
        engine_crush_config_path(home, cp, sizeof(cp));
        snprintf(ip, sizeof(ip), "%s/.config/crush/graph-engine.md", home);
        install_generic_agent_config("Crush", binary_path, cp, NULL, dry_run, engine_upsert_crush_mcp);
        if (g_install_plan) {
            plan_record("Crush", "instructions", ip);
        } else {
            if (!dry_run) {
                if (engine_upsert_instructions(ip, crush_context_content) != CLI_OK) {
                    record_agent_config_error(false, "Crush", "task_context_install", ip);
                }
                if (engine_upsert_crush_context_path(cp, ip) != CLI_OK) {
    ...
    if (agents->goose) {
        ...
        char cp[CLI_BUF_1K];
        char ip[CLI_BUF_1K];
        engine_goose_config_dir(home, config_dir, sizeof(config_dir));
        snprintf(cp, sizeof(cp), "%s/config.yaml", config_dir);
        snprintf(ip, sizeof(ip), "%s/.config/goose/.goosehints", home);
        install_generic_agent_config("Goose", binary_path, cp, ip, dry_run, engine_upsert_goose_mcp);
    }
    if (agents->mistral_vibe) {
        ...
        engine_vibe_config_dir(home, config_dir, sizeof(config_dir));
        snprintf(cp, sizeof(cp), "%s/config.toml", config_dir);
        snprintf(ip, sizeof(ip), "%s/AGENTS.md", config_dir);
        snprintf(skills_dir, sizeof(skills_dir), "%s/skills", config_dir);
        snprintf(ap, sizeof(ap), "%s/agents/graph-engine.toml", config_dir);
        snprintf(prompt_path, sizeof(prompt_path), "%s/prompts/graph-engine.md", config_dir);
        install_generic_agent_config("Mistral Vibe", binary_path, cp, ip, dry_run,
                                     engine_upsert_vibe_mcp);
        ...
        install_tiered_agent_profiles(
            (engine_tiered_profile_set_t){
                ...
                .dialect = ENGINE_GRAPH_DIALECT_VIBE,
            },
            dry_run);
        install_tiered_profile_prompts("Mistral Vibe", prompt_path, ENGINE_GRAPH_DIALECT_VIBE,
                                       legacy_vibe_verify_prompt_content, dry_run);
    }
}

int engine_install_agent_configs(const char *home, const char *binary_path, bool force, bool dry_run) {
    g_agent_install_errors = 0;
    engine_detected_agents_t agents = engine_detect_agents(home);
    if (!g_install_plan) {
    ...
    bool inherit_claude_session =
        agents.claude_code && !dry_run && engine_has_complete_claude_session_hooks(home);
...
static int engine_install_agent_configs_with_previous(const char *home, const char *binary_path,
                                                   const char *previous_managed_binary_path,
                                                   bool force, bool dry_run) {
    const char *saved = g_previous_managed_mcp_command;
    g_previous_managed_mcp_command = previous_managed_binary_path;
    int result = engine_install_agent_configs(home, binary_path, force, dry_run);
    g_previous_managed_mcp_command = saved;
    return result;
}
 ...
 */
int engine_install_handle_existing_indexes(const char *home, bool reset, bool dry_run);
static int engine_install_prepare_existing_indexes(const char *home, bool reset, bool dry_run,
...
int engine_install_handle_existing_indexes(const char *home, bool reset, bool dry_run) {
    ...
    int prepare_result =
        engine_install_prepare_existing_indexes(home, reset, dry_run, &delete_indexes);
    if (prepare_result == 1 && delete_indexes) {
        int removed = engine_remove_indexes(home);
...
static bool engine_detect_self_path(char *buf, size_t buf_sz, const char *home) {
    buf[0] = '\0';
    bool exact = false;
#ifdef _WIN32
    char *module_path = engine_module_path_utf8();
    size_t length = module_path ? strlen(module_path) : 0U;
    exact = module_path && length > 0U && length < buf_sz;
    if (exact) {
        memcpy(buf, module_path, length + 1U);
        engine_normalize_path_sep(buf);
    } else {
        buf[0] = '\0';
    }
    free(module_path);
...
#ifdef _WIN32
        snprintf(buf, buf_sz, "%s/.local/bin/graph-engine.exe", home);
#else
        snprintf(buf, buf_sz, "%s/.local/bin/graph-engine", home);
#endif
    }
    return exact;
}
...
static bool cli_runtime_asset_regular_leaf(const char *path) {
    ...
    return path && engine_path_info_utf8(path, &info) == 0 && info.is_regular && !info.is_symlink;
}

bool engine_cli_verify_runtime_assets_at(const char *binary_path, char *err, size_t err_sz) {
    if (!binary_path || !binary_path[0]) {
        if (err && err_sz > 0U) {
    ...
    *separator = '\0';
    ...
    int integration_length =
        snprintf(integration, sizeof(integration), "%s/%s", directory, ENGINE_INTEGRATIONS_ASSET_NAME);
    if (integration_length <= 0 || (size_t)integration_length >= sizeof(integration) ||
        !cli_runtime_asset_regular_leaf(integration) ||
        !engine_integration_assets_verify_file(integration)) {
        if (err && err_sz > 0U) {
            (void)snprintf(err, err_sz, "adjacent integration assets do not match this binary");
        }
        return false;
    }
    if (engine_ui_assets_supported()) {
        char pack[CLI_BUF_4K];
        int pack_length =
            snprintf(pack, sizeof(pack), "%s/%s", directory, engine_ui_assets_current_pack_name());
        if (pack_length <= 0 || (size_t)pack_length >= sizeof(pack) ||
            !cli_runtime_asset_regular_leaf(pack) || !engine_ui_assets_verify_file(pack)) {
            if (err && err_sz > 0U) {
...
int engine_cmd_verify_runtime_assets(void) {
    const char *home = engine_get_home_dir();
    char self_path[CLI_BUF_4K] = {0};
    char err[CLI_BUF_1K] = {0};
    if (!engine_detect_self_path(self_path, sizeof(self_path), home) ||
        !engine_cli_verify_runtime_assets_at(self_path, err, sizeof(err))) {
...
static char *engine_build_install_plan_json_options(const char *home, const char *binary_path,
                                                 bool skip_config) {
    if (!home || !binary_path) {
        return NULL;
    }
    ...
    engine_install_plan_t plan = {0};
    if (!skip_config) {
        g_install_plan = &plan;
        engine_install_agent_configs(home, binary_path, false, true);
        g_install_plan = NULL;
    }

    engine_detected_agents_t det = engine_detect_agents(home);
    struct {
        bool flag;
        const char *name;
    } names[] = {
    ...
    for (int i = 0; i < plan.count; i++) {
        engine_plan_entry_t *e = &plan.items[i];
    ...
    yyjson_mut_obj_add_str(doc, root, "next_safe_command", "graph-engine install -y");
...
char *engine_build_install_plan_json(const char *home, const char *binary_path) {
    return engine_build_install_plan_json_options(home, binary_path, false);
}

static bool cli_ui_pack_filename_digest(const char *name,
                                        char digest[ENGINE_SHA256_HEX_LEN + CLI_SKIP_ONE]) {
    static const char prefix[] = "engine-ui-";
    ...
    size_t expected_length = prefix_length + ENGINE_SHA256_HEX_LEN + suffix_length;
    ...
    for (size_t index = 0U; index < ENGINE_SHA256_HEX_LEN; index++) {
    ...
    digest[ENGINE_SHA256_HEX_LEN] = '\0';
    return true;
}
...
static void cli_gc_stale_ui_asset_packs(const char *install_dir) {
    ...
    const char *current_pack = engine_ui_assets_current_pack_name();
    engine_dir_t *directory = engine_opendir(install_dir);
    if (!directory) {
        return;
    }
    engine_dirent_t *entry;
    while ((entry = engine_readdir(directory)) != NULL) {
        char expected_digest[ENGINE_SHA256_HEX_LEN + CLI_SKIP_ONE];
        ...
        engine_path_info_t path_info;
        if (engine_path_info_utf8(path, &path_info) != 0 || !path_info.is_regular ||
            path_info.is_symlink) {
            continue;
        }
        engine_activation_transaction_t *removal = NULL;
        engine_activation_transaction_status_t status =
            engine_activation_transaction_stage_removal(path, &removal);
        if (status != ENGINE_ACTIVATION_TRANSACTION_OK || !removal) {
            (void)fprintf(stderr, "note: retained stale UI asset pack %s (cannot stage)\n",
                          entry->name);
            if (removal) {
                (void)engine_activation_transaction_close(&removal);
            }
            continue;
        }
        status = engine_activation_transaction_commit(removal, NULL, NULL);
        const char *retained = engine_activation_transaction_backup_path(removal);
        char actual_digest[ENGINE_SHA256_HEX_LEN + CLI_SKIP_ONE];
        bool owned =
            status == ENGINE_ACTIVATION_TRANSACTION_OK && retained &&
            engine_cli_sha256_file(retained, actual_digest, sizeof(actual_digest)) == CLI_OK &&
            strcmp(actual_digest, expected_digest) == 0;
        if (!owned) {
            if (status == ENGINE_ACTIVATION_TRANSACTION_OK &&
                engine_activation_transaction_rollback(removal) != ENGINE_ACTIVATION_TRANSACTION_OK) {
        ...
        status = engine_activation_transaction_finalize(removal);
        if (status != ENGINE_ACTIVATION_TRANSACTION_OK &&
            status != ENGINE_ACTIVATION_TRANSACTION_DEFERRED) {
...
typedef struct {
    ...
    engine_activation_transaction_t *binary_transaction;
    engine_activation_transaction_t *integration_adjacent_transaction;
    engine_activation_transaction_t *integration_ownership_transaction;
    engine_activation_transaction_t *ui_pack_transaction;
...
static bool cli_install_commit_integration_asset(engine_activation_transaction_t *transaction) {
    return !transaction ||
           engine_integration_assets_commit_install(transaction) == ENGINE_ACTIVATION_TRANSACTION_OK;
}

static bool cli_install_commit_ui_asset(engine_activation_transaction_t *transaction) {
    return !transaction ||
           engine_ui_assets_commit_install(transaction) == ENGINE_ACTIVATION_TRANSACTION_OK;
}
...
static int cli_install_activate(void *opaque) {
    ...
    bool previous_managed_binary_exact = engine_detect_self_path(
    ...
    if (!activation->dry_run &&
        ...
        if (!engine_mkdir_p(activation->bin_dir, CLI_OCTAL_PERM)) {
            (void)fprintf(stderr, "error: cannot create install directory %s\n",
                          activation->bin_dir);
            return CLI_TRUE;
        }
        engine_activation_transaction_t *pre_guard_transaction = activation->binary_transaction;
        const char *restage_source =
            activation->prepared_candidate
                ? activation->prepared_candidate
                : engine_activation_transaction_staged_path(pre_guard_transaction);
        engine_activation_transaction_t *guarded_transaction = NULL;
        engine_activation_transaction_status_t stage_status = engine_activation_transaction_stage_file(
            activation->bin_target, restage_source, &guarded_transaction);
        cli_binary_validator_t restaged_validator = {{0}};
        if (stage_status != ENGINE_ACTIVATION_TRANSACTION_OK || !guarded_transaction ||
    ...
    if (!activation->dry_run && activation->publish_runtime_assets) {
        ...
        if (!engine_integration_assets_stage_install(activation->home, activation->bin_dir,
                                                  &activation->integration_adjacent_transaction,
                                                  &activation->integration_ownership_transaction,
                                                  asset_error, sizeof(asset_error)) ||
            !engine_ui_assets_stage_install(activation->bin_dir, &activation->ui_pack_transaction,
                                         asset_error, sizeof(asset_error))) {
    ...
    if (!activation->skip_config) {
        agent_config_rc = engine_install_agent_configs_with_previous(
    ...
    if (activation->shell_rc[0]) {
        path_rc = engine_ensure_path(activation->bin_dir, activation->shell_rc, activation->dry_run);
        if (path_rc == 0) {
    ...
    if (!activation->dry_run && activation->delete_indexes) {
        ...
        int removed = engine_remove_indexes(activation->home);
        printf("Removed %d index(es).\n\n", removed);
        if (removed != expected) {
...
int engine_cmd_install(int argc, char **argv) {
    ...
    engine_normalize_path_sep(bin_dir);
    char bin_target[CLI_BUF_1K];
#ifdef _WIN32
    int target_length =
        snprintf(bin_target, sizeof(bin_target), "%s/graph-engine.exe", bin_dir);
#else
    int target_length = snprintf(bin_target, sizeof(bin_target), "%s/graph-engine", bin_dir);
#endif
    ...
    if (plan) {
        char *json = engine_build_install_plan_json_options(home, bin_target, skip_config);
        if (!json) {
    ...
    (void)engine_detect_self_path(self_path, sizeof(self_path), home);
    engine_ui_assets_set_binary_path(self_path);

    printf("graph-engine install %s\n\n", ENGINE_VERSION);

    engine_path_info_t target_status;
    bool target_exists = engine_path_info_utf8(bin_target, &target_status) == 0;
    bool same_binary = engine_same_file(self_path, bin_target);
    ...
    bool delete_indexes = false;
    if (engine_install_prepare_existing_indexes(home, reset_indexes, dry_run, &delete_indexes) == 0) {
        return CLI_TRUE;
    }
     ...
     * actually consume, after the replacement decision is final. */
    {
        ...
        if ((publish_runtime_assets || !skip_config) &&
            !engine_integration_assets_require(home, assets_error, sizeof(assets_error))) {
            (void)fprintf(stderr, "error: %s\n", assets_error);
            return CLI_TRUE;
        }
        if (publish_runtime_assets &&
            !engine_ui_assets_warm(home, assets_error, sizeof(assets_error))) {
            (void)fprintf(stderr, "error: %s\n", assets_error);
            return CLI_TRUE;
        }
    }
    ...
    bool prepare_binary = do_copy;
#endif
    engine_activation_transaction_t *binary_transaction = NULL;
    ...
    if (!dry_run && prepare_binary) {
        ...
        int dir_length =
            snprintf(prepared_dir, sizeof(prepared_dir), "%s/engine-install-XXXXXX", engine_tmpdir());
        if (dir_length <= 0 || (size_t)dir_length >= sizeof(prepared_dir) ||
            !engine_mkdtemp(prepared_dir)) {
        ...
        int candidate_length = snprintf(prepared_candidate, sizeof(prepared_candidate),
                                        "%s/graph-engine.exe", prepared_dir);
#else
        int candidate_length = snprintf(prepared_candidate, sizeof(prepared_candidate),
                                        "%s/graph-engine", prepared_dir);
#endif
        if (candidate_length <= 0 || (size_t)candidate_length >= sizeof(prepared_candidate)) {
            (void)engine_rmdir(prepared_dir);
        ...
        if (stage_status != ENGINE_ACTIVATION_TRANSACTION_OK || !binary_transaction ||
            !cli_activation_transaction_expected_build(binary_transaction, &staged_validator)) {
            const char *stage_refusal = engine_activation_transaction_refusal_note();
            (void)fprintf(stderr, "error: failed to stage install candidate: %s%s%s\n",
                          engine_activation_transaction_status_message(stage_status),
        ...
        if (cli_activation_transaction_commit_validated(binary_transaction, &staged_validator,
            ...
            (void)cli_activation_transaction_abort(&binary_transaction);
            (void)engine_unlink(prepared_candidate);
            (void)engine_rmdir(prepared_dir);
            (void)fprintf(stderr, "error: private install candidate preparation failed\n");
            return CLI_TRUE;
        }
#ifdef __APPLE__
        if (sign_binary && engine_macos_adhoc_sign(prepared_candidate) != 0) {
        ...
        has_binary_validator =
            engine_daemon_build_fingerprint_file(prepared_candidate, binary_validator.fingerprint);
        if (has_binary_validator && !g_cli_activation_test_ops_set) {
            const char *candidate_argv[] = {prepared_candidate, "--version", NULL};
            has_binary_validator = engine_exec_no_shell(candidate_argv) == CLI_OK;
        }
...
#ifndef _WIN32
    snprintf(shell_rc, sizeof(shell_rc), "%s", engine_detect_shell_rc(home));
#endif
    ...
    int activation_rc =
        dry_run ? cli_install_activate(&activation)
                : cli_activation_guard(ENGINE_DAEMON_RUNTIME_ACTIVATION_INSTALL, ENGINE_VERSION,
...
static void uninstall_claude_code(const char *home, bool dry_run) {
    ...
    engine_agent_installed_binary_path(home, installed_binary, sizeof(installed_binary));
    char config_dir[CLI_BUF_1K];
    engine_claude_config_dir(home, config_dir, sizeof(config_dir));
    char user_root[CLI_BUF_1K];
    engine_claude_user_root(home, user_root, sizeof(user_root));
    ...
    char skills_dir[CLI_BUF_1K];
    snprintf(skills_dir, sizeof(skills_dir), "%s/skills", config_dir);
    int removed = engine_remove_skills(skills_dir, dry_run);
    ...
    if (!dry_run && engine_remove_editor_mcp_owned(installed_binary, mcp_path) != CLI_OK) {
    ...
    char mcp_path2[CLI_BUF_1K];
    snprintf(mcp_path2, sizeof(mcp_path2), "%s/.claude.json", user_root);
    if (!dry_run && engine_remove_editor_mcp_owned(installed_binary, mcp_path2) != CLI_OK) {
    ...
    snprintf(settings_path, sizeof(settings_path), "%s/settings.json", config_dir);
    if (!dry_run) {
        if (engine_remove_claude_hooks(settings_path) != CLI_OK) {
            record_agent_config_error(true, "Claude Code", "pretool_hook_uninstall", settings_path);
        }
        if (engine_remove_session_hooks(settings_path) != CLI_OK) {
            record_agent_config_error(true, "Claude Code", "session_hook_uninstall", settings_path);
        }
        if (engine_remove_claude_subagent_hooks(settings_path) != CLI_OK) {
        ...
        for (size_t i = 0; i < sizeof(hook_types) / sizeof(hook_types[0]); i++) {
            ...
            const engine_integration_template_t *tpl =
                engine_integration_template(hook_types[i].template_id);
            char current[CLI_BUF_8K];
            if (!tpl || engine_render_integration_body(&tpl->current, installed_binary, NULL, current,
            ...
            char released_storage[CMM_MAX_RELEASED_SCRIPTS][CLI_BUF_8K];
            const char *legacy[CMM_MAX_RELEASED_SCRIPTS];
            size_t legacy_count = engine_render_released_scripts(
                tpl, installed_binary, released_storage, legacy, CMM_MAX_RELEASED_SCRIPTS);
            if (!script_path_valid ||
                engine_remove_owned_hook_script(script_path, current, legacy, legacy_count) < CLI_OK) {
                record_agent_config_error(true, "Claude Code", "hook_script_uninstall",
                                          script_path_valid ? script_path : hook_types[i].name);
            }
#ifdef _WIN32
            if (!hooks_dir_valid ||
                engine_remove_owned_legacy_hook_script(hooks_dir, hook_types[i].legacy_name, current,
                                                    legacy, legacy_count) != CLI_OK) {
                char legacy_path[CLI_BUF_1K];
...
static void uninstall_agent_mcp_instr(mcp_uninstall_args_t paths, bool dry_run,
    ...
    if (!dry_run) {
        ...
        engine_agent_installed_binary_path(engine_get_home_dir(), binary_path, sizeof(binary_path));
    ...
    if (instr_path) {
        if (!dry_run) {
            if (engine_remove_instructions(instr_path) != CLI_OK) {
...
static void uninstall_copilot_durable_context(const char *home, bool dry_run) {
    ...
    snprintf(skills_dir, sizeof(skills_dir), "%s/skills", config_dir);
    snprintf(agent_path, sizeof(agent_path), "%s/agents/graph-engine.agent.md", config_dir);
    engine_agent_installed_binary_path(home, binary_path, sizeof(binary_path));
    if (!dry_run && engine_remove_copilot_hooks(hook_path, binary_path) != CLI_OK) {
...
static int engine_remove_managed_instructions(const char *instructions_path) {
    if (engine_remove_instructions(instructions_path) != CLI_OK) {
        return CLI_ERR;
    }
    struct stat state;
#ifndef _WIN32
    if (lstat(instructions_path, &state) == 0 && S_ISREG(state.st_mode) && state.st_size == 0 &&
        engine_unlink(instructions_path) != 0) {
#else
...
static void uninstall_qoder_durable_context(const char *home, const char *binary_path,
    ...
    if (!dry_run && config_resolved &&
        engine_remove_qoder_context_hook(settings_path, binary_path) != CLI_OK) {
        cleanup_ok = false;
...
static void uninstall_gitlab_durable_context(const engine_agent_registry_context_t *registry,
                                             const char *binary_path, bool dry_run) {
    char hooks_path[CLI_BUF_1K];
    if (!engine_gitlab_hooks_path(registry, hooks_path, sizeof(hooks_path))) {
    ...
    if (!dry_run && engine_remove_gitlab_session_hook(hooks_path, binary_path) != CLI_OK) {
...
static void uninstall_devin_durable_context(const engine_agent_registry_context_t *registry,
                                            const char *binary_path, const char *config_path,
    ...
    bool cleanup_ok = true;
    if (!dry_run && config_resolved &&
        engine_remove_devin_context_hooks(config_path, binary_path) != CLI_OK) {
        cleanup_ok = false;
...
static void uninstall_pi_durable_context(const char *home, bool dry_run) {
    ...
    snprintf(instructions_path, sizeof(instructions_path), "%s/.pi/agent/AGENTS.md", home);
    snprintf(skills_dir, sizeof(skills_dir), "%s/.pi/agent/skills", home);
    if (!dry_run && engine_remove_managed_instructions(instructions_path) != CLI_OK) {
...
static bool remove_cline_context_hooks(const char *cline_root, const char *binary_path,
    ...
    for (size_t i = 0U; i < sizeof(cmm_cline_context_events) / sizeof(cmm_cline_context_events[0]);
        ...
        char hook_path[CLI_BUF_1K];
        char script[CLI_BUF_8K];
        if (engine_cline_hook_path(cline_root, cmm_cline_context_events[i], hook_path,
                                sizeof(hook_path)) != CLI_OK ||
            engine_build_cline_context_script(binary_path, cmm_cline_context_events[i], script,
                                           sizeof(script)) != CLI_OK) {
            ok = false;
        ...
        int result = engine_text_remove_owned_document(hook_path, script);
        if (result < 0) {
            ok = false;
...
static void uninstall_kimi_durable_context(const engine_agent_registry_context_t *registry,
                                           bool dry_run) {
    const char *kimi_home = registry->options.kimi_code_home;
    char default_home[CLI_BUF_1K];
    if (!kimi_home || !kimi_home[0]) {
    ...
    snprintf(skills_dir, sizeof(skills_dir), "%s/skills", kimi_home);
    snprintf(config_path, sizeof(config_path), "%s/config.toml", kimi_home);
    if (!dry_run && engine_remove_kimi_context_hook(config_path) != CLI_OK) {
...
static void uninstall_agent_client_registry(const char *home, bool dry_run) {
    ...
    for (size_t index = 0U; index < engine_agent_client_count(); index++) {
        const engine_agent_client_profile_t *profile = engine_agent_client_at(index);
        if (!profile || !engine_agent_client_cleanup_candidate(profile->id, &registry.options)) {
            continue;
        }
        printf("%s:\n", profile->display_name);
        char config_path[CLI_BUF_1K] = {0};
...
static void uninstall_cli_agents(const engine_detected_agents_t *agents, const char *home,
                                 bool dry_run) {
    if (agents->codex) {
        char config_dir[CLI_BUF_1K];
        char cp[CLI_BUF_1K];
        ...
        if (!dry_run) {
            ...
            snprintf(hooks_json, sizeof(hooks_json), "%s/hooks.json", config_dir);
            if (engine_file_exists(hooks_json) &&
                (engine_build_augment_command(installed_binary, hook_command, sizeof(hook_command)) !=
                     CLI_OK ||
                 engine_remove_paired_lifecycle_hooks_json(hooks_json, hook_command) != CLI_OK)) {
    ...
    if (agents->opencode) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"OpenCode", cp, ip}, dry_run,
                                  engine_remove_opencode_mcp_owned);
    ...
    if (agents->antigravity) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"Antigravity", cp, ip}, dry_run,
                                  engine_remove_antigravity_mcp_owned);
        if (!dry_run) {
            char sp[CLI_BUF_1K];
            snprintf(sp, sizeof(sp), "%s/.gemini/antigravity-cli/settings.json", home);
            if (engine_remove_gemini_session_hooks(sp) != CLI_OK) {
    ...
    if (agents->aider) {
        ...
        snprintf(ip, sizeof(ip), "%s/CONVENTIONS.md", home);
        if (!dry_run) {
            if (engine_yaml_remove_string_list_item(cp, "read", ip) != CLI_OK) {
...
static void uninstall_editor_agents(const engine_detected_agents_t *agents, const char *home,
                                    bool dry_run) {
    char installed_binary[CLI_BUF_1K];
    engine_agent_installed_binary_path(home, installed_binary, sizeof(installed_binary));
    if (agents->zed) {
    ...
    if (agents->kilocode) {
        ...
        snprintf(ap, sizeof(ap), "%s/.config/kilo/agents/graph-engine.md", home);
        if (!dry_run) {
            if (engine_remove_kilo_mcp_owned(installed_binary, cp) != CLI_OK) {
                record_agent_config_error(true, "KiloCode", "mcp_uninstall", cp);
            }
            if (engine_json_like_remove_string(cp, "instructions", ip) != CLI_OK) {
            ...
            snprintf(legacy_ip, sizeof(legacy_ip), "%s/.kilocode/rules/graph-engine.md",
                     home);
            if (engine_file_exists(legacy_cp) &&
                engine_remove_editor_mcp_owned(installed_binary, legacy_cp) != CLI_OK) {
                record_agent_config_error(true, "KiloCode", "legacy_mcp_uninstall", legacy_cp);
            }
            if (engine_file_exists(legacy_ip) && engine_remove_instructions(legacy_ip) != CLI_OK) {
    ...
    if (agents->vscode) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"VS Code", cp, NULL}, dry_run,
                                  engine_remove_vscode_mcp_owned);
    ...
    if (agents->cursor) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"Cursor", cp, NULL}, dry_run,
                                  engine_remove_editor_mcp_owned);
    ...
    if (agents->openclaw) {
        ...
        if (engine_openclaw_config_path(home, cp, sizeof(cp))) {
            ...
            uninstall_agent_mcp_instr((mcp_uninstall_args_t){"OpenClaw", cp, NULL}, dry_run,
                                      engine_remove_openclaw_mcp_owned);
            if (!dry_run && engine_remove_openclaw_compaction(cp) != CLI_OK) {
            ...
            if (workspace_ok) {
                ...
                snprintf(tools_path, sizeof(tools_path), "%s/TOOLS.md", workspace);
                if (!dry_run) {
                    if (engine_remove_instructions(agents_path) != CLI_OK) {
                        record_agent_config_error(true, "OpenClaw", "instructions_uninstall",
                                                  agents_path);
                    }
                    if (engine_remove_instructions(tools_path) != CLI_OK) {
    ...
    if (agents->kiro) {
        ...
        char *legacy_agent_content = engine_build_legacy_kiro_verify_agent_content(installed_binary);
        if (!legacy_agent_content) {
...
static void uninstall_additional_agents(const engine_detected_agents_t *agents, const char *home,
                                        bool dry_run) {
    char installed_binary[CLI_BUF_1K];
    engine_agent_installed_binary_path(home, installed_binary, sizeof(installed_binary));
    if (agents->hermes) {
        ...
        engine_agent_installed_binary_path(home, binary_path, sizeof(binary_path));
        int hook_result =
            dry_run ? ENGINE_YAML_IDENTITY_EDIT_OK : engine_remove_hermes_context_hook(cp, binary_path);
        if (hook_result == ENGINE_YAML_IDENTITY_EDIT_FOREIGN) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"Hermes", cp, NULL}, dry_run,
                                  engine_remove_hermes_mcp_owned);
    ...
    if (agents->openhands) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"OpenHands", cp, NULL}, dry_run,
                                  engine_remove_editor_mcp_owned);
        printf("  removed %d skill(s)\n", engine_remove_skills(skills_dir, dry_run));
    }
    if (agents->augment) {
        char cp[CLI_BUF_1K];
        char ip[CLI_BUF_1K];
        ...
        if (!dry_run) {
            ...
            if (engine_build_augment_coverage_script(binary_path, coverage_script,
                                                  sizeof(coverage_script)) != CLI_OK) {
                record_agent_config_error(true, "Augment/Auggie", "coverage_script_build",
                                          coverage_hp);
            } else {
                int script_rc = engine_text_remove_owned_document(coverage_hp, coverage_script);
                if (script_rc < 0) {
    ...
    if (agents->qwen) {
        ...
        if (!dry_run) {
            ...
            bool windows = false;
#endif
            if (engine_remove_qwen_lifecycle_hooks(cp, installed_binary, windows) != CLI_OK) {
    ...
    if (agents->copilot_cli) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"Copilot CLI", cp, ip}, dry_run,
                                  engine_remove_copilot_mcp_owned);
    }
    if (agents->vscode || agents->copilot_cli) {
    ...
    if (agents->factory_droid) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"Factory Droid", cp, ip}, dry_run,
                                  engine_remove_factory_mcp_owned);
        if (!dry_run && engine_remove_factory_hooks(hp, installed_binary) != CLI_OK) {
    ...
    if (agents->crush) {
        ...
        engine_crush_config_path(home, cp, sizeof(cp));
        snprintf(ip, sizeof(ip), "%s/.config/crush/graph-engine.md", home);
        if (!dry_run && engine_remove_crush_context_path(cp, ip) != CLI_OK) {
        ...
        uninstall_agent_mcp_instr((mcp_uninstall_args_t){"Crush", cp, ip}, dry_run,
                                  engine_remove_crush_mcp_owned);
        char legacy_ip[CLI_BUF_1K];
        snprintf(legacy_ip, sizeof(legacy_ip), "%s/.config/crush/CRUSH.md", home);
        if (!dry_run && engine_file_exists(legacy_ip) &&
            engine_remove_instructions(legacy_ip) != CLI_OK) {
    ...
    if (agents->mistral_vibe) {
        ...
        uninstall_tiered_agent_profiles(
            ...
            dry_run);
        uninstall_tiered_profile_prompts("Mistral Vibe", prompt_path, ENGINE_GRAPH_DIALECT_VIBE,
                                         legacy_vibe_verify_prompt_content, dry_run);
    }
}
...
static bool cli_uninstall_commit_removal(engine_activation_transaction_t *transaction) {
    return !transaction || engine_activation_transaction_commit(transaction, NULL, NULL) ==
                               ENGINE_ACTIVATION_TRANSACTION_OK;
}

static bool cli_uninstall_commit_integration_removal(engine_activation_transaction_t *transaction) {
    return !transaction ||
           engine_integration_assets_commit_removal(transaction) == ENGINE_ACTIVATION_TRANSACTION_OK;
}

static bool cli_uninstall_commit_ui_removal(engine_activation_transaction_t *transaction) {
    return !transaction ||
           engine_ui_assets_commit_removal(transaction) == ENGINE_ACTIVATION_TRANSACTION_OK;
}
...
static int cli_uninstall_activate(void *opaque) {
    ...
    if (!activation->dry_run) {
        engine_path_info_t binary_info;
        activation->binary_exists = engine_path_info_utf8(activation->bin_path, &binary_info) == 0;
        if (activation->binary_exists) {
            engine_activation_transaction_status_t status = engine_activation_transaction_stage_removal(
                activation->bin_path, &activation->binary_transaction);
            if (status != ENGINE_ACTIVATION_TRANSACTION_OK || !activation->binary_transaction) {
                ...
                (void)fprintf(stderr, "error: failed to stage uninstall transaction: %s\n",
                              engine_activation_transaction_status_message(status));
                return CLI_ACTIVATION_PARTIAL;
            }
        }
        char asset_error[CLI_BUF_1K] = {0};
        if (!engine_integration_assets_stage_remove(activation->home, activation->bin_dir,
                                                 &activation->integration_adjacent_transaction,
                                                 &activation->integration_ownership_transaction,
                                                 &activation->foreign_integration_preserved,
                                                 asset_error, sizeof(asset_error)) ||
            !engine_ui_assets_stage_remove(activation->bin_dir, &activation->ui_pack_transaction,
    ...
    if (activation->delete_indexes && !activation->dry_run) {
        int expected = count_db_indexes(activation->home);
        int idx_removed = engine_remove_indexes(activation->home);
        printf("Removed %d index(es).\n", idx_removed);
    ...
    if (activation->foreign_integration_preserved) {
        (void)fprintf(stderr, "note: a modified or foreign %s was preserved\n",
                      ENGINE_INTEGRATIONS_ASSET_NAME);
    }
...
int engine_cmd_uninstall(int argc, char **argv) {
    parse_auto_answer(argc, argv);
    bool dry_run = false;
    const char *requested_bin_dir = NULL;
    for (int i = 0; i < argc; i++) {
    ...
    {
        char assets_error[CLI_BUF_1K];
        if (!engine_integration_assets_require(home, assets_error, sizeof(assets_error))) {
...
#ifdef _WIN32
            use_self_path = strcmp(base, "graph-engine.exe") == 0;
#else
            use_self_path = strcmp(base, "graph-engine") == 0;
#endif
            if (use_self_path) {
                int path_length =
    ...
    if (!use_self_path) {
        ...
        engine_normalize_path_sep(bin_dir_storage);
#ifdef _WIN32
        int target_length = snprintf(bin_path_storage, sizeof(bin_path_storage),
                                     "%s/graph-engine.exe", bin_dir_storage);
#else
        int target_length = snprintf(bin_path_storage, sizeof(bin_path_storage),
                                     "%s/graph-engine", bin_dir_storage);
#endif
    ...
    engine_path_info_t binary_info;
    bool binary_exists = engine_path_info_utf8(bin_path_storage, &binary_info) == 0;
    ...
    if (dry_run) {
        ...
        activation_rc = cli_activation_guard(ENGINE_DAEMON_RUNTIME_ACTIVATION_UNINSTALL, NULL, NULL,
...
static int cli_update_activate_binary(void *opaque) {
     ...
     * exact-build activation window. Otherwise another engine process can start
     * after the binary swap but before its MCP/hook entries are refreshed. */
    printf("Refreshing agent configurations...\n");
    if (engine_install_agent_configs_with_previous(
...
static int extract_and_install_binary(extract_install_args_t args) {
    ...
    if (strcmp(ext, "tar.gz") == 0) {
        bin_data = engine_extract_binary_from_targz(data, (int)fsize, &bin_len);
    } else {
        bin_data = engine_extract_binary_from_zip(data, (int)fsize, &bin_len);
    }
    free(data);
    engine_unlink(tmp_archive);
    ...
    engine_activation_transaction_t *binary_transaction = NULL;
    engine_activation_transaction_status_t stage_status;
    ...
    int prepared_dir_length =
        snprintf(prepared_dir, sizeof(prepared_dir), "%s/engine-update-sign-XXXXXX", engine_tmpdir());
    bool prepared = prepared_dir_length > 0 && (size_t)prepared_dir_length < sizeof(prepared_dir) &&
                    engine_mkdtemp(prepared_dir) != NULL;
    ...
    engine_activation_transaction_t *preparation = NULL;
    stage_status = prepared ? engine_activation_transaction_stage_bytes(prepared_candidate, bin_data,
                                                                     (size_t)bin_len, &preparation)
                            : ENGINE_ACTIVATION_TRANSACTION_IO;
    free(bin_data);
    cli_binary_validator_t unsigned_validator = {{0}};
    prepared = stage_status == ENGINE_ACTIVATION_TRANSACTION_OK && preparation &&
               ...
               engine_macos_adhoc_sign(prepared_candidate) == CLI_OK &&
               engine_daemon_build_fingerprint_file(prepared_candidate, validator.fingerprint);
    const char *prepared_argv[] = {prepared_candidate, "--version", NULL};
    prepared = prepared && engine_exec_no_shell(prepared_argv) == CLI_OK;
    if (prepared) {
        stage_status = engine_activation_transaction_stage_file(bin_dest, prepared_candidate,
                                                             &binary_transaction);
        cli_binary_validator_t staged_validator = {{0}};
        prepared =
            stage_status == ENGINE_ACTIVATION_TRANSACTION_OK && binary_transaction &&
            cli_activation_transaction_expected_build(binary_transaction, &staged_validator) &&
...
#else
    stage_status = engine_activation_transaction_stage_bytes(bin_dest, bin_data, (size_t)bin_len,
                                                          &binary_transaction);
    free(bin_data);
    if (stage_status != ENGINE_ACTIVATION_TRANSACTION_OK || !binary_transaction ||
...
#ifndef _WIN32
    const char *staged = engine_activation_transaction_staged_path(binary_transaction);
    const char *candidate_argv[] = {staged, "--version", NULL};
    if (!staged || engine_exec_no_shell(candidate_argv) != 0) {
    ...
    int activation_rc =
        cli_activation_guard(ENGINE_DAEMON_RUNTIME_ACTIVATION_UPDATE, NULL, validator.fingerprint,
...
static void build_update_url(char *url, int url_sz, const char *os, const char *arch,
    ...
    const char *base_url =
        engine_safe_getenv("ENGINE_DOWNLOAD_URL", base_url_buf, sizeof(base_url_buf), NULL);
    if (!base_url || !base_url[0]) {
        base_url = "https://github.com/DeusData/graph-engine/releases/latest/download";
    }
    ...
    snprintf(url, url_sz, "%s/graph-engine-%s%s-%s%s.%s", base_url, want_ui ? "ui-" : "", os,
             arch, portable, ext);
}
...
static int download_verify_install(const char *url, const char *ext, const char *os,
    ...
    int archive_path_length =
        snprintf(tmp_archive, sizeof(tmp_archive), "%s/engine-update-XXXXXX", engine_tmpdir());
    if (archive_path_length <= 0 || (size_t)archive_path_length >= sizeof(tmp_archive)) {
        return CLI_TRUE;
    }
    int archive_descriptor = engine_mkstemp(tmp_archive);
    ...
    int rc = engine_download_to_file(url, tmp_archive);
    if (rc != 0) {
    ...
    const char *portable = (strcmp(os, "linux") == 0) ? "-portable" : "";
    snprintf(archive_name, sizeof(archive_name), "graph-engine-%s%s-%s%s.%s",
...
static char *fetch_latest_tag(void) {
    FILE *fp = engine_popen(
        "curl -sfI https://github.com/DeusData/graph-engine/releases/latest 2>/dev/null",
        "r");
    if (!fp) {
        return NULL;
    }
    char line[ENGINE_SZ_512];
    char *tag = NULL;
...
static bool check_already_latest(void) {
    char dl_env[ENGINE_SZ_256] = "";
    engine_safe_getenv("ENGINE_DOWNLOAD_URL", dl_env, sizeof(dl_env), NULL);
    if (dl_env[0]) {
    ...
    int cmp = engine_compare_versions(latest, ENGINE_VERSION);
    if (cmp <= 0) {
        if (cmp < 0) {
...
int engine_cmd_update(int argc, char **argv) {
    parse_auto_answer(argc, argv);
     ...
     * it IS the update -- and runs while engine is NOT running. Print the exact
     * command instead of feigning self-update. */
#ifndef ENGINE_CLI_ENABLE_TEST_API
...
#ifdef ENGINE_CLI_ENABLE_TEST_API
    if (g_cli_activation_test_ops_set) {
        (void)fprintf(stderr, "*** engine test seam: portable update flow engaged; the "
...
#endif
    {
        ...
        bool have_dir = false;
        char *last_sep = engine_detect_self_path(self_dir, sizeof(self_dir), engine_get_home_dir())
                             ? strrchr(self_dir, '/')
                             : NULL;
        if (last_sep) {
            *last_sep = '\0';
        ...
        printf("graph-engine update (current: %s)\n\n", ENGINE_VERSION);
#ifdef _WIN32
...
#ifdef _WIN32
        printf("  target: %s/.local/bin/graph-engine.exe\n", home);
#else
        printf("  target: %s/.local/bin/graph-engine\n", home);
#endif
        printf("  variant: %s\n", variant_label);
        printf("  os/arch: %s/%s\n", os, arch);
...
#ifdef _WIN32
    snprintf(bin_dest_storage, sizeof(bin_dest_storage), "%s/.local/bin/graph-engine.exe",
             home);
#else
    snprintf(bin_dest_storage, sizeof(bin_dest_storage), "%s/.local/bin/graph-engine", home);
#endif
    char bin_dir[CLI_BUF_1K];
    snprintf(bin_dir, sizeof(bin_dir), "%s/.local/bin", home);
    if (!engine_mkdir_p(bin_dir, CLI_OCTAL_PERM)) {
    ...
    printf("\nUpdate complete. Verifying:\n");
    {
        ...
        (void)engine_exec_no_shell(ver_argv);
    }
...
char *engine_cli_build_args_json(const char *tool_name, int argc, char **argv, char **err_out) {
    if (err_out) {
        *err_out = NULL;
    }
    ...
    const char *schema_str = engine_mcp_tool_input_schema(tool_name);
...
int engine_cli_print_tool_help(const char *tool_name) {
    const char *schema_str = engine_mcp_tool_input_schema(tool_name);
    if (!schema_str) {
        return CLI_ERR;
    }
    ...
    printf("Usage:\n");
    printf("  graph-engine cli %s --flag value [--flag2 value2 ...]\n", tool_name);
    printf("  graph-engine cli %s --args-file <path-to-json>\n", tool_name);
    printf("  echo '<json>' | graph-engine cli %s\n", tool_name);
    printf("  graph-engine cli %s '<raw-json-args>'\n", tool_name);
