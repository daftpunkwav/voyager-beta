...
static engine_mutex_t s_sink_mutex;
...
static void progress_sink_mutex_ensure(void) {
    ...
    if (atomic_compare_exchange_strong_explicit(&s_sink_mutex_state, &expected, LOCK_INITIALIZING,
        ...
        engine_mutex_init(&s_sink_mutex);
...
static void progress_tool_name(const char *tool_name, char out[ENGINE_SZ_64]) {
    size_t offset = 0;
    if (tool_name) {
        for (; tool_name[offset] && offset + 1 < ENGINE_SZ_64; offset++) {
    ...
    if (offset == 0) {
        (void)snprintf(out, ENGINE_SZ_64, "%s", "tool");
    } else {
        out[offset] = '\0';
    }
}
...
void engine_progress_sink_init(FILE *out) {
    ...
    engine_mutex_lock(&s_sink_mutex);
    s_out = out ? out : stderr;
    atomic_store(&s_needs_newline, 0);
    s_gbuf_nodes = NOT_SET;
    s_gbuf_edges = NOT_SET;
    engine_log_set_sink(engine_progress_sink_fn);
    engine_mutex_unlock(&s_sink_mutex);
}
...
static void on_gbuf_dump(const char *line) {
    char n[ENGINE_SZ_32] = {0};
    char e[ENGINE_SZ_32] = {0};
    if (extract_kv(line, "nodes", n, (int)sizeof(n))) {
        s_gbuf_nodes = (int)strtol(n, NULL, ENGINE_DECIMAL_BASE);
    }
    if (extract_kv(line, "edges", e, (int)sizeof(e))) {
        s_gbuf_edges = (int)strtol(e, NULL, ENGINE_DECIMAL_BASE);
    }
}
...
static void on_extract_progress(const char *line) {
    char done[ENGINE_SZ_32] = {0};
    char total[ENGINE_SZ_32] = {0};
    if (extract_kv(line, "done", done, (int)sizeof(done)) &&
        extract_kv(line, "total", total, (int)sizeof(total))) {
        long d = strtol(done, NULL, ENGINE_DECIMAL_BASE);
        long t = strtol(total, NULL, ENGINE_DECIMAL_BASE);
