...
struct engine_activation_transaction {
...
const char *engine_activation_transaction_refusal_note(void) {
...
void engine_activation_transaction_note_refusal_for_testing(const char *predicate,
...
static engine_activation_transaction_before_absent_publish_for_test_fn
    activation_before_absent_publish_for_test;
static void *activation_before_absent_publish_context_for_test;

void engine_activation_transaction_set_before_absent_publish_for_test(
    engine_activation_transaction_before_absent_publish_for_test_fn hook, void *context) {
...
#endif
    int written =
        snprintf(path, needed, "%s%s.engine-%s-%lu-%" PRIu64, directory,
...
static bool activation_posix_acl_empty(int descriptor) {
    return engine_macos_extended_acl_fd_is_empty(descriptor);
}
...
static bool activation_directory_still_valid(const engine_activation_transaction_t *transaction) {
#ifdef _WIN32
    int ignored = 0;
...
static activation_create_status_t activation_private_file_create(
    const engine_activation_transaction_t *transaction, const char *path, const char *name,
...
static engine_activation_transaction_status_t activation_create_unique(
    const engine_activation_transaction_t *transaction, const char *tag, char **path_out,
    ...
    *file_out = ACTIVATION_INVALID_FILE;
    for (unsigned int attempt = 0; attempt < 1024U; attempt++) {
        ...
        if (!candidate) {
            return ENGINE_ACTIVATION_TRANSACTION_NO_MEMORY;
        }
        ...
        if (created == ACTIVATION_CREATE_OK) {
            *path_out = candidate;
            *name_out = name;
            return ENGINE_ACTIVATION_TRANSACTION_OK;
        }
        free(candidate);
        free(name);
        if (created != ACTIVATION_CREATE_EXISTS) {
            return ENGINE_ACTIVATION_TRANSACTION_IO;
        }
    }
    return ENGINE_ACTIVATION_TRANSACTION_IO;
}
...
static bool activation_posix_entry_snapshot_with_links(
    const engine_activation_transaction_t *transaction, const char *name, nlink_t required_links,
...
static bool activation_entry_snapshot(const engine_activation_transaction_t *transaction,
...
static bool activation_path_matches(const engine_activation_transaction_t *transaction,
...
static bool activation_sync_directory(const engine_activation_transaction_t *transaction) {
#ifdef _WIN32
...
void engine_activation_transaction_rename_failures_set_for_test(unsigned int count) {
#ifdef _WIN32
...
static bool activation_rename(const engine_activation_transaction_t *transaction, const char *source,
...
static void activation_transaction_destroy(engine_activation_transaction_t *transaction) {
    if (!transaction) {
        return;
    }
#ifndef _WIN32
...
static engine_activation_transaction_status_t activation_discard_staged_assets(
    ...
    return ok ? ENGINE_ACTIVATION_TRANSACTION_OK : ENGINE_ACTIVATION_TRANSACTION_IO;
}

static engine_activation_transaction_status_t activation_transaction_prepare(
    const char *target_path, activation_action_t action,
    engine_activation_transaction_t **transaction_out) {
    *transaction_out = NULL;
    activation_refusal_clear();
    engine_activation_transaction_t *transaction = calloc(1, sizeof(*transaction));
    if (!transaction) {
        return ENGINE_ACTIVATION_TRANSACTION_NO_MEMORY;
    }
#ifndef _WIN32
    ...
    if (!activation_target_parts(target_path, &transaction->directory_path,
        ...
        activation_transaction_destroy(transaction);
        return ENGINE_ACTIVATION_TRANSACTION_INVALID_ARGUMENT;
    }
#ifdef _WIN32
    int ignored = 0;
    ...
    if (transaction->target_existed) {
        ...
        engine_activation_transaction_status_t status = activation_create_unique(
...
static void activation_failed_stage_cleanup(engine_activation_transaction_t *transaction) {
    if (transaction) {
        (void)activation_discard_staged_assets(transaction);
        activation_transaction_destroy(transaction);
    }
...
engine_activation_transaction_status_t engine_activation_transaction_stage_bytes(
...
engine_activation_transaction_status_t engine_activation_transaction_stage_file(
...
engine_activation_transaction_status_t engine_activation_transaction_stage_removal(
    const char *target_path, engine_activation_transaction_t **transaction_out) {
    if (transaction_out) {
        *transaction_out = NULL;
    }
...
#ifdef _WIN32
static bool activation_windows_copy_target_to_backup(engine_activation_transaction_t *transaction) {
    if (!activation_directory_still_valid(transaction)) {
        return false;
    }
...
#ifndef _WIN32
static bool activation_linked_backup_pair_valid(const engine_activation_transaction_t *transaction) {
    activation_file_identity_t target_identity;
    activation_file_identity_t backup_identity;
    bool target_exists = false;
...
static bool activation_remove_linked_backup(engine_activation_transaction_t *transaction) {
...
static bool activation_target_still_original(const engine_activation_transaction_t *transaction) {
    activation_file_identity_t current;
    bool exists = false;
    if (!activation_entry_snapshot(transaction, transaction->target_path, transaction->target_name,
                                   &exists, &current) ||
...
static bool activation_absent_target_snapshot(const engine_activation_transaction_t *transaction,
...
static bool activation_remove_absent_published_target(engine_activation_transaction_t *transaction,
                                                      bool linked_pair) {
    if (!linked_pair) {
...
static engine_activation_transaction_status_t activation_rollback_internal(
    engine_activation_transaction_t *transaction) {
    if (transaction->target_existed) {
        bool backup_exists = false;
        if (!transaction->backup_contains_target ||
            ...
            return ENGINE_ACTIVATION_TRANSACTION_ROLLBACK_FAILED;
        }
        bool target_exists = false;
    ...
    return activation_sync_directory(transaction) ? ENGINE_ACTIVATION_TRANSACTION_OK
                                                  : ENGINE_ACTIVATION_TRANSACTION_IO;
}

engine_activation_transaction_status_t engine_activation_transaction_commit(
    engine_activation_transaction_t *transaction, engine_activation_transaction_validator_fn validator,
    void *validator_context) {
    if (!transaction) {
        return ENGINE_ACTIVATION_TRANSACTION_INVALID_ARGUMENT;
    }
    if (transaction->state != ACTIVATION_STAGED) {
        return ENGINE_ACTIVATION_TRANSACTION_INVALID_STATE;
    }
    ...
    if (transaction->target_existed) {
        if (transaction->action == ACTIVATION_REPLACE) {
            ...
            if (published == ACTIVATION_PUBLISH_CHANGED_ERROR) {
                transaction->state = ACTIVATION_RECOVERY_NEEDED;
                engine_activation_transaction_status_t restored =
                    activation_rollback_internal(transaction);
                return restored == ENGINE_ACTIVATION_TRANSACTION_OK
                           ? ENGINE_ACTIVATION_TRANSACTION_IO
                           : ENGINE_ACTIVATION_TRANSACTION_ROLLBACK_FAILED;
            }
        } else {
    ...
    if (!activation_directory_still_valid(transaction) || !activation_sync_directory(transaction)) {
        engine_activation_transaction_status_t restored = activation_rollback_internal(transaction);
        return restored == ENGINE_ACTIVATION_TRANSACTION_OK
                   ? ENGINE_ACTIVATION_TRANSACTION_IO
                   : ENGINE_ACTIVATION_TRANSACTION_ROLLBACK_FAILED;
    }
    if (validator && !validator(transaction->target_path, validator_context)) {
        engine_activation_transaction_status_t restored = activation_rollback_internal(transaction);
        return restored == ENGINE_ACTIVATION_TRANSACTION_OK
                   ? ENGINE_ACTIVATION_TRANSACTION_VALIDATION_FAILED
                   : ENGINE_ACTIVATION_TRANSACTION_ROLLBACK_FAILED;
    }
    return ENGINE_ACTIVATION_TRANSACTION_OK;
}

engine_activation_transaction_status_t engine_activation_transaction_rollback(
    engine_activation_transaction_t *transaction) {
    if (!transaction) {
        return ENGINE_ACTIVATION_TRANSACTION_INVALID_ARGUMENT;
    }
...
engine_activation_transaction_status_t engine_activation_transaction_finalize(
    engine_activation_transaction_t *transaction) {
    if (!transaction) {
        return ENGINE_ACTIVATION_TRANSACTION_INVALID_ARGUMENT;
    }
    ...
    if (transaction->backup_contains_target) {
        ...
        if (removed == ACTIVATION_UNLINK_DEFERRED) {
            ...
            return ENGINE_ACTIVATION_TRANSACTION_DEFERRED;
        }
        transaction->backup_exists = false;
        transaction->backup_contains_target = false;
    }
...
engine_activation_transaction_status_t engine_activation_transaction_close(
    engine_activation_transaction_t **transaction_io) {
    if (!transaction_io || !*transaction_io) {
        return ENGINE_ACTIVATION_TRANSACTION_INVALID_ARGUMENT;
    }
    engine_activation_transaction_t *transaction = *transaction_io;
    if (transaction->state == ACTIVATION_COMMITTED ||
        transaction->state == ACTIVATION_RECOVERY_NEEDED) {
        engine_activation_transaction_status_t status = activation_rollback_internal(transaction);
        if (status != ENGINE_ACTIVATION_TRANSACTION_OK) {
            return status;
        }
    }
    if (transaction->state == ACTIVATION_STAGED || transaction->state == ACTIVATION_ROLLED_BACK) {
        engine_activation_transaction_status_t status = activation_discard_staged_assets(transaction);
        if (status != ENGINE_ACTIVATION_TRANSACTION_OK) {
            return status;
        }
    }
...
const char *engine_activation_transaction_target_path(
    const engine_activation_transaction_t *transaction) {
    return transaction ? transaction->target_path : NULL;
}

const char *engine_activation_transaction_staged_path(
    const engine_activation_transaction_t *transaction) {
    return transaction ? transaction->staged_path : NULL;
}

const char *engine_activation_transaction_backup_path(
    const engine_activation_transaction_t *transaction) {
    return transaction ? transaction->backup_path : NULL;
}

const char *engine_activation_transaction_deferred_path(
    const engine_activation_transaction_t *transaction) {
    return transaction && transaction->deferred_cleanup ? transaction->backup_path : NULL;
}

const char *engine_activation_transaction_status_message(engine_activation_transaction_status_t status) {
    switch (status) {
    case ENGINE_ACTIVATION_TRANSACTION_OK:
        return "activation transaction completed";
    case ENGINE_ACTIVATION_TRANSACTION_DEFERRED:
        return "activation completed; old executable cleanup was deferred";
    case ENGINE_ACTIVATION_TRANSACTION_INVALID_ARGUMENT:
        return "invalid activation transaction argument";
    case ENGINE_ACTIVATION_TRANSACTION_NO_MEMORY:
        return "activation transaction allocation failed";
    case ENGINE_ACTIVATION_TRANSACTION_IO:
        return "activation transaction I/O failed";
    case ENGINE_ACTIVATION_TRANSACTION_INVALID_STATE:
        return "activation transaction is in the wrong state";
    case ENGINE_ACTIVATION_TRANSACTION_VALIDATION_FAILED:
        return "activated candidate failed validation and was rolled back";
    case ENGINE_ACTIVATION_TRANSACTION_ROLLBACK_FAILED:
