 ...
 */
#ifndef ENGINE_CONFIG_JSON_LIKE_H
#define ENGINE_CONFIG_JSON_LIKE_H
 ...
 * members produce ENGINE_JSON_LIKE_OBJECT_MISMATCH. STRING fields may require an
...
typedef enum {
    ...
    ENGINE_JSON_LIKE_VALUE_EMPTY_ARRAY,
    ENGINE_JSON_LIKE_VALUE_SINGLE_STRING_ARRAY,
} engine_json_like_value_shape_t;

enum {
    ENGINE_JSON_LIKE_FIELD_REQUIRED = 1U << 0U,
    ENGINE_JSON_LIKE_FIELD_CAPTURE_STRING = 1U << 1U,
};

typedef struct {
    const char *key;
    engine_json_like_value_shape_t shape;
    const char *expected_string;
    unsigned flags;
} engine_json_like_object_field_t;

enum {
    ENGINE_JSON_LIKE_OBJECT_MATCH = 0,
    ENGINE_JSON_LIKE_OBJECT_MISSING = 1,
    ENGINE_JSON_LIKE_OBJECT_MISMATCH = 2,
};

/* captured_string_out receives malloc-owned decoded content only on MATCH.
 * Returns one of ENGINE_JSON_LIKE_OBJECT_* or -1 for malformed input. */
...
/* Top-level convenience wrapper for engine_json_like_add_unique_string_at_path. */
int engine_json_like_add_unique_string(const char *file_path, const char *array_key,
                                    const char *string_value);

/* Top-level convenience wrapper for engine_json_like_remove_string_at_path. */
...
typedef void (*engine_json_like_precommit_test_hook_t)(const char *file_path, void *context);
...
#endif /* ENGINE_CONFIG_JSON_LIKE_H */
