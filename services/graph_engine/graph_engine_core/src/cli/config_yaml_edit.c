...
#ifdef ENGINE_YAML_ENABLE_TEST_API
static engine_yaml_precommit_test_hook_t yaml_precommit_test_hook = NULL;
static void *yaml_precommit_test_context = NULL;
static engine_yaml_precommit_test_hook_t yaml_prepublish_test_hook = NULL;
static void *yaml_prepublish_test_context = NULL;
static engine_yaml_lock_postcreate_test_hook_t yaml_lock_postcreate_test_hook = NULL;
...
static int yaml_build_lock_path(const char *path, char **out_path) {
    static const char suffix[] = ".engine-yaml.lock";
    size_t path_len = 0U;
...
#ifdef _WIN32
    wchar_t *wide_path = engine_utf8_to_wide(lock->path);
    if (!wide_path) {
        free(lock->path);
        lock->path = NULL;
        return YAML_ERROR;
...
static int yaml_write_atomic(const char *path, const char *data, size_t len,
    ...
    for (unsigned attempt = 0U; attempt < YAML_TMP_ATTEMPTS; attempt++) {
        ...
        int written = snprintf(temp_path, capacity, "%s.engine-yaml-%ld-%u.tmp", path,
...
#endif
    if (!failed && YAML_SYNC(engine_fileno(file)) != 0) {
        failed = true;
    }
    if (fclose(file) != 0) {
        failed = true;
...
#ifdef ENGINE_YAML_ENABLE_TEST_API
void engine_yaml_set_precommit_hook_for_testing(engine_yaml_precommit_test_hook_t hook, void *context) {
    yaml_precommit_test_hook = hook;
    yaml_precommit_test_context = context;
}

void engine_yaml_set_prepublish_hook_for_testing(engine_yaml_precommit_test_hook_t hook, void *context) {
    yaml_prepublish_test_hook = hook;
    yaml_prepublish_test_context = context;
}

void engine_yaml_set_lock_postcreate_hook_for_testing(engine_yaml_lock_postcreate_test_hook_t hook,
                                                   void *context) {
...
int engine_yaml_encode_double_quoted_scalar(const char *value, char **encoded_out) {
    if (!encoded_out) {
        return YAML_ERROR;
    }
    *encoded_out = NULL;
    ...
    if (target.identity_found) {
        ...
        if (existing_len != rendered.len ||
            ...
            if (yaml_splice(&out, &doc, target.identity_start, target.identity_end, NULL, 0U) !=
                ...
                result = ENGINE_YAML_IDENTITY_EDIT_ERROR;
            }
            yaml_buf_free(&out);
        }
    } else if (!remove) {
...
static int yaml_edit_owned_mapping_entry_locked(const char *file_path, const char *section_key,
    ...
    if (target.entry_found) {
        ...
        if (existing_entry_len != canonical.len ||
            ...
            yaml_doc_free(&doc);
            free(data);
            return ENGINE_YAML_IDENTITY_EDIT_FOREIGN;
        }
        if (!remove) {
            yaml_buf_free(&canonical);
            yaml_doc_free(&doc);
    ...
    yaml_doc_free(&doc);
    free(data);
    return result == 0 ? ENGINE_YAML_IDENTITY_EDIT_OK : ENGINE_YAML_IDENTITY_EDIT_ERROR;
}
...
int engine_yaml_upsert_mapping_entry(const char *file_path, const char *section_key,
                                  const char *entry_key, const char *entry_block) {
...
int engine_yaml_remove_mapping_entry(const char *file_path, const char *section_key,
                                  const char *entry_key) {
    yaml_config_lock_t lock;
    if (yaml_lock_acquire(file_path, &lock) != 0) {
        return YAML_ERROR;
...
int engine_yaml_upsert_owned_mapping_entry(const char *file_path, const char *section_key,
    ...
    int release_result = yaml_lock_release(&lock);
    return release_result == 0 ? result : ENGINE_YAML_IDENTITY_EDIT_ERROR;
}

int engine_yaml_remove_owned_mapping_entry(const char *file_path, const char *section_key,
                                        const char *entry_key, const char *canonical_entry_block) {
    yaml_config_lock_t lock;
    if (yaml_lock_acquire(file_path, &lock) != 0) {
        return ENGINE_YAML_IDENTITY_EDIT_ERROR;
...
int engine_yaml_upsert_mapping_sequence_item(const char *file_path, const char *const *sequence_path,
...
int engine_yaml_remove_mapping_sequence_item(const char *file_path, const char *const *sequence_path,
                                          size_t sequence_path_len, const char *identity_key,
                                          const char *identity_scalar, const char *canonical_item) {
    yaml_config_lock_t lock;
    if (yaml_lock_acquire(file_path, &lock) != 0) {
...
int engine_yaml_upsert_string_list_item(const char *file_path, const char *key, const char *item) {
    yaml_config_lock_t lock;
    if (yaml_lock_acquire(file_path, &lock) != 0) {
        return YAML_ERROR;
    }
...
int engine_yaml_remove_string_list_item(const char *file_path, const char *key, const char *item) {
    yaml_config_lock_t lock;
    if (yaml_lock_acquire(file_path, &lock) != 0) {
        return YAML_ERROR;
    }
