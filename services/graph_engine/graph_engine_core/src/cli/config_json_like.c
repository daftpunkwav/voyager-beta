...
static ENGINE_TLS engine_json_like_precommit_test_hook_t jl_precommit_test_hook = NULL;
static ENGINE_TLS void *jl_precommit_test_context = NULL;
static ENGINE_TLS engine_json_like_precommit_test_hook_t jl_prepublish_test_hook = NULL;
static ENGINE_TLS void *jl_prepublish_test_context = NULL;
#endif
...
static int jl_read_file(const char *path, char **content_out, size_t *length_out, bool *missing_out,
    ...
    *missing_out = false;
...
#ifdef _WIN32
    wchar_t *wide_path = engine_utf8_to_wide(path);
    if (!wide_path) {
        return -1;
    }
    ...
    jl_file_snapshot_t after;
    int after_result = fstat(engine_fileno(file), &after_state) == 0
...
#endif
    if (!separator) {
        return engine_strdup(".");
    }
    if (separator == path) {
        return engine_strdup("/");
    }
    return engine_strndup(path, (size_t)(separator - path));
}
...
static int jl_ensure_parent(const char *path) {
    ...
    int result = strcmp(parent, ".") == 0 || engine_mkdir_p(parent, 0755) ? 0 : -1;
    free(parent);
    return result;
}
...
#ifdef _WIN32
    wchar_t *wide_temp = engine_utf8_to_wide(temp_path);
    wchar_t *wide_path = engine_utf8_to_wide(path);
...
static int jl_write_atomic(const char *path, const char *content, size_t length,
    ...
    for (unsigned attempt = 0; attempt < 64U; attempt++) {
        ...
        int written = snprintf(temp_path, temp_capacity, "%s.engine.tmp.%ld.%u", path,
        ...
        errno = 0;
#ifdef _WIN32
        file = engine_fopen(temp_path, "wbx");
#else
#ifndef O_NOFOLLOW
        free(temp_path);
        return -1;
    ...
    if (!failed && expected_snapshot->exists &&
        fchown(engine_fileno(file), expected_snapshot->owner, expected_snapshot->group) != 0) {
        failed = true;
    }
    mode_t mode = expected_snapshot->exists ? expected_snapshot->mode & 0777U : 0600U;
    if (!failed && fchmod(engine_fileno(file), mode) != 0) {
        failed = true;
    }
#endif
    if (!failed && JL_SYNC(engine_fileno(file)) != 0) {
        failed = true;
    }
    if (fclose(file) != 0) {
        failed = true;
...
#ifdef ENGINE_JSON_LIKE_ENABLE_TEST_API
void engine_json_like_set_precommit_hook_for_testing(engine_json_like_precommit_test_hook_t hook,
                                                  void *context) {
    jl_precommit_test_hook = hook;
    jl_precommit_test_context = context;
}

void engine_json_like_set_prepublish_hook_for_testing(engine_json_like_precommit_test_hook_t hook,
                                                   void *context) {
...
int engine_json_like_read_document(const char *file_path, char **content_out, size_t *length_out) {
...
int engine_json_like_get_raw_entry(const char *file_path, const char *const *object_path,
...
int engine_json_like_upsert_entry(const char *file_path, const char *const *object_path,
...
int engine_json_like_upsert_entry_if_unchanged(const char *file_path, const char *const *object_path,
                                            size_t path_len, const char *entry_key,
...
int engine_json_like_remove_entry(const char *file_path, const char *const *object_path,
                               size_t path_len, const char *entry_key) {
    return jl_remove_entry(file_path, object_path, path_len, entry_key, false, NULL, 0U);
}

int engine_json_like_remove_entry_if_unchanged(const char *file_path, const char *const *object_path,
                                            size_t path_len, const char *entry_key,
                                            const char *expected_content, size_t expected_length) {
...
int engine_json_like_add_unique_string_at_path(const char *file_path, const char *const *object_path,
...
int engine_json_like_remove_string_at_path(const char *file_path, const char *const *object_path,
                                        size_t path_len, const char *array_key,
                                        const char *string_value) {
    if (jl_validate_arguments(file_path, object_path, path_len, array_key) != 0 ||
        jl_validate_string_value(string_value) != 0) {
...
static int jl_decode_field_string(const char *text, size_t start, size_t end,
                                  engine_json_like_value_shape_t shape, char **value_out) {
    *value_out = NULL;
    if (shape == ENGINE_JSON_LIKE_VALUE_STRING) {
        if (start >= end || (text[start] != '"' && text[start] != '\'')) {
            return 1;
        }
        return jl_decode_string_value(text, start, end, value_out) == 0 ? 0 : 1;
    }
    if (shape != ENGINE_JSON_LIKE_VALUE_SINGLE_STRING_ARRAY || start >= end || text[start] != '[') {
        return 1;
    }
...
static bool jl_field_shape_matches(const char *text, const jl_member_t *member,
                                   engine_json_like_value_shape_t shape, char **decoded_out) {
    *decoded_out = NULL;
    if (shape == ENGINE_JSON_LIKE_VALUE_EMPTY_ARRAY) {
...
int engine_json_like_match_object_entry(const char *document, size_t document_length,
                                     const char *const *object_path, size_t path_len,
                                     const char *entry_key,
                                     const engine_json_like_object_field_t *fields, size_t field_count,
    ...
    *captured_string_out = NULL;
    ...
    for (size_t i = 0U; i < field_count; ++i) {
        if (!fields[i].key || fields[i].key[0] == '\0' ||
            fields[i].shape > ENGINE_JSON_LIKE_VALUE_SINGLE_STRING_ARRAY ||
            (fields[i].flags &
             ~(ENGINE_JSON_LIKE_FIELD_REQUIRED | ENGINE_JSON_LIKE_FIELD_CAPTURE_STRING)) != 0U ||
            ((fields[i].flags & ENGINE_JSON_LIKE_FIELD_CAPTURE_STRING) != 0U &&
             fields[i].shape == ENGINE_JSON_LIKE_VALUE_EMPTY_ARRAY)) {
            return -1;
        }
        capture_count += (fields[i].flags & ENGINE_JSON_LIKE_FIELD_CAPTURE_STRING) != 0U ? 1U : 0U;
    ...
    if (document_length == 0U) {
        return ENGINE_JSON_LIKE_OBJECT_MISSING;
    }
    ...
    for (size_t i = 0U; i < path_len; ++i) {
        ...
        if (parent.match_count != 1U || document[parent.match.value_start] != '{') {
            return ENGINE_JSON_LIKE_OBJECT_MISMATCH;
        }
        object_start = parent.match.value_start;
    }
    ...
    for (size_t i = 0U; i < field_count; ++i) {
        ...
        if (entry.match_count > 1U ||
            (entry.match_count == 0U && (fields[i].flags & ENGINE_JSON_LIKE_FIELD_REQUIRED) != 0U)) {
            free(captured);
            return ENGINE_JSON_LIKE_OBJECT_MISMATCH;
        }
        ...
        if ((fields[i].flags & ENGINE_JSON_LIKE_FIELD_CAPTURE_STRING) != 0U) {
            captured = decoded;
        } else {
            free(decoded);
        }
    ...
    *captured_string_out = captured;
    return ENGINE_JSON_LIKE_OBJECT_MATCH;
}

int engine_json_like_get_string_at_path(const char *file_path, const char *const *object_path,
...
int engine_json_like_add_unique_string(const char *file_path, const char *array_key,
                                    const char *string_value) {
    const char *root_path[] = {NULL};
    return engine_json_like_add_unique_string_at_path(file_path, root_path, 0U, array_key,
                                                   string_value);
}

int engine_json_like_remove_string(const char *file_path, const char *array_key,
                                const char *string_value) {
    const char *root_path[] = {NULL};
    return engine_json_like_remove_string_at_path(file_path, root_path, 0U, array_key, string_value);
}
