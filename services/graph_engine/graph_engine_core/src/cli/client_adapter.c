...
static void sb_append_len(adapter_sb_t *sb, const char *s, size_t add) {
    ...
    if (sb->len + add + 1 > sb->cap) {
        size_t want = sb->cap ? sb->cap : ENGINE_SZ_2K;
 ...
 * from the hash-verified engine-integrations.json — those child-process-spawning
...
char *engine_client_adapter_pi(const char *binary_path) {
    ...
    char bin[ENGINE_SZ_1K];
    if (!engine_client_adapter_escape_js(binary_path, bin, sizeof(bin))) {
        return NULL;
    }
    int count = engine_mcp_tool_count();
    if (count <= 0) {
        return NULL;
    }
    const engine_integration_template_t *tpl = engine_integration_template("adapter_pi");
    ...
    for (int i = 0; i < count; i++) {
        const char *name = engine_mcp_tool_name(i);
        if (!name || !name[0]) {
            continue;
        }
...
char *engine_client_adapter_opencode(const char *binary_path) {
    ...
    const engine_integration_template_t *tpl = engine_integration_template("adapter_opencode");
