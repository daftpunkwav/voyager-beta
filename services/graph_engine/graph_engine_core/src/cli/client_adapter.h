 ...
 *    plugin surface) comes from the hash-verified engine-integrations.json via
 ...
 *    (engine_mcp_tool_count / engine_mcp_tool_name), never hand-maintained in the
 ...
 */
#ifndef ENGINE_CLI_CLIENT_ADAPTER_H
#define ENGINE_CLI_CLIENT_ADAPTER_H
...
#define ENGINE_ADAPTER_MARKER_START "// graph-engine:start"
#define ENGINE_ADAPTER_MARKER_END "// graph-engine:end"
 ...
 * path such as C:\Users\x\bin\engine.exe must not corrupt the module or, worse,
...
char *engine_client_adapter_pi(const char *binary_path);

/* Emit an OpenCode plugin that augments grep/glob results from the graph.
 *
 * OpenCode HAS MCP (see engine_upsert_opencode_mcp), so this adds no tools — it
 ...
 *
 * Returns NULL on allocation failure or when binary_path is NULL/empty. */
char *engine_client_adapter_opencode(const char *binary_path);
...
bool engine_client_adapter_escape_js(const char *in, char *out, size_t out_sz);

#endif /* ENGINE_CLI_CLIENT_ADAPTER_H */
