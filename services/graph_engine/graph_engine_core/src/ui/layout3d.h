 ...
 */
#ifndef ENGINE_UI_LAYOUT3D_H
#define ENGINE_UI_LAYOUT3D_H
...
} engine_layout_edge_t;

/* ── Layout result ────────────────────────────────────────────── */

typedef struct {
    engine_layout_node_t *nodes;
    int node_count;
    engine_layout_edge_t *edges;
    int edge_count;
    int total_nodes; /* total in project (may exceed returned) */
} engine_layout_result_t;

/* ── API ──────────────────────────────────────────────────────── */

typedef enum {
    ENGINE_LAYOUT_OVERVIEW = 0, /* cluster centroids */
    ENGINE_LAYOUT_DETAIL = 1    /* individual nodes in region */
} engine_layout_level_t;
...
void engine_layout_free(engine_layout_result_t *result);

/* Serialize layout result to JSON string. Caller must free(). */
char *engine_layout_to_json(const engine_layout_result_t *result);

#endif /* ENGINE_UI_LAYOUT3D_H */
