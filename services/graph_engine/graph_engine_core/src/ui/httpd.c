...
#include <winsock2.h>
#include <ws2tcpip.h>
typedef SOCKET engine_sock_t;
#define ENGINE_SOCK_BAD INVALID_SOCKET
#define engine_sock_close closesocket
#else
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
...
#include <time.h>
#include <unistd.h>
typedef int engine_sock_t;
#define ENGINE_SOCK_BAD (-1)
#define engine_sock_close close
#endif

/* Suppress SIGPIPE on send: Linux has the per-call flag, macOS has the
 * per-socket option (set at accept), Windows has no signals. */
#ifdef MSG_NOSIGNAL
#define ENGINE_SEND_FLAGS MSG_NOSIGNAL
#else
#define ENGINE_SEND_FLAGS 0
#endif

/* A non-reading peer may occupy the sequential event loop only briefly. */
#define ENGINE_HTTP_SEND_DEADLINE_MS 1000
#define ENGINE_HTTP_SEND_SLICE_BYTES (64U * 1024U)
#ifdef _WIN32
...
#define ENGINE_HTTP_WAIT_SLICE_MS 50
#endif
...
struct engine_httpd {
    ...
    engine_mutex_t active_mutex;
    struct engine_http_conn *active;
    bool interrupted;
    int send_buffer_for_test;
    int send_deadline_for_test_ms;
};

struct engine_http_conn {
    engine_sock_t fd;
    engine_httpd_t *owner;
    int recv_deadline_ms;
...
static int wait_ready(engine_sock_t fd, bool writing, int timeout_ms) {
#ifdef _WIN32
    fd_set ready;
    fd_set errors;
    FD_ZERO(&ready);
...
static int wait_readable(engine_sock_t fd, int timeout_ms) {
...
static bool socket_set_nonblocking(engine_sock_t fd) {
#ifdef _WIN32
    u_long enabled = 1;
...
static void socket_shutdown(engine_sock_t fd) {
#ifdef _WIN32
...
static bool connection_interrupted(engine_http_conn_t *connection) {
    engine_httpd_t *owner = connection->owner;
    if (!owner)
        return false;
    engine_mutex_lock(&owner->active_mutex);
    bool interrupted = owner->interrupted;
    engine_mutex_unlock(&owner->active_mutex);
    return interrupted;
}
...
static int wait_connection_ready(engine_http_conn_t *connection, bool writing, int64_t deadline) {
    for (;;) {
...
#ifdef _WIN32
        if (timeout_ms > ENGINE_HTTP_WAIT_SLICE_MS)
            timeout_ms = ENGINE_HTTP_WAIT_SLICE_MS;
#endif
...
static int send_all(engine_http_conn_t *connection, const void *data, size_t len, int64_t deadline) {
    const char *p = data;
    size_t off = 0;
    while (off < len) {
        ...
        if (available > ENGINE_HTTP_SEND_SLICE_BYTES)
            available = ENGINE_HTTP_SEND_SLICE_BYTES;
#ifdef _WIN32
        int n = send(connection->fd, p + off, (int)available, ENGINE_SEND_FLAGS);
#else
        ssize_t n = send(connection->fd, p + off, available, ENGINE_SEND_FLAGS);
#endif
...
engine_httpd_t *engine_httpd_listen(int port) {
#ifdef _WIN32
    ...
    engine_sock_t fd = socket(AF_INET, SOCK_STREAM, 0);
    if (fd == ENGINE_SOCK_BAD)
        return NULL;
    ...
    engine_httpd_t *d = calloc(1, sizeof(*d));
    if (!d) {
        engine_sock_close(fd);
        return NULL;
    }
    ...
    d->recv_deadline_ms = ENGINE_HTTP_RECV_DEADLINE_MS;
    engine_mutex_init(&d->active_mutex);
    return d;
}

int engine_httpd_port(const engine_httpd_t *d) {
    return d ? d->port : -1;
}

void engine_httpd_set_recv_deadline_ms(engine_httpd_t *d, int ms) {
    if (d && ms > 0) {
        engine_mutex_lock(&d->active_mutex);
        d->recv_deadline_ms = ms;
        engine_mutex_unlock(&d->active_mutex);
    }
}

void engine_httpd_interrupt(engine_httpd_t *d) {
    if (!d)
        return;
    engine_mutex_lock(&d->active_mutex);
    d->interrupted = true;
...
bool engine_httpd_close(engine_httpd_t *d) {
    if (!d)
        return true;
    engine_mutex_lock(&d->active_mutex);
    d->interrupted = true;
    ...
    engine_mutex_unlock(&d->active_mutex);
    engine_sock_close(d->fd);
    engine_mutex_destroy(&d->active_mutex);
    free(d);
    return true;
}

void engine_httpd_set_send_deadline_for_test(engine_httpd_t *d, int ms) {
...
void engine_httpd_set_send_buffer_for_test(engine_httpd_t *d, int bytes) {
    if (!d)
        return;
    engine_mutex_lock(&d->active_mutex);
...
engine_httpd_activity_t engine_httpd_activity_for_test(engine_httpd_t *d) {
    if (!d)
        return ENGINE_HTTPD_ACTIVITY_IDLE;
    engine_mutex_lock(&d->active_mutex);
    engine_httpd_activity_t activity = ENGINE_HTTPD_ACTIVITY_IDLE;
    if (d->active) {
        activity = atomic_load_explicit(&d->active->response_started, memory_order_acquire)
                       ? ENGINE_HTTPD_ACTIVITY_RESPONDING
                       : ENGINE_HTTPD_ACTIVITY_READING_REQUEST;
    }
    engine_mutex_unlock(&d->active_mutex);
    return activity;
}
...
engine_http_conn_t *engine_httpd_accept(engine_httpd_t *d, int timeout_ms) {
    if (!d)
        return NULL;
    engine_mutex_lock(&d->active_mutex);
    ...
    engine_sock_t cfd = accept(d->fd, NULL, NULL);
    if (cfd == ENGINE_SOCK_BAD)
        return NULL;
    ...
    if (!socket_set_nonblocking(cfd)) {
        engine_sock_close(cfd);
        return NULL;
    }

    engine_http_conn_t *c = calloc(1, sizeof(*c));
    if (!c) {
        engine_sock_close(cfd);
        return NULL;
    }
...
void engine_httpd_conn_close(engine_http_conn_t *c) {
    if (!c)
        return;
    engine_httpd_t *owner = c->owner;
    if (owner) {
        engine_mutex_lock(&owner->active_mutex);
        if (owner->active == c)
            owner->active = NULL;
        engine_sock_close(c->fd);
        engine_mutex_unlock(&owner->active_mutex);
    } else {
        engine_sock_close(c->fd);
    }
...
int engine_http_conn_status(const engine_http_conn_t *c) {
    return c ? c->response_status : 0;
}

size_t engine_http_conn_response_bytes(const engine_http_conn_t *c) {
...
int engine_http_parse_head(const char *data, size_t len, engine_http_req_t *req, size_t *body_offset,
    ...
    *content_length = 0;
    ...
    size_t scan = len < ENGINE_HTTP_MAX_HEAD ? len : ENGINE_HTTP_MAX_HEAD;
    ...
    if (head_end == 0)
        return len >= ENGINE_HTTP_MAX_HEAD ? 431 : ENGINE_HTTP_NEED_MORE;
    ...
    while (p < head_stop) {
        ...
        if (header_name_is(p, nlen, "origin")) {
            ...
            for (const char *v = val; *v; v++) {
                ...
                if (cl > (uint64_t)ENGINE_HTTP_MAX_BODY)
                    return 413;
            }
...
int engine_httpd_read_request(engine_http_conn_t *c, engine_http_req_t *req) {
    if (!c || !req)
        return -1;

    char *head = malloc(ENGINE_HTTP_MAX_HEAD);
    if (!head)
        return -1;
    ...
    for (;;) {
...
#ifdef _WIN32
        int n = recv(c->fd, head + have, (int)(ENGINE_HTTP_MAX_HEAD - have), 0);
#else
        ssize_t n = recv(c->fd, head + have, ENGINE_HTTP_MAX_HEAD - have, 0);
#endif
        if (n < 0 && socket_would_block())
            continue;
        if (n <= 0) {
        ...
        rc = engine_http_parse_head(head, have, req, &body_off, &clen);
        if (rc == 0)
            break;
        if (rc != ENGINE_HTTP_NEED_MORE) {
            free(head);
            return rc;
        }
        if (have >= ENGINE_HTTP_MAX_HEAD) {
            free(head);
            return 431;
        }
    }
...
void engine_http_req_free(engine_http_req_t *req) {
    if (req && req->body) {
        free(req->body);
        req->body = NULL;
        req->body_len = 0;
...
void engine_http_reply_buf(engine_http_conn_t *c, int status, const char *extra_headers, const void *data,
                        size_t len) {
    if (!c)
        return;
...
void engine_http_replyf(engine_http_conn_t *c, int status, const char *extra_headers, const char *fmt,
                     ...) {
    va_list ap;
    va_start(ap, fmt);
    va_list ap2;
    ...
    engine_http_reply_buf(c, status, extra_headers, body, (size_t)need);
    if (body != stack_buf)
        free(body);
}
...
bool engine_http_path_match(const char *str, const char *pattern) {
    if (!str || !pattern)
        return false;
...
bool engine_http_query_param(const char *query, const char *name, char *buf, int bufsz) {
