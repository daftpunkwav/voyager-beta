 ...
 * read-only engine_mcp_server_t instance.
 *
 * Runs in a background pthread, same pattern as the watcher thread.
 */
#ifndef ENGINE_UI_HTTP_SERVER_H
#define ENGINE_UI_HTTP_SERVER_H
...
typedef struct engine_http_server engine_http_server_t;
...
typedef int (*engine_http_index_executor_fn)(void *context, const char *root_path,
                                          const char *project_name);
typedef bool (*engine_http_project_mutation_begin_fn)(void *context, const char *project);
typedef void (*engine_http_project_mutation_end_fn)(void *context, const char *project);

/* Create an HTTP server on the given port.
 * Creates its own engine_mcp_server_t with a separate read-only SQLite connection.
 ...
 * Blocks until engine_http_server_stop() is called. */
 ...
 * before its run loop starts. GET /__engine/ui-readiness returns only a
