 ...
 *
 * Config file: ~/.cache/graph-engine/config.json
...
void engine_ui_config_load(engine_ui_config_t *cfg) {
    ...
    cfg->ui_enabled = ENGINE_UI_DEFAULT_ENABLED;
    cfg->ui_port = ENGINE_UI_DEFAULT_PORT;

    char path[ENGINE_SZ_1K];
    engine_ui_config_path(path, (int)sizeof(path));
    ...
    free(buffer);
    if (!doc) {
        engine_log_warn("ui.config.corrupt", "path", path);
...
bool engine_ui_config_save(const engine_ui_config_t *cfg) {
    if (!cfg || cfg->ui_port <= 0 || cfg->ui_port > 65535) {
        engine_log_error("ui.config.write_fail", "reason", "invalid_config");
        return false;
    }
    char path[ENGINE_SZ_1K];
    engine_ui_config_path(path, (int)sizeof(path));
    ...
    if (directory_ready && !engine_is_dir(dir)) {
        directory_ready = engine_mkdir_p(dir, 0750) || engine_is_dir(dir);
    }
    if (!directory_ready) {
        engine_log_error("ui.config.write_fail", "path", path, "reason", "create_directory");
        return false;
    }
    ...
    if (!json) {
        engine_log_error("ui.config.write_fail", "reason", "serialize");
        return false;
    }
    ...
    free(json);
    if (!saved) {
        engine_log_error("ui.config.write_fail", "path", path, "reason", "atomic_publish");
        return false;
    }

    engine_log_debug("ui.config.saved", "path", path);
    return true;
}
