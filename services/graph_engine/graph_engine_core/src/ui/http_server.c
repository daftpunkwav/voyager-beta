...
static void handle_ui_config(engine_http_conn_t *c, const engine_http_req_t *req) {
    const char *lang = NULL;
    char cache_dir[1024];
    snprintf(cache_dir, sizeof(cache_dir), "%s", engine_resolve_cache_dir());
    engine_config_t *cfg = engine_config_open(cache_dir);
    if (cfg) {
        const char *pinned = engine_config_get(cfg, ENGINE_CONFIG_UI_LANG, "auto");
...
char *engine_ui_git_strip_credentials(const char *url) {
    if (!url)
        return NULL;
...
char *engine_ui_git_web_base(const char *url) {
    if (!url || !url[0])
        return NULL;
 ...
 * libgit2 is initialized once at process start by engine_alloc_init() (which also
...
static void handle_repo_info(engine_http_conn_t *c, const engine_http_req_t *req) {
    char project[256] = {0};
    if (!engine_http_query_param(req->query, "project", project, (int)sizeof(project)) ||
        project[0] == '\0') {
        engine_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing project parameter\"}");
        return;
    }

    char db_path[1024];
    db_path_for_project(project, db_path, sizeof(db_path));
    if (db_path[0] == '\0' || !engine_file_exists(db_path)) {
        engine_http_replyf(c, 404, g_cors_json, "{\"error\":\"project not found\"}");
        return;
    }
    engine_store_t *store = engine_store_open_path_query(db_path);
    if (!store) {
        engine_http_replyf(c, 500, g_cors_json, "{\"error\":\"cannot open store\"}");
        return;
    }

    char root_path[1024] = {0};
    engine_project_t proj;
    memset(&proj, 0, sizeof(proj));
    if (engine_store_get_project(store, project, &proj) == ENGINE_STORE_OK && proj.root_path) {
    ...
    char branch[256] = {0};
    if (root_path[0]) {
        engine_git_context_t gctx;
        memset(&gctx, 0, sizeof(gctx));
        if (engine_git_context_resolve(root_path, &gctx) == 0 && gctx.branch) {
    ...
    char *remote_safe = engine_ui_git_strip_credentials(remote); /* never echo secrets */
    char *web_base = engine_ui_git_web_base(remote);
    ...
    engine_json_escape(esc_root, (int)sizeof(esc_root), root_path);
    engine_json_escape(esc_branch, (int)sizeof(esc_branch), branch);
    engine_json_escape(esc_remote, (int)sizeof(esc_remote), remote_safe ? remote_safe : "");
    engine_json_escape(esc_web, (int)sizeof(esc_web), web_base ? web_base : "");
    engine_json_escape(esc_blob, (int)sizeof(esc_blob), blob_base);
...
/* GET /api/processes — list graph-engine processes via ps */
static void handle_processes(engine_http_conn_t *c) {
    char buf[8192];
    int pos = 0;
...
static void append_roots_json(char *buf, size_t bufsz, int *pos) {
    ...
    for (int i = 0; i < 26; i++) {
         ...
         * offline volume) fails the same engine_is_dir gate browse applies, and
         * a root the picker cannot open must not be offered. */
        char root[4] = {(char)('A' + i), ':', '/', '\0'};
        if (!engine_is_dir(root)) {
            continue;
        }
        if (count++ > 0) {
...
static void handle_browse(engine_http_conn_t *c, const engine_http_req_t *req) {
    char path[1024] = {0};
    const char *home = engine_get_home_dir();
    if (!engine_http_query_param(req->query, "path", path, (int)sizeof(path)) || path[0] == '\0') {
     ...
     * "D:\projects\demo"). Normalize to forward slashes before the engine_is_dir
     * gate, exactly as the MCP repo_path handler and engine_project_name_from_path
     * already do — otherwise a real D:/ directory is rejected (#548). */
    engine_normalize_path_sep(path);

    if (!engine_is_dir(path)) {
        engine_http_replyf(c, 400, g_cors_json, "{\"error\":\"not a directory\"}");
        return;
    }

    DIR *dir = opendir(path);
    if (!dir) {
        engine_http_replyf(c, 403, g_cors_json, "{\"error\":\"cannot open directory\"}");
        return;
    }
    ...
    while ((ent = readdir(dir)) != NULL) {
        ...
        {
            char esc[512];
            engine_json_escape(esc, (int)sizeof(esc), ent->d_name);
...

    {
        ...
        engine_json_escape(esc_parent, (int)sizeof(esc_parent), parent);
...
static void handle_adr_get(engine_http_conn_t *c, const engine_http_req_t *req) {
    char name[256] = {0};
    if (!engine_http_query_param(req->query, "project", name, (int)sizeof(name)) || name[0] == '\0') {
        engine_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing project\"}");
        return;
    }
    ...
    engine_store_t *store = engine_store_open_path_query(db_path);
    if (!store) {
        engine_http_replyf(c, 200, g_cors_json, "{\"has_adr\":false}");
        return;
    }

    engine_adr_t adr;
    memset(&adr, 0, sizeof(adr));
    if (engine_store_adr_get(store, name, &adr) == ENGINE_STORE_OK && adr.content) {
...
static void handle_adr_save(engine_http_server_t *srv, engine_http_conn_t *c, const engine_http_req_t *req) {
    ...
    if (!v_proj || !yyjson_is_str(v_proj) || !v_content || !yyjson_is_str(v_content)) {
        yyjson_doc_free(doc);
        engine_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing project or content\"}");
        return;
    }
    ...
    int rc = engine_store_adr_store(store, proj, content);
    engine_store_close(store);
    if (mutation_held) {
        srv->mutation_end(srv->mutation_context, proj);
    }
    ...
    if (rc == ENGINE_STORE_OK) {
        engine_http_replyf(c, 200, g_cors_json, "{\"saved\":true}");
    } else {
        engine_http_replyf(c, 500, g_cors_json, "{\"error\":\"save failed\"}");
    }
}
...
static int find_cross_repo_targets(engine_store_t *store, const char *project, char **out,
                                   int max_out) {
    struct sqlite3 *db = engine_store_get_db(store);
    if (!db) {
        return 0;
    }
    sqlite3_stmt *s = NULL;
...
static double layout_radius(const engine_layout_result_t *r) {
    if (!r || r->node_count == 0)
        return 0.0;
    double max_r2 = 0.0;
...
static bool attach_missed_graph(yyjson_mut_doc *mdoc, yyjson_mut_val *mroot, engine_store_t *store,
                                const char *project, double primary_radius) {
    char covproj[512];
    engine_store_coverage_shadow_project(covproj, sizeof(covproj), project);
    engine_layout_result_t *ml = engine_layout_compute(store, covproj, ENGINE_LAYOUT_OVERVIEW, NULL, 0, 0);
    if (!ml) {
        return false;
    }
    if (ml->node_count == 0) {
        engine_layout_free(ml);
        return false;
    }
    double miss_radius = layout_radius(ml);
    char *mjson = engine_layout_to_json(ml);
    engine_layout_free(ml);
    if (!mjson) {
        return false;
    }
...
static void handle_layout(engine_http_conn_t *c, const engine_http_req_t *req) {
    char project[256] = {0};
    char max_str[32] = {0};
    char graph_str[32] = {0};
    ...
    if (engine_http_query_param(req->query, "max_nodes", max_str, (int)sizeof(max_str))) {
        int v = atoi(max_str);
        if (v > 0)
            max_nodes = v;
    }
    ...
    if (engine_http_query_param(req->query, "graph", graph_str, (int)sizeof(graph_str))) {
        missed_graph = strcmp(graph_str, "missed") == 0;
    }
    char scoped_project[320];
    if (missed_graph) {
        engine_store_coverage_shadow_project(scoped_project, sizeof(scoped_project), project);
    } else {
    ...
    engine_layout_result_t *layout =
        engine_layout_compute(store, scoped_project, ENGINE_LAYOUT_OVERVIEW, NULL, 0, max_nodes);
    ...
    if (!layout) {
        engine_store_close(store);
        engine_http_replyf(c, 500, g_cors_json, "{\"error\":\"layout computation failed\"}");
        return;
    }
    ...
    char *primary_json = engine_layout_to_json(layout);
    engine_layout_free(layout);
    if (!primary_json) {
        engine_store_close(store);
        engine_http_replyf(c, 500, g_cors_json, "{\"error\":\"JSON serialization failed\"}");
        return;
    }
    ...
    if (linked_count == 0 && missed_graph) {
        engine_store_close(store);
        engine_http_replyf(c, 200, g_cors_json, "%s", primary_json);
        free(primary_json);
        return;
    }
    ...
    if (!pdoc) {
        engine_store_close(store);
        engine_http_replyf(c, 500, g_cors_json, "{\"error\":\"JSON parse failed\"}");
        return;
    }
    ...
    for (int li = 0; li < linked_count; li++) {
        ...
        if (!engine_file_exists(lp_path)) {
            free(linked[li]);
            continue;
        }

        engine_store_t *lp_store = engine_store_open_path_query(lp_path);
        if (!lp_store) {
            free(linked[li]);
            continue;
        }
        ...
        engine_layout_result_t *lp_layout =
            engine_layout_compute(lp_store, linked[li], ENGINE_LAYOUT_OVERVIEW, NULL, 0, max_nodes);

        if (!lp_layout) {
            engine_store_close(lp_store);
            free(linked[li]);
            continue;
        }

        double sat_radius = layout_radius(lp_layout);
        char *lp_json = engine_layout_to_json(lp_layout);
        engine_layout_free(lp_layout);
        if (!lp_json) {
            engine_store_close(lp_store);
            free(linked[li]);
            continue;
        ...
        struct sqlite3 *src_db = engine_store_get_db(store);
        struct sqlite3 *lp_db = engine_store_get_db(lp_store);
        if (src_db && lp_db) {
    ...
    if (final_json) {
        engine_http_replyf(c, 200, g_cors_json, "%s", final_json);
        free(final_json);
    } else {
        engine_http_replyf(c, 500, g_cors_json, "{\"error\":\"JSON write failed\"}");
    }
}
...
static void dispatch_request(engine_http_server_t *srv, engine_http_conn_t *c,
    ...
    if (is_get && engine_http_path_match(req->path, "/api/layout*")) {
        handle_layout(c, req);
        return;
    }

    /* GET /api/repo-info → git remote / branch for GitHub deep-links */
    if (is_get && engine_http_path_match(req->path, "/api/repo-info*")) {
    ...
    if (is_get && engine_http_path_match(req->path, "/api/ui-config")) {
    ...
    if (is_get && engine_http_path_match(req->path, "/api/browse*")) {
        handle_browse(c, req);
        return;
    }

    /* GET /api/adr → get ADR for project */
    if (is_get && engine_http_path_match(req->path, "/api/adr*")) {
        handle_adr_get(c, req);
        return;
    }

    /* POST /api/adr → save ADR for project */
    if (is_post && engine_http_path_match(req->path, "/api/adr")) {
    ...
    /* GET /api/processes → list running graph-engine processes */
    if (is_get && engine_http_path_match(req->path, "/api/processes")) {
        handle_processes(c);
        return;
    }
