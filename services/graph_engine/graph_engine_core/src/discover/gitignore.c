 ...
 * number of groups. Patterns come from a committed .gitignore / .engineignore and
...
static void gi_add_pattern(engine_gitignore_t *gi, const char *line, int len) {
...
engine_gitignore_t *engine_gitignore_parse(const char *content) {
    ...
    engine_gitignore_t *gi = calloc(ENGINE_ALLOC_ONE, sizeof(engine_gitignore_t));
    if (!gi) {
        return NULL;
    }
...
engine_gitignore_t *engine_gitignore_load(const char *path) {
    ...
    engine_gitignore_t *gi = engine_gitignore_parse(buf);
    free(buf);
    return gi;
}
...
bool engine_gitignore_matches(const engine_gitignore_t *gi, const char *rel_path, bool is_dir) {
    return engine_gitignore_match_result(gi, rel_path, is_dir) > 0;
}
...
char *(*engine_gitignore_merge_dup_hook_for_test)(const char *) = NULL;
...
bool engine_gitignore_merge(engine_gitignore_t *dst, const engine_gitignore_t *src) {
    ...
    for (int i = 0; i < src->count; i++) {
        char *pat = engine_gitignore_merge_dup_hook_for_test
                        ? engine_gitignore_merge_dup_hook_for_test(src->patterns[i].pattern)
