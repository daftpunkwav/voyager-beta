/*
 * mcp.h — MCP (Model Context Protocol) server for graph-engine.
 *
...
typedef struct engine_store engine_store_t; /* from store/store.h */
struct engine_watcher;                   /* from watcher/watcher.h */
struct engine_config;                    /* from cli/cli.h */
...
typedef enum {
    ...
    ENGINE_MCP_TOOL_PROFILE_ANALYSIS = 1,
    ENGINE_MCP_TOOL_PROFILE_SCOUT = 2,
} engine_mcp_tool_profile_t;
...
typedef char *(*engine_mcp_index_executor_fn)(void *context, const char *repo_path,
...
typedef bool (*engine_mcp_project_mutation_begin_fn)(void *context, const char *project);
typedef void (*engine_mcp_project_mutation_end_fn)(void *context, const char *project);
...
typedef bool (*engine_mcp_project_mutation_try_begin_fn)(void *context, const char *project);
 ...
 * ENGINE_ALLOWED_ROOT. Returns false when the context is invalid or cannot be
 * copied. */
...
void engine_mcp_server_set_project_mutation_try_guard(engine_mcp_server_t *srv,
                                                   engine_mcp_project_mutation_try_begin_fn try_begin);
...
typedef enum {
    ENGINE_MCP_SUPERVISED_RESULT_FALLBACK = 0,
    ENGINE_MCP_SUPERVISED_RESULT_SUCCESS,
    ENGINE_MCP_SUPERVISED_RESULT_CONTAINED_FAILURE,
    ENGINE_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL,
} engine_mcp_supervised_result_disposition_t;
...
struct engine_pipeline; /* forward decl */

/* Get the currently active pipeline (for signal handler cancellation).
 * Returns NULL if no pipeline is running. */
struct engine_pipeline *engine_mcp_server_active_pipeline(engine_mcp_server_t *srv);
