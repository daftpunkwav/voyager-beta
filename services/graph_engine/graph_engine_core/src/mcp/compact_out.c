...
static bool sb_reserve(engine_sb_t *sb, size_t extra) {
    if (sb->oom) {
        return false;
    }
...
void engine_sb_append(engine_sb_t *sb, const char *s) {
    if (s) {
        engine_sb_append_n(sb, s, strlen(s));
    }
}
...
static void append_quoted(engine_sb_t *sb, const char *s) {
    engine_sb_append_n(sb, "\"", 1);
    for (const char *p = s ? s : ""; *p; p++) {
        switch (*p) {
        case '"':
            engine_sb_append_n(sb, "\\\"", 2);
            break;
        case '\\':
            engine_sb_append_n(sb, "\\\\", 2);
            break;
        case '\n':
            engine_sb_append_n(sb, "\\n", 2);
            break;
        case '\r':
            engine_sb_append_n(sb, "\\r", 2);
            break;
        default: {
            unsigned char c = (unsigned char)*p;
            if (c >= 0x20 && c != 0x7f && c < 0x80) {
                engine_sb_append_n(sb, p, 1);
            } else if (c < 0x20 || c == 0x7f) {
                /* JSON-style escape: the value stays one printable line. */
                char esc[8];
                snprintf(esc, sizeof(esc), "\\u%04x", (unsigned)c);
                engine_sb_append(sb, esc);
            } else {
                size_t len = utf8_sequence_length((const unsigned char *)p);
                if (len) {
                    engine_sb_append_n(sb, p, len);
                    p += len - 1;
                } else {
                    /* Invalid byte → U+FFFD: output is valid UTF-8 by
                     * construction, and the corruption stays visible. */
                    engine_sb_append_n(sb, "\xEF\xBF\xBD", 3);
                }
            }
            break;
        }
...
static void append_value(engine_sb_t *sb, const char *s) {
    if (!s || !*s) {
        engine_sb_append_n(sb, "-", 1); /* stable column positions for empties */
        return;
    }
    if (needs_quotes(s)) {
        append_quoted(sb, s);
    } else {
        engine_sb_append(sb, s);
    }
}
...
void engine_tree_scalar_bool(engine_sb_t *sb, const char *key, bool v) {
    engine_sb_append(sb, key);
    engine_sb_append_n(sb, ": ", 2);
    engine_sb_append(sb, v ? "true" : "false");
    engine_sb_append_n(sb, "\n", 1);
}
