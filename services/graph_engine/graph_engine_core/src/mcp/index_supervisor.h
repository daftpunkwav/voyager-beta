 ...
 */
#ifndef ENGINE_INDEX_SUPERVISOR_H
#define ENGINE_INDEX_SUPERVISOR_H

#include <stdbool.h>
#include <stddef.h>
#include <stdatomic.h>

#include "foundation/subprocess.h" /* engine_proc_outcome_t */
...
#define ENGINE_INDEX_WORKER_SINGLE_THREAD_ARG "--index-worker-single-thread"
#define ENGINE_INDEX_WORKER_MARKER_ARG "--index-worker-marker"
#define ENGINE_INDEX_WORKER_QUARANTINE_ARG "--index-worker-quarantine"
#define ENGINE_INDEX_WORKER_MEMORY_BUDGET_ARG "--index-worker-memory-budget-bytes"
#define ENGINE_INDEX_WORKER_BUILD_ARG "--index-worker-build"
#define ENGINE_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH 64U
#define ENGINE_INDEX_WORKER_BUILD_FINGERPRINT_SIZE 65U
...
typedef struct {
    ...
    size_t memory_budget_bytes;
} engine_index_worker_invocation_t;

typedef enum {
    ENGINE_INDEX_WORKER_ARGV_NOT_WORKER = 0,
    ENGINE_INDEX_WORKER_ARGV_VALID,
    ENGINE_INDEX_WORKER_ARGV_INVALID,
    ENGINE_INDEX_WORKER_ARGV_BUILD_UNAVAILABLE,
    ENGINE_INDEX_WORKER_ARGV_BUILD_MISMATCH,
} engine_index_worker_argv_status_t;
 ...
 * graph-engine binary calls this (first thing in main(), before any
 * subcommand dispatch, so MCP server + CLI + HTTP paths are all covered).
 * EMBEDDERS of engine_mcp_handle_tool (test binaries, future library users) never
 ...
 * this process called engine_index_supervisor_mark_host() (i.e. it IS the real
 ...
 * entry to engine_index_worker_start (including calls made by the synchronous
...
typedef struct {
    engine_proc_outcome_t outcome; /* how the worker ended */
...
} engine_index_worker_result_t;

/* Daemon-owned, nonblocking supervisor for one contained worker process tree. */
typedef struct engine_index_worker_handle engine_index_worker_handle_t;

typedef enum {
    ENGINE_INDEX_WORKER_POLL_ERROR = -1,
    ENGINE_INDEX_WORKER_POLL_RUNNING = 0,
    ENGINE_INDEX_WORKER_POLL_TERMINAL = 1,
} engine_index_worker_poll_t;
 ...
 * and applied after exec by engine_index_set_worker_role_options(). */
 ...
 *   - single_thread   → ENGINE_INDEX_SINGLE_THREAD=1
 *   - marker_file     → ENGINE_INDEX_MARKER_FILE
 *   - quarantine_file → ENGINE_INDEX_QUARANTINE_FILE
...
#endif /* ENGINE_INDEX_SUPERVISOR_H */
