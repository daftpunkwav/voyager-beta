...
#include "foundation/compat_fs.h" /* engine_mkdir_p, engine_fopen */
#include "foundation/constants.h"
#include "foundation/log.h"
#include "foundation/platform.h" /* engine_safe_getenv, path normalization */
...
_Static_assert(ENGINE_INDEX_WORKER_BUILD_FINGERPRINT_SIZE == ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE,
               "worker and daemon build fingerprint sizes must match");

/* ── Worker-role state ────────────────────────────────────────────── */

static bool g_worker_active = false;
static char g_worker_response_out[ENGINE_SZ_4K] = {0};
static size_t g_worker_memory_budget_bytes = 0;
static bool g_build_fingerprint_capture_attempted = false;
static char g_build_fingerprint[ENGINE_INDEX_WORKER_BUILD_FINGERPRINT_SIZE] = {0};

void engine_index_set_worker_role(bool is_worker, const char *response_out) {
...
static void worker_set_local_env(const char *name, const char *value) {
    if (value && value[0]) {
        (void)engine_setenv(name, value, 1);
    } else {
        (void)engine_unsetenv(name);
    }
}

void engine_index_set_worker_role_options(bool is_worker, const char *response_out, bool single_thread,
                                       const char *marker_file, const char *quarantine_file,
                                       size_t memory_budget_bytes) {
    engine_index_set_worker_role(is_worker, response_out);
    g_worker_memory_budget_bytes = is_worker ? memory_budget_bytes : 0;
    if (!is_worker) {
        return;
    }
    worker_set_local_env("ENGINE_INDEX_SINGLE_THREAD", single_thread ? "1" : NULL);
    worker_set_local_env("ENGINE_INDEX_MARKER_FILE", marker_file);
    worker_set_local_env("ENGINE_INDEX_QUARANTINE_FILE", quarantine_file);
}

bool engine_index_worker_active(void) {
    return g_worker_active;
}

const char *engine_index_worker_response_out(void) {
    return g_worker_response_out[0] ? g_worker_response_out : NULL;
}

size_t engine_index_worker_memory_budget_bytes(void) {
    return g_worker_memory_budget_bytes;
}

static bool worker_fingerprint_valid(const char *fingerprint);

bool engine_index_supervisor_capture_build_fingerprint(void) {
    if (g_build_fingerprint_capture_attempted) {
        return g_build_fingerprint[0] != '\0';
    }
    g_build_fingerprint_capture_attempted = true;
#if defined(ENGINE_CLI_ENABLE_TEST_API)
    ...
    char test_fingerprint[ENGINE_INDEX_WORKER_BUILD_FINGERPRINT_SIZE];
    const char *test_value = engine_safe_getenv("ENGINE_TEST_BUILD_FINGERPRINT", test_fingerprint,
...
#endif
    char captured[ENGINE_INDEX_WORKER_BUILD_FINGERPRINT_SIZE] = {0};
    if (!engine_daemon_runtime_process_build_fingerprint((uint64_t)worker_getpid(), captured)) {
        return false;
    }
...
const char *engine_index_supervisor_build_fingerprint(void) {
    return g_build_fingerprint[0] ? g_build_fingerprint : NULL;
}

static bool worker_fingerprint_valid(const char *fingerprint) {
    if (!fingerprint || strlen(fingerprint) != ENGINE_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH) {
        return false;
    }
    for (size_t i = 0; i < ENGINE_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH; i++) {
...
engine_index_worker_argv_status_t engine_index_worker_parse_process_argv(
    int argc, char *const argv[], engine_index_worker_invocation_t *invocation_out) {
    if (invocation_out) {
    ...
    if (!invocation_out || argc < 9 || !argv || !argv[0] || !argv[0][0] || !argv[1] ||
        ...
        return ENGINE_INDEX_WORKER_ARGV_INVALID;
    }

    engine_index_worker_invocation_t parsed = {
        .expected_build_fingerprint = argv[4],
        .args_json = argv[6],
        .response_out = argv[8],
    };
    int next = 9;
    if (next < argc && argv[next] && strcmp(argv[next], ENGINE_INDEX_WORKER_MEMORY_BUDGET_ARG) == 0) {
        if (next + 1 >= argc ||
    ...
    if (next < argc && argv[next] && strcmp(argv[next], ENGINE_INDEX_WORKER_SINGLE_THREAD_ARG) == 0) {
        parsed.single_thread = true;
        next++;
    }
    if (next < argc && argv[next] && strcmp(argv[next], ENGINE_INDEX_WORKER_MARKER_ARG) == 0) {
    ...
    if (next < argc && argv[next] && strcmp(argv[next], ENGINE_INDEX_WORKER_QUARANTINE_ARG) == 0) {
        if (next + 1 >= argc || !argv[next + 1] || !argv[next + 1][0]) {
            return ENGINE_INDEX_WORKER_ARGV_INVALID;
        }
    ...
    if (!g_build_fingerprint[0]) {
        return ENGINE_INDEX_WORKER_ARGV_BUILD_UNAVAILABLE;
    }
    if (strcmp(parsed.expected_build_fingerprint, g_build_fingerprint) != 0) {
        return ENGINE_INDEX_WORKER_ARGV_BUILD_MISMATCH;
    }
    *invocation_out = parsed;
    return ENGINE_INDEX_WORKER_ARGV_VALID;
}

const char *engine_index_worker_argv_status_message(engine_index_worker_argv_status_t status) {
    switch (status) {
    case ENGINE_INDEX_WORKER_ARGV_VALID:
        return "worker invocation accepted";
    case ENGINE_INDEX_WORKER_ARGV_INVALID:
        return "invalid internal worker arguments";
    case ENGINE_INDEX_WORKER_ARGV_BUILD_UNAVAILABLE:
        return "worker executable fingerprint could not be verified";
    case ENGINE_INDEX_WORKER_ARGV_BUILD_MISMATCH:
        return "worker executable build conflicts with its supervisor; close all engine sessions "
               "and retry";
    case ENGINE_INDEX_WORKER_ARGV_NOT_WORKER:
    default:
...
int engine_index_supervisor_spawn_count(void) {
...
int engine_index_supervisor_spawn_st_count(void) {
...
void engine_index_supervisor_mark_host(void) {
    g_host_marked = true;
}

bool engine_index_supervisor_should_wrap(void) {
    if (!g_host_marked) {
    ...
    /* Supervision is a safety boundary for the physical engine host. An ambient
     * variable must never turn a long-lived MCP/CLI parent into the indexer. */
    return true;
}

static bool supervisor_disable_requested(void) {
    char supervisor_setting[ENGINE_SZ_32] = {0};
    return engine_safe_getenv("ENGINE_INDEX_SUPERVISOR", supervisor_setting, sizeof(supervisor_setting),
                           NULL) &&
 ...
 * ENGINE_INDEX_WORKER_TIMEOUT_S override (seconds → ms) tightens it for tests. */
static int worker_quiet_timeout_ms(void) {
    enum { DEFAULT_QUIET_TIMEOUT_MS = 900000 }; /* 15 min with no progress */
    char timeout_seconds[ENGINE_SZ_32] = {0};
...
static char *slurp_worker_response(const char *path, worker_response_read_status_t *status_out) {
    *status_out = WORKER_RESPONSE_READ_ERROR;
    ...
    if ((uint64_t)n > (uint64_t)ENGINE_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
...
enum {
    INDEX_WORKER_PATH_CAP = ENGINE_SZ_4K,
...
struct engine_index_worker_handle {
    engine_subprocess_t *process;
    char response_path[INDEX_WORKER_PATH_CAP];
    char log_path[INDEX_WORKER_PATH_CAP];
    engine_proc_log_cb log_callback;
    void *log_context;
    long relay_tail_pos;
    bool process_terminal;
    engine_proc_result_t process_result;
 ...
 * left undelivered, matching engine_proc_log_cb's completed-line contract. */
static bool worker_relay_log(engine_index_worker_handle_t *handle) {
    if (!handle || !handle->log_callback) {
        return true;
    }

#ifdef _WIN32
    FILE *log = engine_fopen(handle->log_path, "rb");
#else
...
/* engine_mkstemp's Windows compatibility implementation uses an internal buffer.
...
static void worker_result_init(engine_index_worker_result_t *result) {
...
static bool worker_cache_dir(char out[INDEX_WORKER_PATH_CAP]) {
    ...
    int written = snprintf(out, INDEX_WORKER_PATH_CAP, "%s/.cache/graph-engine", home);
    if (written <= 0 || written >= INDEX_WORKER_PATH_CAP) {
        return false;
    }
    engine_normalize_path_sep(out);
...
static bool worker_result_succeeded(const engine_index_worker_result_t *result) {
    return result && result->outcome == ENGINE_PROC_CLEAN && !result->cancellation_requested &&
           result->tree_quiesced && !result->supervision_failed;
}

static void worker_terminal_log(engine_index_worker_handle_t *handle) {
    char signal_text[16];
    char exit_text[16];
    ...
    engine_log_info("index.supervisor.reap", "outcome", engine_proc_outcome_str(handle->result.outcome),
                 "exit_code", exit_text, "signal", signal_text);
    if (handle->result.response_rejected) {
        engine_log_error("index.supervisor.response_rejected", "reason", "payload_too_large", "log",
                      handle->log_path);
    } else if (handle->result.supervision_failed || !handle->result.tree_quiesced) {
        engine_log_error("index.supervisor.containment_failed", "outcome",
                      engine_proc_outcome_str(handle->result.outcome), "log", handle->log_path);
    } else if (handle->result.cancellation_requested) {
        engine_log_warn("index.supervisor.worker_cancelled", "outcome",
                     engine_proc_outcome_str(handle->result.outcome), "log", handle->log_path);
    } else if (handle->result.outcome == ENGINE_PROC_CLEAN && !engine_profile_active) {
        (void)engine_unlink(handle->log_path);
    } else if (handle->result.outcome == ENGINE_PROC_CLEAN) {
        engine_log_info("index.supervisor.profile_log", "log", handle->log_path);
    } else {
        engine_log_warn("index.supervisor.worker_failed", "outcome",
                     engine_proc_outcome_str(handle->result.outcome), "exit_code", exit_text, "log",
                     handle->log_path);
    }
}

int engine_index_worker_start_with_log(const char *args_json, size_t memory_budget_bytes,
                                    bool single_thread, const char *marker_file,
                                    const char *quarantine_file, engine_proc_log_cb log_callback,
                                    void *log_context, engine_index_worker_handle_t **handle_out) {
    if (handle_out) {
        *handle_out = NULL;
    }
    ...
    if (g_host_marked && !g_worker_active && supervisor_disable_requested()) {
        ...
        engine_log_error("index.supervisor.disable_refused", "action", "fail_closed");
        return -1;
    }
    const char *build_fingerprint = engine_index_supervisor_build_fingerprint();
    if (!build_fingerprint) {
        engine_log_error("index.supervisor.build_fingerprint_unavailable", "action", "refuse_spawn");
        return -1;
    }

    char self[INDEX_WORKER_PATH_CAP] = {0};
    if (!engine_http_server_resolve_binary_path(NULL, self, sizeof(self)) || !self[0]) {
        engine_log_error("index.supervisor.no_self_path", "action", "fail_closed");
        return -1;
    }
    engine_index_worker_handle_t *handle = calloc(1, sizeof(*handle));
    if (!handle) {
        return -1;
    }
    ...
    if (!worker_unique_file(handle->response_path, sizeof(handle->response_path), "response") ||
        ...
        (void)engine_unlink(handle->response_path);
        (void)engine_unlink(handle->log_path);
        free(handle);
        return -1;
    }
    ...
    if (engine_subprocess_spawn(&options, &handle->process) != 0) {
        (void)engine_unlink(handle->response_path);
        (void)engine_unlink(handle->log_path);
        free(handle);
        engine_log_error("index.supervisor.spawn_failed", "action", "fail_closed");
        return -1;
    }
    *handle_out = handle;
    return 0;
...
int engine_index_worker_start(const char *args_json, size_t memory_budget_bytes, bool single_thread,
                           const char *marker_file, const char *quarantine_file,
                           engine_index_worker_handle_t **handle_out) {
    return engine_index_worker_start_with_log(args_json, memory_budget_bytes, single_thread,
                                           marker_file, quarantine_file, NULL, NULL, handle_out);
}

engine_index_worker_poll_t engine_index_worker_poll(engine_index_worker_handle_t *handle,
                                              const engine_index_worker_result_t **result_out) {
    if (result_out) {
        *result_out = NULL;
    }
    if (!handle || !result_out || !handle->process) {
        return ENGINE_INDEX_WORKER_POLL_ERROR;
    }
    if (atomic_load_explicit(&handle->terminal, memory_order_acquire)) {
        *result_out = &handle->result;
        return ENGINE_INDEX_WORKER_POLL_TERMINAL;
    }
    bool relay_caught_up = true;
    if (!handle->process_terminal) {
        engine_proc_result_t process_result;
        engine_proc_poll_t state = engine_subprocess_poll(handle->process, &process_result);
        relay_caught_up = worker_relay_log(handle);
        if (state == ENGINE_PROC_POLL_RUNNING) {
            return ENGINE_INDEX_WORKER_POLL_RUNNING;
        }
    ...
    const engine_proc_result_t *process_result = &handle->process_result;
    ...
    if (worker_result_succeeded(&handle->result)) {
        ...
        if (response_status == WORKER_RESPONSE_READ_TOO_LARGE) {
            ...
            handle->result.outcome = ENGINE_PROC_EXIT_NONZERO;
...
bool engine_index_worker_request_cancel(engine_index_worker_handle_t *handle) {
    return handle && !atomic_load_explicit(&handle->terminal, memory_order_acquire) &&
           engine_subprocess_request_cancel(handle->process);
}

const char *engine_index_worker_response_path(const engine_index_worker_handle_t *handle) {
    return handle ? handle->response_path : NULL;
}

const char *engine_index_worker_log_path(const engine_index_worker_handle_t *handle) {
    return handle ? handle->log_path : NULL;
}

void engine_index_worker_destroy(engine_index_worker_handle_t *handle) {
    if (!handle) {
        return;
    }
    if (!atomic_load_explicit(&handle->terminal, memory_order_acquire)) {
        engine_log_error("index.supervisor.destroy_running", "action", "ignored");
        return;
    }
    engine_subprocess_destroy(handle->process);
    free(handle->result.response);
    handle->result.response = NULL;
    free(handle);
}

int engine_index_spawn_worker_with_log_cancel(const char *args_json, bool single_thread,
                                           const char *marker_file, const char *quarantine_file,
                                           engine_proc_log_cb log_callback, void *log_context,
    ...
    engine_index_worker_handle_t *handle = NULL;
    if (engine_index_worker_start_with_log(args_json, 0, single_thread, marker_file, quarantine_file,
                                        log_callback, log_context, &handle) != 0) {
        return -1;
    }
    const engine_index_worker_result_t *cached = NULL;
    ...
    for (;;) {
        if (!cancellation_forwarded && cancel_requested &&
            ...
            cancellation_forwarded = engine_index_worker_request_cancel(handle);
        }
        engine_index_worker_poll_t state = engine_index_worker_poll(handle, &cached);
        if (state == ENGINE_INDEX_WORKER_POLL_TERMINAL) {
            break;
        }
        if (state == ENGINE_INDEX_WORKER_POLL_ERROR) {
            (void)engine_index_worker_request_cancel(handle);
        }
        const struct timespec pause = {0, INDEX_WORKER_SYNC_POLL_NS};
        (void)engine_nanosleep(&pause, NULL);
    }
    *result = *cached;
    result->response = cached->response ? engine_strdup(cached->response) : NULL;
    engine_index_worker_destroy(handle);
    return 0;
}

int engine_index_spawn_worker_with_log(const char *args_json, bool single_thread,
                                    const char *marker_file, const char *quarantine_file,
                                    engine_proc_log_cb log_callback, void *log_context,
                                    engine_index_worker_result_t *result) {
    return engine_index_spawn_worker_with_log_cancel(args_json, single_thread, marker_file,
                                                  quarantine_file, log_callback, log_context, NULL,
                                                  result);
}

int engine_index_spawn_worker(const char *args_json, bool single_thread, const char *marker_file,
                           const char *quarantine_file, engine_index_worker_result_t *result) {
    return engine_index_spawn_worker_with_log(args_json, single_thread, marker_file, quarantine_file,
                                           NULL, NULL, result);
}

void engine_index_worker_result_free(engine_index_worker_result_t *result) {
    if (result) {
