 ...
 *
 * Translates a subset of Cypher into SQL queries against engine_store.
...
static void lex_push(engine_lex_result_t *r, engine_token_type_t type, const char *text, int pos) {
    if (r->count >= r->capacity) {
        r->capacity = r->capacity ? r->capacity * PAIR_LEN : ENGINE_SZ_32;
        r->tokens = safe_realloc(r->tokens, r->capacity * sizeof(engine_token_t));
    }
    r->tokens[r->count++] = (engine_token_t){.type = type, .text = heap_strdup(text), .pos = pos};
}

static void lex_push_n(engine_lex_result_t *r, engine_token_type_t type, const char *start, size_t len,
                       int pos) {
    if (r->count >= r->capacity) {
        r->capacity = r->capacity ? r->capacity * PAIR_LEN : ENGINE_SZ_32;
        r->tokens = safe_realloc(r->tokens, r->capacity * sizeof(engine_token_t));
    ...
    r->tokens[r->count++] =
        (engine_token_t){.type = type, .text = heap_strndup(start, len), .pos = pos};
}
...
static void lex_string_literal(const char *input, int len, int *pos, char quote,
                               engine_lex_result_t *out) {
    int start = *pos;
    char buf[ENGINE_SZ_4K];
    int blen = 0;
    const int max_blen = ENGINE_SZ_4K - 1;
...
static engine_token_type_t keyword_lookup(const char *word) {
...
static bool lex_try_two_char(const char *input, int len, int *i, engine_lex_result_t *out) {
    static const struct {
        char c1, c2;
        engine_token_type_t type;
        const char *text;
...
static engine_token_type_t lex_single_char(char c) {
    switch (c) {
    case '(':
        return TOK_LPAREN;
    case ')':
...
static bool lex_try_ident(const char *input, int len, int *i, engine_lex_result_t *out) {
    char c = input[*i];
    ...
    word[wlen] = '\0';
    engine_token_type_t type = keyword_lookup(word);
    lex_push_n(out, type, input + start, *i - start, start);
    return true;
}

/* Try to lex a number starting at position i. Returns true if matched. */
static bool lex_try_number(const char *input, int len, int *i, engine_lex_result_t *out) {
    char c = input[*i];
...
int engine_lex(const char *input, engine_lex_result_t *out) {
    ...
    while (i < len) {
        ...
        engine_token_type_t stype = lex_single_char(c);
...
void engine_lex_free(engine_lex_result_t *r) {
    if (!r) {
        return;
    }
...
static const engine_token_t *peek(parser_t *p) {
    if (p->pos >= p->count) {
...
static const engine_token_t *advance(parser_t *p) {
    if (p->pos >= p->count) {
...
static bool check(parser_t *p, engine_token_type_t type) {
    return peek(p)->type == type;
}

static bool match(parser_t *p, engine_token_type_t type) {
    if (check(p, type)) {
        advance(p);
        return true;
    }
...
static const engine_token_t *expect(parser_t *p, engine_token_type_t type) {
    if (check(p, type)) {
        return advance(p);
    }
...
static int parse_props(parser_t *p, engine_prop_filter_t **out, int *count) {
    *out = NULL;
    *count = 0;
    ...
    int n = 0;
    engine_prop_filter_t *arr = malloc(cap * sizeof(engine_prop_filter_t));
    if (!arr) {
        return ENGINE_NOT_FOUND;
    }

    while (!check(p, TOK_RBRACE) && !check(p, TOK_EOF)) {
        const engine_token_t *key = expect(p, TOK_IDENT);
        if (!key) {
            free(arr);
            return ENGINE_NOT_FOUND;
        }
        ...
        const engine_token_t *val = expect(p, TOK_STRING);
        if (!val) {
            free(arr);
            return ENGINE_NOT_FOUND;
        }
        ...
        if (n >= cap) {
            ...
            void *tmp = realloc(arr, new_cap * sizeof(engine_prop_filter_t));
            if (!tmp) {
                for (int i = 0; i < n; i++) {
...
static int parse_node(parser_t *p, engine_node_pattern_t *out) {
    memset(out, 0, sizeof(*out));
    ...
    if (match(p, TOK_COLON)) {
        const engine_token_t *label = expect(p, TOK_IDENT);
        if (!label) {
            return ENGINE_NOT_FOUND;
        }
        char lbuf[ENGINE_SZ_256];
        int ll = snprintf(lbuf, sizeof(lbuf), "%s", label->text);
        while (match(p, TOK_PIPE)) {
            const engine_token_t *alt = expect(p, TOK_IDENT);
            if (!alt) {
                return ENGINE_NOT_FOUND;
            }
...
static void parse_hop_range(parser_t *p, int *min_hops, int *max_hops) {
    if (check(p, TOK_NUMBER)) {
        int val = (int)strtol(peek(p)->text, NULL, ENGINE_DECIMAL_BASE);
        advance(p);
        if (match(p, TOK_DOTDOT)) {
            *min_hops = val;
            *max_hops =
                check(p, TOK_NUMBER) ? (int)strtol(advance(p)->text, NULL, ENGINE_DECIMAL_BASE) : 0;
        } else {
            /* *N means 1..N */
            *min_hops = SKIP_ONE;
            *max_hops = val;
...
static int parse_rel_types(parser_t *p, engine_rel_pattern_t *out) {
    int cap = CYP_INIT_CAP4;
    int n = 0;
    ...
    const engine_token_t *t = expect(p, TOK_IDENT);
    if (!t) {
        free(types);
        return ENGINE_NOT_FOUND;
    }
...
static int parse_rel_bracket(parser_t *p, engine_rel_pattern_t *out) {
    /* Optional variable */
...
static int parse_rel(parser_t *p, engine_rel_pattern_t *out) {
    memset(out, 0, sizeof(*out));
...
static void expr_free(engine_expr_t *e) {
    enum { EXPR_FREE_STACK = 128 };
    engine_expr_t *stack[EXPR_FREE_STACK];
    int top = 0;
    if (e) {
        stack[top++] = e;
    }
    while (top > 0) {
        engine_expr_t *cur = stack[--top];
...
static engine_expr_t *expr_leaf(engine_condition_t c) {
    engine_expr_t *e = calloc(ENGINE_ALLOC_ONE, sizeof(engine_expr_t));
    e->type = EXPR_CONDITION;
    e->cond = c;
    return e;
}

static engine_expr_t *expr_binary(engine_expr_type_t type, engine_expr_t *left, engine_expr_t *right) {
    engine_expr_t *e = calloc(ENGINE_ALLOC_ONE, sizeof(engine_expr_t));
    e->type = type;
    e->left = left;
    e->right = right;
...
static engine_expr_t *expr_not(engine_expr_t *child) {
    engine_expr_t *e = calloc(ENGINE_ALLOC_ONE, sizeof(engine_expr_t));
    e->type = EXPR_NOT;
    e->left = child;
    return e;
...
static const char *unsupported_clause_error(engine_token_type_t type) {
    switch (type) {
    case TOK_CREATE:
...
static engine_expr_t *parse_or_expr(parser_t *p);
/* Multi-arg scalar function support, shared with the RETURN-item parser (#874) */
static bool is_multiarg_func_call(parser_t *p);
static int parse_multiarg_func_item(parser_t *p, engine_return_item_t *item);

/* Free a multi-arg function argument array. */
static void func_args_free(engine_func_arg_t *args, int count) {
...
static void func_item_fields_free(engine_return_item_t *item) {
...
static void cond_func_fields_free(engine_condition_t *c) {
...
static engine_expr_t *parse_in_list(parser_t *p, engine_condition_t *c) {
    advance(p);
    c->op = heap_strdup("IN");
    if (!c->op) {
...
static void free_one_node_pattern(engine_node_pattern_t *n) {
...
static void free_one_rel_pattern(engine_rel_pattern_t *r) {
...
static engine_expr_t *parse_exists_predicate(parser_t *p, bool negated) {
    advance(p); /* EXISTS */
    if (!match(p, TOK_LBRACE)) {
        snprintf(p->error, sizeof(p->error), "expected '{' after EXISTS at pos %d", peek(p)->pos);
        return NULL;
    }
    engine_node_pattern_t anchor = {0};
    engine_rel_pattern_t rel = {0};
    engine_node_pattern_t far_node = {0};
...
static engine_expr_t *parse_condition_op(parser_t *p, engine_condition_t *c) {
    /* IS NULL / IS NOT NULL */
    if (check(p, TOK_IS)) {
        advance(p);
        if (match(p, TOK_NOT)) {
    ...
    /* IN [...] */
    if (check(p, TOK_IN)) {
        engine_expr_t *e = parse_in_list(p, c);
        if (!e) {
 ...
 * Returns ENGINE_NOT_FOUND on error, 0 when an operator/value should follow, and
 * COND_LHS_COMPLETE when the condition is already complete (label test). */
static int parse_condition_lhs(parser_t *p, engine_condition_t *c) {
    ...
    const engine_token_t *var = expect(p, TOK_IDENT);
    if (!var) {
        return ENGINE_NOT_FOUND;
    }
    ...
    if (check(p, TOK_COLON)) {
        advance(p);
        const engine_token_t *lbl = expect(p, TOK_IDENT);
        if (!lbl) {
            return ENGINE_NOT_FOUND;
        }
    ...
    if (match(p, TOK_DOT)) {
        const engine_token_t *prop = expect(p, TOK_IDENT);
        if (!prop) {
            return ENGINE_NOT_FOUND;
        }
        c->variable = heap_strdup(var->text);
...
static engine_expr_t *parse_condition_expr(parser_t *p) {
...
/* Atom: ( expr ) | condition */
static engine_expr_t *parse_atom_expr(parser_t *p) { // NOLINT(misc-no-recursion)
    if (match(p, TOK_LPAREN)) {
        if (!parse_depth_enter(p)) {
            return NULL;
        }
        engine_expr_t *e = parse_or_expr(p);
        parse_depth_leave(p);
        expect(p, TOK_RPAREN);
        return e;
    }
...
/* NOT: NOT atom | atom */
static engine_expr_t *parse_not_expr(parser_t *p) { // NOLINT(misc-no-recursion)
    if (match(p, TOK_NOT)) {
        if (!parse_depth_enter(p)) {
            return NULL;
        }
        engine_expr_t *child = parse_not_expr(p);
        parse_depth_leave(p);
...
/* AND: not (AND not)* */
static engine_expr_t *parse_and_expr(parser_t *p) { // NOLINT(misc-no-recursion)
    engine_expr_t *left = parse_not_expr(p);
    if (!left) {
        return NULL;
    }
    while (check(p, TOK_AND)) {
        advance(p);
        engine_expr_t *right = parse_not_expr(p);
        if (!right) {
            expr_free(left);
            return NULL;
        }
...
/* XOR: and (XOR and)* */
static engine_expr_t *parse_xor_expr(parser_t *p) { // NOLINT(misc-no-recursion)
    engine_expr_t *left = parse_and_expr(p);
    if (!left) {
        return NULL;
    }
    while (check(p, TOK_XOR)) {
        advance(p);
        engine_expr_t *right = parse_and_expr(p);
        if (!right) {
            expr_free(left);
            return NULL;
        }
...
/* OR: xor (OR xor)* */
static engine_expr_t *parse_or_expr(parser_t *p) { // NOLINT(misc-no-recursion)
    engine_expr_t *left = parse_xor_expr(p);
    if (!left) {
        return NULL;
    }
    while (check(p, TOK_OR)) {
        advance(p);
        engine_expr_t *right = parse_xor_expr(p);
        if (!right) {
            expr_free(left);
            return NULL;
        }
...
static int parse_where(parser_t *p, engine_where_clause_t **out) {
    if (!match(p, TOK_WHERE)) {
        *out = NULL;
        return 0;
    }

    engine_where_clause_t *w = calloc(ENGINE_ALLOC_ONE, sizeof(engine_where_clause_t));
...
static const char *str_func_name(engine_token_type_t t) {
    switch (t) {
    case TOK_TOLOWER:
        return "toLower";
    case TOK_TOUPPER:
...
static const char *parse_value_literal(parser_t *p) {
    ...
    if (check(p, TOK_IDENT)) {
        ...
        const engine_token_t *v = advance(p);
        if (match(p, TOK_DOT)) {
            const engine_token_t *pr = expect(p, TOK_IDENT);
...
static engine_case_expr_t *parse_case_expr(parser_t *p) {
    /* CASE already consumed */
    engine_case_expr_t *kase = calloc(ENGINE_ALLOC_ONE, sizeof(engine_case_expr_t));
    if (!kase) {
        return NULL;
    }
    int bcap = CYP_INIT_CAP4;
    kase->branches = malloc(bcap * sizeof(engine_case_branch_t));
    if (!kase->branches) {
        free(kase);
        return NULL;
    }
    ...
    while (check(p, TOK_WHEN)) {
        advance(p);
        engine_expr_t *when = parse_or_expr(p);
        ...
        if (kase->branch_count >= bcap) {
            ...
            void *tmp = realloc(kase->branches, new_bcap * sizeof(engine_case_branch_t));
            if (!tmp) {
                expr_free(when);
        ...
        kase->branches[kase->branch_count++] =
            (engine_case_branch_t){.when_expr = when, .then_val = then_val};
    }
...
static int parse_var_dot_prop(parser_t *p, engine_return_item_t *item) {
    const engine_token_t *var = expect(p, TOK_IDENT);
    if (!var) {
        return ENGINE_NOT_FOUND;
    }
...
static int parse_named_func_item(parser_t *p, engine_return_item_t *item) {
...
static int parse_func_arg(parser_t *p, engine_func_arg_t *arg) {
...
static int parse_multiarg_func_item(parser_t *p, engine_return_item_t *item) {
    ...
    expect(p, TOK_LPAREN);
    int cap = CYP_INIT_CAP4;
    item->args = malloc((size_t)cap * sizeof(engine_func_arg_t));
    item->arg_count = 0;
    while (!check(p, TOK_RPAREN) && !check(p, TOK_EOF)) {
        ...
        if (item->arg_count >= cap) {
            cap *= PAIR_LEN;
            item->args = safe_realloc(item->args, (size_t)cap * sizeof(engine_func_arg_t));
        }
...
static int parse_aggregate_item(parser_t *p, engine_return_item_t *item) {
    engine_token_type_t ft = peek(p)->type;
    advance(p);
    expect(p, TOK_LPAREN);
...
static int parse_string_func_item(parser_t *p, engine_return_item_t *item) {
    engine_token_type_t ft = peek(p)->type;
    advance(p);
    expect(p, TOK_LPAREN);
    if (parse_var_dot_prop(p, item) < 0) {
...
static int parse_return_item(parser_t *p, engine_return_item_t *item) {
    ...
    /* Optional AS alias */
    if (match(p, TOK_AS)) {
        const engine_token_t *alias = expect(p, TOK_IDENT);
        if (alias) {
 ...
 * success, ENGINE_NOT_FOUND when the key list exceeds the modeled maximum. */
static int parse_order_by_clause(parser_t *p, engine_return_clause_t *r) {
    expect(p, TOK_BY);
    do {
        if (r->order_key_count >= ENGINE_CYPHER_ORDER_KEYS_MAX) {
            return ENGINE_NOT_FOUND;
        }
        char order_buf[ENGINE_SZ_256];
...
static void free_return_clause(engine_return_clause_t *r);

/* Parse RETURN/WITH clause (shared logic) */
static int parse_return_or_with(parser_t *p, engine_return_clause_t **out, bool is_with) {
    engine_token_type_t tok = (int)is_with ? TOK_WITH : TOK_RETURN;
    ...
    engine_return_clause_t *r = calloc(ENGINE_ALLOC_ONE, sizeof(engine_return_clause_t));
    ...
    r->items = malloc(cap * sizeof(engine_return_item_t));
    ...
    do {
        ...
        if (r->count >= cap) {
            cap *= PAIR_LEN;
            r->items = safe_realloc(r->items, cap * sizeof(engine_return_item_t));
        }
...
tail:
    ...
    /* Optional SKIP */
    if (match(p, TOK_SKIP)) {
        const engine_token_t *num = expect(p, TOK_NUMBER);
        if (num) {
            r->skip = (int)strtol(num->text, NULL, ENGINE_DECIMAL_BASE);
        }
    }
    ...
    if (match(p, TOK_LIMIT)) {
        const engine_token_t *num = expect(p, TOK_NUMBER);
        if (num) {
            r->limit = (int)strtol(num->text, NULL, ENGINE_DECIMAL_BASE);
        }
    }
...
/* Parse RETURN clause */
static int parse_return(parser_t *p, engine_return_clause_t **out) {
    return parse_return_or_with(p, out, false);
}

/* Parse a single MATCH pattern into pat */
static int parse_match_pattern(parser_t *p, engine_pattern_t *pat) {
    memset(pat, 0, sizeof(*pat));
    int node_cap = CYP_INIT_CAP4;
    int rel_cap = CYP_INIT_CAP4;
    pat->nodes = malloc(node_cap * sizeof(engine_node_pattern_t));
    pat->rels = calloc(rel_cap, sizeof(engine_rel_pattern_t));
    ...
    while (check(p, TOK_DASH) || check(p, TOK_LT)) {
        if (pat->rel_count >= rel_cap) {
            rel_cap *= PAIR_LEN;
            pat->rels = safe_realloc(pat->rels, rel_cap * sizeof(engine_rel_pattern_t));
        }
        ...
        if (pat->node_count >= node_cap) {
            node_cap *= PAIR_LEN;
            pat->nodes = safe_realloc(pat->nodes, node_cap * sizeof(engine_node_pattern_t));
        }
...
static void parse_unwind_clause(parser_t *p, engine_query_t *q) {
    advance(p);
    if (check(p, TOK_LBRACKET)) {
        /* Literal list: [1, 2, 3] — collect as JSON array string */
        advance(p);
        char buf[ENGINE_SZ_2K] = "[";
        int blen = SKIP_ONE;
        /* snprintf returns the length it WOULD have written, so a single
         * oversized token (string literals lex up to ENGINE_SZ_4K-1) can push
...
static int parse_match_chain(parser_t *p, engine_query_t *q, int *pat_cap) {
    while (check(p, TOK_MATCH) || check(p, TOK_OPTIONAL)) {
        ...
        if (q->pattern_count >= *pat_cap) {
            *pat_cap *= PAIR_LEN;
            q->patterns = safe_realloc(q->patterns, *pat_cap * sizeof(engine_pattern_t));
...
static int parse_post_where(parser_t *p, engine_query_t *q, // NOLINT(misc-no-recursion)
                            int *pat_cap) {
    ...
    if (check(p, TOK_UNION)) {
        ...
        engine_parse_result_t sub = {0};
        if (engine_parse(&p->tokens[p->pos], p->count - p->pos, &sub) < 0) {
            if (sub.error) {
                snprintf(p->error, sizeof(p->error), "%s", sub.error);
            }
            engine_parse_free(&sub);
            return ENGINE_NOT_FOUND;
        }
...
int engine_parse(const engine_token_t *tokens, int token_count, // NOLINT(misc-no-recursion)
              engine_parse_result_t *out) {
    memset(out, 0, sizeof(*out));
    ...
    engine_query_t *q = calloc(ENGINE_ALLOC_ONE, sizeof(engine_query_t));
    ...
    q->patterns = malloc(pat_cap * sizeof(engine_pattern_t));
...
void engine_parse_free(engine_parse_result_t *r) {
    if (!r) {
        return;
    }
    engine_query_free(r->query);
    free(r->error);
    memset(r, 0, sizeof(*r));
}
...
static void free_pattern(engine_pattern_t *pat) {
    for (int i = 0; i < pat->node_count; i++) {
        engine_node_pattern_t *n = &pat->nodes[i];
        safe_str_free(&n->variable);
        safe_str_free(&n->label);
        for (int j = 0; j < n->prop_count; j++) {
            safe_str_free(&n->props[j].key);
    ...
    for (int i = 0; i < pat->rel_count; i++) {
        engine_rel_pattern_t *r = &pat->rels[i];
        safe_str_free(&r->variable);
        for (int j = 0; j < r->type_count; j++) {
            safe_str_free(&r->types[j]);
        }
...
static void free_where(engine_where_clause_t *w) {
    if (!w) {
        return;
    }
    expr_free(w->root);
...
static void free_case_expr(engine_case_expr_t *k) {
    if (!k) {
        return;
    }
...
void engine_query_free(engine_query_t *q) {
    while (q) {
        engine_query_t *next = q->union_next;
...
int engine_cypher_parse(const char *query, engine_query_t **out, char **error) {
    *out = NULL;
    *error = NULL;

    engine_lex_result_t lr = {0};
    if (engine_lex(query, &lr) < 0 || lr.error) {
        *error = heap_strdup(lr.error ? lr.error : "lex error");
        engine_lex_free(&lr);
        return ENGINE_NOT_FOUND;
    }

    engine_parse_result_t pr = {0};
    if (engine_parse(lr.tokens, lr.count, &pr) < 0) {
        *error = heap_strdup(pr.error ? pr.error : "parse error");
        engine_parse_free(&pr);
        engine_lex_free(&lr);
        return ENGINE_NOT_FOUND;
    }
...
typedef struct {
    ...
    engine_node_t var_nodes[CYP_MAX_VARS];  /* node data */
    int var_count;
    const char *edge_var_names[CYP_MAX_EDGE_VARS]; /* variable names (edges) */
    engine_edge_t edge_vars[CYP_MAX_EDGE_VARS];       /* edge data */
    int edge_var_count;
    engine_store_t *store; /* for computing in_degree/out_degree on demand */
} binding_t;

/* Return a string field from a node by property name.  NULL-safe. */
static const char *node_string_field(const engine_node_t *n, const char *prop) {
    static const struct {
        const char *key;
        size_t offset;
    } fields[] = {
        {"name", offsetof(engine_node_t, name)},
        {"qualified_name", offsetof(engine_node_t, qualified_name)},
        /* Aliases: field-eval agents reach for the short names, and a miss
         * used to return a silent empty column costing a round-trip. */
        {"qn", offsetof(engine_node_t, qualified_name)},
        {"label", offsetof(engine_node_t, label)},
        {"file_path", offsetof(engine_node_t, file_path)},
        {"file", offsetof(engine_node_t, file_path)},
        {"path", offsetof(engine_node_t, file_path)},
    };
...
static void node_fields_free(engine_node_t *n); /* defined below; used by the stub re-fetch */

static const char *node_prop(const engine_node_t *n, const char *prop, engine_store_t *store) {
    if (!n || !prop) {
        return "";
    }
    ...
    if (store && (strcmp(prop, "in_degree") == 0 || strcmp(prop, "out_degree") == 0)) {
        int in_deg = 0;
        int out_deg = 0;
        engine_store_node_degree(store, n->id, &in_deg, &out_deg);
        int val = (strcmp(prop, "in_degree") == 0) ? in_deg : out_deg;
        snprintf(out, ENGINE_SZ_512, "%d", val);
        return out;
    }
    ...
    if (n->properties_json && n->properties_json[0] == '{') {
        const char *v = json_extract_prop(n->properties_json, prop, out, ENGINE_SZ_512);
        if (v && v[0]) {
            return v;
        }
    }
    ...
    if (store && n->id > 0 && !n->file_path && !n->label) {
        engine_node_t full = {0};
        if (engine_store_find_node_by_id(store, n->id, &full) == ENGINE_STORE_OK) {
            const char *res = NULL;
            const char *rv = node_string_field(&full, prop);
            if (rv && rv[0]) {
                snprintf(out, ENGINE_SZ_512, "%s", rv);
                res = out;
            } else if (strcmp(prop, "start_line") == 0) {
                snprintf(out, ENGINE_SZ_512, "%d", full.start_line);
                res = out;
            } else if (strcmp(prop, "end_line") == 0) {
                snprintf(out, ENGINE_SZ_512, "%d", full.end_line);
                res = out;
            } else if (full.properties_json && full.properties_json[0] == '{') {
                const char *jv = json_extract_prop(full.properties_json, prop, out, ENGINE_SZ_512);
                if (jv && jv[0]) {
                    res = out;
                }
            }
 ...
 * in the same row). */
static const char *edge_prop(const engine_edge_t *e, const char *prop) {
    if (!e || !prop) {
        return "";
    }
    ...
    static ENGINE_TLS char ebufs[CYP_BUF_8][ENGINE_SZ_512];
    static ENGINE_TLS int ebuf_idx = 0;
    char *buf = ebufs[ebuf_idx++ & CYP_EBUF_MASK];
    json_extract_prop(e->properties_json, prop, buf, ENGINE_SZ_512);
    return buf;
}

/* Find an edge variable in a binding */
static engine_edge_t *binding_get_edge(binding_t *b, const char *var) {
...
static engine_node_t *binding_get(binding_t *b, const char *var) {
...
static void node_deep_copy(engine_node_t *dst, const engine_node_t *src) {
    *dst = *src;
...
static void node_fields_free(engine_node_t *n) {
    if (!n) {
        return;
    }
...
static void edge_deep_copy(engine_edge_t *dst, const engine_edge_t *src) {
    *dst = *src;
    dst->project = heap_strdup(src->project);
...
static void edge_fields_free(engine_edge_t *e) {
...
static void binding_set_edge(binding_t *b, const char *var, const engine_edge_t *edge) {
...
static void binding_set(binding_t *b, const char *var, const engine_node_t *node) {
    /* Check existing — free old fields first */
    for (int i = 0; i < b->var_count; i++) {
        if (strcmp(b->var_names[i], var) == 0) {
            node_fields_free(&b->var_nodes[i]);
...
static const char *eval_multiarg_func(binding_t *b, const engine_return_item_t *item, char *buf,
                                      size_t bufsz);

/* Resolve the actual property value for a condition from a binding */
static const char *resolve_condition_value(const engine_condition_t *c, binding_t *b) {
    /* Multi-arg scalar function LHS: coalesce(f.depth, 0) >= 2 (#874).
    ...
    if (c->func) {
        static _Thread_local char func_buf[ENGINE_SZ_512];
        engine_return_item_t item = {0};
    ...
    engine_edge_t *e = binding_get_edge(b, c->variable);
    if (e) {
        return edge_prop(e, c->property);
    }
    engine_node_t *n = binding_get(b, c->variable);
    if (!n) {
...
static bool eval_comparison_op(const char *op, const char *actual, const char *expected) {
    ...
    if (strcmp(op, "=~") == 0) {
        engine_regex_t re;
        if (engine_regcomp(&re, expected, ENGINE_REG_EXTENDED | ENGINE_REG_NOSUB) != 0) {
            return false;
        }
        int rc = engine_regexec(&re, actual, 0, NULL, 0);
        engine_regfree(&re);
        return rc == 0;
    }
...
static bool eval_condition(const engine_condition_t *c, binding_t *b) {
    ...
    if (strcmp(c->op, "EXISTS") == 0) {
        ...
        bool result = false;
        if (n && b->store) {
            engine_edge_t *edges = NULL;
            int cnt = 0;
            if (c->exists_dir != 1) { /* outbound or any */
                if (c->value) {
                    engine_store_find_edges_by_source_type(b->store, n->id, c->value, &edges, &cnt);
                } else {
                    engine_store_find_edges_by_source(b->store, n->id, &edges, &cnt);
                }
                result = cnt > 0;
                engine_store_free_edges(edges, cnt);
            }
            if (!result && c->exists_dir != 0) { /* inbound or any */
                edges = NULL;
                cnt = 0;
                if (c->value) {
                    engine_store_find_edges_by_target_type(b->store, n->id, c->value, &edges, &cnt);
                } else {
                    engine_store_find_edges_by_target(b->store, n->id, &edges, &cnt);
                }
                result = cnt > 0;
                engine_store_free_edges(edges, cnt);
            }
...
static bool eval_expr(const engine_expr_t *e, binding_t *b) { // NOLINT(misc-no-recursion)
    if (!e) {
        return true;
    }
    switch (e->type) {
...
static bool eval_where(const engine_where_clause_t *w, binding_t *b) {
    if (!w) {
        return true;
    }
    if (w->root) {
...
static bool check_inline_props(const engine_node_t *n, const engine_prop_filter_t *props, int count,
                               engine_store_t *store) {
    for (int i = 0; i < count; i++) {
        const char *actual = node_prop(n, props[i].key, store);
        if (looks_like_regex(props[i].value)) {
            engine_regex_t re;
            if (engine_regcomp(&re, props[i].value, ENGINE_REG_EXTENDED | ENGINE_REG_NOSUB) == 0) {
                bool matched = engine_regexec(&re, actual, 0, NULL, 0) == 0;
                engine_regfree(&re);
                if (!matched) {
                    return false;
                }
...
static void cypher_deadline_arm(void) {
    ...
    g_cypher_deadline_ms = engine_now_ms() + (uint64_t)budget;
}
...
static bool cypher_deadline_exceeded(void) {
    ...
    if (engine_now_ms() >= g_cypher_deadline_ms) {
...
void engine_cypher_test_set_deadline_ms(int64_t budget_ms) {
...
static const char *binding_get_virtual(binding_t *b, const char *var, const char *prop) {
    ...
    engine_edge_t *e = binding_get_edge(b, var);
    if (e) {
    ...
    engine_node_t *n = binding_get(b, var);
    if (n) {
        if (prop) {
...
static const char *apply_string_func(const char *func, const char *val, char *buf, size_t buf_sz) {
    ...
    if (strcmp(func, "toInteger") == 0) {
        char *end = NULL;
        long long v = strtoll(val, &end, ENGINE_DECIMAL_BASE);
        if (end == val) {
...
static const char *eval_case_expr(const engine_case_expr_t *k, binding_t *b) {
    if (!k) {
        return "";
    }
    for (int i = 0; i < k->branch_count; i++) {
 ...
 * container is freed. */
static void scan_alternation_labels(engine_store_t *store, const char *project, const char *labels,
                                    engine_node_t **out_nodes, int *out_count) {
    *out_nodes = NULL;
    *out_count = 0;
    int cap = 0;
    ...
    for (char *tok = strtok_r(copy, "|", &save); tok; tok = strtok_r(NULL, "|", &save)) {
        engine_node_t *part = NULL;
        int pc = 0;
        engine_store_find_nodes_by_label(store, project, tok, &part, &pc);
        if (pc > 0 && part) {
            if (*out_count + pc > cap) {
                cap = (*out_count + pc) * PAIR_LEN;
                *out_nodes = safe_realloc(*out_nodes, (size_t)cap * sizeof(engine_node_t));
            }
            memcpy(*out_nodes + *out_count, part, (size_t)pc * sizeof(engine_node_t));
            *out_count += pc;
        }
...
static void scan_pattern_nodes(engine_store_t *store, const char *project, int max_rows,
                               engine_node_pattern_t *first, engine_node_t **out_nodes, int *out_count) {
    if (first->label && strchr(first->label, '|')) {
        scan_alternation_labels(store, project, first->label, out_nodes, out_count);
    } else if (first->label) {
        engine_store_find_nodes_by_label(store, project, first->label, out_nodes, out_count);
    } else {
        engine_search_params_t params = {.project = project,
                                      .min_degree = CYP_FOUND_NONE,
                                      .max_degree = CYP_FOUND_NONE,
                                      .limit = max_rows * CYP_GROWTH_10};
        engine_search_output_t sout = {0};
        engine_store_search(store, &params, &sout);
        *out_count = sout.count;
        *out_nodes = malloc(sout.count * sizeof(engine_node_t));
...
static void process_edges(engine_store_t *store, engine_edge_t *edges, int edge_count, bool inbound,
                          const engine_node_pattern_t *target_node, binding_t *b, const char *to_var,
    ...
    engine_node_t *bound_to = binding_get(b, to_var);
    ...
    for (int ei = 0; ei < edge_count; ei++) {
        ...
        engine_node_t found = {0};
        if (engine_store_find_node_by_id(store, tid, &found) != ENGINE_STORE_OK) {
            continue;
        }
 ...
 * during the CURRENT execution; engine_cypher_execute turns it into
...
static void expand_var_length(engine_store_t *store, engine_rel_pattern_t *rel,
                              engine_node_pattern_t *target_node, binding_t *b, engine_node_t *src,
     ...
     * verbatim, driving engine_store_bfs to an unbounded hop count (#887). WARN on
     * clamp — never a silent truncation. */
    int depth_cap = engine_cypher_max_depth();
    ...
    if (max_depth > depth_cap) {
        ...
        engine_log_warn("cypher.depth_capped", "requested", req_buf, "cap", cap_buf);
    ...
    engine_store_bfs(store, src->id, dir, rel->types, rel->type_count, max_depth, ENGINE_PERCENT, &tr);
    ...
    for (int v = 0; v < tr.visited_count; v++) {
        engine_node_hop_t *hop = &tr.visited[v];
...
static void expand_fixed_length(engine_store_t *store, engine_rel_pattern_t *rel,
                                engine_node_pattern_t *target_node, binding_t *b, engine_node_t *src,
                                const char *to_var, binding_t *new_bindings, int *new_count,
                                int max_new, int *match_count) {
    ...
    if (rel->type_count > 0) {
        for (int ti = 0; ti < rel->type_count; ti++) {
            ...
            int edge_count = 0;
            if (is_inbound) {
                engine_store_find_edges_by_target_type(store, src->id, rel->types[ti], &edges,
                                                    &edge_count);
            } else {
                engine_store_find_edges_by_source_type(store, src->id, rel->types[ti], &edges,
                                                    &edge_count);
            }
            process_edges(store, edges, edge_count, is_inbound, target_node, b, to_var, rel_var,
                          new_bindings, new_count, max_new, match_count);
            engine_store_free_edges(edges, edge_count);
        }
        if (is_any) {
            for (int ti = 0; ti < rel->type_count; ti++) {
                engine_edge_t *edges = NULL;
        ...
        int edge_count = 0;
        if (is_inbound) {
            engine_store_find_edges_by_target(store, src->id, &edges, &edge_count);
        } else {
            engine_store_find_edges_by_source(store, src->id, &edges, &edge_count);
        }
        process_edges(store, edges, edge_count, is_inbound, target_node, b, to_var, rel_var,
                      new_bindings, new_count, max_new, match_count);
        engine_store_free_edges(edges, edge_count);
...
static void expand_pattern_rels(engine_store_t *store, engine_pattern_t *pat, binding_t **bindings,
    ...
    for (int ri = 0; ri < pat->rel_count; ri++) {
        ...
        engine_rel_pattern_t *rel = &pat->rels[ri];
        engine_node_pattern_t *target_node = &pat->nodes[ri + SKIP_ONE];
        ...
        for (int bi = 0; bi < *bind_count; bi++) {
            ...
            engine_node_t *src = binding_get(b, *var_name);
            if (!src) {
                continue;
            }
...
static int rb_find_order_column(const result_builder_t *rb, const engine_return_clause_t *ret,
                                const char *key) {
...
static void rb_apply_order_by(result_builder_t *rb, const engine_return_clause_t *ret) {
     ...
     * the whole ORDER BY). */
    int cols[ENGINE_CYPHER_ORDER_KEYS_MAX];
    bool numeric[ENGINE_CYPHER_ORDER_KEYS_MAX];
    bool descs[ENGINE_CYPHER_ORDER_KEYS_MAX];
    int keys = 0;
    ...
    for (int i = 0; i < rb->row_count - SKIP_ONE; i++) {
        for (int j = 0; j < rb->row_count - i - SKIP_ONE; j++) {
            ...
            for (int k = 0; k < keys && cmp == 0; k++) {
                if (numeric[k]) {
                    cmp = (int)strtol(rb->rows[j][cols[k]], NULL, ENGINE_DECIMAL_BASE) -
                          (int)strtol(rb->rows[j + SKIP_ONE][cols[k]], NULL, ENGINE_DECIMAL_BASE);
                } else {
...
static const char *node_keys_list(const engine_node_t *n, char *buf, size_t buf_sz) {
    const struct {
        const char *k;
        bool present;
    } ks[] = {
...
static const char *eval_func_arg(binding_t *b, const engine_func_arg_t *a) {
    if (a->literal) {
        return a->literal;
    }
...
static const char *eval_multiarg_func(binding_t *b, const engine_return_item_t *item, char *buf,
    ...
    if (strcmp(f, "substring") == 0 && n >= 2) {
        ...
        long start = strtol(eval_func_arg(b, &item->args[1]), NULL, ENGINE_DECIMAL_BASE);
        ...
        if (n >= 3) {
            long len = strtol(eval_func_arg(b, &item->args[2]), NULL, ENGINE_DECIMAL_BASE);
            if (len < 0) {
                len = 0;
            }
            if ((size_t)len < take) {
    ...
    if ((strcmp(f, "left") == 0 || strcmp(f, "right") == 0) && n >= 2) {
        const char *s = eval_func_arg(b, &item->args[0]);
        long k = strtol(eval_func_arg(b, &item->args[1]), NULL, ENGINE_DECIMAL_BASE);
        if (k < 0) {
            k = 0;
        }
        size_t slen = strlen(s);
...
static const char *project_item(binding_t *b, engine_return_item_t *item, char *func_buf,
                                size_t buf_sz) {
    if (item->kase) {
    ...
    if (item->func) {
        if (strcmp(item->func, "labels") == 0) {
            engine_node_t *n = binding_get(b, item->variable);
            if (n && n->label) {
        ...
        if (strcmp(item->func, "type") == 0) {
            engine_edge_t *e = binding_get_edge(b, item->variable);
...
static void sort_bindings(binding_t *vbindings, int count, const engine_return_clause_t *wc) {
...
static void with_sort_skip_limit(const engine_return_clause_t *wc, binding_t *vbindings, int *vcount) {
...
static const char *resolve_item_alias(const engine_return_item_t *item, char *name_buf,
                                      size_t buf_sz) {
    if (item->alias) {
        return item->alias;
    }
...
static int with_agg_build_key(engine_return_clause_t *wc, binding_t *b, char *key, size_t key_sz) {
    int kl = 0;
    for (int ci = 0; ci < wc->count; ci++) {
        if (is_aggregate_func(wc->items[ci].func)) {
            continue;
        }
        char vbuf[ENGINE_SZ_512];
...
static int with_agg_find_or_create(with_agg_t **aggs, int *agg_cnt, int *agg_cap,
                                   engine_return_clause_t *wc, binding_t *b, const char *key) {
    ...
    for (int ci = 0; ci < wc->count; ci++) {
        ...
        if (!wc->items[ci].func && !wc->items[ci].property && wc->items[ci].variable) {
            engine_node_t *gn = binding_get(b, wc->items[ci].variable);
            if (gn) {
...
static void with_agg_accumulate(with_agg_t *agg, engine_return_clause_t *wc, binding_t *b) {
    for (int ci = 0; ci < wc->count; ci++) {
        if (!is_aggregate_func(wc->items[ci].func)) {
            continue;
        }
...
static void with_add_vbinding_var(binding_t *vb, const char *alias, const char *val) {
    engine_node_t vn = {.name = heap_strdup(val), .qualified_name = heap_strdup(alias)};
...
static void execute_with_aggregate(engine_return_clause_t *wc, binding_t *bindings, int bind_count,
                                   binding_t **vbindings, int *vcount) {
    int agg_cap = ENGINE_SZ_256;
...
static void execute_with_simple(engine_return_clause_t *wc, binding_t *bindings, int bind_count,
                                binding_t *vbindings, int *vcount) {
    for (int bi = 0; bi < bind_count; bi++) {
        binding_t vb = {0};
...
static void filter_bindings_where(const engine_where_clause_t *where, binding_t *vbindings,
                                  int *vcount) {
    int kept = 0;
...
static void with_proj_key(engine_return_clause_t *wc, binding_t *b, char *key, size_t key_sz) {
    int kl = 0;
    key[0] = '\0';
    char name_buf[ENGINE_SZ_256];
    for (int ci = 0; ci < wc->count; ci++) {
...
static void with_apply_distinct(engine_return_clause_t *wc, binding_t *vbindings, int *vcount) {
    int kept = 0;
    for (int i = 0; i < *vcount; i++) {
        char key[ENGINE_SZ_1K];
...
static void execute_with_clause(engine_query_t *q, binding_t **bindings_ptr, int *bind_count_ptr) {
    engine_return_clause_t *wc = q->with_clause;
    if (!wc) {
        return;
    }
...
static int collect_pattern_vars(engine_query_t *q, const char **vars, int max_vars) {
    int vc = 0;
...
static void build_star_columns(result_builder_t *rb, const char **vars, int vc) {
    ...
    const char *col_names[ENGINE_SZ_128];
    for (int v = 0; v < vc; v++) {
...
static void project_star_var(binding_t *b, const char *var, const char **vals) {
    engine_edge_t *edge = binding_get_edge(b, var);
    if (edge) {
...
static void execute_return_star(engine_query_t *q, binding_t *bindings, int bind_count, int max_rows,
                                result_builder_t *rb) {
    const char *vars[ENGINE_SZ_32];
    int vc = collect_pattern_vars(q, vars, ENGINE_SZ_32);
    build_star_columns(rb, vars, vc);
    for (int bi = 0; bi < bind_count && rb->row_count < max_rows; bi++) {
        const char *vals[ENGINE_SZ_128];
...
static void ret_agg_accumulate(ret_agg_entry_t *entry, engine_return_clause_t *ret, binding_t *b) {
    for (int ci = 0; ci < ret->count; ci++) {
...
static void ret_agg_build_key(engine_return_clause_t *ret, binding_t *b, char *key, size_t key_sz,
                              const char **vals, char valbufs[][ENGINE_SZ_512]) {
    int klen = 0;
    for (int ci = 0; ci < ret->count; ci++) {
        if (is_aggregate_func(ret->items[ci].func)) {
            vals[ci] = "0";
        ...
        const char *v = project_item(b, &ret->items[ci], valbufs[ci], ENGINE_SZ_512);
        if (v != valbufs[ci]) {
            snprintf(valbufs[ci], ENGINE_SZ_512, "%s", v ? v : "");
        }
        vals[ci] = valbufs[ci];
...
static void ret_agg_emit_row(engine_return_clause_t *ret, ret_agg_entry_t *agg, result_builder_t *rb) {
    const char *row[ENGINE_SZ_32];
    char bufs[ENGINE_SZ_32][ENGINE_SZ_64];
    for (int ci = 0; ci < ret->count; ci++) {
        if (!is_aggregate_func(ret->items[ci].func)) {
...
static void execute_return_agg(engine_return_clause_t *ret, binding_t *bindings, int bind_count,
                               result_builder_t *rb) {
    int agg_cap = ENGINE_SZ_256;
    ...
    for (int bi = 0; bi < bind_count; bi++) {
        ...
        char key[ENGINE_SZ_1K] = "";
        const char *vals[ENGINE_SZ_32];
        char valbufs[ENGINE_SZ_32][ENGINE_SZ_512];
...
static void build_return_columns(result_builder_t *rb, engine_return_clause_t *ret) {
    const char *col_names[ENGINE_SZ_32];
    for (int i = 0; i < ret->count && i < ENGINE_SZ_32; i++) {
        engine_return_item_t *item = &ret->items[i];
        if (item->alias) {
...
static void execute_return_simple(engine_return_clause_t *ret, binding_t *bindings, int bind_count,
    ...
    for (int bi = 0; bi < bind_count && rb->row_count < proj_cap; bi++) {
        const char *vals[ENGINE_SZ_32];
        char func_bufs[ENGINE_SZ_32][ENGINE_SZ_512];
        for (int ci = 0; ci < ret->count; ci++) {
            vals[ci] =
...
static void execute_default_projection(engine_pattern_t *pat0, binding_t *bindings, int bind_count,
                                       int max_rows, result_builder_t *rb) {
    ...
    for (int bi = 0; bi < bind_count && rb->row_count < max_rows; bi++) {
        ...
        for (int v = 0; v < vc; v++) {
            engine_node_t *n = binding_get(&bindings[bi], vars[v]);
 ...
 * *out_n on success, ENGINE_NOT_FOUND on overflow. Non-static so the arithmetic
 * boundary can be unit-tested directly. */
int engine_cypher_cross_join_alloc(int bind_count, int extra_count, bool opt, size_t *out_n) {
    ...
    if (rows > (size_t)INT_MAX) {
        return ENGINE_NOT_FOUND; /* new_count would not fit the int binding counter */
    }
    size_t n = rows + SKIP_ONE;
    if (n > SIZE_MAX / sizeof(binding_t)) {
        return ENGINE_NOT_FOUND; /* alloc_n * sizeof(binding_t) would overflow size_t */
    }
    *out_n = n;
    return 0;
}
 ...
 * ENGINE_NOT_FOUND when the allocation is refused (overflow) or fails (OOM); the
 * caller propagates that as a query error rather than silently skipping the
 * MATCH, which would return a wrong (short) result. */
static int cross_join_nodes(binding_t **bindings, int *bind_count, engine_node_t *extra_nodes,
                            int extra_count, const char *nvar, bool opt) {
    size_t alloc_n = 0;
    if (engine_cypher_cross_join_alloc(*bind_count, extra_count, opt, &alloc_n) != 0) {
        return ENGINE_NOT_FOUND; /* product overflows int/size_t: fail the query */
    }
    binding_t *new_bindings = malloc(alloc_n * sizeof(binding_t));
    if (!new_bindings) {
        return ENGINE_NOT_FOUND; /* OOM: propagate, don't silently skip the MATCH */
    }
    size_t new_count = 0;
    for (int bi = 0; bi < *bind_count; bi++) {
...
static void cross_join_with_rels(engine_store_t *store, engine_pattern_t *patn, binding_t **bindings,
                                 int *bind_count, engine_node_t *extra_nodes, int extra_count,
...
static void expand_from_bound_terminal(engine_store_t *store, engine_pattern_t *patn,
                                       binding_t **bindings, int *bind_count, const char *start_var,
                                       bool opt) {
    engine_rel_pattern_t *rel = &patn->rels[0];
    const engine_node_pattern_t *start_node = &patn->nodes[0];
    ...
    for (int bi = 0; bi < *bind_count && new_count < max_new; bi++) {
        binding_t *b = &(*bindings)[bi];
        engine_node_t *term = binding_get(b, patn->nodes[1].variable ? patn->nodes[1].variable : "");
        int match_count = 0;
        if (term) {
            for (int ti = 0;
                ...
                if (rel->type_count > 0) {
                    if (scan_targets) {
                        engine_store_find_edges_by_target_type(store, term->id, rel->types[ti], &edges,
                                                            &edge_count);
                    } else {
                        engine_store_find_edges_by_source_type(store, term->id, rel->types[ti], &edges,
                                                            &edge_count);
                    }
                } else if (scan_targets) {
                    engine_store_find_edges_by_target(store, term->id, &edges, &edge_count);
                } else {
                    engine_store_find_edges_by_source(store, term->id, &edges, &edge_count);
                }
                for (int ei = 0; ei < edge_count && new_count < max_new; ei++) {
                    int64_t sid = scan_targets ? edges[ei].source_id : edges[ei].target_id;
                    engine_node_t found = {0};
                    if (engine_store_find_node_by_id(store, sid, &found) != ENGINE_STORE_OK) {
                        continue;
                    }
...
static int expand_additional_patterns(engine_store_t *store, engine_query_t *q, const char *project,
                                      int max_rows, binding_t **bindings, int *bind_count,
                                      int *bind_cap) {
    for (int pi = SKIP_ONE; pi < q->pattern_count; pi++) {
        engine_pattern_t *patn = &q->patterns[pi];
        ...
        engine_node_t *extra_nodes = NULL;
        int extra_count = 0;
        ...
        engine_store_free_nodes(extra_nodes, extra_count);
        if (rc != 0) {
...
static void execute_return_clause(engine_query_t *q, engine_return_clause_t *ret, binding_t *bindings,
...
static int execute_single(engine_store_t *store, engine_query_t *q, const char *project, int max_rows,
                          result_builder_t *rb) {
    engine_pattern_t *pat0 = &q->patterns[0];

    /* Step 1: Scan initial nodes */
    engine_node_t *scanned = NULL;
    int scan_count = 0;
    ...
    if (expand_additional_patterns(store, q, project, max_rows, &bindings, &bind_count,
        ...
        free(bindings);
        engine_store_free_nodes(scanned, scan_count);
        return ENGINE_NOT_FOUND; /* cross-join allocation refused/failed */
    }
...
int engine_cypher_execute(engine_store_t *store, const char *query, const char *project, int max_rows,
                       engine_cypher_result_t *out) {
    memset(out, 0, sizeof(*out));
    ...
    engine_query_t *q = NULL;
    char *err = NULL;
    if (engine_cypher_parse(query, &q, &err) < 0) {
        out->error = err;
        return ENGINE_NOT_FOUND;
    }
    ...
    /* UNION chain */
    engine_query_t *uq = q->union_next;
    while (uq) {
...
void engine_cypher_result_free(engine_cypher_result_t *r) {
    if (!r) {
        return;
    }
    free(r->warning);
