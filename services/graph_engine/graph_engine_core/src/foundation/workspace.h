#ifndef ENGINE_FOUNDATION_WORKSPACE_H
#define ENGINE_FOUNDATION_WORKSPACE_H
 ...
 * of ENGINE_WS_ALLOW here means "this path is not obviously too broad", never
...
typedef enum {
    ...
    ENGINE_WS_DENY_TOO_SHALLOW,
    /* A filesystem root, a drive root, a UNC share root, or a path that is or
     * contains the engine cache directory. Never overridable. */
    ENGINE_WS_DENY_ABSOLUTE,
 ...
 * human override for a ENGINE_WS_DENY_SENSITIVE path; absolute denials and shallow
 ...
 * ENGINE_ALLOWED_ROOT / session policy, treated as an additional grant, or NULL.
...
#endif /* ENGINE_FOUNDATION_WORKSPACE_H */

/*
 * ── Per-project request manifest (.enginepathwhitelist) ─────────────────────────
 *
...
#define ENGINE_WS_MANIFEST_NAME ".enginepathwhitelist"

enum { ENGINE_WS_MANIFEST_MAX_ENTRIES = 64 };

typedef struct {
    /* Requested roots, verbatim from the file (already control-char screened). */
    char entries[ENGINE_WS_MANIFEST_MAX_ENTRIES][1024];
    int count;
    /* SHA-256 hex of the raw file bytes; empty when there is no manifest. */
    char digest[65];
    bool present;
} engine_ws_manifest_t;

/* Read <project_root>/.enginepathwhitelist. Returns false only on a malformed file
