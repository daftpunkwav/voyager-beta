...
int engine_workspace_path_depth(const char *canonical_path) {
...
engine_ws_verdict_t engine_workspace_classify_root(const char *canonical_path, const char *home_dir,
    ...
    if (!canonical_path || !canonical_path[0] || ws_volume_prefix_len(canonical_path) == 0) {
        ...
        return ENGINE_WS_DENY_ABSOLUTE;
    }

    bool windows_style = ws_is_windows_style(canonical_path);
    int depth = engine_workspace_path_depth(canonical_path);
    ...
    if (home_dir && home_dir[0] && ws_paths_equal(canonical_path, home_dir)) {
        return ENGINE_WS_DENY_SENSITIVE;
    }
    ...
    if (depth < min_depth) {
        return ENGINE_WS_DENY_TOO_SHALLOW;
    }
    ...
    return ENGINE_WS_ALLOW;
}

const char *engine_workspace_verdict_reason(engine_ws_verdict_t verdict) {
    switch (verdict) {
    case ENGINE_WS_ALLOW:
        return "allowed";
    case ENGINE_WS_DENY_TOO_SHALLOW:
        return "path is too broad to index as one root; name a project directory below it";
    case ENGINE_WS_DENY_ABSOLUTE:
        return "path is a volume root or holds the graph-engine cache; it cannot be indexed";
    case ENGINE_WS_DENY_SENSITIVE:
...
bool engine_workspace_verdict_is_overridable(engine_ws_verdict_t verdict) {
    return verdict == ENGINE_WS_DENY_SENSITIVE;
}
...
bool engine_workspace_grant_path(const char *cache_dir, char *out, size_t out_sz) {
...
static int ws_grant_walk(const char *cache_dir,
    ...
    if (!engine_workspace_grant_path(cache_dir, store, sizeof(store))) {
        return 0;
    }
    FILE *f = engine_fopen(store, "r");
    if (!f) {
        return 0;
    }
...
static bool ws_match_visit(const char *root, bool sensitive, void *ctx) {
    ws_match_t *m = ctx;
    if (engine_path_within_root(root, m->candidate)) {
        m->contained = true;
...
bool engine_workspace_grant_list(const char *cache_dir, char *out, size_t out_sz) {
    if (!out || out_sz == 0) {
        return false;
    }
    out[0] = '\0';
...
bool engine_workspace_grant_add(const char *cache_dir, const char *home_dir,
    ...
    engine_ws_verdict_t verdict = engine_workspace_classify_root(canonical_path, home_dir, cache_dir);
    if (verdict != ENGINE_WS_ALLOW) {
        if (!engine_workspace_verdict_is_overridable(verdict)) {
            if (err) {
                snprintf(err, err_sz, "%s", engine_workspace_verdict_reason(verdict));
            }
            return false;
        }
        if (!approve_sensitive) {
            ...
            if (err) {
                snprintf(err, err_sz, "%s; re-run with --approve-sensitive if that is intended",
                         engine_workspace_verdict_reason(verdict));
            }
            return false;
        }
    }
    ...
    bool sensitive = verdict == ENGINE_WS_DENY_SENSITIVE;
...
bool engine_workspace_root_allowed(const char *canonical_path, const char *home_dir,
     ...
     * ENGINE_ALLOWED_ROOT deployments keep working unchanged. */
    bool configured = configured_root && configured_root[0];
    bool configured_contains = configured && engine_path_within_root(configured_root, canonical_path);
     ...
     * walking a project's approved extra roots. engine_workspace_manifest_allows is
     ...
     * lifted at all. */
    if (verdict == ENGINE_WS_DENY_SENSITIVE && match.exact_sensitive) {
        return true;
    }
    if (err) {
        if (engine_workspace_verdict_is_overridable(verdict)) {
            snprintf(err, err_sz,
                     "%s: %s. To index it anyway, run: graph-engine allow-root "
                     "--approve-sensitive %s",
                     canonical_path, engine_workspace_verdict_reason(verdict), canonical_path);
        } else {
            snprintf(err, err_sz, "%s: %s", canonical_path, engine_workspace_verdict_reason(verdict));
        }
    }
    return false;
}
...
const char *engine_workspace_home_dir(void) {
...
const char *engine_workspace_cache_dir(void) {
    return engine_resolve_cache_dir();
}

/* ── Per-project request manifest ─────────────────────────────────────────── */

bool engine_workspace_manifest_read(const char *project_root, engine_ws_manifest_t *out) {
    if (!out) {
        return false;
    }
    ...
    int n = snprintf(path, sizeof(path), "%s/%s", project_root, ENGINE_WS_MANIFEST_NAME);
    ...
    (void)fclose(f);
    out->present = true;
    engine_sha256_hex(raw ? raw : "", raw_len, out->digest);
    ...
    for (size_t i = 0; i <= raw_len && ok; i++) {
        ...
        if (len > 0 && raw[line_start] != '#') {
            if (len >= sizeof(out->entries[0]) || out->count >= ENGINE_WS_MANIFEST_MAX_ENTRIES) {
                ok = false;
                break;
            }
...
bool engine_workspace_manifest_is_approved(const char *cache_dir, const char *project_root,
                                        const engine_ws_manifest_t *manifest) {
...
bool engine_workspace_manifest_approve(const char *cache_dir, const char *home_dir,
                                    const char *project_root, char *err, size_t err_sz) {
    if (err && err_sz) {
        err[0] = '\0';
    }
    engine_ws_manifest_t m;
    if (!engine_workspace_manifest_read(project_root, &m)) {
        if (err) {
            snprintf(err, err_sz,
                     "%s is malformed (control characters, an over-long entry, or "
                     "more than %d entries)",
                     ENGINE_WS_MANIFEST_NAME, ENGINE_WS_MANIFEST_MAX_ENTRIES);
        }
        return false;
    }
    if (!m.present) {
        if (err) {
            snprintf(err, err_sz, "no %s in %s", ENGINE_WS_MANIFEST_NAME,
    ...
    for (int i = 0; i < m.count; i++) {
        engine_ws_verdict_t v = engine_workspace_classify_root(m.entries[i], home_dir, cache_dir);
        if (v != ENGINE_WS_ALLOW) {
            if (err) {
                snprintf(err, err_sz, "requested path %s: %s", m.entries[i],
                         engine_workspace_verdict_reason(v));
            }
            return false;
        }
    }
    ...
    if (engine_workspace_manifest_is_approved(cache_dir, project_root, &m)) {
        return true;
    }
    FILE *f = engine_fopen(store, "a");
    if (!f) {
...
bool engine_workspace_manifest_allows(const char *cache_dir, const char *home_dir,
                                   const char *project_root, const char *candidate) {
    if (!candidate || !candidate[0]) {
        return false;
    }
    engine_ws_manifest_t m;
    if (!engine_workspace_manifest_read(project_root, &m) || !m.present) {
        return false;
    }
    if (!engine_workspace_manifest_is_approved(cache_dir, project_root, &m)) {
        return false;
    ...
    for (int i = 0; i < m.count; i++) {
        ...
        if (engine_workspace_classify_root(m.entries[i], home_dir, cache_dir) != ENGINE_WS_ALLOW) {
            continue;
        }
        if (engine_path_within_root(m.entries[i], candidate)) {
            return true;
        }
    }
    return false;
