 ...
 */
#ifndef ENGINE_DUMP_VERIFY_H
#define ENGINE_DUMP_VERIFY_H

#include <stdbool.h>

/** Repos with at most this many committed nodes skip the ratio gate. */
enum { ENGINE_DUMP_VERIFY_MIN_FLOOR = 50 };

/** Default minimum persisted/committed ratio when env is unset. */
#define ENGINE_DUMP_VERIFY_DEFAULT_RATIO 0.5
 ...
 */
bool engine_dump_verify_is_degraded(int committed_nodes, int persisted_nodes, double ratio,
                                 int min_floor);

/** Read ENGINE_DUMP_VERIFY_MIN_RATIO (0..1); invalid/unset -> default 0.5. Set 0 to disable. */
double engine_dump_verify_min_ratio(void);

#endif /* ENGINE_DUMP_VERIFY_H */
