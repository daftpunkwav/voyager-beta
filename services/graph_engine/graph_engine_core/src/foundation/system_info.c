...
enum { DEFAULT_CORES = 1, MIN_WORKERS = 1, ENGINE_WORKERS_MAX = 256 };
...
static engine_system_info_t detect_system_macos(void) {
    engine_system_info_t info;
...
static engine_system_info_t detect_system_bsd(void) {
    engine_system_info_t info;
    memset(&info, 0, sizeof(info));
...
int engine_detect_cgroup_cpus(const char *cgroup_root) {
    char path[ENGINE_PATH_MAX];
    char buf[ENGINE_SZ_64];
    ...
    long quota = strtol(buf, NULL, ENGINE_DECIMAL_BASE);
    if (quota <= 0) {
        return -1;
    }
    ...
    long period = strtol(buf, NULL, ENGINE_DECIMAL_BASE);
    if (period <= 0) {
        return -1;
    }
...
size_t engine_detect_cgroup_mem(const char *cgroup_root) {
    char path[ENGINE_PATH_MAX];
    char buf[ENGINE_SZ_64];
    ...
    if (read_small_file(path, buf, sizeof(buf)) > 0) {
        ...
        char *end = NULL;
        unsigned long long n = strtoull(buf, &end, ENGINE_DECIMAL_BASE);
        if (end == buf || n == 0) {
            return 0;
        }
        return (size_t)n;
...
static engine_system_info_t detect_system_linux(void) {
    engine_system_info_t info;
    memset(&info, 0, sizeof(info));
    ...
    int cg_cpus = engine_detect_cgroup_cpus("/sys/fs/cgroup");
    ...
    size_t cg_ram = engine_detect_cgroup_mem("/sys/fs/cgroup");
...
#ifdef _WIN32
static engine_system_info_t detect_system_windows(void) {
    engine_system_info_t info;
    memset(&info, 0, sizeof(info));
...
static engine_system_info_t cached_info;

engine_system_info_t engine_system_info(void) {
    if (!info_cached) {
#ifdef _WIN32
...
int engine_default_worker_count(bool initial) {
    /* ENGINE_WORKERS env override (clamped to [1, ENGINE_WORKERS_MAX]).
     ...
     * Same precedence shape as other ENGINE_* env overrides:
     * explicit override > implicit detection. */
    char buf[ENGINE_SZ_32];
    if (engine_safe_getenv("ENGINE_WORKERS", buf, sizeof(buf), NULL) != NULL) {
        long n = strtol(buf, NULL, ENGINE_DECIMAL_BASE);
        if (n >= MIN_WORKERS && n <= ENGINE_WORKERS_MAX) {
            return (int)n;
        }
        engine_log_warn("workers.env.invalid", "value", buf, "fallback", "sysconf");
    }
