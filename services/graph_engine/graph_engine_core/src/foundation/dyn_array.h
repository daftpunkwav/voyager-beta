 ...
 *
 * Usage:
 *   ENGINE_DYN_ARRAY(int) nums = {0};    // zero-init
 *   engine_da_push(&nums, 42);
 *   engine_da_push(&nums, 99);
 ...
 *   - Uses realloc — NOT arena-compatible (use EngineDefArray etc. for arena)
 *   - Header-only: no .c file needed
 */
#ifndef ENGINE_DYN_ARRAY_H
#define ENGINE_DYN_ARRAY_H

#include <stdlib.h>
#include <string.h>

/* Declare a dynamic array type for a given element type. */
#define ENGINE_DYN_ARRAY(T) \
    struct {             \
        T *items;        \
        int count;       \
        int cap;         \
...
#define engine_da_pop(da) ((da)->items[--(da)->count])

/* Get last element without removing. Undefined if empty. */
#define engine_da_last(da) ((da)->items[(da)->count - 1])

/* Clear without freeing (reset count to 0). */
#define engine_da_clear(da) ((da)->count = 0)

/* Free all memory. */
#define engine_da_free(da)     \
    do {                    \
        free((da)->items);  \
        (da)->items = NULL; \
        (da)->count = 0;    \
...
#define engine_da_insert(da, idx, item)                                           \
    do {                                                                       \
        engine_da_push(da, item); /* ensure space */                              \
        if ((idx) < (da)->count - 1) {                                         \
...
#define engine_da_remove(da, idx)                                                 \
    do {                                                                       \
        if ((idx) < (da)->count - 1) {                                         \
