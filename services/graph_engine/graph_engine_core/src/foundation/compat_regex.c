...
_Static_assert(sizeof(regex_t) <= ENGINE_SZ_256,
               "engine_regex_t opaque buffer too small for TRE regex_t");

static int translate_flags_tre(int flags) {
    int tre_flags = 0;
    if (flags & ENGINE_REG_EXTENDED)
        tre_flags |= REG_EXTENDED;
    if (flags & ENGINE_REG_ICASE)
        tre_flags |= REG_ICASE;
    if (flags & ENGINE_REG_NOSUB)
        tre_flags |= REG_NOSUB;
    if (flags & ENGINE_REG_NEWLINE)
...
int engine_regexec(const engine_regex_t *r, const char *str, int nmatch, engine_regmatch_t *matches,
    ...
    if (nmatch <= 0 || !matches) {
        ...
        return rc == 0 ? ENGINE_REG_OK : ENGINE_REG_NOMATCH;
    }
    regmatch_t pmatch[ENGINE_SZ_32];
    int n = nmatch > ENGINE_SZ_32 ? ENGINE_SZ_32 : nmatch;
    int rc = tre_regexec(re, str, (size_t)n, pmatch, eflags);
    if (rc != 0)
        return ENGINE_REG_NOMATCH;
    for (int i = 0; i < n; i++) {
        matches[i].rm_so = (int)pmatch[i].rm_so;
        matches[i].rm_eo = (int)pmatch[i].rm_eo;
    }
    return ENGINE_REG_OK;
}
 ...
 * If this fires, increase engine_regex_t.opaque size. */
_Static_assert(sizeof(regex_t) <= ENGINE_SZ_256, "engine_regex_t opaque buffer too small for regex_t");
