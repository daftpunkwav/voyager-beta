...
#include <stddef.h>
#include <stdio.h>
/* stdlib.h declares getenv (engine_tmpdir) and, on Windows, _putenv_s (engine_setenv/
 * engine_unsetenv). The x86-64 mingw toolchain pulled it in transitively, but the
...
#endif
#include <windows.h>
#define engine_usleep(us) Sleep((DWORD)((us) / 1000))
#else
#include <unistd.h>
#define engine_usleep(us) usleep((useconds_t)(us))
#endif

/* ── strdup / strndup ─────────────────────────────────────────── */
#ifdef _WIN32
#define engine_strdup _strdup
/* Implemented in compat.c */
char *engine_strndup(const char *s, size_t n);
#else
#define engine_strdup strdup
#define engine_strndup strndup
#endif
...
#else
#define engine_getline getline
#endif

/* ── fileno ───────────────────────────────────────────────────── */
#ifdef _WIN32
#define engine_fileno _fileno
#else
#define engine_fileno fileno
#endif

/* ── strcasestr (Windows lacks it) ────────────────────────────── */
#ifdef _WIN32
/* Implemented in compat.c */
char *engine_strcasestr(const char *haystack, const char *needle);
#else
#define engine_strcasestr strcasestr
#endif

/* ── mkdir portability ───────────────────────────────────────── */
#ifdef _WIN32
#include <direct.h>
#define engine_mkdir(path) _mkdir(path)
#else
#include <sys/stat.h>
#define engine_mkdir(path) mkdir(path, 0755)
#endif
...
#endif
/* Implemented in compat.c */
int engine_clock_gettime(int clk_id, struct timespec *tp);
static inline int engine_nanosleep(const struct timespec *req, struct timespec *rem) {
    (void)rem;
    Sleep((DWORD)(req->tv_sec * 1000 + req->tv_nsec / 1000000));
    return 0;
}
#else
#define engine_clock_gettime clock_gettime
#define engine_nanosleep nanosleep
#endif

/* ── gmtime_r (Windows lacks it) ─────────────────────────────── */
#ifdef _WIN32
static inline struct tm *engine_gmtime_r(const time_t *timep, struct tm *result) {
    return gmtime_s(result, timep) == 0 ? result : NULL;
}
#else
#define engine_gmtime_r gmtime_r
#endif

/* ── mkdtemp (Windows lacks it) ──────────────────────────────── */
#ifdef _WIN32
/* Translates /tmp/ to %TEMP%\ and copies result back to tmpl.
 * Callers MUST use char buf[ENGINE_SZ_256] or larger. */
char *engine_mkdtemp(char *tmpl);
#else
#define engine_mkdtemp mkdtemp
#endif

/* ── mkstemp (Windows lacks it) ──────────────────────────────── */
#ifdef _WIN32
int engine_mkstemp(char *tmpl);

#else
#define engine_mkstemp mkstemp
#endif
...
bool engine_path_for_file_api(const char *path, char *out, size_t out_size);

/* ── setenv / unsetenv (Windows lacks them) ──────────────────── */
#ifdef _WIN32
static inline int engine_setenv(const char *name, const char *value, int overwrite) {
    (void)overwrite;
    if (!name || !value) {
        return EINVAL;
    }
...
static inline int engine_unsetenv(const char *name) {
    if (!name) {
        return EINVAL;
    }
    int name_chars = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, name, -1, NULL, 0);
...
#else
#define engine_setenv setenv
#define engine_unsetenv unsetenv
#endif
...
#include <io.h>
#include <fcntl.h>
#define engine_pipe(fds) _pipe(fds, 4096, _O_BINARY)
#else
#define engine_pipe(fds) pipe(fds)
#endif

/* ── Temp directory helper ───────────────────────────────────── */
static inline const char *engine_tmpdir(void) {
#ifdef _WIN32
...
#ifdef _WIN32
#define ENGINE_HAS_SIGACTION 0
#else
#define ENGINE_HAS_SIGACTION 1
#endif
