...
static _Atomic EngineLogLevel g_log_level = ENGINE_LOG_INFO;
static _Atomic EngineLogFormat g_log_format = ENGINE_LOG_FORMAT_TEXT;
...
static _Atomic engine_log_sink_fn g_log_sink = (engine_log_sink_fn)NULL;
static _Atomic EngineLogSinkMode g_log_sink_mode = ENGINE_LOG_SINK_REPLACE;

/* ENGINE_LOG_LEVEL support — distilled from #414 (closes #413, thanks @santanusinha). */
void engine_log_init_from_env(void) {
    ...
    const char *raw = getenv("ENGINE_LOG_LEVEL");
    if (raw && raw[0] != '\0') {
        ...
        if (raw[i] == '\0') { /* fully consumed — candidate textual match */
            for (size_t lvl = 0; lvl < sizeof(names) / sizeof(names[0]); lvl++) {
                if (strcmp(lower, names[lvl]) == 0) {
                    engine_log_set_level((EngineLogLevel)lvl);
                    goto parse_format;
                }
            }
        }

        /* Numeric form: 0=debug .. 4=none, matching EngineLogLevel. */
        char *end = NULL;
        long n = strtol(raw, &end, ENGINE_DECIMAL_BASE);
        if (end != raw && *end == '\0' && n >= ENGINE_LOG_DEBUG && n <= ENGINE_LOG_NONE) {
            engine_log_set_level((EngineLogLevel)n);
        }
    }
...
parse_format:;
    const char *fmt = getenv("ENGINE_LOG_FORMAT");
    if (fmt && fmt[0] != '\0') {
        char lower_fmt[8];
        size_t i = 0;
        ...
        if (fmt[i] == '\0' && strcmp(lower_fmt, "json") == 0) {
            engine_log_set_format(ENGINE_LOG_FORMAT_JSON);
        } else if (fmt[i] == '\0' && strcmp(lower_fmt, "text") == 0) {
            engine_log_set_format(ENGINE_LOG_FORMAT_TEXT);
        }
        return;
    }
...
void engine_log_set_sink(engine_log_sink_fn fn) {
    engine_log_set_sink_ex(fn, ENGINE_LOG_SINK_REPLACE);
}
...
static const char *level_str(EngineLogLevel level) {
    switch (level) {
    case ENGINE_LOG_DEBUG:
        return "debug";
    case ENGINE_LOG_INFO:
        return "info";
    case ENGINE_LOG_WARN:
        return "warn";
...
static void emit_line(const char *line) {
     ...
     * let a concurrent engine_log_set_sink turn a checked pointer into a NULL
     * call. */
    engine_log_sink_fn sink = atomic_load_explicit(&g_log_sink, memory_order_relaxed);
    if (sink) {
        sink(line);
        if (atomic_load_explicit(&g_log_sink_mode, memory_order_relaxed) == ENGINE_LOG_SINK_REPLACE) {
            return;
        }
    }
...
void engine_log(EngineLogLevel level, const char *msg, ...) {
    ...
    char line_buf[ENGINE_SZ_4K];
    size_t pos = 0;
    va_list args;
    va_start(args, msg);

    if (atomic_load_explicit(&g_log_format, memory_order_relaxed) == ENGINE_LOG_FORMAT_JSON) {
...
void engine_log_control_record(const char *msg, ...) {
     ...
     * ENGINE_LOG_LEVEL suppression. Always JSON, independent of ENGINE_LOG_FORMAT,
...
void engine_log_http_request(const char *component, const char *method, const char *path, int status,
    ...
    char safe_path[ENGINE_SZ_1K];
    char status_buf[ENGINE_SZ_16];
    char duration_buf[ENGINE_SZ_32];
    char request_buf[ENGINE_SZ_32];
    char response_buf[ENGINE_SZ_32];
    ...
    EngineLogLevel level = ENGINE_LOG_INFO;
    if (status >= 500) {
        level = ENGINE_LOG_ERROR;
    } else if (status >= 400) {
        level = ENGINE_LOG_WARN;
    }
