 ...
 * The ENGINE_MEM_BUDGET_MB env var, when set to a positive integer, overrides
 ...
 * ENGINE_MEM_BUDGET_MB wins, while a larger/default budget is capped. */
void engine_mem_init_with_cap(double ram_fraction, size_t hard_cap_bytes);

/* Result of engine_mem_resolve_budget: the resolved budget plus the metadata
 * engine_mem_init logs — so the parse/clamp logic lives in exactly ONE place and
 * the caller never re-parses the env string. */
typedef struct {
    size_t budget;      /* resolved budget in bytes */
    const char *source; /* log token: "ram_fraction" | "ENGINE_MEM_BUDGET_MB" */
...
/* Pure budget resolver shared by engine_mem_init (exposed for testing).
 * Returns ram_fraction * total_ram, unless `budget_mb` is a STRICTLY valid
 * positive integer string (the ENGINE_MEM_BUDGET_MB override) — then it returns
...
/* TEST HOOK: overwrite the budget directly, bypassing engine_mem_init's
 ...
 * guard: a later engine_mem_init still initializes normally. Callers must
 * save engine_mem_budget() first and restore it before their assertions.
 ...
 */
enum { ENGINE_MEM_MAP_BUCKETS = 8 };

typedef struct {
    /* Is plain malloc actually served by the configured allocator on this
     * build? If false, every allocator tuning in engine_mem_init (immediate
    ...
    size_t bucket_bytes[ENGINE_MEM_MAP_BUCKETS];
    size_t bucket_blocks[ENGINE_MEM_MAP_BUCKETS];
} engine_mem_map_t;
 ...
 */
enum { ENGINE_MEM_OWNERSHIP_CLASSES = 6 };

typedef struct {
    size_t probe_bytes[ENGINE_MEM_OWNERSHIP_CLASSES]; /* size probed per class */
    bool owned[ENGINE_MEM_OWNERSHIP_CLASSES];         /* predicate said "ours" */
    bool allocated[ENGINE_MEM_OWNERSHIP_CLASSES];     /* the probe itself worked */
    int owned_count;
    int probed_count;
    bool all_owned; /* every successfully probed class came back owned */
} engine_mem_ownership_audit_t;
 ...
 *   - Off unless ENGINE_MEM_PHASES=1, so the hot path pays one atomic load.
 */
 ...
 */
enum { ENGINE_MEM_TRACKED_REGIONS = 6 };
...
typedef struct {
    ...
    void *large_base[ENGINE_MEM_TRACKED_REGIONS];
    size_t large_size[ENGINE_MEM_TRACKED_REGIONS];
 ...
 * read straight out of the daemon log. No-op unless ENGINE_MEM_CENSUS=1. */
void engine_mem_census_log(const char *tag);

/* Census logging gate: ENGINE_MEM_CENSUS=1. Off by default — HeapWalk locks each
