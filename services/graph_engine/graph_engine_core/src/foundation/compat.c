...
char *engine_mkdtemp(char *tmpl) {
     ...
     * Callers must provide buffers >= ENGINE_SZ_256 bytes (all test code does). */
    static char buf[ENGINE_SZ_512];
    ...
    wchar_t *wide_template = engine_utf8_to_wide(buf);
    if (!wide_template || !_wmktemp(wide_template)) {
        free(wide_template);
        return NULL;
    }
    char *expanded = engine_wide_to_utf8(wide_template);
    free(wide_template);
    ...
    if (!win_mkdtemp_private_create(buf)) {
        ...
        wchar_t *wide_directory = engine_utf8_to_wide(buf);
    ...
    /* Copy result back — callers now use char[ENGINE_SZ_256]+ buffers */
    strcpy(tmpl, buf);
    return tmpl;
}
#endif
...
#ifdef _WIN32
    wchar_t *wide = engine_path_to_wide(path);
    char *narrow = wide ? engine_wide_to_utf8(wide) : NULL;
    free(wide);
    if (!narrow) {
        return false;
    }
...
int engine_mkstemp(char *tmpl) {
    /* Rewrite /tmp/ to %TEMP%\ like engine_mkdtemp */
     ...
     * ENGINE_CACHE_DIR, which users may place at non-ASCII paths; the ANSI CRT
    ...
    char *expanded_for_open = engine_wide_to_utf8(wide_template);
    wchar_t *wide_open = expanded_for_open ? engine_path_to_wide(expanded_for_open) : NULL;
