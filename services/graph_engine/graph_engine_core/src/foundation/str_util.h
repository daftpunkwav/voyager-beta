 ...
 */
#ifndef ENGINE_STR_UTIL_H
#define ENGINE_STR_UTIL_H
 ...
 * Everything engine_validate_shell_arg rejects, plus the cmd.exe expansion
 ...
 *
 * Use this — not engine_validate_shell_arg — for every path that crosses into a
 ...
 *
 * Usage: ENGINE_SNPRINTF_APPEND(buf, sizeof(buf), off, "fmt %s", arg);
 * Requires: <stdio.h> included by caller. */
#define ENGINE_SNPRINTF_APPEND(buf, sz, off, ...)                                       \
    do {                                                                             \
        if ((off) >= 0 && (off) < (int)(sz)) {                                       \
            int _engine_r = snprintf((buf) + (off), (sz) - (size_t)(off), __VA_ARGS__); \
            if (_engine_r > 0)                                                          \
                (off) += _engine_r;                                                     \
            if ((off) >= (int)(sz))                                                  \
                (off) = (int)(sz) - 1;                                               \
