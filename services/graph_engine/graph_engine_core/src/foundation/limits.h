...
typedef enum {
    ENGINE_READ_OK = 0,    /* file read successfully */
    ENGINE_READ_OPEN_FAIL, /* could not open (missing / permission) */
    ENGINE_READ_EMPTY,     /* zero/negative size — benign, nothing to index */
    ENGINE_READ_OVERSIZED, /* size exceeds engine_max_file_bytes() */
    ENGINE_READ_OOM,       /* buffer allocation failed */
} engine_read_status_t;
 ...
 * never silently dropped. Override with ENGINE_MAX_FILE_BYTES (a positive integer
 ...
 * leaking across runs. */
long engine_max_file_bytes(void);
 ...
 * ENGINE_CYPHER_MAX_DEPTH (a positive integer). Default 10. */
int engine_cypher_max_depth(void);
 ...
 * Override with ENGINE_MCP_MAX_DEPTH (a positive integer). Default 15. */
int engine_mcp_max_depth(void);
