...
struct engine_dir {
    ...
    wchar_t wide_pattern[ENGINE_PATH_MAX];
    engine_dirent_t entry;
    bool first;
    bool done;
};

engine_dir_t *engine_opendir(const char *path) {
    if (!path) {
        return NULL;
    }
    ...
    if (wlen == 0 || wlen + 2 >= ENGINE_PATH_MAX) {
        free(wpath);
        return NULL;
    }

    engine_dir_t *d = (engine_dir_t *)calloc(ENGINE_ALLOC_ONE, sizeof(engine_dir_t));
    if (!d) {
        free(wpath);
        return NULL;
    }
...
engine_dirent_t *engine_readdir(engine_dir_t *d) {
    if (!d || d->done) {
        return NULL;
    }
    if (!d->first) {
    ...
    char *u8 = engine_wide_to_utf8(d->find_data.cFileName);
    if (!u8) {
        d->done = true;
        return NULL;
    }
    size_t nlen = strlen(u8);
    if (nlen >= ENGINE_DIRENT_NAME_MAX) {
        nlen = ENGINE_DIRENT_NAME_MAX - SKIP_ONE;
    }
...
int engine_path_info_utf8(const char *path, engine_path_info_t *out) {
    if (!path || !out) {
...
void engine_closedir(engine_dir_t *d) {
    if (d) {
        if (d->find_handle != INVALID_HANDLE_VALUE) {
 ...
 * falling back would silently re-arm the deadlock. engine_popen logs a structured
 * warning and returns NULL instead (every call site handles NULL). */

enum { ENGINE_POPEN_MAX = 16 };
static struct {
    FILE *fp;
    HANDLE proc;
} g_popen_tab[ENGINE_POPEN_MAX];
 ...
 * engine_popen(..., "r") stream came from the isolated spawn. Test-only
 * observable; not synchronized across threads. */
static volatile LONG g_popen_last_isolated = 0;

int engine_popen_last_was_isolated(void) {
    return (int)g_popen_last_isolated;
}

static BOOL CALLBACK engine_popen_init(PINIT_ONCE once, PVOID param, PVOID *ctx) {
    (void)once;
    (void)param;
    (void)ctx;
...
static wchar_t *engine_resolve_comspec(void) {
...
static FILE *engine_popen_isolated(const char *cmd, const char **stage, DWORD *gle) {
    *stage = "";
    *gle = 0;
    InitOnceExecuteOnce(&g_popen_once, engine_popen_init, NULL, NULL);
    ...
    wchar_t *app = engine_resolve_comspec();
    ...
    if (app) {
        ...
        if (u8) {
            ...
            wcmdline = engine_utf8_to_wide(u8);
            free(u8);
        }
    }
    ...
    for (int i = 0; i < ENGINE_POPEN_MAX; i++) {
        if (!g_popen_tab[i].fp) {
            g_popen_tab[i].fp = fp;
...
FILE *engine_popen(const char *cmd, const char *mode) {
    ...
    if (mode && mode[0] == 'r' && mode[1] == '\0') {
        const char *stage = "";
        DWORD gle = 0;
        FILE *fp = engine_popen_isolated(cmd, &stage, &gle);
        g_popen_last_isolated = (fp != NULL);
        if (!fp) {
            char glebuf[ENGINE_SZ_16];
            char errnobuf[ENGINE_SZ_16];
            snprintf(glebuf, sizeof(glebuf), "%lu", (unsigned long)gle);
            snprintf(errnobuf, sizeof(errnobuf), "%d", errno);
            engine_log_warn("compat.popen_isolated_failed", "stage", stage, "gle", glebuf, "errno",
                         errnobuf);
        }
        return fp;
    }
...
FILE *engine_fopen(const char *path, const char *mode) {
    wchar_t *wpath = engine_path_to_wide(path);
    if (!wpath) {
        return NULL;
    }
    wchar_t *wmode = engine_utf8_to_wide(mode);
    if (!wmode) {
        free(wpath);
        return NULL;
    }
 ...
 * DACL on a directory engine just created. Under the Administrators-default-owner
...
static void engine_windows_stamp_dir_owner(const wchar_t *path) {
    HANDLE token = NULL;
...
static bool engine_windows_mkdir_component(wchar_t *path) {
    ...
    if (created) {
        engine_windows_stamp_dir_owner(path);
    }
    return true;
}

bool engine_mkdir_p(const char *path, int mode) {
    (void)mode;
    if (!path || path[0] == '\0') {
        return false;
    }
    ...
    size_t start = wlen > 0U && engine_win_path_separator(tmp[0]) ? 1U : 0U;
    if (wlen >= 8U && _wcsnicmp(tmp, L"\\\\?\\UNC\\", 8U) == 0) {
        ...
        while (start < wlen && separators < 2U) {
            if (engine_win_path_separator(tmp[start])) {
                separators++;
            }
            start++;
        }
    } else if (wlen >= 7U && wcsncmp(tmp, L"\\\\?\\", 4U) == 0 && tmp[5] == L':' &&
               engine_win_path_separator(tmp[6])) {
        start = 7U;
    } else if (wlen >= 3U && tmp[1] == L':' && engine_win_path_separator(tmp[2])) {
        start = 3U;
    } else if (wlen >= 2U && engine_win_path_separator(tmp[0]) && engine_win_path_separator(tmp[1])) {
        size_t separators = 0U;
        start = 2U;
        while (start < wlen && separators < 2U) {
            if (engine_win_path_separator(tmp[start])) {
    ...
    for (wchar_t *p = tmp + start; ok && *p; p++) {
        if (engine_win_path_separator(*p)) {
            if (p == tmp || engine_win_path_separator(p[-1])) {
                continue;
            }
            wchar_t separator = *p;
            *p = L'\0';
            ok = engine_windows_mkdir_component(tmp);
            *p = separator;
        }
    }
    if (ok) {
...
int engine_unlink(const char *path) {
    wchar_t *wpath = engine_path_to_wide(path);
    if (!wpath) {
        return ENGINE_NOT_FOUND;
    }
...
int engine_rmdir(const char *path) {
    wchar_t *wpath = engine_path_to_wide(path);
    if (!wpath) {
        return ENGINE_NOT_FOUND;
    }
 ...
 * bytes are treated as UTF-8 and converted to wide via engine_utf8_to_wide,
...
wchar_t *engine_build_cmdline(const char *const *argv) {
     ...
     * whole via engine_utf8_to_wide. Every character the quoting logic acts
    ...
    *w = '\0';
    ...
    wchar_t *out = engine_utf8_to_wide(buf);
    free(buf);
    return out;
}

int engine_exec_no_shell(const char *const *argv) {
    if (!argv || !argv[0]) {
        return ENGINE_NOT_FOUND;
    }

    wchar_t *cmdline = engine_build_cmdline(argv);
    if (!cmdline) {
        return ENGINE_NOT_FOUND;
    }
     ...
     * CreateProcessW sites already set it: subprocess.c and engine_popen_isolated
    ...
    DWORD exit_code = (DWORD)ENGINE_NOT_FOUND;
...
static int engine_open_directory_component(int parent, const char *component, int flags) {
...
bool engine_mkdir_p(const char *path, int mode) {
    ...
    while (ok && *cursor) {
        ...
        if (cursor[0] != '\0' && strcmp(cursor, ".") != 0) {
            int next = engine_open_directory_component(directory, cursor, flags);
            if (next < 0 && errno == ENOENT) {
                if (mkdirat(directory, cursor, (mode_t)mode) != 0 && errno != EEXIST) {
                    ok = false;
                } else {
                    next = engine_open_directory_component(directory, cursor, flags);
                }
            }
            if (ok && next < 0) {
                ok = false;
...
int engine_exec_no_shell(const char *const *argv) {
    ...
    return ENGINE_NOT_FOUND; /* killed by signal */
}
...
int engine_canonical_path(const char *path, char *out, size_t out_sz) {
    ...
    char *utf8 = engine_wide_to_utf8(wfull);
    free(wfull);
    if (!utf8) {
        return 0;
    }
...
int engine_rename_replace(const char *src, const char *dst) {
#ifdef _WIN32
    wchar_t *wsrc = engine_path_to_wide(src);
    wchar_t *wdst = engine_path_to_wide(dst);
    int ret = ENGINE_NOT_FOUND;
    if (wsrc && wdst) {
...
int engine_rename_noreplace(const char *src, const char *dst) {
...
int engine_remove_db_sidecars(const char *db_path) {
...
static int stream_copy_file(const char *src, const char *dst) {
    FILE *in = engine_fopen(src, "rb");
    if (!in) {
        return ENGINE_NOT_FOUND;
    }
    FILE *out = engine_fopen(dst, "wb");
    if (!out) {
        (void)fclose(in);
        return ENGINE_NOT_FOUND;
    }
    char buf[ENGINE_SZ_64K];
    size_t n;
    int rc = 0;
    ...
    if (rc != 0) {
        (void)engine_unlink(dst);
    }
    return rc;
}

int engine_clone_or_copy_file(const char *src, const char *dst) {
    if (!src || !dst) {
        return ENGINE_NOT_FOUND;
    }
#if defined(__APPLE__)
