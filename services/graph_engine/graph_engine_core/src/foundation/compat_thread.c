...
#define ENGINE_DEFAULT_STACK_SIZE ((size_t)8 * ENGINE_SZ_1K * ENGINE_SZ_1K)
...
static size_t engine_thread_stack_floor(size_t requested) {
#if defined(ENGINE_SANITIZED_BUILD) && ENGINE_SANITIZED_BUILD
    const char *env = getenv("ENGINE_THREAD_STACK_MB");
    if (env && env[0]) {
        char *end = NULL;
        unsigned long mb = strtoul(env, &end, 10);
        if (end && *end == '\0' && mb > 0 && mb <= 1024) {
            size_t floor_bytes = (size_t)mb * ENGINE_SZ_1K * ENGINE_SZ_1K;
...
static size_t engine_thread_default_stack_size(void) {
    return engine_thread_stack_floor(ENGINE_DEFAULT_STACK_SIZE);
}
 ...
 *
 * ENGINE_MI_THREAD_DONE=0 disables it, so one binary can demonstrate both
...
static bool thread_release_heap_enabled(void) {
    ...
    if (state < 0) {
        ...
        state =
            (engine_safe_getenv("ENGINE_MI_THREAD_DONE", buf, sizeof(buf), NULL) != NULL && buf[0] == '0')
                ? 0
                : 1;
    }
    return state == 1;
...
static void NTAPI engine_thread_detach_callback(PVOID handle, DWORD reason, PVOID reserved) {
    (void)handle;
    (void)reserved;
...
static const void *const engine_tls_anchor __attribute__((used)) = &_tls_used;
__attribute__((section(".CRT$XLB"), used)) PIMAGE_TLS_CALLBACK engine_thread_detach_tls_cb =
    engine_thread_detach_callback;
...
int engine_thread_create(engine_thread_t *t, size_t stack_size, void *(*fn)(void *), void *arg) {
    if (stack_size == 0) {
        stack_size = engine_thread_default_stack_size();
    } else {
        stack_size = engine_thread_stack_floor(stack_size);
    }
