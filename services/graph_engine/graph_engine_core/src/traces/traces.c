...
const char *engine_extract_service_name(const engine_trace_resource_t *r) {
    if (!r) {
        return "";
    }
...
const char *engine_extract_path_from_url(const char *url, char *buf, size_t buf_sz) {
    if (!url || !buf || buf_sz == 0) {
        if (buf) {
            buf[0] = '\0';
        }
...
int64_t engine_parse_duration(const char *start_nano, const char *end_nano) {
    if (!start_nano || !end_nano) {
        return 0;
    }
    int64_t start = strtoll(start_nano, NULL, ENGINE_DECIMAL_BASE);
    int64_t end = strtoll(end_nano, NULL, ENGINE_DECIMAL_BASE);
    return (end > start) ? (end - start) : 0;
}

/* ── extractHTTPInfo ─────────────────────────────────────────────── */

bool engine_extract_http_info(const engine_trace_span_t *span, const char *service_name,
                           engine_http_span_info_t *out) {
    if (!span || !out) {
        return false;
    }
    ...
    bool has_http = false;
    static char url_buf[ENGINE_SZ_1K];
    ...
    for (int i = 0; i < span->attr_count; i++) {
        ...
        if (strcmp(key, "http.method") == 0 || strcmp(key, "http.request.method") == 0) {
            ...
            const char *path = engine_extract_path_from_url(val, url_buf, sizeof(url_buf));
            if (path[0] != '\0') {
                out->path = path;
                has_http = true;
            }
    ...
    out->duration_ns = engine_parse_duration(span->start_time, span->end_time);
    return true;
}
...
int64_t engine_calculate_p99(int64_t *values, int count) {
    if (!values || count <= 0) {
        return 0;
    }
