...
static void weighted_minhash_update(engine_minhash_t *out, const char *trigram, int len, int w) {
    for (int k = 0; k < ENGINE_MINHASH_K; k++) {
...
static int hash_trigrams(const char **tokens, int token_count, engine_minhash_t *out) {
    for (int k = 0; k < ENGINE_MINHASH_K; k++) {
...
bool engine_minhash_compute(TSNode func_body, const char *source, int language, engine_minhash_t *out) {
    (void)source;
    (void)language;
    ...
    if (token_count < ENGINE_MINHASH_MIN_NODES) {
        return false;
    }
...
double engine_minhash_jaccard(const engine_minhash_t *a, const engine_minhash_t *b) {
    if (!a || !b) {
        return 0.0;
    }
    int matching = 0;
    for (int i = 0; i < ENGINE_MINHASH_K; i++) {
        if (a->values[i] == b->values[i]) {
            matching++;
        }
    }
    return (double)matching / (double)ENGINE_MINHASH_K;
}

/* ── Hex encoding/decoding ───────────────────────────────────────── */

void engine_minhash_to_hex(const engine_minhash_t *fp, char *buf, int bufsize) {
    if (!fp || !buf || bufsize < ENGINE_MINHASH_HEX_BUF) {
        if (buf && bufsize > 0) {
            buf[0] = '\0';
        }
        return;
...
bool engine_minhash_from_hex(const char *hex, engine_minhash_t *out) {
    if (!hex || !out) {
        return false;
    }
    size_t len = strlen(hex);
    if ((int)len != ENGINE_MINHASH_HEX_LEN) {
        return false;
    }
    for (int i = 0; i < ENGINE_MINHASH_K; i++) {
...
struct engine_lsh_index {
    /* b=32 bands, each with LSH_BUCKET_COUNT buckets */
    lsh_bucket_t bands[ENGINE_LSH_BANDS][LSH_BUCKET_COUNT];
    /* All entries stored for lifetime management */
    engine_lsh_entry_t *entries;
    int entry_count;
    int entry_cap;
    /* Candidate result buffer (reused across queries) */
    const engine_lsh_entry_t **result_buf;
    int result_count;
    int result_cap;
};
...
static uint32_t band_hash(const engine_minhash_t *fp, int band) {
    int base = band * ENGINE_LSH_ROWS;
    /* Combine r=2 values into a single hash */
    uint32_t combined[ENGINE_LSH_ROWS];
    for (int r = 0; r < ENGINE_LSH_ROWS; r++) {
...
engine_lsh_index_t *engine_lsh_new(void) {
    engine_lsh_index_t *idx = calloc(SKIP_ONE, sizeof(engine_lsh_index_t));
    return idx;
}

void engine_lsh_insert(engine_lsh_index_t *idx, const engine_lsh_entry_t *entry) {
    ...
    if (idx->entry_count >= idx->entry_cap) {
        ...
        engine_lsh_entry_t *new_entries =
            realloc(idx->entries, (size_t)new_cap * sizeof(engine_lsh_entry_t));
        if (!new_entries) {
            return;
        }
    ...
    for (int b = 0; b < ENGINE_LSH_BANDS; b++) {
...
static bool result_push(engine_lsh_index_t *idx, const engine_lsh_entry_t *candidate) {
    if (idx->result_count >= idx->result_cap) {
        int new_cap =
            idx->result_cap < RESULT_INIT_CAP ? RESULT_INIT_CAP : idx->result_cap * GROW_FACTOR;
        const engine_lsh_entry_t **new_buf =
            realloc(idx->result_buf, (size_t)new_cap * sizeof(const engine_lsh_entry_t *));
        if (!new_buf) {
            return false;
        }
...
void engine_lsh_query(const engine_lsh_index_t *idx, const engine_minhash_t *fp,
                   const engine_lsh_entry_t ***out, int *count) {
    *out = NULL;
    *count = 0;
    ...
    engine_lsh_index_t *mut_idx = (engine_lsh_index_t *)idx;
    ...
    for (int b = 0; b < ENGINE_LSH_BANDS; b++) {
        ...
        for (int i = 0; i < bucket->count; i++) {
            const engine_lsh_entry_t *candidate = &idx->entries[bucket->items[i]];
...
int engine_lsh_query_into(const engine_lsh_index_t *idx, const engine_minhash_t *fp,
                       const engine_lsh_entry_t **out_buf, int out_cap) {
...
void engine_lsh_free(engine_lsh_index_t *idx) {
    if (!idx) {
        return;
    }
    /* Free bucket arrays */
