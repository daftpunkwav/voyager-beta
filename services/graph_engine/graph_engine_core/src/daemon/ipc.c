/*
 * ipc.c — Authenticated, owner-scoped local transport for the engine daemon.
 */
...
enum {
    ENGINE_DAEMON_IPC_SEND_TIMEOUT_MS = 5000,
    ENGINE_DAEMON_IPC_PATH_CAP = 4096,
    ENGINE_DAEMON_IPC_RETRY_INTERVAL_MS = 10,
    ENGINE_DAEMON_IPC_COORDINATION_CLEANUP_MS = 500,
    ENGINE_DAEMON_IPC_WINDOWS_RENDEZVOUS_RETRY_MS = 250,
};
...
const char *engine_daemon_ipc_validation_detail(void) {
...
#ifndef _WIN32
static engine_daemon_ipc_posix_publication_hook_fn g_posix_publication_hook_for_test;
...
#ifdef _WIN32
static engine_daemon_ipc_startup_gate_fn g_startup_gate_for_test;
...
static void ipc_startup_gate_run(void) {
    engine_daemon_ipc_startup_gate_fn gate = g_startup_gate_for_test;
    if (gate) {
...
bool engine_daemon_ipc_windows_legacy_names(const char *canonical_runtime_parent,
    ...
    int pipe_length =
        snprintf(pipe_out, ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP, "\\\\.\\pipe\\engine-daemon-%016llx-%s",
                 (unsigned long long)hash, instance_key);
    int mutex_length =
        snprintf(startup_mutex_out, ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP,
                 "Local\\engine-daemon-%016llx-%s-startup", (unsigned long long)hash, instance_key);
    return pipe_length > 0 && (size_t)pipe_length < ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP &&
           mutex_length > 0 && (size_t)mutex_length < ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP;
}
...
static bool windows_pipe_address_valid(const char *address) {
    static const char prefix[] = "\\\\.\\pipe\\engine-daemon-";
    if (!address || strncmp(address, prefix, sizeof(prefix) - 1U) != 0) {
        return false;
    }
    const char *digest = address + sizeof(prefix) - 1U;
    for (size_t index = 0; index < ENGINE_SHA256_HEX_LEN; index++) {
        char character = digest[index];
        if (!((character >= '0' && character <= '9') || (character >= 'a' && character <= 'f'))) {
            return false;
        }
    }
    return digest[ENGINE_SHA256_HEX_LEN] == '\0';
}
...
bool engine_daemon_ipc_windows_generation_address(
    ...
    static const uint8_t domain[] = "engine-daemon-win-pipe-v1";
    if (!windows_sid_valid(sid, sid_length) || !instance_key_valid(instance_key) || !nonce ||
        !address_out) {
        return false;
    }
    engine_sha256_ctx context;
    engine_sha256_init(&context);
    engine_sha256_update(&context, domain, sizeof(domain) - 1U);
    engine_sha256_update(&context, sid, sid_length);
    engine_sha256_update(&context, instance_key, 16U);
    engine_sha256_update(&context, nonce, ENGINE_DAEMON_IPC_WINDOWS_NONCE_SIZE);
    uint8_t digest[ENGINE_SHA256_DIGEST_LEN];
    engine_sha256_final(&context, digest);

    char digest_hex[ENGINE_SHA256_HEX_LEN + 1U];
    static const char hex[] = "0123456789abcdef";
    for (size_t index = 0; index < sizeof(digest); index++) {
        digest_hex[index * 2U] = hex[digest[index] >> 4U];
        digest_hex[index * 2U + 1U] = hex[digest[index] & 0x0fU];
    }
    digest_hex[ENGINE_SHA256_HEX_LEN] = '\0';
    int written = snprintf(address_out, ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP,
                           "\\\\.\\pipe\\engine-daemon-%s", digest_hex);
    return written > 0 && (size_t)written < ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP;
}
...
bool engine_daemon_ipc_windows_rendezvous_record_encode(
    ...
    memset(record_out, 0, ENGINE_DAEMON_IPC_WINDOWS_RENDEZVOUS_RECORD_SIZE);
    memcpy(record_out, magic, sizeof(magic));
    memcpy(record_out + sizeof(magic), nonce, ENGINE_DAEMON_IPC_WINDOWS_NONCE_SIZE);
    size_t address_length = strlen(address);
    memcpy(record_out + sizeof(magic) + ENGINE_DAEMON_IPC_WINDOWS_NONCE_SIZE, address,
           address_length + 1U);
    return true;
}
...
bool engine_daemon_ipc_windows_rendezvous_record_decode(
    ...
    char address_out[ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP]) {
    static const uint8_t magic[8] = {'C', 'B', 'M', 'R', 'D', 'V', '1', 0};
    if (!record || record_length != ENGINE_DAEMON_IPC_WINDOWS_RENDEZVOUS_RECORD_SIZE || !nonce_out ||
        !address_out || memcmp(record, magic, sizeof(magic)) != 0) {
        return false;
    }
    const uint8_t *encoded_address = record + sizeof(magic) + ENGINE_DAEMON_IPC_WINDOWS_NONCE_SIZE;
    const uint8_t *terminator = memchr(encoded_address, 0, ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP);
    if (!terminator) {
        return false;
    }
    size_t address_length = (size_t)(terminator - encoded_address);
    if (address_length + 1U > ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP) {
        return false;
    }
    for (size_t index = address_length + 1U; index < ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP; index++) {
    ...
    if (!windows_pipe_address_valid(address_out)) {
        memset(address_out, 0, ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP);
        return false;
    }
    memcpy(nonce_out, record + sizeof(magic), ENGINE_DAEMON_IPC_WINDOWS_NONCE_SIZE);
    return true;
}
...
int engine_daemon_ipc_wait_pending(const engine_ipc_pending_ops_t *ops, uint32_t timeout_ms,
    ...
    engine_ipc_pending_wait_status_t wait_status = ops->wait(ops->context, timeout_ms);
    if (wait_status == ENGINE_IPC_PENDING_WAIT_SIGNALED) {
        return ops->finish(ops->context, false, transferred_out) == ENGINE_IPC_PENDING_FINISH_COMPLETED
                   ? 1
                   : -1;
    }
    ...
    engine_ipc_pending_finish_status_t finish_status = ops->finish(ops->context, true, &transferred);
    if (wait_status != ENGINE_IPC_PENDING_WAIT_TIMEOUT) {
        return -1;
    }
    if (finish_status == ENGINE_IPC_PENDING_FINISH_COMPLETED) {
        *transferred_out = transferred;
        return 1;
    }
    return finish_status == ENGINE_IPC_PENDING_FINISH_CANCELLED ? 0 : -1;
}
...
static void posix_publication_stage_reached(engine_daemon_ipc_posix_publication_stage_t stage) {
    engine_daemon_ipc_posix_publication_hook_fn hook = g_posix_publication_hook_for_test;
    if (hook) {
...
static void posix_record_publication_stage_reached(
    ...
    engine_daemon_ipc_posix_publication_stage_t stage;
    if (memcmp(magic, POSIX_SOCKET_PENDING_MAGIC, POSIX_SOCKET_RECORD_MAGIC_SIZE) == 0) {
        stage = linked ? ENGINE_DAEMON_IPC_POSIX_PUBLICATION_PENDING_RECORD_LINKED
                       : ENGINE_DAEMON_IPC_POSIX_PUBLICATION_PENDING_TEMP_SYNCED;
    } else if (memcmp(magic, POSIX_SOCKET_MARKER_MAGIC, POSIX_SOCKET_RECORD_MAGIC_SIZE) == 0) {
        stage = linked ? ENGINE_DAEMON_IPC_POSIX_PUBLICATION_MARKER_RECORD_LINKED
                       : ENGINE_DAEMON_IPC_POSIX_PUBLICATION_MARKER_TEMP_SYNCED;
    } else {
        return;
    }
...
struct engine_daemon_ipc_endpoint {
    char *runtime_dir;
    char *address;
    char *socket_name;
...
struct engine_daemon_ipc_lifetime_reservation {
    int fd;
    struct process_lock_entry *process_entry;
    pid_t owner_pid;
};

struct engine_daemon_ipc_listener {
    int fd;
    int dir_fd;
    char *runtime_dir;
    dev_t dir_device;
    ...
    pid_t owner_pid;
    engine_daemon_ipc_lifetime_reservation_t *lifetime_reservation;
    engine_daemon_ipc_participant_guard_t *participant_guard;
};

struct engine_daemon_ipc_connection {
    int fd;
...
struct engine_daemon_ipc_startup_lock {
    int startup_v2_fd;
    process_lock_entry_t *startup_v2_process_entry;
    int legacy_fd;
    process_lock_entry_t *legacy_process_entry;
    engine_daemon_ipc_endpoint_t *endpoint_snapshot;
    pid_t owner_pid;
    bool prepared;
};

struct engine_daemon_ipc_local_transition {
    int startup_v2_fd;
    process_lock_entry_t *startup_v2_process_entry;
    int legacy_fd;
    process_lock_entry_t *legacy_process_entry;
...
struct engine_daemon_ipc_participant_guard {
    int legacy_fd;
    process_lock_entry_t *legacy_process_entry;
    pid_t owner_pid;
};
...
static int process_lock_claim_mode(const engine_daemon_ipc_endpoint_t *endpoint, const char *lock_name,
...
static int process_lock_claim(const engine_daemon_ipc_endpoint_t *endpoint, const char *lock_name,
...
static int process_lock_is_claimed(const engine_daemon_ipc_endpoint_t *endpoint,
...
static _Noreturn void ipc_coordination_cleanup_fail_stop(const char *component) {
    engine_log_error("daemon.forced_shutdown", "component", component, "action",
...
static void ipc_startup_lock_release_complete(engine_daemon_ipc_startup_lock_t **lock_io) {
    uint64_t deadline = ipc_deadline_after(ENGINE_DAEMON_IPC_COORDINATION_CLEANUP_MS);
...
static void ipc_participant_guard_release_complete(engine_daemon_ipc_participant_guard_t **guard_io) {
    uint64_t deadline = ipc_deadline_after(ENGINE_DAEMON_IPC_COORDINATION_CLEANUP_MS);
...
static bool ipc_retry_pause(uint64_t deadline_ms) {
    ...
    int pause_ms = remaining_ms < ENGINE_DAEMON_IPC_RETRY_INTERVAL_MS
                       ? remaining_ms
                       : ENGINE_DAEMON_IPC_RETRY_INTERVAL_MS;
...
static bool private_runtime_snapshot(int fd, const char *path, bool require_empty_acl,
    ...
    bool valid = fd >= 0 && path && path[0] != '\0' && fstat(fd, &by_handle) == 0 &&
                 ...
                 (!require_empty_acl || engine_macos_extended_acl_fd_is_empty(fd));
    if (valid && status_out) {
...
static bool endpoint_runtime_still_valid(const engine_daemon_ipc_endpoint_t *endpoint) {
...
static bool private_regular_file_snapshot(int directory_fd, const char *base_name, int fd,
    ...
    bool valid = directory_fd >= 0 && base_name && base_name[0] != '\0' && fd >= 0 &&
                 ...
                 engine_macos_extended_acl_fd_is_empty(directory_fd) &&
                 engine_macos_extended_acl_fd_is_empty(fd);
    if (valid && status_out) {
        *status_out = by_handle;
    }
    return valid;
...
static int connection_read_full(engine_daemon_ipc_connection_t *connection, void *buffer,
...
static int connection_write_full(engine_daemon_ipc_connection_t *connection, const void *buffer,
                                 size_t length, uint64_t deadline_ms) {
    if (!connection || atomic_load_explicit(&connection->poisoned, memory_order_acquire)) {
        return -1;
    }
...
engine_daemon_ipc_endpoint_t *engine_daemon_ipc_endpoint_new(const char *instance_key,
    ...
    char canonical_parent[ENGINE_DAEMON_IPC_PATH_CAP];
    if (!engine_canonical_path(requested_parent, canonical_parent, sizeof(canonical_parent))) {
        return NULL;
    }
    ...
    engine_daemon_ipc_endpoint_t *endpoint = calloc(1, sizeof(*endpoint));
    if (!endpoint) {
        return NULL;
    }
    endpoint->dir_fd = -1;
    endpoint->runtime_dir =
        string_format("%s%s%s%lu", parent, parent[strlen(parent) - 1] == '/' ? "" : "/",
                      "engine-daemon-", (unsigned long)geteuid());
    endpoint->socket_name = string_format("engine-%s.sock", instance_key);
    endpoint->socket_anchor_name = string_format("engine-%s.anc", instance_key);
    endpoint->socket_identity_name = string_format("engine-%s.sock.identity", instance_key);
    endpoint->socket_pending_name = string_format("engine-%s.sock.pending", instance_key);
    endpoint->lock_name = string_format("engine-%s.lock", instance_key);
    endpoint->startup_v2_lock_name = string_format("engine-%s.startup-v2.lock", instance_key);
    endpoint->lifetime_lock_name = string_format("engine-%s.lifetime.lock", instance_key);
    if (!endpoint->runtime_dir || !endpoint->socket_name || !endpoint->socket_anchor_name ||
        ...
        !private_runtime_open(endpoint->runtime_dir, &endpoint->dir_fd, &endpoint->dir_device,
                              &endpoint->dir_inode)) {
        engine_daemon_ipc_endpoint_free(endpoint);
        return NULL;
    }
...
void engine_daemon_ipc_endpoint_free(engine_daemon_ipc_endpoint_t *endpoint) {
    if (!endpoint) {
        return;
    }
...
static engine_daemon_ipc_endpoint_t *endpoint_snapshot_new(const engine_daemon_ipc_endpoint_t *endpoint) {
    if (!endpoint_runtime_still_valid(endpoint)) {
        return NULL;
    }
    engine_daemon_ipc_endpoint_t *snapshot = calloc(1, sizeof(*snapshot));
    if (!snapshot) {
        return NULL;
    }
    snapshot->dir_fd = -1;
    ...
    if (!snapshot->runtime_dir || !snapshot->address || !snapshot->socket_name ||
        ...
        engine_daemon_ipc_endpoint_free(snapshot);
        return NULL;
    }
    return snapshot;
}

const char *engine_daemon_ipc_endpoint_address(const engine_daemon_ipc_endpoint_t *endpoint) {
    return endpoint ? endpoint->address : NULL;
}

const char *engine_daemon_ipc_endpoint_runtime_dir(const engine_daemon_ipc_endpoint_t *endpoint) {
    return endpoint ? endpoint->runtime_dir : NULL;
}

engine_private_file_lock_status_t engine_daemon_ipc_private_lock_directory_new(
    const engine_daemon_ipc_endpoint_t *endpoint, engine_private_lock_directory_t **directory_out) {
    if (directory_out) {
        *directory_out = NULL;
    }
    if (!directory_out || !endpoint_runtime_still_valid(endpoint)) {
        return ENGINE_PRIVATE_FILE_LOCK_UNSAFE;
    }
...
#endif
    if (duplicate < 0) {
        return ENGINE_PRIVATE_FILE_LOCK_IO;
    }
    engine_private_file_lock_status_t status =
        engine_private_lock_directory_adopt_posix(duplicate, endpoint->runtime_dir, directory_out);
    if (status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        (void)close(duplicate);
    }
    return status;
}
...
static int posix_named_lock_try_acquire_mode(const engine_daemon_ipc_endpoint_t *endpoint,
...
static int posix_named_lock_try_acquire(const engine_daemon_ipc_endpoint_t *endpoint,
...
static int posix_named_shared_lock_try_acquire(const engine_daemon_ipc_endpoint_t *endpoint,
                                               const char *lock_name, int *fd_out,
                                               process_lock_entry_t **process_entry_out) {
...
static int posix_lifetime_lock_probe(const engine_daemon_ipc_endpoint_t *endpoint,
                                     const char *lock_name) {
...
static int posix_lifetime_lock_try_acquire(const engine_daemon_ipc_endpoint_t *endpoint,
                                           const char *lock_name, int *fd_out,
                                           process_lock_entry_t **process_entry_out) {
    if (fd_out) {
        *fd_out = -1;
...
int engine_daemon_ipc_lifetime_reservation_try_acquire(
    const engine_daemon_ipc_endpoint_t *endpoint,
    engine_daemon_ipc_lifetime_reservation_t **reservation_out) {
    if (reservation_out) {
    ...
    engine_daemon_ipc_lifetime_reservation_t *reservation = malloc(sizeof(*reservation));
    if (!reservation) {
...
void engine_daemon_ipc_lifetime_reservation_release(
    engine_daemon_ipc_lifetime_reservation_t *reservation) {
    if (!reservation) {
        return;
    }
...
static bool lifetime_reservation_matches_endpoint(
    const engine_daemon_ipc_endpoint_t *endpoint,
    const engine_daemon_ipc_lifetime_reservation_t *reservation) {
...
int engine_daemon_ipc_lifetime_reservation_probe(const engine_daemon_ipc_endpoint_t *endpoint) {
    return endpoint ? posix_lifetime_lock_probe(endpoint, endpoint->lifetime_lock_name) : -1;
}

static int posix_startup_lock_probe(const engine_daemon_ipc_endpoint_t *endpoint) {
    if (!endpoint_runtime_still_valid(endpoint)) {
        return -1;
    }
...
int engine_daemon_ipc_legacy_generation_probe(const engine_daemon_ipc_endpoint_t *endpoint) {
    if (!endpoint_runtime_still_valid(endpoint)) {
        return -1;
    }
    struct stat status;
...
static char *private_log_directory_path_copy(const char *directory_path) {
    ...
    for (size_t index = 0; index < sizeof(aliases) / sizeof(aliases[0]); index++) {
        ...
        char resolved[ENGINE_DAEMON_IPC_PATH_CAP];
...
static bool posix_directory_parent_secure(int directory_fd) {
    ...
    if (directory_fd < 0 || fstat(directory_fd, &status) != 0 || !S_ISDIR(status.st_mode) ||
        ...
        !engine_macos_extended_acl_fd_is_deny_only(directory_fd)) {
        return false;
    }
...
static int private_directory_tree_open(const char *directory_path) {
        ...
        if (strcmp(component, ".") == 0) {
            ...
            if (ok && created) {
                ok = status.st_uid == geteuid() && fchmod(next_fd, 0700) == 0 &&
                     engine_macos_extended_acl_fd_clear(next_fd) &&
                     engine_macos_extended_acl_fd_is_empty(next_fd);
            }
            if (ok) {
    ...
    if (ok && !engine_macos_extended_acl_fd_clear(current_fd)) {
    ...
    if (ok && !engine_macos_extended_acl_fd_is_empty(current_fd)) {
...
bool engine_daemon_ipc_private_directory_secure(const char *directory_path) {
...
FILE *engine_daemon_ipc_private_log_open(const char *directory_path, const char *base_name,
...
static posix_record_state_t posix_socket_record_read_links(
    const engine_daemon_ipc_endpoint_t *endpoint, const char *record_name,
    ...
    bool metadata_safe =
        ...
        (before.st_mode & 07777) == 0600 && engine_macos_extended_acl_fd_is_empty(fd) &&
    ...
    bool stable = read_ok && fstat(fd, &after) == 0 &&
                  ...
                  engine_macos_extended_acl_fd_is_empty(fd) && after.st_size == before.st_size &&
...
static int posix_socket_path_identity_read(const engine_daemon_ipc_endpoint_t *endpoint,
...
static int posix_record_artifact_status(const engine_daemon_ipc_endpoint_t *endpoint, const char *name,
                                        struct stat *status_out) {
...
static int posix_socket_record_identity_corroborated(
    const engine_daemon_ipc_endpoint_t *endpoint, const uint8_t magic[POSIX_SOCKET_RECORD_MAGIC_SIZE],
...
static bool posix_socket_record_publish(const engine_daemon_ipc_endpoint_t *endpoint,
...
static bool posix_endpoint_namespace_equal(const engine_daemon_ipc_endpoint_t *left,
                                           const engine_daemon_ipc_endpoint_t *right) {
...
static bool posix_named_lock_is_retained(const engine_daemon_ipc_endpoint_t *endpoint,
...
static bool posix_startup_lock_matches_endpoint(const engine_daemon_ipc_endpoint_t *endpoint,
                                                const engine_daemon_ipc_startup_lock_t *startup_lock) {
...
static int posix_stale_generation_cleanup_locked(const engine_daemon_ipc_endpoint_t *endpoint) {
    ...
    engine_daemon_ipc_lifetime_reservation_t *cleanup_reservation = NULL;
    int reservation_result =
        engine_daemon_ipc_lifetime_reservation_try_acquire(endpoint, &cleanup_reservation);
    ...
    if (pending_publication != 1 || marker_publication != 1) {
        ...
        engine_daemon_ipc_lifetime_reservation_release(cleanup_reservation);
        return result;
    }
...
int engine_daemon_ipc_stale_generation_cleanup(const engine_daemon_ipc_endpoint_t *endpoint,
                                            const engine_daemon_ipc_startup_lock_t *startup_lock) {
...
static void posix_publication_abort(const engine_daemon_ipc_endpoint_t *endpoint,
...
engine_daemon_ipc_listener_t *engine_daemon_ipc_listen_reserved(
    const engine_daemon_ipc_endpoint_t *endpoint,
    ...
    if (!lifetime_reservation_matches_endpoint(endpoint, lifetime_reservation)) {
        engine_log_error("daemon.ipc.listen_failed", "stage", "reservation_validation");
        return NULL;
    }
    ...
    if (!posix_socket_record_temp_name(endpoint->socket_identity_name, identity_temp_name) ||
        ...
        engine_log_error("daemon.ipc.listen_failed", "stage", "temp_names");
        return NULL;
    }
    struct stat existing;
    ...
    if (!endpoint_runtime_still_valid(endpoint) || !namespace_absent) {
        engine_log_error("daemon.ipc.listen_failed", "stage", "namespace_validation");
        return NULL;
    }

    int fd = local_socket_new();
    if (fd < 0) {
        engine_log_error("daemon.ipc.listen_failed", "stage", "socket_creation");
        return NULL;
    }
    engine_daemon_ipc_listener_t *listener = calloc(1, sizeof(*listener));
    if (!listener) {
        engine_log_error("daemon.ipc.listen_failed", "stage", "listener_allocation");
        (void)close(fd);
        return NULL;
    }
    listener->fd = fd;
    ...
    if (listener->dir_fd < 0 || !fd_set_cloexec(listener->dir_fd) || !listener->runtime_dir ||
        ...
        engine_log_error("daemon.ipc.listen_failed", "stage", "listener_initialization");
    ...
    if (!unix_address_set(&address, endpoint->socket_anchor_address, &address_length)) {
        engine_log_error("daemon.ipc.listen_failed", "stage", "socket_address");
        engine_daemon_ipc_listener_close(listener);
        return NULL;
    }
    if (bind(fd, (const struct sockaddr *)&address, address_length) != 0) {
        ...
        engine_log_error("daemon.ipc.listen_failed", "bind_errno", error_text);
        engine_daemon_ipc_listener_close(listener);
        return NULL;
    }
    ...
    if (!secured || listen(fd, 32) != 0 ||
        ...
        engine_log_error("daemon.ipc.listen_failed", "stage", "socket_security");
        engine_daemon_ipc_listener_close(listener);
        return NULL;
    }
    listener->socket_identity = anchor_identity;
    posix_publication_stage_reached(ENGINE_DAEMON_IPC_POSIX_PUBLICATION_ANCHOR_DURABLE);
    ...
    if (!pending_published) {
        engine_log_error("daemon.ipc.listen_failed", "stage", "pending_publication");
        posix_publication_abort(endpoint, &anchor_identity, false, NULL, false, NULL);
        engine_daemon_ipc_listener_close(listener);
        return NULL;
    }
    posix_publication_stage_reached(ENGINE_DAEMON_IPC_POSIX_PUBLICATION_PENDING_DURABLE);
    ...
    if (!stable_valid) {
        engine_log_error("daemon.ipc.listen_failed", "stage", "stable_publication");
    ...
    posix_publication_stage_reached(ENGINE_DAEMON_IPC_POSIX_PUBLICATION_STABLE_DURABLE);
    ...
    bool marker_published = posix_socket_record_publish(
        ...
        &marker_status.st_dev, &marker_status.st_ino);
    if (!marker_published) {
        engine_log_error("daemon.ipc.listen_failed", "stage", "marker_publication");
    ...
    posix_publication_stage_reached(ENGINE_DAEMON_IPC_POSIX_PUBLICATION_MARKER_DURABLE);

    if (!posix_path_unlink_regular_if_matches(endpoint->dir_fd, endpoint->socket_pending_name,
                                              pending_status.st_dev, pending_status.st_ino, 1) ||
        !posix_directory_sync(endpoint->dir_fd)) {
        engine_log_error("daemon.ipc.listen_failed", "stage", "pending_removal");
        posix_publication_abort(endpoint, &committed_identity, true, &pending_status,
                                marker_published, &marker_status);
        engine_daemon_ipc_listener_close(listener);
        return NULL;
    }
    posix_publication_stage_reached(ENGINE_DAEMON_IPC_POSIX_PUBLICATION_PENDING_REMOVED);
    listener->lifetime_reservation = lifetime_reservation;
    *reservation_io = NULL;
    return listener;
}

engine_daemon_ipc_listener_t *engine_daemon_ipc_listen(const engine_daemon_ipc_endpoint_t *endpoint) {
    engine_daemon_ipc_startup_lock_t *startup_lock = NULL;
    if (engine_daemon_ipc_startup_lock_try_acquire(endpoint, &startup_lock) != 1) {
        return NULL;
    }
    engine_daemon_ipc_lifetime_reservation_t *reservation = NULL;
    engine_daemon_ipc_participant_guard_t *participant_guard = NULL;
    if (engine_daemon_ipc_stale_generation_cleanup(endpoint, startup_lock) != 1 ||
        engine_daemon_ipc_participant_guard_try_join(endpoint, &participant_guard) != 1 ||
        engine_daemon_ipc_lifetime_reservation_try_acquire(endpoint, &reservation) != 1) {
        ipc_participant_guard_release_complete(&participant_guard);
        ipc_startup_lock_release_complete(&startup_lock);
        return NULL;
    }
    engine_daemon_ipc_listener_t *listener = engine_daemon_ipc_listen_reserved(endpoint, &reservation);
    if (listener) {
...
static void posix_listener_artifacts_remove_if_matches(engine_daemon_ipc_listener_t *listener) {
    ...
    engine_daemon_ipc_endpoint_t view = {
...
void engine_daemon_ipc_listener_close(engine_daemon_ipc_listener_t *listener) {
    if (!listener) {
        return;
    }
    if (listener->fd >= 0) {
    ...
    engine_daemon_ipc_lifetime_reservation_release(listener->lifetime_reservation);
...
int engine_daemon_ipc_accept(engine_daemon_ipc_listener_t *listener, uint32_t timeout_ms,
                          engine_daemon_ipc_connection_t **connection_out) {
    if (connection_out) {
        *connection_out = NULL;
    }
    ...
    for (;;) {
        ...
        engine_daemon_ipc_connection_t *connection = malloc(sizeof(*connection));
        if (!connection) {
            (void)close(fd);
            return -1;
        }
...
int engine_daemon_ipc_endpoint_probe(const engine_daemon_ipc_endpoint_t *endpoint, uint32_t timeout_ms) {
    if (!endpoint_runtime_still_valid(endpoint)) {
        return -1;
    }
    struct stat status;
...
engine_daemon_ipc_connection_t *engine_daemon_ipc_connect(const engine_daemon_ipc_endpoint_t *endpoint,
                                                    uint32_t timeout_ms) {
    if (!endpoint_runtime_still_valid(endpoint)) {
        return NULL;
    }
...
void engine_daemon_ipc_connection_close(engine_daemon_ipc_connection_t *connection) {
    if (!connection) {
        return;
    }
    if (connection->fd >= 0) {
...
void engine_daemon_ipc_connection_drain(engine_daemon_ipc_connection_t *connection, uint32_t timeout_ms) {
...
void engine_daemon_ipc_connection_interrupt(engine_daemon_ipc_connection_t *connection) {
    if (connection && connection->fd >= 0) {
        (void)shutdown(connection->fd, SHUT_RDWR);
    }
}

uint64_t engine_daemon_ipc_connection_peer_pid(const engine_daemon_ipc_connection_t *connection) {
...
int engine_daemon_ipc_startup_lock_try_acquire(const engine_daemon_ipc_endpoint_t *endpoint,
                                            engine_daemon_ipc_startup_lock_t **lock_out) {
    if (lock_out) {
        *lock_out = NULL;
    }
    ...
    engine_daemon_ipc_startup_lock_t *lock = calloc(1, sizeof(*lock));
    if (lock) {
        lock->endpoint_snapshot = endpoint_snapshot_new(endpoint);
    }
    if (!lock || !lock->endpoint_snapshot) {
        if (lock) {
            engine_daemon_ipc_endpoint_free(lock->endpoint_snapshot);
        }
        free(lock);
...
int engine_daemon_ipc_generation_probe_under_startup_lock(
    const engine_daemon_ipc_endpoint_t *endpoint, const engine_daemon_ipc_startup_lock_t *startup_lock) {
    if (!posix_startup_lock_matches_endpoint(endpoint, startup_lock) || startup_lock->prepared) {
        return -1;
    }
    int lifetime = engine_daemon_ipc_lifetime_reservation_probe(endpoint);
    if (lifetime != 0) {
        return lifetime;
    }
    return engine_daemon_ipc_endpoint_probe(endpoint, 0);
}

bool engine_daemon_ipc_startup_lock_prepare_handoff(engine_daemon_ipc_startup_lock_t *lock) {
    if (!lock || !lock->endpoint_snapshot) {
        return false;
    }
    if (lock->prepared) {
    ...
    lock->prepared = engine_daemon_ipc_stale_generation_cleanup(lock->endpoint_snapshot, lock) == 1;
    return lock->prepared;
}

bool engine_daemon_ipc_startup_lock_release(engine_daemon_ipc_startup_lock_t **lock_io) {
    if (!lock_io) {
        return false;
    }
    engine_daemon_ipc_startup_lock_t *lock = *lock_io;
    if (!lock) {
        return true;
    }
...
int engine_daemon_ipc_participant_guard_try_join(const engine_daemon_ipc_endpoint_t *endpoint,
                                              engine_daemon_ipc_participant_guard_t **guard_out) {
    if (guard_out) {
        *guard_out = NULL;
    }
    ...
    engine_daemon_ipc_participant_guard_t *guard = calloc(1, sizeof(*guard));
    if (!guard) {
        posix_named_lock_release(legacy_fd, legacy_process_entry);
        return -1;
    }
...
bool engine_daemon_ipc_participant_guard_release(engine_daemon_ipc_participant_guard_t **guard_io) {
    if (!guard_io) {
        return false;
    }
    engine_daemon_ipc_participant_guard_t *guard = *guard_io;
    if (!guard) {
        return true;
    }
...
int engine_daemon_ipc_local_transition_try_acquire(
    const engine_daemon_ipc_endpoint_t *endpoint, engine_daemon_ipc_local_transition_t **transition_out) {
    if (transition_out) {
        *transition_out = NULL;
    }
    ...
    engine_daemon_ipc_local_transition_t *transition = calloc(1, sizeof(*transition));
    if (!transition) {
        posix_named_lock_release(legacy_fd, legacy_process_entry);
        posix_named_lock_release(startup_v2_fd, startup_v2_process_entry);
        return -1;
...
int engine_daemon_ipc_local_transition_seal_legacy(engine_daemon_ipc_local_transition_t *transition) {
    ...
    int lifetime = engine_daemon_ipc_lifetime_reservation_probe(transition->endpoint);
...
int engine_daemon_ipc_local_transition_lifetime_probe(
    const engine_daemon_ipc_endpoint_t *endpoint,
    ...
    return engine_daemon_ipc_lifetime_reservation_probe(endpoint);
}

bool engine_daemon_ipc_local_transition_begin_work(engine_daemon_ipc_local_transition_t *transition) {
...
bool engine_daemon_ipc_local_transition_release(engine_daemon_ipc_local_transition_t **transition_io) {
    if (!transition_io) {
        return false;
    }
    engine_daemon_ipc_local_transition_t *transition = *transition_io;
    if (!transition) {
        return true;
    }
...
typedef struct win_generation_address {
    char address[ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP];
    wchar_t *pipe_name;
...
struct engine_daemon_ipc_lifetime_reservation {
    const struct engine_daemon_ipc_endpoint *endpoint;
    engine_private_lock_directory_t *directory;
    engine_private_file_lock_t *lock;
};
...
struct engine_daemon_ipc_endpoint {
    ...
    engine_mutex_t generations_lock;
...
typedef enum {
    ENGINE_DAEMON_IPC_PIPE_ROLE_ACCEPTED_SERVER = 1,
    ENGINE_DAEMON_IPC_PIPE_ROLE_CONNECTED_CLIENT = 2,
} engine_daemon_ipc_pipe_role_t;

struct engine_daemon_ipc_connection {
    HANDLE handle;
    atomic_bool poisoned;
    engine_daemon_ipc_pipe_role_t role;
};

struct engine_daemon_ipc_startup_lock {
    const engine_daemon_ipc_endpoint_t *endpoint;
    engine_private_lock_directory_t *directory;
    engine_private_file_lock_t *startup_v2_lock;
    win_legacy_mutex_guard_t *legacy_guard;
    engine_private_file_lock_t *group_lock;
...
struct engine_daemon_ipc_participant_guard {
    ...
    engine_private_file_lock_t *group_lock;
    HANDLE legacy_sentinel;
    engine_private_file_lock_t *teardown_startup_v2_lock;
    win_legacy_mutex_guard_t *teardown_legacy_guard;
};
...
static bool win_retry_pause(uint64_t deadline_ms) {
    ...
    DWORD pause_ms = remaining_ms < ENGINE_DAEMON_IPC_RETRY_INTERVAL_MS
                         ? remaining_ms
                         : ENGINE_DAEMON_IPC_RETRY_INTERVAL_MS;
    Sleep(pause_ms);
    return true;
...
static bool win_legacy_mutex_guard_release(win_legacy_mutex_guard_t **guard_io) {
    ...
    bool joined = !guard->owner_started || engine_thread_join(&guard->owner_thread) == 0;
    bool released =
...
static int win_legacy_mutex_guard_try_acquire(const engine_daemon_ipc_endpoint_t *endpoint,
    ...
    if (!guard || !guard->ready_event || !guard->stop_event ||
        engine_thread_create(&guard->owner_thread, 0, win_legacy_mutex_owner, guard) != 0) {
        if (guard) {
...
static win_generation_address_t *win_endpoint_generation_snapshot(
    const engine_daemon_ipc_endpoint_t *endpoint);
static int win_legacy_pipe_probe(const engine_daemon_ipc_endpoint_t *endpoint);
...
engine_daemon_ipc_endpoint_t *engine_daemon_ipc_endpoint_new(const char *instance_key,
    ...
    char canonical_parent[ENGINE_DAEMON_IPC_PATH_CAP];
    if (!engine_canonical_path(parent_utf8, canonical_parent, sizeof(canonical_parent))) {
        free(parent_utf8);
        return NULL;
    }
    free(parent_utf8);
    ...
    char legacy_pipe[ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP];
    char legacy_startup[ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP];
    bool legacy_names_ok =
        engine_daemon_ipc_windows_legacy_names(parent_utf8, instance_key, legacy_pipe, legacy_startup);
    endpoint->runtime_dir =
        string_format("%s%sengine-daemon-%s", parent_utf8, has_separator ? "" : "/", instance_key);
    ...
    engine_mutex_init(&endpoint->generations_lock);
...
void engine_daemon_ipc_endpoint_free(engine_daemon_ipc_endpoint_t *endpoint) {
    ...
    engine_mutex_destroy(&endpoint->generations_lock);
    free(endpoint->runtime_dir);
...
engine_private_file_lock_status_t engine_daemon_ipc_private_lock_directory_new(
    ...
    engine_private_file_lock_status_t status =
        engine_private_lock_directory_adopt_windows(directory, endpoint->runtime_dir, directory_out);
    if (status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        (void)CloseHandle(directory);
    }
    return status;
...
static const char WIN_STARTUP_V2_LOCK_NAME[] = "engine-startup-v2.lock";
static const char WIN_PARTICIPANT_GROUP_LOCK_NAME[] = "engine-participant-group-v1.lock";
static const char WIN_LIFETIME_LOCK_NAME[] = "engine-lifetime.lock";

static void win_private_lock_release_complete(engine_private_file_lock_t **lock_io) {
    uint64_t deadline = ipc_deadline_after(ENGINE_DAEMON_IPC_COORDINATION_CLEANUP_MS);
    while (lock_io && *lock_io) {
        (void)engine_private_file_lock_release(lock_io);
        if (!*lock_io) {
            return;
        }
        if (ipc_now_ms() >= deadline) {
...
static void win_endpoint_generation_invalidate(const engine_daemon_ipc_endpoint_t *endpoint) {
    if (endpoint) {
        atomic_store_explicit(&((engine_daemon_ipc_endpoint_t *)endpoint)->current_generation, NULL,
                              memory_order_release);
    }
}

static bool win_endpoint_generation_install(const engine_daemon_ipc_endpoint_t *endpoint,
                                            const char *address) {
    ...
    engine_daemon_ipc_endpoint_t *mutable_endpoint = (engine_daemon_ipc_endpoint_t *)endpoint;
    engine_mutex_lock(&mutable_endpoint->generations_lock);
    ...
    if (current && strcmp(current->address, address) == 0) {
        engine_mutex_unlock(&mutable_endpoint->generations_lock);
...
static win_rendezvous_status_t win_endpoint_refresh_rendezvous(
    ...
    engine_private_lock_directory_t *directory = NULL;
    engine_private_file_lock_t *record_lock = NULL;
    engine_private_file_lock_status_t directory_status =
        engine_daemon_ipc_private_lock_directory_new(endpoint, &directory);
    if (directory_status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        win_endpoint_generation_invalidate(endpoint);
        return WIN_RENDEZVOUS_ERROR;
    }
    engine_private_file_lock_status_t lock_status = engine_private_file_lock_try_acquire(
        directory, ENGINE_DAEMON_IPC_WINDOWS_RENDEZVOUS_FILE, ENGINE_PRIVATE_FILE_LOCK_SH, &record_lock);
    if (lock_status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        win_private_lock_release_complete(&record_lock);
        engine_private_lock_directory_close(directory);
        win_endpoint_generation_invalidate(endpoint);
        return lock_status == ENGINE_PRIVATE_FILE_LOCK_BUSY ? WIN_RENDEZVOUS_BUSY
                                                         : WIN_RENDEZVOUS_ERROR;
    }

    uint8_t record[ENGINE_DAEMON_IPC_WINDOWS_RENDEZVOUS_RECORD_SIZE];
    size_t record_length = 0;
    engine_private_file_lock_status_t read_status =
        engine_private_file_lock_payload_read(record_lock, record, sizeof(record), &record_length);
    uint8_t nonce[ENGINE_DAEMON_IPC_WINDOWS_NONCE_SIZE];
    char address[ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP];
    bool decoded =
        read_status == ENGINE_PRIVATE_FILE_LOCK_OK &&
        engine_daemon_ipc_windows_rendezvous_record_decode(record, record_length, nonce, address);
    char expected[ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP];
    bool bound =
        decoded &&
        engine_daemon_ipc_windows_generation_address(endpoint->user_sid, endpoint->user_sid_length,
                                                  endpoint->instance_key, nonce, expected) &&
        strcmp(expected, address) == 0;
    win_rendezvous_status_t result;
    if (read_status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        win_endpoint_generation_invalidate(endpoint);
...
static bool win_generation_nonce(uint8_t nonce[ENGINE_DAEMON_IPC_WINDOWS_NONCE_SIZE]) {
    return engine_secure_random(nonce, ENGINE_DAEMON_IPC_WINDOWS_NONCE_SIZE);
}

static int win_private_lock_probe(engine_private_lock_directory_t *directory, const char *base_name) {
    engine_private_file_lock_t *probe = NULL;
    engine_private_file_lock_status_t status =
        engine_private_file_lock_try_acquire(directory, base_name, ENGINE_PRIVATE_FILE_LOCK_SH, &probe);
...
static bool win_startup_publish_generation_locked(const engine_daemon_ipc_endpoint_t *endpoint,
                                                  engine_private_file_lock_t *record_lock) {
    uint8_t nonce[ENGINE_DAEMON_IPC_WINDOWS_NONCE_SIZE];
    char address[ENGINE_DAEMON_IPC_WINDOWS_NAME_CAP];
    uint8_t record[ENGINE_DAEMON_IPC_WINDOWS_RENDEZVOUS_RECORD_SIZE];
    if (!record_lock || !win_generation_nonce(nonce) ||
        ...
        !engine_daemon_ipc_windows_rendezvous_record_encode(nonce, address, record)) {
        return false;
    }
    bool written = engine_private_file_lock_payload_write(record_lock, record, sizeof(record)) ==
                   ENGINE_PRIVATE_FILE_LOCK_OK;
...
static int win_startup_prepare_generation(const engine_daemon_ipc_endpoint_t *endpoint,
                                          engine_private_lock_directory_t *directory) {
    uint64_t now = ipc_now_ms();
    uint64_t deadline = now > UINT64_MAX - ENGINE_DAEMON_IPC_WINDOWS_RENDEZVOUS_RETRY_MS
                            ? UINT64_MAX
                            : now + ENGINE_DAEMON_IPC_WINDOWS_RENDEZVOUS_RETRY_MS;
    for (;;) {
        engine_private_file_lock_t *record_lock = NULL;
        engine_private_file_lock_status_t status =
            engine_private_file_lock_try_acquire(directory, ENGINE_DAEMON_IPC_WINDOWS_RENDEZVOUS_FILE,
                                              ENGINE_PRIVATE_FILE_LOCK_EX, &record_lock);
        if (status == ENGINE_PRIVATE_FILE_LOCK_OK) {
...
int engine_daemon_ipc_lifetime_reservation_try_acquire(
    ...
    if (rendezvous == WIN_RENDEZVOUS_ABSENT || rendezvous == WIN_RENDEZVOUS_CORRUPT) {
        engine_daemon_ipc_startup_lock_t *startup = NULL;
        int startup_status = engine_daemon_ipc_startup_lock_try_acquire(endpoint, &startup);
        bool prepared = startup_status == 1 && engine_daemon_ipc_startup_lock_prepare_handoff(startup);
    ...
    engine_private_lock_directory_t *directory = NULL;
    engine_private_file_lock_t *lock = NULL;
    if (engine_daemon_ipc_private_lock_directory_new(endpoint, &directory) !=
        ENGINE_PRIVATE_FILE_LOCK_OK) {
        return -1;
    }
    engine_private_file_lock_status_t status = engine_private_file_lock_try_acquire(
        directory, WIN_LIFETIME_LOCK_NAME, ENGINE_PRIVATE_FILE_LOCK_EX, &lock);
    if (status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        win_private_lock_release_complete(&lock);
        engine_private_lock_directory_close(directory);
        return status == ENGINE_PRIVATE_FILE_LOCK_BUSY ? 0 : -1;
    }
    ...
    engine_daemon_ipc_lifetime_reservation_t *reservation = calloc(1, sizeof(*reservation));
    if (!reservation) {
        win_private_lock_release_complete(&lock);
        engine_private_lock_directory_close(directory);
        return -1;
...
void engine_daemon_ipc_lifetime_reservation_release(
    ...
    engine_private_lock_directory_close(reservation->directory);
    free(reservation);
}
...
int engine_daemon_ipc_lifetime_reservation_probe(const engine_daemon_ipc_endpoint_t *endpoint) {
    engine_private_lock_directory_t *directory = NULL;
    if (!endpoint || engine_daemon_ipc_private_lock_directory_new(endpoint, &directory) !=
                         ENGINE_PRIVATE_FILE_LOCK_OK) {
        return -1;
    }
...
static HANDLE win_legacy_sentinel_new(const engine_daemon_ipc_endpoint_t *endpoint, bool first,
                                      DWORD *error_out) {
    if (error_out) {
...
static int win_startup_v2_try_acquire(engine_private_lock_directory_t *directory,
                                      engine_private_file_lock_t **lock_out) {
    if (lock_out) {
        *lock_out = NULL;
    }
    ...
    engine_private_file_lock_status_t status = engine_private_file_lock_try_acquire(
        directory, WIN_STARTUP_V2_LOCK_NAME, ENGINE_PRIVATE_FILE_LOCK_EX, lock_out);
    if (status == ENGINE_PRIVATE_FILE_LOCK_OK) {
        return 1;
    }
...
static int win_participant_group_join_locked(const engine_daemon_ipc_endpoint_t *endpoint,
                                             engine_private_lock_directory_t *directory,
                                             bool allow_first, engine_private_file_lock_t **group_out,
                                             HANDLE *sentinel_out) {
    if (group_out) {
        *group_out = NULL;
    }
    ...
    engine_private_file_lock_t *exclusive_probe = NULL;
    engine_private_file_lock_status_t group_status = engine_private_file_lock_try_acquire(
        directory, WIN_PARTICIPANT_GROUP_LOCK_NAME, ENGINE_PRIVATE_FILE_LOCK_EX, &exclusive_probe);
    bool first = group_status == ENGINE_PRIVATE_FILE_LOCK_OK;
    if (!first && group_status != ENGINE_PRIVATE_FILE_LOCK_BUSY) {
    ...
    engine_private_file_lock_t *group = NULL;
    group_status = engine_private_file_lock_try_acquire(directory, WIN_PARTICIPANT_GROUP_LOCK_NAME,
                                                     ENGINE_PRIVATE_FILE_LOCK_SH, &group);
    if (group_status != ENGINE_PRIVATE_FILE_LOCK_OK) {
        win_private_lock_release_complete(&group);
        (void)CloseHandle(sentinel);
        return group_status == ENGINE_PRIVATE_FILE_LOCK_BUSY ? 0 : -1;
    }
    *group_out = group;
...
static bool win_participant_group_release_locked(engine_private_file_lock_t **group_io,
                                                 HANDLE *sentinel_io) {
    if (!group_io || !sentinel_io) {
        return false;
    }
    if (*group_io && engine_private_file_lock_release(group_io) != ENGINE_PRIVATE_FILE_LOCK_OK) {
        return false;
    }
...
static void win_participant_group_release_complete(engine_private_file_lock_t **group_io,
                                                   HANDLE *sentinel_io) {
    uint64_t deadline = ipc_deadline_after(ENGINE_DAEMON_IPC_COORDINATION_CLEANUP_MS);
...
static bool win_participant_state_release(const engine_daemon_ipc_endpoint_t *endpoint,
                                          engine_private_lock_directory_t *directory,
                                          engine_private_file_lock_t **startup_v2_io,
                                          win_legacy_mutex_guard_t **legacy_guard_io,
                                          engine_private_file_lock_t **group_io, HANDLE *sentinel_io) {
    ...
    if (!*legacy_guard_io) {
        ...
        if (legacy != 1) {
            (void)engine_private_file_lock_release(startup_v2_io);
            return false;
        }
    }
    ...
    return engine_private_file_lock_release(startup_v2_io) == ENGINE_PRIVATE_FILE_LOCK_OK;
}
...
engine_daemon_ipc_listener_t *engine_daemon_ipc_listen(const engine_daemon_ipc_endpoint_t *endpoint) {
    ...
    engine_daemon_ipc_participant_guard_t *participant_guard = NULL;
    engine_daemon_ipc_lifetime_reservation_t *reservation = NULL;
    if (engine_daemon_ipc_startup_lock_try_acquire(endpoint, &startup) != 1 ||
        !engine_daemon_ipc_startup_lock_prepare_handoff(startup) ||
        engine_daemon_ipc_participant_guard_try_join(endpoint, &participant_guard) != 1 ||
        engine_daemon_ipc_lifetime_reservation_try_acquire(endpoint, &reservation) != 1) {
        engine_daemon_ipc_lifetime_reservation_release(reservation);
        ipc_participant_guard_release_complete(&participant_guard);
...
static engine_ipc_pending_wait_status_t win_pending_wait(void *opaque, uint32_t timeout_ms) {
    ...
    if (result == WAIT_OBJECT_0) {
        return ENGINE_IPC_PENDING_WAIT_SIGNALED;
    }
    return result == WAIT_TIMEOUT ? ENGINE_IPC_PENDING_WAIT_TIMEOUT : ENGINE_IPC_PENDING_WAIT_FAILED;
}
...
static engine_ipc_pending_finish_status_t win_pending_finish(void *opaque, bool blocking,
                                                          uint32_t *transferred_out) {
    win_pending_io_t *pending = (win_pending_io_t *)opaque;
    DWORD transferred = 0;
    if (GetOverlappedResult(pending->handle, pending->overlapped, &transferred,
                            blocking ? TRUE : FALSE)) {
        *transferred_out = transferred;
        return ENGINE_IPC_PENDING_FINISH_COMPLETED;
    }
    return GetLastError() == ERROR_OPERATION_ABORTED ? ENGINE_IPC_PENDING_FINISH_CANCELLED
                                                     : ENGINE_IPC_PENDING_FINISH_FAILED;
}
...
static int win_overlapped_wait(HANDLE handle, OVERLAPPED *overlapped, DWORD timeout_ms,
    ...
    uint32_t transferred = 0;
    int result = engine_daemon_ipc_wait_pending(&ops, timeout_ms, &transferred);
    if (result == 1) {
...
static bool win_pipe_client_is_current_user(HANDLE pipe) {
    ...
    get_named_pipe_server_process_id_fn get_client_pid =
        kernel ? (get_named_pipe_server_process_id_fn)(void (*)(void))GetProcAddress(
               ...
               : NULL;
    if (!get_client_pid) {
        engine_log_warn("daemon.accept.client_identity", "step", "pid_fn_missing");
        return false;
    }
    ULONG process_id = 0;
    if (!get_client_pid(pipe, &process_id) || process_id == 0) {
        ...
        engine_log_warn("daemon.accept.client_identity", "step", "client_pid_query", "error",
                     error_text);
        return false;
    }
    ...
    if (!process) {
        ...
        (void)snprintf(error_text, sizeof(error_text), "%lu", (unsigned long)GetLastError());
        engine_log_warn("daemon.accept.client_identity", "step", "process_open", "pid", pid_text,
                     "error", error_text);
        return false;
    }
    win_security_t security;
    ...
    if (!same_user) {
        const char *step = !initialized ? "security_init"
                           ...
                           : !token_sid ? "token_query"
                                        : "sid_mismatch";
        engine_log_warn("daemon.accept.client_identity", "step", step);
    }
    free(token_user);
    if (token) {
        (void)CloseHandle(token);
...
int engine_daemon_ipc_accept(engine_daemon_ipc_listener_t *listener, uint32_t timeout_ms,
    ...
    atomic_init(&connection->poisoned, false);
    connection->role = ENGINE_DAEMON_IPC_PIPE_ROLE_ACCEPTED_SERVER;
    ...
    if (!win_pipe_client_is_current_user(listener->pipe)) {
        engine_log_warn("daemon.accept.client_rejected", "stage", "client_identity");
        free(connection);
        (void)DisconnectNamedPipe(listener->pipe);
        (void)CloseHandle(listener->pipe);
        listener->pipe = INVALID_HANDLE_VALUE;
...
static bool win_pipe_server_is_current_user(HANDLE pipe) {
    ...
    ULONG process_id = 0;
    if (!get_server_pid) {
        engine_log_warn("daemon.client.server_identity", "step", "pid_fn_missing");
        return false;
    }
    if (!get_server_pid(pipe, &process_id) || process_id == 0) {
        ...
        char pid_text[16];
        (void)snprintf(pid_text, sizeof(pid_text), "%lu", (unsigned long)process_id);
        engine_log_warn("daemon.client.server_identity", "step", "server_pid_query", "error",
    ...
    if (!process) {
        ...
        char error_text[16];
        (void)snprintf(error_text, sizeof(error_text), "%lu", (unsigned long)GetLastError());
        engine_log_warn("daemon.client.server_identity", "step", "process_open", "pid", pid_text,
                     "error", error_text);
        return false;
    }
    win_security_t security;
    ...
    if (!same_user) {
        ...
        char pid_text[16];
        (void)snprintf(pid_text, sizeof(pid_text), "%lu", (unsigned long)process_id);
        engine_log_warn("daemon.client.server_identity", "step", step, "pid", pid_text);
    }
    free(token_user);
    if (token) {
        (void)CloseHandle(token);
...
static int win_current_generation_transport_probe(const engine_daemon_ipc_endpoint_t *endpoint) {
    win_rendezvous_status_t rendezvous = win_endpoint_refresh_rendezvous(endpoint);
...
engine_daemon_ipc_connection_t *engine_daemon_ipc_connect(const engine_daemon_ipc_endpoint_t *endpoint,
    ...
    for (;;) {
        ...
        if (rendezvous == WIN_RENDEZVOUS_CORRUPT || rendezvous == WIN_RENDEZVOUS_ERROR) {
            ...
            engine_log_warn("daemon.client.rendezvous_unreadable", "status",
        ...
        if (!generation) {
            if (win_retry_pause(deadline_ms)) {
                if (!wait_logged) {
                    wait_logged = true;
                    engine_log_warn("daemon.client.rendezvous_wait", "status",
            ...
            engine_log_warn("daemon.client.connect_deadline", "status",
                         rendezvous == WIN_RENDEZVOUS_ABSENT  ? "absent"
                         : rendezvous == WIN_RENDEZVOUS_BUSY  ? "busy"
                         : rendezvous == WIN_RENDEZVOUS_VALID ? "no_generation"
                                                              : "unknown");
    ...
    if (!SetNamedPipeHandleState(pipe, &mode, NULL, NULL) ||
        !win_pipe_server_is_current_user(pipe)) {
        engine_log_warn("daemon.client.pipe_rejected", "stage", "server_identity");
        (void)CloseHandle(pipe);
        return NULL;
    }
    engine_daemon_ipc_connection_t *connection = malloc(sizeof(*connection));
    ...
    atomic_init(&connection->poisoned, false);
    connection->role = ENGINE_DAEMON_IPC_PIPE_ROLE_CONNECTED_CLIENT;
    return connection;
}
...
static int win_io_once(engine_daemon_ipc_connection_t *connection, void *buffer, DWORD length,
...
void engine_daemon_ipc_connection_interrupt(engine_daemon_ipc_connection_t *connection) {
    ...
    (void)CancelIoEx(connection->handle, NULL);
    if (connection->role == ENGINE_DAEMON_IPC_PIPE_ROLE_ACCEPTED_SERVER) {
...
uint64_t engine_daemon_ipc_connection_peer_pid(const engine_daemon_ipc_connection_t *connection) {
    ...
    } else if (connection->role == ENGINE_DAEMON_IPC_PIPE_ROLE_CONNECTED_CLIENT) {
        get_named_pipe_server_process_id_fn get_server_pid =
            kernel ? (get_named_pipe_server_process_id_fn)(void (*)(void))GetProcAddress(
                         kernel, "GetNamedPipeServerProcessId")
                   : NULL;
...
int engine_daemon_ipc_startup_lock_try_acquire(const engine_daemon_ipc_endpoint_t *endpoint,
    ...
    engine_private_file_lock_t *startup_v2 = NULL;
...
bool engine_daemon_ipc_startup_lock_release(engine_daemon_ipc_startup_lock_t **lock_io) {
    ...
    engine_private_lock_directory_close(lock->directory);
    free(lock);
    *lock_io = NULL;
    return true;
}
...
bool engine_daemon_ipc_participant_guard_release(engine_daemon_ipc_participant_guard_t **guard_io) {
    ...
    engine_private_lock_directory_close(guard->directory);
    free(guard);
    *guard_io = NULL;
    return true;
}
...
bool engine_daemon_ipc_local_transition_begin_work(engine_daemon_ipc_local_transition_t *transition) {
    ...
    if (engine_private_file_lock_release(&transition->startup_v2_lock) != ENGINE_PRIVATE_FILE_LOCK_OK) {
        return false;
    }
    transition->work_begun = true;
    return true;
    ...
    if (transition->group_lock || transition->legacy_sentinel != INVALID_HANDLE_VALUE ||
        ...
        if (!win_participant_state_release(transition->endpoint, transition->directory,
            ...
            return false;
               ...
               engine_private_file_lock_release(&transition->startup_v2_lock) !=
                   ENGINE_PRIVATE_FILE_LOCK_OK) {
        return false;
    }
    engine_private_lock_directory_close(transition->directory);
...
bool engine_daemon_ipc_send_frame(engine_daemon_ipc_connection_t *connection,
                               engine_daemon_frame_type_t type, uint16_t flags, const void *payload,
                               uint32_t length) {
    if (!connection || atomic_load_explicit(&connection->poisoned, memory_order_acquire) ||
        (length > 0 && !payload)) {
        return false;
    }
    uint8_t header[ENGINE_DAEMON_FRAME_HEADER_SIZE];
    if (!engine_daemon_frame_header_encode(header, type, flags, length)) {
        return false;
    }
    uint64_t deadline_ms = ipc_deadline_after(ENGINE_DAEMON_IPC_SEND_TIMEOUT_MS);
...
int engine_daemon_ipc_receive_frame_bounded(engine_daemon_ipc_connection_t *connection,
    ...
    if (!engine_daemon_frame_header_decode(header, &frame)) {
        atomic_store_explicit(&connection->poisoned, true, memory_order_release);
        return -1;
    }
...
int engine_daemon_ipc_receive_frame(engine_daemon_ipc_connection_t *connection, uint32_t timeout_ms,
                                 engine_daemon_frame_t *frame_out, uint8_t **payload_out) {
    return engine_daemon_ipc_receive_frame_bounded(connection, timeout_ms, ENGINE_DAEMON_MAX_FRAME_SIZE,
                                                frame_out, payload_out);
}
