/*
 * application.c — Daemon-owned engine application sessions and thin-client wire.
 */
...
enum {
    ...
    APPLICATION_UI_READINESS_REQUEST_SIZE = 1 + ENGINE_SHA256_DIGEST_LEN,
...
typedef struct engine_daemon_application_watch engine_daemon_application_watch_t;
typedef struct engine_daemon_application_session engine_daemon_application_session_t;
typedef struct engine_daemon_application_job engine_daemon_application_job_t;
typedef struct engine_daemon_application_mutation engine_daemon_application_mutation_t;
typedef struct engine_daemon_application_watch_job_subscription
    engine_daemon_application_watch_job_subscription_t;
...
struct engine_daemon_application_watch {
    char *project;
    char *root;
    size_t subscribers;
    engine_daemon_application_watch_t *next;
};

struct engine_daemon_application_session {
    engine_daemon_application_t *application;
    engine_mcp_server_t *mcp;
    ...
    bool request_active;
    engine_daemon_runtime_application_token_t active_request_token;
    engine_daemon_runtime_application_token_t request_cancel_token;
    engine_daemon_application_watch_t *watch;
    engine_daemon_application_job_t *active_job;
    bool active_job_subscribed;
    engine_daemon_application_job_t *auto_index_job;
    ...
    engine_daemon_application_session_t *next;
};

struct engine_daemon_application_job {
    engine_daemon_application_t *application;
    char *project_key;
    char *root_path;
    char *args_json;
    char *response;
    engine_daemon_application_worker_t worker;
    ...
    engine_daemon_application_job_t *next;
};
...
struct engine_daemon_application_watch_job_subscription {
    engine_daemon_application_session_t *session;
    engine_daemon_application_job_t *job;
    engine_daemon_application_watch_job_subscription_t *next;
};

struct engine_daemon_application_mutation {
    char *project_key;
    engine_project_lock_lease_t *project_lock_lease;
    bool releasing;
    engine_daemon_application_mutation_t *next;
};

struct engine_daemon_application {
    engine_mutex_t mutex;
    struct engine_watcher *watcher;
    struct engine_config *config;
    engine_daemon_application_session_t *sessions;
    engine_daemon_application_watch_t *watches;
    engine_daemon_application_job_t *jobs;
    engine_daemon_application_watch_job_subscription_t *watch_job_subscriptions;
    engine_daemon_application_mutation_t *mutations;
    engine_daemon_application_worker_ops_t worker_ops;
    engine_daemon_application_update_ops_t update_ops;
    ...
    engine_daemon_application_update_worker_t update_worker;
    engine_thread_t update_thread;
    ...
    bool stopping;
    /* See engine_daemon_application_set_permanent. */
    bool permanent;
    uint8_t ui_readiness_secret[ENGINE_SHA256_DIGEST_LEN];
    bool ui_readiness_secret_set;
};

static void application_job_unsubscribe_locked(engine_daemon_application_job_t *job);
...
static bool application_watch_job_subscribe_late_session_locked(
    engine_daemon_application_session_t *session, engine_daemon_application_watch_t *watch);
static bool application_unique_recovery_file(char out[APPLICATION_PATH_CAP], const char *kind);
static bool application_update_reap(engine_daemon_application_t *application, bool wait,
                                    uint32_t timeout_ms);
static void *application_job_thread(void *opaque);
static char *application_auto_index_args(const char *root_path);
static engine_daemon_application_job_t *application_job_subscribe_locked(
    engine_daemon_application_t *application, const char *project_key, const char *root_path,
...
void engine_daemon_application_fail_next_job_thread_start_for_test(void) {
...
int engine_daemon_application_background_initializes_for_test(void) {
...
int engine_daemon_application_busy_queue_waits_for_test(void) {
    return atomic_load_explicit(&g_application_busy_queue_waits_for_test, memory_order_acquire);
}

void engine_daemon_application_hold_job_before_start_for_test(bool hold) {
    atomic_store_explicit(&g_application_hold_job_before_start_for_test, hold,
                          memory_order_release);
}

static int application_job_thread_create(engine_thread_t *thread, void *context) {
    if (atomic_exchange_explicit(&g_application_fail_next_job_thread_start_for_test, false,
                                 memory_order_acq_rel)) {
        return -1;
    }
    return engine_thread_create(thread, APPLICATION_JOB_THREAD_STACK, application_job_thread, context);
}

static bool application_request_cancelled_locked(const engine_daemon_application_session_t *session) {
    return session &&
           (session->session_cancelled ||
            (session->request_active &&
             session->active_request_token != ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
...
static void application_project_lock_release_fully(engine_project_lock_lease_t **lease) {
    uint64_t deadline = application_deadline_after(APPLICATION_COORDINATION_CLEANUP_MS);
    while (lease && *lease) {
        (void)engine_project_lock_lease_release(lease);
        if (!*lease) {
            return;
        }
...
static int application_worker_start_default(void *context, const char *args_json,
    ...
    (void)context;
    engine_index_worker_handle_t *worker = NULL;
    int result = engine_index_worker_start(args_json, memory_budget_bytes, false, marker_file,
                                        quarantine_file, &worker);
    *worker_out = worker;
    return result;
}

static engine_index_worker_poll_t application_worker_poll_default(
    void *context, engine_daemon_application_worker_t worker,
    const engine_index_worker_result_t **result_out) {
    (void)context;
    return engine_index_worker_poll((engine_index_worker_handle_t *)worker, result_out);
}

static bool application_worker_cancel_default(void *context,
                                              engine_daemon_application_worker_t worker) {
    (void)context;
    return engine_index_worker_request_cancel((engine_index_worker_handle_t *)worker);
}

static const char *application_worker_log_path_default(void *context,
                                                       engine_daemon_application_worker_t worker) {
    (void)context;
    return engine_index_worker_log_path((engine_index_worker_handle_t *)worker);
}

static void application_worker_destroy_default(void *context,
                                               engine_daemon_application_worker_t worker) {
    (void)context;
    engine_index_worker_destroy((engine_index_worker_handle_t *)worker);
}
...
static engine_daemon_application_watch_t *application_find_watch_locked(
    engine_daemon_application_t *application, const char *project) {
    for (engine_daemon_application_watch_t *watch = application->watches; watch; watch = watch->next) {
...
static void application_remove_watch_entry_locked(engine_daemon_application_t *application,
                                                  engine_daemon_application_watch_t *watch,
                                                  bool unregister_physical_watch) {
    engine_daemon_application_watch_t **cursor = &application->watches;
    ...
    *cursor = watch->next;
    for (engine_daemon_application_session_t *session = application->sessions; session;
    ...
    if (unregister_physical_watch && application->watcher) {
        engine_watcher_unwatch(application->watcher, watch->project);
    }
    free(watch->project);
    free(watch->root);
    free(watch);
...
static void application_remove_watch_locked(engine_daemon_application_t *application,
                                            engine_daemon_application_watch_t *watch) {
    application_remove_watch_entry_locked(application, watch, true);
}

static void application_release_session_watch_locked(engine_daemon_application_session_t *session) {
    engine_daemon_application_watch_t *watch = session->watch;
    if (!watch) {
        return;
    }
    application_watch_job_unsubscribe_session_locked(session);
...
static void application_refresh_watch_locked(engine_daemon_application_session_t *session) {
    engine_daemon_application_t *application = session->application;
    if (!application->watcher || !session->context_set ||
        session->tool_profile != ENGINE_MCP_TOOL_PROFILE_ALL || session->hook_event ||
        session->hook_dialect || session->auto_index_subscribed) {
        return;
    }
    const char *project = engine_mcp_server_session_project(session->mcp);
    const char *root = engine_mcp_server_session_root(session->mcp);
    if (!project || !project[0] || !root || !root[0]) {
        return;
    }
    bool enabled = !application->config ||
                   engine_config_get_bool(application->config, ENGINE_CONFIG_AUTO_WATCH, true);
    ...
    engine_daemon_application_watch_t *watch = application_find_watch_locked(application, project);
    ...
    if (watch) {
        if (strcmp(watch->root, root) == 0) {
            if (!application_watch_job_subscribe_late_session_locked(session, watch)) {
                engine_log_warn("daemon.watch.late_owner_allocation_failed", "project", project,
                             "action", "retry");
                return;
            }
            watch->subscribers++;
            ...
            engine_log_warn("daemon.watch.project_collision", "project", project, "existing_root",
    ...
    if (!engine_watcher_watch(application->watcher, project, root)) {
        engine_log_warn("daemon.watch.registration_failed", "project", project, "action", "retry");
        free(watch->project);
        free(watch->root);
        free(watch);
        return;
...
static void application_refresh_watch(engine_daemon_application_session_t *session) {
    if (!session || !session->application) {
        return;
    }
    engine_mutex_lock(&session->application->mutex);
    application_refresh_watch_locked(session);
    engine_mutex_unlock(&session->application->mutex);
}

static void application_job_free(engine_daemon_application_job_t *job) {
    if (!job) {
        return;
    }
...
static void application_jobs_reap_completed(engine_daemon_application_t *application) {
    for (;;) {
        engine_daemon_application_job_t *reap = NULL;
        engine_mutex_lock(&application->mutex);
        engine_daemon_application_job_t **cursor = &application->jobs;
        while (*cursor) {
        ...
        engine_mutex_unlock(&application->mutex);
        if (!reap) {
            return;
        }
        if (reap->thread_started) {
            (void)engine_thread_join(&reap->thread);
        }
...
typedef struct {
    engine_index_worker_result_t result;
    bool has_result;
...
static bool application_cache_dir(char out[APPLICATION_PATH_CAP]) {
    ...
    if (engine_safe_getenv("ENGINE_CACHE_DIR", configured, sizeof(configured), NULL) && configured[0]) {
    ...
    if (!engine_safe_getenv("HOME", home, sizeof(home), NULL) || !home[0]) {
        (void)engine_safe_getenv("USERPROFILE", home, sizeof(home), NULL);
    }
    if (!home[0]) {
        return false;
    }
    int written = snprintf(out, APPLICATION_PATH_CAP, "%s/.cache/graph-engine", home);
    if (written <= 0 || written >= APPLICATION_PATH_CAP) {
        return false;
    }
    engine_normalize_path_sep(out);
...
static bool application_recovery_files_create(char marker_path[APPLICATION_PATH_CAP],
    ...
    if (!application_unique_recovery_file(quarantine_path, "quarantine")) {
        (void)engine_unlink(marker_path);
        marker_path[0] = '\0';
        return false;
    }
    return true;
...
static void application_recovery_files_remove(const char *marker_path,
    ...
    if (quarantine_path && quarantine_path[0]) {
        (void)engine_unlink(quarantine_path);
    }
}

static bool application_truncate_file(const char *path) {
    FILE *file = engine_fopen(path, "wb");
    return file && fclose(file) == 0;
}

static bool application_job_cancel_requested(engine_daemon_application_job_t *job);

static char **application_read_suspects(engine_daemon_application_job_t *job, const char *path,
                                        int *count_out, bool *cancelled_out) {
    *count_out = 0;
    *cancelled_out = false;
    int64_t marker_size = engine_file_size(path);
    if (marker_size < 0 || marker_size > APPLICATION_MARKER_MAX_BYTES) {
        return NULL;
    }
    FILE *file = engine_fopen(path, "rb");
    if (!file) {
        return NULL;
    }
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (line[0] == 'S') {
            ...
            char *copy = engine_strdup(relative);
            if (!copy) {
                allocation_failed = true;
                break;
            }
...
static bool application_append_quarantine(const char *path, const char *relative,
    ...
    FILE *file = engine_fopen(path, "ab");
    if (!file) {
        return false;
    }
...
static int application_max_restarts(void) {
    char value[32] = {0};
    if (!engine_safe_getenv("ENGINE_INDEX_MAX_RESTARTS", value, sizeof(value), NULL) || !value[0]) {
...
static void application_attempt_init(application_attempt_t *attempt) {
    ...
    attempt->result.outcome = ENGINE_PROC_SPAWN_FAILED;
...
static bool application_job_cancel_requested(engine_daemon_application_job_t *job) {
    engine_daemon_application_t *application = job->application;
    engine_mutex_lock(&application->mutex);
...
static bool application_mutation_conflicts_locked(engine_daemon_application_t *application,
                                                  const char *project_key) {
    for (engine_daemon_application_mutation_t *mutation = application->mutations; mutation;
...
static bool application_job_reserves_project_locked(engine_daemon_application_t *application,
                                                    const char *project_key) {
    for (engine_daemon_application_job_t *job = application->jobs; job; job = job->next) {
...
static bool application_mutation_begin_internal(engine_daemon_application_t *application,
                                                engine_daemon_application_session_t *session,
                                                const char *project_key, bool wait) {
    if (!application || !project_key || !project_key[0]) {
        return false;
    }
    engine_daemon_application_mutation_t *reserved = NULL;
    for (;;) {
        engine_mutex_lock(&application->mutex);
        bool cancelled =
        ...
        if (!cancelled && !busy) {
            engine_daemon_application_mutation_t *mutation = calloc(1, sizeof(*mutation));
            if (mutation) {
                mutation->project_key = engine_strdup(project_key);
            }
        ...
        engine_usleep(APPLICATION_JOB_POLL_US);
    }
    ...
    for (;;) {
        ...
        engine_project_lock_lease_t *lease = NULL;
        engine_private_file_lock_status_t status =
            wait ? engine_project_lock_acquire(application->project_locks, project_key, deadline, NULL,
                                            &lease)
                 : engine_project_lock_try_acquire(application->project_locks, project_key, &lease);
        if (status == ENGINE_PRIVATE_FILE_LOCK_OK && lease) {
            engine_mutex_lock(&application->mutex);
            bool cancelled =
                application->stopping || (session && application_request_cancelled_locked(session));
            engine_mutex_unlock(&application->mutex);
            if (cancelled) {
                ...
                status = ENGINE_PRIVATE_FILE_LOCK_BUSY;
            } else {
        ...
        bool cancelled =
            application->stopping || (session && application_request_cancelled_locked(session));
        if (status != ENGINE_PRIVATE_FILE_LOCK_BUSY || cancelled || !wait) {
            engine_daemon_application_mutation_t **cursor = &application->mutations;
            ...
            free(reserved);
            if (status != ENGINE_PRIVATE_FILE_LOCK_BUSY) {
                engine_log_error("daemon.project_lock_failed", "project", project_key, "action",
                              "refuse_mutation");
            }
            return false;
        }
...
void engine_daemon_application_project_mutation_end(engine_daemon_application_t *application,
    ...
    engine_mutex_lock(&application->mutex);
    engine_daemon_application_mutation_t *mutation = application->mutations;
...
static bool application_watcher_mutation_begin(void *context, const char *project) {
    return engine_daemon_application_project_mutation_try_begin(context, project);
}

static void application_watcher_mutation_end(void *context, const char *project) {
    engine_daemon_application_project_mutation_end(context, project);
}

static void application_watcher_project_pruned(void *context, const char *project) {
    engine_daemon_application_t *application = context;
...
static bool application_session_mutation_begin(void *context, const char *project) {
    engine_daemon_application_session_t *session = context;
    return session &&
...
static void application_session_mutation_end(void *context, const char *project) {
    engine_daemon_application_session_t *session = context;
    if (session) {
        engine_daemon_application_project_mutation_end(session->application, project);
    }
}

static bool application_job_wait_for_mutations(engine_daemon_application_job_t *job) {
    engine_daemon_application_t *application = job->application;
    for (;;) {
        engine_mutex_lock(&application->mutex);
        bool cancelled = job->cancel_requested || application->stopping;
...
static void application_cancelled_result(engine_index_worker_result_t *result) {
    memset(result, 0, sizeof(*result));
    result->outcome = ENGINE_PROC_KILLED;
    result->exit_code = -1;
    result->cancellation_requested = true;
    result->tree_quiesced = true;
}

static application_attempt_status_t application_job_run_attempt(engine_daemon_application_job_t *job,
    ...
    engine_daemon_application_worker_t worker = NULL;
    application_tmp_lock();
    ...
    const engine_index_worker_result_t *borrowed = NULL;
    for (;;) {
        engine_index_worker_poll_t state =
            application->worker_ops.poll(application->worker_ops.context, worker, &borrowed);
        if (state == ENGINE_INDEX_WORKER_POLL_TERMINAL) {
            break;
        }
        engine_mutex_lock(&application->mutex);
        bool cancel_pending =
        ...
        engine_mutex_unlock(&application->mutex);
        if (cancel_pending || state == ENGINE_INDEX_WORKER_POLL_ERROR) {
            (void)application->worker_ops.cancel(application->worker_ops.context, worker);
        }
        engine_usleep(APPLICATION_JOB_POLL_US);
    }
    ...
    if (borrowed) {
        ...
        attempt->result.response = borrowed->response ? engine_strdup(borrowed->response) : NULL;
...
static char *application_job_failure_response(const engine_index_worker_result_t *result,
                                              const char *log_path) {
    char message[1024];
    if (result && result->cancellation_requested) {
        ...
        (void)snprintf(message, sizeof(message),
                       ...
                       engine_proc_outcome_str(result->outcome), log_path ? log_path : "unavailable");
    } else if (result) {
        (void)snprintf(message, sizeof(message),
                       "index worker ended with %s (exit=%d, signal=%d); inspect log: %s",
                       engine_proc_outcome_str(result->outcome), result->exit_code,
...
static engine_mcp_supervised_result_disposition_t application_attempt_disposition(
    engine_daemon_application_job_t *job, application_attempt_t *attempt) {
    if (application_job_cancel_requested(job)) {
        if (!attempt->has_result) {
            application_cancelled_result(&attempt->result);
            attempt->has_result = true;
    ...
    return engine_mcp_supervised_result_disposition(0, attempt->has_result ? &attempt->result : NULL);
}

static void application_record_attempt(const application_attempt_t *attempt,
                                       engine_index_worker_result_t *last_result,
...
static void application_record_cancelled(engine_index_worker_result_t *last_result,
                                         bool *have_last_result,
                                         char last_log[APPLICATION_PATH_CAP]) {
...
static bool application_result_is_attributable_failure(
    const application_attempt_t *attempt, engine_mcp_supervised_result_disposition_t disposition) {
    return disposition == ENGINE_MCP_SUPERVISED_RESULT_CONTAINED_FAILURE && attempt->has_result &&
           (attempt->result.outcome == ENGINE_PROC_CRASH || attempt->result.outcome == ENGINE_PROC_HANG);
}
...
static application_attempt_decision_t application_consume_attempt(
    engine_daemon_application_job_t *job, application_attempt_t *attempt,
    application_job_execution_t *execution, engine_proc_outcome_t *failure_outcome) {
...
static bool application_recovery_record_suspects(
    engine_daemon_application_job_t *job, const char *marker_path, const char *quarantine_path,
    engine_proc_outcome_t outcome, int recovery_index, char ***previous_suspects, int *previous_count,
    ...
    if (!suspects || suspect_count == 0) {
        application_free_suspects(suspects, suspect_count);
        engine_log_warn("daemon.index.recovery_unattributable", "action", "stop");
        return false;
    }
    if (*previous_suspects) {
        ...
        const char *phase = outcome == ENGINE_PROC_HANG ? "hang" : "crash";
        if (!application_append_quarantine(quarantine_path, pick, phase)) {
            engine_log_warn("daemon.index.quarantine_write_fail", "path", pick);
            application_free_suspects(suspects, suspect_count);
            return false;
        }
        (*quarantined)++;
        ...
        engine_log_warn("daemon.index.file_quarantined", "path", pick, "outcome", phase, "attempt",
                     attempt_text);
    }
...
static void application_job_try_partial(engine_daemon_application_job_t *job,
    ...
    engine_proc_outcome_t failure_outcome = ENGINE_PROC_SPAWN_FAILED;
    ...
    if (decision == APPLICATION_ATTEMPT_DECISION_SUCCESS) {
        ...
        engine_log_warn("daemon.index.recovery_partial", "quarantined", quarantined_text);
    }
}

static void application_job_recover(engine_daemon_application_job_t *job,
                                    application_job_execution_t *execution) {
    if (application_job_cancel_requested(job)) {
        application_job_execution_cancel(execution);
        return;
...
static void application_auto_index_retry_pending_locked(engine_daemon_application_t *application) {
    ...
    for (engine_daemon_application_session_t *session = application->sessions; session;
        ...
        const char *project = engine_mcp_server_session_project(session->mcp);
        const char *root_path = engine_mcp_server_session_root(session->mcp);
        ...
        engine_daemon_application_job_t *retry = application_job_subscribe_locked(
        ...
        if (retry) {
            ...
            session->auto_index_retry_pending = false;
            engine_log_info("daemon.autoindex.admission_retried", "project", project);
            continue;
        }
...
static void application_job_publish(engine_daemon_application_job_t *job,
                                    application_job_execution_t *execution) {
...
static void *application_job_thread(void *opaque) {
    engine_daemon_application_job_t *job = opaque;
    application_job_execution_t execution;
...
static engine_daemon_application_job_t *application_find_job_locked(
    engine_daemon_application_t *application, const char *project_key) {
    for (engine_daemon_application_job_t *job = application->jobs; job; job = job->next) {
...
static engine_daemon_application_job_t *application_find_active_job_locked(
    engine_daemon_application_t *application, const char *project_key) {
    for (engine_daemon_application_job_t *job = application->jobs; job; job = job->next) {
...
static char *application_index_project_key(const char *root_path, const char *args_json) {
    char *override = engine_mcp_get_string_arg(args_json, "name");
    char *key = engine_project_name_from_path(override && override[0] ? override : root_path);
    free(override);
    return key;
}

static size_t application_active_job_count_locked(engine_daemon_application_t *application) {
    size_t count = 0;
    for (engine_daemon_application_job_t *job = application->jobs; job; job = job->next) {
        if (!job->terminal) {
            count++;
...
static engine_daemon_application_job_t *application_job_subscribe_locked(
    ...
    *status_out = APPLICATION_JOB_SUBSCRIBE_UNAVAILABLE;
    ...
    if (application_active_job_count_locked(application) >= application->physical_job_limit) {
        ...
        engine_log_warn("daemon.index.admission_busy", "limit", limit, "project", project_key);
    ...
    if (application_job_thread_create(&job->thread, job) == 0) {
        ...
        application_job_free(job);
        *status_out = APPLICATION_JOB_SUBSCRIBE_UNAVAILABLE;
        engine_log_warn("daemon.index.thread_start_failed", "action", "retry");
        return NULL;
    }
    *status_out = APPLICATION_JOB_SUBSCRIBE_OK;
    return job;
...
static engine_daemon_application_job_t *application_job_subscribe(
    engine_daemon_application_t *application, const char *project_key, const char *root_path,
    const char *args_json, application_job_subscribe_status_t *status_out) {
    application_jobs_reap_completed(application);
    engine_mutex_lock(&application->mutex);
    engine_daemon_application_job_t *job = application_job_subscribe_locked(
...
static engine_daemon_application_job_t *application_auto_index_release_locked(
    engine_daemon_application_session_t *session) {
    if (!session || !session->auto_index_job || !session->auto_index_subscribed) {
        return NULL;
    }
    engine_daemon_application_job_t *job = session->auto_index_job;
    session->auto_index_job = NULL;
    session->auto_index_subscribed = false;
...
static void application_auto_index_cancel_join(engine_daemon_application_t *application,
                                               engine_daemon_application_job_t *job) {
    if (!application || !job) {
        return;
    }
...
static void application_update_cancel_locked(engine_daemon_application_t *application) {
...
static bool application_update_owner_release_locked(engine_daemon_application_session_t *session) {
...
static void application_update_publish_terminal_locked(engine_daemon_application_t *application,
...
static void *application_update_thread(void *opaque) {
    engine_daemon_application_t *application = opaque;
    engine_daemon_application_update_worker_t worker = NULL;
    ...
    for (;;) {
        engine_daemon_application_update_poll_t status =
            application->update_ops.poll(application->update_ops.context, worker, &latest_version);
        if (status != ENGINE_DAEMON_APPLICATION_UPDATE_POLL_RUNNING) {
            if (status == ENGINE_DAEMON_APPLICATION_UPDATE_POLL_ERROR) {
                latest_version = NULL;
            }
            break;
        }
        ...
        engine_mutex_unlock(&application->mutex);
        engine_usleep(APPLICATION_UPDATE_POLL_US);
    }
...
static void application_update_subscribe_locked(engine_daemon_application_session_t *session) {
    engine_daemon_application_t *application = session->application;
    ...
    if (engine_thread_create(&application->update_thread, APPLICATION_JOB_THREAD_STACK,
    ...
    application->update_thread_done = false;
    engine_log_warn("daemon.update.thread_start_failed", "action", "retry");
}
...
static bool application_update_reap(engine_daemon_application_t *application, bool wait,
    ...
    for (;;) {
        ...
        engine_mutex_unlock(&application->mutex);
        if (join) {
            bool joined = engine_thread_join(&application->update_thread) == 0;
            engine_mutex_lock(&application->mutex);
            if (joined) {
        ...
        if (!wait || engine_now_ms() >= deadline) {
            return false;
        }
        engine_usleep(APPLICATION_UPDATE_POLL_US);
    }
...
static void application_background_initialize_impl(engine_daemon_application_session_t *session) {
    ...
    bool auto_index = application->config &&
                      engine_config_get_bool(application->config, ENGINE_CONFIG_AUTO_INDEX, false);
    int auto_index_limit =
        application->config ? engine_config_get_int(application->config, ENGINE_CONFIG_AUTO_INDEX_LIMIT,
    ...
    bool within_auto_index_limit =
        ...
        engine_mcp_auto_index_within_file_limit(root_path, auto_index_limit, &tracked_files);
    if (auto_index_candidate && !within_auto_index_limit) {
        char files[32];
        (void)snprintf(files, sizeof(files), "%d", tracked_files);
        engine_log_warn("daemon.autoindex.skipped", "project", project, "reason",
    ...
    if (attempt_auto_index && args) {
        ...
        if (job) {
            ...
            engine_log_warn("daemon.autoindex.admission_failed", "project", project, "action",
...
static void application_background_initialize(engine_daemon_application_session_t *session) {
...
static void application_update_notice_inject(engine_daemon_application_session_t *session,
                                             char **response_io) {
    engine_daemon_application_t *application = session->application;
    engine_mutex_lock(&application->mutex);
    if (session->background_eligible && !session->update_notice_delivered &&
        ...
        engine_mcp_jsonrpc_response_prepend_notice(response_io, application->update_notice)) {
...
static bool application_watch_job_subscription_exists_locked(
    engine_daemon_application_t *application, engine_daemon_application_session_t *session,
    engine_daemon_application_job_t *job) {
    for (engine_daemon_application_watch_job_subscription_t *subscription =
...
static bool application_watch_job_subscribe_late_session_locked(
    ...
    engine_daemon_application_watch_job_subscription_t *subscription =
...
static bool application_watch_job_subscribe_sessions_locked(engine_daemon_application_t *application,
                                                            engine_daemon_application_watch_t *watch,
                                                            engine_daemon_application_job_t *job,
                                                            size_t *matched_out) {
    *matched_out = 0;
    ...
    engine_daemon_application_watch_job_subscription_t *pending = NULL;
    for (engine_daemon_application_session_t *session = application->sessions; session;
         session = session->next) {
        ...
        if (!subscription) {
            while (pending) {
                engine_daemon_application_watch_job_subscription_t *next = pending->next;
                free(pending);
                pending = next;
            }
            return false;
    ...
    while (pending) {
        engine_daemon_application_watch_job_subscription_t *subscription = pending;
...
static void application_watch_job_unsubscribe_session_locked(
    ...
    engine_daemon_application_watch_job_subscription_t **cursor =
        &session->application->watch_job_subscriptions;
    while (*cursor) {
        engine_daemon_application_watch_job_subscription_t *subscription = *cursor;
...
static void application_watch_job_unsubscribe_job_locked(engine_daemon_application_t *application,
                                                         engine_daemon_application_job_t *job) {
    engine_daemon_application_watch_job_subscription_t **cursor =
        &application->watch_job_subscriptions;
    while (*cursor) {
...
static char *application_job_wait_for_session(engine_daemon_application_session_t *session,
                                              engine_daemon_application_job_t *job) {
    engine_daemon_application_t *application = session->application;
    for (;;) {
        engine_mutex_lock(&application->mutex);
        if (session->active_job != job || !session->active_job_subscribed) {
            engine_mutex_unlock(&application->mutex);
            return engine_mcp_text_result("index operation cancelled for this session", true);
        }
        if (job->terminal) {
            ...
            return response ? response
                            : engine_mcp_text_result("index coordinator lost its result", true);
        }
        engine_mutex_unlock(&application->mutex);
        engine_usleep(APPLICATION_JOB_POLL_US);
    }
...
static char *application_index_execute(void *context, const char *root_path,
    ...
    if (!project_key) {
        return engine_mcp_text_result("failed to derive index project identity", true);
    }
    application_job_subscribe_status_t subscribe_status = APPLICATION_JOB_SUBSCRIBE_UNAVAILABLE;
    engine_daemon_application_job_t *job = NULL;
    for (;;) {
    ...
    if (session->active_job) {
        application_job_unsubscribe_locked(job);
        engine_mutex_unlock(&session->application->mutex);
        return engine_mcp_text_result("this session already has an active index operation", true);
    }
...
static engine_daemon_runtime_application_session_t *application_session_open(
    void *context, engine_daemon_client_id_t client_id, uint64_t authenticated_process_id) {
    engine_daemon_application_t *application = context;
    if (!application || client_id == ENGINE_DAEMON_CLIENT_ID_INVALID ||
        authenticated_process_id == 0) {
        return NULL;
    }
    engine_daemon_application_session_t *session = calloc(1, sizeof(*session));
    if (!session) {
        return NULL;
    }
    session->mcp = engine_mcp_server_new(NULL);
    if (!session->mcp || !engine_mcp_server_release_pristine_memory_store(session->mcp)) {
        engine_mcp_server_free(session->mcp);
        free(session);
        return NULL;
    }
    engine_mcp_server_set_background_tasks(session->mcp, false);
    engine_mcp_server_set_config(session->mcp, application->config);
    engine_mcp_server_set_index_executor(session->mcp, application_index_execute, session);
    engine_mcp_server_set_project_mutation_guard(session->mcp, application_session_mutation_begin,
                                              application_session_mutation_end, session);
    engine_mcp_server_set_project_mutation_try_guard(session->mcp,
                                                  application_session_mutation_try_begin);
    session->tool_profile = ENGINE_MCP_TOOL_PROFILE_ALL;
    ...
    engine_mutex_unlock(&application->mutex);
    return (engine_daemon_runtime_application_session_t *)session;
}

static engine_daemon_runtime_application_status_t application_set_context(
    engine_daemon_application_session_t *session, const uint8_t *request, uint32_t request_length) {
    if (session->context_set || request_length < APPLICATION_CONTEXT_HEADER_SIZE) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    ...
    if (request[5] > 1 || root_length == 0 || expected != request_length ||
        ...
        profile_value > (uint8_t)ENGINE_MCP_TOOL_PROFILE_SCOUT ||
        (profile_value != (uint8_t)ENGINE_MCP_TOOL_PROFILE_ALL &&
         (event_length != 0 || dialect_length != 0))) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    engine_mcp_tool_profile_t tool_profile = (engine_mcp_tool_profile_t)profile_value;
    ...
    if (!root || (allowed_present && !allowed) || (event_length && !hook_event) ||
        ...
        !engine_hook_augment_invocation_supported(hook_event, hook_dialect)) {
        free(root);
        free(allowed);
        free(hook_event);
        free(hook_dialect);
    ...
    bool canonical = engine_canonical_path(root, canonical_root, sizeof(canonical_root));
    if (canonical && allowed_present) {
        canonical = engine_canonical_path(allowed, canonical_allowed, sizeof(canonical_allowed));
    }
    struct stat root_status;
    canonical =
        canonical && stat(canonical_root, &root_status) == 0 && S_ISDIR(root_status.st_mode);
    bool set =
        canonical && engine_mcp_server_set_session_context(session->mcp, canonical_root,
...
static char *application_oversized_response_error(const engine_jsonrpc_request_t *request,
    ...
    (void)snprintf(message, sizeof(message),
                   ...
                   response_length, (unsigned)ENGINE_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX);
    ...
    engine_jsonrpc_response_t response = {
        .id = request && request->has_id ? request->id : 0,
        .id_str = request ? request->id_str : NULL,
        .error_json = error_json,
        .error_code = -32603,
    };
    char *encoded = engine_jsonrpc_format_response(&response);
    free(error_json);
    return encoded;
}
...
static char *application_framable_response(char *response, const engine_jsonrpc_request_t *request) {
    if (!response || strlen(response) <= ENGINE_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
        return response;
    }
...
char *engine_daemon_application_framable_response_for_test(char *response,
                                                        const engine_jsonrpc_request_t *request) {
    return application_framable_response(response, request);
}

static engine_daemon_runtime_application_status_t application_mcp_request(
    engine_daemon_application_session_t *session, const uint8_t *request, uint32_t request_length,
    ...
    engine_jsonrpc_request_t parsed = {0};
    bool parsed_ok = engine_jsonrpc_parse(message, &parsed) == 0;
    ...
    char *response = engine_mcp_server_handle(session->mcp, message);
    free(message);
    ...
    if (response) {
        ...
        if (response_length > UINT32_MAX) {
            if (parsed_ok) {
                engine_jsonrpc_request_free(&parsed);
            }
            free(response);
            return ENGINE_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
        }
...
static engine_daemon_runtime_application_status_t application_tool_request(
    engine_daemon_application_session_t *session, const uint8_t *request, uint32_t request_length,
    uint8_t **response_out, uint32_t *response_length_out) {
    ...
    char *response = engine_mcp_handle_tool(session->mcp, tool, args);
    free(tool);
    free(args);
    if (!response) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
...
static engine_daemon_runtime_application_status_t application_set_ui_config(
    engine_daemon_application_t *application, engine_daemon_application_session_t *session,
    const uint8_t *request, uint32_t request_length) {
    const uint8_t valid_mask =
        ENGINE_DAEMON_APPLICATION_UI_CONFIG_ENABLED | ENGINE_DAEMON_APPLICATION_UI_CONFIG_PORT;
    if (!session || !session->context_set || session->tool_profile != ENGINE_MCP_TOOL_PROFILE_ALL ||
    ...
    bool enabled_present = (update_mask & ENGINE_DAEMON_APPLICATION_UI_CONFIG_ENABLED) != 0;
    bool port_present = (update_mask & ENGINE_DAEMON_APPLICATION_UI_CONFIG_PORT) != 0;
    ...
    if (application->stopping) {
        engine_mutex_unlock(&application->mutex);
        return ENGINE_DAEMON_RUNTIME_APPLICATION_UNAVAILABLE;
    }
    engine_ui_config_t config;
    engine_ui_config_load(&config);
    if (enabled_present) {
    ...
    bool saved = engine_ui_config_save(&config);
    engine_mutex_unlock(&application->mutex);
    return saved ? ENGINE_DAEMON_RUNTIME_APPLICATION_OK : ENGINE_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
}

static engine_daemon_runtime_application_status_t application_ui_readiness_proof(
    engine_daemon_application_t *application, const uint8_t *request, uint32_t request_length,
    uint8_t **response_out, uint32_t *response_length_out) {
    if (!application->ui_readiness_secret_set ||
        request_length != APPLICATION_UI_READINESS_REQUEST_SIZE) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    uint8_t *proof = malloc(ENGINE_SHA256_DIGEST_LEN);
    if (!proof) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
    }
    engine_hmac_sha256(application->ui_readiness_secret, sizeof(application->ui_readiness_secret),
                    request + 1, ENGINE_SHA256_DIGEST_LEN, proof);
    *response_out = proof;
    *response_length_out = ENGINE_SHA256_DIGEST_LEN;
    return ENGINE_DAEMON_RUNTIME_APPLICATION_OK;
}

static engine_daemon_runtime_application_status_t application_request_dispatch(
    engine_daemon_application_t *application, engine_daemon_application_session_t *session,
    const uint8_t *request, uint32_t request_length, uint8_t **response_out,
    uint32_t *response_length_out) {
    switch ((engine_daemon_application_request_kind_t)request[0]) {
    case ENGINE_DAEMON_APPLICATION_REQUEST_SET_CONTEXT:
        return application_set_context(session, request, request_length);
    case ENGINE_DAEMON_APPLICATION_REQUEST_MCP:
        return application_mcp_request(session, request, request_length, response_out,
                                       response_length_out);
    case ENGINE_DAEMON_APPLICATION_REQUEST_TOOL:
        return application_tool_request(session, request, request_length, response_out,
                                        response_length_out);
    case ENGINE_DAEMON_APPLICATION_REQUEST_SET_UI_CONFIG:
        return application_set_ui_config(application, session, request, request_length);
    case ENGINE_DAEMON_APPLICATION_REQUEST_UI_READINESS_PROOF:
        return application_ui_readiness_proof(application, request, request_length, response_out,
                                              response_length_out);
    case ENGINE_DAEMON_APPLICATION_REQUEST_HOOK_AUGMENT: {
        if (!session->context_set || request_length <= 1) {
            return ENGINE_DAEMON_RUNTIME_APPLICATION_REJECTED;
        }
        ...
        char *response = engine_hook_augment_process_for(session->mcp, input, session->hook_event,
                                                      session->hook_dialect);
        free(input);
        if (response) {
            size_t response_length = strlen(response);
...
static engine_daemon_runtime_application_status_t application_request(
    void *context, engine_daemon_runtime_application_session_t *opaque_session,
    engine_daemon_runtime_application_token_t request_token, const uint8_t *request,
    uint32_t request_length, uint8_t **response_out, uint32_t *response_length_out) {
    engine_daemon_application_t *application = context;
    engine_daemon_application_session_t *session = (engine_daemon_application_session_t *)opaque_session;
    if (response_out) {
        *response_out = NULL;
    }
    ...
    if (!application || !session || session->application != application ||
        request_token == ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID || !request ||
    ...
    if (session->request_cancel_token != request_token) {
        session->request_cancel_token = ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    session->request_active = true;
    session->active_request_token = request_token;
    bool mcp_scope_started = engine_mcp_server_request_scope_begin(session->mcp);
    ...
    engine_daemon_runtime_application_status_t status =
        cancelled_before_entry
            ? ENGINE_DAEMON_RUNTIME_APPLICATION_CANCELLED
    ...
    engine_mcp_server_request_scope_end(session->mcp);
    session->request_active = false;
    session->active_request_token = ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    if (session->request_cancel_token == request_token) {
        session->request_cancel_token = ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    ...
    bool activate_background =
        !cancelled && status == ENGINE_DAEMON_RUNTIME_APPLICATION_OK &&
    ...
    if (!cancelled && status == ENGINE_DAEMON_RUNTIME_APPLICATION_OK &&
    ...
    if (cancelled) {
        ...
        *response_out = NULL;
        *response_length_out = 0;
        return ENGINE_DAEMON_RUNTIME_APPLICATION_CANCELLED;
    }
...
static void application_cancel_jobs_locked(engine_daemon_application_t *application) {
    for (engine_daemon_application_job_t *job = application->jobs; job; job = job->next) {
        if (!job->terminal) {
            job->cancel_requested = true;
        }
...
static void application_request_cancel(void *context,
                                       engine_daemon_runtime_application_session_t *opaque_session,
    ...
    if (!session->session_cancelled &&
        ...
        if (active_match && session->active_job && session->active_job_subscribed) {
            engine_daemon_application_job_t *job = session->active_job;
            session->active_job = NULL;
            session->active_job_subscribed = false;
            application_job_unsubscribe_locked(job);
        }
        if (active_match) {
            ...
            (void)engine_mcp_server_cancel_active(session->mcp);
        }
    }
    engine_mutex_unlock(&application->mutex);
}
...
static void application_session_cancel(void *context,
    ...
    if (application && session && session->application == application) {
        ...
        engine_daemon_application_job_t *join_auto_index = NULL;
        engine_mutex_lock(&application->mutex);
        ...
        for (engine_daemon_application_session_t *other = application->sessions;
...
static void application_session_close(void *context,
    ...
    engine_daemon_application_job_t *join_auto_index = NULL;
    engine_mutex_lock(&application->mutex);
    engine_daemon_application_session_t **cursor = &application->sessions;
...
engine_daemon_application_t *engine_daemon_application_new(
    const engine_daemon_application_config_t *config) {
    engine_daemon_application_t *application = calloc(1, sizeof(*application));
    if (!application) {
        return NULL;
    }
    engine_mutex_init(&application->mutex);
    application->physical_job_limit = APPLICATION_DEFAULT_PHYSICAL_JOB_LIMIT;
    size_t aggregate_memory_budget_bytes = engine_mem_budget();
    if (config) {
        if ((config->ui_readiness_secret != NULL || config->ui_readiness_secret_length != 0) &&
            ...
            engine_mutex_destroy(&application->mutex);
            engine_secure_zero(application->ui_readiness_secret,
    ...
    if (!application->worker_ops.start) {
        application->worker_ops = (engine_daemon_application_worker_ops_t){
            .context = NULL,
    ...
    if (!application->worker_ops.poll || !application->worker_ops.cancel ||
        ...
        engine_mutex_destroy(&application->mutex);
        engine_secure_zero(application->ui_readiness_secret, sizeof(application->ui_readiness_secret));
        free(application);
        return NULL;
    }
    ...
    if (application->watcher) {
        engine_watcher_set_project_mutation_guard(
...
bool engine_daemon_application_shutdown(engine_daemon_application_t *application, uint32_t timeout_ms) {
    ...
    for (;;) {
        ...
        for (engine_daemon_application_session_t *session = application->sessions; all_done && session;
             session = session->next) {
            if (session->request_active) {
                all_done = false;
            }
...
bool engine_daemon_application_free_with_timeout(engine_daemon_application_t *application,
    ...
    if (!engine_daemon_application_shutdown(application, timeout_ms)) {
         ...
         * resolved. */
        engine_log_error("daemon.application.free_busy", "action", "retain");
        return false;
    }
    if (application->watcher) {
        ...
        engine_watcher_set_project_mutation_guard(application->watcher, NULL, NULL, NULL, NULL);
    }
    engine_mutex_lock(&application->mutex);
    engine_daemon_application_session_t *sessions = application->sessions;
    application->sessions = NULL;
    engine_daemon_application_watch_t *watches = application->watches;
    application->watches = NULL;
    engine_daemon_application_job_t *jobs = application->jobs;
    application->jobs = NULL;
    engine_daemon_application_watch_job_subscription_t *watch_job_subscriptions =
        application->watch_job_subscriptions;
    application->watch_job_subscriptions = NULL;
    engine_daemon_application_mutation_t *mutations = application->mutations;
    application->mutations = NULL;
    engine_mutex_unlock(&application->mutex);
    while (sessions) {
        engine_daemon_application_session_t *next = sessions->next;
        engine_mcp_server_free(sessions->mcp);
        free(sessions);
        sessions = next;
    }
    while (watches) {
        engine_daemon_application_watch_t *next = watches->next;
        if (application->watcher) {
            engine_watcher_unwatch(application->watcher, watches->project);
        }
    ...
    while (watch_job_subscriptions) {
        engine_daemon_application_watch_job_subscription_t *next = watch_job_subscriptions->next;
        free(watch_job_subscriptions);
        watch_job_subscriptions = next;
    }
    while (jobs) {
        engine_daemon_application_job_t *next = jobs->next;
        if (jobs->thread_started) {
            (void)engine_thread_join(&jobs->thread);
        }
        application_job_free(jobs);
        jobs = next;
    }
    while (mutations) {
        engine_daemon_application_mutation_t *next = mutations->next;
...
bool engine_daemon_application_free(engine_daemon_application_t *application) {
    return engine_daemon_application_free_with_timeout(application, 3000);
}

engine_daemon_runtime_application_callbacks_t engine_daemon_application_runtime_callbacks(
    engine_daemon_application_t *application) {
    engine_daemon_runtime_application_callbacks_t callbacks = {
        .context = application,
...
static engine_daemon_runtime_application_status_t application_client_exchange_tagged(
    ...
    engine_daemon_runtime_application_status_t status =
        request_token == ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID
            ? engine_daemon_runtime_client_application_request(client, request, request_length,
                                                            &response, &response_length, timeout_ms)
            : engine_daemon_runtime_client_application_request_tagged(client, request_token, request,
                                                                   request_length, &response,
                                                                   &response_length, timeout_ms);
    free(request);
    if (status != ENGINE_DAEMON_RUNTIME_APPLICATION_OK) {
        free(response);
        return status;
    }
    if (response) {
...
static engine_daemon_runtime_application_status_t application_client_exchange(
    engine_daemon_runtime_client_t *client, uint8_t *request, uint32_t request_length,
    uint8_t **response_out, uint32_t *response_length_out, uint32_t timeout_ms) {
    return application_client_exchange_tagged(client, ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID,
...
engine_daemon_runtime_application_status_t engine_daemon_application_client_set_context(
    ...
    uint64_t total = (uint64_t)APPLICATION_CONTEXT_HEADER_SIZE + root_length + allowed_length +
                     event_length + dialect_length;
    if (tool_profile < ENGINE_MCP_TOOL_PROFILE_ALL || tool_profile > ENGINE_MCP_TOOL_PROFILE_SCOUT ||
        (tool_profile != ENGINE_MCP_TOOL_PROFILE_ALL && (event_length != 0 || dialect_length != 0)) ||
        !engine_hook_augment_invocation_supported(hook_event, hook_dialect) ||
        root_length > UINT32_MAX || allowed_length > UINT32_MAX || event_length > UINT32_MAX ||
        dialect_length > UINT32_MAX || total > ENGINE_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    ...
    request[0] = ENGINE_DAEMON_APPLICATION_REQUEST_SET_CONTEXT;
    ...
    engine_daemon_runtime_application_status_t status = application_client_exchange(
        client, request, (uint32_t)total, &unexpected, &unexpected_length, timeout_ms);
    if (status == ENGINE_DAEMON_RUNTIME_APPLICATION_OK && (unexpected || unexpected_length != 0)) {
...
engine_daemon_runtime_application_status_t engine_daemon_application_client_set_ui_config(
    ...
    request[0] = ENGINE_DAEMON_APPLICATION_REQUEST_SET_UI_CONFIG;
...
engine_daemon_runtime_application_status_t engine_daemon_application_client_ui_readiness_proof(
    engine_daemon_runtime_client_t *client, const uint8_t challenge[ENGINE_SHA256_DIGEST_LEN],
    uint8_t proof_out[ENGINE_SHA256_DIGEST_LEN], uint32_t timeout_ms) {
    if (!client || !challenge || !proof_out) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    engine_secure_zero(proof_out, ENGINE_SHA256_DIGEST_LEN);
    uint8_t *request = malloc(APPLICATION_UI_READINESS_REQUEST_SIZE);
    if (!request) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    }
    request[0] = ENGINE_DAEMON_APPLICATION_REQUEST_UI_READINESS_PROOF;
    memcpy(request + 1, challenge, ENGINE_SHA256_DIGEST_LEN);
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    engine_daemon_runtime_application_status_t status =
        application_client_exchange(client, request, APPLICATION_UI_READINESS_REQUEST_SIZE,
                                    &response, &response_length, timeout_ms);
    if (status == ENGINE_DAEMON_RUNTIME_APPLICATION_OK) {
        if (!response || response_length != ENGINE_SHA256_DIGEST_LEN) {
            status = ENGINE_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
        } else {
            memcpy(proof_out, response, ENGINE_SHA256_DIGEST_LEN);
        }
    }
    if (response) {
        engine_secure_zero(response, response_length);
    }
    free(response);
    return status;
}

static engine_daemon_runtime_application_status_t application_client_text_request_tagged(
    engine_daemon_runtime_client_t *client, engine_daemon_runtime_application_token_t request_token,
    engine_daemon_application_request_kind_t kind, const char *text, uint8_t **response_out,
    uint32_t *response_length_out, uint32_t timeout_ms) {
    if (!client || !text || !text[0]) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    size_t text_length = strlen(text);
    if (text_length + 1U > ENGINE_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
...
static engine_daemon_runtime_application_status_t application_client_text_request(
    engine_daemon_runtime_client_t *client, engine_daemon_application_request_kind_t kind,
    ...
    return application_client_text_request_tagged(
        client, ENGINE_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID, kind, text, response_out,
        response_length_out, timeout_ms);
}
...
engine_daemon_runtime_application_status_t engine_daemon_application_client_mcp(
    ...
    uint32_t *response_length_out, uint32_t timeout_ms) {
    return application_client_text_request(client, ENGINE_DAEMON_APPLICATION_REQUEST_MCP, message,
...
engine_daemon_runtime_application_status_t engine_daemon_application_client_mcp_tagged(
    ...
    return application_client_text_request_tagged(client, request_token,
                                                  ENGINE_DAEMON_APPLICATION_REQUEST_MCP, message,
                                                  response_out, response_length_out, timeout_ms);
}
...
engine_daemon_runtime_application_status_t engine_daemon_application_client_tool(
    ...
    if (tool_length > UINT32_MAX || total > ENGINE_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
        return ENGINE_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    ...
    request[0] = ENGINE_DAEMON_APPLICATION_REQUEST_TOOL;
...
engine_daemon_runtime_application_status_t engine_daemon_application_client_hook_augment(
    ...
    uint32_t *response_length_out, uint32_t timeout_ms) {
    return application_client_text_request(client, ENGINE_DAEMON_APPLICATION_REQUEST_HOOK_AUGMENT,
                                           input_json, response_out, response_length_out,
                                           timeout_ms);
}

static int application_background_index(engine_daemon_application_t *application,
    ...
    struct stat root_status;
    if (!engine_canonical_path(root_path, canonical_root, sizeof(canonical_root)) ||
    ...
    char *default_project = engine_project_name_from_path(canonical_root);
    bool custom_project =
...
size_t engine_daemon_application_job_subscribers(engine_daemon_application_t *application,
    ...
    engine_mutex_lock(&application->mutex);
    engine_daemon_application_job_t *job = application_find_job_locked(application, project_key);
...
bool engine_daemon_application_session_retains_store_for_test(
    const engine_daemon_runtime_application_session_t *opaque_session) {
    const engine_daemon_application_session_t *session =
        (const engine_daemon_application_session_t *)opaque_session;
    return session && session->mcp && engine_mcp_server_store(session->mcp) != NULL;
}
