 ...
 *   cli <tool> <json>   单次工具调用并打印结果
 *   --version / --help  版本与用法
 *   --ui=true/false     启停 HTTP 服务（layout/index/rpc；配置可持久化）
 *   --port=N            HTTP 端口（默认 9750，可持久化）
 *   --tool-profile=...  限制对外工具面
 *
 * 长驻 MCP/hook 前端作为账号级 daemon 的瘦客户端；一次性 CLI 使用隔离本地服务。
 */
#ifdef _WIN32
...
#endif
#include "engine.h"
#include "store/store.h" // engine_alloc_init — bind 3rd-party allocators to mimalloc before any sqlite/git init
...
enum {
    ...
    PARENT_WATCHDOG_STACK_SIZE = 64 * ENGINE_SZ_1K, /* watchdog only polls — tiny stack suffices */
};
...
#include "foundation/win_utf8.h" /* engine_wide_to_utf8 — Windows UTF-8 argv (#423/#20); no-op on POSIX */
#ifdef _WIN32
...
static engine_daemon_runtime_client_t *g_daemon_client = NULL;
...
struct main_local_cli_lease {
    char *project;
    engine_project_lock_lease_t *lease;
    main_local_cli_lease_t *next;
};

typedef struct {
    engine_project_lock_manager_t *manager;
...
static bool main_local_command_cancel(void *opaque) {
    ...
    engine_mutex_lock(&context->mutex);
    bool cancelled = context->server && engine_mcp_server_cancel_active(context->server);
...
static void main_local_maintenance_finish(engine_daemon_maintenance_monitor_t **monitor,
                                          main_local_maintenance_context_t *context,
                                          bool context_initialized, const char *participant) {
    if (monitor && *monitor && !engine_daemon_maintenance_monitor_stop(monitor)) {
        ...
        engine_log_error("participant.maintenance_join_failed", "participant", participant, "action",
                      "process_exit");
        (void)fflush(stdout);
        (void)fflush(stderr);
        _Exit(EXIT_FAILURE);
...
static _Noreturn void main_coordination_cleanup_fail_stop(const char *component) {
    ...
    (void)fprintf(stderr,
                  "graph-engine: coordination cleanup timed out (%s); "
...
static void main_project_lock_release_fully(engine_project_lock_lease_t **lease) {
 ...
 * internal/engine/engine.c are governed by the same boundary. The narrowly validated
...
static bool main_test_worker_project_lock_marker(const main_local_cli_mutation_t *mutation) {
    ...
    if (!engine_safe_getenv("ENGINE_TEST_WORKER_PROJECT_LOCK_PID_FILE", marker_path, sizeof(marker_path),
                         NULL) ||
        !marker_path[0]) {
        return true;
    }
...
static bool main_local_cli_mutation_begin_internal(void *context, const char *project, bool wait) {
    ...
    for (;;) {
        ...
        if (wait) {
            ...
            status = engine_project_lock_acquire(mutation->manager, project, deadline, NULL, &lease);
        } else {
            status = engine_project_lock_try_acquire(mutation->manager, project, &lease);
        }
        if (status == ENGINE_PRIVATE_FILE_LOCK_OK && lease) {
            main_local_cli_lease_t *held = calloc(1, sizeof(*held));
            if (held) {
                held->project = engine_strdup(project);
            }
        ...
        if (status != ENGINE_PRIVATE_FILE_LOCK_BUSY) {
            engine_log_error("cli.project_lock_failed", "project", project, "action",
                          "refuse_mutation");
            return false;
        }
        if (!wait) {
        ...
        if (mutation->feedback && !mutation->waiting_reported) {
            (void)fprintf(mutation->feedback, "Waiting for another engine mutation of %s...\n",
                          project);
...
static void *parent_watchdog_thread(void *arg) {
    ...
    while (!atomic_load(&g_shutdown)) {
        engine_usleep(poll_interval_us);
...
static void worker_containment_unavailable(void) {
    static const char message[] =
        "engine index worker could not start: process-tree containment unavailable\n";
 ...
 *
 * COMPILED OUT of ordinary builds (see TEST_SEAMS in Makefile.engine). "Fork a
...
static bool worker_start_watchdog_test_descendant(void) {
    char pid_path[ENGINE_SZ_4K] = {0};
    if (!engine_safe_getenv("ENGINE_TEST_WORKER_DESCENDANT_PID_FILE", pid_path, sizeof(pid_path), NULL) ||
        !pid_path[0]) {
        return true;
    }
...
static bool worker_start_parent_watchdog(pid_t initial_ppid) {
    ...
    engine_thread_t worker_watchdog_tid;
    if (engine_thread_create(&worker_watchdog_tid, PARENT_WATCHDOG_STACK_SIZE, parent_watchdog_thread,
                          &worker_config) != 0) {
        return false;
    }
    return engine_thread_detach(&worker_watchdog_tid) == 0;
}
...
static bool client_start_parent_watchdog(pid_t initial_ppid) {
    ...
    engine_thread_t watchdog;
    if (engine_thread_create(&watchdog, PARENT_WATCHDOG_STACK_SIZE, parent_watchdog_thread,
                          &client_config) != 0) {
        return false;
    }
    if (engine_thread_detach(&watchdog) != 0) {
        atomic_store(&g_shutdown, 1);
        (void)engine_thread_join(&watchdog);
        return false;
    }
    return true;
}
...
#define CLI_USAGE "Usage: graph-engine cli [--progress] [--json] <tool_name> [json_args]\n"
...
static int run_cli(int argc, char **argv, engine_project_lock_manager_t *project_locks,
    ...
    (void)cli_strip_flag_value(&argc, argv, ENGINE_INDEX_WORKER_BUILD_ARG);
    const char *response_out = cli_strip_flag_value(&argc, argv, "--response-out");
    (void)cli_strip_flag_value(&argc, argv, ENGINE_INDEX_WORKER_MEMORY_BUDGET_ARG);
    bool worker_single_thread = cli_strip_flag(&argc, argv, ENGINE_INDEX_WORKER_SINGLE_THREAD_ARG);
    const char *worker_marker = cli_strip_flag_value(&argc, argv, ENGINE_INDEX_WORKER_MARKER_ARG);
    const char *worker_quarantine =
        cli_strip_flag_value(&argc, argv, ENGINE_INDEX_WORKER_QUARANTINE_ARG);
    engine_index_set_worker_role_options(index_worker, response_out, worker_single_thread,
                                      worker_marker, worker_quarantine,
                                      engine_index_worker_memory_budget_bytes());
    ...
    for (int i = 0; i < rem_argc; i++) {
        if (strcmp(rem_argv[i], "--help") == 0 || strcmp(rem_argv[i], "-h") == 0) {
            if (engine_cli_print_tool_help(tool_name) != 0) {
    ...
    if (args_file_idx >= 0) {
        ...
        char *err = NULL;
        heap_args = engine_cli_build_args_json(tool_name, rem_argc, rem_argv, &err);
        if (!heap_args) {
    ...
    bool progress =
        !index_worker && engine_cli_progress_enabled(progress_requested, cli_isatty(2) != 0);
    uint64_t progress_started_ms = engine_now_ms();
    if (progress) {
        engine_progress_sink_init(stderr);
        engine_cli_progress_start(stderr, tool_name);
    }
    ...
    if (!index_worker) {
        ...
        srv = engine_mcp_server_new(NULL);
        if (srv) {
            ...
            engine_mcp_server_set_background_tasks(srv, false);
            if (project_locks) {
                engine_mcp_server_set_project_mutation_guard(srv, main_local_cli_mutation_begin,
                                                          main_local_cli_mutation_end, &mutation);
                engine_mcp_server_set_project_mutation_try_guard(srv,
        ...
        if (srv && maintenance_context) {
            ...
            result = engine_mcp_handle_tool(srv, tool_name, args_json);
    ...
    if (!result) {
        ...
        engine_mcp_server_free(srv);
        main_local_cli_mutation_release_all(&mutation);
        if (progress) {
            engine_progress_sink_fini();
            engine_cli_progress_finish(stderr, tool_name, false, engine_now_ms() - progress_started_ms);
        }
        free(heap_args);
        return SKIP_ONE;
    }
...

    {
        ...
        const char *ro = engine_index_worker_response_out();
        bool worker_response_written = false;
        if (ro) {
            FILE *rf = engine_fopen(ro, "wb");
            if (rf) {
        ...
        if (raw_json) {
            ...
            exit_code = engine_cli_mcp_result_is_error(result) ? SKIP_ONE : 0;
        } else {
            exit_code = cli_print_mcp_result(result);
        }
        exit_code = engine_cli_exit_status_after_maintenance(exit_code, maintenance_cancelled);
        if (engine_index_worker_active()) {
            ...
            engine_log_info("index.worker.fast_exit", "action", "_Exit");
            fflush(NULL);
    ...
    if (progress) {
        engine_progress_sink_fini();
        engine_cli_progress_finish(stderr, tool_name, exit_code == 0,
                                engine_now_ms() - progress_started_ms);
    }
    free(heap_args);
    return exit_code;
}
...
static void print_help(void) {
    printf("graph-engine %s\n\n", ENGINE_VERSION);
    printf("Usage:\n");
    printf("  graph-engine              Run MCP server on stdio\n");
    printf("  graph-engine cli [--progress] [--json] <tool> [args]\n");
    printf("                                      Run one tool locally, then exit\n");
    printf("  graph-engine install [-y|-n] [--force] [--dry-run] "
           "[--dir=<path>] [--skip-config]\n");
    printf("  graph-engine uninstall [-y|-n] [--dry-run]\n");
    printf("  graph-engine update [-y|-n]\n");
    printf("  graph-engine config <list|get|set|reset>\n");
    printf("  graph-engine --version    Print version\n");
    printf("  graph-engine --help       Print this help\n");
    printf("\nUI options:\n");
    ...
    char *tools_help = engine_mcp_tools_help_list();
    if (tools_help) {
        printf("\n%s", tools_help);
        free(tools_help);
    }
...
static int main_run_allow_root(int argc, char **argv) {
    ...
    const char *cache_dir = engine_workspace_cache_dir();
    ...
    if (list_only || !path) {
        char listing[ENGINE_SZ_8K];
        if (engine_workspace_grant_list(cache_dir, listing, sizeof(listing))) {
        ...
        if (!path && !list_only) {
            (void)fprintf(stderr,
                          "usage: graph-engine allow-root [--approve-sensitive] <path>\n"
                          "       graph-engine allow-root --approve-manifest <project>\n"
                          "       graph-engine allow-root --list\n");
            return EXIT_FAILURE;
        }
        return 0;
    }
    ...
    if (approve_manifest) {
        ...
        char canonical_project[ENGINE_PATH_MAX];
        if (!engine_canonical_path(path, canonical_project, sizeof(canonical_project))) {
            (void)fprintf(stderr, "error: cannot resolve path: %s\n", path);
            return EXIT_FAILURE;
        }
        char merr[ENGINE_SZ_1K];
        if (!engine_workspace_manifest_approve(cache_dir, engine_workspace_home_dir(), canonical_project,
                                            merr, sizeof(merr))) {
    ...
    char canonical[ENGINE_PATH_MAX];
    if (!engine_canonical_path(path, canonical, sizeof(canonical))) {
        (void)fprintf(stderr, "error: cannot resolve path: %s\n", path);
        return EXIT_FAILURE;
    }
    if (!engine_is_dir(canonical)) {
        (void)fprintf(stderr, "error: not a directory: %s\n", canonical);
        return EXIT_FAILURE;
    }

    char err[ENGINE_SZ_1K];
    if (!engine_workspace_grant_add(cache_dir, engine_workspace_home_dir(), canonical, approve_sensitive,
                                 err, sizeof(err))) {
...
static int handle_subcommand(int argc, char **argv, engine_project_lock_manager_t *project_locks,
                             main_local_maintenance_context_t *maintenance_context) {
    /* First scan: global flags */
    for (int i = SKIP_ONE; i < argc; i++) {
        if (strcmp(argv[i], "--profile") == 0) {
            engine_profile_enable();
        }
    }
    for (int i = SKIP_ONE; i < argc; i++) {
        if (strcmp(argv[i], "--verify-runtime-assets") == 0) {
            if (i != SKIP_ONE || argc != MAIN_CLI_ARGC) {
                (void)fprintf(
                    stderr, "graph-engine: --verify-runtime-assets accepts no arguments\n");
                return 2;
            }
            return engine_cmd_verify_runtime_assets();
        }
        ...
        if (strcmp(argv[i], "cli") == 0) {
            engine_mem_init_with_cap(engine_mem_ram_fraction_for_total(engine_system_info().total_ram),
                                  engine_index_worker_memory_budget_bytes());
            return run_cli(argc - i - SKIP_ONE, argv + i + SKIP_ONE, project_locks,
                           maintenance_context);
        }
        if (strcmp(argv[i], "hook-augment") == 0) {
            engine_mem_init(engine_mem_ram_fraction_for_total(engine_system_info().total_ram));
            return engine_cmd_hook_augment(argc - i - SKIP_ONE, argv + i + SKIP_ONE);
        }
        if (strcmp(argv[i], "install") == 0) {
            return engine_cmd_install(argc - i - SKIP_ONE, argv + i + SKIP_ONE);
        }
        if (strcmp(argv[i], "uninstall") == 0) {
            return engine_cmd_uninstall(argc - i - SKIP_ONE, argv + i + SKIP_ONE);
        }
        if (strcmp(argv[i], "update") == 0) {
            return engine_cmd_update(argc - i - SKIP_ONE, argv + i + SKIP_ONE);
        }
        if (strcmp(argv[i], "config") == 0) {
            return engine_cmd_config(argc - i - SKIP_ONE, argv + i + SKIP_ONE);
        }
    }
    ...
    for (int i = SKIP_ONE; i < argc; i++) {
        if (strncmp(argv[i], "--ui=", SLEN("--ui=")) == 0) {
            *ui_enabled = strcmp(argv[i] + MAIN_FLAG_OFF, "true") == 0;
            ...
            update_mask |= ENGINE_DAEMON_APPLICATION_UI_CONFIG_ENABLED;
        }
        if (strncmp(argv[i], "--port=", SLEN("--port=")) == 0) {
            ...
            if (errno == 0 && end != value && end && *end == '\0' && port > 0 &&
                port < MAIN_MAX_PORT) {
                *ui_port = (int)port;
                update_mask |= ENGINE_DAEMON_APPLICATION_UI_CONFIG_PORT;
            }
        }
    }
    return update_mask;
...
static char **engine_win_utf8_argv(int *out_argc) {
    int wargc = 0;
    ...
    for (int i = 0; i < wargc; i++) {
        u8argv[i] = engine_wide_to_utf8(wargv[i]);
        if (!u8argv[i]) {
            for (int j = 0; j < i; j++) {
                free(u8argv[j]);
            }
...
static bool main_resolve_executable(const char *argv0, char out[MAIN_PATH_CAP]) {
    ...
    return engine_http_server_resolve_binary_path(argv0, resolved, sizeof(resolved)) &&
           engine_canonical_path(resolved, out, MAIN_PATH_CAP);
}
...
static main_build_identity_status_t main_build_identity(engine_daemon_build_identity_t *identity) {
    if (!identity) {
        return MAIN_BUILD_IDENTITY_INVALID_OUTPUT;
    }
    if (!engine_index_supervisor_capture_build_fingerprint()) {
        return MAIN_BUILD_IDENTITY_PROCESS_FINGERPRINT;
    }
    const char *fingerprint = engine_index_supervisor_build_fingerprint();
    if (!fingerprint) {
        return MAIN_BUILD_IDENTITY_PROCESS_FINGERPRINT;
    }
    const char *cache = engine_resolve_cache_dir();
    char canonical_cache[MAIN_PATH_CAP];
    static char cache_fingerprint[ENGINE_SHA256_HEX_LEN + 1];
    if (!cache || !cache[0]) {
    ...
    bool cache_ready = engine_canonical_path(cache, canonical_cache, sizeof(canonical_cache));
    if (!cache_ready && engine_mkdir_p(cache, 0700)) {
        cache_ready = engine_canonical_path(cache, canonical_cache, sizeof(canonical_cache));
    }
    if (!cache_ready || !engine_is_dir(canonical_cache)) {
        return MAIN_BUILD_IDENTITY_CACHE_CANONICALIZE;
    }
    engine_normalize_path_sep(canonical_cache);
    ...
    if (!engine_daemon_ipc_private_directory_secure(canonical_cache)) {
    ...
    if (engine_setenv("ENGINE_CACHE_DIR", canonical_cache, 1) != 0) {
        return MAIN_BUILD_IDENTITY_CACHE_ENVIRONMENT;
    }
    engine_sha256_hex(canonical_cache, strlen(canonical_cache), cache_fingerprint);
    *identity = (engine_daemon_build_identity_t){
...
static engine_daemon_ipc_endpoint_t *main_daemon_endpoint_new(void) {
    ...
    runtime_parent = engine_safe_getenv("ENGINE_TEST_DAEMON_RUNTIME_PARENT", seam_runtime_parent,
                                     sizeof(seam_runtime_parent), NULL);
#endif
    return engine_daemon_bootstrap_endpoint_new(runtime_parent);
}
...
static bool main_local_cli_feedback_enabled(int argc, char **argv) {
    ...
    return engine_cli_progress_enabled(requested, cli_isatty(2) != 0);
}

static int main_local_transition_acquire(const engine_daemon_ipc_endpoint_t *endpoint, FILE *feedback,
                                         engine_daemon_ipc_local_transition_t **transition_out) {
    uint64_t deadline = main_deadline_after(MAIN_STARTUP_TIMEOUT_MS);
    bool waiting_reported = false;
    for (;;) {
        int status = engine_daemon_ipc_local_transition_try_acquire(endpoint, transition_out);
        if (status != 0 || engine_now_ms() >= deadline) {
            return status;
        }
        if (feedback && !waiting_reported) {
            (void)fputs("Waiting for engine startup coordination...\n", feedback);
...
static bool main_version_cohort_close(engine_version_cohort_lease_t **lease,
                                      engine_version_cohort_manager_t **manager) {
    bool ok = true;
    uint64_t deadline = main_deadline_after(MAIN_COORDINATION_CLEANUP_MS);
    while (lease && *lease) {
        engine_private_file_lock_status_t status = engine_version_cohort_lease_release(lease);
    ...
    while (manager && *manager) {
        engine_private_file_lock_status_t status = engine_version_cohort_manager_free(manager);
        if (status != ENGINE_PRIVATE_FILE_LOCK_OK) {
            ok = false;
        }
        if (*manager) {
...
static bool main_project_lock_manager_close(engine_project_lock_manager_t **manager) {
    bool ok = true;
    uint64_t deadline = main_deadline_after(MAIN_COORDINATION_CLEANUP_MS);
    while (manager && *manager) {
        engine_private_file_lock_status_t status = engine_project_lock_manager_free(manager);
        if (status != ENGINE_PRIVATE_FILE_LOCK_OK) {
            ok = false;
        }
        if (*manager) {
...
static bool main_local_transition_close(engine_daemon_ipc_local_transition_t **transition) {
    uint64_t deadline = main_deadline_after(MAIN_COORDINATION_CLEANUP_MS);
    while (transition && *transition) {
        ...
        (void)engine_daemon_ipc_local_transition_release(transition);
        if (*transition) {
            if (engine_now_ms() >= deadline) {
...
static bool main_session_context(const char *preferred_root, char root_out[MAIN_PATH_CAP],
    ...
    if (!engine_canonical_path(root, root_out, MAIN_PATH_CAP)) {
        return false;
    }
    char configured[MAIN_PATH_CAP];
    const char *allowed = engine_safe_getenv("ENGINE_ALLOWED_ROOT", configured, sizeof(configured), NULL);
    if (allowed && allowed[0]) {
        if (!engine_canonical_path(allowed, allowed_out, MAIN_PATH_CAP)) {
            return false;
        }
...
static bool main_set_client_context(engine_daemon_runtime_client_t *client, const char *preferred_root,
                                    engine_mcp_tool_profile_t tool_profile, const char *hook_event,
    ...
    return engine_daemon_application_client_set_context(client, root, allowed_ptr, tool_profile,
                                                     hook_event, hook_dialect, timeout_ms) ==
           ENGINE_DAEMON_RUNTIME_APPLICATION_OK;
}
...
static engine_daemon_bootstrap_status_t main_client_bootstrap_with_upgrade(
    const engine_daemon_bootstrap_config_t *config, engine_daemon_bootstrap_result_t *result) {
    engine_daemon_bootstrap_status_t status = engine_daemon_bootstrap_execute(config, result);
    if (status != ENGINE_DAEMON_BOOTSTRAP_CONFLICT) {
        return status;
    }
    engine_daemon_runtime_status_t active;
    if (!engine_daemon_runtime_request_status(config->endpoint, config->identity,
    ...
    (void)fprintf(stderr,
                  "graph-engine: retiring the active permanent daemon (%s, pid %lu) for "
                  "this newer build (%s)\n",
                  active.semantic_version, (unsigned long)active.daemon_pid,
                  config->identity->semantic_version);
    engine_daemon_runtime_activation_result_t drain;
    if (!engine_daemon_runtime_request_activation_shutdown(config->endpoint, config->identity,
                                                        ENGINE_DAEMON_RUNTIME_ACTIVATION_UPDATE,
                                                        MAIN_MCP_STARTUP_TIMEOUT_MS, &drain) ||
        !drain.accepted) {
        (void)fprintf(stderr, "graph-engine: the active daemon did not accept the "
                              "upgrade drain; run `graph-engine daemon stop`\n");
        return status;
    }
    return engine_daemon_bootstrap_execute(config, result);
}
...
static char *main_local_cli_daemon_execute(const char *tool_name, const char *args_json) {
    engine_daemon_ipc_endpoint_t *endpoint = engine_daemon_bootstrap_endpoint_new(NULL);
    char executable_path[MAIN_PATH_CAP] = {0};
    engine_daemon_build_identity_t identity;
    bool prepared =
        endpoint &&
        engine_http_server_resolve_binary_path(NULL, executable_path, sizeof(executable_path)) &&
    ...
    engine_daemon_bootstrap_config_t config = {
        .role = ENGINE_DAEMON_PROCESS_MCP_CLIENT,
        .endpoint = endpoint,
        .identity = &identity,
    ...
    engine_daemon_bootstrap_result_t bootstrap;
    engine_daemon_bootstrap_status_t status = main_client_bootstrap_with_upgrade(&config, &bootstrap);
    engine_daemon_ipc_endpoint_free(endpoint);
    if (status != ENGINE_DAEMON_BOOTSTRAP_CONNECTED || !bootstrap.client) {
        (void)fprintf(stderr, "error: %s\n",
                      bootstrap.message[0] ? bootstrap.message
                                           : "no engine daemon connection for CLI execution");
        return NULL;
    }
    if (bootstrap.daemon_spawned) {
        (void)fprintf(stderr, "hint: this command started a temporary engine daemon. "
                              "`graph-engine daemon start` keeps one warm and removes this "
    ...
    bool context_ok =
        ...
        main_set_client_context(bootstrap.client, session_root, ENGINE_MCP_TOOL_PROFILE_ALL, NULL,
                                NULL, MAIN_CONNECT_TIMEOUT_MS);
    if (context_ok &&
        engine_daemon_application_client_tool(bootstrap.client, tool_name, args_json, &response,
    ...
    (void)engine_daemon_runtime_client_close(bootstrap.client, MAIN_CLOSE_TIMEOUT_MS);
    return result;
}
...
static char *main_hook_cwd(const char *input_json) {
    ...
    char *copy = NULL;
    if (cwd && engine_hook_path_is_abs(cwd)) {
...
static bool main_hook_absent_notice_due(void) {
    ...
    uint64_t now_seconds = engine_now_ms() / 1000U;
    uint64_t stamp_seconds = 0;
    FILE *stamp = engine_fopen(marker, "r");
    if (stamp) {
        char text[32] = {0};
    ...
    FILE *update = engine_fopen(marker, "w");
    if (update) {
...
static void main_hook_report_absent_daemon(const char *hook_dialect) {
    ...
    (void)fprintf(stderr, "graph-engine: no engine daemon is running, so graph "
                          "augmentation is skipped. Start an MCP session or run "
                          "`graph-engine daemon start` to enable it.\n");
    const char *notice = engine_hook_admission_notice(ENGINE_HOOK_ADMISSION_DAEMON_ABSENT, hook_dialect);
    if (notice) {
...
static void main_hook_report_conflicted_daemon(const char *hook_dialect) {
    ...
    const char *notice = engine_hook_admission_notice(ENGINE_HOOK_ADMISSION_BUILD_CONFLICT, hook_dialect);
    if (notice) {
        (void)fputs(notice, stdout);
        (void)fflush(stdout);
    }
...
static int main_run_hook_frontend(engine_daemon_runtime_client_t *client, const char *hook_event,
                                  const char *hook_dialect) {
    char *input = engine_hook_augment_read_stdin();
    if (!input) {
        return 0;
    }
    char *hook_cwd = main_hook_cwd(input);
    bool context_set =
        main_set_client_context(client, hook_cwd, ENGINE_MCP_TOOL_PROFILE_ALL, hook_event,
    ...
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    engine_daemon_runtime_application_status_t status = engine_daemon_application_client_hook_augment(
        client, input, &response, &response_length, MAIN_HOOK_REQUEST_TIMEOUT_MS);
    free(input);
    if (status == ENGINE_DAEMON_RUNTIME_APPLICATION_OK && response && response_length > 0) {
...
static bool main_hook_options(int argc, char **argv, const char **event_out,
    ...
    *dialect_out = NULL;
    ...
    return engine_hook_augment_invocation_supported(*event_out, *dialect_out);
}
...
static void main_daemon_ctl_hex_encode(const uint8_t bytes[ENGINE_SHA256_DIGEST_LEN],
...
static bool main_daemon_ctl_ui_readiness_prepare(engine_daemon_runtime_client_t *client,
                                                 main_daemon_ctl_ui_readiness_t *readiness) {
    uint8_t challenge[ENGINE_SHA256_DIGEST_LEN] = {0};
    uint8_t proof[ENGINE_SHA256_DIGEST_LEN] = {0};
    if (!client || !readiness) {
        return false;
    }
    engine_secure_zero(readiness, sizeof(*readiness));
    bool prepared = engine_secure_random(challenge, sizeof(challenge)) &&
                    engine_daemon_application_client_ui_readiness_proof(client, challenge, proof,
...
static bool main_daemon_ctl_ui_endpoint_ready(int port,
    ...
    int request_length = snprintf(request, sizeof(request),
                                  "GET /__engine/ui-readiness?challenge=%s HTTP/1.1\r\n"
                                  "Host: 127.0.0.1:%d\r\nAccept: text/plain\r\n"
                                  "Connection: close\r\n\r\n",
    ...
    ready = ready && received > 0U && strncmp(response, "HTTP/1.1 200 ", 13) == 0 &&
            ...
            strlen(body) == ENGINE_SHA256_HEX_LEN &&
            main_daemon_ctl_constant_time_equal(body, readiness->proof_hex, ENGINE_SHA256_HEX_LEN);
    (void)main_daemon_ctl_socket_close(socket_handle);
    return ready;
}
...
static uint32_t main_daemon_ctl_ui_ready_timeout_ms(void) {
    ...
    const char *configured = engine_safe_getenv("ENGINE_TEST_DAEMON_UI_READY_TIMEOUT_MS", timeout_text,
...
static bool main_daemon_ctl_wait_for_ui(int port, const main_daemon_ctl_ui_readiness_t *readiness,
    ...
    for (;;) {
        ...
        if (engine_now_ms() >= deadline_ms) {
            return false;
        }
        engine_usleep(MAIN_DAEMON_CTL_UI_PROBE_INTERVAL_US);
    }
}

static int main_daemon_ctl_finish_ui_open(engine_daemon_runtime_client_t **client_io, int port,
                                          bool open_browser);
...
static void main_daemon_ctl_print_ui_configuration(void) {
    ...
    engine_ui_config_t ui_config;
    engine_ui_config_load(&ui_config);
...
static void main_daemon_ctl_open_browser(int port) {
    ...
    if (engine_safe_getenv("ENGINE_TEST_DAEMON_OPEN_MARKER", marker_path, sizeof(marker_path), NULL)) {
        FILE *marker = engine_fopen(marker_path, "wb");
...
static int main_daemon_ctl_finish_ui_open(engine_daemon_runtime_client_t **client_io, int port,
    ...
    engine_daemon_runtime_client_t *client = client_io ? *client_io : NULL;
    ...
    if (client_io && *client_io) {
        engine_daemon_runtime_client_t *owned_client = *client_io;
        *client_io = NULL;
        (void)engine_daemon_runtime_client_close(owned_client, MAIN_CLOSE_TIMEOUT_MS);
    }
    ...
    if (!ready) {
        (void)fprintf(stderr,
                      ...
                      timeout_ms);
        (void)fprintf(stderr,
                      "hint: verify matching runtime assets with `graph-engine "
...
static int main_run_daemon_ctl(int argc, char **argv, const engine_daemon_ipc_endpoint_t *endpoint,
    ...
    if (!arguments_valid || !subcommand) {
        (void)fprintf(stderr, "usage: graph-engine daemon <start|stop|status> "
                              "[--open] [--port=N]\n");
        return EXIT_FAILURE;
    }

    engine_daemon_runtime_status_t status;
    bool active = engine_daemon_runtime_request_status(endpoint, identity,
                                                    MAIN_DAEMON_CTL_PROBE_TIMEOUT_MS, &status);

    if (strcmp(subcommand, "status") == 0) {
        if (!active) {
            printf("daemon: not running\n");
            printf("hint: `graph-engine daemon start` keeps a daemon warm so CLI "
    ...
    if (strcmp(subcommand, "stop") == 0) {
        ...
        engine_daemon_runtime_stop_result_t stop_result;
        if (!engine_daemon_runtime_request_stop(endpoint, identity, MAIN_DAEMON_CTL_STOP_TIMEOUT_MS,
                                             &stop_result)) {
            (void)fprintf(stderr,
    ...
    if (active) {
        ...
        engine_daemon_runtime_client_t *ui_client = NULL;
        engine_daemon_runtime_connect_result_t connect_result;
        if (open_browser) {
            ui_client = engine_daemon_runtime_client_connect(endpoint, identity,
        ...
        if (ui_client) {
            (void)engine_daemon_runtime_client_close(ui_client, MAIN_CLOSE_TIMEOUT_MS);
        }
        return ui_result;
    }

    engine_daemon_bootstrap_config_t start_config = {
        .role = ENGINE_DAEMON_PROCESS_MCP_CLIENT,
        .endpoint = endpoint,
        .identity = identity,
        .executable_path = executable_path,
    ...
    engine_daemon_bootstrap_result_t start_result;
    engine_daemon_bootstrap_status_t start_status =
        main_client_bootstrap_with_upgrade(&start_config, &start_result);
    if (start_status != ENGINE_DAEMON_BOOTSTRAP_CONNECTED || !start_result.client) {
        (void)fprintf(stderr, "error: %s\n",
                      start_result.message[0] ? start_result.message
                                              : "the permanent daemon could not be started");
        if (start_status == ENGINE_DAEMON_BOOTSTRAP_CONFLICT) {
            (void)fprintf(stderr, "hint: a daemon of a different build is active; "
                                  "`graph-engine daemon stop` retires it.\n");
        }
        return EXIT_FAILURE;
    }
    ...
    if (engine_ui_assets_supported()) {
        ...
        bool context_set =
            main_set_client_context(start_result.client, ".", ENGINE_MCP_TOOL_PROFILE_ALL, NULL, NULL,
                                    MAIN_CONNECT_TIMEOUT_MS);
        if (!context_set || engine_daemon_application_client_set_ui_config(
                                start_result.client, update_mask, true, ui_port,
                                MAIN_CONNECT_TIMEOUT_MS) != ENGINE_DAEMON_RUNTIME_APPLICATION_OK) {
            (void)fprintf(stderr,
                          "error: the daemon did not accept the UI configuration; browser was "
                          "not opened\n");
            (void)engine_daemon_runtime_client_close(start_result.client, MAIN_CLOSE_TIMEOUT_MS);
            return EXIT_FAILURE;
        }
    ...
    engine_daemon_runtime_status_t started;
    bool started_ok = engine_daemon_runtime_request_status(endpoint, identity,
    ...
    printf("It survives idle periods and session ends; `graph-engine daemon stop` "
           "retires it.\n");
    int ui_result =
        engine_ui_assets_supported()
...
int main(int argc, char **argv) {
    ...
    {
        int win_argc = 0;
        char **win_argv = engine_win_utf8_argv(&win_argc);
        if (win_argv) {
            argc = win_argc;
            argv = win_argv;
        }
...
#endif
    engine_daemon_process_role_t role = engine_daemon_process_role(argc, argv);
    if (role == ENGINE_DAEMON_PROCESS_INVALID) {
        (void)fprintf(stderr, "graph-engine: invalid internal process arguments\n");
        return EXIT_FAILURE;
    }
#ifndef _WIN32
    if (role == ENGINE_DAEMON_PROCESS_DAEMON) {
        (void)umask(077);
    }
#endif

    engine_cli_set_version(ENGINE_VERSION);
    engine_profile_init();
    ...
    if (role == ENGINE_DAEMON_PROCESS_MCP_CLIENT &&
        ...
        (void)fprintf(stderr, "graph-engine: --tool-profile requires the supported value "
                              "'analysis' or 'scout'\n");
        return 2;
    }
    const char *hook_event = NULL;
    const char *hook_dialect = NULL;
    if (role == ENGINE_DAEMON_PROCESS_HOOK_CLIENT &&
    ...
    if (role == ENGINE_DAEMON_PROCESS_HOOK_CLIENT) {
#ifndef _WIN32
        engine_hook_augment_arm_deadline();
#endif
    }

    if (role == ENGINE_DAEMON_PROCESS_STATELESS) {
        int result = handle_subcommand(argc, argv, NULL, NULL);
        return result >= 0 ? result : EXIT_FAILURE;
    }

    if (role == ENGINE_DAEMON_PROCESS_LOCAL_CLI) {
        bool feedback_enabled = main_local_cli_feedback_enabled(argc, argv);
        FILE *feedback = feedback_enabled ? stderr : NULL;
        if (feedback) {
            (void)fputs("Preparing one-shot local engine command...\n", feedback);
            (void)fflush(feedback);
        }
        engine_daemon_ipc_endpoint_t *local_endpoint = engine_daemon_bootstrap_endpoint_new(NULL);
        char local_executable[MAIN_PATH_CAP];
        engine_daemon_build_identity_t local_identity;
        engine_project_lock_manager_t *project_locks =
            local_endpoint ? engine_project_lock_manager_new(local_endpoint) : NULL;
        engine_version_cohort_manager_t *cohort_manager =
            local_endpoint ? engine_version_cohort_manager_new(local_endpoint) : NULL;
        engine_version_cohort_lease_t *cohort_lease = NULL;
        engine_daemon_ipc_local_transition_t *local_transition = NULL;
        main_local_maintenance_context_t maintenance_context;
        bool maintenance_context_initialized = false;
        engine_daemon_maintenance_monitor_t *maintenance_monitor = NULL;
        engine_daemon_conflict_t cohort_conflict;
        engine_version_cohort_status_t cohort_status = ENGINE_VERSION_COHORT_IO;
        main_build_identity_status_t local_identity_status = MAIN_BUILD_IDENTITY_OK;
        int result = ENGINE_NOT_FOUND;
        ...
        if (coordination_failure) {
            (void)fprintf(
                stderr, "graph-engine: secure CLI coordination could not be created (%s)\n",
                coordination_failure);
            goto local_cli_cleanup;
        }
        engine_http_server_set_binary_path(local_executable);

        cohort_status = engine_version_cohort_acquire(cohort_manager, &local_identity,
                                                   main_deadline_after(MAIN_STARTUP_TIMEOUT_MS),
                                                   &cohort_lease, &cohort_conflict);
        if (cohort_status != ENGINE_VERSION_COHORT_OK) {
            char message[ENGINE_DAEMON_CONFLICT_MESSAGE_SIZE];
            bool formatted = cohort_status == ENGINE_VERSION_COHORT_CONFLICT &&
                             engine_daemon_conflict_format(&cohort_conflict, message, sizeof(message));
            if (cohort_status == ENGINE_VERSION_COHORT_CONFLICT) {
                (void)engine_version_cohort_log_conflict(&cohort_conflict);
            }
            (void)fprintf(stderr, "graph-engine: %s\n",
                          formatted ? message
                                    : "CLI exact-build admission could not be verified; retry "
                                      "after active engine operations exit");
            goto local_cli_cleanup;
        }
        main_local_maintenance_context_init(&maintenance_context);
        maintenance_context_initialized = true;
        maintenance_monitor =
            engine_daemon_maintenance_monitor_start(cohort_manager, main_local_command_cancel,
                                                 &maintenance_context, EXIT_FAILURE, "CLI command");
        if (!maintenance_monitor) {
            (void)fprintf(stderr,
                          "graph-engine: CLI maintenance observer could not start safely\n");
            goto local_cli_cleanup;
        }
        ...
        if (transition_status != 1 || !local_transition) {
            (void)fprintf(stderr,
                          "graph-engine: CLI startup coordination %s; retry after the "
                          "active engine transition exits\n",
                          transition_status == 0 ? "remained busy"
                                                 : "could not be verified safely");
            goto local_cli_cleanup;
        }
        int seal_status = engine_daemon_ipc_local_transition_seal_legacy(local_transition);
        if (seal_status != 1) {
            if (seal_status == 0) {
                (void)engine_version_cohort_log_uncoordinated_daemon(&local_identity);
            }
            (void)fprintf(stderr, "graph-engine: engine CLI could not start because a "
                                  "pre-coordination or unverified engine generation is active; close "
                                  "all engine sessions and commands, then retry\n");
            goto local_cli_cleanup;
        }

        engine_version_cohort_daemon_presence_t daemon_presence =
            engine_version_cohort_daemon_presence_under_transition(cohort_manager, local_endpoint,
                                                                local_transition);
        if (daemon_presence != ENGINE_VERSION_COHORT_DAEMON_ABSENT &&
            daemon_presence != ENGINE_VERSION_COHORT_DAEMON_COORDINATED) {
            if (daemon_presence == ENGINE_VERSION_COHORT_DAEMON_UNCOORDINATED) {
                (void)engine_version_cohort_log_uncoordinated_daemon(&local_identity);
                (void)fprintf(stderr, "graph-engine: engine CLI could not start because "
                                      "an active pre-coordination or unverified engine daemon is "
                                      "running. Close all engine sessions and commands, then "
                                      "retry.\n");
            } else {
                (void)fprintf(stderr, "graph-engine: active daemon coordination could "
                                      "not be verified safely; retry after active engine sessions "
                                      "exit\n");
            }
            goto local_cli_cleanup;
        }
        if (!engine_daemon_ipc_local_transition_begin_work(local_transition)) {
            (void)fprintf(stderr, "graph-engine: CLI startup coordination could not enter "
                                  "local work safely\n");
            goto local_cli_cleanup;
        }
    ...
    local_cli_cleanup:
        ...
        engine_daemon_ipc_endpoint_free(local_endpoint);
        if (!cleanup_ok) {
            (void)fprintf(stderr, "graph-engine: CLI coordination cleanup failed\n");
            return EXIT_FAILURE;
        }
        return exit_code;
    }
    ...
    if (!main_resolve_executable(argv[0], executable_path)) {
        (void)fprintf(stderr,
                      "graph-engine: exact executable identity could not be verified "
                      "(executable-path)\n");
        return role == ENGINE_DAEMON_PROCESS_HOOK_CLIENT ? EXIT_SUCCESS : EXIT_FAILURE;
    }
    main_build_identity_status_t identity_status = main_build_identity(&identity);
    if (identity_status != MAIN_BUILD_IDENTITY_OK) {
        const char *validation_detail = engine_daemon_ipc_validation_detail();
        (void)fprintf(stderr,
                      "graph-engine: exact executable identity could not be verified "
                      "(%s)%s%s\n",
    ...
    engine_http_server_set_binary_path(executable_path);

    if (role == ENGINE_DAEMON_PROCESS_WORKER) {
        engine_index_worker_invocation_t invocation;
        engine_index_worker_argv_status_t worker_status =
            engine_index_worker_parse_process_argv(argc, argv, &invocation);
        if (worker_status != ENGINE_INDEX_WORKER_ARGV_VALID) {
            (void)fprintf(stderr, "engine index worker could not start: %s\n",
                          engine_index_worker_argv_status_message(worker_status));
            return EXIT_FAILURE;
        }
        engine_daemon_ipc_endpoint_t *worker_endpoint = engine_daemon_bootstrap_endpoint_new(NULL);
        engine_project_lock_manager_t *worker_project_locks =
            worker_endpoint ? engine_project_lock_manager_new(worker_endpoint) : NULL;
        engine_version_cohort_manager_t *worker_cohort_manager =
            worker_endpoint ? engine_version_cohort_manager_new(worker_endpoint) : NULL;
        engine_version_cohort_lease_t *worker_cohort_lease = NULL;
        engine_daemon_ipc_local_transition_t *worker_transition = NULL;
        main_local_maintenance_context_t worker_maintenance_context;
        bool worker_maintenance_context_initialized = false;
        engine_daemon_maintenance_monitor_t *worker_maintenance_monitor = NULL;
        engine_daemon_conflict_t worker_conflict;
        int result = ENGINE_NOT_FOUND;
        bool worker_cleanup_ok = true;
        engine_version_cohort_status_t worker_cohort_status =
            worker_project_locks && worker_cohort_manager
                ? engine_version_cohort_acquire(worker_cohort_manager, &identity,
                                             main_deadline_after(MAIN_STARTUP_TIMEOUT_MS),
                                             &worker_cohort_lease, &worker_conflict)
                : ENGINE_VERSION_COHORT_IO;
        if (worker_cohort_status != ENGINE_VERSION_COHORT_OK) {
            char message[ENGINE_DAEMON_CONFLICT_MESSAGE_SIZE];
            bool formatted = worker_cohort_status == ENGINE_VERSION_COHORT_CONFLICT &&
                             engine_daemon_conflict_format(&worker_conflict, message, sizeof(message));
            if (worker_cohort_status == ENGINE_VERSION_COHORT_CONFLICT) {
                (void)engine_version_cohort_log_conflict(&worker_conflict);
            }
            (void)fprintf(stderr, "engine index worker could not start: %s\n",
        ...
        worker_maintenance_monitor = engine_daemon_maintenance_monitor_start(
        ...
        if (!worker_maintenance_monitor) {
            (void)fprintf(stderr,
                          "engine index worker could not start: maintenance observer unavailable\n");
            goto worker_cleanup;
        }
        ...
        if (worker_transition_status != 1 || !worker_transition) {
            (void)fprintf(stderr, "engine index worker could not start: local coordination %s\n",
                          worker_transition_status == 0 ? "remained busy"
                                                        : "could not be verified safely");
            goto worker_cleanup;
        }
        int worker_seal_status = engine_daemon_ipc_local_transition_seal_legacy(worker_transition);
        if (worker_seal_status != 1) {
            if (worker_seal_status == 0) {
                (void)engine_version_cohort_log_uncoordinated_daemon(&identity);
            }
            (void)fprintf(stderr, "engine index worker could not start: a pre-coordination or "
                                  "unverified engine generation is active\n");
            goto worker_cleanup;
        }
        engine_version_cohort_daemon_presence_t worker_daemon_presence =
            engine_version_cohort_daemon_presence_under_transition(worker_cohort_manager,
                                                                worker_endpoint, worker_transition);
        if (worker_daemon_presence != ENGINE_VERSION_COHORT_DAEMON_ABSENT &&
            worker_daemon_presence != ENGINE_VERSION_COHORT_DAEMON_COORDINATED) {
            if (worker_daemon_presence == ENGINE_VERSION_COHORT_DAEMON_UNCOORDINATED) {
                (void)engine_version_cohort_log_uncoordinated_daemon(&identity);
            }
            (void)fprintf(stderr, "engine index worker could not start: active daemon coordination "
                                  "could not be verified safely\n");
            goto worker_cleanup;
        }
        if (!engine_daemon_ipc_local_transition_begin_work(worker_transition)) {
            (void)fprintf(stderr, "engine index worker could not start: local coordination could not "
                                  "enter worker execution\n");
            goto worker_cleanup;
        }
        engine_index_set_worker_role_options(true, invocation.response_out, invocation.single_thread,
...
#endif
        engine_index_supervisor_mark_host();
    ...
    worker_cleanup:
        ...
        worker_cleanup_ok =
            ...
            worker_cleanup_ok;
        engine_daemon_ipc_endpoint_free(worker_endpoint);
        if (!worker_cleanup_ok || worker_cohort_status != ENGINE_VERSION_COHORT_OK || result < 0) {
            return EXIT_FAILURE;
        }
        return result;
    }

    engine_daemon_ipc_endpoint_t *endpoint = main_daemon_endpoint_new();
    if (!endpoint) {
        (void)fprintf(stderr, "graph-engine: secure daemon endpoint could not be created\n");
        return EXIT_FAILURE;
    }

    if (role == ENGINE_DAEMON_PROCESS_DAEMON_CTL) {
    ...
    if (role == ENGINE_DAEMON_PROCESS_DAEMON) {
        ...
        engine_daemon_host_config_t host_config = {
            .endpoint = endpoint,
            .identity = identity,
            .executable_path = executable_path,
        ...
        int result = engine_daemon_host_run(&host_config);
        engine_daemon_ipc_endpoint_free(endpoint);
    ...
    static char seam_hook_build[ENGINE_DAEMON_BUILD_FINGERPRINT_SIZE];
    const char *seam_forced_build = engine_safe_getenv("ENGINE_TEST_HOOK_CLIENT_BUILD", seam_hook_build,
                                                    sizeof(seam_hook_build), NULL);
    if (role == ENGINE_DAEMON_PROCESS_HOOK_CLIENT && seam_forced_build && seam_forced_build[0]) {
        identity.build_fingerprint = seam_hook_build;
    }
#endif

    engine_version_cohort_manager_t *client_cohort_manager = engine_version_cohort_manager_new(endpoint);
    engine_version_cohort_lease_t *client_cohort_lease = NULL;
    engine_daemon_conflict_t client_cohort_conflict;
    engine_version_cohort_status_t client_cohort_status =
        client_cohort_manager
            ? engine_version_cohort_acquire(client_cohort_manager, &identity,
                                         main_deadline_after(role == ENGINE_DAEMON_PROCESS_HOOK_CLIENT
                                                                 ? MAIN_HOOK_REQUEST_TIMEOUT_MS
                                                                 : MAIN_MCP_STARTUP_TIMEOUT_MS),
                                         &client_cohort_lease, &client_cohort_conflict)
            : ENGINE_VERSION_COHORT_IO;
    if (client_cohort_status != ENGINE_VERSION_COHORT_OK) {
        char message[ENGINE_DAEMON_CONFLICT_MESSAGE_SIZE];
        bool formatted =
            client_cohort_status == ENGINE_VERSION_COHORT_CONFLICT &&
            engine_daemon_conflict_format(&client_cohort_conflict, message, sizeof(message));
        if (client_cohort_status == ENGINE_VERSION_COHORT_CONFLICT) {
            (void)engine_version_cohort_log_conflict(&client_cohort_conflict);
        }
        (void)fprintf(stderr, "graph-engine: %s\n",
    ...
    if (role == ENGINE_DAEMON_PROCESS_HOOK_CLIENT) {
        ...
        engine_daemon_runtime_connect_result_t hook_connect;
        engine_daemon_runtime_client_t *hook_client = engine_daemon_runtime_client_connect(
            endpoint, &identity, MAIN_HOOK_CONNECT_TIMEOUT_MS, &hook_connect);
        engine_daemon_ipc_endpoint_free(endpoint);
        if (!hook_client) {
            if (hook_connect.status == ENGINE_DAEMON_RUNTIME_CONNECT_CONFLICT) {
                char conflict_detail[ENGINE_DAEMON_CONFLICT_MESSAGE_SIZE];
                if (engine_daemon_conflict_format(&hook_connect.conflict, conflict_detail,
        ...
        (void)engine_daemon_runtime_client_close(hook_client, MAIN_HOOK_CLOSE_TIMEOUT_MS);
        (void)main_version_cohort_close(&client_cohort_lease, &client_cohort_manager);
        return EXIT_SUCCESS;
    }

    engine_daemon_bootstrap_config_t bootstrap_config = {
        .role = role,
        .endpoint = endpoint,
        .identity = &identity,
        .executable_path = executable_path,
    ...
    engine_daemon_bootstrap_result_t bootstrap_result;
    engine_daemon_bootstrap_status_t bootstrap_status =
        main_client_bootstrap_with_upgrade(&bootstrap_config, &bootstrap_result);
    engine_daemon_ipc_endpoint_free(endpoint);
    if (bootstrap_status != ENGINE_DAEMON_BOOTSTRAP_CONNECTED || !bootstrap_result.client) {
        (void)main_version_cohort_close(&client_cohort_lease, &client_cohort_manager);
        return EXIT_FAILURE;
    }
    ...
    if (role == ENGINE_DAEMON_PROCESS_MCP_CLIENT &&
        !main_set_client_context(g_daemon_client, NULL, tool_profile, NULL, NULL,
                                 MAIN_CONNECT_TIMEOUT_MS)) {
        (void)fprintf(stderr, "graph-engine: daemon session context was rejected\n");
        (void)engine_daemon_runtime_client_close(g_daemon_client, MAIN_CLOSE_TIMEOUT_MS);
    ...
    if (role == ENGINE_DAEMON_PROCESS_MCP_CLIENT && engine_mcp_tool_profile_allows_http(tool_profile)) {
        bool ui_enabled = false;
        int ui_port = 0;
        bool explicitly_enabled = false;
        uint8_t update_mask =
            parse_ui_flags(argc, argv, &ui_enabled, &ui_port, &explicitly_enabled);
        if (update_mask != 0 && engine_daemon_application_client_set_ui_config(
                                    g_daemon_client, update_mask, ui_enabled, ui_port,
                                    MAIN_CONNECT_TIMEOUT_MS) != ENGINE_DAEMON_RUNTIME_APPLICATION_OK) {
            (void)fprintf(stderr, "graph-engine: daemon UI configuration update failed\n");
            (void)engine_daemon_runtime_client_close(g_daemon_client, MAIN_CLOSE_TIMEOUT_MS);
            g_daemon_client = NULL;
            (void)main_version_cohort_close(&client_cohort_lease, &client_cohort_manager);
            return EXIT_FAILURE;
        }
        if (explicitly_enabled && !engine_ui_assets_supported()) {
            (void)fprintf(stderr, "graph-engine: --ui requested, but this binary was built "
                                  "without UI support; rebuild with `make -f Makefile.engine "
                                  "engine-with-ui`.\n");
        }
    }
#ifndef _WIN32
    if (!client_start_parent_watchdog(process_initial_ppid)) {
        (void)fprintf(stderr, "graph-engine: parent-death watchdog could not start\n");
        (void)engine_daemon_runtime_client_close(g_daemon_client, MAIN_CLOSE_TIMEOUT_MS);
        g_daemon_client = NULL;
        (void)main_version_cohort_close(&client_cohort_lease, &client_cohort_manager);
        return EXIT_FAILURE;
    ...
    setup_signal_handlers();
    int result = engine_daemon_frontend_mcp_run(g_daemon_client, client_cohort_manager, stdin, stdout);
