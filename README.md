...
push / MR 由 GitLab CI 先 `ruff check agent deploy` 再跑默认 pytest（`.gitlab-ci.yml`），GitHub Actions 同命令（`.github/workflows/test.yml`），本地等价命令 `uv run ruff check agent deploy` 与 `uv run pytest -q`。
